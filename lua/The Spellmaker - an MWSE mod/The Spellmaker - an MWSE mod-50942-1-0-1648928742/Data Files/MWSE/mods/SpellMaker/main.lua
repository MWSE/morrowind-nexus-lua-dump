...
local function keyDown()
    ...
    if (p.conjuration.type~=0 and p.mysticism.type~=0 and p.illusion.type~=0 and p.alteration.type~=0 and p.destruction.type~=0 and p.restoration.type~=0) then
        tes3.messageBox("You do not qualify for Spellmaking.")
        return
    end
...
local function registerModConfig()	
    ...
    local page = template:createPage({label="Welcome to the spellmaker's configuration menu."})
    page:createKeyBinder{label = "New Spellmaking key. You will need to restart the game for the changes to apply.",
