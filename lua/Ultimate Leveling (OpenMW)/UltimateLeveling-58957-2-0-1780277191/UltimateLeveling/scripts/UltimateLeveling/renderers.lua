...
I.Settings.registerRenderer(ulDef.renderers.hotkey, function(value, set, argument)
...
I.Settings.registerRenderer(ulDef.renderers.number, function(value, set, argument)
    argument = applyDefaults(argument, defaultNumberArgument)
    local lastInput
    return disable(argument.disabled, paddedBox {
        template = I.MWUI.templates.textEditLine,
...
for _, record in pairs(core.stats.Skill.records) do
    table.insert(skillItems, { key = record.id, name = record.name, value = 100 })
end
for _, record in pairs(I.SkillFramework and I.SkillFramework.getSkillRecords() or {}) do
    table.insert(skillItems, { key = record.id, name = record.name, value = 100 })
end
for _, record in pairs(core.stats.Attribute.records) do
    table.insert(attributeItems, { key = record.id, name = record.name, value = 1000 })
end

registerMultiSelectNumberRenderer(ulDef.renderers.per_skill_uncapper, skillItems)
registerMultiSelectNumberRenderer(ulDef.renderers.per_attribute_uncapper, attributeItems)

--[[return {
    updatePerSkillRenderer = function(data)
        local record = data.record

        table.insert(skillItems, { key = record.id, name = record.name, value = 100 }) 
        I.Settings.updateRendererArgument("SettingsPlayerUncapper".. ulDef.MOD_NAME, "perSkillMaxValueUncapper", registerMultiSelectNumberRenderer(ulDef.renderers.per_skill_uncapper, skillItems))
    end,
}--]]
