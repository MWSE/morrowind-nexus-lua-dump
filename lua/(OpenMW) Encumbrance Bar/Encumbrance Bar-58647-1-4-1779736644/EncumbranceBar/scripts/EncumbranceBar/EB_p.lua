...
local function getSnapLayout()
	...
	local ebIconShift = S.BAR_SHOW_ICON and (bbThick + 2) or 0
	...
	if bbPos == "Bottom Left" then
		...
		return {
			thickness = bbThick,
			position = v2(94 + bbIconShift - ebIconShift, above and (-so - 3 * vo) or (-so + vo)),
			anchor = v2(0, 1),
			relativePosition = v2(0, 1),
		}
	else
		...
		return {
			thickness = bbThick,
			position = v2(so + bbIconShift - ebIconShift, above and (so - vo) or (so + 3 * vo)),
			anchor = v2(0, 0),
			relativePosition = v2(0, 0),
		}
	end
...
local function buildLayout()
	...
	local iconWidget = {
		...
		type = ui.TYPE.Image,
		props = {
			resource = lib.getTexture("textures/EncumbranceBar/icon_pixel.png"),
	...
	local iconGap = (S.BAR_SHOW_ICON and not isVertical) and 2 or 0
	...
	if isVertical then
		...
		if S.BAR_SHOW_ICON then
			table.insert(rowChildren, iconWidget)
			table.insert(rowChildren, {props = {size = v2(iconGap, 1)}})
		end
		table.insert(rowChildren, barContainer)
	end
	...
	local totalW = isVertical and math.max(w, iconSize) or (w + iconSize + iconGap + textRightW)
...
local function onMouseWheel(vertical)
	...
	-- root flex
	row.props.size = v2(
		isVertical and math.max(w, iconSize) or (w + iconSize + iconGap),
	...
	if textRightActive then
		...
		row.props.size = v2(w + iconSize + iconGap + textRightW, h)
	end
	G.rootElement:update()
end
