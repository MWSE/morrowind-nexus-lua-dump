...
local function buildLayout()
	...
	local w = isVertical and S.BAR_THICKNESS or effLen
	local h = isVertical and effLen or S.BAR_THICKNESS
	local iconSize = S.BAR_SHOW_ICON and S.BAR_THICKNESS or 0
	...
	local textSize = textRight and (S.BAR_THICKNESS + math.floor(S.BAR_THICKNESS/6)) or math.max(8, S.BAR_THICKNESS * 0.85)
	...
	return {
		layer = S.BAR_LOCK and "HUD" or "Modal",
		type = ui.TYPE.Flex,
		props = {
			...
			anchor = v2(0, 1),
			position = v2(S.BAR_X_OFFSET, S.BAR_Y_OFFSET),
		...
		events = {
			mousePress = async:callback(function(data, elem)
				if data.button == 1 and not S.BAR_LOCK then
