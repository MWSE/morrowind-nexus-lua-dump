...
return {
    engineHandlers = {
        onActive = function()
            core.sendGlobalEvent("Disarm_NpcActivated", { npc = self.object })
            I.Combat.addOnHitHandler(function(attack)
                ...
                if not isUnarmed then
                    if cachedSettings.SHIELD_PROTECTS then
                        ...
                        if not isTwoHanded and shield and shield:isValid() and shield ~= weapon and types.Armor.objectIsInstance(shield) then return end
                    end
