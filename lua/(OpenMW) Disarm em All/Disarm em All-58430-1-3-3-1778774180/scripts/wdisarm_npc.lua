...
local function handleOnHit(attack)
    ...
    if not isUnarmed then
        if cachedSettings.SHIELD_PROTECTS then
            ...
            if shield and shield:isValid() and types.Armor.objectIsInstance(shield) then
                ...
                if not isTwoHanded then return end 
            end
        end
    ...
    if useMult then chance = chance * hitStrength end
    
    local roll = math.random()
    if roll > chance then return end

    I.AnimationController.playBlendedAnimation("disarm1", {
        startKey = "start",
        stopKey  = "stop",
        priority = {
        ...
        blendMask   = animation.BLEND_MASK.LeftArm
                    + animation.BLEND_MASK.RightArm
                    + animation.BLEND_MASK.Torso,
        speed = 1.2,
    })
