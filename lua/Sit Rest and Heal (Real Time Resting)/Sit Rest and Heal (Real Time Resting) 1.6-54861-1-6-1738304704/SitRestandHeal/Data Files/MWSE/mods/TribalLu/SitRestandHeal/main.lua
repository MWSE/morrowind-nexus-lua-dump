...
local function mySitRestHeal()
	...
	if tes3.mobilePlayer.inCombat then 
		tes3.messageBox("You cannot rest while in combat.")
		return 
	end
	...
	if ( mySRH == 0 ) then
		...
		if tes3.player.object.race.id:lower() == "argonian" or tes3.player.object.race.id:lower() == "khajiit" then
		tes3.playAnimation({ reference = tes3.player, group = tes3.animationGroup.idleSneak, })
		else
		tes3.playAnimation({ reference = tes3.player, group = tes3.animationGroup.idleSneak, })
		tes3.loadAnimation({ reference = tes3.player, file = "trib_sit_01.nif", })
		end
		tes3.playAnimation({ reference = tes3.player1stPerson, group = tes3.animationGroup.hit1, })
		tes3.mobilePlayer.cameraHeight = 0.5 * tes3.mobilePlayer.cameraHeight
	...
	elseif ( mySRH == 1 ) then 
		if tes3.player.object.race.id:lower() == "argonian" or tes3.player.object.race.id:lower() == "khajiit" then
		tes3.playAnimation({ reference = tes3.player, file = "base_animKnA.nif", }) 
		tes3.playAnimation({ reference = tes3.player1stPerson, file = "base_animKnA.1st.nif", })
		else 
		tes3.loadAnimation({ reference = tes3.player })
		tes3.loadAnimation({ reference = tes3.player1stPerson })
		end
	...
	if ( not tes3.mobilePlayer.isMovingForward and not tes3.mobilePlayer.isMovingBack and not tes3.mobilePlayer.isMovingLeft and not tes3.mobilePlayer.isMovingRight ) then
		if tes3.mobilePlayer.health.base <= 150 then
			if mySRH == 1 then
				...
				currentHealth = currentHealth + config.lowhealthoption
				...
				if currentHealth >= tes3.mobilePlayer.health.base then currentHealth = tes3.mobilePlayer.health.base end
			end
		else
			if mySRH == 1 then
				tes3.mobilePlayer.health.current = tes3.mobilePlayer.health.current + config.highhealthoption
				currentHealth = currentHealth + config.highhealthoption
				if tes3.mobilePlayer.health.current >= tes3.mobilePlayer.health.base then tes3.mobilePlayer.health.current = tes3.mobilePlayer.health.base end
				if currentHealth >= tes3.mobilePlayer.health.base then currentHealth = tes3.mobilePlayer.health.base end
			end
		end
...
local function defaultTickTotalsCustomization()
	config.lowhealthoption = 5
	config.lowmagickaoption = 5
	config.highhealthoption = 15
	config.highmagickaoption = 10
end
...
--Page1
	...
	local subcat = page:createCategory("On the next page you can change the tick totals. \n Note: If you want the default totals again, Press the Default button.")	
	
	local subcat = page:createCategory(" Default")
    subcat:createButton{label = "Click to Default Tick Totals", callback = defaultTickTotalsCustomization, buttonText = "Default"}
...
--Page2
	...
	messageonoffButton = category1:createOnOffButton({
	...
	soundonoffButton = category2:createOnOffButton({
	
		label = "Sound On/Off",
        variable = mcm:createTableVariable{id = "soundoption", table = config},
		callback = function(self)
			if ( config.soundoption == true ) then
