...
local function updateWeapon(ref, mobile, weapon)
    ...
    local typeNode = ref.sceneNode:getObjectByName(attachSlots[weapon.type])
...
-- INVENTORY FIX --
-------------------
local inMenuInventory = false
local playerInCombatStance = false


local function hideSheathedWeapons()
    if not config.showWeapon then
        return
    end
    ...
    if player and player.sceneNode then
        local weapon = player.sceneNode:getObjectByName("Bip01 Weapon")
        if weapon then
            weapon.appCulled = true
        end
        
        local attachWeapon = player.sceneNode:getObjectByName("Bip01 AttachWeapon")
        if attachWeapon and not weapon then
            attachWeapon.appCulled = true
        end
        
        player.sceneNode:updateNodeEffects()
    end
end


local function showSheathedWeapons()
    if not config.showWeapon then
        return
    end
    ...
    if player and player.sceneNode then
        local attachWeapon = player.sceneNode:getObjectByName("Bip01 AttachWeapon")
        if attachWeapon then
            attachWeapon.appCulled = false
        end
    
        local weapon = player.sceneNode:getObjectByName("Bip01 Weapon")
        if weapon and not playerInCombatStance then
            weapon.appCulled = false
        end
...
event.register("menuEnter", function(e)
    inMenuInventory = true
    hideSheathedWeapons()
end)

event.register("menuExit", function(e)
    inMenuInventory = false
    showSheathedWeapons()
end)

event.register("equipped", function(e)
    if inMenuInventory then
...
event.register("weaponReadied", function(e)
    playerInCombatStance = true
end)

event.register("weaponUnreadied", function(e)
    playerInCombatStance = false
end)
-------------------
