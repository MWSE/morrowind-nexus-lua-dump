...
local function openQuickKeyPopup()
    ...
    if sForceStandardUI then
        message = "ZHI will replace this menu after closing."
    end

    zhiWindows[ZHI_WINDOWS.QuickKeyMenuPopup] = ZHIUI.createMessageBox('ZHI Info', message, false, false)
...
local function openFirstTimePopup()
    ...
    zhiWindows[ZHI_WINDOWS.FirstTimePopup] = ZHIUI.createMessageBox('Notification', firstTimeMessage, true, false, callbacks)
end
...
local function findUIHotkey(hotbar, key)
    ...
    local hotkeyLayout = ZHIUtil.findLayoutByNameRecursive(zhiWindows[ZHI_WINDOWS.Main].layout.content, ident)
...
local function openMagicSelectWindow (hotbar, key)
    ...
    local callbacks = {
        onSelectItem = ZHIUtil.bindFunction(onReceiveMagicHotkeyResult, hotbar, key),
...
local function openInventorySelectWindow(hotbar, key)
    ...
    I.ZHI.setMainWindowAlpha(FG_FADE_ALPHA)
    local callbacks = {
        onSelectItem = ZHIUtil.bindFunction(onReceiveInventoryResult, hotbar, key),
        onFocusLossItem = function(layout)
...
local function onHotkeySelectPressed(mouseEvent, layout)
    ...
    I.ZHI.setMainWindowAlpha(BG_FADE_ALPHA)
    zhiWindows[ZHI_WINDOWS.HotkeySelect] = ZHIUIMain.createHotkeySelectionWindow(ZHIUtil.bindFunction(onHotkeySelectMenuChoice, layout.userData.hotbar, layout.userData.hotkey))
    --I.ZHI.updateUI()
    ZHITooltip.updateTooltip(nil, nil)
end
...
local function getQuickKeyItem(recordId, itemId)
    ...
    if not itemObject then
        itemObject = Actor.inventory(self):find(recordId)
    end
    return itemObject
end
...
local function equipItemForActor(actor, itemObject, equipment, slot, isEnchantment)
    ...
    if (not current) or (current.id ~= itemObject.id) then
        ...
        local sounds = ZHIUtil.getSoundsForItem(itemObject)
...
local function onQuickKeyPressed(keyNum)
    ...
    elseif hotkey and hotkey.data.item then
        ...
        if itemObject then
            ...
            local equipmentSlot = ZHIUtil.getEquipmentSlotForItem(self, itemObject)
...
local function modifierFromText(text)
    if text == "Left Shift" then return ZHI_MODIFIERS.LeftShift end
    if text == 'Left Alt' then return ZHI_MODIFIERS.LeftAlt end
    if text == 'Left Ctrl' then return ZHI_MODIFIERS.LeftCtrl end
    if text == 'Mouse 3' then return ZHI_MODIFIERS.Mouse3 end
    if text == 'Mouse 4' then return ZHI_MODIFIERS.Mouse4 end
    if text == 'Mouse 5' then return ZHI_MODIFIERS.Mouse5 end
...
return {
    ...
    interface = {
        ...
        getFirstInventoryItem = function(recordId)
            local inventory = Actor.inventory(self)
            if not inventory then return nil end
    ...
    engineHandlers = {
        ...
        onActive = function()
            ...
            print('ZHI ZHISaveData.onCloseQuickKeyMenuFirstTimeFlag = ', ZHISaveData.onCloseQuickKeyMenuFirstTimeFlag)
            ...
            print("ZHI sShowHotbarHUD = ", sAutoStanceChange)
            print('ZHI sCompatibilityMode = ', sCompatibilityMode)
            print('ZHI sStanceQueue = ', sStanceQueue)
            print('ZHI sWindowAnchor = ', tostring(sWindowAnchor))
            ...
            for i=1,9 do
                input.registerTriggerHandler(string.format('QuickKey%d', i), async:callback(ZHIUtil.bindFunction(onQuickKeyPressed, i)))
            end

            input.registerTriggerHandler("ToggleHUD", async:callback(onToggleHUDHandler))

            input.registerTriggerHandler('ToggleWeapon', async:callback(ZHIUtil.bindFunction(onToggleStance, Actor.STANCE.Weapon)))
            input.registerTriggerHandler('ToggleSpell', async:callback(ZHIUtil.bindFunction(onToggleStance, Actor.STANCE.Spell)))
