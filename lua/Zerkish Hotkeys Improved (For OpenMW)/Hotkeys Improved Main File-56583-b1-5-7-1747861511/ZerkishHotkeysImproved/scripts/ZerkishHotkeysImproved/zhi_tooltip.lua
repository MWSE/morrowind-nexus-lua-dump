...
local function getEffectLine(effectParams, nameOnly, textColor)
    local root = {
        type = ui.TYPE.Flex,
        props = {
            horizontal = true,
    ...
    local isSkill = ZHIUtil.equalAnyOf(effect.id, core.magic.EFFECT_TYPE.AbsorbSkill,
    ...
    local isAttribute = ZHIUtil.equalAnyOf(effect.id, core.magic.EFFECT_TYPE.AbsorbAttribute,
    ...
    if isSkill then
        ...
        if b and e then
            text = string.sub(text, 1, b - 1) .. ZHIUtil.capitalize(tostring(effectParams.affectedSkill))
        end
    elseif isAttribute then
        local b, e = string.find(text, 'Attribute')
        if b and e then
            text = string.sub(text, 1, b - 1) .. ZHIUtil.capitalize(tostring(effectParams.affectedAttribute))
        end
    end
...
local function getSpellTooltipData(spell, showExtendedTooltips)
    ...
    local school = ZHIUtil.getSpellSchool(spell)
    if school then
        header.content:add({
            type = ui.TYPE.Text,
            props = {
    ...
    addItemLine(rows, string.format("Cost: %d", tonumber(ZHIUtil.getSpellCost(spell))))
    addItemLine(rows, string.format("Cast Chance: %d%%", tonumber(ZHIUtil.getSpellCastChance(spell))))
...
local function getItemRecordContent(itemType, record, itemObject, showExtendedTooltips)
    ...
    if itemType == types.Apparatus then
        if record.quality then
            addItemLine(content, string.format('Quality: %s', ZHIUtil.formatNumber(tostring(record.quality))))
        end
    elseif itemType == types.Armor then
        addItemLine(content, string.format("Armor Rating: %d", ZHIUtil.getArmorRatingForPlayer(record)))
        if showExtendedTooltips then
            if record.baseArmor then
                addItemLine(content, string.format("base rating: %s", ZHIUtil.formatNumber(tostring(record.baseArmor))), TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            end
    ...
    if ZHIUtil.equalAnyOf(itemType, types.Armor, types.Weapon) then
    ...
    if record.weight and record.weight > 0 then
        ...
        local weight = string.format("Weight: %f", record.weight)
        weight = ZHIUtil.formatNumber(weight)

        if itemType == types.Armor then
            weight = string.format("%s (%s)", weight, ZHIUtil.getArmorClass(record))
        end
...
local function getEnchantmentContent(itemType, record, itemObject, showExtendedTooltips)
    ...
    if record.enchant then
        ...
        for i=1,#enchantment.effects do
            table.insert(content, getEffectLine(enchantment.effects[i]))
            table.insert(content, {
                type = ui.TYPE.Widget,
                props = { size = util.vector2(0, 4) }
            })
        ...
        if ZHIUtil.equalAnyOf(enchantment.type, core.magic.ENCHANTMENT_TYPE.CastOnStrike, core.magic.ENCHANTMENT_TYPE.CastOnUse) then
...
local function getItemTooltipData(record, itemType, itemObject, showExtendedTooltips)
    ...
    if itemType == types.Ingredient then
        local alchemySkill = ZHIUtil.getSkillForPlayer('alchemy')
        local numShown = 0
...
local function setTooltipData(layout, data)
    local content = ZHIUtil.findLayoutByNameRecursive(layout.content, 'tooltip_content')
