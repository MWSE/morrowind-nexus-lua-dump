...
local function setupWidget(layout, icon, border, count, cond, charge, magic)
    ...
    local iconLayout = ZHIUtil.findLayoutByNameRecursive(layout.content, 'icon')
    local borderLayout = ZHIUtil.findLayoutByNameRecursive(layout.content, 'border')
    local bgLayout = ZHIUtil.findLayoutByNameRecursive(layout.content, 'bg_magic')
    ...
    local keyNumLayout = ZHIUtil.findLayoutByNameRecursive(layout.content, 'txt_hotbar')
    if keyNumLayout then
       keyNumLayout.props.visible = sShowKey
    end

    local countLayout = ZHIUtil.findLayoutByNameRecursive(layout.content, 'txt_count')
    ...
    local bars = ZHIUtil.findLayoutByNameRecursive(layout.content, 'data_bars')
...
local function setupWidgetFromHotkeyData(hotkeyLayout, hotkeyData)
    ...
    if item then
        ...
        if itemObject then
            ...
            if sShowCharge and itemRecord.enchant then
                ...
                assert(enchant)
                if ZHIUtil.equalAnyOf(enchant.type, core.magic.ENCHANTMENT_TYPE.CastOnStrike, core.magic.ENCHANTMENT_TYPE.CastOnUse) then
                    charge = 1.0
                    if itemData then
    ...
    elseif spell then
        ...
        if sShowSpellCastChance then
            local castChance = ZHIUtil.getSpellCastChance(spellObject)
            if castChance then
                cond = ZHIUtil.getSpellCastChance(spellObject) / 100.0
            end
        end
