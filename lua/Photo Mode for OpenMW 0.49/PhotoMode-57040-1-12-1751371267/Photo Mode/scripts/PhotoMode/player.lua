...
--[[
1.2 changes
add HexDoF shader by wareya
add settings keybind for the DoF shader
add settings toggle to restrict shader use to photo mode only (default yes)
add settings button to toggle if photo mode starts paused (default yes)
]]
...
local function updateCamera(realDt)
    ...
    camera.setPitch(camera.getPitch() + input.getMouseMoveY() * (1 / 650) + input.getAxisValue(input.CONTROLLER_AXIS.RightY) * (1 / 50))
    camera.setYaw(camera.getYaw() + input.getMouseMoveX() * (1 / 650) + input.getAxisValue(input.CONTROLLER_AXIS.RightX) * (1 / 50))

    -- gamepad movement for looking
    -- camera.setPitch(camera.getPitch() + input.getAxisValue(input.CONTROLLER_AXIS.RightY) * (1 / 50))
    -- camera.setYaw(camera.getYaw() + input.getAxisValue(input.CONTROLLER_AXIS.RightX) * (1 / 50))
    ...
    moveForward = moveForward + realDt * -input.getAxisValue(input.CONTROLLER_AXIS.LeftY)
    moveRight = moveRight + realDt * input.getAxisValue(input.CONTROLLER_AXIS.LeftX)
