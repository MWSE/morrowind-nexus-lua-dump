...
--[[
1.4 changes
- added a toggle to invert the Y axis on controller - thanks PenelopeJenga
- also added it for mouse since openmw's invert look settings didn't apply while using photo mode
- moved gamepad settings to their own group in the mod settings
]]
...
local DEADZONE_THRESHOLD_RIGHT = gamepadStorageKey:get('deadzoneRight')
local DEADZONE_THRESHOLD_LEFT = gamepadStorageKey:get('deadzoneLeft')
local GAMEPAD_X_AXIS_INVERT = gamepadStorageKey:get('gamepadInvertX')
local GAMEPAD_Y_AXIS_INVERT = gamepadStorageKey:get('gamepadInvertY')
...
local MOUSE_X_AXIS_INVERT = generalStorageKey:get('mouseInvertX')
local MOUSE_Y_AXIS_INVERT = generalStorageKey:get('mouseInvertY')
