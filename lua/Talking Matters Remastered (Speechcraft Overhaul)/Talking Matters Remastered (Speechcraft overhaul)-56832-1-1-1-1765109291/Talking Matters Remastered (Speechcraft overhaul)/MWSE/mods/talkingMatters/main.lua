--[[
    Mod: Talking Matters Remastered
    Author: rhjelte
    Version: 1.1.1
]]
...
-- Persuasion types enum
local PERSUASION_TYPE = {
    ADMIRE = 1,
    INTIMIDATE = 2,
    TAUNT = 3,
    BRIBE = 4,
...
--- @param npcRefe tes3reference the NPC reference whose disposition we change
--- @param value integer How much disposition should be change
local function modifyDisposition(npcRefe, value)
    tes3.modDisposition({reference = npcRefe, value = value})
end
...
local function closePersuasionMenu()
    local MenuPersuasion = tes3ui.findMenu(GUI_ID_MenuPersuasion)
    if MenuPersuasion then
        MenuPersuasion:destroy()
    end
end

local function issueOffendedGoodbye()
...
--- Trigger offense for an NPC, setting them to offended state
--- @param npcRefe tes3reference The NPC reference
--- @param issueGoodbye boolean|nil If true, closes persuasion menu and issues goodbye message (default: false)
local function triggerOffense(npcRefe, issueGoodbye)
    if not data.npcData[npcRefe.id] then
        return
    end

    data.npcData[npcRefe.id].persuasionTotal = 0
    data.npcData[npcRefe.id].offended = true

    if config.debugEnabled then
        log:debug("OFFENSE TRIGGERED: NPC %s is now offended", npcRefe.id)
    end

    if issueGoodbye then
        issueOffendedGoodbye()
    end
end

--- Process a persuasion attempt result for an NPC
--- @param success boolean Whether the persuasion attempt was successful
--- @param npcRefe tes3reference The NPC reference
--- @param persuasionType integer|nil The type of persuasion (use PERSUASION_TYPE)
local function processPersuasion(success, npcRefe, persuasionType)
    -- Increment topic count if data exists
    if data and data.amountTopics then
    ...
    if success then
        ...
        if config.limitedTopics then
            ...
            if (data.npcData[npcRefe.id].remainingSlots == 2) then
                timer.frame.delayOneFrame ( function()
                    tes3.messageBox(string.format("%s seems to be getting ready to leave", npcRefe.object.name))
                end)
            end
        end
        
        -- Only use Talking Matters persuasion total system if SmoothTalker is not installed
        if not smoothTalker then
...
local function checkPersuasion(npcRefe, idInfo)
    ...
    if failedPersuasionInfos[idInfo] then
        processPersuasion(false, npcRefe, nil)
    elseif successfulPersuasionInfos[idInfo] then
        processPersuasion(true, npcRefe, nil)
    end
end
    ...
    if e.info.type == tes3.dialogueType.journal then 
        if config.limitedTopics then
            ...
            -- It feels weird when an NPC dismisses the player right after they have said something so important that the player character writes in their journal.
                ...
                if config.debugEnabled then
                    log:debug("PICKED A TOPIC: Too few remaning slots detected after journal entry, adding minimum topics")
                end
            end
        end
        return
    ...
    timer.frame.delayOneFrame(function()
        ...
        if config.limitedTopics then
            ...
            elseif (remainingSlots < 0) then
                ...
                modifyDisposition(npcRefe, -lostDisposition)

                -- Integrate with SmoothTalker if available
                if smoothTalker and smoothTalker.getVersion() >= 3 then
                    smoothTalker.modPatience(npcRefe, -1 * config.patienceLost_SmoothTalker)
                end
...
local function hasDialogueChoice()
...
local function menuActivated(e)
    ...
    if config.persuasionLimit then
        if data.npcData[npcRefe.id].offended then
            -- If there's already a choice (e.g. crime), don't issue another
            if hasDialogueChoice() then
...
--- @param eventData SmoothTalkerPersuasionEventData
local function onSmoothTalkerPersuasion(eventData)
	local actionTypeMap = {
		[1] = PERSUASION_TYPE.ADMIRE,      -- ACTION.ADMIRE
		[2] = PERSUASION_TYPE.INTIMIDATE,  -- ACTION.INTIMIDATE
		[3] = PERSUASION_TYPE.TAUNT,       -- ACTION.TAUNT
		[5] = PERSUASION_TYPE.BRIBE        -- ACTION.BRIBE
	}
	local persuasionType = actionTypeMap[eventData.actionType] or PERSUASION_TYPE.OTHER

	if not eventData.patienceDepleted then
		processPersuasion(eventData.success, eventData.npcRef, persuasionType)
	end
end

--- @param eventData SmoothTalkerPatienceDepletedEventData
local function onSmoothTalkerPatienceDepleted(eventData)
	print(string.format("[Talking Matters] Received PatienceDepleted event for %s (menuOpen=%s)",
		eventData.npcRef.object.id or "unknown", tostring(eventData.persuasionMenuOpen)))

	-- Only issue goodbye if SmoothTalker's persuasion menu is open
	-- (otherwise the NPC is being talked to through vanilla dialogue)
	local issueGoodbye = eventData.persuasionMenuOpen

	triggerOffense(eventData.npcRef, issueGoodbye)
end
...
local function initialize(e)
    ...
    -- Check if SmoothTalker is installed
    smoothTalker = include("SmoothTalker.interop")

    -- Register callbacks for SmoothTalker custom events if it's installed
    if smoothTalker then
        event.register("SmoothTalker:PersuasionTriggered", onSmoothTalkerPersuasion)
        event.register("SmoothTalker:PatienceDepleted", onSmoothTalkerPatienceDepleted)
        log:info("[Talking Matters] Registered callbacks for SmoothTalker events")
    end
