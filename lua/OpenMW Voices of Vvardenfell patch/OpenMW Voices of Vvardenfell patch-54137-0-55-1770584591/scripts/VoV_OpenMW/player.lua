...
local infos = { greeting = {}, admire = {}, played = {} }
...
local function isInfoBlocked(info)
	...
	if not info.type then
		info.type = infos.greeting[infoId] and "greeting"
			or nil
	end

	local block
	if settingsGroup:get("blockAdmire") and infos.admire[infoId] then
		block = true
	elseif settingsGroup:get("noRepeat") and infos.played[infoId] then
		block = true
...
local function onInfoGetText(info)
	if not vovActor then			return		end
	if isInfoBlocked(info) then		return		end
...
return {
	eventHandlers = {
		DialogueResponse = function(e)
			useHelper = false
			if vovActor and e.type ~= "voice" then
				onInfoGetText(e)
			end
		end,
		UiModeChanged = function(e)
			if e.newMode == nil then
				if vovActor then
					silenceVoice(vovActor)
					vovActor = nil
				end
			elseif e.newMode == "Dialogue" and e.arg and e.arg ~= vovActor then
				vovActor = e.arg
			end
		end,
		tes3InfoGetText = function(e)
			if useHelper and vovActor then
				onInfoGetText{ infoId=e.info.id }
			end
		end
	}
}
