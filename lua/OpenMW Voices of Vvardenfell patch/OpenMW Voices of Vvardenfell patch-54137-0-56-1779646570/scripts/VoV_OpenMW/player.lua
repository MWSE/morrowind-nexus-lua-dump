...
local infos = storage.playerSection("temp_openmw_vov")
infos:setLifeTime(storage.LIFE_TIME.GameSession)

if core.dialogue and not infos:get("initialized") then
	for _, record in pairs(core.dialogue.greeting.records) do
		for _, v in ipairs(record.infos) do
			infos:set("greeting_" .. v.id, true)
		end
	end
	for _, v in ipairs(core.dialogue.persuasion.records["Admire Fail"].infos) do
		infos:set("admire_" .. v.id, true)
	end
	for _, v in ipairs(core.dialogue.persuasion.records["Admire Success"].infos) do
		infos:set("admire_" .. v.id, true)
	end
	infos:set("initialized", true)
end


local function isInfoBlocked(info)
	local infoId = info.infoId

	-- Backwards compatibility for OpenMW 0.50 and earlier
	if not info.type then
		info.type = infos:get("greeting_" .. infoId) and "greeting" or nil
	end

	local block
	if settingsGroup:get("blockAdmire") and infos:get("admire_" .. infoId) then
		block = true
	elseif settingsGroup:get("noRepeat") and infos:get("played_" .. infoId) then
		block = true
		print("VoV: Not playing repeated dialogue", infoId)
	elseif settingsGroup:get("greetings") and info.type ~= "greeting" then
		block = true
	end
	return block
end
...
local function onInfoGetText(info)
	local vovActor = info.actor
	if not vovActor or isInfoBlocked(info) then
		return
	end
	...
	if vovActor.type == types.NPC then
		local npcRecord = types.NPC.records[actorId]
		race = npcRecord.race
	...
	if not isPathValid(voicePath) then
		print(string.format("VoV: No voice file found at %s", voicePath))
		if debug then ui.showMessage(infoId)		end
		return
	end
	infos:set("played_" .. infoId, true)
		...
		UiModeChanged = function(e)
			if e.newMode == nil then
				if dialogTarget then
					silenceVoice(dialogTarget)
					dialogTarget = nil
				end
			elseif e.newMode == I.UI.MODE.Dialogue and e.arg and e.arg ~= dialogTarget then
				dialogTarget = e.arg
			end
		end,
		tes3InfoGetText = function(e)
			if useHelper and dialogTarget then
				onInfoGetText{ infoId=e.info.id, actor=dialogTarget }
			end
		end
	}
}
