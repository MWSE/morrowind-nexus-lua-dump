...
local useTypes = {
    NoteSuccess = 'Bardcraft_NoteSuccess',
}

if I.SkillFramework then
    I.SkillFramework.registerSkill('bardcraft', {
        name = l10n('Skill_Bardcraft'),
        description = l10n('Skill_Bardcraft_Desc'),
        icon = {
            fgr = 'icons/skillframework/bardcraft.dds',
        },
        attribute = 'agility',
        specialization = I.SkillFramework.SPECIALIZATION.Stealth,
        xpCurve = function(currentValue)
            return currentValue * (10 + math.floor(currentValue / 10) * 3)
        end,
        skillGain = {
            [useTypes.NoteSuccess] = 1.0,
        },
        statsWindowProps = {
            subsection = I.SkillFramework.STATS_WINDOW_SUBSECTIONS.Arts,
            onClick = function()
                omwself:sendEvent('BC_ToggleUI')
            end,
        }
    })

    I.SkillFramework.registerClassModifier('bardcraft', 'bard', 10)
elseif omwself.type == types.Player then
    if I.UI.showInteractiveMessage then
        I.UI.showInteractiveMessage('ERROR (Bardcraft):\nSkill Framework is not installed, or not loaded before Bardcraft. The mod will not function properly.')
    else
        local ui = require('openmw.ui')
        ui.showMessage('ERROR (Bardcraft):\nSkill Framework is not installed, or not loaded before Bardcraft. The mod will not function properly.')
    end
end
...
function P:onLoad(data)
    ...
    if self.stats.performanceSkill.level then -- Migrate old data
        skillStat.base = self.stats.performanceSkill.level
    end
    if self.stats.performanceSkill.xp then
        skillStat.progress = self.stats.performanceSkill.xp / self:getPerformanceXPRequired()
    end
...
function P:getPerformanceXPRequired()
    return I.SkillFramework.getSkillProgressRequirement('bardcraft')
end

function P:getPerformanceProgress()
    return I.SkillFramework.getSkillStat('bardcraft').progress
end

I.SkillFramework.addSkillStatChangedHandler(function(skillId, old, new)
    if skillId ~= 'bardcraft' then return end

    P.stats.performanceSkill = new

    local milestone = nil
    if new.base > old.base then
        for l = old.base + 1, new.base do
            if l % 10 == 0 then
                milestone = l
            end
        end
    ...
    if milestone and omwself.type == types.Player then
        omwself:sendEvent('BC_Milestone', { milestone = milestone })
    end
...
function P.handleStopEvent(data)
    ...
    if omwself and omwself.type == types.Player then
        ...
        local xpReq = P:getPerformanceXPRequired()
        local xpCurr = I.SkillFramework.getSkillStat('bardcraft').progress * xpReq
        core.sendGlobalEvent('BC_PlayerPerfSkillLog', { xpGain = P.xpGain, levelGain = P.levelGain, level = P.stats.performanceSkill.base, xpCurr = xpCurr, xpReq = xpReq })
    end
...
function P.getNoteAccuracy()
    ...
    local accuracy = math.pow(I.SkillFramework.calcStatFactor('bardcraft') / 100, 0.75) * I.SkillFramework.calcFatigueFactor() - difficultyFactor + (math.pow(P.currentConfidence, 1/2) * 0.5)
...
function P.handleNoteEvent(data)
    ...
    if P.performanceType ~= Song.PerformanceType.Ambient then
        if success then
            ...
            local skillStat = I.SkillFramework.getSkillStat('bardcraft')
            local oldLevel = skillStat.base
            I.SkillFramework.skillUsed('bardcraft', { useType = useTypes.NoteSuccess, scale = xp })
            P.levelGain = P.levelGain + (skillStat.base - oldLevel)
