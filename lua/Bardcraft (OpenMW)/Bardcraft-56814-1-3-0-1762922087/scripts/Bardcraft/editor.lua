...
uiTemplates = {
    ...
    performerDisplay = function(npc, itemHeight, thickBorders, onClick)
        ...
        local level = performerInfo and performerInfo.performanceSkill and performerInfo.performanceSkill.modified or 1
        local partIcon = nil
...
setMainContent = function(content)
    if wrapperElement then
        ...
        if mainContent[2] then
            auxUi.deepDestroy(mainContent[2])
        end
...
local function onFinalizeDraft(title, desc, cost)
    ...
    if configGlobal.options.bRequireBlankSheetMusic then
    ...
    local song = Helpers.deepCopy(Editor.song)
    song.title = title
    song.desc = desc
...
local function createFinalizeDraftModal()
    ...
    modalElement = ui.create(uiTemplates.modal(
    {
        ...
        content = ui.content {
            ...
            configGlobal.options.bRequireBlankSheetMusic and {
                template = I.MWUI.templates.textNormal,
                props = {
...
getStatsTab = function()
    ...
    local record = I.SkillFramework.getSkillRecord('bardcraft')
    local stat = I.SkillFramework.getSkillStat('bardcraft')
    local level = stat.base
    local req = I.SkillFramework.getSkillProgressRequirement('bardcraft')
    local maxLevel = level >= record.maxLevel
    local progress = stat.progress
    local xp = stat.progress * req
