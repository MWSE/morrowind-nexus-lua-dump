...
Data.Regions = require('scripts.Bardcraft.data.regions')
...
function C.isImperialTown(cell)
    if not cell then return false end
    for _, name in ipairs(Data.ImperialTowns) do
        if string.find(cell.name, name, 1, true) then
            return true
        end
    end
    return false
