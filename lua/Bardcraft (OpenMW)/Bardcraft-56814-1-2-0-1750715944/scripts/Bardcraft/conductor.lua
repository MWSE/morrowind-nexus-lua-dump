    ...
    if player then
        ...
        if performance.type == Song.PerformanceType.Tavern then
            ...
            if feedbackTree then
                local publicanQuality, patronQuality, impressiveness

                if performance.quality < 15 and performance.time >= 10 then
                    publicanQuality = 'Terrible'
                elseif performance.quality >= 95 and performance.time >= 15 then
                    publicanQuality = 'Excellent'
                elseif performance.quality >= 85 then
                    publicanQuality = 'Great'
                elseif performance.quality >= 70 then
                    publicanQuality = 'Good'
                elseif performance.quality >= 40 then
                    publicanQuality = 'Mediocre'
                else
                    publicanQuality = 'Bad'
                end

                if performance.quality >= 95 and performance.density >= 5.5 then
                    patronQuality = 'Excellent'
                elseif performance.quality >= 85 and performance.density >= 3.25 then
                    patronQuality = 'Great'
                elseif performance.quality >= 70 then
                    patronQuality = 'Good'
                elseif performance.quality >= 40 then
                    patronQuality = 'Mediocre'
                elseif performance.quality >= 15 then
                    patronQuality = 'Bad'
                else
                    patronQuality = 'Terrible'
                end

                if performance.mostDensePart >= 5.5 then
                    impressiveness = 'Impressive'
                elseif performance.mostDensePart < 2.5 then
                    impressiveness = 'Unimpressive'
                else
                    impressiveness = 'Standard'
                end

                local hierarchy = Cell.getRegionHierarchy(player.cell) or {}
                local context = {
                    quality = publicanQuality,
                    ...
                    weather = Data.Weather[currWeather] or 'Clear',
                }

                local publicanFeedback = Feedback.findMatchingNode(feedbackTree.publican, context)
                if publicanFeedback then
                    performanceLog.publicanComment = l10n(getRandomLocalizedLine(publicanFeedback))
                    if hierarchy.province then
                        performanceLog.publicanComment = performanceLog.publicanComment
                            :gsub('%%Province', l10n('Province_' .. hierarchy.province))
                            :gsub('%%LocalDrink', l10n(Data.LocalDrinks[hierarchy.province][math.random(1, #Data.LocalDrinks[hierarchy.province])]))
                    end
                end
                ...
                for _, actor in ipairs(activeActorsCopy) do
                    if actor.type == types.NPC and actor.cell == performance.cell and (not publican or actor.id ~= publican.id) and not isPerforming(actor.id) then
                        ...
                        context.quality = patronQuality
                        context.race = record.race
                        local feedback = Feedback.findMatchingNode(feedbackTree.patron, context)
                        if feedback then
                            local choice = getRandomLocalizedLine(feedback)
                            if choice then
                                -- Check if comment was already made by another NPC
                                local alreadySaid = false
                                for _, existingFeedback in ipairs(picked) do
                                    if existingFeedback == choice then
                                        alreadySaid = true
                                        break
                                    end
                                end
            ...
            local kickOut = performance.quality < 15 and performance.time >= 10
    ...
    if performanceEvalTimer >= performanceEvalInterval then -- Changed > to >= to match typical timer logic
        ...
        if totalNoteCount > 0 then
            ...
            if #partDensities > 0 then
                ...
                performance.mostDensePart = partDensities[1]
            else
...
local function doRandomEvent()
    ...
    if performance.type == Song.PerformanceType.Street then
        ...
        elseif performance.streetType == Cell.StreetType.Village then
            tipChance = tipChance * 0.5
        end
    end
    ...
    if performance.time >= 5 and math.random() < tipChance then
        ...
        elseif tipType == TipType.Wealthy then
            ...
            sfx = performance.type == Song.PerformanceType.Tavern and 'sound\\Bardcraft\\crowd\\coin-one.wav' or ''
            speechXp = 1
        end
        if performance.type == Song.PerformanceType.Tavern then
...
local function tickWeather(dt)
    if weatherCheckTimer <= 0 then
        local player = world.players[1]
        if not player or not player.cell.isExterior then
            return -- Interior cells can sometimes report incorrect weather data -- e.g. "Overcast" when it's really ash-storming outside
        end
        currWeather = world.mwscript.getGlobalVariables(player).bcCurrentWeather or 0
    ...
    eventHandlers = {
        BO_StartPerformance = function(data)
            if not playing then
                ...
                local perfType, streetName, streetType = Cell.canPerformHere(player.cell, data.type)
                if not perfType then
                    ...
                    return
                elseif perfType == Song.PerformanceType.Tavern then
                    ...
                    if configGlobal.options.iDailyPerformanceLimit ~= 0 then
                        local alreadyPerformed = data.playerStats.performedVenuesToday
                        for venue, count in pairs(alreadyPerformed) do
                            if type(count) == 'boolean' then count = 1 end
                            if venue == player.cell.name and count >= configGlobal.options.iDailyPerformanceLimit then
