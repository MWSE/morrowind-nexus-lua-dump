...
local function registerModConfig()
    ...
    page:createYesNoButton({
        label = "Stolen items mechanic - Guard stream",
        description = "This feature is not yet polished, so I have disabled it by default. If you enable it, guards (including Ordinators) will be able to detect stolen items above a certain bounty threshold, and with an opposed skill check",
        configKey = "stolenItemsMechanic_Guard",
    })

    page:createYesNoButton({
        label = "Stolen items mechanic - Guard stream",
        description = "This feature is not yet polished, so I have disabled it by default. If you enable it, owners will be able to detect items stolen from them, and with an opposed skill check ",
    ...
    page:createSlider({
        ...
        description = "Updates the AI refresh time to avoid frozen actors",
        min = 0.25,
        max = 5,
        step = 0.05,
        decimalPlaces = 2,
    ...
    page:createSlider({
        label = "Lenience",
        description = "How easy it is to hide items once detected. 0.50: very hard , 2: very easy",
        min = 0.5,
        max = 2,
        step = 0.25,
        jump = 0.25,
    ...
    page:createSlider({
        label = "Chance for footstep to be heard",
        description = "How often the footsteps sound trigger a skill check. 20: one in five chance, 100: always",
        min = 20,
        max = 100,
        step = 1,
        configKey = "stepTriggerChance",
    })
