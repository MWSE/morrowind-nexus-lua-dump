...
local function didTheyHearThat()
        ...
        for _,actor in pairs(mobileActors) do
            ...
            local notFightCheck = actor.fight < 83
            ...
            -- We check if the disposition is above 25. According to UESP, if fight is 80 and disposition is 25 or below, NPC will attack if we approach to less than 1000 units
            local dispositionCheck = disposition > 25
            if actor.fight > 70 and actor.fight < 83 and dispositionCheck then
                notFightCheck = true
            end

            local stolenCheck = data.currentCrime.npcs[actorName] and true or false -- If there is a list for the actor, then we have currently stolen stuff from them
