...
function CraftingMenu:updateTimeTakenPane()
    ...
    if self.doesTimePass and self:doesTimePass() and craftTime > 0 then
        ...
        -- Обработка часов для русского языка
        -- Часы
        local hoursText = ""
        local hours = math.floor(craftTime)
        if hours >= 1 then
            local lastDigit = hours % 10
            local lastTwoDigits = hours % 100
            ...
            hoursText = string.format("%d %s ", hours, hourWord)
        end

        -- Минуты
        local minutesText = ""
        local minutesTaken = (craftTime - math.floor(craftTime)) * 60
        if minutesTaken >= 1 then
            local lastDigit = math.floor(minutesTaken) % 10
            local lastTwoDigits = math.floor(minutesTaken) % 100
            
            local minuteWord = "минута"
            if lastTwoDigits >= 11 and lastTwoDigits <= 14 then
                minuteWord = "минут"
            else
                if lastDigit == 1 then
            ...
            minutesText = string.format("%d %s", math.floor(minutesTaken), minuteWord)
        end
        -- Конец обработки
...
function CraftingMenu:createMaterialButton(materialReq, list)
    ...
    if materialRecipe then
        ...
        if canCraftMaterial and not hasIngredient then
            requirement.text = requirement.text .. " (Нажмите, чтобы изготовить)"
