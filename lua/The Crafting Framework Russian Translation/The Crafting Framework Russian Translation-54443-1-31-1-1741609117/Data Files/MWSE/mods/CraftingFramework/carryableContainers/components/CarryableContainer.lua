        ...
        for _, result in ipairs(CarryableContainer.getInventory(containerRef)) do
            ...
            if stack.object then
                ...
                if isItself then
                    ...
                    tes3.messageBox("Вы не можете поместить этот контейнер внутрь самого себя")
                    return
                end
            end
        end
    ...
    local textField = mwse.mcm.createTextField(menu, {
        ...
        callback = function()
            ...
            if t.name == "" then
                ...
                tes3.messageBox("Переименовано в  %s", self:getName())
                menu:destroy()
                return
            end
            ...
            if #t.name > 31 then
                ...
                tes3.messageBox("Слишком длинное имя")
                return
            end
            ...
            tes3.messageBox("Переименовано в %s", t.name)
            menu:destroy()
...
function CarryableContainer:transferFiltered(filter)
    ...
    if remainingCapacity < totalWeight then
        tes3.messageBox("Недостаточно места в контейнере")
        return
    end
    ...
    if hasItemsToTransfer then
        ...
        tes3.messageBox("Нечего перемещать")
    end
end
