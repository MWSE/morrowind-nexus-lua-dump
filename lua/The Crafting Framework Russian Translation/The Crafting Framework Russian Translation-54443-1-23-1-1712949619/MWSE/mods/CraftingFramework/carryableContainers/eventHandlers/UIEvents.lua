...
local function onTooltip(e)
    ...
    --Display filter
    local filterText = "Контейнер"
    local filter = carryable:getFilter()
    if filter then
        filterText = filter.name .. " Контейнер"
    end
    ...
    if showOpenTooltip then
        ...
        local tooltipText = string.format("%s + щелчок что бы открыть", key)
        local tooltipLabel = e.tooltip:createLabel{
            text = tooltipText
        }
        tooltipLabel.borderLeft = 10
    ...
    if weightModifier then
        ...
        if carryable.containerConfig.getWeightModifierText then
            ...
            weightModifierText = string.format("Множитель веса: %.1fx", weightModifier)
        end
