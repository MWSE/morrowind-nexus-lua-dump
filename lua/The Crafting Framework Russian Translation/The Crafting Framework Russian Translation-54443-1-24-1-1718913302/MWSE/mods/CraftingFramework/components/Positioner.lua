...
local function showGuide()
    ...
    addLine("Вращать предмет", "Зажать ", config.mcm.keybindRotate.keyCode)
    addLine("Подвесить", "Нажать ", config.mcm.keybindModeCycle.keyCode)
    menu:updateLayout()
...
local function simulatePlacement()
    ...
    elseif (Positioner.active.sceneNode == nil) then
        ...
        tes3.messageBox{ message = "Положение предмета потеряно. Размещение сброшено."}
        Positioner.active.position = Positioner.itemInitialPos
        Positioner.active.orientation = Positioner.itemInitialOri
        endPlacement()
        return
...
local function cellChanged(e)
    ...
    if (Positioner.active.cell.isInterior or e.cell.isInterior) then
        tes3.messageBox{ message = "Вы не можете перемещать предметы между ячейками. Размещение сброшено."}
        Positioner.active.position = Positioner.itemInitialPos
        Positioner.active.orientation = Positioner.itemInitialOri
        endPlacement()
    end
...
Positioner.togglePlacement = function(e)
    ...
    if (target.sourceMod and not target.cell.isInterior) then
        tes3.messageBox{ message = "Нужно сначала взять и положить этот предмет" }
        return
    end
