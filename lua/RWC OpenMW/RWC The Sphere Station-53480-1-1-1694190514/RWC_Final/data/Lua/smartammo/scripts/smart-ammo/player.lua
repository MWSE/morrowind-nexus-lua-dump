...
local function checkAmmo(wpn, ammo)
    -- First check for lockpicks or probes since
    -- they can't be cast to a weapon type (bug #1)
    if types.Lockpick.objectIsInstance(wpn)
        or types.Probe.objectIsInstance(wpn) then
        return
    end
    local wpnType = Weapon.record(wpn).type
    local ammoType = Weapon.record(ammo).type
