...
local onceMessN = false --переменная однократного сообщения про нажатие N для автоогня для rw_gun
...
-- покадровая проверка, используется для определения auto оружия и патронов 
	onUpdate = function(dt)  -- function(dt)
		...
		--функция выдачи сообщения для rw_gun
		local rwgun = A.equipment(self, A.EQUIPMENT_SLOT.CarriedRight)
		local st = A.stance(self)
		if rwgun and rwgun.recordId:find('rw_gun') then
			if st == 1 then
				if onceMessN == false then
				ui.showMessage('Press N to shootburst')
				onceMessN = true
				end
			else
			onceMessN = false
