...
local function getStolenGoods()
	...
	local funcPrefix = modPrefix..': getStolenGoods()'
	if logLevel3 then
		mwse.log('%s "%s"',funcPrefix, pcCell.displayName)
	end
	local prisonMarkerRef = tes3.findClosestExteriorReferenceOfObject({object = prisonMarker})
	if not prisonMarkerRef then
		if logLevel1 then
			mwse.log('%s: prisonMarkerRef not found from "%s"',	funcPrefix, pcCell.displayName)
		end
		return
	end
	...
	if not dest then
		if logLevel1 then
			mwse.log('%s: prisonMarkerRef.destination = %s', funcPrefix, dest)
		end
		return
	end
	...
	if not evidenceCell then
		if logLevel1 then
			mwse.log('%s: prisonMarkerRef.destination.cell = %s', funcPrefix, evidenceCell)
		end
		return
	end
		...
		if ref.id:startswith('stolen_goods') then
			...
			if inventory then
				...
				and (#items > 0) then
					if logLevel1 then
						mwse.log(' "%s" "%s" at "%s" "%s"',
							funcPrefix, obj.id, obj.name, ref.cell.displayName, ref.sourceMod)
					end
					...
					tes3.addTopic({topic = 'stolen goods'})
...
Note: you can find these journal notes under the "stolen goods" topic.]],
		showReset = true,
