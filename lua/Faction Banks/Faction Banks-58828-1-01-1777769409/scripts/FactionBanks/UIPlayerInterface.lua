...
local function renderTextInput(textLines, existingText, editCallback, OKCallback, OKText)
    ...
    for _, text in ipairs(textLines) do
        table.insert(content, I.DaisyUtilsUI.textContent(text))
    end
    local textEdit = I.DaisyUtilsUI.boxedTextEditContent(existingText, async:callback(editCallback))
    local okButton = I.DaisyUtilsUI.boxedTextContent(OKText, async:callback(OKCallback))
...
return {
    interfaceName = "DaisyUtilsUI_Bank",
    interface = {
        version = 1,
