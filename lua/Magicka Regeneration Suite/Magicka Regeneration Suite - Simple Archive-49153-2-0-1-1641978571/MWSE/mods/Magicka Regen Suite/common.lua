...
function common.MRS(ref, maxMana)
	local restored
...
function common.restoreIf(ref, secondsPassed, alot)
    ...
    if common.stunted(ref) then return 0 end

    local maxMana = common.maxMagicka(ref)
    local amount = common.MRS(ref, maxMana) * ( secondsPassed or 1 )
	...
	if alot then
		...
		common.restoreMagicka(ref, amount)
		...
		if config.vampireChanges and common.isVampire(ref) then
			if common.isPCinInterior() or common.isNight() then
