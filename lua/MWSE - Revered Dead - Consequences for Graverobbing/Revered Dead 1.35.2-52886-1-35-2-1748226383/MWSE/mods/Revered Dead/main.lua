...
local function checkInTomb()
    ...
    if playerCell.isInterior and (playerCell.displayName:find("Ancestral Tomb") or playerCell.displayName:find("Ancestral Vaults") or (config.includeBurialCaverns == true and playerCell.displayName:find("Urshilaku,"))) and (common.checkCellWhitelisted(playerCell) == false) then
...
local function scanTombItems()
    ...
    if checkInTomb() then
        if config.warnOnTombEntry then
            tes3.messageBox(string.enteredTomb, playerCell.displayName)
        end
...
local function evaluateMerchant(merchant)
    ...
    elseif merchantActor.class.id == "Ordinator" or (merchantActor.faction and (merchantActor.faction.id == "Temple" or merchantActor.faction.id == "Redoran")) then
        merchantTrust = 70 -- Temple or Redoran-affiliated merchants
    elseif merchantActor.faction and (merchantActor.faction.id == "Telvanni" or merchantActor.faction.id == "Hlaalu" or merchantActor.faction.id == "Ashlanders") then
        merchantTrust = 60 -- Other Great House or Ashlander Tribe-affiliated merchants
...
local function checkSellingGraveGoods(e)
    ...
    if graveGoods then
        ...
        local reactionSeverity = merchantRating + playerTrust + config.difficultyMod
        local messageString
...
--[[
-- Worn reactions - Temporarily disabled due to crashes
...
local function checkEquippedGraveGood(e)
    ...
    if e.itemData and e.itemData.data and e.itemData.data.reveredDead and e.itemData.data.reveredDead.isExtremeGraveGoods then
        local currentWornGraveGoodsEq = tes3.getGlobal("RDWearingGravegoods")
...
local function checkUnequippedGraveGood(e)
    ...
    if e.itemData and e.itemData.data and e.itemData.data.reveredDead and e.itemData.data.reveredDead.isExtremeGraveGoods then
        local currentWornGraveGoodsUneq = tes3.getGlobal("RDWearingGravegoods")
        tes3.setGlobal("RDWearingGravegoods", (currentWornGraveGoods - 1))
        common.log:debug("Player is now wearing " .. tes3.getGlobal("RDWearingGravegoods") .. "obviously plundered items.")
    end
end
