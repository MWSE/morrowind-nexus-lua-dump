...
local function checkInTomb()
    ...
    if playerCell.isInterior and (playerCell.displayName:find("Ancestral Tomb") or playerCell.displayName:find("Ancestral Vaults") or (config.includeBurialCaverns == true and playerCell.displayName:find("Urshilaku"))) then
...
local function scanTombItems()
    ...
    if checkInTomb() then
        if config.warnOnTombEntry then
            tes3.messageBox(playerCell.displayName .. " is a revered burial place...")
        end
