...
--end skip those probably doubled by some silly mod
	...
	if ref.mobile then -- IMPORTANT!!! it seems I can safely access the variable only when mobile is available
		local script = refBaseObj.script
		if script then
			local context = script.context
			if context then
				local companion = context.companion
				if logLevel > 3 then
					logOnce(refId, "%s: %s context = %s, context.companion = %s", bpaprefix, refId, context, companion)
				end
				if companion then
					if logLevel > 0 then
	...
	if not (currHeadId == headId) then
		if logLevel > 0 then
			...
			logOnce(refId, "%s: '%s' in '%s' head mesh changed from %s to %s", bpaprefix, refId, ref.cell, currHeadId, headId)
		end
