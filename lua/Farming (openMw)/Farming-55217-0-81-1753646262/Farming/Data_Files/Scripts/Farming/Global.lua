...
local function SetDifficulty (data)
...
local function CarePlant(data)
	...
	if SavedData.YoungFlora.Cared[data.Plant.id]==0 then
		if types.Actor.stats.dynamic.fatigue(data.Player).current>(50-20*DifficultyMode) then
			data.Player:sendEvent("TakeCare",{Cared=false})
			SavedData.YoungFlora.Cared[data.Plant.id]=types.NPC.stats.skills.restoration(data.Player).modified/2+math.random(types.Actor.stats.attributes.luck(data.Player).modified/10)
		else
			data.Player:sendEvent("TakeCare",{Cared=false})
		end
	else 
			...
			for i, youngplant in pairs(SavedData.YoungFlora.List) do
				if youngplant:isValid() then
					if youngplant.scale<1 then
						local GrowChance=5+(youngplant.scale*10)
						...
						if math.random(100)<GrowChance+DifficultyMode*40 then
...
local function onUpdate()
	...
	if Initialize==false then
		Initialize=true
		for i,container in pairs(types.Container.records) do
			if container.weight==0 and container.isOrganic==true and container.isRespawning==true then 
	...
	elseif ActiveCell~=world.players[1].cell then	
		...
		for i, object in ipairs(world.players[1].cell:getAll(types.Container)) do
			if object.type.record(object).weight==0 and object.type.record(object).isOrganic==true and object.type.record(object).isRespawning==true and object.type.inventory(object):isResolved()==false then
				if math.random(100)<=5+math.random(types.Actor.stats.attributes.luck(world.players[1]).modified/10)+DifficultyMode*40 then
...
return {
	eventHandlers = {CarePlant=CarePlant, GrowPlants=GrowPlants, CreateFlora=CreateFlora,SetScale=SetScale, CreateYoungFlora=CreateYoungFlora, CreateClod=CreateClod,RemoveItem=RemoveItem,SetDifficulty=SetDifficulty},
	engineHandlers = {
        onUpdate = onUpdate,
		onSave=onSave,
		onLoad=onLoad,
