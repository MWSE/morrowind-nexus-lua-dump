...
local function sitDown(cycle)
    ...
    if cycle then
        sitAnimationOffset = (sitAnimationOffset % 8) + 1
    end

    local shouldSit = (not isSitting)
    if not cycle and isSitting then
        return
    end
    ...
    local finalIndex = ((config.sitAnimationGroup + sitAnimationOffset - 1) % 8) + 1
    local player = tes3.player
    tes3.loadAnimation({ reference = player, file = "VA_sitting.nif" })
    tes3.playAnimation({ reference = player, group = finalIndex })
end
...
local function onKeyDown(e)
    ...
    if isSitting and not e.isAltDown then
        standUp()
    else
        sitDown(e.isAltDown)
    end
