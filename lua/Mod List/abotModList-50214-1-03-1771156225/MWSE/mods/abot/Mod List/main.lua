...
function modConfig.onCreate(container)
	...
	s = "Loaded MWSE-Lua mods:\n(note: you need to be registered and logged to the site for github search to work!)\n"
	createListLabel(list, s)
	s2 = s2 .. "\n\n" .. s
	...
	for i = 1, #a do
		...
		local url = [[https://github.com/search?q=repo%3AMWSE%2Fmorrowind-nexus-lua-dump+MWSE%2FMods%2F]]..name..[[&type=code]]
		createListLink(list, s, url)
		s2 = s2 .. s
	end
