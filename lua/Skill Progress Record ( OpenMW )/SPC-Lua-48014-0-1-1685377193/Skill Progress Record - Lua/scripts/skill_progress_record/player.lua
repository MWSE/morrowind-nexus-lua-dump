...
local game = storage.playerSection("Settings_SkillProgressRecord_ASWITCH_GAME_KINDI")
local getL = core.l10n("skill_progress_record")
local currentGame

if game:get("Game") == "Starwind" then
    getL = core.l10n("skill_progress_record_starwind")
end

if core.API_REVISION < 29 then
    error("[Skill Progress Record] requires the latest OpenMW version to function. Please update to play this mod.")
end
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------
local attributesSkills = require('scripts.skill_progress_record.game')[game:get("Game")]
local attributes = { "strength", "intelligence", "willpower", "agility", "speed", "endurance", "personality", "luck" }
local skillsAttributes = {}
local lastSkillValues = {}
local trainedSkillValues = {}
...
local function gameUpdated()
    attributesSkills = require('scripts.skill_progress_record.game')[game:get("Game")]
    skillsAttributes = {}
    ...
    lastSkillValues = setmetatable({}, {
        __index = function()
            return 0
        end
    })
    trainedSkillValues = setmetatable({}, {
        __index = function()
            return 0
        end
    })
...
SPC.refreshSPCMenu = function(attributeName)
    ...
    divider.content:add(SPC.createNewText(nil, string.rep(" ", 4) .. string.rep("_", 80) .. string.rep(" ", 4)))
    ...
    newImage.autoSize = false
    newImage.props.size = util.vector2(30, 0)
    local infoButton = SPC.createNewButton("info_button",
        string.format("%s [%s]", getL("info"), section:get("Open Info")),
    ...
    newImage.autoSize = false
    newImage.props.size = util.vector2(30, 0)
    local resetButton = SPC.createNewButton("reset_button",
        string.format("%s [%s]", getL("reset"), section:get("Open Reset")), SPC.onResetButtonPressed)
    buttonsBlock2.content:add(newImage)
...
SPC.shakeMenu = function(menu)
    ...
    timer = time.runRepeatedly(function()
        ...
        if not menu or not menu.layout then
            timer()
            return
        end
...
return {
    engineHandlers = {
        ...
        onFrame = function(dt)
            ...
            --//waiting for settings changed event
            if currentGame == game:get("Game") then
                return
            else
                currentGame = game:get("Game")
                gameUpdated()
            end
        end,
