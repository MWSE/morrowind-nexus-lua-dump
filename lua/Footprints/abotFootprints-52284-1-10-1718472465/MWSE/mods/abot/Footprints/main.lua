...
maxDistance = 2048, -- max distance from player for footprints spawning (512 - 16384)
dirtyExterior = true,
dirtyInterior = true,
...
fpDuration = 240, -- duration of footprints in real time seconds.
fpMaxNumber = 8000, -- max total number of footprints present at the same time
...
local fpMaxNumber, dirtyDuration, inWaterTimerDelay
...
local simulatedClock = os.clock()

local function getSimulatedClock()
	if not tes3.menuMode() then
		simulatedClock = os.clock()
	end
...
-- higher startZ as I want to check for actor submerged in fake water
	...
	if math.abs(ori.x) > maxFootprintSlope then
		if logLevel1 then
			mwse.log('%s: deg abs(ori.x = %s)) > maxFootprintSlope = %s, too steep, skip',
	...
	if math.abs(ori.y) > maxFootprintSlope then
		if logLevel1 then
			mwse.log('%s: deg abs(ori.y = %s)) > maxFootprintSlope = %s, too steep, skip',
	...
	-- use node name to store creation time (in seconds from game start, 3 decimals)
	child.name = getSimulatedClock()
	...
	if puffsOn
	and string.multifind(tex, texPufflist, 1, true) then
...
local function loaded()
	...
	if ab01fpDirty then
		...
		for actorRefId, sec in pairs(ab01fpDirty) do
			local actorRef = tes3.getReference(actorRefId)
			if actorRef
