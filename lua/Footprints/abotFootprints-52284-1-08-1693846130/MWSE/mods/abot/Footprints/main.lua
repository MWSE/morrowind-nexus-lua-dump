...
-- higher startZ as I want to check for actor submerged in fake water
	...
	if hitRef then
		...
		refScale = hitRef.scale
		if sceneNode then
			refScale = sceneNode.scale
			if logLevel4 then
				mwse.log('%s: hitRef = "%s", mesh = "%s", hitRef.scale = %s, sceneNode.scale = %s',
					funcPrefix, hitRef.id, obj.mesh, hitRef.scale, refScale)
			end
		end
	end
	node.translation = hitNode.translation + pos -- note hitNode coordinates are local for meshes
	node.scale = footprintScale * refScale / attachNode.scale -- e.g. if the attchNode mesh is double scale, we need to halve the footprint scale
