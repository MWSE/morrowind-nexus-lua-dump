...
maxDistance = 1024, -- max distance from player for footprints spawning (512 - 16384)
dirtyExterior = true,
dirtyInterior = true,
...
alphaPerc = 40, -- footprinth alpha percent, lower = more transparent
...
local string_format, string_multifind = string.format, string.multifind
...
local function packDirtyActorRefs()
	...
	doPack = false
	if logLevel5 then
		mwse_log('%s: packDirtyActorRefs()', modPrefix)
	end
end
]]
local function clearDirtyActorRefs()
	for k, _ in pairs(dirtyActorRefs) do
...
local function getFileName(filePath)
	local s = filePath:match("([^\\/]-)%.%w-$")
	if not s then
		if logLevel1 then
			mwse_log('%s getFileName("%s") = %s', modPrefix, filePath, s)
		end
	end
	return s
end
...
---@return boolean
local function isValidActor(actorRef)
...
---@class ab01fpiwd
---@field actRef tes3reference
---@field waterMeshRef tes3reference?
---@field timed boolean?
...
output: actor (positive) in-water depth, 0 if not available, negative means out of water
	...
	if not isValidActor(actorRef) then
		return 0
	end

	if logLevel5 then
		mwse_log('%s: getInWaterDepth({{actRef = "%s", waterMeshRef = "%s"}) cell = "%s", z = %s',
	...
	if logLevel4 then
		mwse_log('%s: getInWaterDepth() cell = "%s", actor "%s" z = %s, waterLevel = %s, minWaterDepth = %s, depth = %s',
	...
	if actorPosZ > minDepthZ then
		...
		if dirtyEnabled then
			if t.timed then
				...
				if sec then
					if logLevel5 then
						mwse_log('%s: timed getInWaterDepth() dirtyActorRefs["%s"] decreased to %s', modPrefix, actorRefId, sec)
					end
				elseif logLevel3 then
					mwse_log('%s: timed getInWaterDepth() actor "%s" dirtyActorRefs timer expired', modPrefix, actorRefId)
				end
	...
	if dirtyEnabled then
		if logLevel5 then
			mwse_log('%s: getInWaterDepth() actor "%s" deep (%s) in water, dirtyActorRefs timer reset',
...
local function badWeatherOff()
	if logLevel2 then
		mwse_log('%s: badWeatherOff()', modPrefix)
	end
	badWeather = false
end
...
local function isBadWeatherIndex(weatherIndex)
 -- 4 Rain, 5 Thunder, 6 Ash, 7 Blight, 8 Snow, 9 Blizzard
	local isBad = (weatherIndex >= 4)
			and (weatherIndex <= 9)
	---tes3ui.showNotifyMenu(string_format("weather = %s, isBad = %s", w, isBad))
	return isBad
end
...
local function checkBadWeather(e)
	...
	if (not dirtyExterior)
	and (not dirtyInterior) then
		return
	end
	local funcPrefix =string_format('%s checkBadWeather()', modPrefix)
	local i
	if e then
		i = e.to.index
	else
	...
	if not i then
		if logLevel1 then
			mwse_log('%s: weather index not found', funcPrefix)
		end
		return
	end
	...
	if currBadWeather == badWeather then
		if logLevel4 then
			mwse_log('%s: badWeather = %s', funcPrefix, badWeather)
		end
		return
	end
	badWeather = currBadWeather
	if logLevel2 then
		mwse_log('%s: set badWeather %s', funcPrefix, badWeather)
	end
	if badWeather then
		if logLevel2 then
			mwse_log('%s: badWeather timer started', funcPrefix)
		end
...
local function weatherChanged(e)
	if logLevel2 then
		mwse_log('%s: weatherChanged()', modPrefix)
	end
	checkBadWeather(e)
end
...
local sneakDeltaAngles = {
{math.rad(3), math.rad(25)},
{math.rad(3), math.rad(6)},
}
...
---@param rayHit niPickRecord|niPickRecord[]
---@param actorRef tes3reference
---@return string?
local function getTexture(rayHit, actorRef)
	local funcPrefix =string_format('%s getTexture(actorRef = "%s")', modPrefix, actorRef)
	local texturingProperty = rayHit.object.texturingProperty
	if not texturingProperty then
		if logLevel5 then
			mwse_log('%s: texturingProperty = %s', funcPrefix, texturingProperty)
		end
		return
	end
	...
	if not baseMap then
		if logLevel5 then
			mwse_log('%s: baseMap = %s', funcPrefix, baseMap)
		end
		return
	end
	...
	if not texture then
		if logLevel5 then
			mwse_log('%s: texture = %s', funcPrefix, texture)
		end
		return
	end
	...
	if not fnam then
		if logLevel5 then
			mwse_log('%s: texture.fileName = %s', funcPrefix, fnam)
		end
		return
	end

	local fileName = getFileName(fnam)
	if not fileName then
		return
	end
	local tex = fileName:lower()
	if logLevel4 then
		mwse_log('%s: tex = "%s"', funcPrefix, tex)
	end
	...
	if hit then
		if logLevel3 then
			if hit == 1 then
				mwse_log('%s: hit cached "%s" texture', funcPrefix, tex)
			else
				mwse_log('%s: hit cached "%s" dirtyActorRefs texture', funcPrefix, tex)
			end
		end
	...
	if actorFeetDirty then
		if cellHasWeather(cell) then
			if dirtyExterior then
				if logLevel3 then
					mwse_log('%s: dirtyActorRefs "%s" hit exterior texture = "%s"',
		...
		elseif dirtyInterior then
			if logLevel3 then
				mwse_log('%s: dirtyActorRefs "%s" hit interior texture = "%s"', funcPrefix, actorRefId, tex)
			end
			return tex -- do not cache it to cachedTex[tex] though
		end
	end

	if tex:find('lava', 1, true) then
		if tex:find('crust', 1, true) then
			hit = 3 -- dirtyActorRefs
		end
	elseif not string_multifind(tex, texBlacklist, 1, true) then
		if string_multifind(tex, texDirtyWhitelist, 1, true) then
			hit = 2
		elseif string_multifind(tex, texWhitelist, 1, true) then
			hit = 1
		end
	end
	...
	if hit == 1 then
		if logLevel3 then
			mwse_log('%s: "%s" hit whitelisted texture', funcPrefix, actorRefId, tex)
		end
	elseif hit == 2 then
		if dirtyTexture then
			dirtyActorRefs[actorRef] = dirtyDuration
			if logLevel3 then
				mwse_log('%s: "%s" hit "%s" dirty whitelisted texture', funcPrefix, actorRefId, tex)
			end
		end
	elseif hit == 3 then
		if dirtyTexture then
			dirtyActorRefs[actorRef] = dirtyDuration
			if logLevel3 then
				mwse_log('%s: dirty "%s" hit "%s" lava texture', funcPrefix, actorRefId, tex)
			end
		end
	end
	return tex
...
---comment
---@param rayParams tes3.rayTest.params
---@return niPickRecord|niPickRecord[]|nil
local function rayTest(rayParams)
	local rayHit = tes3.rayTest(rayParams)
	if not logLevel4 then
		return rayHit
	end
	local funcPrefix =string_format('%s rayTest({position = %s, direction = %s, maxDistance = %s, root = %s})',
	...
	if rayHit then
		...
		if hitRef then
			...
			mwse_log('%s: hitRef = "%s", mesh = "%s"', funcPrefix, hitRef, mesh)
		else
			mwse_log('%s: intersection = "%s"', funcPrefix, rayHit.intersection)
		end
	else
		mwse_log('%s: rayHit = "%s"', funcPrefix, rayHit)
	end
	return rayHit
end
...
-- set in modConfigready
local worldController ---@type tes3worldController?
local worldVFXRoot ---@type niNode?
...
local function simulate(e)
	simulationClock = math_floor((e.delta * 1000) + 0.5) + simulationClock
end
...
local function checkPuffs() -- called from a timer on loaded()
	...
	--[[if count > maxPuffCount then
		maxPuffCount = count
		mwse_log('puffs maxPuffCount = %s', maxPuffCount)
	end]]
	puffs = t
end
...
---@param actorRef tes3reference
---@param meshId string
---@param fpStaSide string
---@return boolean?
local function placeFootprint(actorRef, meshId, fpStaSide)
	local funcPrefix = string_format('%s placeFootprint("%s", "%s", "%s")',
		...
		if rayHit then
			...
			if hitRef then
				...
				if obj.objectType == tes3_objectType_activator then
					local lcMesh = back2slash(obj.mesh:lower())
					if lcMesh:find('water', 1, true) then
						if logLevel1 then
							mwse_log('%s: hit "%s" fake water mesh', funcPrefix, lcMesh)
						end
					...
					if string_multifind(lcMesh, meshBlacklist, 1, true) then
						if logLevel3 then
							mwse_log('%s: hit "%s" blacklisted mesh, skip', funcPrefix, lcMesh)
						end
	...
	if rayHit then
		...
		if hitRef then
			...
			if hitRef.mobile then -- skip any not ignored mobiles
				if logLevel3 then
					mwse_log('%s: hit "%s" mobile, skip', funcPrefix, hitRefId)
				end
			elseif obj.objectType == tes3_objectType_miscItem then -- skip MISC for now
				if logLevel1 then
					mwse_log('%s: hit "%s" MISC item, skip', funcPrefix, hitRefId)
				end
			elseif hitRef.isLocationMarker then -- skip any editor marker
				if logLevel1 then
					mwse_log('%s: hit "%s" editor marker, skip', funcPrefix, hitRefId)
				end
			else
				if lcMesh:find('water', 1, true) then
					if logLevel1 then
	...
	if not tex then
		...
		if rayHit then
			...
			if found then
				if logLevel4 then
					mwse_log('%s: hit "%s" base footprint, skip', funcPrefix, node.name)
				end
				return
			end
		end
		...
		if rayHit then
			...
			if hitRef then
				...
				if obj.objectType == tes3_objectType_light then
					if logLevel1 then
						mwse_log('%s: hit "%s" non carryable light, continue', funcPrefix, hitRefId)
					end
					-- skip any not ignored light, but keep searching
				else
					if lcMesh:find('water', 1, true) then -- fake water mesh exception
						if logLevel1 then
							mwse_log('%s: hit "%s" fake water mesh', funcPrefix, lcMesh)
						end
						getInWaterDepth({actRef = actorRef, waterMeshRef = hitRef}) -- update dirtyActorRefs[actorRef]
					...
					if skipStairs
					and lcMesh:find('stair', 1, true)
					and ( not lcMesh:find('upstair', 1, true) ) then
						if logLevel3 then
							mwse_log('%s: hit "%s" staircase, skip', funcPrefix, lcMesh)
						end
		...
		if not cell.isInterior then
			...
			if rayHit then
				...
				elseif logLevel4 then
					mwse_log('%s: rayHit.distance = %s <= (minRayLen * 0.9) = %s, skip',
						funcPrefix, rayHit.distance, minRayLen * 0.9)
				end
			end
		end
	end
	...
	or (not tex) then
		if logLevel4 then
			mwse_log('%s: no valid texture found, skip', funcPrefix)
		end
	...
	ori = ori + diff
	ori.x = getNormalizedPiAngle(ori.x)
	ori.y = getNormalizedPiAngle(ori.y)
	ori.z = getNormalizedPiAngle(ori.z)

	local slope = math.abs(getNormalizedPiAngle(ori.x))
	if slope > maxFootprintSlope then
		if logLevel2 then
			mwse_log('%s: ori.x = %.02f > maxFootprintSlope = %.02f, too steep, skip',
	...
	if slope > maxFootprintSlope then
		if logLevel2 then
			mwse_log('%s: ori.y = %.02f > maxFootprintSlope = %.02f, too steep, skip',
				funcPrefix, math.deg(slope), math.deg(maxFootprintSlope))
		end
		return -- too steep, skip
	end
	...
	if mob.actorType == tes3_actorType_creature then
		sda = sneakDeltaAngles[2]
	else
		sda = sneakDeltaAngles[1]
	end
 ...
 -- decrease khajiit paw footprint size compared to creatures paw size
		...
		if boundSize_x then
			...
			local boundSize_y = boundSize.y
			if boundSize_y then
				boundSize_y = boundSize_y * k
	...
	local pdmr
	local animationData = getAnimationData(actorRef)
	if animationData then
		pdmr = animationData.positionDeltaMovementRoot
	end
	pos = rayHit.intersection + tes3vector3.new(
		dx + mob.velocity.x, dy + mob.velocity.y, 0.5)
	if pdmr then
		pos = pos + pdmr
	end

	if logLevel4 then
		mwse_log('%s: mesh = "%s", cell = "%s", position = %s, orientation = %s, scale = %s',
			funcPrefix, meshId, cell.editorName, pos, ori, footprintScale)
	end
	local mesh = fpBaseMeshes[meshId]
	if not mesh then
		if logLevel1 then
			mwse_log('%s: fpBaseMeshes["%s"] not found', funcPrefix, meshId)
		end
		return
	end
	...
	local materialProperty = node.children[1].materialProperty
	---assert(materialProperty)
	if not math.isclose(materialProperty.alpha, alphaMul, 0.0001) then
		if logLevel4 then
			mwse_log('%s: footprint alpha changed from %s to %s',
 ...
 -- use this one to skip collisions when possible. Counting on landscape not moving for now
	...
	if puffsOn then
		local puffMatch = string_multifind(tex, texPufflist, 1, true)
		if puffMatch then
			...
			--[[local name =string_format('Bip01 %s Toe0', string_upper(fpStaSide))
			...
			if logLevel4 then
				mwse_log('%s: tex = "%s", color = %s, "%s" footprint puff attached',
...
local function placeBaseFootprint(actorRef, fpStaType, fpStaSide)
	...
	local fpMeshId =string_format("%s_%s", fpStaType, fpStaSide)
...
local function clearVisitedCells()
	for k, _ in pairs(visitedCells) do
		visitedCells[k] = nil
	end
	visitedCells = {}
end
...
local function deleteNodeFootprints(parent)
	...
	---if logLevel4 then
		---mwse_log('%s: deleteNodeFootprints("%s/%s"), %s footprints deleted', modPrefix, parent.parent, parent, i)
	---end
end
...
---@param parent niNode
local function processNodeFootprints(parent)
	...
	for node in table.traverse({parent}) do
		...
		and (node.name == fpPrefix) then
			...
			if child:isOfType(ni_type_NiTriShape) then
				...
				---mwse_log('child.name = "%s"', name)
				-- NiTriShape name used to store creation time in millisec
				local age = simulationClock - tonumber(name) -- in millisec
				local alpha = ( 1.0 - ( age / duration ) ) * alphaMul

				changed = true
				if (age >= duration)
				or (alpha < 0.0125) then
					---mwse_log('>>> alpha = %s, age = %s', alpha, age)
					node.parent:detachChild(node)
					if fpNumber > 0 then
						fpNumber = fpNumber - 1
					end
...
---mwse_log('>>> alpha = %s, age = %s, duration = %s', alpha, age, duration)
...
local function processCellFootprints(cell)
	processNodeFootprints(cell.staticObjectsRoot)
	processNodeFootprints(cell.pickObjectsRoot)
	if not cell.isInterior then
		processNodeFootprints(worldVFXRoot)
		---processNodeFootprints(cell.landscape.sceneNode)
	end
	if logLevel3 then
		mwse_log('%s: processCellFootprints("%s") fpNumber = %s, fpNumMax = %s',
...
local function checkStartfpTimer()
	...
	if logLevel2 then
		mwse_log('%s: checkStartfpTimer() fpDuration = %s, dur = %s', modPrefix, fpDuration, dur)
	end
...
local function checkCellFootprints(cell, now)
	...
	if logLevel3 then
		mwse_log('%s: checkCellFootprints("%s", %s) lastVisited = %s',
	...
	if lastVisited then
		...
		if diff >= footprintDuration then
			if logLevel2 then
				mwse_log('%s: checkCellFootprints("%s", %s) cell not visited for %s hours, delete footprints',
...
local function cellDeactivated(e)
	if visitedCells[e.cell] then
		if logLevel4 then
			mwse_log('%s: cellDeactivated(), deleting cell "%s" footprints', modPrefix, e.cell.editorName)
		end
...
-- in game simulated hours, taking timescale into account
	...
	if previousCell then
		...
		if visitedCells[cell] then
			if logLevel4 then
				mwse_log('%s: cellChanged(), deleting cell "%s" footprints', modPrefix, e.cell.editorName)
			end
			deleteCellFootprints(cell)
		end
	end
...
-- pack dirtyActorRefs table and save it to player.data.ab01fpDirty
	for ref, sec in pairs(dirtyActorRefs) do
		if sec then
			local lcId = ref.id:lower()
			if logLevel4 then
				mwse_log('%s: save() ab01fpDirty["%s"] = %s', modPrefix, lcId, sec)
			end
...
local addSoundPrefix =string_format("%s addSound(e)", modPrefix)
...
local function addSound(e)
	...
	if fpNumber >= fpMaxNumber then
		if logLevel4 then
			mwse_log('%s: fpNumber (%s ) >= fpMaxNumber (%s), skip', addSoundPrefix, fpNumber, fpMaxNumber)
		end
		return
	end
	...
	if not actorType then
		if logLevel4 then
			mwse_log('%s: "%s" is not some npc/creature, skip', addSoundPrefix, actorRef)
		end
		return -- process only NPCs and creatures
	end

	local actorRefId = actorRef.id
	local actorBaseObj = actorRef.baseObject
	--[[if logLevel5 then
		mwse_log('%s: actorRef = "%s"', addSoundPrefix, actorRefId)
	end]]

	-- skip these creatures early
	if actorBaseObj.swims
	or actorBaseObj.flies then
		if not (
			actorBaseObj.walks
			or actorBaseObj.biped
		) then
			if logLevel4 then
				mwse_log('%s: "%s" swimming/flying creature, skip', addSoundPrefix, actorRefId)
			end
			return
		end
	end

	local lcSndId = sound.id:lower()
	local fnam = sound.filename
	if not fnam then
		if logLevel1 then
			mwse_log('%s warning: sound.filename = "%s", skip', addSoundPrefix, fnam)
		end
		return
	end
-- converting backslash to avoid risk of invalid escape sequence on string processing
	local lcFilename = back2slash(fnam:lower())

	if not (
		lcFilename:startswith('fx/')
		or lcFilename:startswith('cr/')
	) then -- skip non-standard sounds
		if logLevel5 then
			mwse_log('%s: actorRef = "%s", sound = "%s", fnam = "%s" not starting with "fx/" or "cr/", skip',
...
-- skip when turning left as it could be an animation often triggered by bump in Smart Companions
		if logLevel4 then
			mwse_log('%s: "%s" turning left, skip', addSoundPrefix, actorRefId)
		end
		return
	end
	...
	if actorRef == player1stPerson then
		if logLevel1 then
			mwse_log('%s: replacing "%s" with "%s"', addSoundPrefix, actorRefId, player.id)
		end
		actorRef = player
	end
	...
	if actorIsPlayer then
		if not playerFootprints then
			if logLevel4 then
				mwse_log('%s: "%s" is player, player footprints disabled, skip', addSoundPrefix, actorRefId)
			end
			return
		end
	elseif actorType == tes3_actorType_creature then
		if not creatureFootprints then
			if logLevel4 then
				mwse_log('%s: "%s" is creature, creature footprints disabled, skip', addSoundPrefix, actorRefId)
			end
			return
		end
	elseif not npcFootprints then
		if logLevel4 then
			mwse_log('%s: "%s" is NPC, NPC footprints disabled, skip', addSoundPrefix, actorRefId)
		end
		return
	end

	local actorMesh = actorBaseObj.mesh
	if (not actorMesh)
	or (actorMesh:len() <= 0) then
		if logLevel4 then
			mwse_log('%s: "%s" actor with no mesh, skip', addSoundPrefix, actorRefId)
		end
		return
	end
	local lcMeshName = getFileName(actorMesh:lower()) -- e.g. 'bear_black_larger'

	if mob.underwater then
		if logLevel4 then
			mwse_log('%s: "%s".underwater, calling getInWaterDepth, then skip', addSoundPrefix, actorRefId)
		end
...
-- includes dead, knocked down, knocked out, paralyzed, jumping, falling, swimming, flying
		if logLevel4 then
			mwse_log('%s: not "%s".canJump, skip', addSoundPrefix, actorRefId)
		end
		return
	end
	...
	if  mob.isFalling then
		...
		if logLevel4 then
			mwse_log('%s: "%s".isFalling, skip', addSoundPrefix, actorRefId)
		end
		return
	end
	...
	if mob.levitate > 0 then
		if logLevel4 then
			mwse_log('%s: "%s".levitate = %s, skip', addSoundPrefix, actorRefId, mob.levitate)
		end
		return
	end
-- canJump is not reliable enough
	...
	if #effects > 0 then
		if logLevel4 then
			mwse_log('%s: "%s" levitate effect, skip', addSoundPrefix, actorRefId)
		end
		return
	end
	...
	if #effects > 0 then
		if logLevel4 then
			mwse_log('%s: "%s" slowFall effect, skip', addSoundPrefix, actorRefId)
		end
		return
	end

	if isDummy(actorRef) then
		if logLevel4 then
			mwse_log('%s: "%s" is a dummy/invisible race NPC, skip', addSoundPrefix, actorRefId)
		end
-- skip practice dummy/mannequins/archery target creatures
	...
	if mob.waterWalking > 0 then -- waterWalking is a number!
		...
		elseif logLevel5 then
			mwse_log('%s: "%s" waterwalking sound, skip', addSoundPrefix, actorRefId)
		end
		return
		---end
	end
	...
	if e.isVoiceover then
		if logLevel4 then
			mwse_log('%s: "%s" e.isVoiceover, skip', addSoundPrefix, actorRefId)
		end
		return
	end

	--[[if logLevel4 then
		mwse_log('%s: actor = "%s"', addSoundPrefix, actorRefId)
	end]]

	local depth = getInWaterDepth({actRef = actorRef}) -- also upgrades dirtyActorRefs[actorRef]
	if depth >= minWaterDepth then
		if logLevel4 then
			mwse_log('%s: "%s" in water depth (%s) >= minWaterDepth (%s), skip',
	...
	if not actorIsPlayer then
		...
		if dist > maxDistance then
			if logLevel4 then
				mwse_log('%s: "%s".playerDistance (%s) > maxDistance (%s), skip',
	...
	if logLevel4 then
		mwse_log('%s: actorRef = "%s", sound = "%s", fnam = "%s"', addSoundPrefix, actorRefId, sound.id, fnam)
	end

	local fpStaType, fpStaSide

	-- try and match special case walk[lr] first
	local fpSndType, fpSndSide = lcFilename:match("(walk)([lr])_") -- e.g. 'fx/foot/walkl_md.wav'
	if logLevel4 then
		mwse_log('%s: fpSndType = "%s", fpSndSide = "%s"', addSoundPrefix, fpSndType, fpSndSide)
	end
	...
	if not fpSndType then
		fpSndType = string_multifind(lcFilename, fpSndTypes, 1, true)
	end
	if not fpSndType then
		if logLevel5 then
			mwse_log('%s: actorRef = "%s", sound = "%s", fnam = "%s" not a standard foot sound type, skip',
				addSoundPrefix, actorRefId, sound.id, fnam)
		end
		return
	end
	...
	if not fpStaSide then
		for i = 1, #fpLongSndSides do
			if lcSndId:find(fpLongSndSides[i], 1, true) then
				fpSndSide = fpLongSndSides[i]
				fpStaSide = fpStaSides[i]
				if logLevel4 then
					mwse_log('%s: "%s":find("%s", 1, true)', addSoundPrefix, lcSndId, fpSndSide)
				end
				break
			end
		end
	...
	--[[if not fpSndSide then
		if logLevel4 then
			mwse_log('%s: fpSndSide not found, skip', addSoundPrefix)
		end
		return
	end]]
	...
	if not fpStaSide then
		if logLevel4 then
			mwse_log('%s: fpStaSide not found, skip', addSoundPrefix)
		end
		return
	end

	if actorType >= tes3_actorType_npc then -- 1 = NPC, 2 = player
		if lcMeshName:startswith('mountedguar') then
		...
		if fpStaType then -- falling NPC or animal
			if logLevel4 then
				mwse_log('%s: falling NPC fpStaType = "%s"', addSoundPrefix, fpStaType)
			end
		...
		if logLevel3 then
			mwse_log('%s: fpSndType = "%s" fpStaType = "%s" lcFilename = "%s"', addSoundPrefix, fpSndType, fpStaType, lcFilename)
		end
		placeBaseFootprint(actorRef, fpStaType, fpStaSide)
	...
	local doLog = false
	local fpMeshId =string_format("%s_%s", lcMeshName, fpStaSide) -- e.g. new fpMeshId is now 'earth golem_r'
	if not (fpMeshId == lastFootprintId) then
		lastFootprintId = fpMeshId
		doLog = logLevel2
	end
	if doLog then
		mwse_log('%s: fpMeshId = "%s"', addSoundPrefix, fpMeshId)
	end
	...
	local rep = string_format('_%s%.nif', fpStaSide) -- e.g. '_r.nif'
	local fpMeshPath = back2slash(actorMesh:lower())
	fpMeshPath = string_gsub(fpMeshPath, '%.nif', rep) -- e.g. now "wormgod/earth golem_r.nif"
	if doLog then
		mwse_log('%s: fpMeshPath = "%s"', addSoundPrefix, fpMeshPath)
	end
	if tes3.getFileExists('Meshes/'..fpMeshPath) then
		if doLog then
			mwse_log('%s: tes3.loadMesh("%s")', addSoundPrefix, fpMeshPath)
		end
...
local function referenceDeactivated(e)
	...
	if dirtyActorRefs[ref] then
		if logLevel4 then
			mwse_log('%s: referenceDeactivated() dirtyActorRefs["%s"] = nil', modPrefix, ref.id)
		end
...
local function death(e)
	...
	if dirtyActorRefs[ref] then
		if logLevel4 then
			mwse_log('%s: death() dirtyActorRefs["%s"] = nil', modPrefix, ref.id)
		end
		dirtyActorRefs[ref] = nil
		---doPack = true
	end
...
local function loaded()
	...
	assert(worldVFXRoot == worldController.vfxManager.worldVFXRoot)
	...
	if ab01fpDirty then
		...
		for actorRefId, sec in pairs(ab01fpDirty) do
			...
			and (actorRef.position:distance(playerPos) <= 16384) then
				if logLevel4 then
					mwse_log('%s: loaded() ab01fpDirty["%s"] = %s', modPrefix, actorRefId, sec)
				end
				dirtyActorRefs[actorRef] = sec
			end
...
and any visible footprint is discarded on game executable restart anyway.]],
	...
	---local controls = sideBarPage:createCategory({label = mcmName})
	local category = sideBarPage:createCategory({})
	---category:createInfo({text = info})

	category:createYesNoButton({
		label = 'Player footprints',
		description = [[Enable player footprints]],
		configKey = 'playerFootprints'
	})

	category:createYesNoButton({
		label = 'NPCs footprints',
		description = [[Enable NPCs footprints]],
		configKey = 'npcFootprints'
	})
	category:createYesNoButton({
		label = 'Creatures footprints',
		description = [[Enable creatures footprints.]],
		configKey = 'creatureFootprints'
	})
...
or increasing it if you have good frame rate and want the footprints to last longer e.g. to track enemies in the wilderness.]]
	...
	category:createSlider({
		...
		description = [[Max duration of each footprint in real time seconds.]]..tweakHint,
		configKey = 'fpDuration',
		min = 30, max = 7200, step = 1, jump = 5,
	})

	category:createSlider({
		label = 'Footprints max distance',
		description = [[Max distance from player for footprints spawning.]]..tweakHint,
	...
	category:createSlider({
		...
		description = [[Max total number of footprints that could be present at the same time.]]..tweakHint,
		configKey = 'fpMaxNumber',
		min = 100, max = 50000, step = 1, jump = 10,
	})

	category:createYesNoButton({
		label = 'Dirty feet in exteriors/interiors with bad weather',
		description = [[Enables dirty feet in exteriors/interiors with bad weather.]],
		configKey = 'dirtyExterior'
	})

	category:createYesNoButton({
		label = 'Dirty feet in interiors',
		description = [[Enables having still dirty feet for a while after entering interiors.]],
		configKey = 'dirtyInterior'
	})

	category:createYesNoButton({
		label = 'Dirty feet in bad weather',
		description = [[Enables dirty feet in bad weather (e.g. raining, snowing).]],
		configKey = 'dirtyWeather'
	})
...
lowering it to "1. Yes, check only standard water" if you have low FPS.]],
		configKey = 'dirtyUnderWater'
	})

	category:createYesNoButton({
		label = 'Dirty feet after waterwalking',
		description = [[Enables dirty feet from waterwalking.]],
		configKey = 'dirtyWaterwalk'
	})

	category:createYesNoButton({
		label = 'Dirty feet from dirty/sticky surfaces',
		description = [[Enables dirty feet from walking over a dirty/sticky surface
e.g. mud, snow, lava, sand...]],
		configKey = 'dirtyTexture'
	})

	category:createYesNoButton({
		label = 'Footprints puffs',
		description = [[Enables Footprints puffs effect on proper surfaces e.g. snow, sand, ash...]],
		configKey = 'puffsOn'
	})

	category:createYesNoButton({
		label = 'Skip stairs',
		description = [[Try detecting and skipping staircases when placing footprints.]],
		configKey = 'skipStairs'
	})

	category:createSlider({
		label = 'Footprints alpha %s%%',
		description = [[Footprint initial alpha percent (lower = more transparent).
...
Note: footprints automatically fade out from this alpha value as time goes by until they are fully deleted.]],
	...
	category:createSlider({
		...
		description = [[Duration of footprints in simulated game hours.
...
current/previous cell footprints will be deleted.]],
		configKey = 'footprintDuration',
		min = 1, max = 48, step = 1, jump = 5,
	})

	category:createSlider({
		label = 'Dirty feet duration: %s sec',
		description = [[Duration in real time seconds of dirty feet effect
(e.g. from wading in water, walking on snow...)
Effective only when some "Dirty feet" option is enabled.]],
		configKey = 'dirtyDuration',
		min = 10, max = 300, step = 1, jump = 10,
	})

	category:createSlider({
		label = 'Wet state detection timer delay: %s ms',
		description = [[Timer delay (in real time milliseconds) to detect actor in water/dripping wet feet.
Effective only when some "Dirty feet" option is enabled.
Updates on reload.]],
		configKey = 'msInWaterTimerDelay',
	...
	category:createYesNoButton({
		...
		description = [[Beast races by default cannot wear visible boots and always have bare feet footprints.
Only enable if you are using a mod allowing beast races to equip boots/shoes visually covering/replacing beast feet.]],
		configKey = 'beastBoots'
	})

	category:createSlider({
		label = 'Delay on cell change: %s',
		description = [[Delay on cell change.
Try increasing it only if you see some footprints spawned mid-hair.]],
		configKey = 'delayOnCellChange',
...
function ()
	...
	daysPassed = worldController.daysPassed
	hour = worldController.hour
	timescale = worldController.timescale
		...
		for j = 1, 2 do
			...
			if tes3.getFileExists(meshPath) then
				...
				if meshNode then
					...
					if (not meshNodeName)
					or (meshNodeName:len() <= 0) then
						if logLevel4 then
							mwse_log('%s "%s".name set to "%s"', modPrefix, fpMeshId, fpPrefix)
						end
					...
					if logLevel5 then
						for k, v in pairs(meshNode.children) do -- pairs needed it can have nil entries
							mwse_log('children[%s] = %s', k, v)
						end
						mwse_log('%s: fpBaseMeshes["%s"] = %s', modPrefix, fpMeshId, fpBaseMeshes[fpMeshId])
					end
				else
					mwse_log('%s error tes3.loadMesh("%s")', modPrefix, meshFile)
				end
			else
				mwse_log('%s error: "%s" not found', modPrefix, meshPath)
			end
		end
	end
