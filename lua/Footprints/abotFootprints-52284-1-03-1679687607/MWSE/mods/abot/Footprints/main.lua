...
local mData = dofile(modPrefix .. '.data')
 -- {'anml','boot','boot2','foot','guar','hoof','paw','puff','rept','skel','taln'}
...
function ()
	...
	-- fpStaTypes = {'anml','boot','boot2','foot','guar','hoof','paw','rept','skel','taln'}
...
local meshBlacklist = {'water','tree','ex_v_ban' --[[,'lift']]}
...
local function checkPuffs() -- called from a timer on loaded()
 ...
 -- current total number of placed footprint, reset in modConfigReady()
local fpNumber = 0
...
-- higher startZ as I want to check for actor submerged in fake water
	...
	local cell = actorRef.cell
	local down = player.upDirection:copy()
	down:negate() -- negate() negates in place!
	local rayParams = {position = pos, direction = down,
		ignore = toIgnore, maxDistance = raySize + minRayLen,
		returnNormal = true, useModelCoordinates = true, returnColor = true,
-- interactable (NPCs, items, plants, activators, doors) + EditorMarkers
		root = cell.pickObjectsRoot
	}

	local tex, hitRef, lcMesh
	local rayHit = rayTest(rayParams)

	if rayHit then
		hitRef = rayHit.reference
		if hitRef then
			lcMesh = back2slash(string.lower(hitRef.object.mesh))
			if hitRef.mobile then -- skip any not ignored mobiles
				if logLevel1 then
					mwse.log('%s: hit "%s" mobile, skip', funcPrefix, hitRef.id)
				end
			elseif hitRef.object.objectType == tes3_objectType_miscItem then -- skip MISC for now
				if logLevel1 then
					mwse.log('%s: hit "%s" MISC item, skip', funcPrefix, hitRef.id)
				end
			elseif hitRef.isLocationMarker then -- skip any editor marker
				if logLevel1 then
					mwse.log('%s: hit "%s" editor marker, skip', funcPrefix, hitRef.id)
				end
			else
	...
	if not tex then
		...
		if rayHit then
			...
			if hitRef then
				...
				if hitRef.baseObject.objectType == tes3_objectType_light then
					if logLevel1 then
					...
					--[[if hitRef.mobile then -- should never happen as worldObjectRoot should only have player
						if logLevel1 then
							mwse.log('%s: WARNING hit "%s" mobile, skip', funcPrefix, hitRef.id)
						end
					else
					end]]
					tex = getTexture(rayHit, actorRef)
	...
	local diff = rotationDifference(player.upDirection, rayHit.normal)
	ori = ori + diff
	...
	if logLevel3 then
		mwse.log('%s: actorRef = "%s", meshId = "%s",\nscale = %s, pos = %s, actorRefPos = %s\ndx = %s, dy = %s, fpNumber = %s',
			funcPrefix, actorRefId, meshId, footprintScale, pos, actorRef.position, dx, dy, fpNumber)
	end
...
local function checkDeleteNodeFootprints(parent, now)
		...
		and (node.name == fpPrefix) then
			...
			if child:isOfType(ni_type_NiTriShape) then
				...
				if age > fpDuration then
					...
					--[[ nope alpha is global for all the footprint meshes
					child.materialProperty.alpha = child.materialProperty.alpha * (fpDuration - age) / fpDuration
					]]
...
local function checkDeleteCellFootprints(cell)
	local now = getRoundedRealtimeSecondsFromStart()
	checkDeleteNodeFootprints(cell.staticObjectsRoot, now)
	checkDeleteNodeFootprints(cell.pickObjectsRoot, now)
	if cell.isInterior then
		return
	end
	checkDeleteNodeFootprints(cell.landscape.sceneNode, now)
end

local function checkDeleteCurrCellFootprints()
	checkDeleteCellFootprints(player.cell)
end
...
local function checkStartfpTimer()
	...
	fpTimer = timer.start({duration = (0.05 * fpDuration) - (0.1 * math.random()),
		callback = checkDeleteCurrCellFootprints, iterations = -1})
end
...
local dummies = {'dumm','mann','target'}
...
local function addSound(e)
	...
	if fpNumber >= fpMaxNumber then
		if logLevel4 then
			mwse.log('%s: fpNumber (%s ) >= fpMaxNumber (%s), skip', funcPrefix, fpNumber, fpMaxNumber)
		end
		return
	end
	...
	if not actorType then
		if logLevel4 then
			mwse.log('%s: "%s" is not some npc/creature, skip', funcPrefix, ref)
		end
...
-- skip when turning left as it is the animation often triggered by bump in Smart Companions
			if logLevel4 then
				mwse.log('%s: "%s" turning left, skip', funcPrefix, ref)
			end
			return
		end
	end
	...
	if mob.underwater then
		if logLevel4 then
			mwse.log('%s: "%s".underwater, getInWaterDepth, then skip', funcPrefix, actorRefId)
		end
...
-- canJump is not reliable enough
	...
	if isDummy(mob) then
		if logLevel4 then
			mwse.log('%s: "%s" is a dummy/invisible race NPC, skip', funcPrefix, actorRefId)
		end
...
-- converting backslash to avoid risk of invalid escape sequence on string processing
	...
	if actorType >= tes3_actorType_npc then -- 1 = NPC, 2 = player
		...
		if fpStaType then -- falling NPC or animal
			if logLevel4 then
				mwse.log('%s: falling NPC fpStaType = "%s"', funcPrefix, fpStaType)
			end
		...
		if logLevel3 then
			mwse.log('%s: fpSndType = "%s" fpStaType = "%s" lcFilename = "%s"', funcPrefix, fpSndType, fpStaType, lcFilename)
		end
		placeBaseFootprint(actorRef, fpStaType, fpStaSide)
	...
	if doLog then
		mwse.log('%s: fpMeshId = "%s"', funcPrefix, fpMeshId)
	end
	...
	if doLog then
		mwse.log('%s: fpMeshPath = "%s"', funcPrefix, fpMeshPath)
	end
...
Max distance from player for footprints spawning.]], 'maxDistance'),
...
Enables dirty feet in bad weather (e.g. raining, snowing).]], 'dirtyWeather'),
	...
	controls:createYesNoButton({
		...
		description = getYesNoDescription([[Default: %s.
Enables dirty feet when wet from wading water.]], 'dirtyWater'),
...
Enables Footprints puffs effect on proper surfaces e.g. snow, sand, ash...]], 'puffsOn'),
	...
	controls:createSlider({
		...
		description = getDescription([[Default: %s%%.
Footprint alpha percent (lower = more transparent).
Use it to tweak footprints transparency/darkness for better blending according to your texture replacers.
Note: as transparency alpha is global for a footprint mesh and all its clones, it can't be used to simulate different decay
state of single footprints.
To obtain a similar effect, I'm slowly sinking each footprint according to time passed instead.
]], 'alphaPerc'),
		variable = createConfigVariable('alphaPerc'),
		min = 15, max = 80, step = 1, jump = 5,
	})
...
'footprintDuration'),
	...
	controls:createSlider({
		...
		description = getDescription([[Default: %s.
Max duration of each footprint in real time seconds.
This is the first setting you should try to lower if too many spawned footprints are badly hitting your framerate,
or to increase if you have good framerate and want the footprints to last longer e.g. to track enemies in the wilderness.
]], 'fpDuration'),
...
Updates on reload.]], 'msInWaterTimerDelay'),
	...
	controls:createSlider({
		...
		description = getDescription([[Default: %s.
Max total number of footprints that could be present at the same time.
If lowering Footprint max duration is not enough, you can try lowering this value too.
]], 'fpMaxNumber'),
		variable = createConfigVariable('fpMaxNumber'),
		min = 500, max = 100000, step = 1, jump = 10,
	})
...
Only enable if you are using a mod allowing beast races to equip VISIBLE boots/shoes covering feet.]], 'beastBoots'),
