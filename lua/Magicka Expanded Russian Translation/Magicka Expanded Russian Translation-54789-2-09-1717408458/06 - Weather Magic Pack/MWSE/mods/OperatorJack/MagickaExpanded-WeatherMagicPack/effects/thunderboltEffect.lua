...
local function onThunderboltCollision(e)
    if e.collision then
        ...
        if (tes3.worldController.weatherController.currentWeather.index ~= tes3.weather.thunder) then
            if (caster == tes3.player) then
                tes3.messageBox("Заклинание срабатывает, но не дает никакого эффекта, если нет грозы.")
            end
            return
        end
...
local function addThunderboltEffect()
	framework.effects.destruction.createBasicEffect({
		...
		name = "Удар молнии",
		description = "Удар молнии. Требуется находиться на улице и в грозу. Громовые молнии наносят урон и оглушают пораженные цели.",
