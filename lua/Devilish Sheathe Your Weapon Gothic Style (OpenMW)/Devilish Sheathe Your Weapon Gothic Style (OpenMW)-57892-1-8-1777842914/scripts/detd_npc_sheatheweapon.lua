...
local ignoreWeaponReaction = false -- new flag
...
-- normalize distance: 0 stays 0, 1..98 becomes 99, 99+ keeps own value
local function normalizeDistance(d)
    if not d or d == 0 then return 0 end
    if d < 99 then return 99 end
    return d
end
...
local function restoreAI()
    if aiWasRemoved then
        if savedPackage and savedPackage.type ~= "Unknown" then
            pcall(function()
                if savedPackage.type == "Wander" then
                    AI.startPackage({
                        type     = "Wander",
                        distance = normalizeDistance(savedPackage.distance),
                        duration = (savedPackage.duration or 0) * 3600,
                        idle     = savedPackage.idle,
                        isRepeat = savedPackage.isRepeat,
                    })
                else
...
-- preventing from drawing the blade
local function detd_SetIgnoreWeaponReaction(ignore)
    ignoreWeaponReaction = ignore
    
    if ignore and aiWasRemoved then
        restoreAI()
...
local function updateLogic()
 ...
 -- print(disp)
    ...
    if playerWeaponDrawn and shouldReact and disp < 90 and not ignoreWeaponReaction then
        if not aiWasRemoved then
            local currentPkg = AI.getActivePackage(self)
            if currentPkg and currentPkg.type == "Wander" then
                savedPackage = {
                    type     = "Wander",
                    distance = currentPkg.distance,
                    duration = currentPkg.duration,
                    idle     = currentPkg.idle,
                    isRepeat = currentPkg.isRepeat,
                }
            elseif currentPkg and currentPkg.type ~= "Unknown" then
...
return {
    eventHandlers = { 
        detd_pcWeaponState = detd_pcWeaponState,
        -- new event for mods
        detd_SetIgnoreWeaponReaction = detd_SetIgnoreWeaponReaction
    },
