...
function M.processControls(dt, dialogTarget)
	...
	if p.choose and math.abs(movey) > 0.9 and p.count < 1 then
		p.count = 1.25
--[[
		local old = p.save
...
--	local deltaPos = dCam.pos - self.position
	...
	lerp = (8 * math.abs(deltaYaw) / math.pi) ^ 2
...
--	local headPos = self.position + util.vector3(0, 0, dCam.playerHeight * 0.974)
--	deltaPos = dCam.pos + util.transform.rotateZ(dCam.object.rotation:getYaw()):apply(dCam.head) - headPos
--	deltaPos = dCam.deltaPos + util.transform.rotateZ(dCam.object.rotation:getYaw()):apply(dCam.head)
--		- util.vector3(0, 0, dCam.playerHeight * 0.974)

	local lengthXY = util.vector2(deltaPos.x, deltaPos.y):length() - dCam.radius
	local deltaPitch = - math.atan2(deltaPos.z, math.max(lengthXY, 10)) - self.rotation:getPitch()
--	local deltaPitch = - math.asin( deltaPos.z / deltaPos:length() ) - self.rotation:getPitch()
        if math.abs(deltaPitch) > math.rad(10) then
		turningToTarget = true
        end
	lerp = (8 * math.abs(deltaPitch) / math.pi) ^ 2
	...
	destVec = destVec * util.clamp((distance - zoom1st.dist), -5, 2000)
--	dCam.dDist = distance		dCam.destVec = destVec		dCam.lxy = lengthXY
	...
	if dCam.aperture > 0 and zoom1st.level > 0.4 then
		...
		dof.uDepth = zoom1st.dist + (dCam.radius / 3)
