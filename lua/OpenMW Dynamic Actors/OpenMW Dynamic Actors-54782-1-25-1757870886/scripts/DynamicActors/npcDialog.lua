...
filters.poseShifts = {
		{ {
	{ "armsakimbo", {loops=1, priority=2, blendMask=12}, 7 },
	{ "idle2_copy", {priority=2, speed=2} },
		},
		{
	{ "armsfolded", {loops=1, priority=2, blendMask=12}, 2.5},
	{ "idle8_copy", {priority=2, speed=2} },
		} },
	...
	{ "idle2_copy", {priority=2, speed=2} },
		...
		{
	{ "armsatback", {loops=1, priority=2, blendMask=12}, 2.5 },
	{ "idle8_copy", {priority=2, speed=2} },
		} }
	}
...
if types.NPC.objectIsInstance(self) then
	...
	if anim.hasGroup(self, "idlespeak") then voice.noanim = false		end
end
...
--	playHandler(name, options)
	animQueue = rand[1]		animQueue.time = core.getSimulationTime() + (animQueue[3] or 0)
	playHandler(rand[2][1], rand[2][2])
end
...
local function closeNPCdiag()
	if anim.hasAnimation(self) then
		if voice.isPlay then anim.cancel(self, voice.idle) end
		if idleAnim.name then
--			anim.cancel(self, idleAnim.name)
...
--        end
	...
	if animQueue and animQueue.time < core.getSimulationTime() then
		playHandler(animQueue[1], animQueue[2])
		animQueue = nil
	end
	if isBeast or poseShiftType == 0 or voice.noanim then		return		end

	if voice.isPlay and not core.sound.isSayActive(self) then
		anim.cancel(self, voice.idle)
		voice.isPlay = false
	elseif voice.isPlay and anim.getActiveGroup(self, 0) ~= voice.baseAnim then
		local active = anim.getActiveGroup(self, 0)
		voice.baseAnim = active
		local newIdle = voice.groups[voice.baseAnim] or voice.groups.base
		if logging then print(voice.idle, newIdle)		end
		if newIdle == "" then
			anim.cancel(self, voice.idle)
			voice.isPlay = false
		elseif voice.idle ~= newIdle then
			voice.options.startPoint = anim.getCompletion(self, voice.idle)
			anim.cancel(self, voice.idle)
