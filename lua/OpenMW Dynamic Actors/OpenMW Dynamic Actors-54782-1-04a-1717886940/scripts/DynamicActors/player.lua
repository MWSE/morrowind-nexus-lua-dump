...
I.Settings.registerPage {
   ...
   l10n = "dynamicactors",
   name = "Dynamic Actors",
   description = "Dynamic Actors for OpenMW\nby Taitechnic\t\tv 1.0\z
\n\nDynamically adds extra idle animations to NPC\'s when in Dialogue with them, \z
and to the Player when stationery for an extended time.\z
\n\nOptions for auto or manual positioning of Player camera during dialogue, \z
and for hotkey triggered activation of custom idles or other animations on the Player."
}
...
I.Settings.registerGroup({
   ...
   settings = {
	...
	default = false,
	renderer = "checkbox",
	name = "Auto-switch to first person view during Dialogue.",
	},
	{key = "dialog_1st_zoom",
	default = false,
	renderer = "checkbox",
	name = "Auto zoom-in when in first person view during Dialogue.",
	},
	{key = "dialog_1st_zoomdist",
	default = 70,
	renderer = "number",
	name = "Auto zoom-in distance. Lower = zoom closer to target.",
	argument = { min = 40, max = 300 },
	},
	{key = "dialog_1st_zoom_offset",
	default = 0,
	renderer = "number",
	name = "Auto zoom-in focal point, horizontal offset +/- degrees.",
	argument = { min = -90, max = 90 },
	},
	{key = "actionHotkey",
	default = input.KEY.P,
	renderer = "inputKeyBox",
	name = "Choose hotkey used to activate special Actions",
	description = "Use Actions hotkey to play a Custom animation on player. \z
Use Actions key in 3rd Person mode during Dialogue, to enable/disable control of camera."
	},
        {key = "baseIdleAnim",
	name = "Base Player idle animation.",
	description ="Animation played on player when standing still.",
	default = "Arms Folded contrapose",
	renderer = "select",
	argument = {
		disabled = false,
		l10n = "LocalizationContext", 
		items = { "None", "Arms Folded contrapose", "Arms Back Clasp contrapose",
	"Arms Straight contrapose", "Hand on Hip contrapose", "Idle2" },
		},
	},
	{key = "rndIdleAnim",
	default = true,
	renderer = "checkbox",
	name = "Add extra random idle animations to the above.",
	},
	{key = "autoHelm",
	default = true,
	renderer = "checkbox",
	name = "Auto Helmet removal when weapon sheathed.",
	},
   },
})
...
local moved, idleAnim, idlenum, counter = true, 1, 0, 0
local headslot = nil
...
anims.items = { "None", "Arms Folded contrapose", "Arms Back Clasp contrapose",
	"Arms Straight contrapose", "Hand on Hip contrapose", "Idle2" }
anims.settings = {}
for i=1, #anims.items do anims.settings[anims.items[i]] = i end

anims.idle = {
	base = { {id="idle", speed=1}, {id="armsFoldPose", speed=0.5}, {id="armsBackClaspPose", speed=0.5},
{id="armsStrPose", speed=0.5}, {id="handhippose", speed=0.5}, {id="idle2", speed=0.5}
	},
	rnd = {
{ {id="armsakimbo", opt={loops=4, speed=0.5}}, {id="idle4", opt={speed=2}} },
{ {id="armsfolded", opt={loops=2, speed=0.5}}, {id="idle8", opt={loops=1, speed=2}} }
		}
	}

anims.combo = { ["armsFoldPose"] = { "handhippose", {mask=3}, "armsfolded", {mask=14, spd=0.5} },
	["armsStrPose"] = { "handhippose", {mask=3} },
	["armsOneBackPose"] = { "handhippose", {mask=3}, "armsatback", {mask=8, spd=0.25} },
	["armsBackClaspPose"] = { "handhippose", {mask=3}, "armsatback", {mask=12, spd=0.25} }
	}

anims.clear = { "handhippose", "idle2", "armsfolded", "armsakimbo", "armsatback" }

anims.fixed = require("scripts.DynamicActors.playlist")


local heights = { ["wood elf"]=0.9, ["breton"]=0.95, ["nord"]=1.06, ["orc"]=1.05, ["high elf"]=1.1 }
...
local dialogCam = {controls=false, block=false, instant=false, auto1st=false, height=100}
local offset3rd = camera.getFocalPreferredOffset()
local zoom1st = {enabled=false, dist=70, offset=0, force=false, level=0, vector=nil}
...
local function updateSettingsPlayer()
	...
	idleAnim = anims.settings[settingsplayer:get("baseIdleAnim")]
end
...
local function animHandler(a, g, o)
	...
	if a == "isplay" and not combo then return animation.isPlaying(self, g) end
	if not combo then
	...
	options.blendmask = combo[2].mask or option.blendmask
...
local function onDialogOpened(data)
	...
	if logging and npc.type == types.NPC then
		local nif = types.NPC.record(npc).model
		local i, j = string.find(nif, "\\[^\\]*$")
		print(string.sub(nif, i+1, j))
	end
...
local function cancelAnim()
	...
	animPlaying = false
	idlenum = 0
	ui.showMessage("Player movement enabled")
end
...
time.runRepeatedly(function()
	...
	counter = counter + 2
	if counter < 32 then
		if counter < 28 and not animHandler("isplay", anims.idle.base[idleAnim].id) and idleAnim > 1 then
			animHandler("play", anims.idle.base[idleAnim].id, {loops=50, priority=1, speed=anims.idle.base[idleAnim].speed})
		end
		return
	end
	counter = 0
	...
	local v = anims.idle.rnd[math.random(2)]
	for i=1, idlenum do
		local a, options = v[i].id, v[i].opt
...
local function playFixed()
	ui.showMessage("Player movement disabled")
	animPlaying = true
...
local function onKeyPress(key)
	...
	if openDialog then
		...
		if camera.getMode() == MD.Preview then
			...
			if dialogCam.controls then ui.showMessage("Camera controls enabled")
			else ui.showMessage("Camera controls disabled") end
		end
		return
	end
...
input.registerTriggerHandler("Jump", async:callback(function()
	...
	if varFixed.choose then ui.showMessage("Animation selector enabled")
	else ui.showMessage("Animation selector disabled") end
end))
...
local function autoCam(dt)
	...
	if zoom1st.force then
		local bezier = 1 - (zoom1st.level - 1) ^ 6
	...
	local turning = false
        ...
        if math.abs(deltaYaw) > math.rad(2) then
            turningToTarget, turning = true, true
        end
	local v = dt * 3.5
	...
	deltaPos = (dialogTarget.position + util.vector3(0, 0, dialogCam.height)) - headPos
...
--	local deltaPitch = - math.asin( deltaPos.z / deltaPos:length() ) - self.rotation:getPitch()
	...
	if dialogCam.instant then dialogCam.instant = false else v = dt * 1.5 end
        if turningToTarget then
            ctrls.pitchChange = util.clamp(deltaPitch, -v, v)
        end
	if turning or (not zoom1st.enabled) then return end

	local distance = deltaPos:length()
	destVec = util.vector2(lengthXY, deltaPos.z) * (distance - zoom1st.dist) / distance
	...
	if zoom1st.level < 1 then zoom1st.level = zoom1st.level + (dt * 1) end	
