...
I.Settings.registerGroup({
   ...
   settings = {
	...
	default = 50,
	renderer = "number",
	name = "Auto zoom-in speed. (10-200)",
...
Use Actions key in 3rd Person mode during Dialogue, to enable/disable control of camera."
	...
	name = "Player base idle animation (main).",
	...
	name = "Player base idle animation (upper body).",
	description = "(None) = use same base idle as main setting.",
	default = "Arms Folded",
	renderer = "select",
	argument = {
		disabled = false,
	...
	default = true,
	renderer = "checkbox",
	name = "Add extra random idle animations over Base Idle.",
	},
	{key = "autoHelm",
	default = true,
	renderer = "checkbox",
...
local dialogCam = {controls=false, block=false, instant=false, auto1st=false, height=100, pos=nil}
...
local function onDialogOpened(data)
	...
	if npc.type == types.NPC then
		file = types.NPC.record(npc).model or ""
	end
	local i, j = string.find(file, "\\[^\\]*$")
	if i then
		file = string.sub(file, i+1, j)
	end
	local height = heights.byAnim[file:lower()]
...
time.runRepeatedly(function()
	if openDialog then dialogCam.pos = dialogTarget.position end
...
return {
	...
	dynUiMessage = function(e) ui.showMessage(e) end
	},
}
