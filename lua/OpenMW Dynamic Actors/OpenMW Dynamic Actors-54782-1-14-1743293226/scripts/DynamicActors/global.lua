...
I.Settings.registerGroup({
   ...
   settings = {
	...
	description = "During unpaused dialog, wandering NPC's will be blocked from speaking or moving if close by.",
      },
      {
         key = "debuglog",
         default = false,
...
local function onDialogClosed()
	...
	if actor then actor:sendEvent("closeNPCdiag")		end
	openDialog = false
end
...
--		print(world.isWorldPaused(), settings:get("unpause_dialog"))
	...
	local block = false
	local file = (o.type.records[actor.recordId].model or ""):lower()
...
return {
	...
	--		print(data.object, "removing", data.script)
		...
		dynDialogChange = function()
			...
			and settings:get("unpause_dialog") then
				world.unpause("ui")
			end
		end,
