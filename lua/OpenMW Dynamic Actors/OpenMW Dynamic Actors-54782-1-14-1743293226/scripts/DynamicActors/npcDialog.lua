...
--	{ "class", "^ordinator", "armsatback", {loops=3, priority=2, blendMask=12}, 2 },
	...
	{ "isMale", false, "posealma3", {loops=1, priority=3}, 1 },
		}
...
local function playHandler(g, o)
	if isBeast then
		if g == "ttthinking" then g = "idle8" end
...
local function initNPCdiag(data)
	...
	interop = {self=self, player=dialogTarget, lookAt=dialogTarget}		local var = interop
