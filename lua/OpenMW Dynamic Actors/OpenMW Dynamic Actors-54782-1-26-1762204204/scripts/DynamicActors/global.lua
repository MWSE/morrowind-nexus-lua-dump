...
--]]
	...
	local groups = npcList.byAnim[file]
	...
	if settings:get("unpause_wanderai") then
		for _, v in ipairs(world.activeActors) do
			if v.type == types.NPC and v ~= o and not types.Actor.isDead(v) then
...
--	o:sendEvent("odarEvent", {event="odarEnabled", eventData=false})
	o:sendEvent("initNPCdiag", { data.player, auto, reset, idle, logging, plugin, groups=groups })
