...
local function onDialogOpened(data)
	...
	if data.pause or not data.near then		return 		end
	if (npc.position - self.position):length() > 500 then		return		end
	...
	d.radius = (d.radius or 1) * npc.scale
	...
	if not posing then
		...
		if d.auto1st and settings.global:get("unpause_dialog_opt") ~= "opt_alwayspause" then
...
local function onKeyPress(key)
	...
	if Actor.activeEffects(self):getEffect("levitate").magnitude > 0 or Actor.isSwimming(self) then
		return
	end
