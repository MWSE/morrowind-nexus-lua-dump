...
local poseTable, plugin, infoList
...
local function initNPCdiag(data)
	...
	local shift, name, options

	if plugin and plugin.baseIdle then
		shift, name, options = table.unpack(plugin.baseIdle)
	end
	if not name then
		shift, name, options = checkFilters(filters.baseIdle)
	end
	if animkna then name = nil end
	if name then
	...
	name = nil
	if plugin and plugin.greeting then
		shift, name, options = table.unpack(plugin.greeting)
	end
	if not name then
		shift, name, options = checkFilters(filters.greeting)
	end
	if name then
		poseShiftType = shift
		playHandler(name, options)
...
local function onInfoGetText(e)
	...
	if playlist then print(playlist.name) end
end
