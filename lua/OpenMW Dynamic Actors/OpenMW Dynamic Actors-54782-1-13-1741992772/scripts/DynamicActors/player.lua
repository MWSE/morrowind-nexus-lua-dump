...
--	["meshes/luce/am/am_luteplaying.nif"] = { }
	...
	byModel = {
		{id="guar.nif$", height=120, scale=0.3},
		{id="^guar_", height=120, scale=0.3},
...
common = {
	poseOpt=poseOpt, anims=anims, zoom1st=zoom1st, dialogCam=dialogCam,
	openmw = { self=self, input=input, core=core, util=util, camera=camera, ui=ui,
		interface=interface }
}

local dCam = require("scripts.DynamicActors.cameraHelper")
...
do
	...
	id = settingsplayer:get("autoHelmItemID2")
	helm.idle = id and types.Actor.inventory(self):find(id)
end
...
--		o.speed = o.speed * 0.25
	...
	if g == "armsFoldPose" then anim.playBlended(self, "armsfolded", o) end
	if g == "armsBackClaspPose" or g == "armsOneBackPose" then anim.playBlended(self, "armsatback", o) end
end
...
local function onDialogOpened(data)
	...
	local npc = data.arg
	dialogTarget, dialogCam.block = npc, true
...
time.runRepeatedly(function()
	...
	if posing then counter = 0 return end
	if idlenum == 0 then
		idlenum = 1
		counter = 26
	end
	...
	for i=1, idlenum do
		...
		anim.playBlended(self, a, options)
	end
...
local function procStanceChange(inCombat)
	...
	local store1, store2, id = settingsplayer:get("autoHelmItemID"), settingsplayer:get("autoHelmItemID2")
	if Actor.getStance(self) == ST.Nothing then
		helm.combat = h
		if h then id = h.recordId		end
		if id and store1 ~= id then settingsplayer:set("autoHelmItemID", id)	end
...
local function processCamera(dt)
	...
	if posing and not dialogTarget then
		dCam.processControls(dt)
	end
	if dialogCam.block or not dialogTarget then return end
	if camera.getMode() == MD.FirstPerson then
		dCam.autoCam(dt)
	end
	if camera.getMode() == MD.Preview and dialogCam.controls then
		dCam.processControls(dt, dialogTarget)
	end
end
