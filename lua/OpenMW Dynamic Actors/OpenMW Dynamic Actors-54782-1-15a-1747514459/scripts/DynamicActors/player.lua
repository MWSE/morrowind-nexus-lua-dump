...
local heights = {
	...
	["am_eater.nif"] = { 45, keys={"idle2"}, head=util.vector3(0, 20, 52) },
	["am_fishman.nif"] = { 45, keys={"idle2", "idle3", "idle"} },
	["am_luteplaying.nif"] = { 45, keys={"idle2", "idle"} },
	["am_reader2.nif"] = { 45, keys={"idle2", "idle3", "idle4"} },
	["am_sitting.nif"] = { 52, keys={"idle2", "idle3", "idle4"}, head=util.vector3(0, 20, 52) },
	...
	barsitter = { 52, keys={"idle2", "idle3", "idle4", "idle5"}, head=util.vector3(0, 20, 52) },
	wallean = { 120, keys={"idle9"}, head=util.vector3(0, -29, 120) },
...
--	["meshes/luce/am/am_luteplaying.nif"] = { }
	...
	byModel = {
		{id="guar.nif$", height=120, radius=140},
		{id="^guar_", height=120, radius=140},
...
local function updateSettingsPlayer(_, key)
	...
	if key == "baseIdleAnim_main" or key == "baseIdleAnim_upper" then
...
local function updateSettings()
	local pause = settings:get("unpause_dialog_opt") == "opt_alwayspause"
...
do
	local id = settingsplayer:get("autoHelmItemID")
	helm.combat = id and types.Actor.inventory(self):find(id)
	local id2 = settingsplayer:get("autoHelmItemID2")
...
local function animHandler(a, g, o)
	...
	if anims.isBeast then
		if g:lower() == "ttthinking" then g = "idle8"		end
...
local function getActorHeight(o)
	if o.type == types.NPC or o.type == types.Player then
		...
		return (o:getBoundingBox().halfSize.z) * o.scale * 2
--		return 128 * o.scale
	end
end
...
local function getActorRatios(o)
	if types.Creature.objectIsInstance(o) then
		return util.transform.scale(1, 1, 0.85)
	end
	local rec = types.NPC.records[o.recordId]
	local gender = rec.isMale and "male" or "female"
	local height = types.NPC.races.records[rec.race].height[gender] * 0.85
...
local function onDialogOpened(data)
	...
	dialogCam.playerHeight = getActorHeight(self)
	dialogCam.object = npc		dialogCam.pos = npc.position
	dialogCam.height = getActorHeight(npc) * 0.85
	dialogCam.head = nil		dialogCam.radius = 0
	dCam.enableShaders(true)
	dialogCam.aperture = dialogCam.shaders and settingsplayer:get("dialog_1st_dof_str") / 100 or 0
	dialogCam.ratio = dialogCam.shaders and settingsplayer:get("dialog_1st_ratio") or 0
	...
	if height then
		...
		if height.head then dialogCam.head = height.head			end
	end
	if types.Creature.objectIsInstance(npc) then
		...
		--		print(v.id, v.height, v.scale)
				...
				if v.head then dialogCam.head = v.head			end
			end
		end
	end
...
--			if v.scale then zoom1st.scale = v.scale			end
	...
	if dialogCam.head then
		...
		dialogCam.head = util.vector3(0, 0, dialogCam.height)
	end
--	print(dialogCam.head, dialogCam.radius)
	...
	if not posing then
		...
		if dialogCam.auto1st and settings:get("unpause_dialog_opt") ~= "opt_alwayspause" then
...
local function procStanceChange(inCombat)
	...
	local store1, store2 = settingsplayer:get("autoHelmItemID"), settingsplayer:get("autoHelmItemID2")
	local id = h and h.recordId
	if Actor.getStance(self) == ST.Nothing then
		helm.combat = h
		if id and store1 ~= id then
			settingsplayer:set("autoHelmItemID", id)
		end
	...
	elseif stance == ST.Nothing then
		...
		if id and id ~= store2 and id ~= store1 then
			settingsplayer:set("autoHelmItemID2", id)
		end
