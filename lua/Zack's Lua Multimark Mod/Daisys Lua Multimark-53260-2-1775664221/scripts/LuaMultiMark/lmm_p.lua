...
local versionCheck= require("scripts.LuaMultiMark.VersionChecker")
...
local function onUpdate(dt)
    if not versionCheck.result then
        if not versionCheck.updated then
            ui.showMessage("Daisy's Lua Multimark: Your OpenMW version is too old. Update to a dev build or 0.51, built on or after April 8 2026")
        elseif versionCheck.hasFile then
         ui.showMessage("Daisy's Lua Multimark: This version of the mod no longer requires the omwaddon to be enabled. Please disable it to continue using the mod.")
