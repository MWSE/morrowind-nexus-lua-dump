...
return {
    eventHandlers = {
        OSL_LockpickSuccess = function(data)
            if data.probe then
        ...
        OSL_PlayerLockpicking = function(data)
            local crimeInfo = {
            ...
            if I.Crimes.commitCrime(data.player, crimeInfo) then
                if types.Player.getCrimeLevel(data.player) > startingCrime then
                    data.player:sendEvent("OSL_PlayerLockpickingSeen")
                end
            end
        end,
        OSL_DrainLockpick = function(data)
            ...
            types.Item.itemData(lockpick).condition = types.Item.itemData(lockpick).condition - amount
            if types.Item.itemData(lockpick).condition <= 0 then
                lockpick:remove()
            end
        end,
        OSL_PauseWorldLockpicking = function(data)
            if data.paused and configGlobal.options.b_PauseTime then
                world.pause('OSL_PlayerLockpicking')
            else
                world.unpause('OSL_PlayerLockpicking')
            end
        end
    }
}