...
local lockpickVisualOffset = util.vector2(0, 0)
...
local attemptTime = nil
local attemptRollInterval = 0.5
local attemptRollTimer = 0
local attemptInfoBox = nil
...
local function updateTension()
    ...
    if not advancedMode then
        ...
        uiPickElement.layout.props.position = util.vector2(-shakeX * 0.5 + lockpickOffsetX, shakeY * 0.5 + lockpickOffsetY) + lockpickVisualOffset
    end
end
...
local function updateInfoBox()
    ...
    if optPick then
        ...
        content.content['InfoBoxPickConditionLine'].content['InfoBoxPickCondition'].props.textColor = attempting and util.color.rgb(1, 1, 0) or nil
    end
    if optKeybinds then
...
local function updateAttemptInfoBox()
    if attemptInfoBox then
        if not attempting then
            attemptInfoBox:destroy()
            uiWrapper.layout.content[uiWrapper.layout.content:indexOf(attemptInfoBox)] = nil
            attemptInfoBox = nil
        else
            ...
            attemptInfoBox.layout.content[1].content[1].content['durability'].props.text = l10n('AttemptInfoBoxDurability'):gsub('%%{durability}', tostring(condition) .. "/" .. activeLockpick.maxCondition)
            attemptInfoBox.layout.content[1].content[1].content['cancel'].props.text = l10n('AttemptInfoBoxCancel'):gsub('%%{cancelKey}', controllerPrompts and l10n('InfoBoxKeybindPickControllerText') or (input.getKeyName(configPlayer.keybinds.keybindPickPin) .. l10n('InfoBoxKeybindPickText')))
        end
    end
end
...
local function dropTumblers(index, badTiming)
    ...
    ambient.playSoundFile("sound/OblivionLockpicking/tumbler_crash.wav")
    for i = 1, activeTumblers do
        local shouldReset = (i == index) or (badTiming and setTumblers[i] and keepUp == 0)
        if shouldReset then
            setTumblers[i] = false
            scaleTumbler(i, 1)
...
local function stopLockpicking(success)
    ...
    if attempting then
        attempting = false
        dropTumblers(movingTumblerIndex, configGlobal.options.b_MissedRollsDropOtherPins)
        movingPickStart = util.vector2(lockpickOffsetX, lockpickOffsetY)
        movingPickTarget = util.vector2(tumblerSpacing * (movingTumblerIndex - 3) + 8, 24)
        movingPickTimer = 0
        movingTumbler = nil
        movingTumblerIndex = nil
        movingTumblerTime = nil
...
local function startLockpicking()
    ...
    attemptRollInterval = configGlobal.tweaks.n_SkillCheckAttemptInterval or 0.5
    ...
    if movingTumbler ~= nil then
        ...
        if not tensionMovingTumblerLockup then
            ...
            if skillRoll and not tooEarly and not tooLate then
                ...
                if not tooEarly and not tooLate and not attempting and configGlobal.options.s_SkillAffectsChance ~= 'SkillChecks_Hardcore' then
                    attempting = true
                    attemptTime = 0
                    attemptInfoBox = ui.create {
                        template = I.MWUI.templates.boxTransparentThick,
                        content = ui.content {
                            {
                                template = I.MWUI.templates.padding,
                    ...
                    uiWrapper.layout.content:add(attemptInfoBox)
                end

                if attempting then
                    attemptRollTimer = attemptRollInterval
...
local function onPrevPin()
    if I.UI.getMode() ~= nil then return end
    if impactPause > 0 or movingTumbler or movingPickTimer then return end
    local nearestTumbler = util.clamp(math.floor(((lockpickOffsetX + 94) + tumblerSpacing / 2) / tumblerSpacing), 0, 4)
...
local mouseMoveXAccum = 0
local mouseMoveYAccum = 0

local function updateLockpick()
    if attempting then
        lockpickVisualOffset = util.vector2(0, math.cos(attemptTime * 2 * math.pi / attemptRollInterval) * 3)
        lockpickOffsetX = tumblerSpacing * (movingTumblerIndex - 3) + 8
        lockpickOffsetY = 8
        uiPickElement.layout.props.position = util.vector2(lockpickOffsetX, lockpickOffsetY) + lockpickVisualOffset
        return
    else
        lockpickVisualOffset = util.vector2(0, 0)
    end
    ...
    if movingPickTimer == nil then
        ...
        if (mouseMoveX > 0 and mouseMoveXAccum < 0) or (mouseMoveX < 0 and mouseMoveXAccum > 0) then
            mouseMoveXAccum = 0
        end
        if mouseMoveY == 0 or (mouseMoveY > 0 and mouseMoveYAccum < 0) or (mouseMoveY < 0 and mouseMoveYAccum > 0) then
            mouseMoveYAccum = 0
        end

        if not movingTumbler then
            mouseMoveXAccum = mouseMoveXAccum + mouseMoveX
            mouseMoveYAccum = mouseMoveYAccum + mouseMoveY
        else
            mouseMoveXAccum = 0
            mouseMoveYAccum = 0
        end
        ...
        if not movingTumbler and not advancedMode then
            if configPlayer.options.b_FreePickMovement then
                ...
                if mouseMoveXAccum < -24 then
                    onPrevPin()
                    mouseMoveXAccum = 0
                    return
                elseif mouseMoveXAccum > 24 then
                    onNextPin()
                    mouseMoveXAccum = 0
                    return
                elseif mouseMoveYAccum < -8 then
                    onPick()
                    mouseMoveYAccum = 0
                    return
                elseif mouseMoveYAccum > 8 then
...
local pickMoveStartY = nil

local function updateMovingTumbler()
    if movingTumbler then
        if attempting then
            scaleTumbler(movingTumblerIndex, 0.05 + math.cos(attemptTime * 2 * math.pi / attemptRollInterval) * 0.05)
            return
        end
        ...
        local pickDropTime = util.clamp(movingPickDuration * 3, 0, fallTime)
        local timeToFall = riseTime + hangTime + fallTime - pickDropTime
        if movingTumblerTime < riseTime then
            lockpickOffsetY = lerp(movingTumblerTime / riseTime, movingTumblerLockpickStartY, 12)
            uiPickElement.layout.props.position = util.vector2(lockpickOffsetX, lockpickOffsetY) + lockpickVisualOffset
            pickMoveStartY = nil
        elseif movingTumblerTime > timeToFall then
            if not pickMoveStartY then
                pickMoveStartY = lockpickOffsetY
            end
            lockpickOffsetY = math.max(lerp((movingTumblerTime - timeToFall) / pickDropTime, pickMoveStartY, 24), lockpickOffsetY)
            uiPickElement.layout.props.position = util.vector2(lockpickOffsetX, lockpickOffsetY) + lockpickVisualOffset
        end
...
local function getNewPick()
    ...
    activeLockpick = getType().records[newPick.recordId]
...
local function handleKeyPress(key)
    ...
    elseif key.code == configPlayer.keybinds.keybindPickPin then
        if attempting then
            stopLockpicking()
        elseif not movingPickTimer then
            onPick()
        end
...
local function frame()
    ...
    if not configGlobal.options.b_EnableMod then
        overrideCombatControls = false
    else
        local hasTool, equippedR, isLockpick, isProbe = toolEquipped()
    ...
    if crimeTimer < crimeInterval then
        if not core.isWorldPaused() then
            crimeTimer = crimeTimer + frameDuration
        end
    else
        if crimeSeen then 
    ...
    if ambientScrapeTimer > 0 then
        ambientScrapeTimer = ambientScrapeTimer - frameDuration
    ...
    if impactPause > 0 then
        impactPause = impactPause - frameDuration
    ...
    if attempting then
        attemptTime = attemptTime + frameDuration
        if attemptRollTimer > 0 then
            attemptRollTimer = attemptRollTimer - frameDuration
        else
            onPick()
        end
    end
