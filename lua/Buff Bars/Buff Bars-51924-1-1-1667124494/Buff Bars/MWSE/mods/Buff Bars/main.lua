...
L.NewBB = function(id, ic, dur, sn)		local B = M.BB[id]
	...
	local N = math.remap(dur, cf.BBred, cf.BBgr, 0, 1)		bw.fillColor = {2-N*2, N*2, 0}
end
...
local function loaded(e)	p = tes3.player		mp = tes3.mobilePlayer		M.BB = {}	local MU = tes3ui.findMenu("MenuMulti")
...
M.BBM.autoWidth = true		M.BBM.autoHeight = true		M.BBM.absolutePosAlignX = cf.slx/1920		M.BBM.absolutePosAlignY = cf.sly/1080
for _, aef in pairs(mp:getActiveMagicEffects{}) do if aef.duration > 1 then L.NewBB(aef.effectId, tes3.getMagicEffect(aef.effectId).bigIcon, aef.duration, aef.serial * 10 + aef.effectIndex) end end
timer.start{duration = 1, iterations = -1, callback = function() local N	for _, aef in pairs(mp:getActiveMagicEffects{}) do		local B = M.BB[aef.effectId]	if B then local bar = B[aef.serial * 10 + aef.effectIndex]
	if bar then local bw = bar.widget	bw.current = bw.max - aef.effectInstance.timeActive		N = math.remap(bw.current, cf.BBred, cf.BBgr, 0, 1)		bw.fillColor = {2-N*2, N*2, 0} end
end end end}
