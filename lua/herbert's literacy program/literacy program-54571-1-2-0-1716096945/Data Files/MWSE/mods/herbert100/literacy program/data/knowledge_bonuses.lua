...
do -- alchemy
    local ps_bonus = new_bonus{name="Brewing Chance", 
        ...
        event_callback = function (self, e)
            ...
            local bonus = self:get_bonus()
            local brew_chance = (1 + bonus) * vanilla_brew_chance

            log:trace("applying a brew chance bonus of %s.\n\tvanillia chance = %s | new chance = %s", 
                bonus, vanilla_brew_chance, brew_chance
            )
    ...
    new_bonus{name="Potion Strength", 
        ...
        event_callback = function(self, e)
            if e.success then
                local bonus = self:get_bonus()
                log:trace("successfully brewed a potion.... applying a potion strength bonus of %s", bonus)
                e.potionStrength = e.potionStrength * (1 + bonus)
            end
        end,
        event_priority = ps_bonus.event_priority-100,
    }
end
...
do -- magic and enchanting

    
    local mc_bonus = new_bonus{name="Spell Magicka Cost", 
        calculate_bonus = function(k)
            -- \ - 0.45\cdot\left(1\ -\ 2^{-\left(0.00004\cdot\left(0.75x^{1.5}+100x\right)\right)}\right)
            return -0.75 * 0.45 * (1 - 2^(-0.00004*(0.75*k^1.5 + 100*k)))
        end, 
        skill_ids = table.values(tes3.magicSchoolSkill),
        event_id = EV.spellMagickaUse,
        event_callback = function (self, e) ---@param e spellMagickaUseEventData
            if e.caster == tes3.player then

                local skill_id = tes3.magicSchoolSkill[e.spell:getLeastProficientSchool(tes3.mobilePlayer)]
                local bonus = self:get_bonus(skill_id)
                local new_cost = e.cost * (1 + bonus)

                log:trace("applying magicka cost bonus of %s to spell %q ... \n\toriginal cost = %s | new cost = %s", 
                    bonus, e.spell.name, e.cost, new_cost
                )
                e.cost = new_cost
            end
        end,
    ...
    local cc_bonus = new_bonus{name="Spell Cast Chance", 
        calculate_bonus = function(k)
            -- 0.455\cdot\left(1-2^{-0.00005\cdot\left(0.45\cdot x^{2}+73x\right)}\right)
            return  0.75 * 0.455 * ( 1 - 2^(-0.00005 * (0.45 * k^(2) + 73 * k)) )
        end, 
        skill_ids = mc_bonus.skill_ids,
        event_id = EV.spellCast,
        event_callback = function (self, e) ---@param e spellCastEventData
            if e.caster ~= tes3.player then return end
            local spell = e.source --[[@as tes3spell]]
            if spell.objectType ~= tes3.objectType.spell then return end

            local skill_id = tes3.magicSchoolSkill[spell:getLeastProficientSchool(tes3.mobilePlayer)]
            local bonus = self:get_bonus(skill_id)
            local new_chance = e.castChance * (1 + bonus)

            log:trace("applying cast chance bonus of %s to spell %q ... \n\toriginal chance = %s | new chance = %s", 
                    bonus, spell.name, e.castChance, new_chance
                )
            e.castChance = new_chance
        end,
        event_priority=-100,
    }
    ...
    local function magic_menu_preupdate(e)
        ...
        local function fmt_number(num)
            -- dont show a decimal place if the number is <= 0, >= 100, or sufficiently close to a whole number (e.g. `8.9` should be shown as `9`)
            return (num <= 0 or num >= 100 or math.abs((num % 1) - 0.5) >= 0.35) and math.round(num)
                or math.round(num, 1)
        end
        ...
        if motte_installed then
            ...
            for i, name_blk in ipairs(name_blks) do
                ...
                if cost_bonus then
                    local new_cost = spell.magickaCost * (1 + cost_bonus)
        ...
        do -- update enchant menu
            ...
            for i, name_blk in ipairs(name_blks) do
                ...
                if chrg_bonus and chrg_bonus ~= 0 then
                    local new_cost = object.enchantment.chargeCost * (1 + chrg_bonus)
