...
local function addPoisonTooltip(e)
    ...
    local label = e.tooltip:createLabel{id="Poison_Tooltip_Name", text="Отравлено"}
...
local function setPoisonInteractive(weapon, weaponData, poison, poisonData)
    ...
    if getPoison(weapon, weaponData) then
        message = ("Нанести яд на %s?\n\nПредыдущий яд будет удален!"):format(weapon.name)
    elseif config.useApplyMessage then
        message = ("Нанести яд на %s?"):format(weapon.name)
    end

    if message then
        tes3.messageBox{message = message, buttons = {"Да", "Нет"}, callback = callback}
    else
        callback()
    end
end
    ...
    e.element:registerBefore("mouseClick", function()
        ...
        if success then
            if tes3.mobilePlayer.inCombat then
                tes3.messageBox("Вы не можете наносить яд во время боя.")
            else
