...
local function startAlchemy(poisonMode)
    ...
    if tes3.mobilePlayer.inCombat then
        tes3.messageBox("Вы не можете готовить зелья во время боя.")
        return
    end

    if not collectApparatus() then
        tes3.messageBox("Для изготовления требуется ступка и пестик.")
        return
    end
    ...
    effects.value = poisonMode and "Эффекты яда" or "Эффекты зелья"
    message.value = poisonMode and "Вам не удалось создать яд" or "Вам не удалось создать зелье"
    success.value = poisonMode and "Вы создали яд" or "Вы создали зелье"
...
local function onActivate(e)
    ...
    tes3.messageBox{
        ...
        buttons = {"Отмена", "Взять", "Изготовить зелье", "Изготовить яд"},
        callback = function(b)
            if b.button == 1 then
                timer.delayOneFrame(function()
...
local function onEquip(e)
    ...
    tes3.messageBox{
        message = "Что вы хотите сделать?",
        buttons = {"Отмена", "Изготовить зелье", "Изготовить яд"},
