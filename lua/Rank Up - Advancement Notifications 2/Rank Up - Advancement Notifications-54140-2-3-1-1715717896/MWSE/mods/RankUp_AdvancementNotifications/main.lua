...
function CheckRequirements(faction)
    ...
    if playerNextRank > #faction.ranks then
        return mwse.log("[Rank Up!] Player is already the highest rank in this faction.")
    end
...
-- Table and check-variable for Menus that the notifications shouldn't be activated in (for convenience reasons)
local excludedMenus = {"MenuOptions","MenuConsole","MenuStatReview","MenuCreateClass","MenuSave","MenuSetValues","MenuSpellmaking","MenuAlchemy","MenuMapNoteEdit","MenuContents"}
local excludedMenuIsOpen = false
...
local function keyPressed(e)
    ...
    -- If there are no notifications displayed already, begin with the checks and displaying:
        ...
        -- Search, if any of the excluded menus are open
        for _, v in pairs(excludedMenus) do
            if tes3ui.findMenu(v) then 
                excludedMenuIsOpen = true
                break
            end
        end
        -- Extra "if" because the dialogue menu is included in "MenuOptions", but I think it's cool to be able to view them in dialogues
        if tes3ui.findMenu("MenuDialog") then
            excludedMenuIsOpen = false
        end

        -- Show the notifications, if none of the exlcuded menus are open.
        if excludedMenuIsOpen == false then
