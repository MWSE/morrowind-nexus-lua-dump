...
local function loaded()
	...
	if not obj then
		mwse.log('%s loaded: error "%s" not found, mod disabled',
			modPrefix, sc_paper_plain_id)
		return
	end
	blanks[sc_paper_plain_id] = 100 -- store enchant change if > 0

	if tes3.getObject(t_sc_blank_id) then
		blanks[t_sc_blank_id] = -1 -- do not change enchant value
	end

-- begin tweakables
-- note: changes to obj cannot be done in initialized event,
-- must wait for loaded after initialized (could be 2nd loaded if a new game is started)
