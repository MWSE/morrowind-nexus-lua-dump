...
--- NC: Guarded against cases where a cell didn't exist.
--- @param cellId string
--- @return tes3region?
function common.getRegion(cellId)
	local cell = tes3.getCell({ id = cellId })
	return cell and cell.region or tes3.getRegion{useDoors = true}
end
...
common.isExport = function(item)
	...
	if common.config[cell] then
		...
		local region = common.getRegion(cell)
		if (region == nil) then
			return false
		end
