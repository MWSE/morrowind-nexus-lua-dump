...
-- self.controls.sneak refleja el estado REAL del motor (agachado o no).
-- Guardamos el valor del frame anterior para detectar el flanco false→true
-- (el momento exacto en que el jugador se agacha).
-- No necesita serializarse: se inicializa leyendo el estado real en el
-- primer onFrame tras cargar.
local prevSneak = false
...
-- -----------------------------------------------------------------------------
-- Helper: ¿está el actor parado en agua?
-- -----------------------------------------------------------------------------
...
local function onFrame(dt)
   -- Leer el estado real de sneak directamente del motor
   local sneakNow = self.controls.sneak

   -- Detectar flanco false→true: el jugador ACABA de agacharse
   if sneakNow and not prevSneak then
      if state.thirst:isEnabled() and isStandingInWater(self) then
...
local function onLoad(data)
   ...
   -- Sincronizar prevSneak con el estado real al cargar para no disparar
   -- accidentalmente en el primer frame si el jugador cargó agachado.
   prevSneak = self.controls.sneak
   HUD.applySettings(s)
   ui.updateAll()
end
