...
function Need.create(key, value, widget, wellEffect)
   local need = setmetatable({}, Need)
   need._enabled = true
   need._value = value
   need._maxValue = 1000
   need._wellEffect = wellEffect or nil
   need._widget = widget
   need._effects = {
...
function Need.status(need)
   return math.min(6, 1 + math.floor(need._value / 200))
end
...
function Need._updateEffects(need, prevStatus)
   ...
   -- Manejar efecto positivo (well_hydrated / well_satisfied)
   if need._wellEffect then
      if status == STATE.None then
         Actor.spells(self):add(need._wellEffect)
      else
         Actor.spells(self):remove(need._wellEffect)
      end
   end
