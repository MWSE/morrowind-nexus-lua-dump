...
local function onPickpocketDoSteal(data)
...
local function onPickpocketShowMessage(data)
    if data and data.message and data.player and data.player:isValid() then
        data.player:sendEvent("PickpocketMessage", { message = data.message })
    end
end

local function onPickpocketReturn(data)
    if not data or not data.player or not data.player:isValid() then return end
    if not data.npc or not data.npc:isValid() then return end
    local choice = data.choice
...
local function onTPRequestRemoval(actor)
    if not actor or not actor:isValid() then return end
    if actor:hasScript("scripts/tpickpocket_local.lua") then
        actor:removeScript("scripts/tpickpocket_local.lua")
    end
end
...
return {
    ...
    eventHandlers = {
        ...
        PickpocketDoSteal     = onPickpocketDoSteal,
        PickpocketShowMessage = onPickpocketShowMessage,
        PickpocketReturn      = onPickpocketReturn,
        TP_RequestRemoval     = onTPRequestRemoval,
    },
}