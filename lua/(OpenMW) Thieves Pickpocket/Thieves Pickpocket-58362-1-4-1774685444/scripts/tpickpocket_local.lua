...
local function isKhajiit()
    local race = types.NPC.record(self.object).race
    return race and race:lower() == "khajiit"
end

local function getRetaliationLine()
    local lines = isKhajiit() and RETALIATION_LINES_KHAJIIT or RETALIATION_LINES
    return lines[math.random(#lines)]
end

local function getApologyLine()
    local lines = isKhajiit() and APOLOGY_LINES_KHAJIIT or APOLOGY_LINES
    return lines[math.random(#lines)]
end

local function clearRetaliation()
            ...
            if retaliationPhase then
                ...
                if retaliationScanAcc >= 1 then
                    ...
                    if not retaliationPlayer or not retaliationPlayer:isValid()
                       or types.Actor.isDead(self) then
                        clearRetaliation()
                    ...
                    elseif retaliationPhase == "standoff" then
                        ...
                        if stance ~= 1 and stance ~= 2 then
                            -- Player sheathed: thief calms down
                            clearRetaliation()
                        else
                            retaliationTimer = retaliationTimer - 1
                            if retaliationTimer < 0 then
                                -- Standoff expired, player didn't sheathe: attack
                                AI.startPackage({ type = "Combat", target = retaliationPlayer })
                                retaliationPhase  = nil
                                retaliationTimer  = 0
                                retaliationPlayer = nil
                                retaliationRadius = nil
