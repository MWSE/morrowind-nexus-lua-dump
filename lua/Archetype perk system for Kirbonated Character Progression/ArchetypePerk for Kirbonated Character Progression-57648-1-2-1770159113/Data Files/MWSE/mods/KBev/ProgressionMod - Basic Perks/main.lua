...
local function wayspellsloaded()
	if not tes3.player.data.basicPerks then
		...
		saveData.warrogueSpellBonus = 10
		saveData.doOncewarrogueWay = 0
		saveData.mist = 0
		saveData.cantUp = 0
		saveData.canStep = 0
		saveData.xDamage = 0
		saveData.NoManaBase = 0
		saveData.PoisonBase = 0
		saveData.doOnceBows = 0
	else
		saveData = tes3.player.data.basicPerks
	end
	...
	if not NoManaSpell then
		NoManaSpell = tes3.createObject({id = "kb_perk_NoManaSpell", name = "Истощение маны", objectType = tes3.objectType.spell, castType = tes3.spellType.spell, effects = {{id = tes3.effect.damageMagicka, rangeType = tes3.effectRange.target, min = saveData.NoManaBase, max = saveData.NoManaBase, duration = 1}}})
	end
	if not PoisonSpell then
		PoisonSpell = tes3.createObject({id = "kb_perk_PoisonSpell", name = "Отравление", objectType = tes3.objectType.spell, castType = tes3.spellType.spell, effects = {{id = tes3.effect.poison, rangeType = tes3.effectRange.target, min = saveData.PoisonBase, max = saveData.PoisonBase, duration = 3}}})
	end
	if not ElixirPoison then
		ElixirPoison = tes3.createObject({ id = "samson_ElixirPoison", objectType = tes3.objectType.alchemy, name = "Эликсир отравляющих ударов", mesh = "AATL\\m\\m_pot00.nif", icon = "AATL\\m\\tx_m_pot00.dds", value = 5000, weight = 1, effects = {{ id = tes3.effect.poison, rangeType = tes3.effectRange.self, min = 10, max = 10, duration = 1 }}})
	end
	if not PoisonHit then
		PoisonHit = tes3.createObject({id = "kb_PoisonHit", name = "Отравляющие удары", objectType = tes3.objectType.spell, castType = tes3.spellType.ability, effects = {{id = tes3.effect.poison, rangeType = tes3.effectRange.self, min = 0, max = 0, duration = 3600}}})
	end
	if not ElixirBlind then
		ElixirBlind = tes3.createObject({ id = "samson_ElixirBlind", objectType = tes3.objectType.alchemy, name = "Эликсир ослепляющих ударов", mesh = "AATL\\m\\m_pot03.nif", icon = "AATL\\m\\tx_m_pot03.dds", value = 5000, weight = 1, effects = {{ id = tes3.effect.blind, rangeType = tes3.effectRange.self, min = 100, max = 100, duration = 1 }}})
	end
	if not BlindHit then
		BlindHit = tes3.createObject({id = "kb_BlindHit", name = "Ослепляющие удары", objectType = tes3.objectType.spell, castType = tes3.spellType.ability, effects = {{id = tes3.effect.blind, rangeType = tes3.effectRange.self, min = 0, max = 0, duration = 3600}}})
	end
	if not ElixirNoMana then
		ElixirNoMana = tes3.createObject({ id = "samson_ElixirNoMana", objectType = tes3.objectType.alchemy, name = "Эликсир истощения маны", mesh = "AATL\\m\\m_pot02.nif", icon = "AATL\\m\\tx_m_pot02.dds", value = 5000, weight = 1, effects = {{ id = tes3.effect.drainMagicka, rangeType = tes3.effectRange.self, min = 25, max = 25, duration = 1 }}})
	end
	if not NoMana then
		NoMana = tes3.createObject({id = "kb_NoMana", name = "Истощение маны", objectType = tes3.objectType.spell, castType = tes3.spellType.ability, effects = {{id = tes3.effect.drainMagicka, rangeType = tes3.effectRange.self, min = 0, max = 0, duration = 3600}}})
	end
	...
	if not axeTwoUnique then
		axeTwoUnique = tes3.createObject({ id = "samson_axeTwoUnique", objectType = tes3.objectType.weapon, type = tes3.weaponType.axeTwoHand, name = "Секира Скорби", mesh = "AATL\\w\\W_Rueful_Axe.nif", icon = "AATL\\w\\tx_w_Rueful_Axe.dds", value = 75000, weight = 70, chopMin = 10, chopMax = 90, slashMin = 10, slashMax = 70, thrustMin = 10, thrustMax = 20, enchantCapacity = 3000, ignoresNormalWeaponResistance = true, maxCondition = 7500, reach = 1.00, speed = 1.25 })
	end
	...
	if not shortBladeUnique then
		shortBladeUnique = tes3.createObject({ id = "samson_shortBladeUnique", objectType = tes3.objectType.weapon, type = tes3.weaponType.shortBladeOneHand, name = "Кинжал Четырех Столпов", mesh = "AATL\\w\\W_DragBoneDag.nif", icon = "AATL\\w\\tx_w_dragbonedag.dds", value = 15000, weight = 5, chopMin = 10, chopMax = 15, slashMin = 10, slashMax = 15, thrustMin = 10, thrustMax = 15, enchantCapacity = 1000, ignoresNormalWeaponResistance = true, maxCondition = 1500, reach = 1.00, speed = 2.5 })
	end
	if not shortBladeUniqueII then
		shortBladeUniqueII = tes3.createObject({ id = "samson_shortBladeUniqueII", objectType = tes3.objectType.weapon, type = tes3.weaponType.shortBladeOneHand, name = "Клинок Славы Альмсиви", mesh = "AATL\\w\\w_waverider.nif", icon = "AATL\\w\\TX_W_Waverider.dds", value = 30000, weight = 20, chopMin = 15, chopMax = 35, slashMin = 15, slashMax = 35, thrustMin = 15, thrustMax = 35, enchantCapacity = 2000, ignoresNormalWeaponResistance = true, maxCondition = 2000, reach = 1.00, speed = 2 })
	end
	if not warrogueSpell then
		warrogueSpell = tes3.createObject({id = "kb_warrogueSpell", name = "Врожденное уклонение", objectType = tes3.objectType.spell, castType = tes3.spellType.ability, effects = {{id = tes3.effect.sanctuary, rangeType = tes3.effectRange.self, min = saveData.warrogueSpellBonus, max = saveData.warrogueSpellBonus, duration = 0}}})
	end
	if not warrogueTwoSpell then
		warrogueTwoSpell = tes3.createObject({id = "kb_warrogueTwoSpell", name = "Уход в туман", objectType = tes3.objectType.spell, castType = tes3.spellType.spell, effects = {{id = tes3.effect.chameleon, rangeType = tes3.effectRange.self, min = 100, max = 100, duration = 5}}})
	end
	if not warrogueStepSpell then
		warrogueStepSpell = tes3.createObject({id = "kb_warrogueStepSpell", name = "Шаг сквозь тень", objectType = tes3.objectType.spell, castType = tes3.spellType.spell, effects = {{id = tes3.effect.chameleon, rangeType = tes3.effectRange.self, min = 0, max = 0, duration = 1}}})
	end
end
event.register(tes3.event.loaded, wayspellsloaded)

local function bookGetTextCallback(e)
	if e.book == ElixirBook and perks.warmage.activated then
		...
		<BR>Цена - 5000 дрейков.<BR>Вес - 1 кг.<BR>
	...
	elseif e.book == ElixirBook and perks.warrogue.activated then
		e.text = [[<DIV ALIGN="CENTER"><FONT COLOR="000000" SIZE="3" FACE="Magic Cards">
		<BR>Книга Эликсиров<BR>
		<BR>В этой книге собраны рецепты уникальных Эликсиров, которые может сварить искусный алхимик:<BR>
		<DIV ALIGN="LEFT">
		<BR>Эликсир ослепляющих ударов - Уникальный Эликсир, чей эффект при нанесении урона может ослепить врага на 50 процентов. Вероятность - 10 процентов, время действия - 60 секунд.<BR>
		<BR>Цена - 5000 дрейков.<BR>Вес - 1 кг.<BR>
		<BR>Требуемые ингредиенты:<BR>Мясо дурзога - 2.<BR>Шкура щетиноспина - 2.<BR>Костяная мука - 5.<BR>Глаз грахла - 1.<BR>Железа с нектаром велка - 2.<BR>
		<BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR>Эликсир истощения маны - Уникальный Эликсир, чей эффект при нанесении урона может отнять 25 процентов маны врага. Вероятность - 25 процентов.<BR>
		<BR>Цена - 5000 дрейков.<BR>Вес - 1 кг.<BR>
		<BR>Требуемые ингредиенты:<BR>Мясо дурзога - 2.<BR>Жир тролля - 2.<BR>Виквит - 5.<BR>Кожа скампа - 3.<BR>Созревшие ягоды белладонны - 3.<BR>
		<BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR>Эликсир отравляющих ударов - Уникальный Эликсир, чей эффект при нанесении урона может отравить врага, отнимая 15% здоровья за 3 секунды. Вероятность - 15 процентов.<BR>
		<BR>Цена - 5000 дрейков.<BR>Вес - 1 кг.<BR>
		<BR>Требуемые ингредиенты:<BR>Лиловый копринус - 5.<BR>Светящаяся сыроежка - 5.<BR>Адамантиновая руда - 2.<BR>Цветы осоки благородной - 3.<BR>Шипы ллорамора - 2.<BR>]]
	elseif e.book == armorerBook and perks.longblade.activated then
		e.text = [[<DIV ALIGN="CENTER"><FONT COLOR="000000" SIZE="3" FACE="Magic Cards">
		<BR>Книга Кузнеца<BR>
	...
	elseif e.book == armorerBook and perks.axe.activated then
		e.text = [[<DIV ALIGN="CENTER"><FONT COLOR="000000" SIZE="3" FACE="Magic Cards">
		<BR>Книга Кузнеца<BR>
		<BR>В этой книге собраны чертежи и требуемые материалы для ковки уникальных доспехов и оружия:<BR>
		<DIV ALIGN="LEFT">
		<BR>Топор Рангарда - Уникальный топор:<BR>
		...
		<BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR>Секира Скорби - Уникальная секира:<BR>
	...
	elseif e.book == armorerBook and perks.bluntweapon.activated then
		e.text = [[<DIV ALIGN="CENTER"><FONT COLOR="000000" SIZE="3" FACE="Magic Cards">
		<BR>Книга Кузнеца<BR>
		<BR>В этой книге собраны чертежи и требуемые материалы для ковки уникальных доспехов и оружия:<BR>
		<DIV ALIGN="LEFT">
		<BR>Благословенный Молот - Уникальный молот:<BR>
	...
	elseif e.book == armorerBook and perks.shortBlade.activated then
		e.text = [[<DIV ALIGN="CENTER"><FONT COLOR="000000" SIZE="3" FACE="Magic Cards">
		<BR>Книга Кузнеца<BR>
		<BR>В этой книге собраны чертежи и требуемые материалы для ковки уникальных доспехов и оружия:<BR>
		<DIV ALIGN="LEFT">
		<BR>Кинжал Четырех Столпов - Уникальный кинжал:<BR>
		<BR>Цена - 15000 дрейков.<BR>Вес - 5 кг.<BR>Рубящий урон 10-15.<BR>Режущий урон 10-15.<BR>Колющий урон 10-15.<BR>Максимум зачарования - 100.<BR>Максимальная прочность - 1500.<BR>Дальность - 1.<BR>Скорость - 2.5.<BR>
		<BR>Требуемые материалы:<BR>Кобальтовая руда - 10 штук.<BR>Железная руда - 10 штук.<BR>Сердце даэдры - 3 штуки.<BR>Кожа даэдра - 3 штуки.<BR>Обрезки металла - 10 штук.<BR>Молот грандмастера-кузнеца  - 1 штука.<BR>Клещи - 1 штука.<BR>
		<BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR><BR>Клинок Славы Альмсиви - Уникальный клинок:<BR>
		<BR>Цена - 30000 дрейков.<BR>Вес - 20 кг.<BR>Рубящий урон 15-35.<BR>Режущий урон 15-35.<BR>Колющий урон 15-35.<BR>Максимум зачарования - 200.<BR>Максимальная прочность - 2000.<BR>Дальность - 1.<BR>Скорость - 2.<BR>
		<BR>Требуемые материалы:<BR>Стекло-сырец - 10 штук.<BR>Орихалковая руда - 5 штук.<BR>Сталгрим-сырец - 5 штук.<BR>Адамантиновая руда - 5 штук.<BR>Сердце даэдры - 3 штуки.<BR>Молот грандмастера-кузнеца  - 1 штука.<BR>Клещи - 1 штука.<BR>]]
	end
end
...
local function perksAfterlevelup()
	...
	if (perks.longblade.activated or perks.axe.activated or perks.bluntweapon.activated or perks.shortBlade.activated) and saveData.doOnceWeapon == 0 then
	...
	if (perks.bow.activated or perks.throw.activated) and saveData.doOnceBows == 0 then
		if tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon} then
			tes3.mobilePlayer:unequip({type = tes3.objectType.weapon})
		end
		saveData.doOnceBows = 1
	end
	...
	if perks.warrogueTree.activated and saveData.doOncewarrogueWay == 1 then
		tes3.removeSpell{reference = tes3.player, spell = warrogueSpell}
		saveData.warrogueSpellBonus = 20
		warrogueSpell.effects[1].min = saveData.warrogueSpellBonus
		warrogueSpell.effects[1].max = saveData.warrogueSpellBonus
		tes3.addSpell{reference = tes3.player, spell = warrogueSpell}
		saveData.doOncewarrogueWay = 2
	elseif perks.warrogue.activated and saveData.doOncewarrogueWay == 0 then
		tes3.addSpell{reference = tes3.player, spell = warrogueSpell}
		saveData.doOncewarrogueWay = 1
	end
end
...
local function armorAbilityOnY(e)
	...
	elseif (perks.heavyarmornighteye.activated or perks.nightVision.activated) and saveData.fullSet == true and saveData.onYbutton == 0 and not tes3.hasSpell{reference = tes3.player, spell = heavyarmornighteyespell} then
		tes3.addSpell{reference = tes3.player, spell = heavyarmornighteyespell}
		tes3.playSound({ reference = tes3.player, sound = "illusion cast" })
		saveData.onYbutton = 1
...
local function armorAbilityOnU(e)
	...
	elseif perks.heavyarmorDefence.activated and saveData.fullSet == true and saveData.onUbutton == 0 then
		...
		saveData.onUbutton = 1
	elseif perks.heavyarmorDefence.activated and saveData.onUbutton == 1 then
		tes3.messageBox("Вы вышли из Глухой обороны!")
		saveData.onUbutton = 0
	elseif perks.ropeArrow.activated and jampToArrow ~= nil then
		jampToArrow = nil
		tes3.messageBox("Вы отсоединились от кошки!")
	end
end
event.register(tes3.event.keyDown, armorAbilityOnU, { filter = tes3.scanCode.u })

local noSwitch = false
local myWeaponID = nil
local function keySwitchToHand(e)
	if perks.mageKiller.activated and noSwitch == false then
		local myWeapon = tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon}
		if myWeapon and myWeapon.object then
			myWeaponID = myWeapon.object.id
			tes3.mobilePlayer:unequip({type = tes3.objectType.weapon})
			noSwitch = true
		end
	elseif perks.mageKiller.activated and noSwitch == true then
		local count = tes3.getItemCount({reference = tes3.player, item = myWeaponID})
		if count >= 1 then
			tes3.mobilePlayer:equip({item = myWeaponID})
		end
		noSwitch = false
	end
end
event.register(tes3.event.keyDown, keySwitchToHand, { filter = tes3.scanCode.F2 })

local function blockwrongEquipp(e)
	if e.reference ~= tes3.player then
		return
	end
	if (tes3.hasSpell{reference = tes3.player, spell = Manaregen} or tes3.hasSpell{reference = tes3.player, spell = Manashield} or tes3.hasSpell{reference = tes3.player, spell = Bloodymant} or tes3.hasSpell{reference = tes3.player, spell = BlindHit} or tes3.hasSpell{reference = tes3.player, spell = NoMana} or tes3.hasSpell{reference = tes3.player, spell = PoisonHit}) and (e.item.id == "samson_ElixirManaregen" or e.item.id == "samson_ElixirManashield" or e.item.id == "samson_ElixirBloodymant" or e.item.id == "samson_ElixirBlind" or e.item.id == "samson_ElixirNoMana" or e.item.id == "samson_ElixirPoison") then
		tes3.messageBox("Одновременно вы можете находиться под эффектом только одного Эликсира!")
		e.block = true
	elseif e.item.id == "samson_ElixirBlind" then
		tes3.addSpell{reference = tes3.player, spell = BlindHit}
	elseif e.item.id == "samson_ElixirNoMana" then
		tes3.addSpell{reference = tes3.player, spell = NoMana}
	elseif e.item.id == "samson_ElixirPoison" then
		tes3.addSpell{reference = tes3.player, spell = PoisonHit}
	...
	if perks.warmage.activated and perks.bluntweapon.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.bluntOneHand and e.item.type ~= tes3.weaponType.bluntTwoClose) then
		tes3.messageBox("Вам доступны лишь молоты!")
		e.block = true
	elseif perks.warrogue.activated and perks.bow.activated and perks.longblade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.longBladeOneHand and e.item.type ~= tes3.weaponType.marksmanBow) then
		tes3.messageBox("Вам доступны лишь одноручные длинные клинки и луки!")
		e.block = true
	elseif perks.warrogue.activated and perks.bow.activated and perks.axe.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.axeOneHand and e.item.type ~= tes3.weaponType.marksmanBow) then
		tes3.messageBox("Вам доступны лишь одноручные топоры и луки!")
		e.block = true
	elseif perks.warrogue.activated and perks.bow.activated and perks.shortBlade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.shortBladeOneHand and e.item.type ~= tes3.weaponType.marksmanBow) then
		tes3.messageBox("Вам доступны лишь короткие клинки и луки!")
		e.block = true
	elseif perks.warrogue.activated and perks.throw.activated and perks.shortBlade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.shortBladeOneHand and e.item.type ~= tes3.weaponType.marksmanThrown) then
		tes3.messageBox("Вам доступны лишь короткие клинки и метательное оружие!")
		e.block = true
	elseif perks.warrogue.activated and perks.throw.activated and perks.axe.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.axeOneHand and e.item.type ~= tes3.weaponType.marksmanThrown) then
		tes3.messageBox("Вам доступны лишь одноручные топоры и метательное оружие!")
		e.block = true
	elseif perks.warrogue.activated and perks.throw.activated and perks.longblade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.longBladeOneHand and e.item.type ~= tes3.weaponType.marksmanThrown) then
		tes3.messageBox("Вам доступны лишь одноручные длинные клинки и метательное оружие!")
		e.block = true
	elseif perks.warrogue.activated and perks.bow.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.marksmanBow and e.item.type ~= tes3.weaponType.longBladeOneHand and e.item.type ~= tes3.weaponType.axeOneHand and e.item.type ~= tes3.weaponType.shortBladeOneHand) then
		tes3.messageBox("Из оружия дальнего боя вам доступны лишь луки. Из ближнего только одноручное!")
		e.block = true
	elseif perks.warrogue.activated and perks.throw.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.marksmanThrown and e.item.type ~= tes3.weaponType.longBladeOneHand and e.item.type ~= tes3.weaponType.axeOneHand and e.item.type ~= tes3.weaponType.shortBladeOneHand) then
		tes3.messageBox("Из оружия дальнего боя вам доступно лишь метательное. Из ближнего только одноручное!")
		e.block = true
	elseif perks.warrogue.activated and perks.longblade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.longBladeOneHand and e.item.type ~= tes3.weaponType.marksmanThrown and e.item.type ~= tes3.weaponType.marksmanBow) then
		tes3.messageBox("Из оружия ближнего боя вам доступны лишь одноручные длинные клинки!")
		e.block = true
	elseif perks.warmage.activated and perks.longblade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.longBladeOneHand and e.item.type ~= tes3.weaponType.longBladeTwoClose) then
		tes3.messageBox("Вам доступны лишь мечи!")
		e.block = true
	elseif perks.warrogue.activated and perks.axe.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.axeOneHand and e.item.type ~= tes3.weaponType.marksmanThrown and e.item.type ~= tes3.weaponType.marksmanBow) then
		tes3.messageBox("Из оружия ближнего боя вам доступны лишь одноручные топоры!")
		e.block = true
	elseif perks.warrogue.activated and perks.shortBlade.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.shortBladeOneHand and e.item.type ~= tes3.weaponType.marksmanThrown and e.item.type ~= tes3.weaponType.marksmanBow) then
		tes3.messageBox("Из оружия ближнего боя вам доступны лишь короткие клинки!")
		e.block = true
	elseif perks.warmage.activated and perks.axe.activated and e.item.objectType == tes3.objectType.weapon and (e.item.type ~= tes3.weaponType.axeOneHand and e.item.type ~= tes3.weaponType.axeTwoHand) then
	...
	if perks.warmage.activated and (e.item.objectType == tes3.objectType.lockpick or e.item.objectType == tes3.objectType.probe) then
		...
		e.block = true
	elseif perks.warrogue.activated and e.item.objectType == tes3.objectType.armor and e.item.slot == tes3.armorSlot.shield then
		tes3.messageBox("Архетип Воин-Разбойник не позволяет вам носить щиты!")
		e.block = true
	end
end
...
local function damageHandToHandCallback(e)
	if perks.mageKiller.activated and e.attackerReference == tes3.player and e.mobile and e.mobile.actionData.animationAttackState == tes3.animationState.casting and tes3.mobilePlayer.inCombat then
		e.mobile:hitStun({ knockDown = true })
		tes3.messageBox("Вы сбили каст противника!")
	end
end
event.register(tes3.event.damageHandToHand, damageHandToHandCallback)

local countThrown = 1
local thrownTimer = nil
local mouseTime = false
local AOETimer = 0
...
local function damageModification(e)
	...
	if (perks.nonShield.activated or perks.warrogue.activated) and e.mobile == tes3.mobilePlayer then
		if not tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.armor, slot = tes3.armorSlot.shield} then
			if perks.noArrowSword.activated and e.mobile == tes3.mobilePlayer and e.projectile then
				...
				if (tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.longBladeOneHand} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.axeOneHand} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.shortBladeOneHand}) and e.mobile.weaponReady and (angle > -45 and angle < 45) then
			...
			if perks.nonShield.activated and randomAviod > 80 and not e.projectile then
				e.block = true
				timer.start({
					duration = 0.03,
					callback = function()
	...
	if tes3.hasSpell{reference = tes3.player, spell = BlindHit} and not saveData.blindActors[e.reference.id] then
	...
	if tes3.hasSpell{reference = tes3.player, spell = NoMana} and e.reference.mobile.magicka.current > 0 then
		if math.random(1, 100) >= 75 then
			saveData.NoManaBase = e.reference.mobile.magicka.base * 0.25
			NoManaSpell.effects[1].min = saveData.NoManaBase
			NoManaSpell.effects[1].max = saveData.NoManaBase
			tes3.applyMagicSource({ reference = tes3.player, target = e.reference, source = "kb_perk_NoManaSpell" })
		end
	elseif tes3.hasSpell{reference = tes3.player, spell = PoisonHit} then
		if math.random(1, 100) >= 85 then
			saveData.PoisonBase = e.reference.mobile.health.base * 0.05
			PoisonSpell.effects[1].min = saveData.PoisonBase
			PoisonSpell.effects[1].max = saveData.PoisonBase
			tes3.applyMagicSource({ reference = tes3.player, target = e.reference, source = "kb_perk_PoisonSpell" })
		end
	end
	if perks.warrogueStep.activated and saveData.xDamage == 1 then
		if perks.deadlyHit.activated and not e.projectile then
			local randomdeadlyHit = math.random(1, 100)
			if (perks.deadlyHitTwo.activated and randomdeadlyHit > 85) or (perks.deadlyHit.activated and randomdeadlyHit > 90) then
				tes3.modStatistic({reference = e.reference, name = "health", current = -e.reference.mobile.health.current})
				timer.start({
					duration = 0.03,
					callback = function()
						tes3.messageBox("Вы нанесли противнику смертельный удар!")
					end
				})
				saveData.xDamage = 0
				return
		...
		if perks.axeBleed.activated and not e.reference.mobile.isDead and not bleedingActors[e.reference.id] and not e.projectile then
			local randomAxeBlood = math.random(1, 100)
			if (perks.axeBleedTwo.activated and randomAxeBlood > 50) or (perks.axeBleed.activated and randomAxeBlood > 65) then
				...
				timer.start({
					...
					callback = function()
						...
						elseif not e.reference.mobile.isDead then
							...
							tes3.modStatistic({ reference = e.reference, name = "health", current = -e.reference.mobile.health.base * 0.03 })
							if tickCount >= 5 then
								bleedingActors[e.reference.id] = nil
		...
		if perks.warrogueStepTwo.activated then
			e.damage = e.damage * 2
			tes3.messageBox("Урон x2")
		else
			e.damage = e.damage * 1.5
			tes3.messageBox("Урон x1.5")
		end
		if perks.staminaHitTwo.activated and not e.projectile then
			tes3.modStatistic({ reference = e.reference, name = "fatigue", current = -e.damage * 3})
		elseif perks.staminaHit.activated and not e.projectile then
			tes3.modStatistic({ reference = e.reference, name = "fatigue", current = -e.damage * 2})
		end
		saveData.xDamage = 0
		return
	end
	if perks.warrogueTwo.activated and saveData.cantUp == 1 then
		e.damage = e.damage * 3
		tes3.messageBox("Урон x3")
		tes3.removeEffects({reference = tes3.player, effect = tes3.effect.chameleon})
		saveData.cantUp = 0
		tes3.mobilePlayer.isSneaking = false
		if perks.warrogueAvoidTwo.activated and saveData.fullSet == true then
			tes3.applyMagicSource({reference = tes3.player, name = "Ускользание", effects = {{id = tes3.effect.sanctuary, rangeType = tes3.effectRange.self, min = 15, max = 15, duration = 3}}})
		elseif perks.warrogueAvoid.activated and saveData.fullSet == true then
			tes3.applyMagicSource({reference = tes3.player, name = "Ускользание", effects = {{id = tes3.effect.sanctuary, rangeType = tes3.effectRange.self, min = 10, max = 10, duration = 3}}})
		end
		if perks.warrogueFasterTwo.activated and saveData.fullSet == true then
			tes3.applyMagicSource({reference = tes3.player, name = "Ускорение", effects = {{id = tes3.effect.fortifyAttribute, attribute = tes3.attribute.speed, rangeType = tes3.effectRange.self, duration = 3, min = 150, max = 150}}})
			tes3.applyMagicSource({reference = tes3.player, name = "Ускорение", effects = {{id = tes3.effect.fortifySkill, skill = tes3.skill.acrobatics, rangeType = tes3.effectRange.self, duration = 3, min = 100, max = 100}}})
		elseif perks.warrogueFaster.activated and saveData.fullSet == true then
			tes3.applyMagicSource({reference = tes3.player, name = "Ускорение", effects = {{id = tes3.effect.fortifyAttribute, attribute = tes3.attribute.speed, rangeType = tes3.effectRange.self, duration = 3, min = 150, max = 150}}})
		end
		return
	end
	...
	if (not e.projectile and (perks.bluntweapon.activated or perks.longblade.activated or perks.axe.activated or perks.shortBlade.activated)) or (e.projectile and (perks.bow.activated or perks.throw.activated)) and tes3.mobilePlayer.weaponReady == true then
		...
		elseif (not e.projectile and (perks.bluntweaponTwo.activated or perks.longbladeTwo.activated or perks.axeTwo.activated or perks.shortBladeTwo.activated)) or (e.projectile and (perks.bowTwo.activated or perks.throwTwo.activated)) then
			e.damage = e.damage * 1.15
		else
			e.damage = e.damage * 1.1
		end
		if perks.AOESword and perks.AOESword.activated and (tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.longBladeOneHand} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.axeOneHand}) and not e.projectile and tes3.mobilePlayer.actionData.physicalAttackType == tes3.physicalAttackType.slash and AOETimer == 0 then
			local AOE = 0
			for _, hostileEnemy in ipairs(tes3.mobilePlayer.hostileActors) do
				if hostileEnemy ~= tes3.mobilePlayer.actionData.target and hostileEnemy.playerDistance <= 200 then
					if perks.AOESwordTwo and perks.AOESwordTwo.activated then
						tes3.modStatistic({ reference = hostileEnemy, name = "health", current = -e.damage * 0.75})
					else
						tes3.modStatistic({ reference = hostileEnemy, name = "health", current = -e.damage * 0.5})
					end
					AOE = AOE + 1
					if hostileEnemy.health.current > 0 then
						timer.start({
							duration = 0.03,
							callback = function()
								tes3.playAnimation({ reference = hostileEnemy, group = tes3.animationGroup.hit1, startFlag = tes3.animationStartFlag.immediate, loopCount = 0 })
							end
						})
					end
				end
			...
			if AOE >= 1 then
				tes3.playSound({ reference = hostileEnemy, sound = "critical damage" })
				tes3.messageBox("АОЕ Урон!")
				AOETimer = 1
				timer.start({
					duration = 5,
					callback = function()
		...
		if perks.farHit and perks.farHit.activated and e.projectile and tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanBow} then
			local damageFarBonus = 0
			if perks.farHitTwo and perks.farHitTwo.activated then
				damageFarBonus = 1 + math.min((e.mobile.playerDistance / 6000), 1)
				if damageFarBonus == 2 then
					tes3.messageBox("Великолепный выстрел. Урон x2!")
				end
			else
				damageFarBonus = 1 + math.min(((e.mobile.playerDistance / 6000) * 0.5), 0.5)
				if damageFarBonus == 1.5 then
					tes3.messageBox("Великолепный выстрел. Урон x1.5!")
				end
			end
			e.damage = e.damage * damageFarBonus
		end
		if perks.bonusThrown and perks.bonusThrown.activated and e.projectile and tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanThrown} then
			if not thrownTimer then
				thrownTimer = os.clock()
			else
				local newthrownTimer = os.clock() - thrownTimer
				if newthrownTimer <= 2 then
					if perks.bonusThrownTwo and perks.bonusThrownTwo.activated then
						if countThrown < 1.5 then
							countThrown = countThrown + 0.1
						end
					else
						if countThrown < 1.25 then
							countThrown = countThrown + 0.05
						end
					end
					e.damage = e.damage * countThrown
					thrownTimer = os.clock()
				else
					thrownTimer = nil
					countThrown = 1
		...
		if perks.tripleProjectile and perks.tripleProjectile.activated and (tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanBow} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanThrown}) and e.projectile and mouseTime == true then
			local TripleHit = 0
			for _, hostileEnemy in ipairs(tes3.mobilePlayer.hostileActors) do
				if hostileEnemy and tes3.mobilePlayer.actionData.target and hostileEnemy ~= tes3.mobilePlayer.actionData.target then
					local enemyAngel = tes3.mobilePlayer:getViewToActor(hostileEnemy)
					local arrowID = tes3.getItemCount({ reference = tes3.player, item = e.projectile.reference.id })
					local distanceEnemy = hostileEnemy.position:distanceXY(tes3.mobilePlayer.actionData.target.position)
					if (enemyAngel > -30 and enemyAngel < 30) and arrowID >=3 and hostileEnemy.inCombat and distanceEnemy <= 1000 then
						tes3.modStatistic({ reference = hostileEnemy, name = "health", current = -e.damage * 0.5 })
						tes3.removeItem({ reference = tes3.player, item = e.projectile.reference.id, count = 1 })
						TripleHit = TripleHit + 1
						if hostileEnemy.health.current > 0 then
							timer.start({
								duration = 0.03,
								callback = function()
...
local function onDamagedModification(e)
	...
	if e.source == tes3.damageSource.attack and perks.fastShort and perks.fastShort.activated and not e.projectile and e.attackerReference.mobile.weaponReady and fiveHits <= 4 then
		fiveHits = fiveHits + 1
		if fiveHits == 5 then
			timer.start({
				duration = 5,
	...
	if e.source == tes3.damageSource.attack and perks.bluntcrush.activated and not e.reference.mobile.isDead and not saveData.crushActors[e.reference.id] and not e.projectile then
		...
		if (perks.bluntcrushTwo.activated and randomCrush > 85) or (perks.bluntcrush.activated and randomCrush > 90) then
			...
			if perks.bluntweapon.activated then
				tes3.messageBox("Ваш удар оглушил противника!")
			elseif perks.shortBlade.activated then
				tes3.messageBox("Ваш удар парализовал противника!")
			end
			tes3.applyMagicSource({reference = tes3.player, name = "Оглушение", bypassResistances = true, effects = {{id = tes3.effect.paralyze, rangeType = tes3.effectRange.touch, duration = 5, min = 0, max = 0}}})
			timer.start({
				duration = 0.04167,
				type = timer.game,
				persist = true,
...
local function onBindsFunction(e)
	...
	if perks.ropeArrow.activated and e.keybind == tes3.keybind.jump and e.transition == tes3.keyTransition.downThisFrame and jampToArrow ~= nil then
		timer.start({
			duration = 0.4,
			callback = function()
				tes3.mobilePlayer:doJump({ velocity = jampToArrow, applyFatigueCost = false, allowMidairJumping = true })
				jampToArrow = nil
			end
		})
	end
	if perks.tripleProjectile.activated and (tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanBow} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanThrown}) and tes3.mobilePlayer.weaponReady and e.keybind == tes3.keybind.use and e.transition == tes3.keyTransition.downThisFrame then
		timestamp = os.clock()
	elseif perks.tripleProjectile.activated and (tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanBow} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanThrown}) and tes3.mobilePlayer.weaponReady and e.keybind == tes3.keybind.use and e.transition == tes3.keyTransition.upThisFrame then
		if timestamp ~= nil then
			local newTime = os.clock() - timestamp
			if newTime >= 3 then
				mouseTime = true
				timestamp = nil
			else
	...
	if perks.warrogueTwo.activated and saveData.cantUp == 1 and e.keybind == tes3.keybind.sneak and e.transition == tes3.keyTransition.downThisFrame then
		e.result = false
	end
	if perks.warrogueTwo.activated and e.keybind == tes3.keybind.sneak and e.transition == tes3.keyTransition.downThisFrame and tes3.mobilePlayer.inCombat and saveData.mist == 0 then
		tes3.applyMagicSource({ reference = tes3.player, target = tes3.player, source = "kb_warrogueTwoSpell" })
		tes3.mobilePlayer.isSneaking = true
		tes3.messageBox("Вы ушли в туман!")
		saveData.cantUp = 1
		for enemy in tes3.getPlayerCell():iterateReferences({tes3.objectType.npc, tes3.objectType.creature}) do
			if enemy then
				mwscript.stopCombat({reference = enemy, target = tes3.mobilePlayer})
			end
		end
		timer.start({
			duration = 0.0416,
			type = timer.game,
			persist = true,
			callback = "Samson:cantUpTimer",
		})
		saveData.mist = 1
		if perks.warrogueFour.activated then
			timer.start({
				duration = 0.0833,
				type = timer.game,
				persist = true,
				callback = "Samson:mistTimer",
			})
		else
			timer.start({
				duration = 0.125,
...
timer.register("Samson:cantUpTimer", function(e)
	if saveData.cantUp == 1 then
		saveData.cantUp = 0
		tes3.mobilePlayer.isSneaking = false
		if perks.warrogueAvoidTwo.activated and saveData.fullSet == true then
			tes3.applyMagicSource({reference = tes3.player, name = "Ускользание", effects = {{id = tes3.effect.sanctuary, rangeType = tes3.effectRange.self, min = 15, max = 15, duration = 3}}})
...
timer.register("Samson:mistTimer", function(e)
	saveData.mist = 0
	tes3.playSound({ reference = tes3.player, sound = "spellmake fail" })
end)
...
local function magicschoolchoise(e)
	...
	if perks.warrogue.activated then
		e.cost = 0
		return
	end
...
local function spellCastCallback(e)
	...
	if perks.warrogue.activated and e.caster == tes3.player and e.source.castType == tes3.spellType.spell then
		e.castChance = 0
		tes3.messageBox("Архетип Воин-Разбойник не позволяет вам использовать заклинания!")
	end
end
...
local function uiActivatedCallback(e)
					...
					tes3.messageBox({
						...
						callback = function(e)
							...
							if e.button == 0 and perks.longblade.activated then
								tes3.messageBox({
									message = "Уникальные длинные клинки",
							...
							elseif e.button == 0 and perks.axe.activated then
								tes3.messageBox({
									...
									buttons = {"Топор Рангарда", "Секира Скорби", "Выйти"},
									callback = function(e)
										if e.button == 0 then
										...
										elseif e.button == 1 then
											if object2 >= 10 and object4 >= 5 and object6 >= 3 and object8 >= 10 and object14 >= 5 and object15 >= 1 and object16 >= 1 then
												if (tes3.mobilePlayer.armorer.base >= 75 and randomWeapon >= 50) or (tes3.mobilePlayer.armorer.base == 100 and randomWeapon >= 25) then
													...
													tes3.playSound({ reference = tes3.player, sound = "cent steam fall" })
													tes3.messageBox("Вы успешно выковали Секиру Скорби!")
												else
													tes3.messageBox("Вам не удалось выковать Секиру Скорби!")
													tes3.removeItem({ reference = tes3.player, item = "T_Com_MetalPieceIron_01", count = 10 })
													tes3.removeItem({ reference = tes3.player, item = "ingred_raw_ebony_01", count = 5 })
													tes3.removeItem({ reference = tes3.player, item = "T_IngCrea_Dragonscales_01", count = 3 })
													tes3.removeItem({ reference = tes3.player, item = "T_IngMine_OreGold_01", count = 10 })
							...
							elseif e.button == 0 and perks.shortBlade.activated then
								tes3.messageBox({
									message = "Уникальные короткие клинки",
									buttons = {"Кинжал Четырех Столпов", "Клинок Славы Альмсиви", "Выйти"},
									callback = function(e)
										if e.button == 0 then
											if object9 >= 10 and object7 >= 10 and object3 >= 3 and object10 >= 3 and object11 >= 10 and object15 >= 1 and object16 >= 1 then
												if (tes3.mobilePlayer.armorer.base >= 75 and randomWeapon >= 50) or (tes3.mobilePlayer.armorer.base == 100 and randomWeapon >= 25) then
													tes3.addItem({ reference = tes3.player, item = "samson_shortBladeUnique", count = 1 })
													...
													tes3.removeItem({ reference = tes3.player, item = "ingred_scrap_metal_01", count = 10 })
													tes3.playSound({ reference = tes3.player, sound = "cent steam fall" })
													tes3.messageBox("Вы успешно выковали Кинжал Четырех Столпов!")
												else
													tes3.messageBox("Вам не удалось выковать Кинжал Четырех Столпов!")
													tes3.removeItem({ reference = tes3.player, item = "T_IngMine_OreCobalt_01", count = 10 })
													tes3.removeItem({ reference = tes3.player, item = "T_IngMine_OreIron_01", count = 10 })
													tes3.removeItem({ reference = tes3.player, item = "ingred_daedras_heart_01", count = 3 })
													tes3.removeItem({ reference = tes3.player, item = "ingred_daedra_skin_01", count = 3 })
										...
										elseif e.button == 1 then
											if object >= 10 and object12 >= 5 and object14 >= 5 and object5 >= 5 and object3 >= 3 and object15 >= 1 and object16 >= 1 then
												if (tes3.mobilePlayer.armorer.base >= 75 and randomWeapon >= 50) or (tes3.mobilePlayer.armorer.base == 100 and randomWeapon >= 25) then
													tes3.addItem({ reference = tes3.player, item = "samson_shortBladeUniqueII", count = 1 })
													...
													tes3.removeItem({ reference = tes3.player, item = "ingred_daedras_heart_01", count = 3 })
													tes3.playSound({ reference = tes3.player, sound = "cent steam fall" })
													tes3.messageBox("Вы успешно выковали Клинок Славы Альмсиви!")
												else
													tes3.messageBox("Вам не удалось выковать Клинок Славы Альмсиви!")
													tes3.removeItem({ reference = tes3.player, item = "ingred_raw_glass_01", count = 10 })
													tes3.removeItem({ reference = tes3.player, item = "T_IngMine_OreOrichalcum_01", count = 5 })
													tes3.removeItem({ reference = tes3.player, item = "ingred_raw_Stalhrim_01", count = 5 })
													tes3.removeItem({ reference = tes3.player, item = "ingred_adamantium_ore_01", count = 5 })
	...
	if perks.warmage and perks.warmage.activated and perks.alchemystTwo and perks.alchemystTwo.activated and e.element.name == "MenuAlchemy" and tes3.mobilePlayer.alchemy.base >= 75 then
		tes3.messageBox({
	...
	elseif perks.warrogue and perks.warrogue.activated and perks.alchemystTwo and perks.alchemystTwo.activated and e.element.name == "MenuAlchemy" and tes3.mobilePlayer.alchemy.base >= 75 then
		tes3.messageBox({
			message = "Хотите ли вы создать уникальный эликсир?",
			buttons = {"Да", "Нет"},
			callback = function(e)
				if e.button == 0 then
					tes3.messageBox({
						message = "Какой эликсир вы хотите создать?",
						buttons = {"Эликсир ослепляющих ударов", "Эликсир истощения маны", "Эликсир отравляющих ударов", "Выйти"},
						callback = function(e)
							local object = tes3.getItemCount({ reference = tes3.player, item = "INGRED_DURZOG_MEAT_01" })
							local object1 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_BOAR_LEATHER" })
							local object2 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_BONEMEAL_01" })
							local object3 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_EYEBALL" })
							local object4 = tes3.getItemCount({ reference = tes3.player, item = "T_IngCrea_VelkNectarSack_01" })
							local object6 = tes3.getItemCount({ reference = tes3.player, item = "T_IngCrea_Trollfat_01" })
							local object7 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_WICKWHEAT_01" })
							local object8 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_SCAMP_SKIN_01" })
							local object9 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_BELLADONNA_02" })
							local object10 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_COPRINUS_01" })
							local object11 = tes3.getItemCount({ reference = tes3.player, item = "ingred_russula_01" })
							local object12 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_ADAMANTIUM_ORE_01" })
							local object13 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_NOBLE_SEDGE_01" })
							local object14 = tes3.getItemCount({ reference = tes3.player, item = "INGRED_LLORAMOR_SPINES_01" })
							local randomPotion = math.random(1, 100)
							if e.button == 0 then
								if object >= 2 and object1 >= 2 and object2 >= 5 and object3 >= 1 and object4 >= 2 then
									if (tes3.mobilePlayer.alchemy.base >= 75 and randomPotion >= 50) or (tes3.mobilePlayer.alchemy.base == 100 and randomPotion >= 25) then
										tes3.addItem({ reference = tes3.player, item = "samson_ElixirBlind", count = 1 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_DURZOG_MEAT_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_BOAR_LEATHER", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_BONEMEAL_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_EYEBALL", count = 1 })
										tes3.removeItem({ reference = tes3.player, item = "T_IngCrea_VelkNectarSack_01", count = 2 })
										tes3.playSound({ reference = tes3.player, sound = "potion success" })
										tes3.messageBox("Вы успешно создали Эликсир ослепляющих ударов!")
									else
										tes3.messageBox("Вам не удалось создать Эликсир ослепляющих ударов!")
										tes3.removeItem({ reference = tes3.player, item = "INGRED_DURZOG_MEAT_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_BOAR_LEATHER", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_BONEMEAL_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_EYEBALL", count = 1 })
							...
							elseif e.button == 1 then
								if object >= 2 and object6 >= 2 and object7 >= 5 and object8 >= 3 and object9 >= 3 then
									if (tes3.mobilePlayer.alchemy.base >= 75 and randomPotion >= 50) or (tes3.mobilePlayer.alchemy.base == 100 and randomPotion >= 25) then
										tes3.addItem({ reference = tes3.player, item = "samson_ElixirNoMana", count = 1 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_DURZOG_MEAT_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "T_IngCrea_Trollfat_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_WICKWHEAT_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_SCAMP_SKIN_01", count = 3 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_BELLADONNA_02", count = 3 })
										tes3.playSound({ reference = tes3.player, sound = "potion success" })
										tes3.messageBox("Вы успешно создали Эликсир истощения маны!")
									else
										tes3.messageBox("Вам не удалось создать Эликсир истощения маны!")
										tes3.removeItem({ reference = tes3.player, item = "INGRED_DURZOG_MEAT_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "T_IngCrea_Trollfat_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_WICKWHEAT_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_SCAMP_SKIN_01", count = 3 })
							...
							elseif e.button == 2 then
								if object10 >= 5 and object11 >= 5 and object12 >= 2 and object13 >= 3 and object14 >= 2 then
									if (tes3.mobilePlayer.alchemy.base >= 75 and randomPotion >= 50) or (tes3.mobilePlayer.alchemy.base == 100 and randomPotion >= 25) then
										tes3.addItem({ reference = tes3.player, item = "samson_ElixirPoison", count = 1 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_COPRINUS_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "ingred_russula_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_ADAMANTIUM_ORE_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_NOBLE_SEDGE_01", count = 3 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_LLORAMOR_SPINES_01", count = 2 })
										tes3.playSound({ reference = tes3.player, sound = "potion success" })
										tes3.messageBox("Вы успешно создали Эликсир отравляющих ударов!")
									else
										tes3.messageBox("Вам не удалось создать Эликсир отравляющих ударов!")
										tes3.removeItem({ reference = tes3.player, item = "INGRED_COPRINUS_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "ingred_russula_01", count = 5 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_ADAMANTIUM_ORE_01", count = 2 })
										tes3.removeItem({ reference = tes3.player, item = "INGRED_NOBLE_SEDGE_01", count = 3 })
...
local function enchantChargeUseCallback(e)
	...
	if perks.enchanter.activated then
		...
		if (perks.enchanterTwo.activated and randomEnchante > 75) or (perks.enchanter.activated and randomEnchante > 85) then
			e.charge = 0
...
local function shadowStep(e)
	local enemy = tes3.mobilePlayer.actionData.target
	if perks.warrogueStep.activated and enemy and enemy.playerDistance <=1000 and saveData.canStep == 0 and saveData.cantUp == 0 and saveData.fullSet == true then
		local angleEnemy = tes3.mobilePlayer:getViewToActor(enemy)
		if (angleEnemy > -15 and angleEnemy < 15) then
			tes3.applyMagicSource({ reference = tes3.player, target = tes3.player, source = "kb_warrogueStepSpell" })
			tes3.player.position = enemy.position + (enemy.reference.forwardDirection * -50)
			angleEnemy = tes3.mobilePlayer:getViewToActor(enemy)
			angleEnemyRad = (angleEnemy * math.pi) / 180
			angleEnemyNew = tes3.player.orientation.z + angleEnemyRad
			tes3.player.orientation = tes3vector3.new(0, 0, angleEnemyNew)
			saveData.canStep = 1
			timer.start({
				duration = 0.0416,
				type = timer.game,
				persist = true,
				callback = "Samson:stepTimer"
			})
			saveData.xDamage = 1
			timer.start({
				duration = 0.0166,
				type = timer.game,
				persist = true,
				callback = "Samson:xDamageTimer"
			})
		end
	end
end
event.register(tes3.event.keyDown, shadowStep, { filter = tes3.scanCode.leftAlt })

timer.register("Samson:stepTimer", function(e)
	saveData.canStep = 0
end)
timer.register("Samson:xDamageTimer", function(e)
	saveData.xDamage = 0
end)
...
local function attackStartCallback(e)
	...
	if perks.fastSword and perks.fastSword.activated and tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.longBladeOneHand} and e.mobile.weaponReady then
		if perks.fastSwordTwo and perks.fastSwordTwo.activated then
			e.attackSpeed = e.attackSpeed * 1.2
		else
			e.attackSpeed = e.attackSpeed * 1.1
		end
	elseif perks.fastShort and perks.fastShort.activated and tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.shortBladeOneHand} and e.mobile.weaponReady then
		if fiveHits == 5 then
			if perks.fastShortTwo and perks.fastShortTwo.activated then
				e.attackSpeed = e.attackSpeed * 2
			else
				e.attackSpeed = e.attackSpeed * 1.5
			end
		end
	elseif perks.fastAxe and perks.fastAxe.activated and tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.axeOneHand} and e.mobile.weaponReady and e.mobile.actionData.target and e.mobile.actionData.target.health.current <= e.mobile.actionData.target.health.base * 0.5 and e.mobile.actionData.target.isDead == false then
		if perks.fastAxeTwo and perks.fastAxeTwo.activated then
			e.attackSpeed = e.attackSpeed * 1.5
		else
			e.attackSpeed = e.attackSpeed * 1.35
		end
	elseif perks.fastReng and perks.fastReng.activated and (tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanBow} or tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanThrown}) and e.mobile.weaponReady then
		if perks.fastRengTwo and perks.fastRengTwo.activated then
			e.attackSpeed = e.attackSpeed * 1.5
		else
			e.attackSpeed = e.attackSpeed * 1.25
		end
	end
end
...
local function projectileHitTerrainCallback(e)
	if perks.ropeArrow and perks.ropeArrow.activated and tes3.getEquippedItem{actor = tes3.player, objectType = tes3.objectType.weapon, type = tes3.weaponType.marksmanBow} and e.firingReference.mobile.weaponReady and jampToArrow == nil then
		jampToArrow = e.velocity
		tes3.messageBox("Кошка закреплена!")
	end
end
...
local function registerPerks()
	perks.warrogue = perkFramework.createPerk({
		id = "kb_perk_warrogue",
		name = "Воин-Разбойник",
		description = "Архетип Воин-Разбойник увеличивает шанс уклонения на 10 процентов.(Выбор архетипа - основополагающий. Дальнейшее развитие персонажа будет происходить в рамках выбранного архетипа.)",
		lvlReq = 2,
		perkExclude = {"kb_perk_warmage"},
		hideInMenu = true,
	})
	perks.warrogueTwo = perkFramework.createPerk({
		id = "kb_perk_warrogueTwo",
		name = "Воин-Разбойник II",
		lvlReq = 4,
		perkReq = {"kb_perk_warrogue"},
		description = "Раз в 15 секунд, находясь в бою, вы можете уйти в туман(кнопка 'красться'), полностью исчезнув на 5 секунд. Удар в этом состоянии нанесет тройной урон врагу и выведет вас из тумана.",
		hideInMenu = true,
	})
	perks.shortBlade = perkFramework.createPerk({
		id = "kb_perk_shortBlade",
		name = "Короткие клинки",
		description = "Вы специализируетесь на коротких клинках, остальное оружие вам недоступно. Но, благодаря отличной подготовке, короткие клинки в ваших руках наносят на 10% больше урона.",
		lvlReq = 6,
		perkReq = {"kb_perk_warrogueTwo"},
		perkExclude = {"kb_perk_longblade", "kb_perk_axe"},
		hideInMenu = true,
	})
	perks.bow = perkFramework.createPerk({
		id = "kb_perk_bow",
		name = "Луки",
		description = "Вы специализируетесь на луках, остальное оружие дальнего боя вам недоступно. Но, благодаря более сильному натяжению тетивы, стрелы наносят на 10% больше урона.",
		lvlReq = 6,
		perkReq = {"kb_perk_warrogueTwo"},
		perkExclude = {"kb_perk_throw"},
		hideInMenu = true,
	})
	perks.ropeArrow = perkFramework.createPerk({
		id = "kb_perk_ropeArrow",
		name = "Стрела-кошка",
		description = "Выстрелив в какой-нибудь объект, например в стену или землю, вы закрепите там выпущенную стрелу, как кошку. Прыгнув после этого, вы полетите в ту сторону. Вы можете нажать кнопку 'U', чтобы отсоединиться от кошки.",
		lvlReq = 12,
		perkReq = {"kb_perk_bow"},
		hideInMenu = true,
	})
	perks.bowTwo = perkFramework.createPerk({
		id = "kb_perk_bowTwo",
		name = "Луки II",
		description = "Теперь стрелы наносят на 15% больше урона.",
		lvlReq = 12,
		perkReq = {"kb_perk_bow"},
		hideInMenu = true,
	})
	perks.throw = perkFramework.createPerk({
		id = "kb_perk_throw",
		name = "Метательное оружие",
		description = "Вы специализируетесь на метательном оружии, остальное оружие дальнего боя вам недоступно. Но, благодаря отличной подготовке, ваши дротики и звездочки наносят на 10% больше урона.",
		lvlReq = 6,
		perkReq = {"kb_perk_warrogueTwo"},
		perkExclude = {"kb_perk_bow"},
		hideInMenu = true,
	})
	perks.throwTwo = perkFramework.createPerk({
		id = "kb_perk_throwTwo",
		name = "Метательное оружие II",
		description = "Теперь дротики и звездочки наносят на 15% больше урона.",
		lvlReq = 12,
		perkReq = {"kb_perk_throw"},
		hideInMenu = true,
	})
	perks.warrogueTree = perkFramework.createPerk({
		id = "kb_perk_warrogueTree",
		name = "Воин-Разбойник III",
		description = "Теперь ваш шанс уклонения увеличен на 20 процентов, вместо 10.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogueTwo"},
		hideInMenu = true,
	})
	perks.warrogueStep = perkFramework.createPerk({
		id = "kb_perk_warrogueStep",
		name = "Шаг сквозь тень",
		description = "Раз в 5 секунд вы можете оказаться за спиной противника. Удар, нанесенный после этого в течении 2 секунд, будет x1.5. Чтобы использовать данный талант, используйте кнопку Left Alt.",
		lvlReq = 12,
		perkReq = {"kb_perk_lightarmor", "kb_perk_warrogue"},
		hideInMenu = true,
	})
	perks.warrogueAvoid = perkFramework.createPerk({
		id = "kb_perk_warrogueAvoid",
		name = "Ускользание",
		description = "После выхода из тумана вы на 3 секунды получаете эффект 'Ускользание', повышающий вероятность уклонения на 10 процентов. Требуется полный комплект легких доспехов.",
		lvlReq = 12,
		perkReq = {"kb_perk_lightarmor", "kb_perk_warrogue"},
		hideInMenu = true,
	})
	perks.warrogueFaster = perkFramework.createPerk({
		id = "kb_perk_warrogueFaster",
		name = "Ускорение",
		description = "После выхода из тумана ваша скорость передвижения значительно увеличивается на 3 секунды. Требуется полный комплект легких доспехов.",
		lvlReq = 12,
		perkReq = {"kb_perk_lightarmor", "kb_perk_warrogue"},
		hideInMenu = true,
	})
	perks.nightVision = perkFramework.createPerk({
		id = "kb_perk_nightVision",
		name = "Ночное зрение",
		description = "За полный комплект легких доспехов вы получаете способность 'Кошачий глаз' 10 пунктов. Способность можно активировать и деактивировать, нажав кнопку 'Y'",
		lvlReq = 12,
		perkReq = {"kb_perk_lightarmor", "kb_perk_warrogue"},
		hideInMenu = true,
	})
	perks.shortBladeTwo = perkFramework.createPerk({
		id = "kb_perk_shortBladeTwo",
		name = "Короткие клинки II",
		description = "Теперь, благодаря отличной подготовке, короткие клинки в ваших руках наносят на 15% больше урона.",
		lvlReq = 12,
		perkReq = {"kb_perk_shortBlade"},
		hideInMenu = true,
	})
	perks.deadlyHit = perkFramework.createPerk({
		id = "kb_perk_deadlyHit",
		name = "Смертельный удар",
		description = "После шага сквозь тень удар, который вы нанесете в течении 2 секунд, может стать для противника последним. Шанс - 10 процентов.",
		lvlReq = 12,
		perkReq = {"kb_perk_shortBlade", "kb_perk_warrogueStep"},
		hideInMenu = true,
	})
	perks.axeBleed = perkFramework.createPerk({
		id = "kb_perk_axeBleed",
		name = "Кровотечение",
		description = "После шага сквозь тень удар, который вы нанесете в течении 2 секунд, может вызвать у противника кровотечение, которое отнимет 15 процентов его здоровья за 10 секунд. Шанс - 35 процентов.",
		lvlReq = 12,
		perkReq = {"kb_perk_axe", "kb_perk_warrogueStep"},
		hideInMenu = true,
	})
	perks.staminaHit = perkFramework.createPerk({
		id = "kb_perk_staminaHit",
		name = "Изматывающие порезы",
		description = "После шага сквозь тень удар, который вы нанесете в течении 2 секунд, так же отнимет усталость противника, в размере x2 от вашего урона.",
		lvlReq = 12,
		perkReq = {"kb_perk_longblade", "kb_perk_warrogueStep"},
		hideInMenu = true,
	})
	perks.mageKiller = perkFramework.createPerk({
		id = "kb_perk_mageKiller",
		name = "Сбитие каста",
		description = "Если противник начинает каст, вы можете сбить его ударом кулака. Для быстрого переключения между вашим оружием и кулаками, нажмите F2. Успешность сбития каста зависит от вашего уровня рукопашного боя.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogue"},
		hideInMenu = true,
	})
	perks.fastSword = perkFramework.createPerk({
		id = "kb_perk_fastSword",
		name = "Быстрый клинок",
		description = "Благодаря искусному владению длинными клинками, ваша скорость атаки увеличена на 10 процентов.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogue", "kb_perk_longblade"},
		hideInMenu = true,
	})
	perks.fastSwordTwo = perkFramework.createPerk({
		id = "kb_perk_fastSwordTwo",
		name = "Быстрый клинок II",
		description = "Теперь ваша скорость атаки увеличена на 20 процентов.",
		lvlReq = 14,
		perkReq = {"kb_perk_fastSword"},
		hideInMenu = true,
	})
	perks.fastShort = perkFramework.createPerk({
		id = "kb_perk_fastShort",
		name = "Шквал",
		description = "Благодаря искусному владению короткими клинками, после 5 ударов ваша скорость атаки увеличивается в 1.5 раза на 5 секунд.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogue", "kb_perk_shortBlade"},
		hideInMenu = true,
	})
	perks.fastShortTwo = perkFramework.createPerk({
		id = "kb_perk_fastShortTwo",
		name = "Шквал II",
		description = "Теперь ваша скорость атаки после 5 ударов увеличивается в 2 раза.",
		lvlReq = 14,
		perkReq = {"kb_perk_fastShort"},
		hideInMenu = true,
	})
	perks.fastAxe = perkFramework.createPerk({
		id = "kb_perk_fastAxe",
		name = "Запах крови",
		description = "Благодаря искусному владению топорами, когда у врага меньше 50 процентов здоровья, ваша скорость атаки увеличивается на 35 процентов.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogue", "kb_perk_axe"},
		hideInMenu = true,
	})
	perks.fastAxeTwo = perkFramework.createPerk({
		id = "kb_perk_fastAxeTwo",
		name = "Запах крови II",
		description = "Теперь ваша скорость атаки увеличивается на 50 процентов, когда здоровье врага ниже 50 процентов.",
		lvlReq = 14,
		perkReq = {"kb_perk_fastAxe"},
		hideInMenu = true,
	})
	perks.fastReng = perkFramework.createPerk({
		id = "kb_perk_fastReng",
		name = "Скорострел",
		description = "Благодаря искусному владению оружием дальнего боя, ваша скорость натяжения тетивы или метания(зависит от выбранного оружия) увеличена на 25 процентов.",
		lvlReq = 12,
		customReq = function() return (perks.warrogue.activated and perks.bow.activated) or (perks.warrogue.activated and perks.throw.activated) end,
		hideInMenu = true,
	})
	perks.fastRengTwo = perkFramework.createPerk({
		id = "kb_perk_fastRengTwo",
		name = "Скорострел II",
		description = "Теперь ваша скорость натяжения тетивы или метания(зависит от выбранного оружия) увеличена на 50 процентов.",
		lvlReq = 14,
		perkReq = {"kb_perk_fastReng"},
		hideInMenu = true,
	})
	perks.farHit = perkFramework.createPerk({
		id = "kb_perk_farHit",
		name = "Дальнобой",
		description = "Теперь ваш урон из лука зависит от расстояния до цели. Чем дальше цель, тем выше урон, вплоть до x1.5.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogue", "kb_perk_bow"},
		hideInMenu = true,
	})
	perks.farHitTwo = perkFramework.createPerk({
		id = "kb_perk_farHitTwo",
		name = "Дальнобой II",
		description = "Теперь ваш урон из лука может быть увеличен, вплоть до x2.",
		lvlReq = 14,
		perkReq = {"kb_perk_farHit"},
		hideInMenu = true,
	})
	perks.bonusThrown = perkFramework.createPerk({
		id = "kb_perk_bonusThrown",
		name = "Накопительный эффект",
		description = "Урон от каждого последующего броска метательного оружия, совершенного в течении 2 секунд от предыдущего попадания, будет увеличен на 5%, вплоть до 25%.",
		lvlReq = 12,
		perkReq = {"kb_perk_warrogue", "kb_perk_throw"},
		hideInMenu = true,
	})
	perks.bonusThrownTwo = perkFramework.createPerk({
		id = "kb_perk_bonusThrownTwo",
		name = "Накопительный эффект II",
		description = "Теперь бонус за быстрое метание - 10%. Вплоть до 50%.",
		lvlReq = 14,
		perkReq = {"kb_perk_bonusThrown"},
		hideInMenu = true,
	})
	perks.tripleProjectile = perkFramework.createPerk({
		id = "kb_perk_tripleProjectile",
		name = "Залп",
		description = "Зажав кнопку атаки с оружием дальнего боя на 3 и более секунды, урон получат сразу три врага в секторе 60 градусов перед вами.",
		lvlReq = 12,
		customReq = function() return (perks.warrogue.activated and perks.bow.activated) or (perks.warrogue.activated and perks.throw.activated) end,
		hideInMenu = true,
	})
	perks.AOESword = perkFramework.createPerk({
		id = "kb_perk_AOESword",
		name = "Вихрь",
		description = "Раз в 5 секунд ваш режущий удар по противнику так же нанесет АОЕ урон по остальным врагам(если таковые имеются), в размере 50 процентов от основного урона.",
		lvlReq = 12,
		customReq = function() return (perks.warrogue.activated and perks.axe.activated) or (perks.warrogue.activated and perks.longblade.activated) end,
		hideInMenu = true,
	})
	perks.AOESwordTwo = perkFramework.createPerk({
		id = "kb_perk_AOESwordTwo",
		name = "Вихрь II",
		description = "Теперь АОЕ наносит 75 процентов от основного урона.",
		lvlReq = 14,
		perkReq = {"kb_perk_AOESword"},
		hideInMenu = true,
	})
	perks.deadlyHitTwo = perkFramework.createPerk({
		id = "kb_perk_deadlyHitTwo",
		name = "Смертельный удар II",
		description = "Теперь шанс убить противника после шага сквозь тень - 15 процентов.",
		lvlReq = 14,
		perkReq = {"kb_perk_deadlyHit"},
		hideInMenu = true,
	})
	perks.axeBleedTwo = perkFramework.createPerk({
		id = "kb_perk_axeBleedTwo",
		name = "Кровотечение II",
		description = "Теперь шанс вызвать кровотечение у противника - 50 процентов.",
		lvlReq = 14,
		perkReq = {"kb_perk_axeBleed"},
		hideInMenu = true,
	})
	perks.staminaHitTwo = perkFramework.createPerk({
		id = "kb_perk_staminaHitTwo",
		name = "Изматывающие порезы II",
		description = "Теперь удар после шага сквозь тень отнимет усталость противника в размере X3 от вашего урона.",
		lvlReq = 14,
		perkReq = {"kb_perk_staminaHit"},
		hideInMenu = true,
	})
	perks.warrogueStepTwo = perkFramework.createPerk({
		id = "kb_perk_warrogueStepTwo",
		name = "Шаг сквозь тень II",
		description = "Теперь, после шага сквозь тень, вы нанесете двойной урон.",
		lvlReq = 14,
		perkReq = {"kb_perk_warrogueStep"},
		hideInMenu = true,
	})
	perks.warrogueAvoidTwo = perkFramework.createPerk({
		id = "kb_perk_warrogueAvoidTwo",
		name = "Ускользание II",
		description = "Теперь, выйдя из тумана, ваша вероятность уклонения увеличивается на 15 процентов.",
		lvlReq = 14,
		perkReq = {"kb_perk_warrogueAvoid"},
		hideInMenu = true,
	})
	perks.warrogueFasterTwo = perkFramework.createPerk({
		id = "kb_perk_warrogueFasterTwo",
		name = "Ускорение II",
		description = "Теперь, выйдя из тумана, так же увеличится и ваша акробатика.",
		lvlReq = 14,
		perkReq = {"kb_perk_warrogueFaster"},
		hideInMenu = true,
	})
	perks.warrogueFour = perkFramework.createPerk({
		id = "kb_perk_warrogueFour",
		name = "Воин-Разбойник IV",
		description = "Теперь вы можете уйти в туман раз в 10 секунд.",
		lvlReq = 14,
		perkReq = {"kb_perk_warrogueTree"},
		hideInMenu = true,
	})
	...
	perks.lightarmor = perkFramework.createPerk({
		...
		lvlReq = 6,
		customReq = function() return perks.warmageTwo.activated or perks.warrogueTwo.activated end,
	...
	perks.longblade = perkFramework.createPerk({
		...
		lvlReq = 6,
		customReq = function() return perks.warmageTwo.activated or perks.warrogueTwo.activated end,
		perkExclude = {"kb_perk_bluntweapon", "kb_perk_axe", "kb_perk_shortBlade"},
		hideInMenu = true,
	})
	perks.axe = perkFramework.createPerk({
		...
		lvlReq = 6,
		customReq = function() return perks.warmageTwo.activated or perks.warrogueTwo.activated end,
		perkExclude = {"kb_perk_bluntweapon", "kb_perk_longblade", "kb_perk_shortBlade"},
		hideInMenu = true,
	})
	...
	perks.armorNoDie = perkFramework.createPerk({
		...
		lvlReq = 12,
		customReq = function() return perks.warmage.activated and (perks.heavyarmor.activated or perks.lightarmor.activated or perks.mediumarmor.activated) end,
		hideInMenu = true,
	})
	...
	perks.lightarmorMoving = perkFramework.createPerk({
		...
		perkReq = {"kb_perk_lightarmor", "kb_perk_warmage"},
		lvlReq = 12,
		hideInMenu = true,
	})
	...
	perks.axenonweapon = perkFramework.createPerk({
		...
		perkReq = {"kb_perk_axe", "kb_perk_warmage"},
		lvlReq = 12,
		hideInMenu = true,
	})
	...
	perks.axenonarmor = perkFramework.createPerk({
		...
		name = "Бронебой",
		description = "Ваши атаки с вероятностью 20% проигнорируют броню противника.",
		customReq = function() return (perks.warrogue.activated and perks.throw.activated) or (perks.warmage.activated and perks.axe.activated) end,
		lvlReq = 12,
		hideInMenu = true,
	})
	perks.axenonarmorTwo = perkFramework.createPerk({
		...
		name = "Бронебой II",
		description = "Теперь ваши атаки проигнорируют броню противника с вероятностью 30%.",
		perkReq = {"kb_perk_axenonarmor", "kb_perk_warmage"},
		lvlReq = 14,
		hideInMenu = true,
	})
	...
	perks.bluntcrush = perkFramework.createPerk({
		...
		description = "Удары молота/коротких клинков(в зависимости от архетипа) с вероятностью 10% могут оглушить/парализовать врага на 5 сек.",
		customReq = function() return perks.bluntweapon.activated or perks.shortBlade.activated end,
		lvlReq = 12,
		hideInMenu = true,
	})
	perks.bluntcrushTwo = perkFramework.createPerk({
		id = "kb_perk_bluntcrushTwo",
		name = "Ошеломляющий удар II",
		description = "Теперь вероятность оглушить/парализовать врага равна 15%.",
	...
	perks.longbladebleed = perkFramework.createPerk({
		...
		perkReq = {"kb_perk_longblade", "kb_perk_warmage"},
		lvlReq = 12,
		hideInMenu = true,
	})
	...
	perks.execute = perkFramework.createPerk({
		...
		customReq = function() return perks.warmage.activated and (perks.longblade.activated or perks.axe.activated) end,
		lvlReq = 12,
		hideInMenu = true,
	})
	...
	perks.enchanter = perkFramework.createPerk({
		...
		description = "У вас есть 15% шанс, что магический предмет не потеряет заряд при использовании. Кроме того, с этим же шансом предмет сработает даже при нулевом заряде.",
		lvlReq = 12,
		hideInMenu = true,
	})
	perks.enchanterTwo = perkFramework.createPerk({
		...
		description = "Теперь ваш шанс не потратить заряд предмета равен 25%.",
		lvlReq = 14,
	...
	perks.noArrowSword = perkFramework.createPerk({
		...
		customReq = function() return (perks.longblade.activated and perks.nonShield.activated) or perks.warrogue.activated end,
		lvlReq = 14,
		hideInMenu = true,
	})
