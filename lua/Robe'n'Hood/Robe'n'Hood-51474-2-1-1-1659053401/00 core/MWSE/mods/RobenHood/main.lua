...
local function isNPCValid(npc)
    ...
    -- Only do this if our previous checks determine that this is a valid actor.
    if npcIsValid then
      -- Loop over every friendly actor of the player (this will not include hostile actors that might have companion share!).
      for mobile in tes3.iterate(tes3.mobilePlayer.friendlyActors) do
        -- Check if the current actor we loop over is our npc,
        if mobile == npc.mobile then
          -- Our npc is only valid if it isn't a follower (~= means not equal).
          npcIsValid = tes3.getCurrentAIPackageId(npc.mobile) ~= tes3.aiPackage.follow
        end
      end
    end
...
local function getHatWeather(npc, weather)
    local hat = npc.data.jett_hatWeather and npc.data.jett_hatWeather[weather.index]
    if hat then
        logger:trace("Got cached hat %s for npc %s", hat, npc)
        return hat
    end
end

local function setHatWeather(npc, weather, itemId)
    if not npc.data.jett_hatWeather then
        npc.data.jett_hatWeather = {}
    end
    npc.data.jett_hatWeather[weather.index] = itemId
    logger:trace("Set cached hat %s for npc %s", itemId, npc)
end

local function getHatFromWeather(npc, weather)
    logger:trace("Getting hat from weather")
    local item
    --get cached item for this npc
    item = getHatWeather(npc, weather)
    ...
    if itemConf  then
        ...
        if item then
            --cache this hat so Npc always wears the same one for this weather
            setHatWeather(npc, weather, item)
        end
        return item
    end
end
...
local function alreadyHasHelm(npc)
    --First time, sets to true if npc has a helmet, otherwise sets to false
