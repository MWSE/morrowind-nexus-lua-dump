...
--------------------------------------------------
-- QUEUE — merges by statKey (one row per stat)
--------------------------------------------------
local function addToQueue(statKey, displayName, baseDelta, modDelta, dmgDelta, category, isSpellChange)
	...
	for ri = 1, #activeRows do
		...
		if row.mergeId == mergeId and not row.fadingOut then
			...
			if row.baseDelta == 0 and row.modDelta == 0 and row.dmgDelta == 0
	...
	for i, d in ipairs(popupQueue) do
		if d.mergeId == mergeId then
			...
			if d.baseDelta == 0 and d.modDelta == 0 and d.dmgDelta == 0
			...
			for _, key in ipairs(ATTR_KEYS) do
				...
				if filterSpells then
					...
					if baseDiff ~= 0 or modDiff ~= 0 or dmgDiff ~= 0 then
						addToQueue(key, ATTR_NAMES[key], baseDiff, modDiff, dmgDiff, "attribute", false)
					end
				elseif splitSpells then
					...
					if natBase ~= 0 or natMod ~= 0 or natDmg ~= 0 then
						addToQueue(key, ATTR_NAMES[key], natBase, natMod, natDmg, "attribute", false)
					end
					if spBase ~= 0 or spMod ~= 0 or spDmg ~= 0 then
						addToQueue(key, ATTR_NAMES[key], spBase, spMod, spDmg, "attribute", true)
					end
				else
					if baseDiff ~= 0 or modDiff ~= 0 or dmgDiff ~= 0 then
						addToQueue(key, ATTR_NAMES[key], baseDiff, modDiff, dmgDiff, "attribute", false)
		...
		if PS_showSkills ~= false then
			for _, key in ipairs(SKILL_KEYS) do
				...
				if filterSpells then
					...
					dmgDiff  = dmgDiff  - (cur.dmg  - prev.dmg)
					if baseDiff ~= 0 or modDiff ~= 0 or dmgDiff ~= 0 then
						addToQueue(key, SKILL_NAMES[key], baseDiff, modDiff, dmgDiff, "skill", false)
					end
				elseif splitSpells then
					local cur  = currentSpellContrib.skills[key] or ZERO_BUCKET
					local prev = previousSpellContrib.skills[key] or ZERO_BUCKET
					...
					local natDmg  = dmgDiff  - spDmg
					if natBase ~= 0 or natMod ~= 0 or natDmg ~= 0 then
						addToQueue(key, SKILL_NAMES[key], natBase, natMod, natDmg, "skill", false)
					end
					if spBase ~= 0 or spMod ~= 0 or spDmg ~= 0 then
						addToQueue(key, SKILL_NAMES[key], spBase, spMod, spDmg, "skill", true)
					end
				else
					if baseDiff ~= 0 or modDiff ~= 0 or dmgDiff ~= 0 then
						addToQueue(key, SKILL_NAMES[key], baseDiff, modDiff, dmgDiff, "skill", false)
...
local function spawnRow(data, baseX, baseY, scale)
	...
	-- Skip if all deltas ended up at zero after merging
	if not PS_showZeroRows and data.baseDelta == 0 and data.modDelta == 0 and data.dmgDelta == 0 then return nil end

	-- Column x positions (right-edge of each number, in pixels)
	local COL_BASE = 175
	local COL_MOD  = COL_BASE+30
	local COL_DMG  = COL_MOD +30

	-- Inner content element — only updated on text/color changes
	local contentItems = {
		{   -- tinted background
			name = "bg",
			type = ui.TYPE.Image,
	...
	-- [2] spell overlay (untinted) — only for spell-sourced changes
	...
	-- Outer wrapper — only updated on position/alpha changes
...
local function rebuildRow(row)
	if row.contentElement and row.contentElement.layout then
				...
				if scrollOffset >= spacingScaled then
					...
					if #activeRows > 0 then
						...
						if ejected.toBeSurvivor then
							-- Still has lifetime: promote to survivor
							-- Compute exact current-frame position (scrollOffset already wrapped)
							--ejected.spawnTime = ejected.spawnTime + 0.5 -- doesnt help
