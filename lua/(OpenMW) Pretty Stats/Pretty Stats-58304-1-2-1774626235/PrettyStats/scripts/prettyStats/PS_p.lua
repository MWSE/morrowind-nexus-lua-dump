...
settingsTemplate["General"] = {
	...
	settings = {
		...
		},
		{
			key = "PS_displayMode",
			name = "Display mode",
			description = "How stat changes are shown.\nDetailed: separate base, modifier, and damage columns.\nSimple: single net change with current effective value.",
			renderer = "select",
			default = "Detailed",
			argument = {
				disabled = false,
				l10n = "none",
				items = { "Detailed", "Simple" },
			},
		},
		--{
		--	key = "PS_aamCompat",
		--	name = "AAM Compatibility",
		--	description = "Account for AbilitiesAreModifiers reclassifying ability buffs as modifiers.\nRequires AbilitiesAreModifiers.",
		--	renderer = "checkbox",
		--	default = false,
		--},
	},
...
for _, template in pairs(settingsTemplate) do
	...
	settingsSection:subscribe(async:callback(function(_, setting)
		...
		--PSInterface.disableAAM = PS_aamCompat
	end))
end
