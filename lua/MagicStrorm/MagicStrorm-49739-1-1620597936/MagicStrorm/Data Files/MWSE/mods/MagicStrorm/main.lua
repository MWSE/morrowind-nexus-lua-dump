...
local function SPELLCASTED(e) if e.caster == p then local s = e.source		if s.castType == 0 then		local si = e.sourceInstance
	if CastK() then	local eff = s.effects		local min, max		mc = 0		local rad = mp:getSkillValue(11)/20
...
end end end		event.register("spellCasted", SPELLCASTED)
