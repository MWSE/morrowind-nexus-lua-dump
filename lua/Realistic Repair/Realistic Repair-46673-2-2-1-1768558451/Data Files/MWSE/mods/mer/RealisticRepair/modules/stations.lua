...
local function checkForStation()
    ...
    logger:trace("Checking for repair station in front of player")
    ...
    if result and result.reference and result.reference.object.objectType == tes3.objectType.static then
        logger:trace("Detected activation of object: %s", result.reference.object.id)
        local station = config.stations[result.reference.object.id:lower()]
        StationService.setCurrentStation(station)
    else
        logger:trace("No repair station detected")
        StationService.setCurrentStation(nil)
        if result and result.reference then
            logger:trace("Detected object: %s", result.reference.object.id)
        end
    end
