...
function RepairService.canBeEnhanced(item, itemData)
    if not RepairService.isPristine(item, itemData) then
        return false, "Item must be fully repaired with no degradation"
    end
    ...
    if currentEnhancement >= enhancementCap then
        return false, "Enhancement cap reached"
    end
...
function RepairService.updateRepairUsesText(conditionAmount)
    ...
    usesLabel.text = string.format("Uses %d", conditionAmount)
...
function RepairService.handleEnhancementAttempt(item, itemData)
    if not isEnhancementEnabled() then
        return false, "Enhancement is disabled"
    end

    if not StationService.isRepairMenuAtStation() then
        return false, "Must be at a station to enhance"
    end
    ...
    if not repairTool then
        ...
        return false, "No repair tool equipped"
    end
    ...
    if repairToolData and repairToolData.condition <= 0 then
        ...
        return false, "Repair tool is depleted"
    end
...
function RepairService.checkFatigueAfterAttempt()
    ...
    if currentFatigue <= 0 then
        ...
        local repairMenu = tes3ui.findMenu("MenuRepair")
        if repairMenu then
            RepairService.blockRepairButtons(repairMenu, "You are too fatigued to repair items.")
        end
    end
end
...
function RepairService.getRepairMenuMessage()
    ...
    if isDegradationEnabled() then
        ...
        if isAtStation then
            ...
            table.insert(messages, string.format("Degradation: %d on Failure | 1/2 repair restored on Success", baseDegradation))
        else
            -- Show only failure degradation (full amount)
            table.insert(messages, string.format("Degradation on Failure: %d", baseDegradation))
        end
    end
    ...
    if isEnhancementEnabled() and isAtStation then
        ...
        table.insert(messages, string.format("Enhancement: %d points", enhancementAmount))
    end
...
function RepairService.canEnhanceItem(item, itemData)
    ...
    if not StationService.isRepairMenuAtStation() then
        return false, "Must be at a station"
    end
...
local function handleSuccessfulNormalRepair(itemData)
    ...
    return itemData, "Repair successful"
end
...
local function handleSuccessfulDegradationRestoration(item, itemData, currentDegradation, repairAmount)
    ...
    return itemData, string.format("Restored %d degradation", actualRestoration)
end
...
local function handleSuccessfulEnhancement(item, itemData, currentEnhancement)
    ...
    if remainingCapacity <= 0 then
        ...
        return itemData, "Enhancement cap reached"
    end
    ...
    return itemData, string.format("Enhanced by %d points", appliedAmount)
end
...
local function applyDegradationDamage(item, itemData, degradationAmount, currentEnhancement, currentDegradation)
    if degradationAmount <= 0 then
        return itemData, "No degradation applied"
    end
    ...
    if currentEnhancement > 0 then
        ...
        if remainingDegradation > 0 and currentDegradation > 0 then
            ...
            return itemData, string.format("Lost %d enhancement, degraded by %d", enhancementLoss, remainingDegradation)
        else
            logger:debug("FAILURE: Lost %d enhancement (no degradation applied)", enhancementLoss)
            return itemData, string.format("Lost %d enhancement", enhancementLoss)
        end
    else
        ...
        if isPristine then
            ...
            return itemData, "Enhancement failed"
        end
        ...
        return itemData, string.format("Degraded by %d points", actualDegradation)
    end
end
...
function RepairService.handleAutomaticRepair(item, itemData, success, isAtStation, repairAmount)
    ...
    if success then
        ...
        return itemData, "Already at maximum condition"
    end
    ...
    return itemData, "No action taken"
