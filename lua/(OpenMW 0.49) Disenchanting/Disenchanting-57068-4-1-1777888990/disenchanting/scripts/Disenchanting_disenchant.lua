...
return function(item, preview, player)
	...
	-- projectile stacks: per-arrow recovery = enchPoints / mult (engine multiplies by mult) mult = 0 is vanilla's 1-arrow-per-soul
	local count = 1
	...
	if preview and not isProjectile then
	...
	if not preview then
		if isProjectile then
			-- projectiles return a matching stackable vanilla record, if exists
			local key = (record.icon or "").."|"..(record.model or "").."|"..tostring(record.type)
			local vanillaId = getProjectileVanillaMap()[key]
			if vanillaId then
				world.createObject(vanillaId, count):moveInto(player)
			end
			item:remove(count)
		else
	...
	soulPoints = soulPoints * S_RECOUPED_SOUL
	if soulPoints >= S_MINIMUM_RECOUPED_SOUL then
	...
	elseif preview then
		print("recouped soulgem too small ("..enchPoints.." * "..S_RECOUPED_SOUL.." = "..soulPoints..")")
	end
	if preview then
		print("-------")
	end
