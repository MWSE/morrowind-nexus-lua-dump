...
local function finishedConsuming(soulValue)
	local skillGain = soulValue^globalSection:get("EXPERIENCE_EXP") * globalSection:get("EXPERIENCE_MULT2") + globalSection:get("EXPERIENCE_ADD2")
	local skill = types.Player.stats.skills.enchant(self).base
	skillGain = skillGain * globalSection:get("CONSUME_MULT") * math.min(1,0.5+skill/200)
...
local function usedSoulgem(item) -- also called when ALLOW_SPELLMAKING
	if globalSection:get("ALLOW_REENCHANTING") then
		addSpells()
	end
end
...
local function UiModeChanged(data)
	...
	if data.oldMode == nil and data.newMode == "Dialogue" and data.arg then
		...
		if record then
			if record.servicesOffered.Spellmaking and globalSection:get("ALLOW_SPELLMAKING") then
				addSpells()
			end
			if record.servicesOffered.Enchanting and globalSection:get("ALLOW_REENCHANTING") then
				addSpells()
			end
		end
	end
...
local function finishedDisenchanting(data)
	...
	--ambient.playSound("sprigganmagic")
	local skill = types.Player.stats.skills.enchant(self).base
	local skillGain = (enchPoints^globalSection:get("EXPERIENCE_EXP") * globalSection:get("EXPERIENCE_MULT2") + globalSection:get("EXPERIENCE_ADD2") + wastedEffects) * math.min(1,0.5+skill/200)
