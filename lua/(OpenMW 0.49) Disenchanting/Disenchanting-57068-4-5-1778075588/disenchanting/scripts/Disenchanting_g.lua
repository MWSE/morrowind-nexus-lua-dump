...
local function scaleEnchantment(srcEnch, magMult)
	if magMult == 1 or not core.magic.enchantments.createRecordDraft then
		return srcEnch.id
	end
	-- copy effects with scaled magnitudes
	local effects = {}
	for _, e in ipairs(srcEnch.effects) do
		table.insert(effects, {
			...
			area              = e.area,
			duration          = e.duration,
			magnitudeMin      = math.max(0, math.floor(e.magnitudeMin * magMult + 0.5)),
			magnitudeMax      = math.max(0, math.floor(e.magnitudeMax * magMult + 0.5)),
		})
	end
	local draft = core.magic.enchantments.createRecordDraft({
		type       = srcEnch.type,
		charge     = srcEnch.charge,
		cost       = srcEnch.cost,
		isAutocalc = srcEnch.isAutocalc,
		effects    = effects,
	})
	local ok, newEnch = pcall(world.createRecord, draft)
	if not ok or not newEnch then
		print("disenchanting: failed to scale enchantment "..srcEnch.id..": "..tostring(newEnch))
		return srcEnch.id
	end
	return newEnch.id
end
...
local function disenchanting_multiplyPaper(data)
	...
	-- scale magnitudes for paper output if requested
	local magMult = S_PAPER_MAGNITUDE_MULT or 1
	if magMult ~= 1 then
		enchantment = scaleEnchantment(enchantmentRecord, magMult)
	end
