...
-- map projectile records by icon+model+type to a cheapest stackable vanilla recordId
local projectileVanilla = nil
local function getProjectileVanillaMap()
	if projectileVanilla then return projectileVanilla end
	projectileVanilla = {}
	local wTypes = types.Weapon.TYPE
	for _, rec in pairs(types.Weapon.records) do
		local rt = rec.type
		if (rt == wTypes.Bolt or rt == wTypes.Arrow or rt == wTypes.MarksmanThrown)
		and (not rec.enchant or rec.enchant == "")
		and rec.id:sub(1, #"Generated") ~= "Generated"
		and not rec.id:lower():find("_uni")
		then
			local key = (rec.icon or "").."|"..(rec.model or "").."|"..tostring(rt)
			local existing = projectileVanilla[key]
			if not existing or (rec.value or 0) < (types.Weapon.records[existing].value or 0) then
				projectileVanilla[key] = rec.id
			end
		end
	end
	return projectileVanilla
end

return function(item, preview, player)
	-- preview falsy = apply, true = log, other = silent
	local verbose = preview == true
	local enchantLevel = types.NPC.stats.skills.enchant(player).modified
	local ret = {}
	...
	local isBook = false
	local wTypes = types.Weapon.TYPE
	if types.Weapon.objectIsInstance(item) and (record.type == wTypes.Bolt or record.type == wTypes.Arrow or record.type == wTypes.MarksmanThrown) then
		isProjectile = true
		count = item.count
		if S_PROJECTILES_PER_CHUNK > 0 then
			count = math.min(count, S_PROJECTILES_PER_CHUNK)
		end
	elseif types.Book.objectIsInstance(item) and S_BOOKS_ENCHANT_MULTIPLIER > 0 then
		isBook = true
		count = item.count
	end
	local stackScale = 1
	if isProjectile then
		local mult = S_PROJECTILES_ENCHANT_MULTIPLIER
		stackScale = (mult > 0) and (count / mult) or count
	elseif isBook then
		stackScale = count * S_BOOKS_ENCHANT_MULTIPLIER
		if record.id:sub(1,#"Generated:") == "Generated:" then
			stackScale = stackScale / S_PAPER_MULT
		end
	end

	-- charge doesnt matter for arrows
	local maxCharges
	if isProjectile then
		maxCharges = getEnchantPoints(enchantmentRecord, false) * count
	elseif isBook then
		maxCharges = enchantmentRecord.charge * count
	else
		maxCharges = getMaxEnchantmentCharge(enchantmentRecord) * stackScale
	end
	ret.effects = {}
	...
	local enchPoints = getEnchantPoints(enchantmentRecord, false) * stackScale

	-- report received projectile
	if isProjectile then
		local key = (record.icon or "").."|"..(record.model or "").."|"..tostring(record.type)
		ret.receivedRecordId = getProjectileVanillaMap()[key]
		ret.count = count
	end

	local template = item.type.record(item)
	local oldCapacity = template.enchantCapacity/0.1*core.getGMST("FEnchantmentMult")
	local buggyApiBestMax = bestInSlot[tostring(item.type).."-"..(template.type or "")] or 99999999
	local bestMax = buggyApiBestMax / 0.1 * core.getGMST("FEnchantmentMult")
	
	if verbose and not isProjectile and not isBook then
		local previewNewCapacity = oldCapacity * S_CAPACITY_MULT + math.min(2*oldCapacity, enchPoints) * S_CAPACITY_FROM_ENCHANTMENT
		print("old capacity: ", f1(oldCapacity), "*"..S_CAPACITY_MULT)
		print("+ old enchantment magnitude: ", enchPoints > 2*oldCapacity and (f1(enchPoints).." -> clamped to "..f1(oldCapacity).."*2") or f1(enchPoints), "*"..S_CAPACITY_FROM_ENCHANTMENT)
		print("= "..f1(previewNewCapacity))
		if enchPoints * S_CAPACITY_CAP_FOR_TRASH > previewNewCapacity then
			previewNewCapacity = enchPoints * S_CAPACITY_CAP_FOR_TRASH
			print("too low, using "..f1(S_CAPACITY_CAP_FOR_TRASH*100).."% of enchantment magnitude instead: "..f1(previewNewCapacity))
		end
		if previewNewCapacity > bestMax * S_BEST_IN_SLOT_CAP then -- buggyApiCapacity
			local bestOtherItem = ""
			...
			print("higher than BiS enchantable: "..bestOtherItem.." has "..f1(bestMax).." (* "..S_BEST_IN_SLOT_CAP.." = "..f1(bestMax * S_BEST_IN_SLOT_CAP)..")")
		end
	...
	local newCapacity =  oldCapacity * S_CAPACITY_MULT + math.max(enchPoints * S_CAPACITY_CAP_FOR_TRASH, math.min(2*oldCapacity, enchPoints)) * S_CAPACITY_FROM_ENCHANTMENT
	
	newCapacity = math.min(newCapacity, bestMax * S_BEST_IN_SLOT_CAP)
	...
	if not isProjectile and not isBook then
		ret.value = template.value * S_VALUE_MULT
	end
	if not preview then
		if isProjectile then
			if ret.receivedRecordId then
				world.createObject(ret.receivedRecordId, count):moveInto(player)
			end
			item:remove(count)
		elseif isBook then
			item:remove(count)
			...
			local drainedName = item.type.record(item).name
			if S_PREPEND_DRAINED_TO_NAME then
				drainedName = "Drained "..drainedName
			end
			if S_APPEND_CAPACITY_TO_NAME then
				drainedName = drainedName.." ["..math.floor(newCapacity).."]"
			end
			local tbl = {name = drainedName,template = template, enchant = 1, value = ret.value, enchantCapacity = buggyApiCapacity}

			if S_VALUE_MULT > 0 then
				...
				player:sendEvent("disenchanting_rememberDrained", newRecord.id)
			end
			item:remove(1)
		end
	end
	
	local soulPoints = enchPoints
	if isBook then
		soulPoints = enchantmentRecord.charge * stackScale
	end
	if soulPoints > maxCharges and verbose then
	...
	soulPoints = soulPoints * (S_RECOUPED_SOUL + S_RECOUPED_SOUL_PER_LEVEL * enchantLevel)
	if soulPoints >= S_MINIMUM_RECOUPED_SOUL or preview == "quiet" then
		...
		local REQUIRE_EMPTY = S_REQUIRE_EMPTY_SOULGEM
	...
	elseif verbose then
		print("recouped soulgem too small ("..enchPoints.." * "..(S_RECOUPED_SOUL + S_RECOUPED_SOUL_PER_LEVEL * enchantLevel).." = "..soulPoints..")")
	end
	if verbose then
		print("-------")
	end
