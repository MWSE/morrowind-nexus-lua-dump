...
this.guids = {
    MenuDialog_TeaService = tes3ui.registerID("MenuDialog_service_TeaService")
}
...
local function getTeaCost(merchantObj)
    ...
    local personalityEffect = math.remap(personality, 0, 100, personalityMulti,
                                         1.0)
    -- do quest to get a lifetime tea merchants discount
    local discountApplied = tes3.getJournalIndex {
        id = "teaMerchants_golden_sedge"
    } >= 100 or tes3.getJournalIndex {id = "teaMerchants_scathecraw"} >= 100
    if discountApplied then return 10 end
...
local function fill(merchant, teaType, bottle)
    ...
    local bottleData =
        require("mer.ashfall.needs.thirstController").getBottleData(bottle)
    ...
    common.helper.fadeTimeOut(0.25, 2, function() end) -- 15 min has been passed 
    tes3.messageBox("A %s filled with %s has been added to your inventory.",
                    tes3.getObject(bottle).name,
...
local function teaSelectMenu()
    local merchant = merchantMenu.getDialogMenu():getPropertyObject(
                         "PartHyperText_actor")
    local buttons = {}
    ...
    for teaType, teaData in pairs(teaConfig.teaTypes) do
        if tes3.getItemCount({item = teaType, reference = merchant.reference}) ~=
            0 then
            table.insert(buttons, {
                ...
                callback = function()
                    ...
                    tes3.removeItem({
                        ...
                        item = teaType
                    }) -- brewing one bottle of tea actually consumes one ingredient
                end,
                tooltip = {
                    header = string.format("%s", teaData.teaName),
                    text = teaData.teaDescription
                }
            })
        end
    end
    tes3ui.showMessageMenu({
        message = "Select a type of tea",
        buttons = buttons,
        cancels = true
    })
end
...
local function makeTooltip()
    ...
    if getDisabled(cost) then labelText = "You do not have enough gold." end
...
local function updateTeaServiceButton(e)
    timer.frame.delayOneFrame(function()
        local menuDialog = merchantMenu.getDialogMenu()
        if not menuDialog then return end
        local teaServiceButton = menuDialog:findChild(this.guids
                                                          .MenuDialog_TeaService)
        local merchant = menuDialog:getPropertyObject("PartHyperText_actor")
        local cost = getTeaCost(merchant.object)
...
local function createTeaButton(menuDialog)
    ...
    local button = parent:createTextSelect{
        id = this.guids.MenuDialog_TeaService,
        text = getTeaMenuText(merchant)
    }
...
event.register("uiActivated", onMenuDialogActivated,
               {filter = "MenuDialog", priority = -99}) -- not sure what priority i should set to 
