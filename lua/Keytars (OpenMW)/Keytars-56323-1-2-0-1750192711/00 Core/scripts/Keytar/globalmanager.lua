...
local function updateBPM(data)
    storage.globalSection("Settings/Keytar/2_CustomMusic"):set("customMusicBpm", data.bpm)
    storage.globalSection("Settings/Keytar/2_CustomMusic"):set("customMusicLength", data.duration)
end
...
local function resyncAudio()
    ...
    realTimeOffset = (realTimeOffset + deltaTime) % configGlobal.customMusic.customMusicLength
    gameTimeOffset = (gameTimeOffset + deltaTime) % configGlobal.customMusic.customMusicLength
...
return {
    ...
    eventHandlers = {
        ...
        DetectBPMStart = detectBPM,
        DetectBPMComplete = updateBPM,
    }
}