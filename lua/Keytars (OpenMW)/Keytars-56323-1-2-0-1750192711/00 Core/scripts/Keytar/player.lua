...
local function handleAmbientCheck()
    if configGlobal.options.silenceAmbientMusic == true and not I.S3maphore then
        ...
        onActive = function()
            if I.S3maphore then
                I.S3maphore.registerPlaylist({
                    ...
                    priority = 1,
                    tracks = {
                        'sound/keytar/silence.opus',
                    },
                    interruptMode = I.S3maphore.INTERRUPT.Other,
                    isValidCallback = function()
                        return anyPlaying and configGlobal.options.silenceAmbientMusic == true
                    end,
                })
            end
        end
