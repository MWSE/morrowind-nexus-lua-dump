...
function K.handleMovement(dt, idleAnim)
    ...
    if K.wasMoving and not isMoving then
        ...
        K.idleTimer = 0.25
    elseif K.idleTimer and K.idleTimer > 0 then
        K.idleTimer = K.idleTimer - dt
        if K.idleTimer <= 0 then
            anim.cancel(self, idleAnim)
            K.idleTimer = nil
        end
    end

    K.wasMoving = isMoving
end
