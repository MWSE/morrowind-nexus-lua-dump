...
local function DeclarePoisonedWeapons(data)
    PoisonedWeapons[data.WeaponId]=data.PoisonRecordId
    for i, player in ipairs(world.players) do
        player:sendEvent("APtWDeclarePoisonedWeapons",{List=PoisonedWeapons})
    end
end
...
I.ItemUsage.addHandlerForType(types.Potion, function(potion, actor)
    if PoisonKeys[actor.id]==true then
        ...
        if CarriedRight then
            if CarriedRight.type==types.Weapon then -----------enlever bow/crossbow/throw weapons from the trigger
    ...
    if PoisonKeys[actor.id]==true then
        ...
        if CarriedRight then
            if CarriedRight.type==types.Weapon then
                core.sendGlobalEvent("APDeclarePoisonedWeapons",{WeaponId=CarriedRight.id,PoisonRecordId=ingredient.recordId})
...
return {
	eventHandlers = {APtWApplyPoison=ApplyPoison,APtWApplyPoisonKey=ApplyPoisonKey,APDeclarePoisonedWeapons=DeclarePoisonedWeapons,
					},
	engineHandlers = {
        onSave=onSave,
        onLoad=onLoad,
