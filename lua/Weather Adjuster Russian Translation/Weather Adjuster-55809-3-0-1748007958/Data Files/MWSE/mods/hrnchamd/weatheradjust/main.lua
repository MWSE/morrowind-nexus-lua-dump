...
cellName = region and region.id or "Нет"
if tes3.isLuaModActive("Pirate.CelDataModule") then
    -- ищем название текущей ячейки в базе ячеек загруженных из файлов cel
    for index, engName in ipairs(engNameCelData) do
        if engName == cellName then
            -- если найдено совпадение, заменяем название на русское.
            cellName = rusNameCelData[index]
            break  -- Прерываем цикл после первого совпадения.
        end
    end
end
-- конец блока перевода.
    local regionLabel = page:createLabel{ text = "Текущий регион: " .. cellName }
