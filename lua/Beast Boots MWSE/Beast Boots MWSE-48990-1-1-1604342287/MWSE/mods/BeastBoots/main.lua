...
local function onInitialized(e)
	...
	mwse.log("Beast Boots: v1.1 initialized.");
end
event.register("initialized", onInitialized);

local function isWearableBoot(item)
	if item.objectType == tes3.objectType.armor then
		...
		for _, wearablePart in pairs(item.parts) do
			...
			elseif (wearablePart.type ~= 255 and (wearablePart.male or wearablePart.female)) then    -- Check for another existing body part.
				hasOther = true
			end
			
			if hasInvalid and hasOther then     -- Allow equipping if the item has another existing body part.
				return true
			end
		end
	end
...
local function onLoad(e)
	for object in tes3.iterateObjects(tes3.objectType.armor) do
		if isWearableBoot(object) then
			for i, wearablePart in ipairs(object.parts) do
