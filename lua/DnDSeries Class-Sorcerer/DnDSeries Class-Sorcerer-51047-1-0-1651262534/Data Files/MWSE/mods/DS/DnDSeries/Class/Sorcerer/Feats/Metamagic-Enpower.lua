...
function this.metaMagic(spellId)
    ...
    local effects = spell.effects[i]
      if not (effects.min == 0 and effects.max == 0) then
          canEnpower = true
      else
         canEnpower = false
      end
    end
    if canEnpower == false then
     tes3.messageBox("this spell cant be prepare with Enpower metamagic")
    else
        local stringM = "[En]"
        local newId
        local k
        ...
        for i=1, #spell.effects do
         ...
         effect.min = newEffect.min*2
         effect.max = newEffect.max*2
        ...
        local conjuration = tes3.mobilePlayer.conjuration.current
        if conjuration > 90 then conjuration = 90 end
        local constant = math.round((100 - conjuration)/100, 2)
        local multiplier = 2.5*constant
