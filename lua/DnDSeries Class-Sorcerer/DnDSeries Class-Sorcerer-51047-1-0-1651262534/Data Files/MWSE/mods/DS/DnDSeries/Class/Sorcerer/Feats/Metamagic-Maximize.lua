...
function this.metaMagic(spellId)
    ...
    for i=1, spell:getActiveEffectCount() do
      ...
      if effects.min ~= effects.max or  not (effects.min == 0 and effects.max == 0) then
          canMaximize = true
      else
         canMaximize = false
      end
    ...
    if canMaximize == false then
     tes3.messageBox("this spell cant be prepare with Maximize metamagic")
    else
        local stringM = "[M]"
        local newId
        local k
        ...
        local destruction = tes3.mobilePlayer.destruction.current
        if destruction > 90 then destruction = 90 end
        local constant = math.round((100 - destruction)/100, 2)
        local multiplier = 2*constant
