...
local function recalculateDifficulty()
	...
	tes3.getWorldController().difficulty = 0.49 + (tes3.player.object.level * 0.01)  -- difficulty / 100 TT: hack it. Starts on hard difficulty and increments by 1 each level, even bypassing the vanilla cap. 
end
...
event.register("menuExit", recalculateDifficulty) -- TT: added this so that it is reset every menu exit
