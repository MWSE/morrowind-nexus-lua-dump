...
--    ui.showMessage("Clicked " .. data.props.text)
    ...
    core.sendGlobalEvent("ButtonClicked",{name = winName,text = data.props.text})
end
...
local function showMessageBox(fwinName, textLines, buttons)
    winName = fwinName
    if not buttons then
        buttons = { "OK" }
    end
    ...
    if winName and winName ~= "" then
        ...
        table.insert(content, renderListItem(winName, nil, true))
...
local function getMessageBox()
    return winCreated
end
return { getMessageBox = getMessageBox, showMessageBox = showMessageBox , closeMessageBox = closeMessageBox}
