...
local function processContainers(index)
    ...
    if (container and container.object) then
        ...
        local useActivation = (
    (gh and not gh.blacklist[cont.id:lower()])
    or (
        cont.script
        and cont.script.id
        and cont.script.id:sub(1, 6) == "MU_GH_"
    )
)

if useActivation then
    tes3.player:activate(container)
    harvestedRefs[container.id] = true
else
...
if (
    contDistance <= config.harvestDistance * 256
    and cont.organic
    and validScript
    and #cont.inventory > 0
    and tes3.getOwner(container) == nil
    and not harvestedRefs[container.id]
    and not config.AHContainerBL[cont.id:lower()]
) then
