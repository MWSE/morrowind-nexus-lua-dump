...
function myMenu.createWindow(qName)
    ...
    if (config.imageonoff) then
        ...
        local notificationImage = imageBlock:createImage{ path = iconpicker()}
...
local function journalValidate(e)
    ...
    elseif (tempqName == nil) then
        log:warn(e.topic:__tostring() .. " Doesnt have a quest name. Notification hidden")
        return false
    else
        return true
    end
...
function myMenu.display()
    ...
    if (qCursor <= qIndex) then
        ...
        if (myMenu.createWindow(qName)) then
            if (config.soundonoff) then
                tes3.playSound({
                    soundPath = "\\SSQN\\quest_update.wav",
