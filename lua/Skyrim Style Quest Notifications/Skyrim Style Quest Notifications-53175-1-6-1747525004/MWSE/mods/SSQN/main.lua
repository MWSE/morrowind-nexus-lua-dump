...
local function journalValidate(e)
    --makes sure the journal has a quest name, is a valid index, and is either a finish or new entry *NEW* also checks if quest is on blocklist
    local qList = getQListData()
    local tempqName = questName(e)
    --mwse.log("SSQN validate " .. e.topic:__tostring() .. " " .. iconlist[e.topic:__tostring()])
    if (e.info == nil) then
    ...
    elseif ((not e.info.isQuestFinished) and (qList[tempqName])) then
        ...
        return false
    elseif (iconlist[e.topic:__tostring()] == "BLOCKED") then
        return false
    else
        return true
    end
