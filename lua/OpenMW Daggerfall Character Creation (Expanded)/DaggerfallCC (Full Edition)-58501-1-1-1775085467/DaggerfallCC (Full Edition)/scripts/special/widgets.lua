...
function Scrollbar:update()
   ...
   if self.options.size <= (self.options.minSizeForVisibility or 1) then
   ...
   for i, line in ipairs(self.options.lines) do
      if i >= self.scrollbar.current then
         ...
         linesFlexContent:add {
            template = isSelected and templates.textHeader or templates.textNormal,
            events = {
               mouseClick = async:callback(function()
                  self.selected = i
                  self:update()
