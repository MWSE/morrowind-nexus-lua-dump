...
local function newSpecialModifiers()
...
local function newDefaultState()
  return {
    ...
    specialModifiers = newSpecialModifiers(),
    rng = { seed = 0, state = 0 },
    chargenSeen = false,
    playerRefId = nil,
...
local function ensureStateDefaults(state)
  ...
  state.specialModifiers = state.specialModifiers or newSpecialModifiers()
  state.specialModifiers.attributes = state.specialModifiers.attributes or {}
  state.specialModifiers.skills = state.specialModifiers.skills or {}
  ...
  if state.statsApplied == nil then
    -- Old saves from before this compatibility API existed.
    -- If they were already finalized, treat them as applied.
    state.statsApplied = state.finalized == true
  end
  if state.compatPendingNotified == nil then
    state.compatPendingNotified = false
  end
...
-- I.DaggerCC is exposed by the player script when the mod is installed/enabled.
-- DaggerCC_ChargenPending tells other player scripts which done event to wait for.
-- finalized is DaggerCC draft finalization; statsApplied is the player-local stat write.
-- DaggerCC_ChargenComplete only fires from player.lua after stats/skills are applied.
local function sendCompatPending(player, phase)
  if not player then
    return false
  end

  player:sendEvent('DaggerCC_ChargenPending', {
    apiVersion = 1,
    modId = 'DaggerCC',
...
local function notifyCompatPending(state, player, phase)
  if state.compatPendingNotified then
    return false
  end

  if sendCompatPending(player, phase) then
    state.compatPendingNotified = true
    return true
  end
...
local function beginFlow(state, player)
  ...
  notifyCompatPending(state, player, 'flow')
...
local function onStatsApplied(data)
  local state = loadState()

  if not state.finalized then
    log.debug('Ignoring DFMWChargen_StatsApplied before finalized state')
    return
  end

  if state.statsApplied then
    return
  end
  ...
  log.info('DaggerCC stat application acknowledged by player script')
end
...
local function onPlayerAdded(player)
  ...
  if state.startedFromNewGame == true
      and state.finalized ~= true
      and state.statsApplied ~= true
      and (state.chargenSeen == true or state.flowActive == true) then
    notifyCompatPending(state, player, 'playerAdded')
  end
...
local function onUpdate()
  ...
  if chargenFinished(player) then
    ...
    notifyCompatPending(state, player, 'vanillaComplete')
    saveState(state)
    player:sendEvent('OpenSpecialsChargen')
  end
end

local function onSpecialsComplete(data)
  local state = loadState()
  ...
  state.specialModifiers = copyTable((data and data.specialModifiers) or newSpecialModifiers())
  state.specialModifiers.attributes = state.specialModifiers.attributes or {}
  state.specialModifiers.skills = state.specialModifiers.skills or {}
...
return {
  ...
  eventHandlers = {
    ...
    DFMWChargen_StatsApplied = onStatsApplied,
