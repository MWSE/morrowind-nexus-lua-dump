...
local specialModifierSection = storage.playerSection('Specials_DaggerCC')
local statsAppliedThisSession = false
...
local COLOR_RED = util.color.rgb(0.80, 0.28, 0.22)
local COLOR_HEADER = util.color.rgb(0.92, 0.82, 0.55)

local NCG_MOD_ID = 'DaggerCC'
local NCG_REGISTRATION_EVENT = 'onChargenModRegistration'
local NCG_FINISHED_EVENT = 'onChargenModFinished'

local ncgRegistrationSent = false
local ncgFinishedSent = false

local function sendNCGConventionEvent(eventName)
  local payload = { modId = NCG_MOD_ID }

  -- NCG and related mods may listen either on the player object or globally.
  selfRef:sendEvent(eventName, payload)
  core.sendGlobalEvent(eventName, payload)
end

local function sendNCGRegistration()
  if ncgRegistrationSent then return end
  ncgRegistrationSent = true
  sendNCGConventionEvent(NCG_REGISTRATION_EVENT)
end

local function sendNCGFinished()
  if ncgFinishedSent then return end
  ncgFinishedSent = true
  sendNCGConventionEvent(NCG_FINISHED_EVENT)
end
...
-- I.DaggerCC means this mod is installed/enabled.
-- shouldWaitForDone() tells other chargen mods to defer finalization.
-- DaggerCC_ChargenPending tells player scripts which done event to wait for.
-- DaggerCC_ChargenComplete fires only after stats/skills are actually applied.
local function getDaggerState()
  return stateSection:getCopy('state') or {}
end

local function isChargenComplete()
  if statsAppliedThisSession then
    return true
  end

  local s = getDaggerState()

  if s.statsApplied ~= nil then
    return s.statsApplied == true
  end
...
local function isChargenActive()
  local s = getDaggerState()
  return s.flowActive == true
end

local function shouldWaitForDone()
  if isChargenComplete() then
    return false
  end

  local s = getDaggerState()

  -- Keep waiting through the finalized-but-not-applied handoff; finalized is
  -- DaggerCC draft completion, while statsApplied is the player-local write.
  return s.startedFromNewGame == true
    and s.statsApplied ~= true
    and (
      s.flowActive == true
      or s.chargenSeen == true
      or s.finalized == true
      or (s.currentPhase ~= nil and s.currentPhase ~= 'idle')
    )
end
...
local function specialDeltaColor(value)
  value = value or 0
  if value > 0 then return COLOR_GREEN end
  if value < 0 then return COLOR_RED end
  return COLOR_DIM
end

local function getSpecialDelta(kind, id)
  local specials = (state.draft and state.draft.specialModifiers) or {}
  local bucket = specials[kind] or {}
  local value = bucket[id]
  if value == nil then
    local stored = specialModifierSection:getCopy('specialModifiers') or {}
    value = ((stored[kind] or {})[id])
  end
...
local function attrRow(attribute)
  ...
  local special = getSpecialDelta('attributes', attribute)
  local total = base + special + roll + alloc

  return hrow({
    cell(dn(attribute), 120, COLOR_BRIGHT),
    cell(tostring(base), 45, COLOR_DIM),
    cell(signedDelta(special), 55, specialDeltaColor(special)),
    cell(tostring(roll), 45, COLOR_GREEN),
    cell(tostring(alloc), 45, alloc > 0 and COLOR_GOLD or COLOR_DIM),
    tinyBtn('-', function()
    ...
    cell(tostring(total), 50, COLOR_GOLD),
  })
end
...
local function renderAttributes()
  ...
  local rows = {
    ...
    spacer(6),
    hrow({
      cell('Attribute', 120, COLOR_DIM), cell('Base', 45, COLOR_DIM),
      cell('Other', 55, COLOR_DIM), cell('Roll', 45, COLOR_DIM),
      cell('Alloc', 45, COLOR_DIM),
      cell('', 28), cell('', 28),
      cell('Total', 50, COLOR_DIM),
    }),
    separator(),
  }
...
local function sRow(skillId)
  ...
  local special = getSpecialDelta('skills', skillId)
  local total = math.max(0, base + alloc + special)
  local cap = getSkillCap(skillId)
  local ac = alloc >= cap and COLOR_GOLD or (alloc > 0 and COLOR_GREEN or COLOR_DIM)

  return hrow({
    cell(dn(skillId), 150, COLOR_BRIGHT),
    cell(tostring(base), 45, COLOR_DIM),
    cell(signedDelta(special), 55, specialDeltaColor(special)),
    ...
    cell(tostring(total), 45, COLOR_GOLD),
  })
end

-- Compact row for misc skills, shorter to preserve the two-column layout.
local function sRowCompact(skillId)
  ...
  return hrow({
    cell(dn(skillId), 125, COLOR_BRIGHT),
    cell(tostring(base), 32, COLOR_DIM),
    cell(signedDelta(special), 36, specialDeltaColor(special)),
    cell(string.format('%d/%d', alloc, cap), 40, ac),
    tinyBtn('-', function()
      if (state.draft.skillDraft.allocations[skillId] or 0) > 0 then
        state.draft.skillDraft.allocations[skillId] = state.draft.skillDraft.allocations[skillId] - 1
        state.draft.skillDraft.poolRemaining = (state.draft.skillDraft.poolRemaining or 0) + 1
    ...
    cell(tostring(total), 36, COLOR_GOLD),
  })
end
...
local function renderSkills()
  ...
  local rows = {
    ...
    spacer(6),
    hrow({
      cell('Skill', 150, COLOR_DIM), cell('Base', 45, COLOR_DIM),
      cell('Other', 55, COLOR_DIM), cell('Alloc', 60, COLOR_DIM),
      cell('', 28), cell('', 28), cell('Total', 45, COLOR_DIM),
    }),
    separator(),
...
local function applyPendingSpecialsBeforeComplete()
  local ok, specials = pcall(function() return I.Specials end)
  if not ok or not specials or not specials.applyPendingForDaggerCC then
    return
  end

  local applyOk, err = pcall(specials.applyPendingForDaggerCC)
  if not applyOk then
    print('[DFMWChargen] Failed to apply pending Specials before chargen complete: ' .. tostring(err))
  end
end
...
local function applyPendingStats()
  ...
  applyPendingSpecialsBeforeComplete()
  statsAppliedThisSession = true
  core.sendGlobalEvent('DFMWChargen_StatsApplied', {
    apiVersion = 1,
    modId = 'DaggerCC',
    statsApplied = true,
  })
  selfRef:sendEvent('DaggerCC_ChargenComplete', {
    apiVersion = 1,
    modId = 'DaggerCC',
    statsApplied = true,
  })
...
return {
  interfaceName = 'DaggerCC',
  interface = {
    apiVersion = 1,
    modId = 'DaggerCC',
    pendingEvent = 'DaggerCC_ChargenPending',
    doneEvent = 'DaggerCC_ChargenComplete',
    isChargenComplete = isChargenComplete,
    isChargenActive = isChargenActive,
    shouldWaitForDone = shouldWaitForDone,
  },
