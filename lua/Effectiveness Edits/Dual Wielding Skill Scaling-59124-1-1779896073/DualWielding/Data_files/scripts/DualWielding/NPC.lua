...
-- ============================================================
-- Off-hand skill scaling helper (added by mechanics extension)
-- Returns multipliers for swing speed, damage, and fatigue cost
-- based on the relevant one-handed weapon skill of the actor.
-- Applies only to player actors so the player's settings can be
-- read; NPCs/creatures keep the original fixed mod behavior.
-- ============================================================
local DW_DEFAULT_SCALARS = { speed = 0.5, damage = 1.0, fatigue = 0.1 }

local function _readScalingSetting(section, key, default)
...
local function offhandScalars(weapon, actor)
  if not (weapon and actor) then return DW_DEFAULT_SCALARS end
  if not types.Player.objectIsInstance(actor) then return DW_DEFAULT_SCALARS end

  local ok, section = pcall(storage.playerSection, 'DualWieldingscaling')
  if not ok or not section then return DW_DEFAULT_SCALARS end

  if _readScalingSetting(section, 'EnableOffhandScaling', true) == false then
    return DW_DEFAULT_SCALARS
  end

  local wtype = types.Weapon.records[weapon.recordId].type
  local skillId = (wtype == types.Weapon.TYPE.ShortBladeOneHand) and 'shortblade'
              or (wtype == types.Weapon.TYPE.LongBladeOneHand)  and 'longblade'
              or (wtype == types.Weapon.TYPE.AxeOneHand)        and 'axe'
              or (wtype == types.Weapon.TYPE.BluntOneHand)      and 'bluntweapon'
              or nil

  local t = 0
  if skillId then
    local s = types.NPC.stats.skills[skillId](actor).modified or 0
    t = math.max(0, math.min(1, s / 100))
  end

  local function lerp(a, b) return a + (b - a) * t end

  local speed   = DW_DEFAULT_SCALARS.speed
  local damage  = DW_DEFAULT_SCALARS.damage
  local fatigue = DW_DEFAULT_SCALARS.fatigue

  if _readScalingSetting(section, 'EnableOffhandSpeedScaling', true) ~= false then
    speed = lerp(
      tonumber(_readScalingSetting(section, 'OffhandSpeedNovice', 0.5)) or 0.5,
      tonumber(_readScalingSetting(section, 'OffhandSpeedMaster', 1.0)) or 1.0
    )
  end

  if _readScalingSetting(section, 'EnableOffhandDamageScaling', true) ~= false then
    damage = lerp(
      tonumber(_readScalingSetting(section, 'OffhandDamageNovice', 0.6)) or 0.6,
      tonumber(_readScalingSetting(section, 'OffhandDamageMaster', 1.0)) or 1.0
    )
  end

  if _readScalingSetting(section, 'EnableOffhandFatigueScaling', true) ~= false then
    fatigue = lerp(
      tonumber(_readScalingSetting(section, 'OffhandFatigueNovice', 0.5)) or 0.5,
      tonumber(_readScalingSetting(section, 'OffhandFatigueMaster', 0.1)) or 0.1
    )
  end

  return { speed = speed, damage = damage, fatigue = fatigue }
end
...
local function Damage(Weapon,AttackType,Actor)
  ...
  rawDamage = rawDamage * offhandScalars(Weapon, Actor).damage
  return(rawDamage)
end  
...
--    print(self.recordId,group,key, LastAttack)----------------------------------------------
    ...
    if group=="weapononehand" then
      ...
      elseif LastAttack=="RightStop" then
        ...
        AttackFatigueLoss(SecondWeapon, offhandScalars(SecondWeapon, self).fatigue)
...
I.AnimationController.addPlayBlendedAnimationHandler(function (groupname, options)
  if SecondWeapon then
    if groupname=="weaponleftslash" or groupname=="weaponleftchop" or groupname=="weaponleftthrust" then
      options.speed = types.Weapon.records[SecondWeapon.recordId].speed * offhandScalars(SecondWeapon, self).speed
    end
  end
end)
