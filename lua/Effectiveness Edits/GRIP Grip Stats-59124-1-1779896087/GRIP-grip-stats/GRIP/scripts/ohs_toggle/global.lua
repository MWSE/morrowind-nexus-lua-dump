...
-- New: grip-stat modifiers (speed/reach). Defaults are no-ops until menu.lua broadcasts.
local EnableGripStats                                               = true
local OneHandSpeedMult, TwoHandSpeedMult                            = 1.0, 1.0
local OneHandReachMult, TwoHandReachMult                            = 1.0, 1.0
...
local function variantUsesCurrentDamage(baseRec, variantRec, is2H)
  ...
  -- New: also revalidate speed/reach. When grip stats are disabled, the
  -- variant should match the base values; when enabled, it should match
  -- base * configured multiplier. If the user toggles or retunes, we want
  -- to detect the mismatch and regenerate the record on the next swap.
  local function closeEnough(a, b)
    return math.abs((a or 0) - (b or 0)) <= 0.001
  end

  local expectedSpeed, expectedReach
  if EnableGripStats then
    local speedMult = is2H and TwoHandSpeedMult or OneHandSpeedMult
    local reachMult = is2H and TwoHandReachMult or OneHandReachMult
    expectedSpeed = (baseRec.speed or 1.0) * speedMult
    expectedReach = (baseRec.reach or 1.0) * reachMult
  else
    expectedSpeed = baseRec.speed
    expectedReach = baseRec.reach
  end

  if baseRec.speed and not closeEnough(variantRec.speed, expectedSpeed) then return false end
  if baseRec.reach and not closeEnough(variantRec.reach, expectedReach) then return false end
...
local function ensureWeaponVariant(baseRec, is2H)
  ...
  -- New: also scale swing speed and reach when grip stats are enabled.
  -- This is what makes 1H vs 2H feel different beyond raw damage.
  if EnableGripStats then
    local speedMult = is2H and TwoHandSpeedMult or OneHandSpeedMult
    local reachMult = is2H and TwoHandReachMult or OneHandReachMult
    if baseRec.speed then draft.speed = baseRec.speed * speedMult end
    if baseRec.reach then draft.reach = baseRec.reach * reachMult end
  end
...
return {
  ...
  eventHandlers = {
    ...
    OHS_UpdateGlobalData = function(data)
      ...
      -- New: grip-stat modifiers
      if data.EnableGripStats ~= nil then EnableGripStats = data.EnableGripStats end
      OneHandSpeedMult = tonumber(data.OneHandSpeedMult) or OneHandSpeedMult
      TwoHandSpeedMult = tonumber(data.TwoHandSpeedMult) or TwoHandSpeedMult
      OneHandReachMult = tonumber(data.OneHandReachMult) or OneHandReachMult
      TwoHandReachMult = tonumber(data.TwoHandReachMult) or TwoHandReachMult
    end,
  },
}
