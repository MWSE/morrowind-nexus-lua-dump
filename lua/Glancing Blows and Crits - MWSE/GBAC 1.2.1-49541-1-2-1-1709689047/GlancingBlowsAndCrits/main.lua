...
local equippedWeaponStack_enchant
local CritMod
local GlanceMod
local f
...
if config.enabled then
    ...
    if (e.hitChance < RolltoHit) then
        ...
        -- crit condition
        if ((CritChance >= RolltoCrit) and config.critEnabled) then
            HitType = 2
        else
            HitType = 1
        end
...
local function SetDamage(e)
    ...
    -- glance damage
    -- sound must also be played here and not in SoundChange to avoid buggy sound effects
    if (HitType == 0) then
        e.damage = GlanceMod*e.damage*f
        tes3.playSound{sound = "Hand To Hand Hit 2", reference = e.reference}     -- hit damage
...
local function CastOnGlance(e)
	if (not config.castOnGlance) then

		--check if the equipped weapon have an enchantment
		equippedWeaponStack_enchant = tes3.getEquippedItem{actor = e.caster, enchanted = true, objectType = tes3.objectType.weapon}

		-- cancel cast-on-strike when glance
		if (equippedWeaponStack_enchant and HitType == 0) then
			e.charge = e.itemData.charge+1 -- spams a messageBox that the weapon is out of charge 
        end
	end
end

-- remove the regular sound effect for hitting when glancing
-- e = damaged
local function SoundChange(e)
    if (HitType == 0) then
        -- sound must be removed during the damaged event, not the damage event, otherwise it causes a bug.
...
local function initialized()
	...
	-- change the cast on strike enchant hit chance
	event.register("enchantChargeUse", CastOnGlance)
    ...
    print("[GBAC 1.2] Glancing Blows and Crits initialized")
...
event.register("modConfigReady", function()
	require("GlancingBlowsAndCrits.mcm")
end)
