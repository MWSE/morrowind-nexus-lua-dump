...
-- local function Kill(e)
	-- if config.KillXP then
		...
		-- for _, effect in pairs(tes3.mobilePlayer:getActiveMagicEffects{}) do
			-- if effect.isSummon then
				-- print('22 '..tostring(e.mobile.object.id))
			-- end
		-- end
		-- if not string.find(ref.object.id, '[Ss]ummon') then
		-- print('33 '..tostring(ref.object.id))
		-- end
		-- if ref and ref.object and (ref.object.objectType == tes3.objectType.creature or ref.object.objectType == tes3.objectType.npc) then
			-- if ref.mobile and ref.mobile.health.current ~= nil then
				-- if ref.mobile.health.current <= 0 and not string.find(ref.object.id, '[Ss]ummon') then
					-- Experience = math.round(ref.mobile.health.base / config.KillRate * ref.mobile.object.level, 2)
				-- else
					-- if ref.mobile.health.current > 0 and ref.mobile.isDead and not string.find(ref.object.id, '[Ss]ummon') then
						-- Experience = math.round(ref.mobile.health.base / config.KillRate * ref.mobile.object.level, 2)
					-- end
				-- end
				-- data.MXPS_xp = data.MXPS_xp + Experience
...
local function onDamaged(e)
	if config.KillXP then
		local attacker = e.attackerReference
		local killingBlow = e.killingBlow
		local XPAdd = 0.0
		local PlayerPlusFallovers = {}
		for mobile in tes3.iterate(tes3.mobilePlayer.friendlyActors) do 
			PlayerPlusFallovers[mobile.reference] = true
		end	
		if config.KillXPSource == 0 then
			if attacker then
				if killingBlow == true then
					if attacker == tes3.mobilePlayer.reference and not PlayerPlusFallovers[e.mobile.reference] then 
						local data = tes3.getPlayerRef().data
						XPAdd = math.round(e.mobile.health.base / config.KillRate * e.mobile.object.level, 2)
						data.MXPS_xp = data.MXPS_xp + XPAdd
						if config.KillMsg then
							tes3.messageBox({message = i18n('KillMsg',{name = e.mobile.object.name, lvl = e.mobile.object.level, count = XPAdd, }), duration = 10})
						end	
					end
				end
			end
		elseif config.KillXPSource == 1 then
			if attacker then
				if killingBlow == true then
					if PlayerPlusFallovers[attacker] and not PlayerPlusFallovers[e.mobile.reference] and attacker.object.objectType == tes3.objectType.npc then 
						local data = tes3.getPlayerRef().data
						XPAdd = math.round(e.mobile.health.base / config.KillRate * e.mobile.object.level, 2)
						data.MXPS_xp = data.MXPS_xp + XPAdd
						if config.KillMsg then
		...
		elseif config.KillXPSource == 2 then
			if attacker then
				if killingBlow == true then
					if PlayerPlusFallovers[attacker] and not PlayerPlusFallovers[e.mobile.reference] then 
						local data = tes3.getPlayerRef().data
						XPAdd = math.round(e.mobile.health.base / config.KillRate * e.mobile.object.level, 2)
						data.MXPS_xp = data.MXPS_xp + XPAdd
						if config.KillMsg then
		...
		elseif config.KillXPSource == 3 then
