		...
		if ref and ref.object and (ref.object.objectType == tes3.objectType.creature or ref.object.objectType == tes3.objectType.npc) then
			if ref.mobile and ref.mobile.health.current ~= nil then
				if ref.mobile.health.current <= 0 and string.find(ref.id, '[Ss]ummon') == nil then
					Experience = math.round(ref.mobile.health.base / config.KillRate * ref.mobile.object.level, 4)
				else
...
function CButton(p1,p2,p3,p4,p5)
	local player = tes3.getPlayerRef()	
	local p1 = p2:createButton{text = tes3.findGMST(p3).value..' '..tostring(player.mobile:getSkillStatistic(p4).base)..' ['..GetSkillStr(p4)..', '..GetAttStr(p4)..'] ('..i18n('XPNeed')..': '..tostring(math.round(p5,0))..')'}
	return p1
end
...
function MouseReg(p1,p2,p3,p4,p5)
	...
	function(e) 						
		...
		if data.MXPS_xp >= PR then
			...
			p5.text = i18n('XP')..': '..tostring(math.round(data.MXPS_xp,0))
			player.mobile:progressSkillToNextLevel(SK)
			PR = player.mobile:getSkillProgressRequirement(SK)
			BU.text = tes3.findGMST(sSK).value..' '..tostring(player.mobile:getSkillStatistic(SK).base)..' ['..GetSkillStr(SK)..', '..GetAttStr(SK)..'] ('..i18n('XPNeed')..': '..tostring(math.round(PR,0))..')'
		end
	end)
end
...
function this.createWindow()
	...
	local label_xp = menu:createLabel{id = this.id_xp, text = i18n('XPTotal')..': '..tostring(math.round(data.MXPS_xp,0))}
