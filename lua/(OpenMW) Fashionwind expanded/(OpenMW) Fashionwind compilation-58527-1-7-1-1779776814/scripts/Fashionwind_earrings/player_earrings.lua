...
local BUFF_SPELLS    = { BEAUT_SPELL_ID }

local VFX_ID   = 'visibleEarrings'
local VFX_BONE = 'head'


local function applyEarringsBuffs()
...
local function removeEarringsBuffs()
...
local function getActiveRecordId()  return FWearringsState:get('activeRecordId') end
local function setActiveRecordId(v) FWearringsState:set('activeRecordId', v) end
...
local function applyVfxForRecordId(recId)
    ...
    if not EheadIsOurs() then return end
...
local function onRemoveEarrings()
    setActiveRecordId(nil)
    removeCurrentVfx()
end
...
local function buildDialog(item)
    ...
    local rows = {
        makeBtn(
            alreadyOn and 'Re-equip earrings' or 'Equip earrings',
    ...
    if alreadyOn then
        ...
        rows[#rows + 1] = makeBtn(
            ...
            function() closeDialog(); onRemoveEarrings() end
        )
    end
    rows[#rows + 1] = spacer
    rows[#rows + 1] = makeBtn(
...
local function onEarringsPrompt(data)
...
return {
    ...
    eventHandlers = {
        Earrings_PromptEquip = onEarringsPrompt,
