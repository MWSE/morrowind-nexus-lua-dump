...
local circletState     = storage.playerSection('Settings_tt_headgear_state')


local BUFF_SPELLS = { 'fx_beautyc' }

local function applyCircletBuffs()
...
local function removeCircletBuffs()
    for _, spellId in ipairs(BUFF_SPELLS) do
...
local function claimGHeadBone() GheadBoneOwner:set('owner', 'headgear') end
...
local function getActiveRecordId()  return circletState:get('activeRecordId') end
local function setActiveRecordId(v) circletState:set('activeRecordId', v) end
...
local function applyVfxForRecordId(recId)
    ...
    if not GheadIsOurs() then return end
...
local function onRemoveCirclet()
    setActiveRecordId(nil)
    removeCurrentVfx()
end
...
local function buildDialog(item)
    ...
    local rows = {
        makeBtn(
            alreadyOn and 'Re-equip circlet' or 'Equip circlet',
    ...
    if alreadyOn then
        ...
        rows[#rows + 1] = makeBtn(
            ...
            function() closeDialog(); onRemoveCirclet() end
        )
    end
    rows[#rows + 1] = spacer
    rows[#rows + 1] = makeBtn(
...
local function onCircletPrompt(data)
...
return {
    ...
    eventHandlers = {
        Circlets_PromptEquip = onCircletPrompt,
