...
local function removeMaskBuffs()
    for _, spellId in ipairs(BUFF_SPELLS) do
...
local function getActiveRecordId()  return masksState:get('activeRecordId') end
local function setActiveRecordId(v) masksState:set('activeRecordId', v) end
...
local function buildDialog(item)
    ...
    local rows = {
        makeBtn(
            alreadyOn and 'Re-equip mask' or 'Equip mask',
    ...
    if alreadyOn then
        ...
        rows[#rows + 1] = makeBtn(
            ...
            function() closeDialog(); onRemoveMask() end
        )
    end
    rows[#rows + 1] = spacer
    rows[#rows + 1] = makeBtn(
...
local function onMasksPrompt(data)
...
return {
    ...
    eventHandlers = {
        Masks_PromptEquip = onMasksPrompt,
