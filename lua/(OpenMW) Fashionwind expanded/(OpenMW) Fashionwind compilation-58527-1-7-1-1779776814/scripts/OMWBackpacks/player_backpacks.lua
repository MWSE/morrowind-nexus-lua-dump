...
local BboneBoneOwner = storage.playerSection('Settings_tt_backpacks')
local FWbackpacksState = storage.playerSection('Settings_tt_backpacks_state')

local BACK_SPELL_ID = 'fx_Backpack'
local BUFF_SPELLS   = { BACK_SPELL_ID }

local VFX_ID   = 'visibleBackpacks'
local VFX_BONE = 'Backpack1'


local function applyBackpackBuffs()
    for _, spellId in ipairs(BUFF_SPELLS) do Actor.spells(self):add(spellId) end
end

local function removeBackpackBuffs()
    for _, spellId in ipairs(BUFF_SPELLS) do Actor.spells(self):remove(spellId) end
end

local function claimBBone() BboneBoneOwner:set('owner', 'backpacks') end
local function BboneIsOurs()
    local owner = BboneBoneOwner:get('owner')
    return owner == nil or owner == 'backpacks'
end

local function getActiveRecordId()  return FWbackpacksState:get('activeRecordId') end
local function setActiveRecordId(v) FWbackpacksState:set('activeRecordId', v) end
...
local function applyVfxForRecordId(recId)
    ...
    if not BboneIsOurs() then return end
...
local function onRemoveBackpack()
    setActiveRecordId(nil)
    removeCurrentVfx()
end
...
local function buildDialog(item)
    ...
    local rows = {
        makeBtn(
            alreadyOn and 'Re-equip backpack' or 'Equip backpack',
    ...
    if alreadyOn then
        ...
        rows[#rows + 1] = makeBtn(
            ...
            function() closeDialog(); onRemoveBackpack() end
        )
    end
    rows[#rows + 1] = spacer
    rows[#rows + 1] = makeBtn(
...
local function onBackpackPrompt(data)
...
return {
    ...
    eventHandlers = {
        Backpacks_PromptEquip = onBackpackPrompt,
