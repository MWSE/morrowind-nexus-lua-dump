...
local scarfSettings  = storage.playerSection('Settings_tt_fashionwind_scarves_ui')
local scarfState     = storage.playerSection('scarves_cycle')  



local BUFF_SPELLS = { 'fx_SCfrostshield' }

local function applyScarfBuffs()
    if not scarfSettings:get('SKARFBUFFS') then return end
    for _, spellId in ipairs(BUFF_SPELLS) do
        Actor.spells(self):add(spellId)
    end
end

local function removeScarfBuffs()
    for _, spellId in ipairs(BUFF_SPELLS) do
...
local function getActiveRecordId()  return scarfState:get('activeRecordId') end
local function setActiveRecordId(v) scarfState:set('activeRecordId', v) end
...
local function buildDialog(item)
    ...
    local rows = {
        makeBtn(
            alreadyOn and 'Re-equip scarf' or 'Equip scarf',
    ...
    if alreadyOn then
        ...
        rows[#rows + 1] = makeBtn(
            ...
            function() closeDialog(); onRemoveScarf() end
        )
    end
    rows[#rows + 1] = spacer
    rows[#rows + 1] = makeBtn(
...
local function onScarfPrompt(data)
...
return {
    ...
    eventHandlers = {
        Scarves_PromptEquip = onScarfPrompt,
        vfxRemoveAll = function()
            refreshCounter = math.random(3, 8)  
            refreshPending = true
        end,
