...
local function applyHornsBuffs()
...
local function removeHornsBuffs()
...
local function getActiveRecordId()  return FWhornsState:get('activeRecordId') end
local function setActiveRecordId(v) FWhornsState:set('activeRecordId', v) end
...
local function applyVfxForRecordId(recId)
    ...
    if not CheadIsOurs() then return end
...
local function buildDialog(item)
    ...
    local rows = {
        makeBtn(
            alreadyOn and 'Re-equip horns' or 'Equip horns',
    ...
    if alreadyOn then
        ...
        rows[#rows + 1] = makeBtn(
            ...
            function() closeDialog(); onRemoveHorns() end
        )
    end
    rows[#rows + 1] = spacer
    rows[#rows + 1] = makeBtn(
...
local function onHornsPrompt(data)
...
return {
    ...
    eventHandlers = {
        Horns_PromptEquip = onHornsPrompt,
