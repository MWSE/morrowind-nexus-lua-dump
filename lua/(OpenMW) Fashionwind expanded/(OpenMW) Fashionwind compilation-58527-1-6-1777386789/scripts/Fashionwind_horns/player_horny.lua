...
local function scanInv(reset)
  ...
  if vfxActive then
      ...
      if hornsSettings:get('HORBUFFS') then
          applyHornsBuffs(self)
      end
  else
      removeHornsBuffs(self)
  end
end
...
local function onFrame(dt)
  ...
  local horVarKeyDown = input.getBooleanActionValue('HORShow')
  ...
  local keyDown = input.getBooleanActionValue('HORChange')
...
local function onActive()
  showhorns = FWhornsState:get('showhorns') or false
  counter = 3
  refresh = true
  lastSkins = nil
end
