local cf = mwse.loadConfig("BattleMage", {KEY = {keyCode = 44}, mc = 150, cast = 50})		local W = {}
local p, mp, ad, D, Bar, MMana, B, S
...
local function MOBILEACTIVATED(e) local m = e.mobile	if m and m.firingMobile == mp then	local si = m.spellInstance
	if si and si.source == W.en then timer.delayOneFrame(function() e.reference.position = W.hitp end) end
end end		event.register("mobileActivated", MOBILEACTIVATED)


local function PROJECTILEEXPIRE(e) if e.firingReference == p then local m = e.mobile	if m.objectType == 0 then	local fw = m.firingWeapon	local en = fw.enchantment
	if en and en.castType == 1 and en.effects[1].rangeType == 2 and fw.type < 11 and fw == mp.readiedWeapon.object then
		W.hitp = m.reference.position:copy()	W.en = en	tes3.applyMagicSource{reference = p, source = en, fromStack = mp.readiedWeapon}
	end
...
local function loaded(e)	p = tes3.player		mp = tes3.mobilePlayer		ad = mp.actionData		D = p.data		W = {}
