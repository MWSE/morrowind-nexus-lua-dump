...
local function init()
    ...
    sb_achievements.registerAchievement {
        ...
        condition = function()
            ...
            if (myData["VivecLessonsCount"] >= 36) then
    ...
    sb_achievements.registerAchievement {
        ...
        condition = function()
            local myData = aData.getData()
            if (myData["AncestralTombCount"] >= 88) then
                event.unregister(tes3.event.cellChanged, aData.countTomb)
                return true
            end
        end,
    ...
    sb_achievements.registerAchievement {
        ...
        condition = function()
            local myData = aData.getData()
            if (myData["mineCount"] >= 43) then
                event.unregister(tes3.event.cellChanged, aData.countMine)
                return true
            end
        end,
