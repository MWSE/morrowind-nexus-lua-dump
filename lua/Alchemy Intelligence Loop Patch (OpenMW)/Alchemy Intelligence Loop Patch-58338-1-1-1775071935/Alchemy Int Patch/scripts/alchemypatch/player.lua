...
local FALLBACK_MODE_ALCHEMY = 'Alchemy'
local FALLBACK_WINDOW_ALCHEMY = 'Alchemy'

local function getUI()
  return baseUI or (I and I.UI)
end

local function getAlchemyModeName(ui)
  if ui and type(ui.MODE) == 'table' and type(ui.MODE.Alchemy) == 'string' then
    return ui.MODE.Alchemy
  end
  return FALLBACK_MODE_ALCHEMY
end

local function getAlchemyWindowName(ui)
  if ui and type(ui.WINDOW) == 'table' and type(ui.WINDOW.Alchemy) == 'string' then
    return ui.WINDOW.Alchemy
  end
  return FALLBACK_WINDOW_ALCHEMY
end

local function isAlchemyMode(mode, ui)
  return type(mode) == 'string' and mode == getAlchemyModeName(ui)
end

local function optionsContainAlchemyWindow(options, ui)
  if type(options) ~= 'table' then
    return false
  end
  local windows = options.windows
  local alchemyWindow = getAlchemyWindowName(ui)
  if type(windows) == 'table' then
    for i = 1, #windows do
      if windows[i] == alchemyWindow then
        return true
      end
    end
  end
...
local function isAlchemyVisible()
  ...
  if type(ui.isWindowVisible) == 'function' then
    local ok, visible = pcall(ui.isWindowVisible, getAlchemyWindowName(ui))
  ...
  if type(ui.getMode) == 'function' then
    ...
    if ok and isAlchemyMode(mode, ui) then
      return true
    end
  end
  return false
...
UIWrapper.setMode = function(mode, options)
  local ui = getUI()
  if isAlchemyMode(mode, ui) or optionsContainAlchemyWindow(options, ui) then
    ensureClamped()
    if DEBUG then
      local intStat = getIntelligenceStat()
