...
local function hackTooltip(e)
    ...
    for name in table.traverse(t.children) do
		...
		if name.name == "HelpMenu_name" then
			if daname[2] and #daname[2] > 0 then
                ...
                if info then
                    for w in info:gmatch("[^\n]*%S+") do
                        ...
                        local color
                        if ci and cf then
							for ic in w:sub(ci + 1, cf - 1):gmatch("%w+%$?") do
								local last = false
								if ic:match("%$$") then last = true end
								if icons[ic] or last then
								table.insert(icongroup, ic)
								end
							end
							if #icongroup > 0 then
								w = w:sub(1, ci - 1)
                        ...
                        b.autoHeight = true
                        b.autoWidth = true
						b.flowDirection = "left_to_right"
						if #icongroup > 0 then
						for i = 1, #icongroup do
						if not icongroup[i]:match("%$$") then
						local ico = b:createImage{path = "Icons/l/Light_De_Lantern_07.dds"}
						ico.contentPath = icons[icongroup[i]]
						ico.width = 18
						ico.height = 18
						ico.scaleMode = true
						ico.borderAllSides = 2
						end
						end
						end
						if w and #w > 0 then
						...
						if #icongroup > 0 then
						for i = 1, #icongroup do
						if icongroup[i]:match("%$$") then
						local ico2 = b:createImage{path = "Icons/l/Light_De_Lantern_07.dds"}
						ico2.contentPath = icons[icongroup[i]:sub(1,#icongroup[i]-1)]
						ico2.width = 18
						ico2.height = 18
						ico2.scaleMode = true
						ico2.borderAllSides = 2
						end
						end
						end
                        if e.object and e.object.objectType and not junk[(e.object.objectType)] then
...
local function getText()
    colorTable = require("kindi.simple renamer and coloring.colors")
	icons = require("kindi.simple renamer and coloring.icons")
