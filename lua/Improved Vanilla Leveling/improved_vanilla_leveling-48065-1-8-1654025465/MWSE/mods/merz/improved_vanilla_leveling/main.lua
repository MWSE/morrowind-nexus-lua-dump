...
local function UpdateHealth()
    if config.retroactive_health then
        ...
        -- Include Fortify Health effects if the MCP patch is enabled.
        -- Without the patch, Fortify Health is effectively temporary health;
        -- it does not increase maximum health and thus need not be included in
        -- the new health calculation.
        if has_fortify_max then
            for _, effect in pairs(mp:getActiveMagicEffects({effect = tes3.effect.fortifyHealth})) do
                health = health + effect.effectInstance.effectiveMagnitude
            end
        end
        health = math.round(health * 10) / 10 -- Round to the nearest 0.1
        -- How much health have we just gained? This should always be >= 0,
        -- unless another mod is also modifying health.
        local health_increase = health - mp.health.base
        -- Update max health to the new calculated value.
        tes3.setStatistic({ reference = mp, name = 'health', base = health })
        -- If max health has increased, increase current health by the same
        -- amount. If not, don't change current health, but print a warning.
        -- This could happen if other mods are also modifying health values.
        if health_increase > 0 then
            tes3.modStatistic({ reference = mp, name = 'health', current = health_increase })
        elseif health_increase < 0 then
            mwse.log(mod .. 'Warning: max health decreased from %f to %f.', health - health_increase, health)
        end
    end
end
...
local function CalcBaseStats()
    ...
    if mp.birthsign then
        ...
        for _, spell in pairs(mp.birthsign.spells) do
...
local function OnMenuStatLevelTooltip(e)
    ...
    CheckForLpaIncrease()
    -- Build the strings describing attribute potential and progress.
    local attribute_info_text = {
        {'Attribute', 'C/M', 'P'}
    }
    local mp = tes3.mobilePlayer
    for i, attribute in ipairs(mp.attributes) do
        local name = tes3.attributeName[i - 1]:gsub('^(%l)', string.upper)
        local cap = save_data.attribute_caps[i] + attribute.base
        local cur_cap = string.format('%d/%d', attribute.base, cap)
        local level_ups = 0
        if HasAssociatedSkill(i - 1) then
        ...
        level_ups = string.format(' %01.1f', level_ups)
        table.insert(attribute_info_text, {name, cur_cap, level_ups})
    end
    -- Layout tooltip.
    -- TODO: is there a better way to lay this out than absolute alignment?
    for i, entry in ipairs(attribute_info_text) do
        local block = layout:createBlock()
        block.flowDirection = tes3.flowDirection.leftToRight
        block.autoHeight = true
        block.autoWidth = true
        block.widthProportional = 1.0
        block.heightProportional = 1.0
        local label = block:createLabel({text = entry[1]})
        label.absolutePosAlignX = 0.0
        label = block:createLabel({text = entry[2]})
        label.absolutePosAlignX = 0.65
        label = block:createLabel({text = entry[3]})
        label.absolutePosAlignX = 0.90
        -- Only create a divider after the header.
        if i == 1 then
...
local function OnInitialized()
    ...
    if (iLevelupMajorMultAttribute ~= 1 or iLevelupMinorMultAttribute ~= 1 or iLevelupMajorMult ~= 1 or
     iLevelupMinorMult ~= 1) and (gmst_fix == nil or not gmst_fix.is_patched) then
        ...
        UpdateGMSTCache()
        is_capped = not tes3.hasCodePatchFeature(tes3.codePatchFeature.attributeUncap)
        has_tooltip = tes3.hasCodePatchFeature(tes3.codePatchFeature.levelupSkillsTooltip)
        has_fortify_max = tes3.hasCodePatchFeature(tes3.codePatchFeature.fortifyMaximumHealth)
