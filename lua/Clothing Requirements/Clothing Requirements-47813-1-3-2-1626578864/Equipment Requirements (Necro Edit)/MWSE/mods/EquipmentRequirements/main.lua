...
local function getItemSkillInfo(item)
    local skillReq, skillId
    local id = item.id:lower()

    if item.objectType == tes3.objectType.weapon
    or item.objectType == tes3.objectType.ammunition then
        skillReq = data.weaponReqs[id]
        skillId = item.skillId
    elseif item.objectType == tes3.objectType.armor then
        skillReq = data.armorReqs[id]
        skillId = data.armorClassSkills[item.weightClass]
    end
    ...
    if skillReq and skillReq <= 5 then
        skillReq = -5000
    end
...
local function reqTooltip(e)
    local skillReq, skillId = getItemSkillInfo(e.object)
...
local function onEquip(e)
    ...
    local skillReq, skillId = getItemSkillInfo(e.item)
    ...
    if tes3.mobilePlayer.skills[mobSkillId].base < skillReq then
        ...
        tes3.messageBox("Your %s skill is too low to equip %s.", tes3.skillName[skillId], name)
        return false
    end
end
...
local function updatePenalty()
    ...
    for itemStack in tes3.iterate(tes3.player.object.equipment) do
        ...
        local skillReq, skillId = getItemSkillInfo(object)
