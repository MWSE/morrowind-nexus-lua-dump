...
if (config == nil) then
	...
	config = { blackList = {}, pollRate = 1, enableEvents = 1 }
else
...
local function updateAllHealthBarValues()
	...
	for companionRefID, companionBar in pairs(companionHealthBars.healthBars) do
		...
		local npcHealth = companionBar.companionRef.mobile.health
...
local function createNewHealthBars(companions)
	...
	for i = #companions, 1, -1 do
		...
		local labelMenu = companionHealthBars.mainHealthBarMenu.menu:createLabel{
			id = labelMenuID,
