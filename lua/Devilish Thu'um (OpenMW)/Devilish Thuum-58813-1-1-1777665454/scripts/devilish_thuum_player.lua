-- scripts/devilish_thuum_player.lua
...
local range = 0.0
local speed = 1.0
local fadeSpeed = 1.25

local shakeActive = false
local cameraCleared = true

-- =============================================================
-- KNOCKDOWN / FATIGUE SPELL
-- =============================================================
local knockActive = false
local knockTimer = 0.0

local fatigueSpellActive = false
local fatigueSpellTimer = 0.0
...
-- =============================================================
local SHAKE = {
    [1] = { speed = 2.25, range = 1.80 },
    [2] = { speed = 3.30, range = 3.00 },
    [3] = { speed = 4.35, range = 4.65 },
}
...
local function clearCameraShake()
    if cameraCleared then return end
...
local function startShake(tier, scale)
    local shake = SHAKE[tier] or SHAKE[1]
    scale = scale or 1.0

    speed = shake.speed
    range = shake.range * scale

    shakeActive = true
    cameraCleared = false
end

local function addFatigueSpell()
    pcall(function()
        types.Actor.spells(self):add(FATIGUE_SPELL_ID)
    end)

    fatigueSpellActive = true
    fatigueSpellTimer = 3.0
end
...
local function removeFatigueSpell()
    ...
    fatigueSpellActive = false
end

local function updateFatigue(dt)
    if not fatigueSpellActive then return end

    fatigueSpellTimer = fatigueSpellTimer - dt
    if fatigueSpellTimer <= 0 then
        removeFatigueSpell()
    end
end

local function startKnockdown()
    addFatigueSpell()

    anim.playBlended(self, 'knockdown', {
...
local function updateKnockdown(dt)
    if not knockActive then return end

    knockTimer = knockTimer - dt
    if knockTimer <= 0 then
        anim.playBlended(self, 'idle1', {
            priority = anim.PRIORITY.Scripted
        })
        knockActive = false
    end
...
local function updateShake(dt)
    ...
    local i = math.floor(t % #keys) + 1
    local j = i + 1
    if j > #keys then j = 1 end

    local a = keys[i]
    local b = keys[j]

    local inv = 1 - f
    local x = a[1]*inv + b[1]*f
    local y = a[2]*inv + b[2]*f
    local z = a[3]*inv + b[3]*f

    camera.setExtraYaw(z * range)
    camera.setExtraPitch(y * range)
    camera.setRoll(x * range)
end

-- =============================================================
-- EVENTS
-- =============================================================
local function onThuumShout(data)
    startShake(
        data and data.tier or 1,
        data and data.shakeScale or 1.0
    )

    if data and data.affectPlayer and data.knockdown then
        startKnockdown()
    end
end
