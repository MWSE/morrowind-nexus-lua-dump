...
logLevel = 2, -- 0 = Minimum, 1 = Low, 2 = Medium, 3 = High
}
...
local function referenceActivated(e)
	...
	if logLevel > 0 then
		...
		local msg = string.format("%s ref = %s)", funcPrefix, ref)
...
local function activateShiftLockCheck(e)
	...
	if logLevel > 2 then
		mwse.log("\n%s: activateShiftLockCheck()", modPrefix)
	end
...
local function activate(e)
	...
	if config.doorCloseSound then
		...
		if not event.isRegistered('soundObjectPlay', soundObjectPlay) then
