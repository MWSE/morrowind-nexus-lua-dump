...
local inputController, DuelActive, Arena_Fight
...
local function getCellDestDoorRef(doorRef, destMarker, destCell, maxDist)
	...
	for destDoorRef in tes3.iterate(destCell.activators) do
		if destDoorRef.object.objectType == tes3_objectType_door then
			---mwse.log("destDoorRef = %s", destDoorRef)
			destDoorDest = destDoorRef.destination
			if destDoorDest then
				...
				---if destDoorDest.cell.id == doorRef.cell.id then
				if destDoorDest.cell == doorRef.cell then
					if not destDoorRef.disabled then
						if not destDoorRef.deleted then
							destDoorRefPos = destDoorRef.position
							destDoorDestMarkerPos = destDoorDest.marker.position
							zdist = math.abs(destDoorRefPos.z - destMarkerPos.z)
							if logLevel > 1 then
								mwse.log("%s zdist = %s, maxZdist = %s", funcPrefix, zdist, maxZdist)
							end
							if zdist <= maxZdist then
								zdist2 = math.abs(doorRefPos.z - destDoorDestMarkerPos.z)
								if logLevel > 1 then
									mwse.log("%s zdist2 = %s, maxZdist = %s", funcPrefix, zdist2, maxZdist)
								end
								if zdist2 <= maxZdist then
									dist = destDoorRefPos:distance(destMarkerPos)
									if dist <= maxDist then
										dist2 = doorRefPos:distance(destDoorDestMarkerPos)
						...
						end -- if not destDoorRef.deleted
					end -- if not destDoorRef.disabled
				end -- if destDoorDest.cell.id
			end -- if destDoorDest
		end -- if destDoorRef.object.objectType
	end -- for destDoorRef
	return nil
end
...
local function getLinkedDoorRef(doorRef)
	...
	if destCell.isInterior then
		ldRef = getCellDestDoorRef(doorRef, destMarker, destCell, maxDist)
		return ldRef
	end

	local culledCells = getCulledCells(destMarker, maxDist)
	for _, culledDestCell in ipairs(culledCells) do
		ldRef = getCellDestDoorRef(doorRef, destMarker, culledDestCell, maxDist)
		if ldRef then
			return ldRef
		end
	end
	...
	if ab01lockLevel then
		if ab01lockLevel > 0 then
			if not (ab01lockLevel == lockLevel) then
				...
				tes3.setLockLevel({reference = doorRef, level = ab01lockLevel})
			end
		end
	end
	if logLevel > 0 then
		local funcPrefix = string.format("%s: %s", modPrefix, 'referenceActivated')
		local msg = string.format("%s doorRef = %s)", funcPrefix, doorRef)
		if lockedChanged then
			msg = string.format('%s\n"%s".locked set to %s', msg, doorRef, ab01locked)
		end
		if lockLevelChanged then
			msg = string.format('%s\n, "%s".lockLevel set to %s', msg, doorRef, ab01lockLevel)
		end
		mwse.log(msg)
	end
	doorRef.data.ab01locked = nil
	doorRef.data.ab01lockLevel = nil
end
...
local function doorTune(doorRef, destDoorRef)
	...
	if doorCell.isInterior then
		if doorCell.behavesAsExterior then
			doLock = true
		elseif destDoorCell.isInterior then
	...
	if logLevel > 0 then
		local msg = string.format("%s: doorTune() doorRef = %s (%s %s %s), destDoorRef = %s (%s %s %s)", modPrefix, doorRef,
			doorPos.x, doorPos.y, doorPos.z, destDoorRef, destDoorPos.x, destDoorPos.y, destDoorPos.z)
		msg = string.format('%s\nBEFORE: doorRef "%s".locked = %s, doorRef"%s".lockLevel = %s\ndestDoorRef "%s".locked = %s, destDoorRef "%s".lockLevel = %s',
			msg, doorRef, locked, doorRef, lockLevel, destDoorRef, destLocked, destDoorRef, destLockLevel)
		mwse.log(msg)
	end

	if not destDoorRef.data then
		-- should not happen much
		---assert(destDoorRef.data)
		destDoorRef.data = {}
	end

	destDoorRef.data.ab01locked = locked
	destDoorRef.data.ab01lockLevel = lockLevel
end
...
local function delayedDoorTune(doorRef, delay)
	...
	timer.start({duration = delay, callback = function ()
...
local playingSound
local function playDoorCloseSound()
...
local function cellChanged()
	...
	playingSound = true
	-- wow, my 1st persistent timer, nice
	timer.start({duration = 1, callback = 'ab01playDoorCloseSound'})
end
...
local function activateToLock(e)
	...
	local shiftPressed = inputController:isKeyDown(tes3_scanCode_lShift)
...
local function activate(e)
	...
	if config.doorCloseSound then
		closeSound = getCloseSound(doorRef.object)
	end
	doorTune(doorRef, destDoorRef)
end
...
local function modConfigReady()
	...
	timer.register('ab01playDoorCloseSound', playDoorCloseSound)
