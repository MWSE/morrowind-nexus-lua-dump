...
local CAZADORID = "corpse hornet"--"skyrender"   --cazador
...
local function ssqnPatch()
    if (ssqn) then
		ssqn.registerQIcon("AGB_Mist_Blind","\\Icons\\agb\\q\\DOD_Blind.dds")
...
local function achievementPatch()
    if sb_achievements then
        ...
        sb_achievements.registerAchievement {
            ...
            progress = function ()
                return tes3.getGlobal("agb_bosseskilled")
            end,
        ...
        sb_achievements.registerAchievement {
            ...
            conditionType = sb_achievements.conditionType.progressAmount,
            progress = function ()
                return tes3.getGlobal("AGB_Health_Charges_Max")-3
            end,
            progressMax = function ()
                return 7
            end,
        ...
        sb_achievements.registerAchievement {
            ...
            condition = function()
                if ( tes3.getJournalIndex { id = "AGB_Mist_Dorans" } >= 40 and
                    tes3.getJournalIndex { id = "AGB_Mist_Dorans" } <= 100 and
            ...
            colour    = sb_achievements.colours.silver,
...
local tooltipData = {
	...
	{ id = "agb_lifesaver", description = "A gift from Berel Ules, after saving him from the cove cannibals.", itemType = "unique" },
    { id = "sch_hiv2_al_skypot", description = "Corpse Hornets are distant relatives of the Skyrenders, and reputed for their fearsome hunting prowess. Their barbed stingers inject a paralyzing, fast-acting and debilitating poison. One sting is enough to kill a fully grown kagouti, and even the hardiest of people writhe from the pain inflicted. Separated from the venom sac however, the lingering dried toxin on the stinger is known to cause adrenaline to shoot in the individual's legs, greatly increasing their jumping ability for the duration of the shock.", itemType = "unique" },
    ...
    --ingredients
    { id = "sch_cci_in_fl01", description = "Flesh removed from a mortal for some disturbed purpose.", itemType = "ingredients" },
    { id = "sch_hiv1_in_go01", description = "A precious metal popular with nobles rich in taste and coveted by artisans as an expensive rust-proof plating material.", itemType = "ingredients" },
    { id = "sch_cci_al_li01", description = "Nicknamed the Green Fairy for its naturally green hue, Absinthe has a dangerously hight concetration of intoxicants, commonly resulting in the indulger temporarily losing control of their motor functions.", itemType = "ingredients" },
    { id = "sch_cci_mi_wine01", description = "At first glance, this seems to be a bottle of fine Bravilian aromatized red wine. And yet, with all the stains and equipment nearby, one cannot help but wonder...", itemType = "ingredients" },
    { id = "sch_tb1_al_acid", description = "A corrosive substance used for corpse preparation and embalming rituals that doubles as a highly potent poison.", itemType = "ingredients" },

    --keys
    { id = "agb_cove_key", description = "Key to the jail cell in the Corpse Cove Meat Processing Shack.", itemType = "key" },
    { id = "agb_kilhaven_townkey", description = "Key to the courtyard gates in Kilhaven.", itemType = "key" },
    { id = "agb_merchant_key", description = "Key to the locked chests in the merchant's room in Kilhaven's Pale Stag.", itemType = "key" },
    { id = "agb_noble_key_01", description = "Key to the Hofia Manor's display case of valuable treasures in Kilhaven.", itemType = "key" },
    { id = "agb_palestag_key", description = "Key to the Pale Stag's front door in Kilhaven.", itemType = "key" },
    { id = "agb_vault_key", description = "Key to the Temple of Meridia Lower Sanctum's vault chest in Dunbal.", itemType = "key" },
    { id = "sch_cci_mi_reskey1", description = "Key to the basement of the Corpse Cove Residence.", itemType = "key" },
    { id = "sch_hiv1_mi_hivkey01", description = "Key to the Glinthollow Mine main entrance in Dunbal.", itemType = "key" },
    { id = "sch_tb1_mi_key01", description = "Key to the door mechanism room in Gundigul Barrow.", itemType = "key" },
	...
	{ id = "agb_mi_flint", description = "While most flint is not that valuable by itself, the flint mined in Dunbal's Glinthollow Mine can be used to relight any unlit light sources you come across.", itemType = "miscItem" },
    { id = "agb_igniting_agent", description = "This strange jar contains a thick, gelatinous fluid presumably used to prime or perhaps enhance explosives.", itemType = "miscItem" },
    { id = "sch_hiv1_in_coal01", description = "This pile of coal should be plentiful enough to power a modestly sized steam machine.", itemType = "miscItem" },
    { id = "sch_tagba1_mi_alt1", description = "This strange artifact should fit on the pedestal in the barrow's central chamber room.", itemType = "quest" },
    { id = "sch_tb2_eflamekey", description = "This torch may seem unassuming and crude, but it can be used to light holy fires that purge rot and corruption from the barrows.", itemType = "quest" },
	...
	{ id = "sch_tagba1_we_RS1", description = "Simply holding this staff floods the user with an uneasy sense of dread. Its presence is strangely anachronistic, its non-Nordic origins shrouded in mystery.", itemType = "unique" },
	{ id = "agb_poisonarrow", description = "Arrows dipped in a slow-acting poison designed to weaken and wear down targets.", itemType = "weapon" },
}
...
local function cazadorFrenzy(e)
    ...
    if effect.id == tes3.effect.sound then
        ...
        if not string.startswith(target.baseObject.id:lower(), CAZADORID) and target.baseObject.id ~= "AGB_Boss_4" then return end
        if not string.startswith(tes3.player.cell.editorName:lower(), "glinthollow") then return end
	    ...
	    if power > minPower then
		    ...
		    for _, cell in pairs(tes3.getActiveCells()) do
                for ref in iterReferenceList(cell.actors) do
                    if ref.mobile and not ref.mobile.isDead and (string.startswith(ref.baseObject.id:lower(), CAZADORID) or ref.baseObject.id == "AGB_Boss_4" ) and ref ~= target and radius > target.position:distance(ref.position) then
                        mobile:startCombat(ref.mobile)
                    end
                end
            end
