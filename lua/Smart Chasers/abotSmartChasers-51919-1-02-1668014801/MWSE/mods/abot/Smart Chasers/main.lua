...
local maxDist = config.chaseMaxDistance
...
local function updateConfig() -- used in modConfigReady template.onClose
	...
	if chaseLevel >= 4 then
		maxDist = config.chaseMaxDistance
	else
		maxDist = processManager.aiDistance
	end
end
...
local enemies = {} -- enemies[enemyRef.id:lower()] true = in combat, nil = dead or deleted
...
--[[
 ...
 e.g. chasers[lcEnemyId] = {cellId = cid, pos = startPos, hours = getInGameHoursPassedFromGameStart()}
 ]]
local chasers = {} -- chasers[enemyRef.id:lower()]
...
--[[local function isBlacklisted(lcObjId, sourceMod, blacklist, funcPrefix)
	...
	if sourceMod then
		if blacklist[sourceMod:lower()] then
			if logLevel > 1 then
...
local function packAndRemoveBlacklistedEnemies()
	...
	local t = {}
	for lcEnemyId, value in pairs(enemies) do
		if value then
			enemyRef = tes3.getReference(lcEnemyId)
			if enemyRef then
				obj = enemyRef.object
			else
				obj = tes3.getObject(lcEnemyId)
			end
			if not obj then
				if logLevel > 0 then
					mwse.log('%s: WARNING tes3.getObject("%s") failed', funcPrefix, lcEnemyId)
				end
			---else
				---lcObjId = obj.id:lower()
...
local combatBlacklist = {'wolf_bone','wolf_skeleton','hircine_spd','hircine_str'}
...
local function canPassThroughDoor(mobile, doorRef)
	...
	if not mobBounds then
		if logLevel > 0 then
			mwse.log('%s: canPassThroughDoor() mobile "%s" has no boundingBox', modPrefix, mobRef.id)
		end
		return true
	end
...
local vampireSpells = {
[1] = tes3.getObject('Vampire Sun Damage'),
[2] = tes3.getObject('Vampire Attributes'),
[3] = tes3.getObject('Vampire Skills'),
[4] = tes3.getObject('Vampire Immunities'),
[5] = tes3.getObject('Vampire Touch'),
[6] = tes3.getObject('Vampire Levitate'),
}
local vampireSunDamage = vampireSpells[1]

local vampireSpecialSpells = {
	['aundae'] = tes3.getObject('Vampire Aundae Specials'),
	['berne'] = tes3.getObject('Vampire Berne Specials'),
	['quarra'] = tes3.getObject('Vampire Quarra Specials'),
}
...
local function fixVampireSpells(vampMob)
	...
	for _, s in ipairs(vampireSpells) do
		assert(s)
		tes3.addSpell({reference = vampRef, spell = s})
		result = true
	end
...
---@diagnostic disable-next-line: undefined-field
		lcFactionId = string.split(faction.id:lower(), '_')[2]-- e.g. 'clan berne' --> 'berne'
		if logLevel > 2 then
		...
		if script then
---@diagnostic disable-next-line: undefined-field
			lcFactionId = string.split(script.id:lower(), '_')[2] -- e.g. 'vampire_berne_boss' --> 'berne'
			if logLevel > 2 then
				mwse.log('%s: fixVampireSpells "%s" lcFactionId = %s', modPrefix, vampRef.id, lcFactionId)
			end
		end
...
local function getCompanion(mobile)
	local companion
		...
		if not crea.biped then
			if not crea.usesEquipment then
				if crea.type == tes3_creatureType_normal then
					...
					local enemyId = enemyRef.id
					local lcObjId = enemyRef.object.id:lower()
					if config.checkHandy then
						if logLevel > 2 then
							mwse.log('%s: not handy enemy creature "%s", skip', funcPrefix, enemyId)
						end
						return true
---@diagnostic disable-next-line: undefined-field
					elseif string.multifind(lcObjId, combatBlacklist, 1, true) then
						if logLevel > 2 then
							mwse.log('%s: enemy creature "%s" blacklisted, skip', funcPrefix, enemyId)
						end
						return true
---@diagnostic disable-next-line: undefined-field
					elseif not string.multifind(lcObjId, combatWhitelist, 1, true) then
						if logLevel > 2 then
							mwse.log('%s: not handy enemy creature "%s" not whitelisted, skip', funcPrefix, enemyId)
						end
						return true
					end
				end
...
local function enemyTooBig(enemyMob, doorRef)
	...
	and (not canPassThroughDoor(enemyMob, doorRef)) then
		if logLevel > 2 then
			local funcPrefix = string.format("%s isNotHandy()", modPrefix)
			local enemyId = enemyMob.reference.id
			mwse.log('%s: enemy "%s" is too big to pass through "%s", skip', funcPrefix, enemyId, doorRef.id)
		end
		return true
	end
	return false
...
local function combatStarted(e)
	...
	if not (mobileTarget == mobilePlayer) then
		...
		for _, mob in pairs(mobilePlayer.friendlyActors) do
			if mob == mobileTarget then
				...
				if mobRef.position:distance(playerPos) < 2048 then
	...
	---local ref = enemyMob.reference
	local companion = getCompanion(enemyMob)
	...
	if companion == 1 then
		if logLevel > 2 then
			mwse.log('%s: enemy "%s" is a companion probably using startcombat to reset the AI, skip', funcPrefix, enemyId)
		end
		return
	end
	...
	if not enemyMob.hasFreeAction then -- note docs are wrong this is not a function
		...
		if logLevel > 1 then
			mwse.log('%s: enemy "%s" has no free action, skip', funcPrefix, enemyId)
		end
		return
	end
				...
				if destination then
					...
					if marker then
						if isSameCellOrExterior(marker.cell, enemyCell) then
							...
							if d > chaseMaxDistance then
								if logLevel > 3 then
									mwse.log('%s: enemy "%s" distance from last locked door destination > %s, skip', funcPrefix, enemyId, chaseMaxDistance)
								end
								return
							end
						end
					...
					if isSameCellOrExterior(lastDoorLockedCell, enemyCell) then
						...
						if d > chaseMaxDistance then
							if logLevel > 3 then
								mwse.log('%s: last not loading door locked, enemy and player in same cell, but enemy "%s" distance from door > %s, skip', funcPrefix, enemyId, chaseMaxDistance)
							end
							return
 ...
 -- in combat. player activate event on a door will call checkDoorChasers(doorRef) that will use enemies
	local lcId = enemyId:lower()
	if logLevel > 2 then
		mwse.log('%s: enemies["%s"] = true', funcPrefix, lcId)
	end
	enemies[lcId] = true
...
local function death(e)
	local lcEnemyId = e.reference.id:lower()
	if enemies[lcEnemyId] then
		enemies[lcEnemyId] = nil -- dead or deleted
		doPackEnemies = true
	end
end
...
local function getGenericName(name)
	if string.find(name:lower(),"^[aeiou]") then
		return 'an ' .. name
	else
		return 'a ' .. name
	end
...
local function getActorRefName(ref)
	...
	if companion == 0 then
		...
		if cloneCount > 1 then
			generic = true
		elseif ref.object.objectType == tes3_objectType_creature then
			generic = true
		end
	end
	if generic then
...
local function observerCanSeeTargetAtDistance(observerRef, targetRef, distance)
	...
	if distance > 8100 then -- at this distance no creature is visible
		return false
	end
				...
				if obj then
					...
					if x > 0 then
						...
						if roll < x then
							ok = true
							if logLevel > 0 then
								mwse.log('%s: enemy "%s" managed to pick the "%s" door lock using a "%s"', funcPrefix, enemyId, doorId, obj.id)
							end
						...
						elseif logLevel > 0 then
							mwse.log('%s: enemy "%s" failed picking the "%s" door lock using a "%s"', funcPrefix, enemyId, doorId, obj.id)
						end -- if roll
					end -- if x > 0
				end -- if obj
			...
			if spell then
				if magnitude >= lockLevel then
					...
					if roll < chance then
						ok = true
						if logLevel > 0 then
							mwse.log('%s: enemy "%s" managed to unlock door "%s" using a "%s" spell', funcPrefix, enemyId, doorId, spell.name)
						end
						if config.doorUnlockedMessage then
					...
					elseif logLevel > 0 then
						mwse.log('%s: enemy "%s" failed unlocking door "%s" using a "%s" spell', funcPrefix, enemyId, doorId, spell.name)
					end -- if roll
				end -- if magnitude
			end --  if spell
		...
		if config.useEnchantment then
			...
			if object then
				if magnitude >= lockLevel then
					...
					if logLevel > 0 then
						mwse.log('%s: enemy "%s" managed to unlock door "%s" using an enchanted "%s"', funcPrefix, enemyId, doorId, object.name)
					end
					if config.doorUnlockedMessage then
					...
					doorSound = spellSound
				elseif logLevel > 0 then
					mwse.log('%s: enemy "%s" failed unlocking door "%s" using an enchanted "%s"', funcPrefix, enemyId, doorId, object.name)
				end -- magnitude
			end -- if object
		end -- config
	end -- if not ok
		...
		if config.useBash then
			...
			if x >= config.minBashingStrength then
				...
				if roll < x then
					ok = true -- lock bash
					if logLevel > 0 then
						mwse.log('%s: enemy "%s" managed to bash door "%s"', funcPrefix, enemyId, doorId)
					end
					if config.doorUnlockedMessage then
					...
					doorSound = bashSound
				elseif logLevel > 0 then
					mwse.log('%s: enemy "%s" failed bashing door "%s"', funcPrefix, enemyId, doorId)
				end -- if roll
			end -- if x
...
---@diagnostic disable-next-line: undefined-field
	...
	local function processEnemy(lcEnemyId)
		local enemyRef = tes3.getReference(lcEnemyId)
		if not enemyRef then
			if logLevel > 1 then
				mwse.log('%s: enemy ref "%s" not found', funcPrefix, lcEnemyId)
			end
			return
		end
		local enemyId = enemyRef.id
		...
		if logLevel > 1 then
			mwse.log('%s: checking enemy "%s"', funcPrefix, enemyId)
		end

		local enemyMob = enemyRef.mobile
		if not enemyMob then
			if logLevel > 1 then
				mwse.log('%s: mobile enemy "%s" not found', funcPrefix, lcEnemyId)
			end
			return
		end
		...
		if not inCombat(enemyMob) then
			if logLevel > 0 then
				mwse.log('%s: enemy "%s" is not in combat any more, skip', funcPrefix, enemyId)
			end
			return
		end
		...
		if isMobileDead(enemyMob) then -- better safe than sorry
			if logLevel > 0 then
				mwse.log('%s: enemy "%s" is dead, skip', funcPrefix, enemyId)
			end
			return
		end

		if chaseLevel <= 2 then
			if not enemyMob.isPlayerDetected then
				if logLevel > 0 then
					mwse.log('%s: enemy "%s" cannot detect player, skip', funcPrefix, enemyId)
				end
				return
			end
		end
		...
		if chaseLevel == 1 then
			if not enemyCanSeePlayerAtDistance(enemyRef, dist) then
				if logLevel > 0 then
					mwse.log('%s: enemy "%s" cannot see player, skip', funcPrefix, enemyId)
				end
				return
			end
		end
				...
				if not vampireChase then
					if dayTime then
						if enemyMob.actorType == tes3_actorType_npc then
							if isVampire(enemyRef) then
								if logLevel > 0 then
									mwse.log('%s: enemy "%s" is vampire, dayTime, skip', funcPrefix, enemyId)
								end
								return
							end
						end
...
---@diagnostic disable-next-line: undefined-field
								if string.multifind(cellName:lower(), {'tomb','burial'}, 1, true) then
									if logLevel > 1 then
										mwse.log('%s: undead creature "%s" is not following outside "%s", skip', funcPrefix, enemyId, cellName)
									end
									return
								end
							end
		...
		if logLevel > 2 then
			mwse.log('%s: enemy "%s" distance from player = %s', funcPrefix, enemyId, dist)
		end

		if dist > maxDist then
			if sameCellOrDifferentKindOfCell(enemyRef.cell, player.cell) then
				if logLevel > 1 then
					mwse.log('%s: enemy "%s" distance from player = %s > max distance = %s, skip', funcPrefix, enemyId, dist, maxDist)
				end
				return
			end
		end
		...
		timer.start({duration = dur, iterations = 3, callback = function ()

			enemyRef = tes3.getReference(lcEnemyId) -- refresh it for safety
			if not enemyRef then
				if logLevel > 1 then
					mwse.log('%s callback: enemy ref "%s" not found', funcPrefix, lcEnemyId)
				end
				return
			end
			...
			if not enemyMob then
				if logLevel > 1 then
					mwse.log('%s callback: mobile enemy "%s" not found', funcPrefix, lcEnemyId)
				end
				return
			end
			...
			and lastDoorLockedRef then
				...
				if playerCell == lastDoorLockedCell then
					...
					if check then
						...
						if d > maxDist then
							if logLevel > 1 then
								mwse.log('%s callback: lastDoorLockedPos:distance(player.position) > %s, skip', funcPrefix, maxDist)
							end
							return
						end
						...
						if not doorCanBeOpened(lastDoorLockedRef, enemyMob) then
							if logLevel > 1 then
								mwse.log('%s callback: not doorCanBeOpened("%s", "%s"), skip', funcPrefix, lastDoorLockedRef, enemyId)
							end
							return
						end
					end
			...
			if not doorDest then
				if logLevel > 1 then
					mwse.log('%s callback: "%s" door "%s" has no destination, skip', funcPrefix, enemyId, doorRef.id)
				end
				return
			end
			...
			local chaser = chasers[lcEnemyId]
			local startCellId = ''
			...
			if chaser then
				...
				and (not playerCell.isInterior) then -- both enemy and player in exterior
					...
					if d > maxDist then
						if logLevel > 1 then
							mwse.log('%s callback: enemy "%s" distance from player = %s > max distance = %s, skip', funcPrefix, enemyId, d, maxDist)
						end
						return
					end
				end
			...
			if ok then
				...
				if not tes3.positionCell({reference = enemyRef, cell = doorDest.cell, position = pos, orientation = ori}) then
					if logLevel > 0 then
						mwse.log('%s callback: trying tes3.positionCell({reference = "%s", startCell = "%s", destCell = "%s", position = %s, orientation = %s}) failed',
							funcPrefix, enemyId, doorRef.cell.editorName, doorDest.cell.editorName, pos, ori)
					end
					return
				end
				if logLevel > 0 then
					mwse.log('%s callback: "%s" followed player through door "%s" from cell "%s" to cell "%s"',
						funcPrefix, enemyId, doorRef.id, doorRef.cell.id, doorDest.cell.editorName)
				end
			end
			...
			if chaser then
				...
				-- store a new chaser
				chasers[lcEnemyId] = {cellId = startCellId, pos = enemyPos:copy(), hours = inGameHoursPassedFromGameStart}
			end

			enemies[lcEnemyId] = true
		...
		if not doorDest then
			if logLevel > 1 then
				mwse.log('%s: "%s" door %s timer started duration = %s', funcPrefix, enemyId, doorRef.id, dur)
				end
			return
			end
		...
		if logLevel > 1 then
			local dest
			if doorDest.cell.isInterior then
				dest = doorDest.cell.id
			else
				dest = doorDest.cell.editorName
			end
			mwse.log('%s: "%s" through door "%s" to cell "%s" timer started duration = %s', funcPrefix, enemyId, doorRef.id, dest, dur)
		end

	end -- local function processEnemy


	for lcEnemyId, _ in pairs(enemies) do
		processEnemy(lcEnemyId)
	end
...
local function casted(e)
	if e.caster == player then
		for _, eff in ipairs(e.source.effects) do
...
local function updateEnemiesAndChasers()
	...
	local function moveBackChaser(lcEnemyId, chaser)
		enemyRef = tes3.getReference(lcEnemyId)
		if not enemyRef then
			if logLevel > 1 then
				mwse.log('%s moveBackChaser: enemy ref "%s" not found', funcPrefix, lcEnemyId)
			end
			return
		end
		...
		if logLevel > 1 then
			mwse.log('%s: "%s" moved back to %s', funcPrefix, lcEnemyId, dest)
		end
	end


	local function processChaserWithDelay(lcEnemyId, chaser)

		enemyRef = tes3.getReference(lcEnemyId)

		if not enemyRef then
			local obj = tes3.getObject(lcEnemyId)
			if obj then
				if chaseEndMessage then
			...
			if enemies[lcEnemyId] then
				enemies[lcEnemyId] = nil
				doPackEnemies = true
			end
			if logLevel > 1 then
				if chaser.cellId == '' then
					mwse.log('%s: stored chaser "%s" not found, removing from enemies/chasers', funcPrefix, lcEnemyId)
				else
					mwse.log('%s: stored chaser "%s".cellId ("%s") not found, removing from enemies/chasers', funcPrefix, lcEnemyId, chaser.cellId)
				end
			end
			return false
		end
		...
		if logLevel > 0 then
			mwse.log('%s: "%s" initial cell "%s" current distance from player = %s', funcPrefix, lcEnemyId, chaser.cellId, dist)
		end
		...
		if playerCanSeeEnemyAtDistance(enemyRef, dist) then

			if dayTime then
				if mobCell.isOrBehavesAsExterior then
					if isVampire(enemyRef) then
						if logLevel > 2 then
							mwse.log('%s: isVampire("%s")', funcPrefix, lcEnemyId)
						end
						if enemyMob.health.normalized < 0.9 then
							if logLevel > 2 then
								mwse.log('%s: "%s".health.normalized < 0.9', funcPrefix, lcEnemyId)
							end
							if enemyMob.hasFreeAction then
								if not isMobileDead(enemyMob) then
									if logLevel > 1 then
										mwse.log('%s: "%s" teleporting away', funcPrefix, lcEnemyId)
									end
			...
			if logLevel > 1 then
				mwse.log('%s: player can see "%s" at %s distance, no reset', funcPrefix, lcEnemyId, dist)
			end
			activechasers[lcEnemyId] = chaser
			enemies[lcEnemyId] = true
			return false
		end
		...
		if dist > maxDist then
			if playerCell.isInterior == mobCell.isInterior then
				if logLevel > 1 then
					mwse.log('%s: "%s" distance from player = %s > max distance = %s, reset', funcPrefix, lcEnemyId, dist, maxDist)
				end
				return false
			end
		end
		...
		if not enemyMob.hasFreeAction then --  paralyzed, dead, stunned, or otherwise unable to take action
			mwse.log('%s: enemy "%s" is not in combat any more, reset', funcPrefix, lcEnemyId)
			return false
		end
		...
		if chaseLevel <= 2 then
			if not enemyMob.isPlayerDetected then
				if logLevel > 0 then
					mwse.log('%s: enemy "%s" cannot detect player, reset', funcPrefix, lcEnemyId)
				end
				return false
			end
		end
		...
		if dayTime then
			if mobCell.isOrBehavesAsExterior then
				if isVampire(enemyRef) then
					if logLevel > 0 then
						mwse.log('%s: enemy "%s" is vampire, dayTime, reset', funcPrefix, lcEnemyId)
					end
					return false
				end
			end
		...
		if hoursDiff > chaseMaxHours then
			if logLevel > 1 then
				mwse.log('%s: %s hours passed since "%s" started combat, reset', funcPrefix, hoursDiff, lcEnemyId)
			end
			return false
		end
	...
	for lcEnemyId, chaser in pairs(chasers) do
		delay = processChaserWithDelay(lcEnemyId, chaser)
		if delay
		or (not activechasers[lcEnemyId]) then
			enRef = tes3.getReference(lcEnemyId)
			if enRef then
				enMob = enRef.mobile
				if enMob then
					if enMob.hasFreeAction then
						if not isMobileDead(enMob) then
							if delay then
								timer.start({duration = 2.5, callback = function() moveBackChaser(lcEnemyId, chaser) end})
							else
								moveBackChaser(lcEnemyId, chaser)
							end
						end
					end
				end
...
local function loaded(e)
	...
	processManager = worldController.mobController.processManager
...
local function modConfigReady()
	...
	local usage = "\nEnemies will try and use their available tools/abilities to open the linked doors if locked by player"
.."\nusing locking magic (e.g. Fenrick's Doorjam spell) or the door key if available."
...
It makes sense to set it high enough (e.g. 3 sec) to give player enough time to cast a lock spell on exterior door to try and lock in enemies.\n%s]], defaultConfig.minDelay, delayFormulaDescr)
	...
	controls:createSlider({
		...
		description = string.format([[Delay divider (default: %s). The higher Delay divider, the faster enemies will be able to chase you through doors.\n%s]], defaultConfig.delayDivider, delayFormulaDescr)
	})
