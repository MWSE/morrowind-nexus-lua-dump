...
local function combatStarted(e)
	...
	if hasCompanion(attackerRef) then
		if logLevel4 then
...
local function objectInvalidated(e)
	...
	if obj then
		if type(obj) == 'userdata' then -- bah hopefully this will be enough to skip weird data
---@diagnostic disable-next-line: undefined-field
			local id = obj.id
			if id then
				...
				if enemies[lcObjId] then
					if logLevel2 then
						mwse.log('%s: objectInvalidated() enemies["%s"] dead or deleted', modPrefix, lcObjId)
					end
