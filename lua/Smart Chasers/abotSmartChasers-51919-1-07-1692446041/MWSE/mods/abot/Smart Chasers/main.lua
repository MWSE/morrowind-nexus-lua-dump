...
local packingEnemies = false
local function packEnemies()
	if packingEnemies then
		return
	end
	packingEnemies = true
	local t = {}
...
local chasers = {} -- e.g. chasers[string.lower(enemyRef.id)] = {cellId = startCellId, pos = startCellPos, hours = inGameHoursPassedFromGameStart}
...
local chaserBlacklist = {'bat','wolf_bone','wolf_skeleton','hircine_spd','hircine_str'}
local handyWhitelist = {'centurion','draugr','fabricant_hulking','frost_giant','goblin','guar',
...
local function getRefVariable(ref, variableId)
	...
	if value then
		--[[
		assert(value == context[string.lower(variableId)]) -- not case sensitive???
		assert(value == context[string.upper(variableId)]) -- not case sensitive???
		if logLevel2 then
...
local function isBlacklistedOrNotHandy(mob)
	...
	if sourceMod then
		if isModBlacklisted(sourceMod) then
			if logLevel > 2 then
				mwse.log('%s: creature "%s" from blacklisted "%s" mod, skip', funcPrefix, mobRefId, sourceMod)
			end
			return true
		end
	end
...
local function isChaserTooBig(mob, doorRef)
	...
	and (not canPassThroughDoor(mob, doorRef)) then
		if logLevel > 2 then
			local funcPrefix = string.format("%s isChaserTooBig()", modPrefix)
...
local function combatStarted(e)
	...
	local attackerRef = attackerMob.reference
	local attackerRefId = attackerRef.id

	if not attackerMob.hasFreeAction then -- note docs are wrong this is not a function
		-- dead or paralyzed or stunned or otherwise unable to take action
		if logLevel > 1 then
			mwse.log('%s: attacker "%s" has no free action, skip', funcPrefix, attackerRefId)
		end
		return
	end

	if isBlacklistedOrNotHandy(attackerMob) then
		return
	end

	local companion = getCompanion(attackerRef)
	if companion == 1 then
		if logLevel > 3 then
			mwse.log('%s: attacker "%s" is a companion probably using startcombat to reset the AI, skip', funcPrefix, attackerRefId)
		end
		return
	end
	...
	if not (mobileTarget == mobilePlayer) then
		...
		for i = 1, #friendlyActors do
			...
			if mob == attackerMob then
				if logLevel > 2 then
					mwse.log('%s: attacker "%s" is friendly, skip', funcPrefix, attackerRefId)
				end
				return
			end
			if mob == mobileTarget then
				mobRef = mob.reference
				if sameCellOrBothExteriors(mobRef.cell, playerCell) then
	...
	if logLevel > 2 then
		mwse.log('%s: e.actor = %s, e.target = %s', funcPrefix, attackerRefId, mobileTarget.reference.id)
	end

	if lastDoorLockedId then
		if lastDoorLockedRef then

			if isChaserTooBig(attackerMob, lastDoorLockedRef) then
				return
			end

			local attackerCell = attackerRef.cell
			...
			if sameCellOrBothExteriors(lastDoorLockedCell, player.cell) then
				...
				if destination then
					...
					if marker then
						if sameCellOrBothExteriors(marker.cell, attackerCell) then
							d = marker.position:distance(attackerRef.position)
							if d > chaseMaxDistance then
								if logLevel > 3 then
									mwse.log('%s: enemy "%s" distance from last locked door destination > %s, skip', funcPrefix,attackerRefId, chaseMaxDistance)
								end
								return
							end
						end
					...
					if sameCellOrBothExteriors(lastDoorLockedCell, attackerCell) then
						d = lastDoorLockedPos:distance(attackerRef.position)
						if d > chaseMaxDistance then
							if logLevel > 3 then
								mwse.log('%s: last not loading door locked, enemy and player in same cell, but enemy "%s" distance from door > %s, skip', funcPrefix, attackerRefId, chaseMaxDistance)
							end
							return
						end -- if sameCellOrBothExteriors(lastDoorLockedCell, attackerCell)
					end
				end -- if destination
	...
	if fixMissingVampireSpells then
		if isVampire(attackerRef) then
			fixVampireSpells(attackerMob)
		end
	end
 -- in combat. player activate event on a door will call checkDoorChasers(doorRef) that will use enemies
	local lcAttackerRefId = string.lower(attackerRefId)
	if logLevel > 3 then
		mwse.log('%s: enemies["%s"] = true', funcPrefix, lcAttackerRefId)
	end
	enemies[lcAttackerRefId] = true
...
local function death(e)
	...
	if enemies[lcRefId] then
		enemies[lcRefId] = nil -- dead or deleted
		doPackEnemies = true
	end
end
...
local function updateEnemiesAndChasers()
	...
	local function processChaserWithDelay(lcEnemyRefId, chaser)
		...
		if not enemyRef then
			...
			if baseObj then
				if chaseEndMessage then
					tes3ui.showNotifyMenu( "You can't hear %s chasing you any more.", getActorName(baseObj) )
				end
			elseif logLevel > 0 then
...
3. Only in AI range = only if they are in AI distance range (for this setting to work better it is suggested to set the standard game AI distance slider to the max = 7000 game units)
