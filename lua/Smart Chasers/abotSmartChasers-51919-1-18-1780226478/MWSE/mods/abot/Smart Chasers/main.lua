...
trespassChase = true, -- Allow trespass crime victims/witnesses as chasers
...
local chaseMaxDistance, followerMaxPlayerDistance, trespassChase
...
local function updateFromConfig() -- used in modConfigReady template.onClose
	...
	trespassChase = config.trespassChase
...
-- enemies[string_lower(enemyRef.id)] true = in combat, nil = dead or deleted
local enemies, refsByRef = {}, {}
local enemyCount = 0
...
local function addEnemy(lcId, ref)
	if enemies[lcId] then
		enemies[lcId] = ref -- update in case it is true instead of ref
		return
	end
	enemies[lcId] = ref
	...
	refsByRef[ref] = lcId
	if logLevel4 then
		mwse_log('%s: addEnemy("%s")', modPrefix, lcId)
	end
end
...
local function removeEnemy(lcId)
	...
	enemyCount = enemyCount - 1
	if logLevel4 then
		mwse_log('%s: removeEnemy("%s")', modPrefix, lcId)
	end
	return true
end
...
-- e.g. chasers[string_lower(enemyRef.id)] = {cellId = startCellId, pos = startCellPos,
--	hours = hoursPassedSinceGameStart, fight = enemyMob.fight}
local chasers = {}

local tes3_actorType_creature, tes3_actorType_npc = tes3.actorType.creature, tes3.actorType.npc
local tes3_creatureType_normal, tes3_creatureType_undead = tes3.creatureType.normal, tes3.creatureType.undead
--[[local tes3_creatureType_daedra, tes3_creatureType_humanoid = tes3.creatureType.daedra, tes3.creatureType.humanoid]]
...
local function startsWithListed(s, list)
	...
	local lcListElement
	for i = 1, #list do
		lcListElement = string_lower(list[i])
		if lcs:startswith(lcListElement) then
			return true
		end
	end
	return false
...
local math_abs, math_floor, math_max, math_min, math_pi, math_random
= math.abs, math.floor, math.max, math.min, math.pi, math.random

---@param mob tes3mobileActor
---@param doorRef tes3reference
---@return boolean
local function canPassThroughDoor(mob, doorRef)
	local mobRef = mob.reference
	local mobBounds = mobRef.object.boundingBox
	if not mobBounds then
		if logLevel2 then
			mwse_log('%s: canPassThroughDoor() mob "%s" has no boundingBox, returning true', modPrefix, mobRef.id)
		end
		return true
	end
	...
	if not doorBounds then
		...
		--- mwse_log('door "%s" has no boundingBox', doorRef.id)
		return true
	end
	...
	local doorSize = math_max(doorSizeX, doorSizeY)
	...
	local mobSize = math_min(mobSizeX, mobSizeY) -- and strafe

	if (mobHeight > doorHeight)
	and (mobSize > doorSize) then
		if logLevel2 then
			mwse_log([[%s: mob height = %s > door height = %s
...
local function getObject(id)
	...
	if not obj then
		mwse_log('%s: getObject("%s") failed ', modPrefix, id)
	end
	return obj
end
...
---@diagnostic disable-next-line: undefined-field
		lcFactionId = faction.id:lower():split('_')[2]-- e.g. 'clan berne' --> 'berne'
		if logLevel3 then
			mwse_log('%s: fixVampireSpells "%s" lcFactionId = %s', modPrefix, vampRef.id, lcFactionId)
		end
	else
		local script = vampObj.script
		if script then
---@diagnostic disable-next-line: undefined-field
			lcFactionId = script.id:lower():split('_')[2] -- e.g. 'vampire_berne_boss' --> 'berne'
			if logLevel3 then
				mwse_log('%s: fixVampireSpells "%s" lcFactionId = %s', modPrefix, vampRef.id, lcFactionId)
			end
		end
	...
	if result then
		if logLevel2 then
			mwse_log('%s: fixVampireSpells "%s" missing vampire spells fixed', modPrefix, vampRef.id)
		end
		return
	end
	return false
...
local function isValidMobile(mob)
	...
	if mob.actorType == tes3_actorType_creature then -- 0 = creature
		local lcId = string_lower(ref.object.id)
...
-- ab01 prefix, probably some abot's creature with AIEscort/AIFollow package, skip
			...
			---@cast mob tes3mobileCreature
			local creature = mob.object ---@type tes3creatureInstance
			if creature then
				...
				if script then
					...
					if lcId2:startswith('ab01') then
						...
						if logLevel3 then
							mwse_log("%s: %s having ab01 prefix, probably some abot's creature with AIEscort/AIFollow package, skip", modPrefix, ref.id)
						end
						return false
					end
				end
...
local function isBlacklistedOrNotHandy(mob)
	...
	and isCellBlacklisted(mobCell) then
		if logLevel3 then
			mwse_log('%s: creature "%s" from blacklisted "%s" cell, skip', funcPrefix, mobRefId, mobCell.editorName)
		end
		return true
	end
	...
	and isModBlacklisted(sourceMod) then
		if logLevel3 then
			mwse_log('%s: creature "%s" from blacklisted "%s" mod, skip',
				funcPrefix, mobRefId, sourceMod)
		end
		return true
	end
	local lcObjId = string_lower(mobObj.id)
	if lcObjId:multifind(chaserBlacklist, 1, true) then
		if logLevel3 then
			mwse_log('%s: chaser "%s" blacklisted, skip', funcPrefix, mobRefId)
		end
		return true
	end
	...
	if lcObjId:multifind(handyWhitelist, 1, true) then
		return true
	end
	if config.checkHandy then
		if logLevel3 then
			mwse_log('%s: not handy creature "%s", skip', funcPrefix, mobRefId)
		end
		return true
	end
	return false
...
---@param cellA tes3cell
---@param cellB tes3cell
---@return boolean
...
local function isChaserTooBig(mob, doorRef)
	...
	and (not canPassThroughDoor(mob, doorRef)) then
		if logLevel3 then
			...
			mwse_log('%s: "%s" is too big to pass through "%s", skip', funcPrefix, mobRefId, doorRef.id)
		end
		return true
	end
	return false
...
local function combatStarted(e)
	...
	local funcPrefix = modPrefix..' '..e['eventType']..'()'

	local enemyRefId = enemyRef.id

	if enemyRef.tempData.ab01smchwtn then
		if logLevel2 then
			mwse_log('%s: enemy "%s" is trespass witness, skip', funcPrefix, enemyRefId)
		end
		return
	end
	...
	if not enemyMob.hasFreeAction then
		...
		if logLevel2 then
			mwse_log('%s: enemy "%s" has no free action, skip', funcPrefix, enemyRefId)
		end
		return
	end
	...
	if hasCompanion(enemyRef) then
		if logLevel4 then
			mwse_log('%s: enemy "%s" is a companion, skip', funcPrefix, enemyRefId)
		end
		return
	end
	...
	if not (mobileTarget == mobilePlayer) then
		...
		for i = 1, #friendlyActors do
			...
			if mob == enemyMob then
				if logLevel3 then
					mwse_log('%s: enemy "%s" is friendly, skip', funcPrefix, enemyRefId)
				end
				return
			end
	...
	if logLevel3 then
		mwse_log('%s: e.actor = %s, e.target = %s', funcPrefix, enemyRefId, mobileTarget.reference.id)
	end
	...
	and lastDoorLockedRef then
		...
		if sameCellOrBothExteriors(lastDoorLockedCell, player.cell) then
			...
			if d > chaseMaxDistance then
				if logLevel4 then
					mwse_log('%s: player distance from last locked door > %s, skip', funcPrefix, chaseMaxDistance)
				end
				return
			end
			...
			if destination then
				...
				and sameCellOrBothExteriors(marker.cell, enemyCell) then
					...
					if d > chaseMaxDistance then
						if logLevel4 then
							mwse_log('%s: enemy "%s" distance from last locked door destination > %s, skip', funcPrefix,enemyRefId, chaseMaxDistance)
						end
						return
					end
				end
			elseif sameCellOrBothExteriors(lastDoorLockedCell, enemyCell) then
				...
				if d > chaseMaxDistance then
					if logLevel4 then
						mwse_log('%s: last not loading door locked, enemy and player in same cell, but enemy "%s" distance from door > %s, skip',
 ...
 -- in combat. player activate event on a door will call checkDoorChasers(doorRef) that will use enemies
	local lcEnemyRefId = enemyRefId:lower()
...
local function death(e)
	local lcRefId = string_lower(e.reference.id)
	removeEnemy(lcRefId)
...
---@param uses integer
local function logObj(actorRef, obj, uses)
	...
	mwse_log('%s getThiefTool(): enemy "%s" using %s "%s" (%s uses)',
		modPrefix, actorRef.id, s, obj.id, uses)
end

---@param actorRef tes3reference
---@param objectType integer
---@return tes3lockpick|tes3probe|?
		...
		and ( not obj.name:lower():multifind({'compass','sextant'}, 1, true) ) then
			...
			for _, iData in ipairs(variables) do
				if iData then
					...
					if condition then
						if condition > 0 then
							-- decrease item condition like when used
							decCondition(obj, iData)
							return obj
						else
...
---@param actorRef tes3reference
---@param lockLevel integer
---@return tes3spell?, integer?, number?
local function getOpenSpell(actorRef, lockLevel)
		...
		or spl.alwaysSucceeds then
			...
			if effectIndex then
				if effectIndex >= 0 then -- returns -1 if not found
					---mwse_log("effectIndex = %s", effectIndex)
					effectIndex = effectIndex + 1
					effect = spl.effects[effectIndex]
					mag = math_floor((effect.min + effect.max) / 2)
					...
					if logLevel2 then
						mwse_log('%s: enemy "%s" spell "%s" magnitude = %s, chance = %s', funcPrefix, actorRef.id, spl.id, mag, cha)
					end
					if (mag >= lockLevel)
					and (cha >= 33) then
	...
	if found then
		...
		if logLevel2 then
			mwse_log('%s: enemy "%s" using spell "%s" ("%s")', funcPrefix, actorRef.id, spell.id, spell.name)
		end
...
---@param actorRef tes3reference
---@return tes3alchemy|tes3apparatus|tes3armor|tes3book|tes3clothing|tes3ingredient|tes3item|tes3light|tes3lockpick|tes3misc|tes3probe|tes3repairTool|tes3weapon|tes3leveledItem|nil, integer?, boolean?
local function getOpenEnchantedItem(actorRef)
	local inventory = actorRef.object.inventory

	local items = inventory.items
	for i = 1, #items do
		local stack = items[i] ---@type tes3itemStack
		...
		if enchantment then
			...
			and (effectIndex >= 0) then
				...
				or (
					...
					and variables
					and (enchantment.chargeCost <= variables['charge'])
				) then
					local effect = enchantment.effects[effectIndex]
					local magnitude = math_floor((effect.min + effect.max) / 2)
					if logLevel2 then
						mwse_log('%s getOpenEnchantedItem(): enemy "%s" using "%s" enchanted item', modPrefix, actorRef.id, object.id)
					end
...
---@param obj tes3door|tes3container
---@return tes3sound
local function getCloseSound(obj)
	...
	if not closeSound then
		...
		if openSound then
			...
			if s then
				s = s:gsub('[Oo][Pp][En][Nn]', 'Close') -- replace 'Open' with 'Close', case insensitive
...
local function playRefSound(ref, snd)
	...
	local vol = math_max( 1 / (dist/3583 + 1), 0.4 )
...
local function getGenericName(name)
	if string_lower(name):find("^[aeiou]") then
		return 'an ' .. name
	else
		return 'a ' .. name
	end
...
---@param doorRef tes3reference
---@param linkedDoorRef tes3reference
---@return boolean
local function doorsAreLinked(doorRef, linkedDoorRef)
	if not doorRef then
		return false
	end
	...
	if d > 350 then
		if logLevel2 then
			mwse_log('%s: markerPos:distance(linkedDoorPos) > 350, not linked', funcPrefix)
		end
		return false
	end
	d = math_abs(linkedDoorPos.z - markerPos.z)
	if d > 192 then
		if logLevel2 then
			mwse_log('%s: math_abs(linkedDoorPos.z - markerPos.z) > 192, not linked', funcPrefix)
		end
		return false
	end
	return true
...
local function ab01smtchsPT1(e)
	...
	local doorRef = refs[1] ---@class tes3reference
	local linkedDoorRef = refs[2] ---@class tes3reference
	local locked = doorRef['locked']
	if logLevel2 then
		local action = 'locking'
		if locked then
			action = 'unlocking'
		end
		mwse_log('%s: linkedDoorsSync() ab01smtchsPT1 %s "%s" like linked "%s"',
	...
	updateDoorData(linkedDoorRef, locked)
end
...
local function linkedDoorsSync(doorRef, linkedDoorRef)
	if not doorsAreLinked(doorRef, linkedDoorRef) then
		return false
	end
	...
	if logLevel2 then
		...
		mwse_log('%s linkedDoorsSync(): %s "%s" like linked "%s"', modPrefix, action, linkedDoorRef.id, doorRef.id)
	end
	if locked then
...
local function ab01smtchsPT2(e)
	...
	if logLevel2 then
		mwse_log('%s: ab01smtchsPT2 linkedDoorsSync("%s", "%s")',
...
---@param observerRef tes3reference
...
local function observerCanSeeTargetAtDistance(observerRef, targetRef, distance)
	...
	radAngleTo = math_abs(radAngleTo) -- not needed it seems but...
...
local function ab01smtchsPT3(e)
	...
	if logLevel3 then
		mwse_log('%s: ab01smtchsPT3 playDoorCloseSound("%s")', modPrefix, doorRef)
	end
...
local function canOpenAutoLockedDoor(doorRef, enemyRef)
	...
	and doorCell.id:find(enemyName, 1, true) then
		return true
	end
	...
	if marker.cell.id:find(enemyName, 1, true) then
		return true
	end
end

---@param doorRef tes3reference
---@param enemyMob tes3mobileActor
---@return boolean
...
---@diagnostic disable-next-line: undefined-field
				...
				if x > 0 then
					...
					if roll < x then
						ok = true
						if logLevel1 then
							mwse_log('%s: enemy "%s" managed to pick the "%s" door lock using a "%s"',
					...
					elseif logLevel1 then
						mwse_log('%s: enemy "%s" failed picking the "%s" door lock using a "%s"',
							funcPrefix, enemyRefId, doorId, obj.id)
					end -- if roll
				end -- if x > 0
			end -- if obj
	...
	and config.useMagic then
		...
		and (magnitude >= lockLevel) then
			...
			if roll < chance then
				ok = true
				if logLevel1 then
					mwse_log('%s: enemy "%s" managed to unlock door "%s" using a "%s" spell',
			...
			elseif logLevel1 then
				mwse_log('%s: enemy "%s" failed unlocking door "%s" using a "%s" spell',
					funcPrefix, enemyRefId, doorId, spell.name)
			end -- if roll
		end --  if spell
	end -- if not ok
	...
	and config.useEnchantment then
		...
		if object then
			if magnitude >= lockLevel then
				...
				if logLevel1 then
					mwse_log('%s: enemy "%s" managed to unlock door "%s" using an enchanted "%s"',
				...
				doorSound = spellSound
			elseif logLevel1 then
				mwse_log('%s: enemy "%s" failed unlocking door "%s" using an enchanted "%s"',
					funcPrefix, enemyRefId, doorId, object.name)
			end -- magnitude
		end -- if object
	end -- if not ok
	...
	and config.useBash then
		...
		if x >= config.minBashingStrength then
			local roll = math_random(60, (lockLevel / 2) + 100)
			if roll < x then
				ok = true -- lock bash
				if logLevel1 then
					mwse_log('%s: enemy "%s" managed to bash door "%s"', funcPrefix, enemyRefId, doorId)
				end
				if config.doorUnlockedMessage then
				...
				doorSound = bashSound
			elseif logLevel1 then
				mwse_log('%s: enemy "%s" failed bashing door "%s"', funcPrefix, enemyRefId, doorId)
			end -- if roll
		end -- if x
	end
...
local function doChase(lcEnemyRefId, doorRef)
	...
	if not enemyRef then
		if logLevel2 then
			mwse_log('%s callback: enemy ref "%s" not found', funcPrefix, lcEnemyRefId)
		end
		return
	end
	local enemyRefId = enemyRef.id
	...
	if not enemyMob then
		if logLevel2 then
			mwse_log('%s callback: mobile enemy "%s" not found', funcPrefix, lcEnemyRefId)
		end
		return
	end
	...
	if not actorType then
		if logLevel2 then
			mwse_log('%s callback: mobile enemy actor "%s" not found', funcPrefix, lcEnemyRefId)
		end
		return
	end
	...
	and lastDoorLockedRef then
		...
		if playerCell == lastDoorLockedCell then
			...
			if enemyCell == lastDoorLockedCell then
				check = true
			elseif doorsAreLinked(doorRef, lastDoorLockedRef) then
				check = true
			end
			if check then
				...
				if d > chaseMaxDistance then
					if logLevel2 then
						mwse_log('%s callback: lastDoorLockedPos:distance(player.position) > %s, skip', funcPrefix, chaseMaxDistance)
					end
					return
				end
				...
				if not doorCanBeOpened(lastDoorLockedRef, enemyMob) then
					if logLevel2 then
						mwse_log('%s callback: not doorCanBeOpened("%s", "%s"), skip', funcPrefix, lastDoorLockedRef, enemyRefId)
					end
					return
				end
			end
	...
	if not doorDest then
		if logLevel2 then
			mwse_log('%s callback: "%s" door "%s" has no destination, skip', funcPrefix, enemyRefId, doorRef.id)
		end
		return
	end
	...
	if logLevel4 then
		if lastDoorLockedRef then
			mwse_log('%s callback: destMarkerPos = "%s", lastDoorLockedRef = "%s"',
				funcPrefix,	destMarkerPos, lastDoorLockedRef)
		else
			mwse_log('%s callback: destMarkerPos = "%s"', funcPrefix, destMarkerPos)
		end
	end
	...
	if chaser then
		...
		and (not playerCell.isInterior) then -- both enemy and player in exterior
			...
			if d > chaseMaxDistance then
				if logLevel2 then
					mwse_log('%s callback: enemy "%s" dist = %d > max distance = %d, skip', funcPrefix, enemyRefId, d, chaseMaxDistance)
				end
				return
			end
		end
	...
	or playerCell.isInterior then
		...
		if d > 128 then
			...
			if not tes3.positionCell({reference = enemyRef, cell = doorDestCell,
				...
				if logLevel1 then
					mwse_log('%s callback: trying tes3.positionCell({reference = "%s", startCell = "%s", destCell = "%s", position = %s, orientation = %s}) failed',
						funcPrefix, enemyRefId, doorRefCell.editorName, doorDestCellEditorName, pos, ori)
				end
				return
			end
			if logLevel1 then
				mwse_log('%s callback: "%s" followed player through door "%s" from cell "%s" to cell "%s"',
...
local function ab01smtchsPT4(e)
	...
	if logLevel3 then
		mwse_log('%s: ab01smtchsPT4 doChase("%s", "%s")', modPrefix, lcEnemyRefId, doorRef)
	end
...
-- called from the door activate event, process enemies table
-- hopefully in the same frame before player changes cell
	...
	if enemyCount <= 0 then
		if logLevel3 then
			mwse_log("%s: no enemies, skip", funcPrefix)
		end
		return
	end
	...
	local function processEnemy(lcEnemyRefId)
		...
		if not enemyRef then
			if logLevel2 then
				mwse_log('%s: enemy ref "%s" not found', funcPrefix, lcEnemyRefId)
			end
			return
		end
		local enemyRefId = enemyRef.id
		...
		if logLevel2 then
			mwse_log('%s: checking enemy "%s"', funcPrefix, enemyRefId)
		end

		local enemyMob = enemyRef.mobile
		if not enemyMob then
			if logLevel2 then
				mwse_log('%s: mobile enemy "%s" not found', funcPrefix, lcEnemyRefId)
			end
			return
		end
		...
		if not isInCombat(enemyMob) then
			if logLevel1 then
				mwse_log('%s: enemy "%s" is not in combat any more, skip', funcPrefix, enemyRefId)
			end
			return
		end
		...
		if isDead(enemyMob) then -- better safe than sorry
			if logLevel1 then
				mwse_log('%s: enemy "%s" is dead, skip', funcPrefix, enemyRefId)
			end
			return
		end
		...
		if chaseLevel <= 2 then
			if not isPlayerDetected(enemyMob) then
				if logLevel1 then
					mwse_log('%s: enemy "%s" cannot detect player, skip', funcPrefix, enemyRefId)
				end
				return
			end
		end
		...
		if sameCellOrBothExteriors(enemyRef.cell, playerCell) then
			...
			if logLevel2 then
				mwse_log('%s: sameCellOrBothExteriors("%s", "%s", dist = %d',
		...
		if dist >= hugeDist then
			if logLevel1 then
				mwse_log('%s: enemy "%s" distance check failed, skip', funcPrefix, enemyRefId)
			end
			return
		end
		...
		and doorDest.cell.isOrBehavesAsExterior then
			...
			and isVampire(enemyRef) then
				if logLevel1 then
					mwse_log('%s: enemy "%s" is vampire, dayTime, skip', funcPrefix, enemyRefId)
				end
				return
			end
			if (not undeadChase)
			...
			and (not enemyMob.cell.isOrBehavesAsExterior) then
				...
				if string_lower(cellName):multifind({'tomb','burial'}, 1, true) then
					if logLevel2 then
						mwse_log('%s: undead creature "%s" is not following outside "%s", skip', funcPrefix, enemyRefId, cellName)
					end
					return
				end
			end
		...
		and ( not enemyCanSeePlayerAtDistance(enemyRef, dist) ) then
			if logLevel1 then
				mwse_log('%s: enemy "%s" cannot see player at %s distance, skip', funcPrefix, enemyRefId, dist)
			end
			return
		end

		if logLevel3 then
			mwse_log('%s: enemy "%s" dist = %d max dist = %d', funcPrefix, enemyRefId, dist, chaseMaxDistance)
		end

		if (dist > chaseMaxDistance)
		and sameCellOrDifferentKindOfCell(enemyRef.cell, player.cell) then
			if logLevel2 then
				mwse_log('%s: enemy "%s" "%s" dist = %d > max dist = %d, skip', funcPrefix, enemyRefId, dist, chaseMaxDistance)
			end
			return
		end
		...
		if not doorDest then
			if logLevel2 then
				mwse_log('%s: "%s" door %s timer started duration = %s', funcPrefix, enemyRefId, doorRef.id, dur)
			end
			return
		end
		...
		if logLevel2 then
			...
			mwse_log('%s: "%s" through door "%s" from cell "%s" to cell "%s" timer started duration = %s',
...
local function ab01smtchsPT5(e)
	...
	if logLevel1 then
		mwse_log('%s: ab01smtchsPT5 "%s":activate("%s")',
...
local function activate(e)
	...
	if logLevel2 then
		mwse_log('%s: "%s" activate("%s") skips = %s',
...
local function spellTick(e)
	...
	if effectId == tes3_effect_lock then
		...
		lastDoorLockedId = lastDoorLockedRef.id:lower()
...
local function magicEffectAdded(e)
	if not (
		(e.effectId == tes3_effect_open)
		or (e.effectId == tes3_effect_lock)
	) then
		return
	end
	local doorRef = e.target
	...
	-- mwse_log(('%s: %s() effect "%s" from "%s" added to "%s"', modPrefix, e['eventType'],
		-- table.find(tes3.effect, e.effectId), e.source.name, e.target))
	---spellTicked = true
	if e.effectId == tes3_effect_lock then
		lastDoorLockedRef = doorRef
		lastDoorLockedId = lastDoorLockedRef.id:lower()
	else -- unlocked, synchronize linked door if it is lastDoorLockedRef
		linkedDoorsSync(doorRef, lastDoorLockedRef)
...
local function moveBackChaser(lcEnemyRefId)
	...
	if not enemyRef then
		if logLevel2 then
			mwse_log(funcPrefix .. 'enemyRef not found')
		end
		return
	end
	local chaser = chasers[lcEnemyRefId]
	if not chaser then
		if logLevel2 then
			mwse_log(funcPrefix .. 'chaser not found')
		end
		return
	end
	...
	local dest
	if cid == '' then
		dest = ('exterior %s'):format(pos)
		tes3.positionCell({reference = enemyRef, position = pos})
	else
		dest = ('"%s" %s'):format(cid, pos)
	...
	if logLevel2 then
		mwse_log(funcPrefix .. 'moved back to ' .. dest)
	end
end
...
local function ab01smtchsPT6(e)
	...
	local lcEnemyRefId = data.lcId
	if logLevel3 then
		mwse_log('%s: ab01smtchsPT6 moveBackChaser("%s")', modPrefix, lcEnemyRefId)
	end
...
local function crimeWitnessedTrespass(e)
	if trespassChase then
		return
	end
	local witnessRef = e.witness
	if not witnessRef then
		return
	end
	local lcWitnessRefId = witnessRef.id:lower()
	if enemies[lcWitnessRefId] then
		if logLevel1 then
			mwse_log('%s %s(): removeEnemy("%s") trespass witness',
				modPrefix, e['eventType'], lcWitnessRefId)
		end
		witnessRef.tempData.ab01smchwtn = true
		removeEnemy(lcWitnessRefId)
	end
end
...
local function updateEnemiesAndChasers()
	...
	local function processChaserWithDelay(lcEnemyRefId, chaser)
		...
		if not enemyRef then
			...
			if not baseObj then
				if logLevel1 then
					mwse_log('%s: WARNING tes3.getObject("%s") failed', funcPrefix, lcEnemyRefId)
				end
			...
			if logLevel2 then
				if chaser.cellId == '' then
					mwse_log('%s: stored chaser "%s" not found', funcPrefix, lcEnemyRefId)
				else
					mwse_log('%s: stored chaser "%s".cellId ("%s") not found', funcPrefix, lcEnemyRefId, chaser.cellId)
				end
			end
			return false
		end
		...
		if logLevel1 then
			mwse_log('%s: "%s" initial cell "%s" dist = %d',
		...
		if not enemyMob then
			if logLevel1 then
				mwse_log('%s: "%s".mobile not available',
		...
		or playerCanSeeEnemyAtDistance(enemyRef, dist) then
			...
			and isVampire(enemyRef) then
				if logLevel3 then
					mwse_log('%s: isVampire("%s")', funcPrefix, lcEnemyRefId)
				end
				if enemyMob.health.normalized < 0.9 then
					if logLevel3 then
						mwse_log('%s: "%s".health.normalized < 0.9', funcPrefix, lcEnemyRefId)
					end
					if enemyMob.hasFreeAction
					and ( not isDead(enemyMob) ) then
						if logLevel2 then
							mwse_log('%s: "%s" teleporting away', funcPrefix, lcEnemyRefId)
						end
---@diagnostic disable-next-line: param-type-mismatch
			...
			if logLevel2 then
				mwse_log('%s: player can see "%s" at %s distance, no reset', funcPrefix, lcEnemyRefId, dist)
			end
		...
		if dist > chaseMaxDistance then
			---if playerCell.isInterior == mobCell.isInterior then
				if logLevel2 then
					mwse_log('%s: "%s" dist = %d > max dist = %d, reset', funcPrefix, lcEnemyRefId, dist, chaseMaxDistance)
				end
				return false
			---end
		end
		...
		if not enemyMob.hasFreeAction then --  paralyzed, dead, stunned, or otherwise unable to take action
			mwse_log('%s: enemy "%s" is not in combat any more, reset', funcPrefix, lcEnemyRefId)
			return false
		end
		...
		and ( not isPlayerDetected(enemyMob) ) then
			if logLevel1 then
				mwse_log('%s: enemy "%s" cannot detect player, reset', funcPrefix, lcEnemyRefId)
			end
			return false
		end
		...
		and isVampire(enemyRef) then
			if logLevel1 then
				mwse_log('%s: enemy "%s" is vampire, dayTime, reset', funcPrefix, lcEnemyRefId)
			end
			return false
		end
		...
		if hoursDiff > chaseMaxHours then
			if logLevel2 then
				mwse_log('%s: %s hours passed since "%s" started combat, reset', funcPrefix, hoursDiff, lcEnemyRefId)
			end
			return false
		end
...
-- and e.object.anyproperty is totally unreliable
	local fsp = e.object -- fucking stale pointer
	if not fsp then
		return
	end
	...
	refsByRef[fsp] = nil
	local key = table_find(enemies, fsp) -- table.find seems to work
	if not key then
		return
	end
	if not removeEnemy(key) then
		return
	end
	if logLevel2 then
		mwse_log('%s: %s() enemies["%s"] cleaned',
...
local function initOnce()
	...
	event.register('magicEffectAdded', magicEffectAdded) --yay!

	event.register('save', save)
	event.register('cellChanged', cellChanged)
	event.register('crimeWitnessed', crimeWitnessedTrespass, {filter = 'trespass'})
...
local function loaded(e)
	...
	if numChasers <= 0 then
		if logLevel3 then
			mwse_log("%s: loaded() chasers table empty", modPrefix)
		end
		return
	end

	-- dalayed to give the AI enough time to update
	timer.start({duration = 3 + math_random(), callback = updateEnemiesAndChasers})
end
	...
	if e.source == tes3_damageSource_magic then
		...
		---if magicEffectInstance then
			...
			if isVampire(ref) then
				...
				mwse_log("ref = %s, damage = %s, newDamage = %s, time = %s ms", ref.id, e.damage, newDamage, tes3.worldController.systemTime)
				e.damage = newDamage
			end
		---end
	end
...
--- 	})
	...
	sideBarPage:createYesNoButton({
		label = 'Trespass witnesses chase outside',
		configKey = 'trespassChase',
		description = [[Allow trespass crime witnesses to chase player outside.
Changes may require reloading to be effective.]]
	})
