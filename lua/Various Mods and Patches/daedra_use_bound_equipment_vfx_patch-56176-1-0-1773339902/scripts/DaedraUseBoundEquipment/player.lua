...
local function processDaedra(daedra)
    ...
    for ind, item in ipairs(daedra.type.inventory(daedra):getAll()) do
        ...
        then
            ...
            equipmentRemoved = processEquipment(item)
        end
    end
    
    if equipmentRemoved then
        if not daedra.recordId:find("dremora") then
            daedra:sendEvent('AddVfx', {
                model = types.Static.record("VFX_Summon_end").model,
            })
