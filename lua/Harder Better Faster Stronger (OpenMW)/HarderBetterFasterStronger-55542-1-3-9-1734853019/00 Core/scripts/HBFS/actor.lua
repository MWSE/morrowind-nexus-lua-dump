...
local function trackChanges(settings)
    if not state.key then
        state.key = settings.key
    end
    self:sendEvent("hbfs_actorIsReady", { settings = settings, changed = state.key ~= settings.key })
    state.key = settings.key
end
