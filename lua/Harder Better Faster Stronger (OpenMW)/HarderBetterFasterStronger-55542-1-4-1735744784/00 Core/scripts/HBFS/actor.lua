...
local function detectStatsDiffs()
    for attributeId, getter in pairs(T.Actor.stats.attributes) do
        ...
        if math.floor(diff + .5) ~= 0 then
            mDebug.print(string.format("%s's %s has been changed by %d since last check (%d -> %d)",
...
local function restoreStats()
    ...
    mDebug.print(string.format("%s stats have been restored: %s", actorId, statsToString(state.baseStats)))
end
...
local function applyNewStats(settings)
    ...
    mDebug.print(string.format("%s stats have been updated: %s", actorId, statsToString(state.updatedStats)))
end
...
local function onActive(settings, eventType)
    if state.baseStats then
        mDebug.print(string.format("%s's stats are already set", actorId))
    else
        setStats(settings)
    end
    self:sendEvent("hbfs_onActorReady", { settings = settings, type = eventType })
...
local function updateStats(settings, eventType)
    if not state.updatedStats or state.areDefaultSettings then
        if not state.updatedStats then
            mDebug.print(string.format("%s has no stats to update", actorId))
        end
        setStats(settings)
    else
        detectStatsDiffs()
...
local function onInactive()
    if not state.baseStats then
        mDebug.print(string.format("%s has no stats to restore", actorId))
        return
    end
...
return {
    ...
    eventHandlers = {
        hbfs_updateStats = function(data) updateStats(data.settings, data.type) end,
        hbfs_onActive = function(data) onActive(data.settings, data.type) end,
    },
}
