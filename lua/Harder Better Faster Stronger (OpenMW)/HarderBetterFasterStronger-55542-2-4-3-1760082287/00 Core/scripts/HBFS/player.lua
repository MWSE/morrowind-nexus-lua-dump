        ...
        if healthDamagingEffectIds[effect.id] then
            ...
            elseif effect.id == core.magic.EFFECT_TYPE.SunDamage then
                if self.cell.isExterior or self.cell:hasTag("QuasiExterior") then
                    ...
                    local sunVisibility = mDef.isOpenMW050 and core.weather.getCurrentSunVisibility(self.cell) or 1
...
local function uiModeChanged(oldMode, newMode, target)
    ...
    if settings[mStore.settings.minimumConditionRepairRequirement.key] then
        if newMode == "Repair" and oldMode ~= "Repair" then
            hideUnfixableItems()
        elseif newMode ~= "Repair" and oldMode == "Repair" then
...
local function addOnHitHandler()
    if not mDef.isOpenMW050 then return end
    ...
    if I.NCGDMW and I.NCGDMW.AddOnHitHandler then
        I.NCGDMW.AddOnHitHandler(handler)
    else
