...
local function getItemConditionRatio(loot, lootState)
    local record = loot.type.record(loot)
    local ratio
    local reason
    if loot.type == T.Container then
        if lootState.submerged then
    ...
    elseif lootState.isCorpse then
        ratio = math.random() ^ 2 * 1 / 2
        reason = "is a corpse"
    ...
    elseif record.class == "guard" then
        ...
        ratio = math.max(0, math.min(1, (1 - (lootState.baseFightValue / 100) ^ 2) - 1 / 4 + math.random() * 1 / 2))
        reason = string.format("has a base fight value of %d", lootState.baseFightValue)
    end
    return ratio, reason
end
...
local function modItemCondition(updates, refreshUi)
...
local function degradeLootItems(loot, lootState)
    ...
    for _, type in ipairs({ T.Armor, T.Weapon }) do
        for _, item in ipairs(inventory:getAll(type)) do
            ...
            if condition then
                local conditionRatio, reason = getItemConditionRatio(loot, lootState)
...
local function onActorDied(actor, actorState)
    ...
    degradeLootItems(actor, actorState)
end
...
local function fixObjects()
    local dataLists = { openedContainers = state.openedContainers }
...
return {
    ...
    eventHandlers = {
        ...
        [mDef.events.modItemCondition] = function(data) modItemCondition(data.updates, data.refreshUi) end,
