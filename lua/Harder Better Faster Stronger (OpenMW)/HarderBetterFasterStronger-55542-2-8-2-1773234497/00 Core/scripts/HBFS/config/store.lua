...
local function getAndCheckSettingValue(setting, value)
    value = deserializeValue(setting, value)
    local default = deserializeValue(setting, setting.default)
...
for _, section in pairs(sections) do
    for key, value in pairs(section.get():asTable()) do
        ...
        if setting and section.key == setting.section.key then
            local fixed
            setting.value, fixed = getAndCheckSettingValue(setting, value)
            if fixed and I.Activation then
