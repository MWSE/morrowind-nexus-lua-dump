...
local defaultPercentArgument = {
    ...
    withIncrease = false,
    allowHeaders = true,
    base = { min = nil, max = nil },
    increase = { min = nil, max = nil },
}
...
I.Settings.registerRenderer(mDef.renderers.percentAndIncrease, function(values, set, argument)
    if argument.requiresOpenMW50 then
        return {
            template = I.MWUI.templates.textNormal,
            props = { text = L("requiresOpenMW50") },
        }
    end
    ...
    }, hasHeader and L("basePercentValue") or nil, true)))
    ...
    if argument.withIncrease then
        ...
        }, hasHeader and L("increasePercentValue") or nil, true)))
        ...
        if not argument.perActor then
            ...
            }, argument.allowHeaders and L("actualPercentValue") or nil, true))
        end
    end

    return {
        type = ui.TYPE.Flex,
        props = { horizontal = true, size = util.vector2(250, 0) },
        content = ui.content {
            { external = { grow = 1 } },
            {
                type = ui.TYPE.Flex,
        ...
        paddedBox({
            ...
            events = {
                ...
                focusLoss = async:callback(function()
                    ...
                    local number = validateNumber(lastFromInput, { min = argument.min, max = values.to })
                    set(number and {
                        from = number,
                        to = values.to,
                        actual = values.actual,
        ...
        paddedBox({
            ...
            events = {
                ...
                focusLoss = async:callback(function()
                    ...
                    local number = validateNumber(lastToInput, { min = values.from, max = argument.max })
                    set(number and {
                        from = values.from,
                        to = number,
                        actual = values.actual
