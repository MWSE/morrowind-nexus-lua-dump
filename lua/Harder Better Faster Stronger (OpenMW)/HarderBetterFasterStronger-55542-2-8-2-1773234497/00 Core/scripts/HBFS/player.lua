...
local function changeMagicDamageTaken(deltaTime)
    if debug.isGodMode()
            or mStore.settings.magicDamagePercent.value.actual == 100
            and mStore.settings.sunDamagePercent.value.actual == 100 then return end
    ...
    if drainDamage ~= state.effectDamage.drain then
        local damage = ((mStore.settings.magicDamagePercent.value.actual - 100) / 100) * (drainDamage - state.effectDamage.drain)
        damageHealth(damage)
        log(string.format("Adding %.2f drain health damage to base %.2f", damage, drainDamage))
        state.effectDamage.drain = drainDamage
    end

    if magicDamageSum ~= 0 or sunDamageSum ~= 0 then
        local damage = ((mStore.settings.magicDamagePercent.value.actual - 100) / 100) * duration * magicDamageSum
                + ((mStore.settings.sunDamagePercent.value.actual - 100) / 100) * duration * sunDamageSum
        damageHealth(damage)
        log(string.format("Taken magic damage was %s by %.2f from %d effect(s) over %.2f seconds",
                (damage > 0) and "increased" or "reduced", damage, numEffects, duration))
    end
end

local function noBackRunning()
    if not mStore.settings.noBackRunning.value then return end
...
local function checkStolenItems(deltaTime)
    if not mStore.settings.deadGuardItemPickingIsCrime.value then return end
...
local function uiModeChanged(oldMode, newMode, target)
    ...
    log(string.format('UI mode changed from %s to %s, target is %s', oldMode, newMode, mTools.objectId(target)))
    if not oldMode and newMode == "Dialogue" and target and target.type.record(target).class == "guard" then
        log(string.format("Talking to guard %s", mTools.objectId(target)))
    ...
    if oldMode == "Dialogue" and not newMode and lastTalkingGuard and lastBounty > 0 then
        if T.Player.getCrimeLevel(self) == 0 then
            log(string.format("Bounty paid to guard %s", mTools.objectId(lastTalkingGuard.recordId)))
            ...
            log(string.format("Bounty not paid to guard %s", mTools.objectId(lastTalkingGuard.recordId)))
        end
    end
    if not oldMode then
        lastTalkingGuard = nil
    end
    if mStore.settings.minimumConditionRepairRequirement.value then
    ...
    if newMode == "Container" and target and target.type == T.Container then
...
local function addHandlers()
    local handler = function(attack)
        if attack.successful and attack.damage.health and (attack.sourceType == "melee" or attack.sourceType == "ranged") then
            local factor = mStore.settings.physicalDamagePercent.value.actual / 100
            log(string.format("Taken physical damage was %.2f, altered to %.2f", attack.damage.health, attack.damage.health * factor))
...
input.registerActionHandler("Use", async:callback(function(use)
    if not use or T.Actor.getStance(self) ~= T.Actor.STANCE.Spell then return end
    local effects, conjurationSource
    local spell = T.Actor.getSelectedSpell(self)
    if spell then
        effects = spell.effects
        conjurationSource = mDef.conjurationSource.spell
    else
        ...
        if item.type == T.Book then
            conjurationSource = mDef.conjurationSource.scroll
        elseif T.Item.getEnchantmentCharge(item) then
            conjurationSource = mDef.conjurationSource.enchant
        else
            log(string.format("Unknown summon type for item %s", mTools.objectId(item)), true)
            return
        end
    end
    for _, effect in ipairs(effects) do
        if effect.effect.school == "conjuration" then
            core.sendGlobalEvent(mDef.events.conjurationCast, { source = conjurationSource, time = core.getSimulationTime() })
            return
        end
    end
end))
