...
module.copyMap = function(source, target)
    for k, v in pairs(source) do
        target[k] = v
    end
end

module.statsToString = function(fromStats, toStats)
    local items = {}
    for id, value in pairs(toStats.attributes) do
        table.insert(items, string.format("%s=%s->%s", id, fromStats and fromStats.attributes[id] or "?", value))
    end
    for id, value in pairs(toStats.dynamicStats) do
        table.insert(items, string.format("%s=%s->%s", id, fromStats and fromStats.dynamicStats[id] or "?", value))
    end
