...
local function changeMagicDamageTaken(deltaTime)
    if debug.isGodMode()
            or mStore.settings.magicDamagePercent.get().actual == 100
            and mStore.settings.sunDamagePercent.get().actual == 100 then return end
    ...
    if drainDamage ~= state.effectDamage.drain then
        local damage = ((mStore.settings.magicDamagePercent.get().actual - 100) / 100) * (drainDamage - state.effectDamage.drain)
        damageHealth(damage)
    ...
    if magicDamageSum ~= 0 or sunDamageSum ~= 0 then
        local damage = ((mStore.settings.magicDamagePercent.get().actual - 100) / 100) * duration * magicDamageSum
                + ((mStore.settings.sunDamagePercent.get().actual - 100) / 100) * duration * sunDamageSum
        damageHealth(damage)
...
local function noBackRunning()
    if not mStore.settings.noBackRunning.get() then return end
...
local function checkStolenItems(deltaTime)
    if not mStore.settings.deadGuardItemPickingIsCrime.get() then return end
...
local function uiModeChanged(oldMode, newMode, target)
    ...
    if mStore.settings.minimumConditionRepairRequirement.get() then
...
local function addHandlers()
    local handler = function(attack)
        if attack.successful and attack.damage.health and (attack.sourceType == "melee" or attack.sourceType == "ranged") then
            local factor = mStore.settings.physicalDamagePercent.get().actual / 100
