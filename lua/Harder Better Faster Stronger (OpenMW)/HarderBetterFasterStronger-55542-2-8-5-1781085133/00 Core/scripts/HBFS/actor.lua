...
local function newState()
    return {
        debugMode = false,
        attributePercents = nil,
        dynamicStatPercents = nil,
        baseStats = nil,
        updatedStats = nil,
        followsPlayer = false,
        followerPercent = nil,
...
-- dedicated actor log function, to prevent loading store.lua for each actor, which is CPU costly
local function log(str)
    if not state or not state.debugMode then return end
    print(string.format("%s: %s", self.recordId, str))
end
...
local function detectStatsDiffs()
    ...
    for attributeId, getter in pairs(T.Actor.stats.attributes) do
        local diff = math.floor(getter(self).base) - math.floor(state.updatedStats.attributes[attributeId])
        if diff ~= 0 then
    ...
    for statId, getter in pairs(T.Actor.stats.dynamic) do
        local diff = math.floor(getter(self).base) - math.floor(state.updatedStats.dynamicStats[statId])
        if diff ~= 0 then
...
local function computeNewStats()
    ...
    if state.summonPercent then
        -- when summonPercent is sent by the global script this means a summon has been detected
        followerFactor = state.summonPercent / 100
    elseif state.followsPlayer then
        -- otherwise, if the actor is following the player, then he is a regular follower (probably NPC)
        followerFactor = state.followerPercent / 100
    end

    for attributeId in pairs(T.Actor.stats.attributes) do
        state.updatedStats.attributes[attributeId] = math.floor(0.5 + state.baseStats.attributes[attributeId] * getStatFactor(state.attributePercents[attributeId], followerFactor))
    end

    for statId in pairs(T.Actor.stats.dynamic) do
        state.updatedStats.dynamicStats[statId] = math.floor(0.5 + state.baseStats.dynamicStats[statId] * getStatFactor(state.dynamicStatPercents[statId], followerFactor))
    end
...
local function onUpdate(deltaTime)
    if deltaTime == 0 or not state or not state.baseStats then return end

    -- first check the stance because it's faster than checking controls
...
local function onLoad(data)
    ...
    if version < 2.84 then
        if data.state.baseStats then
            -- attempt to fix broken old state
            state = data.state
            restoreStats()
            data.state = nil
        end
