...
local function initActorData(actor, newData)
    if newData.isPlayerSummon and state.lastConjurationData then
        ...
        newData.isPlayerSummon = nil
        state.lastConjurationData = nil
    end
    if newData.isCorpse and mStore.settings.conditionalItemDegradation.get() then
...
local function onActorDied(actor)
    if not mStore.settings.conditionalItemDegradation.get() then return end
