...
local BURDEN_SPELLS_HARDCORE = shared.BURDEN_SPELLS_HARDCORE
...
local function refreshCache()
    ...
    C.BURDEN_HARDCORE = getFrom(secGeneral, "BURDEN_HARDCORE")
...
checkEquippedWeapon = function()
    ...
    -- GRIP COMPATIBILITY
    local gripRecords = storage.globalSection("GRIPRecords")
    local originalId = nil
    
    if gripRecords then
        local newToOld = gripRecords:get("NewToOldRecords")
        local sheathToNormal = gripRecords:get("SheathToNormal")
        
        if newToOld and newToOld[weapon.recordId] then
            originalId = newToOld[weapon.recordId]
        elseif sheathToNormal and sheathToNormal[weapon.recordId] then
            originalId = sheathToNormal[weapon.recordId]
        end
    end
    
    if originalId and Weapon.records[originalId] then
        checkRec = Weapon.records[originalId]
    end

    local weaponType = checkRec.type
    if IGNORED[weaponType] then
        clearBurden()
        return
    end
    if IGNORED_IDS[weapon.recordId:lower()] or (originalId and IGNORED_IDS[originalId:lower()]) then
        clearBurden()
        return
    end
    ...
    local tier = getTier(getAvgDamage(checkRec, formula), group)
    ...
    local msg = string.format('"%s" requires: %s',
        checkRec.name or weapon.recordId, table.concat(reasons, ", "))

    if C.BURDEN_ENABLED then
        local spellTable = C.BURDEN_HARDCORE and BURDEN_SPELLS_HARDCORE or BURDEN_SPELLS
        local spellId    = spellTable[tier]
        if not spellId then
            clearBurden()
            return
        end
...
local function onUiModeChanged(evData)
    if not C.MOD_ENABLED then return end
