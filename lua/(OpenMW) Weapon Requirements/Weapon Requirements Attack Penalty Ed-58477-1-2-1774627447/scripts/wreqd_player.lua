...
local shared        = require("scripts.wreqd_shared")
local data          = require("scripts.wreqd_data")
...
local secGeneral = storage.playerSection("SettingsWReqD")
local secReqs    = storage.playerSection("SettingsWReqDReqs")
local secAxe1h   = storage.playerSection("SettingsWReqDAxe1h")
local secAxe2h   = storage.playerSection("SettingsWReqDAxe2h")
local secMace    = storage.playerSection("SettingsWReqDMace")
local secHammer  = storage.playerSection("SettingsWReqDHammer")
local secStaff   = storage.playerSection("SettingsWReqDStaff")
local secBlade1h = storage.playerSection("SettingsWReqDBlade1h")
local secBlade2h = storage.playerSection("SettingsWReqDBlade2h")
local secShort   = storage.playerSection("SettingsWReqDShort")
local secSpear   = storage.playerSection("SettingsWReqDSpear")
local secBow     = storage.playerSection("SettingsWReqDBow")
local secXbow    = storage.playerSection("SettingsWReqDXbow")
local secThrown  = storage.playerSection("SettingsWReqDThrown")
...
-- Current active burden spell (nil if none)
...
local function checkEquippedWeapon()
    ...
    -- Requirements not met, apply burden spell for this tier
...
return {
    ...
    eventHandlers = {
        UiModeChanged = function(evData)
            if not C.MOD_ENABLED then return end
            if evData.oldMode == "Interface" and evData.newMode == nil then
                checkEquippedWeapon()
            end
        end,
    },
