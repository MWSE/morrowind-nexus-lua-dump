...
function hud:updateElements()
	...
	self.logger:debug("Updated elements")
end
	...
	if menuMulti then
		for modElementName, modElementConfig in pairs(self.mod.config.current.mods) do
			...
			if element then
				...
				self.logger:debug(string.format("Updated mod element '%s'", modElementConfig.name))
			end
		end
	end
	self.logger:debug("Updated mod elements")
end
