...
local function formatEnchantEffect(effect, enchantmentType)
...
local function effectInfo(record)
    ...
    for _, effect in ipairs(effectsData) do
        effects[#effects + 1] = formatEnchantEffect(effect)
    end
...
local function addDetailFields(result, item)
    ...
    result.effects = effectInfo(record)
    ...
    for _, item in ipairs(index) do
        ...
        then
            ...
            if limit == nil or #results < limit then
                results[#results + 1] = stripRuntimeFields(item)
            end
        end
    end
