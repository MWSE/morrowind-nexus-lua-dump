...
local function favoritesSettings()
...
local function favoriteIds()
    local stored = favoritesSettings():get('Favorites') or {}
    local favorites = {}
...
local function isFavorite(recordId)
    return favoriteIds()[tostring(recordId or '')] == true
end

local function setFavorite(recordId, value)
    recordId = tostring(recordId or '')
    if recordId == '' then
        return
    end

    local favorites = favoriteIds()
    favorites[recordId] = value == true or nil
    favoritesSettings():set('Favorites', favorites)
    favoritesSettings():set('FavoritesRevision', (favoritesSettings():get('FavoritesRevision') or 0) + 1)

    if callbacks.favoritesChanged then
        callbacks.favoritesChanged()
    end
end
...
local function requestSearch(selectFirst)
    selectFirstOnNextResults = selectFirst == true and (hasSearchQuery() or favoritesOnly)
...
local function makeAddToInventoryPanel()
    ...
    local favorite = item and isFavorite(item.id)

    local favoriteButton = makeButton(
        l10n(favorite and 'button_unstar' or 'button_star'),
        150,
        24,
        function()
            ...
            setFavorite(selected.id, not isFavorite(selected.id))
            if favoritesOnly then
                scrollOffset = 0
                requestSearch(true)
            else
                selected.favorite = isFavorite(selected.id)
                redraw()
            end
        end,
        false,
    ...
    favoriteButton.props.position = v2(0, 0)
    ...
    addButton.props.position = v2(0, 74)
    ...
    toggleGroupsButton.props.position = addAllowed and v2(0, 104) or v2(0, 32)

    local content = { favoriteButton }
    if addAllowed then
        ...
        content[#content].props.position = v2(6, 28)
        content[#content + 1] = {
...
local function makeFilterPanel()
    ...
    content[#content + 1] = makeSpacer(1, 4)
    content[#content + 1] = makeButton(l10n('filter_favorites_only'), 150, 24, function()
        favoritesOnly = not favoritesOnly
        scrollOffset = 0
        requestSearch(true)
        redraw()
    end, favoritesOnly, false)
...
local function makeResultRow(item)
    ...
    local label = string.format('%s%s', isFavorite(item.id) and '<*> ' or '    ', item.displayName)
