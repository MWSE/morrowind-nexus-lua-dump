...
that's a permanent ability (optionally with a particular attribute, and optionally only if the ability is not on the
player's race or birthsign). This isn't ideal because it can happen every frame under some circumstances. If anyone
knows a better way to do this, please let me know. ]]--
function this.getAbilityMag(effect, atr, onlyScript)
    local mag = 0
    ...
    for _ = 1, tes3.mobilePlayer.activeMagicEffectCount do
        ...
        if onlyScript
        and ( this.raceSpells:contains(source)
...
function this.getEffectMagNoScriptAbl(effect)
    ...
    and this.playerBirthsign then
        local scriptAblEffectMag = this.getAbilityMag(effect, nil, true)
...
local function maintainCorrectRatio(stat, newFort, oldFort, previousMaxStat, previousCurStat)
    ...
    --[[ If either of the values calculated for what the current/max stat would be without Fortify Magicka/Fatigue are
    negative, the results are really weird and very incorrect. (This can happen, for example, if the stat is damaged
    such that the current stat is less than the fortify magnitude.) In this case, nevermind, just maintain the existing
    ratio. ]]--
    if maxStatWouldBe >= 0
    and curStatWouldBe >= 0 then
