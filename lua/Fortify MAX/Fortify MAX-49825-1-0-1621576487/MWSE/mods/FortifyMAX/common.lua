...
local function maintainCorrectRatio(stat, newFort, oldFort, previousMaxStat, previousCurStat)
    ...
    if maxStatWouldBe == 0 then
        ratioWouldBe = 0
    else
        ratioWouldBe = curStatWouldBe / maxStatWouldBe
    end

    local maxStatWillBe = currentMaxStat - newFort
    local curStatAfterFortWearsOff = maxStatWillBe * ratioWouldBe
    local curStatRightNowShouldBe = curStatAfterFortWearsOff + newFort
    ...
    if newFort == oldFort then
        ...
        if previousMaxStat == 0 then
            ratioShouldBe = 0
        else
            ratioShouldBe = previousCurStat / previousMaxStat
        end
