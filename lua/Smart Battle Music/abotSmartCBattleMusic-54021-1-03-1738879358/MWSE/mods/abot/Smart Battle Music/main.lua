...
local keepPlayingLastExploreTrack, eventPriority, onlyStartedByPlayerTeam
...
local function updateFromConfig()
	...
	onlyStartedByPlayerTeam = config.onlyStartedByPlayerTeam
...
local function isGuard(mob)
	local objClass = mob.reference.object.class
	if not objClass then
		return false
	end
	if string.lower(objClass.id) == 'guard' then
		return true
	end
	return false
end
...
keep playing "%s" lastExploreTrack]],
			...
			---return false
			e.music = lastExploreTrack
			return
		end

		local currTrack = e.music ---audioController.currentMusicFilePath
		if currTrack
...
now playing "%s" lastExploreTrack]],
	...
	if logLevel2 then
		mwse.log([[%s: musicSelectTrack(e) e.situation = %s (%s) onlyStartedByPlayerTeam = %s]], modPrefix, e.situation, situations[e.situation], onlyStartedByPlayerTeam)
	end
	...
	local teamAttacker = isValidFollower(lastAttacker)
		and isValidMobile(lastAttacker)

	if logLevel1 then
		mwse.log([[%s: musicSelectTrack(e) e.situation = %s (%s)
lastAttacker = "%s", lastTarget = "%s",
onlyStartedByPlayerTeam = %s, teamAttacker = %s]], modPrefix, e.situation, situations[e.situation],
		lastAttacker.reference, lastTarget.reference, onlyStartedByPlayerTeam, teamAttacker)
	end
	...
	if onlyStartedByPlayerTeam then
		if (mobilePlayer == lastAttacker)
		or teamAttacker then
			return
		end
	end

	--[[if mobilePlayer == lastAttacker then
		return
	end
	...
	if logLevel1 then
		mwse.log([[%s: musicSelectTrack(e) e.situation = %s (%s)
lastAttacker = "%s", lastTarget = "%s",
onlyStartedByPlayerTeam = %s, teamAttacker = %s skip changing music]], modPrefix, e.situation, situations[e.situation],
			lastAttacker.reference, lastTarget.reference, onlyStartedByPlayerTeam, teamAttacker)
	end
...
local function initOnce()
	...
	event.register('combatStopped',	combatStopped, {priority = eventPriority})
end
...
Keep playing last Explore track after battle end.]], 'keepPlayingLastExploreTrack'),
	...
	controls:createYesNoButton({
		label = 'Only started by Player Team',
		description = getYesNoDescription([[Default: %s.
Play Battle Music only when battle is started by Player Team.]], 'onlyStartedByPlayerTeam'),
		variable = createConfigVariable('onlyStartedByPlayerTeam')
	})
