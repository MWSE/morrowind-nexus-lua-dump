...
negativeLightColor = 128, -- negative grayscale light color
negativeLightRadiusMul = 2, -- negative light radius multipliergrayscale alpha
logLevel = 0,
}
...
assert(config) -- for Visual Studio plugin
local processNegativeLights, negativeLightColor, negativeLightRadiusMul
...
local function tweakNegativeLights()
	if logLevel1 then
		mwse.log('%s: tweakNegativeLights()', modPrefix)
	end
	...
	for light in tes3.iterateObjects(tes3.objectType.light) do
		if light.isNegative then
			if not (
				...
				for i = 1, 3 do
					color[i] = negativeLightColor
				end
				local radius = 0
				if negativeLightRadiusMul > 0.01 then
					radius = round(light.radius * negativeLightRadiusMul)
				else
				...
				---light.modified = true
				if logLevel2 then
					mwse.log('%s: "%s".color = {%s, %s, %s, %s}, radius = %s',
						modPrefix, light.id, color[1], color[2], color[3], color[4], radius)
				end
			end
		end
	end
...
local function updateFromConfig()
	...
	negativeLightColor = config.negativeLightColor
	negativeLightRadiusMul = config.negativeLightRadiusMul
end
...
local function modConfigReady()
	...
	local sideBarPage = template:createSideBarPage({
		label = modName,
		showHeader = false,
		description = [[Simple interior cells lightness tweaker.
With a couple more handy options.]],
	...
	category:createYesNoButton({
		...
		description = [[Enables changes to interior lightness.
...
but will conflict with other mods changing cell stored ambient/sun/fog values.]],
	...
	category:createSlider({
		...
		configKey = 'lightnessPerc'
		,min = 1, max = 200, jump = 5,
		description = 'No-rest-allowed interiors lightness. Usually houses/rooms.'
	})

	category:createSlider({
		label = 'Rest-allowed Interior lightness %s%%',
		configKey = 'caveLightnessPerc'
		,min = 1, max = 200, jump = 5,
		description = 'Rest-allowed interiors lightness. Usually caves/dungeons.'
	})

	category:createYesNoButton({
		label = 'Fix interior fog density',
		description = [[Automatically fix interior fog density if needed.
Some video cards need fog density > 0 to work correctly, else they can display a black screen in near 0 fog density cells.
There are tools to fix this directly in the bugged mod (tes3cmd scripts) or generating a mod patch for the whole loading list (e.g. tes3cmd multipatch), but this option can fix the fog density on the fly if needed when loading the cell in game.]],
		configKey = 'fixInteriorFogDensity'
	})

	category:createDropdown({
		label = 'Merge colors:',
		options = getOptions(),
		description = [[Meant to be used only with shaders requiring sun/ambient interior color to be near 0.
...
2. Ambient to Sun = when darkening is enabled, if possible decrease interior ambient color to 1 and move/merge original ambient color to sun color instead.]],
		configKey = 'mergeColors'
	})

	category:createYesNoButton({
		label = 'Process negative Lights',
		description = [[Process negative lights (skipping scripted ones). Makes their color configurable.]],
		configKey = 'processNegativeLights',
		restartRequired = true
	})

	category:createSlider({
		label = 'Negative lights color %s',
		description = 'Black/Gray color for negative dark lights.',
		configKey = 'negativeLightColor',
		min = 0, max = 255, jump = 5,
		---restartRequired = true
	})

	category:createSlider({
		label = 'Negative lights radius multiplier %s',
		description = 'Radius multiplier for negative dark lights.',
		configKey = 'negativeLightRadiusMul',
		min = 0, max = 5, decimalPlaces = 1, step = 0.1, jump = 1,
		---restartRequired = true
	})
