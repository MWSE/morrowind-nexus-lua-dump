...
logLevel = 0,
caves = {'cave', 'dungeon', 'grotto', 'ruin', 'sepulc', 'tomb'},
houses = {'inn'},
}
...
local config, caves, houses

local function loadConfig()
	config = mwse.loadConfig(configName, defaultConfig)
	caves = {}
	for i, v in pairs(config.caves) do
		caves[i] = v:lower()
	end
	houses = {}
...
local function isCave(cell)
	local lcId = cell.id:lower()
	if string.multifind(lcId, caves, 1, true) then
		return true
	end
	if string.multifind(lcId, houses, 1, true) then
		return false
	end
	return not cell.restingIsIllegal
end
