...
local function onCrimeWitnessed(e)
	...
	mwse.log(e.victim.id) -- base
	mwse.log(e.witness.baseObject.id) -- instance

	if e.victim.id ~= e.witness.baseObject.id then
		return
	end

	if e.witness.mobile.alarm < e.witness.baseObject.aiConfig.alarm then
		e.witness.mobile.alarm = e.witness.baseObject.aiConfig.alarm
	end
...
local function initialized(e)
	...
	event.register("barterOffer", onBarterEnd, {priority = -99999})
