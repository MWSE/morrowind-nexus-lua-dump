...
-- NOTE - this has always been a stale pointer and our ref cleanup works just fine without it anyway
...
local function conditionCheck()
    ...
    if (cell.isOrBehavesAsExterior) then
        ...
        isBugsVisible = isActiveHours(hour) and isValidWeather(weather) and isValidDay(day) and isWilderness(cell) and
            not (table.empty(availableBugs))
    end
...
event.register("initialized", function()
    if tes3.isModActive("OAAB_Data.esm") then
        ...
        -- this has always been a stale pointer and our ref cleanup works just fine without it anyway
