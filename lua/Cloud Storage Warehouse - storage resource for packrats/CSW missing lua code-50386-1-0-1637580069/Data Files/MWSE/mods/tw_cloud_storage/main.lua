...
local function StoreItems(e) 
    ...
    for _, stack in pairs(tes3.player.object.inventory) do
        if (isItemStoreEnabled(stack.object)) then
...
-- always available from main menu :)
    local ref
        ref = tes3.getReference("tw_weapons_store")
        ...
        ref = tes3.getReference("tw_cloths_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_armour_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_books_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_potions_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_ingred_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_tools_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_misc_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
----------------------------------------------------------------------------------------------------
        ref = tes3.getReference("tw_scrolls_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_soulgems_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_keys_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
        ref = tes3.getReference("tw_apparatus_store")
        for i, stack in pairs(ref.object.inventory) do
            tes3.transferItem{from=ref, to=tes3.player, item=stack.object, count=stack.count, playSound=true}
        end
----------------------------------------------------------------------------------------------------
...
local function OpenStore(e)
    ...
    --local aButtons = {"Weapons", "Armor", "Cloths", "Ingredients", "Alchemy", "Books", "Scrolls", "Soulgems", "Keys", "Tools", "Apparatus", "Misc", "Cancel"}
    if lStore == 1 then 
        messageBox{
            ...
            buttons = { {text = "Weapons",     callback = function() timer.delayOneFrame(function() AllowActivate = true; tes3.player:activate(tes3.getReference("tw_weapons_store")) end) end },
                        ...
                        {text = "Cloths",      callback = function() timer.delayOneFrame(function() AllowActivate = true; tes3.player:activate(tes3.getReference("tw_cloths_store")) end) end },
    ...
    if e.activator == tes3.player then
        if (e.target.id == "tw_store01_act") or (e.target.id == "tw_store02_act") or (e.target.id == "tw_store03_act") or 
           (e.target.id == "tw_store04_act") or (e.target.id == "tw_store05_act") or (e.target.id == "tw_store06_act") then
            -- this is a general store so no direct stoe container.
            lStore = 1      -- Store all               
