...
local tes3_effect_daedricIntervention -- must wait and set it in modConfigReady

-- set in modConfigReady()
local interventions, templeMarkerObj, divineMarkerObj
local daedricMarkerObj
...
local function magicCasted(e)
	...
	---tes3ui.showNotifyMenu("%s casted", interventions[interventionId])
	local markerRef
	...
	elseif interventionId == tes3_effect_divineIntervention then
		markerRef = tes3.findClosestExteriorReferenceOfObject({object = divineMarkerObj})
	elseif ( tes3_effect_daedricIntervention
	and daedricMarkerObj
		and (interventionId == tes3_effect_daedricIntervention)	) then
		markerRef = tes3.findClosestExteriorReferenceOfObject({object = daedricMarkerObj})
	end
...
local function keyDown(e)
	...
	local daedric
	if tes3_effect_daedricIntervention then
	  daedric = getClosestInterventionDestination(tes3_effect_daedricIntervention)
	end
		local s
		if almsivi then
			s = almsivi
...
local function modConfigReady()
	...
	daedricMarkerObj = tes3.getObject('OJ_maker_dae_int')
  ...
  tes3_effect_daedricIntervention = tes3.effect.daedricIntervention
  if tes3_effect_daedricIntervention then
	interventions[tes3_effect_daedricIntervention] = 'Daedric Intervention'
  end
