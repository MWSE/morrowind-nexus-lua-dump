...
local i18n = mwse.loadTranslations("LARPER")
local menuID = tes3ui.registerID("pg_rr_argonianInfo")

if mwse.buildDate == nil or mwse.buildDate < 20230528 then
    mwse.log("[LARPER] Build date of %s does not meet minimum build date of 2023 05 28. Mod not initialized.", mwse.buildDate)
	event.register(tes3.event.modConfigReady, function()
		tes3.messageBox("LARPER not initialized. Run MWSE-Update.exe.")
	end)
    return
end
...
event.register(tes3.event.modConfigReady, function()
	mwse.log("[LARPER] " .. versionString .. " loaded successfully.")
end)
