            ...
            if (table.size(ctx.MethodTraversalRecord) < table.size(ctx.LastMTR)) then
                ...
                if (self.BeatsLastMTR(ctx, taskIndex, currentDecompositionIndex) == false) then
                    ...
                    if (ctx.DebugMTR) then
                        table.insert(ctx.MTRDebug, string.format("REPLAN FAIL %s", self.Subtasks[taskIndex].Name))
                    end
...
function Selector:OnDecomposeCompoundTask(ctx, task, taskIndex, oldStackDepth, result)
    ...
    if (status == EDecompositionStatus.Rejected) then
        if (ctx.LogDecomposition) then
            log("%i - Selector.OnDecomposeCompoundTask:%s: Decomposing %s was rejected.", ctx.CurrentDecompositionDepth, GetKey(status, EDecompositionStatus), task.Name)
        end
        result:clear()
        return EDecompositionStatus.Rejected
    end
    ...
    if (status == EDecompositionStatus.Failed) then
        ...
        if (ctx.LogDecomposition) then
            log("%i - Selector.OnDecomposeCompoundTask:%s: Decomposing %s failed.", ctx.CurrentDecompositionDepth, GetKey(status, EDecompositionStatus), task.Name)
        end
        result:copy(self.Plan)
    ...
    while (table.size(subPlan.list) > 0) do
        local p = subPlan:pop()
        if (ctx.LogDecomposition) then
            log("%i - Selector.OnDecomposeCompoundTask:Decomposing %s:Pushed %s to plan!", ctx.CurrentDecompositionDepth, task.Name, p.Name)
        end
        self.Plan:push(p)
    end
...
function Selector:OnDecomposeSlot(ctx, task, taskIndex, oldStackDepth, result)
    ...
    if (status == EDecompositionStatus.Rejected) then
        if (ctx.LogDecomposition) then
            log("%i - Selector.OnDecomposeSlot:%s: Decomposing %s was rejected.", ctx.CurrentDecompositionDepth, GetKey(status, EDecompositionStatus), task.Name)
        end
        result:clear()
        return EDecompositionStatus.Rejected
    end
    ...
    if (status == EDecompositionStatus.Failed) then
        ...
        if (ctx.LogDecomposition) then
            log("%i - Selector.OnDecomposeSlot:%s: Decomposing %s failed.", ctx.CurrentDecompositionDepth, GetKey(status, EDecompositionStatus), task.Name)
        end
        result:copy(self.Plan)
        return EDecompositionStatus.Failed
    end
    ...
    while (table.size(subPlan.list) > 0) do
        local p = subPlan:pop()
        if (ctx.LogDecomposition) then
            log("%i - Selector.OnDecomposeSlot:Decomposing %s:Pushed %s to plan!", ctx.CurrentDecompositionDepth, task.Name, p.Name)
        end
        self.Plan:push(p)
    end
