...
function Selector:IsValid(ctx)
  ...
  if (CompoundTask.IsValid(self, ctx) == false) then
    if (ctx.LogDecomposition) then print(string.format("Selector.IsValid:Failed:Preconditions not met!\n\t- %i",
  ...
  if (table.size(self.Subtasks) == 0) then
    if (ctx.LogDecomposition) then print(string.format("Selector.IsValid:Failed:No sub-tasks!\n\t- %i",
        ctx.CurrentDecompositionDepth))
    end
    return false
  end
...
function Selector:OnDecompose(ctx, startIndex, result)
  ...
  for taskIndex = startIndex, table.size(self.Subtasks) do
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecompose:Task index: %i: %s\n\t- %i", taskIndex,
    ...
    if (ctx.LastMTR ~= nil and table.size(ctx.LastMTR) > 0) then
      if (table.size(ctx.MethodTraversalRecord) < table.size(ctx.LastMTR)) then
        ...
        if (self.BeatsLastMTR(ctx, taskIndex, currentDecompositionIndex) == false) then
          ...
          if (ctx.LogDecomposition) then print(string.format("Selector.OnDecompose:Rejected:Index %i is beat by last method traversal record!\n\t- %i"
...
function Selector:OnDecomposeTask(ctx, task, taskIndex, oldStackDepth, result)
  if (task:IsValid(ctx) == false) then
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeTask:Failed:Task %s.IsValid returned false!\n\t- %i",
...
function Selector:OnDecomposePrimitiveTask(ctx, task, taskIndex, oldStackDepth, result)
  ...
  if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeTask:Pushed %s to plan!\n\t- %i", task.Name,
      ctx.CurrentDecompositionDepth))
  end
...
function Selector:OnDecomposeCompoundTask(ctx, task, taskIndex, oldStackDepth, result)
  ...
  if (status == EDecompositionStatus.Rejected) then
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeCompoundTask:%s: Decomposing %s was rejected.\n\t- %i",
  ...
  if (status == EDecompositionStatus.Failed) then
    ...
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeCompoundTask:%s: Decomposing %s failed.\n\t- %i",
        GetKey(status, EDecompositionStatus),
        task.Name, ctx.CurrentDecompositionDepth))
    end
    result:copy(self.Plan)
  ...
  while (table.size(subPlan.list) > 0) do
    local p = subPlan:pop()
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeCompoundTask:Decomposing %s:Pushed %s to plan!\n\t- %i"
  ...
  if (ctx.HasPausedPartialPlan) then
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeCompoundTask:Return partial plan at index %i!\n\t- %i",
...
function Selector:OnDecomposeSlot(ctx, task, taskIndex, oldStackDepth, result)
  ...
  -- If status is rejected, that means the entire planning procedure should cancel.
  if (status == EDecompositionStatus.Rejected) then
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeSlot:%s: Decomposing %s was rejected.\n\t- %i",
        GetKey(status, EDecompositionStatus),
        task.Name, ctx.CurrentDecompositionDepth))
    end
    result:clear()
  ...
  if (status == EDecompositionStatus.Failed) then
    ...
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeSlot:%s: Decomposing %s failed.\n\t- %i",
  ...
  while (table.size(subPlan.list) > 0) do
    local p = subPlan:pop()
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeSlot:Decomposing %s:Pushed %s to plan!\n\t- %i",
        task.Name,
  ...
  if (ctx.HasPausedPartialPlan) then
    if (ctx.LogDecomposition) then print(string.format("Selector.OnDecomposeSlot:Return partial plan!\n\t- %i",
        ctx.CurrentDecompositionDepth))
    end
    result:copy(self.Plan)
    return EDecompositionStatus.Partial
