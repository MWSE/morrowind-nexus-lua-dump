...
function Sequence:IsValid(ctx)
    ...
    if (CompoundTask.IsValid(self, ctx) == false) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.IsValid:Failed:Preconditions not met!\n\t- %i",
    ...
    if (table.size(self.Subtasks) == 0) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.IsValid:Failed:No sub-tasks!\n\t- %i",
                ctx.CurrentDecompositionDepth))
        end
        return false
    end
...
function Sequence:OnDecompose(ctx, startIndex, result)
    ...
    for taskIndex = startIndex, table.size(self.Subtasks) do
        ...
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecompose:Task index: %i: %s\n\t- %i", taskIndex, task.Name,
                ctx.CurrentDecompositionDepth))
        end
...
function Sequence:OnDecomposeTask(ctx, task, taskIndex, oldStackDepth, result)
    if (task:IsValid(ctx) == false) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeTask:Failed:Task %s.IsValid returned false!\n\t- %i"
    ...
    elseif (task:isInstanceOf(PausePlanTask)) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeTask:Return partial plan at index %i!\n\t- %i",
...
function Sequence:OnDecomposePrimitiveTask(ctx, task, taskIndex, oldStackDepth, result)
    ...
    if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeTask:Pushed %s to plan!\n\t- %i", task.Name,
            ctx.CurrentDecompositionDepth))
    end
...
function Sequence:OnDecomposeCompoundTask(ctx, task, taskIndex, oldStackDepth, result)
    ...
    if (status == EDecompositionStatus.Rejected) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeCompoundTask:%s: Decomposing %s was rejected.\n\t- %i"
    ...
    if (status == EDecompositionStatus.Failed) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeCompoundTask:%s: Decomposing %s failed.\n\t- %i",
    ...
    while (table.size(subPlan.list) > 0) do
        local p = subPlan:pop()
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeCompoundTask:Decomposing %s:Pushed %s to plan!\n\t- %i"
    ...
    if (ctx.HasPausedPartialPlan) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeCompoundTask:Return partial plan at index %i!\n\t- %i"
                , taskIndex, ctx.CurrentDecompositionDepth))
        end
    ...
    result:copy(self.Plan)
    if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeCompoundTask:Succeeded!\n\t- %i",
            ctx.CurrentDecompositionDepth))
    end
...
function Sequence:OnDecomposeSlot(ctx, task, taskIndex, oldStackDepth, result)
    ...
    -- If result is null, that means the entire planning procedure should cancel.
    if (status == EDecompositionStatus.Rejected) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeSlot:%s: Decomposing %s was rejected.\n\t- %i",
                GetKey(status, EDecompositionStatus),
                task.Name, ctx.CurrentDecompositionDepth))
        end
    ...
    -- If the decomposition failed
    if (status == EDecompositionStatus.Failed) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeSlot:%s: Decomposing %s failed.\n\t- %i",
                GetKey(status, EDecompositionStatus), task.Name, ctx.CurrentDecompositionDepth))
        end
    ...
    while (table.size(subPlan.list) > 0) do
        local p = subPlan:pop()
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeSlot:Decomposing %s:Pushed %s to plan!\n\t- %i",
                task.Name,
    ...
    if (ctx.HasPausedPartialPlan) then
        if (ctx.LogDecomposition) then print(string.format("Sequence.OnDecomposeSlot:Return partial plan at index %i!\n\t- %i",
                taskIndex, ctx.CurrentDecompositionDepth))
        end
        if (taskIndex < table.size(self.Subtasks)) then
            ctx.PartialPlanQueue:push((function()
