...
if configPlayer.tweaks.s_HouseNameDisplay ~= 'HouseNameDisplay_Full' or configPlayer.tweaks.b_ShowFactionRankInList then
    ...
    API.overrideLineBuilder('FACTION', function(factionId)
        ...
        if configPlayer.tweaks.b_ShowFactionRankInList then
            base.value = function()
                if self.type.isExpelled(self, factionId) then
                    return { string = API.Constants.Strings.EXPELLED, color = API.Constants.Colors.DAMAGED }
                else
                    local rank = self.type.getFactionRank(self, factionId)
                    return { string = factionRecord.ranks[rank] and factionRecord.ranks[rank].name or '' }
                end
            end
        end
        return base
