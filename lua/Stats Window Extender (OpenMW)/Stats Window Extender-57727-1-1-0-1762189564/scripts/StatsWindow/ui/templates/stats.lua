...
Templates.LEFT_PANE_RATIO = 0.44
Templates.RIGHT_PANE_RATIO = 1 - Templates.LEFT_PANE_RATIO
...
Templates.LEFT_OFFSET_WIDTH = Templates.BOX_OUTER_PADDING + Templates.BORDER_WIDTH_TOTAL
Templates.MIN_SIZE = v2(244, 4 +
...
Templates.MIN_FULL_SIZE = v2(500, 342)
Templates.TEXTURES = {
...
Templates.updateStatsWindow = function(layout)
    ...
    layout.props.size = util.vector2(
        math.max(layout.props.size.x, Templates.MIN_SIZE.x),
        math.max(layout.props.size.y, Templates.MIN_SIZE.y)
    )

    local windowWidth = layout.props.size.x
    local windowHeight = layout.props.size.y - Templates.HEADER_HEIGHT - Templates.BORDER_WIDTH_TOTAL

    local minLeftWidth = Templates.MIN_FULL_SIZE.x * Templates.LEFT_PANE_RATIO + Templates.BOX_OUTER_PADDING
    local minLeftOffsetWidth = minLeftWidth + Templates.LEFT_OFFSET_WIDTH

    local paneArrangement = configPlayer.window.s_PaneArrangement
    local stackVertically = paneArrangement == 'Panes_Stacked' or (paneArrangement == 'Panes_Auto' and windowWidth <= (minLeftOffsetWidth + 160))
    ...
    if stackVertically then 
        ...
        leftPane.props.size = util.vector2(windowWidth - Templates.BORDER_WIDTH_TOTAL - Templates.BOX_OUTER_PADDING * 2, leftPane.userData.contentHeight)
        ...
        rightPane.props.size = util.vector2(windowWidth - Templates.BORDER_WIDTH_TOTAL - Templates.BOX_OUTER_PADDING * 2, windowHeight - rightPane.props.position.y)
        ...
        rightPane.content[#rightPane.content].props.size = util.vector2(
            0,
            windowHeight - rightPane.props.position.y - (rightPane.userData.contentHeight - lastRightBox.userData.contentHeight) - Templates.BOX_OUTER_PADDING
        )

        if intRe then
            -- paneDivider.template = I.MWUI.templates.horizontalLine
            -- paneDivider.props.position = util.vector2(
            --     Templates.BOX_OUTER_PADDING,
            --     leftPane.props.position.y + leftPane.props.size.y + Templates.BOX_OUTER_PADDING / 2
            -- )
            -- paneDivider.props.size = util.vector2(-Templates.BOX_OUTER_PADDING * 2, Templates.BORDER_THICKNESS)
            paneDivider.props.visible = false
        end
    else
        leftPane.props.position = util.vector2(Templates.BOX_OUTER_PADDING, Templates.BOX_OUTER_PADDING)
        rightPane.props.visible = (windowWidth >= minLeftOffsetWidth)
        if not rightPane.props.visible then
            leftPane.props.size = util.vector2(minLeftWidth - Templates.BOX_OUTER_PADDING * 2, windowHeight)
            rightPane.props.size = util.vector2(0, windowHeight)
        elseif windowWidth < Templates.MIN_FULL_SIZE.x then
            leftPane.props.size = util.vector2(minLeftWidth - Templates.BOX_OUTER_PADDING * 2, windowHeight)
            rightPane.props.position = util.vector2(minLeftWidth, Templates.BOX_OUTER_PADDING)
            rightPane.props.size = util.vector2(windowWidth - minLeftWidth - Templates.BOX_OUTER_PADDING - Templates.BORDER_WIDTH_TOTAL, windowHeight)
        else
            leftPane.props.size = util.vector2(windowWidth * Templates.LEFT_PANE_RATIO - Templates.BOX_OUTER_PADDING, windowHeight)
            rightPane.props.position = util.vector2(windowWidth * Templates.LEFT_PANE_RATIO + Templates.BOX_OUTER_PADDING, Templates.BOX_OUTER_PADDING)
            rightPane.props.size = util.vector2(windowWidth * Templates.RIGHT_PANE_RATIO - Templates.BOX_OUTER_PADDING * 2 - Templates.BORDER_WIDTH_TOTAL, windowHeight)
        end
        ...
        lastLeftBox.props.size = util.vector2(
            0,
            windowHeight - leftPaneVisibleHeight - Templates.BOX_OUTER_PADDING * 2
        )
        lastRightBox.props.size = util.vector2(
            0,
            windowHeight - rightPaneVisibleHeight - Templates.BOX_OUTER_PADDING * 2
        )
    end
...
Templates.statsWindow = function(sections, allowPin, scrollPosList)
    ...
    base.props = {
        ...
        size = util.vector2(
            math.max(windowOptions.f_StatsW * layerSize.x, Templates.MIN_SIZE.x),
            math.max(windowOptions.f_StatsH * layerSize.y, Templates.MIN_SIZE.y)
        ),
    }
