...
local function saveWindowPos()
    ...
    storage.playerSection('Settings/StatsWindow/2_WindowOptions'):set('f_StatsX', helpers.roundToPlaces(statsWindow.element.layout.props.position.x / layerSize.x, 5))
    storage.playerSection('Settings/StatsWindow/2_WindowOptions'):set('f_StatsY', helpers.roundToPlaces(statsWindow.element.layout.props.position.y / layerSize.y, 5))
    storage.playerSection('Settings/StatsWindow/2_WindowOptions'):set('f_StatsW', helpers.roundToPlaces(statsWindow.element.layout.props.size.x / layerSize.x, 5))
    storage.playerSection('Settings/StatsWindow/2_WindowOptions'):set('f_StatsH', helpers.roundToPlaces(statsWindow.element.layout.props.size.y / layerSize.y, 5))
end
...
statsWindow.updateTrackedStats = function()
    for statId, getter in pairs(trackedStats) do
        ...
        if not helpers.tableEquals(newValue, statsWindow.stats[statId]) then
            statsWindow.stats[statId] = newValue

            local function processSections(sections)
                for _, section in pairs(sections or {}) do
                    if section.trackedStats and section.trackedStats[statId] and section.builder then
                        section.lines = {}
                        section.sections = {}
                        section.builder()
                    end
