...
local log = mwse.Logger.new{ modName = "Character Sound Overhaul", moduleName = "soundBuilder", level = config.logLevel }
...
local function createSound(objectId, filename, soundTable, i)
	...
	if not timestamp then
		log:warn("createSound: file missing on disk, skipping '%s' (%s)", objectId, filename)
		return
	end
	...
	if not fileUnmodified and not fileAddedToManifest then
...
function this.build()
	log:debug("build: loading manifest from %s", manifestPath)
	...
	if fileRemovedFromManifest or fileAddedToManifest then
		log:info("build: manifest changed (added=%s, removed=%s), saving", fileAddedToManifest, fileRemovedFromManifest)
