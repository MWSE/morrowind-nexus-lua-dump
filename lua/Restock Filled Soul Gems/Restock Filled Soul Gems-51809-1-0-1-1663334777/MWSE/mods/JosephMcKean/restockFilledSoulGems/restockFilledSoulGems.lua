...
local logger =
    require("JosephMcKean.restockFilledSoulGems.logging").createLogger(
        "restockFilledSoulGems")
local config = require("JosephMcKean.restockFilledSoulGems.config").config
local validFilledSoulGems = require(
                                "JosephMcKean.restockFilledSoulGems.validFilledSoulGems")
...
function tes3reference:restockNonfilledSoulGems()
    ...
    for _, soulGem in pairs(soulGems) do
        ...
        maxUnfilledCount = math.clamp(maxUnfilledCount - count, 0,
                                      maxUnfilledCount)
    end
end
...
function tes3reference:removeOldGems()
    ...
    for _, stack in pairs(self.object.inventory) do
        if stack.object.isSoulGem and
            table.find(validFilledSoulGems.soulGems, stack.object.id:lower()) then
            local removedCount = tes3.removeItem({
                reference = self,
                item = stack.object.id,
                count = stack.count
            })
            logger:debug("Removing %s %s (%s)", removedCount, stack.object.id,
                         stack.object.soul)
        end
    end
end
...
local function checkIfRestockFilledSoulGems(merchantRef)
    local timestamp = merchantRef.data.restockSoulGems and
                          merchantRef.data.restockSoulGems.timestamp
    local newTimestamp = tes3.getSimulationTimestamp()
    if not timestamp then -- first time talking to this merchant 
        merchantRef.data.restockSoulGems =
            merchantRef.data.restockSoulGems or {}
        merchantRef.data.restockSoulGems.timestamp = newTimestamp
        merchantRef:stockSoulGems()
        return true
    end
    if newTimestamp - timestamp >=
        tes3.findGMST(tes3.gmst.fBarterGoldResetDelay).value then
        merchantRef.data.restockSoulGems.timestamp = newTimestamp
        merchantRef:stockSoulGems()
        return true
    end
...
local function stockNegativeCountSoulGems(serviceActorRef)
    serviceActorRef:clone()
    logger:debug("Parsing Negative Items for %s", serviceActorRef.object.id)
    for _, stack in pairs(serviceActorRef.object.inventory) do
        if table.find(validFilledSoulGems.soulGems, stack.object.id:lower()) then
            if stack.count < 0 then
                mwscript.removeItem({
                    reference = serviceActorRef,
                    item = stack.object.id,
                    count = -(stack.count)
                })
                tes3.addItem({
...
local function onMenuDialog(e)
    ...
    if serviceActorRef:isSoulGemsMerchants() then
        logger:debug(
            "Talking to %s, who is a restock filled soul gems merchant.",
            serviceActorRef.object.name)
        -- stockNegativeCountSoulGems(serviceActorRef)
        checkIfRestockFilledSoulGems(serviceActorRef)
    end
end
event.register("uiActivated", onMenuDialog,
               {filter = "MenuDialog", priority = -1})
