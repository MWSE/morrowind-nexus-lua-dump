...
function this.onCreate(parent)
	...
	if (tes3.mobilePlayer) then
		descriptionLabel = container:createLabel({ text = "Easy Escort teleportuje do gracza towarzyszy, gdy s¹ wystarczaj¹co daleko lub w innym pomieszczeniu. Korzystaj¹c z poni¿szych informacji, mo¿esz wyœwietliæ bie¿¹c¹ czarn¹ listê, a tak¿e aktualnych NPC, którzy s¹ uwa¿ani za przyjaznych. Aby uniemo¿liwiæ do³¹czenie przez towarzysza do walki, kliknij przycisk czarnej listy. Mo¿esz tak¿e usun¹æ z niej NPC." })
	else
		descriptionLabel = container:createLabel({ text = "Easy Escort teleportuje do gracza towarzyszy, gdy s¹ wystarczaj¹co daleko lub w innym pomieszczeniu. Korzystaj¹c z poni¿szych informacji, mo¿esz wyœwietliæ bie¿¹c¹ czarn¹ listê, a tak¿e aktualnych NPC, którzy s¹ uwa¿ani za przyjaznych. Aby wyœwietliæ i umieœciæ na czarnej liœcie aktualnie przyjaznych NPC, wczytaj zapisan¹ grê." })
	end
	...
	createConfigSliderPackage({
		parent = container,
		label = "Liczba sekund miêdzy teleportacj¹ towarzysza:",
		config = this.config,
		key = "pollRate",
		min = 1,
		max = 60,
	...
	createConfigSliderPackage({
		parent = container,
		label = "Odleg³oœæ przed teleportacj¹ tej samej komórki:",
		config = this.config,
		key = "followDistance",
		min = 100,
		max = 10000,
