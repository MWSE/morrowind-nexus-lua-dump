local cf = mwse.loadConfig("Star Wound Real Lava Damage", {rad = 500, dmg = 10, blocked = {}, npcs = {}, onOff = true})
local myTimer = nil
...
local function npcFireDamage()
    ...
    for light in playerCell:iterateReferences(tes3.objectType.light) do
        ...
        if (light.object.isFire and not light.object.canCarry and not (string.find(light.object.id, "lantern")) and validateObject(light) and not list[light]) then
...
local function registerModConfig()
    local template = mwse.mcm.createTemplate("SW_Real Lava Damage")
    template:saveOnClose("SW_Real Fire Damage", cf)
    template:register()
    local page = template:createSideBarPage({label = "Settings"})
    page.sidebar:createInfo{ text = "Welcome to \"Real Lava Damage\" Configuration Menu. \n \n \n A mod by Spammer, copied by Malavon for lava damage."}
...
local function initialized()
    ...
    print("[Real Fire Damage, by Spammer copied by Malavon for lava damage] 2.0 Initialized!")
