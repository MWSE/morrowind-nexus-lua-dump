	...
	if interior then
		...
		if localMap.visible then
			if switchToLocal then
				return -- local map already visible in doors filled exterior cell, or hostiles present return
			end
		elseif not switchToLocal then
			return -- world map already visible in exterior cell, not a lot of doors, or hostiles present, return
		end
	end
