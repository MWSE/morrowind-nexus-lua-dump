...
local function updateMap()
	...
	if interior then
		if localMap.visible then
			local s = localMap.text
			if s then
				if string.find(string.lower(s), "^tr_") then -- makes TR_Preview interior cell names less ugly
					local lastExteriorCell = tes3.dataHandler.lastExteriorCell
					if lastExteriorCell then
						s = string.format("%s interior", lastExteriorCell.id)
						localMap.text = s
					end
				end
			end
		...
		if localMap.visible then
			...
			if not switchToLocal then -- not a lot of doors, or hostiles present
				if worldMap.visible then
					local s = worldMap.text
					if s then
						if string.find(string.lower(s), "^tr_") then -- makes potential TR_Preview ugly cell names use region name instead
							local lastExteriorCell = tes3.dataHandler.lastExteriorCell
							if lastExteriorCell then
