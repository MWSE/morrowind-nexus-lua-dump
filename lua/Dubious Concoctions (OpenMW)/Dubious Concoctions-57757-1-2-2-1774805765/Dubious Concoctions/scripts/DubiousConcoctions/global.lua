...
local function isGen(itemid)
    local potionRecord = types.Potion.record(itemid)

    if potionRecord.mwscript == 'sd_loot_tracker' then return false end
    if potionRecord.mwscript == 'sd_liquid_tracker' then return false end
    if potionRecord.effects == nil or #potionRecord.effects == 0 then return false end
    if potionRecord.template then return false end
...
local function tradersWelcome(dialog, factionId, hasRank, hasDisp, hasAlchemi, noPoison )
    ...
    if sufix then
        local lastTime = alredySay[sufix]
        local currTime = world.getSimulationTime()

        if lastTime == nil or lastTime + awaitFor < currTime then
            player:sendEvent('dcShowMessage', {message =  msg(sufix) })
        end
        alredySay[sufix] = currTime
    end
end
