...
local function uncheckJail()
    injail = false
end
local function checkJail()
    if tes3.getMobilePlayer() and (tes3.getMobilePlayer().inJail and not injail) then
        injail = true
        timer.start{
            type = timer.real,
            duration = 1,
            callback = uncheckJail
        }
    end
end
...
local function onActivate(e)
	...
	if e.target.mobile and ((e.target.mobile.object.class.id == "Caravaner") or (e.target.mobile.object.class.id == "Shipmaster")) then
		travel = true
	end
end
...
event.register("enterFrame", checkJail)
