...
event.register("magicEffectsResolved", addWeaponDrainEffect)
...
local function onSpellCast(e)
    ...
    local equippedW = tes3.getEquippedItem({
        actor = tes3.player,
        enchanted = true,
        objectType = tes3.objectType.weapon,
        type = tes3.weaponType.bluntTwoWide
    })
    local equippedR = {}
    for _, stack in pairs(tes3.player.object.equipment) do
        if stack.object.objectType == tes3.objectType.clothing and stack.object.slot ==
            tes3.clothingSlot.ring then table.insert(equippedR, stack) end
    end
    local equippedA = tes3.getEquippedItem({
        actor = tes3.player,
        enchanted = true,
        objectType = tes3.objectType.clothing,
        slot = 9
    })
    if equippedW then chargeW = equippedW.itemData.charge end
    if equippedR[1] and equippedR[1].itemData and equippedR[1].object.enchantment and
        equippedR[1].object.enchantment.castType == 2 then
        chargeR1 = equippedR[1].itemData.charge
    end
    if equippedR[2] and equippedR[2].itemData and equippedR[2].object.enchantment and
        equippedR[2].object.enchantment.castType == 2 then
        chargeR2 = equippedR[2].itemData.charge
    end

    if equippedR[3] and equippedR[3].itemData and equippedR[3].object.enchantment and
        equippedR[3].object.enchantment.castType == 2 then
        chargeR3 = equippedR[3].itemData.charge
    end

    if equippedA then chargeA = equippedA.itemData.charge end
    ...
    elseif chargeA and chargeA >= e.cost then
        ...
        e.cost = 0
    elseif chargeR1 and chargeR1 >= e.cost then
        equippedR[1].itemData.charge = ((chargeR1) - (e.cost))
        e.cost = 0
    elseif chargeR2 and chargeR2 >= e.cost then
        equippedR[2].itemData.charge = ((chargeR2) - (e.cost))
        e.cost = 0
    elseif chargeR3 and chargeR3 >= e.cost then
        equippedR[3].itemData.charge = ((chargeR3) - (e.cost))
        e.cost = 0
    end
end
event.register("spellMagickaUse", onSpellCast)
...
local function onMobileActivated(e)
    ...
    if not (e.mobile.object:offersService(tes3.merchantService.spells)) then return end
    if e.reference.data.spammer_eddoonce then return end
...
event.register("loaded", onLoad, {priority = -500})
...
local function registerModConfig()
    ...
    category:createOnOffButton{
        label = "On/Off",
        description = "If turned on, will automatically give you the spell on your next loading of a save.",
