...
local config = mwse.loadConfig("Loanable funds")
config = config or {}
config.interestRate = config.interestRate or 1
config.maxLoanPeriod = config.maxLoanPeriod or 14
...
local function calcLoanSum(loanValue, loanTime)
	...
	return math.round(loanValue + (config.interestRate * 0.01 * loanValue * loanTime), 0)
end
...
local function createLoanMenu(e)
	...
	slider = block:createSlider({id = GUI_ID_LoanTimeSlider, current = 0, max = config.maxLoanPeriod, jump = 1, step = 1})
	slider.width = 400
...
local modConfig = require("KetsLoan.mcm")
modConfig.config = config
local function registerModConfig()
	mwse.registerModConfig("Loanable Funds", modConfig)
end
