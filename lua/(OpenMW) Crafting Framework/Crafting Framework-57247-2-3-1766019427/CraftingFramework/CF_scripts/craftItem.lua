			...
			if targetRecord then
				...
				for _, item in pairs(allItems) do
					...
					if itemRecord and
					   ...
					   (itemRecord.id:sub(1,10) == "Generated:" or itemRecord.enchant == nil) and
...
local function checkIngredients(recipe, artisansTouch)
    ...
    for _, ingredient in ipairs(getIngredients(recipe, artisansTouch)) do
        ...
        if foundItem then
            local key = getItemKey(foundItem)
            virtuallyConsumed[key] = (virtuallyConsumed[key] or 0) + ingredient.count
            
            if not itemTypes[key] or itemTypes[key].type == "wildcard" then
    ...
    for key, totalNeeded in pairs(virtuallyConsumed) do
        local ingr = itemTypes[key]
        local foundItem, inventoryCount
        if ingr.type == "wildcard" then
            foundItem, inventoryCount = checkIngredient(ingr)
        else
            foundItem, inventoryCount = checkIngredient({id = ingr.id, type = ingr.type, count = totalNeeded})
        end
...
local function consumeIngredients(recipe, artisansTouch)
	...
	if cheatMode then return 10 end
...
local function updateCraftingProgress(dt)
	...
	if craftingState.elapsedTime >= craftingState.duration then
		...
		if consumeIngredients(craftingState.recipe, craftingState.artisansTouch) then
			...
			local diffMod = math.max(0.4, math.min(1,(craftingState.recipe.level or 1)/base))
			if craftingState.artisansTouch then
				diffMod = math.min(1,(craftingState.recipe.level or 1)/base)^0.5
			end
			...
			if craftingState.shiftPressed and getEquipmentSlot({type = craftingState.recipe.type, recordId = craftingState.recipe.id}) == types.Actor.EQUIPMENT_SLOT.Ammunition then
				local eq = types.Actor.getEquipment(self)
				eq[types.Actor.EQUIPMENT_SLOT.Ammunition] = nil
				types.Actor.setEquipment(self, eq)
			end
			core.sendGlobalEvent('CraftingFramework_getItem', {self, craftingState.recipe.type, craftingState.recipe.id, craftingState.recipe.name, craftingState.recipe.count or 1, calculateResultValue(craftingState.recipe, craftingState.artisansTouch), calculateQuality(craftingState.recipe, craftingState.artisansTouch), craftingState.recipe.preserveRecordId, craftingState.shiftPressed })
			
			if craftingState.recipe.secondSkill then
				skill = getSkill(craftingState.recipe.secondSkill)
				base = getBaseSkill(craftingState.recipe.secondSkill)
				diffMod = math.max(0.4, math.min(1,(craftingState.recipe.secondLevel or 1)/base))
				if craftingState.artisansTouch then
					diffMod = math.min(1,(craftingState.recipe.secondLevel or 1)/base)^0.5
				end
				diffMod = diffMod / 2
