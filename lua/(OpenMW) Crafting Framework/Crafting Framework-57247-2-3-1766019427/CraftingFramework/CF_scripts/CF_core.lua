...
-- Generate unique key for inventory tracking
-- Filled soul gems get "recordId:soul:creatureId", everything else just recordId
...
function getRecipeColor(recipe)
	local diffMod = math.max(0, math.min(1,((recipe.level or 1)/getBaseSkill(recipe.firstSkill or "armorer")-0.4) / (1 - 0.4))) -- min exp: 40%
	
	if recipe.secondSkill then
		local secondDiffMod = math.max(0,math.min(1,((recipe.secondLevel or 1)/getBaseSkill(recipe.secondSkill)-0.4) / (1 - 0.4)))
		diffMod = (diffMod + secondDiffMod)/2
	end
	return mixColors(morrowindGold, textColor, diffMod)
end
			...
			if ingredient.type ~= "wildcard" then
				...
				while remainingNeeded > 0 do
					...
					for _, item in ipairs(qualifyingItems) do
						...
						local available = tempInventory[itemKey] and tempInventory[itemKey].count or 0
						if available > 0 then
					...
					if tempInventory[bestItem.itemKey] then
						tempInventory[bestItem.itemKey].count = tempInventory[bestItem.itemKey].count - toRemove
						-- Behalte auch negative Mengen für korrekte Queue-Berechnung
					else
						tempInventory[bestItem.itemKey] = {
							count = -toRemove,
							type = tostring(bestItem.originalItem.type),
...
function findWildcardItems(tempInventory, wildcardFunc)
    ...
    local qualifyingKeys = {}
    for _, item in pairs(qualifyingItems) do
        qualifyingKeys[getItemKey(item)] = true
    end
    
	-- Durchsuche temp Inventar
    for itemKey, data in pairs(tempInventory) do
        if qualifyingKeys[itemKey] and data.count > 0 then
            totalCount = totalCount + data.count
...
function calculateItemExp(recipe, artisansTouch)
	...
	for _, ingredient in pairs(getIngredients(recipe, artisansTouch)) do
		if ore_difficulties[ingredient.id] then
			--itemValue = itemValue + ingredient.count *  (ore_difficulties[ingredient.id]^0.18 - 0.1) * (ingredientExpMultipliers[ingredient.id] or 1) -- 2.1
			itemValue = itemValue + ingredient.count *  (ore_difficulties[ingredient.id]^0.18 - 0.1) * (ingredientExpMultipliers[ingredient.id] or 1) -- new
