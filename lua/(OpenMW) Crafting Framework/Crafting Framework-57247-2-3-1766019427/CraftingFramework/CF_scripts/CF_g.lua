...
local function getItem(data)
	...
	local preserveRecordId = data[8]
	local shiftPressed = data[9]
	...
	local tempItem
	if preserveRecordId or protectedRecordIds[recordId] or recordType ~= "Armor" and recordType ~= "Weapon" and recordType ~= "Clothing" then
	...
	player:sendEvent("CraftingFramework_notifyItem", {tempItem, math.floor(count),recordId, shiftPressed})
end
