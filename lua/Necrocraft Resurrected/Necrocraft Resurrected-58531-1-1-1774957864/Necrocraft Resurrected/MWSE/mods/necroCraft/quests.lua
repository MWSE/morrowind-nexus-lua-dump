...
quests.journal = function(e)
	...
	if e.topic.id == "NC_HelpAdram" then
		if e.index == 100 then
			local skeletonCount = 0
			local bonewalkerCount = 0
			for ref in tes3.getPlayerCell():iterateReferences(tes3.objectType.creature) do
				if undead.isRaisedByPlayer(ref) then
					local undeadType = undead.getType(ref.object)
					if undeadType == "skeletonWarrior" then
...
local function hasMinions()
	...
	return skeletonCount > 1 and bonewalkerCount > 1
end

local function onInfoFilter(e)
	if not e.passes then return end
	local actorId = e.actor.baseObject.id
	local gem = tes3.findGlobal("NC_HasSoulgem")
	local minions = tes3.findGlobal("NC_HasMinions")
	...
	elseif actorId == "dratha" then
		...
		gem.value = 0
		if actorId == "daris adram" then
			if hasMinions() then
				-- tes3.messageBox("Minions detected")
				minions.value = 1
			else
				-- tes3.messageBox("Minions not found")
				minions.value = 0
			end
		end
	end
