...
undead.getType = function(object)
	...
	local key = meshToType[object.mesh]
	if key == nil then return false end
	if key == "skeleton" then
		return object.level >= 10 and "skeletonChampion" or "skeletonWarrior"
	end
	return key
end
...
undead.isRaisedByPlayer = function(reference)
	...
	if utype and string.startswith(reference.id, "NC_") then
...
undead.init = function()
	...
	meshToType[tes3.getObject("skeleton").mesh]          = "skeleton"
	meshToType[tes3.getObject("NC_skeleton_weak").mesh]  = "skeletonCripple"
	meshToType[tes3.getObject("NC_bonespider").mesh]     = "bonespider"
	meshToType[tes3.getObject("bonelord").mesh]          = "bonelord"
	meshToType[tes3.getObject("NC_boneoverlord").mesh]   = "boneoverlord"
	meshToType[tes3.getObject("lich").mesh]              = "lich"
	meshToType[tes3.getObject("lich_barilzar").mesh]     = "lichKing"
	meshToType[tes3.getObject("AB_Und_Zombie").mesh]     = "zombie"
	meshToType[tes3.getObject("bonewalker").mesh]        = "bonewalker"
	meshToType[tes3.getObject("Bonewalker_Greater").mesh] = "greaterBonewalker"
	meshToType[tes3.getObject("BM_wolf_skeleton").mesh]  = "bonewolf"
end
