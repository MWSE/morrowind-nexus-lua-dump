...
local function createPlayerCorpse()
	...
	for _, stack in pairs(tes3.player.object.inventory) do
		local equipped = (tes3.player.object --[[@as tes3npc]]):hasItemEquipped(stack.object --[[@as tes3item]])
...
local function updateContainer(e)
	...
	elseif tes3.player.data.necroCraft.phylactery and e.reference.id == tes3.player.data.necroCraft.phylactery.container then
...
lichdom.playerResurrection = function()
	...
	if container.object.inventory:contains("NC_skeleton_champ_misc") and soulGemLib.releaseSoul{reference = container, gem = "AB_Misc_SoulGemBlack"} then
				...
				timer.start{
					...
					callback = function() 
						...
						if tes3.player.baseObject.race.id ~= "skeletonrace" then
							...
							--[[tes3.addSpell{reference=tes3.player, spell="NC_LichdomAbility"}
							tes3.addSpell{reference=tes3.player, spell="NC_UndeadAbility"}]]
							tes3.addSpell{reference=tes3.player, spell="immune to frost"}
							tes3.addSpell{reference=tes3.player, spell="immune to poison"}
							tes3.addSpell{reference=tes3.player, spell="immune to disease"}
							tes3.addSpell{reference=tes3.player, spell="resist magicka_50"}
							tes3.addSpell{reference=tes3.player, spell="resist shock_50"}
							tes3.addSpell{reference=tes3.player, spell="resist normal weapons_50"}
							tes3.addSpell{reference=tes3.player, spell="NC_Lich_fmm"}
							tes3.addSpell{reference=tes3.player, spell="NC_Lich_rm"}
							tes3.addSpell{reference=tes3.player, spell="NC_Lich_wb"}
						end
					end
				}
			end
