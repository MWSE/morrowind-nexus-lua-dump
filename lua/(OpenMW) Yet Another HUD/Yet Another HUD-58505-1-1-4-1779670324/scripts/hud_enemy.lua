...
local showEnemyBar         = getToggle("SHOW_ENEMY_BAR")
...
-- mutable state. Reset whenever widgets are destroyed
local enemies      = {}
local enemyCount   = 0
local activeId     = nil
local holdTimer    = 0
...
-- widget references. Nil when SHOW_ENEMY_BAR is off
local root, container
local bgBar, fillL, fillR, frameBar
local fatBg, fatFillL, fatFillR
...
local function destroyWidgets()
    if not root then return end
    if fatBg    then fatBg:destroy();    fatBg    = nil end
    if fatFillL then fatFillL:destroy(); fatFillL = nil end
    if fatFillR then fatFillR:destroy(); fatFillR = nil end
    if bgBar    then bgBar:destroy();    bgBar    = nil end
    if fillL    then fillL:destroy();    fillL    = nil end
    if fillR    then fillR:destroy();    fillR    = nil end
    if frameBar then frameBar:destroy(); frameBar = nil end
...
local function muteKnownEnemies()
    for _, e in pairs(enemies) do
        if e.object and e.object:isValid() then
            e.object:sendEvent("HudShutdown")
        end
    end
end

if showEnemyBar then
    buildWidgets()
end
...
togglesSection:subscribe(async:callback(function(_, key)
    if key == nil or key == "SHOW_ENEMY_BAR" then
        local newVal = getToggle("SHOW_ENEMY_BAR")
        if newVal ~= showEnemyBar then
            local wasOn = showEnemyBar
            showEnemyBar = newVal
            if wasOn and not newVal then
                -- tell every known NPC to stop reporting BEFORE we clear state in destroyWidgets()
                muteKnownEnemies()
                destroyWidgets()
            elseif newVal and not wasOn then
                buildWidgets()
                applyColors()
                applyFatigueVisibility()
                -- force an immediate hostile rescan so nearby NPCs wake up
                hostileScanTimer = HOSTILE_SCAN_INTERVAL
            end
        end
    end
...
onEnemyUpdate = function(data)
    if not showEnemyBar then
        if data and data.object and data.object:isValid() then
            data.object:sendEvent("HudShutdown")
        end
        return
    end
    if not root then return end
...
onFrame = function(dt)
    if not showEnemyBar then return end
    if not root then return end
