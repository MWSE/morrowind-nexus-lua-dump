...
local daedricQuestsLimited = Helpers.readSetting(globalSettings,constants.enableDaedricLimitersKey,constants.enableDaedricLimitersDefault)
...
local function readGlobalSettings()
    daedricQuestsLimited = Helpers.readSetting(globalSettings,constants.enableDaedricLimitersKey,constants.enableDaedricLimitersDefault)
end

local function forceDateSync()
    updateTimeOffsets()
end

I.Activation.addHandlerForType(types.Activator, activationPreventHandler)

storage.globalSection(constants.globalSettingsStorageKey):subscribe(async:callback(readGlobalSettings))

return {
    eventHandlers = {
        holidays_internal_forceDateSync = forceDateSync,
    },
}
