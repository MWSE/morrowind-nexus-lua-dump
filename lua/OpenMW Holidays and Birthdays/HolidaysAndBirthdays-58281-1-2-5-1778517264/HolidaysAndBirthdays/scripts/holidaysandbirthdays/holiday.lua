...
local statsWindowSettings = {}

if I.StatsWindow then
    statsWindowSettings = storage.playerSection(constants.statsWindowIntegrationStorageKey)
end
...
local function readGeneralSettings()
    locals.displayMessageStart = Helpers.readSetting(generalSettings, constants.displayMessageStartKey,
        constants.defaultDisplayMessageStart)
    locals.displayMessageUntil = Helpers.readSetting(generalSettings, constants.displayMessageUntilKey,
        constants.defaultDisplayMessageUntil)
    locals.getBirthDayGifts = Helpers.readSetting(generalSettings, constants.getBirthDayGiftsKey,
        constants.getGiftsDefaultSetting)
    locals.combineHolidayMessages = Helpers.readSetting(generalSettings, constants.combineHolidayMessagesKey,
        constants.combineHolidayMessagesDefault)
end

local function readSWESettings()
    if statsWindowSettings then
        locals.statsWindowPane = Helpers.readSetting(statsWindowSettings, constants.statsWindowPaneKey,
            constants.statsWindowPaneDefault)
        locals.statsWindowPlacement = Helpers.readSetting(statsWindowSettings, constants.statsWindowPlacementSettingKey,
            constants.statsWindowPlacementSettingDefault)
        locals.statsWindowIndent = Helpers.readSetting(statsWindowSettings, constants.indentValuesKey,
            constants.indentValuesDefault)
    end
end

local function initReadSettings()
    readGeneralSettings()
end
...
local function getDisplayMonthName(monthIndex)
    ...
    -- end
    local ret = calendar.monthName(monthIndex)
    return ret
end
...
local function showHolidayMessages()
    ...
    if msgs ~= nil then
        if #msgs > 1 then
            if locals.combineHolidayMessages then
                local combinedMessages = table.concat(msgs, "\n And ")
...
local function holidayMessageHandler(dt) -- OnUpdate
    ...
    if not allowedUpdates then return end
    if not chargenFinished() then return end
    ...
    if not ageStatsDefaults.chargenAgeStatsSet and not ageUIActive and locals.inExteriorCell then
        async:newUnsavableSimulationTimer(0.5, function()
            if I.UI.getMode() == nil and not ageStatsDefaults.chargenAgeStatsSet and not ageUIActive and locals.inExteriorCell then
                ageUIActive = true
...
local function isExterior(cell)
    if cell then
        return (cell:hasTag("QuasiExterior") or cell.isExterior)
    else
        return false
    end
end
...
local function onTeleported() -- On Teleported
    ...
    async:newUnsavableSimulationTimer(0.1,
        function() -- Waiting for simulation to start ticking again so we know we are in the new cell
            locals.inExteriorCell = isExterior(self.cell)
            -- newCell = self.cell
...
local function isWaitRestMode(mode)
    return I.UI and I.UI.MODE and ((I.UI.MODE.Rest and mode == I.UI.MODE.Rest) or
        (I.UI.MODE.Travel and mode == I.UI.MODE.Travel))
end

local function onUiModeChanged(data)
    if isWaitRestMode(data.oldMode) and not isWaitRestMode(data.newMode) then
        async:newUnsavableSimulationTimer(0.1, function()
            core.sendGlobalEvent("holidays_internal_forceDateSync")
        end)
    end
end
...
input.registerTriggerHandler(constants.showMessageTriggerKey, async:callback(function()
    core.sendGlobalEvent("holidays_internal_forceDateSync")
    showHolidayMessages()
end))

storage.playerSection(constants.generalSettingsStorageKey):subscribe(
    async:callback(readGeneralSettings))

storage.playerSection(constants.statsWindowIntegrationStorageKey):subscribe(
    async:callback(readSWESettings))

-- startUpdating()

local function onConsoleCommand(mode, command)
    local cmd = command:lower():gsub("lua ", "")
    if cmd == "hb_resetagedata" then
        ui.showMessage("Age data reset. Unpause the game for age UI to appear.")
        ageStatsDefaults.chargenAgeStatsSet = false
        locals.inExteriorCell = true
    end
end
