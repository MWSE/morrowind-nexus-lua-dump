...
return {
    engineHandlers = {
        ...
        onLoad =
            ...
            updateInterventionGlobals()
            print("[Greater Intervention]: Script loaded.")
        end
    },
    eventHandlers = {
        ...
        -- Same as above, but for Console list printing
        requestMarkerDataConsole = function(data)
            local requestedMarkers = markers[data.type] or {}
            world.players[1]:sendEvent('receiveMarkerDataConsole', requestedMarkers)
        end,
        ...
        -- This function adds a boolean to a specific entry in a list
        -- indicating whether to hide that entry during menu building
        hideMarker = function(data)
            if markers[data.type][data.index] ~= nil then
                markers[data.type][data.index].hidden = data.hide
                print("[Greater Intervention]: Hidden index " .. data.index .. " (" .. markers[data.type][data.index].label .. ") from list " .. data.type)
            end
        end,
