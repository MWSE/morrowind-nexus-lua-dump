...
function currencyExchangeMenu.confirmCurrencyExchangeChange()
    ...
    for _, bill in pairs(common.currency) do
        ...
        if (text < 0) then
            ...
            if (diff < 0) then
                ...
                tes3.messageBox({message = "You exchanged " .. math.abs(text) .. " " .. name .. " for " .. math.abs(price * text) .. " copper septims.", showInDialog = false})
        ...
        elseif (text ~= 0) then
            if (actualCopper >=  text * price) then
                common.counts[id].count = common.counts[id].count + math.abs(text)
                tes3.messageBox({message = "You exchanged " .. math.abs(price * text) .. " copper septims for " .. text .. " " .. name .. ".", showInDialog = false})
                actualCopper = actualCopper - math.abs(price * text)
                common.fixCounts()
            elseif (text * price > actualCopper and common.GPBankData.accountBalance < text * price) then
                tes3.messageBox({message = "You don't have " .. text * price .. " copper.", showInDialog = false})
            end
        end
        local menu = tes3ui.findMenu(common.GUI_ID_CurrencyExchangeMenu):findChild("GPExchangeInput" .. id)
        menu.text = 0
    ...
    menu.text = ("Here you can exchange your copper septims for paper bills and higher denomination coins or vice versa. \n \n Copper Septims: " .. actualCopper)
end
...
function currencyExchangeMenu.createcurrencyExchangeMenu(e)
    ...
    local description = menu:createLabel({id = common.GUI_ID_CurrencyExchangeMenuDescription, text = "Here you can exchange your copper septims for paper bills and higher denomination coins or vice versa. \n \n Copper Septims: " .. actualCopper})
    ...
    currenciesBlock.widthProportional = 1
