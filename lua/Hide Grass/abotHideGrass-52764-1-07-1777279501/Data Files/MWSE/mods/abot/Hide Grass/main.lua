...
local function initGrassDict()
	...
	for line in lines do
		local lcLine = doubleSlash( line:trim():lower() )
		if not lcLine:find('^[:;]') then
			local path = lcLine:match('(.+%.nif)%s*=%s*grass')
			if path
...
to keep this setting disabled.]],
	...
	if table.size(grassDict) > 0 then
		...
		mwse.log('%s modConfigReady(): WARNING: table.size(grassDict) <= 0',
			modPrefix)
	end
end
