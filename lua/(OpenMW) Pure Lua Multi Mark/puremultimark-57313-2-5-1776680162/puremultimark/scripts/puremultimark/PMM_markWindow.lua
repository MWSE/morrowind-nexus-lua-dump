...
-- daedric theme swaps all button icons
local ICON_DIR = RENAME_ICON == "Daedric" and "textures/puremultimark/daedric/" or "textures/puremultimark/"
local RENAME_ICON_PATH = RENAME_ICON == "Daedric" and (ICON_DIR .. "rename.png") or (ICON_DIR .. RENAME_ICON .. ".png")
...
topBar.events = {
	...
	mouseMove = async:callback(function(data, elem)
		if elem.userData and elem.userData.isDragging then
			...
			elem.userData.lastMousePos = data.position
			local newPosition = (teleportWindow.layout.props.position or v2(0, 0)) + delta
...
-- Close Button
xButton = makeIconButton(ICON_DIR.."x.png", v2(textSize*1.2, textSize*1.2), function()
	destroyTeleportWindow()
	cancelCasting()
...
function makeTeleportButton(location, index, showEntryAbove)   
	...
	-- Rename Button
	local renameButton = makeIconButton2(RENAME_ICON_PATH, v2(textSize * lineHeightMultiplier * lineHeight2, textSize * lineHeightMultiplier * lineHeight2), function()
		renameLocation(index)
	end,nil,listBox)
	contentFlex.content:add(renameButton)
	...
	if (SORT_DIRECTION == "Down" or index == 1) and index < #teleportLocations then
		sortButton = makeIconButton(ICON_DIR.."down.png", v2(textSize * lineHeightMultiplier * lineHeight2, textSize * lineHeightMultiplier * lineHeight2), function()
		...
		sortButton = makeIconButton(ICON_DIR.."up.png", v2(textSize * lineHeightMultiplier * lineHeight2, textSize * lineHeightMultiplier * lineHeight2), function()
			if input.isShiftPressed() then
	...
	-- Delete Button
	local deleteButton = makeIconButton(ICON_DIR.."x.png", v2(textSize * lineHeightMultiplier * lineHeight2, textSize * lineHeightMultiplier * lineHeight2), function()
