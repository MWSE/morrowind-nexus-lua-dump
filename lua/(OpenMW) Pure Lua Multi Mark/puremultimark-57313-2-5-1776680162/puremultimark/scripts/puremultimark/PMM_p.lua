...
if not markSound or markSound == "" then
	markSound = "mysticism hit"
end
if not recallSound or recallSound == "" then --vanilla = nil in 0.51+, "" pre-0.51
				...
				if b.id == "recall" then
					...
					if #saveData.locations > 1 then
						...
						if math.random() < castChance then
							...
							I.UI.setMode("Interface",{windows = not playerSection:get("CONTROLLER_MODE") and windows or {}})
...
function cancelCasting()
	local castedSpell = Player.getSelectedSpell(self)
	if castedSpell then
		local refund = checkSpell(castedSpell).cost
		local mbspRefund = 0
		if I.MBSP then
			local mbspData = I.MBSP.getNextRefund()
			if mbspData.mode == "EXPERIMENTAL" then
				mbspRefund = mbspData.refund
			end
		end
		refund = refund - mbspRefund
		Player.stats.dynamic.magicka(self).current = Player.stats.dynamic.magicka(self).current + refund
	end
