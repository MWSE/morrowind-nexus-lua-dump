...
local L = {
	...
	WT = {
		...
		[types.Weapon.TYPE.MarksmanCrossbow] = 1,
		[types.Weapon.TYPE.MarksmanThrown] = 1,
		[types.Weapon.TYPE.Arrow] = 1,
		[types.Weapon.TYPE.Bolt] = 1,
	}
}
...
local function playVfx(vfx, hit)
	...
	if vfx.tex then
		opt.particleTextureOverride=vfx.tex
		opt.mwMagicVfx=false
	end
...
local function playSfx(m)
	assert(m.soundTarget, "playSoundEffect: soundTarget must be valid object")
	local dir = matSound[m.material or "Dirt"]
	local dmg = m.weaponType and L.WT[m.weaponType] or 1
	if not noIso[dir] then dir = dir .. dmg		end
	local vol = ( volAdjust[dir] or 1 ) * (m.volume or 0.3)
	if vol > 0 then
		core.sound.playSoundFile3d(pickSound(dir), m.soundTarget,
			{ volume = vol, pitch = math.random(90,110) / 100 })
	end
end

local function spawnEffect(m)
	local mat = m.material
	assert(type(mat) == "string", "spawnEffect: material must be string")
	assert(m.hitPos, "spawnEffect: hitPos must be valid vector")

	if m.soundTarget and not m.noSound then
		playSfx(m)
	end
