...
local matVfx = {
--	Stone = { mesh="meshes/e/impact/stoneSpark.nif" },
...
local function playVfx(vfx, hit)
	local param = { model=vfx.mesh, position=hit }		local opt = {}
	if vfx.tex then opt.particleTextureOverride=vfx.tex	opt.mwMagicVfx=false	end
	if vfx.scale then opt.scale = vfx.scale		end
	if next(opt) then param.options = opt		end
--	print(vfx, vfx.mesh)
	core.sendGlobalEvent("SpawnVfx", param)
end
...
local function getArmorMat(id)
	...
	if not rec then
		mat = { hit="Unknown" }
		matArmor[id] = mat
--		print("notARMOR "..id.." "..mat.hit)
		return mat
	end
	local weight = rec.weight	local type = armorClass[rec.type]
	if id:find("bone") then mat = { hit="ParryArmorBone" }
	elseif weight > type.medium then mat = { hit="ParryArmorHeavy" }
	elseif id:find("steel") or id:find("silver") then mat = { hit="ParryArmorHeavy" }
	elseif id:find("orcish") or id:find("helsethguard") then mat = { hit="ParryArmorHeavy" }
	elseif id:find("glass") then mat = { hit="Glass" }
	elseif id:find("ice_") then mat = { hit="ParryArmorIce" }
	elseif id:find("bear") then mat = { hit="Carpet" }
	elseif id:find("indoril") or id:find("chitin") then mat = { hit="ParryArmorBone" }
	elseif weight > type.light then mat = { hit="ParryArmorMedium" }
	else mat = { hit="Unknown" }
	end
	matArmor[id] = mat
...
--	if hitWater then var.hitWater = hitPos		end
	...
	local vol = ( mat == water and 3.3 or 1 ) * volume	if vol > 1 then vol = 1 end
...
I.AnimationController.addTextKeyHandler("", function(g, key)
	...
	target = pos + camera.viewportToWorldVector(util.vector2(0.5,0.5))
		* (w.dist + camera.getThirdPersonDistance())
...
--			ui.showMessage(o.recordId.." Attacks!")
	...
	interface = {
		version = 0,
		registerHitActorHandler = function(m)
			actorHandlers[#actorHandlers + 1] = m
		end,
		registerHitObjectHandler = function(m)
