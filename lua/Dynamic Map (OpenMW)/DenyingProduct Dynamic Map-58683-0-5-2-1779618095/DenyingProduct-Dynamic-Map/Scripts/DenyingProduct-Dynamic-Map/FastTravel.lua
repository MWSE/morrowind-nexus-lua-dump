...
local function buildCellLookup(exteriorCells)

    local cellLookup = {}

    for _, cell in ipairs(exteriorCells or {}) do
        ...
        c.sumX = c.sumX + cell.mapX
        c.sumY = c.sumY + cell.mapY
        c.count = c.count + 1
    end

    for _, c in pairs(cellLookup) do
        c.mapX = c.sumX / c.count
        c.mapY = c.sumY / c.count
    end
...
local function buildMapNodes(nodes, cellLookup)
...
function FastTravel.processNodes(exteriorCells, nodes)

    nodes = FastTravel.applyWheelAndSpoke(nodes)

    local cellLookup = buildCellLookup(exteriorCells)
    local mapNodes = buildMapNodes(nodes, cellLookup)
...
function FastTravel.buildFastTravel(exteriorCells,TAMRIEL_REBUILD_ENABLED,CYRODIIL_ENABLED,SKYRIM_ENABLED)
    ...
    return FastTravel.processNodes(exteriorCells, siltNodes),
    FastTravel.processNodes(exteriorCells, boatNodes),
    FastTravel.processNodes(exteriorCells, guideNodes)
end
