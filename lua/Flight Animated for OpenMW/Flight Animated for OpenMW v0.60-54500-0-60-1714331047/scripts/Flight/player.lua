...
local flight, moved = false, false
local savedOptions = {}
...
local switch2 = {
	...
	["runback1h"] = "swimrunback",
	["runback2c"] = "swimrunback",
	["runback2w"] = "swimrunback",
	["runbackhh"] = "swimrunback",
...
local flyCancel = { "swimturnleft", "swimturnright", "idleswim", "swimwalkforward", "swimwalkback",
	"swimwalkleft", "swimwalkright", "flyrunforward", "flyrunforwardup", "flyrunforwarddown",
	"swimrunback", "swimrunleft", "swimrunright" }
...
local function switchAnimation(g, o)
	if Actor.activeEffects(self):getEffect("levitate").magnitude == 0 or camera.getMode() == camera.MODE.FirstPerson
		then return g end
	flight = true
...
local function getIdleStance()
	if Actor.getStance(self) == ST.Nothing then return "idleswim" end
	...
	if item.type ~= types.Weapon then return "idleswim" end
	local idle = wpnTypes[types.Weapon.record(item).type]
	if not idle then idle = "idleswim" end
	return idle
end
...
local function onUpdate(dt)
	...
	if isPlay == nil then
		if moved then
			animation.cancel(self, "idleswim")
			moved = false
			local idleStance = getIdleStance()
			if idleStance == "idleswim" then
	animation.playBlended(self, "idleswim", {loops=50, priority=5, speed=0.2})
			else
	animation.playBlended(self, idleStance, {loops=50, blendmask=14, priority=5})
	animation.playBlended(self, "idleswim", {loops=50, blendmask=1, priority=5, speed=0.2})
			end
		end
		return
	end
