...
if (mwse.buildDate == nil) or (mwse.buildDate < 20200521) then
    event.register("initialized", function()
        tes3.messageBox("[Dwemer Teleportation Box] Your MWSE is out of date!" ..
...
local function teleportTick(e)
    ...
    if canTeleport then
        local reference = tes3.findClosestExteriorReferenceOfObject({object = "fargoth"})
    ...
    if (tes3.isModActive("Fargoth Intervention.esp") == true or
        ...
        event.register("journal", function(e)
            if (e.topic.id == "G93_FargothInter" and e.index == 110) then
                mwscript.addSpell({reference = tes3.player, spell = "OJGA_FargothIntervention"})
            end
        end)
    end
end
