...
local lastEquippedHelmetIndex = -1
...
local function getHelmetsInInventory()
    local helmetsList = {}
    print("DEBUG: Scanning inventory for helmets...")
    for _, item in ipairs(types.Actor.inventory(self):getAll()) do
        local isHelmet = false
        if item.type == types.Armor then
            local armorType = types.Armor.record(item).type
            if armorType == 0 then -- Helmets
                isHelmet = true
                print("Detected as helmet: " .. item.recordId)
            end
        end
        if isHelmet then
            table.insert(helmetsList, item)
            print("Found equippable helmet: " .. item.recordId .. " (Armor (Helmet))")
        end
    end
    return helmetsList
end
...
local function onKeyPress(key)
    ...
    elseif key.code == input.KEY.N then
        itemList = getHelmetsInInventory()
        index = lastEquippedHelmetIndex
        slot = types.Actor.EQUIPMENT_SLOT.Helmet
        itemType = "helmets"
        indexVar = "lastEquippedHelmetIndex"
    else
        return
    end
