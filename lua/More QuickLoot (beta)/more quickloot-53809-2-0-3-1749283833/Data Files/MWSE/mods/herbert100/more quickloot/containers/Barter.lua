...
function Barter.new(ref)
	...
	local obj = setmetatable({
		...
		merchant_isnt_smuggler = common.bg_cfg and not common.bg_cfg.smuggler[ref.baseObject.id:lower()] and ref.mobile.alarm ~= 0
	}, meta)
...
function Barter:get_subtitles()
	local subtitles = self.is_buying and {"You are Buying"} or {"You are Selling"}
	local ref = self.handle:getObject()

	subtitles[2] = "Disposition: " .. ref.object.disposition
	return subtitles
...
local function add_npc_items(self)
    ...
    if is_selling then
        ...
        allowed_types[tes3.objectType.ingredient] = allowed_types[tes3.objectType.ingredient] and config.barter.selling.allow_ingredients
    end
    ...
    for _, stack in ipairs(seller_ref.object.inventory.items) do
        ...
        for _, data in ipairs(stack.variables or {}) do
            ...
            if not equipped_index then
                tbl_insert(items, {object=obj, box_handle=handle, data=data,  count=1, 
				...
				elseif show_unavailable then
					...
					tbl_insert(items, { object=obj, box_handle=handle, data=data, equipped=true, count=1,
						-- only compute the value if we are selling
						value= is_selling and tes3.calculatePrice{object=obj, itemData=data, bartering=true, merchant=merchant, selling=is_selling} 
									       or 0
					})
				end
			end
        ...
        if count > 0 then
            tbl_insert(items, {object=obj, box_handle=handle, count=count, 
		...
		for c_ref in tes3.player.cell:iterateReferences(tes3.objectType.container, false) do
			...
			for _, stack in ipairs(c_ref.object.inventory.items) do
				...
				for _, data in ipairs(stack.variables or {}) do
					count = count - 1
					tbl_insert(items, {
						object=obj, box_handle=handle, count=1, data=data, 
				...
				if count > 0 then
					tbl_insert(items, {
						object=obj, box_handle=handle, count=count,
						value=tes3.calculatePrice{
							merchant = merchant_mob,
							bartering = true,
							buying = true,
...
---@param modifier_pressed boolean Is the modifier key pressed?
---@return string[]
function Barter:get_action_names(item, modifier_pressed)
