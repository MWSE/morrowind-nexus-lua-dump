...
local Pickpocket = Class.new{name="Pickpocket Manager", parents={Chance, Living}, 
    ...
    init = function(self, ref)
        self.ref = ref
        ref:clone()
        local npc_mobile = self.ref.mobile ---@type tes3mobileNPC
        self.is_detected = npc_mobile.isPlayerDetected
...
function Pickpocket:_get_iter_params() 
    return { ref=self.ref, equipped_filter = self:make_equipped_filter(cfg.equipped) }
end
...
function Pickpocket:calc_item_chance(item)
    ...
    local chance = cfg.chance_mult * (self.base_chance - penalty)
