...
local Organic = Class.new{name="Organic Manager", parents={Chance},
    ...
    init = function(self, ref)
        ...
        self.base_chance = math.min(200, pm.alchemy.current * 1.25) + 0.5 * pm.intelligence.current
...
function Organic:_get_iter_params()
    return 
        {ref = self.ref}, 
        {
            ref=self.ref, 
            dist=cfg.sn_dist,
            container_filter=cfg.sn_cf,
            owner=self.stealing_from,
        }
end
...
function Organic:calc_item_chance(item)
    return math.clamp(self.base_chance - self.penalty_mult * item.value, cfg.min_chance, cfg.max_chance )
end
...
function Organic:on_successful_take(item, take_stack, update_stuff)
    ...
    local num_taken = item:take_from_container(take_stack, update_stuff)
    if num_taken and item.status == defns.item_status.empty then
        change_plant(item.ref, true)
        if update_stuff and num_taken then
...
function Organic:take_item(modifier_pressed)
    ...
    return super(self).take_item(self, modifier_pressed)
end
