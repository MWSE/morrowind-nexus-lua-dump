...
local Inanimate = Class.new{name="Inanimate Manager", parents={base},
    fields={
        {"taking", default=true},
...
function Inanimate:self_destruct()
    ...
    then
        ...
        ac_interop.try_to_close(self.ref, use_cfg)
    end
...
function Inanimate:_get_iter_params()
    if self.taking then return base._get_iter_params(self) end
    local banned_types = {
    ...
    return { 
        ref = tes3.player, related_ref = self.ref,
