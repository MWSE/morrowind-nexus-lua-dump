...
function Chance:update_item_chances()
    ...
    for _, item in ipairs(self.items) do
        ...
        item.show_chance = show_chances and (item.chance ~= 100 or cfg.show_chances_100)
    end
end
...
function Chance:on_successful_take(item, take_stack, update_stuff) return false end
...
---@param update_stuff boolean? should we update stuff?
---@return integer|false? num_taken the number of items taken, or `false` if no items were taken
function Chance:on_unsuccessful_take(item, take_stack, update_stuff) return false end
...
function Chance:take_all_item_check(item)
    return item.status == defns.item_status.ok and item.chance >= self.config.take_all_min_chance
end
