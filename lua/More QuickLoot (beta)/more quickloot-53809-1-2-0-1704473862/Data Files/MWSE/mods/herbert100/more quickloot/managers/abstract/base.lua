...
---@field item_taken boolean whether an item has been taken
---@field item_type MQL.Item the type of item being managed by this manager
---@field transfer_to tes3reference the container to move items to
---
...
},{i18n_index="looted.regular", item_type = Physical_Item}) -- base manager uses the general config

Manager.config = config.reg
...
do
    ...
    ---@return string label
    function Manager:_get_container_label() return self.ref.object.name end
    ...
    function Manager:_make_items()
        local iter_params = self:_get_iter_params()
        local i = 1
...
function Manager:should_take_stack(item, modifier_pressed)
    log("calculating how many items to take with\n\t\z
        count            = %i\n\t\z
        config.mi        = %i\n\t\z
        config.mi_m      = %i\n\t\z
        modifier_pressed = %s\n\t\z
        ",
        item.count, config.reg.mi.mode, config.reg.mi.mode_m, modifier_pressed
    )
    ...
    local mi = modifier_pressed and config.reg.mi.mode_m or config.reg.mi.mode
    ...
    if mi == defns.mi.ratio_or_total_weight then
       ...
       return item.value / item.weight >= config.reg.mi.min_ratio or item:total_weight() <= config.reg.mi.max_total_weight
    

    -- RATIO
    elseif mi == defns.mi.ratio then
        -- log "mi is 'ratio'"
        return item.value / item.weight >= config.reg.mi.min_ratio 

    -- WEIGHT
    elseif mi == defns.mi.total_weight then
        return item:total_weight() <= config.reg.mi.max_total_weight 

    -- RATIO AND WEIGHT
    elseif mi == defns.mi.ratio_and_total_weight then
        -- log "mi is 'ratio' AND 'total_weight'"
        return item:total_weight() <= config.reg.mi.max_total_weight and item.total_value / item:total_weight() >= config.reg.mi.min_ratio
    end
...
function Manager:take_item(modifier_pressed)
    ...
    local num_taken = item:take_from_container(full_stack, true, self.transfer_to)
...
function Manager:take_all_items(modifier_pressed)
    ...
    if self.config.mi.inv_take_all then
    ...
    for _, item in ipairs(self.items) do
        ...
        if self:take_all_item_check(item) then
            ...
            num_taken = item:take_from_container(take_stack, false, self.transfer_to)
...
--- remake the items and updates the GUI
function Manager:remake_items()
    ...
    -- self.gui:block_and_show_msg("refreshing...")
    
    self.gui:set_name_label(self:_get_container_label())

    -- recompute the `cant_loot` status now that we have remade the items
    self:update(true)
end
...
function Manager:undo()
    ...
    if item:is_instance_of(Physical_Item) then
        ---@cast item MQL.Item.Physical
        item:return_to_container(stack.num_removed, true, self.transfer_to)
    else
