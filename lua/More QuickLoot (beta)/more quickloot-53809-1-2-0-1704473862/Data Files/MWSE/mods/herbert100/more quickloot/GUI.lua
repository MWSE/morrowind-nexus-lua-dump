...
local log = require("herbert100.Logger")("More QuickLoot/GUI") ---@type herbert.Logger
...
local key_btn_names = { take = "", take_all = "", open = "", }
...
---@field key_btn_info MQL.GUI.key_btn_info? table indexed by key_name, where the value is a list containing the label for that key and the relative position in the menu
---@field key_btn_info_m MQL.GUI.key_btn_info? table indexed by key_name, where the value is a list containing the label for that MODIFIED key and the relative position in the menu
...
---@field new fun(name:string, options:MQL.GUI.new_params): MQL.GUI create a new `GUI`
local GUI = Class{name="Quick Loot GUI", new_obj_func="no_obj_data_table",
    ...
    init = function (self, params)
        ...
        do -- remake the UI
            ...
            self.ui_base.minWidth = 150

            -- name the container if we're given a name 
            if params.name and config.UI.show_name then
                -- local name_block = self.ui_base:createBlock()
        ...
        -- end
        if config.UI.show_controls and params.key_btn_info then 
            ...
            for key_name, info in pairs(params.key_btn_info) do
            ...
            if config.UI.show_controls_m and params.key_btn_info_m then
                self.controls_block_m = self.ui_base:createBlock()
                self.controls_block_m.alpha = 0.75
                self.controls_block_m.flowDirection = "left_to_right"
                self.controls_block_m.widthProportional = 1.0
                self.controls_block_m.autoHeight = true
                self.controls_block_m.paddingAllSides = 3
                self.controls_block_m.minWidth = 300 -- new
                for key_name, info in pairs(params.key_btn_info_m) do
                    local label
                    if info.label == false then
                        label = ""
                    else
                        label = string.format("(%s)", info.label)
                    end
                    self.controls_block_m:createLabel({id=key_name, text = label}).absolutePosAlignX = info.pos
                end
            end
        end
...
function GUI:_make_label_block(name)
    ...
    local name_label = self.labels_block:createLabel({id=UIIDs.name_label})
...
function GUI:_resize_labels_block()
    ...
    local new_name_width = self.name_block.width + ICON_SIZE + GOLD_OFFSET
...
function GUI:make_item_blocks(items)
    ...
    self.icon_block.width = ICON_SIZE
    ...
    self.value_block.minWidth = ICON_SIZE + GOLD_OFFSET
    self.value_block.paddingLeft = GOLD_OFFSET
    self.value_block.childAlignX = 0.5
    
    self.weight_block.minWidth = ICON_SIZE + WEIGHT_OFFSET - 15
    ...
    for i, item in ipairs(self.items) do
        ...
        icon.minHeight = ICON_SIZE
        icon.minWidth = ICON_SIZE
...
function GUI:set_name_label(new_name)
...
--- update the names of the control keys that are displayed in the UI
...
function GUI:set_control_labels(t)
...
--- update the names of the keys in the controls block
---@param t table<MQL.GUI.key_names, string|false> the keys to relabel, and what to relabel them
function GUI:set_control_m_labels(t)
    if self.controls_block_m == nil then return end

    for key_name, new_label in pairs(t) do
        log("updating control_m_labels: %s: %s", key_name, new_label)
        if new_label == false then
            self.controls_block_m:findChild(key_name).text = ""
        else
            self.controls_block_m:findChild(key_name).text = string.format("(%s)", new_label)
        end
    end
    self.controls_block_m:updateLayout()
end
