...
local Manager = Herbert_Class.new{name="Quick Loot Manager", new_obj_func ="no_obj_data_table",

    fields = {
        {"ref"},
        {"stealing_from"},
        {"cant_loot", tostring=Herbert_Class_utils.generators.table_find(defns.cant_loot)},
        {"index"},
        {"num_items", },
        {"items", tostring=Herbert_Class_utils.premade.array_tostring},
...
function Manager:add_items(from, to, obj_filter, relref)
    ...
    local obj, count
    log("adding items to %s\n\tfrom = %q, to = %q, related_ref = %q", self, from, to, relref or "N/A")
...
function Manager:add_nearby_items(p)
    ...
    for from in tes3.player.cell:iterateReferences(tes3.objectType.container) do
        ...
        -- distance check
        if dist ~= nil and (pos:distanceXY(from.position) > dist or pos:heightDifference(from.position) > v_dist) then 
            goto next_container 
        end
        ...
        if owner ~= false and owner ~= tes3.getOwner{reference=from} -- owned item check
        or allow_scripted == false and from:testActionFlag(tes3.actionFlag.useEnabled) == false -- skip scripted
        or from == ref -- dont add the same container twice
