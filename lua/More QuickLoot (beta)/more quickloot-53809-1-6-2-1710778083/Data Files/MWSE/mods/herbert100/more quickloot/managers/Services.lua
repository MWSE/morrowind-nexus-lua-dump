...
local Services = Herbert_Class.new{name="Services Manager", parents={Living},
    fields = {
        {"i18n_index", default="looted.barter"},
        {"gold_stack",},
        {"current_service", tostring=Herbert_Class_utils.generators.table_find(defns.services)},
        {"is_buying"},
        {"items"},
        {"item_stack"},
...
do -- training functionality
    local function logmsg_make_items_training(ref, skills)
        local info = tbl_ext.map(skills, function (v, ...)
