local generators = Herbert_Class_utils.generators
...
event.register("initialized", function(e)
    bg_cfg = include("buyingGame.config")
...
local Item = Herbert_Class.new{name="Abstract Item",
    fields={
...
do -- UI methods
    ...
    function Item:get_value_label()
        if bg_cfg and tes3.mobilePlayer and tes3.mobilePlayer.mercantile.current < bg_cfg.knowsPrice then
            return "?"
        end
...
local Physical_Item = Herbert_Class.new{name="Physical Item", parents={Item},
    fields = {
        {"related_ref"}, 
...
local Chance_Item = Herbert_Class.new{name="Chance Item", parents = {Physical_Item},
    fields={
        {"chance"}
    }
}
...
local Training_Item = Herbert_Class.new{name="Training Item", parents={Item},
    fields = {
...
local Barter_Item = Herbert_Class.new{name="Barter Item", parents={Physical_Item},

    ---@param self MQL.Item.Barter
    init = function(self)
        local selling = self.to ~= tes3.player
        ...
        if selling and self.status == is_ok then
            ...
            elseif bg_cfg and mref.mobile.alarm ~= 0 and bg_cfg.forbidden[self.object.id] then
                if not bg_cfg.smuggler[mref.baseObject.id:lower()] then
                    self.status = is_unavailable
