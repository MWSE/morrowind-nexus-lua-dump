...
3) you start sneaking
    ...
    function this.pickpocketing_check()
        ...
        upd_reg{event=tes3.event.simulate, callback=this.pickpocketing_check, register=false, 
...
function this.kill_manager()
    ...
    upd_reg{ tes3.event.simulate, this.monitor_manager, register=false, priority=config.advanced.simulate_priority }
end
...
function this.reset()
    ...
    upd_reg{tes3.event.simulate, this.pickpocketing_check, register=false, priority=config.advanced.simulate_priority}
    this.target = nil
end
...
do -- logic for choosing managers
    ...
    function this.choose_alive_manager(new_target)
        ...
        if config.pickpocket.enable then
            ...
            upd_reg{ tes3.event.simulate, this.pickpocketing_check, register=true, priority=config.advanced.simulate_priority }
        end
    end
...
do -- key press functions
    ...
    function event_callbacks.target_activated(e)
        if e.activator == tes3.player and this.is_manager_valid() then
            ...
            if Herbert_Class.is_instance_of(this.manager, managers.Organic) then
...
upd_reg{event=tes3.event.initialized, callback=event_callbacks.initialized, register=true}
upd_reg{event=tes3.event.modConfigReady, callback=mcm.register, register=true}
