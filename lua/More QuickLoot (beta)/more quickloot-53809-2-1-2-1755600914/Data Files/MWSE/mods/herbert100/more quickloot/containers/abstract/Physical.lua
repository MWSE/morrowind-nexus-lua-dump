...
function Physical_Container:get_action_names(item, modifier_pressed, equip_modifier_held)
	...
	local take_label = count and (count > 0) and fmt("%s %s", take_verb, count) or take_verb
...
function Physical_Container:destruct()
	...
	if #self.history > 0 then
		log("forcing inventory update because container is being destroyed")
	...
	if ty == 1 then -- take one
		...
		elseif res == -1 then
			...
			if taken > 0 and action.equip_modifier_held then
				local equipped = tes3.mobilePlayer:equip { item = selected_item.object, itemData = selected_item.data }
				log("tried to equip %s.\n\tsuccessful? %s", selected_item, equipped)
			end
