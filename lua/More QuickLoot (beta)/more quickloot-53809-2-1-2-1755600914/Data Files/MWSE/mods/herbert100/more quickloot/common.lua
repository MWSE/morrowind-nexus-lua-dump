...
-- Equal to `event.register` or `livecoding.registerEvent`, depending on whether we should be livecoding or not.
if cfg.livecoding and livecoding and type(livecoding.registerEvent) == "function" then
	log:info("Enabling livecoding.")
...
function common.update_equipped_cfg(equipped_cfg)
	...
	if allow_clothing then
		for _, clothing_slot in pairs(tes3.clothingSlot) do
			equipped_cfg.clothing_slots[clothing_slot] = true
		end
	end
	-- jewlery
...
function common.update_single_keybinding(params, action)
	if params.keyCode then
		common.keymap[params.keyCode] = action
	elseif params.mouseButton then
		common.mousemap[params.mouseButton] = action
	else
...
function common.convert_keycode_to_action(e)
	...
	if not ty then
		if cfg.keys.undo.keyCode ~= e.keyCode then
			return
		end

		local undo_cfg = cfg.keys.undo
		if undo_cfg.isAltDown == e.isAltDown
			and undo_cfg.isControlDown == e.isControlDown
			and undo_cfg.isShiftDown == e.isShiftDown
		then
			ty = defns.ActionType.Undo
		end
	end
	...
	for _, stack in pairs(box_ref.object.inventory) do
		...
		if stack.variables then
			for _, data in pairs(stack.variables) do
				tbl_insert(items,
					{ box_handle = box_handle, object = obj, data = data, count = 1, value = tes3.getValue { item = obj, itemData = data } })
				count = count - 1
			end
		end
		...
		if count > 0 then
			tbl_insert(items,
				{ box_handle = box_handle, object = obj, count = count, value = tes3.getValue { item = obj } })
		end
		::next_stack::
	end
...
function common.assert_interface_is_implemented(cls)
	...
	for k, defn in pairs(interface) do
		...
		assert(type(cls[k]) == type(defn),
			string.format("Error: cls.%s is the wrong type. It should be of type \"%s\"", k, type(defn)))
	end
end
...
do -- define function to check if a container is organic
	...
	if cfg.compat.gh_current > defns.misc.gh.never then
		gh_config = include("graphicHerbalism.config") or
			mwse.loadConfig("graphicHerbalism", { blacklist = {}, whitelist = {} })
	end
...
do -- add services functionality
	...
	function common.services.register_service(params)
		...
		local service = {
			cls = cls,
			is_valid_for_service = params.is_valid_for_service,
			display_name = params
				.display_name
		}
