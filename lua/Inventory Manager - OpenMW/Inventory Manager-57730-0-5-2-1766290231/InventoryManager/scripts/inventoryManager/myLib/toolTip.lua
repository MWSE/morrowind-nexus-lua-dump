...
local myTypes           = require('scripts.inventoryManager.myLib.myTypes')
...
local gui               = require('scripts.inventoryManager.myLib.myGUI')
local myVars            = require('scripts.inventoryManager.myLib.myVars')
local mouse             = require('scripts.inventoryManager.myLib.myUtils').mouse
...
toolTip.showToolTip = function(thing, forcePos)
        ...
        if toolTip.currentId ~= thing.id then
                return
        end
        ...
        if record.chopMaxDamage then
                if myTypes.RANGED_WEAPON[record.type] then
                ...
                elseif myTypes.RANGED_AMMO[record.type] then
                        table.insert(nameCol, 'Attack:')
                        table.insert(valueCol, string.format('%d - %d', record.chopMinDamage, record.chopMaxDamage))
                else
        ...
        if data.condition then
                ...
                table.insert(valueCol, string.format('%d / %d',
                        data.condition, max
                ))
        ...
        if record.enchant then
                ...
                if enchantment.type ~= core.magic.ENCHANTMENT_TYPE.CastOnce
                    and enchantment.type ~= core.magic.ENCHANTMENT_TYPE.ConstantEffect then
                        ...
                        table.insert(cRow, addCol({
                                createOtherLayout(string.format('%d / %d', value, max))
                        }))
