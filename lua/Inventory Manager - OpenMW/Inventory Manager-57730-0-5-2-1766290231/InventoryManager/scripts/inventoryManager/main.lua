...
local toolTip = require('scripts.inventoryManager.myLib.toolTip')
...
local function updateKeyBind(settingsObj)
        ...
        if mybind then
                if type(mybind.button) ~= "number" then
                        bindingSection:set(newBind, nil)
                        return
                end
...
local function getSettings(keyForSection, key)
        if key == o.showWindowKey.key then
                updateKeyBind(o.showWindowKey)
        end
...
local function moveItem(info)
        ...
        elseif info.object.parentContainer.type == types.Container then
                ...
                core.sendGlobalEvent(events.moveItem, eventData)
        elseif mySection:get(o.showWhenStealing.key) then
                eventData = {
                        item = info.object,
                        from = currContainer,
                        to = self.object,
        ...
        for index = 1, #g.types.columns do
                ...
                local textEl = {
                        ...
                        props = {
                                ...
                                -- textColor = lists.SORT_BY == index and g.colors.selected or g.colors.normal,
                        },
                }
                ...
                local textContainer = {
                        ...
                        events = {
                                mouseClick = async:callback(function(e, l)
                                        ...
                                        -- for i = 1, #allEls do
                                        --         local ind = #allEls - i + 1
                                        --         allEls[i].content[1].props.textColor = lists.SORT_BY == ind and
                                        --             g.colors.selected or g.colors.normal
                                        -- end

                                        -- MainWindow.layout.userData.contentContainer:update()
                ...
                events = {
                        ...
                        focusGain = async:callback(function()
                                ...
                                g.util.debounce('showToolTip', mySection:get(o.toolTipDelay.key), function()
                                        toolTip.currentId = info.object.id or info.object.recordId
                                        toolTip.showToolTip(info.object)
                                end)

                                parentWidget.layout.template = g.templates.entryHighlight
                                table.insert(g.myVars.myDelayedActions, 1, parentWidget)
                                return true
                        end),
...
local function showPlayerInventory(container)
        ...
        g.myVars.gv.Year = calendar.formatGameTime("%Y", core.getGameTime())
        g.myVars.gv.PCNAME = self.type.record(self).name
        g.myVars.gv.pcname = self.type.record(self).name
        g.myVars.gv.PCName = self.type.record(self).name
        g.myVars.gv.PCClass = types.NPC.classes.records[self.type.record(self).class].name
        g.myVars.gv.PCRace = types.NPC.races.records[self.type.record(self).race].name
        ...
        if container then
                if mySection:get(o.showWhenStealing.key) then
        ...
        g.myVars.mainWindow.element = g.window.createResizableWindow({
                title = 'Interface',
                tabs = tabs,
                default = tabs[1].name,
        })
...
local function onFrame(dt)
        ...
        while #g.myVars.myDelayedActions > 0 do
...
local function onLoad()
        ...
        g.types.lockedStuff.items = locakedItems or {}
end
...
local function newUiModeChanged(data)
        if data.newMode == I.UI.MODE.Interface or data.newMode == I.UI.MODE.Container then
                if mySection:get(o.showWhenInventory.key) then
                        if I.UI.isWindowVisible then
                                if I.UI.isWindowVisible(I.UI.WINDOW.Inventory) and not g.myVars.mainWindow.element.layout then
        ...
        elseif mySection:get(o.showWhenStealing.key) and data.newMode ~= nil then
                if I.UI.isWindowVisible then
                        if I.UI.isWindowVisible(I.UI.WINDOW.Inventory) and not g.myVars.mainWindow.element.layout then
                                showPlayerInventory(data.arg)
                        end
...
return {
        ...
        interfaceName = "InventoryManagerOpenwMWMod",
        interface = {
                version = 1,
                oneKeyActionsListData = {
                        {
                                getName = function()
                                        QuickStack.getValidContainers()
                                        return string.format('Quick Stack (%s nearby)', #QuickStack.validContainers)
                                end,
                                action = function()
                                        if #QuickStack.validContainers < 1 then
                                                return
                                        end
                                        QuickStack.stackItemsToNearbyChests()
                                end,
                        }
                },
        },
        ...
        engineHandlers = {
                ...
                onKeyPress = function(e)
                        -- if e.symbol == showWindowKey then
                        --         showInvKeyHandler()
                        -- end
                end,
        },
