                ...
                content = ui.content {
                        {
                                ...
                                events = {
                                        mouseClick = async:callback(function(_, l)
                                                ...
                                                table.insert(g.myVars.myDelayedActions, 1, el)
                                        end)
                                }
                        },
                        g.gui.makeInt(50, 0),
...
function QuickStack.getValidContainers()
        QuickStack.validContainers = {}
        for i = 1, #nearby.containers do
                ...
                if QuickStack.allContainers[container.id]
                    ...
                    and dist < STACK_DISTANCE then
                        table.insert(QuickStack.validContainers, container.recordId)
                end
        end
end
...
function QuickStack.getPlayerQuickStackButtons()
        ...
        text = ui.create {
                template = I.MWUI.templates.textNormal,
                props = {
                        text = string.format('Quick Stack (%s nearby)', #QuickStack.validContainers),
                        textSize = g.sizes.LABEL_SIZE,
                        textColor = #QuickStack.validContainers > 0 and g.colors.normal or g.colors.disabled,
                },
                events = {
                        mouseClick = async:callback(function()
                                if #QuickStack.validContainers == 0 then
                                        return
                                end
                                stackItemsToNearbyChests()
                                text.layout.props.textColor = g.colors.selected
                                table.insert(g.myVars.myDelayedActions, 1, text)
                        end),
...
QuickStack.stackItemsToNearbyChests = stackItemsToNearbyChests
