...
local getType = require('scripts.inventoryManager.myTypes').getType
...
function QuickStack.updateAllContainers(savedList)
        QuickStack.allContainers = {}
        if savedList then
                for _, v in pairs(savedList) do
                        -- print('loading: ', v.name)
                        QuickStack.allContainers[v.id] = v
                end
        end
end
...
local stackItemsToNearbyChests = function()
        ...
        for pItemIndex = 1, #playerItems do
                ...
                for id, data in pairs(QuickStack.allContainers) do
                        ...
                        if dist > 800 then goto next_container end
                        ...
                        if data.byName then
                                ...
                                for recordId, _ in pairs(itemRecordIds) do
                                        if recordId == item.recordId then
                                                -- core.sendGlobalEvent(events.moveItem, {
                                                --         item = item,
                                                --         from = self,
                                                --         to = obj,
                                                --         count = item.count
                                                -- })
                                                -- table.insert(itemsToBeMoved, item)
...
function QuickStack.getPlayerQuickStackButtons()
        ...
        for i = 1, #nearby.containers do
                ...
                if QuickStack.allContainers[container.id] and QuickStack.allContainers[container.id].quickStackTarget then
