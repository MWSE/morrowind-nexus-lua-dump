        ...
        if (hasMasterIndex) then
            for index, fort in ipairs(positions) do
                if (fort.cell == lastChamber) then
                    world.createObject("zhac_bmi_thunderitem"):moveInto(
                        types.Actor.inventory(actor))
                    actor:teleport(fort.cell,
                        util.vector3(fort.x, fort.y, fort.z), {
                            rotation = util.vector3(0, 0, math.rad(fort.rotation)),
                            onGround = true
                        })
                end
            end
...
local function activatePort(object, actor)
    ...
    for index, fort in ipairs(positions) do
        ...
        if (object.recordId == "active_port_" .. fort.short) then
            ...
            if (playerSneaking and hasMasterIndex) then
                world.createObject("zhac_bmi_thunderitem"):moveInto(types.Actor
                    .inventory(
                        actor))
                actor:teleport("Caldera, Guild of Mages",
                    util.vector3(calderaPos[TpToLevel].x,
                        calderaPos[TpToLevel].y,
                        calderaPos[TpToLevel].z), {
                        rotation = util.vector3(0, 0, math.rad(
                            calderaPos[TpToLevel].rotation)),
                        onGround = true
                    })
            elseif (hasIndex or hasMasterIndex) then
                world.createObject("zhac_bmi_thunderitem"):moveInto(types.Actor
                    .inventory(
                        actor))
                actor:teleport(fort.cell, util.vector3(fort.x, fort.y, fort.z),
                    {
                        rotation = util.vector3(0, 0, math.rad(fort.rotation)),
                        onGround = true
                    })
        ...
        for x, object in ipairs(cxell:getAll(types.Activator)) do
            for l, tfort in ipairs(positions) do
                if (object.recordId == "active_port_" .. tfort.short) then
                    if (types.Activator.record(object).mwscript ~= nil and
                            types.Activator.record(object).mwscript ~= "") then
                        errorState = true
                    end
                    --  local newOb = removeScript(object)
