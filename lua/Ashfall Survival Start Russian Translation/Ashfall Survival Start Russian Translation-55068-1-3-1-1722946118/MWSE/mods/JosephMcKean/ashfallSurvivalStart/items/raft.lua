...
local sailHome = {
	...
	tooltipDisabled = function(e)
		return {
			text = onWater(e) and "Вы не знаете, в какой стороне находится Вварденфелл. Может быть, на этом острове есть карта?" or "Плот нужно поместить на открытую воду.",
		}
	end,
}
...
local function registerBushcraftingRecipe(e)
	...
	local recipe = {
		...
		description = "Плот, на котором можно уплыть.",
