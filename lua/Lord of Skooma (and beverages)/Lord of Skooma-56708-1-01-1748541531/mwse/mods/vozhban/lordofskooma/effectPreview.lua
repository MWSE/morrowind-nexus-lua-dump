...
local function calculate(ref)
    ...
    local function effKey(id, attr, skill)
        if attr and attr ~= -1 then return ("%dA%d"):format(id, attr) end
        if skill and skill ~= -1 then return ("%dS%d"):format(id, skill) end
        return tostring(id)
    end
                            ...
                            for k, e2 in ipairs(ing2.effects) do
                                if e1 == e2 then
                                    ...
                                    if match then
                                        ...
                                        local skill = nil
                                        if me and me.targetsAttributes then
                                            attr = ing1.effectAttributeIds and ing1.effectAttributeIds[j] or nil
                                        elseif me and me.targetsSkills then
                                            skill = ing1.effectSkillIds and ing1.effectSkillIds[j] or nil
                                        end

                                        local key = effKey(e1, attr, skill)
                                        --mwse.log("[mortar] Key: "..key)
                                        if preview.effects[key] then
                                            addEffect(e1, item.quality, item.quality, attr, skill, true)
                                            commitEmpower(item.quality, item.quality)
                                        else
                                            addEffect(e1, 0, 0, attr, skill, true)
                                        end
                                    end
                                end
                            end
    ...
    local function eachEffect(obj, fn)
        ...
        if obj.objectType == tes3.objectType.ingredient then
            for idx, id in ipairs(obj.effects) do
                if id ~= -1 then
                    fn(id,
                    (tes3.getMagicEffect(id).targetsAttributes and obj.effectAttributeIds and obj.effectAttributeIds[idx]) or nil,
                    (tes3.getMagicEffect(id).targetsSkills and obj.effectSkillIds     and obj.effectSkillIds[idx])     or nil)
                end
            end
        elseif obj.objectType == tes3.objectType.alchemy then
            for _, e in ipairs(obj.effects) do
                if e.id ~= -1 then fn(e.id, e.attribute, e.skill) end
            end
        end
    end
            ...
            if app and app.type == tes3.apparatusType.alembic then
                ...
                if srcId then
                    eachEffect(tes3.getObject(srcId), function(id, attr, skill)
                        if attr  == -1 then attr  = nil end   -- <- add these two lines
                        if skill == -1 then skill = nil end
                        local key = effKey(id, attr, skill)
                        --mwse.log("[alembic] Key: "..key)
                        local fx  = preview.effects[key]
                        if fx then
                            if guaranteedMap[key] then
