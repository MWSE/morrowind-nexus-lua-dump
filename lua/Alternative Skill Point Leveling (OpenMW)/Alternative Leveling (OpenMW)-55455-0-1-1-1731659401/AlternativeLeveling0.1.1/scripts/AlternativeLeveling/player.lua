...
local majorMinorSkillPointsBase = 10
local miscSkillPointsBase = 5
local attributeSkillPointsBase = 7
...
local function createButton(clickCallback,buttonText,buttonPos,buttonSize) 
	...
	events = {
		mousePress = async:callback(function(e, thisObject)
			print("onEvent mousePress for: " .. thisObject.userData.textWidget.props.text)
		...
		mouseRelease = async:callback(function(e, thisObject)
			print("onEvent mouseRelease for: " .. thisObject.userData.textWidget.props.text)
		...
		focusGain = async:callback(function(e, thisObject)
			print("onEvent focusGain for: " .. thisObject.userData.textWidget.props.text)
			thisObject.userData.textWidget.props.textColor = thisObject.userData.textColorOver
			end),
		focusLoss = async:callback(function(e, thisObject)
			print("onEvent focusLoss for: " .. thisObject.userData.textWidget.props.text)
...
local function updateMaxStats(oldEndurance)
	...
	if playerSettings:get("retroactiveEndurance") then
		types.Actor.stats.dynamic.health(self).base = (oldhealth - (oldEndurance / 10) * (types.Actor.stats.level(self).current - 2)) + (endurance / 10) * (types.Actor.stats.level(self).current - 1)
	else
