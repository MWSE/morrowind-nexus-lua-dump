...
I.Settings.registerGroup {
    ...
    settings = {
        ...
        },
		{
            ...
            name = 'TrainingDeductionsName',
            description = 'TrainingDeductionsDescription',
            default = true,
        },
		{
            key = 'LevelOffset',
        ...
        },
		{
            ...
            name = 'QuestXPName',
            description = 'QuestXPDescription',
            default = 0,
        },
    },
}
...
local playerQuestsComplete

local function getPlayerCompletedQuests()

	local completedQuests = 0

	for i, quest in pairs(Player.quests(self)) do
...
local function onInit()
	...
	playerQuestsComplete = getPlayerCompletedQuests()
end
...
local confirmFnc = function (thisObject)
	...
	if (thisObject.userData.parentWidget.userData.majorMinorPoints == 0) and (thisObject.userData.parentWidget.userData.miscPoints == 0) and (thisObject.userData.parentWidget.userData.attributePoints == 0) then
		...
		types.Actor.stats.level(self).progress = types.Actor.stats.level(self).progress - 10
...
local function calculateSkillpoints()
	...
	if playerSettings:get('SkillPointDeductions') then
		...
		miscSkillPoints = playerSettings:get('MiscSkillPoints')
		attributeSkillPoints = playerSettings:get('AttributeSkillPoints')
	end
...
local function CheckQuestComplete(questId, stage)
	if  playerSettings:get("QuestLevelProgress") > 0 then

		local newQuestNumber = getPlayerCompletedQuests()

		if newQuestNumber > playerQuestsComplete then
			types.Actor.stats.level(self).progress = types.Actor.stats.level(self).progress + playerSettings:get("QuestLevelProgress")
			ui.showMessage('You have made progress towards your next level.')
			ambient.playSound("skillraise")
			playerQuestsComplete = newQuestNumber
		end
	end
end
		...
		if isBook or isTrainer then
			...
			if isTrainer then
				...
				if majorMinorSkillDeductions > (playerSettings:get('MajorMinorSkillPoints')) then
					majorMinorSkillDeductions = playerSettings:get('MajorMinorSkillPoints')
				end
			end
...
return {
    engineHandlers = {
		...
		onQuestUpdate = function(questId, stage)
			CheckQuestComplete(questId, stage)
		end,
		onSave = onSave,
        onLoad = onLoad,
		onInit = onInit,
