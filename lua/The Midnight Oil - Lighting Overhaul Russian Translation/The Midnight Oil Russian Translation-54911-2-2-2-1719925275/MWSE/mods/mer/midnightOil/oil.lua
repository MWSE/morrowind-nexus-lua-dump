    ...
    if common.isOil(e1.item) then
        ...
        timer.delayOneFrame(function()
            ...
            tes3ui.showInventorySelectMenu{
                title = "Заправка масляной лампы",
                noResultsText = "У вас нет масляных ламп, которые можно было бы заправить.",
                filter = function(e2)
                    return (
                ...
                callback = function(e3)
                    ...
                    tes3.messageBox("Вы пополняете запасы масла в лампе.")
                end
            }
        end)
        return false
...
local function onTooltip(e)
    ...
    if e.object ~= nil then
        if common.isOil(e.object) then
            ...
            local label = block:createLabel{ text = "Оставшееся масло: " }
...
local function onActivateAshMire(e)
    ...
    if e.target and common.isOilSource(e.target.object) then
        tes3ui.showInventorySelectMenu{
            title = "Фляга для масла",
            noResultsText = "У вас нет фляги для пополнения запасов масла.",
            filter = function(e2)
                return (
            ...
            callback = function(e3)
                ...
                tes3.playSound{ reference = tes3.player, sound = "potion success"}
                tes3.messageBox("Вы пополняете свою флягу маслом.")
            end
        }
    end
end
