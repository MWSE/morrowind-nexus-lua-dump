...
local curses = {
    { name = "Ликантропия", spellId = "werewolf blood" },
    { name = "Вампир (Клан Аунда)", spellId = "vampire blood aundae", startScript = "vampire_aundae_pc" },
    { name = "Вампир (Клан Берне)", spellId = "vampire blood berne", startScript = "vampire_berne_pc" },
    { name = "Вампир (Клан Куарра)", spellId = "vampire blood quarra", startScript = "vampire_quarra_pc" },
}
...
function CurseFeature.callback(e)
    ...
    local header = block:createLabel{ text = "Выберите проклятие:" }
    ...
    local randomButton = buttonsBlock:createButton{ text = "Случайное" }
    ...
    local confirmButton = buttonsBlock:createButton{ text = "Подтвердить" }
...
function CurseFeature.getTooltip()
    return CurseFeature.currentCurse and ("Проклятие: " .. CurseFeature.currentCurse.name) or "Проклятие не выбрано."
end
