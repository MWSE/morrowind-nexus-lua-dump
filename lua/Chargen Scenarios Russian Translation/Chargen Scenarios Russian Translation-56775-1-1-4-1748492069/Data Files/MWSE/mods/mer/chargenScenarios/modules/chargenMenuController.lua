...
local function modifyStatReviewMenu(e)
    ...
    okButton:register("mouseClick", function(eMouseClick)
        ...
        tes3.messageBox("Вы должны завершить процесс создания персонажа, прежде чем сможете продолжить.")
    end)
    menu:updateLayout()
end
...
local function modifyClassChoiceMenu(e)
    ...
    local title = menu:createLabel{ text = "Выберите свой класс"}
    ...
    menu:findChild("MenuClassChoice_PickClassbutton").text = "Выбор из списка классов"
    --Rename text of create class button to "Create Custom Class"
    menu:findChild("MenuClassChoice_CreateClassbutton").text = "Создать свой класс"
end
