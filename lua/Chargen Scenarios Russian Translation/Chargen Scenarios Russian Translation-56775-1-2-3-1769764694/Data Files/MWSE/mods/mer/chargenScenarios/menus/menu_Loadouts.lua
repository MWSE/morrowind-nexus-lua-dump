...
local menu = {
    id = "loadoutsMenu",
    name = "Наборы предметов",
    priority = -1500,
    buttonLabel = "Предметы",
    getButtonValue = function(self)
        return string.format("Выбранные наборы: %d", getNumActiveLoadouts(Loadouts.getLoadouts()))
    end,
    getTooltip = function(self)
        local loadouts = Loadouts.getLoadouts()
        local header = "Наборы предметов"
        local description = ""
        for _, itemList in ipairs(loadouts) do
        ...
        if description == "" then
            description = "Не выбрано ни одного набора предметов."
        else
             --remove last newline
...
local function updateLimitLabel(loadouts)
    ...
    local text = string.format("Выбрано наборов: %s/%s", activeLoadouts, limit)
