...
function CompanionsFeature.getDescription(companion)
    ...
    local description = string.format("Имя: %s \nУровень: %s",
...
function CompanionsFeature.updateDetailsPane(detailsPane, companion)
    ...
    -- Level
    local levelLabel = contentBlock:createLabel{ text = string.format("Уровень: %s", companion.object.level) }
    ...
    if isNPC then
        local raceLabel = contentBlock:createLabel{ text = string.format("Раса: %s", companion.object.race.name) }
        local sexLabel = contentBlock:createLabel{ text = string.format("Пол: %s", companion.object.female and "Женский" or "Мужской") }
        local classLabel = contentBlock:createLabel{ text = string.format("Класс: %s", companion.object.class.name) }
        classLabel.borderBottom = 3
    else
        -- For creatures, show type
        local typeLabel = contentBlock:createLabel{ text = "Тип: Существо" }
        typeLabel.borderBottom = 3
    end

    -- Custom description
    if companion.description then
        local descriptionHeader = contentBlock:createLabel{ text = "Описание:" }
    ...
    local selectButton = buttonBlock:createButton{ text = isSelected and "Выбрано" or "Выбрать" }
...
CompanionsFeature.callback = function(e)
    ...
    local header = mainBlock:createLabel{ text = "Выберите компаньонов:" }
    ...
    local listLabel = leftPanel:createLabel{ text = "Компаньоны:" }
    ...
    local detailsLabel = rightPanel:createLabel{ text = "Подробности:" }
    ...
    local randomButton = buttonBlock:createButton{ text = "Случайный выбор" }
    ...
    --Reset
    local resetButton = buttonBlock:createButton{ text = "Сброс" }
    ...
    --Confirm
    local confirmButton = buttonBlock:createButton{ text = "Подтвердить" }
