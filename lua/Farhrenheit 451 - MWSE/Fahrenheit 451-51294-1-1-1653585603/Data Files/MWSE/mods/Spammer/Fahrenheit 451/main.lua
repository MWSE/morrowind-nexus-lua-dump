local mod = { name = "Fahrenheit 451", ver = "1.1"}
local burnt = {}
...
local function onProjectileHit(e)
    ...
    if fire and #targets ~= 0 then
        for _, target in ipairs(targets) do
            ...
            tes3.createVisualEffect({object = "light_fire", scale = .1, position = target.position, lifespan = 10})
...
event.register("bookGetText", function(e)
    if table.find(burnt, e.book) then
        e.text = [[<DIV ALIGN="LEFT"><FONT COLOR="000000" SIZE="3" FACE="Magic Cards"><BR>
        <BR>
        [The text is unreadable, totally burnt up.]<BR>]]
    end
end)
...
local function bookGetText(e)
    ...
    return false
end event.register("activate", bookGetText, {priority = 11})
