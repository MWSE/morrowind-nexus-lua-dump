...
local function getJumpFallLostHealth(depth, soft)
    ...
    return depth <= 0 and 0 or depth * (1 - 0.25 * mC.GMSTs.fFatigueBase)
end
...
local function onJumpFinished(state, stats, factor)
    self:sendEvent(mDef.events.applySkillUsedHandlers, {
        ...
        params = {
            skillGain = stats.skillGain * factor,
...
local function onJumpLanded(state)
    ...
    -- Factors
            ...
            factor, stats.skillGain, stats.skillGain * factor))
...
local function handleJumps(state, isOnGround, hasFlyingEffects)
    ...
    if hasFlyingEffects then
        ...
        log(string.format("Used levitate or slowFall during a jump, gain %.3f -> %.3f", stats.skillGain, stats.skillGain * minFactor))
        onJumpFinished(state, stats, stats.skillGain * minFactor)
    end
...
local function handleAthletics(state, isOnGround, deltaTime)
    ...
    if isMoving then
        ...
        state.skills.scaled.athletics.runningDuration = math.max(0, stats.runningDuration - 2 * deltaTime)
    end
end
...
local function skillUsedHandlerMagickaRefund(_, skillId, params)
    if params.manual then return end
    ...
    for school, ratio in pairs(spellSchoolRatios[spell.id]) do
        if ratio < 1 then
            ...
            if skillId == school then
                ...
                self:sendEvent(mDef.events.applySkillUsedHandlers, { skillId = school, params = { skillGain = gain, useType = params.useType }, afterHandler = "multiSchool" })
            end
        end
    end
end
...
local function skillUsedHandlerScaled(state, skillId, params)
    ...
    if magickaSkills[skillId] then
        ...
        local spell = T.Actor.getSelectedSpell(self)
        local chance = mC.GMSTs.fFatigueBase * mSpells.calcAutoCastChance(spell, self) / 100
        factor = capSkillScaledFactor(
                2 * (1 - chance / 2),
                mS.getSkillGainScaledRange("magickaBasedSkillScalingRange"))
        log(string.format("Cast the spell cost \"%s\" cost %d, chance = %.2f, factor = %.3f, gain %.3f -> %.3f",
    ...
    elseif weaponSkills[skillId] and skillId ~= Skills.marksman.id then
        ...
        if enemy then
            if enemy.type.baseType == T.Actor then
                ...
                log(string.format("Skill used scaled: The target \"%s\" is not an actor", enemy.recordId))
            end
        else
            print("Skill used scaled: No enemy found")
        end
    elseif skillId == Skills.security.id then
        ...
        if not lockableStats then
            print("Skill used scaled: No lockable found")
        else
    ...
    elseif armorSkills[skillId] then
        ...
        if physicalDamage <= 0 then
            print(string.format("No physical damage: %.1f", physicalDamage))
            return
        end
        if mC.self.health.base <= 0 then
            print(string.format("Base health is not positive: %.1f", mC.self.health.base))
            return
        end
    ...
    elseif skillId == Skills.athletics.id then
        ...
        log(string.format("Athletics long run factor duration %.2f velocity %.2f final %.2f, gain %.3f -> %.3f",
                timeRatio, velocityRatio, factor, params.skillGain, params.skillGain * factor))
        stats.deltaTime = 0
        stats.deltaPos = 0
