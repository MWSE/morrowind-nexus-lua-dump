...
local function getTotalPlayerLevel(state)
    local levelGain = 1 / iLevelupTotal
    local totalLevel = 1
...
local function getHealthFactor(state)
    local baseHPRatio = mSettings.healthStorage:get("baseHPRatio")
    local factor = baseHPRatio ~= "full" and mSettings.getBaseHPRatioFactor(baseHPRatio) or 1
    local attrFactor = 0
