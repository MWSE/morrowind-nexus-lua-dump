...
local function calcAutoCastChance(spell, player)
...
local function attrSkillCheck(spell, context)
    for _, effect in ipairs(spell.effects) do
        if effect.affectedSkill then
            ...
            if not skill or skill(context.player).base < mC.GMSTs.iAutoSpellAttSkillMin then
                return false
            end
        end
        if effect.affectedAttribute then
            local attribute = T.Actor.stats.attributes[effect.affectedAttribute]
            if not attribute or attribute(context.player).base < mC.GMSTs.iAutoSpellAttSkillMin then
                return false
            end
        end
        return true
...
local function selectStarterSpell(spell, context)
    ...
    if calcAutoCastChance(spell, context.player) < mC.GMSTs.fAutoPCSpellChance then return end
    ...
    if context.reachedLimit then
        ...
        if #context.selectedSpells == mC.GMSTs.iAutoPCSpellMax then
...
local function autoCalcPlayerSpells(player)
    local context = {
        player = player,
        baseMagicka = mC.GMSTs.fPCbaseMagickaMult * T.Actor.stats.attributes.intelligence(player).base,
        reachedLimit = false,
        weakestSpell = nil,
        minCost = math.huge,
