...
local function updatePlayerStats(forceUpdateAttributes)
    local growthRateNum = S.getGrowthDecayRates(S.attributesStorage:get("growthRate"))
    ...
    local attributesUncapped = S.attributesStorage:get("uncapperEnabled")
    ...
    for attributeId, _ in pairs(toRecalculate) do
        ...
        total = math.floor(math.sqrt(total * growthRateNum / 27)
                ...
                + diff
                + (baseStatsMods.attributes[attributeId] or 0)
        if not attributesUncapped then
...
local function addSkillGain(skillId, skillGain)
    ...
    if excessSkillGain >= 0 then
        ...
        if not S.skillsStorage:get("carryOverExcessSkillGain") or
                not S.skillsStorage:get("uncapperEnabled") and Player.stats.skills[skillId](self).base > 99 then
            progress = 0
        else
...
local function addSkillUsedHandlers()
    ...
    I.SkillProgression.addSkillUsedHandler(function(skillId, _)
        if not S.skillsStorage:get("uncapperEnabled") and Player.stats.skills[skillId](self).base > 99 then
            Player.stats.skills[skillId](self).progress = 0
...
local function onLoad(data)
    ...
    if data then
        ...
        elseif isDataMissing then
            isDisabled = true
            C.setHasStats(false)
            C.showMessage(L("requiresNewGameWithOpenmw49"))
            return
        end
        if C.hasStats() then
            updatePlayerStats(true)
...
local interface = {
    ...
    Attribute = function(attributeId, val)
        local changed
        if attributeId ~= nil then
            if Player.stats.attributes[attributeId] == nil then
                print("NCGDMW/Interface/Attribute(): Invalid attribute name given")
                return
            end
            if val ~= nil then
                changed = C.setStat("attributes", attributeId, val)
            end
...
return {
    ...
    eventHandlers = {
        ...
        updatePlayerStats = function(forceUpdateAttributes)
            if C.hasStats() then
