...
local is049orNewer = core.API_REVISION >= 56
local hasPlugins
...
local function doHealth(deltaTime)
    ...
    if lastUpdateHealthTime < 1 then return end
    lastUpdateHealthTime = 0

    local recalculate = false
    local stateBasedHP = playerStorage:get("stateBasedHP")
    ...
    if recalculate or forceUpdateHealth then
        ...
        if playerStorage:get("perLevelHPGain") == "low" then
            hpGainRatio = 20
        end
        forceUpdateHealth = false
        local currentLevel = Player.stats.level(self).current
        local maxHealth = getHealthFactor(healthAttributes) + (currentLevel - 1) * getHealthFactor(healthAttributes) / hpGainRatio
...
local settingsPlayers = storage.playerSection('SettingsPlayer' .. MOD_NAME)
settingsPlayers:subscribe(async:callback(updatePlayer))
...
if is049orNewer then
    I.SkillProgression.addSkillUsedHandler(function(skillId, params)
        debugPrint(string.format("Skill '%s' used, base gain = %s", skillId, params.skillGain))
        local skillIncreaseConstantFactor = playerStorage:get("skillIncreaseConstantFactor")
        if skillIncreaseConstantFactor ~= "vanilla" then
            if skillIncreaseConstantFactor == "half" then
                params.skillGain = params.skillGain / 2
            elseif skillIncreaseConstantFactor == "quarter" then
                params.skillGain = params.skillGain / 4
            end
            debugPrint(string.format("Skill gain of '%s' reduced by constant, new gain = %s", skillId, params.skillGain))
        end
        local skillIncreaseSquaredLevelFactor = playerStorage:get("skillIncreaseSquaredLevelFactor")
        if skillIncreaseSquaredLevelFactor ~= "disabled" then
            local skillLevelNorm = Player.stats.skills[skillId](self).base / 100
            if skillIncreaseSquaredLevelFactor == "downToHalf" then
                params.skillGain = params.skillGain / (skillLevelNorm * skillLevelNorm + 1)
            elseif skillIncreaseSquaredLevelFactor == "downToAQuarter" then
                params.skillGain = params.skillGain / (3 * skillLevelNorm * skillLevelNorm + 1)
            elseif skillIncreaseSquaredLevelFactor == "downToAEighth" then
                params.skillGain = params.skillGain / (7 * skillLevelNorm * skillLevelNorm + 1)
            end
            debugPrint(string.format("Skill gain of '%s' reduced by square, new gain = %s", skillId, params.skillGain))
        end
    end)

    I.SkillProgression.addSkillLevelUpHandler(function(skillId, source)
        if source == I.SkillProgression.SKILL_INCREASE_SOURCES.Book and not playerStorage:get("skillIncreaseFromBooks") then
            debugPrint(string.format("Preventing skill '%s' level up from book", skillId))
            return false
        end
        if hasStats then
            doAttributes()
