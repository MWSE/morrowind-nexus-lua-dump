...
local hotkeyWidgetName
if is049orNewer then
    hotkeyWidgetName = "NCGDMW_hotkey"
else
    hotkeyWidgetName = "MBSP_Uncapper_hotkey"
    I.Settings.registerRenderer(
        hotkeyWidgetName, function(value, set)
            return {
...
if is049orNewer then
	I.SkillProgression.addSkillUsedHandler(function(skillId, params)
		if (magicSkills[skillId]) then
			local cost = types.Player.getSelectedSpell(self).cost
			params.skillGain = params.skillGain * cost / playerStorage:get("magickaXPRate")
			if playerStorage:get("refundEnabled") then
				modMagicka(getRefund(types.Player.stats.skills[skillId](self).base, cost))
			end
		end
		if playerStorage:get("uncapperEnabled") then
			if types.Player.stats.skills[skillId](self).base > 99 then
				skillProgress[skillId] = skillProgress[skillId] + params.skillGain
				if skillProgress[skillId] >= 1 then
					modSkillLevel(skillId, 1)
					skillProgress[skillId] = 0
					ui.showMessage('Your ' .. skillNames[skillId] .. ' skill increased to ' .. types.Player.stats.skills[skillId](self).base .. '.')
					maybePlaySound()
				end
			end
		end
...
local function onFrame()
	if is049orNewer then return end
	...
	--Magicka Skills
	for skill, _ in pairs(magicSkills) do
		if types.Player.stats.skills[skill](self).base > 99 then
			local dif = types.Player.stats.skills[skill](self).progress
			if dif > 0.001 then
