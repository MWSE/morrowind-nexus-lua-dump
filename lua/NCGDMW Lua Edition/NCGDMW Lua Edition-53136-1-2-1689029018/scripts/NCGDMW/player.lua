...
local function recalculateDecayMemory()
    ...
    local decayRate = rateMap[playerStorage:get("decayRate")]
...
local function doAttributes()
    local decayRate = rateMap[L(playerStorage:get("decayRate"))]
    local growthRate = rateMap[L(playerStorage:get("growthRate"))]
...
local function onKeyPress(key)
    ...
    if key.code == playerStorage:get("statsMenuKey") then
        local menu
        if rateMap[L(playerStorage:get("decayRate"))] > none then
            menu = ncgdUI.decayStatsMenu(decaySkills, decayMemory, playerStorage:get("starwindNames"), playerStorage:get("decayRate"), playerStorage:get("growthRate"))
