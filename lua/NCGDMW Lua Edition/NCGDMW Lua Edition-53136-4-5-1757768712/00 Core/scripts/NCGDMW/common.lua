...
local function addModStatMessage(kind, statId, prevValue, options)
    ...
    local message = L(toShow, { stat = getStatName(kind, statId), value = math.floor(current + 0.5) })
...
module.isMeleeAttacking = function()
    return lastAnimation and module.meleeAttackGroups[lastAnimation.group] and string.sub(lastAnimation.key, -3) == "hit"
end

module.isSpellCasting = function()
