...
local function updateGrowth(requestType)
    local growthRateNum = mS.getAttributeGrowthRates(mS.attributesStorage:get("attributeGrowthRate")) - 1
    local baseStatsMods = mC.getBaseStatsModifiers()
    local attrsToUpdate = mSkills.updateSkills(state, baseStatsMods, requestType)
...
local function onFrame(deltaTime)
    ...
    for _, request in ipairs({ mDef.requestTypes.refreshStats, mDef.requestTypes.skillChange }) do
        if requests[request] then
            requests[request] = false
            updateGrowth(request)
        end
    end
...
local function onUpdate(deltaTime)
    ...
    if requests[mDef.requestTypes.refreshStatsOnResume] then
        ...
        updateGrowth(mDef.requestTypes.refreshStats)
        requests[mDef.requestTypes.health] = true
    end
...
return {
    ...
    eventHandlers = {
        ...
        [mDef.events.updateRequest] = function(requestType) requests[requestType] = true end,
