...
local MaxSlopeAngle = math.rad(46.0);
local lavaIdCache = {}
...
local function checkPhysicalDamage(state, deltaTime)
    ...
    for _, effect in pairs(T.Actor.activeEffects(self)) do
        if mC.healthEffectIds[effect.id] then
...
local function getDepthInfo(stats, pos, fromZ)
    ...
    local result = nearby.castRay(pos, to, {
        ...
        radius = stats.halfExtents.y / 2,
    })
    if result.hitPos then
        return math.max(0, fromZ - result.hitPos.z), result
    end
...
local function getLandingRiskStillFactor(stats)
    ...
    for slice = 1, slices do
        ...
        local function getLandingDepthRiskFactor(depth, hitInfo)
            local depthFactor = getDepthFactor(hitInfo)
            --if trace then table.insert(logStats, string.format("depthFactor %.2f", depthFactor)) end
            return depthFactor * math.min(mCfg.jumpGainLandingFallDamageMaxFactor, getHealthRisk(getFallLostHealth(depth)))
        end

        local function addRiskFactor(factor, slopeAngle, _step)
            ...
            local finalFactor = factor ^ slopeFactor * stepFactor
            -- only consider the highest risk for a given direction
            maxRisk = math.max(maxRisk, finalFactor)
            if trace then table.insert(logStats, string.format("risk %.2f, slope %.2f, stepF %.2f, slopeF %.2f, factor %.2f",
                    factor, slopeAngle, stepFactor, slopeFactor, finalFactor)) end
        end

        local function endOfSlipperySlope()
            addRiskFactor(
                    getLandingDepthRiskFactor(slipperySlope.depth, slipperySlope.hitInfo),
                    mH.angle(slipperySlope.depth, slipperySlope.prevDepth, slipperySlope.dist, slipperySlope.prevDist),
                    slipperySlope.step)
            slipperySlope = nil
        end
        ...
        local function handleStep()
            ...
            local hitInfo
            dist = (1 + step) * stats.halfExtents.y
            ...
            if trace then table.insert(logStats, string.format("depth %d", depth)) end
            local slippery = false
            local slopeAngle = mH.angle(depth, prevDepth, dist, prevDist)
            if slopeAngle > MaxSlopeAngle then
                ...
                slipperySlope.depth = depth
                slipperySlope.hitInfo = hitInfo
            ...
            if not slippery then
                ...
                local riskFactor = getLandingDepthRiskFactor(depth, hitInfo)
                addRiskFactor(riskFactor, slopeAngle, step)
            end
            prevDepth = depth
            prevDist = dist
            return true
...
local function onJumpLanded(state)
    ...
    local jumpLength = mH.groundDist(pos, stats.startPos)
    local maxLength = stats.max.midairVelocity * jumpDuration
    local jumpHeight = state.skills.scaled.acrobatics.maxHeightPos.z - stats.startPos.z
    ...
    local expectedDamage = getFallLostHealth(state.skills.scaled.acrobatics.maxHeightPos.z - pos.z)
    local depthFactor = capFactor(getHealthRisk(math.max(0, riskDamage - expectedDamage)))
    local lastTimeFactor = capFactor(stats.timeSinceLastJump / state.skills.scaled.acrobatics.lastJumpMaxDuration) ^ 4
    local lengthFactor = capFactor(jumpLength / maxLength)
    local heightFactor = capFactor(jumpHeight / stats.max.height)
    ...
    local landingRiskFactor = landingRiskStillFactor * math.max(landingHeightFactor, lengthFactor) ^ 0.5
    local factor = capSkillScaledFactor(
            lastTimeFactor * math.max(
                    mCfg.jumpGainLandingRiskFactor * landingRiskFactor + mCfg.jumpGainBottomFactor * depthFactor,
                    mCfg.jumpGainMaxDistFactor) * math.max(lengthFactor, heightFactor),
            mS.getSkillGainScaledRange("acrobaticsSkillScalingRange"))

    log(string.format("Jumped:\n\tStats: since time %.2f/%.2f, landing risk still %.2f, depth max %.1f avg %.1f, damage risk %d expected %d, length %.1f/%.1f, height %.1f/%.1f, landed at %.1f"
            .. "\n\tFactors: last time %.3f x max( %s x landing risk %.2f + %s x depth %.3f, %s ) x max( length %.3f, height %.3f) ) = final %.3f"
            .. "\n\tGain scale: base %.3f x factor %.3f = %.3f",
    -- Stats
            stats.timeSinceLastJump, stats.max.duration,
            landingRiskStillFactor,
            stats.depth.max, avgDepth,
            riskDamage, expectedDamage,
            jumpLength, maxLength,
            jumpHeight, stats.max.height,
            landingHeight,
    -- Factors
            lastTimeFactor,
            mCfg.jumpGainLandingRiskFactor, landingRiskFactor,
            mCfg.jumpGainBottomFactor, depthFactor, mCfg.jumpGainMaxDistFactor, lengthFactor, heightFactor,
...
local function handleJumps(state, isOnGround, hasFlyingEffects)
    ...
    local finalDepth = depth * getDepthFactor(hitInfo)
...
module.skillUsedHandler = function(state, skillId, params)
    local factor = 1
    if mC.magickaSkills[skillId] then
        ...
        log(string.format("Magicka skills scaling: Cast the spell \"%s\" cost %d, chance = %.2f, factor = %.3f, gain %.3f -> %.3f",
                spell.name, spell.cost, chance, factor, params.skillGain, params.skillGain * factor))
    elseif mC.weaponSkills[skillId] and skillId ~= Skills.marksman.id then
        if not mS.skillUsesScaledStorage:get("weaponSkillScalingEnabled") then return end
        if not mC.isMeleeAttacking() then
        ...
        if enemy.type.baseType == T.Actor then
            ...
            log(string.format("Weapon skills scaling: Hit the enemy \"%s\" (attack %.2f, defense %.2f), chance = %.2f, factor = %.3f, gain %.3f -> %.3f",
    ...
    elseif skillId == Skills.security.id then
        ...
        if lockableStats.lockLevel then
            ...
            log(string.format("Security scaling: Unlocked a lock level %d, chance = %.2f, factor = %.3f, gain %.3f -> %.3f",
        ...
        elseif lockableStats.trapSpell then
            ...
            factor = capSkillScaledFactor(
                    ...
                    mS.getSkillGainScaledRange("securitySkillScalingRange"))
            log(string.format("Security scaling: Disarmed a trap with spell cost %d, chance = %.2f, factor = %.3f, gain %.2f -> %.2f",
                    lockableStats.trapSpell.cost, chance, factor, params.skillGain, params.skillGain * factor))
        end
    elseif mC.armorSkills[skillId] then
        ...
        log(string.format("Armor skills scaling: Hit by an enemy, health ratio = %.2f, factor = %.3f, gain %.3f -> %.3f",
    ...
    elseif skillId == Skills.acrobatics.id then
        ...
        if params.useType == I.SkillProgression.SKILL_USE_TYPES.Acrobatics_Fall then
            ...
            local damage = getFallLostHealth(state.skills.scaled.acrobatics.maxHeightPos.z - self.position.z, true)
            ...
            log(string.format("Acrobatics fall scaling: Took damage from a fall, damage = %.1f, health ratio = %.2f, factor = %.3f, gain %.3f -> %.3f",
    ...
    elseif skillId == Skills.athletics.id then
        ...
        log(string.format("Athletics scaling: Run duration %.2f time ratio %.2f velocity %.2f final %.2f, gain %.3f -> %.3f",
    ...
    elseif skillId == Skills.alchemy.id then
        ...
        log(string.format("Alchemy scaling: Created the %s potion from a recipe (%s) with %d positive effects, factor %.2f, gain %.3f -> %.3f",
