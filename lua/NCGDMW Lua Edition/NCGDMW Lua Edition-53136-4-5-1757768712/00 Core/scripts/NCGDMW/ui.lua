...
local function getStatsMenu(state)
    ...
    local dayDeathBlock = {
        hGap20,
        {
            ...
            content = ui.content {
                ...
                text(tostring(math.floor(mC.totalGameTimeInHours() / 24))),
            }
        }
    }
    ...
    for _, attrId in ipairs(orderedAttributeIds) do
        table.insert(leftBlock, row(
                mC.getStatName("attributes", attrId),
                textCells(
                        textCell(
                                menuCellWidth,
                        ...
                        textCell(
                                menuCellWidth,
                                tostring(T.Actor.stats.attributes[attrId](self).base
                                        - baseAttrMods[attrId] - state.attrs.start[attrId]),
                                nil,
                                tooltipEvent(L("growthAttributeValue"))),
                        textCell(
                                menuCellWidth,
                                tostring(state.attrs.start[attrId]),
                                nil,
    ...
    local maxHealthMod = mC.getMaxHealthModifier()
    ...
    local skillRows = getSkillsRows(state, getSkillsHeader, function(skillId)
        local base = T.NPC.stats.skills[skillId](self).base
        local decayPercent = state.skills.decay[skillId] / mCfg.decayTimeBaseInHours
        return row(
                mC.getStatName("skills", skillId),
                textCells(
                        textCell(
