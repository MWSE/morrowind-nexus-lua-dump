...
if not (core.getGMST("fLevelUpHealthEndMult") == 0.0
        ...
        and core.getGMST("fMiscSkillBonus") == 1.0) then
...
local function recalculateDecayMemory()
    ...
    local decayRate = I.NCGDMW.DecayRate()
...
local function getAttributesToRecalculate()
    ...
    for id, getter in pairs(Player.stats.skills) do
        ...
        if baseSkills[id] ~= stat.base then
            baseSkills[id] = stat.base
        ...
        if stat.base > baseSkills[id] then
        ...
        if stat.base > maxSkills[id] then
...
local function doAttributes()
    local decayRate = I.NCGDMW.DecayRate()
    local growthRate = I.NCGDMW.GrowthRate()
    ...
    if recalculateLuck then
        ...
        total = math.floor(math.sqrt(totalStats * growthRate / 27) + startAttributes.luck)
...
local function onKeyPress(key)
    ...
    if key.code == playerStorage:get("statsMenuKey") then
        local menu
        if I.NCGDMW.DecayRate() > none then
            menu = ncgdUI.decayStatsMenu(decaySkills, decayMemory, playerStorage:get("starwindNames"))
...
local interface = {
    ...
    DecayRate = function(val, asStr)
        if val ~= nil and rateMap[val] == nil then
            print("Interface/DecayRate(): Invalid rate value given")
            return
        elseif val ~= nil then
            playerStorage:set("decayRate", rateMap[val])
        end
        if asStr then
            return L(playerStorage:get("decayRate"))
        else
            return rateMap[L(playerStorage:get("decayRate"))]
        end
    end,

    GrowthRate = function(val, asStr)
        if val ~= nil and rateMap[val] == nil then
            print("Interface/GrowthRate(): Invalid rate value given")
            return
        elseif val ~= nil then
            playerStorage:set("growthRate", rateMap[val])
        end
        if asStr then
            return L(playerStorage:get("growthRate"))
        else
