...
local function skillUsedHandlerMultiSchool(skillId, params)
    ...
    for school, ratio in pairs(spellSchoolRatios[spell.id]) do
        if ratio < 1 then
            C.debugPrint(string.format("Magicka skill \"%s\" increase, base gain = %.5f, multi-school ratio = %.2f, final gain = %.5f",
            ...
            if skillId == school then
                ...
                self:sendEvent("ncgd_applySkillUsedHandlers", { skillId = school, params = { skillGain = gain }, afterHandler = "multiSchool" })
            end
        end
    end
end
...
local skillUsedHandlers = {
    ...
    { name = "mbsp", handler = mbsp.skillUsedHandler },
...
local function addSkillUsedHandlers()
    ...
    I.SkillProgression.addSkillLevelUpHandler(function(skillId, source)
        ...
        self:sendEvent("ncgd_updateProfile")
    end)
end
...
return {
    ...
    eventHandlers = {
        ...
        ncgd_applySkillUsedHandlers = function(data) applySkillUsedHandlers(data.skillId, data.params, data.afterHandler) end,
        ncgd_updateProfile = function()
            updateProfileAsked = true
        end,
        ncgd_updateProfileOnUpdate = function()
            updateProfileOnUpdateAsked = true
        end,
        ncgd_showStatsMenu = showStatsMenu,
        ncgd_refreshDecay = function()
            if C.hasStats() then
                decay.logDecayTime()
            end
        end,
