...
local function skillUsedHandler(skillId, params)
    ...
    C.debugPrint(string.format("MBSP: Magicka skill \"%s\" increase, base gain = %.5f, cost = %d, XP rate = %d, final gain = %.5f",
...
local function onFrame(deltaTime)
    ...
    if S.mbspStorage:get("mbspEnabled") then
        ...
        if storedMagicka - dynamic.magicka(self).current >= 0.5 then
