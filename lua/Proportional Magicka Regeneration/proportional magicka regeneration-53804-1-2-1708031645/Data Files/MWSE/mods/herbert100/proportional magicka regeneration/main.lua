...
local function player_regen_tick()
    ...
    if player.inCombat then
     ...
     local td = tes3.player.data
     if td.herbert_fmr then
        reg = reg * td.herbert_fmr
        if log_level > 1 then
            log:debug("player has fortify magicka regen coefficient of %s ", td.herbert_fmr)
        end
     end
     if td.herbert_dmr then
        reg = reg * td.herbert_dmr
        if log_level > 1 then
            log:debug("player has drain magicka regen coefficient of %s ", td.herbert_dmr)
        end
     end
    ...
    for npc_ref in tes3.getPlayerCell():iterateReferences{tes3.objectType.npc} do
        ...
        if npc  then
            ...
            if mc < mb then
                ...
                local td = npc_ref.data
                if td.herbert_fmr then reg = reg * td.herbert_fmr end
                if td.herbert_dmr then reg = reg * td.herbert_dmr end
