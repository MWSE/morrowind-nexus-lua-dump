...
local function handleHit(groupname, key)
	local ray = getRayResult()
	if not ray.hitObject then
		return
	end
	if not S_SWING_MINING then return end
	
	local hitItem = nodeToItemLookup[ray.hitObject.recordId]
	if not hitItem or types.Container.content(ray.hitObject):isResolved() then
		return
	end

	if not types.Container.objectIsInstance(ray.hitObject) then
		return
	end
