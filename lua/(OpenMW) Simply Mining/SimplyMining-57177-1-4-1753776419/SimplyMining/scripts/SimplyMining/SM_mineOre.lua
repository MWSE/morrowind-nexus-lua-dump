...
local function updateCraftingProgress(dt)
	...
	if currentFxStep > craftingState.lastFxStep then
		...
		if res.hitObject and res.hitObject == craftingState.target  then
			...
			ambient.playSound("Heavy Armor Hit")
		else
			ambient.playSound("SwishL")
...
return function(data)
	...
	if item then
		...
		craftingState.itemName = craftingState.itemName.." ("..(craftingState.isVanillaOre and 0.5 or f1(chance*2))..")"
	else
		return
	end
	local duration = 3.75
