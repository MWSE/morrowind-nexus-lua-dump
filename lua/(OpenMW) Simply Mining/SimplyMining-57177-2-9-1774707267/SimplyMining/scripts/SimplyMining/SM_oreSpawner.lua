...
local COLLISION_MASK = util.bitAnd(nearby.COLLISION_TYPE.AnyPhysical, util.bitNot(nearby.COLLISION_TYPE.Water))
...
local function updateActiveCells()
	...
	for dx = -1, 1 do
		for dy = -1, 1 do
			...
			if (saveData.cellOreCount[targetCellID] or 0) >= (saveData.cellCapacity[targetCellID] or 1) or (saveData.cellFailCount[targetCellID] or 0) >= MAX_EXTERIOR_FAILS then
				-- already full
			else
...
local function getObjectFootprint(obj)
local box =obj:getBoundingBox()
--print(obj, box.halfSize.x, box.halfSize.y, box.halfSize.z)
return box.halfSize.x * box.halfSize.y
end
...
local function isValidSurface(hitObject) --interior
	return (
		hitObject 
		and (
...
local function trySpawnInteriorOre(cellID)
	...
	if not saveData.cellOreCount[cellID] or framesInCell == 4 and saveData.retryCells[cellID] then
		saveData.retryCells[cellID] = false
		local area = calculateActorSurfaceArea()^0.5
		maxOres = 1+area^0.9/600*S_INTERIOR_MULT/100
		if math.random() < maxOres % 1 then
			maxOres = maxOres + 1
		end
		saveData.cellCapacity[cellID] = math.floor(maxOres)
		saveData.cellOreCount[cellID] = 0
		saveData.cellFailCount[cellID] = 0
		print(math.floor(area).." m2, max "..maxOres.." ores", #nearby.actors.." actors")
		fail1 = 0
		fail2 = 0
		fail3 = 0
		fail4 = 0
	...
	if saveData.cellOreCount[cellID] >= saveData.cellCapacity[cellID] or saveData.cellFailCount[cellID] >= MAX_INTERIOR_FAILS then
		if saveData.cellOreCount[cellID] == saveData.cellCapacity[cellID] or saveData.cellFailCount[cellID] == MAX_INTERIOR_FAILS then
			print(cellID..": spawned "..saveData.cellOreCount[cellID].." ores ("..saveData.cellFailCount[cellID].." fails:"..fail1..","..fail2..","..fail3..","..fail4..")")
			if saveData.cellFailCount[cellID] == MAX_INTERIOR_FAILS and fail2 == 0 and fail3 == 0 and fail4 == 0 then
				print("buggy navmesh, retrying next time")
				saveData.retryCells[cellID] = true
			end
			saveData.cellOreCount[cellID] = saveData.cellCapacity[cellID] + 1
	...
	local randomPoint = nearby.findRandomPointAroundCircle(randomActor.position, 3000, {})
	if not randomPoint then
		saveData.cellFailCount[cellID] = saveData.cellFailCount[cellID] + 1
		fail1 = fail1 + #nearbyActors
		return true
	end
...
local function trySpawnOreInCell(cellID)
	if not saveData.cellOreCount[cellID] then
		saveData.cellOreCount[cellID] = 0
		saveData.cellFailCount[cellID] = 0
		local maxOres = S_EXTERIOR_NODES
		if math.random() < maxOres%1 then
			maxOres = maxOres + 1
		end
		saveData.cellCapacity[cellID] = math.floor(maxOres)
	...
	if saveData.cellOreCount[cellID] >= saveData.cellCapacity[cellID] or saveData.cellFailCount[cellID] >= MAX_EXTERIOR_FAILS then
		if saveData.cellOreCount[cellID] == saveData.cellCapacity[cellID] or saveData.cellFailCount[cellID] == MAX_EXTERIOR_FAILS then
			--print(cellID..": spawned "..saveData.cellOreCount[cellID].." ores ("..saveData.cellFailCount[cellID].." fails:"..fail1..","..fail2..")")
			--print(cellID..": spawned "..saveData.cellOreCount[cellID].." ores ("..saveData.cellFailCount[cellID].." fails)")
			saveData.cellOreCount[cellID] = saveData.cellCapacity[cellID] + 1
			saveData.cellFailCount[cellID] = MAX_EXTERIOR_FAILS + 1
		end
		return false
	end
	...
	local res = nearby.castRay(
		util.vector3(rndX, rndY, playerZ + 3500),
		util.vector3(rndX, rndY, playerZ - 4500),
		{ radius = 300, collisionType = COLLISION_MASK}
	)
	
	if res.hit and (not res.hitObject or res.hitObject.recordId:find("rock")) then
		
		--if math.abs(res.hitPos.z -waterLevel) < 10 and not res.hitObject then -- check underwater too
		--	print("probably underwater")
		--	res = nearby.castRay(
		--		util.vector3(rndX, rndY, res.hitPos.z + 1000),
		--		util.vector3(rndX, rndY, res.hitPos.z - 4000),
		--		{ radius = 30, collisionType = COLLISION_MASK}
		--	)
		--end
		local first=res.hitNormal
		...
		if res.hit and (not res.hitObject or res.hitObject.recordId:find("rock") and not nodeToItemLookup[res.hitObject.recordId] and getObjectFootprint(res.hitObject) > 100000 ) and math.deg(math.acos(first:dot(res.hitNormal))) < 25 then
			-- road check (terrain hits only)
			if canQueryTextures and not res.hitObject then
				local texture = core.land.getTextureAt(res.hitPos, self.cell)
				if texture and roadTextures[texture] then
					saveData.cellFailCount[cellID] = saveData.cellFailCount[cellID] + 1
					return true
				end
			end
...
table.insert(G_onFrameFunctions, function()
	framesInCell = framesInCell + 1
	if self.cell then
		if not S_UNINSTALL then
			if not lastCell then
				...
				if not saveData.nerfedCells[self.cell.id] then
					saveData.nerfedCells[self.cell.id] = true
					core.sendGlobalEvent("SimplyMining_nerfLoot", {self, S_ORE_LOOT/100})
				end
			...
			elseif framesInCell > 3 and not G_blockedBySunsDusk then
				-- interior logic
