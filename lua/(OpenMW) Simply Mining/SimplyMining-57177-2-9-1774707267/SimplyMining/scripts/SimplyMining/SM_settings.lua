...
settingsTemplate[tempKey] = {
	...
	settings = {
		...
		{
			key = "ASSISTED_MINING",
			name = L("ASSISTED_MINING.name", "Assisted Mining"),
			description = L("ASSISTED_MINING.desc", "Automatically target nearby ore when swinging in third person\nYour character snaps to face the closest ore in front of you\nDirect aiming always takes priority\nRequires Swing-Based Mining and a free-camera mod (e.g. Move360Combat)"),
			renderer = "checkbox",
			default = false,
		},
		{
...
settingsTemplate[tempKey] = {
	...
	settings = {
		...
		{
			key = "SPAWN_COPPER",
			name = L("SPAWN_COPPER.name", "Spawn Copper"),
			description = L("SPAWN_COPPER.desc", "Include copper ore in spawned veins\nDisabled by default because most recipes don't use copper"),
			renderer = "checkbox",
			default = false,
		},
		{
...
local restockingSection = storage.globalSection('SettingsGlobal'..MODNAME..'Restocking')

if S_USE_TRANSLATIONS ~= nil then
	core.sendGlobalEvent("SimplyMining_syncSetting", {"USE_TRANSLATIONS", S_USE_TRANSLATIONS})
end
core.sendGlobalEvent("SimplyMining_syncSetting", {"SPAWN_COPPER", S_SPAWN_COPPER})
...
for _, template in pairs(settingsTemplate) do
	...
	section:subscribe(async:callback(function(_, setting)
		...
		if setting == "USE_TRANSLATIONS" then
			core.sendGlobalEvent("SimplyMining_syncSetting", {"USE_TRANSLATIONS", S_USE_TRANSLATIONS})
		elseif setting == "SPAWN_COPPER" then
			core.sendGlobalEvent("SimplyMining_syncSetting", {"SPAWN_COPPER", S_SPAWN_COPPER})
