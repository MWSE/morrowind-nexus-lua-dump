...
local function updateCraftingProgress(dt)
	...
	if currentFxStep > craftingState.lastFxStep then
		...
		if res.hitObject and res.hitObject == craftingState.target  then
			...
			core.sendGlobalEvent('SimplyMining_setNodeSize', {
				craftingState.target, 
				1-currentFxStep*0.02*craftingState.speed, 
				(craftingState.elapsedTime - (oreProgressData[craftingState.target.id] or 0)) / craftingState.duration,
				craftingState.elapsedTime / craftingState.duration,
				S_USE_MINING_SKILL and G_skillRegistered and MINING_SKILL_ID or "armorer",
				getSkillLevel()
			})
			oreProgressData[craftingState.target.id] = craftingState.elapsedTime
			if craftingState.noTool then
				...
				ambient.playSoundFile("sound/simplymining/Action_Mining_Metal_01_0"..math.random(1,4)..".ogg", {volume =S_VOLUME/100})
			end
		else
			ambient.playSound("SwishL", {volume =S_VOLUME/100})
	...
	if craftingState.elapsedTime >= craftingState.duration then
		oreProgressData[craftingState.target.id] = nil
		if not res then
			local cameraPos = camera.getPosition()
			local iMaxActivateDist = core.getGMST("iMaxActivateDist")+0.1
			local activationDistance = iMaxActivateDist + camera.getThirdPersonDistance();
...
if G_onFrameFunctions then
	table.insert(G_onFrameFunctions, updateCraftingProgress)
end
...
return function(data)
	...
	if item then
		...
		craftingState.itemName = craftingState.itemName.." ("..(craftingState.isVanillaOre and 0.7*S_YIELD_MULT/100 or f1(chance*2))..")"
		if craftingState.isVanillaOre and isTheft(craftingState.target) then
			color = THEFT_COLOR
		end
	else
		return
	...
	craftingState.elapsedTime = oreProgressData[target.id] or 0
