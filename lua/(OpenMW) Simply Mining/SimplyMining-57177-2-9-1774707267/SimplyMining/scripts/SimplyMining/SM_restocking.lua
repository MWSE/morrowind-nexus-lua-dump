...
local restockSettings = {
	...
	settings = {
		...
		{
			key = "RESTOCK_LEVEL_BIAS",
			name = L("RESTOCK_LEVEL_BIAS.name", "Level Bias (%)"),
			description = L("RESTOCK_LEVEL_BIAS.desc", "How much your mining level affects the types of ores that are restocked\n0 = Disabled\n100 = Only restock ores around your level and gate ores that are way above your skill"),
			renderer = RENDERER_SLIDER,
			default = 100,
			argument = {
				min = 0,
...
--S_SPAWN_COPPER updated on init (before) and when settings get synced
...
local function skillFactor(playerSkill, oreDifficulty)
	...
	local ret = 0.01 -- gap > 23
	if gap > 0 then
		ret = 0.01 + (1 - gap / 23) ^ 2
	else
		ret = math.max(0.35, 1 + gap / 60)
	end
	local biasMult = S_RESTOCK_LEVEL_BIAS/100
	return biasMult * ret + (1-biasMult) * 1
end
...
local function activateNPC_ores(npc, player)
	...
	if toAdd > 0 then
		...
		for j = 1, #oreTable do
			...
			-- skip copper if disabled
			if ore.ingredId == "t_ingmine_orecopper_01" and not S_SPAWN_COPPER then
				-- excluded
			else
