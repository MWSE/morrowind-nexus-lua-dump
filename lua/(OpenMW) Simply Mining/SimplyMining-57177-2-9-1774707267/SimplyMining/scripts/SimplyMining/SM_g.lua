...
local internalSection = storage.globalSection('SimplyMiningInternal')
S_SPAWN_COPPER = internalSection:get("SPAWN_COPPER")
...
local function setNodeSize(data)
    local object = data[1]
    local size = data[2] -- fraction 0-1
    -- NEW (only if players have updated)
    local progressed = data[3] -- fraction 0-1
    local currentProgress = data[4] -- fraction 0-1
    local usedSkill = data[5] -- "armorer" (vanilla) or "mining_skill" (via skill framework)
    local skillLevel = data[6] -- skill level
...
local function syncSetting(data)
	internalSection:set(data[1], data[2])
	S_SPAWN_COPPER = internalSection:get("SPAWN_COPPER")
end
...
return {
	...
	eventHandlers = { 
		...
		SimplyMining_syncSetting = syncSetting,
	}
}