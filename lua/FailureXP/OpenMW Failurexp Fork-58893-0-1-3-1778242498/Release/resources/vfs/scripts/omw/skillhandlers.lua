...
local function computeSkillGain(skillid, options)
...
local function skillUsed(skillid, options)
    ...
    options = computeSkillGain(skillid, options)
    if not options then return end

    auxUtil.callEventHandlers(skillUsedHandlers, skillid, options)
end

-- Iterates the failure handlers in reverse insertion order (matching the
-- iteration order used by aux_util.callEventHandlers for other event lists).
-- See addSkillUsedFailedHandler for the return-value contract. Returns true
-- if any handler asked the engine to suppress its vanilla messageBox.
local function dispatchFailureHandlers(skillid, options)
    local consume = false
    if not skillUsedFailedHandlers then return false end
    for i = #skillUsedFailedHandlers, 1, -1 do
        local ret = skillUsedFailedHandlers[i](skillid, options)
        if ret == false then
            return consume
        elseif ret == true then
            consume = true
        elseif type(ret) == 'table' then
            if ret.consumeVanillaMessage then consume = true end
            if ret.stopPropagation then return consume end
        end
    end
    return consume
end
...
local function skillUsedFailed(skillid, options)
    ...
    return dispatchFailureHandlers(skillid, options)
end
...
return {
    ...
    interface = {
        ...
        --- Add new skillUsedFailed handler for this actor.
        -- Fires when an action that would normally grant skill XP on success has failed
        -- (e.g. a missed weapon swing, failed lockpick, fizzled spell, detected pickpocket).
        -- For load order consistency, handlers should be added in the body of your script.
        -- `options` is a modifiable table containing `skillGain` (the gain a successful use of
        -- the same useType would have produced) plus a shallow copy of the options passed to
        -- @{#SkillProgression.skillUsedFailed}. By default no XP is applied; handlers may grant
        -- a fraction of `skillGain` by routing through @{#SkillProgression.skillUsed}.
        --
        -- The return value of `handler(skillid, options)` is interpreted as follows:
        --
        --   * `false` -> stop propagation (no further handlers run).
        --   * `true`  -> ask the engine to suppress its own vanilla "you failed"
        --                messageBox for this failure (e.g. sRepairFailed, sNotifyMessage9).
        --                Use this when your mod is taking over the failure UI.
        --   * `{ consumeVanillaMessage = bool, stopPropagation = bool }` -> explicit
        --                form when you need to combine signals.
        --   * any other value (including nil) -> neutral; continue propagation
        --                without suppressing the vanilla message.
        --
        -- The "suppress vanilla messageBox" signal is only honored for the player.
        -- For non-player actors the dispatch is asynchronous and the engine has
        -- already moved past the messageBox call site by the time handlers run.
        ...
        -- Same parameter shape as @{#SkillProgression.skillUsed}.
        -- Returns true if any handler asked the engine to suppress its vanilla
        -- "you failed" messageBox for this failure.
        -- @function [parent=#SkillProgression] skillUsedFailed
        skillUsedFailed = skillUsedFailed,

        -- Internal entry point: invoked synchronously by the engine via
        -- callPlayerInterface so the C++ failure paths can learn whether to
        -- suppress their own vanilla messageBox before showing it. Not part of
        -- the public interface; mods should use addSkillUsedFailedHandler.
        _dispatchSkillUseFailedSync = function(skillid, useType, scale)
            return skillUsedFailed(skillid, { useType = useType, scale = scale })
        end,
    ...
    engineHandlers = {
        ...
        -- For non-player actors this flows through the async event queue.
        -- For the player, MWLua::LuaManager::skillUseFailed bypasses the queue
        -- and calls _dispatchSkillUseFailedSync directly so the consume vote
        -- can be returned to C++ before the vanilla messageBox would fire.
