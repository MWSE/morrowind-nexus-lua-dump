...
local maxTargetDistance = 30 * 69
local targetSwitchCooldown = 0.3
local targetSwitchMouseVel = 2

local LOSCheckFailTime = 3
local LOSCheckTimer = 0

local lockflipflop = true
...
local function onFrame(dt)
    ...
    if cameraYaw ~= cameraYaw or cameraPitch ~= cameraPitch or cameraRoll ~= cameraRoll then
        error("NaN detected in camera state")
    end
    ...
    if not isInUI and #badAnimations == 0 then
        ...
        if CamLockTarget then
            ...
            if not isFirstPerson and lockPos then
                lockPos = lockPos + util.vector3(0, 0, 30 + (actorLockPos(CamLockTarget) - camPos):length() * -0.2)
            end
            ...
            newCameraYaw = slerpAngle(cameraYaw, targetYaw, gutils.dtForLerp(dt, 6))
            newCameraPitch = slerpAngle(cameraPitch, targetPitch, gutils.dtForLerp(dt, 6))

            if I.DynamicReticle then
                if isFirstPerson and lockPos then
                    I.DynamicReticle.setReticleWorldPos(lockPos)
                else
                    I.DynamicReticle.setReticleWorldPos(lockPos + util.vector3(0, 0, -40 +(actorLockPos(CamLockTarget) - camPos):length() * 0.2))
                end
            ...
            --omwself.controls.pitchChange = camera.getPitch() * math.cos(yawDelta) - self.rotation:getPitch()
            local centerPos = util.vector2(0.5, 0.5)
            --camera.instantTransition()
            if I.DynamicReticle then

                --I.DynamicReticle.setCurrentEnemy(nil)
            end
        ...
        if CamLockTarget or isFirstPerson then
            ...
            if isFirstPerson then
                if CamLockTarget then
                    local camPos = camera.getPosition()
                    local lockPos = actorLockPos(CamLockTarget) + util.vector3(0, 0, 10)
                    local toTarget = (lockPos - camPos):normalize()
                    local targetYaw = math.atan2(toTarget.x, toTarget.y)
                    local targetPitch = -math.asin(toTarget.z)
                    omwself.controls.yawChange = (util.normalizeAngle((targetYaw - omwself.rotation:getYaw()) - (omwself.controls.sideMovement/(lockPos-camPos):length()) * 0.175)) / .05 * dt
                    omwself.controls.pitchChange = (targetPitch - omwself.rotation:getPitch()) / .05 * dt
                else
                    omwself.controls.yawChange = util.normalizeAngle(deltaModelYaw) * 20 * dt
