...
local function getWeaponType(weapon)
    ...
    common.log("  Type: %s | Not Supported", weapon.typeName)
    return nil
...
local function rebalanceWeapon(weapon)
    ...
    local value = common.getItemValue(
        weapon, gameConfig.weapon.enchantedItem,
        unenchantedValue, unmodifiedWeapon.value)
    ...
    if value < 1 and not isBoundItem then value = 1 end
...
local function cacheWeapon(weapon)
    ...
    if not common.shouldCacheItem(weapon) then return end
...
this.onItemLoaded = function(e)
    ...
    if not gameConfig.weapon.rebalanceEnabled then return end
    if not isWeaponOrAmmunition(e.item) then return end
    
    local baseItem = common.findBaseItem(e.item)
    if baseItem == nil then return end
    
    --------------------------------------------------
    
    common.log("  Chop: %d-%d -> %d-%d", e.item.chopMin, e.item.chopMax, baseItem.chopMin, baseItem.chopMax)
    common.log("  Slash: %d-%d -> %d-%d", e.item.slashMin, e.item.slashMax, baseItem.slashMin, baseItem.slashMax)
    common.log("  Thrust: %d-%d -> %d-%d", e.item.thrustMin, e.item.thrustMax, baseItem.thrustMin, baseItem.thrustMax)
    
    common.log("  Speed: %.2f -> %.2f", e.item.speed, baseItem.speed)
    common.log("  Reach: %.2f -> %.2f", e.item.reach, baseItem.reach)
    common.log("  Weight: %.2f -> %.2f", e.item.weight, baseItem.weight)
    common.log("  Health: %d -> %d", e.item.maxCondition, baseItem.maxCondition)
    
    common.log("  Silver Weapon: %s -> %s",
        tostring(e.item.isSilver),
        tostring(baseItem.isSilver))
    
    common.log("  Ignores Normal Weapon Resistance: %s -> %s",
        tostring(e.item.ignoresNormalWeaponResistance),
        tostring(baseItem.ignoresNormalWeaponResistance))
    
    --------------------------------------------------
    
    e.item.chopMax = baseItem.chopMax
    e.item.slashMax = baseItem.slashMax
    e.item.thrustMax = baseItem.thrustMax
    
    e.item.chopMin = baseItem.chopMin
    e.item.slashMin = baseItem.slashMin
    e.item.thrustMin = baseItem.thrustMin
    
    e.item.speed = baseItem.speed
    e.item.reach = baseItem.reach
    e.item.weight = baseItem.weight
    e.item.maxCondition = baseItem.maxCondition
    
    e.item.isSilver = baseItem.isSilver
    e.item.ignoresNormalWeaponResistance = baseItem.ignoresNormalWeaponResistance
