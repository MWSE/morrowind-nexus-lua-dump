...
local function getSlot(clothing)
    ...
    common.log("  Slot: %s | Not Supported", clothing.slotName)
    return nil
...
local function rebalanceClothing(clothing)
    ...
    local value = common.getItemValue(
        clothing, gameConfig.clothing.enchantedItem,
        unenchantedValue, unmodifiedClothing.value)
...
local function cacheClothing(clothing)
    ...
    if not common.shouldCacheItem(clothing) then return end
...
this.onItemLoaded = function(e)
    ...
    if e.item.objectType ~= tes3.objectType.clothing then return end
