...
local version = config.getDefaultMcmConfig().version
...
local function getModNameAndVersion()
    return string.format("BYO Rebalance v%.1f", version)
end
...
local function getEffectCost(enchantment)
    
    if enchantment.castType ~= 3 then return 0 end
    
    local totalEffectCost = 0
    
    for _, effect in ipairs(enchantment.effects) do
        
        if effect.id >= 0
        and not effect.object.isHarmful then
            
            -- effectCost is the amount of the enchantCapacity that is filled
            -- this is 100x the effectCost of a "Cast When Used" Touch effect
            -- this calculation does not take effect.radius into consideration
            
            local effectCost = effect.object.baseMagickaCost * (effect.min + effect.max) * 2.5
            totalEffectCost = totalEffectCost + effectCost
            
        end
        
    end
    
    return totalEffectCost
    
end

this.logEnchant = function(item, newValue)
    
    if item.enchantment then
        
        log("  Enchant: Enchanted")
        local effectCost = getEffectCost(item.enchantment)
        
        if effectCost > 0 then -- "Constant Effect"
            log("  Enchant Effect Cost: %d", effectCost)
            
        elseif item.isProjectile == true then -- Arrow, Bolt, Thrown
            log("  Enchant Charge Cost: %d", item.enchantment.chargeCost)
            
        else -- non-projectile "Cast When Strikes" or "Cast When Used"
            log("  Enchant Max Charge: %d", item.enchantment.maxCharge)
        end
        
    elseif newValue == nil then 
        log("  Enchant: %.1f", item.enchantCapacity * 0.1)
        
    else -- not enchanted and was rebalanced
        log("  Enchant: %.1f -> %.1f", item.enchantCapacity * 0.1, newValue * 0.1)
    end
...
local function shouldCacheItem(item)
    
    if item.name == "" then return false end
    if item.name == "FOR SPELL CASTING" then return false end
    if item.sourceMod == nil then return false end
    return true
    
end

this.shouldCacheItem = function(item)
    return shouldCacheItem(item)
end

local function isCustomEnchantedItem(item)
    
    if item.sourceMod ~= nil then return false end
    if item.enchantment == nil then return false end
    if item.enchantment.sourceMod ~= nil then return false end
    return true
    
end

this.findBaseItem = function(item)
    
    if not gameConfig.shared.rebalanceCustomEnchantedItems then return nil end
    if not isCustomEnchantedItem(item) then return nil end
    
    for baseItem in sortedIterateObjects(item.objectType) do
        
        if shouldCacheItem(baseItem)
        and baseItem.enchantment == nil
        and baseItem.type == item.type
        and baseItem.mesh == item.mesh
        and baseItem.icon == item.icon then
            
            log("Item ID: %s | Name: %s | Rebalance Custom-Enchanted Items", item.id, item.name)
            log("  Base Item ID: %s", baseItem.id)
            log("  Base Item Name: %s", baseItem.name)
...
this.getItemValue = function(item, eiConfig, unenchantedValue, vanillaValue)
    
    if item.enchantment == nil then
        return unenchantedValue
    end
    
    if not eiConfig.recalculateValue then
        return vanillaValue * eiConfig.valueScale
    end
    
    local value = eiConfig.valueBase + unenchantedValue * eiConfig.valueMult
    local effectCost = getEffectCost(item.enchantment)
    
    if effectCost > 0 then -- "Constant Effect"
        return value + effectCost * eiConfig.valuePerEffectCost
        
    elseif item.isProjectile == true then -- Arrow, Bolt, Thrown
        return value + item.enchantment.chargeCost * eiConfig.valuePerMaxCharge * 0.5
        
    else -- non-projectile "Cast When Strikes" or "Cast When Used"
        return value + item.enchantment.maxCharge * eiConfig.valuePerMaxCharge
    end
...
this.getIsBoundItem = function(object)
    ...
    if gameConfig.shared.boundItemIds[object.id] then
        ...
        log("  Bound Item: true | In \"Bound Items\" List")
        return true
    ...
    and string.find(object.name, gameConfig.shared.boundItemSearchPattern) ~= nil then
        ...
        log("  Bound Item: true | Detect by Name")
        return true
        
    end
    
    log("  Bound Item: false")
    return false
