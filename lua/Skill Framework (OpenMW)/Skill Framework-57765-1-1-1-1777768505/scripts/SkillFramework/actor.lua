...
local function initStatsWindowIntegration()
    if I.StatsWindow then
        ...
        local lineBuilder = function(skillId)
            ...
            return {
                ...
                visibleFn = function()
                    if type(skillRecord.modIntegration.statsWindow.visible) == 'function' then
                        return skillRecord.modIntegration.statsWindow.visible()
                    else
                        return skillRecord.modIntegration.statsWindow.visible
                    end
                end,
                onClick = skillRecord.modIntegration.statsWindow.onClick,
            }
        end
            ...
            builder = function()
                ...
                if configPlayer.options.b_ShowSubsections then
                    for id, rec in pairs(records) do
                        if rec.modIntegration.statsWindow.subsection ~= nil then
                            hasSubsection = true
                            break
                        end
                    end
                ...
                local groups = {}
                for id, rec in pairs(records) do
                    local subsectionName = rec.modIntegration.statsWindow.subsection or API.interface.STATS_WINDOW_SUBSECTIONS.Misc
                ...
                for _, name in ipairs(names) do
                    ...
                    for _, id in ipairs(info.ids) do
                        ...
                        if records[id].modIntegration.statsWindow.shortenedName then
                            I.StatsWindow.modifyLine(id, {
                                label = records[id].modIntegration.statsWindow.shortenedName,
                            })
                        end
                    end
                end
    ...
    elseif types.Player.objectIsInstance(self) then
        print('[SkillFramework] Stats Window Extender not found; skipping integration.')
    end
end
