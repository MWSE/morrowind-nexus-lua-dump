...
local function verifySkillStat(id)
    ...
    local stat = personalStats.skills[id]
    if not stat then
        stat = {}
...
function API.registerSkill(id, props)
    ...
    props.modIntegration = props.modIntegration or {}
    props.modIntegration.statsWindow = props.modIntegration.statsWindow or props.statsWindowProps or {} -- backwards compatibility
    props.modIntegration.statsWindow.visible = props.modIntegration.statsWindow.visible == nil and true or props.modIntegration.statsWindow.visible
...
--- 
--- #### Warning
--- Keep in mind that your callback will be called every frame on every actor it is registered for.
--- Try to avoid expensive operations in the callback and cache results, and unless absolutely necessary, only register it for the player:
--- ```lua
...
local skillStatCache = {}

local function createSkillStatProxy(id)
...
function API.getSkillStat(id)
    ...
    skillStatCache[id] = skillStatCache[id] or createSkillStatProxy(id)
    return skillStatCache[id]
end
...
local function onUpdate()
    if next(_pendingSkillStatChanges) then
    ...
    if next(dynamicModifiers) then
        personalStats.modifierStates = personalStats.modifierStates or {}

        for skillId, callbacks in pairs(dynamicModifiers) do
            if not customSkills[skillId] then goto continue end
            
            local stat = skillStatCache[skillId] or API.getSkillStat(skillId)
    ...
    if next(personalStats.modifierStates) then
