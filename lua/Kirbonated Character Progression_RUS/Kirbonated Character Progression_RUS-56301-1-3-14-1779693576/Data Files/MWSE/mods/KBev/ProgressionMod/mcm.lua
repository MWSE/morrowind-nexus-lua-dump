...
local function registerModConfig(e)
	...
	xpFeatures:createOnOffButton{
		label = "Включить получение опыта за убийство обычных врагов?",
		description = "(По умолчанию: ДА) Если включено, то за победу над обычными врагами вы получите очки опыта.",
		variable = mwse.mcm.createTableVariable{
			id = "enemyXPEnabled",
			table = config,
			defaultSetting = true
	...
	xpFeatures:createOnOffButton{
		label = "Ограничить получение опыта только при чтении книг главных и важных навыков?",
		description = "(По умолчанию: НЕТ) Если включить, то за чтение книг маловажных навыков вы не будете получать опыт.",
		variable = mwse.mcm.createTableVariable{
			id = "limitskillBookXPEnabled",
			table = config,
			defaultSetting = false
		}
	}
	xpFeatures:createOnOffButton{
		label = "Ограничить получение опыта только за обучение у тренеров главным и важным навыкам?",
		description = "(По умолчанию: НЕТ) Если включить, то за обучение у тренеров маловажных навыкам вы не будете получать опыт.",
		variable = mwse.mcm.createTableVariable{
			id = "limittrainerXPEnabled",
			table = config,
			defaultSetting = false
		}
	}
	...
	xpRewards:createTextField{
		label = "Опыт за поверженных врагов",
		description = "(По умолчанию: 5) Задает количество опыта за поверженных врагов. Значение устанавливается для тех, чей уровень ниже вашего. Для тех, чей уровень равен вашему либо выше, опыт увеличится в два раза.(Требуется включить получение опыта за убийство обычных врагов).",
		numbersOnly = true,
		variable = mwse.mcm.createTableVariable{
			id = "enemyXP",
			table = config,
