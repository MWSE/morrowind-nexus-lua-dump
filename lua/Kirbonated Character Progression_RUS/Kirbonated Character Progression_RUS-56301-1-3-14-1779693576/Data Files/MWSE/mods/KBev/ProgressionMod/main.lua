...
local function onSkillRaised(e)
	...
	if (e.source == tes3.skillRaiseSource.book) and (mcm.skillBookXPEnabled) then
		local skillTypes = tes3.mobilePlayer.skills[e.skill + 1]
		if (mcm.limitskillBookXPEnabled) then
			if skillTypes and (skillTypes.type == tes3.skillType.major or skillTypes.type == tes3.skillType.minor) then
				giveXP({message = "Чтение книги навыка", amount = mcm.bkSklXP})
			else
				tes3.messageBox("Повышение маловажных навыков не дает вам опыта")
			end
		else
			giveXP({message = "Чтение книги навыка", amount = mcm.bkSklXP})
		end
	...
	if (e.source == tes3.skillRaiseSource.training) and (mcm.trainerXPEnabled) then
		local skillTypes = tes3.mobilePlayer.skills[e.skill + 1]
		if (mcm.limittrainerXPEnabled) then
			if skillTypes and (skillTypes.type == tes3.skillType.major or skillTypes.type == tes3.skillType.minor) then
...
local goodEnemy = {}
local function enemyWithXP(e)
	if e.actor == tes3.mobilePlayer then
		return
	end
	goodEnemy[e.actor.reference.id] = true
end
event.register(tes3.event.combatStart, enemyWithXP)
...
local function onDeath(e)
	...
	elseif mcm.enemyXPEnabled and e.mobile ~= tes3.mobilePlayer and goodEnemy[e.mobile.reference.id] == true and (e.reference.baseObject.objectType == tes3.objectType.creature or e.reference.baseObject.objectType == tes3.objectType.npc) then
		if e.reference.baseObject.level < tes3.player.object.level then
			giveXP({message = "Враг повержен", amount = mcm.enemyXP})
		elseif e.reference.baseObject.level >= tes3.player.object.level then
			giveXP({message = "Враг повержен", amount = mcm.enemyXP * 2})
		end
		goodEnemy[e.mobile.reference.id] = nil
	end
end
