...
local function isSkillUncapperEnabled()
	return I.Uncapper and I.Uncapper.isSkillUncapperOverride() -- isSkillUncapperOverride instead of isSkillUncapperEnabled
end

local function isXPMultEnabled()
	return I.Uncapper and I.Uncapper.isXPMultEnabled()
end
...
local function skillLevelUpHandler(skillid, source, params)
	local skillStat = NPC.stats.skills[skillid](self)
	local skillCap = 100
	if isSkillUncapperEnabled() then
		skillCap = I.Uncapper.getSkillHardCap(skillid) or math.huge
	end
	if (skillStat.base >= skillCap and params.skillIncreaseValue > 0) or
...
local function skillUsedHandler(skillid, params)
	...
	if (skillStat.base >= skillCap and params.skillGain > 0) or
		(skillStat.base <= 0 and params.skillGain < 0) then
		return false
	end

	-- apply XP multipliers from Uncapper (if loaded and enabled)
	if isXPMultEnabled() then
		local skillMult = I.Uncapper.getSkillMult(skillid)
		if skillMult ~= 1 then
			params.skillGain = params.skillGain * skillMult
		end
		local globalMult = I.Uncapper.getGlobalXPMult()
		if globalMult ~= 1 then
			params.skillGain = params.skillGain * globalMult
		end
		local catchUp = I.Uncapper.getCatchUpSpeed()
		if catchUp > 1 then
			local level = Actor.stats.level(self).current
			local expected = math.max(1, level * 2)
			local speedMult = math.max(1, catchUp * (1 - skillStat.base / expected) + skillStat.base / expected)
			if speedMult > 1 then
	...
	-- soft cap XP penalty
	if isSkillUncapperEnabled() then
		local softCap = I.Uncapper.getSkillSoftCap(skillid)
		if skillStat.base >= softCap then
			local xpMult = I.Uncapper.getSkillXPMultAtSoftCap(skillid)
			params.skillGain = params.skillGain * xpMult
		end
	end
	local gain = params.skillGain / I.SkillProgression.getSkillProgressRequirement(skillid)
	--if I.Uncapper and I.Uncapper.isDebugEnabled() then
	--	print(skillid, string.format("+%.3f%%", gain * 100))
	--end
	skillStat.progress = skillStat.progress + gain
