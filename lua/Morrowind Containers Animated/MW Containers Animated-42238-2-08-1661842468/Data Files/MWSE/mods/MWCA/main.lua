...
local config = mwse.loadConfig('MWCA_config') or {stayOpen = true, playSound = true, items = 0, animTimeToWait = 100, barrelRivet = true, version = 2.08};
...
local function OnMenuExit(eventData)
	...
	if not bagOfHolding and not config.stayOpen and containerRef.data.AC_state == 'open' then
...
local function onEquip(e)
	-- mwse.log('AC onEquip %s', e.item.id);
    if e.item.id == "OJ_BOH_BagOfHolding" then
		-- tes3.messageBox('OJ_BOH_BagOfHolding');
		bagOfHolding = true;
	else
		bagOfHolding = false;
    end
...
local function OnInitialized(eventData)
	...
	mwse.log("[MW Containers Animated v2.08] lua script loaded");
end
