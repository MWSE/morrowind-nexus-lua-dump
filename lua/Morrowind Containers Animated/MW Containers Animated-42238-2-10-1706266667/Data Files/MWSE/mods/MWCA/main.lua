...
local function IsBigStatic(cRef, ref)
		...
		for i, hit in pairs(result) do
			if hit.reference then 
				...
				if hit.reference.id == cRef.object.id then
					...
					return true;
				elseif IsBigStatic(cRef, hit.reference) then
...
local function IsBigStaticAbove(cRef)
	...
	if result then
		for i, hit in pairs(result) do
			if hit.reference and IsBigStatic(cRef, hit.reference) then
...
local function PlayCloseAnimation(cRef) 
	...
	cRef.modified = true;	
	local animGroup = tCloseAnimGroup[sContainerType];
	if animGroup == nil then
		animGroup = 2;
	end
	tes3.playAnimation {reference = cRef, group = animGroup, startFlag = 1};
...
local function PlayOpenAnimation(cRef) 
	...
	cRef.modified = true;	
	local animGroup = tOpenAnimGroup[sContainerType];
	if animGroup == nil then
		animGroup = 1;
	end
	tes3.playAnimation {reference = cRef, group = animGroup, startFlag = 1};
	...
	for sOldMesh, t in pairs(interop) do
		if type(t) == 'table' and type(sOldMesh) == 'string' and type(t[1]) == 'string' and type(t[2]) == 'number' and type(t[3]) == 'number' and type(t[4]) == 'number' and type(t[5]) == 'number' and type(t[6]) == 'string' and type(t[7]) == 'string' and type(t[8]) == 'number' then
			...
			if tMeshNamesToReplace[sOldMesh] then
				mwse.log('[MW Containers Animated] container %s already replaced' , sOldMesh);
			end
...
local function OnInitialized(eventData)
	...
	mwse.log("[MW Containers Animated v2.10] lua script loaded");
end
