...
function KRL_LogTable(arr)
    for k, v in pairs(arr) do
        if type(v) == "table" then
            KRL_LogTable(v)
        else
            KRL_Log(k..": "..tostring(v))
        end
    end
end
...
function KRL_UpdateContainerForAllPlayers(cellName, containerUniqueIndex)
    for pid, player in pairs(Players) do
        if KRL_IsPlayerValid(pid) then
            LoadedCells[cellName]:LoadContainers(pid, LoadedCells[cellName].data.objectData, {containerUniqueIndex})
        end
    end
end
...
function KRL_GetPlayerClass(pid)
    if not Players[pid] then return end
    if not Players[pid].data then return end

    if Players[pid].data.customClass and Players[pid].data.customClass.name then
        return Players[pid].data.customClass
    end

    local className = Players[pid].data.character.class
...
local slowThinkTimer = nil
local fastThinkTimer = nil

function KRL_SlowThink()
    KRL_AttributesSlowThink()

    tes3mp.RestartTimer(slowThinkTimer, time.seconds(1))
end

local secondsSinceServerStart = 0

function KRL_FastThink()
    secondsSinceServerStart = secondsSinceServerStart + 0.1

    tes3mp.RestartTimer(fastThinkTimer, 100)
end

function KRL_CurTime()
    return secondsSinceServerStart
end

customEventHooks.registerHandler("OnServerPostInit", function()
    if not slowThinkTimer then
        slowThinkTimer = tes3mp.CreateTimer("KRL_SlowThink", time.seconds(1))
        tes3mp.StartTimer(slowThinkTimer)
    end

    if not fastThinkTimer then
        fastThinkTimer = tes3mp.CreateTimer("KRL_FastThink", 100)
        tes3mp.StartTimer(fastThinkTimer)
    end
end)
...
function krl_split(str, pattern)
    local r = {}
