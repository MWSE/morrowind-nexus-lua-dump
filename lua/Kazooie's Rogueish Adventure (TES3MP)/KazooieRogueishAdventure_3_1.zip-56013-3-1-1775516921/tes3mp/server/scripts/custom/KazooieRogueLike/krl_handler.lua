...
function KRL_KickFromGameGui(pid)
	tes3mp.CustomMessageBox(pid, kickFromGameGuid, "You must rejoin for the changes to take effect.", "Leave")
end
...
customEventHooks.registerValidator("OnGUIAction", function(eventStatus, pid, guiId, data)
    if not KRL_IsPlayerValid(pid) then return end
    if guiId ~= kickFromGameGuid then return end
