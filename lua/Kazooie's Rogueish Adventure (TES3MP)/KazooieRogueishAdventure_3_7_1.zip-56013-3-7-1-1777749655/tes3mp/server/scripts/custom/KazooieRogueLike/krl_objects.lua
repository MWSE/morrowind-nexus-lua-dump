...
function KRL_GetNextExpectedLevel()
...
local lastDoorUseTime = 0

setupScriptedObject("kazooie_door_inn", function(pid, cellName, object)
    if lastDoorUseTime > KRL_CurTime() then return end
    lastDoorUseTime = KRL_CurTime() + 0.25
    ...
    if not KRL_IsGameWon() then
        ...
        local nextExpectedLevel = KRL_GetNextExpectedLevel()
        ...
        KRL_SaveData("nextAlternatePathRoom", nil)
    end

    KRL_OpenExitDoor("Inn")
end)

setupScriptedObject("krl_inn_trap_door", function(pid, cellName, object)
    if lastDoorUseTime > KRL_CurTime() then return end
    lastDoorUseTime = KRL_CurTime() + 0.25

    if KRL_IsGameWon() then
        tes3mp.MessageBox(pid, -1, "You cannot use this if the game is over.")
        return
    end
    ...
    if activeCell == KRL_SHOP_CELL then
        tes3mp.MessageBox(pid, -1, "You cannot use this in the shop.")
        return
    end
    ...
    if nextExpectedLevel >= KRL_CONFIG.maxRoomLevel then
        tes3mp.MessageBox(pid, -1, "You cannot use this on the last level.")
        return
    end

    local nextAlternatePathRoom = KRL_GetSaveData("nextAlternatePathRoom")

    if not nextAlternatePathRoom then
        local nextCell = KRL_GenerateNextRandomCell(nextExpectedLevel, 0)
        KRL_SaveData("nextAlternatePathRoom", nextCell)
    end

    KRL_DisplayAlternatePathGuid(pid)
end)
...
setupScriptedObject("kazooie_crank_forks", function(pid, cellName, object)
    ...
    local forksChestUniqueId = nil

    for uniqueId, item in pairs(LoadedCells[cellName].data.objectData or {}) do
        if item and item.refId and item.refId == "krl_chest_forks" then
            forksChest = item
            forksChestUniqueId = uniqueId
            break
        end
    end
    ...
    if forksChest then
        ...
        if forksCount > 0 then
            ...
            KRL_UpdateContainerForAllPlayers(cellName, forksChestUniqueId)
        end
    end
end)
...
local cooking_pot_recipes = {{
    name = "Rat Burger",
    result = "krl_rat_burger",
    ingredients = {
    ...
    name = "Mushroom Pasta",
    result = "krl_ingred_mushroom_pasta",
    ingredients = {
        {id = "ingred_saltrice_01", count = 2},
        ...
        {id = "Ingred_horn_lily_bulb_01", count = 1},
        {id = "krl_ingred_cream", count = 1}
    }
}, {
    name = "Ash Frites",
    result = "krl_ingred_ash_frites",
    ingredients = {
        {id = "ingred_ash_yam_01", count = 1},
        {id = "krl_ingred_oil", count = 1}
    }
}, {
    name = "Rat Burger No Veg",
    result = "krl_ingred_rat_burger_noveg",
    ingredients = {
        {id = "ingred_rat_meat_01", count = 1},
        {id = "ingred_bread_01", count = 1}
    }
...
local function doesPotHaveIngredients(cookingPotDict, recipe)
    for _, ingredient in pairs(recipe.ingredients) do
        if (cookingPotDict[string.lower(ingredient.id)] or 0) < ingredient.count then
            return false
        end
    end
...
setupScriptedObject("krl_cooking_pot_crank", function(pid, cellName, object)
    local cookingPot = nil
    local cookingPotRefId = nil

    for index, item in pairs(LoadedCells[cellName].data.objectData or {}) do
        if item and item.refId and item.refId == "krl_cooking_pot" then
            cookingPot = item
            cookingPotRefId = index
            break
        end
    end

    if not cookingPot then
        tes3mp.MessageBox(pid, -1, "ERROR: Could not find the Cooking Pot?")
        return
    end

    if #cookingPot.inventory <= 0 then
        tes3mp.MessageBox(pid, -1, "Put ingredients in the Cooking Pot to cook.")
        return
    end

    local cookingPotDict = {}

    for _, item in pairs(cookingPot.inventory) do
        if item then
            cookingPotDict[string.lower(item.refId)] = item.count
        end
    end

    local cookedRecipe = nil

    for _, recipe in pairs(cooking_pot_recipes) do
        if doesPotHaveIngredients(cookingPotDict, recipe) then
            cookedRecipe = recipe
            break
        end
    end

    if cookedRecipe then
        KRL_GivePlayerItem(pid, cookedRecipe.result, 1)
        tes3mp.MessageBox(pid, -1, "You successfully cooked a "..tostring(cookedRecipe.name)..".")
    else
        tes3mp.MessageBox(pid, -1, "You messed up the recipe. Disgusting!")
    end

    for _, item in pairs(cookingPot.inventory) do
        if item then
            inventoryHelper.removeExactItem(cookingPot.inventory, item.refId, 19208)
        end
    end

    KRL_UpdateContainerForAllPlayers(cellName, cookingPotRefId)
end)

setupScriptedObject("krl_crank_attribute", function(pid, cellName, object)
    KRL_SelectAttribute(pid)
end)
...
setupScriptedObject("krl_crank_debug_vamp", function(pid, cellName, object)
    logicHandler.RunConsoleCommandOnPlayer(pid, "AddSpell, \"vampire blood quarra\"", true)
end)
