...
function KRL_OpenExitDoor(doorType, overrideNextRoom)
	...
	if overrideNextRoom then
		nextCell = overrideNextRoom
		roomType = KRL_ROOM_TYPE_NORMAL
	end
...
function KRL_OnPlayerEntersInn(player)
	local pid = player.pid
	tes3mp.SetBounty(pid, 0)
	tes3mp.SendBounty(pid)

	if Players[pid] and Players[pid].data.cooldowns then
	    for _, cooldown in pairs(Players[pid].data.cooldowns) do
	        tes3mp.ClearCooldownChanges(pid)
	            tes3mp.AddCooldownSpell(pid, cooldown.spellId, 0, 0)
	        tes3mp.SendCooldownChanges(pid)
	    end

	    Players[pid].data.cooldowns = {}
	end

	local attributes_ids = KRL_GetAttributes(pid)

	for _, attribute_id in pairs(attributes_ids) do
		local attribute = KRL_GetAttributeData(attribute_id)

		if attribute.on_inn_entered then
			attribute.on_inn_entered(pid)
		end
	end
end
...
customEventHooks.registerValidator("OnPlayerDeath", function(_, pid)
	...
	if activeCell then
		if activeCell == KRL_START_CELL then return end
		if activeCell == KRL_INN_CELL then return end
	end
...
local alternatePathGuid = 19208777
...
function KRL_DisplayAlternatePathGuid(pid)
    local nextAlternatePathRoom = KRL_GetSaveData("nextAlternatePathRoom")

    if not nextAlternatePathRoom then return end

    local roomDescription = KRL_GetRoomDescription(nextAlternatePathRoom)

    tes3mp.CustomMessageBox(
        pid, 
        alternatePathGuid, 
        "Would you like to take the alternate path and go to the room ["..roomDescription.."]?", 
        "Yes;No"
    )
end
...
customEventHooks.registerValidator("OnGUIAction", function(eventStatus, pid, guiId, data)
    if not KRL_IsPlayerValid(pid) then return end
    if guiId ~= alternatePathGuid then return end
