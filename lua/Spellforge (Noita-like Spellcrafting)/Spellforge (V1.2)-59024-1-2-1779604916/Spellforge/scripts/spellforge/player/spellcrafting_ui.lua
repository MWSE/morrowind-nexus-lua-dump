...
local MAX_WINDOW_SIZE = v2(1080, 740)
local MIN_WINDOW_SIZE = v2(640, 500)
local SCREEN_MARGIN = 8
local OUTER_PADDING_BUDGET = v2(24, 28)
local DEFAULT_TITLE = "New Spell"
local UI_TEXT_SIZE = 12
local UI_BUTTON_TEXT_SIZE = 11
local UI_LIST_TEXT_SIZE = 11
local UI_HEADER_SIZE = 14
local UI_TITLE_SIZE = 17
...
local function layoutMetrics()
    ...
    local content_w = math.max(1, window_w - OUTER_PADDING_BUDGET.x)
    local content_h = math.max(1, window_h - OUTER_PADDING_BUDGET.y)
    local gap = 5
    local palette_w = clamp(math.floor(content_w * 0.20), 112, 165)
    local right_w = clamp(math.floor(content_w * 0.30), 188, 270)
    ...
    local operator_grid_columns = palette_w >= 136 and 2 or 1
    local operator_grid_rows = math.max(1, math.ceil(operator_count / operator_grid_columns))
    local operator_grid_h = 46 + operator_grid_rows * (UI_ROW_HEIGHT + 2)
    local effects_h
    local operators_h
    if main_h < 210 then
        ...
        operators_h = clamp(operator_grid_h, 82, math.max(82, main_h - 135 - gap))
        effects_h = main_h - operators_h - gap
        if effects_h < 135 then
            effects_h = 135
            operators_h = math.max(82, main_h - effects_h - gap)
        end
        effects_h = clamp(effects_h, 135, math.max(135, main_h - 82 - gap))
        operators_h = main_h - effects_h - gap
        if operators_h < 82 then
            effects_h = math.max(110, main_h - gap - 82)
            operators_h = main_h - effects_h - gap
        end
    end
    local saved_h
    ...
    if main_h < 260 then
        ...
        saved_h = clamp(math.floor(main_h * 0.21), 66, 112)
        preview_h = clamp(math.floor(main_h * 0.22), 66, 112)
        editor_h = main_h - saved_h - preview_h - gap * 2
        if editor_h < 110 then
            local deficit = 110 - editor_h
            saved_h = math.max(58, saved_h - math.ceil(deficit / 2))
            preview_h = math.max(58, preview_h - math.floor(deficit / 2))
            editor_h = math.max(72, main_h - saved_h - preview_h - gap * 2)
        end
    end
    ...
    return {
        ...
        operator_button_w = math.max(42, math.floor((math.max(72, palette_w - 24) - (operator_grid_columns - 1) * 3) / operator_grid_columns)),
        effects_h = effects_h,
        operators_h = operators_h,
        effects_visible_rows = clamp(math.floor((effects_h - 108) / (UI_ROW_HEIGHT + 1)), 2, 12),
        operators_visible_rows = clamp(math.floor((operators_h - 34) / (UI_ROW_HEIGHT + 1)), 2, 12),
        recipe_w = recipe_w,
        recipe_button_w = math.max(140, recipe_w - 56),
        recipe_list_h = math.max(48, main_h - 78),
        right_w = right_w,
        right_button_w = math.max(110, right_w - 34),
        saved_h = saved_h,
        saved_visible_rows = clamp(math.floor((saved_h - 36) / (UI_ROW_HEIGHT + 1)), 2, 7),
        editor_h = editor_h,
        preview_h = preview_h,
        ...
        preview_text_h = math.max(24, preview_h - 48),
    }
end
...
local function destroyRoot()
    if state.root then
        ...
        if not ok then
            log.warn(string.format("SPELLFORGE_UI_ROOT_DESTROY_FAILED reason=%s", tostring(err)))
        end
    end
end
...
local function textLayout(text, opts)
    ...
    local default_size = options.header and UI_HEADER_SIZE or UI_TEXT_SIZE
    local props = {
...
local function button(label, callback, opts)
    ...
    local height = options.height or UI_ROW_HEIGHT
    ...
    return {
        ...
        content = openmw_ui.content {
            padded(textLayout(label_text, {
                ...
                color = options.color,
                size = options.text_size or UI_BUTTON_TEXT_SIZE,
            })),
        },
    }
end
...
local function operatorPalette(m)
    ...
    for i = 1, #entries, columns do
        ...
        for column_index = 0, columns - 1 do
            ...
            if entry then
                local label = shortText(entry.display_name or entry.opcode, math.max(7, math.floor(m.operator_button_w / 6)))
...
local function effectResultsLayout(m)
    ...
    category_label = shortText(category_label, math.max(10, math.floor(m.palette_button_w / 6)))
    ...
    items[#items + 1] = button("Refresh", refreshKnownEffects, {
        width = math.min(m.palette_button_w, 72),
        color = COLOR.text,
        text_size = UI_LIST_TEXT_SIZE,
        height = 18,
    })
...
local function effectPalette(m)
    ...
    if warning then
        items[#items + 1] = spacer(0, 2)
        items[#items + 1] = paragraph(warning, v2(m.palette_button_w, 34), { color = COLOR.warning })
    end
    items[#items + 1] = spacer(0, 2)
    items[#items + 1] = framed(textInput(state.effects_filter, setEffectFilter, {
        width = math.max(24, m.palette_button_w - 10),
        color = COLOR.text,
        name = "effect_search_input",
    }), v2(m.palette_button_w, UI_ROW_HEIGHT + 6))
    items[#items + 1] = spacer(0, 2)
...
local function recipeStack(m)
    ...
    local controls = row({
        button("Up", function() moveSelected(-1) end, { width = 44, color = COLOR.text }),
        spacer(4, 0),
        button("Down", function() moveSelected(1) end, { width = 50, color = COLOR.text }),
        spacer(4, 0),
        button("Remove", removeSelected, { width = 64, color = COLOR.text }),
        spacer(4, 0),
        button("New", newRecipe, { width = 50, color = COLOR.text }),
    })
...
local function parameterPickerPanel(effect, kind, m)
    ...
    local content_w = math.max(120, m.right_w - 38)
    local columns = kind == "skill" and (m.right_w >= 245 and 3 or 2) or 2
    local picker_button_w = math.max(48, math.floor((content_w - (columns - 1) * 3) / columns))
    local rows = {
        textLayout(title, { color = COLOR.section }),
        spacer(0, 2),
    }
    if #options == 0 then
        rows[#rows + 1] = paragraph("No options available.", v2(content_w, 28), { color = COLOR.warning })
    else
        for i = 1, #options, columns do
            ...
            for column_index = 0, columns - 1 do
                ...
                if option then
                    local label = shortText(option.display_name or option.name or option.id, math.max(7, math.floor(picker_button_w / 6)))
...
local function parameterEditor(effect, kind, m)
    local current = kind == "skill" and effect.affectedSkill or effect.affectedAttribute
    local label = shortText(selectedParameterLabel(kind, current), math.max(10, math.floor((m.field_input_w - 40) / 6)))
    return row({
        button(label, function()
            openParameterPicker(kind)
        end, {
            width = math.max(70, m.field_input_w - 42),
...
local function selectedEditor(m)
    ...
    if not effect then
        return section("Selected Effect", paragraph(
            ...
            v2(math.max(130, m.right_w - 38), math.max(24, m.editor_h - 56)),
            { color = COLOR.muted }
...
local function previewLines(preview)
    ...
    if #contributors > 0 then
        ...
        for i = 1, math.min(3, #contributors) do
            ...
            parts[#parts + 1] = string.format("%s %.1f", shortText(label, 18), tonumber(contributor.cost) or 0)
        end
    ...
    if #cost_warnings > 0 then
        ...
        lines[#lines + 1] = { text = "Cost Warn : " .. shortText(tostring(reason), 44), color = COLOR.warning }
    end
    ...
    if matrix.deferred_reasons and #matrix.deferred_reasons > 0 then
        lines[#lines + 1] = {
            text = "Deferred   : " .. shortText(rejection_messages.formatDeferredReasons(matrix.deferred_reasons), 64),
            color = COLOR.warning,
        }
    end
    return lines
...
local function bannerHeader(m)
    return {
        ...
        content = openmw_ui.content {
            padded(column({
                textLayout(BANNER_TITLE, { header = true, size = UI_TITLE_SIZE, color = COLOR.title }),
                spacer(0, 2),
                row({
...
local function statusBar(m)
    ...
    local status_w = math.max(120, m.content_w - 220)
    local text = shortText(
        string.format("%s  %s", statusPrefix(kind), state.status or ""),
        math.max(24, math.floor(status_w / 6))
    )
    local saved_label = state.selected_saved_id
        and shortText(string.format("Editing: %s", tostring(state.selected_saved_id)), 34)
        or "Unsaved draft"
    return {
        template = template("box"),
        props = {
...
local function actionButton(label, callback, kind, width)
    return button(label, callback, { width = width or 70, height = UI_ROW_HEIGHT, color = COLOR.text })
end

local function actionButtons(m)
    return row({
        actionButton("Save", saveRecipe, "primary", 54),
        spacer(4, 0),
        actionButton("Validate", validateRecipe, "primary", 68),
        spacer(4, 0),
        actionButton("Preview", previewRecipe, "primary", 66),
        spacer(4, 0),
        actionButton("Create", compileRecipe, nil, 62),
        spacer(4, 0),
        actionButton("Delete", deleteSaved, "danger", 58),
        spacer(4, 0),
...
function spellcrafting_ui.handleKeyPress(key)
    ...
    local escape_key = input.KEY and (input.KEY.Escape or input.KEY.ESCAPE or input.KEY.Esc)
