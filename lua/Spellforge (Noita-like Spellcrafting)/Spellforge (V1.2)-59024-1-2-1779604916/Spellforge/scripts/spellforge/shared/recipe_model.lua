...
function recipe_model.normalize(input, opts)
    ...
    if source.source_kind ~= nil and source.source_kind ~= recipe_model.SOURCE_KIND_EFFECT_LIST then
    ...
    if source.schema_version ~= nil and source.schema_version ~= recipe_model.SCHEMA_VERSION then
        errors[#errors + 1] = validation.error(
    ...
    if options.preserve_request_id and source.request_id ~= nil then
