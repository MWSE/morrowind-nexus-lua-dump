...
local function postfixIsEmptyOrTrigger(value)
    ...
    local op_list = type(ops) == "table" and ops or {}
    local first = op_list[1]
    return #op_list == 1 and type(first) == "table" and first.opcode == "Trigger"
end
...
local function routeBounceTriggerFanoutPayload(route, binding, duplicate_key, is_final, options)
    ...
    local job_ids = ir_bounce_runtime_used and type(ir_enqueue) == "table" and ir_enqueue.job_ids or {}
...
function live_bounce.handleBouncePayload(payload, opts)
    ...
    local route_user_data = type(route.user_data) == "table" and route.user_data or nil
    if route_user_data and runtime_session.shouldDrop(route_user_data["runtime_generation"], "live_bounce_route", {
