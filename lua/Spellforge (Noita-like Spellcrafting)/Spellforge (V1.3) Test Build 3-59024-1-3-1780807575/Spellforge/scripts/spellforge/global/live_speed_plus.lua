...
local function resolveBaseline(context)
    ...
    local numeric_baseline = tonumber(baseline)
    if not isFinite(numeric_baseline) or numeric_baseline <= 0 then
        numeric_baseline = projectile_speed_policy.targetSpellSpeed()
        details = details or {}
        details.baseline_speed = numeric_baseline
        details.source = "fallback"
    end
    return numeric_baseline, details or {}
end
