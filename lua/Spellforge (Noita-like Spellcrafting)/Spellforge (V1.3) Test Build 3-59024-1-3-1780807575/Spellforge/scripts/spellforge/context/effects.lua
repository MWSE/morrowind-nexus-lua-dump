...
local magic_effect_records = content.magicEffects.records
---@type table<string, any>|nil
local static_records = content.statics and content.statics.records or nil
---@type table<string, any>|nil
local sound_records = content.sounds and content.sounds.records or nil
...
magic_effect_records["spellforge_marker_target"] = {
...
local fire_reference = magic_effect_records["fireDamage"]
...
local function resolveDestructionMarkerIcon()
    ...
    for _, record in pairs(magic_effect_records) do
    ...
    local open_reference = magic_effect_records["open"]
...
magic_effect_records["spellforge_marker_target_destruction"] = {
...
local function resolveMagicEffectRecord(effect_id)
    local records = magic_effect_records
...
local function registerSchoolMarker(id, name, school, icon, presentation)
    ...
    magic_effect_records[id] = fields
end
...
local function registerBaseDisplayEffect(entry, opts)
    ...
    magic_effect_records[entry.proxy_id] = fields
...
for key, record in pairs(magic_effect_records) do
...
local burst_display_record = magic_effect_records["spellforge_display_burst"]
if type(burst_display_record) == "table" then
    burst_display_record.name = "Burst - Multicast Pattern"
end

-- Backward-compatibility marker ID for existing saves/content.
magic_effect_records["spellforge_composed"] = {
    name = "Composed Spell",
    school = "alteration",
