...
local state = {
    ...
    pending_release_attempt_id = nil,
    pending_release_started_at = nil,
    pending_release_started_wall = nil,
    skill_handler_registered = false,
    pending_repair_by_spell_id = {},
    generated_spell_aliases = {},
    rehydrate_timer = nil,
    cast_attempt_index = 0,
    pending_intercept_attempt_id = nil,
    pending_intercept_started_at = nil,
    pending_intercept_started_wall = nil,
    intercept_attempt_id = nil,
    intercept_started_at = nil,
    intercept_started_wall = nil,
}

local INTERCEPT_PENDING_TIMEOUT_SECONDS = 1.25
local LATE_AUTHORIZATION_WINDOW_SECONDS = 0.35
local CAST_STATE_WATCHDOG_SECONDS = 5.0
local TARGET_RAY_DISTANCE = 500

local SPELLCAST_START_VARIANTS = {
    ["self start"] = "self",
    ["touch start"] = "touch",
    ["target start"] = "target",
}

local SPELLCAST_RELEASE_VARIANTS = {
    ["self release"] = "self",
    ["touch release"] = "touch",
    ["target release"] = "target",
}

local SPELLCAST_STOP_VARIANTS = {
    ["self stop"] = "self",
    ["touch stop"] = "touch",
    ["target stop"] = "target",
}
...
local function optionalFunction(container, name)
...
local CAST_TRACE_FIELD_ORDER = {
    ...
    "pending_spell_id",
    "intercept_spell_id",
    "pending_release_spell_id",
    "is_casting",
    "pending_cast_authorized",
    "always_succeed",
    "magicka",
    "cost",
    "elapsed",
...
local function simulationTime()
    local get_time = core and core.getSimulationTime
    if type(get_time) == "function" then
        local ok, result = pcall(get_time)
...
local function elapsedFrom(start_time, now_time)
    local start_number = tonumber(start_time)
    local now_number = tonumber(now_time)
    if start_number == nil or now_number == nil then
        return nil
    end
    return now_number - start_number
end

local function elapsedSince(start_time)
    return elapsedFrom(start_time, simulationTime())
end

local function watchdogElapsed(sim_start_time, wall_start_time)
    local sim_elapsed = elapsedSince(sim_start_time)
    local wall_elapsed = elapsedFrom(wall_start_time, wallTime())
    if sim_elapsed ~= nil and sim_elapsed >= 0 and wall_elapsed ~= nil and wall_elapsed >= 0 then
        return math.max(sim_elapsed, wall_elapsed)
    end
    if sim_elapsed ~= nil and sim_elapsed >= 0 then
        return sim_elapsed
    end
    return wall_elapsed
end

local function currentMagicka()
...
local function castStateFields(fields)
    local out = fields or {}
    out.pending_spell_id = out.pending_spell_id or state.pending_intercept_spell_id
    out.intercept_spell_id = out.intercept_spell_id or state.intercept_spell_id
    out.pending_release_spell_id = out.pending_release_spell_id or state.pending_release_spell_id
    out.is_casting = out.is_casting ~= nil and out.is_casting or state.is_casting
    out.pending_cast_authorized = out.pending_cast_authorized ~= nil and out.pending_cast_authorized or state.pending_cast_authorized
    return out
end
...
local function nextCastAttemptId()
    state.cast_attempt_index = (tonumber(state.cast_attempt_index) or 0) + 1
    return string.format("cast-%d-%d", os.time(), state.cast_attempt_index)
end

local function cancelPendingInterceptTimer()
    safeCancelTimer(state.pending_intercept_timer)
    state.pending_intercept_timer = nil
end
...
resetTransientRuntime = function(reason)
    ...
    local had_intercept_timer = state.pending_intercept_timer ~= nil

    safeCancelTimer(state.handshake_timer)
    safeCancelTimer(state.rehydrate_timer)
    safeCancelTimer(state.pending_release_timer)
    cancelPendingInterceptTimer()
    ...
    state.pending_release_attempt_id = nil
    state.pending_release_started_at = nil
    state.pending_release_started_wall = nil
    state.pending_repair_by_spell_id = {}
    state.generated_spell_aliases = {}
    state.rehydrate_timer = nil
    state.pending_intercept_attempt_id = nil
    state.pending_intercept_started_at = nil
    state.pending_intercept_started_wall = nil
    state.intercept_attempt_id = nil
    state.intercept_started_at = nil
    state.intercept_started_wall = nil
    ...
    log.info(string.format(
        "SPELLFORGE_PLAYER_RUNTIME_RESET_ON_LOAD reason=%s backend_before=%s backend_after=%s pending_queries=%s metadata_cache=%s pending_metadata=%s pending_repairs=%s generated_aliases=%s handshake_timer=%s rehydrate_timer=%s release_timer=%s intercept_timer=%s skill_handler_registered=%s animation_diag_registered=%s",
...
local function resolveSelectedSpell()
    local get_core_selected_spell = optionalFunction(core.magic, "getSelectedSpell")
    if get_core_selected_spell then
        local spell = get_core_selected_spell()
...
refreshSpellMetadata = function(spell_id, reason)
    ...
    querySpellMetadata(spell_id, function(meta)
        ...
        if meta.transient == true or meta.timeout == true or meta.error == "metadata query timeout" then
            log.warn(string.format(
                "SPELLFORGE_METADATA_QUERY_TIMEOUT spell_id=%s reason=%s request_id=%s error=%s",
                tostring(spell_id),
                tostring(reason),
...
requestGeneratedSpellRepair = function(spell_id, reason)
    ...
    log.info(string.format(
        ...
        tostring(match and match.saved_recipe_id),
        tostring(spell_id),
        tostring(reason)
    ))
        ...
        if result and result.ok == true then
            ...
            if type(repaired_spell_id) == "string" and repaired_spell_id ~= "" then
                registerGeneratedSpellAlias(
                    spell_id,
                    repaired_spell_id,
                    result.saved_recipe_id or (match and match.saved_recipe_id),
                    "generated-id-repair"
                )
...
local function dispatchInterceptCast(spell_id, attempt_id, variant, dispatch_reason)
    ...
    local ray = nearby.castRay(start_pos, start_pos + (camera_dir * TARGET_RAY_DISTANCE), {
        ignore = self,
        collisionType = nearby.COLLISION_TYPE.Default,
        radius = 0,
    })
    local ray_hit = ray and ray.hit == true
    ...
    core.sendGlobalEvent(events.INTERCEPT_CAST, {
        sender = self.object,
        spell_id = spell_id,
        cast_attempt_id = attempt_id,
        intercept_variant = variant,
        ...
        hit_pos = hit_pos,
        ray_hit = ray_hit,
        ray_distance = TARGET_RAY_DISTANCE,
    })

    castTrace("dispatch_sent", castStateFields({
        ...
        ray_hit = ray_hit,
        ray_distance = TARGET_RAY_DISTANCE,
        hitObject_present = hit_object ~= nil,
        magicka = currentMagicka(),
    }))
    if variant == "target" and not ray_hit then
        castTrace("target_ray_miss", castStateFields({
            attempt_id = attempt_id,
            spell_id = spell_id,
            variant = variant,
            ray_hit = ray_hit,
    ...
    log.info(string.format("intercept dispatch sent spell_id=%s attempt_id=%s", tostring(spell_id), tostring(attempt_id)))
end

local function recordCompiledDispatchSuppressed(spell_id, authorized, reason, attempt_id)
    castTrace("dispatch_suppressed", castStateFields({
        attempt_id = attempt_id,
        spell_id = spell_id,
    ...
    log.warn(string.format(
        "SPELLFORGE_COMPILED_DISPATCH_SUPPRESSED spell_id=%s attempt_id=%s authorized=%s reason=%s",
        tostring(spell_id),
        tostring(attempt_id),
        tostring(authorized),
        tostring(reason)
...
local function cameraForward()
    local get_view_direction = optionalFunction(camera, "getViewDirection")
    local direct = safeCall(function()
        return get_view_direction and get_view_direction() or nil
    end)
    if direct ~= nil then
...
local function cameraRight(forward)
    local get_left = optionalFunction(camera, "getLeft")
    local left = safeCall(function()
        return get_left and get_left() or nil
    end)
    if left ~= nil then
...
local function buildOsscStyleLaunchPayload(spell_id, opts)
    ...
    local ray = safeCall(function()
        return nearby.castRay(camera_pos, ray_end, {
            ignore = self,
            collisionType = nearby.COLLISION_TYPE.Default,
            radius = 0,
        })
...
local function castLosRay(ray_start, target_pos, ignore_object)
    local ok, ray = pcall(function()
        return nearby.castRay(ray_start, target_pos, {
            ignore = ignore_object or self,
            collisionType = nearby.COLLISION_TYPE.Default,
            radius = 0,
        })
    end)
...
local function clearPendingInterceptState()
    cancelPendingInterceptTimer()
    state.pending_intercept_spell_id = nil
    state.pending_intercept_variant = nil
    state.pending_intercept_attempt_id = nil
...
local function armPendingIntercept(spell_id, variant, selected_spell_id)
    clearPendingInterceptState()
    local attempt_id = nextCastAttemptId()
    state.pending_intercept_spell_id = spell_id
    state.pending_intercept_variant = variant
    state.pending_intercept_attempt_id = attempt_id
    state.pending_intercept_started_at = simulationTime()
    state.pending_intercept_started_wall = wallTime()
    state.pending_cast_authorized = false
    state.pending_intercept_timer = async:newUnsavableSimulationTimer(INTERCEPT_PENDING_TIMEOUT_SECONDS, function()
        if state.pending_intercept_spell_id ~= spell_id then
            return
        end
        local timeout_variant = state.pending_intercept_variant
        local timeout_attempt_id = state.pending_intercept_attempt_id
        local elapsed = elapsedSince(state.pending_intercept_started_at)
        state.pending_intercept_timer = nil
        state.pending_intercept_spell_id = nil
        state.pending_intercept_variant = nil
        state.pending_intercept_attempt_id = nil
        state.pending_intercept_started_at = nil
        state.pending_intercept_started_wall = nil
        castTrace("pending_intercept_timeout", castStateFields({
            attempt_id = timeout_attempt_id,
            spell_id = spell_id,
            selected_spell_id = selected_spell_id,
            variant = timeout_variant,
            elapsed = elapsed,
            window = INTERCEPT_PENDING_TIMEOUT_SECONDS,
            magicka = currentMagicka(),
        }), "warn")
        log.warn(string.format(
            "SPELLFORGE_INTERCEPT_PENDING_TIMEOUT spell_id=%s attempt_id=%s selected_spell_id=%s variant=%s window=%.2f",
            tostring(spell_id),
            tostring(timeout_attempt_id),
            tostring(selected_spell_id),
            tostring(timeout_variant),
            INTERCEPT_PENDING_TIMEOUT_SECONDS
        ))
    end)
    castTrace("input_armed", castStateFields({
        attempt_id = attempt_id,
        spell_id = spell_id,
        selected_spell_id = selected_spell_id,
        variant = variant,
...
local function spellAlwaysSucceeds(spell_id)
    ...
    local always_succeed = spell_record["alwaysSucceed"]
    if always_succeed ~= nil then
        return always_succeed == true or always_succeed == 1
    end
    return false
end
...
local function registerSkillProgressionHandler()
    ...
    progression.addSkillUsedHandler(function(skillid, params)
        ...
        if should_log_diag then
            castTrace("skill_use", castStateFields({
                attempt_id = state.intercept_attempt_id or state.pending_release_attempt_id or state.pending_intercept_attempt_id,
                selected_spell_id = selected_spell_id,
                runtime_spell_id = selected_runtime_spell_id,
                useType = params and params.useType,
                expectedSuccess = spellcast_success,
                skillid = skillid,
            }))
        end
        ...
        if selected_is_cached_spellforge and not state.is_casting and state.intercept_spell_id == nil and state.pending_intercept_spell_id == nil then
            castTrace("skill_success_outside_window", castStateFields({
                selected_spell_id = selected_spell_id,
                runtime_spell_id = selected_runtime_spell_id,
        ...
        if state.is_casting then
            ...
            castTrace("skill_success_authorized", castStateFields({
                attempt_id = state.intercept_attempt_id,
                spell_id = state.intercept_spell_id,
                selected_spell_id = selected_spell_id,
                runtime_spell_id = selected_runtime_spell_id,
                variant = state.intercept_variant,
                useType = params.useType,
                expectedSuccess = spellcast_success,
                elapsed = elapsedSince(state.intercept_started_at),
                magicka = currentMagicka(),
            }))
        elseif state.pending_release_spell_id then
            local late_spell_id = state.pending_release_spell_id
            local late_attempt_id = state.pending_release_attempt_id
            local elapsed = elapsedSince(state.pending_release_started_at)
            clearPendingReleaseState()
            castTrace("late_authorization_dispatch", castStateFields({
                attempt_id = late_attempt_id,
                spell_id = late_spell_id,
                selected_spell_id = selected_spell_id,
                runtime_spell_id = selected_runtime_spell_id,
                useType = params.useType,
                expectedSuccess = spellcast_success,
            ...
            dispatchInterceptCast(late_spell_id, late_attempt_id, nil, "late_authorization")
        end
    end)
...
local function registerAnimationTextKeys()
    ...
    interfaces.AnimationController.addTextKeyHandler("spellcast", function(groupname, key)
        ...
        if not state.is_casting then
            ...
            local pending_attempt_id = state.pending_intercept_attempt_id
            local actual_variant = SPELLCAST_START_VARIANTS[key]
            if pending_spell_id and actual_variant then
                ...
                state.intercept_variant = actual_variant
                state.intercept_attempt_id = pending_attempt_id
                state.intercept_started_at = simulationTime()
                state.intercept_started_wall = wallTime()
                clearPendingInterceptState()
                if actual_variant ~= pending_variant then
                    log.warn(string.format(
                        "SPELLFORGE_INTERCEPT_VARIANT_MISMATCH spell_id=%s attempt_id=%s predicted_variant=%s actual_variant=%s key=%s",
                        tostring(pending_spell_id),
                        tostring(pending_attempt_id),
                        tostring(pending_variant),
                        tostring(actual_variant),
                        tostring(key)
                    ))
                end
                castTrace("animation_start", castStateFields({
                    attempt_id = pending_attempt_id,
                    spell_id = pending_spell_id,
                    selected_spell_id = selected_spell and selected_spell.id or nil,
                    variant = actual_variant,
                    key = key,
                    always_succeed = always_succeed,
                    authorized = state.pending_cast_authorized,
                    magicka = currentMagicka(),
                }))
            end
            return
        ...
        local release_variant = SPELLCAST_RELEASE_VARIANTS[key]
        local stop_variant = SPELLCAST_STOP_VARIANTS[key]
        local variant = state.intercept_variant or release_variant or stop_variant or "self"
        if release_variant then
            local spell_id = state.intercept_spell_id
            local attempt_id = state.intercept_attempt_id
            local authorized = state.pending_cast_authorized == true
            if release_variant ~= variant then
                castTrace("release_variant_mismatch", castStateFields({
                    attempt_id = attempt_id,
                    spell_id = spell_id,
                    variant = variant,
                    key = key,
                    actual_variant = release_variant,
                    elapsed = elapsedSince(state.intercept_started_at),
                }), "warn")
            end

            if types.Actor.getStance(self) ~= types.Actor.STANCE.Spell then
                castTrace("release_aborted_stance_changed", castStateFields({
                    attempt_id = attempt_id,
                    spell_id = spell_id,
                    variant = variant,
                    key = key,
            ...
            castTrace("animation_release", castStateFields({
                attempt_id = attempt_id,
                spell_id = spell_id,
                variant = variant,
                key = key,
            ...
            if spell_id and authorized then
                dispatchInterceptCast(spell_id, attempt_id, variant, "authorized_release")
                clearPendingReleaseState()
            else
                ...
                if spell_id and not authorized then
                    ...
                    state.pending_release_attempt_id = attempt_id
                    state.pending_release_started_at = simulationTime()
                    state.pending_release_started_wall = wallTime()
                    state.pending_release_timer = async:newUnsavableSimulationTimer(LATE_AUTHORIZATION_WINDOW_SECONDS, function()
                        if not state.pending_release_spell_id then
                            return
                        end
                        local timeout_spell_id = state.pending_release_spell_id
                        local timeout_attempt_id = state.pending_release_attempt_id
                        local elapsed = elapsedSince(state.pending_release_started_at)
                        clearPendingReleaseState()
                        castTrace("late_authorization_timeout", castStateFields({
                            attempt_id = timeout_attempt_id,
                            spell_id = timeout_spell_id,
                            variant = variant,
                            authorized = false,
                            reason = "late authorization timeout",
                            elapsed = elapsed,
                            window = LATE_AUTHORIZATION_WINDOW_SECONDS,
                            magicka = currentMagicka(),
                        }), "warn")
                        recordCompiledDispatchSuppressed(timeout_spell_id, false, "late authorization timeout", timeout_attempt_id)
                    end)
                    castTrace("release_waiting_for_authorization", castStateFields({
                        attempt_id = attempt_id,
                        spell_id = spell_id,
                        variant = variant,
                        authorized = false,
                    ...
                    clearPendingReleaseState()
                    recordCompiledDispatchSuppressed(spell_id, authorized, reason, attempt_id)
                end
            end
            clearInterceptState()
        elseif stop_variant then
            if stop_variant ~= variant then
                castTrace("stop_variant_mismatch", castStateFields({
                    attempt_id = state.intercept_attempt_id,
                    spell_id = state.intercept_spell_id,
                    variant = variant,
                    key = key,
                    actual_variant = stop_variant,
                    elapsed = elapsedSince(state.intercept_started_at),
                }), "warn")
            end
            castTrace("animation_stop", castStateFields({
                attempt_id = state.intercept_attempt_id,
                spell_id = state.intercept_spell_id,
                variant = variant,
                key = key,
...
local function onInputAction(action)
    local ui_handled = spellcrafting_ui.handleInputAction(action)
    ...
    if meta.is_spellforge ~= true then
        requestGeneratedSpellRepair(runtime_spell_id, "input-non-spellforge")
        if runtime_spell_id ~= selected_spell_id then
            requestGeneratedSpellRepair(selected_spell_id, "input-selected-alias-non-spellforge")
        end
        return true
    end

    if state.pending_intercept_spell_id ~= nil and state.pending_intercept_spell_id ~= runtime_spell_id then
        log.warn(string.format(
            "SPELLFORGE_INTERCEPT_STALE_PENDING_CLEARED pending_spell_id=%s new_spell_id=%s selected_spell_id=%s",
    ...
    if state.is_casting or state.pending_intercept_spell_id ~= nil or state.pending_release_spell_id ~= nil then
        castTrace("input_blocked_by_cast_state", castStateFields({
            attempt_id = state.intercept_attempt_id or state.pending_release_attempt_id or state.pending_intercept_attempt_id,
            selected_spell_id = selected_spell_id,
    ...
    if not canAffordSpell(runtime_spell_id) then
        castTrace("input_skipped_insufficient_magicka", castStateFields({
            selected_spell_id = selected_spell_id,
            runtime_spell_id = runtime_spell_id,
            magicka = currentMagicka(),
        }))
    ...
    armPendingIntercept(runtime_spell_id, variant, selected_spell_id)

    return true
end

local function checkCastStateWatchdog()
    local intercept_elapsed = watchdogElapsed(state.intercept_started_at, state.intercept_started_wall)
    if state.is_casting and intercept_elapsed ~= nil and intercept_elapsed > CAST_STATE_WATCHDOG_SECONDS then
        castTrace("watchdog_cleared_intercept_state", castStateFields({
            attempt_id = state.intercept_attempt_id,
            spell_id = state.intercept_spell_id,
            variant = state.intercept_variant,
            elapsed = intercept_elapsed,
            window = CAST_STATE_WATCHDOG_SECONDS,
            magicka = currentMagicka(),
        }), "warn")
        clearInterceptState()
        clearPendingReleaseState()
    ...
    local release_elapsed = watchdogElapsed(state.pending_release_started_at, state.pending_release_started_wall)
    if state.pending_release_spell_id ~= nil
        and release_elapsed ~= nil
        and release_elapsed > (LATE_AUTHORIZATION_WINDOW_SECONDS + 1.0)
    then
        castTrace("watchdog_cleared_pending_release", castStateFields({
            attempt_id = state.pending_release_attempt_id,
            spell_id = state.pending_release_spell_id,
    ...
    local pending_elapsed = watchdogElapsed(state.pending_intercept_started_at, state.pending_intercept_started_wall)
    if state.pending_intercept_spell_id ~= nil
        and pending_elapsed ~= nil
        and pending_elapsed > (INTERCEPT_PENDING_TIMEOUT_SECONDS + 1.0)
    then
        castTrace("watchdog_cleared_pending_intercept", castStateFields({
            attempt_id = state.pending_intercept_attempt_id,
            spell_id = state.pending_intercept_spell_id,
            variant = state.pending_intercept_variant,
...
local function onInterceptDispatchResult(payload)
    ...
    if payload.ok == true then
        if dispatch_kind == "compiled_spellforge" or dispatch_kind == "compiled_spellforge_2_2c_helper" then
            castTrace("dispatch_result_ok", castStateFields({
                attempt_id = payload.cast_attempt_id,
                spell_id = payload.spell_id,
                recipe_id = payload.recipe_id,
                cast_id = payload.cast_id,
                dispatch_count = payload.dispatch_count,
                live_2_2c = payload.live_2_2c,
                projectile_id = payload.projectile_id,
                projectile_count = type(payload.projectile_ids) == "table" and #payload.projectile_ids or nil,
                job_id = payload.job_id,
            }))
            log.info(string.format(
                "SPELLFORGE_COMPILED_DISPATCH_OK spell_id=%s attempt_id=%s dispatch_count=%s",
                tostring(payload.spell_id),
                tostring(payload.cast_attempt_id),
    ...
    castTrace("dispatch_result_failed", castStateFields({
        attempt_id = payload.cast_attempt_id,
        spell_id = payload.spell_id,
        recipe_id = payload.recipe_id,
        cast_id = payload.cast_id,
    ...
    log.error(string.format("intercept dispatch failed spell_id=%s attempt_id=%s err=%s", tostring(payload.spell_id), tostring(payload.cast_attempt_id), tostring(payload.error)))
    recordCompiledDispatchSuppressed(payload.spell_id, "unknown", payload.error or "dispatch failed", payload.cast_attempt_id)
end
...
return {
    engineHandlers = {
        ...
        onLoad = onLoad,
        onFrame = function()
            spellcrafting_ui.handleFrame()
            checkCastStateWatchdog()
