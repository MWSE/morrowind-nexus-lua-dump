...
local function addParameterizedEntries(indexed, entry)
    ...
    if entry.hasAttribute == true then
        local attribute_options = effect_registry.attributeOptions()
        if type(attribute_options) ~= "table" then
            attribute_options = {}
        end
        for _, option in ipairs(attribute_options) do
    ...
    elseif entry.hasSkill == true then
        local skill_options = effect_registry.skillOptions()
        if type(skill_options) ~= "table" then
            skill_options = {}
        end
        for _, option in ipairs(skill_options) do
            local suffix = normalized .. "_" .. option.id
            indexed[suffix] = {
                id = normalized,
