...
local function calculateSkillLimit(e)
	...
	for _, skill in pairs(pcRace.skillBonuses) do
		playerSkillLimit[skill.skill] = playerSkillLimit[skill.skill] + skill.bonus
	end
	local spec = pcClass.specialization * 9 
	for skill = spec, spec + 8 do
		playerSkillLimit[skill] = playerSkillLimit[skill] + 5
	end
end
...
local function onStatsMenuRefreshed(e)
	local scrollPaneChildren = e.element:findChild(tes3ui.registerID("PartScrollPane_pane")).children
