...
local function loadReplacer(objectsToReplace)
...
local function swapObjects(ref, cell, newObj)
...
-- DEPRECATED Main function for replacing at runtime. Runs on every cell change
local function onCellChange(e)
    -- Iterates through all references in a cell
    for ref in e.cell:iterateReferences() do
        ...
        for _, obj in pairs(mergedObjects) do
            ...
            if (ref.baseObject.id == obj.oldObject and not attemptedReplace) then
                ...
                for _, newObj in pairs(obj.newObjects) do
                    ...
                    if (((newObj.specificCell and newObj.specificCell == e.cell.id) or not newObj.specificCell) and not actuallyReplaced) then
                        -- If guaranteed replace chance, then replace
                        if (newObj.replaceChance == 1) then
                            actuallyReplaced = true
                            swapObjects(ref, e.cell, newObj)
        ...
        if (ref.baseObject.id == obj.oldObject and not attemptedReplace) then
            ...
            for _, newObj in pairs(obj.newObjects) do
                ...
                if (((newObj.specificCell and newObj.specificCell == ref.cell.id) or not newObj.specificCell) and not actuallyReplaced) then
                    ...
                    if (newObj.replaceChance == 1) then
                        actuallyReplaced = true
                        swapObjects(ref, ref.cell, newObj)
                    -- Otherwise calculate replace with random float
                    else
                        local randChance = math.random()
                        if (randChance < newObj.replaceChance) then
...
local function compareNewObjects(newObject1, newObject2)
...
local function compareObjects(newObject1, newObject2)
...
local function mergeReplacers()
    ...
    for _, obj in pairs(mergedObjects) do
        ...
        for _, newObj in pairs(obj.newObjects) do
            ...
            table.sort(sortedObjects, compareNewObjects)
        end
        obj.newObjects = sortedObjects
    end
    -- Sorts mergedObjects list
    table.sort(mergedObjects, compareObjects)
end

-- Registers cellChanged event and loads/merges _OR_ files
