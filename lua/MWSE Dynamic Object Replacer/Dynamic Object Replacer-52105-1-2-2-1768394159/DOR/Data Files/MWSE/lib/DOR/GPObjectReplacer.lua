local OR = {allObjects = {}, mergedObjects = {}}
...
function OR.loadReplacer(objectsToReplace)
    ...
    if (not objectsToReplace) then
        mwse.log(
            "[Object Replacer] objectsToReplace is nil! Check your _OR_ files!")
        return
    end
    if (not objectsToReplace.objectToReplace0) then
        mwse.log(
            "[Object Replacer] objectToReplace0 is nil! Check your _OR_ files!")
        return
    end
...
--- @param newRef tes3reference
--- @param oldRef tes3reference
local function cloneReferenceData(newRef, oldRef)
    -- Copy over basic item data.
    local oldItemData = oldRef.itemData
    if (oldItemData) then
        local newItemData = newRef.itemData or
                                tes3.addItemData({reference = newRef})
        newItemData.charge = oldItemData.charge
        -- newItemData.count = oldItemData.count
        newItemData.condition = oldItemData.condition
        newItemData.owner = oldItemData.owner
        newItemData.requirement = oldItemData.requirement

        -- Copy over script variable data.
        if (newRef.object.script and newRef.object.script ==
            oldRef.object.script) then
            local newContext = newItemData.context
            local oldContext = oldItemData.context
            for name, oldVarData in pairs(oldContext:getVariableData()) do
                newContext[name] = oldVarData.value
            end
        end
        ...
        if oldRef.data then
            table.copy(oldRef.data, newRef.data)
            table.copy(oldRef.tempData, newRef.tempData)
        end
    end

    -- Copy over any mobile data.
    local newMobile = newRef.mobile
    local oldMobile = oldRef.mobile
    if (newMobile and oldMobile) then
        -- TODO: Implement. Though we probably don't want to swap actors anyway.
    end
end

-- Function for actually swapping references
function OR.swapObjects(ref, cell, newObj)
    if ref.disabled or ref.deleted then return end
...
function OR.checkForCell(cellList, refCell)
    for _, cell in pairs(cellList) do
        ...
        if ((cell.cellName and
            (string.find(cellString2, cellString1) or cellString2 == cellString1)) or
            not cell) then return true end
    end
end
...
function OR.onReferenceActivated(e)
    ...
    for _, obj in pairs(OR.mergedObjects) do
        ...
        if (ref.baseObject.id == obj.oldObject and
            (not ref.data or not ref.data.GPDOR or
                not ref.data.GPDOR.attemptedReplace) and attemptedReplace ==
            false) then
            ...
            for _, newObj in pairs(obj.newObjects) do
                ...
                if newObj.specificCell then
                    specificCell = OR.checkForCell(newObj.specificCell,
                                                   ref.cell.id)
                end
                -- ends early if current cell is excluded
                if newObj.excludedCell then
                    excludedCell = OR.checkForCell(newObj.excludedCell,
                                                   ref.cell.id)
                end
                if excludedCell then return end
                -- Checks for matching cell and that the object has not *actually* been replaced
                if ((specificCell or not newObj.specificCell) and
                    (not ref.data or not ref.data.GPDOR or
                        not ref.data.GPDOR.actuallyReplaced) and
                    actuallyReplaced == false) then
...
function OR.loadORFiles()
    ...
    for filePath, dir, fileName in lfs.walkdir(subDir) do
        ...
        if string.find(fileName, "_OR_") then
            mwse.log("[Object Replacer] Found _OR_ file with file name: %s",
                     fileName)
            -- Loads the config file
            local modifiedPath = string.gsub(filePath,
