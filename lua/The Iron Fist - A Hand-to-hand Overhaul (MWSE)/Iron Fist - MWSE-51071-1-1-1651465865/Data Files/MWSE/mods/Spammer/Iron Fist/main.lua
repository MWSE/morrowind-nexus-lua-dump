...
local function shaolinMonk(e)
    ...
    if equippedLeft then
        weightLeft = equippedLeft.object.weightClass or 0
    else weightLeft = -1
    end
    if equippedRight then
        weightRight = equippedRight.object.weightClass or 0
    else weightRight = -1
    end
    ...
    if leftRight == 0 then
        result = e.mobile:applyDamage({damage = ((boom*(weightLeft+1)*lcon*cf.dmg/3000)), applyArmor = true, resistAttribute = 12, playerAttack = true, applyDifficulty = true})
       -- tes3.playAnimation({reference = e.reference, group = 23, loopCount = 0})
        leftRight = 1
    elseif leftRight == 1 then
        result = e.mobile:applyDamage({damage = ((boom*(weightRight+1)*rcon*cf.dmg/3000)), applyArmor = true, resistAttribute = 12, playerAttack = true, applyDifficulty = true})
       -- tes3.playAnimation({reference = e.reference, group = 23, loopCount = 0})
        leftRight = 0
    end
    ...
    if equippedRight then
        ...
        equippedRight.itemData.charge = ((equippedRight.itemData.charge)-(equippedRight.object.enchantment.chargeCost)) end
        end
...
local function shaolinTooltip(e)
    ...
    if (e.object.objectType == armorCheck and (e.object.slot == 6 or e.object.slot == 7)) then
        ...
        if con == -1 then
            ...
            text = string.format("Attack: %d - %d", ((weight+1)*cf.dmg*con/300), ((weight+1)*cf.dmg*con/100)) end
...
local function initialized()
    ...
    print("[Iron Fist : A Hand-to-Hand Overhaul, by Spammer] 1.0 Initialized!")
