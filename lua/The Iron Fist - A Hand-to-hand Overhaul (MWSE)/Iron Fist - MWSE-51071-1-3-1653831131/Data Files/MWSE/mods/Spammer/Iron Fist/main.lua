...
local function shaolinMonk(e)
    ...
    if equippedLeft then
        ...
        if encantol and encantol.enchantment then
            local chargeCost = tes3.calculateChargeUse{mobile = tes3.mobilePlayer, enchantment = encantol.enchantment}
        if (equippedLeft.itemData.charge >= chargeCost) and (equippedLeft.object.enchantment.castType ~= 3) then
        ...
        equippedLeft.itemData.charge = ((equippedLeft.itemData.charge)-(chargeCost)) end
        end
    ...
    if equippedRight then
        ...
        if encantor and encantor.enchantment then
            local chargeCost = tes3.calculateChargeUse{mobile = tes3.mobilePlayer, enchantment = encantor.enchantment}
        if equippedRight.itemData.charge >= chargeCost and (equippedRight.object.enchantment.castType ~= 3) then
            if cf.onOff and leftRight == 0 and rspell then
                ...
                equippedRight.itemData.charge = ((equippedRight.itemData.charge)-(chargeCost))
            end
        end
...
local function shaolinTooltip(e)
    ...
    if (e.object.objectType == armorCheck and (e.object.slot == 6 or e.object.slot == 7)) then
        ...
        elseif weight == 1 then
            armor = tes3.mobilePlayer.mediumArmor.current/20
...
local function initialized()
    ...
    print("[Iron Fist : A Hand-to-Hand Overhaul, by Spammer] 1.3 Initialized!")
