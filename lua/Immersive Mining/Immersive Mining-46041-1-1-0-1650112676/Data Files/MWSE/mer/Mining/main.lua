...
local function onAttack(e)
	...
	--Pick strength
	local weapon = mobilePlayer.readiedWeapon
	local chopFloor = 10
	local chopCeiling = 50
	local pickStrength = weapon.object.chopMax
	...
	if distanceToTarget < minLookDistance and swingType == chop and string.find(string.lower(weapon.object.id), "pick") then
		...
		if (string.lower(string.sub(containerRef.id, 1, 4)) == "rock") or string.find(containerRef.id:lower(), "mine_ore") then
			...
			if oreCount == 0 then
				...
				tes3.messageBox("This vein has been depleted")
			...
			weapon.variables.condition = weapon.variables.condition - conditionDamage
			--Unequip if broken
			if weapon.variables.condition <= 0 then
				weapon.variables.condition = 0
				mobilePlayer:unequip({ item = weapon.object })
			end
			...
			if swings >= swingsNeeded then
				...
				for item,count in pairs(contents) do
					if count > 0 then
						...
						tes3.addItem({ reference=playerRef, item=item.id, count=harvestCount })
						tes3.removeItem({ reference=containerRef, item=item.id, count=harvestCount })
						itemName = item.name
						break
					end
				end
				tes3.playSound({reference=playerRef, sound="Item Misc Up"})
				tes3.messageBox("You mine %.0f %s", harvestCount, itemName)
				swings = 0
			else
			--Slap on the wrist crime
				tes3.triggerCrime({
					type = 6, --tresspass 5g
					victim = tes3.getOwner(containerRef),
				})
			end
		end
	end
...
local function onActivate(e)
	...
	if (container.objectType == tes3.objectType.container) then
		if string.lower(string.sub(target.id, 1, 4)) == "rock" then
			...
			--if oreCount == 0 then
				--tes3.messageBox("This vein has been depleted")
			if string.find(string.lower(weaponID), "pick") then
				if oreCount == 0 then return
				elseif oreCount < 3 then
					tes3.messageBox("This vein is nearly depleted")
				elseif oreCount < 5 then
					tes3.messageBox("This vein is rich in ore")
				else
					tes3.messageBox("This vein is very rich in ore")
				end
			else
				tes3.messageBox("Requires a pick axe")
			end
			return false
		else
...
local function initialized()
	...
	event.register("activate", onActivate, { priority = 300})
	event.register("activate", onActivate2, {priority = 0})
