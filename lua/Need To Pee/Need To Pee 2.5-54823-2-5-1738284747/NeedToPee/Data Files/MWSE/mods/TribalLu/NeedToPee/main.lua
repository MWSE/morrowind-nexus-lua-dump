...
--Page2
	...
	local category6 = page0:createCategory("Water Requirement")
	
	waterreqButton = category6:createYesNoButton({
	
        label = "Do you want the Requirement of being by Water to Reset Pain Level?",
		variable = mcm:createTableVariable{id = "waterreq", table = config},
		callback = function(self)
			if ( config.waterreq == true ) then
				tes3.messageBox("Water Requirement enabled.")
			else
				tes3.messageBox("Water Requirement disabled.")
			end
		end
    })
