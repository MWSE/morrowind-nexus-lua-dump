...
local function mobileActivated(e)	local r = e.reference	local d = r.data		if d and d.born then
	...
	else local m = e.mobile		local nod = PG[d.curX]		R[r] = {m = m, d = d, ad = m.actionData, ai = m.aiPlanner, curnod = nod, lastnod = nod} end
...
local function cellChanged(e)	if not p.cell.isInterior then	local mob, c, r, pos	local st = tes3.getSimulationTimestamp()	local deathhour = st + math.min(cf.live, 20 - hour.value)
	local eye = tes3.getPlayerEyePosition()		local Day = hour.value > 8 and hour.value < 20		
	
	for _, cell in pairs(tes3.getActiveCells()) do c = cell.gridX .. " " .. cell.gridY		if C[c] then
		if not SC[c] then
			for i, nod in pairs(cell.pathGrid.nodes) do PG[math.ceil(nod.position.x)] = nod end
			SC[c] = true
		end
...
if cf.AI > 0 then T = timer.start{duration = cf.AI, iterations = -1, callback = function()
	for r, t in pairs(R) do if not t.m.inCombat then		--	local cur = t.ai:getActivePackage()
		local CON = t.curnod.connectedNodes		local delind = #CON > 1 and table.find(CON, t.lastnod)
		if delind then CON[delind] = nil end
		t.lastnod = t.curnod
...
local function initialized(e)		--for c, t in pairs(C) do C[c].m = #t.p end
	...
	for bp in tes3.iterateObjects(1497648962) do if bp.partType == 0 and B[bp.part] and not bp.playable then 
		if cf.heads or bp.sourceMod == "Morrowind.esm" then table.insert(B[bp.part][bp.raceName][bp.female and "f" or "m"], bp) end
	end end
