...
-- Follower tracking for silt strider travel.
-- This table is kept in sync via AttendMe's AttendMeFollowerStatus events.
-- When the global script fires Rss_CollectFollowers we send the current list back.
-- If AttendMe is not installed this table stays empty and everything still works.
...
-- Called by AttendMe's follower actor scripts whenever a follower starts or stops following the player.
-- We maintain a live mirror of the current follower set so we can hand it to the global
-- script instantly when travel starts (no search delay, no nearby-only limitation).
local function attendMeFollowerStatus(data)
  local actor = data.actor
  local status = data.status -- true = now following, false = stopped following
  if not actor or not actor:isValid() then return end
  local id = tostring(actor)
  if status then
    currentFollowers[id] = actor
  else
    currentFollowers[id] = nil
  end
end

-- Called by the global script at the start of every silt strider ride.
-- We convert our follower map to a plain list and send it back so the global script
-- can disable and later restore them. Sends an empty list if AttendMe is not installed.
local function collectFollowers()
  local list = {}
  for _, actor in pairs(currentFollowers) do
    if actor:isValid() then
      table.insert(list, actor)
    end
  end
  core.sendGlobalEvent('Rss_SetFollowersForTravel', { followers = list })
end
...
return {
    ...
    eventHandlers = { 
      ...
      AttendMeFollowerStatus = attendMeFollowerStatus,
      Rss_CollectFollowers = collectFollowers,
    }
}