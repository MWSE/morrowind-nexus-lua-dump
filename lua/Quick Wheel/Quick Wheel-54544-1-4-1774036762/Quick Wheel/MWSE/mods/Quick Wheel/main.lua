local cf = mwse.loadConfig("Quick Wheel", {qwhkey = {keyCode = 16}, whalf = 0, whn = 16, whwid = 700, mbwh = 3, scric = true, tim = 1})
local L = {}	local M = {}	local G = {pi2 = math.pi*2}
local p, mp, inv, D, MB, wc
...
p0:createDecimalSlider{label = "Time speed", min = 0.05, max = 1, variable = var{id = "tim", table = cf}}
...
L.KeyUpWH = function(e) if e.keyCode == cf.qwhkey.keyCode then
	...
	wc.mouseSensitivityX = G.MosX			wc.mouseSensitivityY = G.MosY
	event.unregister("mouseAxis", L.MousAx)		event.unregister("keyUp", L.KeyUpWH)	M.WH[0]:destroy()	M.WH = nil
	if cf.tim < 1 then wc.simulationTimeScalar = 1 end
end end



local function KEYDOWN(e) if not tes3ui.menuMode() and e.keyCode == cf.qwhkey.keyCode and not M.WH then		local ob, ic, enc, count
	G.WHang = G.pi2 / cf.whn		G.MoX = 0		G.MoY = 0		G.WHlast = nil		G.WHcur = nil		wc.mouseSensitivityX = G.MosX/100		wc.mouseSensitivityY = G.MosY/100
	...
	if cf.tim < 1 then wc.simulationTimeScalar = cf.tim end
...
local function loaded(e) p = tes3.player	 mp = tes3.mobilePlayer		D = p.data		inv = p.object.inventory	MB = tes3.worldController.inputController.mouseState.buttons	
	...
	wc = tes3.worldController		G.MosX = wc.mouseSensitivityX	G.MosY = wc.mouseSensitivityY
	if cf.tim < 1 then wc.simulationTimeScalar = 1 end
