...
local staticConfig = config.getStaticConfig()
...
local function createSpellHomePage(template)
    ...
    page:createYesNoButton({
        ...
        description =
            ...
            "\n\nNPC spells have a ".. staticConfig.spell.npcMaxDuration .." second maximum duration."..
            " This is to prevent NPCs from wasting magicka on effects they won't be alive long enough to benefit from."..
            "\n\nOnce spells are created in a save file, they are stored in all subsequent saves."..
            " Spells created this way are not removed if this setting is later disabled."..
...
local function createMainHomePage(template)
    ...
    page.sidebar:createInfo({ text =
        "Always reload (start a new game or load any save) after making changes to this mod's settings."..
        "\n\nIf a setting tells you a restart is required, restart your game after making all desired changes."
    })
