...
local function getEffectCost(effect, isCustomSpell, useUnmodifiedBaseMagickaCost)
    ...
    if useUnmodifiedBaseMagickaCost then
        -- nil check handles effects from uninstalled mods
        local unmodifiedMagicEffect = common.getUnmodifiedMagicEffect(effect.id)
        if unmodifiedMagicEffect ~= nil then baseMagickaCost = unmodifiedMagicEffect.baseMagickaCost end
    end
...
local function getMinMax(effectId, magnitude)
    ...
    if variance == 0 or magnitude < 2 then
    ...
    elseif variance == 100 then
        ...
        local roundTo = 1
        if magnitude >= 40 then roundTo = 10
        elseif magnitude >= 10 then roundTo = 5 end
...
local function getEffectDurMinMax_FromMagickaCost(effectId, magickaCost, minDuration)
    ...
    duration = util.clamp(duration, minDuration, nil)
...
local function addStartSpell(effectId)
    ...
    local isCurrent = gameConfig.spell.addStartSpells and gameConfig.spell.startEffectIds[effectId]
...
local function addNpcSpells(effectId)
    ...
    for _, rangeType in ipairs(rangeTypes) do
        for tier = 1, gameConfig.spell.npcTierCount do
            ...
            -- keep updating spells that already exist in the game
            local isCurrent = gameConfig.spell.addNpcSpells and gameConfig.spell.npcEffectIds[effectId]
            local alreadyExists = tes3.getObject(spellId) ~= nil
            ...
            if rangeType == tes3.effectRange.target then
            ...
            or effectId == tes3.effect.absorbMagicka then
                ...
                -- slow down effects to give players more time to react
                minDuration = gameConfig.spell.npcMinDuration

            end

            local durMinMax = getEffectDurMinMax_FromMagickaCost(effectId, magickaCost, minDuration)
            local duration, min, max = table.unpack(durMinMax)

            -- limit NPC max duration to 60 so they don't waste magicka
            duration = util.clamp(duration, nil, gameConfig.spell.npcMaxDuration)

            if effect.id == tes3.effect.paralyze then

                -- being paralyzed sucks, max duration of 10 seems reasonable
                duration = util.clamp(duration, nil, gameConfig.spell.npcMaxDurationParalyze)

            elseif effect.id == tes3.effect.blind then

                -- being 100% blind sucks, max magnitude of 75 seems reasonable
                min = util.clamp(min, nil, gameConfig.spell.npcMaxMagnitudeBlind)
                max = util.clamp(max, nil, gameConfig.spell.npcMaxMagnitudeBlind)

            elseif effect.id == tes3.effect.sound then

                -- high sound magnitudes are incredibly loud, 150 is manageable
                min = util.clamp(min, nil, gameConfig.spell.npcMaxMagnitudeSound)
                max = util.clamp(max, nil, gameConfig.spell.npcMaxMagnitudeSound)
