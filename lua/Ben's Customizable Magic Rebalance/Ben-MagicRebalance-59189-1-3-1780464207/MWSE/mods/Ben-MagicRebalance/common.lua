...
local function logEffect(effect, prefix)
    ...
    log(format, prefix, effectName, table.unpack(values))
...
local function logEffects(magicSource, prefix)
    ...
    for _, effect in ipairs(magicSource.effects) do
        if effect.object ~= nil then logEffect(effect, prefix) end
    end
