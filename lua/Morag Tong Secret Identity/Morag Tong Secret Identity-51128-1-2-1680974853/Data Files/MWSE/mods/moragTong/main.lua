...
local function isWritTarget(reference)
	...
	-- mwse.log(inspect(common.config.writTargets))

	-- mwse.log(reference.baseObject.id)
...
local function onCrimeWitnessed(e)
	...
	if not crimeTimestamp and tes3.mobilePlayer.bounty > 0 then
	...
	if crimeTimestamp and tes3.getSimulationTimestamp() > crimeTimestamp + 0.02 then
		valid = false
	end
...
local function onInfoGetText(e)
	...
	if infoActor and tes3.player.data.moragTong.identityWitness[infoActor.id] then
		return
	end
...
local function onInfoFilter(e)
	...
	if not infoActor then
		infoActor = tes3ui.getServiceActor()
		infoActor = infoActor and infoActor.object or e.actor
	end
	...
	if infoActor and (infoActor.faction == moragTong or tes3.player.data.moragTong.identityWitness[infoActor.id]) then
		if globalVar then
			globalVar.value = 1
		end
		return
...
local function updateIdentity()
	...
	globalVar = tes3.findGlobal("MT_Identity")
end
