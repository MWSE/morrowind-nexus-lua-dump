...
local auto_close = defns.auto_close



function Container:check_auto_close()
	if config.auto_close == auto_close.never then 
		return false
	end
	if config.auto_close == auto_close.if_nonempty then
		-- can be closed if it has items, cant be closed if it has no items
		return not self.ref.isEmpty and #self.ref.object.inventory.items > 0
	end
	if config.auto_close == auto_close.always then 
		return true
	end
end
...
function Container:close()
	...
	if config.play_sound and anim_info.close_sound then
		if (tes3.getFileExists(anim_info.close_sound)) then
...
function Container:opened(show_contents_menu) 
	...
	if config.auto_close ~= defns.auto_close.never then
		...
		timer.start{duration = 0.5, callback=function()
			log("trying to close %s", self)

			if config.auto_close == defns.auto_close.if_nonempty 
			and (self.ref.isEmpty or #self.ref.object.inventory.items == 0)
			then
...
function Container:open(show_contents_menu) 
	...
	if config.play_sound and anim_info.open_sound then
		if (tes3.getFileExists(anim_info.open_sound)) then
...
function Container:can_close(check_auto_close)
	if check_auto_close and not self:check_auto_close() then return false end
