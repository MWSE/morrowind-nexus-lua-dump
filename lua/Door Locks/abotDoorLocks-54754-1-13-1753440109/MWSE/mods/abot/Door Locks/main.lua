...
'greathall','hut','mine','shack','storage'
}
...
' fletch','temple','food','emporium', 'store'}

local houseCells = {' house',' home',' hut',' shack', ' farm'}
...
local function isGuard(ref_object)
	if ref_object.isInstance then
		return ref_object.isGuard
	end
	if find2(string.lower(ref_object.id), string.lower(ref_object.name), 'guard') then
		return true
	end
	return false
end
...
local function isTooYoungToOpen(actorRef)
	if string.multifind(string.lower(actorRef.object.id),
			{'child','kid','boy','girl'}, 1, true) then
		return true
	end
	return false
end
...
local function checkActorRef(actorRef, destPosition, destCell)
	...
	or actorRef.isDead
	or isDummy(actorRef)
	or isSleeping(actorRef)
	or isTooYoungToOpen(actorRef) then
		return rec
	end
...
wiil return 0 which is evaluated true, else it will return nil)
			...
			and (not class.bartersClothing) then
				...
				if logLevel3 then
					mwse.log('%s.5-6: cell "%s" "%s" "%s" class "%s" detected npcType = %s %s',
						funcPrefix, destCellId, actorRefId, actorObj.name,
						class.id, rec.npcType, npcTypes[rec.npcType])
				end
				return rec
			end
		end
	...
	if class then
		...
		and string.multifind(string.lower(class.id),
			...
			if logLevel3 then
				mwse.log('%s.3: cell "%s" "%s" "%s" class "%s" detected npcType = %s %s',
					funcPrefix, destCellId, actorRefId, actorObj.name,
					class.id, rec.npcType, npcTypes[rec.npcType])
			end
			return rec
	...
	if (disposition >= minKnockDisposition)
	and ( not string.find(string.lower(actorObj.id), 'child', 1, true) ) then
		if logLevel4 then
	...
	if logLevel2 then
		...
		if ref then
			mwse.log('%s getBestKnownNPCrec("%s"): actor = "%s" "%s" npcType = %s %s',
				modPrefix, destCell.displayName, ref.id, ref.object.name,
...
local function isPubLike(lcCellId)
	if string.multifind(lcCellId, pubLikeCells, 1, true)
	and ( not string.multifind(lcCellId, {'hut','keeper'}, 1, true) ) then
		if logLevel2 then
			mwse.log('%s isPubLike("%s") = true', modPrefix, lcCellId)
		end
		return true
	end
	return false
...
local function isHouse(lcCellId)
	if string.multifind(lcCellId, houseCells, 1, true)
	and ( not string.find(lcCellId, 'lighthouse', 1, true) ) then
		if logLevel2 then
...
local function getCellPrefix(cell)
	...
	if cell.isInterior then
		...
		s = cell.editorName -- e.g. "Narsis, Foreign Quarter (6, -52)"
		i = string.find(s, ' (', 1, true)
		---mwse.log([[>>> getCellPrefix("%s") string.find(s, ' (', 1, true) = %s]], s, i)
	end
	if i
	and (i > 1) then
		s2 = string.sub(s, 1, i - 1)
	else
		s2 = s
	end
	local s3 = string.gsub(s2, '%s?[Rr]egion', '')
	---mwse.log([[>>> %s getCellPrefix("%s") = "%s"]], modPrefix, s, s3)
	return s3
end
...
local function isIllegalResting(cell)
	...
	for ref in cell:iterateReferences(tes3_objectType_activator) do
		if not ref.disabled then
			local script = ref.object.script
			if script
			and string.find(string.lower(script.id), 'bed', 1, true) then
...
local function mobileDeactivated(e)
	...
	local ab01drlkStart -- e.g. {int_x, int_y, int_z, deg_angle}
...
local function activate(e)
	...
	local function delayedActivate(withKnockSound)
		...
		local refHandle = tes3.makeSafeObjectHandle(targetRef)
	...
	if lcDestCellId == lcDoorCellId
	and not string.find(string.lower(obj.id),'trap', 1, true)
	and not string.find(string.lower(obj.name),'trap', 1, true) then
		if logLevel1 then
			mwse.log('%s: skipping non-trapdoor leading to the same "%s" cell',
	...
	and string.multifind(lcDoorCellId, destCellBlackList, 1, true) then
		if logLevel1 then
			mwse.log('%s: skipping door in blacklisted cell "%s"',
				funcPrefix, doorCell.editorName)
		end
		local data = targetRef.data
		if data
		and data.ab01drlkhp then
			data.ab01drlkhp = nil
		end
		return
	end
	...
	local destCellIsPubLike = isPubLike(lcDestCellId)
	...
	if hasDestSameLocationPrefix then
		...
		if doorCell.isOrBehavesAsExterior then
			if logLevel3 then
				mwse.log('%s: "%s" -> "%s" hasDestSameLocationPrefix = true, doorCellisOrBehavesAsExterior = true, skip = false',
...
-- 1 'Not found', 2 'NPC', 3 'Trader/Service', 4 'Faction member', 5 'Publican', 6 'Bed renting'
	...
	elseif npcType == 3 then -- trader/service
		someoneInside = true
		newLockLevel = hardLockLevel
		if destCellIsPubLike then
			someoneCanOpen = true
		elseif destCellIsHouse then
	...
	elseif npcType == 2 then -- house like
		...
		if destCellIsPubLike then
			isOpenTime = (hour >= maxPubCloseHour)
				or (hour <= minPubCloseHour)
			if logLevel3 then
				mwse.log('%s: isOpenTime = %s (door to "%s" close time set between %s and %s)',
					funcPrefix, isOpenTime, destCell.id, minPubCloseHour, maxPubCloseHour)
			end
...
[["Please, I really need to enter!"]],[["Let me in!"]]
		...
		if someoneInside then
			if not someoneCanOpen then
				if math.random(100) >= 80 then
					...
					if (hour > 6)
					and (hour < 7) then
						knockNotify([["It's probably too early in the morning."]])
					else
	...
	and (not destIsTent) then
		...
		if someoneCanOpen then
			...
			if ( not isIllegalResting(destCell) )
			and (not destCellIsPubLike)
			and (not destCellIsService)
			and (not destCellIsHouse) then
				if logLevel1 then
...
local function modConfigReady()
	...
	local sideBarPage = template:createSideBarPage{
		label = modName,
		showHeader = false,
		description = [[Dynamic loading doors locking according to time of day cycle, location, faction, disposition...
...
Also can use keys contained in a recognized keyring while opening doors.]],
	...
	sideBarPage:createYesNoButton({
		label = 'Enabled',
		description = [[Enable the mod effects.
...
Also if you do it while sneaking you can add/remove the door/container reference to/from the mod blacklist to make the mod ignore it.]],
	...
	sideBarPage:createInfo({text = string.format('House doors opening between %s and %s',
		minOpenHour, maxOpenHour)})
	sideBarPage:createSlider({
		label = 'Min house door opening hour %s',
		description = [[Minimun door opening hour for house-like places.]],
		configKey = 'minOpenHour'
		,min = 5, max = 9
	})
	sideBarPage:createSlider({
		label = 'Max house door opening hour %s',
		description = [[Maximum door opening hour for house-like places.]],
		configKey = 'maxOpenHour'
		,min = 17, max = 21
	})

	sideBarPage:createSlider({
		label = 'Max house door knock hour %s',
		description = [[Maximum door knocking hour for house-like places.]],
		configKey = 'maxKnockHour'
		,min = minKnockHour, max = 23
	})

	sideBarPage:createInfo({text = string.format('Service doors locking between %s and %s',
		minPubCloseHour, maxPubCloseHour)})
	sideBarPage:createSlider({
		label = 'Min service door locking hour %s',
		description = [[Minimun door locking hour for tavern-like places (having a publican or trade service NPC).]],
		configKey = 'minPubCloseHour'
		,min = 0, max = 4
	})

	sideBarPage:createSlider({
		label = 'Max service door locking hour %s',
		description = [[Maximum door locking hour for tavern like places (e.g. having a publican or trade service NPC).]],
		configKey = 'maxPubCloseHour'
		,min = 5, max = 9
	})

	sideBarPage:createSlider({
		label = 'Min lock/unlock interval (%s hours)',
		description = [[Minimum hours interval between scheduled doors lock/unlock.]],
		configKey = 'lockUnlockIntervalHour'
		,min = 1, max = 24
	})
	...
	sideBarPage:createDropdown{
		label = 'Skip Scripted:',
		options = getOptions(),
		description = [[Set if/when to skip processing vanilla scripted doors.
Note:
changes related to "LD/LCV Nighttime locks vanilla scripts" option are effective only on next Morrowind.exe restart.]],
		configKey = 'skipScript'
	}

	sideBarPage:createYesNoButton({
		label = 'Skip Persistent',
		description = [[Skip processing persistent doors.]],
		configKey = 'skipPersistent'
	})

	sideBarPage:createYesNoButton({
		label = 'Skip Chargen',
		description = [[Skip processing Chargen doors.]],
		configKey = 'skipChargen'
	})

	sideBarPage:createYesNoButton({
		label = 'Skip Guards',
		description = [[Do not allow guard NPCs to open doors for player.]],
		configKey = 'skipGuard'
	})

	sideBarPage:createYesNoButton({
		label = 'Use Keyring',
		description = [[Try and use keys contained in a recognized keyring while opening doors.
Currently working with MWSE Containers and Inventory Helpers keyrings.]],
		configKey = 'keyRing'
	})
...
local onlyEffective = '\nOnly effective when "Knock on doors" is enabled.'
	...
	sideBarPage:createYesNoButton({
		...
		description = [[Enable knocking on doors (e.g. doors to player Guilds).]],
		configKey = 'knockOn'
	})

	sideBarPage:createYesNoButton({
		label = 'Lock empty houses',
		description = [[When enabled empty houses/homes will be locked when nobody's home regardless of the hour.]]..onlyEffective,
		configKey = 'lockEmptyHouses'
	})

	sideBarPage:createYesNoButton({
		label = 'Allow knock for service/vendors',
		description = [[Enable service/vendor actors to open doors (e.g. doors to pawnbrokers, but also to temple if player is not the same faction but there is some vendor/service inside).]]..onlyEffective,
		configKey = 'serviceKnock'
	})

	sideBarPage:createSlider({
		label = 'Max door knock hearing distance',
		description = [[Maximum distance of a friendly NPC from door destination marker to be able to hear player knocking on the door.]]..onlyEffective,
		configKey = 'maxHeardKnockDist'
		,min = 256, max = 8192, jump = 16
	})

	sideBarPage:createSlider({
		label = 'Max door knock vertical hearing distance',
		description = [[Maximum vertical/Z axis distance of a friendly NPC from door destination marker to be able to hear player knocking on the door.]]..onlyEffective,
		configKey = 'maxHeardKnockZDist'
		,min = 92, max = 1024, jump = 16
	})

	sideBarPage:createSlider({
		label = 'Max Knock Fight',
		description = [[Maximum NPC Fight setting to be still considered friendly/willing to open to a door-knocking player.]]..onlyEffective,
		configKey = 'maxKnockFight'
		,min = 75, max = 100, jump = 5
	})

	sideBarPage:createSlider({
		label = 'Min Knock Disposition',
		description = [[Minimum friendly NPC base object disposition to open to a door-knocking player.
Only effective for services/home-like interiors and only when "Knock on doors" is enabled.]],
		configKey = 'minKnockDisposition'
		,min = 30, max = 100, jump = 5
	})

	sideBarPage:createSlider({
		label = 'Knock Notify Perc',
		description = [[Percent probability of player/npc saying something when player knocks on door.]]..onlyEffective,
		configKey = 'knockNotifyPerc'
		,min = 0, max = 100, jump = 5
	})

	sideBarPage:createYesNoButton({
		label = 'Move the actor opening the door',
		description = [[Move the actor opening the door. Make it more immersive in places like 2 story houses/castles where sometimes nearby actors are not visible.]]..onlyEffective,
		configKey = 'moveOpeningActor'
	})

	sideBarPage:createYesNoButton({
		label = 'Check script context',
		description = [[Useful to check for bed renting services.
Try disabling it only in case of crashes while activating a door.]],
		configKey = 'checkScriptContext'
	})
