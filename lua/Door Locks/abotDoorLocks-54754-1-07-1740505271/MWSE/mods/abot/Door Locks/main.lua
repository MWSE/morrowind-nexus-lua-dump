...
-- it will get/set 0 as value
	...
	return value -- usually 0 if present, else nil
end
...
'council hall','district','plaza','square',
'works','market'}
	...
	if logLevel2 then
		if rec.ref then
			mwse.log('%s getBestKnownNPCrec("%s"): actor = "%s" "%s" npcType = %s',
				modPrefix, destCell.displayName, rec.ref.id, rec.ref.object.name, rec.npcType)
		else
			mwse.log('%s getBestKnownNPCrec("%s"): npcType = %s',
				modPrefix, destCell.displayName, rec.npcType)
		end
	end
	return rec
end
...
local lastOpenerHandle, lastOpenerPos

local function cellChangedResetLastOpenerPos(e)
	local lastOpenerRef = handleToRef(lastOpenerHandle)
	if not lastOpenerRef then
		return
	end
	if not lastOpenerPos then
		return
	end
	if not e.previousCell then
		return
	end
	if not lastOpenerRef.cell then
		return
	end
	if not (e.previousCell == lastOpenerRef.cell) then
		return
	end
	if logLevel2 then
		mwse.log('%s cellChangedResetLastOpenerPos(): "%s".position reset to "%s" %s',
			modPrefix, lastOpenerRef.id, lastOpenerRef.cell.id, lastOpenerPos)
	end	
	lastOpenerRef.position = lastOpenerPos
end
...
isOpenTime = %s, someoneInside = %s, someoneCanOpen = %s, npcType = %s %s]],
	...
	if ab01drlkhp then
		local hoursPassedSinceLastLockedOrUnlocked = hoursPassedSinceGameStart - ab01drlkhp
		if hoursPassedSinceLastLockedOrUnlocked < lockUnlockIntervalHour then
			if logLevel1 then
				...
				mwse.log('%s: door dest = "%s", hoursPassedSinceLastLockedOrUnlocked = %s < %s, skip',
					funcPrefix, getCellId(destCell), hoursPassedSinceLastLockedOrUnlocked, lockUnlockIntervalHour)
			end
			...
			if knockNotifyPerc >= i then
				...
				if lastOpenerRef then
					...
					if config.moveOpeningActor then
						lastOpenerPos = lastOpenerRef.position:copy()
						lastOpenerHandle = tes3.makeSafeObjectHandle(lastOpenerRef)
						event.register('cellChanged',
							cellChangedResetLastOpenerPos, {doOnce = true})
-- multiply vector by scalar see https://mwse.github.io/MWSE/types/tes3vector3/#multiplication 
						local pos = (destMarker.forwardDirection * 96) + destMarker.position
						if logLevel2 then
							mwse.log('%s knockUnlockAndActivateDoor(): "%s".position set to "%s" %s',
								modPrefix, lastOpenerRef.id, lastOpenerRef.cell.id, pos)
						end							
						lastOpenerRef.position = pos
					end
				end
				if i > 75 then
				...
				elseif lastOpenerRef then
					...
					if npcType <= 4 then
						tes3ui.showNotifyMenu([["It's me, %s!"]], playerName)
					else
						tes3ui.showNotifyMenu([[%s: "Ah, it's you. Welcome, %s."]], actorName, playerName)
					end
				end
			end
		end
