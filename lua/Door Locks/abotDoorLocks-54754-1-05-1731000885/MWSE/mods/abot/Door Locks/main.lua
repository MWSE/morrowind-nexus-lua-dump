...
-- it will get/set 0 as value
	...
	if logLevel4 then
		mwse.log('%s: context = %s', funcPrefix, context)
	end
	---local context = ref['context'] -- check at least if variable exists
	local value = context[variableId]
	if value then
		if logLevel3 then
			mwse.log('%s: context["%s"] = %s)', funcPrefix, variableId, value)
		end
		---assert(value == context[string.upper(variableId)]) -- just to be sure it is case insensitive
...
local doorWhiteList = {'barrack','colony','door','entrance','greathall','hut','shack'}
...
variable value is not relaiable.
...
local function activate(e)
	...
	local hoursPassedSinceGameStart = getInGameHoursPassedSinceGameStart()
					...
					elseif config.keyRing then
						for keyRingId, keyRingContainerId in pairs(keyRings) do
							if hasKeyFromKeyRing(inventory, keyId, keyRingId, keyRingContainerId) then
								...
								targetRef.data.ab01drlkhp = hoursPassedSinceGameStart
								delayedActivate()
								return 2
							end
						end
	...
	and level then
		...
		elseif level == 1 then
			if logLevel1 then
				mwse.log('%s: skipping door/container marked as openable (locked 1, then locked/unlocked by e.g. Revolving Doors Block mod)', funcPrefix)
			end
			return
		elseif data then
			...
			and (not data.ab01drlkhp) then
				...
				data.ab01drlkhp = hoursPassedSinceGameStart
				return
			end
		end
		if logLevel3 then
...
isOpenTime = %s, someoneInside = %s, someoneCanOpen = %s, npcType = %s %s]],
	...
	if lockNode then
		if hasAlreadyBeenLockedOrUnlockedToday then
