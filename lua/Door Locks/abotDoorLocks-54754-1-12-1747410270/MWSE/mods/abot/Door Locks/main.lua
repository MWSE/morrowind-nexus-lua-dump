...
Enable knocking on doors (e.g. doors to player Guilds).]], 'knockOn'),
	...
	controls:createYesNoButton({
		...
		description = getYesNoDescription([[Default: %s.
When enabled empty houses/homes will be locked when nobody's home regardless of the hour.]]..onlyEffective, 'lockEmptyHouses'),
	...
	controls:createYesNoButton({
		...
		description = getYesNoDescription([[Default: %s.
Enable service/vendor actors to open doors (e.g. doors to pawnbrokers, but also to temple if player is not the same faction but there is some vendor/service inside).]]..onlyEffective, 'serviceKnock'),
...
Percent probability of player/npc saying something when player knocks on door.]]..onlyEffective, 'knockNotifyPerc'),
	...
	controls:createYesNoButton({
		...
		description = getYesNoDescription([[Default: %s.
Move the actor opening the door. Make it more immersive in places like 2 story houses/castles where sometimes nearby actors are not visible.]]..onlyEffective, 'moveOpeningActor'),
