...
function mcm:onCreate()
	...
	local function createColorSliders(parent, elementName)
        return{
            parent:createSlider{
                label = "Красный: %s%%",
                description = string.format("Задает уровень красного цвета в элементе.\n\nПо умолчанию: %d", config.default[elementName].color.r),
	...
	highlightCategory.description = "Выделяет элемент интерфейса, редактируемый в данный момент. Виден только в меню конфигурации, при наведении курсора на различные настройки."
	...
	modsPage:createYesNoButton{
		...
		description = string.format("Эта опция определяет, будут ли храниться настройки сторонних модификаций в файле конфигурации Настройщика интерфейса,  после их удаления. Если установлено значение \"Нет\" настройки будут сохранены.\n\nПо умолчанию: %s", booleanToReadableString(config.default.deleteInvalidModConfigs)),
