...
local Weapon={Weapons[1].DefaultWeapon,Weapons[1].DefaultWeapon}
...
local CreditUI=ui.create({ layer = 'Windows', type = ui.TYPE.Text, template=I.MWUI.templates.textHeader, props = {Tempo=2, textSize=50*TextRatio, visible=false, autoSize=true, text="CREDIT(S)  "..Credit, relativePosition = util.vector2(0.5,0.95), anchor = util.vector2(0.5, 0.5),}})
local Player2RequestUI=ui.create({ layer = 'Windows', type = ui.TYPE.Text, template=I.MWUI.templates.textHeader, props = {Tempo=2, textSize=50*TextRatio, visible=false, autoSize=true, text="PRESS START BUTTON", relativePosition = util.vector2(0.85,0.95), anchor = util.vector2(0.5, 0.5),}})
...
local Cursor={	ui.create({ layer = 'Windows', type = ui.TYPE.Image, props = {color=util.color.rgb(1,1,1),visible=false, relativeSize = util.vector2(1/25, 1/20), relativePosition = util.vector2(0.5, 0.5), anchor = util.vector2(0.5, 0.5), resource = ui.texture { path = 'textures/crosshair.png' , }, },}),
						...
						if Weapons[Player].spellOnHitRecord[Weapon[Player]] then
							...
							for i, effect in pairs(core.magic.spells.records[Weapons[Player].spellOnHitRecord[Weapon[Player]]].effects) do
								if effect.area>1 then
									core.sendGlobalEvent("SpawnVfx",{model=types.Static.records[effect.effect.areaStatic].model, position=Ray.hitPos, options={scale=effect.area*self.scale}})
									if not(NearbyActors[Player]) then
										...
										for j, actor in pairs(NearbyActors) do
											if (Ray.hitPos-actor.position):length()<effect.area*80*self.scale then
												local attack = {
													attacker = self,
													weapon = nil,
													sourceType = I.Combat.ATTACK_SOURCE_TYPES.Melee,
							...
							core.sendGlobalEvent("SpawnVfx",{model=types.Static.records[core.magic.effects.records["firedamage"].areaStatic].model, position=Ray.hitPos, options={scale=0.05*self.Scale,useAmbientLight=false}})
						end
					end
...
local function Move(Target,dt)
	...
	elseif TargetAngle.y<0 then
		self.controls.pitchChange = -dt/2
	elseif TargetAngle.y<0.45 then
		self.controls.pitchChange = -dt/5
	elseif TargetAngle.y>0.55 then
		self.controls.pitchChange = dt/5
	end
	
	if (util.vector2(self.position.x,self.position.y)-util.vector2(Target.position.x,Target.position.y)):length()<100*self.scale then
...
local function onUpdate(dt)
	if RS_Gameplay==true and dt>0 then
--		print(CurrentPathPoint)
		...
		if (not(types.Actor.getEquipment(self,types.Actor.EQUIPMENT_SLOT.Pants)) or types.Actor.getEquipment(self,types.Actor.EQUIPMENT_SLOT.Pants).recordId~="invisiblecloth") and types.Actor.inventory(self):find("InvisibleCloth") then
			local Equipment={}
			--print(types.Actor.getEquipment(self,types.Actor.EQUIPMENT_SLOT.Pants),types.Actor.getEquipment(self,types.Actor.EQUIPMENT_SLOT.Pants).recordId,types.Actor.inventory(self):find("InvisibleCloth"))
			Equipment[types.Actor.EQUIPMENT_SLOT.Pants]="InvisibleCloth"
			types.Actor.setEquipment(self,Equipment)
		end
...
local function StopRS()
	...
	for i, player in pairs(BulletsUI) do
		for j, ui in pairs(player) do
			ui:destroy()
		end
	end
	BulletsUI={{},{}}

	for i, player in pairs(HealthUIs) do
		for j, ui in pairs(player) do
			ui:destroy()
		end
	end
	...
	BossBar.layout.props.visible=false
	BossBar:update()

	BloodSpatter[1].layout.props.visible=false
	BloodSpatter[1]:update()

	BloodSpatter[2].layout.props.visible=false
	BloodSpatter[2]:update()

	core.sendGlobalEvent("RSRemove",{object=types.Actor.inventory(self):find("invisiblecloth")})
