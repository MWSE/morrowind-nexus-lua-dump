...
local cnt = require('Scripts.Scribo.container')
...
local function removeItemByID(itemId)
    cnt.remove(itemId, 1)
end
...
local function initMisc()
    ...
    elseif #global.inkwells == 3 then
        local template = cmn.inkwellTemplates[4]
        local inkwellId = createMisc(template)
        table.insert(global.inkwells, {
            id = inkwellId,
            colorName = template.colorName,
...
local function updateInkwellStatus(itemid)
    ...
    local inkwellsCount = cnt.count(itemid) --inventory:countOf(itemid) or 0
...
local function isWritableBook(bookid)
    local bookRecord = types.Book.record(bookid)
    -- if uncontrolEdit and cmn.isGenBook(bookRecord.id) then
    --     return true
    -- end
...
local function hasWritingMaterials()
    -- local inventory = types.Actor.inventory(player)
    local hasQuill = cnt.find(cmn.quillID) -- (inventory:find(cmn.quillID) ~= nil)
    local hasInkwell = cnt.find(cmn.inkwellID) -- (inventory:find(cmn.inkwellID) ~= nil)

    if not hasInkwell and not hasQuill then
        player:sendEvent('scrShowMessage', {
...
local function hasPages(bookid)
    ...
    --local inventory = types.Actor.inventory(player)
    local hasCleanPage = cnt.find(cmn.cleanPageID) --inventory:find(cmn.cleanPageID)
    local hasEmptyScroll = cnt.find(global.emptyScrollId) --inventory:find(global.emptyScrollId)
...
local function chooseOrigamiModel()
    ...
    for _, ink in pairs(global.inkwells) do
        availableInks[ink.colorName] = cnt.find(ink.id)
    end
    -- Добавляем базовые черные чернила
    availableInks["black"] = cnt.find(cmn.inkwellID)
    ...
    for _, model in ipairs(global.origami) do
        ...
        for color, hasInk in pairs(availableInks) do
            ...
            if not hasInk and string.find(model.model, color, 1, true) then
                --print("Модель ", model.model, " не хватает цвета ", color)
                canUse = false
                break
            end
        end
...
local function removePageForScroll(bookid)
    ...
    if bookRecord.enchant then -- магические книги и свитки требуют качественной страницы
        cnt.remove(global.emptyScrollId, 1)
    elseif cnt.find(cmn.cleanPageID) then --cleanPage then -- если есть обычная страница используем ее
        cnt.remove(cmn.cleanPageID, 1)
...
local function removeMaterialForBook(bookid)
    ...
    if cnt.count(cmn.cleanPageID) >= 2 then
    ...
    for _, material in ipairs(materials) do
        if cnt.count(material.id) < material.count then
            player:sendEvent('scrShowMessage', {
    ...
    -- Удаляем материалы
    for _, material in ipairs(materials) do
        cnt.remove(material.id, material.count)
    end
...
local function speedUpTime()
    core.sound.playSoundFile3d('Sound/Scribo/snd_write', player)
...
local function maxLengthText(bookid)
    ...
    if bookRecord.isScroll then
        local icon = bookRecord.icon:lower()  -- приводим к нижнему регистру для надёжности

        if bookRecord.id == cmn.dirtyPageID or endsWith(icon, "ic_sc_drty_txt.dds") then
            maxLengthText = 200
        elseif bookRecord.id == cmn.cleanPageID or endsWith(icon, "ic_sc_rlld_txt.dds") then
            maxLengthText = 660
        elseif bookRecord.id == global.emptyScrollId 
            or endsWith(icon, "ic_sc_fine_blnk.dds") 
            or endsWith(icon, "ic_sc_fine_txt.dds") then
            maxLengthText = 1320
        end
    end
...
local function isGoodDisposition(actor)
    ...
    local class = ""
    if actor ~= nil and types.NPC == actor.type then
        class = types.NPC.record(actor).class
    end
...
local function checkFail(chanceRuin, chanceDestroy, bookId, showMessage)
    ...
    if chance <= chanceRuin then
        ...
        if showMessage then
            player:sendEvent('scrShowMessage', { message = msg("failPage") })
        end
        return false
    end
...
local function writeBook(data)
    ...
    if isWritableBook(bookRecord.id) then -- если это книга или свиток для записи
        -- print("scribo: write book or scroll")
        if not hasWritingMaterials() then
            return
        end
        -- if bookRecord.id ~= global.emptyBookId and not bookRecord.isScroll and not hasPages(bookRecord.id) then
        --     return
        -- end
        ...
        if not allowDup then
            local bounty = types.Player.getCrimeLevel(player)+bookRecord.value
            crimes.commitCrime(player, {
                arg = bookRecord.value,
                type = player.type.OFFENSE_TYPE.Theft,
                victim = owner,
                victimAware = true
            })

            local newBounty = types.Player.getCrimeLevel(player)
            if bounty > newBounty then
                types.Player.setCrimeLevel(player, bounty)
            end
...
local function checkPenState()
    ...
    if math.random(1, 100) <= 5 then
        ...
        if cnt.find(cmn.quillID) then
            cnt.remove(cmn.quillID, 1)
            --            if penCount == 0 then
            player:sendEvent('scrShowMessage', {
...
local function unpdateAvailableColors(bookid)
    ...
    if bookid == cmn.dirtyPageID or types.Book.record(bookid).model == cmn.scrollTemplates.dirty.model and
...
local function updateTradersItems(data)
    ...
    local baseItems = {{
        ...
        count = 3,
        interval = 3,
        traders = {'jobasha', 'dorisa darvel', 'codus callonus', 'simine fralinie', 'artalamel', 'elar fandas', 'felisel gavos', 'edheldur telrehn', 'mogduk gro-atulg', 'bolnora omani	', 'farel sadrethi', 'amalie perenne', 'uleni tenandus', 'sevaen ondyn', 'marilus arjus', 'relvina llaren', 'tilresi', 'uttrashi', 'galasa savas', "jo'agghu"},
        max_ingredients = 5
    }, -- Добротный лист
    {
