...
{
    icon = "icons/scribo/ic_ink_sphr.dds",
    name = msg("sphrInk"),
    model = "meshes/scribo/m_ink_sphr.nif",
    colorName = "blue",
    colorNameShort = "b",
    color = "3f5896",
    value = 30,
...
local function colorHex2Name(hexColor)
    ...
    for _, template in ipairs(inkwellTemplates) do
        if template.color == hexColor then
            return template.colorNameShort
        end
    end
...
local function colorName2Hex(str, availableColors)
    ...
    if colorName == "blk" or colorName == "bk" then
        colorName = "black"
    end
