...
local common = hlib.import("common")    ---@type herbert.QS.common
        ...
        if stack.variables then
            for _, item_data in ipairs(stack.variables) do
                if item_data.soul then
                    ...
                    table.insert(sg_options, Option.new{ data=item_data, item=stack.object, count=item_data.count or 1,
...
function get_options.tools()
    ...
    for _, tbl in pairs(tools_by_obj_type) do
        table.sort(tbl, function (a, b)
            ...
            if a_cond ~= b_cond then
                if a_cond and b_cond then return a_cond > b_cond
                elseif a_cond then return false
                elseif b_cond then return true
                end
            end
    ...
    local total_tiles = cfg.num_cols * cfg.num_rows 
