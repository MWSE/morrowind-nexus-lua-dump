...
local Menu = hlib.import("QS_Menu") ---@type herbert.QS.Menu
...
local metadata = (hlib.get_active_mod_info() or {}).metadata
...
local function sanitize_player_data()
    ...
    if player_data.version ~= nil then return end
...
local function update_recently_equipped(item)
    ...
    for i = cfg.num_rows * cfg.num_cols + 1, #recent_item_ids do
...
local function make_recents_options()
    ...
    -- log("options = %s", require("inspect").inspect, options)
    return options
...
-- if tes3.isInitialized() then initialized() end

-- if tes3.player then loaded() end
...
event.register("modConfigReady", function (e)
    ...
    ui_cat:new_slider{id="num_rows", label="Number of rows",
        min=1, max=15, jump=3,
        desc="Quick select menus will have this many rows."
    }

    ui_cat:new_slider{id="num_cols", label="Number of columns",
        min=1, max=15, jump=3,
    ...
    local function make_tab_options(p)
        ...
        local underline_rect, bg_leave_rect, bg_over_rect ---@type tes3uiElement?, tes3uiElement?, tes3uiElement?
        ...
        color_opts.component.postCreate = function(self)
            log("creating color rectangle for %s", p.label)
            local block = self.elements.labelBlock
            if block then
                block.flowDirection = tes3.flowDirection.leftToRight
                block.childAlignY = 0.5
                underline_rect = block:createRect{color=p.config.color}
                ...
                local bg_rect_container = block:createRect{color={0, 0, 0}}
                ...
                bg_leave_rect = bg_rect_container:createRect{color=p.config.color}
                bg_leave_rect.widthProportional = 1.0
                bg_leave_rect.heightProportional = 1.0
                bg_leave_rect.alpha = cfg.option_leave_alpha

            end
            -- rect.absolutePosAlignX = 0.95
            -- rect.absolutePosAlignY = 0.05
            -- rect.height = 10
            -- rect.minHeight = 10
            -- rect.width = 10
            -- rect.minWidth = 10
        ...
        for i, c in ipairs({"red", "green", "blue"}) do
            color_opts:new_pslider{label=c, id=i, dp=1, callback=function(self)
                if underline_rect then
                    underline_rect.color = p.config.color
                ...
                if bg_over_rect then
                    ...
                    bg_over_rect.alpha = cfg.option_over_alpha
                    bg_over_rect.color = p.config.color
                ...
                if bg_leave_rect then
                    ...
                    bg_leave_rect.alpha = cfg.option_leave_alpha
                    bg_leave_rect.color = p.config.color
