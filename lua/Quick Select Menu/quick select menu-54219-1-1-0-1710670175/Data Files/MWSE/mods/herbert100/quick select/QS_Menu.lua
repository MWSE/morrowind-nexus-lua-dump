...
local Menu = Herbert_Class.new{
    fields={
        ...
        {"x_scale", default=1, converter=function (v) return math.clamp(v, 0.05, 1) end},
        {"y_scale", default=1, converter=function (v) return math.clamp(v, 0.05, 1) end},
        {"options", tostring=Herbert_Class_utils.premade.array_tostring},
        {"root"},
        {"tab_index", default=1, },
        {"tabs", tostring=Herbert_Class_utils.premade.array_tostring, },
        {"typed_index", factory=function() return 0 end},
    },
    ---@param self herbert.QS.Menu
    post_init=function (self)
        log:trace("initialized menu with tabs = %s", inspect, self.tabs)
        ...
        -- qs_root.minHeight = height * ROOT_UI_SCALE
        qs_root.height = root_height
        qs_root.minHeight = root_height
        qs_root.maxHeight = root_height

        qs_root.positionX = -root_width / 2
        qs_root.positionY = root_height / 2
        ...
        -- tab_container.alpha = 0.5
        -- tab_container.absolutePosAlignX = 0.5
        -- tab_container.absolutePosAlignY = 0
        -- tab_container.ignoreLayoutY = true
        -- tab_container.positionY = main_block.positionY + 25
        tab_container.minHeight = 25
        tab_container.maxHeight = 25
        tab_container.height = 25
        -- tab_container.autoHeight = true
        tab_container.childAlignY = 0.5
        -- tab_container.borderLeft = ROW_BORDER
        -- tab_container.borderRight = ROW_BORDER
        self.tab_container = tab_container
        tab_container.borderTop = cfg.border_rows
        tab_container.borderBottom = cfg.border_rows
        -- tab_container.borderLeft = cfg.border_cols
        -- tab_container.borderRight = cfg.border_cols
        ...
        -- main_block.absolutePosAlignX = 0.5
        -- main_block.absolutePosAlignY = 1
        main_block.width = root_width
        main_block.heightProportional = 1.0
        -- main_block.height = root_height - 25
        -- main_block.width =  width * UI_X_SCALE
        -- main_block.height =  height * UI_Y_SCALE

        self.main_block = main_block

        
        -- label.absolutePosAlignX = 0.5
        -- label.absolutePosAlignY = 0.2  
        -- main_block:updateLayout()
        qs_root:updateLayout()
        ...
        for k, tab in ipairs(self.tabs) do
            ...
            log:trace("made tab = %s", inspect, tab)

            underline = tab_rect:createRect{id=UIDs.tab_underline, 
                color=tab.color or {math.random(), math.random(), math.random()}
            }
            -- underline.widthProportional = 1.0
            -- underline.heightProportional = 1
            -- underline.heightProportional = 0.1
            ...
            -- underline.height = 0.1 * 30
        end
        -- tab_rect.borderRight = 0
        -- local label = tab_container:createLabel{text=label_text}
        -- label.widthProportional = 1.0
...
function Menu:make_option_block(index, parent)
    ...
    -- number_label.maxHeight = 18
    -- number_label.ignoreLayoutY = true
    -- number_label.positionY = - 0.05 * parent.width
    ...
    local label_text, icon_path, header_text
    if option then
    ...
    if icon_path then
        local icon = rect:createImage{id=UIDs.option_icon, path=icon_path}
        parent:updateLayout()
        -- icon.maxHeight = math.min(0.3 * rect.height, 20)
        -- icon.maxWidth = math.min(0.3 * rect.width, 20)
        icon.height = math.min(0.3 * rect.height, 70)
        icon.width = math.min(0.3 * rect.width, 70)
        icon:updateLayout()
    ...
    label.borderTop = math.max(5, 0.1 * parent.height)
    label.wrapText = true
    label.justifyText = tes3.justifyText.center
    label.borderLeft = math.min(8, math.max(2,0.05 * parent.width))
    label.borderRight = math.min(8, math.max(2,0.05 * parent.width))
    ...
    if header_text then
        ...
        -- header_label.borderTop = math.max(5, 0.1 * parent.height)
        -- header_label.width  = 30
        -- header_label.wrapText = true
        -- header_label.justifyText = tes3.justifyText.center
        header_label.absolutePosAlignX = 0.5
        header_label.absolutePosAlignY = 0.075
        -- header_label.borderLeft = math.min(8, math.max(2,0.05 * parent.width))
        -- header_label.borderRight = math.min(8, math.max(2,0.05 * parent.width))
        -- header_label.borderBottom = 3
    end
...
function Menu:fill_tab()
    ...
    for _=1, num_rows do
        ...
        for _=1, num_cols do
            ...
            -- block.paddingAllSides = 50
            
            row:updateLayout()
            -- log("block(%i, %i): w=%s, h=%s", i, j, block.width, block.height)
            -- log("spell = %s", spell)
            local rect = self:make_option_block(k, block)
            k = k + 1
            block:updateLayout()
        end
    end
...
function Menu:get_option_block(index)
    ...
    log("found block %q, have row = %s, col = %s", function() return  blk.children[3] and blk.children[3].text, row, col end)
    return blk
end
...
function Menu:change_tab(tab_index)
    ...
    -- tes3.player.data.herbert_QS.tab_index = tab_index

    -- use index to update tab
...
event.register(tes3.event.keyDown, arrow_key_left, {filter=tes3.scanCode.keyLeft})
event.register(tes3.event.keyDown, arrow_key_right, {filter=tes3.scanCode.keyRight})
event.register(tes3.event.keyDown, arrow_key_left, {filter=tes3.scanCode.a})
event.register(tes3.event.keyDown, arrow_key_right, {filter=tes3.scanCode.d})
event.register(tes3.event.mouseWheel, mouse_wheel)
event.register(tes3.event.keyDown, number_key_pressed)
