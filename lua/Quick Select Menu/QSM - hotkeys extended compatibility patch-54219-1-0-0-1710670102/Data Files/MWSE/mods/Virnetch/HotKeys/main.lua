...
local function quickEquip(type, num)
	if data[type][num].id == nil then return end
	...
	if data[type][num].isMagic then 	--Magic
		...
		tes3.mobilePlayer:equipMagic{source=tes3.getObject(id), equipItem=obj.objectType ~= tes3.objectType.spell}
	else								--Item
		tes3.mobilePlayer:equip{item=tes3.getObject(id)}
	end
end
...
local function registerModConfig()
	...
	page:createSlider{ label = "Hold/Double Tap Delay",
		description = "The time in seconds between key presses for it to be considered a double tap. \z
			Also changes the time that you need to hold a key down for the hold hotkey to be activated. Default: 0.4",
		min = .1, max = 2, step = .1, jump = .2, decimalPlaces=1,
		variable = mwse.mcm.createTableVariable{ id = "holdDelay", table = config },
	}

	page:createOnOffButton{ label = "Show All Menus",
		description = "Disabling this will show all menus on different pages/tabs that you can browse through. \z
			This should be disabled with lower resolutions or when using MGE XE UI scaling if the menus are not shown correctly.",
		variable = mwse.mcm.createTableVariable{ id = "showAll", table = config }
	}
	...
	page:createOnOffButton{
		...
		variable = mwse.mcm.createTableVariable{ id = "itemIconPosition", table = config }
	}
