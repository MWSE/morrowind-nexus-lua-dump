...
function this.onCreate(container)
	...
	mainPane.paddingAllSides = 6
		...
		createBooleanConfigPackage({
		parent = mainPane,
		label = "Merchants report crime when trading illegal items",
		config = this.config,
		key = "contrabandReport",
		tooltip = "If enabled, merchants with low disposition to the player character will report crime on unsuccessful attempt to trade an illegal item. Works only if previous option is enabled.",
	})
end
