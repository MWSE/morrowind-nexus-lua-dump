...
local function MOUSEBUTTONUP(e) if not tes3ui.menuMode() and e.button == 0 and ad.animationAttackState == 2 then	local dir = ad.attackDirection	if dir < 4 and not V.dfr then	local pass = MB[cf.mb] == 128
	if cf.auto or pass then		local w = mp.readiedWeapon		w = w and w.object		local vec = tes3.getPlayerEyeVector()	if vec.z < 0.9 then		local acr = mp:getSkillValue(20)*cf.acr/2000
		...
		local hit = tes3.rayTest{position = tes3.getPlayerEyePosition(), direction = vec, ignore = {p}}		local hitd = hit and hit.distance or 20000
		if (pass or (hit and hit.reference and hit.reference.mobile and not hit.reference.mobile.isDead)) and hitd > wr * cf.min then
			...
			if mp.fatigue.current > stc then	local vk = math.max(-0.4 - vec.z, 0)/3
				...
				tes3.findGMST("fCombatDistance").value = (DD + 50)/wr		tes3.findGMST("fCombatAngleXY").value = math.clamp(150/DD, 0.1, 0.5)
				...
				if hit and hitd > 500 and w and w.type == 4 and dir == 2 and ad.attackSwing > 0.95 and vec.z < -1 + acr and mp.velocity:length() > 0 then
					local dmg = w.chopMax * (1 + mp.strength.current/200) * math.max(mp.readiedWeapon.variables.condition/w.maxCondition, 0.3) * hitd/3000		local arm, fdmg, m
					timer.start{duration = 0.2/w.speed, callback = function() tes3.playSound{sound = table.choice({"endboom1", "endboom2", "endboom3"})}	--"fabBossLeft", "fabBossRight"
						for r in tes3.iterate(p.cell.actors) do m = r.mobile	if m and not m.isDead and 200 > hit.intersection:distance(r.position) then
							arm = GetArmor(m)	fdmg = dmg*dmg/(arm+dmg)	m:applyHealthDamage(fdmg)	CrimeAt(m)
							if cf.msg then tes3.messageBox("Hammer Slam! Height = %d   %s  Dmg = %.1f  (%d  Armor = %.1f)", hitd, r, fdmg, dmg, arm) end
						end end
					end}
				end
			end
