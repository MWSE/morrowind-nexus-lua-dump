...
function module.normalizeStartPackage(data)
    if not (data and data.type) then return nil, "missing_package_type" end

    local package = shallowCopy(data)
    if package.type == "Travel" then
        if package.destPosition == nil and package.destination ~= nil then
            package.destPosition = package.destination
        end
        package.destination = nil
        if package.destPosition == nil then return nil, "missing_travel_destPosition" end
...
function module.preservedStartBlockedByActivePackage(package, activePackage)
    if not (package and package.preserveExternalPackage == true and activePackage and activePackage.type) then
        return false, nil
    end

    if activePackage.type == "Wander" then return false, nil end

    if activePackage.type == "Travel" and package.type == "Travel" then
        local activeDest = module.packageDestination(activePackage)
        local packageDest = module.packageDestination(package)
        local radius = tonumber(package.destinationTolerance or package.ownTravelRadius or 120) or 120
        if near(activeDest, packageDest, radius) then return true, "preserve_matching_travel" end
        return true, "preserve_external_travel"
    end

    return true, "preserve_external_" .. tostring(activePackage.type)
end
