-- interactions/sitting/facingRefiner.lua
---@omw-context none
--
...
local function hitLooksLikeFocusSurfaceClutter(env, hitObject, hitPos, focusPos)
    if not (hitObject and hitPos and focusPos) then return false end
    local kind = clutterItems.kind(env, hitObject)
    if kind ~= "hard_blocker" and kind ~= "soft_item_blocker" and kind ~= "soft_surface" then return false end
    local distance = flatDistance(hitPos, focusPos)
    local dz = math.abs((hitPos.z or 0) - (focusPos.z or 0))
    return distance <= 140 and dz <= 96
end
...
local function seatSurfaceMaxDistance(category, kind)
    ...
    if kind == "lectern" then
        ...
        if category == "stool" or category == "barstool" then
            return 1050
        end
        return 1150
    end
    if kind == "grinder" then
        return 300
    end
    return 360
end

local function workSurfaceKind(kind)
    return kind == "table" or kind == "bar" or kind == "lectern" or kind == "grinder"
end

local function nearbyWorkSurfaceCandidate(sitPosition, category, facingCandidates, lectureAudienceTarget)
    if not sitPosition then return false end
    local best, bestDistance = nil, nil
    for _, candidate in ipairs(facingCandidates or {}) do
        ...
        if pos and workSurfaceKind(kind) and (not lectureAudienceTarget or kind == "lectern") then
            local distance = flatDistance(sitPosition, pos)
            if distance <= seatSurfaceMaxDistance(category, kind) and (not bestDistance or distance < bestDistance) then
                best = candidate
                bestDistance = distance
            end
        end
    end
    return best ~= nil, best, bestDistance
end
    ...
    for i = 0, 15 do
        ...
        if result.hit and result.hitObject and result.hitPos then
            ...
            elseif objectLooksLikeTableSurface(env, result.hitObject) then
                kind = objectMatchers.objectSurfaceKind(result.hitObject, env.profiles.objectModelPath, objectName(env, result.hitObject)) or "table"
            end
            if kind then
                local distance = flatDistance(sitPosition, result.hitPos)
                if distance >= 18 and distance <= seatSurfaceMaxDistance(category, kind) then
                    local score = distance
                    if kind == "bar" then
                        score = score + (category == "barstool" and -55 or 45)
                    end
                    if kind == "table" then score = score - 20 end
                    if kind == "lectern" then score = score + (category == "bench" and -45 or 60) end
                    if kind == "grinder" then score = score - 15 end
...
function M.refine(env, sitPosition, data, profile, currentObject)
    ...
    local lectureAudienceTarget = data.lectureAudienceTarget == true and data.facingKind == "lectern"
    local best, bestScore = nil, nil
    if not lectureAudienceTarget then
        best, bestScore = radialSurfaceCandidate(env, sitPosition, category, currentObject)
    end
    local hasNearbyWorkSurface, nearestWorkSurface, nearestWorkSurfaceDistance =
        nearbyWorkSurfaceCandidate(sitPosition, category, facingCandidates, lectureAudienceTarget)
    ...
    for _, candidate in ipairs(facingCandidates) do
        ...
        local kind = candidate and candidate.kind or nil
        if obj and pos
            and (kind == "table" or kind == "bar" or kind == "fire" or kind == "lectern" or kind == "grinder")
            and (not lectureAudienceTarget or kind == "lectern") then
            local distance = flatDistance(sitPosition, pos)
            local maxDistance = kind == "fire" and 260 or seatSurfaceMaxDistance(category, kind)
            if distance <= maxDistance then
                local skipFireFallback = kind == "fire" and hasNearbyWorkSurface == true
                if skipFireFallback then
                    if env.debugLog then
                        env.debugLog(
                            "sitting fire focus rejected nearby work surface",
                            "object", tostring(currentObject and currentObject.recordId),
                            "fire", tostring(obj and obj.recordId),
                            "fireDistance", tostring(distance),
                            "workSurface", tostring(nearestWorkSurface and nearestWorkSurface.object and nearestWorkSurface.object.recordId),
                            "workKind", tostring(nearestWorkSurface and nearestWorkSurface.kind),
                            "workDistance", tostring(nearestWorkSurfaceDistance),
                            "reason", "table_bar_work_surface_preferred"
                        )
                    end
                else
                local targetPos = pos
                ...
                if kind == "lectern" then score = score + (category == "bench" and -50 or 70) end
                if kind == "grinder" then score = score - 18 end
                ...
                if kind ~= "fire" then
                    ...
                    if result.hit and result.hitObject and result.hitPos then
                        ...
                        elseif (kind == "table" or kind == "bar" or kind == "grinder")
                            and hitLooksLikeFocusSurfaceClutter(env, result.hitObject, result.hitPos, pos) then
                            targetPos = result.hitPos
                            surfaceHit = true
                            score = score - 20
                        elseif kind == "lectern" or kind == "table" or kind == "bar" or kind == "grinder" then
                            blockedFocus = true
                            if env.debugLog then
                                env.debugLog(
                                    "sitting focus candidate rejected blocked",
                                    "object", tostring(currentObject and currentObject.recordId),
                                    "focus", tostring(obj and obj.recordId),
                                    "kind", tostring(kind),
                                    "hit", tostring(result.hitObject and result.hitObject.recordId),
                                    "distance", tostring(distance),
                                    "reason", "blocked_or_occluded"
                                )
                            end
                        else
                            score = score + 30
                ...
                local originOnlyBacklessTableFocus = (category == "bench" or category == "single_seat_bench")
                    and (kind == "table" or kind == "bar" or kind == "grinder")
                    and surfaceHit ~= true
                    and not objectLooksLikeWallShelfTable(env, obj)
                local closeBacklessTableFocus = originOnlyBacklessTableFocus
                    and (kind == "table" or kind == "bar")
                    and (distance or math.huge) <= 180
                local rejectBenchOriginOnlyFocus = originOnlyBacklessTableFocus
                    and closeBacklessTableFocus ~= true
                    or (
                        category == "bench"
                        and kind ~= "fire"
                        and surfaceHit ~= true
                ...
                if not blockedFocus and not rejectBenchOriginOnlyFocus then
                    ...
                    if canReplaceLocalSurface and (not bestScore or score < bestScore) then
                        ...
                        if kind ~= "fire" and objectLooksLikeBarSurface(env, obj) then
                            refinedKind = "bar"
                        elseif kind ~= "fire" and objectLooksLikeTableSurface(env, obj) then
                            refinedKind = objectMatchers.objectSurfaceKind(obj, env.profiles.objectModelPath, objectName(env, obj)) or kind
                ...
                elseif env.debugLog then
                    env.debugLog(
                        ...
                        "kind", tostring(kind),
                        "distance", tostring(distance),
                        "surfaceHit", tostring(surfaceHit == true),
                        "reason", originOnlyBacklessTableFocus and "backless_table_focus_requires_visible_surface" or "local_surface_preferred"
                    )
                end
                end
            end
    ...
    data.facingObjectRefId = best.object and best.object.id or data.facingObjectRefId
    ...
    data.facingObjectScale = best.object and best.object.scale or data.facingObjectScale
    data.facingObjectContentFile = best.object and best.object.contentFile or data.facingObjectContentFile
    data.facingObjectDistance = best.distance
