-- interactions/sitting/posePlanner.lua
---@omw-context local
...
local function focusSurfaceRequired(kind)
    return kind == "table" or kind == "bar" or kind == "grinder"
end

local function focusSurfaceVisible(ctx, kind)
    if not focusSurfaceRequired(kind) then return true end
    if not ctx.focusSurfaceHitFromSeat then return false end
    return ctx.focusSurfaceHitFromSeat() == true
end

local function clearIgnoredFocus(ctx, reason, kind, dot)
    if ctx and ctx.clearIgnoredFacingFocus then
        ctx.clearIgnoredFacingFocus(reason, kind, dot)
    end
end
...
function module.determineFacingDirection(ctx, sitPosition, orientation, preferredFacingDirection, preferredFacingKind, profile, obj)
    ...
    local category = ctx.sittingSeatCategory(profile, obj)
    local tableLikeFocus = preferredFacingKind == "table" or preferredFacingKind == "bar" or preferredFacingKind == "lectern" or preferredFacingKind == "grinder"
    ...
    if category == "single_seat_bench" then
        ...
        if preferred and tableLikeFocus then
            ...
            local focusSurface = focusSurfaceVisible(ctx, preferredFacingKind)
            local allowOriginOnlyTableFocus = (preferredFacingKind == "table" or preferredFacingKind == "bar")
            local dot = module.directionDot2(preferred, bestPhysical)
            if focusVisible and (focusSurface or allowOriginOnlyTableFocus) and dot > 0.25 and module.directionIsUsable(sitPosition, bestPhysical, 48) then
            ...
            local reason = focusVisible and (focusSurface and "single_seat_bench_focus_misaligned" or "single_seat_bench_focus_missing_surface") or "single_seat_bench_focus_occluded"
            clearIgnoredFocus(ctx, reason, preferredFacingKind, dot)
            debugLog("single-seat bench table focus blocked", "object", tostring(obj and obj.recordId), "focus", tostring(preferredFacingKind), "best", bestName, "surface", tostring(focusSurface), "reason", reason)
        end
        if preferred then
            if not tableLikeFocus then
                clearIgnoredFocus(ctx, "single_seat_bench_focus_not_table", preferredFacingKind, nil)
            end
    ...
    if module.profileLocksBodyToFurniture(ctx, profile, obj) then
        ...
        if preferred and tableLikeFocus then
            local objectForward = module.objectForwardDirection(obj)
            local dot = module.directionDot2(preferred, objectForward)
            local focusVisible = true
            if ctx.focusVisibleFromSeat then
                focusVisible = ctx.focusVisibleFromSeat(sitPosition) ~= false
            end
            if focusVisible and dot > 0.45 then
                debugLog("backed_chair_table_focus_aligned_with_physical_forward", "object", tostring(obj and obj.recordId), "focus", tostring(preferredFacingKind), "dot", tostring(dot))
            else
                if ctx.clearIgnoredFacingFocus then
                    ctx.clearIgnoredFacingFocus("physical_forward_mismatch", preferredFacingKind, dot)
                end
                debugLog("backed_chair_table_focus_cleared_due_to_physical_forward", "object", tostring(obj and obj.recordId), "focus", tostring(preferredFacingKind), "dot", tostring(dot), "visible", tostring(focusVisible))
            end
        end
    ...
    if category == "bench" then
        ...
        if preferred and tableLikeFocus then
            ...
            local allowOriginOnlyTableFocus = (preferredFacingKind == "table" or preferredFacingKind == "bar")
            local dot = module.directionDot2(preferred, bestPhysical)
            if focusVisible and (focusSurface or allowOriginOnlyTableFocus) and dot > 0.18 then
            ...
            local reason = focusVisible and (focusSurface and "open_side_bench_focus_misaligned" or "open_side_bench_focus_missing_surface") or "open_side_bench_focus_occluded"
            clearIgnoredFocus(ctx, reason, preferredFacingKind, dot)
            debugLog("bench facing table blocked", "object", tostring(obj and obj.recordId), "focus", tostring(preferredFacingKind), "surface", tostring(focusSurface), "reason", reason)
        elseif preferred then
            clearIgnoredFocus(ctx, "open_side_bench_focus_not_table", preferredFacingKind, nil)
            debugLog("bench facing non-table focus ignored", "object", tostring(obj and obj.recordId), "focus", tostring(preferredFacingKind))
        end
