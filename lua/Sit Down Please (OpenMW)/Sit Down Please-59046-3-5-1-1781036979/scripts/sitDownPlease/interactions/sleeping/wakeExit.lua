-- interactions/sleeping/wakeExit.lua
---@omw-context none
local module = {}
...
function module.create(env)
    ...
    local infoLog = env.infoLog or noopLog
    ...
    local function standDestinationRejectReason(item, pos)
        ...
        if item and item.interactionType == "sleeping" and vectorLooksValid(item.finalPosition) then
            ...
            local maxHorizontal = tonumber(item.maxSleepExitHorizontal or item.maxFloorExitHorizontal or 420) or 420
            if horizontal > maxHorizontal then
                return "sleep_exit_destination_too_far"
            end
    ...
    function M.queueStandTeleport(item)
        ...
        pendingStandTeleports[npc.id] = {
            ...
            returnOriginRotation = item.returnOriginRotation,
            finalPosition = item.finalPosition,
            floorDrop = item.floorDrop,
            maxSleepExitHorizontal = item.maxSleepExitHorizontal,
            maxFloorExitHorizontal = item.maxFloorExitHorizontal,
            clearSleepHomeOnSuccess = item.clearSleepHomeOnSuccess == true,
            maxSleepExitDrop = item.maxSleepExitDrop,
            maxFloorExitDrop = item.maxFloorExitDrop,
            maxWakeExitWalkDrop = item.maxWakeExitWalkDrop,
    ...
    function M.queueWakeExitWalk(npc, wakeExitWalkPosition, reason, opts)
        ...
        local isDialogueWake = tostring(reason or "") == "activated_by_player_dialogue"
        local timeout = tonumber(opts and opts.timeout) or 6.0
        local radius = tonumber(opts and opts.radius) or 80
        local maxNudges = tonumber(opts and opts.maxNudges) or 1
        local firstNudgeAfter = tonumber(opts and opts.firstNudgeAfter) or 2.5
        if isDialogueWake then
            timeout = math.min(timeout, 3.25)
            radius = math.max(radius, 96)
            maxNudges = math.max(maxNudges, 2)
            firstNudgeAfter = math.min(firstNudgeAfter, 0.9)
        end
        pendingWakeExitWalks[npc.id] = {
            ...
            radius = radius,
            reason = reason,
            interactionType = opts and opts.interactionType,
            finalPosition = opts and opts.finalPosition,
            floorDrop = opts and opts.floorDrop,
            maxSleepExitHorizontal = opts and opts.maxSleepExitHorizontal,
            maxFloorExitHorizontal = opts and opts.maxFloorExitHorizontal,
            maxSleepExitDrop = opts and opts.maxSleepExitDrop,
            maxFloorExitDrop = opts and opts.maxFloorExitDrop,
            maxWakeExitWalkDrop = opts and opts.maxWakeExitWalkDrop,
            nudges = 0,
            maxNudges = maxNudges,
            nextNudgeAt = startedAt + firstNudgeAfter,
        }
    ...
    function M.processWakeExitWalks()
        ...
        for npcId, item in pairs(pendingWakeExitWalks) do
            ...
            if not (item and isObjValid(npc) and vectorLooksValid(dest)) then
                ...
                local rejectReason = wakeExitWalkRejectReason(npc, dest, item)
                if rejectReason then
                    pendingWakeExitWalks[npcId] = nil
                    debugLog("wake exit walk skipped", npc.recordId or npc.id, tostring(item.reason), "reason", tostring(rejectReason), "dest", tostring(dest))
                else
    ...
    function M.processPostWakeActivations()
        ...
        for npcId, item in pairs(pendingPostWakeActivations) do
            ...
            if not item or (item.due and item.due > t) then
                -- not due yet
            elseif env.isNpcBusyForPostWake and env.isNpcBusyForPostWake(npcId) and (not item.queuedAt or t - item.queuedAt < (item.maxBusyWait or 2.25)) then
                item.due = t + 0.10
