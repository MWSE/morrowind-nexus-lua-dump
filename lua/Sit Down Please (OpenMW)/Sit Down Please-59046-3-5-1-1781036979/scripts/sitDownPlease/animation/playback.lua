...
-- centralize missing-animation behavior.
local self = require('openmw.self')
local anim = require('openmw.animation')

local module = {}
local ARMS_ON_KNEES_GROUP = "sdparmsonkneessitidle1"
local SLEEP_GROUPS = {
    "sdpvasitting8",
    "sdpvasitting9",
    "vasitting8",
...
function module.chooseAvailable(profile, data)
    ...
    if data and data.interactionType == "sitting" then
        ...
        local legacy = firstStrictAvailable({ ARMS_ON_KNEES_GROUP, "sitidle1", "SitIdle1" })
...
function module.forceCancelSittingGroups(debugLog, reason, selfRef)
    if not (anim and anim.cancel) then return false end
    local actorSelf = selfRef or self
    local attempted = 0
    local failed = 0
    local seen = {}
    for _, groupName in ipairs(SITTING_GROUPS) do
    ...
    if type(debugLog) == "function" then
        debugLog(
            "sitting animation groups force-cancelled",
            "reason", tostring(reason or "interaction_cleanup"),
            "groups", tostring(attempted),
            "failed", tostring(failed)
        )
    end
