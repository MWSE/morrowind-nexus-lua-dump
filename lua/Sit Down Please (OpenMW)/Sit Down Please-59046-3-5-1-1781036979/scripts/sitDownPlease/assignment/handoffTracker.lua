...
-- compile limit while preserving retry/readiness handoff behavior.
...
function M.releaseOnResult(npcId)
    ...
    local item = pendingLocalResponses[npcId]
    local hadPending = item ~= nil
    pendingLocalResponses[npcId] = nil
    return hadPending, item
end
...
local function isCalibrationHandoff(candidate)
    return candidate ~= nil and (candidate.calibrationFill == true
        or candidate.calibrationTestNpc == true
        or candidate.explicitFillOverride == true
        or candidate.testingOverride == true
        or candidate.manualAssignOverrideApplied == true
        or candidate.calibrationFillLabel ~= nil
        or candidate.calibrationFillSessionId ~= nil)
end

local function shouldHoldInvalidHandoff(item, now)
    if not (item and isCalibrationHandoff(item.candidate)) then return false end
    local age = now - (item.firstSentAt or now)
    return age >= 0 and age < 1.25
end
...
function M.process()
    ...
    for npcId, item in pairs(pendingLocalResponses) do
        ...
        if not (item and isObjValid(npc) and candidate and candidate.object) then
            if shouldHoldInvalidHandoff(item, now) then
                debugLog(
                    "holding calibration local handoff during spawn/validity grace",
                    tostring(npc and (npc.recordId or npc.id) or npcId),
                    "type", tostring(candidate and candidate.interactionType),
