...
local function readYamlFile(path)   
    ...
    if vfs.fileExists(path) then
        ...
        if fileHandle then
            ...
            fileHandle:close()
            parsedData = markup.decodeYaml(yamlContent or "")
        else
...
function Actor:getDumpableInventoryItems()
    ...
    for i, item in pairs(invItems) do
        ...
        if not isEquippedWearable then
...
function Actor:isVampire()
    -- Based on Urm's function. A missing vampirism effect means not a vampire;
    -- the previous negation treated almost every actor as a vampire when this
    -- helper was used.
    local vampirism = self:activeEffects():getEffect('vampirism')
    return vampirism ~= nil and (tonumber(vampirism.magnitude or 0) or 0) > 0
end
...
local function isMyFriend(actor)
    ...
    return actor.id ~= omwself.id and not types.Player.objectIsInstance(actor) and sameType and
        not wasMyTarget(actor) and fightVal and fightVal.modified >= BaseFriendFightVal
end
