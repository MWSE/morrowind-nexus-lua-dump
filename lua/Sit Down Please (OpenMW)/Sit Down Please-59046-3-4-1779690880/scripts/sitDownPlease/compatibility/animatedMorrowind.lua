...
function M.chooseNearbySeat(actor, candidates, profiles)
    ...
    for _, candidate in ipairs(candidates) do
        local ok = M.candidateLooksLikeSeat(candidate, profiles)
        ...
        if obj and obj.position then
            ...
            if dist and dist <= M.SEAT_RADIUS and vertical <= M.SEAT_VERTICAL_TOLERANCE then
...
function M.actorIsAmBard(actor)
    return lower(actor and actor.recordId):find("^am_bard") ~= nil
end
...
function M.actorUsesSdpRootOffset(actor)
    local recordId = lower(actor and actor.recordId)
    return recordId:find("^am_writer") ~= nil
...
function M.expectedExternalSeatedZ(actor, surfaceZ, sdpExpectedZ, originalZ)
    ...
    if M.actorUsesSdpRootOffset(actor) then
        ...
        if recordId:find("^am_writer") ~= nil then
