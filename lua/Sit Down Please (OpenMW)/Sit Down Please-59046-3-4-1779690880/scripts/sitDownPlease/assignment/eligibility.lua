...
function M.isActorDead(actor, types)
    local dead = M.actorDeadReason(actor, types)
    return dead == true
end
...
function M.slotOccupiedByTestNpc(occupiedSlots, assignedActors, slotKey)
    ...
    if recordId == "ken" then return true, data end
    if data and data.calibrationTestNpc == true then return true, data end
    return false, data
end

function M.hiddenOrStagedNpcReason(npc)
    local recordId = npc and npc.recordId and string.lower(tostring(npc.recordId)) or ""
    if recordId == "" then return nil end
    if recordId == "mehra milo" then
        return "quest_travel_actor"
    end
...
function M.sendDaytimeWakeCleanupProbe(npc, source, assignedActors, isObjValid, debugLog, isCurrentTimeInSleepWindow)
    if type(isCurrentTimeInSleepWindow) ~= "function" then return false end
    local inWindow, windowReason = isCurrentTimeInSleepWindow()
    if inWindow == true then return false end
    return M.sendWakeCleanupProbe(
        npc,
        windowReason or "outside_allowed_time_window",
        source,
        assignedActors,
        isObjValid,
        debugLog
