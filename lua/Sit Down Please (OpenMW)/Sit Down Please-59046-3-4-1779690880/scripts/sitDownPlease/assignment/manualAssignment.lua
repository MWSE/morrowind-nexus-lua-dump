...
-- Developer/manual reassignment cleanup helpers. Kept separate so calibration/menu.lua
...
function M.cleanupBeforeReassign(ctx, actor, candidate, reason)
    ...
    local infoLog = ctx.infoLog or function() end
...
function M.bypassLocalRejection(ctx, data, reason, bypass)
    ...
    local debugLog = ctx and ctx.debugLog or function() end
...
function M.tryManualRouteFallback(ctx, npc, data, distance, stuckTimedOut, hardTimedOut)
    ...
    if data.interactionType == "sleeping" and npc.position then
        ...
        if vertical > 90 then
            if vertical > MAX_MANUAL_SLEEP_FALLBACK_VERTICAL or directDistance > MAX_MANUAL_SLEEP_FALLBACK_DISTANCE then
