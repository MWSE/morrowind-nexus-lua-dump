...
local REQUIRE_MODIFIER_FOR_SIT = false
local SIT_MODIFIER = "shift"
...
local EXTERNAL_SEATED_ACTOR_EXACT_IDS = {
    ["lack_qac_pianist"] = true,
    ["lack_qac_pelim"] = true,
}

local EXTERNAL_SEATED_ACTOR_PREFIXES = {
    "am_sitternight",
    "am_sitter",
    "am_eater",
    "am_reader",
...
local EXTERNAL_SEATED_MODEL_TOKENS = {
    "am/am_sitting.nif",
    "am/am_reader",
    "am/am_eater.nif",
    "am/am_bard",
...
local EXTERNAL_SEATED_SCRIPT_TOKENS = {
    "_bard_script",
    "bard_script",
}
...
local function normalizedModelPath(value)
...
local function hasToken(value, tokens)
    ...
    for _, token in ipairs(tokens or {}) do
        token = normalizedModelPath(token)
        if token ~= "" and value:find(token, 1, true) then return true end
    end
    return false
end

local function actorTypeRecord(actor)
...
local function actorModelPath(actor)
    local rec = actorTypeRecord(actor)
    return rec and rec.model or nil
end

local function actorScriptName(actor)
    local rec = actorTypeRecord(actor)
    return rec and rec.script or nil
end

local function knownExternalSeatedActorReason(actor)
    local id = lower(actor and actor.recordId)
    if EXTERNAL_SEATED_ACTOR_EXACT_IDS[id] then return "external_seated_actor" end
    if hasPrefix(id, EXTERNAL_SEATED_ACTOR_PREFIXES) then return "external_seated_actor" end
    if hasToken(normalizedModelPath(actorModelPath(actor)), EXTERNAL_SEATED_MODEL_TOKENS) then
        return "external_seated_actor"
    end
    if hasToken(lower(actorScriptName(actor)), EXTERNAL_SEATED_SCRIPT_TOKENS) then
        return "external_seated_actor"
    end
    return nil
end
...
local function isSittable(recordId)
    ...
    local lowerId = lower(recordId)
    if BLACKLIST[lowerId] then return false end
    for _, p in ipairs(SITTABLE_PATTERNS) do
        if lowerId:find(p, 1, true) then return true end
    end
...
local function sitModifierHeld()
    if SIT_MODIFIER == "ctrl" then return input.isCtrlPressed() end
    if SIT_MODIFIER == "alt" then return input.isAltPressed() end
    return input.isShiftPressed()
end

local function playerSitInputEnabled()
    return REQUIRE_MODIFIER_FOR_SIT ~= true or sitModifierHeld()
end
...
local function occupiedSeatRadius(recordId)
    local lowerId = lower(recordId)
    if lowerId:find("bench", 1, true) then return OCCUPIED_SEAT_RADIUS.bench end
    if lowerId:find("chair", 1, true) then return OCCUPIED_SEAT_RADIUS.chair end
    if lowerId:find("stool", 1, true) or lowerId:find("barstool", 1, true) then return OCCUPIED_SEAT_RADIUS.stool end
    if isCushion(lowerId) then return OCCUPIED_SEAT_RADIUS.cushion end
    return OCCUPIED_SEAT_RADIUS.default
end

local function actorUsesTableTaskSeat(actor)
    local id = lower(actor and actor.recordId)
    return id:find("^am_writer") ~= nil
        or id:find("^am_reader") ~= nil
        or id:find("^am_eater") ~= nil
end

local function furnitureLooksLikeTableTaskSeat(furniture)
    local id = lower(furniture and furniture.recordId)
    if id == "" then return false end
    if id:find("barstool", 1, true) then return false end
    return id:find("chair", 1, true) ~= nil
        or id:find("throne", 1, true) ~= nil
        or id:find("com_m_chr", 1, true) ~= nil
        or id:find("bench", 1, true) ~= nil
end

local function actorClaimsPosition(actor, pos, radius, verticalLimit)
    if not (actor and actor.position and pos) then return false end
    local vertical = math.abs((actor.position.z or 0) - (pos.z or 0))
    return vertical <= verticalLimit and horizontalDistance(actor.position, pos) <= radius
end

local function externalActorClaimsSeat(actor, furniture, seatPos)
    if not knownExternalSeatedActorReason(actor) then return false end

    local primaryRadius = occupiedSeatRadius(furniture and furniture.recordId)
    local objectRadius = primaryRadius
    local verticalLimit = 130
    if actorUsesTableTaskSeat(actor) and furnitureLooksLikeTableTaskSeat(furniture) then
        primaryRadius = 156
        objectRadius = 220
        verticalLimit = 175
    end

    return actorClaimsPosition(actor, seatPos, primaryRadius, verticalLimit)
        or actorClaimsPosition(actor, furniture and furniture.position, objectRadius, verticalLimit)
end

local function seatIsClaimedByActor(furniture, seatPos)
    if not (furniture and furniture.position) then return false, nil end
    local radius = occupiedSeatRadius(furniture.recordId)
    local referencePos = seatPos or furniture.position
    for _, actor in ipairs(nearby.actors) do
        if actor and actor ~= self.object and actor.position then
            local vertical = math.abs((actor.position.z or 0) - (referencePos.z or 0))
            local horizontal = horizontalDistance(actor.position, referencePos)
            local playingSeatAnimation = actorIsPlayingSeatAnimation(actor)
            if externalActorClaimsSeat(actor, furniture, seatPos) then
                return true, actor
            end
            if playingSeatAnimation and vertical <= math.max(OCCUPIED_SEAT_VERTICAL_LIMIT, 115) and horizontal <= radius then
                return true, actor
            end
        end
    end
...
local function getFallbackOffset(recordId)
    ...
    for pattern, h in pairs(SIT_PIVOT_OFFSET_FALLBACK) do
        if pattern ~= "default" and lowerId:find(pattern, 1, true) then
            return h
        end
    end
...
local function startSitting(furniture)
    ...
    claimed, actor = seatIsClaimedByActor(furniture, basePos)
    if claimed then
        showOccupiedSeatMessage(actor)
        pendingFurniture = nil
        if returnToFirstPerson then
...
local function onFrame(dt)
    ...
    if isSitting then
        ...
        if fatigueTimer >= FATIGUE_TICK_RATE then
            ...
            if fatigue and fatigue.current and fatigue.base then
    ...
    if eDown and not eWasDown then
        if isSitting then
            ...
            if furniture and playerSitInputEnabled() then
                local claimed, actor = seatIsClaimedByActor(furniture)
                if claimed then
                    showOccupiedSeatMessage(actor)
                else
