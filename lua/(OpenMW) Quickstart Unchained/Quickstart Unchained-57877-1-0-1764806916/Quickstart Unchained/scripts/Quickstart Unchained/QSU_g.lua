...
local function getBestWeaponSkill(player)
    local bestSkill = nil
    local bestValue = -1
...
local function getMatchingWeapons(skillId, minValue, maxValue)
...
local function getBestArmorSkill(player)
    local bestSkill = nil
    local bestValue = -1
...
local function getMatchingArmor(skillId, minValue, maxValue)
    local matchingArmor = {}
...
local function giveRandomArmor(player)
    if not player then
...
local function getSpellPrimarySchool(spell)
...
local function getBestMagicSkill(player)
    local bestSkill = nil
    local bestValue = -1
...
local function getMatchingSpellTomes(schoolId, maxCost)
    local matchingTomes = {}
...
local function giveRandomSpellTome(player)
    if not player then
        return false
    end
...
local function giveRandomGold(player)
    if not player then
...
local function giveRandomAlcohol(player)
    if not player then
...
local function giveLockpicks(player)
    if not player then
...
local function giveSoulGem(player)
...
local function giveApparatus(player)
...
local function giveRepairHammer(player)
...
local function giveAmmo(player)
...
local function giveJewelry(player)
...
local function giveLight(player)
...
local function giveBook(player)
    local books = {}
...
local function Quickstart_Unchained(data)
	local player = data[1]
	local plus = data[2]
	...
	async:newUnsavableSimulationTimer(0.05, async:callback(function() 
		if plus then
			--if types.Player.getBirthSign(player) == "" then
			--	types.Player.setBirthSign(player, "Charioteer")
			--end
		...
		async:newUnsavableSimulationTimer(0.15, async:callback(function()
		...
		--async:newUnsavableSimulationTimer(0.25, async:callback(function()
		--	if plus then
		--		if types.Player.getBirthSign(player) == "" then
		--			types.Player.setBirthSign(player, "Charioteer")
		--		end
...
return {
	...
	eventHandlers = {
		Quickstart_Unchained = Quickstart_Unchained,
	}
}