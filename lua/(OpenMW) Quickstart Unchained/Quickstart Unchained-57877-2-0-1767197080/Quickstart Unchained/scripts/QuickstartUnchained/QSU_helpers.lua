...
-- ╰──────────────────────────╯
if ui then
	local makeBorder = require("scripts.QuickstartUnchained.ui_makeborder")
	...
	-- Standalone tooltip for QSU
	local tooltipFontSize = 20
	local tooltipFontColor = getColorFromGameSettings("fontColor_color_normal")
		...
		return ui.create {
			...
			content = ui.content {
				{ props = { size = v2(0, padding) } },
				{
					...
					content = ui.content {
						{ props = { size = v2(padding, 0) } },
						{
							type = ui.TYPE.Text,
							props = {
								text = text,
								textSize = tooltipFontSize,
								textColor = tooltipFontColor,
								multiline = true,
							},
						},
						{ props = { size = v2(padding, 0) } },
	...
	function addTooltip(elem, text)
		elem.events = elem.events or {}

		elem.events.mouseMove = async:callback(function(data, elem)
...
local function alphaFromValue(x)
	...
	elseif HUD_ALPHA == "Gradual + better visible" then
		local step = math.floor( x * 6)
		return math.min(1, math.min(step / 5, 1)^0.5 + 0.3)
	else
		local step = math.floor( x * 6)
...
local function alphaFromValue06666(x)
	...
	elseif HUD_ALPHA == "Gradual + better visible" then
		...
		return math.min(1, math.min(step / (stepCount - 1), 1)^0.5 + 0.3)
	else
		local stepCount = 4
		local step = math.floor(x * stepCount)
