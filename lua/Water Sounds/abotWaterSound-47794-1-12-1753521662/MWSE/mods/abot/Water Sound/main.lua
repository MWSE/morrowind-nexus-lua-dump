...
local removeWaterLayerSound2Registered = false

local function removeWaterLayerSound2()
	tes3.removeSound({sound = waterLayerSound2})
	if removeWaterLayerSound2Registered then
		removeWaterLayerSound2Registered = false
...
local function simulateWaterSound()
	...
	local equippedArmor = tes3.getEquippedItem({actor = player,
		objectType = tes3_objectType_armor, slot = armorSlot})
	...
	if waterLayerVolume > 0 then
		if tes3.player.cell.isOrBehavesAsExterior then
			...
			if not removeWaterLayerSound2Registered then
				removeWaterLayerSound2Registered = true
...
local validWater = {
'_water','water_','stream','dg-wt_','dg_circle','waterfall','terrain_ashmire_',
'tad_fountain_balmora','nom_ma_pool00','nom_bc_pool00','nom_ashlands_pool00','nom_ac_pool00','furn_pycave_pool00','terrain_bc_scum_'
}
local invalidWater = {'spray', 'barrel', 'strider'}
...
local function checkCollidingFakeWater()
	...
	local waterLevel = getWaterLevel(player.cell)
	if waterLevel then
		if playerPos.z < waterLevel then
			return -- already in realwater, no need to check for fake water
		end
	end
...
local function calcMoveSpeed(e)
	...
	---tes3ui.showNotifyMenu("calcMoveSpeed(e)")
...
local function initialized()
	...
	if not waterLayerSound2 then
		waterLayerSound2 = tes3.createObject({objectType = tes3.objectType.sound,
			id = 'WaterLayer2',	filename = waterLayerSound.filename,
			volume = waterLayerSound.volume})
	end
	...
	if waterLayerIntSound then
		...
		if not waterLayerIntSound2 then
			waterLayerIntSound2 = tes3.createObject({objectType = tes3.objectType.sound, id = 'WaterLayerInt2',
				filename = waterLayerIntSound.filename,
				volume = waterLayerIntSound.volume})
		end
	end
	underwaterSound2 = tes3.getSound('Underwater2')
	if not underwaterSound2 then
		underwaterSound2 = tes3.createObject({objectType = tes3.objectType.sound,
		id = 'UnderWater2',	filename = underwaterSound.filename,
		volume = underwaterSound.volume})
	end
	event.register('loaded', loaded)
	
	if tes3.getScript('TR_Po_WaterSound512') then
		tes3ui.showNotifyMenu('WARNING: Water Sound MWSE-Lua mod makes abotTRWaterSound mod obsolete, you should uninstall it')
	end
