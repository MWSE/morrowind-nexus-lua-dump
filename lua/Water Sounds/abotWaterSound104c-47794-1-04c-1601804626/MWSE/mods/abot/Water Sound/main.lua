...
local function checkCollidingFakeWater()
	...
	local result = tes3.rayTest({ position = ppos, direction = VEC3UP,
		maxDistance = 2048, findAll = false, ignore = {player}, useBackTriangles = true})
	---local result = false
	if not result then
		cancelt2restartMAOscript()
		return
	...
	if not string.multifind(lcMesh, {'_water', 'water_', 'dg-wt_'}, 1, true) then
		cancelt2restartMAOscript()
		return
	end
...
local function loaded()
	...
	AO_PCBeastRace = tes3.findGlobal('AO_PCBeastRace') -- nil if not found, else value (could be 0)
