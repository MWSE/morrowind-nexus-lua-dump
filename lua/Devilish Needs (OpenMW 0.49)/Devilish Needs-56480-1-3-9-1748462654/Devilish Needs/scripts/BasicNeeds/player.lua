...
-- 2025 -- Refactored by DetailDevil for settings-update reposition
-- -----------------------------------------------------------------------------
...
local function checkForWaterEffects()
   ...
   for _, spell in pairs(active) do
       if spell.name and string.find(string.lower(spell.name), "cooking pot")  then
...
local function playerConsumedToxicSubstance(eventData)
	--identify & handle diseases
	if(eventData.key == "raw meat" and math.random(100) <= eventData.chance ) then
...
return {
   ...
   eventHandlers = {
	  ...
	  PlayerConsumedToxicSubstance = playerConsumedToxicSubstance,
   },
}
