...
function Need._updateEffects(need, prevStatus)
   ...
   local effectId = need._effects[status]

   -- kill the player only if they DON’T have the jail-check ability
   if status == STATE.Death
      and not Actor.activeSpells(self):isSpellActive("detd_jailcheck") then
      Actor.stats.dynamic.health(self).current = -1000

   -- otherwise add the stage effect, but ONLY if one exists
   elseif effectId then
      Actor.spells(self):add(effectId)
   end
end
