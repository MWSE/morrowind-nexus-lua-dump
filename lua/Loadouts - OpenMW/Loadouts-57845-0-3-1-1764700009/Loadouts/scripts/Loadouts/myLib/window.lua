...
function o:moveWindow(e, layout)
        if e.button == 1 then
                ...
                local newX = math.min(
                        ...
                        (Res.x / Scale) - layout.props.size.x + self.MOVE_ANCHOR_X * layout.props.size.x
                )
                local newY = math.min(
                        math.max(e.position.y, self.MOVE_ANCHOR_Y * layout.props.size.y),
                        (Res.y / Scale) - layout.props.size.y + self.MOVE_ANCHOR_Y * layout.props.size.y
                )
...
function o:getTitleBarLayout()
        return {
                template = g.templates.getTemplate('none', { 0, 0, 0, 0 }, true, g.textures.menuBG, false, true),
                props = {
...
---@param tabsData Tab[]
---@return myWindow
function o:new(title, x, y, tabsData)
        Res = ui.screenSize()
        ...
        inst.element = ui.create {
                layer = "Windows",
                template = g.templates.getTemplate('thick', { 6, 6, 6, 5 }, true, g.textures.black),
                props = {
                        position = inst.pos,
                        size = inst.size,
                        anchor = inst.anchor,
