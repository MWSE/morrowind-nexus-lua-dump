...
-- while true, this NPC neither gets knocked out nor knocks out the player.
local questExempt = false
-- set in onDied for resurrection
local hasDied = false
local hitHandlerRegistered = false
-- used by BH
local bhMinLevel = 3

local cachedFollowerIds = {}

local isNPC    = types.NPC.objectIsInstance(self.object)

local recId             = string.lower(self.object.recordId)
local excludedByRecord  = isNPC and shared.EXCLUDED_NPCS[recId] or false
local cf                = self.object.contentFile
local excludedByContent = (isNPC and cf and shared.EXCLUDED_CONTENT_FILES[cf:lower()]) or false

local excludedByPrefix = false
if isNPC then
    for _, prefix in ipairs(shared.EXCLUDED_ID_PREFIXES or {}) do
        if recId:sub(1, #prefix) == prefix then
...
local excluded = excludedByRecord or excludedByContent or excludedByPrefix
...
local function startKnockdown(attacker)
    ...
    if attacker and attacker:isValid() then
        if types.Player.objectIsInstance(attacker) then
            ...
            -- follower attacker: relay through global to reach the player
            core.sendGlobalEvent("GKD_RelayShowMessage", {
                name = getNpcName(),
                npc  = self.object,
            })
        end
...
local function handleOnHit(attack)
    ...
    -- this script instance already saw its actor die, resurrection path
    ...
    -- quest-exempt: skip knockout entirely
    if questExempt then return end
    ...
    -- only player and followers trigger knockouts
    local isPlayerAttacker   = types.Player.objectIsInstance(attack.attacker)
    local isFollowerAttacker = cachedFollowerIds[attack.attacker.id] or false
    if not isPlayerAttacker and not isFollowerAttacker then return end
    ...
    if cachedSettings.FIGHT_THRESHOLD_ENABLED then
        ...
        if originalFight >= cachedSettings.FIGHT_THRESHOLD then
            if cachedSettings.BH_COMPAT and isPlayerAttacker then
    ...
    -- weird things happen with simattack not giving the exact dmg dealt by the player
    -- that results in kills when an NPC has low health and you spam low strength attacks while at high difficulty
    -- workaround since i've no idea what causes discrepancy
    -- basically to ensure an NPC is considered to be dealt a lethal blow
...
local function onInactive()
    -- always unregister so we don't leak dead followers
    ...
    if knockedDown and not types.Actor.isDead(self.object) then
        knockedDown = false

        local healthStats = types.Actor.stats.dynamic.health(self)
        local maxHp       = healthStats.base + healthStats.modifier
        healthStats.current = math.max(1, math.floor(maxHp * cachedSettings.HP_AFTER_KNOCKDOWN))
        ...
        types.Actor.stats.ai.fight(self).base = cachedSettings.SET_FIGHT

        core.sendGlobalEvent("GKD_FastRecoverInactive", { npc = self.object })
    end
...
local function onDisableKnockdown()
    disableKnockdown = true
end

-- sent by gko_global when this NPC enters or leaves a quest exemption window
local function onSetQuestExempt(d)
    if not d then return end
    questExempt = d.value and true or false
end

-- current min level
local function onBhMinLevelUpdated(d)
    if d and d.MIN_PRISONER_LEVEL then
        bhMinLevel = d.MIN_PRISONER_LEVEL
    end
end

local function onBhPlayerEscortState(d)
    bhPrisonerId = d.prisonerId
end

local function onInternalStartKnockdown(d)
...
local function onStartWeaponPickup(d)
...
local function onSetDisposition(d)
...
local function onStopCombatFull()
...
-- we do be suppressing stoopid followers AND the hostile crowd
local function onFollowerSuppressCombat(d)
    ...
    if playerTarget then
        ...
        -- engine keeps re-issuing combat against valid targets, so keep filtering
        local deadline = core.getSimulationTime() + d.duration
        local function loop()
            if not self:isActive() or types.Actor.isDead(self.object)
               or core.getSimulationTime() >= deadline then
...
local function onFollowerSetUpdate(d)
    if not d or not d.ids then return end
    cachedFollowerIds = d.ids
end

-- a part of resurrection clusterfuck
local function onDied()
    hasDied      = true
    knockedDown  = false
    savedFatigue = nil
    core.sendGlobalEvent("GKD_ScriptCleanupOnDeath", { npc = self.object })
end
...
return {
    ...
    eventHandlers = {
        ...
        GKD_DisableKnockdown       = onDisableKnockdown,
        GKD_SetQuestExempt         = onSetQuestExempt,
        BH_MinLevelUpdated         = onBhMinLevelUpdated,
        BH_PlayerEscortState       = onBhPlayerEscortState,
        GKD_InternalStartKnockdown = onInternalStartKnockdown,
        GKD_StartWeaponPickup      = onStartWeaponPickup,
        GKD_Reequip                = onReequip,
        GKD_SetDisposition         = onSetDisposition,
        GKD_StopCombatFull         = onStopCombatFull,
        GKD_LootPlayer             = onLootPlayer,
        GKD_FollowerSuppressCombat = onFollowerSuppressCombat,
        GKD_FollowerSetUpdate = onFollowerSetUpdate,
    },
}