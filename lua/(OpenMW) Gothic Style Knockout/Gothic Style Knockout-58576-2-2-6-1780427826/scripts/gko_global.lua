...
-- the actor's local script reports its own death
-- a part of resurrection clusterfuck
local function onScriptCleanupOnDeath(d)
    if not d or not d.npc or not d.npc:isValid() then return end
    local npc = d.npc

    originalFights[npc.id] = nil
    if followerIds[npc.id] then
        followerIds[npc.id] = nil
        broadcastFollowerSet()
    end
    for dropped, ownerId in pairs(droppedWeapons) do
        if ownerId == npc.id then
            droppedWeapons[dropped] = nil
        end
    end
end
...
return {
    ...
    eventHandlers = {
        ...
        GKD_ScriptCleanupOnDeath = onScriptCleanupOnDeath,
    },
}