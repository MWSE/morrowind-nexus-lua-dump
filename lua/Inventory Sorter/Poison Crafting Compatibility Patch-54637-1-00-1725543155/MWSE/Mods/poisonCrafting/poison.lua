...
--[[
local function getCursorElement()
	return tes3ui.findHelpLayerMenu('CursorIcon')
end

local function getCursorTile()
	if getCursorElement() then -- can easily not exists any more /abot 
		return el:getPropertyObject("MenuInventory_Thing",
			"tes3inventoryTile")
	end
	return nil
end
...
local lastClickedPotionId

local function afterMouseClickSomethingElse()
	lastClickedPotionId = nil
end

local function itemDropped(e)
	if e.reference.object.id == lastClickedPotionId then
		lastClickedPotionId = nil
	end
end
...
local function beforeMouseClickPotion(e)
	local el = e.source
	local tile = el:getPropertyObject("MenuInventory_Thing", "tes3inventoryTile")
	if tile then
		lastClickedPotionId = tile.item.id -- store the potion id when you click it
		---mwse.log('>>> lastClickedPotionId = "%s"', lastClickedPotionId)
	end
end

local function afterMouseClickWeaponToPoison(e)
	if not lastClickedPotionId then
		return
	end
	local el = e.source
	local tile = el:getPropertyObject("MenuInventory_Thing", "tes3inventoryTile")
	...
	---mwse.log('>>> tile.item.id = %s', tile.item.id)
	local pcInventory = tes3.player.object.inventory
	local itemStack = pcInventory:findItemStack(lastClickedPotionId)
	if not itemStack then
		return
	end
	---mwse.log('>>> itemStack.object.id = %s', itemStack.object.id)
	...
	if variables then
		---mwse.log('>>> variables = %s', variables)
	...
	lastClickedPotionId = nil
	--[[mwse.log('>>> setPoisonInteractive("%s", %s, "%s", %s)',
		tile.item.id, tile.itemData, itemStack.object.id, itemData)]]
	setPoisonInteractive(tile.item, tile.itemData, itemStack.object, itemData)
end
...
local function itemTileUpdated(e)
	local el = e.element
	local item = e.item
	if el:getPropertyBool('g7pocrTC') then
		return
	end
	if item.objectType == tes3_objectType_alchemy then
		el:setPropertyBool('g7pocrTC', true)
		el:registerBefore("mouseClick", beforeMouseClickPotion)
	elseif (
		item.isMelee
		or item.isProjectile
	)
	and (not e.menu:getPropertyObject("MenuContents_Actor"))
	and (not e.menu:getPropertyObject("MenuContents_ObjectContainer")) then
		el:setPropertyBool('g7pocrTC', true)
		el:registerAfter("mouseClick", afterMouseClickWeaponToPoison)
	else
		el:setPropertyBool('g7pocrTC', true)
		el:registerAfter("mouseClick", afterMouseClickSomethingElse)
	end
end
event.register("itemTileUpdated", itemTileUpdated, { filter = "MenuInventory" })
...
local function onAlchemySceneNodeCreated(e)
    if e.reference.object.objectType ~= tes3_objectType_alchemy then
        return
    end
...
local function onAlchemyTileUpdated(e)
    if config.useEffectIcons then
        if e.item.objectType == tes3_objectType_alchemy then
