...
local function getItemStat(item, itemData)
	...
	and canTransferLight(item) then
		local t = 0
		if itemData
...
local function equipUnequip(tile)
	...
	or (
		...
		local prms = {item = itm, itemData = itmData, playSound = false}
...
---@diagnostic disable-next-line: need-check-nil
			mobilePlayer:unequip(prms)
		else
-- replacement for mob:equip(params) that triggers the equip event
			prms.reference = player
			prms.bypassEquipEvents = false
			tes3.equip(prms)
		end
		return true
	end
...
local function toggleEvents(on)
	...
	regOrUnreg('itemTileUpdated', itemTileUpdated, {priority = 300000}) -- high priority
...
local function beforeDestroyTileMenu(e)
	...
	if menuTitleElements[menu_id] then
		menuTitleElements[menu_id] = nil
	end
end
...
function ()
	...
	event.register('save', save, {priority = -300000}) -- with low priority so it can still be skipped
