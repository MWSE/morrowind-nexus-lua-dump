...
local logLevel1, logLevel2, logLevel3, logLevel4, logLevel5
...
local function getMenuTitle(menu)
	local el = menuTitleElements[menu.id]
	if not el then
...
local function equipUnequip(tile)
	...
	local cursorIcon = tes3ui.findHelpLayerMenu(idCursorIcon)
	if cursorIcon then
		return
	end
	if not tile then
...
local function beforeMouseClickTileElement(e)
	---tes3.messageBox('beforeMouseClickTileElement')
	local el = e.source
	local tile = el:getPropertyObject(
		idMenuInventory_Thing, 'tes3inventoryTile')
	return equipUnequip(tile)
end
...
local function itemTileUpdated(e)
	...
	--[[if not el:getPropertyBool(ab01insoTCid) then
		...
		if menu.id == idMenuInventory then
			---mwse.log("%s:registerBefore('mouseClick', beforeMouseClickTileElement)", item.id)
			el:registerBefore('mouseClick', beforeMouseClickTileElement)
			---el:register('mouseClick', mouseClickTileElement)
		end
		---el:registerAfter('mouseClick', afterMouseClickTileElement)
	end]]
	if ctrlClickEquip
	and (menu.id == idMenuInventory)
