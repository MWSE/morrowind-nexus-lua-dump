--[[ Crafting of items using Leather and Resin kit kit.
...
local Recipes = require("Morrowind_Crafting_3.Leather_Resin_kit.recipes")
...
local sortBy = "Normal"
local tGroupIdx = 1
local tGroup = { "All", "Bear Hide", "Bonemold", "Chitin", "Exotic Materials", "Kagouti Hide", "Netch", "Wolf Hide", "Wooden" }
...
-- Register IDs
UID_ListPane = tes3ui.registerID("OrganicMenu::List")
UID_ListLabel = tes3ui.registerID("OrganicMenu::ListBlockLabel")
UID_classLabel = tes3ui.registerID("OrganicMenu::menuLabel")
UID_groupLabel = tes3ui.registerID("OrganicMenu::menuLabel")
UID_filterText = tes3ui.registerID("OrganicMenu::Input")

function this.init()
	this.id_menu = tes3ui.registerID("OrganicMenu")
	this.id_menulist = tes3ui.registerID("Organiclist")
	this.id_cancel = tes3ui.registerID("Organic_cancel")
	...
	--
	Recipes = mc.getAddInFiles("Leather", Recipes) -- Get add-in recipes
	Recipes = mc.cleanDeprecated(Recipes)
end
...
local function onClickSelectedItem(e)
	if mc3_timeOut then
		...
		local recipeNum = e.source:getPropertyInt("OrganicMenu:Index")
		...
		for i, xt in ipairs(Recipes[recipeNum].ingreds) do
			if xt.count > 0 then
				if xt.id == "Frost Atronach Soul" then
					mc.removeFrostSoul()
				else
...
local function showOrganicTooltip(e)
	config = mwse.loadConfig(configPath)
	local itemDesc
	local recipeNum = e.source:getPropertyInt("OrganicMenu:Index")
	...
	if thing ~= nil then
		...
		for itip, xt in ipairs(Recipes[recipeNum].ingreds) do
			...
			if xt.id == "Frost Atronach Soul" then
				local showtip = tipmenu:createLabel({ text = "Frost Atronach Soul ("
				..mc.countFrostSouls().." of "..xt.count..")  " })
			else
...
function this.createWindow()
	...
	local filterBlock = menu:createBlock({})
		...
		filterInputBorder.widthProportional = .6
		...
		buttonClass.widthProportional = 0.6
		...
		buttonGroup.widthProportional = 0.8
	...
	for index, x in ipairs(Recipes) do
		...
		if thing ~= nil then
			...
			if ttemp then
				...
				if ttemp == true then
					...
					for itip, xt in ipairs(Recipes[index].ingreds) do
						ingred = tes3.getObject(xt.id)
						if xt.id == "Frost Atronach Soul" then
							if mc.countFrostSouls() < xt.count then
								haveMaterial = false
							end
						elseif mwscript.getItemCount({ reference = "player", item = xt.id }) < xt.count then
							haveMaterial = false
						end
					end
					...
					itemBlock:setPropertyInt("OrganicMenu:Index", index)
					if haveMaterial == true then
						itemBlock:register("mouseClick", onClickSelectedItem)
					end
					itemBlock:register("help", showOrganicTooltip)
			...
			if x.invalid ~= true then
				mwse.log("[MC3 Checker] Invalid ID found: ("..x.id..")")
				x.invalid = true
			end
		end
	end
...
UIID_CraftingMenu_ListBlock_Label = tes3ui.registerID("OrganicMenu:ListBlockLabel")
event.register("initialized", this.init)

local function onEquip(e)
	if e.item.id == "mc_organic_kit" then
