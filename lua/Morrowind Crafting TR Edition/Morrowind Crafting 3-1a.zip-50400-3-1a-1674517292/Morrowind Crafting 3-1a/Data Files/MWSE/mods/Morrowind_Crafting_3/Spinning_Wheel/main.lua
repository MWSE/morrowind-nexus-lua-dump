...
function this.qtySlider()
	...
	local slider = pBlock:createSlider({ id = "sliderUID", current = batchCount, max = batchCount, step = 1, jump = 2 })
...
local function mc_spinningwheel_p_activate(e)
	if (e.activator == tes3.player) then
		if ( e.target.object.id == "mc_spinningwheel_p" ) or ( e.target.object.id == "mc_spinningwheel2_p" ) then
