...
scrapRecipes = mc.getAddInFiles("Shears", scrapRecipes) -- Get add-in recipes
scrapRecipes = mc.cleanDeprecated(scrapRecipes)
	...
	if mc3_timeOut then 
		-- do nothing
			...
			if x.id == e.item.id then
				...
				if (x.byProduct) then
					for idxBp, xBp in ipairs(x.byProduct) do
						tes3.addItem({reference = tes3.player, item = xBp.id, count = xBp.yield * batchSize, playSound = false})
					end
				end
