	...
	if mc3_timeOut then
		...
		if mc.skillCheck(skillID, (Recipes[recipeNum].difficulty * modDifficulty)) == true then -- Succeeded
			...
			if Recipes[recipeNum].byproduct then -- making this item gives something else as a by-product
				for byIdx, bx in ipairs(Recipes[recipeNum].byproduct) do
					tes3.addItem({ reference = tes3.player, item = bx.id, count = bx.yield, playSound = false })
				end
			end
		else
...
local function outsideActivate(e)
	this.createWindow()
end
event.register("mc_cookingActivate", outsideActivate)

local function mc_tool_activate(e)
	if (e.activator == tes3.player) then
		if ( e.target.object.id == "mc_logfire" ) or ( e.target.object.id == "mc_campfire" ) or ( e.target.object.id == "mc_campfire_perm" ) or ( e.target.object.id == "lm_cookstove") or string.startswith(e.target.object.id,"light_logpile") then
