...
local this = {}
local thing, itemID, skillID, itemObject, skillValue, numberMaterial, menu, recipeNum, hasPick, hasNordPick, viewThing2
...
local function tipStats()
	local stats = "", baseDiff, effDiff, effChance, currDiff
	local recNum = 0
	--local viewThing2 = tes3.getPlayerTarget()
	if viewThing2 ~= nil then
		local viewThingID = viewThing2.baseObject.id
		local qty = viewThing2.data.mcMaterial + 1
		if qty == nil then qty = 1 end
		for i, x in ipairs(recipes) do
			if viewThingID == recipes[i].id then
				recNum = i
			end
		end
		if (recNum ~= 0) then
			if (viewThing2.data.mcShattered) and (viewThing2.data.mcShattered == 0) then
				baseDiff = mc.ttDifficulty(recipes[recNum].difficulty)
				currDiff = qty * recipes[recNum].difficulty
				effDiff = recipes[recNum].difficulty * qty * mc.calcAttr().modInt * mc.calcAttr().modAgi
				effChance = mc.ttChance(skillID, currDiff)
				effDiff = mc.ttDifficulty(effDiff)
				currDiff = mc.ttDifficulty(currDiff)
				stats = "\nDifficulty: "..currDiff
				if config.feedback ~= "Off" then
					if currDiff ~= effDiff then
						stats = stats.." (handicapped to: "..effDiff..")"
					end
				end
...
local function extraTooltip(e)
	...
	viewThing2 = tes3.getPlayerTarget()
	if e.object.objectType == tes3.objectType.container then
		...
		if e.reference.data.mcMaterial then
			...
			if miningDesc ~= nil then
				...
				miningDesc = miningDesc.." "..tipStats()
...
function this.onFastGather()
	...
	until(doneFlag == true) -- Possibly busted rock
	if busted == false then
		tes3.playSound{ sound = "critical damage", volume = 1.0, pitch = 1.0 }
		thing.data.mcShattered = 1
		timer.delayOneFrame(function()
			tes3.setGlobal("mc_skill_results", -1)
		end)
	else -- Rock wasn't busted, pick is instead
		tes3.playSound{ sound = "critical damage", volume = 1.0, pitch = 1.0 }
		tes3.messageBox({ message = "Your pick has broken."})
	end
...
local function checkPick()
    ...
    local x = (
		...
		or inventory:contains("steel miner's pick")
	)
	return x
end
...
function this.onAttack()
	...
	if config.animatedMining == true then
		...
		if tes3.mobilePlayer then
			...
			and (config.animatedMining == true) then -- check to see if a valid ore-source
				recipeNum = 0
...
function this.createWindow()
	...
	elseif numberMaterial > 1 then
		labelText = "You see a deposit that has been mined "..numberMaterial.." times. "
	else
	...
	labelText = tipStats()
	local label1 = menu:createLabel{ text = labelText }
