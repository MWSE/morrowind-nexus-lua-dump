--[[ Crafting of items using Pottery Wheel.
...
local Recipes = require("Morrowind_Crafting_3.Pottery_Wheel.recipes")
...
local textFilter = ""
local sortBy = "Normal"
local tGroup = { "All", "Urns", "Bowls", "Cups", "Pots", "Pitchers", "Liquor Bottles", "Miscellaneous" }
local tGroupIdx = 1
...
-- Register IDs
UID_ListPane = tes3ui.registerID("MasonryMenu::List")
UID_ListLabel = tes3ui.registerID("MasonryMenu::ListBlockLabel")
UID_classLabel = tes3ui.registerID("MasonryMenu::menuLabel")
UID_groupLabel = tes3ui.registerID("Masonry::menuLabel")
UID_filterText = tes3ui.registerID("MasonryMenu::Input")

function this.init()
	this.id_menu = tes3ui.registerID("MasonryMenu")
	this.id_menulist = tes3ui.registerID("Masonrylist")
	this.id_cancel = tes3ui.registerID("Masonry_cancel")
	...
	--
	Recipes = mc.getAddInFiles("Pottery", Recipes) -- Get add-in recipes
	Recipes = mc.cleanDeprecated(Recipes)
end
...
local function onClickSelectedItem(e)
	if mc3_timeOut then
		...
		local recipeNum = e.source:getPropertyInt("MasonryMenu:Index")
...
local function showMasonryTooltip(e)
	config = mwse.loadConfig(configPath)
	local itemDesc
	local recipeNum = e.source:getPropertyInt("MasonryMenu:Index")
		...
		if (thing ~= nil) then
			...
			if ttemp then
				...
				if ttemp == true then
					...
					itemBlock:setPropertyInt("MasonryMenu:Index", index)
					if haveMaterial == true then
						itemBlock:register("mouseClick", onClickSelectedItem)
					end
					itemBlock:register("help", showMasonryTooltip)
...
local function onEquip(e)
	--if (e.activator == tes3.player) then
		if e.item.id == "<Activator_ID>" then
...
local function onActivate(e)
	if (e.activator == tes3.player) then
		if (e.target.object.id == "mc_pottery_wheel_p") or (e.target.object.id == "mc_potterywheel02_p") then
			if mc.uninstalling() == false then
				if not tes3.menuMode() then
					currentKit = e.target
				end
		...
		elseif e.target.object.id == "mc_pottery_wheel_perm" then
			if not tes3.menuMode() then
				currentKit = e.target
			end
			-- check to see if the ritual for creating Daedric ebony has been completed (true/false)
