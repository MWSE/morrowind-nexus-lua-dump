...
local Recipes = require("Morrowind_Crafting_3.Loom.recipes")
...
local textFilter = ""
local sortBy = "Normal"
local tGroup = { "All", "Cloth Bolts", "Cloth Rugs", "Zodiac Tapestries", "Common Tapestries", "Expensive Tapestries",
				"Elegant Tapestries", "Extravagant Tapestries", "Exquisite Tapestries" }
local tGroupIdx = 1
...
-- Register IDs
UID_ListPane = tes3ui.registerID("weavingMenu::List")
UID_ListLabel = tes3ui.registerID("weavingMenu::ListBlockLabel")
UID_classLabel = tes3ui.registerID("weavingMenu::menuLabel")
UID_groupLabel = tes3ui.registerID("weavingMenu::menuLabel")
UID_filterText = tes3ui.registerID("SmithingMenu::Input")

function this.init()
	this.id_menu = tes3ui.registerID("weavingMenu")
	this.id_menulist = tes3ui.registerID("weavinglist")
	this.id_cancel = tes3ui.registerID("weaving_cancel")
	...
	--
	Recipes = mc.getAddInFiles("Loom", Recipes) -- Get add-in recipes
	Recipes = mc.cleanDeprecated(Recipes)
end
...
local function onClickSelectedItem(e)
	if mc3_timeOut then
		...
		local recipeNum = e.source:getPropertyInt("WeavingMenu:Index")
		local SkillValue = mc.fetchSkill(skillID)
		local thing = tes3.getObject(Recipes[recipeNum].id)
		local batchSize = Recipes[recipeNum].yieldCount
		...
		if mc.skillCheck(skillID, Recipes[recipeNum].difficulty) == true then -- Succeeded
			...
			tes3.addItem({ reference = tes3.player, item = Recipes[recipeNum].id, count = batchSize, playSound = false })
...
local function showWeavingTooltip(e)
	config = mwse.loadConfig(configPath)
	local itemDesc
	local recipeNum = e.source:getPropertyInt("WeavingMenu:Index")
	...
	for index, x in ipairs(Recipes) do
		...
		if ttemp then
			...
			if ttemp == true then
				...
				itemBlock:setPropertyInt("WeavingMenu:Index", index)
				if haveMaterial == true then
					itemBlock:register("mouseClick", onClickSelectedItem)
				end
				itemBlock:register("help", showWeavingTooltip)
...
local function onActivate(e)
	if (e.activator == tes3.player) then
		if (e.target.object.id == "mc_loom_p") or (e.target.object.id == "mc_loom01_p") or (e.target.object.id == "mc_loom01_p") or (e.target.object.id == "mc_loom01_perm") then
		...
		elseif e.target.object.id == "mc_loom_perm" then
			if not tes3.menuMode() then
				currentKit = e.target
			end
			this.createWindow()
