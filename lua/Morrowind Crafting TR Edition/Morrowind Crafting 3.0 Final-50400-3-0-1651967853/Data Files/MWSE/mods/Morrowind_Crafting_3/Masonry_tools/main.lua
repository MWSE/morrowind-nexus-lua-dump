--[[ Crafting of items using MasonWorking kit.
...
local Recipes = require("Morrowind_Crafting_3.Masonry_Tools.recipes")
local skillModule = require("OtherSkills.skillModule")
local mc = require("Morrowind_Crafting_3.mc_common")
local shiftKey = 0
local this = {}
local skillID = "mc_Masonry"
...
local textFilter = ""
local sortBy = "Normal"
local tGroup = { "All", "Miscellaneous", "Foundations", "Furnishings", "Forge/Hearth" }
local tGroupIdx = 1
...
-- Register IDs
UID_ListPane = tes3ui.registerID("MasonWorkingMenu::List")
UID_ListLabel = tes3ui.registerID("MasonWorkingMenu::ListBlockLabel")
UID_classLabel = tes3ui.registerID("MasonWorkingClass::menuLabel")
UID_groupLabel = tes3ui.registerID("MasonWorkingGroup::menuLabel")
UID_filterText = tes3ui.registerID("MasonWorkingFilterMenu::Input")

function this.init()
	this.id_menu = tes3ui.registerID("MasonWorkingMenu")
	this.id_menulist = tes3ui.registerID("MasonWorkinglist")
	this.id_cancel = tes3ui.registerID("MasonWorking_cancel")
	...
	--
	Recipes = mc.getAddInFiles("Masonry", Recipes) -- Get add-in recipes
end
...
local function onClickSelectedItem(e)
	if mc3_timeOut then
		...
		local recipeNum = e.source:getPropertyInt("MasonWorkingMenu:Index")
		...
		if mc.skillCheck(skillID, Recipes[recipeNum].difficulty) == true then -- Succeeded
            ...
            tes3.addItem({ reference = tes3.player, item = Recipes[recipeNum].id, 
                count = Recipes[recipeNum].yieldCount, playSound = false })
            if Recipes[recipeNum].byProduct then
                tes3.addItem({ reference = tes3.player, item = Recipes[recipeNum].byProduct.id, 
                    count = Recipes[recipeNum].byProduct.count, playSound = false })
            end
		...
		mc.timePass( 0.1 * Recipes[recipeNum].taskTime )
...
function this.createWindow()
	...
	local filterBlock = menu:createBlock({})
		...
		filterTextInput.widthProportional = .4
		...
		buttonGroup.widthProportional = .6
	...
	for index, x in ipairs(Recipes) do
		...
		if ttemp then
			...
			if ttemp == true then
				...
				itemBlock:setPropertyInt("MasonWorkingMenu:Index", index)
...
event.register("keyDown", onKeyDown, { filter = 42 })
event.register("keyDown", onKeyDown, { filter = 54 })
event.register("keyUp", onKeyUp, { filter = 42 })
event.register("keyUp", onKeyUp, { filter = 54 })

UIID_CraftingMenu_ListBlock_Label = tes3ui.registerID("MasonWorkingMenu:ListBlockLabel")
event.register("initialized", this.init)

local function onEquip(e)
	--if (e.activator == tes3.player) then
		if e.item.id == "mc_masonry_kit" then
