--[[ Morrowind Crafting Precision Placement (MCPP)     c/r 2019, Drac and Toccatta
...
local function snapToGrid() -- Snap to whole degrees, x/y/z coords, whole angle degrees, 1/100th unit scale
    ...
    thing.position = tes3vector3.new(math.round(x,0), math.round(y,0), math.round(z,0))
...
local function onKeyUpEnd(e) -- Exit 'moving' mode
    if active == true then
        ...
        if mc.getCtrl() == true then
            mwse.log("[MCPP Export: ID = "..thing.id..string.format("  Location = (%0.4f, %0.4f. %0.4f)  Rotation = (%0.4f, %0.4f. %0.4f), Scale = (%0.2f)", thing.position.x, thing.position.y, thing.position.z, math.deg(thing.orientation.x), math.deg(thing.orientation.y), math.deg(thing.orientation.z), thing.scale))
        end
        active = false
        granuChoice = 2
        axisChoice = 1
