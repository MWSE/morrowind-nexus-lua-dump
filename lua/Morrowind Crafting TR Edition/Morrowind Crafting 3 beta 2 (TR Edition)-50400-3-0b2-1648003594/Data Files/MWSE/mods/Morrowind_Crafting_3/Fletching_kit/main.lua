--[[ Crafting of items using Fletching kit.
...
local Recipes = require("Morrowind_Crafting_3.Fletching_kit.recipes")
local skillModule = require("OtherSkills.skillModule")
local mc = require("Morrowind_Crafting_3.mc_common")

local this = {}
local skillID = "mc_Fletching"
...
local sortBy = "Normal"
local tGroupIdx = 1
local tGroup = { "All", "Arrows", "Bolts", "Darts" }
...
-- Register IDs
UID_ListPane = tes3ui.registerID("FletchingMenu::List")
UID_ListLabel = tes3ui.registerID("FletchingMenu::ListBlockLabel")
UID_classLabel = tes3ui.registerID("FletchingMenu::menuLabel")
UID_groupLabel = tes3ui.registerID("Fletching::menuLabel")
UID_filterText = tes3ui.registerID("SmithingMenu::Input")

function this.init()
	this.id_menu = tes3ui.registerID("FletchingMenu")
	this.id_menulist = tes3ui.registerID("Fletchinglist")
	this.id_cancel = tes3ui.registerID("Fletching_cancel")
	...
	--
	Recipes = mc.getAddInFiles("Fletching", Recipes) -- Get add-in recipes
end
...
local function onClickSelectedItem(e)
	...
	local recipeNum = e.source:getPropertyInt("FletchingMenu:Index")
...
local function showFletchingTooltip(e)
	config = mwse.loadConfig(configPath)
	local recipeNum = e.source:getPropertyInt("FletchingMenu:Index")
		...
		if thing ~= nil then
			...
			if ttemp then
				...
				if ttemp == true then
					...
					itemBlock:setPropertyInt("FletchingMenu:Index", index)
					if haveMaterial == true then
						itemBlock:register("mouseClick", onClickSelectedItem)
					end
					itemBlock:register("help", showFletchingTooltip)
...
UIID_CraftingMenu_ListBlock_Label = tes3ui.registerID("FletchingMenu:ListBlockLabel")
event.register("initialized", this.init)

local function onEquip(e)
	--if (e.activator == tes3.player) then
		if e.item.id == "mc_fletching_kit" then
