--[[ Script for showing special tooltips outside of other tools
    ...
    function this.createWindow()
        ...
        local thing = tes3.rayTest{
            position = tes3.getCameraPosition(),
            direction = tes3.getCameraVector() } -- get item reference (if any)
        ...
        if thing.reference.data.alias then
            textFilter = thing.reference.data.alias
        else
            textFilter = ""
        end
		...
		filterTextInput:register("keyEnter", 
			function()
			local textMC = filterTextInput.text
			if (textMC == "") then
				textFilter = ""
			else
                textFilter = textMC
                forceInstance(thing.reference)
                if textFilter == " " then textFilter = nil end
                thing.reference.data.alias = textFilter
    ...
    local function makeLabel()
        ...
        if mc.getAlt() == true then --Verify is container & unowned
            local thing = tes3.rayTest{
                position = tes3.getCameraPosition(),
                direction = tes3.getCameraVector() } -- get item reference (if any)
            if thing and mc.playerAllowed(thing.reference) and (thing.reference.object.objectType == tes3.objectType.container) then
                this.createWindow()
			else
