...
local function onMenuInventorySelectActivated(e) -- Adding a 'switch' button to the showInventorySelect menu
	...
	local smeltingButton = buttonContainer:createButton({ id = "btnInventorySelectSwitchToSmithing", text = "Switch to Smithing" })
	local label = buttonContainer:createLabel({ id = UID_spacerLabel, text = "" })
	label.widthProportional = 1.0
    smeltingButton:register("mouseClick", function(clickEvent)
		...
		-- do nothing
			...
			if x.id == e.item.id then
    	        ...
    	        -- Got this far, must be enough. Add the material to the player's inventory
					...
					if (x.byproduct) then
						tes3.addItem({ reference = tes3.player, item = x.byproduct.id, count = batchSize * x.byproduct.yield})
					end
