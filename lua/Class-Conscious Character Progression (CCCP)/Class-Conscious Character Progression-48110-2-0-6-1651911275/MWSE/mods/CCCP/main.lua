...
local barIds = {
    health = tes3ui.registerID("MenuStat_health_fillbar"),
...
-- Sometimes when max health/magicka is changed the game won't actually update the display unless we force it to.
local function updateMenu(stat, menu)
    if menu then
        local statMax = tes3.mobilePlayer[stat].baseRaw
        local statCurrent = tes3.mobilePlayer[stat].currentRaw
...
-- Returns the magnitude of the effect that's caused by abilities.
...
local function setMagicka(fromGameLoaded, fromInitCalcs)
    ...
    local currentTotalFMMag = tes3.getEffectMagnitude{
        reference = tes3.player,
        effect = tes3.effect.fortifyMagicka,
    }

    -- The Fortify Magicka magnitude that's caused by abilities. This is important because Fortify Magicka abilities
    -- should always affect max magicka (unlike other sources of the effect, which only affect current magicka).
    local currentAblFMMag

    if currentTotalFMMag == 0 then
        currentAblFMMag = 0
    else
        currentAblFMMag = getAbilityMag(tes3.effect.fortifyMagicka)
    end
    ...
    debugMsg(string.format("currentTotalFMMag: %f", currentTotalFMMag))
    debugMsg(string.format("newCurrentMagicka: %f", newCurrentMagicka))

    -- If the player is under a Fortify Magicka effect (or was last frame), we need to compensate for that. This is
    -- because Fortify Magicka increases the ratio between current and max magicka. The problem is basically the same as
    -- with Fortify Health in the health calculations, except there's no related MCP feature so it's simpler.
    if currentTotalFMMag > 0
    or ( savedData.lastFrameTotalFMMag
    and savedData.lastFrameTotalFMMag > 0 ) then
        ...
        if fortifyMAXMagickaEnabled then
            ...
            -- Any Fortify Magicka magnitude caused by abilities increases max magicka.
            newMaxMagicka = newMaxMagicka + currentAblFMMag
            newCurrentMagicka = newMaxMagicka * tes3.mobilePlayer.magicka.normalized
            ...
            mobilePlayer because it's already been changed. Therefore, we need to save these values in our persistent
            player data, which we do at the bottom of this function and on enterFrame. We do a nil check here just in
            case, but these values should always exist. ]]--
            if savedData.lastMaxMagicka
            and savedData.lastFrameCurrentMagicka
            and savedData.lastFrameTotalFMMag
            and savedData.lastFrameAblFMMag then

                -- What max magicka would have been last frame without Fortify Magicka. Also, sanity check, these values
                -- should never be negative.
                local maxMagickaWouldBe = savedData.lastMaxMagicka - savedData.lastFrameAblFMMag

                -- What the player's (last frame) current magicka would be if not for Fortify Magicka.
                local currentMagickaWouldBe = savedData.lastFrameCurrentMagicka - savedData.lastFrameTotalFMMag

                -- This is what the magicka ratio would be if not for Fortify Magicka. This is the ratio we'll be
                -- maintaining.
                local ratioWouldBe

                --[[ If this function is being run right after initial calculations (i.e. right after chargen), and the
                player has a racial/birthsign Fortify Magicka ability, magicka ratio can get totally screwed up because
                that ability isn't effective yet but the mod is expecting it to be. So just maintain the actual current
                ratio in that case. ]]--
                if fromInitCalcs then
                    ratioWouldBe = tes3.mobilePlayer.magicka.normalized
                else
                    ratioWouldBe = safeGetRatio(currentMagickaWouldBe, maxMagickaWouldBe)
                end

                -- What the new max magicka will be after any ability-caused Fortify Magicka magnitude goes away.
                local maxMagickaWillBe = newMaxMagicka - currentAblFMMag
                ...
                --[[ Add back the Fortify Magicka magnitude, to ensure the ratio is correct after it wears off. The same
                problem exists here as with health - when the calculated "would/will be" values for current/max magicka
                are negative (or 0 in the case of max magicka), weirdness results, so just maintain the existing ratio.
                ]]--
                if maxMagickaWouldBe > 0
                and currentMagickaWouldBe >= 0
                and maxMagickaWillBe > 0 then
                    newCurrentMagicka = currentMagickaWillBe + currentTotalFMMag
                else
                    local ratioToMaintain = safeGetRatio(savedData.lastFrameCurrentMagicka, savedData.lastMaxMagicka)
                    newCurrentMagicka = newMaxMagicka * ratioToMaintain
                end

                debugMsg(string.format("savedData.lastMaxMagicka: %f", savedData.lastMaxMagicka))
                debugMsg(string.format("savedData.lastFrameCurrentMagicka: %f", savedData.lastFrameCurrentMagicka))
                debugMsg(string.format("savedData.lastFrameTotalFMMag: %f", savedData.lastFrameTotalFMMag))
                debugMsg(string.format("savedData.lastFrameAblFMMag: %f", savedData.lastFrameAblFMMag))
                debugMsg(string.format("currentTotalFMMag: %f", currentTotalFMMag))
                debugMsg(string.format("currentAblFMMag: %f", currentAblFMMag))
                debugMsg(string.format("newMaxMagicka: %f", newMaxMagicka))
                debugMsg(string.format("maxMagickaWouldBe: %f", maxMagickaWouldBe))
                debugMsg(string.format("currentMagickaWouldBe: %f", currentMagickaWouldBe))
                debugMsg(string.format("ratioWouldBe: %f", ratioWouldBe))
                debugMsg(string.format("maxMagickaWillBe: %f", maxMagickaWillBe))
                debugMsg(string.format("currentMagickaWillBe: %f", currentMagickaWillBe))
                debugMsg(string.format("newCurrentMagicka: %f", newCurrentMagicka))

                --[[ There's a problem that can happen when the following two things both happen in the same frame: (1)
                intelligence changes while under a Fortify Magicka effect, and (2) there is a gain/loss of current
                magicka due to some other cause, such as spellcasting. This mod knows what ratio to maintain based on
                the current/max magicka on the previous frame, before the magicka change from the other cause. If
                there's been some change in current magicka, we need to compensate for that; otherwise, that change
                would basically be reverted (causing, for example, the player to be able to cast a spell for free). ]]--
                local diff = 0

                --[[ And there's another problem. We can only compensate for the above when the Fortify Magicka
                magnitude has not also changed this frame. If it has (in other words, add "(3) Fortify Magicka magnitude
                has changed" to the list above of things that all have to happen in the same frame for this to be a
                problem), we can't compensate for that because the outcome will be different depending on what order the
                game applies the intelligence and FM magnitude change in, and we don't know what that order is. This is
                why we check to make sure FM magnitude hasn't changed here, to avoid unpredictable results.

                What this means is that, for example, in the unlikely event that the player casts a spell and, in the
                exact same frame that they lose current magicka due to the spellcast, their intelligence and FM
                magnitude both also change, the player will get the spell for free, with no magicka cost.

                Fortunately, the loss of magicka from casting a spell and the effects of that spell don't happen on the
                same frame, so it would be very difficult to intentionally exploit this - it would require extremely
                precise timing. However, it could still happen very rarely by accident. ]]--
                if currentTotalFMMag == savedData.lastFrameTotalFMMag then

                    -- What the ratio actually was last frame. This is the same as what it *should* be right now
                    -- assuming no gain/loss of magicka this frame.
                    local ratioShouldBe

                    if savedData.lastMaxMagicka <= 0 then
                        ratioShouldBe = 0
                    else
                        ratioShouldBe = math.max(savedData.lastFrameCurrentMagicka / savedData.lastMaxMagicka, 0)
                    end

                    -- What current magicka should be right now if there were no gain/loss in the past frame.
                    local curMagShouldBe = tes3.mobilePlayer.magicka.base * ratioShouldBe
                    diff = tes3.mobilePlayer.magicka.current - curMagShouldBe

                    debugMsg("There is a Fortify Magicka magnitude and it has not changed since the last frame. Compensating for any gain/loss of current magicka in the past frame.")
                    debugMsg(string.format("Magicka ratio was last frame: %f", ratioShouldBe))
                    debugMsg(string.format("Current magicka is now (before correction): %f", tes3.mobilePlayer.magicka.current))
                    debugMsg(string.format("Current magicka should be now (before correction) if there were no magicka gain/loss in the past frame: %f", curMagShouldBe))
                    debugMsg(string.format("Difference: %f", diff))
                end

                -- Sometimes there will be an extremely small difference (something like 0.000002) when there shouldn't
                -- be. This check allows us to ignore those tiny differences.
                if math.abs(diff) > 0.01 then
                    newCurrentMagicka = newCurrentMagicka + diff

                    debugMsg("There is a difference between what current magicka is and what it should be, indicating magicka gain/loss in the past frame.")
                    debugMsg(string.format("Current magicka needs to be to account for this difference: %f", newCurrentMagicka))
                end
            end
        end
    end
    ...
    updateDisplay("magicka")

    -- Save these values in player data, so we can refer to them next time if needed. (They're actually also saved on
    -- enterFrame and a couple other circumstances, but just in case.)
    savedData.lastMaxMagicka = newMaxMagicka
    savedData.lastFrameCurrentMagicka = newCurrentMagicka
    savedData.lastFrameTotalFMMag = currentTotalFMMag
    savedData.lastFrameAblFMMag = currentAblFMMag
...
local function setHealth()
    ...
    -- The player has disabled health management in the MCM. Hopefully the player is using some other mod like MWSE
    -- State-Based Health that manages health without relying on the vanilla levelup mechanic; otherwise, max health
    -- will NEVER increase.
    if not config.manageHealth then
        debugMsg("Health management is disabled! Skipping health calculations.")
        return
    end
    ...
    -- The magnitude of Fortify Health that affects max health.
    local fortifyHealthAffectingMax = 0

    -- If the player has the "fortify maximum health" MCP feature enabled, then all Fortify Health affects increase max
    -- as well as current health. Otherwise, only Fortify Health effects granted by abilities affect max health. (When
    -- Fortify Health wears off, max health will be lowered to the calculated value.)
    ...
    debugMsg(string.format("fortifyHealthAffectingMax: %f", fortifyHealthAffectingMax))
    debugMsg(string.format("newMaxHealth: %f", newMaxHealth))
    ...
    if fortifyHealthMagnitude > 0 then
        ...
        -- What the player's (present) max health would be if not for the Fortify Health effect. math.max is a sanity
        -- check - this should never be negative.
        local maxHealthWouldBe = tes3.mobilePlayer.health.base - fortifyHealthAffectingMax
        ...
        -- And what the player's *new* max health would be without Fortify Health.
        ...
        local ratioWouldBe = safeGetRatio(currentHealthWouldBe, maxHealthWouldBe)
    ...
    -- Force the game to update the health display.
...
-- We need to save this data every frame, on game load, and after initial calculations.
local function saveMagickaInfo()
    savedData.lastFrameCurrentMagicka = tes3.mobilePlayer.magicka.current
    savedData.lastMaxMagicka = tes3.mobilePlayer.magicka.base

    savedData.lastFrameTotalFMMag = tes3.getEffectMagnitude{
        reference = tes3.player,
        effect = tes3.effect.fortifyMagicka,
    }

    if savedData.lastFrameTotalFMMag == 0 then
        savedData.lastFrameAblFMMag = 0
    else
        savedData.lastFrameAblFMMag = getAbilityMag(tes3.effect.fortifyMagicka)
    end
end
...
local function onInitialized()
    ...
    local needMod = string.format("[%s %s] This mod requires Attribute Effect Tweaks. Please install Attribute Effect Tweaks to use this mod.", mod, version)

    if (not attributeEffectTweaks)
    or (not attributeEffectTweaks.enabled) then
    ...
    -- Set this here on initialized so we only need to check it once (changing this setting in Fortify MAX requires a
    -- restart).
    if fortifyMAX then
...
event.register("initialized", onInitialized, { priority = -20 })
