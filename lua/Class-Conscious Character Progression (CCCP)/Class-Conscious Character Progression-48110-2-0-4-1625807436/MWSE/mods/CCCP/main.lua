    ...
    and savedData.lastFrameTotalFMMag > 0 ) then
        ...
        if fortifyMAXMagickaEnabled then
            ...
            and savedData.lastFrameAblFMMag then
                ...
                elseif maxMagickaWouldBe == 0 then
                    ratioWouldBe = 0
                else
                ...
                -- problem exists here as with health - when the calculated "would be" values for current/max magicka
                -- are negative, weirdness results, so just maintain the existing ratio.
                if maxMagickaWouldBe >= 0
                and currentMagickaWouldBe >= 0 then
...
local function setHealth()
    ...
    if fortifyHealthMagnitude > 0 then
        ...
        --[[ Add back Fortify Health magnitude, to ensure the ratio is correct after it wears off. If either of the
        values calculated for what current/max health would be without Fortify Health are negative, the results are
        really weird and very incorrect. (This can happen, for example, if the player takes damage such that their
        current health is less than their Fortify Health magnitude.) In this case, nevermind, just maintain the existing
        ratio. ]]--
        if maxHealthWouldBe >= 0
        and currentHealthWouldBe >= 0 then
