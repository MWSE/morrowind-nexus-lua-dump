...
local function onUpdate(dt)
    ...
    if not recoveryTimerInit and types.Player.isCharGenFinished(self) then
        restCounter = storage.playerSection('F_GeneralSettings'):get('RestRecovery')*hourSeconds
        core.sendGlobalEvent('onRestCounterReceived', restCounter)
        recoveryTimer()
    ...
    if recoveryTimerComplete then
        local restRecovery = storage.playerSection('F_GeneralSettings'):get('RestRecovery')
