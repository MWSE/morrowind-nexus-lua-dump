...
function config.GetPlayerFirstName()
    local names = {}
    for n in tes3.player.object.name:gmatch("%w+") do
        names[#names + 1] = n
    end
    return names[1]
end

function config.GetPlayerLastName()
    local names = {}
    for n in tes3.player.object.name:gmatch("%w+") do
        names[#names + 1] = n
    end
    return names[#names] or names[1]
end
...
function config.GetHour()
    return string.format("%02d",math.floor(tes3.getGlobal("GameHour")))
end

function config.GetMinute()
    return string.format("%02d",math.floor((tes3.getGlobal("GameHour") % 1) * 60))
end

function config.GetSecond()
    return string.format("%02d",math.floor(((tes3.getGlobal("GameHour") * 60) % 1) * 60))
end

function config.GetAmPmHour()
    local hour = tonumber(config.GetHour())
    return hour == 0 and "12" or string.format("%02d", 12 - -hour % 12)
end

function config.GetAmPm()
    return tonumber(config.GetHour()) < 12 and "am" or "pm"
end
...
function config.SetGlobal(name, value)
    tes3.setGlobal(name, value)
    return ""
end

function config.ExecuteCode(code)
    return loadstring(code) or ""
end
