tes3.claimSpellEffectId("shockAura", 24014)
        ...
        for k = 1, #hostileList do
            if mobileList[i].object.id == hostileList[k].object.id then
                ...
                tickParams:trigger({
                    ...
                    attribute = tes3.effectAttribute.resistShock,
                ...
                local vfx = tes3.createVisualEffect ({reference = nearbyActor.reference, object = "VFX_LightningHit", lifespan = 1})
                -- local executed = tes3.playSound ({sound = "shock hit", reference = nearbyActor.reference, volume = 0.5})
            end
        end
    end
    tickParams:trigger({})
...
event.register(tes3.event.magicEffectsResolved, function()
    tes3.addMagicEffect({
        ...
        icon = "IB\\IB_s_shock_aura.tga",
        particleTexture = "vfx_map39.tga",
        hitVFX = "VFX_LightningShield",
