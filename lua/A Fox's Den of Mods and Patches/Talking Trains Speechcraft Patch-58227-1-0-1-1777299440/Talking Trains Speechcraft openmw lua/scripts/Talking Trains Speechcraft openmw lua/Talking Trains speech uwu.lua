...
local function debugPrint(msg)
    if settings:get("debugLogging") then
        print("[" .. MOD_NAME .. "] " .. msg)
    end
end
...
local function markNPCSpoken(npcId)
    ...
    debugPrint("Marked NPC as spoken: " .. npcId)
end
    ...
    settings = {
        {
            ...
            renderer = "checkbox",
            name = "Debug Logging",
            description = "Enable debug messages in the console log.",
            default = false
        },
        {
    ...
    engineHandlers = {
        ...
        onLoad = function(data)
            if data then
                ...
                debugPrint("Loaded spokenNPCs: " .. #spokenNPCs .. " entries")
            end
        end,
        ...
        UiModeChanged = function(data)
            if data.newMode == 'Dialogue' and data.oldMode == nil and not doonce then
                ...
                if not npc or npc.type ~= types.NPC then
                    debugPrint("No valid NPC in dialogue; skipping progress update")
                    return
                end
                local npcId = npc.id
                ...
                debugPrint("Before: Base = " .. speechcraft.base .. ", Progress = " .. currentProgress .. ", NPC = " .. npcId .. ", Gain = " .. progressGain)

                -- Apply the new progress
                speechcraft.progress = newProgress

                debugPrint("After Progress Update: Base = " .. speechcraft.base .. ", Progress = " .. speechcraft.progress)
                ...
                if currentProgress < 1.0 and newProgress >= 1.0 then
                    ...
                    debugPrint("Level Up Triggered: Base = " .. speechcraft.base .. ", Progress = " .. speechcraft.progress)
                end
                ...
                debugPrint("Final: Base = " .. speechcraft.base .. ", Progress = " .. speechcraft.progress)
