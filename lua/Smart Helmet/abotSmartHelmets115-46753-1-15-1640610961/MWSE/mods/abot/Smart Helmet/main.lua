...
Store preferred helmet pressing Alt key while equipping the helmet
]]
...
unequipPreferred = false, -- Allow automatic unequipping of preferred helmet (equipped while pressing Alt key)
swapHotKey = {keyCode = tes3.scanCode.z, isShiftDown = false, isAltDown = true, isControlDown = false}, -- default ALt+z
...
--[[
reference: tes3reference. Read-only. A reference to the actor that is equipping the item.
...
local function cellChanged(e)
	...
	if equippedHelmetStack then
		if not doEquip then
			...
			if lastHelmetId then
				if not config.unequipPreferred then
					local ob = equippedHelmetStack.object
					if ob then
						if ob.id == lastHelmetId then
							return -- skip unequipping preferred helmet
						end
					end
				end
			end
...
local function keyDown(e)
	...
	local swapHotKey = config.swapHotKey
	if not (keyCode == swapHotKey.keyCode) then
		return
	end
	if not (e.isAltDown == swapHotKey.isAltDown) then
		return
	end
	if not (e.isShiftDown == swapHotKey.isShiftDown) then
		return
	end
	if not (e.isControlDown == swapHotKey.isControlDown) then
		return
	end
...
local function modConfigReady()
	...
	controls:createYesNoButton{
		label = "Swap helmet according to area danger",
		variable = createConfigVariable("autoSwapHostiles"),
		description = [[Automatically swap player helmet on cell change depending on hostiles distance. Default: Yes.]]
	}

	controls:createSlider{
		label = "Min actor AI Fight setting to be judged hostile",
		variable = createConfigVariable("minActorAiFightTrigger"),
		min = 78, max = 100, step = 1, jump = 5,
		description =
[[Min actor AI Fight setting to be judged hostile. Default: 82.
Only effective when "Swap helmet according to area danger" option is Yes.]]
	}

	controls:createSlider{
		label = "Max distance of hostile actor from player to trigger helmet equipping",
		variable = createConfigVariable("maxHostileDistanceTrigger"),
		min = 1000, max = 7000, step = 1, jump = 100,
		description =
[[Max distance of hostile actor from player to trigger helmet equipping. Default: 3500.
Only effective when "Swap helmet according to area danger" option is Yes.]]
	}

	controls:createYesNoButton{
		label = "Allow automatic swapping of enchanted helmets",
		variable = createConfigVariable("autoSwapEnchanted"),
		description = [[Allow automatic swapping of enchanted helmets. Default: Yes.]]
	}

	controls:createYesNoButton{
		label = "Automatically equip any enchanted vision helmet in the dark",
		variable = createConfigVariable("autoSwapLight"),
		description =
[[Automatically equip any enchanted vision helmet in the dark. Default: Yes.
Effective only when "Allow automatic swapping of enchanted helmets" is Yes.]]
	}

	controls:createSlider{
		label = "Light threshold to consider an interior dark",
		variable = createConfigVariable("lightThreshold"),
		min = 1, max = 30, step = 1, jump = 5,
		description =
[[Light threshold to consider an interior dark.
Effective only when "Automatically equip any enchanted vision helmet in the dark" option is set to Yes.]]
	}

	controls:createYesNoButton{
		label = "Allow automatic helmet swapping according to weather",
		variable = createConfigVariable("autoSwapWeather"),
		description =
[[Allow automatic helmet swapping according to weather. Default: Yes.
Mostly a cosmetic/roleplay thing (it is raining, let's put an helmet on to avoid ruining our hairstyle)
unless you have some other mod implementing weather penalties/weather protective helmets.]]
	}

	controls:createYesNoButton{
		label = "Allow automatic unequipping of preferred helmet",
		variable = createConfigVariable("unequipPreferred"),
		description =
[[Allow automatic unequipping of an helmet set as preferred one. Default: No.
You can store your preferred helmet pressing Alt key while equipping the helmet.]]
	}

	controls:createKeyBinder({
		label = 'Helmet swap Hotkey', allowCombinations = true,
		variable = createConfigVariable("swapHotKey"),
		description =[[Default: Alt+z.]]
	})
