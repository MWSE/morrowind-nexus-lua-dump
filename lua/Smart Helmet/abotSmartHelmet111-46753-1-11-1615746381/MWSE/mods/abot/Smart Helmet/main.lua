...
local visionEnhanceHelmetCache = {} -- cache for constant vision enchancement enchanted helmets
...
--[[
local function getCondition(obj)
	if obj.hasDurability then
		if obj.itemData then
			if obj.itemData.condition then
				return obj.itemData.condition
			end
		end
	end
	return -1
	...
	---if not visionOnly then
		if lastHelmetId then
			...
			for stack in tes3.iterate(inventory.iterator) do
				...
				if ob then
					itemId = ob.id
					---mwse.log("%s itemId = %s lastHelmetId = %s", modPrefix, itemId, lastHelmetId)
					if itemId == lastHelmetId then
						if inGoodCondition(ob) then
							mwscript.equip({reference = player, item = itemId})
							if config.logLevel > 0 then
								mwse.log("%s equipping helmet %s %s", modPrefix, itemId, ob.name)
							end
							return
						end
					end
					...
					if ob then
						if ob.objectType == ARMO then
							if ob.slot == HELMETslot then
								if inGoodCondition(ob) then
									itemId = ob.id
									found = visionEnhanceHelmetCache[itemId]
									if not found then
										if hasConstantVisionEnchantment(ob) then
											visionEnhanceHelmetCache[itemId] = true
											found = true
										end
									end
									if found then
										mwscript.equip({reference = player, item = itemId})
										if config.logLevel > 0 then
											mwse.log("%s equipping vision enhanced helmet %s %s", modPrefix, itemId, ob.name)
										end
										return
									end
								end
			...
			if ob.objectType == ARMO then
				if ob.slot == HELMETslot then
					if inGoodCondition(ob) then
						...
						if itemId then
							v = ob.value
							table.insert(helmets, { id = itemId, value = v } )
						end
					end
				end
			end
...
local function cellChanged(e)
	...
	if lastHelmetId then
		if not config.autoSwapEnchanted then -- auto swapping of enchanted helmet disabled
			local ob = tes3.getObject(lastHelmetId)
			if ob then
				if ob.enchantment then
					return -- return as enchanted helmet quipped
				end
			end
		end
	end
