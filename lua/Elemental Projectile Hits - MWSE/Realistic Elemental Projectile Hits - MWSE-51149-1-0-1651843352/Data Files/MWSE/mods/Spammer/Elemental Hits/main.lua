local mod = { name = "Elemental Projectile Hits", ver = "1.0"}
local poisonMesh
local frostMesh
local shockMesh
local fireMesh
...
local function onProjectileHit(e)
    ...
    if fire and (e.collisionPoint or e.target) then
        local boom = tes3.createReference({object = fireMesh, scale = .5, position = e.collisionPoint})
    ...
    if poison and (e.collisionPoint or e.target) then
        local boom = tes3.createReference({object = poisonMesh, scale = .5, position = e.collisionPoint})
        tes3.playSound({sound = "destruction hit", reference = e.mobile, loop = false})
        local objectHandle = tes3.makeSafeObjectHandle(boom)
        timer.start({
            duration = 10,
    ...
    if frost and (e.collisionPoint or e.target) then
        local boom = tes3.createReference({object = frostMesh, scale = .5, position = e.collisionPoint})
        tes3.playSound({sound = "destruction hit", reference = e.mobile, loop = false})
        local objectHandle = tes3.makeSafeObjectHandle(boom)
        timer.start({
            duration = 10,
    ...
    if shock and (e.collisionPoint or e.target) then
        local boom = tes3.createReference({object = shockMesh, scale = .5, position = e.collisionPoint})
        tes3.playSound({sound = "destruction hit", reference = e.mobile, loop = false})
        local objectHandle = tes3.makeSafeObjectHandle(boom)
        timer.start({
            duration = 10,
