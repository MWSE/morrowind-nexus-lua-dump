...
local function isMeleeWeaponEquipped()
    ...
    if weapon then
        ...
        return weaponType == 0 or weaponType == 1 or weaponType == 2 or weaponType == 3 or weaponType == 4 or weaponType == 5 or weaponType == 6
    end
    return false
end
...
local function onUpdate(dt)
    ...
    if triggerAttackSound and attackDelayTimer > 0 then
        ...
        if attackDelayTimer <= 0 then
            if isWeaponStance and pressTime > 0.5 and (currentTime - lastAttackSoundTime >= attackCooldown) and isMeleeWeaponEquipped() then
