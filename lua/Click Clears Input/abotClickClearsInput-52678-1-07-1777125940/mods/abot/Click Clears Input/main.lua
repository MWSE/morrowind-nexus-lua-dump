...
local function afterTextInputParentMouseClick(e)
	...
	for i = #children, 1, -1 do
		...
		if child.type == tes3_uiElementType_textInput then
			...
			if widget then
				local isPlaceHolding
				if widget['getIsPlaceholding'] then
					isPlaceHolding = widget:getIsPlaceholding()
				end
				if isPlaceHolding then
					widget:setIsPlaceholding(false) -- hopefully get rid of the buggy thing
				end
				...
				-- local hidePlaceHoldingText = false
				-- local placeHoldingText
				-- local gpt = widget.getPlaceholdingText -- bah could be nil
				-- if gpt then
					-- local placeHoldingText = widget:getPlaceholdingText()
					-- hidePlaceHoldingText = placeHoldingText
				...
				if isPlaceHolding then
					widget:setIsPlaceholding(isPlaceHolding)
				end
			end
	...
	if changed then
		...
		if menu then
			...
			if not menu:getPropertyBool('ab01clclin') then
				menu:setPropertyBool('ab01clclin', true)
				menu:registerBefore('destroy', function () enableJournalKeybind() end)
			end
			menu:updateLayout()
		end
	end
...
local function onKeyDownCtrlV(e)
	...
	if widget then
		local getPlaceholdingText = widget.getPlaceholdingText
		if getPlaceholdingText then --bah the tes3uiParagraphInput does not have it
			local placeholdingText = widget:getPlaceholdingText()
			if placeholdingText
...
'MenuInventorySelect','MenuMagic','MenuMagicSelect','MenuName','MenuSave','MenuSpellmaking'}
	...
	event.register('keyDown', onKeyDownCtrlV, {filter = tes3.scanCode.v, priority = 100000})
