...
local function rightHandWeaponRecord(actor)
  ...
  if not types.Weapon.objectIsInstance(rh) then return nil end  -- types.Weapon.objectIsInstance
...
-- Sheath-fix detection
local function isSpear(rec) return rec and rec.type == WTYPE.SpearTwoWide end
local function isStaffLikeTwoHand(rec)
  if not rec then return false end
  if rec.type ~= WTYPE.BluntTwoClose and rec.type ~= WTYPE.BluntTwoWide then return false end
  local n = rec.name or rec.id or ''
  return n:lower():find('staff', 1, true) ~= nil
end
local function needsSheatheFix(baseRec) return isSpear(baseRec) or isStaffLikeTwoHand(baseRec) end
...
local function ensureLongVariant(baseRec)
  ...
  local sheathFix = needsSheatheFix(baseRec)
  local ridLower  = lower(id)
  bucket.longByEnchant[encKey] = id
  state.varTo[ridLower] = { base = baseLower, role = 'long', needsSheatheFix = sheathFix }
  state.baseTo[baseLower].needsSheatheFix = sheathFix

  -- pre-warm sheath marker for spear/staff bases so sheath swap never waits on record creation
  if sheathFix then
    pcall(ensureSheathVariant, baseRec)
  end

  return id
end

-- ========= Sheath marker (enchant-aware) =========
local function ensureSheathVariant(baseRec)
  local baseLower  = lower(baseRec.id)
  local encKey     = _lowerIdOrNone(baseRec.enchant)
  local bucket     = _getEnchantCache(baseLower)

  local cachedId = bucket.sheathByEnchant[encKey]
  if cachedId and types.Weapon.record(cachedId) then
    return cachedId
  end

  local targetName = ((baseRec.name and #baseRec.name>0) and baseRec.name or baseRec.id) .. SUFFIX_1H
  local tryExisting = preferExistingByNameAndType(targetName, baseRec.type, baseRec.health, baseRec.enchant)
  ...
  if tryExisting then
    ...
    local draft = types.Weapon.createRecordDraft({
      template = baseRec, name = targetName, type = baseRec.type, -- 2H base type for back sheath
      health   = baseRec.health, reach = baseRec.reach, speed = baseRec.speed,
      enchant  = baseRec.enchant, enchantCapacity = baseRec.enchantCapacity or 0,
    })
    id = world.createRecord(draft).id
    dbg('Created sheath marker', id, 'for base', baseRec.id)
  end

  bucket.sheathByEnchant[encKey] = id
  state.varTo[lower(id)] = { base = baseLower, role = 'sheath', needsSheatheFix = needsSheatheFix(baseRec) }
  state.baseTo[baseLower].needsSheatheFix = needsSheatheFix(baseRec)
  return id
end
...
-- NEW: stance handler rewritten to use our variant-role map so we only swap when holding our own variants
local function onStanceChanged(actor, _, newS)
  if dbgEnabled() then print('[OHS][GLOBAL]', 'Stance changed to', tostring(newS)) end

  local rec = rightHandWeaponRecord(actor)
  if not rec then return end
  if isBoundLike(rec) then return end

  local eq  = types.Actor.getEquipment(actor)
  local cur = eq and eq[EQUIP.CarriedRight]
  if not (cur and cur:isValid()) then return end

  -- Resolve base of the equipped record
  local baseRec = resolveBaseRecordFromVariant(rec)
  if not baseRec then return end

  local ridLower = lower(rec.id or '')
  local vinfo    = state.varTo and state.varTo[ridLower] or nil

  if newS == STANCE.Nothing then
    -- Only go to the 2H sheath marker if we are currently holding our (1H) long variant
    if needsSheatheFix(baseRec) and ((vinfo and vinfo.role == 'long') or rec.type == WTYPE.LongBladeOneHand) then
      local sheathId = ensureSheathVariant(baseRec)
      if lower(sheathId) ~= ridLower then
        requestEquip(actor, getOrCreateObject(actor, sheathId), cur)
      end
    end
    return
  end

  if newS == STANCE.Weapon then
    -- Only go back to the (1H) long variant if we drew while holding our 2H sheath marker
    if needsSheatheFix(baseRec) and vinfo and vinfo.role == 'sheath' then
      local longId = ensureLongVariant(baseRec)
      if lower(longId) ~= ridLower then
        requestEquip(actor, getOrCreateObject(actor, longId), cur)
      end
    end
    return
...
return {
  ...
  eventHandlers = {
    ...
    OHS_StanceChanged = function(data)
      local a = data and data.actor
      if a and a:isValid() then onStanceChanged(a, data.old, data.new) end
    end,
