...
local actorIdBlacklist = {'hrsct','1m_cat','guar_pack'}
...
local function activate(e)
	...
	--- force cloning ASAP as we may start things before it happens naturally
	local inventory = targetObj.inventory
	if inventory then
		if (not targetObj.isInstance)
		or targetObj.organic then
			local cloned = target:clone() -- returns a boolean, autoupdates the reference
			if cloned then -- resolve container contents
				targetObj = target.object
				assert(targetObjType == targetObj.objectType)
	...
	and (skips < 1) then
		...
		elseif (targetObjType == tes3_objectType_npc)
			...
			---assert(targetMob)
			---mwse.log('>>> "%s" activating "%s".targetObjType = %s',
				---activatorMob.reference.id, target.id, mwse.longToString(targetObjType))
			if isDummy(target) then
				---mwse.log('>>> isDummy("%s")', target)
				skips = 2 -- mannequins
			else
				---mwse.log('>>> "%s".isSneaking = %s',
					---activatorMob.reference.id, activatorMob.isSneaking)
				---mwse.log('>>> "%s".targetMob.isDead = %s, ok = false',
					---targetMob.reference.id,targetMob.isDead)
				if (not activatorMob.isSneaking)
				and targetMob
		...
		elseif targetObjType == tes3_objectType_book then
			...
			if script then
				...
				and text:lower():find('short%s-pcskipequip') then
					if logLevel3 then
						mwse_log('%s: skipping "%s" pcskipequip scripted object',
				...
				if not checkSkip(activator) then
					...
					if animationData then
						...
						and animationGroups[tes3_animationGroup_pickProbe + 1] then
							if playActivationAnim(activator) > 0 then
