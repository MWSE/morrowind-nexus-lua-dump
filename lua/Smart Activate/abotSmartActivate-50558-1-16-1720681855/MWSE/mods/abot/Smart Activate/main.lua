...
local function fixImmobile(ref)
	...
	and animationController.animationData
...
local function ab01smactiPT1(e)
	...
	if logLevel2 then
		mwse.log('%s: ab01smactiPT1() playerActivate("%s")', modPrefix, ref.id, skips)
	end
...
-- Fixed at 0 while the blink animation plays.
	...
	-- blinking done
	if logLevel4 then
		mwse.log('%s: ab01smactiPT4() playerActivate("%s") timeToNextBlink = %s, headMorphTiming = %s',
...
local function ab01smactiPT3(e)
	...
	if logLevel2 then
		mwse.log('%s: ab01smactiPT3() playerActivate("%s")', modPrefix, mobRef.id)
	end
