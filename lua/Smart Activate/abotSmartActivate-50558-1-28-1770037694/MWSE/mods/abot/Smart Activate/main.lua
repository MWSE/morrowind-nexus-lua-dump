...
local doorArmAnimBlacklist = {'continue','crevice','dark'}
...
-- making it look as if it would have been hinge rotating.
	...
	if add then
		doorRef.position = doorRef.position + rotVec * data.radius
	else
		doorRef.position = doorRef.position - rotVec * data.radius
	end
	doorRef:updateSceneGraph()
...
-- also using doorRef.forwardDirection/rightDirection wuth radius is simpler
...
yAlignedWidth = %s, opposite = %s, swapHinge = %s]],
	...
	timer.start({duration = duration + 0.4,
