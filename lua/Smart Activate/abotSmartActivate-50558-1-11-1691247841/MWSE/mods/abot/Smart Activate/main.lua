...
local function ab01smacPT1(e)
	...
	if logLevel2 then
		mwse.log('%s: ab01smacPT1() player:activate("%s")', modPrefix, ref.id)
	end
	player:activate(ref)
end
...
local function ab01smacPT3(e)
	...
	skips = 2
	if logLevel2 then
		mwse.log('%s: ab01smacPT3() player:activate("%s")', modPrefix, ref.id)
	end
	player:activate(ref)
end
...
local function faceAndTalk(mobRef)
	...
	if ok
	and (mob.actorType == tes3.actorType.creature) then
		if autoFace < 2 then
			ok = false
		else
			local s = string.sub(string.lower(mobRef.object.id), 1, 6)
			if (s == 'ab01ss')
			or (s == 'ab01bo')
			or (s == 'ab01go')
			or (s == 'ab01gu') then
			...
			callback = function (ev)
				...
				if index < lastButtonIndex then
					...
					and canStartDialogueWith(mobi) then
						...
						skips = 1
						local baseObj = activateRef.baseObject
						if baseObj.objectType == tes3_objectType_container then
							...
							if mesh then
								...
								if string.find(lcMesh, 'ac/anim_', 1, true) then
									if logLevel2 then
										mwse.log("%s: animated container, 1 more skip", funcPrefix)
									end
									skips = 2
								end
							end
