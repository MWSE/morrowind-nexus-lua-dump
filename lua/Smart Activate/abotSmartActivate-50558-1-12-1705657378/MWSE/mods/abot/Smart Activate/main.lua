...
idleOnActivate = 1, -- 0 = Off, 1 = Followers, 2 = All actors
...
local selectableTypes = {tes3_objectType_npc, tes3_objectType_creature,
...
local function ab01smacPT1(e)
	...
	if logLevel2 then
		mwse.log('%s: ab01smacPT1() player:activate("%s") skips = %s', modPrefix, ref.id, skips)
	end
	skips = 2 -- safer most times here
	player:activate(ref)
end

local function delayedPlayerActivate(targetRef, delaySec)
	local refHandle1 = tes3.makeSafeObjectHandle(targetRef)
	timer.start({ duration = delaySec, type = timer.real, callback = 'ab01smacPT1',data = {handle = refHandle1} })
end
...
local function ab01smacPT5(e)
	...
	tes3.playAnimation({reference = ref, group = 0}) -- like playgroup idle
end
	...
	if animGroup then
		if loops then
			...
			if animOk then
				...
				---tes3.playAnimation({reference = mobRef, group = animGroup, loopCount = loops})
				tes3.playAnimation({reference = mobRef, lower = animGroup, loopCount = loops})
			end
		end
	end
	if idleOk then
		if animOk then
			if loops > 0 then 
				...
				timer.start({ duration = delay, callback = 'ab01smacPT5', data = {handle = refHandle5} })
				return
			end
		end
		tes3.playAnimation({reference = mobRef, group = 0}) -- like playgroup idle
	end
end
...
local function ab01smacPT2(e)
	...
	if logLevel4 then
		mwse.log('%s: ab01smacPT2("%s") radStep = %s, i = %s, iterations = %s',
			modPrefix, ref.id, radStep, i, iterations)
	end
...
-- Fixed at 0 while the blink animation plays.
	if timeToNextBlink then
		if timeToNextBlink == 0 then
			if i < iterations then -- wait for the blinking animation to end, but limited for safety
				data.i = i + 1
				if logLevel5 then
					mwse.log('%s: ab01smacPT4() player:activate("%s") i = %s, iterations = %s',
	...
	-- blinking done
	if logLevel4 then
		mwse.log('%s: ab01smacPT4() player:activate("%s") timeToNextBlink = %s, headMorphTiming = %s',
...
local function checkBlinkFix(mob)
	...
	local dur = 0.1
	local iters = math.ceil(blinkMorphDuration / dur)
	local refHandle4 = tes3.makeSafeObjectHandle(mobRef)
	if logLevel2 then
		mwse.log([[%s: checkBlinkFix("%s") ab01smacPT4timer = timer.start({duration = %s, iterations = %s, callback = 'ab01smacPT4'})]],
			modPrefix, mobRef.id, dur, iters)
	end
	ab01smacPT4timer = timer.start({ duration = dur, iterations = iters,
...
local function ab01smacPT3(e)
	...
	if logLevel2 then
		mwse.log('%s: ab01smacPT3() "%s":startDialogue()', modPrefix, ref.id)
	end
	skips = 1
	player:activate(ref)
	---mob:startDialogue()
...
-- 0 degrees is directly in front of the actor, the negative values are on the actor's left side, and positive values on the actor's right.
		...
		if absAngleTo >= config.minFaceAngle then
			...
			local loops = math.max(math.floor(iters / 6), 0)
			local secStep = 0.05
			...
			timer.start({ type = timer.real, duration = secStep, iterations = iters,
	...
	if activationDelay > 0 then
		...
		timer.start({ duration = activationDelay, callback = 'ab01smacPT3', data = {handle = refHandle3} })
		return
	end
	...
	if logLevel2 then
		mwse.log('%s: faceAndTalk() "%s":startDialogue()', modPrefix, mobRef.id)
	end
	skips = 1
...
local function activate(e)
	...
	timer.delayOneFrame(
		function ()
			tes3ui.showMessageMenu({id = 'ab01smac', buttons = msgBtns,
...
Selected actor slowly turns around and face player on activate.]], 'autoFace'),
	...
	controls:createYesNoButton({
		label = 'Blink Fix',
		description = getYesNoDescription([[Default: %s.
Try and reduce actor blinking eyes remaining closed while talking with player.]], 'blinkFix'),
