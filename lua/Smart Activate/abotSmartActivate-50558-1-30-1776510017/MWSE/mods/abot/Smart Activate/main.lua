...
local faceMeshBlacklist = {'va_sitting','roo_papcre'}
...
local doorArmAnimBlacklist = {'continue','crevice','dark','entrance'}
...
-- doors that need lower opening angles/use swapped hinge (marked with negative value)
...
-- for doors, this has been quite difficult to figure out
	...
	local data = {}
	local specialDoorSwingDeg = specialDoorsSwingDeg[getFileName(lcMesh)]
	if specialDoorSwingDeg then
		totSwingDeg = math.abs(specialDoorSwingDeg)
	end
	if doorRef.data.ab01smactiSwp then
		data.swapHinge = true
	else
		if specialDoorSwingDeg
		and (specialDoorSwingDeg < 0) then
			data.swapHinge = true
		else
	...
	---data.forward = tes3vector3.new(math.sin(doorOri.z), math.cos(doorOri.z), 0)
	...
	if not doorRef.cell.isOrBehavesAsExterior then
		totSwingDeg = math.min(totSwingDeg, 1.5) -- try and avoid opening through walls
	end
	...
	local secondsPassed = math.max( math.abs(tes3.worldController.deltaTime), 0.0001 )
	local fps = math.max(1 / secondsPassed, 12)
	local steps = math.max( math.floor(fps * 0.5 * duration + 0.5), 6 ) -- note each could require a couple frames
	data.steps = steps
	data.stepAngle = math.rad(totSwingDeg / steps)
	...
	d2.z = 0
	data.radius = math.min(#d1, #d2)
	data.step = 0
	local interval = duration / steps
	local itr = steps
	---if not doorRef.cell.isOrBehavesAsExterior then
		---itr = math.floor(itr * 0.7 + 1)
	---end
	if logLevel1 then
...
yAlignedWidth = %s, opposite = %s, swapHinge = %s]],
	...
	if chimeRef then
		...
		tes3.playSound({sound = 'R0_snd_doorchime_0'..math.random(1,4),
