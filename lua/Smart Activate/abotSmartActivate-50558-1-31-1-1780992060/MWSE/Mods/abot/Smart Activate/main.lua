...
animDoor = true, -- add a short door rotation animation when opening a loading door
animLockedDoor = true, -- add an even shorter door rotation animation when trying to open a locked loading door
...
local function updateFromConfig()
	...
	animDoor = config.animDoor
	animLockedDoor = config.animLockedDoor
...
local math_abs, math_deg, math_floor, math_max, math_min, math_pi, math_rad, math_random
= math.abs, math.deg, math.floor, math.max, math.min, math.pi, math.rad, math.random

-- set in modConfigReady
local bumpityLoaded = false

---@param ref tes3reference
---@param maxDistanceFromRef number?
---@return tes3cell[]
...
-- try marking cells that can be skipped
	...
	for _, c in ipairs(ac) do
		key = tostring(c.gridX)..sep..tostring(c.gridY)
	...
	for gridX = cellGridX - 1, cellGridX + 1 do
		for gridY = cellGridY - 1, cellGridY + 1 do
			key = tostring(gridX)..sep..tostring(gridY)
			if not skip[key] then
				if not added[key] then
...
local targetBlacklist = {'roht_mask_compass','zi_skull_curse'}

---@param obj tes3object
local function isTargetBlacklisted(obj)
...
local function isValidMob(aRef, allowDead)
	...
	if mob
	and ( not isTargetBlacklisted(obj) )
...
local function getSelectablesInProximity(targetRef, range)
	...
	local function processRef(aRef)
		...
		if aRef.deleted then
			assert(aRef.disabled) -- temporary sebug safety test
		end
	...
	local function processCell()
		for aRef in cell:iterateReferences(selectableTypes, false) do
			processRef(aRef)
		end
	end
...
---@param mob tes3mobileActor
---@param p tes3aiPackageActivate|tes3aiPackageEscort|tes3aiPackageFollow|tes3aiPackageTravel|tes3aiPackageWander?
...
local function fixImmobile(ref)
	...
	for i = 1, #chances do
		...
		elseif idle.chance < 1 then
			idle.chance = math_random( round(chance * 0.5), round(chance) )
		else
			break
		end
	end
...
local validTypes = {
[tes3_objectType_npc] = 1,
[tes3_objectType_creature] = 1,
[tes3_objectType_container] = 1,
[tes3_objectType_door] = 1,
[tes3_objectType_book] = 1,
}

local faceActorIdBlacklist = {'hrsct','1m_cat','fly','guar_pack','whale'}
local faceAnimBlacklist = {'kwama queen'}

---@param mobRef tes3reference
---@return boolean
local function isFaceAnimBlacklisted(mobRef)
	...
	if obj.id:lower():multifind(faceActorIdBlacklist, 1, true) then
		return true
	end
	local s = back2slash( mesh:lower() )
	if s:multifind(faceAnimBlacklist, 1, true) then
		return true
	end
	return false
end
...
local improvedAnimationSupport ---@type boolean?
...
local blacklistedAnims = {'\\am_','/am_','anim_ly','lean',
'lay','lye','scamp','sitbar','sittin','wait'}

---@param mob tes3mobileActor
---@return boolean
local function hasBlacklistedAnim(mob)
	local aiTargetMob = getMobAiFollowOrEscortTarget(mob)
	---if ( mobilePlayer == aiTargetMob ) then
	if aiTargetMob then
		return false -- skip current followers/escorters
	end
	local mesh = mob.reference.object.mesh
	if not mesh then
		return false
	end
	if mesh:len() == 0 then
	...
	if mesh:lower():multifind(blacklistedAnims, 1, true) then
		return true
	end
	return false
end
...
local function checkPlayAnimation(mob, animGroup, loops)
	...
	if animGroup then
		local randGroup = talkAnims[math_random(1, #talkAnims)]
...
---@return number
local function normalizedPiAngle(radAngle)
...
local function getAngleTo(ref, target)
	...
	and mob.actorType then
		...
		angleTo = math_rad(mob:getViewToPoint(target.position))
	else
		angleTo = ref:getAngleTo(target)
	end
	if logLevel3 then
		mwse_log([[%s: getAngleTo("%s","%s") = %.2f]],
			modPrefix, ref.id, target.id, math_deg(angleTo))
	end
	return angleTo
end
...
local function getFacingAngle(ref, target)
	...
	local refAngle = normalizedPiAngle(ref.facing)
	local facingAngle = refAngle + angleTo
	facingAngle = normalizedPiAngle(facingAngle)
	if logLevel3 then
		mwse_log([[%s: getFacingAngle("%s","%s")
refAngle = %.2f, angleToTarget = %.2f, facingAngle = %.2f]],
			modPrefix, ref.id, target.id, math_deg(ref.facing),
			math_deg(angleTo), math_deg(facingAngle))
	end
	return facingAngle
end

local minFacingAngle = math_rad(3)

---@param ref tes3reference
---@param target tes3reference
local function face(ref, target)
	local a = getFacingAngle(ref, target)
	if math_abs(a) >= minFacingAngle then
		ref.facing = a
	end
end


local radStep = math_rad(9)
...
local function checkBlinkFix(mob)
	...
	---animationData.timeToNextBlink = math_round( (blinkMorphStartTime + blinkMorphEndTime) * 0.5, 2 )
...
local turnAnimDict = {
[tes3_animationGroup_turnRight] = 1,
[tes3_animationGroup_turnLeft] = 1,
...
local function faceAndTalk(mobRef)
	...
	local skipAnim = isFaceAnimBlacklisted(mobRef)
	or isDummy(mobRef)
	or hasBlacklistedAnim(mob)
	...
	and (mob.actorType == tes3_actorType_creature) then
		if autoFace < 2 then
			...
			doFace = not isRefScenicTravelCreature(mobRef)
		end
	end
	...
	if doFace then
		...
		local angleTo = math_deg(getAngleTo(mobRef, player))

		local absAngleTo = math_abs(angleTo)
		local degreeStep = 9
		...
		if absAngleTo >= config.minFaceAngle then
			...
			local iters = math_max(math_floor((absAngleTo - degreeStep) / degreeStep), 0)
			if iters > 0 then
				local loops = math_max(math_floor(iters / 12), 1)
				local secStep = 0.05
				...
				if logLevel4 then
					mwse_log('%s: ab01smactiPT2("%s") radStep = %s (%s deg), iterations = %s',
						modPrefix, mobRef.id, radStep, math_deg(radStep), iters)
				end
				if isMoving then
					mob.activeAI = false
				end
	...
	if activationDelay > 0 then
		...
		timer.start({ type = timer_real, duration = activationDelay,
			callback = 'ab01smactiPT3', data = {handle = refHandle3} })
		return
	elseif doFace
...
local faceMeshBlacklist = {'anim_','va_sitting','roo_papcre'}
...
local doorArmAnimBlacklist = {'continue','crevice','dark','entrance','passage'}
...
--[[
---@param ref tes3reference
local function checkCloneActor(ref)
	local obj = ref.object
...
--	end
	...
	if cloned then -- resolve container contents
		if logLevel1 then
			mwse.log('%s: checkCloneActor("%s") cloned', modPrefix, ref.id)
		end
		return true
	end
end
...
'marker',
'mh_temple_door_01',
'pc_galleon_door_01_l',
'pc_galleon_door_01_m',
'pc_galleon_door_01_r',
'pc_galleon_door_02',
'pc_galleon_door_02_in',
'pelt',
'portal',
'rope',
---'rockdoor',
...
'trap',
'v_cantondoor',
'velothi_entrance_01_a',
}


-- .nif match for doors that need lower opening angles/use swapped hinge (marked with negative value)
-- using the abs value anyway, negative means using rightDirection instead of forwardDirection
local specialDoorsSwingDeg = {
['bar_ex_door01bta'] = -3.0,
['ex_common_door_01'] = 0.9,
['ex_t_door_slavepod_01'] = 0.15,
['ex_velothi_loaddoor_01'] = -3.0,
['in_c_door_arched'] = 2.2,
['in_c_door_wood_square'] = 1.0,
['in_impsmall_loaddoor_01'] = 3.0,
['in_de_shack_door'] = -3.0,
}


-- ---@param ref tes3reference
-- local function isLocked(ref)
	-- return ref.lockNode
	-- and ref.lockNode.locked
-- end

local function normalizedDeg(radAngle)
	return math_deg(radAngle) % 360
end

---@param e mwseTimerCallbackData
local function ab01smactiPT9(e)
	local e_timer = e.timer
	local data = e_timer.data
	if not data then
		return
	...
	local handle1 = data.handle1---@type mwseSafeObjectHandle
	local doorRef = handleToRef(handle1)
	if not doorRef then
		return
	end
	local handle2 = data.handle2 ---@type mwseSafeObjectHandle
	local activatorRef = handleToRef(handle2)
	if not activatorRef then
		return
	end
	local handle3 = data.handle3 ---@type mwseSafeObjectHandle
	if handle3 then
		local chimeRef = handleToRef(handle3)
		if chimeRef then
			---chimeRef.sceneNode.appCulled = false
			---chimeRef.sceneNode:update()
			chimeRef.position = chimeRef.data.ab01chimePos:copy()
			chimeRef.data.ab01chimePos = nil
		end
	end
	if logLevel2 then
		mwse.log('%s: ab01smactiPT9() "%s" SwingDoor "%s" reset)',
			modPrefix, activatorRef, doorRef)
	end
	doorRef.position = data.startPos
	---doorRef:updateSceneGraph() -- not needed as player...
	doorRef.orientation = data.startOri
	---doorRef:updateSceneGraph() -- ...is in destination cell
end

---@param e mwseTimerCallbackData
local function ab01smactiPT8(e)
	...
	local ori = doorRef.orientation -- this works just updating .z
	local prevPos = doorRef.position:copy()
	local prevDir
	if data.yAlignedWidth then -- try to use the more "sensible" direction vector
		prevDir = doorRef.forwardDirection:copy()
	else
		prevDir = doorRef.rightDirection:copy()
	end
	ori.z = ori.z + data.stepAngle
	doorRef:updateSceneGraph()
-- give some delay to update doorRef.forwardDirection/doorRef.rightDirection
	----timer.frame.delayOneFrame(function ()

-- luckily forwardDirection/rightDirection is updated, get the movement vector
	local rotVec
	if data.yAlignedWidth then
		rotVec = doorRef.forwardDirection - prevDir
		if logLevel2 then
			mwse.log('%s: ab01smactiPT8() prevDir = %s, doorRef.forwardDirection = %s, rotVec = %s',
				modPrefix, prevDir, doorRef.forwardDirection, rotVec)
		end
	else
		rotVec = doorRef.rightDirection - prevDir
		if logLevel2 then
			mwse.log('%s: ab01smactiPT8() prevDir = %s, doorRef.rightDirection = %s, rotVec = %s',
				modPrefix, prevDir, doorRef.rightDirection, rotVec)
		end
	end

-- add movement vector to the rotation center
-- purpose is to get the hinge approx. back to the previous pos,
-- making it look as if it would have been hinge rotating.
	local add = true
	-- if data.opposite then
		-- if not data.swapHinge then
			-- add = false
		-- end
	-- elseif data.swapHinge then
		-- add = false
	-- end
	if data.opposite then
		add = false
	end
	if data.swapHinge then
		add = not add
	end
	local radius = data.radius
	if add then
		doorRef.position = doorRef.position + rotVec * radius
		if chimeRef then
			chimeRef.position = chimeRef.position + rotVec * (radius * 2)
		end
	else
		doorRef.position = doorRef.position - rotVec * radius
		if chimeRef then
	...
	doorRef:updateSceneGraph()
	if chimeRef then
		chimeRef:updateSceneGraph()
	end
	if logLevel2 then
		mwse.log('%s: ab01smactiPT8() prevPos = %s, doorRef.Position = %s, delta = %s',
			modPrefix, prevPos, doorRef.position, doorRef.position - prevPos)
	end

-- note: changing position needs a new vector,
-- directly changing the (non-actor) coordinates does not work

-- also using doorRef.forwardDirection/rightDirection with radius is simpler
-- than guessing the correct roto-translation formula with the mess of doors
-- to move the door rotation point so the hinge does not seem to move

	if logLevel1
	and (data.step >= data.steps) then
		mwse.log('%s: ab01smactiPT8() "%s" SwingDoor "%s" ',
			modPrefix, activatorRef, doorRef)
	end
end
...
local function getFileName(s)
	local result = s:match('([^\\/]+)%.%w+$')
	if logLevel2 then
		mwse.log('%s: getFileName("%s") = "%s"', modPrefix, s, result)
	end
	return result
end
...
local function doorSwing(doorRef, activatorRef, duration)
-- as usual, given the total Bethesda mess abou using standards
-- for doors, this has been quite difficult to figure out
	if doorRef.data.ab01smactiBlk then
		return
	end
	local totSwingDeg = 2.7 ---2.5
	...
	local data = {}
	local sdsd = specialDoorsSwingDeg[getFileName(lcMesh)]
	if sdsd then
		totSwingDeg = math_abs(sdsd)
	end
	if doorRef.data.ab01smactiSwp then
		data.swapHinge = true
	else
		if sdsd
		and (sdsd < 0) then
			...
			data.swapHinge = false
		end
	end
	local locked = doorRef.lockNode
		and doorRef.lockNode.locked
	if locked then
		if animLockedDoor then
			totSwingDeg = 0.6
		else
			return
		end
	elseif not doorRef.destination then
		return
	end
	if not (
		doorRef.destination
	...
	local bBox = obj.boundingBox
	local size = bBox.max - bBox.min
	if (size.z < size.x)
	and (size.z < size.y) then
		data.trapadoor = true
		return -- skip trapdoors for now
	else
		data.trapadoor = false
	end
	local doorPos = doorRef.position
	local doorOri = doorRef.orientation
	local dirToPlayer = (player.position - doorPos):normalized()
	---data.forward = tes3vector3.new(math_sin(doorOri.z), math_cos(doorOri.z), 0)
	---local dot2 = dirToPlayer:dot(data.forward)
	local dot = dirToPlayer:dot(doorRef.forwardDirection)
	if dot > 0 then
		data.opposite = true
	else
		data.opposite = false
	...
	data.yAlignedWidth = false
	if size.x < size.y then
		if ( size.y > (1.75 * size.z) )
		and (not data.trapdoor) then
			return -- skip probable y aligned double doors
		end
		data.yAlignedWidth = true -- Y axis aligned
	elseif ( size.x > (1.75 * size.z) )
		and (not data.trapdoor) then
		return -- skip probable x aligned double doors
	end

	if not doorRef.cell.isOrBehavesAsExterior then
		totSwingDeg = math_min(totSwingDeg, 1.5) -- try and avoid opening through walls
	end

	local chimeRef, chimeObj, dist
	local maxDist = size:length() * 1.1
	if logLevel1 then
		mwse.log('%s: maxDist = %s', modPrefix, maxDist)
	end

	-- https://mwse.github.io/MWSE/types/tes3cell/?h=tes3cell#iteratereferences
	-- note: 2nd iterateReferences parameter set to false
	-- basically skips disabled references, not so clear from the docs
	for ref in doorRef.cell:iterateReferences(tes3_objectType_activator, false) do
		---if not ref.disabled then
			chimeObj = ref.object
			---mwse.log('%s: "%s" found in cell "%s"', modPrefix, ref, doorRef.cell)
			if chimeObj.id:lower():startswith('r0_furn_com_doorchime_0') then
				dist = ref.position:distance(doorPos)
				if dist <= maxDist then
					chimeRef = ref
					if logLevel1 then
						mwse.log('%s: chime "%s" %s from "%s" door found in cell "%s"',
							modPrefix, ref, dist, doorRef, doorRef.cell)
					end
					break
				end
			end
	...
	local fps = math_max(1 / secondsPassed, 12)
	local steps = math_max( math_floor(fps * 0.5 * duration + 0.5), 6 ) -- note each could require a couple frames
	data.steps = steps
	data.stepAngle = math_rad(totSwingDeg / steps)

	-- important to be able to restore the door original position
	data.startPos = doorPos:copy()
	data.startOri = doorOri:copy()

	local worldTransform = doorRef.sceneNode.worldTransform
	local minPos = worldTransform * bBox.min
	minPos.z = 0
	local maxPos = worldTransform * bBox.max
	maxPos.z = 0
	local d1 = minPos - doorPos
	d1.z = 0
	local d2 = maxPos - doorPos
	d2.z = 0
	data.radius = math_min(#d1, #d2)
	data.step = 0
	local interval = duration / steps
	local itr = steps
	---if not doorRef.cell.isOrBehavesAsExterior then
		---itr = math_floor(itr * 0.7 + 1)
	---end
	if logLevel1 then
		mwse.log([[%s: doorSwing("%s", "%s", %s) lcMesh = "%s",
doorPos = %s, bBox.min = %s, bBox.max = %s,
minPos = %s, maxPos = %s, d1 = %s, d2 = %s,
radius = %s, angle = %s, doorRef.forwardDirection = %s, dot = %s,
yAlignedWidth = %s, opposite = %s, swapHinge = %s]],
		modPrefix, doorRef, activatorRef, duration, lcMesh,
		doorPos, bBox.min, bBox.max,
		minPos, maxPos, d1, d2,
		data.radius, normalizedDeg(doorRef.orientation.z), doorRef.forwardDirection, dot,
		data.yAlignedWidth, data.opposite, data.swapHinge)
	end

	data.handle1 = tes3.makeSafeObjectHandle(doorRef)
	data.handle2 = tes3.makeSafeObjectHandle(activatorRef)
	if chimeRef then
		data.handle3 = tes3.makeSafeObjectHandle(chimeRef)
		---chimeRef.sceneNode.appCulled = true
		---chimeRef.sceneNode:update()
		chimeRef.data.ab01chimePos = chimeRef.position:copy()
		tes3.playSound({sound = 'R0_snd_doorchime_0'..math_random(1,4),
			reference = activatorRef})
	end

	-- reset the position/orientation of the door when done
	timer.start({duration = duration + 0.5,
		callback = 'ab01smactiPT9', data = data})

	-- swing open the door
	timer.start({type = timer_real, iterations = itr, duration = interval,
		callback = 'ab01smactiPT8',	data = data})
end
...
local function activate(e)
	...
	local targetIsActor = (targetObjType == tes3_objectType_npc)
		or (targetObjType == tes3_objectType_creature)
...
--- early skip test
	if target_script
	and ( not targetIsActor ) -- no actor as some mods like Sabrina may block activation
	and ( not target:testActionFlag(tes3_actionFlag_useEnabled) ) then
		if logLevel1 then
			mwse_log('%s: "%s" not "%s":testActionFlag(tes3_actionFlag_useEnabled), skip',
	...
	and (skips == 0) then
		...
		if targetObjType == tes3_objectType_door then
			...
			if lcId:multifind(doorArmAnimBlacklist, 1, true) then
				...
				and ( mesh:len() > 0) then
					local s = back2slash( mesh:lower() )
					if s:multifind(doorArmAnimBlacklist, 1, true) then
			...
			if target_script then
				...
				if (
					...
					if text then
						...
						if lcText:find('onactivate', 1, true)
						or lcText:find('short%s+pcskipequip') then
							if logLevel1 then
		...
		if ok then
			...
			and ( not checkSkip(activator) ) then
				...
				-- if checkCloneActor(target) then
				...
				if animationData then
					if activator.tempData.ab01smactiBusy then
						activator.tempData.ab01smactiBusy = false
						return --activatorRef:activate(targetRef)
					else
						activator.tempData.ab01smactiBusy = true
					end
					local animationGroups = animationData.animationGroups
					if animationGroups
					...
					and (playActivationAnim(activator) > 0) then
						...
						if animDoor
						and (targetObjType == tes3_objectType_door)
						and ( not multifind2(targetObj.mesh:lower(),
							targetObj.name:lower(), doorAnimBlacklist) ) then
								doorSwing(target, activator, animCutSec * 0.95)
						end
	...
	if not validTypes[targetObjType] then
		if logLevel2 then
	...
	if targetMob then
		...
		if actorsMenu then
			...
			if boundSize then
				local boundMin = math_min(boundSize.x, boundSize.y)
		...
		if canStartDialogueWith(targetMob) then
			if isMobScenicTravelCreature(targetMob) then
				e.claim = true
				return
			end
	...
	if numSelectables < 2 then
		...
		and canStartDialogueWith(targetMob) then
			...
			or isMobScenicTravelCreature(targetMob) then
				if logLevel2 then
			...
			msgBtns[k] = {text = aName,
				callback = function ()
					if aRef then
						...
						if aMob
						and canStartDialogueWith(aMob) then
							if isMobScenicTravelCreature(aMob) then
								playerActivate(aRef)
								return
							end
							-- starting dialog directly is more robust with scripted NPCs than trying to player:activate()
						...
						if logLevel2 then
							mwse_log('%s: delayedPlayerActivate("%s", 0.15)',
								funcPrefix, aRef.id)
						end
						skips = 2
...
(Only one door because often loading doors are near each other so better to avoid activating the linked one)]],
	...
	local activationNote = '\n\n'..[[Note: if something does not activate properly with this or other options enabled, you can usually bypass the problem keeping Alt or Shift pressed while activating.
If a door does not animate properly or not using the correct handle/hinge positions, you can try setting it as blacklisted/swapped hinge having Alt + Shift pressed and clicking the left/right mouse button with the mouse curson over the door.]]
...
Compatible/works great with the "Animated Pickup" mod.]]..activationNote,
		configKey = 'animActivate'
	})

	sideBarPage:createYesNoButton({
		label = 'Loading door animation',
		description = [[When "Activation animation" is enabled,
most loading doors play a short rotation animation when opening.]]..activationNote,
		configKey = 'animDoor'
	})

	sideBarPage:createYesNoButton({
		label = 'Locked door animation',
		description = [[When "Activation animation" is enabled,
trying to open most locked doors will play a shorter door rotation animation.]]..activationNote,
		configKey = 'animLockedDoor'
	})
...
local function mouseButtonUp(e)
	...
	local lcObjId = obj.id:lower()
	local lcMesh = obj.mesh:lower()
	local data = target.data
	if e.button == 0 then
		local s
		if data.ab01smactiBlk then
			data.ab01smactiBlk = nil
			s = 'removed from'
		else
			data.ab01smactiBlk = 1
			s = 'added to'
		end
		saveConfig()
		tes3.messageBox('"%s" door mesh "%s" %s blacklist', lcObjId, lcMesh, s)
		return
	end
	if e.button == 1 then
		local s
		if data.ab01smactiSwp then
			data.ab01smactiSwp = nil
			s = 'off'
		else
			data.ab01smactiSwp = 1
			s = 'on'
		end
		saveConfig()
		tes3.messageBox('"%s" door mesh "%s" hinge swapped %s', lcObjId, lcMesh, s)
	end
end
...
event.register('initialized', function ()
	...
	timer.register('ab01smactiPT8', ab01smactiPT8)
	timer.register('ab01smactiPT9', ab01smactiPT9)
