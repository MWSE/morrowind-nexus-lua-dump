...
local function activate(e)
	...
	and (skips < 1) then
		...
		elseif (targetObjType == tes3_objectType_container)
				or (targetObjType == tes3_objectType_npc)
				or (targetObjType == tes3_objectType_creature) then
			if logLevel2 then
				mwse_log('%s: "%s" activating "%s".targetObjType = %s',
					funcPrefix, activatorMob.reference.id, target.id,
					mwse.longToString(targetObjType))
			end
			if ( not (targetObjType == tes3_objectType_container) )
					and isDummy(target) then
				if logLevel2 then
			...
			elseif (not activatorMob.isSneaking) then
				if targetMob
					...
					if logLevel2 then
						mwse_log('%s: "%s".isSneaking = %s',
							funcPrefix, activatorMob.reference.id, activatorMob.isSneaking)
						if targetMob then
				...
				elseif target_script
					...
					skips = 1
					ok = false -- activator not sneaking, scripted for possiblr activation delay, no animation
					if logLevel2 then
						mwse_log('%s: "%s".isSneaking = %s',
							funcPrefix, activatorMob.reference.id, activatorMob.isSneaking)
						mwse_log('%s: "%s" not "%s":testActionFlag(tes3_actionFlag_useEnabled), ok = %s',
						funcPrefix, activatorMob.reference.id, target.id, ok)
					end
				end
			end
