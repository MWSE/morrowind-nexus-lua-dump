...
local actorsMenu, immobileFix, animActivate
...
local function isScenicTravelCreature(mobRef)
	local s = string.sub(mobRef.object.id:lower(), 1, 6) -- get the id prefix
...
---@return 0|1|2
local function playPickProbeAnim(npcRef, skips)
	local funcPrefix = modPrefix .. ' playPickProbeAnim()'
	...
	and ( not tes3.is3rdPerson() ) then
		...
		play3rdPerson = false
		if logLevel2 then
			mwse_log('%s: playPickProbeAnim("%s") play3rdPerson = %s',
	...
	and play3rdPerson then
		...
		if logLevel3 then
			mwse_log('%s: tes3.playAnimation({reference = "%s", group = %s, upper = %s, loopCount = 0})',
				funcPrefix, ref, animGroupsDict[group], animGroupsDict[upperGroup])
		end
		tes3.playAnimation({reference = ref, group = group, upper = upperGroup, loopCount = 0})
		return 1
	end

	if logLevel3 then
		mwse_log('%s: tes3.playAnimation({reference = "%s", group = %s, loopCount = 0})',
			funcPrefix, ref, animGroupsDict[group])
	end
	tes3.playAnimation({reference = ref, group = group, loopCount = 0})
	return 2
...
local function activate(e)
	...
	if animActivate then
		...
		if targetObjType == tes3_objectType_door then
			if targetObj.id:lower():multifind(doorBlackList, 1, true) then
		...
		if ok then
			...
			and ( not (activatorActorType == tes3_actorType_creature) ) then
				if not checkSkip(activator) then
					...
					if animationData then
						local animationGroups = animationData.animationGroups
						if animationGroups
						and animationGroups[tes3_animationGroup_pickProbe + 1] then
							...
							and targetHandle then
								if playPickProbeAnim(activator, skips) > 0 then
	...
	if not validTypes[targetObjType] then
		if logLevel2 then
			mwse_log("%s: targetObjType = %s, skip", funcPrefix, mwse.longToString(targetObjType))
		end
		return
	end
	...
	if targetMob then
		if target.data
		and target.data.rfuzzo_invincible then
			return -- skip moving immersive travel actors
		end
