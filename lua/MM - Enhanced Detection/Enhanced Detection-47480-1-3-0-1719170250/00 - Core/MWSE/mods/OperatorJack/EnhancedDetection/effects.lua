...
local effectVfx = {
    animal = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    key = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    enchantment = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    daedra = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    automaton = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    humanoid = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    dead = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    undead = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    door = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    trap = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    landAnimal = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"},
    underwaterAnimal = {castVFX = "VFX_MysticismCast", hitVFX = "VFX_MysticismHit"}
}
...
local function addMagicEffects()
    if tes3.getFileExists("meshes\\OJ\\ED\\hit\\ED_H_Animal.nif") == true then
        for id, mesh in pairs(statics) do
            if (tes3.getObject(id) == nil) then
                if (tes3.getFileExists("meshes\\" .. mesh) == true) then
                    tes3.createObject({objectType = tes3.objectType.static, id = id, mesh = mesh})
                else
                    print("[Enhanced Detection: ERROR] Static " .. id .. " missing mesh at path " ..
                              mesh)
                    return
                end
            end
        ...
        effectVfx = {
            animal = {castVFX = staticIds.animalCast, hitVFX = staticIds.animalHit},
            key = {castVFX = staticIds.keyCast, hitVFX = staticIds.keyHit},
            enchantment = {castVFX = staticIds.enchantmentCast, hitVFX = staticIds.enchantmentHit},
            daedra = {castVFX = staticIds.daedricCast, hitVFX = staticIds.daedricHit},
            automaton = {castVFX = staticIds.automatonCast, hitVFX = staticIds.automatonHit},
            humanoid = {castVFX = staticIds.humanoidCast, hitVFX = staticIds.humanoidHit},
            dead = {castVFX = staticIds.deadCast, hitVFX = staticIds.deadHit},
            undead = {castVFX = staticIds.undeadCast, hitVFX = staticIds.undeadHit},
            door = {castVFX = staticIds.doorCast, hitVFX = staticIds.doorHit},
            trap = {castVFX = staticIds.trapCast, hitVFX = staticIds.trapHit},
            landAnimal = {castVFX = staticIds.landAnimalCast, hitVFX = staticIds.landAnimalHit},
            underwaterAnimal = {
        ...
        for effectType, pair in pairs(effectVfx) do
            for vfxType, staticId in pairs(pair) do
                if (tes3.getObject(staticId) == nil) then
                    print("[Enhanced Detection: ERROR] Static " .. staticId ..
                              " failed to be created for effect type " .. effectType)
                    return
                end
            end
        end
