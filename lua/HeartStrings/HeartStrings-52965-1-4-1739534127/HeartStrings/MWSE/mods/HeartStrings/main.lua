...
local function RandomMP3(dir) local Tab = Cach[dir]
	if not Tab then Tab = {}	for file in lfs.dir(dir) do if file:endswith("mp3") then table.insert(Tab, file) end end
		if #Tab == 0 then for file in lfs.dir("data files\\music\\Explore") do if file:endswith("mp3") then table.insert(Tab, file) end end end
		Cach[dir] = Tab
	end
