...
settings:createOnOffButton{
    label = "Механика поворота камеры при шаге",
    description = "Определяет, должна ли камера немного поворачиваться при каждом шаге игрока. Поворот происходит в направлении горизонтального покачивания головы.",
    defaultSetting = defaultConfig.stepRotationEnabled,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "stepRotationEnabled",
        table = config
    }
}
...
local sneakPage = template:createSideBarPage({
    ...
    description = "Эти настройки определяют плавность перехода камеры в режим скрытности. Отключение настройки камеры скрытности сбросит значение GMST для высоты в режиме скрытности.",
    showReset = true
})
...
local stepRotationPage = template:createSideBarPage({
    label = "Настройки поворота при шаге",
    description = "Эти настройки определяют работу функции поворота камеры при шаге.",
    showReset = true
})

stepRotationPage:createSlider{
    label = "Сила поворота при шаге",
    description = "Это множитель, определяющий величину поворота камеры при каждом шагом. Работает только при включенной механике поворота при шаге.",
    max = 250,
    min = 50,
    defaultSetting = defaultConfig.stepRotationMultiplier,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "stepRotationMultiplier",
        table = config
    }
}

stepRotationPage:createSlider{
    label = "Плавность поворота при шаге",
    description = "Это значение определяет, насколько плавным будет поворот.",
    max = 25,
    min = 1,
    defaultSetting = defaultConfig.stepRotationSmoothing,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "stepRotationSmoothing",
        table = config
    }
}

local sneakStepRotationTweaks = stepRotationPage:createCategory ("Настройки поворота при шаге в режиме скрытности")

sneakStepRotationTweaks:createSlider{
    label = "Множитель шума для поворота при шаге в режиме скрытности",
    description = "Это значение определяет, насколько сильно шум влияет на поворот при шаге в режиме скрытности. Если установлено 0, функция добавления шума полностью отключается.",
    max = 5,
    min = 0,
    step = 0.1,
    decimalPlaces = 1,
    defaultSetting = defaultConfig.sneakStepRotationNoiseAmplitude,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "sneakStepRotationNoiseAmplitude",
        table = config
    }
}

sneakStepRotationTweaks:createSlider{
    label = "Скорость шума для поворота при шаге в режиме скрытности",
    description = "Это значение определяет, насколько быстрым будет шум при поворотах во время шага в режиме скрытности. Множитель шума должен быть больше 0, чтобы эта настройка действовала.",
    max = 25,
    min = 0,
    step = 0.1,
    decimalPlaces = 1,
    defaultSetting = defaultConfig.sneakStepRotationNoiseScale,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "sneakStepRotationNoiseScale",
        table = config
    }
}

local walkStepRotationTweaks = stepRotationPage:createCategory ("Настройки поворота при шаге во время ходьбы")

walkStepRotationTweaks:createSlider{
    label = "Множитель шума для поворота при шаге во время ходьбы",
    description = "Это значение определяет, насколько сильно шум влияет на поворот при шаге во время ходьбы. Если установлено 0, функция добавления шума полностью отключается.",
    max = 5,
    min = 0,
    step = 0.1,
    decimalPlaces = 1,
    defaultSetting = defaultConfig.walkStepRotationNoiseAmplitude,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "walkStepRotationNoiseAmplitude",
        table = config
    }
}

walkStepRotationTweaks:createSlider{
    label = "Скорость шума для поворота при шаге во время ходьбы",
    description = "Это значение определяет, насколько быстрым будет шум при поворотах во время шага при ходьбе. Множитель шума должен быть больше 0, чтобы эта настройка действовала.",
    max = 25,
    min = 0,
    step = 0.1,
    decimalPlaces = 1,
    defaultSetting = defaultConfig.walkStepRotationNoiseScale,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "walkStepRotationNoiseScale",
        table = config
    }
}

local runStepRotationTweaks = stepRotationPage:createCategory ("Настройки поворота при шаге во время бега")

runStepRotationTweaks:createSlider{
    label = "Множитель шума для поворота при шаге во время бега",
    description = "Это значение определяет, насколько сильно шум влияет на поворот при шаге во время бега. Если установлено 0, функция добавления шума полностью отключается.",
    max = 5,
    min = 0,
    step = 0.1,
    decimalPlaces = 1,
    defaultSetting = defaultConfig.runStepRotationNoiseAmplitude,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "runStepRotationNoiseAmplitude",
        table = config
    }
}

runStepRotationTweaks:createSlider{
    label = "Скорость шума для поворота при шаге во время бега",
    description = "Это значение определяет, насколько быстрым будет шум при поворотах во время шага при беге. Множитель шума должен быть больше 0, чтобы эта настройка действовала.",
    max = 25,
    min = 0,
    step = 0.1,
    decimalPlaces = 1,
    defaultSetting = defaultConfig.runStepRotationNoiseScale,
    showDefaultSetting = true,
    variable = mwse.mcm.createTableVariable{
        id = "runStepRotationNoiseScale",
        table = config
    }
}