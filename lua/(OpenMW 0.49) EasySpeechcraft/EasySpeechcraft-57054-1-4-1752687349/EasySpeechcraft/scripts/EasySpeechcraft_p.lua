...
I.SkillProgression.addSkillUsedHandler(function(skillid, params)
    ...
    if skillid == 'speechcraft' then
		...
		if newFight <= oldFight then
			--copypasta
			local minLevel = playerSection:get("SubtractBase")
			local mercantile = math.max(0, types.NPC.stats.skills.mercantile(self).modified - minLevel) * playerSection:get("MercantileDispoMult")
			local speechcraft = math.max(0, types.NPC.stats.skills.speechcraft(self).modified - 5) * playerSection:get("SpeechcraftDispoMult")
			local personality = math.max(0, types.NPC.stats.attributes.personality(self).modified - minLevel) * playerSection:get("PersonalityDispoMult")
			local luck = math.max(0, types.NPC.stats.attributes.luck(self).modified - minLevel) * playerSection:get("LuckDispoMult")
...
local function UiModeChanged(data)
	...
	if data.newMode == "Dialogue" and data.arg and types.NPC.objectIsInstance(data.arg) and not currentNPC then
		...
		if lastDispositions[currentNPC.id] then
			...
			currentNPC:sendEvent("EasySpeechcraft_setDisposition",{self,math.min(lastDispositions[currentNPC.id])})
