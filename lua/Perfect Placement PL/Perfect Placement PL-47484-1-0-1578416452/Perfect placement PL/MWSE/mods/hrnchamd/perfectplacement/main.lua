...
local function showGuide()
    ...
    addLine("Obróæ przedmiot", "", config.keybindRotate)
    addLine("Obrót w trybie pionowym", "", config.keybindVertical)
    ...
    addLine("Upuœæ przedmiot", "", config.keybind)
...
local function cellChanged(e)
    ...
    if (this.active.cell.isInterior or e.cell.isInterior) then
        tes3.messageBox{ message = "Nie mo¿na przenosiæ elementów miêdzy komórkami. Reset miejsca docelowego."}
        this.active.position = this.itemInitialPos
        this.active.orientation = this.itemInitialOri
        endPlacement()
    end
    ...
    elseif (target) then
        ...
        if (itemdata and itemdata.owner) then
            ...
            if (owner.objectType == tes3.objectType.faction and owner.playerJoined and owner.playerRank >= itemdata.requirement) then
                ...
                tes3.messageBox{ message = "Ten przedmiot do kogoœ nale¿y." }
                return
            end
        end
        ...
        if (target.sourceMod and not target.cell.isInterior) then
            tes3.messageBox{ message = "Najpierw musisz podnieœæ i upuœciæ ten przedmiot." }
            return
        end
...
mwse.log("[Perfect Placement] pomyœlnie wczytane.")
