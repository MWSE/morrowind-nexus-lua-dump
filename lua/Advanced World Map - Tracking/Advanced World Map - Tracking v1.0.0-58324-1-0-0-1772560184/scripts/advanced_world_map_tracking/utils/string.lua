...
function this.getValueEnumString(tb, max, framePattern, returnTable, customNumber, valueFormat)
    ...
    for _, value in ipairs(tb) do
        if count >= max then
            if returnTable then
                table.insert(str, string.format(l10n("andMorePattern"), (customNumber or tableLib.count(tb)) - count))
            else
                str = string.format("%s "..(l10n("andMorePattern")), str, (customNumber or tableLib.count(tb)) - count)
            end
            break
        end
