              ...
              if hurtLogic then -- only do buff if enough fatigue for it
                if doHurt and dashOnEmpty and (cfat < fatigueCost) and not debug.isGodMode() then
                  if ambient and (sfxVolume > 0.0) then
                    ambient.playSound("Health Damage", {volume = (1.0*sfxVolume)})
                  end
                  if verbose or not (ambient and (sfxVolume > 0.0)) then
                ...
                if ambient and (sfxVolume > 0.0) then
                  ambient.playSound("torch out",{volume=(0.7*sfxVolume), pitch=(1.75 + 0.1*math.random())})
                end
                if verbose and not (doHurt and dashOnEmpty and (cfat < fatigueCost)) then
                  ui.showMessage('DASH!')
                end
