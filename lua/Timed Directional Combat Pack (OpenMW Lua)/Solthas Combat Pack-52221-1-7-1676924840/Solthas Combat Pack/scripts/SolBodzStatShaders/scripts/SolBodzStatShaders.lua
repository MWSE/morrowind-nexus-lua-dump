  ...
  engineHandlers = { 
    ...
    if enabled then
      ...
      if updateTime >= updateInteveral then
        updateTime = 0.0
        if healthIntensity > 0.0 then
          local heaPct = math.sqrt(1.0 - math.min(1.0,(2.0*dynamic.health(self).current/dynamic.health(self).base)))
          shader:setFloat("uHealthFactor",healthIntensity*heaPct)
        end
        if magicIntensity > 0.0 then
          local magPct = math.sqrt(1.0 - math.min(1.0,(2.0*dynamic.magicka(self).current/dynamic.magicka(self).base)))
          shader:setFloat("uMagickaFactor",magicIntensity*magPct)
        end
        if fatigueIntensity > 0.0 then
          local fatPct = math.sqrt(1.0 - math.min(1.0,(2.0*dynamic.fatigue(self).current/dynamic.fatigue(self).base)))
          shader:setFloat("uFatigueFactor",fatigueIntensity*fatPct)
        end
      end
    end
  end
