...
local function moveClothing(data)
    if not checkDoNotUse(data.container) then
        for _, item in ipairs(data.list)
        do
            if not (types.Actor.hasEquipped(data.actorObject, item) or string.find(item.recordId, "autosort_"))
            then
                ...
                if not (clothingType == types.Clothing.TYPE.Ring or clothingType == types.Clothing.TYPE.Amulet)
                then
                    item:moveInto(types.Container.inventory(data.container))
                end
            end
...
local function moveJewelry(data)
    if not checkDoNotUse(data.container) then
        for _, item in ipairs(data.list)
        do
            if not (types.Actor.hasEquipped(data.actorObject, item) or string.find(item.recordId, "autosort_"))
            then
                local clothingType = types.Clothing.record(item).type
                if (clothingType == types.Clothing.TYPE.Ring or clothingType == types.Clothing.TYPE.Amulet)
                then
                    item:moveInto(types.Container.inventory(data.container))
                end
            end
...
return {
    ...
    eventHandlers = {
        moveItems = moveItems,
        moveClothing = moveClothing,
        moveJewelry = moveJewelry,
        moveBooks = moveBooks,
        moveGold = moveGold,
        moveKeys = moveKeys,
        moveMisc = moveMisc,
