...
I.Settings.registerGroup {
    ...
    settings = {
		...
		},
        {
            ...
            renderer = "inputBinding",
            name = "Backward",
            description = 'Move the Camera Backward.',
            default = "b",
            argument = {
                type = "action",
                key = "Backward"
...
local Cursor=ui.create({ layer = 'Console', type = ui.TYPE.Image, props = {color=util.color.rgb(1,1,1),visible=false, relativeSize = util.vector2(1/25, 1/20), relativePosition = util.vector2(0.5, 0.5), anchor = util.vector2(0.5, 0.5), resource = CursorTexture, }, })
...
--    print("selecteSame")
    ...
    for i, actor in ipairs(nearby.actors) do
        if types.Actor.isDead(actor)==false and 
            ...
            or (actor.type==types.Creature and types.Actor.spells(actor)[ActiveFaction.." spellflag"] and actor.recordId==Ray.hitObject.recordId)) then
...
time.runRepeatedly(function() 	
    if RTS==true then
        ...
        if EnnemyFactions then
            for i, actor in ipairs(nearby.actors) do
                if types.Actor.isDead(actor)==false then
                    actor:sendEvent("DeclareFactions",{PlayerFaction=ActiveFaction, EnnemyFactions=EnnemyFactions})
                end
            end
        end
...
--            print("NewCell")
            ...
            for x=util.round(Minimap.layout.MaxX/SizeRatio),util.round(Minimap.layout.MinX/SizeRatio) do
                for y=util.round(Minimap.layout.MinY/SizeRatio),util.round(Minimap.layout.MaxY/SizeRatio) do   
                    ...
                    local Ray=nearby.castRay(util.vector3(x*SizeRatio,y*SizeRatio,MaxZ+5000),util.vector3(x*SizeRatio,y*SizeRatio,MaxZ-15000))
                            --,{collisionType=nearby.COLLISION_TYPE.HeightMap + nearby.COLLISION_TYPE.Water+ nearby.COLLISION_TYPE.World})
                    local Z=0
                    local color
        ...
        for i , actor in ipairs(nearby.actors) do
            if types.Actor.isDead(actor)==false then
                ...
                (actor.type==types.Creature and types.Actor.spells(actor)[ActiveFaction.." spellflag"])) then
                    color=util.color.rgb(0,1,0)
                
                elseif EnnemyFactions then
                    for i, faction in pairs(EnnemyFactions) do
                        if (actor.type==types.NPC or actor.type==types.Creature) and types.Actor.isDead(actor)==false and actor~=self and 
                            ...
                            (actor.type==types.Creature and types.Actor.spells(actor)[faction.." spellflag"])) then
 ...
 --       print("speak")
        ...
        for i, selected in ipairs(SelectedActors) do
            ...
            elseif Cursor.layout.props.resource == AttackTexture and Ray.hitObject then
                ...
                if EnnemyFactions==nil then
                    EnnemyFactions={}
                end
                for j, faction in ipairs(core.factions.records) do
                    if EnnemyFactions[faction.id]==nil then
                        ...
                        or (Ray.hitObject.type==types.Creature and types.Actor.spells(Ray.hitObject)[faction.id.." spellflag"])) then
                            EnnemyFactions[faction.id]=faction.id
                        end
                    end
                end
...
--                    print(ActorId)
		...
		Backward = input.getBooleanActionValue('Backward')
		Forward = input.getBooleanActionValue('Forward')
...
--                            print("DoubleClick")
        ...
        if SelectedActors[1] and SelectedActors[1][1] then
            ...
            if (Collision(Cursor,ShowInventory).UL or Collision(Cursor,Minimap).UL or Collision(Cursor,DefendStance).UL or Collision(Cursor,AttackStance).UL or Collision(Cursor,NothingStance).UL or Collision(Cursor,Patrol).UL ) and Cursor.layout.props.resource~=CursorTexture then
                Cursor.layout.props.resource=CursorTexture

                if Collision(Cursor,DefendStance).UL or Collision(Cursor,AttackStance).UL or Collision(Cursor,NothingStance).UL or Collision(Cursor,Patrol).UL then
            ...
            elseif Collision(Cursor,ShowInventory).UL==nil and Collision(Cursor,Minimap).UL==nil and Collision(Cursor,DefendStance).UL==nil and Collision(Cursor,AttackStance).UL==nil and Collision(Cursor,NothingStance).UL==nil and Collision(Cursor,Patrol).UL==nil  then
                if Ray.hitObject and Ray.hitObject.type and (Ray.hitObject.type==types.NPC or Ray.hitObject.type==types.Creature ) and types.Actor.isDead(Ray.hitObject)==false and 
                    ((Ray.hitObject.type==types.NPC and types.NPC.getFactions(Ray.hitObject)[1]~=ActiveFaction) or 
                    (Ray.hitObject.type==types.Creature and types.Actor.spells(Ray.hitObject)[ActiveFaction.." spellflag"]==nil)) then
        ...
        if StanceExplainations.layout.props.visible==true and Collision(Cursor,DefendStance).UL==nil and Collision(Cursor,AttackStance).UL==nil and Collision(Cursor,NothingStance).UL==nil and Collision(Cursor,Patrol).UL==nil then
        ...
        or (Ray.hitObject.type==types.Creature and (types.Actor.spells(Ray.hitObject)[ActiveFaction.." spellflag"]==nil or types.Actor.isDead(Ray.hitObject)==true ))) then
            if ActorQuickInfos.layout.props.visible==false then
...
--                print(ActorTable[1].id)
	...
	return{SaveRTS=RTS,SaveTeams=SaveTeams,EnnemyFactions=EnnemyFactions}
end
