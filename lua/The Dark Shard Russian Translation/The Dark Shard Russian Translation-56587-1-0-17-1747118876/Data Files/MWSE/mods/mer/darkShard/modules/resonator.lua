...
StaticActivator.register{
    ...
    additionalUI = function(self, parent)
        ...
        if wheelColor == "blue" then rusColor = "синий" end
        if wheelColor == "green" then rusColor = "зеленый" end
        if wheelColor == "red" then rusColor = "красный" end
        local text = string.format("Калибровка: %s", rusColor)
...
event.register("DarkShard:TelescopeCalibrated", function()
    ...
    if resonator and Telescope.getCometSeen() then
        logger:debug("Telescope is calibrated and comet was seen, removing resonator")
