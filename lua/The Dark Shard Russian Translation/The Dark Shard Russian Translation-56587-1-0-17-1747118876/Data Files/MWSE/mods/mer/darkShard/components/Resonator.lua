...
function Resonator.findNearbyResonator(e)
    ...
    if not resonatorRef then
        logger:debug("Failed to find resonator reference")
        return
    end
...
function Resonator:init()
    self.data.calibration = self:getCalibration()
    Telescope.initTargetCalibration(Resonator.getRandomCalibration())

    self:updateNodes()
    Telescope.updateOffsetData(self:getOffsets())
    self.reference.modified = true
end

function Resonator:getCalibration()
    return self.data.calibration or { red = 1, green = 1, blue = 1 }
end
...
function Resonator:getOffsets()
    ...
    local target = Telescope.getTargetCalibration()
    local offsets = {}
...
function Resonator:removeResonator()
    ...
    tes3.messageBox("Вы извлекаете Цветовой резонатор.")
    tes3.addItem{
        reference = tes3.player,
        item = common.config.static.resonator_misc_id,
    }
    local controlPanel = ControlPanel.getNearbyPanel{ position = tes3.player.position}
    local calibration = self:getCalibration()
    local targetCalibration = Telescope.getTargetCalibration()
    local isCalibrated = calibration.red == targetCalibration.red
        and calibration.green == targetCalibration.green
        and calibration.blue == targetCalibration.blue
    if controlPanel ~= nil and not isCalibrated then
        controlPanel:close()
    else
        logger:error("No control panel found")
    end
    resonatorRef:delete()
end

function Resonator.attachResonator(attachNode, attachRef)
    local resonatorRef = tes3.createReference{
        object = Resonator.object_id,
        position = attachNode.worldTransform.translation,
        orientation = attachNode.worldTransform.rotation:toEulerXYZ(),
        cell = attachRef.cell
    }
    --resonatorRef.sceneNode.appCulled = true

    local resonator = Resonator:new({ reference = resonatorRef }) --[[@as DarkShard.Resonator]]
    resonator:init()

    timer.delayOneFrame(function()
        resonator:updateNodes()
        attachNode:updateEffects()
        attachNode:update()
        --resonatorRef.sceneNode.appCulled = false
    end)
    tes3.messageBox("Вы устанавливаете Цветовой резонатор.")
    tes3.removeItem{
        reference = tes3.player,
        item = common.config.static.resonator_misc_id,
    }
    local controlPanel = ControlPanel.getNearbyPanel{ position = attachRef.position}
