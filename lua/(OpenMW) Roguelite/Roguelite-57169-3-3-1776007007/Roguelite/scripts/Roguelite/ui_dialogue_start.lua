...
if saveData.hadRogueliteBefore then
	flexTable.content:add{
			type = ui.TYPE.Text,
			props = {
				relativePosition = util.vector2(0, 1),
				relativeSize = util.vector2(1, 0.5),
				anchor = util.vector2(0, 1),
				text = "Attribute & skill tax already paid",
				textColor = fontColor,--util.color.rgba(1, 1, 1, 1),
				--textAlignH = ui.ALIGNMENT.Center,
				textSize = 19,
			}
	...
	for _, attributeRecord in pairs(core.stats.Attribute.records) do
		...
		local currentValue = playerAttributes[attributeRecord.id].base
		local newValue = math.floor(currentValue * S_ATTRIBUTE_MULT) - math.floor(S_ATTRIBUTE_SUBTRACT)
	...
	for _, skillId in pairs(majorSkills) do
		...
		local currentValue = playerSkills[skillId].base
		local newValue = math.floor(currentValue * S_SKILL_MULT) - math.floor(S_SKILL_SUBTRACT)
