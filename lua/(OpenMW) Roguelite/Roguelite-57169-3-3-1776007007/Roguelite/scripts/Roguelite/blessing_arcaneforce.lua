...
local function getCastChance(spell)
	...
	if not godmode then --if (stats.getMagicEffects().getOrDefault(ESM::MagicEffect::Silence).getMagnitude() && !godmode)
		if playerActiveEffects:getEffect("silence").magnitude > 0 then
			return 0 --return 0;
		end			
	end
	
	if spell.type == core.magic.SPELL_TYPE.Power then --if (spell->mData.mType == ESM::Spell::ST_Power)
		if playerActiveSpells:canUsePower(spell) then --return stats.getSpells().canUsePower(spell) ? 100 : 0;
			return 1
		else
			return 0
		end
	...
	for _,effect in pairs(spell.effects) do --for (const ESM::IndexedENAMstruct& effect : spell->mEffects.mList)
		...
		local s = 2 * playerSkills[effect.effect.school].base --float s = 2.0f * actor.getClass().getSkill(actor, magicEffect->mData.mSchool);
	...
	local actorWillpower = playerAttributes["willpower"].modified --float actorWillpower = stats.getAttribute(ESM::Attribute::Willpower).getModified();
	local actorLuck = playerAttributes["luck"].base --float actorLuck = stats.getAttribute(ESM::Attribute::Luck).getModified();
	...
	local castBonus =  -playerActiveEffects:getEffect("sound").magnitude
	castChance = castChance + castBonus --float castChance = baseChance + castBonus;
	--castChance *= stats.getFatigueTerm();
	local max = playerFatigue.base --float max = getFatigue().getModified();
	local current = playerFatigue.current --float current = getFatigue().getCurrent();
	...
	if saveData.blessings and saveData.blessings.arcaneforce then
		if groupname == "spellcast" then
			if key == "self start" or key == "touch start" or key == "target start" then
				...
				if castedSpell then
					if not debug.isGodMode() and playerActiveEffects:getEffect("silence").magnitude <= 0 and castedSpell.type ~= core.magic.SPELL_TYPE.Power and playerMagicka.current < lastMagicka then
						...
						if castChance < 1 then
							...
							missingCastChance = (1+missingCastChance)^1.75-1
							local additionalCost = 3 * missingCastChance + (cost * missingCastChance / 3)
							if playerMagicka.current >= additionalCost then
								playerMagicka.current = playerMagicka.current - additionalCost
								playerSpells:add("roguelite_soundhack")
							else--if castChance <= 0 then
								local alreadyRefunded = 0
								if I.MBSP then
									local temp = I.MBSP.getNextRefund()
									if temp.mode == "EXPERIMENTAL" then
										alreadyRefunded = temp.refund
									end
								end
								playerSpells:add("roguelite_silencehack")
								playerMagicka.current = playerMagicka.current + cost - alreadyRefunded
								ui.showMessage("requires ".. math.floor( cost + additionalCost + 	0.5) .." magicka")
							end
						end
					end
				end
			elseif key == "self stop" or key == "touch stop" or key == "target stop" then
				playerSpells:remove("roguelite_soundhack")
				playerSpells:remove("roguelite_silencehack")
			--	castedSpell = nil
			--	casting = false
