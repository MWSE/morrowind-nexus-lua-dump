...
local function calculateSpeedIncrease(npc)
	...
	local targetRunSpeed = calculateRunSpeed(currentSpeed)*(0.95+playerSkills.sneak.modified/1000)
	--print((0.95+playerSkills.sneak.modified/1000))
	...
	return math.ceil(speedIncrease)
end
		...
		if saveData.blessings and saveData.blessings.sneak then
			...
			if isSneaking and not isSwimming and lastFatigueTick then
				...
				if dt > 0 then					
					...
					local runPerSec   = (fFatigueRunBase  + encRatio * fFatigueRunMult) * math.max(0, 0.73 - playerSkills.sneak.modified/300)
					--print(math.max(0, 0.73 - playerSkills.sneak.modified/300))
					-- Add only the difference so final = runPerSec * dt * movement
					local delta = (runPerSec - sneakPerSec) * dt * movement
					
					local before = playerFatigue.current
					playerFatigue.current = math.max(0, before - delta)
					
					--	print(string.format(
					--	"[Sneak→Run] enc=%.3f movement=%.2f dt=%.4f S=%.3f R=%.3f Δ=%.3f cur: %.2f -> %.2f",
					--	encRatio, movement, dt, sneakPerSec, runPerSec, delta, before, playerFatigue.current))
				end
			...
			if isSneaking and not saveData.speedMod and not isSwimming then
				if core.getRealTime() > sneakThrottle then
					...
					--print(speedMod)
					playerAttributes.speed.base = playerAttributes.speed.base + speedMod
					saveData.speedMod = speedMod
					playerSpells:add("roguelite_shadowdancing")
					
				end
			elseif (isSwimming or not isSneaking) and saveData.speedMod then
				playerAttributes.speed.base = playerAttributes.speed.base - saveData.speedMod
				saveData.speedMod = nil
				playerSpells:remove("roguelite_shadowdancing")
			end
			lastFatigueTick = now
		end
	end
