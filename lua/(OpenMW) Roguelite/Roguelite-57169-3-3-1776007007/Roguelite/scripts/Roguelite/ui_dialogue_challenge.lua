if S_SELECTABLE_CHALLENGES == 0 then
	challengeSelectionReturn({})
	return
end
...
local REQUIRED_CHALLENGES = math.min(countChallenges,math.max(S_CHALLENGES_TARGET,S_SELECTABLE_CHALLENGES))
...
if S_PENALTY_PER_DEATH then
	flex.content:add(textElement("Challenge requirements will increase by "..S_DYING_PENALTY.."% every time you die", darkerFont, fontSize*0.8))
else
	flex.content:add(textElement("Challenge requirements will increase by "..S_DYING_PENALTY.."% when you die the first time", darkerFont, fontSize*0.8))
end
