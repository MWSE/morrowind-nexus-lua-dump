return function (challengeFilter)
	...
	if (runDB:get(saveData.runId) or 0) > 0 then
		--print("Roguelite Hardcore Penalty","*",S_DYING_PENALTY)
		if S_PENALTY_PER_DEATH then
			penaltyMult = (1 + runDB:get(saveData.runId) * S_DYING_PENALTY / 100)
		else
			penaltyMult = (1 + S_DYING_PENALTY / 100)
		end
	end
	...
	for _, challengeId in ipairs(sortedIds) do
		...
		--for challengeId in pairs(challengeData) do
			...
			local challengeRequirement = challenge.requirement * S_CHALLENGE_DIFFICULTY
			...
			if value == 0 then
				if (saveData.progress[challengeId] or 0) >= challengeRequirement then
					...
					if saveData.completedChallenges > (runDB:get(saveData.runId.."completedChallenges") or 0) and saveData.completedChallenges % S_CHALLENGES_TARGET == 0
					and (saveData.completedChallenges <= S_CHALLENGES_TARGET or not S_ONE_UNLOCK_PER_RUN) then
			...
			-- Show current dungeon status below the dungeons challenge
			if challengeId == "dungeons" and dungeonStatus then
				...
				flex.content:add(statusContainer)
				
				if dungeonStatus.cleared then
					statusContainer.content:add{
						type = ui.TYPE.Text,
						props = {
							text = dungeonStatus.cellName..": Cleared ",
							textColor = util.color.rgb(0.2,0.8,0.2),
							textShadow = true,
							textShadowColor = util.color.rgba(0,0,0,1),
							textSize = fontSize - 2,
				...
				elseif dungeonStatus.qualifies then
					statusContainer.content:add{
						type = ui.TYPE.Text,
						props = {
							text = dungeonStatus.cellName..": "..dungeonStatus.enemiesKilled.."/"..dungeonStatus.enemiesTotal.." ",
							textColor = morrowindGold,
							textShadow = true,
							textShadowColor = util.color.rgba(0,0,0,1),
							textSize = fontSize - 2,
