return function (challengeFilter)
	...
	if (runDB:get(saveData.runId) or 0) > 0 then
		--print("Roguelite Hardcore Penalty","*",playerSection:get("HARDCORE_PENALTY"))
		penaltyMult = playerSection:get("HARDCORE_PENALTY")
	end
	...
	for _, challengeId in ipairs(sortedIds) do
		...
		--for challengeId in pairs(challengeData) do
			...
			challengeRequirement = challengeRequirement * penaltyMult
