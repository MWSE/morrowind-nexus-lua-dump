...
local selectedBirthsignIndex
local iterateBirthsignsIndex
local birthsigns = {}
local selectedBirthsign = nil


function scrollBirthsignSelectionDialogue(dir)
	if not birthsignSelectionDialogue then return end
	if not selectedBirthsignIndex then
		selectedBirthsignIndex = dir > 0 and 1 or #birthsigns
	else
		local newSelectedIndex = (selectedBirthsignIndex or 0) + dir
		newSelectedIndex = math.max(1, math.min(#birthsigns, newSelectedIndex))
		if newSelectedIndex == selectedBirthsignIndex then
			if newSelectedIndex == #birthsigns then
				newSelectedIndex = 1
			elseif newSelectedIndex == 1 then
				newSelectedIndex = #birthsigns
			end
		end
		selectedBirthsignIndex = newSelectedIndex
	end
	selectedBirthsign = birthsigns[selectedBirthsignIndex].id
...
local descriptionFontSize = 21 --16
...
for id, birthsign in pairs(types.Player.birthSigns.records) do
	if birthsign.id ~=playerBirthsign then
		...
		countBirthsigns = countBirthsigns + 1
	end
end
local screenHeight = ui.layers[ui.layers.indexOf("HUD")].size.y
local uiHeight = (countBirthsigns+1)*math.floor(fontSize*lineHeightMultiplier) + fontSize
while uiHeight > screenHeight do
	lineHeightMultiplier = math.max(1.1, lineHeightMultiplier - 0.1)
	fontSize = fontSize - 1
	uiHeight = (countBirthsigns+1)*math.floor(fontSize*lineHeightMultiplier) + fontSize
end

--if uiHeight > screenHeight then
--	fontSize = fontSize - 2
--	uiHeight = (countBirthsigns+1)*fontSize*lineHeightMultiplier + fontSize
--	if uiHeight > screenHeight then
--		local tempScreenHeight = screenHeight - fontSize
--		lineHeightMultiplier = math.max(1.1, lineHeightMultiplier * tempScreenHeight / uiHeight)
--		uiHeight = (countBirthsigns+1)*fontSize*lineHeightMultiplier + fontSize
--		while uiHeight > screenHeight do
--			lineHeightMultiplier = math.max(1.1, lineHeightMultiplier - 0.1)
--			fontSize = fontSize - 1
...
local function textElement(str, color, size)
	return { 
		type = ui.TYPE.Text,
		props = {
...
local function updateDescriptionText()
	if selectedBirthsign and descriptionText then
		local birthsign = nil
		for i, bs in ipairs(birthsigns) do
			...
			local function addFlavorLine(text)
				flavorTextContainer.content:add{
					...
					props = {
						...
						textSize = descriptionFontSize - 2,
...
rightPanel.content:add(textElement("Description:", fontColor, descriptionFontSize))
rightPanel.content:add{ props = { size = v2(1, 1) * 1 } }
...
local descriptionContainer = {
	type = ui.TYPE.Widget,
	props = {
		size = v2(descriptionWidth-8, (uiHeight - fontSize - descriptionFontSize - math.floor(fontSize*lineHeightMultiplier) - 3)*1),
...
function updateAllBirthsignButtons()
...
local function makeBirthsignButton(birthsign)
	...
	local box = {
		...
		type = ui.TYPE.Widget,
		props = {
			size = v2(350, math.floor(fontSize* lineHeightMultiplier)),
		},
		content = ui.content {}
	}
	...
	local tempBirthsignIndex = iterateBirthsignsIndex
	clickbox.events = {
		mouseRelease = async:callback(function(_, elem)
			if birthsignButtonFocus == elem.userData.birthsignId then
				...
				selectedBirthsignIndex = tempBirthsignIndex
				updateAllBirthsignButtons()
			end
		end),
...
for i, birthsign in ipairs(birthsigns) do
	iterateBirthsignsIndex = i
	birthsignList.content:add(makeBirthsignButton(birthsign))
	--birthsignList.content:add{ props = { size = v2(1, 1) * 1 } }
end

mainFlex.content:add{ props = { size = v2(1, 1) * 2 } }
