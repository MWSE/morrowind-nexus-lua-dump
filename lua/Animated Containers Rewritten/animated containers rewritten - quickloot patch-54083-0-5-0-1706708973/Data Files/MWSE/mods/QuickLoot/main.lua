--[[
	...
	patched by herbert100 to add support for animated containers
	all new lines will have a comment that says "-- NEW"
]] --

local config = include("QuickLoot.config")

-- if config is `nil`, then "QuickLoot" isn't installed, 
-- and this file doesn't have anything to do
if not config then return end -- NEW

local ac_interop = include("herbert100.animated containers.interop") ---@type MWCA.interop?  -- NEW

local interop = require("QuickLoot.interop")

local target_safe_handle ---@type mwseSafeObjectHandle -- NEW
...
local function refreshItemsList()
	...
	if canLoot and ac_interop and currentTarget then 	-- NEW
		ac_interop.try_to_open(currentTarget, false)	-- NEW
	end													-- NEW
...
local function clearQuickLootMenu(destroyMenu)
	...
	if not destroyMenu  then return end

	if ac_interop and target_safe_handle and target_safe_handle:valid() then 	-- NEW
		ac_interop.try_to_close(target_safe_handle:getObject(), true)			-- NEW
	end																			-- NEW

	if quickLootGUI then
...
local function onActivationTargetChanged(e)
	...
	-- check again if we should close the container
	if ac_interop and target_safe_handle and target_safe_handle:valid() then	-- NEW
		ac_interop.try_to_close(target_safe_handle:getObject(), true)			-- NEW
	end																			-- NEW
	
	currentTarget = newTarget
	target_safe_handle = tes3.makeSafeObjectHandle(currentTarget)				-- NEW
