...
]] ---@class herbert.AC.Animation_Info :herbert.AC.Animation_Info.new_params
...
Animation_Info.__index = Animation_Info

---@param params herbert.AC.Animation_Info.new_params
---@return herbert.AC.Animation_Info
function Animation_Info.new(params)
	local self = setmetatable(params or {}, Animation_Info)

	self.open_time = tonumber(rawget(self, "open_time"))
	self.close_time = tonumber(rawget(self, "close_time"))

	---@cast self herbert.AC.Animation_Info
	if self.sound_id then
		self.open_sound = self.open_sound or
			string.format("AC_%s_open", self.sound_id)
		self.close_sound = self.close_sound or
			string.format("AC_%s_close", self.sound_id)
	end

	if self.open_sound and not string.find(self.open_sound, "%.wav$") then
		self.open_sound = string.format("AC\\%s.wav", self.open_sound)
	end
	-- Make sure the sound paths exist, to avoid future errors.
	-- Credit to rhjelte for the idea.
	if self.open_sound and not tes3.getFileExists("Data Files\\Sound\\" .. self.open_sound) then
		log:warn("Tried to set the opening sound, but the provided file did not exist: %q", self.open_sound)
		self.open_sound = nil
	end
	...
	if self.close_sound and not tes3.getFileExists("Data Files\\Sound\\" .. self.close_sound) then
		log:warn("Tried to set the closing sound, but the provided file did not exist: %q", self.close_sound)
