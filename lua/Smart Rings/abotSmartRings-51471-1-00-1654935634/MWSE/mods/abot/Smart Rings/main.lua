...
local function equip(e)
	...
	--[[
	timer.start({duration = 0.2, type = timer.real, callback = function ()
		skipOnce = true
		mobilePlayer:equip({item = ringId})
	end
	})
	return false
	]]
...
local function equipped(e)
	...
	if not (obj.slot == tes3_clothingSlot_ring) then
		return
	end
	local ringId = obj.id:lower()
	mwse.log('%s: equipped() "%s" "%s"', modPrefix, ringId, obj.name)
end
...
local function unequipped(e)
	...
	local ringId = obj.id:lower()
	mwse.log('%s: unequipped() "%s" "%s"', modPrefix, ringId, obj.name)
end
]]
