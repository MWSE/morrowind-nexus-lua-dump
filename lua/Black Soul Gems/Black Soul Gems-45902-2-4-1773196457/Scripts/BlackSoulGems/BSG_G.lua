...
local function getSoulId(id)
    if types.Creature.createRecordDraft then
        local name = types.NPC.records[id].name
        if not savedActors[id] then
            local creature = types.Creature.createRecordDraft {
                template = types.Creature.records["soul_generic"]
                , name = name

            }
            creature = world.createRecord(creature)
            savedActors[id] = creature.id
        end
...
local function NPCTrapped(NPC, fgemId)
    ...
    for index, soulOb in ipairs(soulObLs) do
        if not soulOb.type.getSoul(soulOb) then
            ...
            NPC:sendEvent("BSG_addVfx","VFX_Soul_Trap")--anim.addVfx(NPC, "VFX_Soul_Trap")
            return true
        end
    end
    return false
...
local function fixCell()
    ...
    if greefOb:isValid() and greefOb.count > 0 then
        greefOb:remove()
        fixedCell = true
    end
    if tableOb:isValid() and tableOb.count > 0 then
        tableOb:remove()
        fixedCell = true
    end
end
...
local function onSave()
    return { fixedCell = fixedCell, savedActors = savedActors }
end
local function onPlayerAdded(plr)
    ...
    alterObject = world.getObjectByFormId(core.getFormId("blacksoulgems.esp", 2))
end
...
local function onLoad(data)
    ...
    if data.savedActors then
        savedActors = data.savedActors
    end
    checkGemID()
    alterObject = world.getObjectByFormId(core.getFormId("blacksoulgems.esp", 2))
end
local function useHandler(obj)
    print(obj.recordId)
    print(gemId)
    if obj.recordId == gemId and types.Miscellaneous.getSoul(obj) then
        world.players[1]:sendEvent("BSG_OpenEnchantMenu", obj)
        return false
    end
end
