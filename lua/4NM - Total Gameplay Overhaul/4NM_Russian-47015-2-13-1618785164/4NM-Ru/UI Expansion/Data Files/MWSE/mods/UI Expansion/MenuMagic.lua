...
local function searchSubList(titleElement, listElement, isSpellFilter)
	...
	--	titleElement.visible = true		-- закомментить чтобы слова Заклинания/Способности не становились видны при филтрации
		listElement.visible = true
		return true
	else
	--	titleElement.visible = false	-- закомментить чтобы слова Заклинания/Способности не становились видны при филтрации
...
local function searchSpellsList()
	...
	local elements = tes3ui.findMenu(GUI_ID_MenuMagic):findChild(GUI_ID_MagicMenu_spells_list).widget.contentPane.children -- это MM:findChild(-1155)
	...
	-- Figure out dividers.		закомментить чтобы разделительные полоски не становились видны при фильтре
...
--	magicFilters:clearFilter() 	-- закомментить чтобы фильтр сохранялся при входе в меню 
