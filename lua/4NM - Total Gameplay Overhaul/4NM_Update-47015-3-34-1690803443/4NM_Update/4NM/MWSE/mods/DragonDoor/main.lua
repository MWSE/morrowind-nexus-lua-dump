...
if not H[r] and not BList[bid] and (m.actorType == 1 and m.flee < 70 and (cf.vamp or not r.baseObject.head.vampiric) or (ob.biped or ob.type ~= 0 or ob.usesEquipment))
...
if cf.help and r.cell.isInterior and not TH[r] and not BList[bid] then
...
if actp and door.destination then
	...
	if cf.fix2 then		local bb = door.object.boundingBox		local DX = (bb.max.x - bb.min.x) > (bb.max.y - bb.min.y)	local ang		Matr:toRotationZ(StartZ)
...
if door and door.object.objectType == tes3.objectType.door and not door.destination and not tes3.getLocked{reference = door} then local r = e.reference		local m = e.mobile
...
elseif not (e.cell.isInterior or e.previousCell.isInterior) then	local c
	for r, t in pairs(H) do c = r.cell
		if c then
			if not c.isInterior and p.position:distance(r.position) > 13000 then
		...
		else H[r] = nil			--tes3.messageBox("ref    problem")
		end
	end
else
