...
local function giveSecretHint(e)
    ...
    for k, v in pairs(temp2) do
        for npc in tes3.getCell {id = v}:iterateReferences(tes3.objectType.npc) do
            if
                core.filterNpcLittleSecret and
...
local function amuletEquipped(this)
    ...
    if not cell then
        cell = tes3.getCell{id = item.name:sub(0,-8)}
    end

    if equipor == tes3.player.mobile then
        data.amuletTableTaken[item.id] = this.item.name:sub(0, -8)
        tes3.messageBox {
...
local function initialization()
	...
	tes3activator.create{id = "atakinditelevfx", mesh = "ata_kindi_tele.nif", name = "Beautiful Effect", script = "sprigganeffect"}
