...
core.listTheTomb = function()
    ...
    for k, v in pairs(data.source) do
        ...
        for i = 1, table.size(data.tombTable) do
            local v = "ata_kindi_amulet_" .. i
            local tomb = tes3.getObject(v).name:sub(0, -8)

            if data.waitCont.object.inventory:contains(v) then
                insideDummy[tomb] = v
            else
                outsideDummy[tomb] = v
            end
        ...
        for y, z in pairs(v) do
            if
                outsideDummy[z] and tes3.player.object.inventory:contains(outsideDummy[z]) and
            ...
            if
                outsideDummy[z] and not tes3.player.object.inventory:contains(outsideDummy[z]) and core.alternate and
                    z:lower():match(key:lower())
             then
...
core.dropBad = function()
    ...
    for k, amulet in pairs(tes3.player.object.inventory) do
        if string.match(amulet.object.id, "ata_kindi_amulet_") then
            if
                not data.door[amulet.variables[1].data.tomb] or
                    not table.find(core.doorxmod, amulet.object.name:sub(0, -8))
             then
                tes3.messageBox("Dropping bad reference " .. amulet.object.id)
                mwse.log(
                    "[[Ancestral Tomb Amulets log]] ~ Dropping bad reference " ..
...
core.chestSetup = function(cont)
    ...
    for k, v in pairs(data.newAm) do
        ...
        local tempstr = string.match(newAmulet.name, "%a+")

        if data.tooltipsComplete then
            if data.customAmuletTooltip[tempstr .. " Ancestral Tomb"] then
                data.tooltipsComplete.addTooltip(
                    tostring(newAmulet.id),
                    string.format("%s", data.customAmuletTooltip[tempstr .. " Ancestral Tomb"])
                )
            elseif tempstr then
                data.tooltipsComplete.addTooltip(
                    tostring(newAmulet.id),
...
core.amuletCreation = function(cell)
    ...
    if config.tombRaider then
        if cell.id:match("Ancestral Tomb") then
            chance = 100
        end
    end
    ...
    for _, amulet in pairs(data.waitCont.object.inventory) do
        ...
        if data.amuletTable[cell.id] == amulet.object.id then
            raiderItem = amulet
        end
    end
...
core.amuletInfoCheat = function(item)
    ...
    for _, cell in pairs(tes3.dataHandler.nonDynamicData.cells) do
        for ref in cell:iterateReferences() do
            if (ref.object.inventory and ref.object.inventory:contains(item)) or ref.id == item then
                loc = ref.cell
                refs = ref
            end
        end
