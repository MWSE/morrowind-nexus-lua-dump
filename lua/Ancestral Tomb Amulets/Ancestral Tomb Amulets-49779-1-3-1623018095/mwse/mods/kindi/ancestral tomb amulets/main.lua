...
local function giveSecretHint(e)
    ...
    for k, v in pairs(temp2) do
        for npc in tes3.getCell {id = v}:iterateReferences(tes3.objectType.npc) do
            if core.filterNpcLittleSecret(npc) and npc ~= actor then
...
local function amuletEquipped(this)
    ...
    if not cell then
        cell = tes3.getCell {id = table.find(data.amuletTable, this.item.id)}
        this.itemData.data.tomb = cell.id
    end

    if equipor == tes3.player.mobile then
        data.amuletTableTaken[item.id] = this.itemData.data.tomb
        tes3.messageBox {
            message = this.itemData.data.tomb,
...
local function initialization()
    for _, cell in pairs(tes3.dataHandler.nonDynamicData.cells) do
        for doors in cell:iterateReferences(tes3.objectType.door) do
            if
                doors.destination and doors.destination.cell and doors.destination.cell.id and
                    string.match(doors.destination.cell.id, "Ancestral Tomb")
             then
                if not string.match(doors.destination.cell.id, ", ") and not doors.cell.isInterior then
                    data.door[doors.destination.cell.id] = doors
                elseif not data.tombExtra[doors.destination.cell.id] then
                    data.rejectedTombs[doors.destination.cell.id] = doors.cell.id
                end
            end
        end
    end
...
local function setDataforAmuletsInPlayerInventory(this)
    ...
    for k, v in pairs(tes3.player.object.inventory) do
        if string.startswith(v.object.id, "ata_kindi_amulet_") then
            if table.find(data.amuletTable, v.object.id) then
                data.amuletTableTaken[v.object.id] = v.object.id
                if not v.variables then
                    local datas = tes3.addItemData {to = tes3.player, item = v.object.id}
                    if tes3.getCell {id = table.find(data.amuletTable, v.object.id)} then
                        datas.data.tomb = table.find(data.amuletTable, v.object.id)
                        data.amuletTableTaken[v.object.id] = datas.data.tomb
                    else
                        mwse.log(
                            string.format(
                                "ATA ERROR: %s [%s] is not properly initialised",
                                v.object.name,
                                v.variables[1].data.tomb
                            )
                        )
                    end
                end
    ...
    if string.startswith(_.id, "ata_kindi_amulet_") then
        data.amuletTableTaken[_.id] = _.data.tomb or _.object.name:sub(0, -8)
    end
end
...
local function loadDataAndCheckMod(loaded)
    if not tes3.isModActive("Ancestral Tomb Amulets.esm") then
        tes3.messageBox {
            message = "[Ancestral Tomb Amulets] mod is missing the ESM, ensure that the mod is installed properly.",
            buttons = {tes3.findGMST(26).value}
        }
    end
    local fileName = loaded.filename

    local toLoadData = tes3.player.data.ata_kindi_data or {}
...
local function saveData(saved)
    ...
    tes3.player.data.ata_kindi_data = toSaveData
end
