...
local PROFILE_DEFAULTS = {
    deathCount    = 0,
    luckPenaltyForgivenDeaths = 0, -- deaths whose death-counter Luck penalty has been reset/forgiven
    trainCount    = 0,
    jailCount     = 0,
    highestBounty = 0,
    mostGold      = 0,
    ...
    cmcDiseasedCreaturesCured = 0, -- diseased creatures cured via Creature Mercy and Contagion
    cmcBlightedCreaturesCured = 0, -- blighted creatures cured via Creature Mercy and Contagion
    cmcDiseasesSpread  = 0,   -- diseases spread via Creature Mercy and Contagion
    cmcBlightsSpread   = 0,   -- blights spread via Creature Mercy and Contagion
...
local PROFILE_EXTRA_KEYS = {
    ...
    "distMount_skyrender",
    "distMount_nix",
    "distMount_cliffracer",
    "distMount_boar",
}

-- These counters intentionally remain profile-global across save loads.
-- Deaths drive the optional Luck penalty, and quickloads/save-scums are
-- themselves load actions, so rolling either back would defeat the counter.
local GLOBAL_PROFILE_KEYS = {
    deathCount = true,
    luckPenaltyForgivenDeaths = true,
    quickloadCount = true,
}
...
local function initProfileId()
    ...
    for key, default in pairs(PROFILE_DEFAULTS) do
    ...
    if not section:get("slavesFreedStr") then section:set("slavesFreedStr", "") end
    state.profileId = profileId
    profileSection = section
end

local function profileSnapshotValue(key)
    if not profileSection then return nil end
    return profileSection:get(key)
end

local function buildProfileSnapshot()
    local values = {}
    if profileSection then
        for key in pairs(PROFILE_DEFAULTS) do
            local value = profileSnapshotValue(key)
            if value ~= nil then values[key] = value end
        end
        for _, key in ipairs(PROFILE_EXTRA_KEYS) do
            local value = profileSnapshotValue(key)
            if value ~= nil then values[key] = value end
        end
    end
...
local function restoreProfileSnapshot(snapshot)
    if not profileSection or type(snapshot) ~= "table" then return end

    local values = snapshot.values or snapshot
    if type(values) ~= "table" then return end

    local globalValues = {}
    for key in pairs(GLOBAL_PROFILE_KEYS) do
        globalValues[key] = profileSection:get(key)
    end

    for key, default in pairs(PROFILE_DEFAULTS) do
        local value = values[key]
        if value == nil then value = default end
        profileSection:set(key, value)
    end

    for _, key in ipairs(PROFILE_EXTRA_KEYS) do
        local value = values[key]
        if value == nil and key ~= "chargenLuck" then
        ...
        if value ~= nil then profileSection:set(key, value) end
    end

    for key in pairs(GLOBAL_PROFILE_KEYS) do
        local savedValue = values[key]
        local globalValue = globalValues[key]
        if type(savedValue) == "number" and type(globalValue) == "number" then
            profileSection:set(key, math.max(savedValue, globalValue))
        elseif globalValue ~= nil then
            profileSection:set(key, globalValue)
        elseif savedValue ~= nil then
            profileSection:set(key, savedValue)
        end
    end
end
...
-- Sorted display caches. These avoid re-sorting favourite/top-N maps every
-- time the summary page or Stats Window asks for the same value. Mutating
-- functions mark the relevant cache dirty; readers rebuild it lazily.
local function buildSortedCountList(cache)
    local sorted = {}
    for name, count in pairs(cache or {}) do
...
local function copySortedSlice(sorted, limit)
    local result = {}
    local count = #sorted
...
local function serializeCountMap(cache)
    local parts = {}
    for name, count in pairs(cache or {}) do
...
local function serializeSetMap(cache)
    local parts = {}
    for id in pairs(cache or {}) do
        parts[#parts + 1] = id
    end
    return table.concat(parts, "\n"), #parts
end
...
V.artifactsSeenSaveDirty = false
...
local function flushArtifactsSeenCache()
    if not V.artifactsSeenSaveDirty or not profileSection then return end
    profileSection:set("artifactsSeenStr", serializeSetMap(V.artifactsSeenCache))
    V.artifactsSeenSaveDirty = false
end
...
local function flushDiseasesSeenCache()
    if not V.diseasesSeenSaveDirty or not profileSection then return end
    profileSection:set("diseasesSeenStr", serializeSetMap(V.diseasesSeenCache))
    V.diseasesSeenSaveDirty = false
end
...
local spellEffectsSeenSaveDirty = false

local function loadSpellEffectsSeenCache()
    local raw = profileSection:get("spellEffectsSeenStr") or ""
    spellEffectsSeenCache = {}
    spellEffectsSeenSaveDirty = false
    for id in raw:gmatch("[^\n]+") do spellEffectsSeenCache[id] = true end
end

local function flushSpellEffectsSeenCache()
    if not spellEffectsSeenSaveDirty or not profileSection then return end
    profileSection:set("spellEffectsSeenStr", serializeSetMap(spellEffectsSeenCache))
    spellEffectsSeenSaveDirty = false
end
...
local function markSpellEffectSeen(effectId)
    ...
    spellEffectsSeenSaveDirty = true
    return true
end
...
V.weaponTallyCache = {}
V.weaponTopCache = nil
V.weaponTopCacheDirty = true
V.weaponTallySaveDirty = false
...
local function flushWeaponTallyCache()
    if not V.weaponTallySaveDirty or not profileSection then return end
    profileSection:set("weaponTallyStr", serializeCountMap(V.weaponTallyCache))
    V.weaponTallySaveDirty = false
end

local function saveWeaponTallyCache()
    V.weaponTallySaveDirty = true
end
...
local function getTopWeapons(limit)
    if V.weaponTopCacheDirty or not V.weaponTopCache then
        V.weaponTopCache = buildSortedCountList(V.weaponTallyCache)
        V.weaponTopCacheDirty = false
    end
    return copySortedSlice(V.weaponTopCache, limit or 5)
end
...
V.spellTallyCache = {}
V.spellTopCache = nil
V.spellTopCacheDirty = true
V.spellTallySaveDirty = false
...
V.recentSpellCastTimes = {}  -- short-lived dedupe: source/id/name -> simulation time
...
local function loadPeopleMetCache()
    ...
    for id in raw:gmatch("[^\n]+") do
        if id ~= "" and not V.peopleMetCache[id] then
            V.peopleMetCache[id] = true
            V.peopleMetCount = V.peopleMetCount + 1
        end
    end
end

local function flushPeopleMetCache()
    if not V.peopleMetSaveDirty or not profileSection then return end
    local serialized = serializeSetMap(V.peopleMetCache)
    profileSection:set("peopleMetStr", serialized)
    V.peopleMetSaveDirty = false
end

local function recordPeopleMet(npcId)
    if not npcId or npcId == "" or V.peopleMetCache[npcId] then return false end
    V.peopleMetCache[npcId] = true
    V.peopleMetCount = (V.peopleMetCount or 0) + 1
    set("peopleMet", V.peopleMetCount)
    V.peopleMetSaveDirty = true
    return true
end

local function flushSpellTallyCache()
    if not V.spellTallySaveDirty or not profileSection then return end
    profileSection:set("spellTallyStr", serializeCountMap(V.spellTallyCache))
    V.spellTallySaveDirty = false
end

local function saveSpellTallyCache()
    V.spellTallySaveDirty = true
end

local function recordSpellCast(spellName, dedupeKey)
    if not spellName or spellName == "" then return end

    -- Several systems can report the same successful cast in the same
    -- simulation instant: SkillProgression, activeSpells, Spell Framework
    -- Plus/MagExp, and enchanted-item charge polling. Suppress only those
    -- near-simultaneous duplicate reports. This is intentionally a very short
    -- window, so later manual casts of the same spell are still counted.
    local now = core.getSimulationTime()
    local key = dedupeKey or spellName
    local last = V.recentSpellCastTimes[key]
    if last and (now - last) < 0.35 then return end
    V.recentSpellCastTimes[key] = now
    for k, t in pairs(V.recentSpellCastTimes) do
        if (now - t) > 2.0 then V.recentSpellCastTimes[k] = nil end
    end
...
V.recordSpellRecordCast = function(spellId, fallbackName, source)
    if spellId == nil or spellId == "" then return false end
    spellId = tostring(spellId)

    local displayName = fallbackName
    local ok, rec = pcall(function() return core.magic.spells.records[spellId] end)
    if ok and rec then
        -- Only player-cast spells/powers belong in this tally. Abilities,
        -- diseases, blights, and curses may appear as active effects but are
        -- not voluntary casts.
        if rec.type ~= nil and rec.type ~= 0 and rec.type ~= 5 then return false end
        if rec.name and rec.name ~= "" then displayName = rec.name end
    end

    recordSpellCast(displayName or spellId, "spell:" .. spellId)
    return true
end

V.getItemDisplayName = function(item, fallbackName)
    if not item then return fallbackName end
...
V.recordSelectedSpellCast = function(source)
    local selected = nil
    for _, getter in ipairs({
        function() return core.magic.getSelectedSpell() end,
        function() return T.Actor.getSelectedSpell(self) end,
        function() return T.Player.getSelectedSpell(self) end,
    }) do
        local ok, result = pcall(getter)
        if ok and result then selected = result; break end
    end

    if not selected or selected == "" then return false end

    if type(selected) == "string" then
        return V.recordSpellRecordCast(selected, nil, source or "selected")
    elseif type(selected) == "table" then
        if selected.item then
            local name = V.getItemDisplayName(selected.item, selected.name or selected.id)
            if name then
                recordSpellCast(name, "item:" .. tostring(selected.id or name))
                return true
            end
        end
        return V.recordSpellRecordCast(selected.id, selected.name, source or "selected")
    elseif type(selected) == "userdata" then
        local isObject = false
        pcall(function() if selected.recordId then isObject = true end end)
        if isObject then
            local name = V.getItemDisplayName(selected, selected.recordId)
            recordSpellCast(name, "item:" .. tostring(selected.recordId or name))
            return true
        end

        local id, name = nil, nil
        pcall(function() id = selected.id end)
        pcall(function() name = selected.name end)
        return V.recordSpellRecordCast(id, name, source or "selectedUserdata")
    end

    return false
end

V.recordSelectedEnchantedItemUse = function(source)
    local item = nil
    pcall(function() item = T.Actor.getSelectedEnchantedItem(self) end)
    if not item then return false end

    local name = V.getItemDisplayName(item, item.recordId)
    if not name or name == "" then return false end
    recordSpellCast(name, "item:" .. tostring(item.recordId or name))
    return true
end

V.skillUseTypeName = function(useType)
    local ok, typesTable = pcall(function() return I.SkillProgression.SKILL_USE_TYPES end)
    if not ok or type(typesTable) ~= "table" then return nil end
    for name, value in pairs(typesTable) do
        if value == useType then return tostring(name) end
    end
    return nil
end

V.isSpellcastSkillUse = function(params)
    if not params then return false end

    local ok, spellcastSuccess = pcall(function()
        return I.SkillProgression.SKILL_USE_TYPES.Spellcast_Success
    end)
    if ok and params.useType == spellcastSuccess then return true end

    local name = V.skillUseTypeName(params.useType)
    if not name then return false end
    name = name:lower()
    return name:find("spellcast", 1, true) ~= nil and name:find("success", 1, true) ~= nil
end

V.isEnchantItemUse = function(params)
    if not params then return false end

    local name = V.skillUseTypeName(params.useType)
    if name then
        local lower = name:lower()
        if lower:find("create", 1, true) then return false end
        if lower:find("enchant", 1, true)
           and (lower:find("use", 1, true)
    ...
    -- Fallback for OpenMW/interface versions where the enum name is not
    -- exposed: self-enchanting uses type 0 in vanilla, so avoid that and only
    -- count gameplay uses, not Enchanting UI actions.
    return params.useType ~= 0 and not V.currentUiMode
end

V.onMagExpCastRequest = function(data)
    if not state.profileId then return end
    if type(data) ~= "table" then return end

    if data.isItem or data.itemName or data.itemRecordId then
        local name = data.itemName or data.spellName or data.itemRecordId or data.spellId
        recordSpellCast(name, "item:" .. tostring(data.itemRecordId or name))
        return
    end

    V.recordSpellRecordCast(data.spellId, data.spellName, "magexp:spell")
end
            ...
            if activeSpell.fromEquipment then
                ...
                if not dominated then
                    ...
                    if displayName and displayName ~= "" then
                        recordSpellCast(displayName, activeSpell.id and ("spell:" .. tostring(activeSpell.id)) or ("active:" .. tostring(activeSpell.activeSpellId or displayName)))
                    end
                end
            end
...
local function getTopSpells(limit)
    if V.spellTopCacheDirty or not V.spellTopCache then
        V.spellTopCache = buildSortedCountList(V.spellTallyCache)
        V.spellTopCacheDirty = false
    end
    return copySortedSlice(V.spellTopCache, limit or 5)
end
...
local function pollEnchantedItemUse()
    ...
    if currentId ~= V.lastEnchItemId then
        ...
        if V.lastEnchItemIsScroll and V.lastEnchItemName and not V.currentUiMode then
            recordSpellCast(V.lastEnchItemName, "item:" .. tostring(V.lastEnchItemId or V.lastEnchItemName))
        end
...
V.creatureTopCache = nil
V.creatureTypeTopCache = nil
V.creatureTopCacheDirty = true
V.creatureTypeTopCacheDirty = true
V.creatureKillsSaveDirty = false
V.creatureTypesSaveDirty = false
...
local function flushCreatureKillsCache()
    if not V.creatureKillsSaveDirty or not profileSection then return end
    profileSection:set("creatureKillsStr", serializeCountMap(V.creatureKillsCache))
    V.creatureKillsSaveDirty = false
end

local function saveCreatureKillsCache()
    V.creatureKillsSaveDirty = true
end

local function flushCreatureTypesCache()
    if not V.creatureTypesSaveDirty or not profileSection then return end
    profileSection:set("creatureTypesStr", serializeCountMap(V.creatureTypesCache))
    V.creatureTypesSaveDirty = false
end

local function saveCreatureTypesCache()
    V.creatureTypesSaveDirty = true
end
...
local function getTopCreatureKills(limit)
    if V.creatureTopCacheDirty or not V.creatureTopCache then
        V.creatureTopCache = buildSortedCountList(V.creatureKillsCache)
        V.creatureTopCacheDirty = false
    end
    return copySortedSlice(V.creatureTopCache, limit or 10)
end

-- Get creature kills grouped by category (Undead, Daedra, etc.)
local function getCreatureKillsByType()
    if V.creatureTypeTopCacheDirty or not V.creatureTypeTopCache then
        V.creatureTypeTopCache = buildSortedCountList(V.creatureTypesCache)
        V.creatureTypeTopCacheDirty = false
    end
    return copySortedSlice(V.creatureTypeTopCache)
end

local flushSlavesFreedCache

local function flushDeferredSerializedCaches()
    flushWeaponTallyCache()
    flushSpellTallyCache()
    flushCreatureKillsCache()
    flushCreatureTypesCache()
    flushPeopleMetCache()
    flushSlavesFreedCache()
    flushArtifactsSeenCache()
    flushDiseasesSeenCache()
    flushSpellEffectsSeenCache()
end
...
local function getChargenLuck(luckAttr)
    local chargenLuck = profileSection:get("chargenLuck")
    if not chargenLuck then
        chargenLuck = luckAttr.base
        profileSection:set("chargenLuck", chargenLuck)
    end
    return chargenLuck
end

local function showLuckModifierNotification(current, target, showNotification)
    if not showNotification or not mS.healthStorage:get("showLuckChangeNotification") then return end
...
local function applyLuckModifier(showNotification)
    ...
    local chargenLuck = getChargenLuck(luckAttr)
    local deaths = get("deathCount")
    local forgivenDeaths = 0
    if modifier < 0 then
        forgivenDeaths = math.min(get("luckPenaltyForgivenDeaths"), deaths)
    end
    local penalizedDeaths = math.max(0, deaths - forgivenDeaths)
    local target = math.floor(chargenLuck + penalizedDeaths * modifier)
    target = math.max(0, target)
    if current == target then return end
    luckAttr.base = target
    showLuckModifierNotification(current, target, showNotification)
end

local function resetDeathLuckPenalty(showNotification)
    if not state.profileId then return end
    local modifier = mS.healthStorage:get("luckModifierPerDeath") or 0
    local deaths = get("deathCount")
    if modifier >= 0 or deaths <= 0 then return end

    local luckAttr = T.Actor.stats.attributes.luck(self)
    local current  = luckAttr.base
    local chargenLuck = getChargenLuck(luckAttr)
    local target = math.max(0, math.floor(chargenLuck))

    -- Forgive all currently recorded deaths for penalty purposes. Future
    -- deaths still apply the configured penalty from this restored baseline.
    set("luckPenaltyForgivenDeaths", deaths)
...
local function registerSkillHandlers()
    ...
    -- Spell/item casts: activeSpells only sees effects that land on the
    -- player. Successful SkillProgression events see normal vanilla target
    -- and touch spells, while the short dedupe window prevents double-counting
    -- self effects also seen by activeSpells. Enchanted item use is included
    -- here as a fallback for vanilla item casts whose charge updates are not
    -- visible to player-side polling in every OpenMW/mod combination.
    I.SkillProgression.addSkillUsedHandler(function(skillId, params)
        if (skillId == "alteration" or skillId == "conjuration"
            or skillId == "destruction" or skillId == "illusion"
            or skillId == "mysticism" or skillId == "restoration")
           and V.isSpellcastSkillUse(params) then
            V.recordSelectedSpellCast("skill:" .. tostring(skillId))
        elseif skillId == "enchant" and V.isEnchantItemUse(params) then
            V.recordSelectedEnchantedItemUse("skill:enchant")
        end
    end)
end
...
-- Vanilla/Tamriel-style slaves remain classed as "Slave" after being freed.
-- The actual liberated state is normally script-local slaveStatus == 3,
-- set by the "go free" dialogue result. Therefore this counter uses the
-- DialogueResponse event and inspects the selected dialogue info's result
-- script instead of comparing nearby slave counts or NPC class changes.
-- ============================================================

V.slaveDialogueSnapshot = nil -- legacy save compatibility; no longer used
...
local function isSlaveNpc(obj)
    if not T.NPC.objectIsInstance(obj) then return false end
    local ok, rec = pcall(function() return T.NPC.record(obj) end)
    if ok and rec and rec.class then
        return string.lower(tostring(rec.class)) == "slave"
    end
    return false
end

local function loadSlavesFreedCache()
    if not state.profileId then return end
    V.slavesFreedCache = {}
    V.slavesFreedSaveDirty = false
    local raw = profileSection:get("slavesFreedStr") or ""
    for id in raw:gmatch("[^\n]+") do
        if id ~= "" then V.slavesFreedCache[id] = true end
    end
end

flushSlavesFreedCache = function()
    if not V.slavesFreedSaveDirty or not profileSection then return end
    profileSection:set("slavesFreedStr", serializeSetMap(V.slavesFreedCache))
    V.slavesFreedSaveDirty = false
end

local function recordSlaveFreed(actor)
    if not actor then return false end
    local key = getObjectKey(actor)
    if not key or key == "" or V.slavesFreedCache[key] then return false end
    V.slavesFreedCache[key] = true
    V.slavesFreedSaveDirty = true
    increment("slavesFreed")
    return true
end

local function getDialogueInfo(eventData)
    if not eventData or not eventData.type or not eventData.recordId or not eventData.infoId then return nil end
    local store = core.dialogue and core.dialogue[eventData.type]
    if not store or not store.records then return nil end

    local ok, record = pcall(function() return store.records[eventData.recordId] end)
    if not ok or not record or not record.infos then return nil end

    local infoId = tostring(eventData.infoId)
    for _, info in pairs(record.infos) do
        if tostring(info.id) == infoId then return info end
    end
    return nil
end

local function resultScriptFreesSlave(resultScript)
    if type(resultScript) ~= "string" then return false end
    local script = string.lower(resultScript)

    -- Vanilla/Tamriel-style freeing: set slaveStatus to 3.
    -- Keep the pattern whitespace-tolerant so it survives formatting differences.
    if script:find("set%s+slavestatus%s+to%s+3") then return true end
    if script:find("set%s+slave_status%s+to%s+3") then return true end
    if script:find("set%s+slave state%s+to%s+3") then return true end
    if script:find("set%s+slavestate%s+to%s+3") then return true end

    return false
end

local function dialogueResponseFreesSlave(eventData)
    if not eventData or eventData.type ~= "topic" then return false end
    local topicId = string.lower(tostring(eventData.recordId or ""))
    if topicId ~= "go free" then return false end
    if not eventData.actor or not isSlaveNpc(eventData.actor) then return false end

    local info = getDialogueInfo(eventData)
    return info ~= nil and resultScriptFreesSlave(info.resultScript)
end

local function pollSlavesFreed()
    -- Intentionally inert. Older versions used nearby slave-count deltas, which
    -- caused false positives when leaving a cell. Actual freeing is counted in
    -- onDialogueResponse, where the selected dialogue result is visible.
end
...
local function onUiModeChanged(data)
    ...
    if data.newMode ~= data.oldMode then
        if mS.healthStorage:get("resetDeathLuckPenalty") == "done" then
            mS.healthStorage:set("resetDeathLuckPenalty", false)
        end
...
local function flushTimedSerializedCaches()
    flushDeferredSerializedCaches()
end
...
local pollTasks = {
    ...
    { name = "serializedCaches",     interval = POLL_INTERVAL,              acc = -1.95, fn = flushTimedSerializedCaches },
...
local function resetPollTaskAccumulators()
    for _, task in ipairs(pollTasks) do
        ...
        elseif task.name == "serializedCaches" then task.acc = -1.95
...
local function pollSettingsCascade()
    ...
    if state.profileId then
        if mS.healthStorage:get("resetDeathLuckPenalty") == true then
            resetDeathLuckPenalty(true)
            pcall(function() mSummaryUi.update(state, 999) end)
            mS.healthStorage:set("resetDeathLuckPenalty", "done")
        end
...
local function doInit(profileSnapshot)
    V.playSecondsBuffer = 0
    resetPersonalRecordBuffer()
    V.unlockPollAccumulator = 0
    initProfileId()
...
local function onLoad(data)
    ...
    doInit(data and data.profileSnapshot)
    sessionSection:set("playthroughId", state.playthroughId)
end
...
local function onSave()
    ...
    saveState.profileSnapshot = buildProfileSnapshot()
    return saveState
end
...
local function onDialogueResponse(e)
    ...
    -- Slaves Freed: count only the actual liberation dialogue result.
    -- Class remains "Slave" after freeing, so class transitions are not reliable.
    if dialogueResponseFreesSlave(e) then
        recordSlaveFreed(e.actor)
    end
...
-- ============================================================
-- MOD INTEGRATION: CREATURE MERCY AND CONTAGION
-- CMC sends CMC_EnumeratioCounter to the player after successful
-- cure/spread transformations. Unknown counter names are ignored
-- so the integration is safe across CMC versions.
-- ============================================================

local CMC_COUNTER_KEYS = {
    diseasedCreaturesCured = "cmcDiseasedCreaturesCured",
    blightedCreaturesCured = "cmcBlightedCreaturesCured",
    diseasesSpread = "cmcDiseasesSpread",
    blightsSpread = "cmcBlightsSpread",
}

local function onCmcEnumeratioCounter(data)
    if not state.profileId then return end
    local key = data and CMC_COUNTER_KEYS[data.counter]
...
return {
    ...
    eventHandlers = {
        ...
        CMC_EnumeratioCounter = onCmcEnumeratioCounter,  -- Creature Mercy and Contagion integration
        SC_MagExpCastRequest = V.onMagExpCastRequest,  -- Spell Framework Plus / OSSC spell/enchantment cast tracking
