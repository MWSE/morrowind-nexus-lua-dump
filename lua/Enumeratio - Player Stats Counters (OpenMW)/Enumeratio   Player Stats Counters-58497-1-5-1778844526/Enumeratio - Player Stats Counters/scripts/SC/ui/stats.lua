...
local profileSectionRef = nil

local function get(profileId, key)
    if profileId and profileId == profileIdRef and profileSectionRef then
        return profileSectionRef:get(key) or 0
    end
...
local function flushSection(sectionId)
    ...
    elseif sortMode == "sortCounters_highest" then
        for _, entry in ipairs(queue) do
            entry.sortValue = entry.sortValueFn and entry.sortValueFn() or 0
        end
        table.sort(queue, function(a, b)
            local va = a.sortValue or 0
            local vb = b.sortValue or 0
...
module.setStatsWindow = function(state)
    ...
    profileSectionRef = profileIdRef and storage.playerSection(profileIdRef) or nil
    ...
    queueLine("SC_COMBAT", "KD_RATIO", {
        ...
        value = function()
            ...
            if ratio == math.huge then return { string = "Inf." } end
    ...
    queueLine("SC_COMBAT", "WEAPONS_USED", {
        ...
        labelColor = C.Colors.DEFAULT,
        value = function()
            if not state.getTopWeapons then return { string = "N/A" } end
    ...
    addLine("SC_MAGIC", "CMC_DISEASED_CREATURES_CURED", "statCmcDiseasedCuredTitle", "statCmcDiseasedCuredDesc", function()
        return { string = fmtNum(stat("cmcDiseasedCreaturesCured")) }
    end, "showCmcDiseasedCreaturesCured", "cmcDiseasedCreaturesCured")

    addLine("SC_MAGIC", "CMC_BLIGHTED_CREATURES_CURED", "statCmcBlightedCuredTitle", "statCmcBlightedCuredDesc", function()
        return { string = fmtNum(stat("cmcBlightedCreaturesCured")) }
    end, "showCmcBlightedCreaturesCured", "cmcBlightedCreaturesCured")

    addLine("SC_MAGIC", "CMC_DISEASES_SPREAD", "statCmcDiseasesSpreadTitle", "statCmcDiseasesSpreadDesc", function()
        return { string = fmtNum(stat("cmcDiseasesSpread")) }
    end, "showCmcDiseasesSpread", "cmcDiseasesSpread")

    addLine("SC_MAGIC", "CMC_BLIGHTS_SPREAD", "statCmcBlightsSpreadTitle", "statCmcBlightsSpreadDesc", function()
        return { string = fmtNum(stat("cmcBlightsSpread")) }
    end, "showCmcBlightsSpread", "cmcBlightsSpread")
    ...
    queueLine("SC_MAGIC", "SPELLS_CAST", {
        ...
        labelColor = C.Colors.DEFAULT,
        value = function()
            if not state.getTopSpells then return { string = "N/A" } end
