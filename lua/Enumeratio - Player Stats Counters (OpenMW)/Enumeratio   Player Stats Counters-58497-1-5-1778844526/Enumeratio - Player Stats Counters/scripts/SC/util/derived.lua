...
local cachedProfileId = nil
local cachedSection = nil

local function getSection(profileId)
    if profileId ~= cachedProfileId or not cachedSection then
        cachedProfileId = profileId
        cachedSection = storage.playerSection(profileId)
    end
    return cachedSection
end

local function get(profileId, key)
    return getSection(profileId):get(key) or 0
end
