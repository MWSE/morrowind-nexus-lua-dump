...
local function onBookOpened(data)
    ...
    text_parser.applyBookObjectTextToWindow(data.activatedBookObject, documentWindow, chosenDocumentWindowStyle, data.replacedBookText)
...
return {
    ...
    eventHandlers = {
        ...
        UiModeChanged = function(data)
            if data.newMode == I.UI.MODE.Book or data.newMode == I.UI.MODE.Scroll then
                if data.arg ~= nil then
                    core.sendGlobalEvent(
                        "openmwBooksEnhancedBookTakenCheckItForVariables",
                        { player = self, activatedBookObject = data.arg })
                end
            end
        end
    },
