...
function TP.applyBookObjectTextToWindow(activatedBookObject, documentWindow, documentWindowData, replacedBookText)
    ...
    local modifiedBookRecord = record_text_modifier.tryToReplaceWithFilledValues(replacedBookText)
    if modifiedBookRecord then
        modifiedBookRecord = record_text_modifier.overwriteHtmlTags(modifiedBookRecord)
    else
        modifiedBookRecord = record_text_modifier.overwriteHtmlTags(bookRecord)
    end
