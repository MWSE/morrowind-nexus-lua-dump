...
local Tweener = {
    ...
    add = function(self, duration, easingFunc, updateFunc, callback)
        local anim = {
            duration = duration,
        ...
        if callback then
            self.callbacks[#self.animations] = callback
        end
    ...
    tick = function(self, dt)
        ...
        if t >= 1 then
            -- Execute callback if it exists for this animation
