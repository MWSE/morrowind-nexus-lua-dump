local log = require("logging.logger").new({ name = "OotG Sixth House Quest", logLevel = "INFO" })
local sixthHouseQuestID = "GG_6thHouse"
local bouncerForceGreetInfoID = "2020366741859230377"
local dagothID = "GG_6_ash_ghoul_nasro"
local troopers = { "GG_alynu_menas", "GG_dartis_iba", "GG_dralane_murith", "GG_tennus_dolovas" }
...
local function bleedingSkullTooltip(e)
	if e.object.id == skullID then
		...
		local label = (block:createLabel{
			...
			text = "Примет один смертельный удар вместо носителя",
		})
		label.wrapText = true
	end
end
...
local function onSpellResistBleedingSkullEffect(e)
	if tes3.getItemCount({ reference = tes3.player, item = skullID }) > 0 and e.target == tes3.player and e.effect and
	isDamageHealthEffect[e.effect.object.id] then
		log:debug("spell effect %s max magnitude is %s", e.effect.object.name, e.effect.max)
		if e.effect.max >= tes3.player.mobile.health.current then
			log:debug("the max damage the player will take will kill the player!")
			e.resistedPercent = 100
			timer.delayOneFrame(function()
				if tes3.getItemCount({ reference = tes3.player, item = skullID }) > 0 then
					tes3.removeItem({ reference = tes3.player, item = skullID, playSound = false })
					tes3.playSound({ sound = "restoration hit" })
					tes3.messageBox({ message = "Кровоточащий череп принял удар на себя." })
				end
			end, timer.simulate)
		end
	end
...
local function onDamageBleedingSkullEffect(e)
	if tes3.getItemCount({ reference = tes3.player, item = skullID }) > 0 then
		if e.reference == tes3.player then
			if e.damage then
				log:debug("player is taking this amount of damage: %s!", e.damage)
			end
			if e.source then
				log:debug("player is taking damage from source: %s!", e.source)
			end
			if e.damage >= tes3.player.mobile.health.current then
				log:debug("the damage the player will take will kill the player!")
				tes3.removeItem({ reference = tes3.player, item = skullID, playSound = false })
				tes3.playSound({ sound = "restoration hit" })
				tes3.messageBox({ message = "Кровоточащий череп принял удар на себя." })
				return false
...
local function trooperForceGreetPlayer()
	...
	if tes3.player.data.OotG.trooperGreeted then
		return
	end
	if not dagothRef.mobile.isDead then
		return
	end
	for _, trooperID in pairs(troopers) do
		...
		if not trooperRef.mobile.inCombat then
			...
			if trooperRef.mobile.playerDistance < 256 then
				...
				if trooperRef.mobile.position:distance(dagothRef.mobile.position) >= 256 then
					log:debug("%s is far away enough from dagoth nasro", trooperID)
					if tes3.testLineOfSight({ reference1 = trooperRef, reference2 = tes3.player }) then
						local wasShown = tes3.showDialogueMenu({ reference = trooperRef })
						if wasShown then
	...
	if e.info.id == bouncerForceGreetInfoID then
		if tes3.player.cell.id == "Ossuary of Ayem, Bone Pit" then
			if tes3.player.data.OotG.playerEnteredFromWesternCatacombs then
				e.text = "Эй, ты не один из Призраков. Как ты сюда попал?"
			end
		end
	end
end
...
local function checkIfEnterFromWesternCatacombs(e)
	if e.cell.id == "Ossuary of Ayem, Bone Pit" then
		if tes3.getJournalIndex { id = sixthHouseQuestID } >= 10 then
			if e.previousCell.id == "Ghostfence, Western Catacombs" then
