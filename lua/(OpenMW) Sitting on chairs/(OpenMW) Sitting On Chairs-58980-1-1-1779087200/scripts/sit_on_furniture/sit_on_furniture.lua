...
local function getFurnitureAhead()
    ...
    for _, ray in ipairs(FLAT_RAYS) do
        ...
        local obj  = castPiercingRay(from, from + flatDir * RAY_DISTANCE)
    ...
    for _, ray in ipairs(PITCHED_RAYS) do
        ...
        local obj = castPiercingRay(origin, origin + dir * RAY_DISTANCE)
        if obj then return obj end
    end
...
local function onFrame(dt)
    ...
    if eDown and not eWasDown then
        if isSitting then
            ...
            if furniture then trySit(furniture) end
        end
    end
    eWasDown = eDown
end
