...
local BURDEN_SPELLS_HARDCORE = data.BURDEN_SPELLS_HARDCORE
...
local function activeBurdenSpells()
    return C.BURDEN_HARDCORE and BURDEN_SPELLS_HARDCORE or BURDEN_SPELLS
end
...
local function getBurdenSpell(item, slot)
    ...
    if not prefix then return nil end
    local spells  = activeBurdenSpells()
    return evalItem(item, slot, spells[prefix], false, skillId, prefix)
end
...
local function getBurdenSpellBound(item, slot)
    ...
    local prefix  = getClassPrefix(skillId or "")
    local spells  = activeBurdenSpells()
    return evalItem(item, slot, spells.BOUND, true, skillId, prefix)
end

local function checkArmorBurden()
...
-- UNEQUIP MODE: queue + check
local unequipQueue = {}

local function checkArmorItem(item)
    ...
    table.insert(unequipQueue, item)
...
local function checkBoundArmorUnequip()
...
local function checkAllArmorUnequip()
    if not C.MOD_ENABLED then return end
    local eq = Actor.getEquipment(self.object)
...
-- DISPATCH: pick the active mode
local function checkArmor()
    if C.BURDEN_ENABLED then
        checkArmorBurden()
    else
        clearAllBurdens()
        checkAllArmorUnequip()
    end
end
...
local function onFrame(dt)
    ...
    if not C.BURDEN_ENABLED then
        if #unequipQueue > 0 then
            local itemToDrop = table.remove(unequipQueue, 1)
            self.object:sendEvent('Unequip', { item = itemToDrop })
        end
    end
...
local function UiModeChanged(data)
    if not C.MOD_ENABLED then return end
    if (data.oldMode == "Interface" or data.oldMode == "Book" or data.oldMode == "Scroll") and data.newMode == nil then
        checkArmor()
    end
end
