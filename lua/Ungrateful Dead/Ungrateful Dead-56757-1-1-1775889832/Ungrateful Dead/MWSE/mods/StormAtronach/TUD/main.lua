...
local log = mwse.Logger.new({ name = config.name, level = config.logLevel })

---Utility functions
local function getRandomWisdom()
	if not wisdoms or #wisdoms == 0 then
		log:error("No wisdoms available.")
		return nil
	end
...
local function applyEffects(effects)
	...
	for _, effect in pairs(effects) do
		...
		log:debug("Applying effect: id=%s min=%s max=%s duration=%s", tostring(effect.id), tostring(effect.min),
		          tostring(effect.max), tostring(effect.duration))
	end

	tes3.applyMagicSource { reference = tes3.player, bypassResistances = true, name = "Ancestral Wisdom",
                         effects = effects }
...
local function ancestralWisdom(e)
	if not config.enabled then
		log:info("Mod is disabled; skipping ancestralWisdom.")
		return
	end
	-- Check if it the caster is the player
	if e.caster ~= tes3.player then
		log:debug("Caster is not player; skipping.")
		return
	end
	-- Check if it is a power (or spells are allowed)
	if config.powersOnly and not e.source.isPower then
		log:debug("Source is not a power; skipping.")
		return
	end
	...
	if not hasAncestralGhost then
		log:debug("Spell does not have summonAncestralGhost effect; skipping.")
		return
	end
	...
	log:info("Displayed wisdom: %s", wisdom.saying)
