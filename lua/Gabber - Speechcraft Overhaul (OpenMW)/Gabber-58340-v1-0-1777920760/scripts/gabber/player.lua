...
local function modDisposition(data)
  ...
  if not factionOps[faction] and not altFactions[faction] then
    faction = "none"
  end
  ...
  if preMult > 1 and key ~= "gold" then
    I.SkillProgression.skillUsed("speechcraft", {skillGain = preMult, useType = 1}) 
  end
...
local function handleResponse(data)
  ...
  if not (id == "idle") and not (id == "hello") and not string.find(id, "greeting") and not uiOpen then
    destroyUI()
    if id == "- talk" then
      multiplier = 1
      local speechcraft = math.min(100, types.NPC.stats.skills.speechcraft(self).modified)
      local fatigueUse = (110 - speechcraft) * settings:get("fatigueMult")
      types.Actor.stats.dynamic.fatigue(self).current = math.max(0, types.Actor.stats.dynamic.fatigue(self).current - fatigueUse)
