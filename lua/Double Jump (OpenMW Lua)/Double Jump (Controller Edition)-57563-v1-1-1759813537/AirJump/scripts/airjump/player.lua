-- AirJump (PLAYER) – mid-air pad + buffered jump triggered by the vanilla Jump action.
-- OpenMW 0.49/0.50 API.
...
-- Timing windows
local TRACK_PAD_FRAMES_MAX         = 16        -- follow feet while waiting for contact
...
-- Robust read of *vanilla Jump* with fallback to deprecated enum path.
-- Docs: getBooleanActionValue(key) for actions; ACTION.Jump with isActionPressed(actionId) (deprecated). 
local function readVanillaJump()
  -- If the 'Jump' action exists as a Boolean action, use it…
  if input.actions and input.actions.Jump then
    return input.getBooleanActionValue('Jump') or false
  end
  -- …otherwise fall back to legacy enum
  return input.isActionPressed(input.ACTION.Jump) or false
end
...
return {
  engineHandlers = {
    onFrame = function(dt)
      -- Use vanilla Jump (rising edge)
      local down        = readVanillaJump()
