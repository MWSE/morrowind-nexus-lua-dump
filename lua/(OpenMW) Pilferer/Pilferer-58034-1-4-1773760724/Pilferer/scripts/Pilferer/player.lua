...
local function createEyeHud()
    ...
    hudEye = ui.create({
        ...
        props = {
            anchor = v2(0, 0),
            position = pos,
            alpha = 0--saveData.isDetected and 0.9 or 0
        },
        content = ui.content {},
        userData = {
...
async:newUnsavableSimulationTimer(0.1, function()
    sneakIsGoodNow = I.SneakIsGoodNow
end)
...
local function onUpdate()
    ...
    if sneakIsGoodNow and not saveData.isDetected then
		local detectorName
		for _, ast in pairs(sneakIsGoodNow.observerActorStatuses) do
			if (ast.progress or 0) >= 1.0 then
				local rec = ast.actor.type.record(ast.actor)
				detectorName = rec and rec.name or ast.actor.recordId
				break
			end
		end
		if detectorName then
			saveData.isDetected = true
			updateEyeVisibility()
			core.sendGlobalEvent("Pilferer_sneakDetected", {name = detectorName, player = self.object})
		end
    end
end
