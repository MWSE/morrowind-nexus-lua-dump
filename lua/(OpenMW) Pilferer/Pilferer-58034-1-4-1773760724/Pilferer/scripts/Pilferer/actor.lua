...
local function getNearestPlayer()
    ...
    for _, player in ipairs(nearby.players) do
        ...
        if dist <= GREETING_DISTANCE and dist < bestDist then
            bestPlayer = player
            bestDist = dist
        end
    end
    return bestPlayer or nearby.players[1]
end

local function checkGreetingBehavior(now)
    local player = greetingTarget
    if not player or not player:isValid() then return false, "no player" end
...
local function onUpdate()
    ...
    if isSaying and not wasSaying then
        ...
        if saveData.helloBoosted then
            resetGreetingState()
            greetingTarget = getNearestPlayer()
            if not greetingTarget then
                resetGreetingState()
            else
                -- Only mute if far away
                local distance = (greetingTarget.position - self.position):length()
    ...
    if greetingCheckPending then
        ...
        if elapsed <= GREETING_CHECK_DURATION then
            ...
            if result == true then
                ...
                core.sendGlobalEvent("Pilferer_greetingDetected", {name = name, player = greetingTarget})
                resetGreetingState()
