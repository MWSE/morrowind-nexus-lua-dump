...
event.register("modConfigReady", function()
	...
	config  = require("NecroCraft.config")
...
local function onCellChanged(e)
	...
	for ref in e.cell:iterateReferences(tes3.objectType.book) do
		if ref.id == "sc_summonskeletalservant" then 
			ref:disable()
			local books = {"nc_bk_skeleton_cr", "nc_bk_bonespider", "nc_bk_corpse1"}
			local book = books[math.random(#books)]
			book = tes3.createReference{object = book, position = ref.position, orientation=ref.orientation, cell=e.cell}
	...
	if not string.find(e.cell.id, " Temple") and not string.find(e.cell.id, " Tomb") then return end
	local ashpit
	local ashpits = {
	...
	for ref in e.cell:iterateReferences(tes3.objectType.static) do
		...
		if ashpit then
			ashpit = tes3.createReference{object = ashpit, position = ref.position, orientation=ref.orientation, cell=e.cell}
			tes3.setOwner({
				reference = ashpit,
	...
	if count == 3 then
		...
		local yesButton = {	text = strings.yes, 
							callback = function()
										...
										skillStatus = "active"
										tes3.findGlobal("NC_CorpsePreparation").value = 1
...
local function onDamage(e)
	...
	if e.mobile.health.current - math.abs(e.damage) <= 1 then
		...
		-- if tes3.isAffectedBy{reference = e.reference, effect = tes3.effect.blackSoulTrap} then
		-- 	local soul = e.reference.object.baseObject
		-- 	if soul.objectType == tes3.objectType.npc or soulGemLib.blackSoulCreature[soul.id] then
		-- 		soulGemLib.captureSoul{reference = tes3.player, gem = "Misc_SoulGem_Black", soul = soul, position=e.reference.position, text=true}
		-- 		tes3.removeEffects{reference = e.reference, effect = tes3.effect.blackSoulTrap}
		-- 		if e.reference.object.baseObject.id == "worm lord" then
		-- 			e.reference.object.baseObject.isRespawn = false
		-- 		end
		-- 	end
		-- end
	end
...
local function onSpellTick(e)
	...
	if not config.editSummonUndeadEffects then return end
