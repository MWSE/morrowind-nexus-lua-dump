local mod = { name = "Infectious Egg Mines", ver = "1.0" }
local cf = require("AU.Infectious Egg Mines.config")
...
local function getCells()
	...
	for _, cell in pairs(tes3.dataHandler.nonDynamicData.cells) do
		...
		if string.find(cell.id:lower(), "egg mine") then
...
local function registerModConfig()
	...
	subcat:createSlider{
		label = "Chance: " .. "%s%%",
		description = "Here you can choose the odds to get a disease by wandering the egg mines. [Default:30]",
		min = 0,
		max = 100,
		step = 1,
		jump = 10,
	...
	template:createExclusionsPage{
		...
		description = "Here you can choose which disease you can contract by wandering the egg mines.",
