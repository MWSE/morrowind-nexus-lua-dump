...
local function addPAFlask(e)
  ...
  if (e.newlyCreated) then
	...
	companionShareButton:registerBefore(tes3.uiEvent.mouseClick, function(j)
    local companion = tes3ui.getServiceActor()
    local count = tes3.getItemCount({reference=companion.reference, item="misc_pa_flask"})
    if count == 0 then
        tes3.addItem({ reference=companion.reference, item="misc_pa_flask", count=1 })
    elseif count > 1 then
        tes3.removeItem({ reference=companion.reference, item="misc_pa_flask", count=(count-1) })
    end
	end)
end
event.register(tes3.event.initialized, function()
    ...
    event.register(tes3.event.uiActivated, addPAFlask, { filter = "MenuDialog" })
end)

---@param e deathEventData
event.register(tes3.event.death, function(e)
    local isPotentialCompanion = (e.reference and e.reference.context and e.reference.context["companion"]) and true or false
    if not isPotentialCompanion then return end
    
    local count = tes3.getItemCount({reference=e.reference, item="misc_pa_flask"})
    if count > 0 then
      tes3.removeItem({ reference=e.reference, item="misc_pa_flask", count=(count) })
    end
end)
...
local function addPotionTooltip(e)
    ...
    for _, tmpPotion in ipairs(shared.tmpPotions) do
      local block = parent:createBlock()
      block.flowDirection = "left_to_right"
      block.widthProportional = 1.0
      block.autoWidth = true
      ...
      local icon = block:createImage({ path = string.format("icons/%s", tmpPotion.icon) })
      icon.borderRight = 6

      block:createLabel({ text = tmpPotion.name })

      local timeLeft = tmpPotion.duration
...
event.register(tes3.event.uiObjectTooltip, addPotionTooltip, { priority = -70 }) -- After UI Expansion
