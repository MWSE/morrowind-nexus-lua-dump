...
function this.administerPotionInteractive(reference, potion, potionData)
    ...
    -- record temp potion for immediate display
    --this.tmpPotions[#this.tmpPotions] = { name = potion.name, icon = potion.icon, duration = 1 }
    table.insert(this.tmpPotions, { name = potion.name, icon = potion.effects[1].object.icon, duration = this.getLongestPotionDuration(potion) })
