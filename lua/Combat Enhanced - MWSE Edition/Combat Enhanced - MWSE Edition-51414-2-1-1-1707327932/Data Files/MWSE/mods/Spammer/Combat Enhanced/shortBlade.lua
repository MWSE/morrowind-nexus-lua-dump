...
function list.damage(type, target)
    ...
    if type == tes3.physicalAttackType.thrust then
        ...
        if mod.puncture == 3 then
            tes3.playSound({
                ...
                reference = target,
                soundPath =
                "MWE\\MWE_ShortBladePuncture.wav"
            })
            if cf.mb then
            ...
            mod.puncture = 0
        elseif mod.puncture >= 1 then
            mod.puncture = mod.puncture + 1
        end
        if mod.stab == 4 and (target.mobile.health.normalized <= (cf.deadlyHit / 100)) then
            tes3.playSound({
                mixChannel = tes3.soundMix.master,
                reference = target,
                soundPath =
                "MWE\\MWE_ShortBladeStab.wav"
            })
            if cf.mb then
    ...
    elseif type == tes3.physicalAttackType.slash then
        if mod.throat == 2 then
            tes3.setStatistic({ reference = target, name = "magicka", value = (0 - (2 * skill)) })
            tes3.applyMagicSource { reference = target, name = "Silence", target = target, bypassResistances = true, effects = {
                {
            ...
            tes3.playSound({
                ...
                reference = target,
                soundPath =
                "MWE\\MWE_ShortBladeThrtSlsh.wav"
            })
            tes3.playSound({
                mixChannel = tes3.soundMix.voice,
                reference = target,
                soundPath =
                "Vo\\MWE\\MWE_ChokeSound.wav"
            })
            ...
            if target.object.objectType == tes3.objectType.npc then
                for _, layer in pairs(tes3.activeBodyPartLayer) do
                    ...
                    if activePart and activePart.node then
                        ...
                        if activePart.bodyPart and (layer ~= tes3.activeBodyPartLayer.base) then
                            local glove = (tes3.getEquippedItem { actor = target, objectType = tes3.objectType.armor, slot = tes3.armorSlot.leftGauntlet }) or
                                (tes3.getEquippedItem { actor = target, objectType = tes3.objectType.clothing, slot = tes3.clothingSlot.leftGlove })
                            if glove then
...
Silences the enemy.]],
    slice = [[Slice
Thrust, Thrust, Slash
Chops off the enemy's hand.]],
    dice = [[Dice
Chop, Thrust, Slash, Slash
Increases your weaponspeed for a few seconds.]],
    puncture = [[Puncture
Chop, Thrust, Thrust, Thrust
Punctures a vital organ, causing continuous damage.]],
    stab = [[Stab
Slash, Chop, Slash, Slash, Thrust
Finishing move with a strike straight through the heart.]]
}
return list
