...
function list.damage(type, target)
    ...
    if type == tes3.physicalAttackType.thrust then
        ...
        if mod.bbreak == 3 then
            if target.readiedWeapon and target.readiedWeapon.itemData then
                ...
                tes3.playSound({
                    ...
                    reference = target,
                    soundPath =
                    "MWE\\MWE_LongBladeBladeBreak.wav"
                })
                if cf.mb then
        ...
        if mod.shock == 2 then
            ...
            tes3.playSound({
                ...
                reference = target,
                soundPath =
                "MWE\\MWE_LongBladeShockwave.wav"
            })
            if cf.mb then
    ...
    elseif type == tes3.physicalAttackType.chop then
        ...
        elseif mod.dismember == 4 then
            if (target.object.objectType == tes3.objectType.npc) and (target.mobile.health.normalized <= (cf.deadlyHit / 100)) then
                for _, layer in pairs(tes3.activeBodyPartLayer) do
                    ...
                    if activePart and activePart.node then
                        ...
                        if activePart.bodyPart and (layer == tes3.activeBodyPartLayer.base) then
                            local head = tes3.createObject({
                                objectType = tes3.objectType.miscItem,
                                getIfExists = false,
                                mesh =
                                    activePart.bodyPart.mesh,
                                name = "Beheaded Head",
                                icon = "MWE\\MWE_BeheadIcon.tga",
                                weight = 2
                            })
            ...
            tes3.playSound({
                ...
                reference = target,
                soundPath =
                "MWE\\MWE_LongBladeDismember.wav"
            })
            mod.dismember = 0
        else
            mod.dismember = 0
    ...
    elseif type == tes3.physicalAttackType.slash then
        ...
        if mod.disarm == 3 then
            if target.readiedWeapon and target.readiedWeapon.object then
                tes3.dropItem({
                    reference = target,
                    item = target.readiedWeapon.object,
                    itemData = target.readiedWeapon
                        .itemData or nil
                })
                tes3.playSound({
                    mixChannel = tes3.soundMix.master,
...
list.def = {
    cut = [[Cut
Thrust, Thrust, Slash
A deep cut that saps the opponent's strength.]],
    shock = [[ShockWave
Slash, Slash, Thrust
Powerful thrust that knocks down the enemy.]],
    bbreak = [[BladeBreak
Slash, Chop, Chop, Thrust
Breaks your enemy's weapon.]],
    disarm = [[Disarm
Thrust, Chop, Chop, Slash
Knocks opponent's weapon out of their hand.]],
    dismember = [[Decapitate
Thrust, Thrust, Chop, Slash, Chop
Finishing move that takes off the opponent's head.]]
}
return list
