...
function list.damage(type, target)
    ...
    if type == tes3.physicalAttackType.thrust then
        ...
        if mod.offb == 2 then
            ...
            tes3.playSound({mixChannel = tes3.soundMix.master,  reference = target, soundPath = "MWE\\MWE_SpearOffBalance.wav"})
            if cf.mb then
        ...
        if mod.quik == 3 then
            ...
            timer.start({duration = cf.slider, iterations = 1, callback = function ()
                tes3.mobilePlayer.readiedWeapon.object.speed = tes3.mobilePlayer.readiedWeapon.object.speed/5
            end})
            tes3.playSound({mixChannel = tes3.soundMix.master,  reference = target, soundPath = "MWE\\MWE_SpearQuickthrust.wav"})
            if cf.mb then
        ...
        if mod.impale == 4 then
            mod.impale = 0
            tes3.setStatistic({reference = target, name = "health", current = 0})
            tes3.playSound({mixChannel = tes3.soundMix.master,  reference = target, soundPath = "MWE\\MWE_SpearImpale.wav"})
            if cf.mb then
                tes3.messageBox{message = table.choice(message.Impale)}
            end
        elseif mod.impale == 3 or mod.impale == 2 then mod.impale = mod.impale+1 else mod.impale = 0 end
    ...
    elseif type == tes3.physicalAttackType.slash then
        ...
        if mod.sweep == 3 then
            ...
            mod.sweep = 0
        elseif mod.sweep == 2 then mod.sweep = mod.sweep+1 else mod.sweep = 0 end
    end
    if myTimer then
        myTimer:reset()
    else
...
list.def = {gore = [[Gore
Sequence: Thrust, Slash, Thrust
Description: A strong thrust to the opponent's hip that slows them and makes them fall down more often.]],
            offb = [[Hamp Hit
Sequence: Chop, Chop, Thrust
Description: A thrust to the stomach that knocks the wind out of your opponent.]],
            sweep = [[Sweep
Sequence: Chop, Thrust, Slash, Slash
Description: Damages and knocks the enemy down.]],
            quik = [[QuickThrust
Sequence: Slash, Thrust, Thrust, Thrust
Description: Increases weapon speed for a few seconds as long as you keep thrusting.]],
            impale = [[Impale
Sequence: Slash, Chop, Thrust Thrust, Thrust
Description: Finishing move with a spear through the opponent's body.]]}
return list