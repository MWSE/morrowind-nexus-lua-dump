local message = {
	...
	Slam = {
		"All tremble around your powerful Slam.",
...
function list.damage(type, target)
    ...
    if type == tes3.physicalAttackType.thrust then
        ...
        if mod.knock == 2 then
            ...
            tes3.playSound({mixChannel = tes3.soundMix.master, reference = target, soundPath = "MWE\\MWE_BluntKnockdown.wav"})
            if cf.mb then
    ...
    elseif type == tes3.physicalAttackType.chop then
        ...
        if mod.slam == 3 then
            ...
            tes3.playSound({mixChannel = tes3.soundMix.master, reference = target, soundPath = "MWE\\MWE_BluntDisarmShield.wav"})
            if cf.mb then
        ...
        mod.slam = 0
        if mod.bash == 3 then
            tes3.playAnimation{reference = target, group = tes3.animationGroup.idleStorm, loopCount = 4}
        ...
        mod.bash = 0
        if mod.knockout < 2 then mod.knockout = mod.knockout+1
        elseif mod.knockout == 4 then
            mod.knockout = 0
            tes3.setStatistic({reference = target, name = "fatigue", current = (0-(skill)), limit = false})
            tes3.playSound({mixChannel = tes3.soundMix.master, reference = target, soundPath = "MWE\\MWE_BluntKnockOut.wav"})
            if cf.mb then
                tes3.messageBox{message = table.choice(message.Knockout)}
            end
        else mod.knockout = 0 end
    elseif type == tes3.physicalAttackType.slash then
        ...
        if mod.bone == 2 then
            ...
            tes3.playSound({mixChannel = tes3.soundMix.master, reference = target, soundPath = "MWE\\MWE_BluntBoneBreak.wav"})
            if cf.mb then
...
list.def = {bone = [[Bone Break
Sequence: Slash, Chop, Slash
Description: Damages the opponent's weapons skills.]],
            knock = [[KnockDown
Sequence: Chop, Slash, Thrust
Description: A strong thrust to the opponent's hip that knocks them down.]],
            slam = [[Slam
Sequence: Thrust, Slash, Slash, Chop
Description: Breaks your enemy's helmet.]],
            bash = [[Stun
Sequence: Slash, Slash, Thrust, Chop
Description: Temporarily paralyzes the enemy and does extra damage.]],
            knockout = [[KnockOut
Sequence: Chop, Chop, Slash, Slash, Chop
Description: Finishing move that knocks the enemy unconscious.]]}
return list