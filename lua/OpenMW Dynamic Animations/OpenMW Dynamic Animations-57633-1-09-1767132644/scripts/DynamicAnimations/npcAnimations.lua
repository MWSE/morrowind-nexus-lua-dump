...
local L = {
	...
	idleStance = types.Actor.STANCE.Nothing,
...
local anims = {
	idleGroups = { "armsfolded", "armsakimbo", "armssunshield", "armsatback",
		"idle4", "idle5", "idle7", "idle8"
	}
}
...
local function playHandler(g, o)
	if L.getStance(self) ~= L.idleStance then
	--	playSwaps = false
		return
	end
...
--	print(aux_util.deepToString(e, 5))
	...
	if m then
		walk.wander = m.id		walk.velocity = m.velocity
...
local function cancelAnims()
	for _, v in ipairs(anims.idleGroups) do
...
local function updateAI()
	maxTurn = nil
	if disabled or not AI.wander or not swaps or AI.stance ~= L.idleStance then
...
I.ODAR.addHandler("AIChange", function(p)
	...
	if playSwaps and AI.stance ~= L.idleStance then
		cancelAnims()
	end
	updateAI()
end)
