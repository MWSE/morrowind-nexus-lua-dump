...
local handlers = { onUpdate = {}, onInit = {}, onActive = {}, onSecond = {}, AIChange = {} }
handlers.scripts = {
	rebootAI = function()
...
local updateHandlers_n = 0
local odarScripts = {}

local AI = {}
handlers.updateAI = function()
	local p = I.AI.getActivePackage() or {}
	if p.type ~= AI.type or AI.stance ~= types.Actor.getStance(self) then
		AI = { type=p.type, distance=p.distance, idle=p.idle }
		AI.stance = types.Actor.getStance(self)
...
--	onLoad = function() print("ODAR ONLOAD")		end
                ...
                if m and m.engineHandlers then
                    for k, v in pairs(m.engineHandlers) do
                        ...
                        --    if h.n then h.n = #h		end
                            if k == "onUpdate" then updateHandlers_n = #h		end
    ...
    interface = {
        ...
        addHandler = function(e, fn)
            ...
            h[#h + 1] = fn
        --    if h.n then h.n = #h		end
            if e == "onUpdate" then updateHandlers_n = #h		end
        end,
