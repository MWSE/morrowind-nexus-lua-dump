...
---@param params table
---@return boolean
local function mobequip(mob, params)
	local success = mob:equip(params)
	if success then
		local prms = table.copy(params)
		prms.reference = mob.reference
		event.trigger('equip', prms)
	end
	return success
end
...
funcPrefix, newPosFound, notInPlayerCell, mobRef, v_pos, math.deg(ori.z), pcCell.editorname)
										...
										mobPos.z = v_pos.z
										mobPos.x, mobPos.y = v_pos.x, v_pos.y
										if logLevel2 then
											mwse.log(
...
function L.autoMove()
	...
	local count = 0
	filteredDict = {}
	timer.start({duration = 1.3, type = timer.real,
