...
noFriendlyFireMagic = 2, -- 0 = No, 1 = Companions, 2 = Any follower, 3 = Companions, no aggro, 4 = Any follower, no aggro
noFriendlyFireDamage = 0, -- 0 = No, 1 = Companions, 2 = Any follower, 3 = Companions, no aggr, 4 = Any follower, no aggro
...
local function getAiFollowOrEscortTargetMob(mob)
...
local function targetIsFriendlyWith(targetMob, attackerMob,
		companionOnly, withPlayer)
	local targetMobAItarget = getAiFollowOrEscortTargetMob(targetMob)
	if (
		withPlayer
		and (targetMobAItarget == mobilePlayer)
	) or (targetMobAItarget == attackerMob) then
		local result = true
		if companionOnly
		and ( not getCompanionVar(targetMob.reference) ) then
			result = false
		end
		if logLevel2
		and result then
			mwse.log('%s: targetIsFriendlyWith("%s", "%s") = true',
				modPrefix, targetMob.reference.id, attackerMob.reference.id)
		end
		return result
	end
	return false
...
local function areFriendly(targetMob, attackerMob, companionOnly)
	local result = targetIsFriendlyWith(targetMob, attackerMob, companionOnly, true)
...
local function damage(e)
	...
	if areFriendly(targetMob, attackerMob, noFriendlyFireDamageCO) then
		...
		if noFriendlyFireDamage >= 3 then
			...
			targetMob:stopCombat(noFriendlyFireDamageCO)
		end
		return false
	end
...
local function spellResist(e)
	...
	if attackerMob
	and (attackerMob == targetMob) then
...
function L.packLastPlayerPositions()
	...
	local t = {}
	local j = 0
	for _, v in pairs(lastPlayerPositions) do
		---assert(v)
		if v then
			if v.pos then
				j = j + 1
...
function L.cleanLastPlayerPositions()
...
currMountRef.facing = %s, player1stPerson.facing = %s]],
			modPrefix, math.deg(dza), math.deg(normalizedAngle(mountFacing)),
...
local function warpFollowers()
	...
	local playerHasSlowfall = tes3.isAffectedBy({reference = mobilePlayer, effect = tes3.effect.slowFall})
...
---@diagnostic disable-next-line: assign-type-mismatch
					effect = tes3.effect.slowFall, object = ab01mountNPCAbility})
				) then
...
-- comparison with standard 1 is important as it may be used as flag with different walues e.g. in guar mod
				...
				---or mobIsCreature then
					...
					and ( not isAffectedByEffect(mob, tes3.effect.waterWalking) ) then
...
local function fixRefConstEnch(ref)
	...
	local function isFollowingOrEscorting(mob, targetActor)
		if targetActor then
			local aiTargetMob = getAiFollowOrEscortTargetMob(mob)
	...
	if not isFollowingOrEscorting(mob, mobilePlayer) then
		return
	end
	local items = inventory.items
	local t = {}
...
local function fixAiWalkDestinations(mob, actionData)
...
local function fixActionData(mob)
	fixAiWalkDestinations(mob, mob.actionBeforeCombat)
	---fixAiWalkDestinations(mob, mob.actionData)
end
...
function L.loaded()
	...
	L.stopMoveTravellers()
	travelType = 0
	local tv = player.data.ab01travellers
	if tv then
		travellers = tv
		L.packTravellers()
	else
...
function L.modConfigReady()
	...
	waterWalkSpell = createSpell('ab01smcompWaterwalking', 'Water Walking', {{id = tes3.effect.waterWalking, min = 1, max = 1}})
...
"No aggro" options do not increase the actor friendly hits counter.
