...
local function getCompanionVars(mobile) -- out companion, spread
	local companion = nil
	local spread = nil
	local ref = mobile.reference
	if ref.disabled then
		return companion, spread
	end
	if ref.deleted then
		return companion, spread
	end
	...
	if context then
		...
		if spread then
			if config.logLevel >= 3 then
				mwse.log("%s: %s, spread = %s", modPrefix, ref.id, spread)
			end
		end
	end
	return companion, spread
...
local function travelEnd()
	...
	for id, t in pairs(travellers) do
		if config.logLevel >= 2 then
			mwse.log("%s: mwscript.removeSpell({reference = %s, spell = %s}), invisibility = %s, acrobatics = %s", modPrefix, id, ability, t.inv, t.acro)
		end
		mwscript.removeSpell({reference = t.mob, spell = ability}) -- remove travelling spell
		t.mob.invisibility = t.inv -- remove invisibility
		t.mob.acrobatics.current = t.acro -- reset acrobatics
	end
	travellers = {}
	numTravellers = 0
	travelType = 0
...
local function timedTravelProcess()
		...
		if not (mobile == mobilePlayer) then
			if mobile.actorType == 1 then -- NPC
				if isValidFollower(mobile) then
					local _, spread = getCompanionVars(mobile)
					if not spread then -- companion script already providing scenic travelling
						...
						if dist <= maxDist then
							...
							if not travellers[id] then
								travellers[id] = {mob = mobile, inv = mobile.invisibility, acro = mobile.acrobatics.current} -- store mobile, invisibility and acrobatics of follower
	...
	if numTravellers > 0 then
		...
		for id, t in pairs(travellers) do
			if config.logLevel >= 2 then
				mwse.log("%s: mwscript.addSpell({reference = %s, spell = %s}), invisibility = 1, acrobatics = 200", modPrefix, id, ability)
			end
			mwscript.addSpell({reference = t.mob, spell = ability})
			t.mob.invisibility = 1 -- setInvisible to avoid aggro cliffracers
			t.mob.acrobatics.current = 200 -- high acrobatics to avoid damage if cell changed
		end
		startMoveTravellers()
	end
end
	...
	if ab01travellers then
		...
		for id, t in pairs(ab01travellers) do
			...
			if ref then
				...
				if mobile then
					if config.logLevel >= 2 then
						mwse.log("%s: loaded travellers[%s] = {inv = %s, acro = %s}", modPrefix, id, t.inv, t.acro)
					end
					travellers[id] = {mob = mobile, inv = t.inv, acro = t.acro}
					numTravellers =	numTravellers + 1
				end
			end
		end
...
-- cannot store pointers in saves, they have to be reconstructed from id on reload
			ab01travellers[id] = {inv = t.inv, acro = t.acro}
		end
	end
...
local function companionActivate(targetRef)
	...
	for mobile in tes3.iterate(mobilePlayer.friendlyActors) do
		if not (mobile == mobilePlayer) then
			local companion, spread = getCompanionVars(mobile)
			if companion == 1 then
				mobileRef = mobile.reference
				if config.logLevel >= 3 then
					mwse.log("%s: getCompanionVars(%s) companion = %s, spread = %s", modPrefix, mobileRef.id, companion, spread)
				end
	...
	if activatorRef == player then
		...
		if targetMobile then
			...
			if not deadMobile then
				...
				local companion, _ = getCompanionVars(targetMobile)
