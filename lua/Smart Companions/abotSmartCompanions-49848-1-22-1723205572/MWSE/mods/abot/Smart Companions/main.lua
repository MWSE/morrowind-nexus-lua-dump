...
local function getSetRefVariable(ref, variableId, newValue)
	...
	if logLevel4 then
		mwse.log('%s: getSetRefVariable("%s", "%s", %s)',
	...
	if logLevel4 then
		mwse.log('%s: getSetRefVariable("%s", "%s", %s) context = %s',
			modPrefix, ref.id, variableId, newValue, context)
	end
	-- need more safety
	local value = context[variableId]
	if value then
		if logLevel3 then
			mwse.log('%s: getSetRefVariable("%s", "%s", %s) context["%s"] was %s)',
				modPrefix, ref.id, variableId, newValue, variableId, value)
		end
		if newValue then
			value = newValue
			context[variableId] = value
		end
		return value
	end
	return nil
...
-- comparison with standard 1 is important as it may be used as flag with different walues e.g. in guar mod
			...
			and (
				...
				local notInPlayerCell = not (mobCell == pcCell)
				...
				local d = 0
				if notInPlayerCell
				or pcCell.isInterior
				or mobCell.isOrBehavesAsExterior
				or isStayOutside(mob) then
		...
		and t.inv then
			...
			if mobRef then
				...
				if mob then
					...
					if t.ns > 0 then
						local nospread = getSetRefVariable(mobRef, 'nospread', 0)
						if nospread
						and logLevel3 then
...
local function aiWander0(mobRef)
	...
	tes3.setAIWander({reference = mobRef, duration = 0, idles = {40, 0, 0, 0, 0, 0, 0, 0}, reset = false})
		...
		if mob.actorType == tes3_actorType_npc then
			if isValidScenicFollower(mob) then
				...
				and nospread then
					...
					mobRef.object.script['context'].nospread = 1
				end
...
local function companionLootContainer(companionRef, targetRef)
	...
	if lootedCount > 0 then
		...
		if companionActorType == tes3_actorType_npc then -- NPC
			if niceLoot then
				---tes3ui.showNotifyMenu(table.choice(niceLootMsg), strip(niceLoot))
				tes3ui.showNotifyMenu(niceLootMsg[math.random(#niceLootMsg)], strip(niceLoot))
			end
...
local function companionActivate(targetRef)
	...
	for i = 1, #friendlyActors do
		...
		if not (mob == mobilePlayer) then
			...
			local companion = getCompanion(mobileRef)
			if companion then
				if companion == 1 then
					if logLevel3 then
					...
					if targetMob.fatigue.current <= 0 then
						...
						if skip then
							...
							and (knockout == 0) then
								...
								getSetRefVariable(targetRef, 'knockout', 1)
								if logLevel1 then
	...
	for i = 1, #friendlyActors do
		...
		if valid > 0 then
			...
			if playerCellIsInterior then
				local mobRefCell = mobRef.cell
				if mobRefCell.isOrBehavesAsExterior
				and ( not (playerCell == mobRefCell) )
