...
-- not to get/set its value!!!
	---local context = script['context'] -- will this work better? nope here as I need to chnage the value
	...
	if value then
		if logLevel3 then
			mwse.log('%s: context["%s"] was %s)', funcPrefix, variableId, value)
		end
		if newValue then
			value = newValue
			context[variableId] = value
			if logLevel3 then
				mwse.log('%s: context["%s"] is %s)', funcPrefix, variableId, value)
			end
		end
		return value
	end
