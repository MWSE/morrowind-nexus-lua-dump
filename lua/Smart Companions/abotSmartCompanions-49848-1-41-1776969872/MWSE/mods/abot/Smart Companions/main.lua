...
scanInterval = 0.25, -- make followers more reactive, increase it if causing problems
...
-- usage: replace all calls to mob:equip(params) with this one
---@param mob tes3mobileActor
---@param e table
---@return boolean
function L.mobequip(mob, e)
	local item = e.item
	if item
	and ( item.type == 'string' ) then
---@diagnostic disable-next-line: param-type-mismatch
		e.item = tes3.getObject(item)
	end
	e.reference = mob.reference
	e = event.trigger('equip', e)
	if e.block then
		return false
	end
	return mob:equip(e)
...
local function isMobilePlayer(mob)
	assert(tes3.mobilePlayer == mobilePlayer)
	local result = (mob.actorType == tes3_actorType_player)
	assert(result == (mob == mobilePlayer))
	return result
end
...
-- mwse.log('>>> 1 validFollower("%s", %s, %s)',
	-- mobRef, anyFollower, checkWarpFightingCompanions)
		return 0
	end
...
-- mwse.log('>>> 2 validFollower("%s", %s, %s)',
	-- mobRef, anyFollower, checkWarpFightingCompanions)
					return 0
				end
			end
...
-- mwse.log('>>> 3 validFollower("%s", %s, %s)',
	-- mobRef, anyFollower, checkWarpFightingCompanions)
					return 0
				end
			end
...
-- mwse.log('>>> 4 validFollower("%s", %s, %s)',
	-- mob, anyFollower, checkWarpFightingCompanions)
		return 0
...
-- mwse.log('>>> 5 validFollower("%s", %s, %s)',
	-- mobRef, anyFollower, checkWarpFightingCompanions)
		return 0
...
-- mwse.log('>>> 6 validFollower("%s", %s, %s)',
	-- mobRef, anyFollower, checkWarpFightingCompanions)
	return 0
end
...
---@diagnostic disable-next-line: param-type-mismatch
		pcall( mob:unequip(prms) )
---@diagnostic disable-next-line: param-type-mismatch
		pcall( L.mobequip(mob, prms) )
		---pcall( mwscript.equip({reference = mob.reference, item = itm.id})
		if logLevel3 then
...
function L.combatStopped(e)
	...
	for i = 1, #friendlyActors do
		local mob = friendlyActors[i]
		if not (mob.actorType == tes3_actorType_player) then
		---if not isMobilePlayer(mob) then
