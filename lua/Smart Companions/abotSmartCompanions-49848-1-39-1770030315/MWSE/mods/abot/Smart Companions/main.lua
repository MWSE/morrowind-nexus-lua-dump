...
scanInterval = 0.2, -- make followers more reactive, increase it if causing problems
...
local function isDummy(mobRef)
	...
	if mesh:lower():multifind(dummies, 1, true) then
		return true
	end
	return false
end
...
local function isStayOutside(mob)
	local mobRef = mob.reference
	if mobRef then
		local lcId = mobRef.baseObject.id:lower()
...
local function isLootable(obj, script)
	...
	if objType == LIGH_T then -- not scripted but...
		...
		if obj.canCarry then
			if (obj.radius < 17)
			or (obj.weight < 0.0001) then
				return false -- probably a light used as icon, skip
			end
		else
			return false -- skip non carriable lights if any
		end
	end
	if not script then
...
function L.companionLootContainer(companionRef, targetRef)
	...
	if targetObj.objectType == CONT_T then
		...
		and (not lcId:find('chest', 1, true)) then -- skip guild chests
			local gHerb = false
			...
			if empty then
				if gHerb then
					...
					if mesh
					and mesh:lower():find('flora', 1, true) then
						targetRef:disable()
					end
				end
			end
			...
			if not targetMobIsDead then
				...
				or (validFollower(targetMob, anyFollower) > 0) then
					...
					if targetMob.fatigue.current <= 0 then
						local skip = fixFatiguedDict[targetObj.id:lower()]
						if skip then
...
local function combatStart(e)
	...
	if isDummy(attackerMob.reference)
	or isDummy(targetMob.reference) then
		return false
	end
end
...
function L.autoMove()
	...
	local count = 0
	filteredDict = {}
	timer.start({duration = 1.4, type = timer.real,
...
You can enable this if you want to avoid companions chasing enemies too far, or if they are in a big danger.]],
	...
	scanIntervalSlider = sideBarPage:createSlider({
		...
		description = [[Minimum AI Scan Interval for followers. Lower should make followers more reactive but could be CPU intensive.
