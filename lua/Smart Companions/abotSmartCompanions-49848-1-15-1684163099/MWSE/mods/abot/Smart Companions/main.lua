...
local function validFollower(mob, anyFollower, checkWarpFightingCompanions)
	...
	elseif ai == tes3_aiPackage_escort then
		if logLevel4 then
			mwse.log('%s: "%s" aiescort aCOmpanion = %s', modPrefix, mobRef.id, aCompanion)
		end	
		if aCompanion then
			return 2
		end
			...
			if playerLevitate > 0 then
				if not movingTravellers then
					if valid >= warpLevitate then
						...
						alignToPlayerZ(mobRef) -- let's try again (nope bugs too often while levitating)
					end
				end
			elseif mob:isAffectedByObject(levitateSpell)
	...
	if numTravellers > 0 then
		...
		for id2, t in pairs(travellers) do
			if t then -- needed safety
				...
				tes3.playAnimation({reference = mobRef1, group = 0}) -- reset animation
			end
		end
		startMoveTravellers()
	end
