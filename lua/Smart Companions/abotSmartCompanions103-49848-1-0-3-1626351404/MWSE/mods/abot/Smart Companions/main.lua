...
local function travelEnd()
	...
	for id, t in pairs(travellers) do
		...
		mob.invisibility = t.inv -- remove invisibility
	...
	tes3.applyMagicSource({ -- apply a short chameleon/invisibility to try and fix player appearance sometimes buggy after travelling with setinvisible
		reference = player,
		name = "End Travelling",
		effects = { {id = tes3.effect.chameleon, duration = 2, min = 1, max = 1}, {id = tes3.effect.invisibility, duration = 3}}
	})
	travellers = {}
	numTravellers = 0
	travelType = 0
					...
					if doMove then
						...
						if dist <= maxDist then
							...
							if not travellers[id] then
								tes3.setAIWander({reference = mobile, range = 0, idles = {10, 10, 10, 10, 0, 0, 0, 0}, reset = true}) -- hopefully local NPC script will stop warping in wander mode
