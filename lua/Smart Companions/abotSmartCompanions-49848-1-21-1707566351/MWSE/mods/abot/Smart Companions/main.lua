...
local autoWarp, autoAttack, autoBurden
local ignoreMannequinAttacks, warpFightingCompanions, warpWaterWalk
local warpLevitate, muteCC, fixConstEnch
...
local infoFilterRegistered = false
local function setInfoFilter(on)
	if infoFilterRegistered
	or event.isRegistered('infoFilter', infoFilter) then
		if on then
			return
		end
		infoFilterRegistered = false
		event.unregister('infoFilter', infoFilter)
		return
	end
	if on then
		infoFilterRegistered = true
...
local skipConstEnchFix = false -- reset to false in cellChanged(e)
...
local function combatStopped(e)
	...
	local isPlayer = (actorMob == mobilePlayer)
