...
encumbranceFixOnActivate = false, -- try and fix follower encumbrance on activate
...
scanInterval = 0.22, -- make followers more reactive, increase it if causing problems
...
-- to be set in initialized()
local worldController

-- to be set in initialized(), refreshed in loaded()
...
[T3OT.armor] = true, [T3OT.book] = true, [T3OT.clothing] = true,
...
local muteCC
local noFriendlyFireDamage, isNFFDamageCompanions
local noFriendlyFireMagic, isNFFMagicCompanions
local logLevel, logLevel1, logLevel2, logLevel3, logLevel4, logLevel5

--[[
---@param ref tes3reference
local function getAnimationData(ref)
-- 3 different places for this
...
function L.updateFromConfig()
	...
	isNFFDamageCompanions = (noFriendlyFireDamage == 1)
		or (noFriendlyFireDamage == 3)
	isNFFMagicCompanions = (noFriendlyFireMagic == 1)
...
---@param mob tes3mobileActor|tes3mobileCreature
...
local function getSetRefVariable(ref, variableId, newValue)
	...
	if logLevel1 then
		if newValue then
			funcPrefix = ('%s: getSetRefVariable("%s", "%s", %s)'):format(
				modPrefix, ref.id, variableId, newValue)
		else
			funcPrefix = ('%s: getSetRefVariable("%s", "%s")'):format(
	...
	if not script_context then
		if logLevel1 then
			mwse.log('%s: "%s".object.script["context"] = %s',
				funcPrefix, ref.id, script_context)
		end
		return
	end
	...
	if ref.attachments then
		if ref.attachments.variables then
			local script = ref.attachments.variables.script
			if not script then
				if logLevel5 then
					mwse.log('%s: "%s".attachments.variables.script = %s',
						funcPrefix, ref.id, script)
				end
				return
			end
		else
...
local function getCompanionVar(ref)
	...
	if logLevel4 then
		mwse.log('%s: getCompanionVar("%s") = %s', modPrefix, ref, result)
		local result2 = getRefVariable(ref, 'comPanion')
...
---@param ref tes3reference
---@return number?
local function getStayOutsideVar(ref)
	local result = getRefVariable(ref, 'stayoutside')
	if logLevel4 then
		mwse.log('%s: getStayOutsideVar("%s") = %s', modPrefix, ref, result)
		local result2 = getRefVariable(ref, 'stayouTside')
		assert(result == result2)
	end
	return result
end
...
---@param s2 string
---@param patterns string[]
local function multifind2patt(s1, s2, patterns)
	return s1:multifind(patterns)
	or s2:multifind(patterns)
end

local dummiesPatt = {'[_ ]?dumm?[_ ]?', '[_ ]?mann?[_ ]?', '[_ ]?target[_ ]?', 'invis'}

---@param mobRef tes3reference
---@return boolean
function L.isDummy(mobRef)
	local obj = mobRef.baseObject
	if multifind2patt(obj.id:lower(), obj.name:lower(), dummiesPatt) then
		return true
	end
	local race = obj.race
	if race then
	...
	if mesh:lower():multifind(dummiesPatt) then
		return true
	end
	return false
end
...
--- better spare some local variables room tes3_aiPackage_none = tes3.aiPackage.none
...
local function hasBlacklistedAnim(mob)
	...
	local lcMesh = mesh:lower()
	if not lcMesh:multifind(animBlacklist, 1, true) then
		return false
	end
	local aiTargetMob = getMobAiFollowOrEscortTarget(mob)
	if aiTargetMob then
		return false -- skip followers/escorters
	end
	...
	and hasOneTimeMove(mobRef) then
		...
		return false -- skip oneTimeMove companions
	end
	return true
end
...
---@param targetMob tes3mobileActor
---@param attackerMob tes3mobileActor
---@param companionOnly boolean?
---@return boolean
...
---@param mob tes3mobileActor
function L.checkDecreaseHitCount(mob)
	...
	if hitCount > 0 then
		...
		if logLevel3 then
			mwse.log('%s: checkDecreaseHitCount("%s") hitCount reset from %s to %s',
				modPrefix, mob.reference, hitCount, mob.friendlyFireHitCount)
		end
	end
end
...
local function isMobilePlayer(mob)
	...
	-- refresh needed it could be different in first referenceActivated
	mobilePlayer = tes3.mobilePlayer
	if not mobilePlayer then
		return false
	end
	local result = (mob.actorType == tes3_actorType_player)
	---assert( result == (mobilePlayer == mob) )
	---assert( result == isPlayer(mob.reference) )
	return result
end

---@param targetMob tes3mobileActor
local function checkNoAggro(targetMob)
	L.checkDecreaseHitCount(targetMob)
	...
	for i = 1, #friendlyActors do
		...
		and (
			not (
				(mob == targetMob)
				or isMobilePlayer(mob)
			)
		)
		and mob.actionData
		---and mob.actionData.target
		and (mobilePlayer == mob.actionData.target) then
			if logLevel4 then
				mwse.log('%s: checkNoAggro("%s") stopCombat()', modPrefix, mob.reference)
			end
			mob:stopCombat(true)
		end
	end
...
local function damage(e)
	...
	local attackerIsPlayer = isMobilePlayer(attackerMob)
	if attackerIsPlayer
	and (noFriendlyFireDamage >= 3) then
		checkNoAggro(targetMob)
	end
	if e.damage <= 0 then
	...
	if attackerIsPlayer then
		if config.friendlyFireSoulTrapping then
			local target = targetMob.reference
			if tes3.isAffectedBy({reference = target, effect = tes3_effect_soultrap})
			or tes3.isAffectedBy({reference = target, effect = tes3_effect_npcSoulTrap}) then
				if not getCompanionVar(target) then
					if logLevel1 then
						mwse.log('%s: damage() from "%s" to "%s" friendlyFireSoulTrapping, skip',
							modPrefix, attackerMob.reference.id, target.id)
					end
	...
	if areFriendly(targetMob, attackerMob, isNFFDamageCompanions) then
		if logLevel2 then
...
local function spellResist(e)
	...
	if not magicEffect then
		...
		if effect then
			...
			for i = 1, #effects do
			    eff = effects[i]
				local mageff = eff.object
				if mageff
			    and mageff.isHarmful
	...
	and (config.friendlyFireSoulTrapping) then
		if tes3.isAffectedBy({reference = targetRef, effect = tes3_effect_soultrap})
		or tes3.isAffectedBy({reference = targetRef, effect = tes3_effect_npcSoulTrap}) then
	...
	if areFriendly(targetMob, attackerMob, isNFFMagicCompanions) then
		skipNotify = true
...
local function uiActivatedMenuNotify(e)
	if skipNotify then
		skipNotify = false
		if e.element then
			---e.element.:destroy() -- could crash
			e.element.visible = false -- safer
		end
	end
end
...
local function getEnabledRef(ref)
	if ref
	and ref:isValid()
...
local function isValidMobile(mob)
	local mobRef = mob.reference
	local function log0(step)
		if logLevel4 then
			mwse.log('%s: %s isValidMobile("%s") = false', modPrefix, step, mobRef)
		end
	end
	if not mobRef then
		log0(1)
	...
	if not getEnabledRef(mobRef) then
		log0(2)
		return false
	end
	if isDead(mob) then
...
-- ab01 prefix, probably some abot's creature having AIEscort/AIFollow package, skip
			if logLevel3 then
				mwse.log("%s: %s having ab01 prefix, probably some abot's creature with AIFollow/AIEscort package, skip", modPrefix, mobRef.id)
			end
			return false
		end
	end
...
---@param mob tes3mobileActor
---@param anyFollower boolean?
---@param checkWarpFightingCompanions boolean?
---@return 0|1|2
local function validFollower(mob, anyFollower, checkWarpFightingCompanions)
	local mobRef = mob.reference
	local function log0(step)
		if logLevel4 then
			mwse.log('%s: %s validFollower("%s", %s, %s) return 0',
				modPrefix, step, mobRef, anyFollower, checkWarpFightingCompanions)
		end
	end
	...
	if checkWarpFightingCompanions then
		if not config.warpFightingCompanions then
			if mobIsCompanion
			or mobRefObj.id:lower():find('summon', 1, true) then
	...
	if ai == tes3_aiPackage_follow then
		...
		if anyFollower
		and (not mobRefObj.isGuard) then -- better to skip guards, could be temporarily following
			return 1
		end
		log0(4)
		return 0
	elseif ai == tes3.aiPackage.none then
		if mobIsCompanion then
			return 2
		end
		log0(5)
...
local travelParams = {
-- boat/riverstrider
[1] = {spell = 'ab01boSailAbility', spread = 32, maxInLine = 5, maxY = 100},

-- strider
[2] = {spell = 'ab01ssMountAbility', spread = 27, maxInLine = 5, maxY = 90},

-- gondola
[3] = {spell = 'water walking (unique)', spread = 22, maxInLine = 1, maxY = 60},

-- guar
[4] = {spell = 'ab01mountNPCAbility', spread = 40, maxInLine = 1, maxY = 60},
}
...
local function isValidScenicFollower(mob)
	...
	if validFollower(mob, anyFollower) > 0 then
		local mobRef = mob.reference
		if not (mobRef.data
			and mobRef.data.rfuzzo_invincible
			and (mob.activeAI == false)
		) then -- skip moving immersive travel actors
			local boundSize_y = mob.boundSize.y * mobRef.scale
			local maxY = travelParams[travelType].maxY
			if boundSize_y > maxY then -- skip big mesh actors
				if logLevel2 then
					mwse.log('%s: isValidScenicFollower("%s"), boundSize.y %s > %s, skipped', modPrefix, mobRef.id, boundSize_y, maxY)
				end
				return false
			end
			if logLevel3 then
...
local function round0(x)
 ...
 -- reset in loaded()
-- e.g. travelers[lcMobRefId] = {inv = mob.invisibility, acro = mob.acrobatics.current, ns = tns}
-- store invisibility, acrobatics, nospread
local travelers = {}
local numTravelers = 0
local doPackTravelers = false
local busyPackingTravelers = false

local refsById, refsByRef = {}, {}
...
-- so it turns out
-- for objectInvalidated(e) event
-- e.object is already a stale pointer
-- fsp used to be tes3baseObject, tes3mobileObject, tes3uiElement, niObject
-- and e.object.anyproperty is totally unreliable
-- and Lua bugs out spamming the log if you even try
-- accessing e.object.anyproperty
	local fsp = e.object
	if not fsp then
		return -- better safe than sorry
	end
	if not (type(fsp) == 'userdata') then
		return -- bah hopefully this will be enough to skip weird data
	end
	if not refsByRef[fsp] then
		return
	end
	refsByRef[fsp] = nil
	local key = table_find(refsById, fsp) -- table.find seems to work
	if not key then
		return
	end
	refsById[key] = nil
	if logLevel1 then
		mwse.log('%s: %s() refsById["%s"] cleaned',
			modPrefix, e['eventType'], key)
	end
end

--- @param lcRefId string
---@return tes3reference?
local function getActorRef(lcRefId)
	local ref = refsById[lcRefId]
	if getEnabledRef(ref) then
		return ref
	end
	...
	for i = 1, #mobileActors do
		mob = mobileActors[i]
		if mob
		and mob:isValid() then
			ref = mob.reference
			if getEnabledRef(ref) then
				---mwse.log('ref = "%s"', ref)
				if ref.id:lower() == lcRefId then
					refsByRef[ref] = lcRefId
					refsById[lcRefId] = ref
					return ref
				end
			end
		end
	...
	if getEnabledRef(ref) then
		if logLevel4 then
			mwse.log('%s: getActorRef() refsById["%s"] cached', modPrefix, lcRefId)
		end
		refsByRef[ref] = lcRefId
		refsById[lcRefId] = ref
		return ref
...
function L.packTravelers()
	busyPackingTravelers = true
	local t = {}
	numTravelers = 0
	for lcMobRefId, v in pairs(travelers) do
		if v.inv then
	...
	travelers = t
	busyPackingTravelers = false
end

function L.cleanTravelers()
	numTravelers = 0
...
local ab01boDestValue, ab01ssDestValue, ab01goDestValue, ab01compMount = 0, 0, 0, 0

 -- cached globals, found in modConfigReady
local ab01boDest, ab01ssDest, ab01goDest
local ab01goAngle, NPCVoiceDistance, ab01compMounted

-- reset in loaded()
local currMountRef, currMountHandle, lastMountFacing

---@param travelType 0|1|2|3|4
---@return tes3reference?
...
-- travelType 0 = none, 1 = boat, 2 = strider, 3 = gondola, 4 = guar
	...
	for i = 1, #mobs do
		...
		if mob then
			...
			local idPrefix = obj.id:sub(1, 6):lower()
...
function L.initScenicTravelAvailable()
	local funcPrefix = modPrefix..': initScenicTravelAvailable()'
	if logLevel5 then
		mwse.log(funcPrefix)
	end
	local function logMsg(varId, varValue)
		mwse.log('%s %s = %s', funcPrefix, varId, varValue)
	end
	if ab01boDestValue then
		scenicTravelAvailable = true
	elseif ab01ssDestValue then
		scenicTravelAvailable = true
	elseif ab01goDestValue then
		scenicTravelAvailable = true
	elseif ab01compMount then
		scenicTravelAvailable = true
	else
	...
	if ab01boDest then
		ab01boDestValue = round0(ab01boDest.value)
		if logLevel2 then
			logMsg('ab01boDestValue', ab01boDestValue)
		end
	end
	if ab01ssDest then
		ab01ssDestValue = round0(ab01ssDest.value)
		if logLevel2 then
			logMsg('ab01ssDestValue', ab01ssDestValue)
		end
	end
	if ab01goDest then
		ab01goDestValue = round0(ab01goDest.value)
		if logLevel2 then
			logMsg('ab01goDestValue', ab01goDestValue)
		end
	end
	if ab01compMounted then
		...
		if currMountRef then
			...
			if localVar then
				...
				if logLevel2 then
					logMsg('ab01compMount', ab01compMount)
				end
			end
		end
	end
...
-- updated in startMoveTravelers()
local travelRadStep = 0


---@param a number
---@param radStep number
...
local function getCosSin(a, radStep)
	...
	local cosa = {
		[1] = math_cos(a),
		[2] = math_cos(a - radStep),
		[3] = math_cos(a + radStep),
		[4] = math_cos(a - radStep2),
		[5] = math_cos(a + radStep2),
		[6] = math_cos(a - radStep3),
		[7] = math_cos(a + radStep3),
	}
	local sina = {
		[1] = math_sin(a),
		[2] = math_sin(a - radStep),
		[3] = math_sin(a + radStep),
		[4] = math_sin(a - radStep2),
		[5] = math_sin(a + radStep2),
		[6] = math_sin(a - radStep3),
		[7] = math_sin(a + radStep3),
	}
	return cosa, sina
end
...
---@out number -- -pi <= radians <= pi
local function normalizedHalfPiRad(a)
	a = a % double_pi
	if a > pi then
		return a - double_pi
	end
...
local function checkTravelFacing()
	...
	local dza = normalizedHalfPiRad(mountFacing - lastMountFacing)
	...
	local a = normalizedHalfPiRad(player1stPerson.facing + dza)
...
local movingTravelers = false

local function moveTravelers()
	...
	local playerPos, mobPos
	for lcMobRefId, t in pairs(travelers) do
		local invalid = true
		if t -- needed safety
		and t.inv then
...
---mwse.log(">>> mobRef = %s, mobPos = %s", mobRef, mobPos)
				...
				mobPos.z = playerPos.z -- again

				---mobRef.position = mobPos + mobilePlayer.velocity -- just a test
		...
		if invalid then
			travelers[lcMobRefId].inv = nil -- flag to be invalid/packed
			doPackTravelers = true
			if numTravelers > 0 then
				numTravelers = numTravelers - 1
			else
				break
			end
...
local function isMount(mobRef)
	if mobRef.id:lower():multifind({'guar', 'horse', 'mount'}, 1, true) then
		return true
	end
	return false
end
...
local function isStayOutside(mob)
	local mobRef = mob.reference
	if mobRef then
		local baseObj = mobRef.baseObject
		local lcId = baseObj.id:lower()
		if forceStayOutsideDict[lcId] then
			return true
		end
		if baseObj.mesh:lower():startswith('mer_tgw') then
			return false -- fucking shit sets stayoutside to 1 but does not respect it
		end
		local stayoutside = getStayOutsideVar(mobRef)
		if stayoutside
...
local function warpFollowers()
	...
	local maxAfterWarpDist = round0( (dd * k_steps) + minDist )
	...
	---@param cell_1_id string
	---@param cell_2 tes3cell
	---@return boolean
	local function sameCellOrExterior(cell_1_id, cell_2)
		return (cell_1_id == cell_2.id)
		or (
			(cell_1_id:len() == 0) -- stored cell_1_id:len() == 0 means cell1 is exterior
		...
		if playerHasSlowfall then
			if mobIsCreature then
				...
				and tes3.isAffectedBy({reference = mobRef,
					effect = tes3_effect_slowFall, object = ab01mountNPCAbility}) then
-- e.g. npc riding a ab01guguarpackmount, as we are here, try and reduce bounding box so guar can be activated
		...
		if ok then
			if playerLevitate > 0 then
				...
				elseif validLev then
					...
					and ( not mob:isAffectedByObject(levitateSpell) ) then
						...
						---if movingTravelers and mobIsCreature then
			...
			---if (mob.waterWalking > 0)
			---or isAffectedByEffect(mob, tes3_effect_waterWalking) then
...
-- comparison with standard 1 is important as it may be used as flag with different walues e.g. in guar mod
			...
			and (
				...
				local mobPosCopy = mobRef.position:copy()
				local doWarp = false
				...
				if notInPlayerCell then
					...
					local d = mobPosCopy:distance(playerPos)
					if d > warpDist then
						doWarp = true
					end
				end
				...
				if not config.warpFightingCompanions then
					if isCompanion(mobRef)
					or mobRef.object.id:lower():find('summon', 1, true) then
					    if (not mob.canAct)
 -- drawing/sheathing their weapon, attacking, casting magic or using a lockpick or probe
					    or inCombat(mob) then
						    doWarp = false
						end
					end
								...
								if d <= maxAfterWarpDist then -- only if at least half way
									if isBehindPlayer(v_pos) then
										if tes3.testLineOfSight({position1 = v_pos:copy(), height1 = 120,
											...
											-- in a clear place
											mobPosCopy.z = v_pos.z
											mobPosCopy.x = v_pos.x
											mobPosCopy.y = v_pos.y
											newPosFound = 2
											if logLevel3 then
...
funcPrefix, newPosFound, mobRef, mobPosCopy, math.deg(ori.z), pcCell)
							end
							tes3.positionCell({reference = mobRef, position = mobPosCopy,
								orientation = ori, cell = pcCell})
						else
							if logLevel2 then
								mwse.log(
'%s: newPosFound = %s InPlayerCell mobRef = "%s", pos = %s, ori.z = %.02fs',
funcPrefix, newPosFound, mobRef, mobPosCopy, math.deg(ori.z))
							end
...
funcPrefix, newPosFound, notInPlayerCell, mobRef, v_pos, math.deg(ori.z), pcCell.editorname)
										...
										mobPosCopy.z = v_pos.z
										mobPosCopy.x, mobPosCopy.y = v_pos.x, v_pos.y
										if logLevel2 then
											mwse.log(
...
funcPrefix, newPosFound, notInPlayerCell, mobRef, v_pos, math.deg(ori.z))
						...
						if newPosFound == 0 then
							mobPosCopy.x, mobPosCopy.y, mobPosCopy.z =
								playerPos.x - (dist * sina[angleIndex]),
								playerPos.y - (dist * cosa[angleIndex]), playerPos.z

							if math.abs(mobilePlayer:getViewToPoint(mobPosCopy)) > 105 then
								-- only if at player shoulders
								if tes3.testLineOfSight({position1 = mobPosCopy, height1 = 120,
									position2 = playerPos:copy(), height2 = 96}) then
									-- in a clear place
									local from = mobRef.position
									from.x, from.y, from.z = mobPosCopy.x, mobPosCopy.y, mobPosCopy.z
									newPosFound = 1
								end
							end
						end
...
local moveTravelersRegistered = false

---@param on boolean
function L.setMoveTravelers(on)
	if moveTravelersRegistered
	or event.isRegistered('simulate', moveTravelers) then
		if on then
			return
		end
		moveTravelersRegistered = false
		event.unregister('simulate', moveTravelers)
		return
	end
	if on then
		moveTravelersRegistered = true
		event.register('simulate', moveTravelers)
	end
end

function L.startMoveTravelers()
	local tp = travelParams[travelType]
	followersInLine = math.max( math.min(numTravelers, tp.maxInLine), 1 )
	travelRadStep = math.rad(170 / followersInLine)
	if logLevel2 then
		mwse.log("%s: startMoveTravelers() numTravelers = %s, followersInLine = %s, tp.maxInLine = %s",
			modPrefix, numTravelers, followersInLine, tp.maxInLine)
	end
	movingTravelers = true
	L.setMoveTravelers(movingTravelers)
...
function L.stopMoveTravelers()
	if logLevel2 then
		mwse.log("%s: stopMoveTravelers()", modPrefix)
	end
	followersInLine = 5
	movingTravelers = false
	L.setMoveTravelers(movingTravelers)
	currMountRef = nil
	currMountHandle = nil
end
...
function L.resetTransparency(mob)
	mob.animationController.opacity = 1.0 -- simpler, faster test
--[[
...
---@param mob tes3mobileActor
function L.fixEncumbrance(mob)
--  mostly from Merlord's player encumbrance fix
	local ref = mob.reference
	local burden = tes3.getEffectMagnitude({reference = mob, effect = tes3.effect.burden})
	local feather = tes3.getEffectMagnitude({reference = mob, effect = tes3.effect.feather})
	local realWeight = burden - feather
	local inventory = ref.object.inventory
	if inventory then
		realWeight = realWeight + inventory:calculateWeight()
	end
	local currWeight = mob.encumbrance.currentRaw
	if math.abs(currWeight - realWeight) > 0.01 then
		if logLevel1 then
			mwse.log('%s: L.fixEncumbrance("%s") encumbrance reset to %s', modPrefix, ref.id, realWeight)
		end
		tes3.setStatistic({reference = mob, name = 'encumbrance', current = realWeight})
	end
end
...
function L.handsDown(mob)
	...
	if mob.castReady then
		---@cast mob tes3mobilePlayer
		mob.castReady = false
	end
end
...
function L.travelStop()
	...
	local behindPlayerPos = player.position - (player.forwardDirection * 72)
	...
	for lcMobRefId, t in pairs(travelers) do
		...
		and t.inv then
			...
			if mobRef then
				...
				-- ensure followers move to player cell
				tes3.positionCell({reference = mobRef, position = behindPlayerPos, orientation = pori, cell = pcell})
			end
		end
	end
...
---@param mobRef tes3reference
---@return boolean
local function isRefScenicTravelCreature(mobRef)
	if scenicTravelPrefixDict[mobRef.object.id:sub(1, 6):lower()] then
		return true
	end
	return false
end
...
---@param mob tes3mobileActor
---@return boolean
local function isMobScenicTravelCreature(mob)
	if (mob.actorType == tes3_actorType_creature)
	and scenicTravelPrefixDict[mob.reference.object.id:sub(1, 6):lower()] then
		return true
	end
	return false
end
...
function L.timedTravelProcess()
	local funcPrefix = modPrefix..': timedTravelProcess()'
	if logLevel5 then
		mwse.log(funcPrefix)
	end
	if travelStopping then
		if logLevel1 then
			mwse.log('%s: travelStopping = %s, skip', funcPrefix, travelStopping)
		end
		return
	end
	local boDestValue = 0
	if ab01boDest then
		boDestValue = round0(ab01boDest.value)
		if logLevel4 then
			mwse.log('%s: boDestValue = %s', funcPrefix, boDestValue)
		end
	end
	local ssDestValue = 0
	if ab01ssDest then
		ssDestValue = round0(ab01ssDest.value)
		if logLevel4 then
			mwse.log('%s: ssDestValue = %s', funcPrefix, ssDestValue)
		end
	end
	local goDestValue = 0
	if ab01goDest then
		goDestValue = round0(ab01goDest.value)
		if logLevel4 then
			mwse.log('%s: goDestValue = %s', funcPrefix, goDestValue)
		end
	end
	...
	if ab01compMounted then
		if currMountRef then
			...
			if localVar then
				compMount = localVar
				if logLevel4 then
					mwse.log('%s: currMountRef = "%s", compMount = %s', funcPrefix, currMountRef, compMount)
				end
			end
		end
	end
	...
	if travelType == 1 then -- boat
		if (boDestValue <= 0)
		and (ab01boDestValue > 0) then
			stop = true
		end
	elseif travelType == 2 then -- strider
		if (ssDestValue <= 0)
		and (ab01ssDestValue > 0) then
			stop = true
		end
	elseif travelType == 3 then -- gondola
		if (goDestValue <= 0)
		and (ab01goDestValue > 0) then
			stop = true
		end
	...
	if (travelType == 0)
	and (not (boDestValue == ab01boDestValue)) then
		ab01boDestValue = boDestValue
		travelType = 1 -- boat
		startMove = true
	end
	if (travelType == 0)
	and (not (ssDestValue == ab01ssDestValue)) then
		ab01ssDestValue = ssDestValue
		travelType = 2 -- strider
		startMove = true
	end
	if (travelType == 0)
	and (not (goDestValue == ab01goDestValue)) then
		ab01goDestValue = goDestValue
		travelType = 3 -- gondola
		ab01goAngle.value = 10000 -- reset it
		startMove = true
	end
	if (travelType == 0)
	and ab01compMount
	...
	if logLevel3 then
		mwse.log( '%s: timedTravelProcess()', modPrefix)
	end

	---@param mob tes3mobileActor
	---@return number?, number?
	local function getSpreads(mob) -- out spread, nospread
		local spread, nospread
	...
	-- dummy function to override travelers script
	...
	for i = 1, #friendlyActors do
		...
		if logLevel3 then
			mwse.log( '%s: timedTravelProcess() "%s" actorType = %s',
				modPrefix, mob.reference, table.find(tes3.actorType, mob.actorType) )
		end
		if (mob.actorType == tes3_actorType_npc) then
			---assert(not (mob == tes3.mobilePlayer))
			if isValidScenicFollower(mob) then
				...
				local dist = mobRef.position:distance(player.position)
				if dist <= maxDist then
					local lcMobRefId = mobRef.id:lower()
					local traveler = travelers[lcMobRefId]
					local addIt = false
					if traveler then
						if traveler.inv then
							numTravelers = numTravelers + 1
							if travelType == 4 then
					...
					if addIt then
						...
						---aiWander0(mobRef)
						---wanderInPlace(mobRef) -- test hoprfully it will work well without this
						if tns == 0 then
							local script = mobRef.object.script
							if script then
								if logLevel5 then
						...
						if logLevel2 then
							mwse.log("%s: %s, dist = %s added to travelers", modPrefix, mobRef.id, dist)
						end
						-- store invisibility, acrobatics, nospread of follower
						travelers[lcMobRefId] = {inv = mob.invisibility, acro = mob.acrobatics.current, ns = tns}
						numTravelers = numTravelers + 1
...
---@return boolean
local function canTransferLight(light)
	if light.canCarry then
		if (light.radius < 17)
		or (light.weight < 0.001)
		or (light.mesh:len() <= 0)
		or (
			light.script
			and (light.script.id:lower() == 'g7_inventory_scri')
		) then
			return false
		end
		return true
...
-- hopefully this will avoid those pesky WARNING: An unknown object type was identified with a virtual table address of
	...
	if script then
		local s = script.text
		if s
		and s:lower():find('onactivate', 1, true) then
			skip = true
...
---comment
---@param activatorRef tes3reference
...
-- local MWCAloaded = tes3.getFileExists("MWSE\\mods\\herbert100\\animated containers\\main.lua")
	-- or tes3.getFileExists("MWSE\\mods\\MWCA\\main.lua")
-- local GHerbLoaded = tes3.getFileExists("MWSE\\mods\\graphicHerbalism\\main.lua")

-- set in initialized()
local MWCAloaded, GHerbLoaded ---@type boolean?
...
---@param contRef tes3reference
---@param index 0|1|2
---@return boolean
local function updateHerbalismSwitch(contRef, index)
	if logLevel2 then
		mwse.log('%s: updateHerbalismSwitch("%s", %s)', modPrefix, contRef, index)
	end
	...
	if logLevel3 then
		mwse.log('%s: updateHerbalismSwitch("%s", %s) contRef.data.GH = %s)',
			modPrefix, contRef, index, contRef.data.GH)
	end
	return true
end
...
[["Hey, I've found some %s! Would you like one?"]],
...
---@param targetRef tes3reference
function L.companionLootContainer(companionRef, targetRef)
	if logLevel3 then
	...
	if checkCloneContainer(targetRef) then
		targetObj = targetRef.object
		inventory = targetObj.inventory
	end

	if targetRef.isEmpty then
		return
	end
	...
	local function processStack(i)
		...
		--[[
		if stackObj.id:lower() == 'gold_001' then
			weight = 0.0001
	...
	if lootedCount > 0 then
		...
		local transferCount = #items2transfer
		local targetMob = targetRef.mobile
		for i = 1, transferCount do
			local stack = items2transfer[i]
			...
			and targetMob then
				local e = {
					...
					mobile = targetMob,
					reference = targetRef
				} ---@type unequippedEventData
				event.trigger('unequipped', e) -- mostly to update weaoon sheathing
			end
		end
	...
	if targetObj.objectType == CONT_T then
		...
		and ( not lcId:multifind({'chest','supply'}, 1, true) ) then -- skip guild chests
			local gHerb = false
			...
			if empty then
				if gHerb then
					...
					if (not targetRef.disabled)
					and targetObj.respawns then
						targetRef:delete()
					end
				end
			end

		elseif MWCAloaded then -- Morrowind Containers Animated or alternative loaded
			---mwse.log(">MWCAloaded")
...
local function getThiefTool(actorRef, objectType)
	local function decCondition(obj, iData)
		local condition = iData.condition
		if logLevel2 then
			mwse.log('%s: getThiefTool("%s", "%s") obj = "%s" uses = %s)',
				modPrefix, actorRef,  mwse.longToString(objectType), obj.id, condition)
		end
		iData.condition = condition - 1
	end
	...
	if inventory then
		...
		for i = 1, #items do
			...
			and obj.name
			and ( not obj.name:lower():multifind({'compass','sextant'}, 1, true) ) then
				local variables = stack.variables
				if not variables then -- pristine
					local iData = tes3.addItemData({to = actorRef, item = obj})
					decCondition(obj, iData)
					return obj
				end
				for _, iData in ipairs(variables) do
					if iData then
						local condition = iData.condition
						if condition then
							if condition > 0 then
...
---@param mob tes3mobileNPC|tes3mobileCreature
---@param seconds number
function L.sneakForSec(mob, seconds)
...
---@param companionRef tes3reference
---@param targetRef tes3reference
function L.tryCompanionThievery(companionRef, targetRef)
	if logLevel2 then
	...
	if unlocked then
		...
		if objType == CONT_T then
			if config.allowLooting > 0 then
				setSkipPlayerAltActivate()
				L.companionLootContainer(companionRef, targetRef)
			end
...
function L.takeItem(destActorRef, targetRef)
	local obj = targetRef.baseObject
...
function L.companionActivate(targetRef)
	...
	for i = 1, #friendlyActors do
		local mob = friendlyActors[i]
		if not isMobilePlayer(mob) then
			---assert(mob)
	...
	and (maxCapacity <= weight) then
		...
		elseif overburdenAllowed then
			...
			elseif i > 75 then
				tes3ui.showNotifyMenu("%s is overburdened by the %s.", companionName, inam)
			end
		end
	end
	if logLevel3 then
	...
	and targetMob.actorType then
		if config.allowLooting > 0 then
			L.companionLootContainer(lastCompanionRef, targetRef)
		end
		return
	end
	...
	if weight then
		if config.allowLooting > 0 then
			if lootType == T3OT.book then -- special case for books
				L.takeItem(lastCompanionRef, targetRef) -- add one copy to inventory and delete the in-world original
	...
	if ( (lootType == CONT_T)
	or (lootType == DOOR_T) ) then
		L.tryCompanionThievery(lastCompanionRef, targetRef)
	end
end


function L.checkCompanionActivate(targetRef)
	if logLevel2 then
...
function L.lightLoopSoundFix(mobileCrea)
	...
	if tes3.transferInventory({from = mobRef, to = persContRef, filter = onlyLights,
		...
		timer.start({duration = 0.75, type = timer.real, callback = 'ab01smcompPT5',
			data = {handle = tes3.makeSafeObjectHandle(mobRef)}
		})
		-- timer.start({duration = 0.5, type = timer.real, callback = 'ab01smcompPT5',
			-- data = {handle = tes3.makeSafeObjectHandle(mobRef)}
		-- })
		return
	end
...
local attackOrHelloDict = table.invert({'Attack', 'Hello'})
...
local function infoFilter(e)
	...
	if tes3.menuMode() then
		if logLevel5 then
			mwse.log('%s: infoFilter("%s") menuMode, skip', modPrefix, refId)
		end
		return
	end
	local dialogueId = e.dialogue.id
	if not attackOrHelloDict[dialogueId] then
		return
	end
	if logLevel4 then
		mwse.log('%s: infoFilter("%s") dialogueId = %s', modPrefix, refId, dialogueId)
	end
	if muteCC == 0 then
		return
	end
		...
		if c > 0 then
			if logLevel3 then
				mwse.log('%s: infoFilter("%s") %s muteCC == 1 filteredDict["%s"] = %s',
					modPrefix, refId, dialogueId, refId, c)
			end
			c = c - 1
			if c <= 0 then
...
---@param mob tes3mobileActor
---@param targetActor tes3mobileActor
---@return boolean
...
local function fixRefConstEnch(ref)
	...
	for i = 1, #items do
		...
		if enchantment
		and (enchantment.castType == tes3_enchantmentType_constant)
		and (refObj:hasItemEquipped(item)) then
			count = count + 1
			t[count] = item
		end
	end
	...
	for i = 1, count do
		...
		if logLevel3 then
			...
			modPrefix, ref.id, refObj.name, itm.id, itm.name)
		end
-- using pcall to try and avoid triggering Better Clothes warnings spamming the log
---@diagnostic disable-next-line: param-type-mismatch
		pcall( mob:unequip({item = itm}) )
-- replacement for mob:equip(params) that triggers the equip event
---@diagnostic disable-next-line: param-type-mismatch
		pcall( tes3.equip({reference = ref, item = itm, bypassEquipEvents = false}) )
	end
	ref:updateEquipment()
...
local function checkIdleFix(mob)
	...
	---or inCombat(mob)
	or isMobScenicTravelCreature(mob)
	or hasBlacklistedAnim(mob) then
		return
	end
	playGroupIdle(ref)
end
...
local function referenceDeactivated(e)
	if e.reference == lastTargetRef then
		lastTargetRef = nil
	end
end
...
function L.checkRemoveBurden(mob)
	local ref = mob.reference
	if logLevel5 then
		mwse.log('%s: checkRemoveBurden("%s")', modPrefix, ref)
	end
	if mob:isAffectedByObject(burdenSpell) then
		if logLevel3 then
			mwse.log('%s: checkRemoveBurden tes3.removeSpell({reference = "%s", spell = burdenSpell})', modPrefix, ref)
		end
		tes3.removeSpell({reference = ref, spell = burdenSpell})
		-- nope conflict with removeSpellEffects in local script
		-- tes3.removeEffects({reference = ref, effect = tes3.effect.burden})
	end
end

L.fixFatiguedDict = {['lack_qac_aaiona'] = true, ['lack_qac_aandren'] = true,
...
function L.activate(e)
	...
	if logLevel1 then
		mwse.log("%s activatorRef = %s, targetRef = %s", funcPrefix, activatorRef.id, targetRef.id)
	end

	-- never say never
	if isPlayer(targetRef) then
		return
	end

	local targetObj = targetRef.baseObject
	if isPlayer(activatorRef) then
		local targetMob = targetRef.mobile
		...
		and targetMob.actorType then
			...
			if not targetMobIsDead then
				...
				or (validFollower(targetMob, anyFollower) > 0) then
					if logLevel1 then
						mwse.log('%s valid follower "%s"', funcPrefix, targetObj.name)
					end
					...
					if config.encumbranceFixOnActivate then
						L.fixEncumbrance(targetMob)
					end
					if (targetMob.alarm > 0)
					and config.fixAlarm then
						targetMob.alarm = 0
					end
					if targetMob.actorType == tes3_actorType_npc then -- 0 = creature, 1 = NPC, 2 = player
						if logLevel1 then
							mwse.log('%s "%s".acrobatics.current = %s',
								funcPrefix, targetObj.name, targetMob.acrobatics.current)
						end
						if (targetMob.waterBreathing < 1)
						and config.fixWaterBreathing then
							targetMob.waterBreathing = 1
						end
						if (targetMob.acrobatics.current < 150)
						and config.fixAcrobatics then
							targetMob.acrobatics.current = 150
							if logLevel1 then
								mwse.log('%s "%s".acrobatics.current = %s',
									funcPrefix, targetObj.name, targetMob.acrobatics.current)
							end
						end
						if (targetMob.athletics.current < 220)
						and config.fixAthletics then
							targetMob.athletics.current = 220
						end
					end
					if targetMob.fatigue.current <= 0 then
						local skip = L.fixFatiguedDict[targetObj.id:lower()]
						if skip then
							...
							and (knockout == 0) then
								...
								if logLevel1 then
									mwse.log('%s resetting "%s" fatigue', funcPrefix, targetObj.name)
								end
							end
						end
						if not skip then
			...
			if targetMobIsDead then
				if isLootable(targetObj) then
					timer.start({ duration = 0.1,
						callback = function ()
							L.checkCompanionActivate(targetRef)
						end
					})
		...
		local function getValidObjLootType(obj)
			...
			if logLevel3 then
				mwse.log("%s obj.objectType = %s", funcPrefix, obj.id, mwse.longToString(ot))
			end
	...
	end -- if isPlayer(activatorRef)


	-- not isPlayer(activatorRef) below
	...
	local activatorMob = activatorRef.mobile
	if not activatorMob
	or (not activatorMob.canAct)
-- drawing/sheathing their weapon, attacking, casting magic or using something
	or inCombat(activatorMob) then
		return
	end
...
function L.combatStopped(e)
	...
	local playerStoppedCombat = isMobilePlayer(actorMob)
	...
	for i = 1, #friendlyActors do
		...
		if not isMobilePlayer(mob) then
			...
			elseif actorRef == mob.reference then
				if fixConstEnch then
					if assert( not isPlayer(actorRef) ) then
...
function L.isMountedAbotGuar(mob)
	...
	local ab01compMount = getRefVariable(ref, 'ab01compMount')
	if ab01compMount
	and (ab01compMount == 3) then
		return true
	end
...
local function combatStart(e)
	...
	if L.isDummy(attackerMob.reference)
	or L.isDummy(targetMob.reference) then
		return false
	end
end
...
local function combatStarted(e)
	...
	if attackerMob == mobilePlayer then
		return
	end
	if (config.autoAttack == 0)
	and (not config.autoBurden) then
	---and (not config.fixConstEnch) then
		return
	end
	local friendlyActors = mobilePlayer.friendlyActors
	...
	if attackerIsFollower then
		if logLevel3 then
			mwse.log('%s: combatStarted() follower "%s" attacking("%s"), skip',
			...
			if valid > 0 then
				if valid == 2 then -- companion
					...
					and (not L.isMountedAbotGuar(mob)) then
						if logLevel1 then
							mwse.log('%s: combatStarted() "%s":startCombat("%s") attacker',
								modPrefix, mob.reference.id, attackerMob.reference.id)
						end
...
function L.save()
	...
	player_data.ab01travelers = travelers
	player_data.ab01warpers = warpers

	local function getTable3FromVec3(v)
		return {round0(v.x), round0(v.y), round0(v.z)}
	end
...
function L.autoMove()
	...
	local mobCell
	filteredDict = {}
	---timer.start({duration = 2, type = timer.real,
	timer.start({duration = 3, type = timer.real, -- test
		callback = 'ab01smcompPT6'}) -- reset filteredDict to nil

	for i = 1, #friendlyActors do
		local mob = friendlyActors[i]
		---local valid = validFollower(mob, anyFollower, true)
		local valid = validFollower(mob, anyFollower)
		if valid > 0 then
			local ref = mob.reference
			mobCell = ref.cell
			ok = true
			...
			if ok then
				count = count + 1
				moving[count] = mob
				if filteredDict then
					c = filteredDict[ref.id]
					if c then
						c = c + 1
					else
						c = 1
					end
					filteredDict[ref.id] = c
				end
			end
		end
	end
	...
	for i = 1, count do
		...
		if warp2player then
			...
			destPos = ref.position:copy()
		end
		if autoMoveCC then
			size = mob.boundSize.y * ref.scale
		...
		if logLevel3 then
			mwse.log('%s: autoMove() tes3.positionCell({reference = "%s", cell = "%s", position = %s})',
				modPrefix, ref.id, pcCell.editorName, destPos)
		end
		--- aiWander0(ref) --  no better to keep follow package
		playGroupIdle(ref)
		tes3.positionCell({reference = ref, cell = pcCell,
		...
		right = not right
		--- wanderInPlace(ref) -- no better to keep follow package
	end
end

function L.cellChanged(e)
	---refsById = {}
...
function L.initOnce()
	...
	event.register('cellChanged', L.cellChanged)
	event.register('combatStopped', L.combatStopped)
...
function L.loaded()
	...
	---assert(inputController == worldController.inputController)
	...
	local player_data = player.data
	player_data.ab01travelType = travelType
	local tv = player_data.ab01travelers
	if tv then
		travelers = tv
		L.packTravelers()
	else
	...
	if ab01travelType then
		...
		if (travelType > 0)
		and (numTravelers > 0) then
			if logLevel2 then
				mwse.log("%s: travelType = %s, numTravelers = %s", funcPrefix, travelType, numTravelers)
			end
			L.startMoveTravelers()
		end
	end

	function L.startWarpFollowers()
...
function L.modConfigReady()
	...
	local function onClose()
		...
		and NPCVoiceDistance then -- NPCVoiceDistance = 750 by default
	-- increase NPCVoiceDistance accordingly to avoid annoying "Hey! Wait for me"
			local v = round0(config.warpDistance * 1.5)
			if NPCVoiceDistance.value < v then
				NPCVoiceDistance.value = v
			end
		end
		L.updateFromConfig()
		L.checkCombatRegistering()
	...
	sideBarPage:createDropdown({
		...
		options = getOptions(),
		description = [[When enabled, Alt + activate to make your companions try and loot the target item/container/corpse
(with/without potential overburdening).]],
		configKey = 'allowLooting'
	})

	sideBarPage:createSlider({
		label = 'Minimum item Value/Weight ratio',
		description = [[Minimum Value/Weight ratio for an item to be taken by a companion when looting a container.]],
		min = 1, max = 100, step = 1, jump = 5,
		configKey = 'minValueWeightRatio'
	})

	sideBarPage:createYesNoButton({
		label = 'Always loot from organic containers',
		description = [[When enabled, companion NPCs will always loot from organic containers
(e.g. plants) regardless of the Minimum item Value/Weight ratio setting.]],
		configKey = 'alwaysLootOrganic'
	})

	sideBarPage:createSlider({
		label = 'Max Loot Distance',
		description = [[Maximum distance for an item to be activated by some companion.]],
	...
	sideBarPage:createDropdown({
		...
		options = getOptions(),
		description = [[Try and reset actors AI/animation/pose when activating them.
...
Note: if you have >= 2 followers and you make one hostile, the other one will usually still attack you by default.
	...
	sideBarPage:createDropdown({
		...
		options = getOptions(),
		description = [[No harmful magical friendly fire from/to selected actors.]] .. ffInfo,
		configKey = 'noFriendlyFireMagic'
	})

	sideBarPage:createDropdown({
		label = 'No (non-magic) friendly fire:',
		options = getOptions(),
		description = [[No (non magical) health damaging friendly fire from/to selected actors.]] .. ffInfo,
		configKey = 'noFriendlyFireDamage'
	})

	sideBarPage:createYesNoButton({
		label = 'Allow player friendly fire soultrapping',
		description = [[Allow player damage to non-companion friendly targets affected by soultrap.
Only effective with some previous friendly fire option enabled.]],
		configKey = 'friendlyFireSoulTrapping'
	})
	...
	sideBarPage:createDropdown({
		...
		options = getOptions(),
		description = [[Alternative to "Diligent Defenders" mod.
If enabled, it should override it having higher priority and delaying the event to next frame.]],
		configKey = 'autoAttack'
	})

	sideBarPage:createDropdown({
		label = 'Warp to player:',
		options = getOptions(),
		description = [[Warp valid companions/followers (mostly) at player shoulders if distance from player is more than Max Warp Distance.
This option is meant as a (better) alternative to "Easy Escort" mod.
...
warping control from this MWSE-Lua mod does not override warping control from the companion vanilla script if present, they coexist, so results may vary.]],
	...
	sideBarPage:createDropdown({
		...
		options = getOptions(),
		description = [[Selected actors levitate like player when warping.]],
		configKey = 'warpLevitate'
	})

	sideBarPage:createDropdown({
		label = 'Automatic Water Walk when warping:',
		options = getOptions(),
		description = [[Selected actors water walk like player when warping.]],
		configKey = 'warpWaterWalk'
	})

	sideBarPage:createSlider({
		label = 'Max Warp Distance',
		description = [[Maximum distance before triggering warp to player]],
		min = 512, max = 7200, step = 1, jump = 5,
		configKey = 'warpDistance'
	})

	sideBarPage:createYesNoButton({
		label = 'Warp fighting companions',
		description = [[When warping is enabled, this option should make them warp to player even during fights.
You can enable this if you want to avoid companions chasing enemies too far, or if they are in a big danger.]],
		configKey = 'warpFightingCompanions'
	})

	sideBarPage:createYesNoButton({
		label = 'Automove followers on cell change',
		description = [[Automove any followers around the player e.g. after using loading doors, after warping to a different cell so they overlap less with the player.]],
		configKey = 'autoMoveCC'
	})
	...
	sideBarPage:createDropdown({
		...
		options = getOptions(),
		description = [[Briefly disable Attack/Hello voices on cell change. Especially useful when you recall to crowded taverns. More noticeable if you play with subtitles enabled.]],
		configKey = 'muteCC'
	})

	sideBarPage:createYesNoButton({
		label = 'Tweak followers AI Scan Interval',
		description = [[Toggle to enable/disable followers AI Scan Interval tweaking below.]],
		configKey = 'scanIntervalTweak'
	})

	scanIntervalSlider = sideBarPage:createSlider({
		label = 'Min followers AI Scan Interval',
		description = [[Minimum AI Scan Interval for followers. Lower should make followers more reactive but could be more CPU intensive.
Increase it in case of problems.
Effective only when "Tweak followers AI Scan Interval" option is enabled.
Note: the real minimum scan interval used by the game seems to be AI Scan Interval + 1 so even at 0 setting actors will often still take half a second on average to react.]],
		min = 0, max = maxScanInterval, decimalPlaces = 2, step = 0.05, jump = 1,
	...
	sideBarPage:createYesNoButton({
		...
		description = [[Ignore attacks from creatures/npcs emulating mannequins/targets/practice dummies.
This way player followers should not attack them any more.]],
		configKey = 'ignoreMannequinAttacks'
	})

	sideBarPage:createYesNoButton({
		label = 'Auto burden followers on combat',
		description = [[Autoburden non-companion followers on combat start, so they are not running after enemies.
N.B.:
- it does not work when you set "Auto attack enemies" option to "2. Yes, Any follower".
- it can make escort mission too easy.]],
		configKey = 'autoBurden'
	})

	sideBarPage:createYesNoButton({
		label = 'Allow Companions to use Probes',
		description = [[When enabled, companion NPCs are allowed to use their stats,
...
Probes with no uses left should be automatically dropped.]],
		configKey = 'allowProbes'
	})

	sideBarPage:createYesNoButton({
		label = 'Allow Companions to use Lockpicks',
		description = [[When enabled, companion NPCs are allowed to use their stats, skill and lockpicks
to try and open the target you Alt + activate.
lockpicks with no uses left should be automatically dropped.]],
		configKey = 'allowLockpicks'
	})

	sideBarPage:createYesNoButton({
		label = 'Allow Companions to use Open spells',
		description = [[When enabled, companion NPCs are allowed to use their stats, skill and spells
to try and open the target you Alt + activate.]],
		configKey = 'allowMagic'
	})

	sideBarPage:createYesNoButton({
		label = 'Followers 0 Alarm',
		description = [[When enabled, followers are given 0 alarm on activate if not already having it.
Useful to avoid annoying "Thief!" messages.]],
		configKey = 'fixAlarm'
	})
	sideBarPage:createYesNoButton({
		label = 'Followers constant enchantment items fix',
		description = [[When enabled, followers will unequip/reequip constant enchantment items on cell change, hopefully fixing constant enchantment effects overflow bug.]],
		configKey = 'fixConstEnch'
	})
	sideBarPage:createYesNoButton({
		label = 'Follower NPCs Water Breathing',
		description = [[When enabled, follower NPCs are given water breathing on activate if not already having it.
Useful to avoid them getting drowned.]],
		configKey = 'fixWaterBreathing'
	})
	sideBarPage:createYesNoButton({
		label = 'Follower NPCs High Acrobatics',
		description = [[When enabled, follower NPCs are given high acrobatics on activate if not already having it.
Useful to avoid them getting damaged on jumping or teleporting.]],
		configKey = 'fixAcrobatics'
	})
	sideBarPage:createYesNoButton({
		label = 'Follower NPCs High Athletics',
		description = [[When enabled, follower NPCs are given high athletics on activate if not already having it.
Useful to better adapt follower animation speed to player speed.]],
		configKey = 'fixAthletics'
	})
	sideBarPage:createYesNoButton({
		label = 'Followers clamp levitate',
		description = [[When enabled, followers constant levitate abilities/curses are clamped to min/max value = 5.
This should help avoiding possible levitate overflow to negative value on cell changes.]],
		configKey = 'clampLevitate'
	})

	sideBarPage:createYesNoButton({
		label = 'Skip follower activation while sneaking/unconscious',
		description = [[When enabled, you will not be able to activate a follower while you are sneaking or while the follower is unconscious,
avoiding the risk to trigger a pickpocket attempt crime reaction.]],
		configKey = 'skipActivatingFollowerWhileSneaking'
	})

	sideBarPage:createYesNoButton({
		label = 'Fix follower transparency on activate',
		description = [[When enabled, try and fix follower transparency on activate.
You can try it if a follower keeps staying transparent without apparent reason.]],
		configKey = 'transparencyFixOnActivate'
	})
	sideBarPage:createYesNoButton({
		label = 'Fix follower encumbrance on activate',
		description = [[When enabled, try and fix follower encumbrance on activate.
You can try it if a follower looks encumbered when it should not be.]],
		configKey = 'encumbranceFixOnActivate'
	})
	sideBarPage:createYesNoButton({
		label = 'Looping actor animations fix',
		description = [[When enabled, try and fix improperly looping actor animations.]],
		configKey = 'fixAnim'
	})
...
event.register('initialized', function ()
	...
	MWCAloaded = tes3.isLuaModActive('herbert100.animated containers')
		or tes3.isLuaModActive('MWCA')
	GHerbLoaded = tes3.isLuaModActive('graphicHerbalism')

	worldController = tes3.worldController
	inputController = worldController.inputController

	improvedAnimationSupport = tes3.hasCodePatchFeature(
		tes3.codePatchFeature.improvedAnimationSupport)

	---@param spellId string
	---@param spellName string
	---@param spellEffects table
	---@return tes3spell
	...
	if logLevel2
	and ab01ssDest then
		mwse.log('%s ab01ssDest.value = %s', funcPrefix, ab01ssDest.value)
	end

	ab01boDest = tes3.findGlobal('ab01boDest')
	if logLevel2
	and ab01boDest then
		mwse.log('%s ab01boDest.value = %s', funcPrefix, ab01boDest.value)
	end

	ab01goDest = tes3.findGlobal('ab01goDest')
	if logLevel2
	and ab01goDest then
		mwse.log('%s ab01goDest.value = %s', funcPrefix, ab01goDest.value)
	end

	ab01goAngle = tes3.findGlobal('ab01goAngle')

	ab01compMounted = tes3.findGlobal('ab01compMounted')
	if logLevel2
	and ab01compMounted then
		mwse.log('%s ab01compMounted = %s', funcPrefix, ab01compMounted)
	end

	NPCVoiceDistance = tes3.findGlobal('NPCVoiceDistance')
