...
AIfixOnActivate = 0, -- 0 = No, 1 = Companions, 2 = All followers, 3 = All actors
...
local function validFollower(mob, anyFollower, checkWarpFightingCompanions)
	...
	if (ai == tes3_aiPackage_follow)
	or (ai == tes3_aiPackage_none) then
...
local function warpFollowers()
	...
	for j = 1, #friendlyActors do
		...
		if valid > 0 then
			...
			validFollowers[numFollowers] = {m = mob, v = valid}
				...
				if not movingTravellers then
					if valid >= warpLevitate then
						if not mob:isAffectedByObject(levitateSpell) then
							if not isAffectedByEffect(mob, tes3_effect_levitate) then
...
local filtered = {} -- dictionary of actor ids to have filtered out attcak/hello voices

local function infoFilter(e)
	if not e.passes then
		return -- early skip when not needed
	end
	if not (e.dialogue.type == tes3_dialogueType_voice) then
	...
	mwse.log('%s: infoFilter("%s") [%s]:%s"%s"', modPrefix, refId, dialogueId, spacing, s)
...
local function combatFix(mob)
	filtered = {mob.reference.id}
	setInfoFilter(true) -- disable attack/greeting voices/subtitles. ab01smcoPT6 will re-enable them
	timer.start({duration = 0.5, type = timer.real, callback = 'ab01smcoPT6'})
	if logLevel3 then
		local refId = mob.reference.id
	...
	timer.start({duration = 0.1, type = timer.real, callback =
		function ()
			...
			if not mob.isFlying then
				timer.start({duration = 0.1, type = timer.real, callback = function () mob.reference:enable() end})
