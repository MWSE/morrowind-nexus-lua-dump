...
-- bah, remeber this silly language only supports 200 local variables in the main
...
-- not to get/set its value!!!
	...
	if not (success and value) then
		if logLevel4 then
			mwse.log('%s: "%s" %s, %s = script_context["%s"]', funcPrefix, ref.id, value, variableId)
		end
		return nil
	end
	...
	elseif ai == tes3.aiPackage.wander then
		...
		if mobIsCompanion then
			...
			if not oneTimeMove then
				oneTimeMove = getRefVariable(mobRef, 'c_move') -- used by Striders Nest companions scripts
			end
			if not oneTimeMove then
...
local function clampMobLevitate(mob)
	local lcMobId = string.lower(mob.reference.baseObject.id)
	if clampedLevitateMobs[lcMobId] then
		return
	end
	...
	clampedLevitateMobs[lcMobId] = true
end
						...
						elseif mob.levitate <= 0 then  -- try and fix weird case <= 0 too
							if logLevel1 then
								if mob.levitate < 0 then  -- try and fix weird case <= 0 too
									mwse.log('%s: warning "%s".levitate = %s, setting it to 1', modPrefix, lcMobRefId, mob.levitate)
								end
							end
							mob.levitate = 1
						end
				...
				---or mobIsCreature then
					if valid >= config.warpWaterWalk then
						if mob.waterWalking <= 0 then -- try and fix weird case < 0 too
							if not isAffectedByEffect(mob, tes3.effect.waterWalking) then
