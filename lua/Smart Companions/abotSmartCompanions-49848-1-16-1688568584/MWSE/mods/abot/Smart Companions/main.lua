...
local function getLocalVariable(ref, variableId)
	...
	if context then
		result = context[variableId] -- e.g. context['oneTimeMove']
		if not result then
			result = context[string.lower(variableId)] -- e.g. context['onetimemove']
			if not result then
				result = context[string.gsub(variableId, "^%l", string.upper)] -- e.g. context['OneTimeMove']
			end
		end
	end
	if logLevel4 then
		mwse.log('%s: getLocalVariable("%s", "%s") = %s', modPrefix, ref.id, variableId, result)
	end
	return result
end
...
local function getOneTimeMove(ref)
	return getLocalVariable(ref, 'oneTimeMove')
end
...
-- e.g. travellers[id] = {mob = mob, inv = mob.invisibility,
-- acro = mob.acrobatics.current, ns = tns} -- store mob, invisibility, acrobatics
local travellers = {}
...
local function alignToPlayerZ(mobRef, downOnly)
	...
	local dz = player.position.z - mobPosZ
...
local tes3_spellType_ability = tes3.spellType.ability
local tes3_spellType_curse = tes3.spellType.curse
...
local function clampMobLevitate(mob)
	...
	local spells = tes3.getSpells({target = mob, spellType = tes3_spellType_ability, getRaceSpells = false, getBirthsignSpells = false })
	for i = 1, #spells do
		clampSpellEffect(spells[i], tes3_effect_levitate, 5)
	end
	spells = tes3.getSpells({target = mob, spellType = tes3_spellType_curse, getRaceSpells = false, getBirthsignSpells = false })
	for i = 1, #spells do
		clampSpellEffect(spells[i], tes3_effect_levitate, 5)
	end
...
local hugeNegative = math.nfhuge * 0.5
...
local function warpFollowers()
	...
	for j = 1, #friendlyActors do
		...
		if valid > 0 then
			k = k + 1
			validFollowers[k] = {m = mob, v = valid}
...
---@diagnostic disable-next-line: assign-type-mismatch
		...
		if ok then
			...
			if playerLevitate > 0 then
				if not movingTravellers then
					if valid >= warpLevitate then
						if not mob:isAffectedByObject(levitateSpell) then
							if not mob:getActiveMagicEffects({effect = tes3_effect_levitate}) then
								tes3.addSpell({reference = mobRef, spell = levitateSpell})
							end
						end
						if mob.levitate <= 0 then  -- try and fix weird case <= 0 too
							mwse.log('warning "%s".levitate = %s, setting it to 1', mobId, mob.levitate)
							mob.levitate = 1
						end
				...
				if mob:isAffectedByObject(levitateSpell)
				or mob:getActiveMagicEffects({effect = tes3_effect_levitate})
				or (not mob.levitate == 0) then
					if mobilePlayer.lastGroundZ < hugeNegative then -- player on the ground
						if mob.lastGroundZ > hugeNegative then -- npc not on the ground
				...
				if mobIsEncumbered then
					alignToPlayerZ(mobRef, true) -- only align down if encumbered
				end
			end
...
-- comparison with standard 1 is important as it may be used as flag with different walues e.g. in guar mod
				if valid >= warpWaterWalk then
					if mob.waterWalking <= 0 then -- try and fix weird case < 0 too
						if not mob:getActiveMagicEffects({effect = tes3_effect_waterWalking}) then
...
local function timedTravelProcess()
			...
			if isValidScenicFollower(mob) then
				...
				if dist <= maxDist then
					...
					or (not traveller.mobile) then
						...
						-- store mob, invisibility, acrobatics, nospread of follower
						travellers[id] = {mob = mob, inv = mob.invisibility, acro = mob.acrobatics.current, ns = tns}
						numTravellers = numTravellers + 1
					end
				end
			end
	...
	if numTravellers > 0 then
		...
		for id2, t in pairs(travellers) do
			if t then -- needed safety
				mob1 = t.mob
				mobRef1 = mob1.reference
				if logLevel2 then
					mwse.log("%s: tes3.addSpell({reference = %s, spell = %s}), invisibility = 1, acrobatics = 200, blind = 100", modPrefix, id2, ability)
				end
				---mwscript.addSpell({reference = mob, spell = ability})
				tes3.addSpell({reference = mobRef1, spell = ability})
				...
				mobRef1.facing = player.facing
				tes3.playAnimation({reference = mobRef, group = tes3_animationGroup_idle}) -- reset animation
			end
		end
		startMoveTravellers()
	end
...
local function ab01smcoPT5(e)
	...
	for i = 1, #items do
		...
		if stack then
			...
			movedCount = tes3.transferItem({from = persContRef, to = ref, item = itm, itemData = stack.itemData,
...
local function combatFix(mob)
	...
	audioController.volumeVoice = 0.01 -- silence combat voices
	-- note: delay should be enough to silence also other companions attack sounds

	timer.start({duration = 1.3, type = timer.real, callback =
		function ()
			if volumeVoice > 0.01 then
				if logLevel3 then
				...
				or isValidScenicFollower(targetMob) then
					...
					if mobilePlayer.isSneaking then
						...
						if targetMob.fatigue.current <= 0 then
							if not skipFatigued[string.lower(targetObj.id)] then
...
local function checkCombatRegistering()
	...
	elseif combatStartOn then
		event.register('combatStarted', combatStart, combatEventSettings)
	end
...
local function loaded()
	...
	if ab01lastPlayerPositions then
		for i = 1, #ab01lastPlayerPositions do
			local v = ab01lastPlayerPositions[i]
...
local function cellChanged(e)
	...
	for i = 1, count do
		...
		mobRef = mob.reference
		size = mob.boundSize.y * mobRef.scale
		dist = (playerSize + size) * 0.5
		pos = mobRef.position
		pos.x = pos.x - (dist * math.sin(a))
		pos.y = pos.y - (dist * math.cos(a))
		mobRef.facing = player.facing
		if logLevel3 then
			mwse.log('"%s: cellChanged() "%s" moved to %s %s a = %s dist = %s',
				modPrefix, mobRef.id, pos.x, pos.y, math.deg(a), dist)
		end
		k = i * aStep
		if right then
			a = a - k
