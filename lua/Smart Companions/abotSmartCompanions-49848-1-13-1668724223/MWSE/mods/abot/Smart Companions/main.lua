...
local persContId = 'ab01smcoTempCont'
...
local function lightLoopSoundFix(mobileCrea)
	local ref = mobileCrea.reference
	if logLevel >= 2 then
		mwse.log('%s: lightLoopSoundFix("%s")', modPrefix, ref.id)
	end
	...
	for _, stack in pairs(ref.object.inventory) do
		if stack then
			...
			if itm.objectType == tes3_objectType_light then
				...
				movedCount = tes3.transferItem({from = ref, to = persContRef, item = itm, itemData = stack.itemData,
				...
				if logLevel >= 3 then
					mwse.log('%s: tes3.transferItem({from = "%s", to = "%s", item = "%s", count = %s}) = %s',
						modPrefix, ref.id, persContRef.id, itm.id, cnt, movedCount)
				end
			end
		end
	end
	if found then
		timer.start({duration = 0.3, type = timer.real,
			callback = function ()
				-- move lights back to creature inventory
				for _, stack in pairs(persContRef.object.inventory) do
					if stack then
						itm = stack.object
						cnt = stack.count
...
local function combatFix(mobile)
	...
	timer.start({duration = 1.1, callback = function ()
...
-- does it still work?
	...
	-- reset animation
	tes3.playAnimation{reference = mobile, group = tes3.animationGroup.idle}

	timer.start({duration = 1.5, callback = function ()
		combatFix(mobile) -- note to self: the delay is important!
	end})
