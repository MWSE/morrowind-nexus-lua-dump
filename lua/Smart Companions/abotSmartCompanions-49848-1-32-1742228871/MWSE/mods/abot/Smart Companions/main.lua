...
local function targetIsFriendlyWith(targetMob, attackerMob, 
	...
	if (
		...
		local result = true
		if companionOnly
		and ( not getCompanionVar(attackerMob.reference) ) then
			result = false
		end
		if logLevel2
		and result then
...
function L.cleanLastPlayerPositions()
 ...
 -- important!!! pay attention to make v.pos point to a copy of player.position,
 -- not directly to player.position else it will crash
				v.pos = nil
			end
			lastPlayerPositions[i] = nil
		end
