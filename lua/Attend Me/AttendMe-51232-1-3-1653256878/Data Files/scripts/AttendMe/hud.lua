...
I.Settings.registerGroup {
    ...
    settings = {
        ...
        },
        {
            ...
            description = "Remove dark background and outer borders",
            default = false,
            renderer = 'checkbox',
            permanentStorage = true,
        },
        ...
        {
            key = 'showCombatMode',
            name = "Show combat icon",
            description = "Show the follower's weapon or spell icon",
            default = true,
            renderer = 'checkbox',
            permanentStorage = true,
        },
        {
            ...
            name = "Only show the combat icon when in combat",
            default = true,
            renderer = 'checkbox',
            permanentStorage = true,
        },
...
local function renderFollower(actor)
    ...
    local shouldRenderCombat = hudSettings:get('showCombatMode') and (
        ...
        actor.type.stance(actor) ~= types.Actor.STANCE.Nothing
    )
    local bodyLayout = {
        type = ui.TYPE.Flex,
        props = {
...
return function(followers)
    local function updateFollowerList()
        ...
        followerList.props.horizontal = hudSettings:get('horizontal')

        local content = {}
        for _, follower in pairs(validFollowers) do
            table.insert(content, renderFollower(follower))
        end
        local lineTemplate = hudSettings:get('horizontal') and 'verticalLine' or 'horizontalLine'
        if not minimal then
            content = insertBetween(content, {
                external = {
                    stretch = 1.0,
                },
                props = {
