...
local DAMAGE_RAMP_DURATION = 15
local CAST_RAMP_DURATION = 15

-- OUT OF COMBAT BONUS
local OOC_RAMP_DURATION = 30
local OOC_MAX_MULT = 3.0

-- FATIGUE BREAKPOINT
local NEAR_FULL_FATIGUE_THRESHOLD = 0.999
local NEAR_FULL_FATIGUE_MULT = 0.8
...
-- OUT OF COMBAT RAMP
local function getOOCMultiplier(fatiguePct, dt, damageTimer, castTimer)
    -- Must be fully recovered AND not in combat
    if fatiguePct >= NEAR_FULL_FATIGUE_THRESHOLD and damageTimer <= 0 and castTimer <= 0 then
...
return {
    engineHandlers = {
        onUpdate = function(dt)
            ...
            local oocMult = getOOCMultiplier(fatiguePct, dt, damageTimer, castTimer)
