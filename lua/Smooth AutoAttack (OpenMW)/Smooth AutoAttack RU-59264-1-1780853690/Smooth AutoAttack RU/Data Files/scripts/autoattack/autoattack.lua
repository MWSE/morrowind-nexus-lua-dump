...
local function doToggleAutoAttack()
    ...
    setAutoOn(newState)
    local typeStr = (currentAttackType == 'Fastest') and 'Лучшая скорость' or 'Лучший урон'
    notify(newState and ('Автоатака ВКЛ  |  ' .. typeStr) or 'Автоатака ВЫКЛ')
end
...
local function doToggleAttackType()
    ...
    fastestTimer     = 0.0
    local typeStr = (currentAttackType == 'Fastest') and 'Лучшая скорость' or 'Лучший урон'
    notify('Тип атаки: ' .. typeStr .. '  |  Автоатака ' .. (isAutoOn() and 'ВКЛ' or 'ВЫКЛ'))
end
