...
--	snd_custom = "custom",
	snd_custom = "Fx\\envrn\\bell2.wav",
	snd_custom_2 = "Fx\\envrn\\bell2.wav",
	snd_custom_3 = "Fx\\envrn\\bell2.wav",
...
local fallback = {
	...
	snd_sky_quest = sounds.snd_mw_quest_1,
	snd_ob_quest = sounds.snd_mw_objective,
}
...
for k, v in pairs(sounds) do
	if v ~= "" and v ~= "custom" and v ~= "same" then
		...
		if not vfs.fileExists(path) then
			if fallback[k] then
				print("Using fallback sound file for "..k)
				sounds[k] = fallback[k]
			else
