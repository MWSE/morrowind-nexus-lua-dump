...
local tes3ui_getMenuOnTop, tes3ui_createTooltipMenu = tes3ui.getMenuOnTop, tes3ui.createTooltipMenu
...
local function keyDown(e)
	...
	local menu = tes3ui_getMenuOnTop()
	if not menu then
		return
	end
	...
	for el, code in pairs(accelerators) do
		if code == keyCode then
			...
			if ok then
				...
				-- nope crashing tes3ui.acquireTextInput() -- avoid spamming text inputs
				local el2 = getActiveTextInput()
				if el2 then
					el2.disabled = true
					timer.start({type = timer.real, duration = 0.084, callback = function ()
						local el3 = getActiveTextInput(true)
						if el3 then
							el3.disabled = false
						end
					end})
...
local function help(e)
	newTooltipBlock( tes3ui_createTooltipMenu(),
		e.source:getLuaData('ab01acclrtrsTip') )
end

---local idHelpMenu = tes3ui.registerID('HelpMenu')
local table_traverse = table.traverse
...
local function setAccelerators(menu)
	...
	local function validAccelerator(el, c, i)
		...
		and (not accCodes[keyCode]) then
			...
			and el.name:startswith('UIEXP:FilterButton') then
				...
				el:setLuaData('ab01acclrtrsTip', tip)
				el:register('help', help)
			end
			return true
		end
		return false
	...
	for el in table_traverse({menu}) do
...
-- skip
local menuBlackList = {'MenuMulti','MenuNotify1','MenuNotify2','MenuNotify3','MenuMessage'}
...
local function uiActivated(e)
	...
	if setAccelerators(menu) then
		...
		if not keyDownRegistered then
			...
			event.register('keyDown', keyDown, {priority = 1000000})
		end
	end
end
...
Note: If you are using the Cheat Menu mod, try to avoid using a possible Alt + accelerator combo for the Kill cheat commands! e.g. in the Cheat mod use something like Shift + Ctrl + PageUp imstead of Alt + K that would be a frequent button accelerator for this mod. Or you could add the K letter to this mod "Invalid Characters" text input.
	...
	textField = sideBarPage:createTextField({
		...
		description = [[Enter invalid alphanumeric characters you do not want to use as Alt accelerator.
