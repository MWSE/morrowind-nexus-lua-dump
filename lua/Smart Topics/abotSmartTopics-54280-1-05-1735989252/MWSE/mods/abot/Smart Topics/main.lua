...
local function initTopics()
	...
	for i = 1, #dialogues do
		local d = dialogues[i]
		if d
		and (d.type == tes3_dialogueType_topic)
		and (not knownTopicsDict[string.lower(d.id)]) then
			j = j + 1
...
If you set the Log Level Full or higher it will log to MWSE.log interesting data from dialogueFiltered() and infoResponse() events without needing the key combos, very useful to debug dialog, just remember to set the Logging Level back to Off for normal playing.
]]
})
