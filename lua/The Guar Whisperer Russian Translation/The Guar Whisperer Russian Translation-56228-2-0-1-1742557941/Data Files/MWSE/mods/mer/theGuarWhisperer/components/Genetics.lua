...
function Genetics:getGender()
    if not self.guar.refData.gender then
        self.guar.refData.gender = math.random() < 0.55 and "самец" or "самка"
    end
...
function Genetics:getCanConceive()
    ...
    if not ( self.guar.refData.gender == "самка" ) then return false end
...
function Genetics:canBeImpregnatedBy(guar)
    ...
    if not (guar.refData.gender == "самец" ) then return false end
...
function Genetics:breed()
    ...
    if #partnerList > 0 then
        ...
        table.insert( buttons, { text = "Отмена"})

        tes3ui.showMessageMenu{
            message = self.guar:format("С каким партнером вы хотели бы спарить {name}?"),
            buttons = buttons
        }
    else
        tes3.messageBox("Поблизости нет подходящих партнеров.")
    end
end
