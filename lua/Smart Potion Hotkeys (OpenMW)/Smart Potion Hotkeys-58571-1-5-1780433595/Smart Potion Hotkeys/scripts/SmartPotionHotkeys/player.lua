...
local function registerSmartPotionSuppressors()
    ...
    -- action, try to make SPH win for that frame. Do this only while the player
    -- is in world input: vanilla quick-key assignment and menu navigation must
    -- remain untouched. Some OpenMW builds expose only a subset of built-in
    -- actions as bindable string keys, so every binding here is guarded and
    -- non-fatal.
        ...
        if input.actions[actionKey] then
            local ok, err = pcall(function()
                input.bindAction(actionKey, async:callback(function(dt, vanillaValue, ...)
                    if I.UI.getMode() ~= nil then return vanillaValue end
...
-- Earlier builds tried to guard raw number-row/numpad presses here by mapping
-- physical digits directly to SPH slot numbers. That was too broad: vanilla
-- spell quick keys also use those same direct digits, and the guard restored the
-- previous selected spell even when the digit was not actually bound to SPH.
-- Guarding now happens only from useSmartPotion(), i.e. after an SPH action or
-- legacy trigger actually fires.
...
useSmartPotion = function(slotNum)
    -- Hotkeys should only consume potions during normal world input. OpenMW can
    -- still deliver Lua action/trigger callbacks while text fields, inventory,
    -- barter, dialogue, naming, or other menus are active; ignore those presses
    -- so SPH bindings remain safe to use as ordinary UI/text keys.
...
local function checkAutoUse()
    ...
    -- book reading, dialogue, naming, inventory, etc.
    if I.UI.getMode() ~= nil then return end
    if core.isWorldPaused() then return end
