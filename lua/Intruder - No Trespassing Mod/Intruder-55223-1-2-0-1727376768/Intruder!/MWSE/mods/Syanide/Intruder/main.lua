    ...
    elseif isMatchingCell(playerCell.id) and (currentTime >= config.startTrespass or currentTime <= config.endTrespass) then
        ...
        for _, mobile in ipairs(mobileList) do
            ...
            if actor ~= tes3.player and (not follower or actor ~= follower) then
                if tes3.testLineOfSight({ reference1 = actor, reference2 = tes3.player }) or (follower and tes3.testLineOfSight({ reference1 = actor, reference2 = follower })) or (detectedActors[actor] and detectedActors[actor].detected) then
