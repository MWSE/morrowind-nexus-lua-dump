...
function this.onCreate(parent)
	...
	local descriptionLabel = container:createLabel({ text = "Этот мод добавляет шкалу здоровья ваших компаньонов в игровой интерфейс игры.\nИногда персонажи, не являющиеся компаньонами, также могут получать индикатор здоровья. В этом случае их необходимо вручную внести в черный список. Все персонажи из черного списка не отображают индикатор здоровья." })
	...
	createConfigToggleButton(container, "Включить обновление Здоровья при изменении:", "enableEvents")
	createConfigToggleButton(container, "Показать шкалу Магии/Усталости", "fatigka")
	container:createDivider()
	...
	if (tes3.mobilePlayer) then
		...
		local label = activeListBox:createLabel({ text = "Список компаньонов:" })
		label.borderBottom = 6
