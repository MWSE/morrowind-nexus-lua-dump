...
if (config == nil) then
	...
	if type(config.fatigka) == "number" then
		config.fatigka = math.clamp(config.fatigka, 0, 1)
	else
		config.fatigka = 0
	end
...
event.register("menuExit", function()
    local sephConfig = mwse.loadConfig("seph.hudCustomizer", {})
    if not table.empty(sephConfig, true) then
        local custom = sephConfig.mods["CompanionHealthBars:Menu"]
        if custom then
            config.absolutePosAlignX = (custom.positionX and (custom.positionX / 1000)) or config.absolutePosAlignX
            config.absolutePosAlignY = (custom.positionY and (custom.positionY / 1000)) or config.absolutePosAlignY
        end
    end
end)
...
local function registerModConfig()
	mwse.registerModConfig("Шкала здоровья компаньонов", modConfig)
end
...
local function updateAllHealthBarValues()
	...
	for _, companionBar in pairs(companionHealthBars.healthBars) do
		...
		if config.fatigka == 0 then
			companionBar.barMenu.absolutePosAlignY = 0.5
		else
			companionBar.barMenu.absolutePosAlignY = 0
			companionBar.barMenu2.absolutePosAlignY = 0.5
			companionBar.barMenu3.absolutePosAlignY = 1
		end
		...
		if weapon and weapon.object and weapon.object.enchantment then
        	...
        	image1.width = 42
        	image1.height = 42
			local image2 = image1:createImage({path = "Icons\\"..weapon.object.icon})
		...
		elseif weapon and weapon.object then
			...
			image1.width = 42
			image1.height = 42
		elseif magic1 and magic1.object then
			local image1 = image:createImage({path = "Textures\\menu_icon_magic_mini.tga"})
        	image1.scaleMode = true
        	image1.width = 42
        	image1.height = 42
		...
		elseif magic2 then
			...
			local image2 = image:createImage({path = "Icons\\k\\Stealth_HandToHand.tga"})
			image2.scaleMode = true
			image2.width = 42
			image2.height = 42
		end
...
local function createNewHealthBars(companions)
	...
	for i = #companions, 1, -1 do
		...
		fatigueBarMenu.width = 100
		magickaBarMenu.width = 100
		healthBarMenu.width = 100
		fatigueBarMenu.height = 18
		magickaBarMenu.height = 18
		healthBarMenu.height = 18
		if config.fatigka == 0 then
			healthBarMenu.absolutePosAlignY = 0.5
		end
		...
		local image = block:createThinBorder():createRect({id = "CHB_SpaFork_images", color = {0,0,0}})
		image.parent.height = 42
		image.parent.width = 42
		image.parent.borderAllSides = 2
		image.parent.borderTop = 6
		image.parent.paddingAllSides = 1
		image.alpha = tes3.worldController.menuAlpha
...
---@diagnostic disable-next-line: need-check-nil
		seph:registerElement("CompanionHealthBars:Menu", "Шкала здоровья компаньонов", {positionX = config.absolutePosAlignX, positionY = config.absolutePosAlignY, visible = true}, {position = true, visibility = true})
	end
end)