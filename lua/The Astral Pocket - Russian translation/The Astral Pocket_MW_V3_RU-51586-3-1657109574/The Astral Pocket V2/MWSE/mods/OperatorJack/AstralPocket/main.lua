...
if (mwse.buildDate == nil) or (mwse.buildDate < 20190821) then
    local function warning()
        tes3.messageBox(
            "[Astral Pocket ERROR] Ваш MWSE устарел!"
            .. " Вам нужно обновить его до более новой версии, чтобы использовать этот мод."
        )
    end
...
if (framework == nil) then
    local function warning()
        tes3.messageBox(
            "[Astral Pocket ERROR] Magicka Expanded framework не установлен!"
            .. " Вам нужно установить его, чтобы использовать этот мод."
        )
    end
    event.register("initialized", warning)
    event.register("loaded", warning)
...
local function onTeleportToAstralPocketTick(e)
    ...
    if canTeleport then
		...
		tes3.messageBox("Вы не можете использовать это заклинание здесь.")
	end
...
local function addTeleportToAstralPocketEffect()
	framework.effects.mysticism.createBasicEffect({
		...
		name = "Телепорт в Астральный карман",
		description = "Телепортирует заклинателя в Астральный карман.",
...
local function readAstrologicalElements(e)
    ...
    if tes3.mobilePlayer.mysticism.current > 80 then
        ...
        tes3.messageBox("Вы пытаетесь выучить заклинание для телепортации в Астральный карман, но обнаруживаете, что недостаточно искусны в искусстве мистицизма.")
    end
end
