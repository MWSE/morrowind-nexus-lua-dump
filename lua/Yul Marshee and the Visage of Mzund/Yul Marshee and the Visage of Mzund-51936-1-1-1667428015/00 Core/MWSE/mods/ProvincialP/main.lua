...
function Button2PressedCallback(e)
    print(e.button)
    if (e.button == 0) then
        local ref = tes3.mobilePlayer
        ...
        if result then
            ...
            local pos = land.positionRef(ref, result, 0, 0, 405)
            tes3.createReference({ object = "PP_Airship_Stage_03", position = pos, orientation = orient, cell = tes3.player.cell, scale = 1 })
            tes3.createReference({ object = "PP_Airship_Door_MWSE", position = pos, orientation = orient, cell = tes3.player.cell, scale = 1 })
            ...
            tes3.player.scale = 1
            tes3.togglePOV()
            local pos = land.positionRef(ref, result, 250, 0, 20)
            tes3.mobilePlayer.position = pos
...
local function simul(t)
    ...
    elseif ( airdoorref.disabled ) then
        airdoorref:enable()
        airshipref:enable()
        oldshipref:disable()
        olddoorref:disable()
    ...
    Time = tes3.worldController.deltaTime
    ...
    if PlayerKey(tes3.keybind.back) then
        if IsShiftPressed then
           if (tes3.mobilePlayer.position.z > waterLevel ) then
            ...
            if HorSpeed > 0 then
                HorSpeed = math.max(0, HorSpeed - HorAcc * 2 * Time)
            end
        end
    end
    Drag(0.025)
    ...
    if tes3.getJournalIndex({id ="PP_q06"}) >= 200 then
        ...
        elseif (t.target.object.id == "PP_Airship_I_Door") then
            if (OriRef) then
                ...
                local sin = math.sin(facing)
                local cos = math.cos(facing)
                local pos = tes3vector3.new(OriRef.position.x + 200 * cos, OriRef.position.y + 200 * sin, OriRef.position.z - 450)
                tes3.positionCell({
