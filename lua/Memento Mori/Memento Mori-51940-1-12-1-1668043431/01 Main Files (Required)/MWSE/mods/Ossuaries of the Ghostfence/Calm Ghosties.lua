...
local function skellyShallNotPass()
    if not catacombsCells[tes3.player.cell.id] then return end
    for creatureRef in tes3.player.cell:iterateReferences(tes3.objectType
                                                              .creature) do
        if isCatacombsCreature[creatureRef.baseObject.id] then
            if creatureRef.mobile.isDead then return end
            if creatureRef.mobile.playerDistance > 4096 then return end
            local minDistance, minDoorRef =
                getDistanceOfClosestDoor(creatureRef.position)
            ...
            if minDistance < 160 then
                ...
                if minDoorRef and
                    minDoorRef.position:distance(tes3.player.mobile.position) >
                    160 then
                    log:debug("player is outside the door trigger area",
                              creatureRef.id)
                    for nearbyCreatureRef in
                        tes3.player.cell:iterateReferences(tes3.objectType
                                                               .creature) do
                        if nearbyCreatureRef.mobile.position:distance(
                            creatureRef.mobile.position) < 512 or
                            nearbyCreatureRef == creatureRef then
                            log:debug("distance between %s and %s is %s",
                                      creatureRef.id, nearbyCreatureRef.id,
                                      nearbyCreatureRef.mobile.position:distance(
                                          creatureRef.mobile.position))
                            if nearbyCreatureRef.leveledBaseReference then
                                log:debug("%s is a leveled spawn from %s",
                                          nearbyCreatureRef.id,
                                          nearbyCreatureRef.leveledBaseReference
                                              .id)
                                log:debug("%s previous position %s",
                                          nearbyCreatureRef.id,
                                          nearbyCreatureRef.position)
                                nearbyCreatureRef.position =
                                    nearbyCreatureRef.leveledBaseReference
                                        .position
                                log:debug("%s position repositioned %s",
                                          nearbyCreatureRef.id,
                                          nearbyCreatureRef.position)
                            end
                            minDoorRef.sceneNode:updateEffects()
                            minDoorRef.orientation =
                                minDoorRef.startingOrientation
                            nearbyCreatureRef.mobile:stopCombat(true)
                            if nearbyCreatureRef.mobile.inCombat then
                                log:debug("%s stop combat failed",
                                          nearbyCreatureRef.id)
                            end
                        end
                    end
