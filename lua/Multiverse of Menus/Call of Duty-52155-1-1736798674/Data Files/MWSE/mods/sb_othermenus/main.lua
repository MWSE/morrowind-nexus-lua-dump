...
local function uiMenuOptionsActivatedCallback(e)
    if (tes3.player == nil) then
        ...
        if (e.newlyCreated) then
            local logo = e.element:getContentElement():createImage{path = "Textures\\sb_othermenus\\logo.tga"}
            logo.imageScaleX = newScale
            logo.imageScaleY = newScale
            logo.positionX = 32 * newScale
            logo.positionY = -32 * newScale
        ...
        content.positionY = -240 * newScale
        ...
        for _, element in ipairs(content.children) do
            ...
            if (contentPath) then
                ...
                for i = 1, 3 do
                    element.children[i].width = 320
                    element.children[i].height = 800
                    element.children[i].imageScaleX = newScale
                    element.children[i].imageScaleY = newScale
                    if (i == 2) then
