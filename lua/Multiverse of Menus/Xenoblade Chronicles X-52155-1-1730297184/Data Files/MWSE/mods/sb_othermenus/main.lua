...
local function uiMenuOptionsActivatedCallback(e)
    if (e.newlyCreated and tes3.player == nil) then
        ...
        e.element.absolutePosAlignX = 1
        e.element.absolutePosAlignY = 0.9
        e.element.autoWidth = true
        e.element.autoHeight = true
        e.element:getContentElement().children[1].flowDirection = tes3.flowDirection.leftToRight
        ...
        for _, element in ipairs(e.element:getContentElement().children[1].children) do
            ...
            if (contentPath) then
                ...
                for i = 1, 3 do
                    element.children[i].width = (304 / iw) * w * 0.5
        ...
        local texture = niSourceTexture.createFromPath("Icons\\w\\tx_saber.dds")
        map.texture = texture
