...
local function uiMenuOptionsActivatedCallback(e)
    if (e.newlyCreated and tes3.player == nil) then
        local iw, ih = 1024, 512
        local w, h = tes3ui.getViewportSize()

        e.element.absolutePosAlignX = 1 - 32 / iw
        e.element.absolutePosAlignY = 1 - 32 / ih
        ...
        for _, element in ipairs(e.element:getContentElement().children[1].children) do
            ...
            if (contentPath) then
                ...
                -- element.borderTop = _ ~= 1 and (32 / iw * w * 0.66) or 0
                for i = 1, 3 do
                    element.children[i].width = (512 / iw) * w * 0.66
                    element.children[i].height = ((128 - 32) / ih) * h * 0.66
                    element.children[i].imageScaleX = (w / iw) * 0.66
                    element.children[i].imageScaleY = (h / ih) * 0.66
                    if (i == 2) then
