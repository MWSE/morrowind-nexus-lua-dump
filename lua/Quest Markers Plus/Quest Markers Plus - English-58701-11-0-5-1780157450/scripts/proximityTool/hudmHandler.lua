    ...
    for refId, dt in pairs(this.activeByObject) do
        for markerId, data in pairs(dt) do
            ...
            elseif not data.marker.hidden and not activeData[getHashVal(refId, markerId)] then
                ...
                local markerIconLower = data.marker.params.icon and data.marker.params.icon:lower()
                local isDoor = markerIconLower and markerIconLower:find("door")
                params.icon = isDoor and "textures/icons/skyrim_compass/quest_marker1.dds" or "textures/icons/skyrim_compass/quest_marker.dds"
                params.color = getQuestColor()
...
local function addMarkers(marker, ref)
    if (marker.version or 0) > this.version or marker.invalid
            or not marker.params.icon then return end

    local iconLower = marker.params.icon:lower()
    if iconLower:find("quest_guider") and iconLower:find("exclamation") then return end
    if not iconLower:find("quest_guider") and not vfs.fileExists(marker.params.icon) then return end
    ...
    if not marker.hidden then
        ...
        params.object = ref
        local isDoor = iconLower and iconLower:find("door")
        params.icon = isDoor and "textures/icons/skyrim_compass/quest_marker1.dds" or "textures/icons/skyrim_compass/quest_marker.dds"
        params.color = getQuestColor()
