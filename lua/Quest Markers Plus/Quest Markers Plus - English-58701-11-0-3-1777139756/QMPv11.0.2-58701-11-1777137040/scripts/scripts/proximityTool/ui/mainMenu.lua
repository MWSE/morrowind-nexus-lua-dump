        ...
        for i = #contentOwner.content, 1, -1 do
            ...
            if topMarkerRecord.record and topMarkerRecord.record.userData
                ...
                elseif not hide then
                    local isDoor = false
                    if trackedObj and trackedObj.cell then
                        isDoor = not trackedObj.cell.isExterior
                    end

                    if not isDoor and player.cell.isExterior and trackedObj then
                        local ok, isC = pcall(types.Container.objectIsInstance, trackedObj)
                        if ok and isC then
                            skipCompass = true
                        end
                    end
