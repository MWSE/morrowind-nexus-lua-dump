...
function this.create(params)
    ...
    base.props = {
        ...
        alpha = params.showBorder and (isMainHidden and 0 or 1) or 0,
...
function this.update(params)
            ...
            if disabled then
                if element.props.visible then
                    ...
                    if element.props.alpha <= 0 then
                        ...
                        if not element.userData.crossCellTracking then
    ...
    local function processGroup(contentOwner, parent)
        ...
        for i = #contentOwner.content, 1, -1 do
            ...
            if trackingData.hidden and not elem.props.visible and not elem.userData.crossCellTracking then
                goto continue
            end
            ...
            if trackingData.nextUpdate < timestamp or not trackingData.lastTrackedObject then
                ...
                elseif not next(trackingPositionsData) then
                    if not player.cell.isExterior and topMarkerRecord.record
                            and topMarkerRecord.record.userData
                            and topMarkerRecord.record.userData.type == "tracking" then
                        table.insert(compassMarkerData, {
                            name = topMarkerRecord.record.name or "",
                            crossCell = true,
                        })
                        elem.userData.disabled = true
                        elem.userData.crossCellTracking = true
                    else
                        uiUtils.removeFromContent(contentOwner.content, i)
                    end
                    doUpdate = true
            ...
            if topMarkerRecord.record and topMarkerRecord.record.userData
                    and topMarkerRecord.record.userData.type == "tracking" then
                local trackedObj = trackingData.lastTrackedObject
                local isCrossCell = not player.cell.isExterior
                        and trackedObj and trackedObj.cell
                        and trackedObj.cell ~= player.cell

                local skipCompass = false
                ...
                elseif not trackingData.hidden then
                    local isDoor = topMarkerRecord.record and topMarkerRecord.record.icon
                        and topMarkerRecord.record.icon:lower():find("door") and true or false

                    if not skipCompass then
                        table.insert(compassMarkerData, {
                            x = trackingPos.x,
                            y = trackingPos.y,
                            z = trackingPos.z,
                ...
                if not skipCompass then
                    elem.userData.crossCellTracking = isCrossCell or nil
                    elem.userData.disabled = true
                    doUpdate = true
                    doLayoutUpdate = true
                    goto continue
    ...
    playerObj:sendEvent("SkyrimCompass:setMarkers", compassMarkerData)
end
