...
adjustBookmarkWidth = false, -- adjust bookmark size to fit longer text
logLevel = 0, -- 0 = Low, 1 = missing quest name warnings, 2 = Medium, 3 = High
}
-- end tweakables
...
local function initQuests() -- called in loaded() as they do not seem to be yet ok in initialized/modConfigReady
	...
	quests = {}
	local disposition, text, questId, flags

	local missingQuestNames = {}

	local logLevelGT0 = config.logLevel > 0
	local logLevelGT4 = config.logLevel > 4
								...
								if string.len(text) > 0 then
									...
									if ( -- quest name marker flag, plus not relevant bit 0
									 ...
									 or (flags == 72)
										...
										if logLevelGT4 then
											mwse.log("%s: initQuests() quests['%s'] = {name = '%s', sourceMod = '%s'} ", modPrefix, questId, text, info.sourceMod)
										end
										quests[questId] = {name = text, sourceMod = info.sourceMod}
										break -- for info
									elseif logLevelGT0 then
										if not string.multifind(text:lower(), {'--','dummy'}, 1, true) then -- skip Antares', TR dummy entries
				...
				if journalIndex == disposition then
					...
					if s then
						if string.len(s) > 0 then
							...
							tip = true
							--[[ -- sourcemod is usually better/more updated than info.sourceMod
							if not (sourceMod == info.sourceMod) then
								if config.logLevel >= 2 then
									mwse.log("%s sourceMod = %s different from info.sourceMod = %s", modPrefix, sourceMod, info.sourceMod)
								end
							end]]
...
local function uiActivatedMenuJournal(e)
	...
	if e.newlyCreated then
		...
		if config.adjustBookmarkWidth then
			if menuJournal_bookmark then
				menuJournal_bookmark.absolutePosAlignX = 0.925
				menuJournal_bookmark.width = 400
				local menuJournal_bookmark_layout = menuJournal_bookmark:findChild(GUI_ID_MenuJournal_bookmark_layout)
...
local function newBookmarkTexture(on)
	local wide = texturesPath .. 'Tx_menubook_bookmark_ab01sj.dds'
	local result
	if stdDDStextureFound then
		if on then
			if lfs.fileexists(backupTexture) then
				result = copyFile(wide, stdDDStexture)
		...
		if on then
			...
			result = os.remove(stdDDStexture)
		end
	end
	return result
end
...
local function modConfigReady()
	...
	function template.onClose()
		...
		--[[
			...
			local s
			if config.upgradeJournalMessage then
				-- clear old 'Your journal has been updated.' message, it will be replaced by our new messageBox
				s = ''
			else
				s = sJournalEntry -- reset it ( was stored on modConfigReady() )
			end
			sJournalEntryGMST.value = s
			if config.logLevel > 1 then
				mwse.log("%s: sJournalEntryGMST.value set to '%s'", modPrefix, sJournalEntryGMST.value)
			end
		end]]
	end
	...
	controls:createOnOffButton{
		...
		description = [[
Add quest name information to sJournalEntry message e.g.
...
to: 'You take a note in your Journal under section "Antabolis Informant"'.]],
	...
	controls:createDropdown{
		...
		description =
[[Add a prefix in order to group quest names.
0. No prefix,
...
Well worth IMO, but you decide.]],
	...
	controls:createDropdown{
		label = "Log level:",
		description =
[[The amount of debug information logged to MWSE.log and/or screen.
0. Low
1. Log Journal entries possibly missing the Quest Name flag.
...
3. High
		...
		options = {
			{ label = "0. Low", value = 0 },
			{ label = "1. Log missing quest names", value = 1 },
	...
	controls:createOnOffButton{
		label = 'Adjust bookmark width (needs restarting Morrowind.exe)',
		description = string.format('Adjust bookmark width to fit longer text.\n'
		.. 'Needs a tweaked\n"%s"\nbookmark texture installed and to restart Morrowind.exe to work as intended.\n'
		.. 'In case of problems with this option, you should be able to restore the original texture copying "%s"'
		.. '\nover\n"%s"', stdDDStexture, backupTexture, stdDDStexture),
