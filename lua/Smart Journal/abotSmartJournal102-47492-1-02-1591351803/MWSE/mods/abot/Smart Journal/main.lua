...
tk[1] = 'clearTopicsWithNoEntries'
tk[2] = 'collapseDates'
tk[3] = 'skipLinksInsideWords'
tk[4] = 'questPrefix'
tk[5] = 'questSort'
tk[6] = 'questHintQuestId'
tk[7] = 'questHintSourceMod'
tk[8] = 'questHintAltSourceInfo'
tk[9] = 'questHintCtrlAltURL'
...
local function getHint(e)
	...
	if config.questHintQuestId then
		hint = string.format("%s ", sm.questId)
		tip = true
	end
...
local function updateJournalElement(element)
	if config.collapseDates
	or config.skipLinksInsideWords then
		local page_1 = element:findChild(GUI_ID_MenuBook_page_1)
		local page_2 = element:findChild(GUI_ID_MenuBook_page_2)
	...
	or config.questHintAltSourceInfo then
		updateBookmark(element:findChild(GUI_ID_MenuJournal_bookmark))
	end
end
...
local function mouseClick(e)
	---mwse.log("mouseClick %s", e.source.name)
	checkQuestNames()
	...
	if event.newlyCreated then
		...
		if menuJournal_bookmark then
			...
			if menuJournal_button_bookmark_quests_active then
				menuJournal_button_bookmark_quests_active:register("mouseClick", mouseClick)
			end
			...
			if menuJournal_button_bookmark_quests_all then
				menuJournal_button_bookmark_quests_all:register("mouseClick", mouseClick)
			end
		end
		local menuBook_button_take = el:findChild(GUI_ID_MenuBook_button_take)
		if menuBook_button_take then
			menuBook_button_take:register("mouseClick", mouseClick)
		end
	end
	updateJournalElement(el)
end
