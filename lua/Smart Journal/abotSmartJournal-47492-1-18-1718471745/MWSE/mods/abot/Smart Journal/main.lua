...
local function getHint(e)
 ...
 -- exit loop when current journalIndex dialog info found
	...
	and (journalIndex > 0) then
		hint = hint..'GetJournalIndex("'..questId..'") == '..journalIndex..'\n\n'
		tip = true
	end
...
local function setInfoGetText(on)
	...
	or event.isRegistered('infoGetText', infoGetText, settings) then
		if on then
			return
		end
		infoGetTextRegistered = false
		event.unregister('infoGetText', infoGetText, settings)
		return
	end
	if on then
		infoGetTextRegistered = true
		event.register('infoGetText', infoGetText, settings)
	end
end
