...
-- not yet copyPaste = true, -- Ctrl + right click to copy clicked journal entry to OS clipboard, Shift + right click to paste to Calendar if present
...
---local GUI_ID_PartParagraphInput_text_input = tes3ui.registerID('PartParagraphInput_text_input')
...
local function updateBookmark(menu)
	...
	if searchInput then
		if not searchInput.disabled then
			showFilter = true
		end
	end
	...
	if showFilter then -- quest/topic list over bookmark
		update = true
		if getHintsOrQuestPrefixOrFilterOn() then
			local el
			for i = 1, #children do
		...
		if el.id == GUI_ID_MenuBook_hypertext then
			if not el:getPropertyBool(ab01sjd) then
				...
				if text then
					...
					---if copyPaste then
						---el:register('mouseOver', mouseOver)
						---el:register('mouseLeave', mouseLeave)
					---end
				end
			end
		end
...
local function updateJournalPages(menu)
	...
	or skipOrCopy then
		...
		if skipOrCopy then
			...
			--[[if copyPaste then
				local el = page_2:findChild(GUI_ID_MenuJournal_bookmark)
				if el then
					el = el:findChild(GUI_ID_MenuJournal_calendar_pane)
					if el then
						if el.visible then
							el:register('mouseOver', mouseOver)
							el:register('mouseLeave', mouseLeave)
...
local function modConfigReady()
	...
	function template.onClose()
		...
		--[[if not (copyPaste == config.copyPaste) then
			copyPaste = config.copyPaste
			if event.isRegistered('mouseButtonUp', mouseButtonUp) then
				if copyPaste then
					event.unregister('mouseButtonUp', mouseButtonUp)
				end
			elseif copyPaste then
...
-- 		description = [[Ctrl + right click to copy clicked journal entry to OS clipboard,
-- Shift + right click to paste to Calendar if present.]],
