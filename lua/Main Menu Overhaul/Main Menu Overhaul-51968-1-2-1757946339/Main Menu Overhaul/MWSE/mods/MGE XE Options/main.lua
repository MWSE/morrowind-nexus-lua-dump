local gui = require("MGE XE Options.gui")		gui.i18n = mwse.loadTranslations("MGE XE Options")
local cf = mwse.loadConfig("MGE XE Options", {})
...
local function onCreatedMenuOptions(e) if e.newlyCreated then		local MM = e.element	local PAR = MM:findChild("MenuOptions_New_container").parent
	...
	MM:findChild("MenuOptions_MCM_container"):registerAfter("mouseClick", function() timer.frame.delayOneFrame(function() local El = tes3ui.getMenuOnTop()
		...
		El:registerBefore("destroy", function(ee) cf.ModX = El.positionX	cf.ModY = El.positionY 	 cf.ModW = El.width		cf.ModH = El.height		mwse.saveConfig("MGE XE Options", cf) end)
	end) end)
