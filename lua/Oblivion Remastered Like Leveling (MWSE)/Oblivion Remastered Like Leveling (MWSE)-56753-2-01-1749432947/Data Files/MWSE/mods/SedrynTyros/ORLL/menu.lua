...
local attributeIconPathName = {
    [1] = "strength",
    [2] = "int",
    [3] = "willpower", --fixed typo of 'wilpower' in this leftover prep work for SAD mod tooltip icons
    [4] = "agility",
    [5] = "speed",
    [6] = "endurance",
    [7] = "personality",
...
this.build = function (element, nextLevel)
    ...
    pageLeft.paddingAllSides = 5
        ...
        local pointsLabel = pageLeft:createLabel{text = "Virtues remaining: " .. state.points}
        ...
        local function modButtonValue(i, delta)
            ...
            state.points = state.points - delta * cost
        ...
        local function updatePlusButtons()
            for i = 1, 8 do
                ...
                local canAfford = state.points >= cost
                ...
                local outOfVirtues = state.points == 0
                ...
                if isLuck then
                    if spent == 0 then
                        ...
                        if activeTotal >= 3 or state.points == 0 then
                            disable = true
                        end

                        -- ✅ Keep Luck clickable for feedback when < 4 points
                        if activeTotal < 3 and state.points > 0 and state.points < 4 then
                            disable = false
                        end
                    end
                ...
                function ()
                    ...
                    if noPoints then
                        for j, data in ipairs(atr) do
                            ...
                            if (max == 0 or max > data.pointsSpent) and state.points >= cost then
                                enableButton{index = j, type = "button_plus"}
                            end
                        end
                    end
                    pointsLabel.text = "Virtues remaining: " .. state.points
                    updateBars(atr)
            ...
            atr[i].button_plus:register("mouseClick",
                function ()
                    ...
                    if state.points < cost then
                        if i == 8 and state.points > 0 then
    ...
    local function levelUpDone()
        ...
        for i = 1, 8 do
            ...
            if not atMax and not atCap and not newPickLockedOut and cost <= pointsLeft and not luckRestricted then
