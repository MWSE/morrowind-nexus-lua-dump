...
disableEmptyThreats = true, -- skip dead actors attack voices sometimes still threatening player
disableDeathSound = true, -- disable annoying hit sound from already dead actors
onlyBedsAllowResting = false, -- rest is only allowed while using bed-like items or traveling
-- delete disabled spawned sourceless references on deactivate.
-- May decrease savefiles size
-- 0 = off, 1 = 1 spawned from scripts, 2 = spawned from scripts and leveled lists
deleteDisabledSourceless = 2,
logLevel = 0,
}
...
local disableEmptyThreats, disableDeathSound, moreStackables, onlyBedsAllowResting
local fixConstantEffects, deleteDisabledSourceless
...
local function updateFromConfig()
	...
	disableDeathSound = config.disableDeathSound
	moreStackables = config.moreStackables
	onlyBedsAllowResting = config.onlyBedsAllowResting
	deleteDisabledSourceless = config.deleteDisabledSourceless
...
local function addAndEquipItemOnce(mobRef, itemId, classId)
	if not mobRef.object.inventory:contains(itemId) then
		...
		assert(mob)
		local _, itm = tes3.addItem({reference = mobRef, item = itemId, playSound = false})
		if itm then
-- replacement for mob:equip(params) that triggers the equip event
			tes3.equip({reference = mobRef, item = itm,
				bypassEquipEvents = false, playSound = false})
		end
	end
end
...
local function processActor(mobRef)
	...
	and back2slash(obj.mesh):lower():multifind(
...
local function referenceSceneNodeCreated(e)
	...
	if not mob then
		...
		and ref.sourceless
		and (not ref.isLeveledSpawn)
		and ref.supportsActivate then
			if logLevel3 then
...
local function fixBadCarriableLights()
	for obj in tes3.iterateObjects(tes3_objectType.light) do
		---@cast obj tes3light
		if obj.canCarry -- nil by default
	    and (obj.mesh:len() <= 0)
		and (obj.radius >= 17) then
			-- as we are here try and fix the empty light
			if pcall(function () obj.canCarry = false end) then
				obj.modified = true
			elseif pcall(function () obj.canCarry = nil end) then
				obj.modified = true
			end
			if obj.modified then
				if obj.sourceMod then
					mwse.log('%s: "%s" light from "%s" canCarry set to %s', modPrefix, obj.id, obj.sourceMod, obj.canCarry)
				else
					mwse.log('%s: "%s" light canCarry set to %s', modPrefix, obj.id, obj.canCarry)
				end
			end
			removeBadLights(obj)
		end
...
local function toggleEvent(on, eventName, eventFunc, options)
	...
	local regOrUnreg = event[un..'register']
	if logLevel2 then
		local opt = ''
		if options then
			opt = json.encode(options)
		end
		mwse.log('%s: "%s"%s %sregistered', modPrefix, eventName, opt, un)
	end
...
local function infoFilter(e)
	...
	---mwse.log('>>> e.info.type = %s', e.info.type)
	local dialogue = e.dialogue
	local diaId = dialogue.id
	if not (
		(
			(diaId == 'Attack')
			and disableEmptyThreats
		)
		or (
			(diaId == 'Hit')
			and disableDeathSound
		)
	) then
		return
	end
	...
	if isDead(mob) then
		if logLevel1 then
			local s = 'hollow threat'
			if diaId == 'Hit' then
				s = 'hit voice'
			end
			mwse.log('%s: infoFilter() "%s" dead actor "%s" %s skipped',
				modPrefix, ref.id, diaId, s)
		end
		e.passes = false
	end
end
...
local function uniformEvent(e, value)
	...
	if legionUniformsDict[lcId] then
		if wearingLegionUni then
			if logLevel1 then
				mwse.log('%s: uniformEvent("%s", "%s") wearingLegionUni.value = %s',
					modPrefix, e.reference.id, e.item.id, value)
			end
...
Effective on game restart.
Note: this also includes/replaces the old fix for more reliably update the WearingLegionUni, WearingOrdinatorUni global variables.]],
		restartRequired = true,
...
Only if the YAC_CI_GlobalScript from the mod is detected.]],
	...
	if sao then
		sideBarPage:createYesNoButton({
			...
			description = [[Automatically disables a detected MGE-XE SAO shader when looking at a known water source activator (e.g waterfalls).
This should make water shadow artifacts less noticeable.]],
	...
	sideBarPage:createYesNoButton({
		...
		description = [[Skip dead actors attack voices sometimes still threatening player.
Equivalent of Empty Threats Disabler mod, with MWSE-Lua advantages (e.g. working with any/future attack dialog voices).]],
		configKey = 'disableEmptyThreats'
	})

	sideBarPage:createYesNoButton({
		label = 'Disable dead actors deathsound',
		description = [[Disable annoying hit sound from already dead actors e.g. the ones you can sometimes hear entering a cell containing 0 health/dead-from-the start non-persistent actors.]],
		configKey = 'disableDeathSound'
	})

	sideBarPage:createYesNoButton({
		label = 'Only beds allow resting',
		description = [[Rest is only allowed while using bed-like items or traveling.]],
		configKey = 'onlyBedsAllowResting'
	})

	optionList = {'Off', 'Spawned from scripts', 'Spawned from scripts and leveled lists'}
	sideBarPage:createDropdown({
		label = 'Delete disabled spawned references',
		options = getOptions(),
		description = [[Delete disabled spawned sourceless references on deactivate.
May decrease savefiles size.]],
		configKey = 'deleteDisabledSourceless'
	})
...
local modListDict = {} -- set in initialized
local skipNextDeactivated = false

--- @param e referenceDeactivatedEventData
local function referenceDeactivated(e)
	if skipNextDeactivated then
		skipNextDeactivated = false
		return
	end
	if deleteDisabledSourceless == 0 then
		return
	end
	local ref = e.reference
	if not ref.disabled then
		return -- only interested in disabled references
	end
	if ref.deleted then
		---assert(false) -- should never happen
		return
	end
	if ref.cell == tes3.getPlayerCell() then
-- skip player cell to avoid possible problems with e.g. dropped items, summons...
		return
	end
	if ref.sourceMod -- not a spawned reference
	and modListDict[ref.sourceMod:lower()] then
		-- and source mod still available/loaded, so skip
		return
	end
	if (deleteDisabledSourceless == 1)
	and ref.isLeveledSpawn then
		-- 1 = process spawned from scripts only
		return
	end
	if not (deleteDisabledSourceless == 2) then
		--  2 = process spawned from scripts and leveled lists
		return
	end
	if ref.object.id:lower():find('summon', 1, true) then
		return -- skip summons
	end
	if logLevel3 then
		local s
		if ref.isLeveledSpawn then
			s = 'leveled list'
		else
			s = 'script'
		end
		mwse.log('%s: %s("%s") "%s" sourceless reference spawned from %s disabled, delete it',
			modPrefix, e['eventType'], ref, ref.cell.editorName, s)
	end
	--- hmmm maybe as sourceless, just skip saving it instead?
	-- for now forcing delete too seems to work better

	-- needed as delete() is retriggering referenceDeactivated()
	skipNextDeactivated = true
	
	---ref.position.z = ref.position.z + 16384
	ref:delete() -- try and force deletion of the sourceless reference from memory
	if ref.deleted then
		ref.modified = false -- as it is sourceless, just skip saving it
	end
end
...
---@diagnostic disable-next-line: undefined-field
			...
			if slot then
				...
				or (slot == tes3_armorSlot_leftGauntlet) then
					s = obj.name
					if s then
						local s2 = s:gsub('uantlet','auntlet')
						if not (s == s2) then
	...
	event.register('referenceDeactivated', referenceDeactivated, {priority = -2^53}) -- try to run it last
end, {doOnce = true})
