...
local closeInspectionTweaks, saoWaterTweaks, fixDummySubtitles
...
local function legionUniCheck()
	...
	v = math.floor(v + 0.5)
	local v2 = 0
	local equippedArmorStack = tes3.getEquippedItem({ actor = player,
...
local placeables = {
[T3OT.armor] = true, [T3OT.container] = true, [T3OT.light] = true, [T3OT.miscItem] = true
}
...
local function referenceSceneNodeCreated(e)
	...
	if not mob then
		...
		and placeables[ref.baseObject.objectType] then
			ref:updateLighting()
...
local function preventRest(e)
	...
	and NOM_shelter
	and (math.floor(NOM_shelter.value + 0.5) > 0) then
		return false
	end
end
...
local function activateCheck(e)
	if not (e.activator == tes3.player) then
		return
	end
	local targetRef = e.target
	if targetRef == lastTargetRef then
		return
	end
	...
	if checkCloseInspectionReset() then
		timer.frame.delayOneFrame(
			function ()
				timer.frame.delayOneFrame(
					function () tes3.player:activate(targetRef) end
				)
			end
		)
		return false
...
local function fixSteelCuirass()
	...
	if string.lower(sourceMod) == 'bloodmoon.esm' then
...
local worldController -- set in modConfigReady()
...
local function loadOriginalText(e)
	...
	-- cascheing the fucking thing as it is crashy if you call it
	-- from different mods, same (infoGetText/postInfogetText) event
...
local function infoGetText(e)
	...
	if not (
		(info_type == tes3_dialogueType_voice)
	...
	if logLevel2 then
		local dialogue = info:findDialogue()
		mwse.log([[%s: infoGetText(e) fixed dialogue.id = "%s" info.originalText = "%s", info.sourceMod = "%s"]],
			modPrefix, dialogue.id, s, info.sourceMod)
	end
	stringToNotNotify = s
end
...
local function toggleEvents()
	toggleEvent(fixDummySubtitles, 'infoGetText', infoGetText)
	...
	toggleEvent(on, 'activate', activateCheck)
	...
	local function getSaoShader()
		...
		for i = 1, #shaders do
			...
			if v then
				local name = v.name
				if name then
					if string.find(string.lower(name), 'sao', 1, true) then
						result = v
						break
					end
				end
...
'saoWaterTweaks'),
	...
	controls:createYesNoButton({
		...
		description = getYesNoDescription(
[[Hide short dummy subtitles needed by some mods to start scripting from dialog.]],
'fixDummySubtitles'),
		variable = createConfigVariable('fixDummySubtitles')
	})
