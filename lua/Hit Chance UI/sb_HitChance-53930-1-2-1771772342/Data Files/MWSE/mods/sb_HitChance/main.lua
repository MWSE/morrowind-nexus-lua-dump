...
local function calcEvasion(mobile)
	...
	local chameleon = tes3.getEffectMagnitude({ reference = mobile.reference, effect = tes3.effect.chameleon })
	return ((agility / 5) + (luck / 10)) * (0.75 + (0.5 * (fatigueCurrent / fatigueBase))) + sanctuary + (chameleon / 5)
end
