local mod = {
    ...
    ver = "1.2",
    cf = {onOff = true, onOff2 = true, key = {keyCode = tes3.scanCode.l, isShiftDown = false, isAltDown = false, isControlDown = false}, dropDown = 0, slider = 4, sliderpercent2 = 32, blocked = {}, npcs = {}, textfield = "hello", switch = false, next = 1, keyP = {keyCode = tes3.scanCode.keyLeft, isShiftDown = false, isAltDown = false, isControlDown = false}, keyN = {keyCode = tes3.scanCode.keyRight, isShiftDown = false, isAltDown = false, isControlDown = false}}}
...
local function press(e)
    if tes3.isLuaModActive("Virnetch\\HotKeys") and cf.onOff then
        return
    end
    local dont = true
    for i = 22, 30 do
        ...
        if e.keyCode == tes3.scanCode.keyLeft or e.keyCode == tes3.scanCode.keyRight then
            dont = false
        end
    end
    if dont then
    ...
    for _,v in ipairs(clear) do
        ...
        local darkness1 = border:createRect({id = "darkness", color = {0.0, 0.0, 0.0}})
        darkness1.autoWidth = true
        darkness1.autoHeight = true
        border.autoWidth = true
        border.autoHeight = true
        border.borderAllSides = 0
        border.paddingAllSides = 2
...
local vernList = {
    [1] = "quickAH", [2] = "quickH", [3] = "quickB2",
    [4] = "quickB", [5] = "quick2", [6] = "quickBH",
    [7] = "quickA", [8] = "quickA2", [9] = "quick_"
}

local function mb(data)
local messages = {
[1] = string.format("%s + Hold", table.find(tes3.scanCode, data.keyA):upper()),
[7] = string.format("%s + Press", table.find(tes3.scanCode, data.keyA):upper()),
[8] = string.format("%s + Double Tap", table.find(tes3.scanCode, data.keyA):upper()),
[3] = string.format("%s + Double tap", table.find(tes3.scanCode, data.keyB):upper()),
[4] = string.format("%s + Press", table.find(tes3.scanCode, data.keyB):upper()),
[6] = string.format("%s + Hold", table.find(tes3.scanCode, data.keyB):upper()),
[2] = "Hold",
[5] = "Double Tap",
[9] = "Press"
}
return messages[cf.next]
end
local function virnCompat()
    local block = menu:findChild("Spa_HotQuick")
    if cf.onOff then
        block:destroyChildren()
        else
    ...
    if not seph then
        ...
        for slot = 1, 9 do
            local quickKey = tes3.player.data.quickKeys[vernList[cf.next]][slot]
...
local function vern(e)
    if not (tes3.isLuaModActive("Virnetch\\HotKeys") and cf.onOff) then
        return
    end
    if not (tes3.player and tes3.player.data.quickKeys) then
        return
    end
    if not menu then
        return
    ...
    --debug.log(dont)
    if e.keyCode == cf.keyP.keyCode then
        if cf.next == 9 then cf.next = 1 else cf.next = cf.next+1 end
        virnCompat()
        if not dont then
            tes3.messageBox{ message = mb(tes3.player.data.quickKeys), duration = 3}
        end
    end
    if e.keyCode == cf.keyN.keyCode then
        if cf.next == 1 then cf.next = 9 else cf.next = cf.next-1 end
        virnCompat()
        if not dont then
            tes3.messageBox{ message = mb(tes3.player.data.quickKeys), duration = 3}
        end
    end
end event.register("keyUp", vern, {priority = -1})
...
local function register()
    ...
    if tes3.isLuaModActive("Virnetch\\HotKeys") and cf.onOff then return end
    for slot = 1, 9 do
        local quickKey = tes3.getQuickKey({slot = slot})
...
local block = menu:createRect({id = "Spa_HotQuick", color = {0.0, 0.0, 0.0}})
    ...
    if tes3.isLuaModActive("Virnetch\\HotKeys") then
        tes3.tapKey(cf.keyP.keyCode)
        tes3.tapKey(cf.keyN.keyCode)
    end
end event.register(tes3.event.uiActivated, loaded, { filter = "MenuMulti" })

event.register("loaded", function()
    timer.start{duration = 1, type = timer.real, callback = function() dont = false end}
end)
local function registerModConfig()
    local template = mwse.mcm.createTemplate("Панель быстрых клавиш")
    template:saveOnClose(mod.name, cf)
    template:register()

    local page = template:createSideBarPage({label="Настройки \"Панель быстрых клавиш\""})
    page.sidebar:createInfo{ text = "Добро пожаловать в меню \"Панель быстрых клавиш\" \n \n"..mod.name.."\n \nАвтор: Spammer."}
    ...
    local category0 = page:createCategory("Отображать панель быстрых клавиш?")
    category0:createYesNoButton{label = "Да/Нет", description = " ", variable = mwse.mcm.createTableVariable{id = "onOff2", table = cf, restartRequired = false, restartRequiredMessage = "Необходимо перезагрузить сохраненную игру, чтобы вернуть границу."}}

    local category1 = page:createCategory("")
    local elementGroup = category1:createCategory("Выберите, с какой стороны экрана будет отображаться панель быстрого доступа:")

    elementGroup:createDropdown { description = " ",
        options  = {
            { label = "Сверху", value = 0 },
            { label = "Снизу", value = 1 },
            { label = "Слева", value = 2 },
            { label = "Справа", value = 3 },
        },
    ...
    local category2 = page:createCategory("Размер панели:")
    local subcat = category2:createCategory(" ")

    subcat:createSlider{label = "Расстояние между слотами", description = "[По умолчанию: 4]", min = -1, max = 10, step = 1, jump = 1, variable = mwse.mcm.createTableVariable{id = "slider", table = cf}}

    subcat:createSlider{label = "Размер панели", description = "[По умолчанию: 32]", min = 0, max = 128, step = 1, jump = 10, variable = mwse.mcm.createTableVariable{id = "sliderpercent2", table = cf}}
    if tes3.isLuaModActive("Virnetch\\HotKeys") then
        local page2 = template:createSideBarPage({label="Совместимость с модом \"Расширенные быстрые клавиши\""})
        page2.sidebar:createInfo{ text = "Добро пожаловать в меню \"Панель быстрых клавиш\" \n \n"..mod.name.."\n \nАвтор: Spammer."}
        page2.sidebar:createHyperLink{ text = "Spammer's Nexus Profile", url = "https://www.nexusmods.com/users/1cf.slider01391cf.slider8?tab=user+files" }
        local category = page2:createCategory(" ")
    category:createOnOffButton{label = "Совместимость Вкл/Выкл", description = "Включить режим совместимости с модом HotKeys Extended.", variable = mwse.mcm.createTableVariable{id = "onOff", table = cf}}

    category:createKeyBinder{label = "Следующая панель", description = " ", allowCombinations = false, variable = mwse.mcm.createTableVariable{id = "keyN", table = cf, restartRequired = false, defaultSetting = {keyCode = tes3.scanCode.keyRight, isShiftDown = false, isAltDown = false, isControlDown = false}}}
    category:createKeyBinder{label = "Предыдущая панель", description = " ", allowCombinations = false, variable = mwse.mcm.createTableVariable{id = "keyP", table = cf, restartRequired = false, defaultSetting = {keyCode = tes3.scanCode.keyLeft, isShiftDown = false, isAltDown = false, isControlDown = false}}}
    end
end event.register("modConfigReady", registerModConfig)

local function initialized()
    if seph then
        seph:registerElement("Spa_test", "Панель быстрых клавиш", nil, nil)
    end
