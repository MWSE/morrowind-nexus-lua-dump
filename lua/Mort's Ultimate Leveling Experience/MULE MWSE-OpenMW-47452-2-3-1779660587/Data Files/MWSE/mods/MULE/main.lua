...
if (mwse.buildDate == nil or mwse.buildDate < 20240101) then
	mwse.log("[MULE] Build date of %s does not meet minimum build date of >= 2024.", mwse.buildDate)
	event.register(
		"initialized",
		function()
...
local config = mwse.loadConfig("mortLeveling", {
	...
	magickaPerWillpower = 0
})

if ( tes3.hasCodePatchFeature(tes3.codePatchFeature.levelupSkillsTooltip) ) then	
	mwse.log("[MULE] - failed check")
	event.register(
		"initialized",
		function()
			tes3.messageBox("MULE does not utilize the Level-Up Skills Tooltip enabled in Morrowind Code Patch. Disable this option by rerunning Morrowind Code Patch and disabling it under Interface Changes")
		end
	)	
end
...
local function incrementSkill(skill, amount)
	...
	if skill == 777 then --luck special case
		...
		while path.luckProgress >= 1.0 and tes3.mobilePlayer.luck.base < config.attributeMaximum do
	...
	if skill == 20 then --acrobatics special case
		amount = (config.acrobaticsMod * 0.01) * amount
	end

	if tes3.attributeName[tes3.getSkill(skill).attribute] == "strength" then 
	...
	elseif tes3.attributeName[tes3.getSkill(skill).attribute] == "intelligence" then
		...
		if path.intelligenceProgress >= 1.0 and tes3.mobilePlayer.intelligence.base < config.attributeMaximum then
			...
			if config.magickaPerWillpower > 0 then
				tes3.modStatistic{reference=tes3.player,name="magicka",value = path.willpowerMagickaBonus}
			end
	...
	elseif tes3.attributeName[tes3.getSkill(skill).attribute] == "willpower" then
		...
		if path.willpowerProgress >= 1.0 and tes3.mobilePlayer.willpower.base < config.attributeMaximum  then
			...
			if config.magickaPerWillpower > 0 then
				tes3.modStatistic{reference=tes3.player,name="magicka",value = config.magickaPerWillpower}
				path.willpowerMagickaBonus = path.willpowerMagickaBonus + config.magickaPerWillpower
			end
	...
	elseif tes3.attributeName[tes3.getSkill(skill).attribute] == "luck" then
		path.luckProgress = (path.luckProgress + amount)
...
local function onSkillUp(e)
	...
	local threshold = tes3.findGMST("iLevelupTotal").value -- 10 [9 for major/minor, 10 for misc, this affects how many skills levels you up]
...
local function setupMULE(e)
	...
	if path == nil then
		...
		path.miscSkillsRaised = 0
		path.willpowerMagickaBonus = 0
	end
	
	if path.willpowerMagickaBonus == nil then
		path.willpowerMagickaBonus = 0
	end
...
local function onExerciseSkill(e)
	local majors = tes3.player.object.class.majorSkills
	local minors = tes3.player.object.class.minorSkills
	local expRate = config.miscExpRate -- "misc" --default code for skills, will get set to major/minor
	local skillName = tes3.getSkill(e.skill).name
	
	for _,skill in pairs(majors) do
		if skill == e.skill then
			expRate = config.majorExpRate
		end
	end
	
	for _,skill in pairs(minors) do
		if skill == e.skill then
			expRate = config.minorExpRate
		end
	end
	
	e.progress = e.progress * (expRate / 100)
end
...
local function registerModConfig()
	...
	-- categoryMain:createSlider{ label = "Skill Gains Per Level",
							-- variable = createtableVar("skillsPerLevel"),
							-- max = 30,
							-- jump = 1,
							-- defaultSetting = 10}							
								
	categoryMain:createSlider{ label = "Attribute Maximum (Overrides MCP Setting)",
							variable = createtableVar("attributeMaximum"),
							max = 500,
							jump = 10,
							defaultSetting = 200}
							
	-- categoryMain:createSlider{ label = "Luck gain per level (x0.1, so 10 = 1 luck per level)",
							-- variable = createtableVar("luckPerLevel"),
							-- max = 20,
							-- defaultSetting = 10}
							
	categoryMain:createYesNoButton{ label = "Use alternative flatrate HP gain system (Recommended)", 
	...
	categoryMain:createSlider{ label = "Magicka gained per Willpower increase (default: 0)",
						variable = createtableVar("magickaPerWillpower"),
						max = 10,
						defaultSetting = 0}
						
	local categoryExp = page:createCategory("Experience Rate")
	categoryExp:createSlider{ label = "Experience rate multiplier for Major Skills (%)",
							variable = createtableVar("majorExpRate"),
							max = 200,
							jump = 10,
							defaultSetting = 100}	
	
	categoryExp:createSlider{ label = "Experience rate multiplier for Minor Skills (%)",
							variable = createtableVar("minorExpRate"),
							max = 200,
							jump = 10,
							defaultSetting = 100}	
								
	categoryExp:createSlider{ label = "Experience rate multiplier for Misc Skills (%)",
							variable = createtableVar("miscExpRate"),
							max = 200,
							jump = 10,
							defaultSetting = 100}	
	...
	local categoryDecay = page:createCategory("Skill Decay")
	categoryDecay:createYesNoButton{ label = "Skill Decay", 
							variable = createtableVar("skillDecay"),
							defaultSetting = true}
	
	categoryDecay:createYesNoButton{ label = "Display Skill Decay Messagebox", 
								variable = createtableVar("skillDecayMessage"),
								defaultSetting = true}
								
	categoryDecay:createYesNoButton{ label = "Use base stats as decay minimum (only valid on new characters)", 
								variable = createtableVar("skillDecayUseBase"),
								defaultSetting = false}
								
	categoryDecay:createSlider{ label = "Days of no use before a skill decays",
	...
	categoryDecay:createSlider{ label = "Minimum a skill can decay to",
