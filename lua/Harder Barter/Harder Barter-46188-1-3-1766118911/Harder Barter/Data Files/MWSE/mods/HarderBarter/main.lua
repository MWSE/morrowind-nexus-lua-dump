	...
	if ( config.modEnabled == true ) then
		if e.buying == false then
			...
			if config.easyMode then
				if basePricePer > 100 then
					e.price = e.price / (math.log(basePricePer)-3.6)
				end			
			else
...
local function initialize()
	if (mwse.buildDate == nil or mwse.buildDate < 20240715) then
...
local function registerModConfig()
	...
	categoryMain:createYesNoButton{ label = "Enable Lite Mode (see readme)",
								variable = createtableVar("easyMode"),
