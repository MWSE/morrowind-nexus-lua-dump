...
local function press(e)
    ...
    -- проверка нажатия клавиши факела из Brighter Lights
    if cfTorchBR and cfTorchBR.lightKey.keyCode and cfTorchBR.lightKey.keyCode == e.keyCode then
        dont = false
    end
    ...
    if menu then
        ...
        if cf.HotBarExtended then
            ...
            timer.start({duration = pressUpdateInterval, type = timer.real, callback = updateHotbarExtended })
        else
            --updateHotbar()
            timer.start({duration = pressUpdateInterval, type = timer.real, callback = updateHotbar })
        end
    end
...
local function initialized()
    ...
    -- загрузка конфигурации - Brighter Lights
    if tes3.isLuaModActive("BeefStranger.Brighter Lights") then
        cfTorchBR = mwse.loadConfig("Brighter Lights")
    end
    -- режим совместимости с модом Auto Sheathe Before Drawing
    if tes3.isLuaModActive("weapons_sheath_when_equip_change") then
        pressUpdateInterval = 0.6
    else
        pressUpdateInterval = 0.01
    end
