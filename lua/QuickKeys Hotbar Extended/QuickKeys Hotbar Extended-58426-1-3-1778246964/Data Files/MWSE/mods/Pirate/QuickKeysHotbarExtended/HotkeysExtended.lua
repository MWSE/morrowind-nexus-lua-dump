	...
	if data[type][num].id ~= nil then
		if data[type][num].id == 0 then			--Hand to Hand
			...
			if data[type][num].isMagic then 	--Magic
				...
				if mwscript.getItemCount{ reference = tes3.player, item = id } > 0 then
					local object = tes3.getObject(id)
                    local itemData = common.getItemData(object.id)
                    if itemData and (object.objectType == tes3.objectType.weapon or object.objectType == tes3.objectType.armor) then
                        if itemData.condition and itemData.condition <= 0 then
                            tes3.messageBox(tes3.findGMST(tes3.gmst.sInventoryMessage1).value)
                            return
                        end
                    end
					local eventData = { item = object, reference = tes3.player }
					local resultPayload = event.trigger("equip", eventData, { filter = tes3.player })
					local equip = not resultPayload.block
					if equip then
						tes3.mobilePlayer:equip{ item = id }
						if object then
							if object.objectType == tes3.objectType.weapon then
					...
					elseif object and object.objectType == tes3.objectType.weapon then
						lastWeaponEquipped = nil
					end
				else
					local object = tes3.getObject(id)
...
local function quickMenuOpen()
	...
	if isEmpty == false then
        ...
        local clearButton = okBlock:createButton{ text = i18n("HotBar.ClearAll.Label") }
        clearButton:register("mouseClick", function()
            tes3.messageBox{
                message = i18n("HotBar.ClearAll.Confirm"),
                ...
                callback = function(e)
                    if e.button == 0 then
                        for _, type in ipairs(types) do
                            for i = 1, 9 do
                                data[type][i].id = nil
                                data[type][i].savedItemData = nil
                                data[type][i].name = nil
                                data[type][i].icon = nil
                                data[type][i].isMagic = false
                                data[type][i].isItem = false
                            end
                        end
                        local quickMainMenu = tes3ui.findMenu(quickMainid)
                        if quickMainMenu then
                            quickMainMenu:destroy()
                            quickMenuOpen()
                        end
                    end
        ...
        local spacer = okBlock:createBlock()
        spacer.widthProportional = 1
