...
local function getItemData(itemId)
    ...
    -- Перебираем все экипированные предметы
        ...
        if quickKey and quickKey.item then
            ...
            if quickKey.item.enchantment then
                if cf.StatusBar then
                    ...
                    if quickKey.itemData and quickKey.item.maxCondition and quickKey.item.maxCondition > 0 then
                        local conditionBar = borde:createFillBar({current = quickKey.itemData.condition, max = quickKey.item.maxCondition})
                    ...
                    if quickKey.itemData and (quickKey:getMagic() or quickKey.item.enchantment.castType == 1) then
                        local chargeBar = borde:createFillBar({current = quickKey.itemData.charge, max = quickKey.item.enchantment.maxCharge})
            ...
            if cf.SlotNumber then
                ...
                if cf.StatusBar then
                    ...
                    if quickKey.itemData and quickKey.item.objectType == tes3.objectType.light then
                        if quickKey.item.time and quickKey.item.time > 0 then
                            local durationBar = borde:createFillBar({current = quickKey.itemData.timeLeft, max = quickKey.item.time})
    ...
    if not cf.PanelsInOneLine then
        ...
        for slot = 1, cf.numberVisibleSlot do
            ...
            if quickKey and quickKey.id and (quickKey.id ~= 0) then
                ...
                if cf.StatusBar then
                    local itemData = nil
                    local liveData = getItemData(quickKey.id)
                    if liveData then
                        ...
                        --quickKey.savedItemData.soul = liveData.soul and liveData.soul.id

                        itemData = quickKey.savedItemData  -- используем обновлённые данные
                    else
                        -- если предмет не экипирован, используем сохранённые данные
...
local function tooltipHotbarExtended (panelName, slot)
    ...
    if quickKey.isMagic and not quickKey.isItem then
        ...
        -- ищем предмет в инвентаре и вызываем его подсказку
        ...
        if menuInventory then
            ...
            if scrollPane then
                ...
                if pane then
                    local item = tes3.getObject(quickKey.id)
                    if item then
                        local icon = "Icons\\"..item.icon
                        -- Ищем предмет с такой иконкой в инвентаре
                        for _, row in pairs(pane.children) do
                            for _, child in pairs(row.children) do
                                if child.children and child.children[1] and child.children[1].contentPath == icon then
                                    child:triggerEvent("help")
                                    return
                                end
                            end
                        end
...
local function visibility()
    ...
    menu.visible = cf.HotBarVisible and fill.visible
