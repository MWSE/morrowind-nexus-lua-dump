...
local function getSettings(sectionKey, key)
        ...
        if key == o.showMagicWindow.key then
                updateKeyBind(o.showMagicWindow)
        end
end
        ...
        el = ui.create {
                ...
                events = {
                        mousePress = async:callback(function(e, l)
                                ...
                                table.insert(g.myDelayedActions, MainWindow.element)
                        end),
                        mouseMove = async:callback(function(e)
                                if not MainWindow.element.layout then return end
                ...
                events = {
                        mousePress = async:callback(function(e, l)
                                ...
                                table.insert(doLater, {
                                        action = function()
                                                if not el.layout or not el.layout.userData.list.focus then return end
                                                local items = getEquipmentEls()
                                                table.sort(items, equipmentSortCallback)
                                                el.layout.userData.list.updateItems(items)
                                        end,
                                        skip = 2
                                })
                        end),
...
local function createMagicWindow(savedSpellsIDs)
        ...
        local window = g.window:new('Magic Window', 0, 0, tabs)

        return window
end

local activeWindows
local ShowWindow = async:callback(function()
...
input.registerTriggerHandler(o.showMagicWindow.argument.key, ShowWindow)
