...
local flexTBg = require('scripts.spellsBookmark.lib.myGUI').flexTBg
...
-- ---@type Vector2
-- local newPos = util.vector2(30, 30)
-- ---@type Vector2
-- local newAnchor = util.vector2(0, 0)
---@type Vector2
local newPos = util.vector2(o.windowX.value, o.windowY.value)
---@type Vector2
local newSize = util.vector2(o.windowW.value, o.windowH.value)
---@type Vector2
local newAnchor = util.vector2(o.windowAnchX.value, o.windowAnchY.value)
...
---@type table <string, ui.Layout>
local allTabsLayouts = {}


---@param tab ui.Layout
local function selectTab(tab)
        for i, v in pairs(allTabsLayouts) do
                allTabsLayouts[i].template = myTemplates.InactiveTab
                allTabsLayouts[i].content.tabName.props.textColor = colors.black
        end
        tab.template = myTemplates.activeTab
        tab.content.tabName.props.textColor = colors.normal
end

local function createTab(props)
        local tab = {
                type = ui.TYPE.Flex,
                template = myTemplates.InactiveTab,
                props = {
                ...
                content = ui.content {
                        {
                                ...
                                props = {
                                        ...
                                        -- textShadow = true,
                                        -- textShadowColor = util.color.hex('555555'),
                                },
                        }
                },
                ...
                events = {
                        mouseClick = async:callback(function(e, l)
                                selectTab(allTabsLayouts[props.name])
                                window.layout.content.mainFlex.content.mainContent = props.getContent()
                                window:update()
                        end),
                },
        }

        allTabsLayouts[props.name] = tab
...
---@param args { title: string, tabs: Tab[] }
---@return ui.Element
local function createResizableWindow(args)
        ...
        local tabsLayouts = {}
        for i, props in pairs(args.tabs) do
                -- if i % 2 == 0 then
                --         -- table.insert(tabsLayouts, { template = I.MWUI.templates.verticalLine })
                --         table.insert(tabsLayouts, makeInt(4, 0))
                -- end

                table.insert(tabsLayouts, createTab(props))
        end
        ...
        local windowLayout = {
                layer = "Windows",
                template = myTemplates.getTemplate('thick', { 6, 6, 6, 6 }, true, textures.black),
                props = {
                ...
                content = ui.content {
                        {
                                ...
                                content = ui.content {
                                        ...
                                        {
                                                -- type = ui.TYPE.Flex,
                                                template = myTemplates.getTemplate('none', { 0, 0, 0, 0 }, true, textures.menuBG, false, true),
                                                -- template = getTemplate('none', { 0, 0, 0, 0 }, true, ui.texture({ path = 'Textures/' .. imageName })),
                                                props = {
        ...
        -- allTabsLayouts[args.tabs[1].name].template = myTemplates.activeTab

        selectTab(allTabsLayouts[args.tabs[1].name])
...
return { createResizableWindow = createResizableWindow, getWindowProps = getWindowProps, setWindowProps = setWindowProps }
