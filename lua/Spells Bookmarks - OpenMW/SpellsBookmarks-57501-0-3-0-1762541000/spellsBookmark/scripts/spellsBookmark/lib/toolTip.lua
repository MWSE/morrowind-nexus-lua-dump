...
---@param spell Spell
local function getSpellLayout(spell)
...
---@params item Item
local function getItemLayout(item)
        for i, _ in pairs(maxWidths) do
                maxWidths[i] = 0
        end


        local record = core.magic.enchantments.records
            [item.type.record(item).enchant]




        local effectsInfos = record.effects
        local allTexts = {}
        ...
        local myContent = {
                type = ui.TYPE.Flex,
                content = ui.content {
                        { template = I.MWUI.templates.textHeader, props = { text = item.name } },
                        makeInt(0, PADDING),
                        table.unpack(allTexts),
                }
        }
