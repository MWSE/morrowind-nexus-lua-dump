...
local function createResizableWindow(args)
        ...
        for _, tabProps in ipairs(args.tabs) do
                -- tabManager.registerTab(tabProps.name, tabProps.getContent)
                tabManager.registerTab(tabProps)
        end

        -- local tabBarLayouts, contentContainer = tabManager.initialize(args.tabs[1].name)
        local tabBarLayouts, contentContainer = tabManager.initialize(args.defaultTab)
