...
-- OpenMW 0.49 – Bribery Minigame UI (proper title bar drag) with l10n
...
local L = core.l10n(S.l10n, 'en')  -- use your mod’s l10n context, defaulting to English
...
-- Layout ---------------------------------------------------------------
-- Map an inflation percent (>= 0) to a localized label.

local function inflationLabel(pct)
  local p = tonumber(pct) or 0
  if p < 1 then
    return L('inflation_none')
  elseif p < 20 then
    return L('inflation_low')
  elseif p < 50 then
    return L('inflation_medium')
  elseif p < 100 then
    return L('inflation_high')
  elseif p < 200 then
    return L('inflation_very_high')
  else
    return L('inflation_extreme')
  end
end
...
local function buildContent(triesLeft, inflationPct, playerGold, statusText)
  return ui.content({
    ...
    row({
      ...
      spacer(16, 1),
      text(L('ui_your_gold', { gold = tonumber(playerGold) or 0 }), 16),
    }),
    ...
    row({
      button(L('ui_submit'), function()
        if M.onSubmit then M.onSubmit(parseOffer(currentOffer)) end
      end),
      spacer(24, 1),
      button(L('ui_cancel'), function() destroy() end),
    }),
  })
end
  ...
  local titleRow = {
    ...
    content = ui.content({
      spacer(8, 0),
      {
        type = ui.TYPE.Text,
        props = {
          text = L('ui_title', { name = npcName or "" }),
          textSize = 20,
          textColor = COLOR_NORMAL,
          textShadow = true,
          autoSize = true,
