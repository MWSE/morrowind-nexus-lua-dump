...
-- Returns (canCarryAll, maxCount)
-- canCarryAll: true if the entire stack fits
-- maxCount:    how many items from the stack the NPC can carry (0 = none)
...
local function lureDoPickup()
    ...
    if item and item:isValid() and item.count > 0 and item.cell then
        ...
        core.sendGlobalEvent("NpcLurePickupItem", {
            item     = item,
            npc      = self.object,
            maxCount = not canAll and maxCount or nil,
        })
        lure.phase = "picked_up"
        async:newUnsavableSimulationTimer(cachedSettings.LURE_RETURN_DELAY, function()
...
local function effectiveArmorRating(item, npcObj)
    ...
    return (rec.baseArmor or 0) * armorConditionRatio(item) * skill / 100
end
...
local function tryEquipArmor(item)
    ...
    -- picked armor is better or slot is empty, equip it
    eq[slot] = item
