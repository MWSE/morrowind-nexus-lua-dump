...
local info = [[CTRL + Click in to rename a player made spell or enchanted item from player magic menu.
CTRL + Equip on paperdoll to rename a player made constant enchanted item from player inventory menu.
]]

local function menuMagicAfterUpdate(e)
...
local function uiActivatedMenuMagic(e)
	if e.newlyCreated then
		--[[if logLevel > 1 then
			tes3.messageBox(info)
		end]]
		e.element:registerAfter('update', menuMagicAfterUpdate)
		---updateMenuMagic(e.element)
	end
end
