...
local function checkSummon(data)
    ...
    if hasSummon then
        if not alredySummonCrime and not allowSummon(data) then
            ...
            local res = crimes.commitCrime(player, {
                -- arg = 400,
                type = player.type.OFFENSE_TYPE.Assault
            })

            local crimeCost = 400
            local deltaCrime = types.Player.getCrimeLevel(player) - crimeLevel
            if res.wasCrimeSeen and deltaCrime < crimeCost then
                types.Player.setCrimeLevel(player, crimeCost - deltaCrime)
            end

            if res.wasCrimeSeen then
...
local function punishSuspicious(data)
    -- print("suspiciousCrime", suspiciousCrime, data.hasEffect)
    if not data.hasEffect then
        suspiciousCrime = false
        return
    end

    if suspiciousCrime then
        return
    end
...
return {
    eventHandlers = {
        ...
        SaneMagicSettings = function(data)
            ...
            settings:set("smCharmSneakLimit", data.smCharmSneakLimit)
        end,

        smMagicOff = function(data)
            types.Player.setControlSwitch(player, types.Player.CONTROL_SWITCH.Magic, false)
        end
    },
    engineHandlers = {
        onActorActive = function(actor)
            actor:sendEvent("smOnActive")
        end
    }
}
