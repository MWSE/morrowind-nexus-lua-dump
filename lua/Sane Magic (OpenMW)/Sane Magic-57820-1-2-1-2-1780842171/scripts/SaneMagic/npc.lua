...
local function checkPlayerSneak(data)
    ...
    if sneakLimit == nil then sneakLimit = settings:get("smCharmSneakLimit") or 25  end
    ...
    if sneak >= sneakLimit then
        ...
        if sneakLimit == 100 then
            ...
            player:sendEvent('smShowMessage', {message = msg("smNextCharm", {sneak = sneakLimit})})
        end

    end
end

local randomFirstCharm = {
    "smFirstCharm",
    "smFirstCharm2"
}
local function checkEffectsOnNPC()
    ...
    for spellid, param in pairs(spells) do
        if param.temporary and param.caster and param.caster.type == types.Player then
            for _, effect in ipairs(param.effects) do
                if not underEffect[effect.id] then
                    ...
                    elseif (effect.id == EFF.Charm) and settings:get("smCharm") then
                        local sneak = skills.sneak(player).modified
                        if sneakLimit == nil then sneakLimit = settings:get("smCharmSneakLimit") or 25  end
                        ...
                        elseif sneak >= sneakLimit then
                            --if charmCount <= 6 then
                                local msgFirstCharm = randomFirstCharm[math.random(1, #randomFirstCharm)]
                                player:sendEvent('smShowMessage', {message = msg(msgFirstCharm)})
                            --end
                        else
                            player:sendEvent('smShowMessage', {message = msg("smFirstCharmNotEnough")})
                        end
                    end 
                end
    ...
    for effectId, wasUnderEffect in pairs(underEffect) do
        if not hasEffect[effectId] and wasUnderEffect then
            ...
            if (effectId == EFF.Charm) and settings:get("smCharm") then
                checkPlayerSneak()
            end
        end
    end
...
local function isFightingPlayer()
    local isFightingPlayerTrue = nil
...
local function onUpdate(dt)
    ...
    player:sendEvent('smInCombat', {npc = self.recordId, inCombat = isFighting})
end
...
local function onLoad(data)
    ...
    sneakLimit = data and data.sneakLimit or settings:get("smCharmSneakLimit")
