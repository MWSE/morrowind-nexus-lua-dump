...
local function onActive()
    ...
    if state.aiMode ~= mData.aiModes.Dead then
        ...
        if state.lastActiveGameTime then
            core.sendGlobalEvent('fairCare_sendGameTime', self)
        end
    end
end
...
local function setGameTime(gameTime, gameTimeScale)
    if state.aiMode == mData.aiModes.Default then
        local passedSeconds = (gameTime - state.lastActiveGameTime) / gameTimeScale
        ...
        state.lastActiveGameTime = nil
    else
        state.lastActiveGameTime = gameTime
    end
end
...
return {
    ...
    eventHandlers = {
        ...
        fairCare_clearHealer = function() mWounded.clearHealer(state) end,
        ...
        fairCare_declineHealHelp = function() mHealer.declineHealHelp(state) end,
        fairCare_healFriend = function() mHealer.healFriend(state) end,
        ...
        fairCare_setFollowerTeam = function(followerTeam) mFollow.setFollowerTeam(state, followerTeam) end,
        fairCare_sendGameTime = function(data) setGameTime(data.time, data.scale) end,
    },
}