...
if not mSettings.isLuaApiRecentEnough then
    mSettings.getStorage(mSettings.globalKey):set("selfHealingEnabled", false)
    mSettings.getStorage(mSettings.globalKey):set("touchHealingEnabled", false)
    mSettings.getStorage(mSettings.globalKey):set("healthRegenEnabled", false)
    mSettings.getStorage(mSettings.globalKey):set("addingPotionsEnabled", false)
end

local function sendGameTime(actor)
    actor:sendEvent("fairCare_sendGameTime", { time = world.getGameTime(), scale = world.getGameTimeScale() })
end

return {
    eventHandlers = {
        fairCare_sendGameTime = sendGameTime,
    }
}