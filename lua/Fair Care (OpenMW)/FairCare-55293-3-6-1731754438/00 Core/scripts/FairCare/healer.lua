...
-- Non biped creatures cannot run and cast spells at the same time, then they will run then heal
local moveOrHeal = self.type == T.Creature and not mTools.getRecord(self).isBiped
...
local function clearState(state)
    ...
    if state.aiMode == mData.aiModes.Healing then
        ...
        state.healer.woundedPartner:sendEvent("fairCare_clearHealer")
...
local function setRetreatPosition(state)
    local position, angle, distance = mActors.getRetreatPosition(self, mActors.getCloseActors(self, mCfg.maxEscapeTime))
    if not position then return end

    state.healer.travel.path = mActors.getPath(self, position)
    if state.healer.travel.path then
        mTools.debugPrint(string.format("%s will escape with angle %d at distance %d with %d points",
                actorId, math.deg(angle), distance, #state.healer.travel.path))
    end
end
...
local function healFriend(state)
    if not state.healer.woundedPartner then
        -- answer is too late?
        state.healer.woundedPartner:sendEvent("fairCare_clearHealer")
        return
    end
...
local function touchSpellSucceeded(state)
    ...
    local spellTouched = mActors.isCloseEnoughForTouchSpell(self, state.healer.woundedPartner, state.healer.travel.touchHealDistance)
...
local function onAnimationKey(state, _, key)
    ...
    if state.aiMode == mData.aiModes.Healing and state.healer.healing and key == state.healer.animConfig.releaseKey then
        if not isSelfHealing(state) and touchSpellSucceeded(state) then
...
local function handleHeal(state, deltaTime)
    if state.aiMode ~= mData.aiModes.Healing then
        if state.healer.helpAnswerTime then
            ...
            if state.healer.helpAnswerTime > mCfg.checkNeedsHealerRefreshRate / 2 then
                mTools.debugPrint(string.format("%s did not get any help answer from %s", actorId, mTools.actorId(state.healer.woundedPartner)))
    ...
    if isSelfHealing(state) then
        if state.ai.enemies and not state.healer.isHidden and state.anim.spellCastAttackGroups.spellcast and not state.healer.hasArrived then
            if state.healer.travel.path then
        ...
        if state.healer.hasArrived and moveOrHeal then
            mActors.faceToPoint(state.healer.controls, self, state.healer.woundedPartner:getBoundingBox().center, true, deltaTime)
        else
    ...
    if moveOrHeal and state.healer.hasArrived then
        state.healer.controls.movement = 0
    end
    local isReadyToCast = false
    if not moveOrHeal or state.healer.hasArrived then
        isReadyToCast = prepareToCast(state)
    end
