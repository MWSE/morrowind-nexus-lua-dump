local config = {
    ...
    -- With mods like Mercy CAO, the engine's AI is disabled and injured actors can rarely use they equipment items to heal themselves.
    -- If Mercy is detected, and an actor health is low, and the NPC has restore health items, then Mercy is disabled and default AI enabled for a fraction of a second.
    -- Health ratio threshold under which default AI will be re-enabled
    allowAiSelfHealHealthRatio = 0.5,
    ...
    -- When self-healing, actors move away to avoid blows, they run during a random number of seconds
    minEscapeTime = 2,
    maxEscapeTime = 4,
