...
local function setHealerChances(spellId, wounded, healer, healerState, healChances)
    ...
    if mActors.isOverEncumbered(healer) then
        if not mActors.isCloseEnoughForTouchSpell(healer, wounded, healerState and healerState.travel.touchHealDistance or mMagic.touchHealDistance(healer, wounded)) then
