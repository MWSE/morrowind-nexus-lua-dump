...
local function canAct(actor)
    if mMagic.isParalyzed(actor) then
        mTools.debugPrint(string.format("%s cannot heal because he is paralyzed", mTools.objectId(actor)))
        return false
    end

    local fatigue = T.Actor.stats.dynamic.fatigue(actor)
    if fatigue.current <= 0 or fatigue.base == 0 then
        mTools.debugPrint(string.format("%s cannot heal because he is knocked out", mTools.objectId(actor)))
        return false
    end

    if not isOverEncumbered(actor) and not T.Actor.canMove(actor) then
        mTools.debugPrint(string.format("%s cannot heal because seems to be knocked down", mTools.objectId(actor)))
        return false
    end
...
local function getPath(actor, position)
    ...
    if status == nearby.FIND_PATH_STATUS.PartialPath then
        ...
        mTools.debugPrint(string.format("%s found a partial path distant from %d to his destination", mTools.objectId(actor), distance))
...
local function getPathToTarget(actor, target, distanceTolerance)
    ...
    if status == nearby.FIND_PATH_STATUS.PartialPath then
        ...
        if distance <= distanceTolerance then
            mTools.debugPrint(string.format("%s found a partial path distant from %d to %s", mTools.objectId(actor), distance, mTools.objectId(target)))
        else
            return nil
        end
    elseif status ~= nearby.FIND_PATH_STATUS.Success then
...
local function getRandomDestinationAvoidingTargets(actor)
    ...
    --mTools.debugPrint(string.format("%s found escape angles from %d targets\nAngles %s\nTargets %s",
    ...
    --mTools.debugPrint(string.format("%s found escape point %s with angle %d", mTools.objectId(actor), destination, escapeAngle))
    return {
    ...
    if actor.type == T.Creature or target.type == T.Creature then
        ...
        elseif actor.type == T.Creature and target.type == T.Creature then
            if actorRecord.type == targetRecord.type then
                disp = disp + mSettings.getSection(mSettings.healingTweaksKey):get("creatureTypeDispositionBoost")
            end
        end
...
local function isHealAgainDelayOk(healDelayStartTime, chance)
    return healDelayStartTime + mSettings.getSection(mSettings.healingTweaksKey):get("timeBeforeHealAgainMaxChances")
...
local function canRegen(actor, record, state)
    return mSettings.getSection(mSettings.globalKey):get("healthRegenEnabled")
            and (not state.selfHealSpellId or mSettings.getSection(mSettings.globalKey):get("healthRegenForHealers"))
