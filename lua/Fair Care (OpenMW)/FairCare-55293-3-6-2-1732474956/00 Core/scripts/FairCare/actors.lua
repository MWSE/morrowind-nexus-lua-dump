...
local function getPath(actor, position)
    ...
    for _, point in ipairs(path) do
        table.insert(points, util.vector3(point.x, point.y, point.z))
    end
    return points
end
...
local function getRandomDestinationAvoidingTargets(actor)
    ...
    --        mTools.actorId(actor), #targets, aux_util.deepToString(targetAngles, 3), aux_util.deepToString(escapeAngles, 3)))
    ...
    --mTools.debugPrint(string.format("%s found escape point %s with angle %d", mTools.actorId(actor), destination, escapeAngle))
    return {
