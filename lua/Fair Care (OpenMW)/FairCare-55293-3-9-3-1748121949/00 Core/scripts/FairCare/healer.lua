...
local function clearState(state)
    if state.healer.woundedPartner then
        mTools.debugPrint(string.format("%s stops healing %s", actorId, mTools.objectId(state.healer.woundedPartner)))
    end
...
local function askHealMe(state, woundedPartner, healChances, woundedEnemies, hasFollowBounds)
    ...
    if not state.ai.enemies or mActors.isHealAgainDelayOk(state.healDelayStartTime, healChances.chance) then
        ...
        mTools.debugPrint(string.format("%s won't heal %s, too early, chances:%s",
                actorId, mTools.objectId(woundedPartner), mChances.toString(healChances)))
        woundedPartner:sendEvent("fairCare_answerHealMe", { healChances = healChances })
    end
end
...
local function setPathToFriend(state)
    ...
    if not state.healer.travel.path then
        mTools.debugPrint(string.format("%s cannot find a path to %s anymore", actorId, mTools.objectId(state.healer.woundedPartner)))
        return false
    end
    return true
end
...
local function forceOtherSpellcastAnimation(state)
    if isSelfHealing(state) then
        if state.anim.spellcastAnimKeys[mData.spellcastKeys.self] then
            return false
        end
        return state.anim.spellcastAnimKeys[mData.spellcastKeys.touch] and mData.spellcastKeys.touch or mData.spellcastKeys.target
    else
        if state.anim.spellcastAnimKeys[mData.spellcastKeys.touch] then
            return false
        end
        return state.anim.spellcastAnimKeys[mData.spellcastKeys.target] and mData.spellcastKeys.target or mData.spellcastKeys.self
    end
end
...
local function healFriend(state, woundedPartner)
    ...
    mTools.debugPrint(string.format("%s will try to heal %s", actorId, mTools.objectId(state.healer.woundedPartner)))
    state.healer.helpAnswerTime = nil
    ...
    if state.aiMode == mData.aiModes.Healing and state.healer.healing and key == state.healer.animConfig.releaseKey then
        if isSelfHealing(state) then
            if state.healer.forceOtherSpellcastAnimation then
                self:sendEvent("fairCare_applyFakeHealSpell", state.selfHealSpellId)
            end
        elseif touchSpellSucceeded(state) then
            state.healer.woundedPartner:sendEvent("fairCare_applyFakeHealSpell", state.touchHealSpellId)
        end
        clearState(state)
    end
end
...
local function updateHealAction(state, deltaTime)
    ...
    if isSelfHealing(state) then
        ...
        healChances.aborted = not mChances.setHealerChances(state.touchHealSpellId, state.healer.woundedPartner, self, state.healer, healChances)
    end
    ...
    if healChances.aborted or not healChances.isSuccess and not mChances.areNewChancesGoodEnough(healChances.chance, state.healer.recurrentChances.chance) then
        mTools.debugPrint(string.format("%s stops trying to heal %s. Chances = %s",
                actorId, mTools.objectId(state.healer.woundedPartner), mChances.toString(healChances)))
        return false
    end
    return true
end
...
local function handleHeal(state, deltaTime)
    if state.aiMode ~= mData.aiModes.Healing then
        if state.healer.helpAnswerTime then
            ...
            if state.healer.helpAnswerTime > mCfg.checkNeedsHealerRefreshRate / 2 then
                mTools.debugPrint(string.format("%s did not get any help confirmation answer from %s", actorId, mTools.objectId(state.healer.woundedPartner)))
    ...
    if isStuck(state, deltaTime) then
        ...
        elseif not state.healer.hasArrived then
            mTools.debugPrint(string.format("%s got stuck while trying to heal %s", actorId, mTools.objectId(state.healer.woundedPartner)))
            clearState(state)
            return
        end
    end
    ...
    if not state.healer.healing and state.healer.hasArrived and isReadyToCast then
        mTools.debugPrint(string.format("%s starts casting the spell on %s", actorId, mTools.objectId(state.healer.woundedPartner)))
        state.healer.animStartTime = core.getSimulationTime()
