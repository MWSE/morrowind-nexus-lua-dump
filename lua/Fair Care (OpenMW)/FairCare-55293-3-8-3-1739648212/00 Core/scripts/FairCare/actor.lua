...
local function clearState()
    mTools.debugPrint(string.format("Clearing %s's healing state...", actorId))
    ...
    if state.config.potions ~= cfgStates.set then
        if canHavePotions() then
            if state.config.potions == cfgStates.delay
                    and state.lastActiveGameTime + mSettings.getSection(mSettings.potionSettingsKey):get("potionRestockDelayHours") * 3600 < core.getGameTime() then
                state.config.potions = cfgStates.reset
            end
            if state.config.potions == cfgStates.reset then
                core.sendGlobalEvent("fairCare_clearActorData", { actor = self, types = { [mData.globalDataTypes.potions] = true }, callback = "fairCare_addPotions" })
            end
...
local function onCombatStart()
    mTools.debugPrint(string.format("%s starts combat", actorId))
...
return {
    ...
    eventHandlers = {
        Died = onDead,
        hbfs_onActorReady = function(data) onHBFSActorReady(data.settings, data.type) end,
        ...
        fairCare_clearPotionsState = clearPotionsState,
        fairCare_addPotions = addPotions,
        fairCare_onCombatStart = function() onCombatStart() end,
