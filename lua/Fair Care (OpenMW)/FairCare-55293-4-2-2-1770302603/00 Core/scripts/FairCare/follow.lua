...
local function addFollowTeamMembers(data)
    ...
    if followTeamRemainingRequests == 0 then
        mTools.debugPrint(string.format("%s follower team is %s", actorId, mTools.objectsIds(followTeam)))
...
local function clearFollowing(state, following)
    if mTools.areObjectEquals(following, state.follow.following) then
        mTools.debugPrint(string.format("%s lost his following %s", actorId, mTools.objectId(following)))
        state.follow.following = nil
    end
end
...
local function addFollower(state, follower)
    mTools.debugPrint(string.format("%s has a new follower %s", actorId, mTools.objectId(follower)))
...
local function clearFollower(state, follower)
    if state.follow.followers[follower.id] then
        mTools.debugPrint(string.format("%s lost his follower %s", actorId, mTools.objectId(follower)))
