...
local function getItemsValueSum(actor)
    ...
    for _, item in ipairs(inventory:getAll()) do
        sum = sum + (mTools.getRecord(item).value or 0)
    end
...
local function addPotions(actor)
    ...
    local expectedValueRatio = mSettings.getSection(mSettings.potionSettingsKey):get("maxPotionsTotalValueOverNpcWealth") / 100
    ...
    mTools.debugPrint(string.format("%s (%s HP) gained potions to restore %d%% of his HP up to %d total value:\n---- %s",
...
local settingsKeyFunctions = {
    [mData.globalDataTypes.potions] = function()
        ...
        return table.concat({
            ...
            tostring(potionSection:get("maxPotionsTotalValueOverNpcWealth")),
        }, ",")
    end
}
