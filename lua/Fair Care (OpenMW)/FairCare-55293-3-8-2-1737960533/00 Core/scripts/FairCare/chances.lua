...
local function getChanceImpact(typeKey)
    if mData.chanceTypes[typeKey].action == mData.actions.selfHeal then
        return mCfg.chanceImpacts[mSettings.getSection(mSettings.woundedImpactsKey):get(mSettings.getHealChanceImpactKey(typeKey))]
    else
        return mCfg.chanceImpacts[mSettings.getSection(mSettings.healerImpactsKey):get(mSettings.getHealChanceImpactKey(typeKey))]
    end
end
...
local function setSelfHealingChances(spellId, wounded, healChances)
    ...
    if mMagic.isSilenced(wounded) then
        mTools.debugPrint(string.format("%s cannot self-heal because he is silenced", mTools.objectId(wounded)))
        return false
    end
    ...
    if castChances <= 0 then
        mTools.debugPrint(string.format("%s cannot self-heal because cast chances are nul. Sound=%s", mTools.objectId(wounded), sound))
        return false
    end
...
local function setHealerChances(spellId, wounded, healer, healerState, healChances)
    -- SILENCE
    if mMagic.isSilenced(wounded) then
        mTools.debugPrint(string.format("%s cannot heal %s because he is silenced", mTools.objectId(healer), mTools.objectId(wounded)))
        return false
    end
    ...
    if castChances <= 0 then
        mTools.debugPrint(string.format("%s cannot heal %s. Sound = %s", mTools.objectId(healer), mTools.objectId(wounded), sound))
        return false
    end
    ...
    if mActors.isOverEncumbered(healer) then
        if not mActors.isCloseEnough(healer, wounded, healerState and healerState.travel.touchHealDistance or mMagic.touchHealDistance(healer, wounded)) then
            mTools.debugPrint(string.format("%s is over encumbered and too far to heal %s", mTools.objectId(healer), mTools.objectId(wounded)))
            return false
        else
            mTools.debugPrint(string.format("%s is over encumbered but close enough to heal %s", mTools.objectId(healer), mTools.objectId(wounded)))
        end
    else
        ...
        if path then
            ...
            add(healChances, mData.chanceTypes.healerTravelTime, 1 - travelTime / mSettings.getSection(mSettings.healingTweaksKey):get("travelTimeToHealMinChances"))
        else
            mTools.debugPrint(string.format("%s cannot find a path to %s", mTools.objectId(healer), mTools.objectId(wounded)))
            return false
        end
    end
...
local function selectActorsChances(wounded, actor, healChances)
    for _, spellId in ipairs(mData.selfTouchSpellIds) do
        if T.Actor.spells(actor)[spellId] then
            return setHealerChances(spellId, wounded, actor, nil, healChances)
        end
    end
    return false
end
