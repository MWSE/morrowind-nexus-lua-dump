...
local state = {
    config = {
        anim = mTypes.cfgStates.unset,
        spells = mTypes.cfgStates.unset,
        potions = mTypes.cfgStates.unset,
    },
    ...
    anim = mTypes.newAnimState(),
    health = nil,
    activeEffects = nil,
}

local firstActivation = true
local notPersistedStateKeys = { health = true, activeEffects = true }

local function setDynamicState()
    log("Initializing dynamic state...")
    state.ai = mAi.newAiState()
    state.wounded = mWounded.newWoundedState()
    state.healer = mHealer.newHealerState()
    state.follow = mFollow.newFollowState()
    state.anim = mTypes.newAnimState()
    state.config.anim = mTypes.cfgStates.reset
end

local function removeDynamicState()
    log("Clearing dynamic state...")
...
local function addTouchHealSpell()
    if state.touchHealSpellId then
        log("Adding touch healing spell")
...
local function removeTouchHealSpell()
    if state.touchHealSpellId then
        log("Removing touch healing spell")
...
local function isExcludedForCastingHeal()
    ...
    for _, spell in pairs(T.Actor.spells(self)) do
        if spell.type == core.magic.SPELL_TYPE.Spell then
            --log(string.format("Keeping creature \"%s\", as it can cast spells", self))
            return false
        end
    end
    log("Excluding creature as it cannot cast spells")
    return true
end
...
local function selectHealSpell(spellIds)
    ...
    for i, spellId in ipairs(spellIds) do
        if self.type ~= T.Creature or T.Actor.stats.level(self).current >= mCfg.minCreatureLevelForSpellIndex[i] then
            ...
            if mMagic.getSpellCost(spell) <= T.Actor.stats.dynamic.magicka(self).base then
                local castChance = mMagic.fFatigueBase * mMagic.calcAutoCastChance(spell, self) / 100
                --log(string.format("Casting chance for spell \"%s\" is %s", spell.id, castChance))
...
local function addHealSpells()
    ...
    if spellId then
        ...
        log(string.format("Already has a self healing spell \"%s\"", spellId))
    else
        state.selfHealSpellId = selectHealSpell(mTypes.selfHealSpellIds)
        if state.selfHealSpellId then
            log(string.format("Selected self healing spell \"%s\"", state.selfHealSpellId))
        end
    end
    state.touchHealSpellId = selectHealSpell(mTypes.selfTouchSpellIds)
    if state.touchHealSpellId then
        self:sendEvent(mDef.events.addTouchHealSpell)
        log(string.format("Selected touch healing spell \"%s\"", state.touchHealSpellId))
    end
end
...
local function removeHealingSpells()
    for _, spell in ipairs(T.Actor.spells(self)) do
        if string.sub(spell.id, 1, 9) == "fair care" then
            log(string.format("Removing old spell \"%s\"", spell.id))
...
local function hasHealingHandled()
    return state.anim.hasHealingAnimations and (state.selfHealSpellId or state.touchHealSpellId)
end
...
local function handleAnimationCapacities()
    ...
    if not state.anim.hasHealingAnimations then return end
    ...
    if state.anim.spellcastAttackGroups.spellcast then
        ...
        if not hasAtLeastOneSpellcastAnimation then
            log("Has the spellcast animation group but no animation keys")
            state.anim.hasHealingAnimations = false
        end
    end
end
...
local function canHavePotions()
    return not T.Actor.isDead(self)
            and self.type == T.NPC
            and mStore.settings.addingPotionsEnabled.value
            and (not state.selfHealSpellId or mStore.settings.potionsForHealers.value)
end

local function clearPotionsState()
    state.config.potions = mTypes.cfgStates.unset
end
...
local function configure()
    if state.config.anim then
        handleAnimationCapacities()
        state.config.anim = nil
    end

    if state.anim.hasHealingAnimations and state.config.spells then
        if state.config.spells == mTypes.cfgStates.reset then
            removeHealingSpells()
        end
        if not isExcludedForCastingHeal() then
            addHealSpells()
        end
        state.config.spells = nil
    end

    if state.config.potions then
        if canHavePotions() then
            if state.config.potions == mTypes.cfgStates.delay
                    and state.lastActiveGameTime + mStore.settings.potionRestockDelayHours.value * 3600 < core.getGameTime() then
                state.config.potions = mTypes.cfgStates.reset
            end
            if state.config.potions == mTypes.cfgStates.reset then
            ...
            if state.config.potions == mTypes.cfgStates.unset then
                addPotions()
            end
        end
        if state.config.potions ~= mTypes.cfgStates.delay then
            state.config.potions = nil
        end
    end
...
local function doRegenSinceLastActive()
    if state.health.current < state.health.base and mActors.canRegen(state, self, self.type.record(self)) then
        local passedSeconds = (core.getGameTime() - state.lastActiveGameTime) / core.getGameTimeScale()
        log(string.format("Was inactive %.1f seconds and will regenerate his health accordingly", passedSeconds))
...
local function onCombatStart()
    ...
    state.config.potions = mTypes.cfgStates.delay
end
...
local function onActive()
    if not I.HBFS or self.type == T.Player then
        onActorActive()
    end
end
...
local function onHBFSActorReady(_, eventType)
    if eventType == "reset" then
        state.config.spells = mTypes.cfgStates.reset
        state.config.potions = mTypes.cfgStates.reset
    end
    ...
    -- If HBFS rescales actors, we'll update spells on next activation
    if eventType ~= "rescale" then
        onActorActive()
    end
end
...
local function onInactive()
    if state.ai and mTools.isObjectInvalid(self.object) then
        log("Self object is inactive and invalid")
...
local function onUpdate(deltaTime)
    if deltaTime == 0 or not state.ai then return end
    ...
    mAi.onUpdate(state, deltaTime)
    mWounded.onUpdate(state, deltaTime)
    if hasHealingHandled() then
        mHealer.onUpdate(state, deltaTime)
    end
end

local function onInit()
    state.health = T.Actor.stats.dynamic.health(self)
    state.activeEffects = T.Actor.activeEffects(self)
end

local function onSave()
    return {
        state = mTools.addAllToMap({}, state, function(k, _) return not notPersistedStateKeys[k] end),
...
return {
    ...
    eventHandlers = {
        ...
        [mDef.events.healFriend] = function(woundedPartner) mHealer.heal(state, woundedPartner) end,
        ...
        [mDef.events.addFollowTeamMembers] = function(data) mFollow.addFollowTeamMembers(state, data) end,
