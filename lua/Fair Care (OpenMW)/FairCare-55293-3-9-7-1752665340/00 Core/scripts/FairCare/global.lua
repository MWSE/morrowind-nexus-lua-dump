...
local function addPotions(actor)
    if actor.type == T.NPC and T.NPC.isWerewolf(actor) then
        log(string.format("NPC \"%s\" won't get any potions because he's a werewolf", actor.recordId))
        return
    end
    if actor.type.record(actor).mwscript == "slavescript" then
        log(string.format("NPC \"%s\" won't get any potions because he's a slave", actor.recordId))
        return
    end
...
local function cleanState()
    ...
    if count ~= 0 then
        log(string.format("Removed %d invalid tracked actors", count))
    end
end
