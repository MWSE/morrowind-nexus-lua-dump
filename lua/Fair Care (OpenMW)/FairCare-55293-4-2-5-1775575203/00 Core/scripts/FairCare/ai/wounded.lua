...
local function checkHealRequests(state, injured, deltaTime)
    ...
    for _, answer in ipairs(state.wounded.healAnswers) do
        if injured and not foundHealer and answer.healChances.isSuccess then
            log(string.format("Will be healed by %s, chances:%s", mTools.objectId(answer.healerPartner), mChances.toString(answer.healChances)))
            ...
            log(string.format("Won't be healed by %s, injured:%s, chances:%s",
                    mTools.objectId(answer.healerPartner), injured, mChances.toString(answer.healChances)))
            answer.healerPartner:sendEvent(mDef.events.declineHealHelp, self)
        end
    end
...
local function isNonHealingCreature(actor, record)
    return actor.type == T.Creature and not mStore.settings[mStore.getCreatureHealKey(record.type)].value
end
...
local function selectSelfHealer(state, healRequests)
    ...
    if not state.selfHealSpellId
            or not mStore.settings.selfHealingEnabled.value
            or isNonHealingCreature(self, actorRecord)
    ...
    if healChances.zeroed then
        log(string.format("Will not try to heal himself, zeroed chances:%s", mChances.toString(healChances)))
...
local function selectHealer(state, actor, healRequests, hasFollowBounds)
    if actor.type == T.Player
            ...
            or isNonHealingCreature(actor, actor.type.record(actor))
    ...
    if healChances.zeroed then
        log(string.format("Won't be healed by %s, zeroed chances:%s", mTools.objectId(actor), mChances.toString(healChances)))
        healChances.aborted = true
    end
    if not healChances.aborted then
        mChances.setChance(healChances)
...
module.sendHealRequests = function(state, followTeam)
    ...
    if mStore.settings.touchHealingEnabled.value then
