...
module.isOverEncumbered = function(actor)
...
module.canAct = function(actor)
    ...
    if not module.isOverEncumbered(actor) and not T.Actor.canMove(actor) then
...
module.getPath = function(actor, position)
...
module.getPathToTarget = function(actor, target, distanceTolerance)
    local status, path = nearby.findPath(
            actor.position,
            target.position,
            {
...
module.getRetreatPosition = function(actor)
...
module.isCloseEnough = function(actor, target, distance)
    return (actor.position - target.position):length() <= distance
end

module.actorHalfHeight = function(actor)
...
module.getPathTravelTime = function(actor, path)
...
module.faceToPoint = function(controls, actor, target, deltaTime)
    local pitch, yaw = actor.rotation:getAnglesXZ()
    local position = target.position and module.actorHalfHeight(target) or actorDistantHalfHeight(actor, target)
    local deltaPos = position - module.actorHalfHeight(actor)
...
module.travel = function(controls, actor, path, deltaTime)
    if #path == 1 then
        controls.movement = 0
        return true
    end
    if (actor.position - path[2]):length() < (#path == 2
        ...
        module.faceToPoint(controls, actor, path[2], deltaTime)
    end
    controls.movement = 1
    return false
end

module.travelToTarget = function(controls, actor, path, target, targetDistanceTolerance, deltaTime)
    if #path == 1 or (actor.position - target.position):length() < targetDistanceTolerance then
        module.faceToPoint(controls, actor, module.actorHalfHeight(target), deltaTime)
        controls.movement = 0
...
module.getDisposition = function(actor, target)
    if actor.type == T.NPC and target.type == T.Player then
        return mTools.clamp(T.NPC.getDisposition(actor, target), 0, 100)
    end

    local actorRecord, targetRecord = actor.type.record(actor), target.type.record(target)
    ...
    if actor.type == T.Creature or target.type == T.Creature then
        ...
        elseif actor.type == T.Creature and target.type == T.Creature then
            if actorRecord.type == targetRecord.type then
                disp = disp + mStore.settings.creatureTypeDispositionBoost.value
            end
        end
...
module.isHealAgainDelayOk = function(state, chance)
    return state.healDelayStartTime + mStore.settings.timeBeforeHealAgainMaxChances.value
            * (1 - chance) ^ 0.5 < core.getSimulationTime()
end

module.getCastAnimationConfig = function(attackAnimations, forcedSpellcastKey, isSelfSpell)
...
module.canRegen = function(state, actor, record)
    return mStore.settings.healthRegenEnabled.value
            and (not state.selfHealSpellId or mStore.settings.healthRegenForHealers.value)
            and mStore.settings[mStore.getActorTypeRegenKey(actor.type, record.type)].value
end

module.doRegen = function(state, actor, deltaTime)
    state.health.current = math.min(
            state.health.base,
            state.health.current
                    + mStore.settings.healthRegenRatio.value
                    * deltaTime
