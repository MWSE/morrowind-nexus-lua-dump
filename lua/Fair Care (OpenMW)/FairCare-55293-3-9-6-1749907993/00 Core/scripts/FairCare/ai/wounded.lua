...
local function handleHealingItems(state)
    if state.health.current / state.health.base > mCfg.trySelfHealingItemHealthRatio
            or state.health.current == 0 then
        return false
    end
...
local function checkNeedsHealing(state, deltaTime)
    ...
    if state.health.current >= state.health.base then return end
