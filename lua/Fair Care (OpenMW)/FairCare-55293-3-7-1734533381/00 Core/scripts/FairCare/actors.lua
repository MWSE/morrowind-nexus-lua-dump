...
local function getPathToTarget(actor, target, distanceTolerance)
    ...
    if status ~= nearby.FIND_PATH_STATUS.Success or path == nil or #path == 0 then
        if (actor.position - target.position):length() <= distanceTolerance then
            mTools.debugPrint(string.format("%s has no path to %s but is close enough", mTools.objectId(actor), mTools.objectId(target)))
