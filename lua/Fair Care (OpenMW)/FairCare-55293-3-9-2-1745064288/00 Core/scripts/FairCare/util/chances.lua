...
local function setChance(state, healChances)
    local chance = 1
...
local function toString(state, healChances)
    local msg = {}
...
local function setSelfHealingChances(spellId, wounded, healChances)
    ...
    add(healChances, mTypes.chanceTypes.woundedHealth, 1 - health.current / health.base)

    return true
end
module.setSelfHealingChances = setSelfHealingChances

local function setHealerChances(state, spellId, wounded, healer, healerState, healChances)
    -- SILENCE
    if mMagic.isSilenced(wounded) then
...
local function selectActorsChances(state, wounded, actor, healChances)
    for _, spellId in ipairs(mTypes.selfTouchSpellIds) do
        if T.Actor.spells(actor)[spellId] then
            return setHealerChances(state, spellId, wounded, actor, nil, healChances)
        end
    end
    return false
end
