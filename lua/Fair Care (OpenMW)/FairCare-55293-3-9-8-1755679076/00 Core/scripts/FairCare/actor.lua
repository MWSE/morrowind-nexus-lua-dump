...
local notPersistedStateKeys = { health = true, settings = true }
...
local function doRegenSinceLastActive()
    if state.health.current < state.health.base and mActors.canRegen(state, self, actorRecord) then
...
local function onUpdate(deltaTime)
    if deltaTime == 0 or not isHandled() then return end
