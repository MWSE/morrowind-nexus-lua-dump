    ...
    for _, itemType in ipairs({ T.Armor, T.Book, T.Clothing, T.Potion, T.Weapon }) do
        for _, item in ipairs(T.Actor.inventory(actor):getAll(itemType)) do
            if itemType == T.Book or itemType == T.Potion or T.Actor.hasEquipped(actor, item) then
                local record = mUtil.getRecord(item)
                local effects
                local available
                ...
                if effects and available and filter(effects) then
                    mUtil.debugPrint(string.format("\"%s\" has an available \"%s\" \"%s\"", mUtil.getRecord(actor).id, mUtil.itemTypes[itemType], record.id))
                    return true
                end
            end
        end
...
local function hasEnoughMagicka(actor, spellId)
    local spell = core.magic.spells.records[spellId]
    local magicka = T.Actor.stats.dynamic.magicka(actor).current
    return magicka > spell.cost
end
module.hasEnoughMagicka = hasEnoughMagicka
...
local function calcWeakestSchool(spell, actor)
    ...
    for _, effect in ipairs(spell.effects) do
        ...
        local s = 0
        local skillBase = getActorSkillBase(actor, effect.effect.school)
        if skillBase then
            s = 2 * skillBase
        end
...
local function touchHealTargetDistance(actor, target)
    ...
    return actorBounds.halfExtents.y + targetBounds.halfExtents.y + fCombatDistance
end
