...
local function getAvailableEquipmentWithEffects(actor, filter, effectName)
    ...
    if count ~= 0 then
        mTools.debugPrint(string.format("%s has %d available items with %s effect", mTools.objectId(actor), count, effectName))
    end
    return items
end
...
local function getPotionStats(potionId)
...
module.getPotionStats = getPotionStats
...
local function getSelfHealingEquipment(actor)
...
module.getSelfHealingEquipment = getSelfHealingEquipment
...
local function getBestPotion(potions)
    local bestValue = 0
    local bestPotion
    for _, potion in ipairs(potions) do
        local value = getEffectsAverageRestoredHealthPerSec(mTools.getRecord(potion).effects)
...
module.getBestPotion = getBestPotion
