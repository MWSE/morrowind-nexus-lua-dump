...
local function speakRaw(text)
	...
	stopIfSpeaking()
	local line = text
	line = string.gsub(line,		'<[Ff][Oo][Nn][Tt].+[Ff][Aa][Cc][Ee].*=.*"[Dd][Aa][Ee][Dd][Rr][Ii][Cc]">(.+)</[Ff][Oo][Nn][Tt]>',
...
local function journal(e)
	...
	if text
	and (not string.multifind(string.lower(text),
		{'dummy','dialog filter','only a test'}, 1, true)) then
...
-- 0 = dialog, 1 = voice, 2 = greeting, 3 = persuasion/service, 4 = journal
		and (infoType >= tes3_dialogueType_voice)
		and (infoType <= tes3_dialogueType_service) then
...
local function answerMouseClick(e)
	...
	if string.lower(s) == 'continue' then
		return
	end
...
local function checkReadElement(el, text, ref)
	...
	if (string.len(text) < 2)  -- skip some empty pregreeting/hello
	or (string.lower(text) == 'continue') then -- skip single continue choice
		el:setPropertyBool(alreadyReadId, true)
...
local function multistartswith(s, t)
	for i = 1, #t do
		if string.startswith(s, t[i]) then
			return true
		end
	end
	return false
			...
			if text then
				...
				if id == idMenuDialog_header then
					if readDialog > 1 then
						...
						if ok then
							dontStop = checkReadElement(node, text, mobRef) -- so it does not stop on next idMenuDialog_hyper one
						end
					end
				...
				elseif id == idMenuDialog_hyper then
					if readDialog > 0 then
						...
						local lcMobRefId = string.lower(mobRefId)

						if string.startswith(lcMobRefId, 'dagoth_ur_1') then
						...
						--[[if responsePlayingRefsDict[mobRef] then
							if logLevel >= 4 then
								mwse.log('%s: updateDialog() skipping "%s" as already talking',
									modPrefix, mobRefId)
							end
							ok = false -- skip actor already playing voiceover
							skipNextNotify = true -- test will this work?
							---responsePlayingRefsDict = {}
							responsePlayingRefsDict[mobRef] = nil
						end]]
					...
					if node.visible then
						if config.readDialogChoice then
							if skipNextNotify then
								...
								if string.find(string.lower(text), 'goodbye', 1, true) then
									skipNextNotify = false
								else
