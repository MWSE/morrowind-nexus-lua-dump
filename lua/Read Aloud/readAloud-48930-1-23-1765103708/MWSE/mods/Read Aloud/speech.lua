...
if comInterface
and (comInterface:GetVoices().Count == 0) then
	comInterface = nil
end
...
function speech.getFiltered(text, logLevel)
	...
	local ptrn = "[%^%%]([%w_]+)"
	local oldLine = line
	line = line:gsub(ptrn, expandGlobalVar)
	if logLevel5
	and ( not (line == oldLine) ) then
		mwselog("oldLine:\n%s\npattern = %s\newline:\n%s", oldLine, ptrn, line)
	end
...
function speech.speak(text, params, logLevel)
	...
	if logLevel1 then
		if not ( text:len() == oldText:len() ) then
			mwselog('oldText =\n%s\ntext =\n%s', oldText, text)
		end
	end
	if not line then
    ...
    if SAPI then
        local SpeechVoiceSpeakFlags = SAPI.SpeechVoiceSpeakFlags
        if SpeechVoiceSpeakFlags then
            local SVSFlagsAsync = SpeechVoiceSpeakFlags.SVSFlagsAsync
            if SVSFlagsAsync then
                success = pcall(
                    function ()
                        comInterface:Speak(line, SVSFlagsAsync)
                    end
                )
            end
        end
	...
	and ( not tes3.menuMode() ) then
		...
		tes3ui.showNotifyMenu('Read Aloud: WARNING! speech unavailable probably due to Low Memory condition')
	end

    -- will this help reducing memory usage?
	---collectgarbage()
	---collectgarbage()
end
...
function speech.isSpeaking()
    ...
    if not comInterface.Status then
        return false
    end
    if not SAPI.SpeechRunState then
        return false
    end
