...
local function answerMouseClick(e)
	...
	local s = stripPrefixNumber(text)
	if not s then
		return
	end
	...
	local msec = math.round(220 * timePerChar * numChars * speedDelta) + 1050
...
local function infoResponse(e)
	...
	if command
	and string.find(string.lower(command),'say%s-,?%s-"') then
...
local function infoFilter(e)
	...
	--[[if not tes3ui.menuMode() then
		return -- nope voices start before menu opening
	end]]
	local ref = e.reference
	if not ref then
		return
	end
	responsePlayingRefId = ref.id
	if config.logLevel >= 3 then
		mwse.log('%s: infoFilter(e) "%s" voice filtered out', modPrefix, responsePlayingRefId)
	end	
end
					...
					if node.visible then
						...
						elseif readDialog > 2 then
							if skipNextNotify then
								...
								dontStop = checkReadElement(node, stripPrefixNumber(text))
							end
						else
							skipNextNotify = false
						end
					...
					if readDialog > 0 then
						...
						if responsePlayingRefId then
							...
							if mob_ref_id == responsePlayingRefId then
								ok = false -- skip actor already playing voiceover
								if logLevel >= 4 then
									mwse.log('%s: updateDialog(e) skipping %s as already talking', modPrefix, mob_ref_id)
								end
							end
