...
local function doAutosave()
    ...
    local charName = tes3.mobilePlayer.object.name
    local cellName = tes3.mobilePlayer.cell.name
    local cellDisplayName = tes3.mobilePlayer.cell.displayName
    local file = string.gsub(string.lower(charName .. cellName .. os.time()), "%s+", "")
    local name = charName .. " " .. cellDisplayName
    -- Otherwise just do the autosave.
    tes3.saveGame({ file = file, name = name})
end
...
local function cellChangeEvent(e)
    ...
    if config.dontSaveOnExtTransitions then
        local wasInterior = e.previousCell.isInterior -- would fail if we didn't ignore the first run
        local isInterior = e.cell.isInterior
...
event.register("initialized", function() mwse.log("[SimpleAutoSave] v1.0 Initialized") end)
