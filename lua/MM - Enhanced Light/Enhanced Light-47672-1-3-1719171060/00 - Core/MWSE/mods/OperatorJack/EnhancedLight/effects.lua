...
local function onTick(e)
    ...
    if (isLit == false and e.effectInstance.state ~= tes3.spellState.retired) then
        ---@type tes3effect | nil
        local effect = framework.functions
                           .getEffectFromEffectOnEffectEvent(e, tes3.effect.magelight)
...
local function addEffect()
    local vfx = {
        area = {id = "VFX_OJ_EL_LightArea", mesh = "OJ\\EL\\LightArea.nif"},
        cast = {id = "VFX_OJ_EL_LightCast", mesh = "OJ\\EL\\LightCast.nif"},
        hit = {id = "VFX_OJ_EL_LightHit", mesh = "OJ\\EL\\LightHit.nif"}
    }
    for _, obj in pairs(vfx) do
        local object = tes3.getObject(obj.id)
        if (object == nil) then
            tes3.createObject({objectType = tes3.objectType.static, id = obj.id, mesh = obj.mesh})
        end
    end
