...
local debtStarted = false
local persist

if interfaces.CharacterTraits then
    settings.debugPrint("CharacterTraits mod found. Creating trait.")
    interfaces.CharacterTraits.addTrait {
        id = MOD_NAME,
        type = "background",
        name = localization("backgroundName"),
        description = localization("backgroundDescription", { startDebt = settings.main.startDebt }),
    ...
    settings.debugPrint("CharacterTraits mod not found. Enabling debt.")
    debtStarted = true
end
...
local function onExitingInterior(data)
    ...
    if not persist.enabled then
        settings.debugPrint("debt is disabled")
        return
    end
    ...
    local spawnPoint = destPosition + forward + (util.vector3(math.random(), math.random(), 0) * (spawnDist / 4))

    local spot = safeLocation(spawnPoint, destPosition, spawnDist)
    if spot then
        spawn(destCell, spot)
    else
