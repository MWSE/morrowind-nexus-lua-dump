	...
	if refreshCycle%4 == 3 or detectItemOrKeyChanged then
		...
		if detectItem>0 or SHOW_KEYS and detectKey>0 or herbRange > 0 then
			for _, cont in pairs(nearby.containers) do
				if (cont.position - cameraPos):length() < math.max(detectItem,detectKey, herbRange) * 22+500 and types.Container.objectIsInstance(cont) then
					if not types.Container.inventory(cont):isResolved() then
						...
						if types.Container.record(cont).isOrganic and not organicContainers[cont.recordId] then
							if miningNodes[cont.recordId] then
								table.insert(HUDMarkers, {object = cont, icon = ORE_COLORS and "HUDM_Textures/"..ICON_SET.."/"..miningNodes[cont.recordId]..".dds" or ORE_ICON, range = math.max(detectItem,detectIngredient), bonusSize = 7, boundingBoxCenter = true})
...
local function onFrame(dt)
	...
	detectIngredient =playerSection:get("DETECT_INGREDIENT_MULT") * ((playerSection:get("SHOW_INGREDIENTS") and itemMagnitude or 0) + detectIngredientBonus)
	detectHerb = playerSection:get("DETECT_HERB_MULT") * ((playerSection:get("SHOW_HERBS") and itemMagnitude or 0) + detectHerbBonus)
	...
	if true or playerSection:get("RAYTRACING") then
		...
		for i=1,30 do
			...
			if not raytracing[nextRay]  or raytracing[nextRay].distance > playerSection:get("MAX_DISTANCE") then
				
			else
				rayCounter = rayCounter + 1
				--print("queuing "..raytracing[nextRay].object.id)
				--print(camera.getPosition(), raytracing[nextRay].objectPos)
				--local rayTarget = (raytracing[nextRay].objectPos-camera.getPosition()):normalize():emul(v3(playerSection:get("MAX_DISTANCE")))
				local rayTarget = nil
		...
		for a,b in pairs(raytracing) do
			if b.distance > playerSection:get("MAX_DISTANCE") then
				raytracing[a] = nil
			end
		end
...
local function recheckObject(object)
	...
	if types.Item.objectIsInstance(object) then
		if detectItem > 0 or SHOW_KEYS and detectKey>0 then
			for i, tbl in pairs( interfaceMarkers.itemHUDMarkers) do
				if tbl.object.id == objectId then
