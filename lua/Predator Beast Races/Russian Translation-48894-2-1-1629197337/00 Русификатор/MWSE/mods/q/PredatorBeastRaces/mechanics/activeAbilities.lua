	...
	if tes3.mobilePlayer:isAffectedByObject(abilities[e.keyCode]) then
		...
		if e.keyCode == common.settings.scentKey.keyCode and
		   ...
		   common.isKhajiit(tes3.player) then
			...
			common.message("Вы не можете использовать эту способность под водой.")
			return
		end
		...
		if currentFatigue < minFatigue then
			...
			common.message("Ваш запас сил слишком мал, чтобы использовать эту способность.")
			return
		end
			...
			callback = function ()
				...
				if currentFatigue < minFatigue then
					...
					if e.keyCode == common.settings.scentKey.keyCode then
						...
						common.message("Ваш запас сил слишком мал, нюх отключен.")

					elseif e.keyCode == common.settings.visionKey.keyCode then

						common.message("Ваш запас сил слишком мал, улучшенное зрение отключено.")
