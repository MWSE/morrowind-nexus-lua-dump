...
function deform.getControlPoints()
    return controlPoints or {}
end

-- Get the indices of the triangle containing the given world position
-- Returns nil if no triangle contains the point (extrapolation case)
-- Also returns the triangle used for extrapolation as second value
function deform.getCurrentTriangleIndices(worldX, worldY)
    if not triangles or #triangles == 0 then
        return nil, nil
    end

    -- First check if inside any triangle
    for _, tri in ipairs(triangles) do
        local p1 = controlPoints[tri[1]]
        local p2 = controlPoints[tri[2]]
        local p3 = controlPoints[tri[3]]
        ...
        if w1 and deform.isInsideTriangle(w1, w2, w3) then
            return { tri[1], tri[2], tri[3] }, nil
        end
    end

    -- Outside all triangles - find nearest for extrapolation
    local bestTri = nil
    local bestDist = math.huge
    ...
    if bestTri then
        return nil, { bestTri[1], bestTri[2], bestTri[3] }
    end
