...
local TEMP_EFFECT_CHECK_INTERVAL = 0.25
local tempEffectCheckTimer = math.random() * TEMP_EFFECT_CHECK_INTERVAL
...
local function onUpdate(dt)
    ...
    tempEffectCheckTimer = tempEffectCheckTimer + (tonumber(dt) or 0)
    if tempEffectCheckTimer < TEMP_EFFECT_CHECK_INTERVAL then return end

    while tempEffectCheckTimer >= TEMP_EFFECT_CHECK_INTERVAL do
        tempEffectCheckTimer = tempEffectCheckTimer - TEMP_EFFECT_CHECK_INTERVAL
    end
...
local function runtimeSaysPlayerHasStaff()
    return get('active', false) and get('staffRecordId', nil) ~= nil
end

local function isStaffAttack(attack)
    -- The player script already gates Runtime_Staves.active with its equipped
    -- staff detector. If OpenMW/GRIP hands us an unreadable generated weapon in
    -- the Hit payload, trust that runtime state instead of dropping the perk.
    if not attack or not attack.weapon then return runtimeSaysPlayerHasStaff() end
    if not types.Weapon.objectIsInstance(attack.weapon) then return runtimeSaysPlayerHasStaff() end
    local ok, record = pcall(types.Weapon.record, attack.weapon)
    if not ok or not record then return runtimeSaysPlayerHasStaff() end
    if isStaffRecord(record) then return true end

    local original = gripOriginalRecordFor(weaponIdCandidates(attack.weapon, record))
    return isStaffRecord(original) or runtimeSaysPlayerHasStaff()
end
...
local function onHit(attack)
    ...
    -- equipped enchanted staff. The global script performs the inventory write.
