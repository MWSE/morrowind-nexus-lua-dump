...
local function setItemEnchantmentCharge(eventData)
    if not eventData or not eventData.item or eventData.value == nil then return end

    local ok, idata = pcall(types.Item.itemData, eventData.item)
    if not ok or not idata then return end

    local value = tonumber(eventData.value)
    if value == nil then return end

    -- Current OpenMW exposes enchant charge as ItemData.enchantmentCharge.
    -- Keep the legacy charge fallback guarded for older API builds.
    local wrote = false
    if idata.enchantmentCharge ~= nil then
        idata.enchantmentCharge = value
        wrote = true
    end
...
return {
    ...
    eventHandlers = {
        ...
        Staves_SetItemEnchantmentCharge = setItemEnchantmentCharge,
    },
}
