...
local function onLoaded(e)
	if not tes3.player.data.mwsePerks then 
		tes3.player.data.mwsePerks = table.copy( common.defaultplayerData)
		tes3.player.data.mwsePerks.perks = {}
		tes3.player.data.mwsePerks.activatedPerks = {}
	end
	common.playerData = tes3.player.data.mwsePerks
	
	--Remove perks without valid IDs
	for _, perkData in pairs(common.perkList) do
		perkData.activated = false
	end
	local deadPerks = {}
	for perk, active in pairs( common.playerData.perks) do
		if not common.perkList[perk] then
			table.insert(deadPerks, perk)
		end
	end
	for _, perk in ipairs(deadPerks) do
		common.playerData.perks[perk] = nil
	end
	
	for perk, active in pairs( common.playerData.activatedPerks) do
		if active and common.perkList[perk] then 
			common.perkList[perk].activated = true
		end
	end
...
public = {
	...
	grantPerk = function(perkID) 
		if not common.perkList[perkID] then common.err("attempted to grant nonexistent perk \"" .. id .. "\" to the player") return false end
		if not  common.playerData.perks[perkID] then 
			 common.playerData.perks[perkID] = true
			if not (common.perkList[perkID].delayActivation) then 
				event.trigger("KBPerks:activatePerk", {id = perkID})
			end
		end
		return true
	end,
	
	removePerk = function(perkID)
		 common.playerData.perks[perkID] = nil
		event.trigger("KBPerks:deactivatePerk", {id = perkID})
	end,
