...
local maxEnchantDef = 6553
local maxEnchantArmor = 10737418 

local function getRemoveImprLimit()
    return storage.globalSection("SettingsAetheriusAltar"):get("RemoveImprLimit")
end
...
local function createEnchantObject(data)
    ...
    --print(item.recordId)
    if getRemoveImprLimit() == false then
        if contains(itemHistoryList, item.recordId) then
            -- if is in the list, return
            world.players[1]:sendEvent('showPlayerMsg', {msg="I have already worked with this item, bring me something new"})
            return
        end
    end
    ...
    local newEnchantCapacity = originalRecord.enchantCapacity * data.enchantCapMult
    print(newEnchantCapacity)
    --print(item.type)
    if (item.type == types.Armor) then -- For some reason armor allows significantly larger enchantment point values, 2^32
        if (newEnchantCapacity > maxEnchantArmor) then
            world.players[1]:sendEvent('showPlayerMsg', {msg="You wish to improve this armor beyond what I am capable of"})
            newEnchantCapacity = maxEnchantArmor
            print(newEnchantCapacity)
        end
    elseif newEnchantCapacity > maxEnchantDef then
        world.players[1]:sendEvent('showPlayerMsg', {msg="You wish to improve this beyond what I am capable of"})
        newEnchantCapacity = maxEnchantDef
        print(newEnchantCapacity)
    end
    local itemTable = {enchantCapacity = newEnchantCapacity, template = originalRecord}
