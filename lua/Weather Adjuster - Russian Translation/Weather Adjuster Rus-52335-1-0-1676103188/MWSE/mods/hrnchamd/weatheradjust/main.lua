...
local weatherNames = { "Чисто", "Облачно", "Туманно", "Пасмурно", "Дождь", "Гроза", "Пепельная буря", "Мор", "Снег", "Метель" }
...
local function refreshEditor()
    ...
    local weatherTxt
    if (wc.nextWeather) then
        weatherTxt = string.format("%s в %s", weatherNames[wc.currentWeather.index+1], weatherNames[wc.nextWeather.index+1])
    else
...
local function createTabRegions()
    ...
    for region in tes3.iterate(tes3.dataHandler.nonDynamicData.regions) do
        ...
        local itemRegion = item:createLabel{ text = "- " .. (config.regions[region.id] or "по умолчанию") }
        ...
        item:register(tes3.uiEvent.mouseClick, function(e)
            if (isShiftPressed()) then
                ...
                -- Load region preset.
                switchPreset(config.regions[region.id] or "по умолчанию")
            end
        end)
    end
    ...
    if (this.lerp == nil and not wc.nextWeather) then
        if (this.secondTransition) then
            ...
            if (config.messageOnRegionChange) then
                tes3.messageBox{ message = "Weather Adjuster переход завершен." }
            end
        end
    end
end
...
local function init()
    ...
    mwse.log("[Weather Adjuster] v%s загружен успешно.", verString)
end
