...
zoomOutWheelSteps = 3, -- 1 .. 8
zoomStartRate = 1.8, -- 0.5 .. 4
zoomStartRateAuto = 0.4, -- 0.25 .. 3
zoomEndRate = 0.76, -- 0.25 .. 2
zoomInAmount = 2.2, -- 1 .. 4
zoomOutAmount = 0.052, --0.0625
...
autoInteriorCameraOffsetY = 128,
autoInterior1stPerson = true,
autoZoomDelay = 1.8,
autoZoom = true,
logLevel = 0,
}
...
Now including "Close Inspection" like auto-zoom on activation target change, and other camera position tweaks.
...
A value higher than "Zoom in end rate" is suggested.]],
		...
		decimalPlaces = 2, min = 0.25, max = 4, step = 0.01, jump = 0.05
	})
...
A value lower than "Zoom in start rate" is suggested.]],
		configKey = 'zoomEndRate',
		decimalPlaces = 2, min = 0.25, max = 3, step = 0.01, jump = 0.05
	})
...
Affects (constant) speed of zoom-out when zoomed in/1st person view.]],
	...
	sideBarPage:createYesNoButton({
		label = 'Auto Zoom on target cursor change.',
		description = [[Toggle Automatic Zoom on target cursor change.]],
		configKey = 'autoZoom',
	})

	sideBarPage:createSlider({
		label = 'Auto Zoom in start rate: %s',
		description = [[Auto zoom in starting rate.
Affects starting speed of auto zoom-in when zoomed out/1st person view.]],
		configKey = 'zoomStartRateAuto',
		decimalPlaces = 2, min = 0.25, max = 3, step = 0.01, jump = 0.05
	})

	sideBarPage:createSlider({
		label = 'Auto Zoom on target delay %s sec',
		description = [[Automatic Zoom on target cursor after set seconds delay.
similar/alternative to old pre MWSE-Lua classic "Close Inspection" mod.]],
		configKey = 'autoZoomDelay',
		decimalPlaces = 2, min = 0, max = 4, step = 0.01, jump = 0.05
	})
	...
	sideBarPage:createSlider({
		...
		decimalPlaces = 2, min = 1, max = 6, step = 0.01, jump = 0.05
	})

	sideBarPage:createMouseBinder({
		label = 'Camera Reset Mouse Combo',
		description = [[Mouse button combo to reset the camera position/zoom.
The combo must include a mouse button and cannot include the Alt key.]],
	...
	sideBarPage:createYesNoButton({
		label = 'Automatic 1st person view entering interiors',
		description = [[Automatically toggle to 1st person view when entering interiors.]],
		configKey = 'autoInterior1stPerson',
	})

	sideBarPage:createSlider({
		label = 'Automatic interior negative camera Y offset: %s',
		description = [[Automatic camera negative Y offset after entering an interior cell (0 = disable).
Useful to try and reduce 3rd person view camera positioning out of entered interior walls.]],
		configKey = 'autoInteriorCameraOffsetY',
