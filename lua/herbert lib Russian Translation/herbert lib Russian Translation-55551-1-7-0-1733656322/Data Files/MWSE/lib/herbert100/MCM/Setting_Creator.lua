...
local MCM_log = Herbert_Logger.new{mod_name="herbert lib", mod_dir="lib/herbert100", file_path="MCM/Setting_Creator.lua"}
...
local SC = require("herbert100.Class").new{name="Setting_Creator",
...
function SC:add_log_settings(mod_name)
    ...
    if Herbert_Class.is_instance_of(mod_name, Herbert_Logger) then ---@cast mod_name herbert.Logger
        return mod_name:add_to_MCM{component=self.component, config=self.config}
    end

    local logger = Herbert_Logger.get(mod_name or self:get_mod_name())
    if logger then
        return logger:add_to_MCM{component=self.component, config=self.config}
    end

    local mod_info = require("herbert100").get_mod_info()
    if mod_info then
        logger = Herbert_Logger.get_by_dir(mod_info.mod_dir) 
            or Herbert_Logger.get(mod_info.short_mod_name) 
            or Herbert_Logger.get(mod_info.mod_name)

        mod_name = mod_name or mod_info.short_mod_name or mod_info.mod_name or self:get_mod_name()
    end

    logger = logger or Herbert_Logger.new(mod_name)
    return logger:add_to_MCM{component=self.component, config=self.config}
end
...
---@return herbert.MCM.new_slider.params
function SC:update_slider_parameter(p)
    local dist = p.max and (p.max - (p.min or 0))
    if p.max and not p.dp then
        if dist <= 10 then
            p.dp = 1
        elseif dist <= 2 then
            p.dp = 2
        end
    elseif p.dp and not p.max then
        if p.dp == 2 then
            p.max = 100
        elseif p.dp == 1 then
            p.max = 10
...
function SC:new_slider(p)
    ---@diagnostic disable-next-line: param-type-mismatch
    return self.component:createSlider(self:update_slider_parameter(p))
end
...
function SC:new_pslider(p)
    ...
    p.dp = p.dp or 0
    ---@diagnostic disable-next-line: param-type-mismatch
    return self.component:createPercentageSlider(self:update_slider_parameter(p))
end
...
---@field options nil|string[]|mwseMCMDropdownOption[]|{[1]: string, [2]: number|string}[] options for this setting. 
...
function SC:new_dropdown(p)
    ---@diagnostic disable-next-line: param-type-mismatch
    return self.component:createDropdown(self:update_dropdown_options(p))
end
...
function SC:new_textfield(p)
    p.converter = p.converter or p.numeric and tonumber or nil
    ---@diagnostic disable-next-line: param-type-mismatch
    return self.component:createTextField(self:update_parameter(p))
end
