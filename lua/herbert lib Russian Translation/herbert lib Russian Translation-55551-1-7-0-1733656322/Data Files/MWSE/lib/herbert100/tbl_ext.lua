...
---@param tbl T|{[K]: V}
---@return boolean
function tbl_ext.is_empty(tbl) return next(tbl) == nil end
...
---@param sep string?
---@return string
function tbl_ext.concat(arr, sep)
	return table.concat(arr, sep)
end

tbl_ext.concat = table.concat
...
---@param value V
---@return boolean
function tbl_ext.contains(tbl, value)
...
function tbl_ext.values(tbl, sorter)
	...
	if type(tbl) == "table" then
		iterator, state = pairs(tbl)
		setmetatable(arr, getmetatable(tbl) or tbl_ext.metatable)
	else
		iterator = tbl
		setmetatable(arr, tbl_ext.metatable)
	end

	local i = 0
	for _, v in iterator, state do i = i + 1; arr[i] = v end
	if sorter then
		if sorter == true then table.sort(arr) else table.sort(arr, sorter) end
	end
	return arr
end


tbl_ext.to_array = tbl_ext.values
...
function tbl_ext.keys(tbl, sorter)
	...
	local i = 0
	for k in iterator, state do i = i + 1; arr[i] = k end
	if sorter then
		if sorter == true then table.sort(arr) else table.sort(arr, sorter) end
	end
	return arr
...
-- this is just `table.bininsert`, but with type hints
---@generic K, V, T
---@param tbl T|V[]
---@param value V
---@param comp nil|(fun(a: K, b: K): boolean|any)
---@return integer index
function tbl_ext.bin_insert(tbl, value, comp)
	return table.bininsert(tbl, value, comp)
end

tbl_ext.bin_insert = table.bininsert
tbl_ext.bininsert = table.bininsert

-- this is just `table.bininsert`, but with type hints
---@generic K, V, T
---@param tbl T|V[]
---@param value V
---@param comp nil|(fun(a: K, b: K): boolean|any) comparison to use when sorting
---@param find_all boolean? find all matches?
---@return integer? min_index
---@return integer? max_index
function tbl_ext.bin_search(tbl, value, comp, find_all)
	return table.binsearch(tbl, value, comp, find_all)
end

-- this is just `table.remove`, but with type hints
---@generic K, V, T
---@param arr T|V[] array to remove a value from
---@param index integer? index to remove. if not provided, will default to `#arr`
---@return V? value value that got removed (if the index was in range)
function tbl_ext.remove(arr, index) end
tbl_ext.remove = table.remove


tbl_ext.bin_search = table.binsearch
tbl_ext.binsearch = table.binsearch
...
---@generic T, K, V, R
--- @param tbl T|{[K] : V}
--- @param map (fun(k: K, v:V, ...): R) map to apply to keys
--- @param ... any
---@return T|{[R] : V}
function tbl_ext.map_keys2(tbl, map, ...)
	local res = {}
	for k, v in pairs(tbl) do
		res[map(k, v, ...)] = v
	end
...
-- map (key, value) pairs in one table to another table
-- NOTE: function acts on BOTH keys and values
---@generic T, KEY_TYPE, VAL_TYPE, NEW_KEY_TYPE, NEW_VAL_TYPE
--- @param tbl T|{[KEY_TYPE] : VAL_TYPE}
--- @param key_map (fun(k: KEY_TYPE, v:VAL_TYPE, ...): NEW_KEY_TYPE) map to apply to keys
--- @param val_map (fun(v:VAL_TYPE, ...): NEW_VAL_TYPE) map to apply to values
--- @param ... any
---@return T|{[NEW_KEY_TYPE] : NEW_VAL_TYPE}
function tbl_ext.map_keys2_and_vals1(tbl, key_map, val_map, ...)
	local res = {}
	for k, v in pairs(tbl) do
		res[key_map(k, v, ...)] = val_map(v, ...)
	end
	return setmetatable(res, getmetatable(tbl) or tbl_ext_meta)
end
...
--- @param tbl T|{[KEY_TYPE] : VAL_TYPE}
--- @param key_map (fun(k: KEY_TYPE, v:VAL_TYPE, ...): NEW_KEY_TYPE) map to apply to keys
--- @param val_map (fun(k: KEY_TYPE, v:VAL_TYPE, ...): NEW_VAL_TYPE) map to apply to values
--- @param ... any
---@return T|{[NEW_KEY_TYPE] : NEW_VAL_TYPE}
function tbl_ext.map_keys2_and_vals2(tbl, key_map, val_map, ...)
	local res = {}
	for k, v in pairs(tbl) do
		res[key_map(k, v, ...)] = val_map(k, v, ...)
	end
	return setmetatable(res, getmetatable(tbl) or tbl_ext_meta)
end
...
---@generic V, T
--- @param arr V[]
--- @param pred (fun(i: integer, v:V, ...): boolean|any) predicate to satisfy
--- @param ... any
---@return V[]|T
...
---@param key K
---@param val V
function tbl_ext.iadd(tbl, key, val) 
    tbl[key] = tbl[key] + val
end

--- performs in-place subtraction: `tbl[key] = tbl[key] - val`
---@generic K, V, T
---@param tbl T|{[K]: V}
---@param key K
---@param val V
function tbl_ext.isub(tbl, key, val) 
    tbl[key] = tbl[key] - val
end

--- performs in-place multiplication: `tbl[key] = tbl[key] * val`
---@generic K, V, T
---@param tbl T|{[K]: V}
---@param key K
---@param val V
function tbl_ext.imul(tbl, key, val) 
    tbl[key] = tbl[key] * val
end

--- performs in-place division: `tbl[key] = tbl[key] / val`
---@generic K, V, T
---@param tbl T|{[K]: V}
---@param key K
---@param val V
function tbl_ext.idiv(tbl, key, val) 
    tbl[key] = tbl[key] / val
end

-- get a random element from this table. this runs in O(n) time.
---@generic K, V, T
---@param tbl T|{[K]: V}
---@return V?
function tbl_ext.choice(tbl)
	local s = 0
	for _ in pairs(tbl) do s = s + 1 end
	if s == 0 then return end
	local key, val, i, j = nil, nil, 0, math.random(1, s)
	repeat key, val = next(tbl, key); i = i + 1 until i == j
	return val
end

-- get a random element from this array. this runs in O(1) time.
---@generic V, T
---@param arr T|V[]
---@return V
function tbl_ext.choice_array(arr) return arr[1] and arr[math.random(1, #arr)] or nil end
...
--- Set a value in a table, handling nested subtables.
---@generic K, V
---@param tbl {[K]: V}
---@param key string|K
---@param val V? should missing entries be created? default: true
---@return V? val
function tbl_ext.recursive_set(tbl, key, val)
	if type(key) ~= "string" then tbl[key] = val; return val end
