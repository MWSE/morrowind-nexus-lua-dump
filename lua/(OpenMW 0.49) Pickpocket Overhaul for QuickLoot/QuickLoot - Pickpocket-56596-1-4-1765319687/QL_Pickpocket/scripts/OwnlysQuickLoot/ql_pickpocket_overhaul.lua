...
local function getFatigueTerm(self, target)
	local minMod = QLPP_FATIGUE_MIN_MODIFIER
	local maxMod = QLPP_FATIGUE_MAX_MODIFIER
...
local function awardExp(self, target, item, mult, add)
	...
	local add = add or 0
	local skillGain =  mult * (qlpp.calcDifficulty(self,target,item) * QLPP_EXPERIENCE_MULT + QLPP_EXPERIENCE_ADD)+add
	if QLPP_DEBUG_MODE then
		print("########################")
		print(" + "..math.floor(skillGain*100)/100 .." exp")
	end
	if QLPP_SECURITY_SKILL then
...
local function buildCache(self, target)
	...
	qlpp.cache.targetSkill = tarSkill * QLPP_TARGET_SKILL_MULT
	...
	qlpp.cache.selfSkill = (selfSkill + qlpp.cache.baseSkill) * QLPP_OWN_SKILL_MULT
	qlpp.cache.maxAttempts = QLPP_MAX_THEFTS_PER_NPC + math.floor(qlpp.cache.baseSkill / QLPP_SKILL_FOR_BONUS_ATTEMPTS)
	...
	if math.abs(relativeAngle) < math.pi/4 then
		 qlpp.cache.chanceFacingMult = QLPP_FRONT_MODIFIER
	elseif math.abs(relativeAngle) > 3*math.pi/4 then
		qlpp.cache.chanceFacingMult = QLPP_BACK_MODIFIER
	else
		local mod = (math.abs(relativeAngle)-math.pi/4)/(math.pi/2)
		qlpp.cache.chanceFacingMult = (1-mod)*QLPP_FRONT_MODIFIER+mod * QLPP_BACK_MODIFIER
	end
	...
	if not npcRecord or not npcRecord.servicesOffered.Barter then
		...
		qlpp.cache.merchantAdd = QLPP_MERCHANT_ADD
		qlpp.cache.merchantMult = QLPP_MERCHANT_MULT
	end
	
	if  types.Actor.getStance(target) ~= types.Actor.STANCE.Nothing then
		qlpp.cache.inCombatModifier = QLPP_IN_COMBAT_MODIFIER or 50
	else
...
qlpp.calcDifficulty = function(self,target,item,dbg,deposit)
	local difficulty = QLPP_BASE_DIFFICULTY
	if dbg then
		print("-----")
	...
	if item then
		...
		elseif deposit and types.Potion.objectIsInstance(item) then
			...
			for _, effect in pairs(record.effects) do
				if effect.effect.harmful then
					...
					--end
					x= math.max(0,x)
					calculatedCost = calculatedCost + x*QLPP_POISON_DIFFICULTY
				end
			end
		...
		local valueAdd = QLPP_VALUE_ADD
		local valueExp = QLPP_VALUE_EXP
		local valueMult = QLPP_VALUE_MULT
		if dbg then		
		...
		local weightMult = QLPP_WEIGHT_MULT
		...
		if types.Actor.hasEquipped(target, item) then
			difficulty = difficulty + QLPP_EQUIPPED_MULT
			if dbg then print("* "..QLPP_EQUIPPED_MULT.." (equipped)") end
			if types.Weapon.objectIsInstance(item) then
				difficulty = difficulty + (QLPP_EQUIPPED_WEAPON_BONUS)
				if dbg then print("+ "..QLPP_EQUIPPED_WEAPON_BONUS.." (equipped weapon)") end
			elseif types.Clothing.objectIsInstance(item) and (record.type == types.Clothing.TYPE.Ring or record.type == types.Clothing.TYPE.Ring or item.TYPE == types.Clothing.TYPE.Amulet) then
				difficulty = difficulty + (QLPP_EQUIPPED_JEWELRY_BONUS)
				if dbg then print("+ "..QLPP_EQUIPPED_JEWELRY_BONUS.." (equipped jewelry)") end
			else--if types.Armor.objectIsInstance(item) then
				difficulty = difficulty + (QLPP_EQUIPPED_ARMOR_BONUS)
				if dbg then print("+ "..QLPP_EQUIPPED_ARMOR_BONUS.." (equipped armor)") end
			end
		end
	elseif dbg then
		print(difficulty)
...
qlpp.calcChance = function(self, target, item, benchmark, dbg, deposit)
	...
	if not QLPP_DEBUG_MODE then dbg = false end
	...
	if dbg then
		...
		print(qlpp.cache.selfSkill .." own skill (* "..QLPP_OWN_SKILL_MULT..")")
		print("= "..math.floor(chance*100)/100 .."%")
	end
	
	local maxChance = chance * QLPP_BACK_MODIFIER
	--maxChance = maxChance * QLPP_FATIGUE_MAX_MODIFIER
	
	chance = chance * qlpp.cache.chanceFacingMult
	chance = math.floor(chance * qlpp.cache.chanceFatigueMult+0.5)
	local attempts = savegameData[target.id] or 0
	chance = chance - QLPP_USED_ATTEMPT_MODIFIER *  attempts
	if dbg then
		print("* ".. math.floor(qlpp.cache.chanceFacingMult*100)/100 .."(facing)")
		print("* ".. math.floor(qlpp.cache.chanceFatigueMult*100)/100 .."(fatigue)")
		if attempts > 0 and QLPP_USED_ATTEMPT_MODIFIER > 0 then 
			print("- "..(QLPP_USED_ATTEMPT_MODIFIER *  attempts).."% ("..attempts.." attempts used)")
		end
		print("= "..chance.."%")
	end
	...
	chance = math.min(chance, QLPP_MAX_PICKPOCKET_CHANCE)
...
qlpp.stealItem = function(self, target, item)
	...
	if math.random() * 100 < chance then
		...
		awardExp(self,target,item, QLPP_FAILED_ATTEMPT_EXPERIENCE_MULT*chance/100)
		local minSkillForNoPunishment = QLPP_MIN_SKILL_FOR_NO_PUNISHMENT
		...
		pickpocketAttempts = pickpocketAttempts + QLPP_FAILED_ATTEMPT_COST
		savegameData[target.id] = pickpocketAttempts
		updateFooterText(target)
		return false
	end
...
qlpp.validateTarget = function(self, target, input)
	...
	if inCombat then
		local requiredSkill = QLPP_SKILL_FOR_IN_COMBAT
		local selfSkill
	...
	--local pickpocketAttempts = savegameData[target.id] or 0
	--local maxAttempts = QLPP_MAX_THEFTS_PER_NPC
...
qlpp.filterItems = function(self, target, containerItems)
	if not target:isValid() or target.count == 0 then return {} end
	...
	if qlpp.showContents or qlpp.cache.baseSkill >= QLPP_SKILL_TO_BYPASS_SKILL_CHECK or revealChance >= 100 then
		...
		if not qlpp.filteredItems then
			...
			local visibilityChance = QLPP_ITEM_VISIBILITY_MULT / qlpp.cache.baseSkill

			
			for _, item in pairs(containerItems) do
				local _, maxItemChance = qlpp.calcChance(self, target, item)
				if maxItemChance < QLPP_ONLY_SHOW_ABOVE_CHANCE or math.random() > visibilityChance then -- [string "scripts/ownlysquickloot/ql_pickpocket_overhau..."]:544: attempt to compare nil with number
		...
		if qlpp.undisplayedItems > 0 then
			if #tempContainerItems == 0 then
				...
				--qlpp.message = qlpp.message .." (min. "..math.floor((minSkill+1+QLPP_ONLY_SHOW_ABOVE_CHANCE)/qlpp.cache.chanceFacingMult).." skill)"
				qlpp.message = qlpp.message .." (min. "..math.ceil(QLPP_ONLY_SHOW_ABOVE_CHANCE/ QLPP_BACK_MODIFIER+minDifficulty).." skill)"
			else
