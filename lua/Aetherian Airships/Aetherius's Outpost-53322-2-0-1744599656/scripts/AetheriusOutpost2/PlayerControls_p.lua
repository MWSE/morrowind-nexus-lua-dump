...
local uiUtil = require("scripts.AetheriusOutpost2.util.ui")
...
local function onKeyPress(k)
    if k.code == keys.Mount and inControlRange() then
        if airshipMode then
            ...
            ASKP = true
            drawUI()
            core.sendGlobalEvent("lockShipObject", false)
        elseif airshipOb and ownedShips[airshipOb.id] then
            POVAirship()
            drawUI()
            core.sendGlobalEvent("lockShipObject", true)
        end
    end
    if airshipMode then
        local keyData = {}
...
local function compassActivate(recordId)
    ...
    elseif recordId == "zhac_ae_compass_silver" and not ownedShips[recordId] and not hasKey("zhac_shipkey_1") == 0 then
        ui.showMessage("You don't have control of this ship.")
    elseif recordId == "zhac_ae_compass_silver" and not ownedShips[recordId] and hasKey("zhac_shipkey_1") then
        ...
        drawUI()
        core.sendGlobalEvent("lockShipObject", true)
    elseif recordId == "zhac_ae_compass_steam" and not ownedShips[recordId] and not hasKey("zhac_shipkey_2") == 0 then
        ui.showMessage("You don't have control of this ship.")
    elseif recordId == "zhac_ae_compass_steam" and not ownedShips[recordId] and hasKey("zhac_shipkey_2") then
        ownedShips[recordId] = true
        ownedShips[airshipOb.id] = true
        ui.showMessage("You take control of the ship")
        POVAirship()
...
local function collisionCheck(data)
    ...
    for index, value in pairs(data) do
        if endsWith(index, "inner") then
            ...
            local outer = "zhac_collisionchecker_" .. idNum .. "_outer"
            if data[outer] then
                local rayC = nearby.castRay(data[index], data[outer])
                if rayC.hit then
                    hit = true
                    if rayC.hitObject then
                        if rayC.hitObject.recordId ~= "zhac_silvercascade_ship" and rayC.hitObject.recordId ~= "zhac_dship_x" then
...
local function onUpdate(dt)
    ...
    if airshipMode then
        ...
        core.sendGlobalEvent("airshipKeysPressed", { keyData = keyData, dt = dt })
    else
        local keyData = {}
        for index, value in pairs(keys) do
...
return {
    interfaceName = "AOutpost_PlayerControls",
    interface = {
        version = 1,
        onInit = onInit,
