--[[
    ...
    Handles:
      ...
      - Trade Ledger snapshot generation
...
local LOYALTY_TIERS = {
    ...
    { threshold = 50000, name = "Honored",   messageKey = "loyalty_honored",  discount = 5 },
}
...
local TRAVEL_TIERS = {
    ...
    { threshold = 12, name = "Favored Passenger",  discount = 10, voucherChance = 0.05, messageKey = "travel_favored" },
...
local function calcDispositionGain(absValue, dispScale)
    if absValue < 10 then return 0 end
    return (math.log(absValue) / math.log(10)) * dispScale
end
...
local function sendMessage(player, message, kind)
    ...
    if kind == "smallTransaction" and not getSetting("showSmallTransactionMessages") then
        return
    end
...
local function processLoyalty(merchant, player, absValue, isBuying, regionKey)
    ...
    local data = loyaltyData[merchantId]
    data.name = merchantDisplayName(merchant)
    data.id = merchantId
...
local function onTransaction(data)
    ...
    if gain <= 0 then
        sendMessage(player, lfmt("msg_transaction_too_small", merchantName), "smallTransaction")
    ...
    -- remember the gain so the restore/leak-neutralization pass can set the
...
local function getTierIndexForValue(tiers, value)
    local tierIdx = 0
    for i, tier in ipairs(tiers) do
        if value >= tier.threshold then tierIdx = i end
    end
    return tierIdx
end

local function decorateTierSnapshot(snapshot, tiers, value)
    local tierIdx = snapshot.tier or getTierIndexForValue(tiers, value or 0)
    snapshot.tier = tierIdx
    snapshot.tierName = tierIdx > 0 and tiers[tierIdx].name or "None"
    local nextTier = tiers[tierIdx + 1]
    if nextTier then
        snapshot.nextName = nextTier.name
        snapshot.nextThreshold = nextTier.threshold
        snapshot.nextRemaining = math.max(0, nextTier.threshold - (value or 0))
    end
    return snapshot
end
...
local function sortedTop(entries, valueKey, maxEntries)
    local out = {}
    for key, data in pairs(entries or {}) do
        local value = data[valueKey] or 0
        if value > 0 then
            out[#out + 1] = {
                id = data.id or key,
                name = data.name or data.id or key,
                spent = data.spent,
                trips = data.trips,
                tier = data.tier or 0,
    ...
    while #out > (maxEntries or 5) do table.remove(out) end
    return out
end

local function buildMerchantLedgerSnapshot(merchant, player)
    if not merchant or not types.NPC.objectIsInstance(merchant) then return nil end
    local merchantId = merchant.recordId
    local data = loyaltyData[merchantId] or { spent = 0, tier = 0 }
    local snapshot = decorateTierSnapshot({
        name = merchantDisplayName(merchant),
        id = merchantId,
        spent = data.spent or 0,
        tier = data.tier or 0,
        rebatePct = getMerchantDiscountPct(merchantId),
        disposition = types.NPC.getBaseDisposition(merchant, player),
        dailyGain = dailyDispGains[merchantId] or 0,
        dailyCap = (getSetting("dailyDispCap") or 0) > 0 and getSetting("dailyDispCap") or nil,
        disabled = not getSetting("enableLoyalty"),
    }, LOYALTY_TIERS, data.spent or 0)
    return snapshot
end

local function buildRegionalLedgerSnapshot(merchant)
    local regionKey, regionName = getRegionForMerchant(merchant)
    if not regionKey then return nil end
    local data = regionalData[regionKey] or { spent = 0, tier = 0, name = regionName }
    return decorateTierSnapshot({
        name = data.name or regionName or regionKey,
        id = regionKey,
        spent = data.spent or 0,
        tier = data.tier or 0,
        welcomeDisp = getRegionalWelcomeDisposition(regionKey),
        rebatePct = getRegionalRebatePct(regionKey),
        disabled = not getSetting("enableRegionalLoyalty"),
    }, REGIONAL_TIERS, data.spent or 0)
end

local function buildTransportLedgerSnapshot(operator)
    if not operator or not types.NPC.objectIsInstance(operator) then return nil end
    local key = getTravelKey(operator)
    local serviceType = normalizeServiceType(operator)
    local operatorRecord = types.NPC.record(operator)
    local operatorName = (operatorRecord and operatorRecord.name) or operator.recordId or key
    local data = travelLoyaltyData[key] or { trips = 0, tier = 0, vouchers = 0 }
    return decorateTierSnapshot({
        name = operatorName,
        id = key,
        ...
        tier = data.tier or 0,
        discount = getTravelDiscount(key),
        vouchers = data.vouchers or 0,
        voucherChance = getTravelVoucherChance(key),
        disabled = not getSetting("enableTransportLoyalty"),
    }, TRAVEL_TIERS, data.trips or 0)
end

local function onRequestLedger(data)
    ...
    local merchantSpent, regionSpent, totalTrips, totalVouchers = 0, 0, 0, 0
    for _, entry in pairs(loyaltyData or {}) do
        merchantSpent = merchantSpent + (entry.spent or 0)
    end
    for _, entry in pairs(regionalData or {}) do
        regionSpent = regionSpent + (entry.spent or 0)
    end
    for _, entry in pairs(travelLoyaltyData or {}) do
        totalTrips = totalTrips + (entry.trips or 0)
        totalVouchers = totalVouchers + (entry.vouchers or 0)
    end

    local topMerchants = sortedTop(loyaltyData, "spent", 5)
    for _, entry in ipairs(topMerchants) do
        local tierIdx = entry.tier or 0
        entry.tierName = tierIdx > 0 and LOYALTY_TIERS[tierIdx].name or "None"
    end

    local topRegions = sortedTop(regionalData, "spent", 5)
    for _, entry in ipairs(topRegions) do
        local tierIdx = entry.tier or 0
        entry.tierName = tierIdx > 0 and REGIONAL_TIERS[tierIdx].name or "None"
    end

    local topTransport = sortedTop(travelLoyaltyData, "trips", 5)
    for _, entry in ipairs(topTransport) do
        local tierIdx = entry.tier or 0
        entry.tierName = tierIdx > 0 and TRAVEL_TIERS[tierIdx].name or "None"
    end

    player:sendEvent("FairTrade_LedgerSnapshot", {
        currentMerchant = buildMerchantLedgerSnapshot(merchant, player),
        region = buildRegionalLedgerSnapshot(merchant),
        transport = buildTransportLedgerSnapshot(operator),
        summary = {
            merchantCount = countTableEntries(loyaltyData),
            regionCount = countTableEntries(regionalData),
            transportCount = countTableEntries(travelLoyaltyData),
            merchantSpent = merchantSpent,
            regionSpent = regionSpent,
...
return {
    ...
    eventHandlers = {
        ...
        FairTrade_RequestLedger          = onRequestLedger,
    },
}
