...
local function debug(actor, e)
    ...
    if storage.playerSection("Settings_PGFOMW_ZDebug_Key_KINDI"):get("Debug") then
        ui.showMessage(string.format("%s of %s class from %s attacks %s", guard.name, guard.class, actor.cell.name,
            agg.name))
    end
end
...
return {
    engineHandlers = {
        onActive = function()
            local str = string.format("[%s] mod requires OpenMW version 0.48 or newer!", modInfo.MOD_NAME)
