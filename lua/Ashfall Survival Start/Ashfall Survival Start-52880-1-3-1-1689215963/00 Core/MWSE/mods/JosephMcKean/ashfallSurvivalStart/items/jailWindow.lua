...
local function activateJailWindow(reference)
	...
	if not inAnchoressCell() then
		if not tes3.player.data.ass.hasMap then tes3.messageBox("Something is inside there.") end
	else
		local readiedWeapon = tes3.mobilePlayer.readiedWeapon
		if readiedWeapon and readiedWeapon.object and readiedWeapon.object.id:lower():find("pick") then
			tes3.messageBox("I can use the pick axe to break this wall.")
		else
			tes3.messageBox("I need a pick axe to break this wall.")
		end
	end
end

CraftingFramework.StaticActivator.register({ objectId = jailWindowId, name = "Cell Wall", onActivate = activateJailWindow })
