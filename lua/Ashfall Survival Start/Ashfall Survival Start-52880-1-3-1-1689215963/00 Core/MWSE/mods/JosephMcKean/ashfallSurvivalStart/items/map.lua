...
local function onUiEvent(e)
	...
	if not tes3.player.cell then return end
	if not tes3.player.cell.id:startswith("Masartus") then return end
	if tes3.player.data.ass and tes3.player.data.ass.hasMap then return end
...
local function readMap(e)
	...
	if e.book.id ~= "jsmk_ass_bk_map" then return end
	tes3.player.data.ass.hasMap = true
	if tes3.getJournalIndex({ id = "jsmk_ass" }) >= 40 then return end
