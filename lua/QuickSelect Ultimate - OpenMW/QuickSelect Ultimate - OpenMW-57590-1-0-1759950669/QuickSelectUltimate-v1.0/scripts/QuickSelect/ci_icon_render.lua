...
local function imageContent(resource, half, customSize)
    local size = customSize or getIconSize()
    local opacity = 1
    if half then
        opacity = 0.5
    end
...
local function getItemIcon(item, half, selected, size)
    local itemIcon = nil
    ...
    if selected then
        selectedContent = imageContent(selectionResource, half, size)
    end
    local context = ui.content {
        selectedContent,
        imageContent(magicIcon, half, size),
        imageContent(itemIcon, half, size),
        textContent(tostring(text))
    }

    return context
end
local function getSpellIcon(iconPath, half, selected, size)
    local itemIcon = nil
