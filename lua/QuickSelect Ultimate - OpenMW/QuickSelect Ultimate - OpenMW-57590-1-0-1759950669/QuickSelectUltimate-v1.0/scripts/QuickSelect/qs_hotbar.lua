...
local function createHotbarItem(item, xicon, num, data, half)
    ...
    local padding = utility.renderItemBoxed(ui.content { boxedIcon },
        util.vector2(sizeX * 1.6, sizeY * 1.6),
...
return {
    ...
    engineHandlers = {
        ...
        onKeyPress = function(key)
            if I.QuickSelect_Win1.isMessageBoxOpen() or I.QuickSelect_Win1.isQuickSelectOpen() then
                return
            end
            ...
            if settings:get("useArrowKeys") then
                ...
                elseif key.code == input.KEY.Enter then
                    ...
                    endPickingMode()
                elseif key.code == input.KEY.Mouse3 then
                    if not settings:get("enableMiddleMouse") then
                        return
                    end
                    if not enableHotbar then
                        return
        ...
        onControllerButtonPress = function(btn)
            ...
            if btn == input.CONTROLLER_BUTTON.DPadLeft then
                if not settings:get("enableGamepadControls") then
                    return
                end
                selectNextOrPrevHotKey("prev")
        ...
        onMouseWheel = function(delta)
            if not settings:get("enableMouseControls") then
                return
            end
            if I.QuickSelect_Win1.isMessageBoxOpen() or I.QuickSelect_Win1.isQuickSelectOpen() then
                return
