...
local function onInputAction(action)
    if I.QuickSelect_Hotbar and I.QuickSelect_Hotbar.isBlockedMode() then return end
    if action >= input.ACTION.QuickKey1 and action <= input.ACTION.QuickKey9 then
        ...
        if slot then
            ...
            local maxBars = tonumber(keyboardSettings:get("hotbarCount") or storage.playerSection("SettingsQuickSelect"):get("hotbarCount")) or 3
            if slot > 0 and slot <= maxBars and modifierPressed then
                selectedPage = slot - 1
                I.QuickSelect_Hotbar.drawHotbar()
                return
            end
            I.QuickSelect_Storage.equipSlot(slot + (selectedPage * 10))
            if I.QuickSelect_Hotbar and I.QuickSelect_Hotbar.endPickingMode then
                I.QuickSelect_Hotbar.endPickingMode()
            elseif I.QuickSelect_Hotbar and I.QuickSelect_Hotbar.resetActivity then
                I.QuickSelect_Hotbar.resetActivity()
                I.QuickSelect_Hotbar.drawHotbar()
            end
        end
    end
