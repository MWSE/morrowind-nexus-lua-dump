...
local drawSpellSelect
local drawItemSelect
local hoveredOverId
...
local lastSpellHash = 0

local powersExpanded = settings:get("spellsExpandedDefault")
local spellsExpanded = settings:get("spellsExpandedDefault")
local enchantmentsExpanded = settings:get("spellsExpandedDefault")
...
local function mouseMove(mouseEvent, data)
    ...
    local sdata = data.data or (data.props and data.props.spellData)
    if not sdata then return end
    
    if sdata.item then
        tooltip = utility.drawListMenu(tooltipData.genToolTips(sdata.item),
...
local function showQuickMenuMessageBox(slot)
    local slotData = I.QuickSelect_Storage.getFavoriteItemData(slot)
    local confirmK = keyboardSettings:get("confirmKey")
    local confirmB = gamepadSettings:get("confirmButton")
    messageBoxTextLines = { "Quick Menu Selection", "W/S or DPad Up/Down to navigate", string.format("%s or %s to confirm", confirmK, confirmB), " " }
    messageBoxButtons = { core.getGMST("sQuickMenu2"), core.getGMST("sQuickMenu3") }
    if slotData and (slotData.item or slotData.spell or slotData.enchantId) then
        table.insert(messageBoxButtons, core.getGMST("sQuickMenu4"))
    end
    table.insert(messageBoxButtons, core.getGMST("sCancel"))
...
local function mouseClick(mouseEvent, data)
    if not data then return end
    local sdata = data.data or (data.props and data.props.spellData)
    if sdata and sdata.header then
        if sdata.header == "powers" then
            powersExpanded = not powersExpanded
        elseif sdata.header == "spells" then
            spellsExpanded = not spellsExpanded
        elseif sdata.header == "enchantments" then
    ...
    if sdata then
        if not slotToSave then
            showQuickMenuMessageBox(sdata.num)
            slotToSave = sdata.num
        else
            local saved = false
            if sdata.item then
                I.QuickSelect_Storage.saveStoredItemData(sdata.item.recordId, slotToSave)
                saved = true
            elseif sdata.id then
                if sdata.enchant then
                    I.QuickSelect_Storage.saveStoredEnchantData(sdata.enchant, sdata.id, slotToSave)
                else
                    I.QuickSelect_Storage.saveStoredSpellData(sdata.id, "Spell", slotToSave)
                end
                saved = true
            end
            ...
            if saved then
                ...
                I.UI.setMode()
                slotToSave = nil; spellMode = false; itemMode = false
            else
                -- If we clicked an empty slot but we were in selection mode, maybe we want to pick this new slot?
                if sdata.num then
                     slotToSave = sdata.num
                     showQuickMenuMessageBox(sdata.num)
                     drawQuickSelect()
...
local function mouseMoveButton(event, data)
    ...
    if spellMode and sdata and sdata.spellIndex and sdata.spellIndex ~= selectedSpellIndex then
        selectedSpellIndex = sdata.spellIndex
        drawSpellSelect()
    end
end
...
local function createItemIcon(item, spell, num, isSelected)
    local icon
    local baseSize = utility.iconSize * 1.5
    local iconSize = baseSize  -- No zoom for selected
    local borderSize = baseSize + 4
    ...
    local padding
    if isSelected then
        -- Highlight selected items without resizing or moving
        padding = utility.renderItemBoxed(ui.content { boxedIcon },
            util.vector2(borderSize, borderSize),
            I.MWUI.templates.boxSolid)
    else
        padding = utility.renderItemBoxed(ui.content { boxedIcon },
            util.vector2(borderSize, borderSize),
        ...
        if data then
            ...
            elseif data.spell or data.enchantId or (data.spellType and data.spellType:lower() == "enchant") then
                ...
                elseif data.spellType:lower() == "enchant" then
                    local enchant = utility.getEnchantment(data.enchantId)
                    if enchant then
                        local effect = enchant.effects[1]
                        icon = effect.effect.icon
                    end
...
local function buildInv()
    ...
    for _, item in ipairs(allInv) do
        ...
        if item.type == types.Weapon or item.type == types.Armor then
            include = true
        elseif item.type == types.Clothing then
            if ench then include = true end -- Include all enchanted jewelry (On Use, Constant, etc)
        elseif item.type == types.Book then
            if ench then include = true end -- Include Scrolls
        elseif item.type == types.Potion or item.type == types.Ingredient then
            include = true
        elseif item.type == types.Light or item.type == types.RepairItem or item.type == types.Lockpick or item.type == types.Probe then
            include = true
        end
...
function drawItemSelect()
    ...
    table.insert(content, utility.renderItemBold("W/S/A/D or DPad to highlight item", nil, nil, true))
    table.insert(content, utility.renderItemBold(string.format("%s or %s to add the item", confirmK, confirmB), nil, nil, true))

    for i = 1, numRows do
        table.insert(content,
            utility.renderItemBoxed(utility.flexedItems(getItemRow(inv, selectedIndex, i), true), utility.scaledVector2(666, 90),
                I.MWUI.templates.padding,
                util.vector2(0.5, 0.5)))
    end
...
function drawSpellSelect()
    ...
    num = 1 -- Reset num for list drawing
    local confirmK = keyboardSettings:get("confirmKey")
    local confirmB = gamepadSettings:get("confirmButton")
    ...
    table.insert(content, utility.renderItemBold(string.format("%s or %s to add the spell", confirmK, confirmB), nil, nil, true))
    table.insert(content,
...
local function drawQuickSelect()
    ...
    table.insert(content, utility.renderItemBold(string.format("W/S/A/D or DPad for navigation || %s or %s or Mouse1 to select", confirmK, confirmB), nil, nil, true))

    local selectedSlot = (selectedRow - 1) * 10 + selectedCol
    local count = tonumber(settings:get("hotbarCount")) or 3
    for i = 1, count do
        -- Each row draws its own 10 slots; num MUST be reset per row
        num = (i - 1) * 10 + 1
        local rowSelectedSlot = (selectedRow == i) and ((selectedRow - 1) * 10 + selectedCol) or -1
        table.insert(content,
            utility.renderItemBoxed(utility.flexedItems(getHotbarItems(rowSelectedSlot), true), utility.scaledVector2(800, 75),
                I.MWUI.templates.padding,
                util.vector2(0.5, 0.5)))
    end
...
local function UiModeChanged(data)
    if not I.UI.getMode() then
        ...
        -- Also destroy message box in case engine closed unexpectedly via native Escape
        messageBoxUtil.destroyMessageBox()
        messageBoxOpen = false
        
        -- Reset all modal states fully BEFORE doing any redraws
...
-- Explicit map so string→code lookup always works regardless of OpenMW userdata implementation
local keyNameMap = {
    A=input.KEY.A, B=input.KEY.B, C=input.KEY.C, D=input.KEY.D,
    E=input.KEY.E, F=input.KEY.F, G=input.KEY.G, H=input.KEY.H,
    I=input.KEY.I, J=input.KEY.J, K=input.KEY.K, L=input.KEY.L,
    M=input.KEY.M, N=input.KEY.N, O=input.KEY.O, P=input.KEY.P,
    Q=input.KEY.Q, R=input.KEY.R, S=input.KEY.S, T=input.KEY.T,
    U=input.KEY.U, V=input.KEY.V, W=input.KEY.W, X=input.KEY.X,
    Y=input.KEY.Y, Z=input.KEY.Z,
    Space=input.KEY.Space,
    Tab=input.KEY.Tab, Escape=input.KEY.Escape,
    F1=input.KEY.F1,   F2=input.KEY.F2,   F3=input.KEY.F3,
    F4=input.KEY.F4,   F5=input.KEY.F5,   F6=input.KEY.F6,
    F7=input.KEY.F7,   F8=input.KEY.F8,   F9=input.KEY.F9,
    F10=input.KEY.F10, F11=input.KEY.F11, F12=input.KEY.F12,
}
local function getKeyCode(name, fallback)
    return keyNameMap[name] or fallback
end
local function getConfirmKeyCode()
    return getKeyCode(keyboardSettings:get("confirmKey"), input.KEY.F)
end
...
local function onKeyPress(key)
    if messageBoxOpen then
        ...
        elseif key.code == input.KEY.S then
            ...
            redrawMessageBox()
            return true
        elseif key.code == getConfirmKeyCode() or key.code == input.KEY.ENTER then
    ...
    if spellMode and QuickSelectWindow then
        ...
        elseif key.code == input.KEY.A then
            ...
            selectedSpellIndex = math.max(1, selectedSpellIndex - 21)
            startOffset = math.max(0, startOffset - 21)
            endOffset = startOffset + 20
            drawSpellSelect()
            -- Show tooltip
            local spellData = cachedSpellsAndIds[selectedSpellIndex]
            ...
            if spellData and spellData.id then
                if spellData.enchant then
                    ...
                    tooltip = utility.drawListMenu(tooltipData.genToolTips({ spell = core.magic.spells.records[spellData.id] }), utility.itemWindowLocs.BottomCenter, nil, "HUD")
                end
            end
            return true
        elseif key.code == input.KEY.D then
            -- Scroll list down by 21
            selectedSpellIndex = math.min(maxCount, selectedSpellIndex + 21)
            startOffset = math.max(0, math.min(startOffset + 21, math.max(0, maxCount - 21)))
            endOffset = startOffset + 20
            drawSpellSelect()
            -- Show tooltip
            local spellData = cachedSpellsAndIds[selectedSpellIndex]
    ...
    if not spellMode and not itemMode then
        ...
        if key.code == input.KEY.W then
            ...
            return true
        elseif key.code == input.KEY.S then
            local maxRows = tonumber(settings:get("hotbarCount")) or 3
...
local function onControllerButtonPress(id)
    if messageBoxOpen then
        if id == input.CONTROLLER_BUTTON.DPadUp then
            if not gamepadSettings:get("enableGamepadControls") then return true end
            currentButtonSelected = math.max(1, currentButtonSelected - 1)
            redrawMessageBox()
            return true
    ...
    if spellMode and QuickSelectWindow then
        ...
        elseif id == input.CONTROLLER_BUTTON.DPadDown then
            ...
            return true
        elseif id == input.CONTROLLER_BUTTON.LeftShoulder or id == input.CONTROLLER_BUTTON.DPadLeft then
            if not gamepadSettings:get("enableGamepadControls") then return true end
            -- Scroll list up by 21
            selectedSpellIndex = math.max(1, selectedSpellIndex - 21)
            startOffset = math.max(0, startOffset - 21)
            ...
            return true
        elseif id == input.CONTROLLER_BUTTON.RightShoulder or id == input.CONTROLLER_BUTTON.DPadRight then
            if not gamepadSettings:get("enableGamepadControls") then return true end
            -- Scroll list down by 21
            selectedSpellIndex = math.min(maxCount, selectedSpellIndex + 21)
            startOffset = math.max(0, math.min(startOffset + 21, math.max(0, maxCount - 21)))
    ...
    if not spellMode and not itemMode then
        ...
        elseif id == input.CONTROLLER_BUTTON.DPadRight then
            ...
            showTooltipForSelectedSlot()
            return true
        elseif id == input.CONTROLLER_BUTTON[gamepadSettings:get("confirmButton")] then
            if not gamepadSettings:get("enableGamepadControls") then return true end
            local slot = (selectedRow - 1) * 10 + selectedCol
            slotToSave = slot
            showQuickMenuMessageBox(slot)
            return true
        elseif id == input.CONTROLLER_BUTTON[gamepadSettings:get("cancelButton")] then
            if not gamepadSettings:get("enableGamepadControls") then return true end
            I.UI.setMode()
            return true
        end
...
local function ButtonClicked(data)
    ...
    elseif text == core.getGMST("sQuickMenu3") then
        ...
        messageBoxUtil.destroyMessageBox()
        messageBoxOpen = false
        powersExpanded = settings:get("spellsExpandedDefault")
        spellsExpanded = settings:get("spellsExpandedDefault")
        enchantmentsExpanded = settings:get("spellsExpandedDefault")
        cachedSpellsAndIds = nil
    ...
    elseif text == core.getGMST("sQuickMenu4") then
        ...
        messageBoxOpen = false
        I.UI.setMode()
    elseif text == core.getGMST("sCancel") then
        if QuickSelectWindow then
            QuickSelectWindow:destroy()
            QuickSelectWindow = nil
        end
...
return {
    ...
    engineHandlers = {
        onKeyPress = function(key)
            if onKeyPress(key) then return true end
            if QuickSelectWindow or messageBoxOpen then return true end
        end,
        onControllerButtonPress = function(btn)
            -- Aggressively consume D-Pad to block other mods
            local dpad = (btn >= 15 and btn <= 18)
            if (QuickSelectWindow or messageBoxOpen) and dpad then
                onControllerButtonPress(btn)
                return true
            end
            if onControllerButtonPress(btn) then return true end
            if QuickSelectWindow or messageBoxOpen then return true end
        end,
        onMouseWheel = function (vert)
            if not mouseSettings:get("enableMouseControls") and not itemMode then return end
            if not QuickSelectWindow then return end
        ...
        onUpdate = function(dt)
            if not QuickSelectWindow or not spellMode then 
                holdKey = nil
                holdTimer = 0
                return 
            end
            ...
            if input.isKeyPressed(input.KEY.W) or input.isControllerButtonPressed(input.CONTROLLER_BUTTON.DPadUp) then
                currentKey = "up"
            elseif input.isKeyPressed(input.KEY.S) or input.isControllerButtonPressed(input.CONTROLLER_BUTTON.DPadDown) then
                currentKey = "down"
            end
            
            if currentKey then
                if holdKey == currentKey then
                    holdTimer = holdTimer + dt
                    if holdTimer > 0.4 then -- Start acceleration after 0.4s
                        local scrollDelay = math.max(0.01, 0.2 / (1 + (holdTimer - 0.4) * 2)) -- Exponential speed
                        if core.getRealTime() - lastScrollTime > scrollDelay then
                            if currentKey == "up" then
                                selectedSpellIndex = math.max(1, selectedSpellIndex - 1)
                            else
                                selectedSpellIndex = math.min(maxCount, selectedSpellIndex + 1)
                            end
                            lastScrollTime = core.getRealTime()
                            drawSpellSelect()
                        end
                    end
                else
