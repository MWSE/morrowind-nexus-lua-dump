...
local function renderItemX(item, bold, fontSize)
    ...
    local tScale = calculateTextScale()
    fontSize = fontSize * tScale
    return {
        type = ui.TYPE.Container,
        content = ui.content {
            {
...
local textTemplate = I.MWUI.templates.textNormal
    ...
    return {
        ...
        props = {
            ...
            size = (isSpell and spellData and spellData.header) and util.vector2(350, 30) or nil,
        },
        events = { 
            mouseMove = events and events.mouseMove, 
            mouseClick = events and events.mouseClick,
            mousePress = events and events.mousePress
        },
        content = ui.content {
            {
                template = I.MWUI.templates.padding,
