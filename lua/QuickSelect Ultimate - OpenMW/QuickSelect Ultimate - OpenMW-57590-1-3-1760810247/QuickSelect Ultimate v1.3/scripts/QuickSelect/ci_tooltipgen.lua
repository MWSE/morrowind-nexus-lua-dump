...
local function genMagicTooltips(list, spellRecord, item)
    ...
    -- Don't add the name for alchemy items to avoid double name
    if not (item and (item.type == types.Potion or item.type == types.Ingredient)) then
    ...
    for index, value in ipairs(spellRecord.effects) do
        ...
        if item and item.type == types.Ingredient then
            -- For ingredients, only show effect name
            line = string.format("%s", name)
        elseif item and item.type == types.Potion then
            -- For potions, show exact magnitudes and durations
            if value.magnitudeMin == value.magnitudeMax then
                line = string.format("%s %g pts for %g secs%s", name, value.magnitudeMax, value.duration, range and " on " .. range or "")
            else
                line = string.format("%s %g to %g pts for %g secs%s", name, value.magnitudeMin, value.magnitudeMax,
                    value.duration, range and " on " .. range or "")
            end
        elseif spellRecord.type == core.magic.ENCHANTMENT_TYPE.ConstantEffect then
            -- For constant enchantments, omit duration
            if value.magnitudeMin == value.magnitudeMax then
                line = string.format("%s %g pts%s", name, value.magnitudeMax, range and " on " .. range or "")
            else
                line = string.format("%s %g to %g pts%s", name, value.magnitudeMin, value.magnitudeMax, range and " on " .. range or "")
            end
...
local function getConditionLine(item)
    ...
    if item.type == types.Light then
        local itemData = types.Item.itemData(item)
       -- local timeLeft = (itemData and itemData.timeLeft) or item.type.records[item.recordId].time or 0
        --line = "Time Left: " .. tostring(math.floor(timeLeft / 60 * 10) / 10) .. " minutes"
    ...
    elseif item.type == types.Lockpick or item.type == types.Probe or item.type == types.Repair then
