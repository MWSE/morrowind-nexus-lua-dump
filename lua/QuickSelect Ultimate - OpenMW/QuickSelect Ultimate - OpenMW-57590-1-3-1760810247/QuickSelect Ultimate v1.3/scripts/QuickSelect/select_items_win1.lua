...
local function buildInv()
    ...
    for _, item in ipairs(allInv) do
        if item.type ~= types.Miscellaneous and item.type ~= types.Book then
            -- Include onCast enchanted rings and amulets, exclude unenchanted ones
            local include = true
            ...
            if item.type and types and types.Clothing and types.Clothing.objectIsInstance(item) then
                ...
                if not ench then
                    -- Exclude unenchanted rings and amulets
                    include = false
                elseif ench.type == core.magic.ENCHANTMENT_TYPE.CastOnUse then
                    -- Include onCast enchanted rings and amulets
                    include = true
                else
                    -- Exclude other enchantment types for rings and amulets
                    include = false
                end
            end
            if include then
