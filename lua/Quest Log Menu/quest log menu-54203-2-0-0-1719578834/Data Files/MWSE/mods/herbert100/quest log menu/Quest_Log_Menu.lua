...
function Quest_Log.new()
    ...
    self.search_timer = timer.start{duration=0.15, type=timer.real, callback=function()
        ...
        if search_text:len() > 1 and search_text ~= "Search..." then
...
function Quest_Log:make_right_button_bar(parent)
    ...
    local btns_info = {
        {"Keyword search", cfg.search, "keywords", reset_state},
        ...
        {"Show technical quest information", cfg, "show_technical_info", function()
...
function Quest_Log:make_left_button_bar(parent)
    ...
    local btn_infos = {
        ...
        {"Show hidden", "show_hidden", {childAlignX = 1}},
    }
...
function Quest_Log:make_ui()
    ...
    do -- fill left block
        ...
        search_bar = search_bar:createTextInput{id=uids.search_bar_text, placeholderText="Search...", autoFocus=true}
    ...
    local lbl = pane_content:createLabel{text = "No active quests :("}
...
local function make_actor_heard_from_tooltip(actor)
    ...
    local pronoun = obj.female and "She" or "He"
    ...
    if ref == tes3.player then
        ...
        local you_are_lbl = bio_entry:createLabel{text="You are"}
    ...
    if faction then
        ...
        if starts_with_vowel(rank_str) then
            faction_entry:createLabel{text=pronoun .. " is an"}
        else
            faction_entry:createLabel{text=pronoun .. " is a"}
        end
        ...
        faction_entry:createLabel{text="in the"}
    ...
    local opinion_str = "doesn't know you very well"
    if disp and disp ~= obj.baseDisposition then
        opinion_str =   disp < 10 and "hates you"
                     or disp < 30 and "doesn't like you"
                     or disp < 50 and "doesn't mind you"
                     or disp < 80 and "likes you"
                     or               "really likes you"
    end
...
local function make_location_data_tooltip(loc_data)
    ...
    local cellname_prefixes = {"Cell:", [#loc_data.path] = "Closest exterior cell:"}

    for j, cell in ipairs(loc_data.path) do
        -- local row = tooltip:createBlock()
        left_col:createLabel{text=cellname_prefixes[j] or "Within:"}
    ...
    if ext_cell and ext_cell.region then
        local region_lbl = left_col:createLabel{text="Region:"}
...
local function make_actor_location_tooltip(actor)
    ...
    if #filtered_and_flattened <= 1 then
        ...
        if highlighted_part == region_name then
            tokens = {"in the", region_name}
        end
    end
    tokens = tokens or {"in", loc_data:format_as_address(cfg.ui.region_names)}
    tokens[1] = "can be found " .. tokens[1]
...
function Quest_Log:make_quest_header(quest)
    ...
    for i, info_index in ipairs(info_indices) do
        ...
        if loc_data then
            local in_sep = info_blk:createLabel{ text = "in" }
            in_sep.borderLeft = 5
...
function Quest_Log:make_quest_progress(quest)
    ...
    if #quest.infos == 0 then
        local dummy_lbl = progress_blk:createLabel{text="No progress made."}
...
function Quest_Log:make_relevant_info(quest)
    ...
    for i, topic_id in ipairs(quest.topics) do
        ...
        local dotdotdot_lbl = topic_container:createLabel{text="(Click to expand)"}
    ...
    if next(added_topics) == nil then
        local lbl = relevant_information:createLabel{text="You don't know anything."}
        lbl.wrapText = true
...
function Quest_Log:make_technical_info(quest)
    ...
    local footer_info = {
        {"Faction:", faction and string.format('"%s"', faction.name) or "N/A"}
    }
    if #ids == 1 then
    ...
    if #indices == 1 then
        footer_info[3] = {"Journal Index:", tostring(indices[1] or "N/A")}
    else
        footer_info[3] = {"Journal Indices:", table.concat(indices, ", ")}
    end
    if #sources > 0 then
        footer_info[4] = {"Modified by:", table.concat(sources, ", ")}
    else
        footer_info[4] = {"Modified by:", "N/A"}
    end

    footer_info[1][3] = "The faction that the quest giver belongs to."
    footer_info[2][3] = "The various dialogues associated to this quest."
    footer_info[3][3] = "The journal index of each dialogue associated to this quest."
    footer_info[4][3] = "A list of all mods that modified a dialogue associated to this quest.\n\z
        \"N/A\" means this quest hasn't been touched by a mod."
...
function Quest_Log:make_hidden_btn(quest)
    ...
    local set_hidden_btn = btn_container:createButton{
        ...
        text = quest.is_hidden and "Unhide Quest" or "Hide Quest"
    }
    ...
    set_hidden_btn:register(tes3.uiEvent.mouseClick, function (e)
        ...
        e.source.text = quest.is_hidden and "Unhide Quest" or "Hide Quest"
