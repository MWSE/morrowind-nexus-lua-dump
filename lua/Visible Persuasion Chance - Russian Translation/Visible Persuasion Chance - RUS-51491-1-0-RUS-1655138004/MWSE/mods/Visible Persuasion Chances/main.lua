local i18n = mwse.loadTranslations("Visible Persuasion Chances")
...
local function persuasionChance(e)
	...
	for i, button in ipairs(buttonList.children) do
		...
		button:register("help", function()
			...
			t.autoHeight = true
			t.autoWidth = true
			local chanceText = t:createLabel{ text = i18n("Chance: ") .. targets[i] }
		end)
	end	
end
...
function modConfig.onCreate(container)
	...
	local label = settingBlock:createLabel{ text = i18n("Immersive Mode") }
	label.absolutePosAlignX = 0.0
	label.absolutePosAlignY = 0.5

	local onOffButton = settingBlock:createButton{ text = config.immersiveMode and i18n("On") or i18n("Off") }
	...
	onOffButton:register("mouseClick", function(e)
		...
		onOffButton.text = (config.immersiveMode and i18n("On") or i18n("Off"))
	end)
...
local function registerModConfig()
	mwse.registerModConfig(i18n("Visible Persuasion Chance"), modConfig)
end
