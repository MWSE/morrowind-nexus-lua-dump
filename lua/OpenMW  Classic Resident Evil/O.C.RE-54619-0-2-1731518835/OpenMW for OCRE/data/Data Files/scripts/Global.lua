...
local function Container(data)
	if types.Container.record(data.container).mwscript=="_container_linked" then
		--print("THEREE")
		if data.action=="in/out" then
...
local function AskObjectsInWorld(data)
	...
	for i, cell in ipairs(world.cells) do
		...
		for i, object in ipairs(cell:getAll()) do
			if object.type==types.Weapon or object.type==types.Potion or object.type==types.Miscellaneous or object.type==types.Lockpick or (object.type==types.Light and types.Light.record(object).isCarriable==true) then
				--print(object.recordId)
				table.insert(objects,object.recordId)
			elseif object.type==types.Container then
				if types.Container.record(object).mwscript=="_container_linked" then
...
--	print(world.mwscript.getLocalScript(data.Object,data.Player).variables[1])
	if data.Variable=="crowbarvalue" and world.mwscript.getLocalScript(data.Object,data.Player).variables[data.Variable] and world.mwscript.getLocalScript(data.Object,data.Player).variables[data.Variable]>0 and world.mwscript.getLocalScript(data.Object,data.Player).variables[data.Variable]<11 then
        ...
        onUpdate = function()
            ...
            for i, player in ipairs(players) do
				...
				for i , container in pairs(player.cell:getAll(types.Container)) do 
					...
					elseif types.Container.record(container).mwscript=="_container_linked" then
						--interfaces.Activation.addHandlerForObject(container,function(container,actor) return false end)
					end
				end
