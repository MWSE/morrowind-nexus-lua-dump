...
--    nonmouse_controller.initiateNonMouseControls(documentWindow)
	...
	if filesList.documentWindow.layout.userData.lines[#filesList.documentWindow.layout.userData.lines].userData.page ==1 then
		filesList.documentWindow.layout.content:add({name="ArrowR", type = ui.TYPE.Image, props = { visible=false, relativeSize = util.vector2(1/40, 1/25),relativePosition=util.vector2(0.9, 0.4),  anchor = (util.vector2(0, 0)), color = Colors.Green, resource = ui.texture { path = "textures/choice select cursor.dds"},}})
		filesList.documentWindow.layout.content:add({name="ArrowL", type = ui.TYPE.Image, props = { visible=false, relativeSize = util.vector2(1/40, 1/25),relativePosition=util.vector2(0.1, 0.4),  anchor = (util.vector2(0, 0)), color = Colors.Green, resource = ui.texture { path = "textures/choice select cursor left.dds"},}})
		filesList.documentWindow.layout.content:add({name="EXIT", type = ui.TYPE.Text, props = { visible=true, textSize = 30*textSizeRatio,relativePosition=util.vector2(0.9, 0.4),  anchor = (util.vector2(0, 0)), textColor = Colors.Grey, text="Exit",}})
	else
		filesList.documentWindow.layout.content:add({name="ArrowR", type = ui.TYPE.Image, props = { visible=true, relativeSize = util.vector2(1/40, 1/25),relativePosition=util.vector2(0.9, 0.4),  anchor = (util.vector2(0, 0)), color = Colors.Green, resource = ui.texture { path = "textures/choice select cursor.dds"},}})
		filesList.documentWindow.layout.content:add({name="ArrowL", type = ui.TYPE.Image, props = { visible=false, relativeSize = util.vector2(1/40, 1/25),relativePosition=util.vector2(0.1, 0.4),  anchor = (util.vector2(0, 0)), color = Colors.Green, resource = ui.texture { path = "textures/choice select cursor left.dds"},}})
		filesList.documentWindow.layout.content:add({name="EXIT", type = ui.TYPE.Text, props = { visible=false, textSize = 30*textSizeRatio,relativePosition=util.vector2(0.9, 0.4),  anchor = (util.vector2(0, 0)), textColor = Colors.Grey, text="Exit",}})
	end
		...
		for i, ammo in ipairs(InventoryAmmunitionTypes) do
			...
			if ammo == item2.recordId then
				...
				if types.Item.itemData(item1).condition - 10000 == i then
					if types.Actor.inventory(self):countOf(item2.recordId) > (core.magic.enchantments.records[types.Weapon.record[item1.recordId].enchant].charge - types.Item.getEnchantmentCharge(item1)) then
					...
					if types.Actor.inventory(self):countOf(item2.recordId) >= core.magic.enchantments.records[types.Weapon.records[item1.recordId].enchant.id].charge then
						core.sendGlobalEvent('setCharge',
							{ Item = item1, value = core.magic.enchantments.records[types.Weapon.records[item1.recordId].enchant.id].charge })
						core.sendGlobalEvent('RemoveItem',
							{ Item = item2, number = core.magic.enchantments.records[types.Weapon.records[item1.recordId].enchant.id].charge })
...
local function ShowInventory()
	...
	if types.Actor.activeEffects(self):getEffect("poison") and types.Actor.activeEffects(self):getEffect("poison").magnitude > 0 then
		HealthPath.path1 = 'textures/Lifebar/Poison.jpg'
	elseif (onFrameHealth / types.Actor.stats.dynamic.health(self).base) >= 0.8 then
		HealthPath.path1 = 'textures/Lifebar/Fine.jpg'
	elseif (onFrameHealth / types.Actor.stats.dynamic.health(self).base) <= 0.3 then
		HealthPath.path1 = 'textures/Lifebar/Danger.jpg'
	else
		HealthPath.path1 = 'textures/Lifebar/Caution.jpg'
	end

	Inventory.LifeBar.UI = { name = "LifeBar", type = ui.TYPE.Image, props = { relativeSize = util.vector2(3/10, 1 / 6), relativePosition = util.vector2(1/5, 1 / 6), anchor = util.vector2(0, 0), resource = ui.texture { path = HealthPath.path1, 
	...
	if MapsUtils["RoomsVisited"][Area] then
		if MapsUtils["RoomsVisited"][Area][2][Zone] then
			...
			for i=1,10 do
				...
				if i==10 and MapsUtils.ObjectsInWorld[Maps[Area][2][Zone][2][Cell]][11] then
				...
				elseif MapsUtils.ObjectsInWorld[Maps[Area][2][Zone][2][Cell]][i] then
					table.insert(FlexObjectsContent,{ type = ui.TYPE.Text,  props = { text = MapsUtils.ObjectsInWorld[Maps[Area][2][Zone][2][Cell]][i],relativePosition=util.vector2(2/16, 7/16), textSize = 25*textSizeRatio, textColor = Colors.White } }) 
				...
				for i, door in ipairs(nearby.doors) do
					...
					if (dist < 80 and ((door.position.z - self.position.z) <= 150) and InFront(door) == true) or door==nearby.castRay(util.vector3(math.sin(self.rotation:getYaw())*10, math.cos(self.rotation:getYaw())*10, 80) + self.position,util.vector3(math.sin(self.rotation:getYaw())*80, math.cos(self.rotation:getYaw())*80, 80) + self.position).hitObject then
						...
						--	print(DoorTransition.TeleportDoor)
							...
							--end
							if types.Door.isTeleport(door)==true and types.Lockable.isLocked(door)==false and types.Door.records[types.Door.records[door.recordId].name] then
								print("ok2")

								core.sendGlobalEvent('Teleport',
								{ object = self, DestCell="DoorStransition", position = util.vector3(0,-400,0), rotation = nil})
							end	
						elseif (storage.playerSection('RESettings1'):get('DoorTransition')==false or types.Door.records[types.Door.records[door.recordId].name]==nil) and types.Lockable.isLocked(door)==false then
							FadeEffect(dt)
...
--	end
	...
	if I.UI.getMode() then
		...
		--Puzzles.RunningPuzzles(DeltaT,MoveForward,MoveBackward,TurnLeft,TurnRight,Colors,ElectricalPanelPuzzleUI,MenuSelection,Lockpicking,Cross,CrowbarPuzzleUI,ButtonToggle,Toggle)
		if ElectricalPanelPuzzleUI.UI and ElectricalPanelPuzzleUI.UI.layout then
			Puzzles.ElectricalPanel(DeltaT,MoveForward,MoveBackward,TurnLeft,TurnRight,Colors,ElectricalPanelPuzzleUI,Cross,ButtonToggle,Toggle)
		...
		elseif CrowbarPuzzleUI.UI then
			Puzzles.CrowbarPuzzle(DeltaT,MoveForward,MoveBackward,TurnLeft,TurnRight,Colors,Cross,CrowbarPuzzleUI,ButtonToggle,Toggle)
		end
...
--					end
				...
				if Inventory.UI.layout then
					if (core.getRealTime() - Inventory.LifeBar.Timer) > 0.04 then
						...
						Inventory.UI.layout.content.LifeBar.props.resource= ui.texture { path = HealthPath.path1, 
																							offset = util.vector2(HealthPath.path2*320, 0),
																							size = util.vector2(320, 158)}
						Inventory.UI:update()
					end
				end
...
--								ui.showMessage(tostring(InventoryItems[InventoryItemSelected[2]].recordId))
	...
	--						end
							...
							for i, item in ipairs(CombinedItems) do							
								if itemscombined == false and item1.recordId == (string.lower(item[1])) and item2 and item2.recordId == (string.lower(item[3])) then
								...
								elseif itemscombined == false and (item1.recordId == (string.lower(item[3])) and item2 and item2.recordId == (string.lower(item[1]))) then
						...
						elseif SubInventoryTexts.content[InventoryItemSelected[3]].props.text=="Check" then ---------- CHECK
							...
							if ItemChecked["Icon"] then
								...
								for i, item in ipairs(ExaminedItems) do
									if InventoryItems[InventoryItemSelected[2]].recordId == string.lower(ExaminedItems[i][1]) then
										core.sendGlobalEvent('RemoveItem',
							...
							if ItemChecked["Icon"] == nil then
								if ItemDescriptions[InventoryItems[InventoryItemSelected[2]].recordId] then
									ShowItem(InventoryItems[InventoryItemSelected[2]],ItemDescriptions[InventoryItems[InventoryItemSelected[2]].recordId])
								else
