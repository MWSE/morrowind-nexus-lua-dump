...
func_table.Saving=function(MoveForward,MoveBackward,TurnLeft,TurnRight,Colors,Saves,SavingMenuUI, MapUI, RoomsVisited, Maps,Cross)
	...
	if SavingMenuUI and SavingMenuUI.layout then
		...
		if Cross == true and ToggleUseButton == true and Saving==false then
			...
			SavingDescription=SavingSelection.." . "..types.NPC.records[self.recordId].race:gsub("^%l", string.upper).."  /"..(Saves[11]).."/ "..self.cell.name
...
--			SavingDescription="       I can't stay anymore....."-------------------------------------------------------------------
			...
			types.Player.sendMenuEvent(self, 'Save', {value=types.NPC.records[self.recordId].race:gsub("^%l", string.upper).."  /"..(Saves[11]).."/ "..self.cell.name,savenum=(Saves[11])})
