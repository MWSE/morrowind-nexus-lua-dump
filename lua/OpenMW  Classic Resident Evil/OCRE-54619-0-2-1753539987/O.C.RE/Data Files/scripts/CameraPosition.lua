...
func_table.PositionnningCamera=function(util,input,camera,core,BGDepth,activecam,activeBkg,MSKlist,TurnLeft,TurnRight,MoveForward,MoveBackward,SwitchZonePoints,ROOMS)
	...
	if I.UI.getMode()=="MainMenu" and (CameraMenu==nil or CameraMenu.layout==nil) then
		...
		CameraMenu=ui.create{layer = 'Console',  type = ui.TYPE.Flex,props={relativePosition=util.vector2(7/10, 1/30), autoSize=true, anchor=util.vector2(0,0)},
							content=ui.content{
									...
									{ type = ui.TYPE.Text,  props = { text = "P : Show Datas (Ctrl+A, Ctrl+C)", textSize = 25*textSizeRatio, textColor = White, visible = false} },
									...
									{type = ui.TYPE.Text,  props = {  text = "Player Position X : "..math.floor(self.position.x+0.5).." Y "..math.floor(self.position.y+0.5), textSize = 25*textSizeRatio, textColor = White, visible = false} ,},
	...
	if CameraMenu and I.UI.getMode()=="MainMenu" then
		...
		elseif ShowSwitchZone==1 and CameraMenu.layout.content[21].props.visible==false and SwitchZonePoints["SwitchZone1"] then
			...
			CameraMenu.layout.content[27].props.visible=true
			CameraMenu.layout.content[27].props.text = "Player Position X : "..math.floor(self.position.x+0.5).." Y "..math.floor(self.position.y+0.5)
			CameraMenu:update()
		elseif ShowSwitchZone==0 and CameraMenu.layout.content[21].props.visible==true then
			...
			CameraMenu.layout.content[27].props.visible=false
			CameraMenu:update()
		end
	end
	...
	if I.UI.getMode()=="MainMenu" and input.isKeyPressed(input.KEY._0) then
		...
		if input.isKeyPressed(input.KEY.B) then
			...
			elseif input.isKeyPressed(input.KEY.B) and  MoveForward(-0.2)=="Up" then
				BGDepth=BGDepth+5
				ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
			end
...
--[[
		...
		elseif input.isKeyPressed(input.KEY.P) and ToggleButton==false then
			ToggleButton=true
			if CamDatasPrinted.layout then
				CamDatasPrinted:destroy()
				CameraMenu.layout.content[2].props.text="P : Show Datas (Ctrl+A, Ctrl+C)"
				CameraMenu:update()
			elseif CameraMenu.layout.content[2].props.text=="P : Show Datas (Ctrl+A, Ctrl+C)" then

				local text =(tostring(self.cell.name).."="..table_to_string(ROOMS[self.cell.name]))
				CamDatasPrinted=ui.create{layer = 'Console',template = TextEditBox2,  type = ui.TYPE.TextEdit,props={text=text, textSize=20*textSizeRatio, relativeSize=util.vector2(3, 10),  relativePosition=util.vector2(1/2, 3/4), autoSize=false, anchor=util.vector2(0,0)},}
				CameraMenu.layout.content[2].props.text="P : Hide Datas"
				CameraMenu:update()
			end
		...
		elseif input.isKeyPressed(input.KEY.H) and ToggleButton==false then
			...
			if ShowSwitchZone==0 then
				CameraMenu.layout.content[20].props.text="H : Hide Switchzones"
				core.sendGlobalEvent('SHSwitchZones',{	Player=self,
														SH=ShowSwitchZone, 
														cam=activecam,
														AxisPosition=nearby.castRay(camera.getPosition(),camera.viewportToWorldVector(util.vector2(0.5,0.5))*10000).hitPos,
														AxisScale=(camera.getPosition()-nearby.castRay(camera.getPosition(),camera.viewportToWorldVector(util.vector2(0.5,0.5))*10000).hitPos):length()/3000})
				ShowSwitchZone=1
		...
		elseif input.isKeyPressed(input.KEY.M) then
			...
			if MoveBackward(0.2)=="Down" and MSKlist[Mask].depth>1 then
			...
			elseif MoveForward(-0.2)=="Up" then
				...
				print(MSKlist[Mask].depth)
				ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
			elseif TurnRight(0.2)=="Right" and vfs.fileExists('meshes/Masks/'..MSKlist[Mask+1].idname..'.nif') and ToggleButton==false then
				Mask=Mask+1
				ui.showMessage("Mask"..Mask)
				ToggleButton=true
			elseif TurnLeft(-0.2)=="Left" and Mask>1 and ToggleButton==false  then
				Mask=Mask-1
				ui.showMessage("Mask"..Mask)
				ToggleButton=true
			end
		elseif input.isKeyPressed(input.KEY._9) and camera.getMode()==0 then
			...
			CameraMenu:update()
			if TurnLeft(-0.2) == "Left" then
				camera.setStaticPosition(util.transform.move(util.vector3(-2, 0, 0)) * camera.getPosition())
				ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
			...
			elseif input.isKeyPressed(input.KEY._8) == true then
				...
				if MoveBackward(0.2) == "Down" then
				CameraMenu:update()
					camera.setStaticPosition(util.transform.move(util.vector3(0, 0, -2)) * camera.getPosition())
					ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
				elseif MoveForward(-0.2) == "Up" then
					camera.setStaticPosition(util.transform.move(util.vector3(0, 0, 2)) * camera.getPosition())
					ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
			...
			elseif MoveBackward(0.2) == "Down" then
				camera.setStaticPosition(util.transform.move(util.vector3(0, -2, 0)) * camera.getPosition())
				ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
			elseif MoveForward(-0.2) == "Up" then
				camera.setStaticPosition(util.transform.move(util.vector3(0, 2, 0)) * camera.getPosition())
				ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
			end
		elseif TurnLeft(-0.2) == "Left" and camera.getMode()==0 then
			camera.setYaw(camera.getYaw() - 0.007)
			ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
		elseif TurnRight(0.2) == "Right" and camera.getMode()==0 then
			camera.setYaw(camera.getYaw() + 0.007)
			ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
		elseif MoveBackward(0.2) == "Down" and camera.getMode()==0 then
			camera.setPitch(camera.getPitch() + 0.007)
			ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
		elseif MoveForward(-0.2) == 'Up' and camera.getMode()==0 then
			camera.setPitch(camera.getPitch() - 0.007)
			ChangeCam(activeBkg,MSKlist,activecam,BGDepth)
		end
		if ToggleButton==true and TurnRight(0.2)~="Right" and TurnLeft(-0.2)~="Left" and input.isKeyPressed(input.KEY.P)==false and input.isKeyPressed(input.KEY.H)==false and input.isKeyPressed(input.KEY.T)==false then
			ToggleButton=false
