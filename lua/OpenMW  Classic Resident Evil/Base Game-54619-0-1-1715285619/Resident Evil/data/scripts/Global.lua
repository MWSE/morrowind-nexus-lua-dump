...
local function PlaceBgdMsk(activecell,activecam,player)
	for i, switchzone in pairs(SwitchZonePoints) do
		for p, point in pairs(switchzone) do
			if point.type==types.Activator and point:isValid() then
				point.enabled=false
				point:remove()
			end
		end
    ...
    engineHandlers = {
        ...
        onUpdate = function()
            ...
            for i, player in ipairs(players) do
				...
				if activecam~=lastcam or lastcam==nil or activecell~=lastcell or ((BGD==nil or BGD.enabled==false) and ROOMS[player.cell.name][activecam].bgd) then
