...
function Camerapos(data)
	...
	if storage.playerSection('RESettings1'):get('FixedCamera')==true then
		camera.setMode(0)
...
local function ShowMap(Area,Zone,Cell)
	...
	for i, Room in pairs(Maps[Area][2][Zone][2]) do
		if MapsUtils["RoomsVisited"][Area][2] then
	...
	if CheckTableUD(MapsUtils["RoomsVisited"],AreaMap,"+")~=0 then
...
local function onUpdate()
	...
	if (input.isActionPressed(input.ACTION.ToggleWeapon) == true or input.isActionPressed(input.ACTION.Jump) == true) and types.Actor.getEquipment(self, 16) and (AmmoChecked == 1 or types.Actor.getEquipment(self, 16)) then ----degainer l'arme
		...
		if input.isActionPressed(input.ACTION.Use) == true and ((types.Item.getEnchantmentCharge(types.Actor.getEquipment(self, 16))~=nil and types.Item.getEnchantmentCharge(types.Actor.getEquipment(self, 16))>0) or types.Weapon.record(types.Actor.getEquipment(self, 16)).type ~= 10) and (core.getGameTime() - shootTimer) > (actionbasetime / types.Weapon.record(types.Actor.getEquipment(self, 16)).speed) then -- Fire!!
			...
			---------------------------shotshell -----------en cours
			if types.Weapon.record(types.Actor.getEquipment(self, 18)) and types.Weapon.record(types.Actor.getEquipment(self, 18)).enchant and core.magic.enchantments.records[types.Weapon.record(types.Actor.getEquipment(self, 18)).enchant] and string.find(core.magic.enchantments.records[types.Weapon.record(types.Actor.getEquipment(self, 18)).enchant].id, "shotshell") then
		...
		elseif input.isActionPressed(input.ACTION.Use) == true and types.Item.getEnchantmentCharge(types.Actor.getEquipment(self, 16)) == 0 and (core.getGameTime()- shootTimer) > (actionbasetime / types.Weapon.record(types.Actor.getEquipment(self, 16)).speed) then
	...
	if negativeshader:isEnabled() and (LiveSelectionUI == nil or LiveSelectionUI.layout==nil) and LiveSelectionTimer==nil then
		LiveSelectionTimer=core.getGameTime()
	elseif negativeshader:isEnabled() and (core.getGameTime()-LiveSelectionTimer)>3 and (LiveSelectionUI == nil or LiveSelectionUI.layout==nil) then
		LiveSelectionTimer=nil
...
local function onFrame(dt)
	...
	if MapsUtils["RoomsVisited"][1]==nil then
	...
	if LastRoom~=self.cell.name then
		LastRoom=self.cell.name
		for i, Area in ipairs(Maps) do
			--print(Area[1])
			for j, Zone in ipairs(Area[2]) do
...
--		end
	...
	if MapUI then
		if MapUI.layout then
			...
			if MoveForward(-0.2) and MenuSelectStop==false then 
				MenuSelectStop=true
					...
					if Maps[AreaMap][2][ZoneMap+i] then
						...
						elseif MapsUtils["RoomsMapped"][AreaMap] then
							if MapsUtils["RoomsMapped"][AreaMap][2][ZoneMap+i] then
								--print("Mapped "..MapsUtils["RoomsMapped"][AreaMap][2][ZoneMap+i][1])
								ZoneMap=ZoneMap+i
								RoomMap=1
								core.sendGlobalEvent('AskObjectsInWorld',{player=self,maps=Maps})
								ambient.playSound("Book Page")
			...
			elseif MoveBackward(0.2) and MenuSelectStop==false then
				MenuSelectStop=true
				for i, zone in pairs(Maps[AreaMap][2]) do
					--print("zone "..ZoneMap.." i "..i)
					if (ZoneMap-i)>0 then
						--print(MapsUtils["RoomsVisited"][AreaMap][2][ZoneMap-i])
						if  MapsUtils["RoomsVisited"][AreaMap][2][ZoneMap-i] then
							--print("Visited")
							ZoneMap=ZoneMap-i
							for j, room in pairs(Maps[AreaMap][2][ZoneMap][2]) do
			...
			if MoveBackward(0.2) and MenuSelectStop==false then
				MenuSelectStop=true
				ZoneMap=CheckTableUD(MapsUtils["RoomsVisited"][AreaMap][2],ZoneMap,"-")+ZoneMap
				RoomMap=CheckTableUD(MapsUtils["RoomsVisited"][AreaMap][2][ZoneMap][2],0,"+")
				for i, room in pairs(MapsUtils["RoomsVisited"][AreaMap][2][ZoneMap][2]) do
					if room==self.cell.name then
						RoomMap=i
					end
				end
	...
	--if input.isActionPressed(input.ACTION.Sneak) and (LiveSelectionUI == nil or LiveSelectionUI.layout==nil) then
	--		self:sendEvent("LiveSelection", { Choice1 = "Shot First", Choice2 = "Respond" })
	--end




	if LiveSelectionUI and LiveSelectionUI.layout then
		if LiveSelectionTimer==nil then
		...
		elseif (core.getRealTime()-LiveSelectionTimer)>0 and I.UI.getMode() == nil then
				I.UI.setMode(I.UI.MODE.Interface, { windows = {} })
		end

		if (MoveBackward(0.2) or MoveForward(-0.2)) and MenuSelectStop==false and not(negativeshader:isEnabled()) then
			MenuSelectStop=true
			ambient.playSound("Cursor")
		...
		if input.isActionPressed(input.ACTION.Use) and I.UI.getMode() == nil then
			if LiveSelectionUI.layout.content[1].template then
