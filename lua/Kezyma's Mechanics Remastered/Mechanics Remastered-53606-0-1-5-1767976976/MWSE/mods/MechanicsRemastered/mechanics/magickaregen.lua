...
local function regenMagicka()
    if (config.MagickaRegenEnabled == true) then
        ...
        -- NPC and creature magicka regeneration
        if (config.MagickaRegenNPC == true) then
            for _, cell in pairs(tes3.getActiveCells()) do
                for ref in cell:iterateReferences{tes3.objectType.npc, tes3.objectType.creature} do
                    if (ref.mobile) then
                        if (ref.mobile.magicka.current < ref.mobile.magicka.base and ref.mobile.health.current > 0) then
                            ...
                            if (npcatronach == false) then
                                ...
                                tes3.setStatistic{ reference = ref, name = "magicka", current = newNpcMagicka }
                            end
                        end
                    end
                end
...
local function calcRestInterruptCallback(e)
    if (config.MagickaRegenEnabled == true and config.MagickaRegenWhileWaiting == true and e.waiting == true) then
        local atronach = tes3.isAffectedBy({ reference = tes3.mobilePlayer, effect = tes3.effect.stuntedMagicka })
