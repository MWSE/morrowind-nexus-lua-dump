...
local function regenHealth()
    if (config.HealthRegenEnabled == true) then
        ...
        local canRegenPlayer = (config.HealthRegenOutOfCombatOnly == false) or (tes3.mobilePlayer.inCombat == false)
        if (canRegenPlayer) then
        ...
        -- NPC and creature health regeneration
        if (config.HealthRegenNPC == true) then
            for _, cell in pairs(tes3.getActiveCells()) do
                for ref in cell:iterateReferences{tes3.objectType.npc, tes3.objectType.creature} do
                    if (ref.mobile) then
                        local canRegenNPC = (config.HealthRegenOutOfCombatOnly == false) or (ref.mobile.inCombat == false)
                        if (canRegenNPC and ref.mobile.health.current > 0) then
                            ...
                            if (ref.mobile.health.current < npcMaxHealth) then
                                ...
                                tes3.setStatistic{ reference = ref, name = "health", current = newNpcHealth }
                            end
                        end
                    end
                end
...
local function calcRestInterruptCallback(e)
    if (config.HealthRegenEnabled == true and config.HealthRegenWhileWaiting == true and e.waiting == true) then
