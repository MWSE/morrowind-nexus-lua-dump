...
local function calcHitChanceCallback(e)
    if (config.CombatEnabled == true) then
        -- Record the hit chance for this attack (needed for other features even if AlwaysHit is off).
        ...
        if (config.CombatAlwaysHit == true and e.hitChance > 0) then
            e.hitChance = 100
        end
    end
end
...
local function damageCallback(e)
    if (config.CombatEnabled == true and config.CombatDamageScaling == true and e.source == tes3.damageSource.attack) then
...
local function damageHandToHandCallback(e)
    if (config.CombatEnabled == true and config.CombatDamageScaling == true) then
        -- Modify the damage based on the hit chance.
        local hitChance = combatHitChances[e.attackerReference.id]
        if (hitChance) then
            if (hitChance > 0) then
...
local function damagedCallback(e)
    if (config.CombatEnabled == true and config.CombatStunScaling == true and e.source == tes3.damageSource.attack) then
        local hitChance = combatHitChances[e.attackerReference.id]
        if (hitChance) then
...
local function damagedHandToHandCallback(e)
    if (config.CombatEnabled == true and config.CombatStunScaling == true) then
        local hitChance = combatHitChances[e.attackerReference.id]
        if (hitChance) then
            local rollStun = math.random(100) <= hitChance
            if (rollStun == false) then
...
local function exerciseSkillCallback(e)
    if (config.CombatEnabled == true) then
        -- Attack skill XP scaling
        if (config.CombatAttackXPScaling == true and combatLastPlayerChance ~= nil) then
        ...
        -- Defense skill XP scaling
        if (config.CombatDefenseXPScaling == true and combatLastNPCChance ~= nil) then
...
local function enchantChargeUseCallback(e)
    if (config.CombatEnabled == true and config.CombatEnchantScaling == true and e.isCast) then
