...
--Create Mark Menu
	...
	MarkMenu.autoHeight = false
	MarkMenu.autoWidth = false
	MarkMenu.width = 570
	MarkMenu.height = 560

	local labelsBlock = MarkMenu:createBlock{}
	labelsBlock.autoHeight = true
	labelsBlock.widthProportional = 1
	labelsBlock.borderBottom = 5

	local MarkLabelBlock = labelsBlock:createBlock{}
	MarkLabelBlock.autoHeight = true
	MarkLabelBlock.widthProportional = 7 / 6
	local MarkLabel = MarkLabelBlock:createLabel({ text = "У вас осталось " .. MarksLeft .. "/" .. MarksTotal .. " пометок. Какую пометку использовать для запоминания вашего текущего местоположения?" })
	MarkLabel.wrapText = true
	MarkLabel.widthProportional = 1

	local companionLabelBlock = labelsBlock:createBlock{}
	companionLabelBlock.autoHeight = true
	companionLabelBlock.widthProportional = 5 / 6
	local companionLabel = companionLabelBlock:createLabel({ text = "Добавить компаньонов, чтобы затем призвать их к себе." })
	companionLabel.wrapText = true
	companionLabel.widthProportional = 1


	local markMenuBlock = MarkMenu:createBlock{}
	markMenuBlock.heightProportional = 1
	markMenuBlock.widthProportional = 1
	markMenuBlock.childAlignX = 0.5

	local markBlock = markMenuBlock:createBlock{}
	markBlock.flowDirection = "top_to_bottom"
	markBlock.heightProportional = 1
	markBlock.widthProportional = 7 / 6
	markBlock.childAlignX = 0.5

	local markBorder = markBlock:createThinBorder{}
	markBorder.flowDirection = "top_to_bottom"
	markBorder.heightProportional = 1
	markBorder.widthProportional = 1
	markBorder.childAlignX = 0.5
	markBorder.childAlignY = 0.5

	local markList = markBorder:createVerticalScrollPane{}
	markList.heightProportional = 1
	markList.widthProportional = 1
...
--Companion List
	...
	companionBlock.heightProportional = 1
	companionBlock.widthProportional = 5 / 6
	...
	companionBorder.heightProportional = 1
	companionBorder.widthProportional = 1
	...
	companionList.heightProportional = 1
	companionList.widthProportional = 1
							...
							function()
								if table.find(markedCompanions, companionid) then
									...
									if (companion.reference.object.objectType == tes3.objectType.npc) and (disposition < config.dispositionRequired) then
										tes3.messageBox("Компаньон не настолько привязан к вам, чтобы позволить вам телепортировать его.")
									else
										table.insert(markedCompanions, companionid)
										tes3.messageBox("Добавлен(а) "..companionName)
...
local function RecallToOk(Markid)
	local recallToMenu = tes3ui.findMenu(RecallToMenuid)
	local recallMenu = tes3ui.findMenu(RecallMenuid)

	tes3ui.leaveMenuMode()
	if (recallToMenu) then recallToMenu:destroy() end
...
local function RecallToMark(Markid)
	...
	if (tes3.mobilePlayer.magicka.current >= cost) then
		...
		if (config.skipRecallConfirmation and not inputController:isShiftDown()) then
			RecallToOk(Markid)
			return
		end
		...
		local RecallToLabel = RecallToMenu:createLabel{ text = "Возврат в "..tes3.player.data.multiMark.MarkSlots[Markid].Name..". Стоимость: "..math.round(cost, 0).." Шанс: "..math.round(chance, 0) }
...
--Create Recall Menu
	...
	RecallMenu.autoHeight = false
	RecallMenu.autoWidth = false
	RecallMenu.width = 570
	RecallMenu.height = 560

	local labelsBlock = RecallMenu:createBlock{}
	labelsBlock.autoHeight = true
	labelsBlock.widthProportional = 1
	labelsBlock.borderBottom = 5

	local MarksUsed = #tes3.player.data.multiMark.MarkSlots
	local RecallLabelBlock = labelsBlock:createBlock{}
	RecallLabelBlock.autoHeight = true
	RecallLabelBlock.widthProportional = 7 / 6
	local RecallLabel = RecallLabelBlock:createLabel({ text = 'У вас '..MarksUsed..' пометок. Какаю из них использовать для возврата?' })
	RecallLabel.wrapText = true
	RecallLabel.widthProportional = 1

	local companionLabelBlock = labelsBlock:createBlock{}
	companionLabelBlock.autoHeight = true
	companionLabelBlock.widthProportional = 5 / 6
	local companionLabel = companionLabelBlock:createLabel({ text = "Возврат компаньонов к вашему местоположению." })
	companionLabel.wrapText = true
	companionLabel.widthProportional = 1


	local recallMenuBlock = RecallMenu:createBlock{}
	recallMenuBlock.heightProportional = 1
	recallMenuBlock.widthProportional = 1
	recallMenuBlock.childAlignX = 0.5

	local recallBlock = recallMenuBlock:createBlock{}
	recallBlock.flowDirection = "top_to_bottom"
	recallBlock.heightProportional = 1
	recallBlock.widthProportional = 7 / 6
	recallBlock.childAlignX = 0.5

	local recallBorder = recallBlock:createThinBorder{}
	recallBorder.flowDirection = "top_to_bottom"
	recallBorder.heightProportional = 1
	recallBorder.widthProportional = 1
		...
		recallSelect:register(
			...
			function()
				...
				if (destination) then
					...
					header = toolTip:createLabel({ text = "Неизвестная локация" })
				end
				header.color = tes3ui.getPalette(tes3.palette.headerColor)

				if (distance > 0) then
					toolTip:createLabel({ text = string.format("Расстояние: %d", distance) })
				end

				toolTip:createLabel({ text = string.format("Стоимость: %d", cost[i]) })
				toolTip:createLabel({ text = string.format("Шанс: %d", chanceM) })
			end
		)
		recallSelect:register(
			"mouseClick",
		...
		if companionRef then
			...
			companionButton:register(
				...
				function()
					...
					toolTip:createLabel{ text = "Цена: "..companionCost[i].." Шанс: "..chanceM }
				end)
			companionButton:register(
				"mouseClick",
				function()
					if (companionObject.objectType == tes3.objectType.npc and disposition < config.dispositionRequired) then
						...
						if (tes3.mobilePlayer.magicka.current >= companionCost[i]) then
							...
							local RecallCompanionLabel = RecallCompanionMenu:createLabel{ text = "Возврат "..companionName.." в вае местоположение. Цена: "..companionCost[i].." Шанс: "..companionChance[i] }
...
local function registerModConfig()
	...
	markCategory:createOnOffButton{
		label = "Пропустить подтверждение при возврате",
		description = "Если эта опция включена, запрос подтверждения при возврате в выбраное местоположение будет пропущен. Удерживая shift при нажатии на местоположение, этот параметр будет игнорироваться, что позволит удалить отмеченные местоположения. По умолчанию: выкл",
		variable = EasyMCM.createTableVariable{
			id = "skipRecallConfirmation",
			table = config,
		},
	}
