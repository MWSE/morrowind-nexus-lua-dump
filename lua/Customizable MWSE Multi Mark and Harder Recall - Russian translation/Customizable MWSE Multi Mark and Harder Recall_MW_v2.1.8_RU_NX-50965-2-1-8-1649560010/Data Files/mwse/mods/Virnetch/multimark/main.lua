	...
	if lastCastCost > 0 and config.enableMisCast == true then	--Miscast, only for spells
		if functions == nil then
			...
			if (chance <= 15 or functions.isDebug() == true) then
				...
				functions.gatedMessageBox("Ваша неудачная попытка произнести заклинание привела к телепортаци в случайное место.")
			end
		end
	end
end
...
local function RecallToMark(Markid)
	...
	if (tes3.mobilePlayer.magicka.current >= cost) then
		...
		local RecallToLabel = RecallToMenu:createLabel{ text = "Возврат в "..tes3.player.data.multiMark.MarkSlots[Markid].Name..". Цена: "..math.round(cost, 0).." Шанс: "..math.round(chance, 0) }
	...
	for i = 1, MarksUsed do
		...
		recallSelect:register(
			...
			function()
				...
				toolTip:createLabel{ text = "Цена: "..cost[i].." Шанс: "..chanceM }
			end
		)
		recallSelect:register(
			"mouseClick",
			...
			companionButton:register(
				...
				function()
					if (companionObject.objectType == tes3.objectType.npc and disposition < config.dispositionRequired) then
						tes3.messageBox("омпаньон не астолько привязан к вам, чтобы позволить вам телепортировать его.")
					else
						if (tes3.mobilePlayer.magicka.current >= companionCost[i]) then
							...
							local RecallCompanionLabel = RecallCompanionMenu:createLabel{ text = "Призыв "..companionName.." к вам. Цена: "..companionCost[i].." Шанс: "..companionChance[i] }
							...
							local buttonRemove = companionButtonBlock:createButton{ id = "V1R_MM_RecallCompanionRemove", text = "Удалить" }
