...
local function onTooltipDrawn(e)
    ...
    for ingred in getIngredients(ref.object.inventory) do
        ...
        local label = parent:createLabel{id=GUI_ID.weight, text=string.format("Peso: %.2f", ingred.weight)}
        label.wrapText = true

        local label = parent:createLabel{id=GUI_ID.value, text=string.format("Valore: %d", ingred.value)}
        label.wrapText = true
