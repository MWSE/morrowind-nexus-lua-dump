...
local function addSpells(spellList)
    for _, spell in ipairs(spellList) do
        local spellObject = tes3.getObject(spell)
        if spellObject then
...
local function startClass()
    ...
    if config.mcm.enableSpells then
        addSpells(starters.spellList)
    end
    if config.mcm.enableGear then
        addGear()
    end
end
...
--- Register Skill Modifiers
if SkillsModule then
    for classId, getConfig in pairs(presetClasses.pickStarters) do
        local config = getConfig()
        if config.customSkillList then
            for _, modification in ipairs(config.customSkillList) do
                SkillsModule.registerClassModifier{
                    class = classId,
                    skill = modification.skillId,
                    amount = modification.value,
                }
            end
        end
    end
