...
local function checkGroupAggro(player)
    -- tamed creatures never join group aggro
    if tamerExempt then return end
...
local function goHostile(player)
    -- a tamed creature must never be forced hostile by this mod
    if tamerExempt then
        stopTracking()
        return
    end
...
local function onSave()
    return {
        didSetCliffSpeed = didSetCliffSpeed,
        tamerExempt      = tamerExempt,
    }
end
...
local function onLoad(saved)
    if saved then
        ...
        tamerExempt      = saved.tamerExempt or false
    end
    lastGrowlTime = 0
    onInit()
end
...
-- TAMER MOD INTEGRATION
-- sent by the Tamer mod when this creature becomes tamed
local function onTamerTamed()
    tamerExempt = true
    -- abandon any in-progress warning/stalking behaviour
    stopTracking()
    if aiWasRemoved then
        aiWasRemoved = false
        savedPackage = nil
...
-- sent by the Tamer mod when the creature is no longer tamed
local function onTamerUntamed()
    tamerExempt = false
end
...
return {
    ...
    eventHandlers = {
        Tamer_Tamed   = onTamerTamed,
        Tamer_Untamed = onTamerUntamed,
    },
}