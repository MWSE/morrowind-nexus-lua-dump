...
function Rider.showMountInstructions()
    local instructions = {
        {
            message = "Command your guar to move towards where you are looking. Hold down to move through difficult terrain.",
            code = tes3.getInputBinding(tes3.keybind.forward).code,
        },
        {
            message = "Command your guar to stop moving.",
            code = tes3.getInputBinding(tes3.keybind.back).code
        },
        {
            message = "Command your guar to crouch down.",
            code = tes3.getInputBinding(tes3.keybind.sneak).code,
        },
        {
            message = "Open menu to dismount or issue other commands.",
            code = common.config.mcm.commandToggleKey.keyCode
        },
        {
            message = "Toggle between walking and running.",
...
function Rider:mount()
    tes3.messageBox(self.guar:format("Riding {name}"))
    if Rider.getRefBeingRidden() then
        logger:debug(self.guar:format("{Name} is already riding"))
        return
    end
    Rider.lockPlayer()
    self.guar.ai:follow()
    ...
    timer.start{
        ...
        callback = function()
            ...
            if self.guar:isOverEncumbered() then
                tes3.messageBox(self.guar:format("{Name} is over encumbered and can not move."))
            end
