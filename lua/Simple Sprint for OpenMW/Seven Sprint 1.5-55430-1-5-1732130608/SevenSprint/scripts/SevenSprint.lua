...
I.Settings.registerGroup({
   ...
   settings = {
	...
	{
         key = 'holdShiftSprint',
         default = holdShiftSprint,
         renderer = 'checkbox',
         name = 'holdShiftSprint_name',
      },
	{
         key = 'holdUseSprint',
...
local function updateSettings()
  ...
  holdShiftSprint = settingsGroup:get('holdShiftSprint')
...
--	if  types.Actor.isOnGround(self) and is_run and currentFatigue >= 1 and mf > 0 and not isSneakPressed and self.controls.use==0 then
	...
	if holdShiftSprint and input.isShiftPressed() then
		sprintKey = true
	end
