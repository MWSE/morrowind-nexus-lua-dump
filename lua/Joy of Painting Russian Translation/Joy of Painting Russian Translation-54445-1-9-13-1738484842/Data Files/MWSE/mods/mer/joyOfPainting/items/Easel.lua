...
function Easel:openAttachCanvasMenu()
    timer.delayOneFrame(function()
        CraftingFramework.InventorySelectMenu.open{
            title = "Выберите холст",
            noResultsText = "Холсты не найдены",
            callback = function(e)
                timer.delayOneFrame(function()
                    if e.item then
            ...
            noResultsCallback = function()
                tes3.messageBox("У вас нет холста для рисования")
            end,
    ...
    local buttons =  {
        ...
        },
        {
            ...
            tooltipDisabled = function(e)
                ...
                local text = ""
                if not easel then
                    text = "Это не мольберт."
                elseif  not easel:hasCanvas() then
                    text = "Сначала прикрепите холст к мольберту"
                elseif easel:hasPainting() then
                    text = "На мольберте уже есть картина"
                end
                return {
                    text = text
                }
