...
function UIHelper.createNamePaintingField(parent, e)
    local textField = mwse.mcm.createTextField(parent, {
        buttonText = e.setNameText or "Переименовать",
        ...
        callback = function()
            ...
            if e.dataHolder.paintingName:len() > 31 then
                tes3ui.showMessageMenu{
                    message = "Слишком длинное имя. Не более 32 символов.",
                    buttons = {
                        {
                            text = "OK"
                        }
...
function UIHelper.openPaintingMenu(e)
    ...
    --do cancel
    if e.cancels then
        local button = buttonBlock:createButton{ text = "Отмена", id = "JOP.CloseButton" }
...
function UIHelper.scrapePaintingMessage(callback)
    tes3ui.showMessageMenu{
        message = "Счистить картину с холста?",
        buttons = {
            {
                text = "Да",
                callback = function()
                    callback()
                    tes3.messageBox("Вы счищаете краску с холста")
                    tes3.playSound{
