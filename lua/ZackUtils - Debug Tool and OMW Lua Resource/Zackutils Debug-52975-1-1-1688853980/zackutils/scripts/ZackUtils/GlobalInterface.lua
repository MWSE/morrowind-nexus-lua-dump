...
local function playSound(soundId)
    --This is a hack, and relies on a MWScript item, but is capable of playing all vanilla sounds.
    local player = interfaces.ZackUtilsG.getPlayer()
    for index, value in ipairs(soundLines) do
        if (value:lower() == soundId:lower()) then
            world.mwscript.getGlobalScript("zhac_playsoundscript").variables.soundId = index
            print(value)
            return
        end
    end
