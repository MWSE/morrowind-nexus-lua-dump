    ...
    local recorder = {
        ...
        events = {
            focusGain = async:callback(function()
                ...
                ui.showMessage("Type soemthing ins")
...
local function clickItem(x, y)
    ...
    if bindingSection:get(y.props.keyName) == "Press Button..." then
        ...
        if mousePress then
            bindingSection:set(buttonToSet, { id = mousePress })
            I.AA_Bindings.updateBindingKey(y.props.keyName, mousePress)
            bindingSection:set(buttonToSet, defaultName(mousePress))
            buttonToSet = nil
        end
   ...
   -- print(y.props.keyName)
    --print( bindingSection:get(y.props.keyName))
    prevVal[y.props.keyName] = bindingSection:get(y.props.keyName)
    bindingSection:set(y.props.keyName, "Press Button...")
...
local function focusLoss(x, y)
    ...
    buttonToSet = y.props.keyName
    if bindingSection:get(y.props.keyName) == "Press Button..." then
        bindingSection:set(y.props.keyName, prevVal[y.props.keyName])
    end
end
local function keyPress(x, y)
    ...
    bindingSection:set(buttonToSet, { id = x.code })
    I.AA_Bindings.updateBindingKey(y.props.keyName, x.code)
    bindingSection:set(buttonToSet, input.getKeyName(x.code) .. " Key")
    buttonToSet = nil
    skipThis = true
end
