...
local function moveToActor(objectToTeleport, targetName, justReturn)
    ...
    local foundObject
    if targetName:lower() == "player" then
        foundObject = world.players[1]
    end
    for index, value in ipairs(objectToTeleport.cell:getAll()) do
        if value.recordId == targetName:lower() then
            foundObject = value
        end
    end
    ...
    if foundObject then
        local destPos = foundObject.position
        local destRot = foundObject.rotation
        local destCell = foundObject.cell
        if (justReturn == true) then
            objectToTeleport:sendEvent("selectReturn", foundObject)
            return
        end
        objectToTeleport:teleport(destCell, destPos, destCell)
        return
    else
        error("Unable to find" .. targetName)
    end
end
...
local function ZackUtilsTeleportToCell(data)
    ...
    if (data.item.recordId == "player") then
        if (data.item.id ~= "1_-1") then
      --      return
        end
    end
    local cell
    if (data.cellname and data.cellname.name ~= nil) then
        data.cellname = data.cellname.name
        cell = world.getCellByName(data.cellname)
    elseif data.cellId ~= nil then
        cell = world.getCellById(data.cellId)
    else
        cell =  world.getCellByName(data.cellname)
    end
...
local function ZackUtilsCreate(data)
    ...
    item.enabled = true
    item:teleport(world.getCellById(data.cell), data.position, data.rotation)
    print(data.cell,data.itemid)
    print(data.position,data.rotation)
...
local function cellCopy(sourcecellName, targetcellName, basePosition, player)
    ...
    for i, ref in ipairs(obs) do
        if (sourcecellName ~= "WhiterunBreezehome" or ((ref.type == types.ESM4Door or ref.type == types.ESM4Static) and not interfaces.DebugModeGlobal.isBlackListed(ref))) then
            local newref = ZackUtilsCreateInterface(ref.recordId, targetcellName, ref.position + basePosition,
                ref.rotation)
...
local function ZackUtilsPauseWorld(doPause, val)
    if (val ~= nil) then
...
local function renameActivator(object, newName)
    ...
    local newActData = {
        name = newName,
        model = removeFirstInstance(object.type.records[object.recordId].model, "meshes\\"),
        mwscript = object.type.records[object.recordId].mwscript,
    }
    ...
    local function table_to_lua(tbl, indent)
        ...
        for k, v in pairs(tbl) do
            ...
            if type(v) == "table" then
                luaTable = luaTable .. nextIndent .. "[" .. k .. "] = "
                luaTable = luaTable .. table_to_lua(v, nextIndent)
                luaTable = luaTable .. ",\n"
            ...
            elseif  v.x ~= nil and v.y ~= nil and v.z ~= nil then
                    ...
                    luaTable = luaTable .. nextIndent .. "[" .. k .. "] = "
                    luaTable = luaTable .. table_to_lua(v3, nextIndent)
                    luaTable = luaTable .. ",\n"
            else
                ...
                elseif type(v) == "number" then
                    v = tostring(v)
                elseif v and v.x ~= nil and v.y ~= nil and v.z ~= nil then
                    v = {
                        x = v.x,
                        y = v.y,
                        z = v.z
                ...
                luaTable = luaTable .. nextIndent .. "[" .. k .. "] = " .. tostring(v) .. ",\n"
            end
        end

        luaTable = luaTable .. indent .. "}"
        return luaTable
    end

    local content = table_to_lua(tbl, "")
    util.writeToFile("return " .. content, "/applications/openmw/" .. filename)
end
...
return {
    ...
    eventHandlers  = {
        ...
        ZackUtilsAddItems = ZackUtilsAddItems,
        setDisabled = setDisabled,
        ZackUtilsEmptyInto = ZackUtilsEmptyInto,
        createContainerFilledWithType = createContainerFilledWithType,
        setPos = setPos,
        setGameTime = setGameTime,
        ZHC_playSound = playSound,
