...
-- returns control to the AI, as the AI knows that is the actor's neutral idle state.
	if logLevel2 then
		mwse.log('%s: playGroupIdle("%s")', modPrefix, mobRef)
	end
	--- nope tes3.playAnimation({reference = mob, group = tes3_animationGroup_idle})
...
local function checkPlayAnimGroup(mob, animGroup)
	...
	if animationController then
		...
		if animGroupMovement then
			...
			if logLevel4 then
		        local s = animationGroupDict[animGroupMovement]
