...
changeWanderRadius = true, -- allow changing Wander radius
logLevel = 0
}
...
assert(config)
local bumpity, bump, processCreatures, skipCreatures, changeWanderRadius
...
local function updateFromConfig()
	...
	changeWanderRadius = config.changeWanderRadius
...
local function collision(e)
	...
	and (player.data.ab01travelType > 0) then
		if not travelMsgOnce then
			if logLevel1 then
				travelMsgOnce = true
				mwse.log([[%s: player.data.ab01travelType > 0, skip collision]], modPrefix)
			end
		end
		return -- skip while Smart Companions travelling
	end
	if travelMsgOnce then
		travelMsgOnce = false
	end
	...
	and targetMob.actorType then
		...
		or isMobScenicTravelCreature(targetMob)
		or isDummy(target)
		or isCreatureToSkip(targetMob) then
			return
		end
	...
	if (not mobIsMobilePlayer) then
		...
		if activePackage then
			if activePackage.type == tes3_aiPackage_wander then
				if changeWanderRadius
				and activePackage.isMoving
				and (  not ( math_abs(activePackage.distance) < 0.01 )  )
				and (  not ( ref == tes3.getPlayerTarget() )  )

-- Note: skipping in combat as it seems it seems sometimes
-- changing wander distance could mess with melee combat position distance
...
-- removing normals from root collision nodes
		...
		and (activePackage.isMoving) then
			assert(not skipRefMove)
			assert(not mobIsMobilePlayer)
...
---assert( not isPlayer(ref) )
			...
			and (not ref.tempData.ab01bmbp2) then
				if activePackage then
					...
					if (activePackage.type == tes3_aiPackage_wander)
					and changeWanderRadius
					and tes3.getLocked({reference = target}) then
						changeAiDistance(activePackage)
					end
					---activePackage.isDone = true
					---activePackage.isFinalized = true
...
local function modConfigReady()
	...
	category:createYesNoButton({
		label = 'Change Wander Radius',
		description = [[Change Wander Radius toggle. Can improve actors mobility.
Only reason to try and disable this option is if you are experiencing some weird collision behavior with hostile actors.]],
		configKey = 'changeWanderRadius'
	})
