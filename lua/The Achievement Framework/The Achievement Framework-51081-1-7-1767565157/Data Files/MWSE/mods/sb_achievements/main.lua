    ...
    if (tes3.player and tes3.player.data["achievements"]) then
        ...
        for _, achievementList in ipairs(achievements) do
            ...
            for _, achievement in ipairs(achievementList) do
                local conditionCheck = (achievement.conditionType == 1 and (achievement.condition() == true)) or ((achievement.conditionType == 2 or achievement.conditionType == 3) and achievement.progress() >= achievement.progressMax())
                if (tes3.player.data["achievements"][achievement.id] == false and conditionCheck) then
            ...
            if (interop.countTotalAchievements(category) == 0) then
                ...
                for index, achievementElement in ipairs(menuAchievements[category]) do
                    ...
                    if (unlocked == false and ((choice and group) or (grouped and (hide or group)))) then
                        ...
                        local conditionProgress = (achievement.conditionType == 1 and "") or (achievement.conditionType == 2 and " (" .. achievement.progress() .. "/" .. achievement.progressMax() .. ")") or (achievement.conditionType == 3 and " (" .. math.min(100, math.round((achievement.progress() / achievement.progressMax()) * 100, 1)) .. "%)")
                        achievementElement.title.text = achievement.title .. conditionProgress
