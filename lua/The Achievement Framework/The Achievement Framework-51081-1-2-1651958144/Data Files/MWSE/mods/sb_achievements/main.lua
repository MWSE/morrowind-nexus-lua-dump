        ...
        for category, _ in ipairs(interop.category) do
            ...
            if (interop.countTotalAchievements(category) == 0) then
                ...
                for index, achievementElement in ipairs(menuAchievements[category]) do
                    ...
                    interop.unlockedAchievements[category] = interop.unlockedAchievements[category] + (unlocked and 1 or 0)

                    achievementElement.icon.color = unlocked == true and achievements[category][index].colour or interop.colours.white
                    achievementElement.desc.text = ((unlocked == false and achievements[category][index].hideDesc and mcm.config.showHiddenAchievements == 0) and achievements[category][index].replaceDesc)
                            or (mcm.config.showHiddenAchievements == 1 and "")
                            or achievements[category][index].desc
            ...
            menuCategories[category].text = tostring(interop.countUnlockedAchievements(category)) .. " / " .. tostring(interop.countTotalAchievements(category))
        end

        menuCategories[0].text = tostring(interop.countUnlockedAchievements()) .. " / " .. tostring(interop.countTotalAchievements())
    end
end
    ...
    if (e.element.id == tes3ui.registerID("MenuStat") and e.newlyCreated) then
        ...
        for _, achievementList in ipairs(achievements) do
            ...
            for index, achievement in ipairs(achievementList) do
                ...
                achievementElement.desc.text = achievement.hideDesc and achievement.desc
