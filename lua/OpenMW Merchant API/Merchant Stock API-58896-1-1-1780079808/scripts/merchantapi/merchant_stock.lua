...
local YAML_SCAN_PREFIXES = {
    'database/merchantapi/',
}
...
local state = {
    ...
    merchantInventoriesByActorCell = {},
    config = {},
...
local function validateItemChances(value, sourcePath, sourceIndex)
    if value == nil then return true end
    if type(value) ~= 'table' then
        logValidation('ERROR', sourcePath, sourceIndex, '"item-chances" must be a table/map of itemId: chancePercent.')
        return false
    end

    local ok = true
    for itemId, chancePercent in pairs(value) do
        if type(itemId) ~= 'string' or normalize(itemId) == '' then
            logValidation('ERROR', sourcePath, sourceIndex, '"item-chances" keys must be non-empty item ID strings.')
            ok = false
        end
        local chanceNumber = tonumber(chancePercent)
        if chanceNumber == nil then
            logValidation('ERROR', sourcePath, sourceIndex, string.format('"item-chances.%s" must be numeric 0..100.', tostring(itemId)))
            ok = false
        elseif chanceNumber < 0 or chanceNumber > 100 then
            logValidation('WARN', sourcePath, sourceIndex, string.format('"item-chances.%s" is outside 0..100 and will be clamped.', tostring(itemId)))
        end
    end
...
local function parseQuantityRangeSpec(spec)
    if type(spec) == 'number' then return spec, spec end
    if type(spec) ~= 'string' then return nil, nil end
    local trimmed = spec:gsub('^%s+', ''):gsub('%s+$', '')
    if trimmed == '' then return nil, nil end

    local minRaw, maxRaw = trimmed:match('^([^,]+)%s*,%s*([^,]+)$')
    if minRaw ~= nil and maxRaw ~= nil then
        local minValue = tonumber(minRaw)
        local maxValue = tonumber(maxRaw)
        if minValue == nil or maxValue == nil then return nil, nil end
        return minValue, maxValue
    end

    local singleValue = tonumber(trimmed)
    if singleValue ~= nil then return singleValue, singleValue end
    return nil, nil
end

local function validateItemQuantities(value, sourcePath, sourceIndex)
    if value == nil then return true end
    if type(value) ~= 'table' then
        logValidation('ERROR', sourcePath, sourceIndex, '"item-quantities" must be a table/map of itemId: "min,max".')
        return false
    end

    local ok = true
    for itemId, rangeSpec in pairs(value) do
        if type(itemId) ~= 'string' or normalize(itemId) == '' then
            logValidation('ERROR', sourcePath, sourceIndex, '"item-quantities" keys must be non-empty item ID strings.')
            ok = false
        end
        local minValue, maxValue = parseQuantityRangeSpec(rangeSpec)
        if minValue == nil or maxValue == nil then
            logValidation('ERROR', sourcePath, sourceIndex, string.format('"item-quantities.%s" must use "min,max" or a single numeric value.', tostring(itemId)))
            ok = false
        else
            if minValue < QUANTITY_MIN or minValue > QUANTITY_MAX then
                logValidation('WARN', sourcePath, sourceIndex, string.format('"item-quantities.%s" min is outside %d..%d and will be clamped.', tostring(itemId), QUANTITY_MIN, QUANTITY_MAX))
            end
            if maxValue < QUANTITY_MIN or maxValue > QUANTITY_MAX then
                logValidation('WARN', sourcePath, sourceIndex, string.format('"item-quantities.%s" max is outside %d..%d and will be clamped.', tostring(itemId), QUANTITY_MIN, QUANTITY_MAX))
            end
        end
    end
...
local function validateRuleData(ruleData, sourcePath, sourceIndex)
    ...
    if not validateItemChances(ruleData['item-chances'], sourcePath, sourceIndex) then ok = false end
    if not validateItemQuantities(ruleData['item-quantities'], sourcePath, sourceIndex) then ok = false end
...
local function parseItemChanceByRecordId(ruleData, pool)
    local chances = {}
    if type(ruleData) ~= 'table' or type(pool) ~= 'table' or type(pool.recordIdSet) ~= 'table' then return chances end
    local raw = ruleData['item-chances']
    if type(raw) ~= 'table' then return chances end

    for itemId, chancePercent in pairs(raw) do
        local normalizedId = normalize(itemId)
        if normalizedId ~= '' and pool.recordIdSet[normalizedId] == true then
            chances[normalizedId] = clampNumber(chancePercent, 0, 100, 100)
        elseif normalizedId ~= '' then
            print(string.format('[MerchantStock] item-chances ignored for unknown item-table ID "%s".', tostring(itemId)))
        end
    end
...
local function parseItemQuantityRangeByRecordId(ruleData, pool)
    local quantities = {}
    if type(ruleData) ~= 'table' or type(pool) ~= 'table' or type(pool.recordIdSet) ~= 'table' then return quantities end
    local raw = ruleData['item-quantities']
    if type(raw) ~= 'table' then return quantities end

    for itemId, rangeSpec in pairs(raw) do
        local normalizedId = normalize(itemId)
        if normalizedId ~= '' and pool.recordIdSet[normalizedId] == true then
            local minValue, maxValue = parseQuantityRangeSpec(rangeSpec)
            if minValue ~= nil and maxValue ~= nil then
                local minClamped = math.floor(clampNumber(minValue, QUANTITY_MIN, QUANTITY_MAX, QUANTITY_MIN))
                local maxClamped = math.floor(clampNumber(maxValue, QUANTITY_MIN, QUANTITY_MAX, QUANTITY_MAX))
                if minClamped > maxClamped then minClamped, maxClamped = maxClamped, minClamped end
                quantities[normalizedId] = { min = minClamped, max = maxClamped }
            end
        elseif normalizedId ~= '' then
            print(string.format('[MerchantStock] item-quantities ignored for unknown item-table ID "%s".', tostring(itemId)))
        end
    end
...
local function addStockRule(ruleData, sourcePath, sourceIndex)
    ...
    local itemChanceByRecordId = parseItemChanceByRecordId(ruleData, pool)
    local itemQuantityRangeByRecordId = parseItemQuantityRangeByRecordId(ruleData, pool)
    local seedRaw = nil
    ...
    state.stockRules[#state.stockRules + 1] = {
        ...
        itemChanceByRecordId = itemChanceByRecordId,
        itemQuantityRangeByRecordId = itemQuantityRangeByRecordId,
    }
...
local function collectYamlPaths()
    ...
    for _, prefix in ipairs(YAML_SCAN_PREFIXES) do
        for path in vfs.pathsWithPrefix(prefix) do
            if isYamlPath(path) and seen[path] ~= true then
                seen[path] = true
                paths[#paths + 1] = path
            end
        end
...
local function chooseCountInRange(minValue, maxValue, rng)
    local minv = math.floor(clampNumber(minValue, QUANTITY_MIN, QUANTITY_MAX, QUANTITY_MIN))
    local maxv = math.floor(clampNumber(maxValue, QUANTITY_MIN, QUANTITY_MAX, QUANTITY_MAX))
    if minv > maxv then minv, maxv = maxv, minv end
    local range = maxv - minv + 1
    if range <= 1 then return minv end
    if type(rng) == 'function' then return minv + (rng() % range) end
...
local function getRegionValues(cell)
    ...
    -- Cache lookup result (including empty lookups) to avoid repeated cell traversal.
    if cacheKey ~= '' then state.exteriorRegionValuesByCell[cacheKey] = regionValues end
    return regionValues
end
...
local function buildMerchantPlanForRule(rule, stableActorId)
    ...
    local targetSlots = math.min(requested, #allOrder)
    if targetSlots < 1 then return nil end
    local plan = {}
    local itemChanceByRecordId = type(rule.itemChanceByRecordId) == 'table' and rule.itemChanceByRecordId or {}
    local itemQuantityRangeByRecordId = type(rule.itemQuantityRangeByRecordId) == 'table' and rule.itemQuantityRangeByRecordId or {}

    local function rollItemChance(recordId)
        local chancePercent = tonumber(itemChanceByRecordId[recordId])
        if chancePercent == nil then return true end
        if chancePercent <= 0 then return false end
        if chancePercent >= 100 then return true end
        local roll = nil
        if type(rng) == 'function' then
            roll = rng() % 100
        else
    ...
    local accepted = 0
    for i = 1, #allOrder do
        if accepted >= targetSlots then break end
        local recordId = allOrder[i]
        if not rollItemChance(recordId) then
            -- Skip failed item and try later unique items for this slot.
        else
            local quantityRange = itemQuantityRangeByRecordId[recordId]
            local count = nil
            if type(quantityRange) == 'table' then
                count = chooseCountInRange(quantityRange.min, quantityRange.max, rng)
            else
                count = chooseStackCount(generationConfig, rng)
            end
...
local function getActorCellInventoryCacheKey(actor)
    if actor == nil then return '' end
    local actorKey = normalize(tostring(actor.id or actor.recordId or ''))
    if actorKey == '' then return '' end
    local cellKey = getCellTraversalKey(actor.cell)
    if cellKey == '' then return '' end
    return string.format('%s@%s', actorKey, cellKey)
end

local function getMerchantInventories(actor)
    local cacheKey = getActorCellInventoryCacheKey(actor)
    if cacheKey ~= '' then
        local cached = state.merchantInventoriesByActorCell[cacheKey]
        if type(cached) == 'table' and type(cached.allInventories) == 'table' then
            return cached.allInventories, cached.preferredInventory
        end
    end
    ...
    if cacheKey ~= '' then
        state.merchantInventoriesByActorCell[cacheKey] = {
            allInventories = allInventories,
            preferredInventory = preferredInventory,
        }
    end
...
local function onLoad(data)
    ...
    state.merchantInventoriesByActorCell = {}
