...
local cf = mwse.loadConfig("Impact Sounds", {newsnd = true, metblood = true, wdmg = true,
volimp = 0.75, volarm = 1, voldmg = 0.75, volsw = 0.75, volit = 1, npcfdist = 2000, volfoot = 0.75, volfootnpc = 1, volfarm = 0.75, volfarmnpc = 1, volswim = 0.5, volcont = 1})
...
ps:createYesNoButton{label = eng and "Impacts to surfaces will damage the weapon" or "Удары по поверхностям будут повреждать оружие", variable = var{id = "wdmg", table = cf}}
	...
	if wt < 9 then
		if not e.targetMobile then
			...
			if hit then local r = hit.reference	local mob = r and r.mobile		local Mat, dir		local dmg = WT[wt].iso
				...
				if w and Mat then
					if cf.wdmg then rw.variables.condition = math.max(rw.variables.condition - ad.physicalDamage * L.MatD[Mat] * 0.1, 0) end
		...
		if m and (r == p or m.playerDistance < cf.npcfdist) then	local water = L.WATERSND[sid] and (L.FOOTS[sid] ~= "J" and m.isSwimming and 1 or 0)
			if cf[r == p and "volfarm" or "volfarmnpc"] > 0 and water ~= 1 then	local ob = r.object		local cui
				if ob.objectType == tes3.objectType.creature then
					...
					if ob.type == tes3.creatureType.undead and ob.blood == 1 then tes3.playSound{reference = r, soundPath = L.RSound("Bones"), volume = cf["volfarmnpc"], mixChannel = 3, pitch = math.random(90,110)/100} end
				...
				if cui then tes3.playSound{reference = r, soundPath = L.RSound("Armor" .. cui), volume = cf[r == p and "volfarm" or "volfarmnpc"], mixChannel = 3, pitch = math.random(90,110)/100} end
			end
			...
			if water then
				...
				tes3.playSound{reference = r, soundPath = L.RSound((Mat or "Dirt") .. L.FOOTS[sid]), volume = cf[r == p and "volfoot" or "volfootnpc"], mixChannel = 3, pitch = math.random(90,110)/100}
				e.block = true
			end
		end
