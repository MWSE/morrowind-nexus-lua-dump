...
I.Settings.registerGroup({
	...
	settings = {
		...
		numbSetting('moveMult',moveMult, false,0.5,4),
...
return {
  engineHandlers = { 
    ...
    onUpdate = function(dt)
		if enabled then
			local buttonPressed = input.isActionPressed(input.ACTION.ToggleSpell) and input.isActionPressed(input.ACTION.Activate)
			if (endEarly) or (buttonPressed and not buttonWasPressed) then
				...
				if savedMode then
					...
					if savedMode == camera.MODE.Static then
						...
						lastHurt = core.getSimulationTime() -- stop first tick from triggering
						if doShader then
						  shader:enable()
						end
					end
				...
				if doOnce then
					...
					if doShader then
					  if wobbleIntensity == 0 then
						  ...
						  shader:setFloat("displacement",wobbleMax*(0.35*math.max(fatPct,magPct) + 0.65*heaPct))
					  end
					end
			...
			--		camera.setStaticPosition(camera.getPosition() + offset)
				...
				if doHealthDrain then
				  ...
				  if hurtTime > 10 then -- if enough time has passed
				    hurtTime = 0
					--costMult = math.max(0.01, 2 - 0.02*skills.mysticism(self).modified)
					costMult = 100/(50 + skills.mysticism(self).modified)
					dynamic.health(self).current = math.max(0,dynamic.health(self).current - math.ceil(costMult*healthCost)) -- don't set to below 0
					--costMult = math.max(0.01, 2 - 0.02*attributes.willpower(self).modified)			
					costMult = 100/(50 + attributes.willpower(self).modified)
					dynamic.magicka(self).current = math.max(0,dynamic.magicka(self).current - math.ceil(costMult*magickaCost)) -- don't set to below 0
					--costMult = math.max(0.01, 2 - 0.02*attributes.endurance(self).modified)
					costMult = 100/(50 + attributes.endurance(self).modified)
					dynamic.fatigue(self).current = math.max(0,dynamic.fatigue(self).current - math.ceil(costMult*fatigueCost)) -- don't set to below 0
					-- modify shader effect based on junk
					if doShader then
					  if wobbleIntensity > 0 then
						  local heaPct = math.sqrt(1 - (dynamic.health(self).current/dynamic.health(self).base))
				...
				local offset = util.transform.rotateZ(camera.getYaw()) * util.vector3(moveRight*speedBoost*200, moveForward*speedBoost*200, moveUp*speedBoost*150)
				camera.setStaticPosition(camera.getPosition() + offset)
			end
		end
	end
