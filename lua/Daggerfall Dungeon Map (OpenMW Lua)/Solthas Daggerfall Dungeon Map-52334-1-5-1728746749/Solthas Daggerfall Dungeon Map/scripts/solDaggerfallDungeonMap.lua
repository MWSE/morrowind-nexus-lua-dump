			...
			--		camera.setStaticPosition(camera.getPosition() + offset)
				...
				if doHealthDrain then
				  ...
				  if hurtTime > 10 then -- if enough time has passed
					...
					dynamic.health(self).current = math.max(0,dynamic.health(self).current - math.ceil(costMult*healthCost*0.01*dynamic.health(self).base)) -- don't set to below 0
					costMult = 150/(50 + attributes.willpower(self).modified)
					dynamic.magicka(self).current = math.max(0,dynamic.magicka(self).current - math.ceil(costMult*magickaCost*0.01*dynamic.magicka(self).base)) -- don't set to below 0
					costMult = 150/(50 + attributes.endurance(self).modified)
					dynamic.fatigue(self).current = math.max(0,dynamic.fatigue(self).current - math.ceil(costMult*fatigueCost*0.01*dynamic.fatigue(self).base)) -- don't set to below 0
					-- modify shader effect based on junk
					if doShader then
					  if wobbleIntensity > 0 then
						  local heaPct = math.sqrt(1 - (dynamic.health(self).current/dynamic.health(self).base))
				...
				camera.setPitch(camera.getPitch() + (input.getMouseMoveY() * 0.005) + (input.getAxisValue(input.CONTROLLER_AXIS.LookUpDown) * 0.05))
				camera.setYaw(camera.getYaw() + (input.getMouseMoveX() * 0.005) + (input.getAxisValue(input.CONTROLLER_AXIS.LookLeftRight) * 0.05))
				camera.setRoll(0)
