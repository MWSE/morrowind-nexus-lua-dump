...
local function unlife(reference)
	-- It seems that simply forcing enabled didn't work, so now a more sophisticated approach is followed:
	if reference.disabled then
		local check = reference:enable()
		-- If it was not enabled properly, then we don't try to resurrect the reference.
		if not check then
			return
		end
	end
	-- Just checking that it has not been resurrected already
	if not reference.isDead then return end
	-- Checking that the mobile is there
	...
	local mobileCell = reference.mobile.cell and reference.mobile.cell.id
...
timer.register("sa_kiwf_unlifeStart", function(e)
	local SH = e.timer.data.safeRef
	if not SH:valid() then
		return
	end
	local actor = SH:getObject()
...
local function damagedCallback(e)
	...
	-- Has the actor the soul trap effect?
	local result = e.mobile:getActiveMagicEffects({ effect = tes3.effect["soultrap"] })
	local isSoultrapped = next(result) ~= nil

	-- If it has fire damage, then do nothing
	if hasFire then
		if config.messageEnabled then tes3.messageBox("Killed it with fire!") end
		return
	elseif isSoultrapped then
		if config.messageEnabled then tes3.messageBox("It shall not rise again!") end
		return
	end
	...
	local safeHandle = tes3.makeSafeObjectHandle(e.reference)

	timer.start({
		type = timer.game,
		duration = config.timeToUnlife,
		callback = "sa_kiwf_unlifeStart",
		data = {safeRef = safeHandle}
	})
