		...
		local isNpc = types.NPC.objectIsInstance(actor) 
			if isNpc == true then
				local dist = (world.players[1].position - actor.position):length()
        			if dist < 200 then
					local effect = types.Actor.activeSpells(world.players[1]):isSpellActive("kokosz_animate_weak_skeleton")
					if effect == true then
						local isDead = types.Actor.isDead(actor)
						if isDead == true then
							for i, Spellid in pairs(types.Actor.activeSpells(world.players[1])) do 																if Spellid.id == "kokosz_animate_weak_skeleton" then
									types.Actor.activeSpells(world.players[1]):remove(Spellid.activeSpellId)
									zombie = world.createObject('kokosz_skeleton_weak', 1)
