local mod = "Полоса здоровья, магии и усталости друзей и недругов"
local version = "1.5.2"
...
local function updateTarget()
	if tes3.player and tes3.mobilePlayer then
		local hitResult = tes3.rayTest({ 
			...
			ignore = { tes3.player },
			maxDistance = (tes3.findGMST(tes3.gmst.iMaxActivateDist).value) + 5000
		})
...
local function onModConfigReady(e)
	...
	colorCategory:createDropdown{
		label = "Цвет полосы магии",
		description = "По умолчанию: Синий",
		options = {
			{label = "Серый", value = 0},
			{label = "Красный", value = 1},
			{label = "Зеленый", value = 2},
	...
	colorCategory:createDropdown{
		label = "Цвет полосы усталости",
		description = "По умолчанию: Зеленый",
		options = {
			{label = "Серый", value = 0},
			{label = "Красный", value = 1},
			{label = "Зеленый", value = 2},
	...
	local sizeCategory = page:createCategory("Размер")
	...
	local positionCategory = page:createCategory("Позиция")
	
	positionCategory:createSlider{
		label = "Позиция по оси X",
