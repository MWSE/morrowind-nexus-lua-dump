...
local function toggleRepairEnhancementLists(menu)
    ...
    if repairList.visible then
        toggleButton.text = "Перейти к усилению"
        if modeLabel then
            modeLabel.text = "Ремонт"
        end
    else
        toggleButton.text = "Перейти к ремонту"
        if modeLabel then
            modeLabel.text = "Усиление"
        end
    end
    ...
    if StationService.isRepairMenuAtStation() and config.mcm.enableEnhancement then
        ...
        if titleLayout then
            ...
            local modeLabel = labelBlock:createLabel{
                ...
                text = showEnhancements and "Усиление" or "Ремонт"
            }
        ...
        if okButton and okButton.parent then
            local toggleButton = okButton.parent:createButton{
                ...
                text = showEnhancements and "Перейти к ремонту" or "Перейти к усилению"
            }
...
function RepairMenuUI.createEnhancementRow(enhancementList, item, itemData)
    ...
    icon:register("mouseClick", function()
        ...
        if config.mcm.enableFatigueCost and currentFatigue <= 0 then
            ...
            tes3.messageBox("Вы слишком устали, чтобы заниматься усилением предметов.")
            return
        end
        ...
        if success then
            tes3.messageBox("Усиление прошло успешно: %s", message)
        else
            tes3.messageBox("Усиление не удалось: %s", message)
        end
