...
local function onActivationTargetChanged(e)
	...
	unconsciousTarget = (currentTarget.mobile.fatigue.current <= 0)

	createpickpocketGUI(newTarget)
end

local function showFullMenu()
	if currentTarget then
		unconsciousTarget = true
		refreshItemsList()
	end
end
...
local function takeItem()
	...
	if block ~= nil then
		...
		if currentTarget.mobile.fatigue.current <= 0 then
			-- You cannot stop me now!
		elseif ( tes3ui.findMenu(GUI_Pickpocket_multi):findChild(GUI_Pickpocket_sneak).visible == false ) then
			return
		end
...
local function onInitialized()
	...
	event.register("load", clearPickpocketMenu)
	event.register("Pickpocket:ShowFullMenu", showFullMenu)
	event.register("Pickpocket:HideMenu", hideMenu)
