...
assert(config)
local modDisabled, notifyMsgEnabled, filterTopics, autoFocus
...
---@param e tes3uiEventData
local function leftPaneAcquireTextInput(e)
...
local function uiActivatedMenuDialog(e)
	...
	local leftPane = menuDialog:findChild(idMenuDialog_scroll_pane)
	if leftPane then
		leftPane:registerAfter('mouseClick', leftPaneAcquireTextInput)
	end
	...
	elseif leftPane then
		tes3ui.acquireTextInput(leftPane)
	end
...
Note:
When the Topics Filter is focused, to allow UI Expansion Choice numeric shortcut keys you should click some whitespace in the Dialogue left panel to focus it before pressing the numeric key.
You can restore the focus to the Topics Filter by clicking the Topics Filter input box or some whitespace in the Dialogue right panel topics list.]],
		configKey = 'filterTopics'
	})
	category:createYesNoButton({
		label = 'Filter Autofocus',
		description = [[Toggle autofocus on topics filtee. With this disabled, you will have to click the Topics Filter before starting a searchm but you will be able to directly use numeric shortcuts for the dialog choices.
With this enabled, you will automatically filter the visible topics directly pressing a key, but will have to click on the left panel before using use numeric shortcuts for the dialog choices.]],
		configKey = 'autoFocus'
	})
