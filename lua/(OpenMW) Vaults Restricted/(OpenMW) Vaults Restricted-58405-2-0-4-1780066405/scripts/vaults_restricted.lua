...
local searchStartHP    = nil       -- guard HP when search1 began
local searchHpTimer    = nil       -- handle to stop the search poll timer
...
local function abortSearch(reEnableAI)
    if searchHpTimer then searchHpTimer() searchHpTimer = nil end
    if lostPauseActive then
        anim.cancel(self, 'search1')
        if reEnableAI and self:isValid() and not types.Actor.isDead(self) then
            self:enableAI(true)
        end
    end
    ...
    if seesPlayer then
        ...
        elseif guardState == "INVESTIGATING" then
            if not activePackage or activePackage.type ~= "Travel" then
                ...
                self:enableAI(false)
                searchStartHP = types.Actor.stats.dynamic.health(self).current

                anim.playBlended(self, 'search1', {
                ...
                local elapsed   = 0
                local SAFETY_CAP = 6.0   -- hard ceiling if completion never resolves
                local started   = false  -- becomes true once the group is seen active
                ...
                local function finishSearch()
                    ...
                    local p = nearby.players[1]
                    if p and p:isValid() then
                        p:sendEvent("GuardWarning", { message = getLostLine() })
                    end
                ...
                searchHpTimer = time.runRepeatedly(function()
                    if not lostPauseActive then return false end
                    if not self:isValid() or types.Actor.isDead(self) then
                        abortSearch(false)
                        return false
                    end

                    -- attacked during the search: abort and engage
                    if types.Actor.stats.dynamic.health(self).current < searchStartHP then
                        local p = nearby.players[1]
                        abortSearch(true)
                        if p and p:isValid() then attackPlayer(p, false) end
                        return false
                    end

                    elapsed = elapsed + 0.1

                    -- end when the animation finishes (or after the safety cap)
                    local completion = anim.getCompletion(self, 'search1')
                    if completion ~= nil then started = true end
                    if (started and completion == nil)        -- group finished and was removed
                       or (completion ~= nil and completion >= 1)
                       or elapsed >= SAFETY_CAP then
                        finishSearch()
                        return false
                    end
                end, 0.1 * time.second)
