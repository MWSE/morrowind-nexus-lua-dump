local cf = mwse.loadConfig("Ultra AI", {atak = true, stels = true, vis = 200, m4 = false, m11 = false})
...
R[m.reference] = {m = m, a = m.actionData, at = (m.actorType == 1 or m.object.biped) and 1 or 3, lim = math.max(70 + m.object.level*5, 100), rc = L.MAC[m.object.baseObject.id]}
...
if not T.timeLeft then	T = timer.start{duration = 1, iterations = -1, callback = function() local s, beh, HD, status
	for r, t in pairs(R) do s = t.m.combatSession	beh = t.a.aiBehaviorState	HD = nil
		if s and s.selectedAction == 7 or (beh == 6 or beh == 5) then
		...
		elseif not t.m.inCombat then
			if t.m.fight > 30 then	HD = math.abs(pp.z - r.position.z) > 128 * (t.at == 1 and t.m.readiedWeapon and t.m.readiedWeapon.object.reach or 0.7)
				if tes3.isAffectedBy{reference = r, effect = t.m.actorType == 1 and 119 or 118} then status = "CONTROL!"	R[r] = nil
				elseif HD then
					if t.at == 1 then
						if (not t.m.readiedWeapon or t.m.readiedWeapon.object.type < 9) and not r.object.inventory:contains(L.stone) then
...
tes3.findGMST("fAIFleeFleeMult").value = 0		tes3.findGMST("fAIFleeHealthMult").value = 88.888		tes3.findGMST("fFleeDistance").value = 5000			tes3.findGMST("fAIRangeMeleeWeaponMult").value = 70
