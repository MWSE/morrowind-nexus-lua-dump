...
local cf = mwse.loadConfig("Ultra AI", {atak = true, stels = true, vis = 200, m4 = false, m11 = false, gmst = 70, AIsec = 2})
...
R[m.reference] = {m = m, a = m.actionData, c = 0, at = (m.actorType == 1 or m.object.biped) and 1 or 3, lim = math.max(70 + m.object.level*5, 100), rc = L.MAC[m.object.baseObject.id]}
...
if s.selectedAction == 7 then	local m = s.mobile 	local t = R[m.reference]
	if t and m.health.normalized > 0.1 and m.flee < t.lim then s.selectedAction = t.at
...
p0:createYesNoButton{label = "More frequent enemy attacks (Requires game restart)", variable = var{id = "atak", table = cf}}
