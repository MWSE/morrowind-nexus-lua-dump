...
if not T.timeLeft then	T = timer.start{duration = 1, iterations = -1, callback = function() local s, sa, w, beh, HD, status
	...
	for r, t in pairs(R) do s = t.m.combatSession	sa = s and s.selectedAction		beh = t.a.aiBehaviorState	w = t.m.readiedWeapon	w = w and w.object		HD = nil
		if s then
			...
			elseif beh == -1 then
				if tes3.getCurrentAIPackageId(t.m) == 3 then	status = "CONTROL!"		R[r] = nil
				else status = "EXTRA COMBAT!"	t.m:startCombat(mp)		t.a.aiBehaviorState = 3 end
