...
event.register("activate", function(e)
    ...
    tes3.messageBox { message = "What do you want to do?", buttons = { "Pick Up", "Browse", "Stash All", "Retrieve All" }, callback = function(
        ...
        if f.button == 0 then
            currentRef = nil
            tes3.addItem { reference = tes3.player, item = e.target.object }
            target:delete()
        elseif f.button == 1 then
            activateOrEquip()
        elseif f.button == 2 then
...
event.register("activate", function(e)
    ...
    local container = e.target and e.target.baseObject
    if not container then return end
    if not containers[container.id] then return end
    currentRef = e.target.baseObject.id
    tes3.messageBox { message = "What do you want to do?", buttons = { "Pick Up", "Browse", "Stash All", "Retrieve All" }, callback = function(f)
        if f.button == 0 then
            tes3.addItem { reference = tes3.player, item = containers[container.id] }
...
event.register("itemTileUpdated", function(e)
    ...
    local ref = tes3.getReference(items[id])
    if ref then
        ref:clone()
...
event.register("uiObjectTooltip", function(e)
    ...
    if items[id] or containers[id] then
        local ref = (items[id] and tes3.getReference(items[id])) or tes3.getReference[id]
        if ref then
            ref:clone()
            local inventory = ref.object.inventory.items
