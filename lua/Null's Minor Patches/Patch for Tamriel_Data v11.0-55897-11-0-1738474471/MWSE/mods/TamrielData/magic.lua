...
function this.radiantShieldSpellResistEffect(e)
	-- NC: Handles case where the target has no mobile.
	local mobile = e.target.mobile
	if (not mobile) then
		return
	end

	local radiantShieldEffects = mobile:getActiveMagicEffects({ effect = tes3.effect.T_alteration_RadShield })
