...
function this.activate(e)
	...
	if not container then
		...
		return --- NC-EDIT: Returned here instead of erroring.
	end
...
event.register("modConfigReady",function ()
	...
	page:new_button{id="activate_on_open"}
	page:new_dropdown{id="auto_close", options={
		{"Never.", 1}, {"Only if container has items", 2}, {"Always", 3},
	}}
	for _, id in ipairs{"stay_open_between_loads", "play_sound", "activate_to_close"} do
	...
	do -- advanced settings
		...
		do -- collision settings 
			...
			-- collision:new_slider{id="distance", max=22.1*7, step=22.1,jump=22.1*3, dp=1}.convertToLabelValue = dist_label_converter
			collision:new_dslider{id="max_xy_dist", max=22.1*7, step=22.1, jump=22.1*3,dp=1, }.convertToLabelValue = dist_label_converter

			collision:new_dslider{id="max_z_dist", max=200, dp=1, step=22.1, jump=22.1*3,}.convertToLabelValue = dist_label_converter
			collision:new_dslider{id="initial_raytest_max_dist", max=22.1*2.5, dp=2, step=22.1/2, jump=22.1}.convertToLabelValue = dist_label_converter
			collision:new_dslider{id="obj_raytest_max_dist", max=22.1*9, dp=1, step=22.1, jump=22.1*3,}.convertToLabelValue = dist_label_converter

			local bb_settings = collision:new_category{id = "bounding_box"}
			do -- bounding box settings
				bb_settings:new_button{id="bb_check"}
				bb_settings:new_dslider{id="bb_xy_scale", max=2, dp=2, step = 0.01, jump = 0.05, }
				bb_settings:new_dslider{id="bb_z_top_scale",  max=2, dp=2, step = 0.01, jump = 0.05, min=0.5, }
