...
event.register("modConfigReady", function()
    dofile("Data Files\\MWSE\\mods\\DynamicTimescale\\mcm.lua")
end)
...
local function waryCounterIncrement()
    ...
    -- waryCounter represents the number of wariness-triggering events that have occured within the past config.waryTime seconds.
    -- Player is wary when waryCounter > 0. (waryCounter should never be < 0.)
    -- When any wariness-triggering event occurs, increment waryCounter, wait config.waryTime seconds, then decrement waryCounter.
...
local function onDamaged(e)
    ...
    if config.waryOnDamage and e.reference == tes3.player and e.damage >= ( tes3.mobilePlayer.health.base * 0.01 * config.damageThreshold ) then
        waryCounterIncrement()
    end
end
...
local function onAttack(e)
    ...
    if config.waryOnAttacks and ( e.reference == tes3.player or e.targetReference == tes3.player ) then
        waryCounterIncrement()
    end
end

-- Runs each time the travel service menu is opened. This function exists so gondola travel will use town instead of wilderness timescale.
...
-- Runs each time any menu is opened. Needed to handle the case where the player opens the menu while in Fast Forward / Turbo mode.
...
local function checkCell()
    if tes3.player.cell.isInterior then
        ...
        -- This nil check is needed because only interior cells behaving as exteriors (Mournhold, Akulakhan's Chamber) have a region.
        if tes3.player.cell.region then

            -- Player is in one of the "exterior" Mournhold cells (that are technically interiors, but should be treated as city/town cells).
        ...
        -- This cell connects the Kogoruhn stronghold with a geographically distant exit, so "should" be much larger than depicted.
        -- Depending on mod configuration, this cell can be treated as a wilderness cell.
...
local function changeTimescale()
    ...
    local newTimescale = oldTimescale
    ...
    -- Player has been still for at least config.stillTime seconds and the still timescale is enabled.
    ...
    -- Wilderness timescale is the default (nothing else applies, player is in an exterior cell that's legal to rest in and has no specific name).
    else
        newTimescale = config.wildernessTimescale
    end

    -- If it's nighttime, and we're not in Fast Forward/Turbo mode, apply the night multiplier.
    -- tonumber() is needed because the text entry box in the MCM changes the value to a string in the config file.
    if speedUp == "neither" and ( tes3.findGlobal("GameHour").value >= tonumber(config.nightBegin) or tes3.findGlobal("GameHour").value <= tonumber(config.nightEnd) ) then
...
-- Returns true only when the player is not moving.
-- No need to check isSwimming, because isWalking or isRunning will also be true if the player is moving in the water.
-- isWalking is always true when isSneaking is true, whether the player is moving or not, so the player will never be considered still while in sneak mode.
local function checkMoving()
    return tes3.mobilePlayer.isWalking or tes3.mobilePlayer.isRunning or tes3.mobilePlayer.isJumping or tes3.mobilePlayer.isFlying
end
...
local function stillCount()
    ...
    if stillState then
        ...
        if checkMoving() then
            ...
            -- stillCounter is incremented by 1 when the player starts moving, and decremented by 1 config.stillTime seconds after they stop moving.
            -- Therefore it's only 0 if the player is currently still *and* has been still for at least config.stillTime.
            -- Player has just started to move, so increment stillCounter.
...
local function onLoaded()
    ...
    if config.stillTime == 0 then
        ...
        -- Otherwise, act as though the player just stopped moving.
        -- It will be at least config.stillTime seconds before the still timescale is applied.
        stillCounter = 1
        timer.start{
            duration = config.stillTime,
            callback = function()
...
local function onInitialized()
    ...
    mwse.log("[Dynamic Timescale 2.0] Initialized.")
end
