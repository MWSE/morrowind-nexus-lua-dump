...
if (mwse.buildDate == nil) or (mwse.buildDate < 20200521) then
  event.register("initialized", function()
      tes3.messageBox(
          "[Call Incarnate] Your MWSE is out of date!"
...
local function onInitialized()
    if (tes3.isModActive("Call Incarnate.esp") == true or
        tes3.isModActive("Call_Incarnate.esp") == true) then
        event.register("journal", function(e)
            if (e.topic.id == "G93_CallGhost" and e.index == 100) then
                mwscript.addSpell({
                    reference = tes3.player,
                    spell = "OJGA_CallIncarnate"
                })
            end
        end)
    end
