...
local function delayedLeaveMenuMode()
	timer.start({duration = 0.5, -- you can tweak the delay here /abot
			type = timer.real,
			callback = function () tes3ui.leaveMenuMode() end
	})	
end

local function helvilMinePheroTimer()
	tes3.player.data.helvilMine.mineData.upgrades.pheromones = true
	tes3.messageBox({
		message = [[A courier delivers a message from the Helvil Mine Foreman.
Ever since that mist was released into the mine, the queen has laid twice as many eggs a day!
Perhaps you should try more of those experiments...]], buttons = {"OK"}
	})
end
timer.register("helvilMine:PheroTimer", helvilMinePheroTimer)
