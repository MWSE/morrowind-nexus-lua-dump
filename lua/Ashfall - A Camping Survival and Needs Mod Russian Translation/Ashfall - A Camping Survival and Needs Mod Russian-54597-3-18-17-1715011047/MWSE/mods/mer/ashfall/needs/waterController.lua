...
local function douse(bottleData)
    ...
    if bottleData then
        ...
        if liquidContainer then
            ...
            tes3.messageBox("Вы обливаете себя водой.")
        end
    end
...
local function callWaterMenu(e)
    ...
    local message = "Вода (Чистая)"
    if e.waterType == "dirty" then
        message = "Вода (Грязная)"
...
event.register(
    ...
    function(e)
        if e.activator.owned then
            ...
            if innkeeper then
                tes3.messageBox("Поговорите с  %s, что бы набрать воды.", innkeeper.object.name)
                return
            end
        end
        callWaterMenu()
            ...
            tes3ui.showMessageMenu{
                ...
                buttons = {
                    ...
                    },
                    {
                        ...
                        ---@diagnostic disable-next-line
                        tooltipDisabled = {
                            text = "Вы должны закончить приготовление рагу."
                        },
                        callback = function()
