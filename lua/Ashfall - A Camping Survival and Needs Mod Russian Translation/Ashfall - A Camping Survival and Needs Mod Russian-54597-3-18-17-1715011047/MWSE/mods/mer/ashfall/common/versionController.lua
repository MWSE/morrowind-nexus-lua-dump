...
local function showUpdateMessageBox()
    local msg = string.format('Доступна новая версия Пеплопада!')
    ---@type tes3ui.showMessageMenu.params.button[]
    local buttons = {
        {
            text = string.format('Загрузить пеплопад %s', latestVersion),
            callback = showConfirmUpdate
        }, {
            text = "Отключить проверку обновлений",
...
showConfirmUpdate = function()
    ...
    tes3ui.showMessageMenu {
        message = "Выйти из Morrowind и загрузить последнюю версию Пеплопад?",
        buttons = buttons,
        cancels = true,
...
showConfirmDisableNotifications = function()
    local message = "Отключить проверку обновлений?"
    ---@type tes3ui.showMessageMenu.params.button[]
    local buttons = {
        {
            text = tes3.findGMST(tes3.gmst.sYes).value,
            callback = function()
                ...
                config.save()
                tes3ui.showMessageMenu{
                    message = "Проверка обновлений отключена. Вы можете снова включить ее в меню настройки мода во вкладке для разработчиков.",
                    buttons = {
