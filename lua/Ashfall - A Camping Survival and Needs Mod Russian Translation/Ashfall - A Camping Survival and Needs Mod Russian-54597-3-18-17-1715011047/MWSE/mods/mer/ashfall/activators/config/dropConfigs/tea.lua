...
return {
    dropText = function(campfire, item, itemData)
        ...
        return string.format("Заварить %s", teaData.teaName)
    end,
    canDrop = function(campfire, item, itemData)
        ...
        if not hasWater then
            return false, "Нет воды на костре"
        end
        ...
        if not waterClean then
            return false, "Для приготовления чая требуется чистая вода."
        end
    ...
    onDrop = function(campfire, reference)
        ...
        tes3.messageBox("Добавлено %s", reference.object.name)
