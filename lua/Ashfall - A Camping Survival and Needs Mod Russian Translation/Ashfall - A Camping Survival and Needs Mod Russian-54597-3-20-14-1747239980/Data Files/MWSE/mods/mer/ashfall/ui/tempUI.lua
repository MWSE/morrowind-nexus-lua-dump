                ...
                ---\
                    ...
                    ---\
                        ...
                        leftTempPlayerBar:register( "help", function()
                            ...
                            local headerText = string.format("Температура персонажа: %.2f", playerTemp:getValue() )
                            local labelText = "Ваша текущая температура. Напрямую определяет эффекты от перегрева и переохлаждения."
                    ...
                    ---\
                        ...
                        leftTempLimitBar:register( "help", function()
                            local headerText = string.format("Предельная температура: %.2f", common.data.tempLimit)
                            local labelText = "Представляет температуру, которой вы достигнете при сохранении текущих условий."
                            common.helper.createTooltip({header = headerText, text = labelText})
                        end)
...
local function createWetnessIndicator(parentBlock)
    ...
    wetnessBlock:register("help", function()
        ...
        local labelText = "Чем больше вы промокнете, тем сильнее будете мерзнуть, и станете уязвимее к повреждениям."
        common.helper.createTooltip({header = headerText, text = labelText})
    end)
...
local function createShelteredIndicator(parentBlock)
    ...
    rainSheltered:register("help", function()
        local headerText = "Укрыт от дождя"
        local labelText = "В настоящее время вы укрыты и не промокнете под дождем."
        common.helper.createTooltip({header = headerText, text = labelText})
    end)
    ...
    rainUnsheltered:register("help", function()
        local headerText = "Не защищен от дождя"
        local labelText = "Найдите укрытие, чтобы не промокнуть под дождем."
        common.helper.createTooltip({header = headerText, text = labelText})
    end)
    ...
    sunSheltered:register("help", function()
        local headerText = "Укрыт от солнца"
        local labelText = "В настоящее время вы защищены от палящего солнца."
        common.helper.createTooltip({header = headerText, text = labelText})
    end)
    ...
    sunUnsheltered:register("help", function()
        local headerText = "Не защищен от солнца"
        local labelText = "Найдите тень, чтобы избежать палящего солнца."
        common.helper.createTooltip({header = headerText, text = labelText})
    end)
...
local function createConditionStateIndicator(parentBlock)
    ...
    conditionLabelBlock:register("help", function()
        ...
        local labelText = "Ваш текущий температурный режим."
        common.helper.createTooltip({header = headerText, text = labelText})
    end)
end
