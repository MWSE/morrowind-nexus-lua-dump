...
function this.updateRatingsUI()
    ...
    if inventoryMenu then
        ...
        warmthLabel.text = string.format("Тепло: %d    " , warmthValue)
        ...
        coverageLabel.text = string.format("Покров: %d%%", coverageValue)
...
local function createspecialFlagsLabel(parent, object)
    if common.staticConfigs.shadeEquipment[object.id:lower()] then
        ...
        local shadeHeader = shadeBlock:createLabel({ id = IDs.shadeHeader, text = "Солнцезащита" })
...
local function createWarmthLabel(parent, object)
    ...
    local warmthHeader = warmthBlock:createLabel({ id = IDs.warmthHeader, text = "Тепло: " })
...
local function createCoverageLabel(parent, object)
    local coverageBlock = parent:createBlock({ id = IDs.coverageBlock })
        ...
        local coverageHeader = coverageBlock:createLabel({ id = IDs.coverageHeader, text = "Покров: " })
