...
function HarvestService.showIllegalToHarvestMessage(harvestConfig)
    tes3.messageBox("Вы должны находиться в дикой местности, что бы заниматься сбором ресурсов.")
end
...
function HarvestService.showHarvestedMessage(numHarvested, harvestName)
    local message = string.format("Вы собрали %s %s %s", numHarvested, numHarvested > 1 and "шт." or "шт.", harvestName)
