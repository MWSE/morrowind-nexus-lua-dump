...
function Planter:addItems()
    ...
    for _, stack in pairs(plant.inventory) do
        ...
        if item then
            ...
            tes3.messageBox(string.format("Вы собрали %s %s.", count, item.name))
        end
    end
end
...
function Planter:getTooltipMessages()
    ...
    if self.waterAmount > 0 then
        table.insert(messages,
            string.format("Вода: %d/%d", math.floor(self.waterAmount), self.MAX_WATER_AMOUNT))
    end
    if self.plantId then
        if self.plantProgress < 1 then
            table.insert(messages, string.format("Рост: %d%%", self.plantProgress*100))
        else
            table.insert(messages,
                (self.timeUntilHarvestable > 0) and "Урожай собран" or "Можно собирать урожай")
        end
        local plant = tes3.getObject(self.plantId)
        if plant then
...
Planter.buttons = {
    ...
    plantSeed = {
        ...
        tooltip = function()
            return common.helper.showHint(
                "Вы можете посадить семена, перетащив их из своего инвентаря прямо в клумбу."
            )
        end,
        showRequirements = function(e)
            local reference = e.reference or e
        ...
        callback = function(e)
            ...
            timer.delayOneFrame(function()
                common.helper.showInventorySelectMenu{
                    ...
                    noResultsText = "Увас нет растений для выращивания",
                    filter = function(e)
    ...
    water = {
        ...
        tooltip = function()
            return common.helper.showHint(
                "Вы можете полить растение, перетащив емкость с водой прямо на него."
            )
        end,
        tooltipDisabled = function(e)
            ...
            if not hasRoomForWater then
                return{text = "Вы уже поливали эту клумбу"}
            end
        end,
        enableRequirements = function(e)
            ...
            if not hasRoomForWater then
                return false, "Вы уже поливали эту клумбу"
            end
        ...
        callback = function(e)
            ...
            timer.delayOneFrame(function()
                common.helper.showInventorySelectMenu{
                    ...
                    noResultsText = "У вас отсутствует емкость с водой.",
                    filter = function(e)
