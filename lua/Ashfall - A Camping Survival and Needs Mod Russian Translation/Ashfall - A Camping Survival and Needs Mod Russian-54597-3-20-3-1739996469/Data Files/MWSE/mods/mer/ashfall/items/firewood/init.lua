...
local function placeCampfire(e)
    ...
    if not ground then
        tes3.messageBox{ message = "Здесь нельзя разводить костер.", buttons = {tes3.findGMST(tes3.gmst.sOK).value}}
        return
    end
    local tooSteep = (
    ...
    if tooSteep then
        tes3.messageBox{ message = "Здесь слишком крутой склон.", buttons = {tes3.findGMST(tes3.gmst.sOK).value}}
        return
    end
...
local function onActivateFirewood(e)
    ...
    tes3ui.showMessageMenu{
        message = string.format("У вас есть %d %s.", e.target.stackSize, e.target.object.name),
        buttons = {
            {
                text = "Сложить костер",
                ...
                tooltipDisabled = function()
                    if common.helper.getRefUnderwater(e.target) then
                        return {
                            text = "Вы не сможете развести костер под водой."
                        }
                    end
                    return {
                        text = "Вы не можете развести костер в поселении."
                    }
                end
            },
            {
                ...
                tooltip = function()
                    return common.helper.showHint(
                        string.format("Вы можете напрямую собирать дрова, активировав их, удерживая клавишу %s.",
