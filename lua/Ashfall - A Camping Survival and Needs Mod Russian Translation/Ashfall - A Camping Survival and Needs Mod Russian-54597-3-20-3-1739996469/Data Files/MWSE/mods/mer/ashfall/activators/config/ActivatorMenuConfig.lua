        ...
        tooltipExtra = function(reference, tooltip)
                ...
                if fuelLevel > 0 then
                    local fuelLabel = tooltip:createLabel{
                        text = string.format("Топливо: %.1f часов", fuelLevel )
                    }
    ...
    ATTACH_BELLOWS = {
        ...
        tooltipExtra = function(reference, tooltip)
            if reference.data.bellowsId then
                ...
                local text = string.format("%sx Нагрев | %sx Потребление топлива",
        ...
        tooltipExtra = function(ref, tooltip)
            ...
            if ref.data.woodAmount then
                local label = tooltip:createLabel{
                    text = string.format("Дрова: %d/%d", ref.data.woodAmount, WoodStack.getCapacity(ref.object.id ))
                }
                centerText(label)
            end
        end,
