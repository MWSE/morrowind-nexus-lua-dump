...
return {
    dropText = function(campfire, item, itemData)
        return string.format("Установить %s", common.helper.getGenericUtensilName(item))
    end,
        ...
        if isGrill then
            if canAttachGrill then
                if campfire.data.grillId then
                    return false, "На костре уже установлен гриль"
                end
            else
                return false
            end
        elseif isBellows then
            if canAttachBellows then
                if campfire.data.bellowsId then
                    return false, "У костра уже установлены мехи"
                end
            else
                return false
            end
    ...
    onDrop = function(campfire, reference)
        ...
        tes3.messageBox("Установлено: %s", common.helper.getGenericUtensilName(reference.object))
