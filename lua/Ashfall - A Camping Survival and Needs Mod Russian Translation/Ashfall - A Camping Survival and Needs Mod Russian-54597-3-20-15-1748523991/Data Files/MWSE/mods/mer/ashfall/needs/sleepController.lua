...
local function setRestValues(e)
    ...
    local tempText = ( tempLimit < 0 ) and "холодно" or "жарко"
    local restText = ( e.allowRest ) and "отдыха" or "ожидания"

    interruptText = string.format("Сейчас слишком %s для %s, вы должны найти убежище!", tempText, restText)
...
local function activateRestMenu (e)
    ...
    if doForceWait then
        forceWait(restMenu)
        labelText.text = "Вам нужно найти кровать или пойти в помещение, чтобы отдохнуть."
    end

    if ( tempLimit < coldRestLimit ) or ( tempLimit > hotRestLimit ) then
        labelText.text = string.format("Сейчас слишком %s для %s, вы должны найти убежище!",
            ( tempLimit < 0 and "холодно" or "жарко"),
            ( mustWait and "отдыха" or "ожидания" )
        )
        hideSleepItems(restMenu)
    elseif hunger:getValue() > hunger.states.starving.min then
        labelText.text = string.format("Вы слишком голодны для %s.",
            ( mustWait and "ожидания" or "отдыха")
        )
        hideSleepItems(restMenu)
    elseif thirst:getValue() > thirst.states.dehydrated.min then
        labelText.text = string.format("Вы слишком хотите пить для %s.",
            ( mustWait and "ожидания" or "отдыха")
        )
        hideSleepItems(restMenu)
    elseif tiredness:getValue() > tiredness.states.exhausted.min and mustWait then
        labelText.text = "Вы слишком устали, чтобы ждать, вам нужно найти постель."
        hideSleepItems(restMenu)
    end
...
local function checkInterruptSleep()
    if common.helper.getIsSleeping() or common.helper.getIsWaiting() then
        ...
        if tempLimit < coldRestLimit or tempLimit > hotRestLimit then
            wakeUp()
            tes3.messageBox({ message = interruptText, buttons = { "ОК" } })
        end
        --Needs
        if hunger:getValue() >= hunger.states.starving.min - 1 then
            ...
            wakeUp()
            --Message PC
            tes3.messageBox({ message = "Вы умираете от голода.", buttons = { "ОК" } })
        elseif thirst:getValue() > thirst.states.dehydrated.min - 1  then
            ...
            --Message PC
            tes3.messageBox({ message = "Вы обезвожены.", buttons = { "ОК" } })
        elseif (tiredness:getValue() > tiredness.states.exhausted.min - 1) and common.helper.getIsWaiting() then
            ...
            wakeUp()
            --Message PC
            tes3.messageBox({ message = "Вы измучены.", buttons = { "ОК" } })
        end
