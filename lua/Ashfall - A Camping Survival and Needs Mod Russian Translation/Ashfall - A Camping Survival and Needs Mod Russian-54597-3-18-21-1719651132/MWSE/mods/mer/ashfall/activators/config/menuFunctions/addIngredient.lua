...
local function ingredientSelect(campfire, foodType)
    ...
    timer.delayOneFrame(function()
        ...
        common.helper.showInventorySelectMenu{
            ...
            noResultsText = string.format("У вас отсутствуют %ss.", string.lower(foodType)),
            filter = function(e)
                return (
            ...
            callback = function(e) --select how many
                ...
                if e.item then
                    ...
                    if not liquidContainer then
                        tes3.messageBox("Ошибка: Не удалось найти контейнер для жидкости")
                        return
                    end
...
return {
    ...
    tooltipDisabled = {
        text = "Для приготовления рагу необходим ковш."
    },
    tooltip = function()
        return common.helper.showHint("Вы можете добавлять ингредиенты, опуская их прямо в кастрюлю.")
    end,
    callback = function(reference)
        ...
        for _, foodType in ipairs(foodTypes) do
            ...
            table.insert(buttons, {
                ...
                tooltipDisabled = {
                    text = hasFood and string.format("Вы не можете добавить больше %s.", foodType)
                        or string.format("У вас отсутствуют %s.", foodType)
                },
                tooltip = {
        ...
        tes3ui.showMessageMenu{
            message = "Выберите тип ингредиента:",
            buttons = buttons,
            cancels = true,
        }
    end
