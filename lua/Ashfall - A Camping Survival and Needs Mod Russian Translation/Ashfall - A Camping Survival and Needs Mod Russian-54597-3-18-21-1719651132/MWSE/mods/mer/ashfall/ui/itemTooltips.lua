...
local function addWaterTooltips(e)
    ...
    if waterHeat and waterHeat > 0 then
        common.helper.addLabelToTooltip(e.tooltip,
            string.format("Нагрев: %d/100", waterHeat)
        )
    end
    ...
    if capacity then
        ...
        common.helper.addLabelToTooltip(e.tooltip,
            string.format("Вода: %d/%d %s",
                math.floor(waterAmount),
                capacity,
                ( waterType and waterType == "dirty" and "(Грязная) " or "")
            )
        )
    end
    ...
    if unfilteredWater and capacity then
        common.helper.addLabelToTooltip(e.tooltip,
            string.format("Нефильтрованная вода: %d/%d",
...
local function addFoodTooltips(e)
    ...
    if thisFoodType then
        ...
        if nutrition and nutrition ~= 0 then
            labelText = string.format("Питательность: %d", nutrition)
        ...
        if common.helper.isModifierKeyPressed() then
            local actionLabel  = e.tooltip:createLabel({ text = "Есть" })
    ...
    if e.itemData and teaConfig.teaTypes[e.itemData.data.waterType] then
        ...
        if progress >= 100 then
            ...
            if teaData.duration then
                ...
                hoursText = string.format(" в течении %d час%s", duration, (duration > 1 and "ов" or "а") )
            end
        ...
        if progress == 0 then
            teaLabelText = string.format("%s (Незаваренный)", teaLabelText)
        elseif progress < 100 then
            teaLabelText = string.format("%s (Заварен на %d%%)", teaLabelText, progress)
        elseif isCold then
            teaLabelText = teaLabelText .. " (Холодный)"
        end
...
local function addStewTooltips(e, liquidContainer)
    ...
    local stewName = foodConfig.isStewNotSoup(liquidContainer.stewLevels) and "Рагу" or "Суп"

    local progressText

    logger:trace("progress: %d", liquidContainer.stewProgress)
    if liquidContainer.stewProgress < 100 then
        progressText = string.format("%s (Готовность: %d%%)", stewName, liquidContainer.stewProgress )
    elseif liquidContainer.waterHeat < common.staticConfigs.hotWaterHeatValue then
        progressText = string.format("%s (Холодное)", stewName)
    else
        progressText = string.format("%s (Готово)", stewName)
    end
    ...
    for foodType, ingredLevel in pairs(liquidContainer.stewLevels) do
        ...
        if liquidContainer.stewProgress >= 100 then
            ...
            --points " 25 points "
            local pointsText = string.format("%d ед.", common.helper.calculateStewBuffStrength(value, stewBuff.min, stewBuff.max) )
            --for X hours
            local duration = common.helper.calculateStewBuffDuration(liquidContainer.waterHeat)
            local hoursText = string.format("в течении %d час%s", duration, (duration > 1 and "ов" or "а") )
...
local function addWoodStackTooltips(item, itemData, tooltip)
    ...
    if itemData.data.woodAmount then
        common.helper.addLabelToTooltip(tooltip,
            string.format("Дрова: %d/%d", itemData.data.woodAmount, WoodStack.getCapacity(item.id ))
        )
    end
end
