...
local function doCook(ingredReference, showMessage)
    ...
    if showMessage then
        tes3.messageBox("%s полностью готово.", ingredReference.object.name)
    end
end
...
local function doBurn(ingredReference, showMessage)
    ...
    event.trigger("Ashfall:ingredCooked", { reference = ingredReference})
    if showMessage then
        tes3.messageBox("%s подгорело.", ingredReference.object.name)
    end
end
...
local function startCookingIngredient(ingredient, timestamp)
    ...
    if not justChangedCell then
        local message = string.format("%s начинает готовиться.", ingredient.object.name)
...
local function doAddingredToStew(campfire, reference)
    if not foodConfig.getStewBuffForId(reference.object) then
        tes3.messageBox("%s нельзя добавлять в рагу.", reference.object.name)
    ...
    if amountAdded < amount then
        ...
        if amountAdded >= 1 then
            tes3.messageBox("Добавляет %s %s в рагу.", amountAdded, reference.object.name)
        else
            tes3.messageBox("Вы не можете добавить больше %s.", foodConfig.getFoodTypeResolveMeat(reference.object):lower())
        end
        common.helper.pickUp(reference)
    else
        tes3.messageBox("Добавляет %s %s в рагу.", amountAdded, reference.object.name)
