...
local function createNeedsTooltip(e)
    ...
    if bellowsData then
        common.helper.addLabelToTooltip(tooltip,
            string.format("%sx Потребление топлива", bellowsData.burnRateEffect) )
        common.helper.addLabelToTooltip(tooltip,
            string.format("%sx Нагрев", bellowsData.heatEffect) )
    end
    --Fuel Consumers
    if e.itemData and e.itemData.data.fuelLevel then
        common.helper.addLabelToTooltip(tooltip,
            string.format("Топливо: %.1f часов", e.itemData.data.fuelLevel) )
    end
end
