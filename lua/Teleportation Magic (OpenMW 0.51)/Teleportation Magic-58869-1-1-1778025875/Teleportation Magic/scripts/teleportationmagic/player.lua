...
local teleportEffects = require('scripts.teleportationmagic.tpDatabase') -- teleportEffects[effId] = {spell, name, cell, position , yaw, pitch, cost}
...
async:newUnsavableSimulationTimer(0.001, function()
	if I.TRSpells then
		solidMask = util.bitOr(CT.World, CT.Door)
	end
end)
...
I.SkillProgression.addSkillUsedHandler(function(skillid, params)
	...
	for _, eff in ipairs(spell.effects) do
		if teleportEffects[eff.id] then
			entry = teleportEffects[eff.id]
			magicId = eff.id
			break
		end
	end
	...
	local direction = getPlayerVector()
	...
	local function sendSpawn(spawnPos, timeToHit)
		core.sendGlobalEvent("TeleportationMagic_spawnPortal", {
			player = self,
			position = spawnPos,
			magicId = magicId,
			yaw = castYaw,
	...
	local timeToHit = (hitPos - startPos):length() / fTargetSpellMaxSpeed
	...
	-- hit wall, go back 50 units and make sure the portal has minimum height from floor
...
local function onLoad()
	async:newUnsavableSimulationTimer(0.001, function()
		...
		for _, entry in pairs(teleportEffects) do
