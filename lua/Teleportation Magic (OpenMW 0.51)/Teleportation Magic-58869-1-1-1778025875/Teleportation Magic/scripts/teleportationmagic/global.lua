...
local function onLoad(data)
	...
	-- migrate: old saves stored portal id strings, new code expects gameobjects
	for k, v in pairs(saveData.activeByPlayer) do
		if type(v) == "string" then
			saveData.activeByPlayer[k] = nil
		end
	end
end
...
-- savable despawn for the departure portal
local despawnDepartureCb = async:registerTimerCallback("TPM_despawnDeparture", function(data)
	local portal = data.portal
	if not portal then return end
	if saveData.portalMagic[portal.id] ~= data.magicId then return end
	if portal:isValid() then
		playEffectsAt(portal, DESPAWN_VFX_OFFSET)
		portal:remove()
	end
	saveData.portalMagic[portal.id] = nil
	local active = saveData.activeByPlayer[data.playerId]
	if active and active.id == portal.id then
		saveData.activeByPlayer[data.playerId] = nil
	end
end)
...
local function handlePortalActivation(portal, actor)
	...
	saveData.portalMagic[portal.id] = nil
	local active = saveData.activeByPlayer[actor.id]
	if active and active.id == portal.id then
...
local function spawnPortal(data)
	...
	async:newUnsavableSimulationTimer(timeToHit + POST_IMPACT_DELAY, function()
		...
		local prevPortal = saveData.activeByPlayer[player.id]
		if prevPortal then
			if prevPortal:isValid() then
				saveData.portalMagic[prevPortal.id] = nil
		...
		saveData.activeByPlayer[player.id] = portal

		async:newSimulationTimer(DEPARTURE_LIFETIME, despawnDepartureCb, {
			portal = portal,
			magicId = magicId,
			playerId = player.id,
		})
...
return {
	...
	eventHandlers = {
		TeleportationMagic_spawnPortal = spawnPortal,
		TeleportationMagic_actorCaught = actorCaught,
	},
}