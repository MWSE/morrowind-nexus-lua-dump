...
local math_abs, math_floor, math_isclose, math_max = math.abs, math.floor, math.isclose, math.max
...
local function simulate(e)
	local secondsPassed = math_max( 0.0001, math_abs(e.delta) )
	fps = math_max(1 / secondsPassed, 7)
end
...
local function getHeatSpell(damage)
	local dps = math_abs(damage) * damageMul * fps
	if heatEffects then
	...
	if logLevel1 then
		mwse_log('%s: getHeatSpell(damage = %s) dps = %s', modPrefix, damage, roundInt(dps))
	end
	return heatSpell
end

local casters = {} -- e.g. casters[lcMobRefId] = casterObjRefId
...
-- and e.object.anyproperty is totally unreliable
	...
	if not fsp then
		assert(fsp) -- should never happen, right?
		return
	end
	...
	refsById[key] = nil
	if logLevel1 then
		mwse_log('%s: %s() refsById["%s"] cleaned',
...
---@param lcMobRefId string
---@return tes3reference
local function getCasterObjRef(mobRef, lcMobRefId)
	local casterObjRefId = casters[lcMobRefId]
	local casterObjRef
	if casterObjRefId then
		casterObjRef = getRef(casterObjRefId)
	end
	local pos = mobRef.position
	local delta = tes3vector3.new(64, 64, 64)
	local mob = mobRef.mobile
	if mob then
		local boundSize = mob.boundSize
		if boundSize then
			delta.x, delta.y, delta.z = 0, -boundSize.y * 0.75, boundSize.z * 0.5
		end
	end
-- try and skip colliding with the small caster object (e.g. ring)
	pos = pos + delta
...
local function collDamage(e)
	...
	local ref = getRef(lcMobRefId)
	skip[lcMobRefId] = nil
	if not ref then
		return
	end
	if logLevel4 then
		mwse_log('%s: collDamage("%s")', modPrefix, ref.id)
	end
...
local function collision(e)
	...
	if logOnce then
		mwse_log('%s: %s({collType = %s, mobRef = "%s", targetRef = "%s", targetRef.object.mesh = "%s"',
			modPrefix, e['eventType'], collType, lcMobRefId, targetRef.id, targetObj.mesh)
	end
...
---@diagnostic disable-next-line: deprecated
	---if mwscript.disable({reference = ref, modify = true}) then
		if logLevel2 then
			mwse_log('%s: reference "%s deleted', modPrefix, ref.id)
		end
...
local function damage(e)
	...
	if logLevel4 then
		mwse_log('%s: %s() lcMobRefId = %s', modPrefix, e['eventType'], lcMobRefId)
	end
	...
	if logLevel4 then
		mwse_log('%s: %s() collRef = %s', modPrefix, e['eventType'], collRef)
	end
	...
	if logLevel4 then
		mwse_log('%s: %s() casterObjRefId = %s', modPrefix, e['eventType'], casterObjRefId)
	end
	...
	if tes3ui.menuMode() then
		-- should not happen
		if logLevel3 then
			mwse_log('%s: %s() tes3ui.menuMode(), skipping', modPrefix, e['eventType'])
		end
		return
	end
	...
	timer.start({duration = spellDuration + 3, callback =
		function ()
			local ref = getRef(casterObjRefId)
			...
			and hazeShaderOn then
				if logLevel1 then
					mwse_log('%s: haze shader disabled', modPrefix)
				end
				hazeShaderOn = false
	...
	if logLevel2 then
		mwse_log('%s: %s() collType = %s, caster = %s, target = %s, spell = %s',
			modPrefix, e['eventType'], collType, casterObjRef, lcMobRefId, heatSpell)
	end
	...
	and (mobRef == tes3.player) then
		if logLevel1 then
			mwse_log('%s: haze shader enabled', modPrefix)
		end
		hazeShaderOn = true
...
local function calcMoveSpeed(e)
	...
	if math_isclose(speedInLavaMul, 1, 0.001) then
		return
	end
	...
	if logLevel5 then
		mwse_log('%s %s(): e.speed before = %s', modPrefix, e['eventType'], e.speed)
	end
	e.speed = e.speed * speedInLavaMul
	if logLevel5 then
		mwse_log('%s %s(): e.speed after = %s', modPrefix, e['eventType'], e.speed)
	end
end
...
local function loaded()
	...
	if data then
		...
		if casters then
			for k, lcRefId in pairs(casters) do
				if lcRefId then
					local ref = getRef(lcRefId)
					if ref then
						deleteRef(ref)
					end
					casters[k] = nil
...
Use it e.g. if lava does not seem to hurt.]],
	...
	assert(heatCasterObj.id == 'ab01heatCasterObj')
