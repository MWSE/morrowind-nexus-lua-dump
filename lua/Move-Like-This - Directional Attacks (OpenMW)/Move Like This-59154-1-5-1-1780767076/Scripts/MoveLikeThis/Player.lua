...
local function MLT_DoStagger(eventData)
	staggerAnim = nil
	if not (MLTUtils.isPlayingStaggerAnim(self) or MLTUtils.isPlayingKnockBackOrOutAnim(self)) then
...
local function MLT_CriticalHitMessage(eventData)
	ui.showMessage(l10n("CriticalStrike"))
...
return {
    eventHandlers = {
		...
		MLT_mobilityBuff = MLT_ApplyMobilityBuff,
		MLT_blindDebuff = MLT_ApplyBlindDebuff
    },
}
