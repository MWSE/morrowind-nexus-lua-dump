...
local function getRecentPendingThrow()
    ...
    return {
        ...
        perksEnabled = runtimeSection:get('pendingPerksEnabled'),
        criticalEnabled = runtimeSection:get('pendingCriticalEnabled'),
        twinFlightEnabled = runtimeSection:get('pendingTwinFlightEnabled'),
        bleedEnabled = runtimeSection:get('pendingBleedEnabled'),
        paralyzeEnabled = runtimeSection:get('pendingParalyzeEnabled'),
        age = age,
    }
end
...
local function onHit(attack)
    ...
    local function pendingFeatureEnabled(settingKey, pendingKey, default)
        local value = pending[pendingKey]
        if value ~= nil then return value end
        return featureEnabled(settingKey, default)
    end

    local perksEnabled = pendingFeatureEnabled('perksEnabled', 'perksEnabled', true)

    if perksEnabled and pendingFeatureEnabled('criticalEnabled', 'criticalEnabled', true)
    ...
    if perksEnabled and pendingFeatureEnabled('twinFlightEnabled', 'twinFlightEnabled', true)
    ...
    if perksEnabled and pendingFeatureEnabled('bleedEnabled', 'bleedEnabled', true)
    ...
    if perksEnabled and pendingFeatureEnabled('paralyzeEnabled', 'paralyzeEnabled', true)
