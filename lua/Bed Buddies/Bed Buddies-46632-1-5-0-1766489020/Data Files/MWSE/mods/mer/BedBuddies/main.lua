...
local function hasPermission(reference)
    ...
    if (not ownerRef) or publicanClasses[ownerRef.object.class.id:lower()] then
        return tes3.hasOwnershipAccess{ target = reference}
    --Dead
    elseif ownerRef.isDead then
            return true
    --Alive
    else
...
local function registerMCM()
    ...
    local template = mwse.mcm.createTemplate("Bed Buddies")
