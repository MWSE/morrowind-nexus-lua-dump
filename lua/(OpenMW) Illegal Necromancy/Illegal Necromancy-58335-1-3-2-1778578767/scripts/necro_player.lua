...
local reportedThisSession = false
...
local cachedSettings = {
    ...
    WITNESS_INTERVAL       = get("WITNESS_INTERVAL"),
    PERIODIC_CHECK_ENABLED = get("PERIODIC_CHECK_ENABLED"),
    SIGN_COMPAT            = get("SIGN_COMPAT"),
}

local function onSettingsUpdated(_, key)
    if key then
...
section:subscribe(async:callback(onSettingsUpdated))
...
-- returns the recordId of the first active non-exempt summon effect, or nil
local function getActiveReportableSummon()
    local eff = types.Actor.activeEffects(self.object)
    if not eff then return nil end

    for summonId, effectName in pairs(SUMMON_MAP) do
        local effectId = core.magic.EFFECT_TYPE[effectName]
        if effectId then
            local effect = eff:getEffect(effectId)
            if effect and effect.magnitude and effect.magnitude > 0 then
                if not (cachedSettings.FACTION_EXEMPT_ENABLED
                        and isSummonExemptForPlayer(summonId)) then
                    return summonId
                end
            end
        end
...
local function periodicWitnessCheck()
    if not cachedSettings.MOD_ENABLED then return end
    if not cachedSettings.PERIODIC_CHECK_ENABLED then return end

    if not getActiveReportableSummon() then
        -- no summons up: reset so a future summon can trigger a new report
        reportedThisSession = false
        return
    end

    -- already caught for this summon session
    if reportedThisSession then return end

    -- skip if current cell is exempt
...
local function onNecroCheckWitness(data)
    if not cachedSettings.MOD_ENABLED then return end
    if reportedThisSession then return end

    -- if the triggering summon is exempt, only bail when no other non-exempt summon is also active
    if data.summonId
       and cachedSettings.FACTION_EXEMPT_ENABLED
       and isSummonExemptForPlayer(data.summonId) then
        if not getActiveReportableSummon() then return end
    end

    checkWitnesses()
end

time.runRepeatedly(periodicWitnessCheck,
    cachedSettings.WITNESS_INTERVAL * time.second,
    { initialDelay = math.random() * cachedSettings.WITNESS_INTERVAL * time.second })

return {
    eventHandlers = {
        NecroCheckWitness = onNecroCheckWitness,
    },
}