...
-- SA Crazy GC obsesion optimization
local perlinConfig = {
    noiseScale = 0,
    noiseAmplitude = 0
}
local maxPeekAmount = {left = 0, right = 0}
local bobOscillate = {x = 0, y = 0}

-- Caching another config for the step rotation perlin noise
local stepRotationPerlinConfig = {
    noiseScale = 0,
    noiseAmplitude = 0
}
...
-- Testin Step rotation featuer
local currentStepRotation = 0
local targetStepRotation = 0

--------------------------------------------------------------------------------------------------------------------------------------------------------------- Utility functions
----- SA crazy GC optimization gets overrriden by G7 common sense:
local cachedRotationMatrix = tes3matrix33:new()
local function createRollRotationMatrix(rotationValue)
    cachedRotationMatrix:toRotationY(rotationValue)
    return cachedRotationMatrix
end

local cachedPitchRotationMatrix = tes3matrix33:new()
local function createPitchRotationMatrix(rotationValue)
    cachedPitchRotationMatrix:toRotationX(rotationValue)
    return cachedPitchRotationMatrix
end
...
local function wrapPi(angle)
    ...
    if angle > math.pi then
        angle = angle - 2 * math.pi  -- Shift to [-π, π]
    end
    return angle
end
...
--- Headbob Cache
--- 
local r_matrix = tes3matrix33:new()
local upVector = tes3vector3:new()
local rightVector = tes3vector3:new()
local targetPosition = tes3vector3:new()
local originalPosition = tes3vector3:new()
...
local function headBob(e)
    ...
    if mobilePlayer.mouseLookDisabled then
        return
    end
    ...
    r_matrix = e.cameraTransform.rotation
    upVector = r_matrix:getUpVector()
    rightVector = r_matrix:getRightVector()

    --Get positions
    targetPosition = e.cameraTransform.translation:copy()
    originalPosition = e.cameraTransform.translation:copy()
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------- Camera position
    ...
    -- Reset scale and amplitude to 0
    stepRotationPerlinConfig = {
        noiseScale = 0,
        noiseAmplitude = 0
    }
    ...
    if not mobilePlayer.isJumping and not mobilePlayer.isFalling then
        if isMoving(mobilePlayer) then
            if mobilePlayer.isSneaking then
                ...
                if config.stepRotationEnabled then
                    stepRotationPerlinConfig = {
                        noiseScale = config.sneakStepRotationNoiseScale,
                        noiseAmplitude = config.sneakStepRotationNoiseAmplitude
                    }
                end
            elseif mobilePlayer.isWalking then
                ...
                if config.stepRotationEnabled then
                    stepRotationPerlinConfig = {
                        noiseScale = config.walkStepRotationNoiseScale,
                        noiseAmplitude = config.walkStepRotationNoiseAmplitude
                    }
                end
            elseif mobilePlayer.isRunning then
                ...
                if config.stepRotationEnabled then
                    stepRotationPerlinConfig = {
                        noiseScale = config.runStepRotationNoiseScale,
                        noiseAmplitude = config.runStepRotationNoiseAmplitude
                    }
                end
            elseif mobilePlayer.isFlying then
            ...
            elseif mobilePlayer.isSwimming then
            ...
            -- When standing still, for levitating and swimming, just use half the standard values to make a floaty effect
            if mobilePlayer.isFlying then
    ...
    if isPeeking or togglePOVisDown or mobilePlayer.isParalyzed then
        targetBobAmplitudeMultiplier.x = 0
        targetBobAmplitudeMultiplier.y = 0
    end
    ...
    bobOscillate.x = math.sin(bobPhase.x)
    bobOscillate.y = math.sin(bobPhase.y)
    ...
    if config.peekEnabled then
        ...
        maxPeekAmount.left  = config.peekLength
        maxPeekAmount.right = config.peekLength
        ...
        if mobilePlayer.isParalyzed == false then
    ...
    -- Perlin camera noise
    if config.noiseEnabled and mobilePlayer.isParalyzed == false then
        timeOffset = timeOffset + dt
        perlinConfig.noiseScale = config.noiseScale
        perlinConfig.noiseAmplitude = config.noiseAmplitude
    ...
    if config.sneakCameraSmoothingEnabled then
        ...
        if mobilePlayer.isSneaking then
            ...
            if config.sneakCameraSmoothingEnabled then
                mobilePlayer.cameraHeight = lerp(mobilePlayer.cameraHeight, this.defaultCameraHeight * (1 - ((config.sneakCameraHeight * heightMultiplier * 0.01)/(100))), 1 - math.exp(-dt * config.sneakCameraSmoothing))
            end
        else
            if config.sneakCameraSmoothingEnabled then
                mobilePlayer.cameraHeight = lerp(mobilePlayer.cameraHeight, this.defaultCameraHeight, 1 - math.exp(-dt * config.sneakCameraSmoothing))
            end
        end
    end
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------- Camera rotation
    ...
    if config.viewRollingEnabled then
        if mobilePlayer.isMovingRight then
            targetZrotation = -config.viewRollingMaxAngle
        elseif mobilePlayer.isMovingLeft then
            targetZrotation = config.viewRollingMaxAngle
        end
    end

    -- Rotate the camera in the direction of every step, if the feature is enabled.
    if config.stepRotationEnabled then
        if not mobilePlayer.isJumping and not mobilePlayer.isFalling and not mobilePlayer.isSwimming and not mobilePlayer.isFlying then
            if this.lastBobOscillateX ~= nil then
                -- Less than 0 means camera is going left, more than 0 means going right
                local direction = bobOscillate.x - this.lastBobOscillateX
                
                if direction < 0 then
                    targetStepRotation = 0.001 * currentBobAmplitudeMultiplier.x * config.stepRotationMultiplier
                elseif direction > 0 then
                    targetStepRotation = -0.001 * currentBobAmplitudeMultiplier.x * config.stepRotationMultiplier
                end
                
                -- Add some noise to the rotation
                if stepRotationPerlinConfig.noiseAmplitude > 0 then
                    local noise = perlin.sample(timeOffset, stepRotationPerlinConfig) 
                    noise = noise - (stepRotationPerlinConfig.noiseAmplitude * 0.5)
                    noise = noise * currentBobAmplitudeMultiplier.x

                    targetStepRotation = targetStepRotation + noise
                end

                -- Smooth it
                currentStepRotation = lerp(currentStepRotation, targetStepRotation, 1 - math.exp(-dt * config.stepRotationSmoothing))
                -- Make a rotation matrix
                local stepRotation = createRollRotationMatrix(math.rad(currentStepRotation))
                
                -- Apply rotation
                newRotation = newRotation * stepRotation
            end
        end
        -- Save down boboscillate so we can measure the direction of the oscilation and apply rotation accordingly
        this.lastBobOscillateX = bobOscillate.x
    end
    ...
    if config.jumpEnabled and mobilePlayer.isParalyzed == false then
        local previousCameraZ = this.previousZPosition or mobilePlayer.position.z
        local dz = mobilePlayer.position.z - previousCameraZ

        -- Calculate the jumping speed
        local jumpVelocity = dz / (dt*10000)

        -- Detect landing and set pitch target
        if this.fallingLastFrame and not mobilePlayer.isFalling and not mobilePlayer.isJumping then
        ...
        if mobilePlayer.isJumping or mobilePlayer.isFalling then
            -- On our way up
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------- Extra rotation exclusive to 1st person camera
    if mobilePlayer.is3rdPerson ~= true then
        ...
        elseif this.freeLookRotateZ ~= 0 and mobilePlayer.isParalyzed == false then
    ...
    if togglePOVisDown == false and mobilePlayer.isParalyzed == false then
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------- Arm follow and body inertia 
    ...
    if not animController.is3rdPerson and firstPersonNode ~= nil then
        ...
        if mobilePlayer.hasFreeAction then
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------- Handle audio
    ...
    if config.syncFootsteps then
        if animController.is3rdPerson == false then
             if not mobilePlayer.isJumping and not mobilePlayer.isFlying and not mobilePlayer.isSwimming and not mobilePlayer.isFalling then
                if isMoving(mobilePlayer) and not isPeeking then
                    if cosY == 0 or (cosY > 0 and cosineYprev < 0) then
                        if bobOscillate.y <= 0 then
                            ...
                            local playerVerticalPosition = mobilePlayer.position.z
                            ...
                            local sneakVolumeMultiplier = 1
                            if mobilePlayer.isSneaking then
                                sneakVolumeMultiplier = 0.8
                            end

                            if playWaterSound then
                                if bobOscillate.x <= 0 then
                                    tes3.playSound({sound = footStepSounds.water.left, reference = tes3.player, mixChannel = tes3.soundMix.footsteps, volume = 0.7 * sneakVolumeMultiplier})
                                else
                                    tes3.playSound({sound = footStepSounds.water.right, reference = tes3.player, mixChannel = tes3.soundMix.footsteps, volume = 0.7 * sneakVolumeMultiplier})
                                end
                            elseif bootWeightClass == nil then
                                if bobOscillate.x <= 0 then
                                    tes3.playSound({sound = footStepSounds.bare.left, reference = tes3.player, mixChannel = tes3.soundMix.footsteps, volume = 0.7 * sneakVolumeMultiplier})
                                else
                                    tes3.playSound({sound = footStepSounds.bare.right, reference = tes3.player, mixChannel = tes3.soundMix.footsteps, volume = 0.7 * sneakVolumeMultiplier})
                                end
                            else
                                if bobOscillate.x <= 0 then
                                    tes3.playSound({sound = weightClassFootStepMapping[bootWeightClass].left, reference = tes3.player, mixChannel = tes3.soundMix.footsteps, volume = 0.75 * sneakVolumeMultiplier})
                                else
                                    tes3.playSound({sound = weightClassFootStepMapping[bootWeightClass].right, reference = tes3.player, mixChannel = tes3.soundMix.footsteps, volume = 0.75 * sneakVolumeMultiplier})
                                end
                            end
                        end
                    end
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------- Set things to be checked for previous frame values and reset for the next frame
    this.previousZPosition = mobilePlayer.position.z
    this.fallingLastFrame = mobilePlayer.isFalling or mobilePlayer.isJumping
...
local function calcMoveSpeedCallback(e)
    if e.reference == tes3.player then
        local isPeeking = peek.left or peek.right
        if isPeeking and config.peekEnabled then
            e.speed = 0
        end
        playerSpeed = e.speed
    end
...
function common.updateSneakSettingsFromMenu()
    if config.modEnabled and config.sneakCameraSmoothingEnabled then
        tes3.findGMST(tes3.gmst.i1stPersonSneakDelta).value = 0
        tes3.messageBox(string.format("Sneak camera height GMST set to 0 for smooth sneak camera movement to work.\n\nThe value for it (%d) is saved down and will be reset when the mod or the smooth sneak camera feature is disabled.",this.savedDownSneakGMST))
    else
        tes3.findGMST(tes3.gmst.i1stPersonSneakDelta).value = this.savedDownSneakGMST
        tes3.messageBox(string.format("Resetting Sneak camera height GMST to saved down value of %d.",this.savedDownSneakGMST))
    end
