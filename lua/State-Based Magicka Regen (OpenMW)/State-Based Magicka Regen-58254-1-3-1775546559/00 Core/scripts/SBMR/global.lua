...
local refreshTime = 1
local lastCheckTime = refreshTime
...
local function onUpdate(dt)
    ...
    if lastCheckTime < refreshTime then return end
    lastCheckTime = 0
    ...
    for _, actor in ipairs(world.activeActors) do
        ...
        if actors[actor.id] and actors[actor.id].lastActiveTime then
            local passedTime = core.getGameTime() - actors[actor.id].lastActiveTime
...
return {
    ...
    eventHandlers = {
        [mDef.events.updateArguments] = updateArguments,
    }
}
