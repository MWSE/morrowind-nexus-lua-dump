...
local function init(clearAll)
    ...
    setStartAttributes()
    local baseAttrMods = mC.getBaseAttrMods()
...
local function updateGrowth()
    local baseStatsMods = mC.getBaseAttrMods()
    mSkills.updateSkills(state, baseStatsMods)
    local growthRateNum = mS.getAttributeGrowthRate()
    local attributesCappedValue = mS.attributesStorage:get("attributeUncapper")
    ...
    local function growAttribute(attrId)
        ...
        local value = growth
                ...
                + diff
                -- Add base modifiers
                + (baseStatsMods[attrId] or 0)
        value = math.max(5, math.min(value, cappedValue))

        mC.setAttr(state, attrId, value)
    end
