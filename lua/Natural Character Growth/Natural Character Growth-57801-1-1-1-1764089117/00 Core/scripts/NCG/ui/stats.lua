...
local function setAttributeTooltipProgress(state, attr, content)
...
local function setAttributeTooltipDetails(state, attr, content)
    local text
    local legend
    local baseMod = mC.getBaseAttrMods()[attr.id] or 0
    ...
    if mod ~= 0 then
        text = string.format("%s\n%s", text, L("tooltipAttributeMod", { mod = mod }))
    end
    ...
    for _, attr in ipairs(core.stats.Attribute.records) do
        API.modifyLine(attr.id, {
            ...
            tooltip = function()
                ...
                setAttributeTooltipProgress(state, attr, content)
                if mS.attributesStorage:get("showAttributeValueDetails") then
                    setAttributeTooltipDetails(state, attr, content)
                end
