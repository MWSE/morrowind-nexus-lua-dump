...
module.getBaseAttrMods = function()
    local baseAttrMods = {}
        ...
        if spell.affectsBaseValues then
            for _, effect in pairs(spell.effects) do
                if effect.affectedAttribute and effect.id == core.magic.EFFECT_TYPE.FortifyAttribute then
                    baseAttrMods[effect.affectedAttribute] = (baseAttrMods[effect.affectedAttribute] or 0) + effect.magnitudeThisFrame
                end
            end
        end
    end
    if next(baseAttrMods) then
        local baseAttrModsKey = mH.mapToString(baseAttrMods)
        if baseAttrModsKey ~= currBaseAttrModsKey then
            currBaseAttrModsKey = baseAttrModsKey
            log(string.format("Detected new base attributes modifiers: %s", baseAttrModsKey))
        end
    end
    return baseAttrMods
end
...
module.setAttr = function(state, attrId, value)
    local current = T.NPC.stats.attributes[attrId](self).base
    value = math.floor(value)
    if current == value then return end

    state.attrs.base[attrId] = value - (module.getBaseAttrMods()[attrId] or 0)
...
module.upgradeOldState = function(_, _)
