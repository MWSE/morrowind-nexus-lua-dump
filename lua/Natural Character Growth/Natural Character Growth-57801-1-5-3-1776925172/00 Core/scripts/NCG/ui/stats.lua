...
local function getHealthTooltipDescription(state)
    ...
    local baseHealthMod = mCore.getMaxHealthModifier(self)
    ...
    for _, attrRecord in ipairs(core.stats.Attribute.records) do
        API.modifyLine(attrRecord.id, {
            tooltip = function()
                ...
                local luckModPerDeath = mS.healthStorage:get("luckModifierPerDeath")
                ...
                if attrRecord.id == "luck" and mS.healthStorage:get("deathCounter") and luckModPerDeath * deathCount ~= 0 then
                    table.insert(content, BASE.padding(4))
                    table.insert(content, {
                        template = BASE.textNormal,
