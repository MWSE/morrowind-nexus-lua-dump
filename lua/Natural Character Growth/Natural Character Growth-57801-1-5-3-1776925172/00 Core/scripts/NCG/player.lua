...
local state = {
    ...
    health = {
        ...
        diff = 0,
        attributes = mH.initNewTable(0, mCfg.healthAttributeFactors),
    },
    messagesLog = {},
...
local function setHealth()
    ...
    if health.base - prevHealth ~= 0 then
        log(string.format("Player's base health changed from %d to %d (diff %d)", prevHealth, health.base, health.base - prevHealth))
    end
end

local function setHealthAttributes()
    local changed = false
...
local function init(clearAll)
    ...
    -- Old setting values can be restored by the engine after the script init
    mS.fixSettings()
    ...
    for attrId, value in pairs(state.attrs.start) do
        ...
        T.Actor.stats.attributes[attrId](self).base = state.attrs.start[attrId] + (baseAttrMods[attrId] or 0) + state.attrs.diffs[attrId]
    end
    if clearAll then
        state.health.diff = 0
    elseif state.health.diff ~= 0 then
        log(string.format("Preserving previous health external change of %d", state.health.diff))
    end
    state.health.attributes = mH.initNewTable(0, mCfg.healthAttributeFactors)
    setHealthAttributes()
    setHealth()
...
local function updateGrowth()
    ...
    local function growAttribute(attrId)
        ...
        mC.setAttr(attrId, base + (baseAttrMods[attrId] or 0) + diff)
    end
...
local function updateHealth(deltaTime)
    ...
    local recalculate = setHealthAttributes()
...
return {
    ...
    eventHandlers = {
        ...
        StatsWindow_Shown = function() requests[mDef.requestTypes.refreshStats] = true end,
