...
local function getGenericGuard(mobile, ref)
    -- Guards are identified by respawn flag, isGuard flag or alarm 100
    return ref.object.isRespawn and (mobile.object.isGuard or mobile.alarm == 100)
end
...
local function processCell()
    ...
    for ref in cell:iterateReferences(tes3.objectType.npc) do
        ...
        if mobile then
            ...
            -- Guards logic
            if getGenericGuard(mobile, ref) then
...
local function onCombatStarted(eCombat)
    ...
    -- Exclude guards
    if config.excludeGuards and getGenericGuard(actor, actor.reference) then return end
