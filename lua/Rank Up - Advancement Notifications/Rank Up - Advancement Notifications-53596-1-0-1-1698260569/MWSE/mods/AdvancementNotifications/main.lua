...
local playerHasEnoughRep = false
...
-- New variables for Reputation implementation. Seperate messages need seperate variables.
local clanAundaeRepShown = false
local clanBerneRepShown = false
local clanQuarraRepShown = false
local fightersGuildRepShown = false
local houseHlaaluRepShown = false
local imperialCultRepShown = false
local imperialLegionRepShown = false
local magesGuildRepShown = false
local moragTongRepShown = false
local houseRedoranRepShown = false
local houseTelvanniRepShown = false
local templeRepShown = false
local thievesGuildRepShown = false
local twinLampsRepShown = false
local eastEmpireRepShown = false




-- Vanilla functions
-- Since v1.0.0 Reputation Check
function reputationCheck(guildName,rep1,rep2,rep3,rep4,rep5,rep6,rep7,rep8,rep9)
    ...
    if guildName.playerRank == 0 then
        if (guildName.playerReputation >= rep1) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
            return playerHasEnoughRep
        end
    end

    -- Rang 1
    if guildName.playerRank == 1 then
        if (guildName.playerReputation >= rep2) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 2 then
        if (guildName.playerReputation >= rep3) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 3 then
        if (guildName.playerReputation >= rep4) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 4 then
        if (guildName.playerReputation >= rep5) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 5 then
        if (guildName.playerReputation >= rep6) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 6 then
        if (guildName.playerReputation >= rep7) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 7 then
        if (guildName.playerReputation >= rep8) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
    ...
    if guildName.playerRank == 8 then
        if (guildName.playerReputation >= rep9) then
            playerHasEnoughRep = true
            return
        else
            playerHasEnoughRep = false
...
function menuEnterCallback(e)
    ...
    if HFRFullInstalled == false and HFRMorrowindOnlyInstalled == false then
        ...
        if magesGuild.playerJoined == true and magesGuildShown == false and magesGuildRepShown == false then

            advancementCheckGuilds(magesGuild, 17, 12, 11, 10, 13, 15, 2, 3)
            -- Hier werden in den Parametern die verschiedenen Reputation requirements pro Rank eingetragen.
            -- So müssen nicht mehrere Funktionen für verschiedene Rep anforderungen geschrieben werden.
            reputationCheck(magesGuild,5,10,20,30,45,60,80,100,125)

            -- Der Rep Check und der Skill&Attribute Check muss erfolgreich sein. Und die Nachricht darf noch nicht angezeigt worden sein.
            if playerHasEnoughRep == true and playerIsQualified == true and magesGuildRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Mages Guild.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                magesGuildRepShown = true
                return magesGuildRepShown
            end

            -- magesGuildRepShown muss false sein, damit falls die Rep Bedingungen auch erfüllt sind, nur die Rep Message angezeigt wird und nicht beide.
            if playerIsQualified == true and magesGuildShown == false and magesGuildRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Mages Guild.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
        ...
        if fightersGuild.playerJoined == true and fightersGuildShown == false and fightersGuildRepShown == false then
            ...
            reputationCheck(fightersGuild,5,10,20,35,45,60,90,110,125)

            if playerHasEnoughRep == true and playerIsQualified == true and fightersGuildRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Fighters Guild.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                fightersGuildRepShown = true
                return fightersGuildRepShown
            end

            if playerIsQualified == true and fightersGuildShown == false and fightersGuildRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Fighters Guild.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
        ...
        if houseHlaalu.playerJoined == true and houseHlaaluShown == false and houseHlaaluRepShown == false then
            ...
            reputationCheck(houseHlaalu,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and houseHlaaluRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Great House Hlaalu.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                houseHlaaluRepShown = true
                return houseHlaaluRepShown
            end

            if playerIsQualified == true and houseHlaaluShown == false and houseHlaaluRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Great House Hlaalu.",
                    buttons = {"OK"}, 
                    callback = messageButtonPressed,
                    showInDialog = true, 
                    duration = 15
        ...
        if thievesGuild.playerJoined == true and thievesGuildShown == false and thievesGuildRepShown == false then

            advancementCheckGuilds(thievesGuild, 21, 22, 24, 19, 23, 20, 4, 7)
            reputationCheck(thievesGuild,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and thievesGuildRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Thieves Guild.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                thievesGuildRepShown = true
                return thievesGuildRepShown
            end

            if playerIsQualified == true and thievesGuildShown == false and thievesGuildRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Thieves Guild.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
        ...
        if moragTong.playerJoined == true and moragTongShown == false and moragTongRepShown == false then

            advancementCheckGuilds(moragTong, 21, 13, 24, 22, 23, 20, 5, 4)
            reputationCheck(moragTong,10,20,30,40,50,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and moragTongRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Morag Tong.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                moragTongRepShown = true
                return moragTongRepShown
            end

            if playerIsQualified == true and moragTongShown == false and moragTongRepShown == false then
                tes3.messageBox({ 
        ...
        if houseRedoran.playerJoined == true and houseRedoranShown == false and houseRedoranRepShown == false then
            
            advancementCheckGuilds(houseRedoran, 2, 9, 4, 6, 3, 8, 6, 1)
            reputationCheck(houseRedoran,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and houseRedoranRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Great House Redoran.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                houseRedoranRepShown = true
                return houseRedoranRepShown
            end

            if playerIsQualified == true and houseRedoranShown == false and houseRedoranRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Great House Redoran.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
        ...
        if houseTelvanni.playerJoined == true and houseTelvanniShown == false and houseTelvanniRepShown == false then

            advancementCheckGuilds(houseTelvanni, 12, 14, 11, 10, 13, 15, 2, 3)
            reputationCheck(houseTelvanni,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and houseTelvanniRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Great House Telvanni.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                houseTelvanniRepShown = true
                return houseTelvanniRepShown
            end

            if playerIsQualified == true and houseTelvanniShown == false and houseTelvanniRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Great House Telvanni.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
        ...
        if temple.playerJoined == true and templeShown == false and templeRepShown == false then

            advancementCheckGuilds(temple, 17, 5, 14, 15, 16, 18, 2, 7)
            reputationCheck(houseTelvanni,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and templeRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Tribunal Temple.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                ...
                templeRepShown = true
                return templeRepShown
            end

            if playerIsQualified == true and templeShown == false and templeRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Tribunal Temple.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
        ...
        if imperialLegion.playerJoined == true and imperialLegionShown == false and imperialLegionRepShown == false then

            advancementCheckGuilds(imperialLegion, 9, 1, 5, 4, 6, 8, 6, 7)
            reputationCheck(imperialLegion,5,10,20,30,45,60,70,80,100)

            if playerHasEnoughRep == true and playerIsQualified == true and imperialLegionRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Imperial Legion.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                imperialLegionRepShown = true
                return imperialLegionRepShown
            end

            if playerIsQualified == true and imperialLegionShown == false and imperialLegionRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Imperial Legion.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
        ...
        if imperialCult.playerJoined == true and imperialCultShown == false and imperialCultRepShown == false then

            advancementCheckImperialCult(imperialCult, 5, 14, 15, 16, 10, 26, 18, 7, 3)
            reputationCheck(imperialCult,5,10,20,30,40,50,60,80,100)

            if playerHasEnoughRep == true and playerIsQualified == true and imperialCultRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Imperial Cult.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                imperialCultRepShown = true
                return imperialCultRepShown
            end

            if playerIsQualified == true and imperialCultShown == false and imperialCultRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the Imperial Cult.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
        ...
        if clanAundae.playerJoined == true and clanAundaeShown == false and clanAundaeRepShown == false then
            ...
            reputationCheck(clanAundae,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and clanAundaeRepShown == false then
                tes3.messageBox({ 
                    message = "You are now ready to advance in the Aundae Vampire Clan.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                clanAundaeRepShown = true
                return clanAundaeRepShown
            end

            if playerIsQualified == true and clanAundaeShown == false and clanAundaeRepShown == false then
                tes3.messageBox({ 
        ...
        if clanBerne.playerJoined == true and clanBerneShown == false and clanBerneRepShown == false then

            advancementCheckVampireClans(clanBerne, 21, 9, 24, 23, 20, 18, 3, 5)
            reputationCheck(clanBerne,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and clanBerneRepShown == false then
                tes3.messageBox({ 
                    message = "You are now ready to advance in the Berne Vampire Clan.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                clanBerneRepShown = true
                return clanBerneRepShown
            end

            if playerIsQualified == true and clanBerneShown == false and clanBerneRepShown == false then
                tes3.messageBox({ 
        ...
        if clanQuarra.playerJoined == true and clanQuarraShown == false and clanQuarraRepShown == false then

            advancementCheckVampireClans(clanQuarra, 21, 9, 11, 27, 6, 18, 1, 6)
            reputationCheck(clanQuarra,5,10,20,30,45,60,80,100,125)

            if playerHasEnoughRep == true and playerIsQualified == true and clanQuarraRepShown == false then
                tes3.messageBox({ 
                    message = "You are now ready to advance in the Quarra Vampire Clan.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                clanQuarraRepShown = true
                return clanQuarraRepShown
            end

            if playerIsQualified == true and clanQuarraShown == false and clanQuarraRepShown == false then
                tes3.messageBox({ 
        ...
        if eastEmpire.playerJoined == true and eastEmpireShown == false and eastEmpireRepShown == false then

            advancementCheckEastEmpireCompany(eastEmpire, 26, 25, 19, 6, 3, 7, 3)
            reputationCheck(eastEmpire,10,20,30,40,50,60,70,100,5000) -- letzte Zahl ist random, weil East Empire nur 9 Ränge hat.

            if playerHasEnoughRep == true and playerIsQualified == true and eastEmpireRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the East Empire Company.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                eastEmpireRepShown = true
                return eastEmpireRepShown
            end

            if playerIsQualified == true and eastEmpireShown == false and eastEmpireRepShown == false then
                tes3.messageBox({ 
                    message = "You meet the skill and attribute requirements to advance in the East Empire Company.",
                    buttons = {"OK"}, 
                    showInDialog = true, 
                    duration = 15 
                })
    ...
    if OAABtwinLampsInstalled == true then
        if twinLamps.playerJoined == true and twinLampsShown == false and twinLampsRepShown == false then

            advancementCheckTwinLamps(twinLamps, 26, 20, 19, 18, 9, 5, 2)
            reputationCheck(twinLamps,10,20,25,30,40,45,50,4000,4000) -- letzten beide Werte random, weil es nur 8 Ränge gibt

            if playerHasEnoughRep == true and playerIsQualified == true and twinLampsRepShown == false then
                tes3.messageBox({ 
                    message = "[Congratulations!] You are now ready to advance in the Twin Lamps.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
                })
                twinLampsRepShown = true
                return twinLampsRepShown
            end

            if playerIsQualified == true and twinLampsShown == false and twinLampsRepShown == false then
                tes3.messageBox({ 
    ...
    if HFRFullInstalled == true then
        ...
        if moragTong.playerJoined == true and moragTongShown == false and moragTongRepShown == false then
            ...
            if playerHasEnoughRep == true and playerIsQualified == true and moragTongRepShown == false then
                tes3.messageBox({ 
                    message = "You are now ready to advance in the Morag Tong.",
                    buttons = {"OK"},
                    callback = messageButtonPressed, 
                    showInDialog = true, 
                    duration = 15 
...
local function loadCallback(e)
    ...
    clanAundaeRepShown = false
    clanBerneRepShown = false
    clanQuarraRepShown = false
    fightersGuildRepShown = false
    houseHlaaluRepShown = false
    imperialCultRepShown = false
    imperialLegionRepShown = false
    magesGuildRepShown = false
    moragTongRepShown = false
    houseRedoranRepShown = false
    houseTelvanniRepShown = false
    templeRepShown = false
    thievesGuildRepShown = false
    twinLampsRepShown = false
    eastEmpireRepShown = false
end
