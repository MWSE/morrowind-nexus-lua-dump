...
local lastNPC, sLastNPCname, sLastNPCraceName;
...
local function OnInfoGetText(eventData)
	-- mwse.log('OnInfoGetText %s', type(eventData.info.text));

	-- if eventData.info.text then
		-- mwse.log('OnInfoGetText');
	-- end
end
...
local function OnCellChanged(e)
	...
	-- tes3.messageBox('OnCellChanged %s' , e.cell);
...
local function OnInfoResponse(e)
	...
	if not tKnownIDs[NPC.id] and e.dialogue.id == 'Background' then
			-- tes3.messageBox('disposition %d', e.info.disposition);
		if e.reference.object.disposition >= e.info.disposition then
			tKnownIDs[NPC.id] = true;
			dialogMenuTitle.text = NPC.name;
			-- tes3.messageBox('OninfoResponse text %s', e.info.text);
		end
	end
end
...
local function OnEquip(eventData)
	...
	if eventData.item.objectType == iAlchType then
		...
		-- tes3.messageBox('OnEquip %s',eventData.item);
	end
end
...
local function OnUIobjectTooltip(eventData)
	...
	-- tes3.messageBox('OnUIobjectTooltip %s', eventData.tooltip);
	bFoundName = false;
	...
	if not bBartering then
		...
		if obj.objectType == iMiscType and IsKey(obj) and not tKnownIDs[obj.id]then
...
--]]
	if ref.object.objectType == iNPCtype then 
		...
		sLastNPCraceName = ref.object.race.name;
			...
			-- tes3.messageBox('health %s', ref.mobile.health.current);
		if not tKnownIDs[ref.baseObject.id] then 

			-- tes3.messageBox('lastNPCname %s', lastNPCname);
...
local function BarterMenuActivated(eventData) 
	-- tes3.messageBox('OnuiActivated %s', eventData.element);
...
local function OnUIpreEvent(e) 
		-- tes3.messageBox('uiPreEvent %s', e.block);
...
local function OnActivationTargetChanged(eventData)
	...
	if ref then
			...
			-- tes3.messageBox('Is Key %s', IsKey(ref.object));

		-- if ref.destination and ref.destination.cell then
			-- tes3.messageBox('leads to %s', ref.destination.cell);
		-- if  ref.itemData and ref.itemData.owner then
			-- tes3.messageBox('owner %s', ref.itemData.owner  );
		-- end
	end
end
...
local function Test()
	...
	-- tes3.messageBox('sGameLang %s', loc.book[sLang]);

	if ref then
		-- tes3.messageBox('sourceMod %s', ref.object.sourceMod );
		-- local d = string.match(ref.object.id, '%D')

		-- tes3.messageBox('tKnownIDs[obj.id] %s', tKnownIDs[ref.object.id]);
		-- if ref.itemData and ref.itemData.owner then
			-- tes3.messageBox('remove owner %s', ref.itemData.owner);
			-- ref.itemData.owner = nil;
		-- end
	end
end
...
-- local function OnUIevent(eventData)
	-- if tes3.menuMode() then
		-- if eventData.parent.name == 'MenuInventory' then
			-- bTraderInv = false;
		-- elseif eventData.parent.name == 'MenuBarter' then
			-- bTraderInv = true;
		-- end
	-- end
-- end
