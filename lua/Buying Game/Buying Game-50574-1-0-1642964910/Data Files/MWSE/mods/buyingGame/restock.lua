...
local function restockItems(params)
	...
	-- Removing every new item if config option is enabled and merchant isn't a Pawnbroker
	if common.config.removeBought then
		if not (mercant.baseObject.class and mercant.baseObject.class.id == "Pawnbroker") then
			for _, stack in pairs(inventory) do -- loop over cloned inventory
				local count = math.abs(stack.count)
				count = count + math.floor(count * investment)
				local base = math.abs(getItemCount( reference.object.baseObject.inventory, stack.object.id))
				--mwse.log("%s, base: %s count: %s", stack.object.id, base, count)
				if base < count then
					tes3.removeItem({ reference = reference, item = stack.object.id, count = (count-base)})
				end
			end
		end
	end
	...
	for _, stack in pairs(reference.object.baseObject.inventory) do -- loop over base inventory
		...
		if current < count and ( stack.count < 0 or reference.object.baseObject.respawns ) then
			...
			-- Removing only previously resolved leveled items if the merchant is Pawnbroker or the config option is disabled
		
			if not common.config.removeBought or ( mercant.baseObject.class and mercant.baseObject.class.id == "Pawnbroker") then
			...
			if not reference.object.capacity or inventory:calculateWeight() < reference.object.capacity then
				mwscript.addItem{reference = reference, item = stack.object.id, count = (count-current)}
			end
		end
	end
...
restock.check = function(trader)
	...
	timestamp = trader.data.buyingGame and trader.data.buyingGame.timestamp
