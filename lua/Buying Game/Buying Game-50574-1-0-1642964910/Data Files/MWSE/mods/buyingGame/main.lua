...
local function onBarterBegin(e)
	...
	if tes3.mobilePlayer.mercantile.current >= common.config.canBarterEquipped then
		...
		for _, stack in pairs(mobileActor.object.equipment) do
			--[[tes3.addItem{reference = npcRef, item = stack.object, count = 2}
			mwse.log("Adding %s", stack.object.name)]]
			...
			timer.start{
				...
				callback = function()
					...
					for i, item in ipairs(equipment) do
						mobileActor:equip{item = item}						
					end
					mobileActor.reference:updateEquipment()
				end
			}
		end
	end
...
local function onBarterOffer(e)
	...
	if e.offer > e.value then
		local trueValue = common.getArrayValue(e.selling) - common.getArrayValue(e.buying)
...
local function onBarterSuccess(e)
	...
	actor.reference.data.buyingGame = actor.reference.data.buyingGame or {}
	local timestamp = actor.reference.data.buyingGame.timestamp
	if not timestamp then
		 actor.reference.data.buyingGame.timestamp = tes3.getSimulationTimestamp()
	end
end
