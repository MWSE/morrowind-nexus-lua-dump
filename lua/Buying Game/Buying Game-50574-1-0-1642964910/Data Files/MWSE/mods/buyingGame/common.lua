...
common.isExport = function(item)
	...
	if common.config[cell] and common.config[cell].export and common.config[cell].export[item] then
		return true
	else
...
common.isImport = function(item)
	local cell = string.sub(tes3.player.cell.id, string.find(tes3.player.cell.id, "^[%w -]+"))
	if common.config[cell] and common.config[cell].import and common.config[cell].import[item] then
		return true
	else
		local region = tes3.getCell{id = cell}.region or tes3.getRegion{useDoors = true}
...
common.deltaTrade = function(mobile) 
	local rang = mobile.object.faction and mobile.object.faction.playerRank + 1 or 0		
	local playerTrade = tes3.mobilePlayer.mercantile.current + tes3.mobilePlayer.personality.current/5 + tes3.mobilePlayer.luck.current/10 + rang*10
	local traderTrade = mobile:getSkillValue(tes3.skill.mercantile) + mobile.personality.current/5 + mobile.luck.current/10 + 100 - math.min(mobile.object.disposition or 50, 100)
	return (traderTrade - playerTrade)/100
end

common.getArrayValue = function(barterArray)
	local value = 0
	for i, tile in ipairs(barterArray) do
		value = value + tile.item.value*tile.count
	end
	return value
end

common.hasForbiddenItems = function(barterArray)
	for i, tile in ipairs(barterArray) do
		if common.config.forbidden[tile.item.id] then
			return tile.item
		end
	end
	return false
