...
local function updateBarterCategories(ai)
	local aiPrev = {}
...
local function updateDialogueMenu(actor)
	local menu = tes3ui.findMenu("MenuDialog")
	if not menu then return end
	timer.frame.delayOneFrame(function()
		local barter = menu:findChild(tes3ui.registerID("MenuDialog_service_barter"))
		if tes3.mobilePlayer.mercantile.current >= common.config.canTradeWithEveryone then
...
local function onMenuDialog(e)
	dialogueActor = tes3ui.getServiceActor().reference
	local barter = e.element:findChild(tes3ui.registerID("MenuDialog_service_barter"))
	dialogueMenu = e.element
	timer.frame.delayOneFrame(function()
		local ai = dialogueActor.object.aiConfig
		if barter.visible then
			restock.check(dialogueActor)
			if tes3.mobilePlayer.mercantile.current >= common.config.canTradeWithEveryone then
				timer.frame.delayOneFrame(function()
					updateBarterCategories(ai)
				end)
			...
			if tes3.mobilePlayer.mercantile.current < common.config.canInvest then return end 
			if dialogueActor.data.buyingGame and dialogueActor.data.buyingGame.investment then return end
		...
		elseif tes3.mobilePlayer.mercantile.current >= common.config.canTradeWithEveryone then
			if dialogueActor.baseObject.objectType == tes3.objectType.npc then
				barter.visible = true
				timer.frame.delayOneFrame(function()
					updateBarterCategories(ai)
				end)
...
local function onDialogue(e)
	updateDialogueMenu(dialogueActor)
end
...
local function onInitialized(e)
	if common.config.modEnabled then
		...
		event.register("infoGetText", onDialogue)
	else
