local defaultConfig = {
	...
	["Molagreahd Region"] = {
		export = {
			-- ebony from Ranyon-ruhn: check TR_m1_RR_MQ
			["ingred_hackle-lo_leaf_01"] = true,
			ingred_wickwheat_01 = true,
			ingred_marshmerrow_01 = true,
...
local function isSmuggler(actor)
	return actor.class.id == "Smuggler" or actor.class.id == "Necromancer" or (actor.faction and (actor.faction.id == "Thieves Guild" or actor.faction.id == "Camonna Tong"))
end
...
addCategoryToRegionExport("kwama", {"Molag Amur Region", "West Gash Region", "Ashlands Region", "Sacred Lands Region", "Boethiah's Spine Region", "Mephalan Vales Region", "Aanthirin Region"})
...
addCategoryToRegionExport("dwemer", {"Molag Amur Region", "Boethiah's Spine Region", "Sheogorad Region"})
addCategoryToRegionExport("sea", {"Bitter Coast Region", "Azura's Coast Region", "Sheogorad Region", "Padomaic Ocean Region", "Sea of Ghosts Region", "Ascadian Bluffs Region", "Telvanni Isles Region"})
addCategoryToRegionExport("undead", {"Sacred Lands Region", "Aranyon Pass Region"})
