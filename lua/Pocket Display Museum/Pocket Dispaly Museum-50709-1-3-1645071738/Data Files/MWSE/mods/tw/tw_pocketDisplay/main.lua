...
--[[
local function setDoOnce3(ref,Var)
    local refData = ref.data -- Crossing the C-Lua border result is usually not a bad idea.
    refData.tw_pocketDisplay = refData.tw_pocketDisplay or {} -- Force initializing the parent table.
    refData.tw_pocketDisplay.doOnce3 = Var -- Actually set your value.
end

local function getDoOnce3(ref)
    local refData = ref.data
    return refData.tw_pocketDisplay and refData.tw_pocketDisplay.doOnce3
end

local function setDoOnce4(ref,Var)
    local refData = ref.data -- Crossing the C-Lua border result is usually not a bad idea.
    refData.tw_pocketDisplay = refData.tw_pocketDisplay or {} -- Force initializing the parent table.
    refData.tw_pocketDisplay.doOnce4 = Var -- Actually set your value.
end

local function getDoOnce4(ref)
    local refData = ref.data
    return refData.tw_pocketDisplay and refData.tw_pocketDisplay.doOnce4
end
--]]
----------------------------------------------------------------------------------------------------
...
--[[        
        for i, item in pairs(MiscDisplays) do 
            if (eitem.id:lower() == item.id ) then
               -- it's a valid misc display item
--]]
...
--mwse.log("***********  item = %s *** count = %x ", item.id, item.count )              
...
local function spellCastMuseum(e)
 ...
 --if (e.CastSpell == "tw_museum_teleport") then  --  filter already applied
