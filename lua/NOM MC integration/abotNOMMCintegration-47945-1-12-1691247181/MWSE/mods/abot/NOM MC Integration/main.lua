...
local tes3_objectType_alchemy = tes3.objectType.alchemy
local tes3_objectType_ingredient = tes3.objectType.ingredient
local tes3_objectType_miscItem = tes3.objectType.miscItem
local tes3_effect_restoreFatigue = tes3.effect.restoreFatigue
local tes3_skill_armorer = tes3.skill.armorer
...
local function addAnvilBuff()
	tes3.modStatistic({reference = player, skill = tes3_skill_armorer, current = 15})
	ab01anvilBuff = true
...
local function equip(e)
	...
	if objType == tes3_objectType_miscItem then -- NOM water bottles
		---mwse.log('%s tes3_objectType_miscItem equipped', modPrefix)
	...
	if objType == tes3_objectType_ingredient then
		...
		if not (effId == tes3_effect_restoreFatigue) then
			return
		end
		eat(20, obj)
		return
	...
	if not (objType == tes3_objectType_alchemy) then -- potions & beverages
		return
	end
	...
	if not (eff.id == tes3_effect_restoreFatigue) then
		return
	end
	drink(20, obj)
end
...
local function checkActivate()
	...
	local results, result, ref, v
	local eyePos = tes3.getPlayerEyePosition()
	local params = {position = eyePos,
		direction = tes3.getPlayerEyeVector(), maxDistance = maxDist,
		findAll = true,	ignore = {player, tes3.player1stPerson},
		root = tes3.game.worldPickRoot}

	local function rayTest()
		results = tes3.rayTest(params)
		if results then
			for i = 1, #results do
				v = results[i]
				if v then -- better safe than sorry
					if v.reference then
						result = v
						return
					end
	...
	rayTest()
	if not result then
		params.direction = tes3.getPlayerEyeVector()
		params.root = tes3.game.worldObjectRoot
		rayTest()
	end
	...
	local isStatic = (objType == tes3_objectType_static)
	local isActi = (objType == tes3_objectType_activator)
	local isLight = (objType == tes3_objectType_light)
	...
	or isLight then
		if NoM_roastScript then
			if multifind2(lcMesh, lcId, {'firepit', 'light_logpile', 'coals_hot', 'ashfall_campfire'}) then
				if not mwscript.scriptRunning({script = NoM_roastScriptId}) then
	...
	if NOM_well_waterScript then
		if not mwscript.scriptRunning({script = NOM_well_waterScriptId}) then
			local ok3 = false
			if isStatic then
			...
			if not ok3 then
				if multifind2(lcMesh, lcId, {'well','_water','water_','_pool','pool_','waterfall','stream',
					'wo_basin','basin_dg','barrel_dg','bathhalfbarrel','waterpump'}) then
					if not multifind2(lcMesh, lcId, {'barrier', 'lava'}) then
						if isStatic then
							ok3 = true
						elseif isActi then
...
local tes3_keybind_activate = tes3.keybind.activate
...
local function activateKeyDown()
	if not activated then
		...
		if inputController:keybindTest(tes3_keybind_activate) then
			activated = true
		end
	end
end
...
local function activateKeyUp()
	if activated then
		timer.frame.delayOneFrame(checkActivate)
	end
end
...
local function addIngredientTo(creaObj)
	...
	if not inventory then
		mwse.log("%s tes3_objectType_creature %s inventory = %s", modPrefix, id, inventory)
		return
	end
	local s, notWerewolf, notFalseRacer, ingredient, chanceNone, ok
	for crKey, ingredients in pairs(crList) do
		---mwse.log("%s crKey %s ingredients %s", modPrefix, crKey, ingredients)
		s = false
		ok = true
		if crKey == 'guar' then
			if string.find(id, 'guard', 1, true)
			or string.find(name, 'guard', 1, true) then
				ok = false
			end
		end
		if ok then
			if string.find(id, crKey, 1, true) then
				s = id
			elseif string.find(name, crKey, 1, true) then
				s = name
			end
		end
		if s then
			---mwse.log("%s tes3_objectType_creature %s ingredients %s", modPrefix, s, ingredients)
			notWerewolf = not (
				string.find(s, 'wolf', 1, true)
			and string.find(s, 'were', 1, true)
			)
			notFalseRacer = true
			if string.find(s, 'racer', 1, true) then
				notFalseRacer = string.find(s, 'cliff', 1, true)
			end
			----mwse.log("%s notWerewolf %s notFalseRacer %s", modPrefix, notWerewolf, notFalseRacer)
			if notWerewolf
			and notFalseRacer then
				ingredient = 'none'
				chanceNone = 1000
				local v
				...
				---mwse.log("%s tes3_objectType_creature %s ingredient %s chanceNone %s", modPrefix, s, ingredient, chanceNone)
				if ( math.random(100) > chanceNone ) then
					if ( tes3.getObject(ingredient) ) then
						if not inventory:contains(ingredient) then
							...
							mwscript.addItem({ reference = id, item = ingredient, count = qty })
...
local function death(e)
	...
	if not (obj.objectType == tes3_objectType_creature) then
		return
	end
	local id = obj.id
	---assert(id)
...
--[[
	if ref.object.objectType == iBookType and ref:testActionFlag(tes3.actionFlag.useEnabled) then
		if not tes3.hasOwnershipAccess({ target = ref }) then
			---tes3.messageBox('Stolen book %s', ref.object)
...
local skipFire
local function ab01nmciPT1()
	skipFire = nil -- reset it
end
...
local function collision(e)
	...
	if objType == tes3_objectType_static then
		if not multifind2(lcMesh, lcId, {'firepit_f', 'coals_hot', 'terrain_lava'}) then
			return
		end
	elseif objType == tes3_objectType_light then
...
Returns: newReference (tes3reference)
	...
	skipFire = true
	-- fire spell is 1 sec, 1.2 <= spawn delay <= 1.5
	-- persistent timer ensure skipFire is reset
	timer.start({duration = 0.3*math.random() + 1.2, callback = 'ab01nmciPT1'})
	tes3.createReference({object = 'ab01NOMFireCastA', position = targetRef.position,
		cell = targetRef.cell }) -- PlaceAtMe ab01NOMFireCastA 1 0 0
end
...
local function save(e)
	...
	if NOM_well_waterScript then
		if mwscript.scriptRunning({script = NOM_well_waterScriptId}) then
			skipSaving = true
			mwscript.stopScript({script = NOM_well_waterScriptId})
		end
	end
	if NoM_roastScript then
		if mwscript.scriptRunning({script = NoM_roastScriptId}) then
			skipSaving = true
	...
	player.data.ab01anvilBuff = ab01anvilBuff
	if skipSaving then
...
local function testIngredients()
	for creaObj in tes3.iterateObjects(tes3_objectType_creature) do
		...
		if NOM_thirst_v then
			...
			if NOM_fire_hurt then
				if tes3.getObject('ab01NOMFireCastA') then
					timer.register('ab01nmciPT1', ab01nmciPT1)
