...
local function eat(value, obj)
	local v = tes3.getGlobal('hungry') -- Morrowind Crafting float variable
	if v then
		if not (v == 0) then
			if v > 0 then
				v = v - 1
...
local function drink(value, obj)
	local v = tes3.getGlobal('NOM_Thirst_v')
	if v > -30 then
		v = v - value
...
local function rayTest(tes3GameRoot)
	return tes3.rayTest({position = tes3.getPlayerEyePosition(), direction = tes3.getPlayerEyeVector(),
		maxDistance = 2 * iMaxActivateDist, findAll = true,	ignore = {player}, root = tes3GameRoot})
end
...
tes3.game.worldPickRoot -- activators??
	...
	local results = rayTest(tes3.game.worldPickRoot)
	local results2 = rayTest(tes3.game.worldObjectRoot)
				...
				if string.multifind(lcMesh, {'well', '_water', 'water_'}, 1, true) then
					...
					elseif isActi then
						...
						if not script then
							...
							if not ok3 then
								if tes3.getGlobal('ashfall_sleep_illegal') then
