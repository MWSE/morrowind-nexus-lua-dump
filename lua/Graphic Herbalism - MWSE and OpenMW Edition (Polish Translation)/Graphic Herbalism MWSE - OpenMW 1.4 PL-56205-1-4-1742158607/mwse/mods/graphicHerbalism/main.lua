...
if (mwse.buildDate == nil) or (mwse.buildDate < 20190501) then
    event.register("initialized", function()
        tes3.messageBox(
            "[Graphic Herbalism] MWSE jest niezaktualizowany!"
            .. " Musisz zaktualizowaж do nowszej wersji aby uїywaж tego moda."
        )
    end)
    return
end
...
if (quickloot.version == nil) or (quickloot.version < 1.3) then
    ...
    if mode == "file" then
        event.register("initialized", function()
            tes3.messageBox(
                "[Graphic Herbalism] QuickLoot jest niezaktualizowany!"
                .. " Wersja 1.3 lub wyїsza jest wymagana do kompatybilnoњci z tym modem."
            )
        end)
    end
end
...
local function onTooltipDrawn(e)
    ...
    for ingred in getIngredients(ref.object.inventory) do
        ...
        local label = parent:createLabel{id=GUI_ID.weight, text=string.format("Ciкїar: %.2f", ingred.weight)}
        label.wrapText = true

        local label = parent:createLabel{id=GUI_ID.value, text=string.format("Wartoњж: %d", ingred.value)}
        label.wrapText = true
...
local function updateBlacklist()
    for obj in tes3.iterateObjects(tes3.objectType.container) do
        ...
        then
            ...
            then
                mwse.log('[Graphic Herbalism] Pojemnik "%s" zostaі dodany do Czarnej Listy.', id)
