...
function updateTimeDisplay(force)
	...
	if force or currentMinute >= nextUpdate then
...
function onLoad(data)
	...
	settingsSection:set("HUD_Y_POS", math.floor(math.max(0, math.min(HUD_Y_POS, hudLayerSize.x - FONT_SIZE))))
...
-- print('UiModeChanged from', data.oldMode , 'to', data.newMode, '('..tostring(data.arg)..')')
	...
	elseif HUD_DISPLAY == "Interface Only" then
		timeHud.layout.props.visible = data.newMode == "Interface" and I.UI.isHudVisible()
		timeHud:update()
	elseif HUD_DISPLAY == "Hide on Interface" then
		timeHud.layout.props.visible = data.newMode == nil and I.UI.isHudVisible()
		timeHud:update()
	elseif HUD_DISPLAY == "Hide on Dialogue Only" then
		timeHud.layout.props.visible = data.newMode ~= "Dialogue" and I.UI.isHudVisible()
		timeHud:update()
	end
