...
settingsTemplate[tempKey] = {
	...
	page = MODNAME,
	l10n = "none",
	name = L("settings.timedate", "Time & Date").."                                          ", -- lol
	permanentStorage = true,
	order = getOrder(),
	settings = {
		{
...
--			description = L("settings.border.desc", "Draw a border along the edges of the background."),
			renderer = "checkbox",
			default = true,
		},
		{
...
--[[
tempKey = "Order" --- idk if key can have space
settingsTemplate[tempKey] = {
	key = 'Settings'..MODNAME..tempKey,
	page = MODNAME,
	l10n = "none",
	name = L("settings.order", "Order").."											 ", -- lol
	description = "Change the location of time, date and others.",
	permanentStorage = true,
	order = getOrder(),
	settings = {
			{
			key = "HUD_ORDER_1",
			name = L("settings.order.one", "1"),
			description = "",
			...
			argument = {
				disabled = false,
				l10n = "none", 
				items = { "In-game time", "Date", "Season", "Temperature", "Real Time", "None" },
			},
		},
		{
			key = "HUD_ORDER_2",
			name = L("settings.order.two", "2"),
			description = "",
			default = "Date", 
			renderer = "select",
			argument = {
...
local hasSunsDusk = types.Book.records['sd_book_1_needs']
...
settingsTemplate[tempKey] = {
	...
	settings = {
			...
			renderer = "select",
			argument = {
				disabled = not hasSunsDusk,
				l10n = "none", 
				items = { "Hidden", "External Temp", "Ambient Temp", "Player Temp", "Current > Target" },
			},
		},
		{
			key = "SD_TEMP_STATE",
...
-- settings migration
local legacySection = storage.playerSection('Settings'..MODNAME..'Time and Date')
if legacySection:get("FONT_SIZE") then
	for id, template in pairs(settingsTemplate) do
		...
		for i, entry in pairs(template.settings) do
			settingsSection:set(entry.key, settingsSection:get(entry.key) or legacySection:get(entry.key) or entry.default)
		end
	end
	legacySection:reset()
end
