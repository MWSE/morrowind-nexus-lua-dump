...
local function AttachByObjectType(Item, ObjectType, confOn, Light, Weight, confItemValue, count, Ref, meshName, confSize)
	if Item.object.objectType == ObjectType then
		if confOn == true then
			if math.round(tes3.getValue({item = Item.object.id, useDurability = false}) / Weight,0) >= confItemValue then
				if Light == true then
...
local function AttachItemsContainer(Item, count, Weight, Ref, meshName, confValue, confSize, confGold)
	if config.FindingValueContainer == true then
		...
		if tes3.getOwner({reference = Ref}) ~= nil then
			count = AttachByObjectType(Item, tes3.objectType.light, config.ItemLightOwner, true, Weight, config.ItemLightOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.book, config.ItemBookOwner, false, Weight, config.ItemBookOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.alchemy, config.ItemAlchemyOwner, false, Weight, config.ItemAlchemyOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.ammunition, config.ItemAmmunitionOwner, false, Weight, config.ItemAmmunitionOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.apparatus, config.ItemApparatusOwner, false, Weight, config.ItemApparatusOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.armor, config.ItemArmorOwner, false, Weight, config.ItemArmorOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.clothing, config.ItemClothingOwner, false, Weight, config.ItemClothingOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.enchantment, config.ItemEnchantmentOwner, false, Weight, config.ItemEnchantmentOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.ingredient, config.ItemIngredientOwner, false, Weight, config.ItemIngredientOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.lockpick, config.ItemLockpickOwner, false, Weight, config.ItemLockpickOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.miscItem, config.ItemMiscItemOwner, false, Weight, config.ItemMiscItemOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.probe, config.ItemProbeOwner, false, Weight, config.ItemProbeOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.repairItem, config.ItemRepairItemOwner, false, Weight, config.ItemRepairItemOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.spell, config.ItemSpellOwner, false, Weight, config.ItemSpellOwnerValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.weapon, config.ItemWeaponOwner, false, Weight, config.ItemWeaponOwnerValue, count, Ref, meshName, confSize)
			if string.find(Item.object.id, '[Gg]old_0') ~= nil or string.find(Item.object.id, '[Gg]old_1') ~= nil then
				if confGold == true then
					count = count + Item.count
					MeshAttach(Ref, meshName, confSize)
				else
					if math.round(tes3.getValue({item = Item.object.id, useDurability = false}) / Weight * Item.count,0) >= config.GoldValueOwner then
						count = count + Item.count
						MeshAttach(Ref, meshName, confSize)
					end
				end
			...
			count = AttachByObjectType(Item, tes3.objectType.light, config.ItemLightOn, true, Weight, config.ItemLightValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.book, config.ItemBookOn, false, Weight, config.ItemBookValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.alchemy, config.ItemAlchemyOn, false, Weight, config.ItemAlchemyValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.ammunition, config.ItemAmmunitionOn, false, Weight, config.ItemAmmunitionValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.apparatus, config.ItemApparatusOn, false, Weight, config.ItemApparatusValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.armor, config.ItemArmorOn, false, Weight, config.ItemArmorValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.clothing, config.ItemClothingOn, false, Weight, config.ItemClothingValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.enchantment, config.ItemEnchantmentOn, false, Weight, config.ItemEnchantmentValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.ingredient, config.ItemIngredientOn, false, Weight, config.ItemIngredientValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.lockpick, config.ItemLockpickOn, false, Weight, config.ItemLockpickValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.miscItem, config.ItemMiscItemOn, false, Weight, config.ItemMiscItemValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.probe, config.ItemProbeOn, false, Weight, config.ItemProbeValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.repairItem, config.ItemRepairItemOn, false, Weight, config.ItemRepairItemValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.spell, config.ItemSpellOn, false, Weight, config.ItemSpellValue, count, Ref, meshName, confSize)
			count = AttachByObjectType(Item, tes3.objectType.weapon, config.ItemWeaponOn, false, Weight, config.ItemWeaponValue, count, Ref, meshName, confSize)
			if string.find(Item.object.id, '[Gg]old_0') ~= nil or string.find(Item.object.id, '[Gg]old_1') ~= nil then
				if confGold == true then
					count = count + Item.count
					MeshAttach(Ref, meshName, confSize)
				else
					if math.round(tes3.getValue({item = Item.object.id, useDurability = false}) / Weight * Item.count,0) >= config.GoldValue then
						count = count + Item.count
						MeshAttach(Ref, meshName, confSize)
					end
				end
...
local function Detect()
	...
	if currentValueOwner ~= newValueOwner then
		...
		config.GoldValueOwner = newValueOwner
