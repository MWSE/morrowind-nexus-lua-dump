...
local function getIngoreList(ref)
    local function doIgnoreMesh(meshRef)
        return meshRef.baseObject.objectType == tes3.objectType.npc 
            or meshRef.baseObject.objectType == tes3.objectType.creature
    end
    local ignoreList = {}
...
local function doOrient(result)
    return not (config.mcmConfig.noOrientNonStatic 
        and (result.reference and result.reference.object.objectType ~= tes3.objectType.static))
end
...
local function positionRef(ref, rayResult)
...
local function orientRef(ref, rayResult)
