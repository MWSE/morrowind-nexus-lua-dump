--[[
    ...
    to write to common.log:debug or a custom log file.
...
--[[
    ...
    @string outputFile
        common.log:debug or nil: reverts back to writing to common.log:debug
        Otherwise, creates a new file to start writing to
]]
function Logger:setOutputFile(outputFile)
    if outputFile == nil or string.lower(outputFile) == "common.log:debug" then
        self.outputFile = nil
    else
...
function Logger:write(logLevel, message, ...)
    ...
    if self.outputFile then
        ...
        --otherwise straight to common.log:debug
        print(output)
    end
end
