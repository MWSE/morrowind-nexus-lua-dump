...
function this.createWindow()
    ...
    -- Create layout
    local input_label = menu:createLabel{ text = "Rename your location" }
    ...
    input.text = this.name  -- initial text
    input.borderLeft = 5
    input.borderRight = 5
...
function this.onOK(e)
    ...
    if (menu) then
        ...
        --tes3.messageBox{ message = this.name .. " renamed to " .. name }
        this.name = name
        --tes3.messageBox{ message = this.name }
