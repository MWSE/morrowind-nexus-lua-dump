...
(fn player-marksman-base [] (player-stat-base "skills" "marksman"))

(fn player-speed-base [] (player-stat-base "attributes" "speed"))

(fn effect-magnitude [c]
  ;; This magic number was selected after some play testing.
  ;; Maybe there's a "better" way to get here..
  (local magic-num .02685)
...
(fn on-load [data] (set active data.active))

(fn on-save [] {:active active :version script-version})

(fn on-update [dt]
  (have-item)
  (when (and active
             (not= do-effect (is-bow-prepared)))
    (do
  ...
  (when (not= (camera.getMode) camera.MODE.ThirdPerson)
    (set effect 0))
  (when (and (not= use-aiming-offset (> effect 0.4))
             do-effect)
    (do
...
local function have_item()
  do
    ...
    local refId = item_refId
    local has = (inv:countOf(refId) > 0)
