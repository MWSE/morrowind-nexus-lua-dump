...
local function isPlaceable(target)
    ...
    local isNonCarryableLight = target.object.objectType == tes3.objectType.light
        and target.object.canCarry ~= true

    local canPickUp = tes3.hasOwnershipAccess{ target = target }
        and target.object.value 
        and not isNonCarryableLight

    return isInShop or canPickUp
end
