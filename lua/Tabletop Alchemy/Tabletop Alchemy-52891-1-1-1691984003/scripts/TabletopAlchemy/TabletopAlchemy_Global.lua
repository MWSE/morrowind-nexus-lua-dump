...
local appar = types.Apparatus
local baseQuality = { apprentice = appar.record("apparatus_a_mortar_01").quality,
    journeyman = appar.record("apparatus_j_mortar_01").quality, master = appar.record("apparatus_m_mortar_01").quality,
    grandMaster = appar.record("apparatus_g_mortar_01").quality,
    secretMaster = appar.record("apparatus_sm_mortar_01").quality }
local player = nil
...
local function returnFromPlayer()
    local allConts = player.cell:getAll()
                ...
                for x, cont in ipairs(allConts) do
                    if (cont.id == tcontId and ingred.count > 0) then
                        if (tcount < ingred.count) then --if we are moving less than the ingredients in this container
                            ingred:split(tcount):moveInto(getInventory(cont))
                            break
                        else
...
local function apparatusActivate(object, actor)
    ...
    if (valid) then
        ...
        waitCount = 0
        local tmQuality = types.Apparatus.record(tempMortar).quality
        for x, appar in ipairs(holdingCell) do
            if (tmQuality == baseQuality.grandMaster and appar.recordId == "zhac_aha_gm") then
                appar:teleport(player.cell, player.position)
            end
            if (tmQuality == baseQuality.master and appar.recordId == "zhac_aha_m") then
                appar:teleport(player.cell, player.position)
            end
            if (tmQuality == baseQuality.journeyman and appar.recordId == "zhac_aha_j") then
                appar:teleport(player.cell, player.position)
            end
            if (tmQuality == baseQuality.apprentice and appar.recordId == "zhac_aha_a") then
                appar:teleport(player.cell, player.position)
            end
        end
