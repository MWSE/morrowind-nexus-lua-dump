...
local function ownerCheck(object, skipSetting)
    ...
    if (object.owner.recordId ~= nil) then
        return false
    elseif (object.owner.factionId ~= nil) then
        return false
    end
    return true
end
...
local function removeItem(itemId,obj)
    if obj then
        if obj.count > 0 then
            obj:remove()
        end
...
local function TTA_ExitAlchemy()
...
local function TTA_EnterAlchemy()
...
return {
    ...
    eventHandlers  = {
        ...
        TTA_EnterAlchemy = TTA_EnterAlchemy,
TTA_ExitAlchemy = TTA_ExitAlchemy
    }
}
