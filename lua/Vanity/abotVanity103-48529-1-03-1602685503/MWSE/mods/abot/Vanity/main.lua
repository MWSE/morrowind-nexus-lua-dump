	...
	if not visual then
		...
		if visual then
			...
			if cacheSize > visualsCacheMaxSize then
				...
				visualsCache = {} -- not ideal, but fast
			end
...
local function isPlayable(race)
	assert(race.flags)
	return bit.band(race.flags, 0x01) == 0x01
end

--[[
local function isBeast(race)
	assert(race.flags)
	return bit.band(race.flags, 0x02) == 0x02
end
--]]
...
local function giveDistinctItems(npcRef)
	...
	-- add 1 copy of different looking available gauntlets/cuirasses to player
	...
	tes3.messageBox({
		...
		callback = function(e)
			...
			--[[
				...
				msg = string.format(
					"%s test helper:\nRestore default configuration? (Warning, it will clean position settings)"
				, modPrefix)
				tes3.messageBox({
					message = msg,
					buttons = { sYes, sNo },
...
from NPCs inventory/barter menus so it's better to avoid messing with how default game/other mods equip them.
	...
	local onUseRingExtraSlot = tes3.hasCodePatchFeature(66)
...
--]]
	...
	controls:createHyperlink{
		...
		exec = 'explorer "https://www.nexusmods.com/morrowind/mods/19510?tab=files&file_id=1000007846"',
		description = mcpDesc
	}
	controls:createHyperlink{
		text = "Morrowind Code Patch Beta Update",
		exec = 'explorer "https://www.nexusmods.com/morrowind/mods/26348?tab=files&file_id=1000013757"',
		description = mcpDesc
	}
	controls:createHyperlink{
		text = "MGE-XE",
		exec = 'explorer "https://www.nexusmods.com/morrowind/mods/41102"',
	}
	controls:createHyperlink{
		text = "Latest MWSE-Lua development build",
		exec = 'explorer "https://nullcascade.com/mwse/mwse-dev.zip"'
	}
	controls:createHyperlink{
		text = "Better Bodies",
		exec = 'explorer "https://www.nexusmods.com/morrowind/mods/42395"',
		description = [[
...
using vanilla/other body replacers may not work the same.
	...
	controls:createHyperlink{
		...
		exec = 'explorer "http://mw.modhistory.com/download-10-11364"',
		description = [[
...
using vanilla/other body replacers may not work the same.
	...
	controls:createHyperlink{
		...
		exec = 'explorer "https://www.nexusmods.com/morrowind/mods/47925"',
		description = [[
If you install Assetless No Glow and delete any Textures/magicitem/caustXX.dds
you will still haveno-glow armors but keep the pretty shiny glow on amulets and rings!
]]
	}
	controls:createHyperlink{
		text = "abot's Morrowind mods on Nexus",
		exec = 'explorer "https://www.nexusmods.com/users/38047?tab=user+files&BH=0"',
	}
	controls:createHyperlink{
		text = "abot's Morrowind site",
		exec = 'explorer "http://abitoftaste.altervista.org/morrowind/"',
	}
