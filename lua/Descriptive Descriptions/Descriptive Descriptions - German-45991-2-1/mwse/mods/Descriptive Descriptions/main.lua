...
local function extraTooltip(e)
    ...
    if e.object.objectType == tes3.objectType.weapon then
        local textSpeed = string.format("Geschwindigkeit: %.2f", speed)
        local textReach = string.format("Reichweite: %.2f", reach)
	...
	if e.object.enchantment == nil then
        ...
        local textEnch = string.format("Verzauberungswert: %u", enchValue)
    ...
    elseif e.object.objectType == tes3.objectType.light then
		...
		local textDuration = string.format("Dauer:    ")
    ...
    elseif isSoulGem == true then
        ...
        local textSoulSize = string.format("Seelenkapazität: %u", soulValue)
...
function modConfig.onCreate(container)
	...
	local function toggleLightColor(e)
		...
		descriptionLabel.text = config.lightColor and
			"Einzigartige Farbe für den Dauerbalken."
			or
			"Generische Farbe für den Dauerbalken."
	end
	...
	do
		...
		makeButton(optionBlock, "Einzigartige Farbe für den Dauerbalken?", buttonText, toggleLightColor)
		...
		descriptionLabel = descriptionBlock:createLabel({ text =
			...
			"Es fügt Werte wie Geschwindigkeit, Reichweite, Verzauberungswert, Seelenkapazität under Dauer hinzu."
		})
