...
local function calculatePlayerEffectiveness(action, bribeAmount)
	...
	if action == "bribe" and bribeAmount then
		effectiveness = effectiveness + math.min(persuasionModifiers.bribeLimitingFactor, math.log10(bribeAmount)*math.sqrt(bribeAmount)/persuasionModifiers.bribeLimitingDiv) * math.log10(bribeAmount) * persuasionModifiers.bribeAmountMultiplier
	end
...
function persuasion.isGiftIllegal(item, npcRef)
    ...
    for _, illegalItemId in ipairs(persuasionModifiers.illegalItems) do
        if item.id == illegalItemId then
            -- Check if NPC is exempt from illegal bribe mechanics
            if persuasionModifiers.isBribeExempt(npcRef) then
