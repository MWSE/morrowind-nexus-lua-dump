local cf = mwse.loadConfig("PvP", {animsw = true, npcsw = true, pvpspd = 1, pvpproj = 1, pvpprdod = true, par = true, duel = true, pvpcont = true, tim = true, wspd = true, cone = true, round = true,
fastanim = true,
atspdm = -0.4, hitr = 0, volimp = 0.8, dmgmult = 0.5, atkmult = 0.25, dodcost = 30, dodmult = 1, acr = 75, agil = 100, scan = 1, dodkey = {keyCode = 54}, dmgind = true, m3 = false})
...
p0:createDecimalSlider{label = "Minimum delay in seconds before attack for NPCs (the lower the value, the more frequent the attacks, -0.4 by default, requires save reload)", min = -0.9, max = 0.6, variable = var{id = "atspdm", table = cf}}
...
L.SectorDod = function(ar) if cf.pvpproj > 0 and not T.Dom.timeLeft then		T.Dom = timer.start{duration = 0.3, callback = function() end}
	for i, m in pairs(prm.allMobileActors) do
		if not L.ASN[m.actionData.animationAttackState] and not m.isFalling and not m.isHitStunned then	local ang = mp:getViewToActor(m)
			if m.inCombat and math.abs(ang) < 15 and math.abs(m:getViewToActor(mp)) < 45 then	local ch = m.agility.current + m.sanctuary - (ar and m.playerDistance/50 or 0)		
...
--	tes3.messageBox("At hit!  Sw = %.3f  FizD = %.2f", ad.attackSwing, ad.physicalDamage)
	...
	if cf.par and t then	local tad = t.actionData		if tad.animationAttackState == 4 and tad.blockingState ~= 2 then		local tr = t.reference
		...
		if w and wt < 9 and tw and twt < 9 and not ar.tempData.Parried and (w.reach + tw.reach) * 150 > ar.position:distance(tr.position) then
			...
			if math.abs(t:getViewToActor(a)) < 90 then
				...
				if a == mp then
					if cf.dmgind then L.ParInd(imp/20, park > 1 and 5 or (park > 0.75 and 3 or 7)) end
					...
					if cf.pvpcont and a:getSkillValue(WT[wt].s) + a.agility.current > math.random(200) then a:forceWeaponAttack{swing = math.random(50,100)/100} end
...
local function MOBILEACTIVATED(e)	local m = e.mobile 		if m then	local r = e.reference	local firm = m.firingMobile
	if firm then	local si = m.spellInstance
		if firm == mp then L.SectorDod(not si) end
