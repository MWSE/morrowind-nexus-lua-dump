...
hitr = 0, volimp = 0.8, cofat = 0.5, comult = 1, dmgmult = 0.5, atkmult = 0.25, dodcost = 30, dodkey = {keyCode = 54}, dmgind = true, m3 = false})
...
--		end
		...
		if actort then	local ob = r.object		AF[r] = {}		if actort == 1 or ob.biped then r.tempData.hum = true end
			N[r] = {m = m}
		end
	end
...
mwse.memory.writeByte{address = 0x46CADC, byte = 0x83}	-- Разрешает менять скорость атак кулаков и кричеров
