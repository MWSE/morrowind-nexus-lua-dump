...
local function silence()
	if (nukeHeartOnce == false) then
		...
		tes3.messageBox("[Heartthrum] The Heart has been destroyed.")
		nukeHeartOnce = true
		TimerRunning = false
	end
end
...
local function calcDistance()
	...
	tes3.messageBox("[Heartthrum] distance from the Heart: %.2f", distance)
	--the log sometimes omits the values, but it works correctly in-game
	--so sure as Oblivion I'm not going to fix that
	mwse.log("[Heartthrum] distance from the Heart: %.2f", distance)
...
local function checkWorldspace()
	sourcePos = tes3.getReference("RFD Sound Heart ext").position:copy()
	distance = tes3.player.position:distance(sourcePos)
	tes3.messageBox("[Heartthrum] (checkWorldspace) distance from the Heart: %.2f", distance)
	if distance < 40000 then
		tes3.messageBox("[Heartthrum] TimerRunning true.")
		if not TimerRunning then
			TimerRunning = true
			runTimer()
		end
	else
		tes3.messageBox("[Heartthrum] TimerRunning false.")
		if TimerRunning then
			TimerRunning = false
			HRTimer:cancel()
		end
...
checkPlayerCell = function()
	if tes3.getPlayerCell().isInterior then
		if (tes3.getPlayerCell().id:find("Dagoth Ur") or tes3.getPlayerCell().id:find("Akulakhan")) then
			--tes3.messageBox("[Heartthrum] You're inside Dagoth Ur.")
...
local function loaded()
	if (tes3.getGlobal("HeartDestroyed") == 0) then
		...
		tes3.messageBox("[Heartthrum] The Heart no longer exists in this save game.")
...
local function initialized()
	...
	tes3.messageBox("[Heartthrum] initialized")
