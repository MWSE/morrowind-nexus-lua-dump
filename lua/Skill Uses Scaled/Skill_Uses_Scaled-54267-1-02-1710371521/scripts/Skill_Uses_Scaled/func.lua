...
local unarmored_hit_decay = time.runRepeatedly(function()
    if Dt.counters.unarmored(0) > 0.01 then
      Dt.counters.unarmored(- math.max(Dt.counters.unarmored(0)/Mui.getSetting('Unarmored_Decay_Time'), 2/(Mui.getSetting('Unarmored_Decay_Time'))))
...
local acrobatics_jump_decay = time.runRepeatedly(function()
    if Dt.counters.acrobatics(0) > 0.01 then
      Dt.counters.acrobatics(- math.max(Dt.counters.acrobatics(0)/Mui.getSetting('Acrobatics_Decay_Time'), 2/(Mui.getSetting('Acrobatics_Decay_Time'))))
...
local athletics_run_decay  = time.runRepeatedly(function()
    ...
    if Dt.counters.athletics_debug(0) > 9.01 then
      if Mui.getSetting("SUS_DEBUG") then print('SUS [Athletics] Decay Timer: '.. string.format('%.0f', Dt.counters.athletics(0))) end
    end
  end, time.second)
    ...
    Dt.scalers:new{
      ...
      func = function(_, xp)
        ...
        if Fn.has_effect('disintegratearmor') then
          if Mui.getSetting("SUS_DEBUG") then print('SUS - Disintegrate Armor in effect, returning vanilla XP') end
          return xp
        end
...
--       if #Fn.get_unarmored_slots() <= 3 and (race == 'argonian' or race == 'khajiit') then beast_factor = Mui.getSetting('Unarmored_Beast_Races')/ #Fn.get_unarmored_slots() end
      local gank_factor  = (Mui.getSetting('Unarmored_Start') - Mui.getSetting('Unarmored_Min')) / (2 * Dt.counters.unarmored(1) - 1) + Mui.getSetting('Unarmored_Min')
      ...
      local skill_factor = 100 / (35 + rating + skill + armor_weight * Mui.getSetting('Unarmored_Armor_Mult')) -- Rating is added here alongside skill, because unarmored has exponential scaling baked in.
  ...
  for _, _skillid in ipairs(Dt.scaler_groups.SPELL) do
    Dt.scalers:new{ name = _skillid, 
      func = function(_, xp)
        ...
        if refund > 0 then
          if Mui.getSetting("SUS_DEBUG") then print('SUS [Dynamic Spell Cost] Refund: '.. string.format('%.2f', refund/spell.cost*100)..'% | '.. string.format('%.2f', refund) ..' MP') end
        else
          if Mui.getSetting("SUS_DEBUG") then print('SUS [Dynamic Spell Cost] Penalty: '.. string.format('%.2f', refund/spell.cost*100)..'% | '.. string.format('%.2f', refund) ..' MP') end
        end
...
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'acrobatics', 
    func = function(useType, xp)
      ...
      local encumbered_mult = Mui.getSetting('Acrobatics_Encumbrance_Min') + (Mui.getSetting('Acrobatics_Encumbrance_Max') - Mui.getSetting('Acrobatics_Encumbrance_Min'))
...
-----------------------------------------------------------------------------------------------------------
  Dt.scalers:new{ name = 'athletics', 
    func = function(useType, xp)
      ...
      local encumbered_mult = Mui.getSetting('Athletics_Encumbrance_Min') + (Mui.getSetting('Athletics_Encumbrance_Max') - Mui.getSetting('Athletics_Encumbrance_Min'))
                  * types.Actor.getEncumbrance(self) / (Dt.GMST.fEncumbranceStrMult * types.Player.stats.attributes.strength(self).base)
      local recursive_mult  = (Mui.getSetting('Athletics_Start')) + (Mui.getSetting('Athletics_Marathon') - (Mui.getSetting('Athletics_Start'))) * (Dt.counters.athletics(2) -2)/Mui.getSetting('Athletics_Decay_Time')
...
-----------------------------------------------------------------------------------------------------------
  ...
  if Mui.getSetting("SUS_DEBUG") then print('SUS: Scalering Commenced') end
end
