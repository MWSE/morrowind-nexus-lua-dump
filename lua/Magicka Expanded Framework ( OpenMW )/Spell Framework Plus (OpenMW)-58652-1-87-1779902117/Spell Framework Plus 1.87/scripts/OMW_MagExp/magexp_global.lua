...
local function deriveMagicEffectImpactStaticId(mgef)
    ...
    local staticId = mgef.hitVfx
    if staticId and staticId ~= "" then return staticId end
    return nil
end
...
local function _magExpEnsureActorVfxScript(actor)
    ...
    -- Non-player actors need a local handler to interpret AddVfx/RemoveVfx events and paralysis.
...
local function applySpellToActor(spellId, caster, target, hitPos, isAoe, itemObject, forcedEffects, unreflectable, casterLinked, userData, muteAudio, muteLight, continuousVfx, effectScale, sourceData)
    ...
    -- Ensure NPC actor script is attached to handle physical paralysis and interpret events
    if target.type ~= types.Player then
        _magExpEnsureActorVfxScript(target)
    end
            ...
            if rawEff and mgef then
                ...
                if static and static.model and target.enabled then
                    if isPersistentLoop then
                        ...
                        -- Attach the VFX handler script to non-player actors
                        if target.type ~= types.Player then
                            _magExpEnsureActorVfxScript(target)
                        end
                    end
...
local function detonateSpellAtPos(spellId, caster, pos, cell, itemObject, forcedEffects, unreflectable, casterLinked, vfxOverride, impactSpeed, maxSpeed, areaVfxScale, excludeTarget, userData, muteAudio, muteLight, continuousVfx, effectScale)
    ...
    if areaVfxEff then
        ...
        if mgef then
            ...
            -- Replaces the broken playSound3d call
...
local function spawnCastVfxForAllEffects(attacker, spell, spellId, opts)
    ...
    local showAll = true
    pcall(function()
        local sec = storage.globalSection('SettingsMagExp_General')
        if sec then
            local val = sec:get('ShowAllCastVfx')
            if val ~= nil then showAll = val end
        end
    end)

    -- Attach cast VFX to ACTOR ROOT only (no hands/head/etc).
    local rootBone = ""
    for i, eff in ipairs(spell.effects) do
        if not showAll and i > 1 then
            break
        end
        local mgef = core.magic.effects.records[eff.id]
        if mgef then
            local castStaticId = (mgef.castStatic and mgef.castStatic ~= "") and mgef.castStatic or _magExpFallbackCastStaticId(mgef, eff.id)
            local model = _magExpModelFromStaticId(castStaticId)
            local vfxId = string.format("MagExpCastStart_%s_%d_%s", tostring(spellId), i, tostring(eff.id):lower())
            if model then
...
local function launchSpell(data)
    ...
    -- Spawn cast-start VFX for ALL effects (respects setting)
    ...
    for gi, group in ipairs(schoolGroups) do
        ...
        if not proj or not proj:isValid() then
            ...
            local initPayload = {
                ...
                boltModel       = boltModels[1],        
                particleTextures = particleTextures,     
                particle        = particleTextures[1],   
...
local function onUpdate(dt)
    ...
    -- continuousVFX Actor Registry Cleanup
