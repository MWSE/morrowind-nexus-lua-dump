...
-- gmsts are constant per session; cache once instead of per-effect lookup
...
return function (spell, player)
	...
	if not godmode then --if (stats.getMagicEffects().getOrDefault(ESM::MagicEffect::Silence).getMagnitude() && !godmode)
		if types.Actor.activeEffects(player):getEffect("silence").magnitude > 0 then
			return 0 --return 0;
		end			
	end
	
	if spell.type == core.magic.SPELL_TYPE.Power then --if (spell->mData.mType == ESM::Spell::ST_Power)
		if types.Actor.activeSpells(player):canUsePower(spell) then --return stats.getSpells().canUsePower(spell) ? 100 : 0;
			return 1
		else
			return 0
		end
	...
	for _,effect in pairs(spell.effects) do --for (const ESM::IndexedENAMstruct& effect : spell->mEffects.mList)
		...
		local s = 2 * types.NPC.stats.skills[effect.effect.school](player).modified --float s = 2.0f * actor.getClass().getSkill(actor, magicEffect->mData.mSchool);
	...
	local actorWillpower = types.Player.stats.attributes["willpower"](player).modified --float actorWillpower = stats.getAttribute(ESM::Attribute::Willpower).getModified();
	local actorLuck = types.Player.stats.attributes["luck"](player).modified --float actorLuck = stats.getAttribute(ESM::Attribute::Luck).getModified();
	...
	local castBonus =  types.Actor.activeEffects(player):getEffect("sound").magnitude
	castChance = castChance + castBonus --float castChance = baseChance + castBonus;
	--castChance *= stats.getFatigueTerm();
	local max = types.Actor.stats.dynamic.fatigue(player).base --float max = getFatigue().getModified();
    local current = types.Actor.stats.dynamic.fatigue(player).current --float current = getFatigue().getCurrent();
	...
	return math.max(0, math.min(1000000, castChance/100))
end