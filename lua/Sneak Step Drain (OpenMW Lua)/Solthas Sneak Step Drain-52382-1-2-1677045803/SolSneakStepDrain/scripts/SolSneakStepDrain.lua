      ...
      if isSneak == nil then
        ...
        if isSneak or buffTotal ~= 0 then
          ...
          if math.abs(buffType) == 1 then
            ...
            perTick = math.ceil(100*(maxMod/maxCharge)*chargeInterval/math.sqrt(getWeights()))/100
            --perTick = ((maxMod/maxCharge)*chargeInterval/math.sqrt(getWeights()))
            if buffType < 0 then -- if not sneaking, gain at reduced rate
                perTick = math.ceil(0.5*perTick) -- gain at half rate
            end
          ...
          elseif math.abs(buffType) == 2 then
            ...
            perTick = math.ceil(100*(maxMod/maxDrain)*chargeInterval*getWeights())/100
            --perTick = (maxDrain*getWeights()*chargeInterval)
            if buffType < 0 then -- if not sneaking, gain at reduced rate
                perTick = math.ceil(1.5*perTick) -- decrease at double rate
            end
