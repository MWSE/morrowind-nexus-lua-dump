local cf = mwse.loadConfig("Eternal Shine", {N = 80, pl = false, radmult = 0.3, int = 0.25, ext = 0.4, rain = 0.5, fog = 0.75, storm = 0.75, mind = 10000, maxd = 20000})
...
local function loaded(e)	p = tes3.player		pp = p.position		Cam = tes3.getCamera()		S = mge.shaders.find{name = "Eternal Shine"}
	if S then	if not event.isRegistered("simulate", simulate) then event.register("simulate", simulate) end
	else timer.start{duration = 3, callback = function() S = mge.shaders.find{name = "Eternal Shine"}	if S and not event.isRegistered("simulate", simulate) then event.register("simulate", simulate) end end} end
