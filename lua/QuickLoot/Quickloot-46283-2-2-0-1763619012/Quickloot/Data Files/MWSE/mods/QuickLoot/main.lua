...
local function setSelectionIndex(index)
	...
	if ((currentIndex and index == currentIndex) or index < 1 or index > currentInventorySize) then
		return
	end
	...
	local showStealing = config.showOwned and not tes3.hasOwnershipAccess({ reference = tes3.player, target = currentTarget })
	for i, block in pairs(children) do
		if (i == index) then
			...
			if showStealing then
				label.color = tes3ui.getPalette("big_answer_pressed_color")
			else
				label.color = tes3ui.getPalette("active_color")
			end
		elseif (currentIndex and i == currentIndex) then
			-- If this is the old index, change the color back to normal.
			local label = block:findChild(GUIID_QuickLoot_ContentBlock_ItemLabel)
			if showStealing then
				label.color = tes3ui.getPalette("big_answer_color")
			else
...
local function svengOnKeyDown(e)
	if e.element ~= nil then return end
      
    if e ~= config.svengKey then
		return
...
local function refreshItemsList()
	...
	local showStealing = config.showOwned and not tes3.hasOwnershipAccess({ reference = tes3.player, target = currentTarget })
	if showStealing then
		nameLabel.text = currentTarget.object.name .. " (Owned)"
		nameLabel.color = tes3ui.getPalette("negative_color")
	else
	...
	for _, stack in pairs(container.inventory) do
		...
		-- Main container block for this item
		local block = contentBlock:createBlock({})
		block.flowDirection = "left_to_right"
		block.autoWidth = true
		block.autoHeight = true
		block.paddingAllSides = 3
		block.widthProportional = 1.0  -- Take full width
		...
		local leftBlock = block:createBlock({})
		leftBlock.flowDirection = "left_to_right"
		leftBlock.autoWidth = true
		leftBlock.autoHeight = true
		
		local icon = leftBlock:createImage({id = GUIID_QuickLoot_ContentBlock_ItemIcon, path = "icons\\" .. item.icon})
		icon.borderRight = 5
		...
		-- add souls to the soulgem label
		if item.soulGemData ~= nil then
			for _, data in pairs(stack.variables or {}) do
				labelText = item.name .. " (" .. data.soul.name .. ")"
			end
		end
		
		local label = leftBlock:createLabel({id = GUIID_QuickLoot_ContentBlock_ItemLabel, text = labelText})
		--label.absolutePosAlignY = 0.5
		if showStealing then
			label.color = tes3ui.getPalette("big_answer_color")
		end
		
		-- RIGHT SIDE: Gold and weight info
		if (config.showGoldAndWeight == true) then
		    local spacer = block:createBlock({})
			spacer.widthProportional = 1.0
			spacer.autoHeight = true
		
			local rightBlock = block:createBlock({})  -- Child of main block
			rightBlock.flowDirection = "right_to_left"
			rightBlock.autoWidth = true
			rightBlock.autoHeight = true
			--rightBlock.absolutePosAlignX = 1.0  -- Right align this block
			
			local total_price = tes3.getValue({item = stack.object}) * math.abs(stack.count)
			local total_weight = round((item.weight) * math.abs(stack.count), 1)
			
			-- modify by durability and soul price
			if stack.variables then				
				for _, itemData in pairs(stack.variables or {}) do 
					total_price = tes3.getValue({item = stack.object, itemData = itemData})
				end
			end
			
			local bracketStart = rightBlock:createLabel({text = " ["})
			--bracketStart.absolutePosAlignY = 0.5
			
			if (buyingGameConfig and (tes3.mobilePlayer.mercantile.current < buyingGameConfig.knowsPrice)) or not buyingGameConfig then
				local goldlabel = rightBlock:createLabel({text = "" .. total_price})
				--goldlabel.absolutePosAlignY = 0.5
				
				local goldicon = rightBlock:createImage({id = GUIID_QuickLoot_ContentBlock_ItemIcon, path = "icons\\gold.tga"})
				--goldicon.absolutePosAlignY = 0.5
				goldicon.borderRight = 5
				goldicon.borderLeft = 5
			end
			
			local weightlabel = rightBlock:createLabel({text = "" .. total_weight})
			--weightlabel.absolutePosAlignY = 0.5
			
			local weighticon = rightBlock:createImage({id = GUIID_QuickLoot_ContentBlock_ItemIcon, path = "icons\\weight.tga"})
			--weighticon.absolutePosAlignY = 0.5
			weighticon.borderRight = 5
			weighticon.borderLeft = 5
			
			local endlabel = rightBlock:createLabel({text = "]"})
...
local function takeItemActivate(e)
	...
	if tes3.worldController.inputController:isShiftDown() then return end
...
local deleteRefPassthrough

local function setDeleteDelay(e)
	mwscript.setDelete{reference=deleteRefPassthrough}
end
...
local function takeAllItems(e)
	...
	if (currentTarget.object.objectType == tes3.objectType.npc) or (currentTarget.object.objectType == tes3.objectType.creature) then
		if (reason == "Empty") then
			if (config.disposePersistent or not currentTarget.object.persistent) then
				mwscript.disable{reference=currentTarget}
				deleteRefPassthrough = currentTarget
				timer.delayOneFrame( setDeleteDelay, timer.simulate )
			else
				tes3.messageBox( tes3.findGMST(tes3.gmst.sDisposeCorpseFail).value )
			end
			return
		end
	end
...
local function onInitialized()
	...
	buyingGameConfig = include("buyingGame.config")
	...
	event.register("keyDown", takeAllItems, { priority = 5000000000 })
	event.register("keyDown", takeItemKey, { priority = 5000 })
	event.register("keyDown", arrowKeyScroll, { priority = 5000 })
	event.register("mouseWheel", onMouseWheelChanged, { priority = 5000 })
	event.register("menuEnter", clearQuickLootMenu)
	event.register("keyDown", svengOnKeyDown, { priority = 5000 })
	event.register("activate", takeItemActivate, { priority = 5000 })
