...
local function RemoveAllActiveSounds()
	if CreatureUtils.getCreatureSoundbank() == nil then return end

	for _, sound in pairs(CreatureUtils.getCreatureSoundbank().sounds or {}) do
...
local function onUpdate()
	...
	if not _settings.enableCreatureComponent or core.isWorldPaused() then return end

	if _currentFrameCount == 0 then

		if _creatureIsActive and not CreatureUtils.hasNoAnim(self) and CreatureUtils.getCreatureSoundbank() ~= nil and
			...
			--   if CreatureUtils.StringContains(types.Creature.record(self).id, "lich")
			--   then
			--   	CreatureUtils.WriteLog("Creature anim:" .. CreatureUtils.dump(CreatureUtils.getCurrentAnimationGroups(self).lowerbody))
			--   	--CreatureUtils.WriteLog("player distance: " .. CreatureUtils.distanceToPlayer(_playerPosition, self) )	
			--  	CreatureUtils.WriteLog("creature cell: " ..  CreatureUtils.dump(self.cell)  )	
