...
function Utils.loadSoundBanks()
    ...
    for file in vfs.pathsWithPrefix("scripts\\DynamicSounds\\soundbanks") do
       if Utils.stringEndswith(file, ".lua") then   
...
function Utils.getCurrentWeather()
  ...
  -- Early exit if running OpenMW 0.49
    ...
    local interiorRegion = _interiorCells[currentCell.id]
    if interiorRegion then
        return _regionWeathers[interiorRegion or _lastVisitedRegion] or 1
    end

   -- Scan nearby doors for an exterior cell destination    
    local interiorRegion = nil
    for _, v in pairs(currentCell:getAll(types.Door)) do
        ...
        if destCell and Utils.cellIsExterior(destCell) then
            _interiorCells[currentCell.id] = destCell.region
            interiorRegion = destCell.region
            break
        end
    end
...
function Utils.getWeatherDataToStorage()
    
    local weatherData = {
        interiorCells = {}, --_interiorCells, -- lets not save int cells on the storage...
        regionWeathers = _regionWeathers,
        lastVisitedRegion = _lastVisitedRegion
    }
...
function Utils.loadWeatherDataFromStorage(weatherData)

    if weatherData == nil then return end

    _interiorCells = weatherData.interiorCells or {}
    _regionWeathers = weatherData.regionWeathers or {}
    _lastVisitedRegion = weatherData.lastVisitedRegion or ""

    Utils.WriteLog(Utils.dump(_interiorCells) )
    Utils.WriteLog(Utils.dump(_regionWeathers) )
    Utils.WriteLog(Utils.dump(_lastVisitedRegion) )
...
function Utils.stringEndswith(string, match)      
    local s, _ = string.find(string, match)

    if s == nil then return false end 

    return s+#match-1 == string.len(string)
end
