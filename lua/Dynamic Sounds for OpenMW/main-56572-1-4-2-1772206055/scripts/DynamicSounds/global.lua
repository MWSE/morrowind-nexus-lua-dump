...
local _framesSkip = nil -- only do stuff every x frames (depends on the framerate)
...
local function stopPrevCellSounds(newCellAmbientSounds)
	...
	for _, sound in ipairs(_currentAmbientSounds) do
		if not Utils.soundListContains(newCellAmbientSounds, sound) then
			...
			player:sendEvent('stopAmbientLoopSound', sound.soundPath)
			sound.loaded = false
		else
...
local function configArea()
	...
	Utils.WriteLog("Loading cell or region " .. Utils.getCellNameOrRegion(currentCell))

	--Utils.WriteLog(Utils.getOutsideWeather(_player.cell),true)
	...
	for _, cellObjectSettings in ipairs(areaSettings.objects) do
		...
		for index, cellObject in ipairs(cellObjects) do
			_tableInsert(newCellObjectSounds, { cellObject, Utils.getSoundsForCell(cellObjectSettings) })
		end
	end
...
local function playAllSounds()
	if _framesSkip ~= nil and _currentFrameCount == 0 then
...
local function saveGameFramerate(dt)
	if tonumber(_framesSkip) == nil then
		_framesSkip = math.floor(1/dt)
	    Utils.WriteLog("calculated framerate: " .. _framesSkip)
	end
end
...
local function restoreWeatherData()	
	local modStorage = storage.globalSection('DynamicSoundsSettings.Weather')
	local weatherdata = modStorage:asTable()
	
	if weatherdata == nil then return end

	Utils.loadWeatherDataFromStorage(weatherdata.WeatherData)
end

local function saveWeatherData()	
	local modstorage = storage.globalSection('DynamicSoundsSettings.Weather')
	modstorage:setLifeTime(0)
	modstorage:set("WeatherData", Utils.getWeatherDataToStorage())
end

-- runs each frame
local function onUpdate(dt)
	if SettingsManager.currentSettings().enableMod and Utils.soundbanksLoaded() then
		saveGameFramerate(dt)
		configArea()
		playAllSounds()
...
local function onItemActivated(cellObject)
	...
	if not Utils.soundbanksLoaded then return end

	Utils.WriteLog("onItemActivated!")
	Utils.WriteLog( Utils.dump(cellObject) )

	local areaSettings = Utils.getConfigSectionForCell(_player.cell)
	local _tableInsert = table.insert -- for performance optimization
    local _tableRemove = table.remove -- for performance optimization

	-- -- loop sounds assigned to objects
	-- for index, objectInfo in ipairs(_currentObjectSounds) do
	-- 	if cellObject.recordId == objectInfo[1].recordId then    
	-- 		Utils.WriteLog("object already exists on table. Removing...", true)
	-- 		_tableRemove(_currentObjectSounds, index)
	-- 		return 
	-- 	end
	-- end
	...
	for _, cellObjectSettings in ipairs(areaSettings.objects) do
		if Utils.StringContains(cellObject.recordId, cellObjectSettings[1]) then
			_tableInsert(_currentObjectSounds, { cellObject, Utils.getSoundsForCell(cellObjectSettings) })
		end			
	end
end
...
return {
	engineHandlers = {
		...
		onUpdate = onUpdate,
		onSave = onSave,
		--onItemActive = onItemActivated
	},
    eventHandlers  = {
		setUserSettings = setUserSettings,
		onItemActivated = onItemActivated, -- recieved from item.lua when an item is activated (picked, ....)
    }	
