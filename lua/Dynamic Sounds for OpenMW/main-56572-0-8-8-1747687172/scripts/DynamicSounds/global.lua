...
local function stopPrevCellSounds(newCellAmbientSounds)
	...
	--_currentObjectSounds = {}
	--_currentAmbientSounds = {}
end
...
local function configArea()
	...
	Utils.WriteLog("Loading cell " .. Utils.getCellName(currentCell), true)
...
local function playAmbientLoopSounds()
	...
	for _, sound in ipairs(_currentAmbientSounds) do
		if (sound.soundPath ~= nil and not sound.loaded and not Utils.isInvalidSound(sound)) then
			local eventArgs = {
				path = sound.soundPath,
				volume = (sound.volume ~= nil and sound.volume or 1)
			}

			-- is a loop sound. Play immediately
			if (sound.loop ~= nil and sound.loop == true) then
				if Utils.debugMode() then
