...
local vectors = require("scripts.CandleSmoke.configLanternMoths")
...
function self.onObjectActive(o)
	...
	if devMode then player:sendEvent("showMessage", "MOTH REG "..o.recordId)		end
end
...
--	print("MOTHS updateNearby "..(#targets))
	...
	for i = #targets, 1, -1 do
		...
		if not active[v.ref.id] then
			if devMode then player:sendEvent("showMessage", "REMOVE "..v.ref.recordId)	end
...
function self.onUpdate(dt)
	...
	timer = timer + dt		local noScan = true
	for _, v in ipairs(targets) do
		local cell = v.ref.cell
		if not cell.isExterior and cell ~= player.cell then
			noScan = false
			break
		end
...
--			if chance > 50 then
			if math.random(100) > common.mothChance then
				...
				if devMode then
					player:sendEvent("showMessage", "MOTHS "..v.ref.recordId)
				end
--				chance =  chance - 100
			end
		end
	end
	if timer < 10 and noScan then 	return		end			timer = 0
--	if devMode then player:sendEvent("showMessage", "GETNEARBY moths "..(#targets))	end
