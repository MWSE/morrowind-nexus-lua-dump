...
local function onObjectActive(o)
	...
	if dwemer and type == static and addEffect[o.recordId] then
		lightning[#lightning + 1] = { ref=o, pos=o.rotation:apply(addEffect[o.recordId])
			* o.scale + o.position }	noCheck = false
...
local function onUpdate(dt)
	if dt == 0 or noCheck then return		end
	if #lightning == 0 then noCheck = true	print("VFX CLEARED")		return		end
	timer = timer + dt
	for k, v in ipairs(lightning) do
		local o, inactive = v.ref
		if o.cell.isExterior then
			if not player.cell.isExterior then inactive = true
			else
				local x = o.position.x - player.position.x
				local y = o.position.y - player.position.y
				local d = (x*x + y*y) ^ 0.5
				if d > 20000 then print("INACTIVE "..d)	inactive = true	end
			end
		elseif o.cell ~= player.cell then inactive = true
		end
		if inactive then
			table.remove(lightning, k) player:sendEvent("showMessage", "REMOVED")
		elseif timer > 5 then
--			local pos = o.rotation:apply(addEffect[o.recordId]) * o.scale + o.position
			world.vfx.spawn("meshes/e/taitech/lightning.nif", v.pos, { useAmbientLight=false })
