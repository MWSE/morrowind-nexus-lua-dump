...
local devMode = common.devMode		local debug = common.debug
...
elseif core.contentFiles.has("the red tower.esp") then

vectors = {
	-- # 1 7
	{ v3(13363, 63833, 24218), rotate(354, 0, 0), 1 },
	{ v3(14856, 60369, 23938), rotate(354, 11, 135), 1 },
	{ v3(12828, 60093, 22865), rotate(354, 34, 225), 1 },

	-- # 2 8
	{ v3(19201, 71624, 23163), rotate(348, 357, 299), 1 },
	{ v3(18431, 69379, 22276), rotate(354, 0, 263), 1 },
	{ v3(22109, 67380, 23780), rotate(11, 0, 125), 1 },

	-- # 3 7
	{ v3(27805, 61153, 20287), rotate(0, 0, 172), 1 },
}

vectors.xyPos = util.vector2(20000, 63000)		vectors.radius = 15000
vectors.winds = v3(22415, 70835, 32000)		vectors.smoke = v3(22415, 70835, 32000)
...
function self.onUpdate(dt)
	...
	local c = common	if not c.doMount then		return		end
--	local pos = player.position	local xy = util.vector2(pos.x, pos.y)
	local xy = c.posXY		local distance = math.min(c.viewDistance, 150000)
	...
	for i = #lightning, 1, -1 do
		...
		if v.timer < 1 then
			...
			if d < distance and math.random(100) < ch then
				...
				if d < 20000 then c.doStrike = true			end
