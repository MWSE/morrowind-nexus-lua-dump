	...
	if aeAmmoType == THROWN_TYPE then
		...
		if mobilePlayer.readiedWeapon then
			...
			if tes3.menuMode() then
				timer.delayOneFrame(updateInventory, timer.real)
			end
		end
		return
	end
...
--]]
			if animState > 1 then -- player animation
				timer.delayOneFrame(AequipItem, timer.real)
				return
			end
		end
	end
	...
	if player.object.inventory:contains(aeItemId) then
...
local function autoEquipAmmo()
	...
	if player.data then
		if player.data.ab01HandpickedAmmoId then
			...
			if ammoId then
				if player.object.inventory:contains(ammoId) then -- important safety!!!
					aeItemId = ammoId
...
local function projectileExpire(e)
	...
	if not firingWeaponObj then
		return -- may happen with bound weapons?
	end
