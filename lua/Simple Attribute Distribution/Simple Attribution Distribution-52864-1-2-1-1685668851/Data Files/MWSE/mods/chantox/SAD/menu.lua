local compat = require("chantox.SAD.compat")
...
this.createStat = function (table, name, value, columns, onHover)
    ...
    if table.base >= config.attributeLvlCap then
        table.button_plus.disabled = true
        table.button_plus.widget.state = tes3.uiState.disabled
    end
end
...
this.build = function (element, nextLevel)
        ...
        for i, a in pairs(tes3.mobilePlayer.attributes) do
            ...
            atr[i].button_minus:register("mouseClick",
                function ()
                    ...
                    if (max > atr[i].pointsSpent or
                        base < config.attributeLvlCap) then
                        enableButton{index = i, type = "button_plus"}
                    end

                    if noPoints then
                        for j, data in ipairs(atr) do
                            if (max == 0 or max > data.pointsSpent) then
            ...
            atr[i].button_plus:register("mouseClick",
                function ()
                    ...
                    local max = config.maxPointsPerAttribute
                    local base = atr[i].base + atr[i].pointsSpent
                    if (max > 0 and max <= atr[i].pointsSpent or
                        base >= config.attributeLvlCap) then
                        disableButton{index = i, type = "button_plus"}
                    end

                    if state.points <= 0 then
                        disableButton{type = "button_plus"}
                    end
    ...
    pageRight.paddingAllSides = 5
        local imageFrame = pageRight:createThinBorder{id = "SAD_ImageBlock"}
    ...
    local function levelUpDone()
        ...
        for i = 1, 8 do
            local max = config.maxPointsPerAttribute
            if max == 0 or atr[i].pointsSpent < max then
                local base = atr[i].base + atr[i].pointsSpent
