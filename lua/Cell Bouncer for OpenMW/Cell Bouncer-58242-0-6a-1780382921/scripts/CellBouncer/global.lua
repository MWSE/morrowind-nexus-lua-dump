...
-- Options: wall, flip, teleport
local BOUNCER_MODE = "wall"
		...
		if endsWith(fileName, ".yaml") or endsWith(fileName, ".yml") then
			...
			if ok and type(data) == 'table' then
				...
				table.insert(BLOCK_BEHAVIORS, {
				  ...
				  boundaryDestination = data.boundaryDestination,
...
local function onUpdate()
	...
	for _, player in ipairs(world.players) do
		...
		--local id = player.id
		if lastCellIds[player.id] ~= currentCell.id and isCellBlocked(currentCell) then
			-- Get most recent safe spot
			local spot = safeSpots[player.id]
			...
			local dst      = behavior and behavior.boundaryDestination or FALLBACK_CELL_NAME
			...
			if spot.cellName then
				...
				elseif mode == "teleport" then
					player:teleport(dst, FALLBACK_POS, { rotation = player.rotation, onGround = true })
				elseif mode == "kill" then
					player:sendEvent('KillPlayer')
				elseif mode == "ground" then
					player:teleport(spot.cellName, spot.pos, { rotation = player.rotation, onGround = true })
				else
			...
			lastCellIds[player.id] = currentCell.id
		end
	end
end
