...
tooltips["isCave"] 				= "25% increase - caves"
tooltips["isMine"] 				= "50% increase - mine"
tooltips["isTomb"] 				= "25% increase - tomb"
tooltips["isSewer"] 				= "150% increase - Sewers and Underworks"
tooltips["isTemple"] 				= "50% decrease - Temple"
tooltips["isDaedric"] 				= "25% increase - Daedric Ruin"
tooltips["isDwemer"] 				= "50% increase - Dwemer Ruin"

-- weather modifier tooltips
tooltips["ash"]				= "300% increase - ash storm"
tooltips["ashstorm"]		= "300% increase - ash storm"
tooltips["blight"] 			= "400% increase - blight storm"
tooltips["blizzard"]		= "40% increase - blizzard"
tooltips["snow"]			= "25% increase - snow"
...
-- Ingredient soap (can be consumed AND interacted with in overworld)
local ingredientSoap = "ingred_sload_soap_01"

BUGMUSK_CHARGES = 3
TOWEL_CHARGES = 5
SOAP_CHARGES = 5
BATHPRODUCT_CHARGES = 1

local function getMaxCharges(productType)
...
local bugMuskId = "potion_t_bug_musk_01" -- spell is sd_c_sp_bath_t_bug_musk
...
local function matchesBathProduct(recordId)
	for productType, productData in pairs(bathProducts) do
		...
		elseif productData.pattern then
			-- Single pattern
			if recordId:find(productData.pattern) then
				return productType, productData
			end
		end
	end
...
local function applyCleanBuff()
	...
	if level >= 3 then
		...
		for _, s in pairs(active) do
			local spellId = s.id
			if spellId:find("^sd_c_sp_soap_") or spellId:find("^sd_c_sp_bath_") then
...
local function applyBathProductBuff(productType, productData)
	...
	ui.showMessage(productData.message)
...
local function applyBugMuskBuff()
	...
	ui.showMessage("The Telvanni bug musk enhances your bath!")
...
-- refill and bathe ? AB_Furn_Barrel01Water ; AB_Furn_ComBucket02Water ; T_Imp_FurnR_Basin_02_w ; T_Imp_FurnR_Basin_01 ; T_Com_Var_BucketWater_01
-- bathe AB_Furn_WashBasin
--[[
T_De_Furn_Basin_01
T_Com_Furn_Basin_01
...
local function performBathing(usedSoap, soapItem, hasBugMusk)
	...
	if houseType and houseBathBuffs[houseType] then
		...
		if core.magic.spells.records[buffId] then
			...
			ui.showMessage("The "..houseType:sub(1,1):upper()..houseType:sub(2).." bath invigorates you!")
		end
	end
	...
	ui.showMessage("You feel clean and refreshed.")
	updateWidget()
end
...
local function findBugMuskInInventory()
	...
	for _, item in pairs(inventory:getAll()) do
		if item.recordId == bugMuskId then
			return item
		end
	end
	return nil
...
local function incrementItemUses(itemId, productType)
	...
	return cleanData.itemUses[itemId] >= getMaxCharges(productType)
end
...
local function handleBathingItemUse(item, wasConsumed)
	...
	log(3, "[SunsDusk:Clean] handleBathingItemUse - checking if soap:", allSoapItems[itemId])
	...
	if allSoapItems[itemId] then
		local _, reason = canBathe()
		if not eligible then
			if reason == "clothing" then
				ui.showMessage("You need to remove armor to bathe (helmet and boots are fine).")
			else
				if G_STARWIND_INSTALLED then
					ui.showMessage("You need to be wet, in water or in a bath to use soap.")
				else
					ui.showMessage("You need to be in water or in a bath to use soap.")
				end
			end
			if wasConsumed then
				async:newUnsavableSimulationTimer(0.1, function()
	...
	if isTowelItem(itemId) then
		...
		if not isWearingBathingClothing() then
			...
			ui.showMessage("You need to remove armor to dry off (helmet and boots are fine).")
			if wasConsumed then
				async:newUnsavableSimulationTimer(0.1, function()
					core.sendGlobalEvent("SunsDusk_addItem", {self, itemId, 1})
				end)
		...
		--
		--if depleted then
		--	ui.showMessage("The towel is too worn to use again.")
		--	resetItemUses(itemId)
		...
		--	end
		--else
		--	ui.showMessage("You dry yourself thoroughly with the towel. (" .. usesRemaining .. " uses remaining)")
	...
	if itemId == bugMuskId then
		if not eligible then
			local _, reason = canBathe()
			if reason == "clothing" then
				ui.showMessage("You need to remove armor to infuse the bath (helmet and boots are fine).")
			else
				if G_STARWIND_INSTALLED then
					ui.showMessage("You need to be wet, in water or in a bath to use bug musk.")
				else
					ui.showMessage("You need to be in water or in a bath to use bug musk.")
				end
			end
		...
		if dirtLevel > 2 then
			ui.showMessage("You are too dirty for bug musk to cover your stench ...")
	...
	if productType and productData then
		...
		handleItemTracking(item, wasConsumed)
		return true
	end
end
...
local function raycastChanged()
	...
	if G_raycastResult then
		...
		log(3, "[SunsDusk:Clean] G_raycastResult is nil")
	end
	
	-- Check if looking at a soap or towel item in the world
	local hitRecordId = G_raycastResult and G_raycastResult.hitObject and G_raycastResult.hitObject.recordId
	log(3, "[SunsDusk:Clean] hitRecordId:", hitRecordId)
	
	local isLookingAtSoap = hitRecordId and (allSoapItems[hitRecordId] or allSoapItems[hitRecordId])
	...
	local isLookingAtBugMusk = hitRecordId == bugMuskId
	log(3, "[SunsDusk:Clean] isLookingAtBugMusk:", isLookingAtBugMusk)
	
	local isLookingAtCleanItem = isLookingAtSoap or isLookingAtTowel or isLookingAtBugMusk
	log(3, "[SunsDusk:Clean] isLookingAtCleanItem:", isLookingAtCleanItem)
	
	if isLookingAtCleanItem and not saveData.playerInfo.isInWerewolfForm then
		...
		if hudVisible then
			...
			local validIconHsv = {rgbToHsv(TOOLTIP_FONT_COLOR)}
			...
			local invalidIconHsv = {rgbToHsv(TOOLTIP_FONT_COLOR)}
			...
			local textColor = actualEligible and TOOLTIP_FONT_COLOR or invalidIconRgb
			...
			local maxUses = getMaxCharges(productType)
			...
			line1.content:add{
				type = ui.TYPE.Image,
				props = {
					resource = getTexture("textures/SunsDusk/F_symbolic.dds"),
					tileH = false,
					tileV = false,
					size  = v2(TOOLTIP_FONT_SIZE,TOOLTIP_FONT_SIZE),
					alpha = 0.6,
					color = iconColor,
				}
			}
...
local function updateDirt()
	...
	if buffLevel ~= currentLevel then
		...
		if currentLevel == 2 and (buffLevel == 0 or buffLevel == 1 or buffLevel == nil) then
			ui.showMessage("You are feeling dirty and need to bathe.")
		end
	end
	...
	if cleanData.daysSinceLastBath >= CLEAN_DISEASE_DAYS and not cleanData.diseaseApplied then
		...
		ui.showMessage("Your poor hygiene has made you sick!")
...
-- Per-hour update (perHourJobs only get clockHour as argument)
local function module_clean_hour(clockHour)
    log(5, "[SunsDusk:Clean] module_clean_hour called, clockHour:", clockHour)
    if not NEEDS_CLEAN then return end
    if not cleanData then return end
    ...
    local increaseRate = (1 / totalHoursToMaxDirt) * getDirtinessModifier()
    local oldDirt = cleanData.dirt
    cleanData.dirt = math.min(1, cleanData.dirt + increaseRate)
    cleanData.daysSinceLastBath = cleanData.daysSinceLastBath + (1 / 24)
...
table.insert(G_perHourJobs, module_clean_hour)
