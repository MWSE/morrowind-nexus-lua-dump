...
local function getMagicTemperatureModifier(currentTemp, targetTemp)
	...
	if racialData.wetComfortDrift then
		...
		if isWetOrSwimming then
			...
			if not inColdClimate and waterTemp > 5 then
				...
				tempDbg.p(13, string.format("  %s: wet comfort drift: +%i resistances and +-%s", 
	...
	if G_currentBed then
		...
		tempDbg.p(13, string.format("  %s: bed comfort drift: +%i resistances and +-%s", 
...
local function getWaterTemperatureModifier(currentPlayerTemp, currentEnvironmentTemp)
	...
	if isInRain and rainIntensity > 0 and exposedFraction > 0 then
		...
		totalWaterExposure = totalWaterExposure * (1-equipmentData.hasUmbrella)
	...
	if wetness > 0.05 and exposedFraction > 0 then
		...
		evaporativeCooling = evaporativeCooling * exposedFraction
	...
	if isInRain and rainIntensity > 0 and exposedFraction > 0 then
		...
		rainCooling = rainCooling * exposedFraction
	...
	if G_isInWater > 0 then
		...
		local waterModifier = waterAirDiff * submersionLevel * waterStrength
		...
		if waterModifier ~= 0 then
			tempDbg.p(12, string.format("Water mod: %s, Rate: x%.2f", (((waterModifier)>=0) and "+" or "")..formatTemperatureModifier(waterModifier), waterRateMult))
		end
...
function calculateEnvironmentTemperature()
	...
	-- ═══════════════════════════════════════════════════════════════════
	if NERF_FIRE_SHIELD or fireShield <= 0 then
...
local function onConsumedWater(liquid, remainingWater)
	...
	if remainingWater > 0 then
		if remainingWater >=0.99 then
			...
			ui.showMessage("You pour water on your head")
			module_temp_minute(nil,  0)
			return 0
		else
			ui.showMessage("Your thirst is quenched")
		end
	end
