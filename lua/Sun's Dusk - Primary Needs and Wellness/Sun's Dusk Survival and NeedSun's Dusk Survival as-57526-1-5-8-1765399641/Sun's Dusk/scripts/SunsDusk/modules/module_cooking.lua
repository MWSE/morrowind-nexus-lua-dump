...
function updateMealPreview()
	...
	if not cookingWithInnkeeper then
		...
		ingredientRow.content:add{
			...
			type = ui.TYPE.Text,
			props = {
				text = "Water ("..waterAmountStr.." / "..COOKING_WATER_MULT*250 .."ml)", 
				textColor = G_morrowindGold,
				textShadow = true,
				textShadowColor = util.color.rgb(0,0,0),
...
function G_isCookingActivator(object, objectType)
	...
	local dbEntry = dbStatics[object.recordId] and dbStatics[object.recordId].cookingspot
	if dbEntry ~= nil then
...
input.registerTriggerHandler('Activate', async:callback(function()
    if not SDCookingUI and (cookingTooltip or G_raycastResult and G_raycastResult.hitObject and G_raycastResult.hitObject.recordId:match( "^sd_wood_(%d+)_lit$")) then
        makeCookingUi()
...
local function raycastChanged()
	local isCookingActivator = G_raycastResultType and G_isCookingActivator(G_raycastResult.hitObject, G_raycastResultType)
	if isCookingActivator and I.UI.isHudVisible() and not saveData.playerInfo.isInWerewolfForm then
...
--table.insert(G_onFrameJobsSluggish,checkForStartCookingBuff)
table.insert(G_sluggishScheduler[5],checkForStartCookingBuff)
G_onFrameJobsSluggish.checkForStartCookingBuff = checkForStartCookingBuff

local function onConsume(item)
	local record = item.type.record(item)
	if record.name:sub(1, 7) == " Stew [" then
