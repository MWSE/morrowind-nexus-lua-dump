-- Tent Building
-- Note: G_tentStages, G_tentUpgrades, and G_tentDowngrades are defined in constants.lua

local sparklingWood = {}
local replaceFirewood

-- world vfx needs both spawn and remove (0.51+)
...
G_onUpdateJobsSluggish.camping = function(now)
...
G_onObjectActiveJobs.camping = function(object)
	...
	if not types.Miscellaneous.objectIsInstance(object) then return end
	if object.recordId:sub(1,8) == "sd_wood_" then
	...
	if object.recordId == "sd_campingitem_tent" or object.recordId == "sd_campingitem_tent_perm" then
		table.insert(G_delayedUpdateJobs, {
			3,
			function()
				if not object:isValid() or object.count < 1 or not object.cell then return end
				...
				if nearestPlayer then
					...
					position = nearestPlayer.position + dir * 100 + v3(0,0,10)
				end
				
				local isPerm = object.recordId == "sd_campingitem_tent_perm"
				local rotation = object.rotation
				object:remove()
				
				if isPerm then
					-- complete tent with bedroll
					local tent = world.createObject("sd_tent_4")
					tent:teleport(cell, position + v3(0,0,10), {rotation = rotation, onGround = true})
					tent:setScale(0.65)
					
					local bedroll = world.createObject("sd_campingobject_bedrolltent", 1)
					bedroll:teleport(cell, position + v3(0,0,3), {rotation = rotation, onGround = true})
					bedroll:setScale(1.09)
					
					saveData.builtTents = saveData.builtTents or {}
					saveData.builtTents[tent.id] = {
						tent = tent,
						stage = 4,
					...
					tent:teleport(cell, position, {rotation = rotation, onGround = true})
					tent:setScale(1.5)
					
					local bedrollRotation = rotation * util.transform.rotateZ(math.rad(180))
					local yaw = rotation:getYaw()
					local bedrollPos = position + v3(math.sin(yaw), math.cos(yaw), 0) * 38 - v3(0,0,10)
					
					local bedroll = world.createObject("sd_campingobject_bedrolltent", 1)
					bedroll:teleport(cell, bedrollPos, {rotation = bedrollRotation, onGround = true})
	...
	elseif object.recordId == "sd_campingitem_tanningrack" or object.recordId == "sd_campingitem_workbench" then
		table.insert(G_delayedUpdateJobs, {
			3,
			function()
				if not object:isValid() or object.count < 1 or not object.cell then return end
				...
				-- place in front of nearest player
				local position = object.position
				local nearestPlayer
				local shortestDistance = math.huge
				for _, player in pairs(cell:getAll(types.Player)) do
				...
				if nearestPlayer then
					local yaw = nearestPlayer.rotation:getYaw()
					local dir = v3(math.sin(yaw), math.cos(yaw), 0):normalize()
					position = nearestPlayer.position + dir * 100 + v3(0,0,10)
				end
				
				local isTanningRack = object.recordId == "sd_campingitem_tanningrack"
				local placedId = isTanningRack and "sd_campingobject_tanningrack" or "sd_campingobject_workbench"
				
				local rotation = object.rotation
				object:remove()
				
				local placed = world.createObject(placedId, 1)
				placed:teleport(cell, position, {rotation = rotation, onGround = true})
				if isTanningRack then
					placed:setScale(0.5)
				end
			end
		})
...
--  crafting station break down
local stationReturns = {
	sd_campingobject_tanningrack = "sd_campingitem_tanningrack",
	sd_campingobject_workbench   = "sd_campingitem_workbench",
}

local function breakDownStation(data)
	local player = data[1]
	local object = data[2]
	if not object or not object:isValid() then return end
	local returnId = stationReturns[object.recordId]
	if not returnId then return end
	local pos = object.position
	object:remove()
	if player then
		world.createObject(returnId, 1):moveInto(types.NPC.inventory(player))
		player:sendEvent("SunsDusk_playSound", "Item Misc Up")
	end
	if useVfx then
		world.vfx.spawn(
			"meshes/e/magic_hit_conjure.nif",
			pos - v3(0, 0, 10),
			{scale = 0.4, useAmbientLight = false}
		)
	end
end

-- ======================================
-- Tent Building Event Handlers
-- ======================================

local function upgradeTent(data)
	local player = data[1]
	local object = data[2]
	local upgradeIndex = data[3] or 1
	
	local upgrades = G_tentUpgrades[object.recordId]
	if not upgrades or not upgrades[upgradeIndex] then return end
	local upgrade = upgrades[upgradeIndex]
	
	-- consume from inventory
	local inv = types.NPC.inventory(player)
	if upgrade.component then
		local component = inv:find(upgrade.component)
		if not component then
			local name = G_tentComponentNames[upgrade.component] or "a tent component"
			player:sendEvent("SunsDusk_messageBox", {2, "You are missing " .. name .. "."})
			return
		end
		component:remove(1)
	end
	
	local currentStage = G_tentStages[object.recordId] or 0
	local offset
	if currentStage == 0 then
		offset = 10
		if object.count > 1 then
			world.createObject(object.recordId, object.count - 1):moveInto(inv)
			player:sendEvent("SunsDusk_playSound", "Item Misc Up")
		end
	end
	...
	local pos = object.position
	if offset then
		pos = pos + v3(0,0,offset)
	end
	local cell = object.cell
	local rotation = object.rotation
	local oldId = object.id
	
	local prevTracking = saveData.builtTents and saveData.builtTents[oldId]
	if prevTracking then
		saveData.builtTents[oldId] = nil
	end
	
	object:remove(1)
	
	local resultId = upgrade.result
	local upgradedObject = world.createObject(resultId)
	upgradedObject:teleport(cell, pos, {rotation = rotation, onGround = true})
	upgradedObject:setScale(0.65)
	
	-- track the tent, carry flawless state through rebuilds
	saveData.builtTents = saveData.builtTents or {}
	local newStage = G_tentStages[resultId] or 1
	local wasStartedComplete = prevTracking and (prevTracking.complete or prevTracking.startedComplete) or false
	local flawless = prevTracking and prevTracking.flawless
	if newStage >= 4 and flawless == nil then
		flawless = true
	end
	saveData.builtTents[upgradedObject.id] = {
		tent = upgradedObject,
		stage = newStage,
		time = world.getGameTime(),
		complete = newStage >= 4 or nil,
		flawless = flawless,
		startedComplete = wasStartedComplete or nil,
	}
	
	-- spawn bedroll inside completed tent
	if newStage >= 4 then
		local bedroll = world.createObject("sd_campingobject_bedrolltent", 1)
		bedroll:teleport(cell, pos-v3(0,0,7), {
			rotation = rotation,
			onGround = true
		})
		bedroll:setScale(1.09)
		saveData.builtTents[upgradedObject.id].bedroll = bedroll
	end
	...
	if useVfx then
		world.vfx.spawn(
			"meshes/e/magic_hit_conjure.nif",
			pos - v3(0, 0, 10),
			{scale = 0.3, useAmbientLight = false}
		)
	end
end

local function damageTent(data)
	local player = data[1]
	local object = data[2]
	local returnItem = data[3]
	
	if not object or not object:isValid() or object.count < 1 then return end
	
	local info = G_tentDowngrades[object.recordId]
	if not info or not info.result then
		destroyBuiltTent(data)
		return
	end
	...
	-- read teardown state before removing tracking
	local flawless = nil
	local startedComplete = false
	if saveData.builtTents and saveData.builtTents[oldId] then
		local tbl = saveData.builtTents[oldId]
		flawless = tbl.flawless
		startedComplete = tbl.complete or tbl.startedComplete or false
		if tbl.bedroll and tbl.bedroll:isValid() and tbl.bedroll.count > 0 then
			tbl.bedroll:remove()
		end
		saveData.builtTents[oldId] = nil
	end
	if not returnItem and flawless ~= nil then
		flawless = false
	end
	
	object:remove()
	
	-- return the removed component to the player
	if returnItem and info.returns and player then
		world.createObject(info.returns, 1):moveInto(types.NPC.inventory(player))
		player:sendEvent("SunsDusk_playSound", "Item Misc Up")
	end
	
	local downgradedObject = world.createObject(info.result)
	downgradedObject:teleport(cell, pos, {rotation = rotation})
	downgradedObject:setScale(0.65)
	
	-- track the downgraded tent, carrying teardown state forward
	saveData.builtTents = saveData.builtTents or {}
	local newStage = G_tentStages[info.result] or 1
	saveData.builtTents[downgradedObject.id] = {
		tent = downgradedObject,
		stage = newStage,
		time = world.getGameTime(),
		flawless = flawless,
		startedComplete = startedComplete or nil,
	}
	...
	if useVfx then
		world.vfx.spawn(
			"meshes/e/magic_hit_conjure.nif",
			pos - v3(0, 0, 10),
			{scale = 0.2, useAmbientLight = false}
		)
	end
end

local function destroyBuiltTent(data)
	local player = data[1]
	local object = data[2]
	local returnItem = data[3]
	...
	-- flawless teardown from a complete tent: consume components, reward packed tent
	local flawlessDone = false
	if startedComplete and flawless and player then
		local inv = types.NPC.inventory(player)
		local skip = info and info.returns
		local components = {
			"sd_campingitem_sticks",
			"sd_campingitem_rope",
			"sd_campingitem_tarp",
			"sd_campingitem_bedroll",
		}
		local allFound = true
		for _, compId in ipairs(components) do
			if compId ~= skip and not inv:find(compId) then
				allFound = false
				break
			end
		end
		if allFound then
			for _, compId in ipairs(components) do
				if compId ~= skip then
					inv:find(compId):remove(1)
				end
			end
			world.createObject("sd_campingitem_tent_perm", 1):moveInto(inv)
			player:sendEvent("SunsDusk_playSound", "Item Misc Up")
			player:sendEvent("SunsDusk_messageBox", {1, "Breaking camp was a success. Flawless!"})
			player:sendEvent("SunsDusk_playSound", "skillraise")
			flawlessDone = true
		end
	end
	
	if not flawlessDone and returnItem and info and info.returns and player then
		world.createObject(info.returns, 1):moveInto(types.NPC.inventory(player))
		player:sendEvent("SunsDusk_playSound", "Item Misc Up")
	end
...
local function isTentObject(object)
	if not object then return false end
	return G_tentStages[object.recordId] ~= nil
end
...
G_onUpdateJobs.woodcutting = function(dt)
local GRAVITY = -627
	...
	for woodId, woodData in pairs(saveData.fallingWood) do
		...
		if woodObject and woodObject:isValid() and not woodObject.id:find("deleted") then --and woodObject.enabled then
			if woodObject.cell then
				...
				elseif woodData.phase == "settling" then
					...
					if woodData.settleTimer >= SETTLE_TIME then
						...
						if WOOD_SPARKLES and useVfx then
							world.vfx.spawn(
			...
			-- object was destroyed or disabled, stop tracking
			saveData.fallingWood[woodId] = nil
		end
	end
end
...
local function removeTree(data)
	...
	if not tree or tree.count < 1 then
		return
	end
...
local function spawnSpriggan(data)
	...
	-- try using spriggan from TD leveled list
	...
	for _, creatureRecord in pairs(types.Creature.records) do
		if (creatureRecord.id:find("spriggan") or creatureRecord.name:lower():find("spriggan")) 
		and not creatureRecord.id:find("unique") 
		and not creatureRecord.id:find("bm_spriggan_co") then
...
-- ======================================
-- Woodcutting VFX and looting Campfires
-- ======================================
...
function replaceFirewood(object)
	if not object:isValid() or object.count < 1 then return end
	if object.cell and (object.count > 1 or object.recordId == "sd_wood_publican" or object.recordId == "sd_wood_merchant") then
...
G_onUpdateJobsSluggish.campfires = function(now)
...
G_eventHandlers.SunsDusk_upgradeTent          = upgradeTent
G_eventHandlers.SunsDusk_damageTent           = damageTent
G_eventHandlers.SunsDusk_destroyTent          = destroyBuiltTent
G_eventHandlers.SunsDusk_breakDownStation     = breakDownStation