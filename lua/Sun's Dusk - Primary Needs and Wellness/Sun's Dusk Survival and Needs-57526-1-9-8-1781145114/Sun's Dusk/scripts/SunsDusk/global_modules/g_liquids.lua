...
function resolveWaterType(cell, position)
	...
	if not cell.isExterior then
		...
		if info.isExterior then
			...
			-- dbStatics override wins
			if info.waterType then waterType = info.waterType
			elseif info.isSewer then waterType = "susWater"
...
-- fast-path open-vessel test: a tuned offsets entry is always an open drinkware vessel.
-- false entries are blacklist (not an open-vs-closed signal), so fall through.
-- handles converted potion records by resolving back to the original misc id.
local function isOpenVessel(rec)
	if not rec then return false end
	local id = rec.id
	local rev = saveData.reverse and saveData.reverse[id]
	if rev then
		id = rev.orig
		rec = types.Miscellaneous.records[id]
		if not rec then return false end
	end
	if vesselOffsets[id] then return true end
	return isVesselRecord(rec, false) and true or false
end
...
function ensurePotionFor(origId, q, liquidKey)
	...
	-- mesh swap: certain (origId, liquidKey) pairs use a pre-filled mesh instead of the empty cup model
	local subTable = G_potionMeshSubstitutes and G_potionMeshSubstitutes[origId]
	local substituteMesh = subTable and subTable[liquidKey or 'water']
	local modelToUse = (miscRec.model and miscRec.model ~= '') and miscRec.model or tmpl.model
	if substituteMesh then
		modelToUse = substituteMesh
	end
	...
	saveData.reverse[newId] = { orig = origId, q = q, liquid = liquidKey or 'water', meshSwapped = substituteMesh ~= nil or nil }
...
-- spawn a beverage object and stamp its timestamp once the mwscript is live
function G_createBeverage(id, quantity)
...
local createObject = G_createBeverage
...
-- player may be nil for non-drink consumption (brewing etc.); skips the consumedWater event.
local function downgradeWorldConsumable(data)
	local player = data[1]
	local object = data[2]
	local steps  = data[3] or 1   -- 250ml units to consume
	...
	-- drop stale vfx tied to the doomed object
	if saveData.consumableVfx[object.id] then
		G_cleanupVfxEntry(saveData.consumableVfx[object.id])
...
local function refillSpillables(data)
	...
	for _, item in ipairs(inv:getAll(Potion)) do
		if item:isValid() and item.count > 0 then
			...
			if rev then
				local origId = rev.orig
				local isOpen = isOpenVessel(Misc.records[origId])
...
local function refillSingleBottle(data)
	...
	if rec then
		-- tuned offsets entry is a known open vessel, skip the substring scans
		local isVessel = isOpenVessel(rec) or isVesselRecord(rec, true)
		if isVessel then
			local origId = rec.id
			local fullId = ensurePotionFor(origId, resolveMaxQ(origId), liquidType)
...
-- 0.161 baked into drinkwareOffsets min/maxScale; inverse keeps steam at its pre-bake size
local STEAM_SCALE_MULT = 1 / 0.161

G_cleanupVfxEntry = function(data)
...
G_onObjectActiveJobs.liquids = function(object)
	...
	-- mesh-swapped records carry their own filled mesh; no liquid/steam vfx needed
	if rev.meshSwapped then return end
		...
		if useVfx then
			...
			if data.scale then
				if data.steamStatic then
					data.steamStatic:setScale(data.scale * STEAM_SCALE_MULT)
				end
				if data.static then
					data.static:setScale(data.scale)
				end
			end
			return
		end
	...
	if vesselData ~= false then
		...
		if useVfx then
			...
			waterStatic:setScale(finalScale)
		end
				...
				if ageInHours < 3 then
					if useVfx then
						...
						world.vfx.spawn(steamModel, object.position + finalOffset, {
							loop = true,
							vfxId = steamVfxId,
							scale = finalScale * STEAM_SCALE_MULT,
							useAmbientLight = false,
						})
						...
						steamStatic:setScale(finalScale * STEAM_SCALE_MULT)
					end
				end
			end
		end
...
G_onUpdateJobsSluggish.cookingLootDetector = function(now)
...
G_onUpdateJobsSluggish.liquidsSteamAgeChecker = function(dt)
	local currentTime = core.getGameTime()
