...
local function createStew(data)
	...
	for _, batch in ipairs(batches) do
		...
		local foodWeight = 0.3
		if batch.foodwareRecordId and not foodData.virtualFoodware then
			local rec = types.Miscellaneous.records[batch.foodwareRecordId]
			if rec then
				foodWeight = foodWeight + rec.weight
			end
		end
		...
		saveData.stewRegistry[rec.id] = {
			...
			isCookedMeal    = true,
			virtualFoodware = foodData.virtualFoodware or nil,
		}
...
G_onObjectActiveJobs.cooking = function(object)
	...
	if foodwareAdjust then
		...
		print("[cooking] bbox fallback: recipe=" .. tostring(stewData.recipeId) .. " foodware=" .. tostring(stewData.foodwareRecordId) .. " scale=" .. tostring(foodwareScale) .. " offset=" .. tostring(foodwareOffset))
	end
...
--G_onUpdateJobsSluggish.cooking = function(dt)
...
local function returnContainer(data)
	...
	if stewData and stewData.foodwareRecordId and not stewData.virtualFoodware then
		local inv = types.Actor.inventory(player)
