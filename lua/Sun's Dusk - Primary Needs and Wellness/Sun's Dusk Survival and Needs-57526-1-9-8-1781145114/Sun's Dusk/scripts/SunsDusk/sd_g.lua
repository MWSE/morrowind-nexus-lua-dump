...
local onUpdateJobsSluggishIterator = nil
...
local function onUpdate(dt)
	...
	onUpdateJobsSluggishIterator, job = next(G_onUpdateJobsSluggish, onUpdateJobsSluggishIterator)
	if onUpdateJobsSluggishIterator then
		job(now)
	end
...
--end)
--
--I.ItemUsage.addHandlerForType(types.Ingredient, function(item, actor)
--	local res, typ = I.SunsDusk.isConsumable(item)
--	if typ == "cooked" or typ == "database" and not res.isGreenPact then
--		return false
--	end
...
G_eventHandlers["PerfectPlacement:Begin"] = function(e)
	local object = e.activeObj
	if not object then return end
	ppGrabbedObject = object
end

G_eventHandlers["PerfectPlacement:Move"] = function(e)
	if ppGrabbedObject then
		movement = { activeObj = e.activeObj, position = e.newPosition, rotation = e.newRotation }
	end
	local object = ppGrabbedObject
	if not object or not object:isValid() or object.count < 1 then return end
	if saveData and saveData.consumableVfx then
		local vfxData = saveData.consumableVfx[object.id]
		if vfxData then
			if vfxData.vfxId then world.vfx.remove(vfxData.vfxId) end
			if vfxData.steamVfxId then world.vfx.remove(vfxData.steamVfxId) end
			if vfxData.static and vfxData.static:isValid() and vfxData.static.count > 0 then
				vfxData.static:remove()
			end
			if vfxData.steamStatic and vfxData.steamStatic:isValid() and vfxData.steamStatic.count > 0 then
				vfxData.steamStatic:remove()
			end
...
G_eventHandlers["PerfectPlacement:End"] = function(e)
	local object = ppGrabbedObject
	ppGrabbedObject = nil
	if not object or not object:isValid() or object.count < 1 then return end
	G_delayedUpdateJobs["pp_vfx_respawn"] = {
		2,
		function()
...
-- --------------------- inventory extender world placement ----------------------

-- inventory extender re-teleports the placed item one frame later
-- to adjust for its bounding box, so we have to wait before respawning vfx
G_eventHandlers["IE_PostTeleport"] = function(e)
	local object = e.obj
	if not object or not object:isValid() or object.count < 1 then return end
	G_delayedUpdateJobs["mi_vfx_respawn_"..object.id] = {
		2,
		function()
			if not object:isValid() or object.count < 1 then return end
			if saveData and saveData.consumableVfx then
...
G_eventHandlers.Roguelite_startVampirism               = checkIfVampireWerewolf
...
G_eventHandlers.SunsDusk_Vaermina_triggerFade = function(player)
	world.mwscript.getGlobalVariables(player).sd_blackout_toggle = 1
end

-- esp globals for mwscript/esp mod compatibility, written from the player's push
G_eventHandlers.SunsDusk_setCookingGlobal = function(data)
	local gv = world.mwscript.getGlobalVariables(data[1])
	gv.sunsdusk_installed = 1
	gv.sunsdusk_cookingskill_current = data[2]
end

-- settings export/import: menu buttons can only reach global, so relay to the player ui
G_eventHandlers.SunsDusk_SettingsIO = function(data)
	for _, player in pairs(world.players) do
		player:sendEvent("SunsDusk_SettingsIO", data)
	end
end

-- holds off the global preset cascades while an imported snapshot is being written
G_eventHandlers.SunsDusk_SetImportFlag = function(data)
	G_importingSettings = data and data.on or false
end
...
return {
	...
	interface = {
		...
		isConsumable = function(obj)
			local recordId = type(obj) == "string" and obj or obj.recordId
			if not dbConsumables then
				require('scripts.SunsDusk.spreadsheetParser')
			end
			local typ = nil
			local ret = nil
			if saveData.stewRegistry[recordId] then
				typ = "cooked"
				ret = saveData.stewRegistry[recordId]
				--{
				--	timestamp = timestamp,
				--	recipeId = recipeId,
			...
			elseif dbConsumables[recordId] then
				...
				--	ingredientClass = (f() or ""):lower(),
				--	ingredientRank = tonumber(f()) or 1 or nil,
				--	alchEffect1 = e ~= "" and (spellDiffs[e] or e) or nil,
				--	alchEffect2 = e ~= "" and (spellDiffs[e] or e) or nil,
				--	alchEffect3 = e ~= "" and (spellDiffs[e] or e) or nil,
				--}
			elseif saveData.reverse[recordId] then
				typ = "drink"
				ret = saveData.reverse[recordId] 
				--{ 
				--	orig = origId, 
				--	q = q, 
				--	liquid = liquidKey or 'water' 
				--}
			end
			return ret, typ
		end,
