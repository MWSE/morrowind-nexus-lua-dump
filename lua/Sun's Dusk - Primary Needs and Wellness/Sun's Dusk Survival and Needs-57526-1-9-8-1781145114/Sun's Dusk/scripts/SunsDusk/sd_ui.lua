...
local function calculateLeftShift()
	...
	for _, spellInstance in pairs(typesActorActiveSpellsSelf) do
		local activeSpellId = spellInstance.activeSpellId
		if not iconCountCache[activeSpellId] then
			iconCountCache[activeSpellId] = {}
			for _, effectInstance in pairs(spellInstance.effects) do
				if effectInstance.id then
					table.insert(iconCountCache[activeSpellId], effectInstance.id)
				end
			end
		end
		for _, effectInstance in pairs(iconCountCache[activeSpellId]) do
			iconMap[effectInstance] = true
		end
	end
