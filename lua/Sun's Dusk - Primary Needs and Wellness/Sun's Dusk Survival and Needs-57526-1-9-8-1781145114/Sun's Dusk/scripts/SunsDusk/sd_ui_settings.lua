...
uiSettingsTemplate.MOD_INTEGRATION = {
	...
	settings = {
		...
		{
			key = "LIQUID_NAME_FORMAT",
			name = "Liquid format",
			description = "How a container's fill level reads in Inventory Extender.\n\nCharges: (3/4)\nWords/hybrid: (half), else (3/4)\nUnchanged: the original (750 mL/1L Water) name.\n\nRequires List Mode in Inventory Extender.",
			default = "Words/hybrid (half), (3/4)",
			renderer = RENDERER_SELECT,
			argument = {
				l10n = "none",
				items = { "Charges (3/4)", "Words/hybrid (half), (3/4)", "Unchanged (750 mL/1L)" },
				width = 240,
			},
		},
		{
			key = "LIQUID_INFO_DISPLAY",
			name = "Liquid display",
			description = "Where a container's fill amount appears.\n\nHidden: never shown.\nOnly tooltips: only display liquid info in tooltips - mL of water is only displayed in the tooltips, not in the name that appears in the rows for the list mode.\nUnchanged: shown in both rows and tooltips.\n\nRequires Inventory Extender.",
			default = "Unchanged",
			renderer = RENDERER_SELECT,
			argument = {
				l10n = "none",
				items = { "Hidden", "Only tooltips", "Unchanged" },
				width = 180,
			},
		},
		{
