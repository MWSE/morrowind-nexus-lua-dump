...
local function getBedRank()
	...
	elseif G_currentBed.owner.factionId and types.NPC.getFactionRank(self, G_currentBed.owner.factionId) == 0 then
		return 1.5
	elseif G_currentBed.owner.factionId and types.NPC.getFactionRank(self, G_currentBed.owner.factionId) < (G_currentBed.owner.factionRank or 0) then
		return 1.5
	end
	return 2.9
end
...
local function getRestorationPerHourOfSleep()
	local endurance = typesPlayerStatsSelf.endurance.modified --const float endurance = stats.getAttribute(ESM::Attribute::Endurance).getModified();
	local health = 0.1 * endurance --const float health = 0.1f * endurance;
	local magicka = fRestMagicMult * typesPlayerStatsSelf.intelligence.modified --const float magicka = fRestMagicMult * stats.getAttribute(ESM::Attribute::Intelligence).getModified();
	local strength = typesPlayerStatsSelf.strength.modified
	...
	magicka = magicka * (1 - typesActorActiveEffectsSelf:getEffect(core.magic.EFFECT_TYPE.StuntedMagicka).magnitude)
	--log(typesActorActiveEffectsSelf:getEffect(core.magic.EFFECT_TYPE.StuntedMagicka).magnitude)
	
	return health, fatigue, magicka
end

local function updateWidget()
	if not sleepData or S_HIDE then return end
	...
	-- path and the separate sleeping-profile widget below are skipped
	if skinData.content then
		local ctx = {
			skin = skinData,
			value = sleepData.tiredness,
			buff = sleepData.currentTirednessBuff,
			sleepingProfile = sleepData.currentSleepingProfile,
			wellRestedPool = sleepData.wellRestedPool,
		}
		local widget = G_renderContentSkin({ name = "m_sleep", order = "needs-sleep" }, ctx)
	...
	if sleepData.currentSleepingProfile and S_SP_DISPLAY then
		...
		if lastSleepingProfile ~= sleepData.currentSleepingProfile then
			...
			if profileWidget.layout.userData.order ~= profileOrder then
				profileWidget.layout.userData.order = profileOrder
				G_columnsNeedUpdate = true
			end
			lastSleepingProfile = sleepData.currentSleepingProfile
		...
		-- update tooltip
		local tooltipStr = not sleepData.currentSleepingProfile and "Loading..." or tooltips[sleepData.currentSleepingProfile] or "Error: "..tostring(sleepData.currentSleepingProfile)
	...
	if sleepData.currentSleepingProfile ~= "insomniac" then
		...
		local alphaInput = HUD_INVERT_ALPHA and (1 - sleepData.tiredness) or sleepData.tiredness
		local currentAlpha = HUD_ALPHA == "Static" and 1 or getWidgetAlpha(alphaInput)
		if S_HIDE_NO_BUFF then
			if sleepData.currentTirednessBuff == false then
				-- no active effect for the current mode (false only happens in restricted modes): hide
				currentAlpha = 0
			elseif NEEDS_TIREDNESS_BUFFS_DEBUFFS ~= "Buffs and debuffs" then
		...
		local bgAlpha = S_BACKGROUND == "Classic" and (HUD_ALPHA == "Static" and 1 or currentAlpha^2) or (currentAlpha > 0 and 0.5 or 0)
		-- Determine texture
		...
		if lastTirednessAlpha ~= currentAlpha then
			...
			if ((lastTirednessAlpha or 1) > 0) ~= (currentAlpha > 0) then
				G_columnsNeedUpdate = true
			end
			lastTirednessAlpha = currentAlpha
			tirednessWidget.layout.userData.effectiveAlpha = currentAlpha
			needsUpdate = true

			-- Update background alpha for all styles
			if tirednessBackground and tirednessBackground.props then
				tirednessBackground.props.alpha = bgAlpha
			end
		end
		
		-- Update well rested icon visibility
		local wellRestedVisible = sleepData.wellRestedPool > 0 and not S_SP_HIDE_WR and math.min(1, 0.3 + sleepData.wellRestedPool*2*0.7)*currentAlpha or 0
...
local function module_sleep_minute(clockHour, minute, minutesPassed)
	...
	if G_isLongTravel then
		...
		sleepData.recentSleepBits = 0
		return
	elseif G_isSleeping then
		sleepData.tiredness = math.max(0, sleepData.tiredness - HOURS_PER_SLEEP/HOURS_PER_RESTED_STATE/6/60*minutesPassed)
		log(6, tostring(minutesPassed).."m passed  ;  currently sleeping at ...  "..clockHour..":00  ;  current tiredness  is: "..f2(sleepData.tiredness) .." ; lost  "..f2(tirednessPrior - sleepData.tiredness).." ; sleepingCycle: "..sleepData.sleepingCycle[clockHour])
		local health, fatigue, magicka = getRestorationPerHourOfSleep() --fatigue regen is extremely high, ignore that
		health = health / 60
		fatigue = fatigue / 60
		magicka = magicka / 60
		local oldHealth  = typesPlayerStatsSelf.health.current
		local oldMagicka = typesPlayerStatsSelf.magicka.current
		...
		local maxMagicka = typesPlayerStatsSelf.magicka.base
		
		typesPlayerStatsSelf.health.current =  math.max(oldHealth, math.min(maxHealth,  newHealth ))
		typesPlayerStatsSelf.magicka.current = math.max(oldMagicka, math.min(maxMagicka, newMagicka))

		log(6, "+".. newHealth-oldHealth .." Health")
		log(6, "+".. newMagicka-oldMagicka .." Magicka")
		
		-- Well rested: accumulate or drain based on bed quality
		local bedRank = getBedRank() -- returns nil, 1.5 or 2.9
		local theoreticalRecovery = HOURS_PER_SLEEP/HOURS_PER_RESTED_STATE/6/60*minutesPassed
		if bedRank then
		...
		log(6, tostring(minutesPassed).."m passed  ;  currently not sleeping at ...  "..clockHour..":00  ;  current tiredness is: "..f2(sleepData.tiredness).." ; gained  "..f2((sleepData.tiredness -tirednessPrior )*100).."%")
	...
	if minutesPassed > 0 then
		...
		while remaining > 0 do
			...
			if firstLoop then
				sleepData.recentSleepBits = clearBit(sleepData.recentSleepBits, hour)
				firstLoop = false
			elseif posInHour == 60 then
				if G_isSleeping then
					sleepData.recentSleepBits = setBit(sleepData.recentSleepBits, hour)
				else
					sleepData.recentSleepBits = clearBit(sleepData.recentSleepBits, hour)
				end
			end
	...
	if sleepData.longLastingDuration and minutesPassed > 0 then
		...
		local restored = availableDuration * sleepData.longLastingMagnitude
	...
	--apply new buff
	-- currentTirednessBuff: nil = loading, false = no buff for current setting+level, string = applied spell id
	local oldTirednessBuff = sleepData.currentTirednessBuff
	local oldSleepingProfileBuff = sleepData.currentSleepingProfileBuff
	sleepData.currentTirednessBuff = false
	sleepData.currentSleepingProfileBuff = nil
	...
	if SLEEP_PERSONALITY then
		...
		-- insomniac
		do
			local hoursSleptLast24 = countBits(sleepData.recentSleepBits)
			if hoursSleptLast24 > 3 then
				if minute%60 == 0 or minutesPassed > 50 then
					log(3, "insomniac:  skipped, slept", hoursSleptLast24, "hrs in last 24h")
				end
			else
				local sleepScore = 0
				...
				if minute%60 == 0 or minutesPassed > 50 then
					log(3, "insomniac:  slept",f1dot(sleepScore), "/", sleepThreshold, "(", hoursSleptLast24, "hrs actual)")
				end
	...
	if not isInsomniac then
		...
		local newTirednessLevel = math.max(0, math.floor(sleepData.tiredness * 6 - 0.00001))
		if newTirednessLevel ~= sleepData.tirednessLevel then
			log(3, "[SunsDusk:Sleep] Tiredness level:", sleepData.tirednessLevel, "->", newTirednessLevel)
			-- notify only when worsening into a negative tier
			if newTirednessLevel > sleepData.tirednessLevel and newTirednessLevel >= 3 then
				local msg = needsLevelMessages["tiredness_"..newTirednessLevel]
				if msg then
					messageBox(newTirednessLevel >= 5 and 2 or 3, msg)
				end
			end
		end
		sleepData.tirednessLevel = newTirednessLevel
	...
	-- swap buffs
	if oldSleepingProfileBuff ~= sleepData.currentSleepingProfileBuff then

		if oldSleepingProfileBuff and core.magic.spells.records[oldSleepingProfileBuff] then
			log(5, "removing", oldSleepingProfileBuff)
			typesActorSpellsSelf:remove(oldSleepingProfileBuff)
		end

		if sleepData.currentSleepingProfileBuff then
			if not G_preventAddingAnyBuffs then
				typesActorSpellsSelf:add(sleepData.currentSleepingProfileBuff)
			else
				sleepData.currentSleepingProfileBuff = nil
			end
		end
	end
	if oldTirednessBuff ~= sleepData.currentTirednessBuff then
		if oldTirednessBuff and core.magic.spells.records[oldTirednessBuff] then
			log(5, "removing", oldTirednessBuff)
			typesActorSpellsSelf:remove(oldTirednessBuff)
		end

		if sleepData.currentTirednessBuff then
			if not G_preventAddingAnyBuffs then
				typesActorSpellsSelf:add(sleepData.currentTirednessBuff)
			else
				sleepData.currentTirednessBuff = nil
			end
		end
...
-- force buff/widget refresh; ignores call when needId is set and not "sleep"
local function refreshNeeds(needId)
	if needId and needId ~= "sleep" then return end
	if not NEEDS_TIREDNESS or not sleepData then return end
	local clockHour = math.floor((saveData.lastUpdate / 60 + G_clockOffset)) % 24
	if clockHour == 0 then clockHour = 24 end
...
local function onLoad(originalData)
	...
	if not sleepData.recentSleepBits then
		sleepData.recentSleepBits = 0
	end
...
local function settingsChanged(sectionName, setting, oldValue)
	if setting == "NEEDS_TIREDNESS" and NEEDS_TIREDNESS ~= oldValue then
	...
	elseif setting == "S_BACKGROUND" or setting == "S_SKIN" then
		G_destroySleepUi()
	end
end
table.insert(G_settingsChangedJobs, settingsChanged)

function G_getWakeValue(item)
	if not sleepData then return nil end
	
	local entry = saveData.registeredConsumables[item.recordId] or dbConsumables[item.recordId]
	if not entry or not entry.wakeValue or entry.wakeValue == 0 then
		return nil
	end
	
	local wakeRestore = entry.wakeValue * WAKEVALUE_MULT
	local longLastingMagnitude = nil
	local longLastingDuration = nil
	
	if entry.wakeValue2 and entry.wakeValue2 > 0 then
		longLastingMagnitude = entry.wakeValue2 / 360 * WAKEVALUE_MULT
		longLastingDuration = 360
	end
	
	return wakeRestore, longLastingMagnitude, longLastingDuration
end

local function onConsume(item)
	local wakeRestore, longLastingMagnitude, longLastingDuration = G_getWakeValue(item)
	
	if not wakeRestore then return end
	
	sleepData.tiredness = math.min(1, math.max(0, sleepData.tiredness - wakeRestore))
	
	if longLastingDuration then
		sleepData.longLastingMagnitude = longLastingMagnitude
		sleepData.longLastingDuration = longLastingDuration
	end
