-- 0.50 exclusive: using absorb health as a vampire restores hunger // rend flesh for werewolves: using claws in combat restores hunger

-- diseases:
-- slaughterfish : ataxia ; greenspore
-- nix hound : dampworm ; rattles
-- cliff-racer : helljoint
-- alit : rockjoint
-- guar : rockjoint
-- rat : rust chancre ; witbane
-- dreugh : wither
-- kagouti : yellow tick

commonDiseaseOnConsume = {
	"ataxia",
	"dampworm",
	"greenspore",
	"helljoint",
	"rattles",
	"rockjoint",
	"rust chancre",
	"witbane",
	"wither",
	"yellow tick",
}

local hungerData

-- Widget element references (created once, updated as needed)
local hungerIcon = nil
local hungerBackground = nil
local hungerWidget = nil
local profileIcon = nil
local profileBackground = nil
local profileWidget = nil

-- State tracking for conditional updates
local lastHungerLevel = nil
local lastHungerValue = nil
local lastHungerAlpha = nil
local lastFoodProfile = nil
local lastTooltipStr = nil

-- Destroy UI elements (for cleanup/reset)
function G_destroyHungerUi()
	hungerIcon = nil
	hungerBackground = nil
	profileIcon = nil
	profileBackground = nil
	
	if hungerWidget then
		hungerWidget:destroy()
		hungerWidget = nil
	end
	if profileWidget then
		profileWidget:destroy()
		profileWidget = nil
	end
	
	if G_columnWidgets then
		if G_columnWidgets.m_hunger then
			G_columnWidgets.m_hunger:destroy()
			G_columnWidgets.m_hunger = nil
		end
		if G_columnWidgets.m_hunger_profile then
			G_columnWidgets.m_hunger_profile:destroy()
			G_columnWidgets.m_hunger_profile = nil
		end
	end
	
	lastHungerLevel = nil
	lastHungerValue = nil
	lastHungerAlpha = nil
	lastFoodProfile = nil
	lastTooltipStr = nil
	G_columnsNeedUpdate = true
end
table.insert(G_destroyHudJobs, G_destroyHungerUi)

local function buildSeveritySuffix()
	local suffix = ""
	if saveData.countCompanions >= 1 and NEEDS_HUNGER_COMPANION then
		suffix = "_c"
	end
	if NEEDS_SEVERITY_HUNGER == "Hard" then
		suffix = "_2"
	end
	if NEEDS_SEVERITY_HUNGER == "Hardcore" then
		suffix = "_3"
	end
	--if saveData.specialCompanion then
	--	suffix = "_n"
	--end
	return suffix
end

local function removeBuffs()
	if not hungerData then return end
	-- main hunger buff
	if hungerData.currentHungerBuff then
		local buff = hungerData.currentHungerBuff
		if core.magic.spells.records[buff] then
			typesActorSpellsSelf:remove(buff)
		else
			log(2, "[SunsDusk] Skipping removal of missing spell:", buff)
		end
		hungerData.currentHungerBuff = nil
	end
	-- food profile buff (applied magic effect only - currentFoodProfile remains)
	if hungerData.currentFoodProfileBuff then
		local buff = hungerData.currentFoodProfileBuff
		if core.magic.spells.records[buff] then
			typesActorSpellsSelf:remove(buff)
		else
			log(2, "[SunsDusk] Skipping removal of missing spell:", buff)
		end
		hungerData.currentFoodProfileBuff = nil
	end
end
table.insert(G_removeAbilitiesJobs, removeBuffs)

local function updateWidget()
	if not NEEDS_HUNGER or H_HIDE then return end
	
	-- Initialize G_columnWidgets if needed
	if not G_columnWidgets then
		G_columnWidgets = {}
	end
	
	local skinData = G_iconPacks.hunger[H_SKIN]

	-- content skin owns its whole look (icon plus any profile/state art); the staged
	-- path and the separate food-profile widget below are skipped
	if skinData.content then
		local ctx = {
			skin = skinData,
			value = hungerData.hunger,
			buff = hungerData.currentHungerBuff,
			foodProfile = hungerData.currentFoodProfile,
		}
		local widget = G_renderContentSkin({ name = "m_hunger", order = "needs-hunger" }, ctx)
		local tooltipStr = math.floor(hungerData.hunger*100).."%"
		if hungerData.longLastingDuration then
			tooltipStr = tooltipStr.."\nWell fed: "..formatTimeLeft(hungerData.longLastingDuration)
		end
		if hungerData.currentHungerBuff == nil then
			tooltipStr = tooltipStr.."\nLoading..."
		elseif hungerData.currentHungerBuff == false then
			tooltipStr = tooltipStr.."\nNo effects"
		else
			tooltipStr = tooltipStr.."\n"..(tooltips[hungerData.currentHungerBuff] or "Error: "..tostring(hungerData.currentHungerBuff))
		end
		addTooltip(widget.layout, tooltipStr)
		return
	end

	-- ===== FOOD PROFILE WIDGET =====
	if hungerData.currentFoodProfile and H_SP_DISPLAY and hungerData.currentFoodProfile ~= "fasting" then
		local foodProfileTexture = getTexture(skinData.base.."hunger_"..hungerData.currentFoodProfile..skinData.extension)
		
		-- Initialize profile widget if it doesn't exist
		if not profileWidget then
			profileBackground = H_BACKGROUND ~= "No Background" and {
				name = "profile_background",
				type = ui.TYPE.Image,
				props = {
					resource = H_BACKGROUND == "Classic" and getTexture(skinData.base.."BlankTexture"..skinData.extension) or foodProfileTexture,
					color = H_BACKGROUND == "Classic" and H_BACKGROUND_COLOR or util.color.rgb(0,0,0),
					tileH = false,
					tileV = false,
					relativeSize = v2(1,1),
					relativePosition = S_BACKGROUND == "Shadow" and v2(0.04,0.027) or nil,
					alpha = 1,
				}
			} or {}
			
			profileIcon = {
				name = "profile_icon",
				type = ui.TYPE.Image,
				props = {
					resource = foodProfileTexture,
					color = H_COLOR,
					tileH = false,
					tileV = false,
					relativeSize = v2(1,1),
					alpha = 1,
				}
			}
			
			profileWidget = ui.create{
				name = "m_hunger_profile",
				type = ui.TYPE.Widget,
				props = {
					size = v2(HUD_ICON_SIZE, HUD_ICON_SIZE),
				},
				userData = {
					order = "profiles-hunger",
				},
				content = ui.content {
					profileBackground,
					profileIcon,
				}
			}
			
			G_columnWidgets.m_hunger_profile = profileWidget
			lastFoodProfile = hungerData.currentFoodProfile
			G_columnsNeedUpdate = true
		end
		
		-- Update profile widget if food profile changed
		if lastFoodProfile ~= hungerData.currentFoodProfile then
			profileIcon.props.resource = foodProfileTexture
			if H_BACKGROUND ~= "No Background" and H_BACKGROUND ~= "Classic" and profileBackground then
				profileBackground.props.resource = foodProfileTexture
			end
			lastFoodProfile = hungerData.currentFoodProfile
			profileWidget:update()
		end
		
		-- Update tooltip
		local tooltipStr = not hungerData.currentFoodProfile and "Loading..." or tooltips[hungerData.currentFoodProfile] or "Error: "..tostring(hungerData.currentFoodProfile)
		addTooltip(profileWidget.layout, tooltipStr)
		
	elseif profileWidget then
		-- Remove profile widget if no longer needed
		profileWidget:destroy()
		profileWidget = nil
		profileIcon = nil
		profileBackground = nil
		if G_columnWidgets.m_hunger_profile then
			G_columnWidgets.m_hunger_profile = nil
			G_columnsNeedUpdate = true
		end
		lastFoodProfile = nil
	end
	
	-- ===== MAIN HUNGER WIDGET =====
	-- Calculate current values
	local currentHungerLevel = math.max(0, math.floor(hungerData.hunger * skinData.stages - 0.00001))
	local alphaInput = HUD_INVERT_ALPHA and (1 - hungerData.hunger) or hungerData.hunger
	local currentAlpha = HUD_ALPHA == "Static" and 1 or getWidgetAlpha(alphaInput)
	if H_HIDE_NO_BUFF then
		if hungerData.currentHungerBuff == false then
			-- no active effect for the current mode (false only happens in restricted modes): hide
			currentAlpha = 0
		elseif NEEDS_HUNGER_BUFFS_DEBUFFS ~= "Buffs and debuffs" then
			-- restricted modes: keep an active buff readable even at a low need level
			currentAlpha = HUD_ALPHA == "Static" and 1 or getWidgetAlpha(alphaInput, 3, 0.45)
		end
	end
	local bgAlpha = H_BACKGROUND == "Classic" and (HUD_ALPHA == "Static" and 1 or currentAlpha^2) or (currentAlpha > 0 and 0.5 or 0)
	-- Determine texture
	local hungerTexture
	if skinData.stages > 1 then
		hungerTexture = getTexture(skinData.base.."hunger_"..currentHungerLevel..skinData.extension)
	else
		hungerTexture = getTexture(skinData.base.."hunger"..skinData.extension)
	end
	
	-- Initialize widget if it doesn't exist
	if not hungerWidget then
		hungerBackground = H_BACKGROUND ~= "No Background" and {
			name = "hunger_background",
			type = ui.TYPE.Image,
			props = {
				resource = H_BACKGROUND == "Classic" and getTexture(skinData.base.."BlankTexture"..skinData.extension) or hungerTexture,
				color = H_BACKGROUND == "Classic" and H_BACKGROUND_COLOR or util.color.rgb(0,0,0),
				tileH = false,
				tileV = false,
				relativeSize = v2(1,1),
				relativePosition = H_BACKGROUND == "Shadow" and v2(0.04,0.027) or nil,
				alpha = bgAlpha,
			}
		} or {}
		
		hungerIcon = {
			name = "hunger_icon",
			type = ui.TYPE.Image,
			props = {
				resource = hungerTexture,
				color = H_COLOR,
				tileH = false,
				tileV = false,
				relativeSize = v2(1,1),
				alpha = currentAlpha,
			}
		}
		
		hungerWidget = ui.create{
			name = "m_hunger",
			type = ui.TYPE.Widget,
			props = {
				size = v2(HUD_ICON_SIZE, HUD_ICON_SIZE),
			},
			userData = {
				order = "needs-hunger",
				effectiveAlpha = currentAlpha,
			},
			content = ui.content {
				hungerBackground,
				hungerIcon,
			}
		}

		G_columnWidgets.m_hunger = hungerWidget
		
		-- Initialize tracking variables
		lastHungerLevel = currentHungerLevel
		lastHungerValue = hungerData.hunger
		lastHungerAlpha = currentAlpha
		
		G_columnsNeedUpdate = true
	end
	
	-- Check if we need to update
	local needsUpdate = false
	
	-- Update icon texture if hunger level changed
	if lastHungerLevel ~= currentHungerLevel then
		hungerIcon.props.resource = hungerTexture
		lastHungerLevel = currentHungerLevel
		needsUpdate = true
		
		-- Update background texture if not using Classic style
		if H_BACKGROUND ~= "No Background" and H_BACKGROUND ~= "Classic" and hungerBackground then
			hungerBackground.props.resource = hungerTexture
		end
	end
	
	-- Update alpha if it changed
	if lastHungerAlpha ~= currentAlpha then
		hungerIcon.props.alpha = currentAlpha
		-- visibility toggles re-sort columns when sort-by-visibility is on
		if ((lastHungerAlpha or 1) > 0) ~= (currentAlpha > 0) then
			G_columnsNeedUpdate = true
		end
		lastHungerAlpha = currentAlpha
		hungerWidget.layout.userData.effectiveAlpha = currentAlpha
		needsUpdate = true

		-- Update background alpha for all styles
		if hungerBackground and hungerBackground.props then
			hungerBackground.props.alpha = bgAlpha
		end
	end
	
	local tooltipStr = math.floor(hungerData.hunger*100).."%"
	if hungerData.longLastingDuration then
		tooltipStr = tooltipStr.."\nWell fed: "..formatTimeLeft(hungerData.longLastingDuration)
	end
	if hungerData.currentHungerBuff == nil then
		tooltipStr = tooltipStr.."\nLoading..."
	elseif hungerData.currentHungerBuff == false then
		tooltipStr = tooltipStr.."\nNo effects"
	else
		tooltipStr = tooltipStr.."\n"..(tooltips[hungerData.currentHungerBuff] or "Error: "..tostring(hungerData.currentHungerBuff))
	end

	if hungerData.currentFoodProfile == "fasting" then
		tooltipStr = tooltipStr.."\n\n"..(tooltips[hungerData.currentFoodProfile] or "Error: "..tostring(hungerData.currentFoodProfile))
	end
	
	if lastTooltipStr ~= tooltipStr then
		lastTooltipStr = tooltipStr
		addTooltip(hungerWidget.layout, tooltipStr)
		needsUpdate = true
	end
	
	-- Only call update if something actually changed
	if needsUpdate then
		hungerWidget:update()
	end
end
table.insert(G_refreshWidgetJobs, updateWidget)

local function module_hunger_hour(clockHour)
	if not NEEDS_HUNGER then return end
	for i, entry in pairs(hungerData.foodProfiles) do
		if i ~= "broken pact" then
			hungerData.foodProfiles[i] = math.max(0, entry*0.95 - 0.03)
		end
	end
end
table.insert(G_perHourJobs, module_hunger_hour)

local function module_hunger_minute(clockHour, minute, minutesPassed)
	if not NEEDS_HUNGER then return end
	--if not hungerData then return end
	local hungerPrior = hungerData.hunger
	hungerData.fastingMinutes = hungerData.fastingMinutes + minutesPassed
	-- fast metabolism
	local raceConsumptionMult = 1
	if NEEDS_RACES_HUNGER then
		if saveData.playerInfo.isAltmer then
			raceConsumptionMult = raceConsumptionMult * 1.15
		elseif saveData.playerInfo.isRedguard then
			raceConsumptionMult = raceConsumptionMult * 1.1
		end
	end
	if G_isInJail then
		if hungerData.currentFoodProfile ~= "fasting" then
			hungerData.hunger = 0.5
			hungerData.fastingMinutes = 0
		end
	else
		if G_isSleeping or G_isTravelling then
			-- hunger is always increasing
			if not G_isLongTravel then
				hungerData.hunger = math.min(1, hungerData.hunger + 0.5/HOURS_PER_HUNGER_STATE/6/60*minutesPassed*raceConsumptionMult)
			end
		else
			-- change sleeping cycle for current hour of being awake -0.2
			hungerData.hunger = math.min(1, hungerData.hunger + 1/HOURS_PER_HUNGER_STATE/6/60*minutesPassed*raceConsumptionMult)
		end
		if hungerData.longLastingDuration and minutesPassed > 0 then
			local availableDuration = math.min(hungerData.longLastingDuration, minutesPassed)
			local restored = availableDuration * hungerData.longLastingMagnitude
			hungerData.hunger = math.max(0, hungerData.hunger - restored)
			hungerData.longLastingDuration = hungerData.longLastingDuration - availableDuration
			if hungerData.longLastingDuration <= 0 then
				hungerData.longLastingDuration = nil
			end
		end
	end
	--if NEEDS_HUNGER_BUFFS then -- prevent hunger from going above 0.5 (hungerLevel 3 starts at 0.5 or so)
	--	hungerData.hunger = math.min(1-3/6-0.0001, hungerData.hunger)
	--end
	if hungerData.foodProfiles["corprus"] > 0 and hungerData.currentFoodProfile ~= "corprus_eater" then
		--if minutesPassed > 0 then
		--	print(hungerData.foodProfiles["corprus"])
		--end
		hungerData.foodProfiles["corprus"] = math.max(0, hungerData.foodProfiles["corprus"] - 0.34*minutesPassed)
	end
	
	-- compute desired food profile buff
	local newFoodProfile = nil
	local newFoodProfileBuff = nil
	if NEEDS_HUNGER_NOURISHMENT then
		-- well nourished
		local foodProfile = nil
		local buff = nil

		local highestCategory = 0

		highestCategory = math.max(highestCategory, hungerData.foodProfiles["very small"])
		highestCategory = math.max(highestCategory, hungerData.foodProfiles["light meal"])
		highestCategory = math.max(highestCategory, hungerData.foodProfiles["medium meal"])
		highestCategory = math.max(highestCategory, hungerData.foodProfiles["filling meal"])
		highestCategory = math.max(highestCategory, hungerData.foodProfiles["hearty meal"])
		highestCategory = math.max(highestCategory, hungerData.foodProfiles["raw meat"])

		local countCategories = 0
		if hungerData.foodProfiles["very small"] > 1 then
			countCategories = countCategories + 1
		end
		if hungerData.foodProfiles["light meal"] > 1 then
			countCategories = countCategories + 1
		end
		if hungerData.foodProfiles["medium meal"] > 1 then
			countCategories = countCategories + 1
		end
		if hungerData.foodProfiles["filling meal"] > 1 then
			countCategories = countCategories + 1
		end
		if hungerData.foodProfiles["hearty meal"] > 1 then
			countCategories = countCategories + 1
		end
		if hungerData.foodProfiles["raw meat"] > 1 then
			countCategories = countCategories + 1
		end
		if countCategories >= 3 then
			foodProfile = "well_nourished"
			buff = "sd_h_sp_varied"
		end
		log(6, string.format("Meal counts - VerySmall:%d, Light:%d, Medium:%d, Filling:%d, Hearty:%d, RawMeat:%d | GreenPact:%d, Vegan:%d, Corprus:%d, BrokenPact:%d | Computed - highestCategory:%d, countCategories:%d",
			hungerData.foodProfiles["very small"],
			hungerData.foodProfiles["light meal"],
			hungerData.foodProfiles["medium meal"],
			hungerData.foodProfiles["filling meal"],
			hungerData.foodProfiles["hearty meal"],
			hungerData.foodProfiles["raw meat"],
			hungerData.foodProfiles["green pact"],
			hungerData.foodProfiles["vegan"],
			hungerData.foodProfiles["corprus"],
			hungerData.foodProfiles["broken pact"],
			highestCategory,
			countCategories
		))

		-- green pact
		if minute%60 == 0 then
			log(3, "Green Pact: "..hungerData.foodProfiles["green pact"]..">3, Vegan: "..hungerData.foodProfiles["vegan"].."==0, Broken Pact: "..hungerData.foodProfiles["broken pact"].."m left" )
			log(3, "Well Nourished: "..countCategories..">3, Corprus: "..hungerData.foodProfiles["corprus"]..">3 and >Highest Category: "..highestCategory )
		end
		if hungerData.foodProfiles["vegan"] == 0 and hungerData.foodProfiles["green pact"] > 3 then
			foodProfile = "green_pact"
			buff = "sd_h_sp_greenpact"
		end

		-- corprus eater
		if hungerData.foodProfiles["corprus"] > 3 and hungerData.foodProfiles["corprus"] >= highestCategory then
			foodProfile = "corprus_eater"
			buff = "sd_h_sp_corprus"
		end

		-- broken pact (decrements regardless of whether buff is applied)
		if hungerData.foodProfiles["broken pact"] > 0 then
			hungerData.foodProfiles["broken pact"] = hungerData.foodProfiles["broken pact"] - minutesPassed
			foodProfile = "broken_pact"
			buff = "sd_h_sp_greenpact_2"
		end

		if hungerData.fastingMinutes > 24*60 then
			buff = "sd_h_sp_fasting"
			foodProfile = "fasting"
		end

		newFoodProfile = foodProfile
		newFoodProfileBuff = buff
	end

	-- virtual state for UI (always reflects desired profile, even when buff isn't applied)
	hungerData.currentFoodProfile = newFoodProfile

	-- diff-apply food profile buff (applied magic effect)
	if newFoodProfileBuff ~= hungerData.currentFoodProfileBuff then
		local oldBuff = hungerData.currentFoodProfileBuff
		if oldBuff and core.magic.spells.records[oldBuff] then
			typesActorSpellsSelf:remove(oldBuff)
		end
		hungerData.currentFoodProfileBuff = nil
		if newFoodProfileBuff and not G_preventAddingAnyBuffs then
			typesActorSpellsSelf:add(newFoodProfileBuff)
			hungerData.currentFoodProfileBuff = newFoodProfileBuff
			log(3, "[SunsDusk:Hunger] Applied food profile buff:", newFoodProfileBuff)
		end
	end

	local suffix = buildSeveritySuffix()

	local newHungerLevel = math.max(0, math.floor(hungerData.hunger * 6 - 0.00001))
	if newHungerLevel ~= hungerData.hungerLevel then
		log(3, "[SunsDusk:Hunger] Level:", hungerData.hungerLevel, "->", newHungerLevel)
		-- notify only when worsening into a negative tier
		if newHungerLevel > hungerData.hungerLevel and newHungerLevel >= 3 then
			local msg = needsLevelMessages["hunger_"..newHungerLevel]
			if msg then
				messageBox(newHungerLevel >= 5 and 2 or 3, msg)
			end
		end
	end
	hungerData.hungerLevel = newHungerLevel

	-- remove severitiy suffixes (_2 and _3) for positive buffs (level 0-2)
	-- note: no companion buffs on hardcore

	if hungerData.hungerLevel <= 3 and (suffix == "_2" or suffix == "_3") then
		suffix = ""
	end

	-- compute desired hunger buff
	-- false = no buff for current setting+level, string = spell id to apply
	local newHungerBuff = false
	if NEEDS_HUNGER_BUFFS_DEBUFFS == "Only buffs" and hungerData.hungerLevel <= 2
	or NEEDS_HUNGER_BUFFS_DEBUFFS == "Only debuffs" and hungerData.hungerLevel >= 3
	or NEEDS_HUNGER_BUFFS_DEBUFFS == "Buffs and debuffs" then
		newHungerBuff = "sd_hunger_"..hungerData.hungerLevel..suffix
	end

	-- diff-apply hunger buff
	-- nil = loading, false = no buff for current setting+level, string = applied spell id
	if newHungerBuff ~= hungerData.currentHungerBuff then
		local oldBuff = hungerData.currentHungerBuff
		if oldBuff and core.magic.spells.records[oldBuff] then
			typesActorSpellsSelf:remove(oldBuff)
		end
		if newHungerBuff then
			if G_preventAddingAnyBuffs then
				hungerData.currentHungerBuff = nil
			else
				typesActorSpellsSelf:add(newHungerBuff)
				hungerData.currentHungerBuff = newHungerBuff
				log(3, "[SunsDusk:Hunger] Applied hunger buff:", newHungerBuff)
			end
		else
			hungerData.currentHungerBuff = false
		end
	end
	
	if DEATH_BY_STARVATION and not G_isTravelling and not G_isInJail and hungerData.currentFoodProfile ~= "fasting" then
		local volume = (hungerData.hunger-0.95)*20
		if volume > 0 then
			G_heartbeatFlags.hunger = volume
			G_vignetteFlags.hunger = 0.6*volume
			
			if hungerData.hunger >=0.99 then
				G_vignetteColorFlags.hunger = "default"
				if not debug.isGodMode() then
					typesPlayerStatsSelf.health.current = typesPlayerStatsSelf.health.current - math.min(7, 0.5 * minutesPassed)
				end
				G_flashVignette = 0.7
			end
		else
			G_heartbeatFlags.hunger = nil
			G_vignetteFlags.hunger  = nil
			G_vignetteColorFlags.hunger = nil
		end
	else
		G_heartbeatFlags.hunger = nil
		G_vignetteFlags.hunger  = nil
		G_vignetteColorFlags.hunger = nil
	end

	updateWidget()
end
table.insert(G_perMinuteJobs, module_hunger_minute)

-- force buff/widget refresh; ignores call when needId is set and not "hunger"
local function refreshNeeds(needId)
	if needId and needId ~= "hunger" then return end
	if not NEEDS_HUNGER or not hungerData then return end
	module_hunger_minute(24, 1, 0)
end
table.insert(G_refreshNeedsJobs, refreshNeeds)

local function onLoad(originalData)
	if not NEEDS_HUNGER then return end
	if not saveData.m_hunger then
		saveData.m_hunger = {
			hunger = (1/6),
			version = 2,
		}
	end
	hungerData = saveData.m_hunger
	-- migration:
	if not hungerData.hungerLevel then
		hungerData.hungerLevel = 1
	end
	if not hungerData.hungerLevel then
		hungerData.hungerLevel = 1
	end
	if not hungerData.foodProfiles then
		hungerData.foodProfiles = {
			["very small"] = 0,
			["light meal"] = 0,
			["medium meal"] = 0,
			["filling meal"] = 0,
			["hearty meal"] = 0,
			["beverage"] = 0,
			["alcohol"] = 0,
			["raw meat"] = 0,
			["corprus"] = 0,
			["human"] = 0,
			["green pact"] = 0,
			["vegan"] = 0,
			["broken pact"] = 0,
		}
	end
	if not hungerData.fastingMinutes then
		hungerData.fastingMinutes = 0
	end
	-- migration: convert old uppercase foodProfiles keys to lowercase
	if not hungerData.version then
		local keysToMigrate = {}
		for key in pairs(hungerData.foodProfiles) do
			local lower = string.lower(key)
			if lower ~= key then
				keysToMigrate[key] = lower
			end
		end
		for oldKey, newKey in pairs(keysToMigrate) do
			hungerData.foodProfiles[newKey] = hungerData.foodProfiles[oldKey]
			hungerData.foodProfiles[oldKey] = nil
		end
		hungerData.version = 2
	end
end

table.insert(G_onLoadJobs, onLoad)
table.insert(G_onLoadJobs, updateWidget) --after all onload jobs, also update widget

local function settingsChanged(sectionName, setting, oldValue)
	if setting == "NEEDS_HUNGER" and NEEDS_HUNGER ~= oldValue then
		if oldValue == false then
			onLoad()
		else
			removeBuffs()
			saveData.m_hunger = nil
			G_destroyHungerUi()
			G_heartbeatFlags.hunger = nil
			G_vignetteFlags.hunger  = nil
			G_vignetteColorFlags.hunger = nil
		end
	elseif setting == "H_BACKGROUND" or setting == "H_SKIN" then
		G_destroyHungerUi()
	end
end
table.insert(G_settingsChangedJobs, settingsChanged)

function G_getFoodValue(item)
	if not hungerData then return nil end
	if G_isUsedBathingItem(item.recordId) then return nil end
	
	local entry = saveData.registeredConsumables[item.recordId] or dbConsumables[item.recordId]
	
	-- Handle ingredients
	if not entry or not entry.foodValue or entry.foodValue == 0 then
		if types.Ingredient.objectIsInstance(item) then
			return 0.1, nil, nil, nil, nil, "light meal", 1, nil, nil, nil, nil, nil
		end
		return nil
	end
	
	local corprusPoisoning = false
	local foodValue = tonumber(entry.foodValue) or 0
	local foodValue2 = tonumber(entry.foodValue2) or 0
	
	-- Apply aging
	if entry.timestamp then
		local age = (core.getGameTime() - entry.timestamp)/time.hour
		if age > 3 then
			foodValue = math.max(foodValue*0.4, foodValue * (1 - age/72))
			foodValue2 = math.max(foodValue2*0.4, foodValue2 * (1 - age/72))
		end
	end
	
	local category = entry.consumeCategory
	local isRawMeat = category == "raw meat"
	local isBlood = G_bloodItemIds[item.recordId]
	local healthDamage = nil
	
	-- Corprus check
	if category == "corprus" and hungerData.currentFoodProfile ~= "corprus_eater" then
		healthDamage = typesPlayerStatsSelf.health.base/3 + 20
		corprusPoisoning = true
	end
	
	local foodDiseaseChance = RAW_MEAT_DISEASE_CHANCE / 100
	local toxicityChance = 1
	local restore = foodValue
	
	-- Racial modifiers
	if NEEDS_RACES_HUNGER then
		-- BEASTS
		if saveData.playerInfo.isBeast then
			if isRawMeat or isBlood then
				foodDiseaseChance = 0
			end
			if isRawMeat then
				restore = restore * (200 / 150)
			end
		end
		-- ORCS
		if saveData.playerInfo.isOrc then
			foodDiseaseChance = 0
			toxicityChance = 0
		end
		-- ALTMER
		if saveData.playerInfo.isAltmer then
			foodDiseaseChance = foodDiseaseChance * 1.25
			if isRawMeat then
				restore = restore * 0.75
			end
			if entry.isCookedMeal then
				restore = restore * 1.25
			end
		end
		-- IMPERIAL
		if saveData.playerInfo.isImperial then
			foodDiseaseChance = foodDiseaseChance * 1.15
			if isRawMeat then
				restore = restore * 0.85
			end
			if entry.isCookedMeal then
				restore = restore * 1.15
			end
		end
		-- BOSMER
		if saveData.playerInfo.isBosmer then
			foodDiseaseChance = 0
			if entry.isGreenPact then
				restore = restore * 1.25
			else
				restore = restore * 0.5
			end
		end
	end
	
	-- Vampire modifiers
	if saveData.playerInfo.isVampire > 0 then
		foodDiseaseChance = 0
		if NEEDS_HUNGER_VW ~= "Disable" then
			if isRawMeat or category == "human" then
				if NEEDS_HUNGER_VW == "Immortal" then
					restore = restore * 1.5
				else --Supernatural
					restore = restore * 1.2
				end
			elseif NEEDS_HUNGER_VW == "Immortal" then
				restore = restore * 0.1
			end
		end
	end
	
	-- Werewolf modifiers
	if saveData.playerInfo.isWerewolf > 0 then
		foodDiseaseChance = 0
		if NEEDS_HUNGER_VW ~= "Disable" then
			if entry.isGreenPact then
				if NEEDS_HUNGER_VW == "Immortal" then
					restore = restore * 1.5
				else --Supernatural
					restore = restore * 1.2
				end
			elseif NEEDS_HUNGER_VW == "Immortal" then
				restore = restore * 0.1
			end
		end
	end

	-- non-vampires get reduced restore from blood
	if saveData.playerInfo.isVampire == 0 and isBlood then
		restore = restore * 0.4
	end

	local foodRestore = restore * FOODVALUE_MULT
	local longLastingMagnitude = nil
	local longLastingDuration = nil
	
	-- Long-lasting effects
	if foodValue2 ~= 0 then
		longLastingMagnitude = (restore/entry.foodValue)*entry.foodValue2/360*FOODVALUE_MULT
		longLastingDuration = 360
	end
	
	local applyDisease = nil
	local applyFoodPoisoning = nil
	local debuffAmount = 0
	
	-- Disease roll for raw meat / blood
	if RAW_MEAT_DISEASE_CHANCE > 0 and (isRawMeat or isBlood) and foodDiseaseChance > 0 then
		if math.random() < foodDiseaseChance then
			applyDisease = true
			longLastingMagnitude = nil
			longLastingDuration = nil
			debuffAmount = 0.5
		end
	end
	
	-- Food poisoning
	if NEEDS_HUNGER_TOXICITY and entry.isToxic or corprusPoisoning then
		if math.random() < toxicityChance then
			applyFoodPoisoning = true
			longLastingMagnitude = nil
			longLastingDuration = nil
			debuffAmount = 2
		end
	end
	
	-- Food profile increments
	local foodProfileIncrement = nil
	if entry.consumeCategory and hungerData.foodProfiles[entry.consumeCategory] then
		if debuffAmount > 0 and not corprusPoisoning then
			foodProfileIncrement = -debuffAmount
		else
			foodProfileIncrement = (entry.isCookedMeal and 2 or 1)
		end
	end
	
	-- Green Pact tracking
	local greenPactIncrement = nil
	local brokenPactDuration = nil
	local veganIncrement = 1
	
	if entry.isGreenPact then
		greenPactIncrement = 1
	else
		if hungerData.currentFoodProfile == "green_pact" then
			brokenPactDuration = 1440
		end
	end
	
	return foodRestore, longLastingMagnitude, longLastingDuration, applyDisease, applyFoodPoisoning, 
		entry.consumeCategory, foodProfileIncrement, healthDamage, greenPactIncrement, 
		veganIncrement, brokenPactDuration, entry
end

local function onConsume(item)
	local foodRestore, longLastingMagnitude, longLastingDuration, applyDisease, applyFoodPoisoning,
		foodProfile, foodProfileIncrement, healthDamage, greenPactIncrement, 
		veganIncrement, brokenPactDuration, entry = G_getFoodValue(item)
	if not foodRestore then return end
	
	hungerData.fastingMinutes = 0
	
	-- Apply health damage (corprus)
	if healthDamage then
		typesPlayerStatsSelf.health.current = typesPlayerStatsSelf.health.current - healthDamage
	end
	
	
	-- FOOD HEALING
	local isIngredient = types.Ingredient.objectIsInstance(item)
	local isCookedMeal = entry and entry.timestamp
	local shouldHeal = false
	
	if FOOD_HEALS == "All food" then
		shouldHeal = true
	elseif FOOD_HEALS == "Only ingredients" and isIngredient then
		shouldHeal = true
	elseif FOOD_HEALS == "Only cooked" and isCookedMeal then
		shouldHeal = true
	end
	
	-- Apply hunger restoration and healing
	if foodRestore > 0 then
		local previousHunger = hungerData.hunger
		hungerData.hunger = math.max(0, hungerData.hunger - foodRestore)
		
		-- Split into useful and overshoot
		local usefulFoodValue = math.min(foodRestore, previousHunger)
		local overshootFoodValue = math.max(0, foodRestore - previousHunger)
		
		if shouldHeal and (usefulFoodValue > 0 or overshootFoodValue > 0) and not applyDisease and not applyFoodPoisoning then
			local maxHealth =  typesPlayerStatsSelf.health.base
			local currentHealth =  typesPlayerStatsSelf.health.current
			local playerLevel = typesPlayerStatsSelf.level.current
			
			-- Base Heal value
			local baseConversion = maxHealth * 0.8
			baseConversion = baseConversion + 2 + (playerLevel * 0.2) -- base value + level scaling (flat values)
			if isIngredient then
				local ingredientClass = entry and entry.ingredientClass
				if not ingredientClass 
				 or ingredientClass == "monster" 
				 or ingredientClass == "dubious"
				 or ingredientClass == "greens"
				 or ingredientClass == "salts"
				 or ingredientClass == "spices"
				 then
					baseConversion = baseConversion * 0.7
				end
			elseif isCookedMeal then
				if COOKING_MODE == "Immersive" then
					baseConversion = baseConversion * 0.4
				else
					baseConversion = baseConversion * 1.2
				end
			end
			
			local healFromUseful = usefulFoodValue * baseConversion
			local healFromOvershoot = overshootFoodValue * baseConversion * 0.3 -- only 30% healing from wasted food
			local totalHeal = (healFromUseful + healFromOvershoot)
			
			local newHealth = math.min(maxHealth, currentHealth + totalHeal)
			
			-- Dont remove fortify health effects:
			if newHealth > currentHealth then
				typesPlayerStatsSelf.health.current = newHealth
			end
		end
		
		-- Long-lasting effects
		if longLastingDuration then
			hungerData.longLastingMagnitude = longLastingMagnitude
			hungerData.longLastingDuration = longLastingDuration
		end
	end
	
	-- Apply disease
	if applyDisease then
		hungerData.longLastingMagnitude = nil
		hungerData.longLastingDuration = nil
		local randomDisease = commonDiseaseOnConsume[math.random(1, #commonDiseaseOnConsume)]
		local magicRecord = core.magic.spells.records[randomDisease]
		if magicRecord then
			messageBox(1, "You've contracted "..tostring(magicRecord.name))
			typesActorSpellsSelf:add(randomDisease)
		end
	end
	
	-- Apply food poisoning
	if applyFoodPoisoning then
		local suffix = buildSeveritySuffix()
		hungerData.longLastingMagnitude = nil
		hungerData.longLastingDuration = nil
		messageBox(1, "You're getting food poisoning")
		typesActorActiveSpellsSelf:add({
			id = "sd_h_sp_foodpoisoning" .. suffix,
			effects = { 0, 1 },
			ignoreResistances = true,
			ignoreSpellAbsorption = true,
			ignoreReflect = true,
			stackable = true,
		})
	end
	
	-- Update food profiles
	if foodProfile and foodProfileIncrement then
		hungerData.foodProfiles[foodProfile] = math.max(0, hungerData.foodProfiles[foodProfile] + foodProfileIncrement)
	end
	
	if greenPactIncrement then
		hungerData.foodProfiles["green pact"] = hungerData.foodProfiles["green pact"] + greenPactIncrement
	else
		if brokenPactDuration then
			hungerData.foodProfiles["broken pact"] = brokenPactDuration
			log(3, "Broke Green pact", item)
		end
		hungerData.foodProfiles["vegan"] = hungerData.foodProfiles["vegan"] + (veganIncrement or 0)
	end
	
	module_hunger_minute(24, 1, 0)
end
table.insert(G_onConsumeJobs, onConsume)

-- ────────────────────────────────────
-- Vampire Feeding
-- ────────────────────────────────────

-- Can be called by: SD_Hunger_ApplyRestoreFraction(frac)
-- frac in [0..1], reduces hunger by that fraction and refreshes buffs/UI.
local function landedHit(target, attack)
	if saveData.playerInfo.isInWerewolfForm and NEEDS_HUNGER_VW ~= "Disable" and attack.successful and math.random() < 0.4 and attack.sourceType == I.Combat.ATTACK_SOURCE_TYPES.Melee  then
		ambient.playSoundFile("sound/sunsdusk/Swallowing_water_by_180242.ogg",{volume = 1.5})
		hungerData.foodProfiles["raw meat"] = hungerData.foodProfiles["raw meat"] + 0.4
		hungerData.foodProfiles["green pact"] = hungerData.foodProfiles["green pact"] + 0.4
		local restoredHunger = 0.1 * attack.strength
		if NEEDS_HUNGER_VW == "Supernatural" then
			restoredHunger = restoredHunger * 2/3
		end
		hungerData.hunger = math.max(0,hungerData.hunger - restoredHunger)
	end
end

table.insert(G_landedHitJobs, landedHit)

local function landedSpellHit(target, spell)
	if saveData.playerInfo.isVampire > 0 and NEEDS_HUNGER_VW ~= "Disable" then
		local totalMagnitude = 0
		for i,effect in pairs(spell.effects) do
			if effect.id == "absorbhealth" then
				totalMagnitude = totalMagnitude + effect.duration* (effect.maxMagnitude+effect.minMagnitude)/2
			end
		end
		if totalMagnitude > 0 then
			local restoredHunger = 0.06+totalMagnitude/333
			if NEEDS_HUNGER_VW == "Supernatural" then
				restoredHunger = restoredHunger * 0.6
			end
			hungerData.hunger = math.max(0,hungerData.hunger - restoredHunger)
			module_hunger_minute(24, 1, 0)
		end
	end
end

table.insert(G_landedSpellHitJobs, landedSpellHit)