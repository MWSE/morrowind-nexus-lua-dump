-- Sun's Dusk - Bathing and Cleanliness
...
tooltips["sd_c_sp_bath_imp"]  = "Imperial Bath: +2 Agility, +2 Intelligence, +2 Luck"
tooltips["sd_c_sp_bath_n"]    = "Nord Bath: +3 Strength, Resist Frost 10%"
...
local function applyCleanBuff()
	...
	-- levels 0-2 = clean (buffs), 3-5 = dirty (debuffs)
	local isBuff = level <= 2
	local settingAllowsBuff = NEEDS_CLEAN_BUFFS_DEBUFFS == "Only buffs" and isBuff
		or NEEDS_CLEAN_BUFFS_DEBUFFS == "Only debuffs" and not isBuff
		or NEEDS_CLEAN_BUFFS_DEBUFFS == "Buffs and debuffs"
	...
	elseif G_preventAddingAnyBuffs then
		...
		newBuff = "sd_clean_" .. level
	end
	
	log(3, "[SunsDusk:Clean] Applying buff:", tostring(newBuff), "for dirt value:", dirt, "level:", level)
	
	if newBuff ~= cleanData.currentCleanBuff then
		local oldBuff = cleanData.currentCleanBuff
		if oldBuff and core.magic.spells.records[oldBuff] then
			typesActorSpellsSelf:remove(oldBuff)
		end
		cleanData.currentCleanBuff = newBuff
		if newBuff then
			typesActorSpellsSelf:add(newBuff)
			log(3, "[SunsDusk:Clean] Applied bathing buff:", newBuff)
		end
	end
...
local function getCurrentHouseType()
	...
	if G_cellInfo.isIndoril then return "indoril" end
	if G_cellInfo.isImperial then return "imperial" end
	if G_cellInfo.isNord then return "nord" end
	
	-- Check cell name for other bath buffs
...
-- full clean tooltip, shared by the staged path and content skins
local function buildCleanTooltip()
...
local function updateWidget()
	...
	if not cleanData or C_HIDE then return end
	...
	if skinData.content then
		local ctx = {
			skin = skinData,
			value = cleanData.dirt,
			buff = cleanData.currentCleanBuff,
		}
		local widget = G_renderContentSkin({ name = "m_clean", order = "needs-clean" }, ctx)
		addTooltip(widget.layout, buildCleanTooltip())
		return
	end
	...
	local alphaInput = HUD_INVERT_ALPHA and (1 - cleanData.dirt) or cleanData.dirt
	local currentAlpha = HUD_ALPHA == "Static" and 1 or getWidgetAlpha(alphaInput)
	if C_HIDE_NO_BUFF then
		if cleanData.currentCleanBuff == false then
			-- no active effect for the current mode (false only happens in restricted modes): hide
			currentAlpha = 0
		elseif NEEDS_CLEAN_BUFFS_DEBUFFS ~= "Buffs and debuffs" then
	...
	local bgAlpha = C_BACKGROUND == "Classic" and (HUD_ALPHA == "Static" and 1 or currentAlpha^2) or (currentAlpha > 0 and 0.5 or 0)
	...
	if lastAlpha ~= currentAlpha then
		...
		lastAlpha = currentAlpha
		cleanWidget.layout.userData.effectiveAlpha = currentAlpha
		needsUpdate = true

		-- Update background alpha for all styles
		if cleanBackground and cleanBackground.props then
			cleanBackground.props.alpha = bgAlpha
		end
	end
	
	-- Build tooltip string
	local tooltipStr = buildCleanTooltip()
...
local function settingsChanged(sectionName, setting, oldValue)
	...
	if (setting == "NEEDS_CLEAN" or setting == "NEEDS_TEMP") and _G[setting] ~= oldValue then
				...
				content = ui.content{
					{
						...
						content = ui.content{{
							...
							content = ui.content{{
								type = ui.TYPE.Text,
								props = {
									text = "Choose soap to bathe with:",
									multiline = true,
									textAlignH = ui.ALIGNMENT.Center,
									textColor = bathMenuTheme.header,
									textSize = bathMenuTheme.baseSize,
								},
							}},
						}},
					},
