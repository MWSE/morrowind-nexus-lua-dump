...
function createMealPreview()
	...
	local flavorButton
	flavorButton = makeButton(nil, {size = v2(100, 100)}, confirmCooking, G_morrowindGold, flex_V_H_V3, getTexture("textures/SunsDusk/pot0.png"), nil, getTexture("textures/SunsDusk/potConfirm.png"))
	mealPreviewElements.flavorImage = flavorButton.image
	mealPreviewElements.cookButton = flavorButton
	-- white pot art tinted gold (same as xButton); defaults handle the over/pressed tint on hover/press
	flavorButton.image.props.color = G_morrowindGold
...
-- mirror the player's own cooking skill into the mwscript global for other mods
-- pushes only on change; the innkeeper override on cookingLevelModified is ignored
local lastPushedCookingSkill
local function pushCookingGlobal()
	local skill = math.floor(cookingStat.modified)
	if skill ~= lastPushedCookingSkill then
		lastPushedCookingSkill = skill
		core.sendGlobalEvent("SunsDusk_setCookingGlobal", {self, skill})
	end
end
G_onFrameJobsSluggish.pushCookingGlobal = pushCookingGlobal
