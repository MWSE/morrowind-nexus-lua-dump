...
local function raycastChanged()
	...
	-- lower priority wins contested slots (matches crafting framework convention); default 0
...
-- ------------------------------ teamod cup return ------------------------------
local teaModContainerCups = {
	kb_mug = "teamod_mug_kb",
	kb02 = "teamod_teacup_kb02",
	calsilv = "teamod_teacup_cali_silv",
	calred = "teamod_teacup_cali_red",
	q2 = "teamod_teacup_q2",
	q6 = "teamod_teacup_q6",
	q7 = "teamod_teacup_q7",
	qg = "teamod_teacup_qg",
	st01 = "teamod_teacup_st01",
	st02 = "teamod_teacup_st02",
	st03 = "teamod_teacup_st03",
	st04 = "teamod_teacup_st04",
	st05 = "teamod_teacup_st05",
	st06 = "teamod_teacup_st06",
	st07 = "teamod_teacup_st07",
	st08 = "teamod_teacup_st08",
	st09 = "teamod_teacup_st09",
	st10 = "teamod_teacup_st10",
	st11 = "teamod_teacup_st11",
	st12 = "teamod_teacup_st12",
}

-- tmt_(cof|tea)_*                     basic served form, st12 default (Aziza uses st10)
-- tm_(cof|tea)_<container>_<type>NN   cup-specific brewed form, container names the cup
-- tm_cof_gNN, tm_tea_<type>NN         containerless brewed form, silver kb02 cup
local function resolveTeaModCup(id)
	if id:sub(1, 8) == "tmt_tea_" or id:sub(1, 8) == "tmt_cof_" then
		return (id == "tmt_tea_aziza") and "teamod_teacup_st10" or "teamod_teacup_st12"
	end
	if id:sub(1, 3) ~= "tm_" then return nil end
	local container = id:match("^tm_[a-z]+_(.-)_%a+%d+$")
	if container then return teaModContainerCups[container] end
	if id:match("^tm_cof_g%d+$") or id:match("^tm_tea_%a+%d+$") then
		return "teamod_teacup_kb02"
	end
	return nil
end
	...
	getActions = function(object, objectType)
		...
		return {{
			...
			handler = function(obj)
				...
				-- teamod potions: world swap to empty cup instead of downgrade
				local cupId = resolveTeaModCup(object.recordId)
				local lastInStack = obj.count == 1
				core.sendGlobalEvent('UseItem', {object = object, actor = self})
				if lastInStack then
					if cupId then
						core.sendGlobalEvent('SunsDusk_TeaMod_replaceWorldObj', {sourceObject = object, cupId = cupId})
					else
