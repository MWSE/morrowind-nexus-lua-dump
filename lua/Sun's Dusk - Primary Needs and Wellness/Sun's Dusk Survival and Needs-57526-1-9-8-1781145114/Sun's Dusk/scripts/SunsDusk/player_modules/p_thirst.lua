...
-- storms that parch you faster (substring match on weatherName:lower())
local thirstyWeatherTypes = {
	["ashstorm"]   = 1.7, -- gritty ash dries you out
	["blight"]     = 2.0, -- tainted red mountain air, worst
	["blizzard"]   = 1.2, -- cold dry air, mild
	["sporestorm"] = 1.5, -- modded weather
	["spore"]      = 1.5, -- modded weather alias
}
...
local function updateWidget()
	if not NEEDS_THIRST or T_HIDE then return end

	-- Calculate current values
	local skinData = G_iconPacks.thirst[T_SKIN]

	-- content skin owns its whole look; the staged path below is skipped
	if skinData.content then
		local ctx = {
			skin = skinData,
			value = thirstData.thirst,
			buff = thirstData.currentThirstBuff,
		}
		local widget = G_renderContentSkin({ name = "m_thirst", order = "needs-thirst" }, ctx)
	...
	local alphaInput = HUD_INVERT_ALPHA and (1 - thirstData.thirst) or thirstData.thirst
	local currentAlpha = HUD_ALPHA == "Static" and 1 or getWidgetAlpha(alphaInput)
	if T_HIDE_NO_BUFF then
		if thirstData.currentThirstBuff == false then
			-- no active effect for the current mode (false only happens in restricted modes): hide
			currentAlpha = 0
		elseif NEEDS_THIRST_BUFFS_DEBUFFS ~= "Buffs and debuffs" then
	...
	local bgAlpha = T_BACKGROUND == "Classic" and (HUD_ALPHA == "Static" and 1 or currentAlpha^2) or (currentAlpha > 0 and 0.5 or 0)
	...
	if lastAlpha ~= currentAlpha then
		...
		if ((lastAlpha or 1) > 0) ~= (currentAlpha > 0) then
			G_columnsNeedUpdate = true
		end
		lastAlpha = currentAlpha
		thirstWidget.layout.userData.effectiveAlpha = currentAlpha
		needsUpdate = true

		-- Update background alpha for all styles
		if thirstBackground and thirstBackground.props then
			thirstBackground.props.alpha = bgAlpha
		end
	end
...
function module_thirst_minute(clockHour, minute, minutesPassed)
	...
	-- environmental thirst multiplier (storms + body heat; skipped in jail)
	local envThirstMult = 1
	if not G_isInJail then
		-- storms (only outdoors, only when uncovered)
		if saveData.weatherInfo and G_cellInfo.isExterior
		and saveData.weatherInfo.hasWindCover ~= true then
			local weatherName = saveData.weatherInfo.weatherName
			if weatherName then
				weatherName = weatherName:lower()
				for weatherType, weatherMod in pairs(thirstyWeatherTypes) do
					if weatherName:find(weatherType) and weatherMod > envThirstMult then
						envThirstMult = weatherMod
					end
				end
			end
		end
		-- body heat ramp; 1.0x at 30c, linear to 2.5x cap at 55c+
		if saveData.m_temp and saveData.m_temp.currentTemp then
			local heatMult = math.min(2.5, 1 + math.max(0, saveData.m_temp.currentTemp - 30) * 0.06)
			if heatMult > envThirstMult then
				envThirstMult = heatMult
			end
		end
	end
	...
	if G_isInJail then
		...
		thirstData.thirst = math.min(1, thirstData.thirst + baseStep * minutesPassed * raceConsumptionMult * envThirstMult)
	end
	...
	local newThirstLevel = math.max(0, math.floor(thirstData.thirst * 6 - 0.00001))
	if newThirstLevel ~= thirstData.thirstLevel then
		log(3, "[SunsDusk:Thirst] Level:", thirstData.thirstLevel, "->", newThirstLevel)
		-- notify only when worsening into a negative tier
		if newThirstLevel > thirstData.thirstLevel and newThirstLevel >= 3 then
			local msg = needsLevelMessages["thirst_"..newThirstLevel]
			if msg then
				messageBox(newThirstLevel >= 5 and 2 or 3, msg)
			end
		end
	end
	thirstData.thirstLevel = newThirstLevel
	...
	if newThirstBuff ~= thirstData.currentThirstBuff then
		...
		if newThirstBuff then
			...
			thirstData.currentThirstBuff = false
		end
	end
...
local function settingsChanged(sectionName, setting, oldValue)
	if setting == "NEEDS_THIRST" and NEEDS_THIRST ~= oldValue then
	...
	elseif setting == "T_BACKGROUND" or setting == "T_SKIN" then
		G_destroyThirstUi()
	end
end
...
function G_getDrinkValue(item)
	...
	if entry and entry.drinkValue and entry.drinkValue ~= 0 then
		...
		-- Vampire penalty
		if isVampire and NEEDS_THIRST_VW == "Immortal" and not item.recordId:find("blood") and not item.type.record(item).name:lower():find("blood") then
			drinkValue = drinkValue * 0.1
		end
		-- non-vampires get reduced restore from blood
		if not isVampire and G_bloodItemIds[item.recordId] then
			drinkValue = drinkValue * 0.4
		end
...
local function landedSpellHit(target, spell)
	if saveData.playerInfo.isVampire > 0 and NEEDS_THIRST_VW ~= "Disable" then
		...
		if totalMagnitude > 0 then
			local restoredThirst = 0.06+totalMagnitude/333
			if NEEDS_THIRST_VW == "Supernatural" then
				restoredThirst = restoredThirst * 0.6
			end
