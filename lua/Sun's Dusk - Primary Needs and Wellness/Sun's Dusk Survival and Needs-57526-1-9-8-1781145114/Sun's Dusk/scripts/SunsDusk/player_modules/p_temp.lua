...
--- second digit:
--- log level ( "Silent", "Quiet", "Chatty", "Deep", "Trace", "Spammy" )
-- tempDbg2.p(level%10,...)
--local TESTING_WIDGET2 = true
...
local function getRacialCellTypeBonus()
	...
	if not racialData or not racialData.cellTypeBonus then
		return 0
	end
...
-- Returns temperature, climateType, redMountainInfluence, tooltipEntries
local function calculateClimateAtPosition(pos)
	...
	for _, source in ipairs(CLIMATE_SOURCES) do
		...
		if distance < source.radius then
			...
			if source.name:lower():find("red mountain") and influence >= 0.1 then
				redMountain = redMountain + influence
			end
			...
			table.insert(tooltipEntries,string.format("  %s (%s, mode=%d): dist=%.0f, influence=%.2f, intensity=%s", 
	...
	local finalTemp = WORLD_BASE_TEMP + additiveIntensity + maxIntensity + smoothIntensity
	
	return finalTemp, dominantType, redMountain, tooltipEntries, additiveIntensity, maxIntensity, smoothIntensity
end
...
local function getClimateTempFromPosition(dt)
	...
	if not cell or not cell.isExterior then 
		if G_cellInfo.isExterior then
			if G_cellInfo.fixedTemperature then
				-- Use provided temperature directly
				climateTemperature = G_cellInfo.temperature
				climateType = G_cellInfo.climateType or "urban"
				climateTempTooltip3 = string.format("  Cell temperature: %s", formatTemperature(climateTemperature))
			else
				-- Original QUASI_EXTERIOR_CLIMATE_SOURCES lookup
				climateType = "urban"
			...
			G_cellInfo.temperature = climateTemperature
			G_cellInfo.climateType = climateType
		end
		-- Next exterior climate (cached until cellInfo flush)
		if G_cellInfo.nextExteriorPosition and not G_cellInfo.nextExteriorTemperature then
			if G_cellInfo.nextExteriorCellIsQuasiExterior then
				-- quasi-exterior: match by cell ID
				local neClimate = "urban"
				local neTemp = 26
				local maxPriority = -99999
				if G_cellInfo.nextExteriorCell then
					for _, source in pairs(QUASI_EXTERIOR_CLIMATE_SOURCES) do
						if G_cellInfo.nextExteriorCell.id:find(source.match, 1, true) and source.priority > maxPriority then
							neClimate = source.climate
							neTemp = source.temperature
							maxPriority = source.priority
						end
					end
				end
				G_cellInfo.nextExteriorTemperature = neTemp
				G_cellInfo.nextExteriorClimateType = neClimate
			else
				-- true exterior: vector calculation
				local neTemp, neClimate = calculateClimateAtPosition(G_cellInfo.nextExteriorPosition)
				G_cellInfo.nextExteriorTemperature = neTemp
				G_cellInfo.nextExteriorClimateType = neClimate
			end
		end
	...
	local finalTemp, dominantType, redMountain, tooltipEntries, addI, maxI, smoothI = calculateClimateAtPosition(pos)
	
	isOnRedMountain = redMountain
	climateTempTooltip2 = tooltipEntries
	...
	climateTempTooltip3 = string.format("  Base: %s, Add: %+.1f, Max: %+.1f, Smooth: %+.1f, Final: %s", 
		formatTemperature(WORLD_BASE_TEMP), addI, maxI, smoothI, formatTemperature(finalTemp))
...
local function getSolarHeating(hour, climate)
	if not climate then 
		return 0 
	end
	...
	if timeOfDayFactor <= 0.1 then -- Night time or near night
		...
		if climate == "ashland" or climate == "arctic" or climate == "cold" then
			-- Ashlands get cold at night (desert wasteland)
			nightEffect = -8
			
			tempDbg.p(14, string.format("  %s night cooling: %s", climate, formatTemperatureModifier(-8)))
			
		elseif climate == "volcanic" or climate == "tropical" then
			-- these climates stay warm from heat retention
			nightEffect = 4
			
			tempDbg.p(14, string.format("  %s night warmth: %s", climate, formatTemperatureModifier(4)))
		
--[[		elseif climate == "mountain" then
--			-- mountains get colder at night from altitude
--			nightEffect = -5
			tempDbg.p(14, " Mountain night cooling: " .. formatTemperatureModifier(5))
]]
		elseif climate == "coast" or climate == "urban" then
			-- Coast has moderate temperature from ocean effect ; urban also moderate temperature bc of population + lights and whatnot
			nightEffect = -2
			
			tempDbg.p(14, string.format("  %s moderate night: %s", climate, formatTemperatureModifier(-2)))
		else
	...
	tempDbg.p(13, string.format("  Solar: hour=%.1f, factor=%.2f, heat=%s, climate=%s", hour, timeOfDayFactor, formatTemperatureModifier(solarHeat), climate or "temperate"))
...
local function getInteriorTemperature(cell)
	...
	if G_cellInfo then
		...
		elseif G_cellInfo.isCave or G_cellInfo.isCastle or G_cellInfo.isTomb then
			...
			insulation = G_cellInfo.isCave and 0.8 or 0.9
...
--[[	elseif G_cellInfo.isAshlander then
		...
		-- Fixed temperature overrides base temp, keeps type-based insulation
		if G_cellInfo.fixedTemperature then
			temp = G_cellInfo.temperature
		end
		
		-- Exterior climate bleeds into interior
		if G_cellInfo.nextExteriorTemperature and insulation < 1 then
			local actualExteriorTemp = G_cellInfo.nextExteriorTemperature + (G_cellInfo.solarHeating or 0)
			local exteriorDelta = actualExteriorTemp - temp
			local shift = exteriorDelta * (1 - insulation)
			temp = temp + shift
			tempDbg.p(12, string.format("  Exterior: %s (%s), insulation=%.0f%%, shift=%s",
				formatTemperature(actualExteriorTemp),
				G_cellInfo.nextExteriorClimateType or "?",
				insulation * 100,
				formatTemperatureModifier(shift)))
		end
	end
	if tempData.cellTempModCellId then
		if tempData.cellTempModCellId ~= cell.id then
			tempData.cellTempModCellId = nil
		else
			temp = temp + (tempData.cellTempMod or 0)
		end
	end
...
local function getNearbyHeatSources()
	...
	local function processHeatSource(object, sourceTable, isStaticFire)
		if not (object:isValid() and object.position) then
			return
		end
		...
		if dbEntry ~= nil then
			if dbEntry == false then
				return
			end
			matchingDistance = dbEntry.radius or matchingDistance
			matchingHeat = dbEntry.temperatureMod or matchingHeat
			if dbEntry.comfort and distance < matchingDistance/2 then
				hearthfireMagnitude = math.max(hearthfireMagnitude, dbEntry.comfort)
			end
			ignoreTemperatureLimit = dbEntry.ignoresMaxTemp
		else
		...
		if distance < matchingDistance then
			...
			if not isStaticFire and saveData.companions[object.id] then
	...
	for _, object in ipairs(G_cellInfo.fires) do
		processHeatSource(object, fireTags, true)
	end
	
	-- Process actor-based sources
	for _, object in ipairs(nearby.actors) do
		if types.Creature.objectIsInstance(object) or dbStatics[object.recordId] and dbStatics[object.recordId].heatsource then
			processHeatSource(object, actorTags, false)
		end
	end
	
	-- alternatively with refid support:
	--for _, object in ipairs(nearby.actors) do
	--	local refEntry, recordEntry = dbStatics[object.id], dbStatics[recordId]
	--	local dbEntry = refEntry and refEntry.heatsource
	...
	--	end
	--
	--	if types.Creature.objectIsInstance(object) or dbEntry then
	--		processHeatSource(object, actorTags, false)
	--	end
	--end
...
G_fatigueSpentJobs.combatWarmth = function(fatigue, dt)
	if not tempData then return end
	if fatigue > 1 then
		tempData.combatWarmth = math.min(3, tempData.combatWarmth + math.min(0.3, fatigue/20))
		--print(tempData.combatWarmth,"att", math.min(0.3, fatigue/20))
	end
end
...
local function getHeatSourceModifier(currentTemp)
	...
	-- movement slows cooling
	if self.controls.movement ~= 0 or self.controls.sideMovement ~= 0 then
		G_coolRate = G_coolRate * 0.5
		tempDbg.p(12, "Moving: 0.50x cool rate")
	end

	-- combat exertion accumulator, ramps in ~10s, drains in ~20s
	if G_combatTargetCount > 0
	and (self.controls.movement ~= 0 or self.controls.sideMovement ~= 0) then
		tempData.combatWarmth = math.min(3, tempData.combatWarmth + secondsPassed * 0.3)
		--print(tempData.combatWarmth,secondsPassed * 0.3)
	else
		tempData.combatWarmth = math.max(0, tempData.combatWarmth - secondsPassed * 0.15)
		--print(tempData.combatWarmth,- secondsPassed * 0.15)
	end
	...
	-- combat exertion: own additive warmth, caps at ideal
	if tempData.combatWarmth > 0 and effectiveTemp < G_idealTemperature then
		local addition = math.min(tempData.combatWarmth, G_idealTemperature - effectiveTemp)
		effectiveTemp = effectiveTemp + addition
		tempDbg.p(12, string.format("Combat exertion: %s", formatTemperatureModifier(addition)))
	end
...
local function getActiveTemperatureEffects()
	fireRes = typesActorActiveEffectsSelf:getEffect("resistfire").magnitude
	frostRes = typesActorActiveEffectsSelf:getEffect("resistfrost").magnitude
	fireWeak = typesActorActiveEffectsSelf:getEffect("weaknesstofire").magnitude
	frostWeak = typesActorActiveEffectsSelf:getEffect("weaknesstofrost").magnitude

	fireShield = typesActorActiveEffectsSelf:getEffect("fireshield").magnitude
	frostShield = typesActorActiveEffectsSelf:getEffect("frostshield").magnitude
	fireDamage = typesActorActiveEffectsSelf:getEffect("firedamage").magnitude
	frostDamage  = typesActorActiveEffectsSelf:getEffect("frostdamage").magnitude
end
getActiveTemperatureEffects()
G_perMinuteJobs.getActiveTemperatureEffects = getActiveTemperatureEffects
table.insert(G_equipmentChangedJobs, getActiveTemperatureEffects)
table.insert(G_onConsumeJobs, getActiveTemperatureEffects)
...
local function getMagicTemperatureModifier(currentTemp, targetTemp)
	...
	if fireDamage > 0 then
		fireDamage = fireDamage / math.max(0.01, 1-fireRes/100)
	...
	if frostDamage > 0 then
		frostDamage = frostDamage / math.max(0.01, 1-frostRes/100)
	...
	if comfortDrift > 0 then
		...
		local driftMod = math.max(-maxMod, math.min(tempGap, maxMod))
		modifier = modifier + driftMod
	end
...
function calculateEnvironmentTemperature()
	...
	if G_cellInfo.isExterior then
		...
		--end
		-- 2. Solar heating
		local solarHeat = getSolarHeating(hour, climateType)
		G_cellInfo.solarHeating = solarHeat
		...
		climateType = G_cellInfo.climateType or "temperate"
		G_cellInfo.solarHeating = getSolarHeating(hour, G_cellInfo.nextExteriorClimateType)
	...
	if tempData.stewDuration then
		local modifier
		if tempData.stewMagnitude > 0 then
			-- Warming effect: reduces cold, capped by magnitude
			modifier = math.min(tempData.stewMagnitude, math.max(0, 21 - temperature))
		else
			-- Cooling effect: reduces heat, capped by magnitude
			modifier = math.max(tempData.stewMagnitude, math.min(0, 21 - temperature))
		end
		temperature = temperature + modifier
		tempData.stewDuration = tempData.stewDuration - secondsPassed
		if tempData.stewDuration <=0 then
			tempData.stewDuration = nil
			tempData.stewMagnitude = nil
		else
				tempDbg.p(11, string.format("Lasting temp mod: %s", formatTemperatureModifier(modifier)))
		end
	end
...
local function removeCurrentTempBuff()
	if tempData and tempData.currentTempBuff then
		if core.magic.spells.records[tempData.currentTempBuff] then
...
local function removeCurrentTempSlowDebuff()
	if tempData.currentSlowDebuff then
		if core.magic.spells.records[tempData.currentSlowDebuff] then
...
local function removeTempBuffs()
	...
	if tempData.water.wetnessDebuff then
		if core.magic.spells.records[tempData.water.wetnessDebuff] then
	...
	if tempData.currentHearthfireBuff then
		if core.magic.spells.records[tempData.currentHearthfireBuff] then
...
local function applyTemperatureEffects(state)
	...
	-- if receiving warmth bonus from lava then take spell damage
		...
		if nearbyLava and not hasFireImmunity then
			if tempData.water.wetness < 0.1 then
				...
				if math.random() < 0.3*secondsPassed then
					G_flashVignette = 0.4
					typesPlayerStatsSelf.health.current = typesPlayerStatsSelf.health.current - 0.5
				end
			end
		end
		...
		if debuff then
			...
			-- Apply damage
			local maxHealth = typesPlayerStatsSelf.health.base
			local dmg = math.min(maxHealth * 0.2, (0.175+maxHealth * 0.0007) * severity * secondsPassed * resistanceMult)
			if not debug.isGodMode() then
				typesPlayerStatsSelf.health.current = typesPlayerStatsSelf.health.current - dmg
			end
		else
	...
	-- nil = loading, false = no buff for current setting+state, string = applied spell id
	local spellId = state.spellId
	local isBuff = spellId == "sd_temp_2" or spellId == "sd_temp_3"
	local settingAllowsBuff = TEMP_BUFFS_DEBUFFS == "Only buffs" and isBuff
		or TEMP_BUFFS_DEBUFFS == "Only debuffs" and not isBuff
		or TEMP_BUFFS_DEBUFFS == "Buffs and debuffs"

	local newBuff
	if not settingAllowsBuff then
		newBuff = false
	elseif G_preventAddingAnyBuffs then
		newBuff = nil
	else
		newBuff = spellId
	end

	if tempData.currentTempBuff ~= newBuff then
		removeCurrentTempBuff()
		if newBuff then
			typesActorSpellsSelf:add(newBuff)
			tempDbg.p(03, string.format("Applied buff: %s (%s)", newBuff, state.name))
		end
		tempData.currentTempBuff = newBuff
	end
end
...
local function module_temp_minute(c, refreshUi)
	...
	local minutesPassed = math.max(0, (nowGT - lastGametimeUpdate) / time.minute)
	
	if c and minutesPassed < 30 then -- dont update on regular hourly events, only onFrameSluggish
		return
	end
	secondsPassed = math.max(0, (nowGT - lastGametimeUpdate) /  math.max(0.1, core.getGameTimeScale()))
	...
	local remainingFactor = math.max(0, 1 - G_temperatureRate/100)^secondsPassed
	...
	if tempData.lastTempStateName ~= state.name then
		log(3, "[SunsDusk:Temp] State:", tempData.lastTempStateName or "?", "->", state.name)
		-- notify only when worsening (moving away from Comfortable)
		local tempStateOrder = {
			Freezing    = 1,
			Cold        = 2,
			Chilly      = 3,
			Comfortable = 4,
		...
		local oldDist = math.abs((tempStateOrder[tempData.lastTempStateName or "Comfortable"] or 4) - 4)
		local newDist = math.abs((tempStateOrder[state.name] or 4) - 4)
		if tempData.lastTempStateName and newDist > oldDist then
			local msg = needsLevelMessages["temp_"..state.name:lower()]
			if msg then
				messageBox(newDist >= 3 and 2 or 3, msg)
			end
		end
		tempData.lastTempStateName = state.name
	end
	...
	if tempData.currentTempBuff == nil then
		G_temperatureWidgetTooltip = G_temperatureWidgetTooltip.."\nLoading..."
	elseif tempData.currentTempBuff == false then
		G_temperatureWidgetTooltip = G_temperatureWidgetTooltip.."\nNo effects"
	else
		G_temperatureWidgetTooltip = G_temperatureWidgetTooltip.."\n"..(tooltips[tempData.currentTempBuff] or "Error: "..tostring(tempData.currentTempBuff))
	end
	...
	and G_isInWater <= 0.5
	and not saveData.playerInfo.isFarmingTool
	then
		local temperatureMitigation = math.max(0,(climateTemperature-20)/10)
		...
		G_temperatureWidgetTooltip = G_temperatureWidgetTooltip.."\n"..(not newWetnessDebuff and "Loading..." or tooltips[newWetnessDebuff] or "Error: "..tostring(newWetnessDebuff))
	end
	
	if newWetnessDebuff ~= tempData.water.wetnessDebuff then
		if tempData.water.wetnessDebuff and core.magic.spells.records[tempData.water.wetnessDebuff] then
			typesActorSpellsSelf:remove(tempData.water.wetnessDebuff)
		end
	...
	if not G_preventAddingAnyBuffs and hearthfireMagnitude >= 2 then
	...
	if newHearthfireBuff ~= tempData.currentHearthfireBuff then
		if tempData.currentHearthfireBuff and core.magic.spells.records[tempData.currentHearthfireBuff] then
			typesActorSpellsSelf:remove(tempData.currentHearthfireBuff)
		end
...
local function onConsume(item)
	...
	-- Check if food is fresh (no timestamp or less than 3 hours old)
	local isFresh = true
	if entry and entry.timestamp then
		local age = core.getGameTime() - entry.timestamp
		isFresh = age < (3 * time.hour)
	end
	
	if isFresh then
		if entry and entry.warmthValue and entry.warmthValue ~= 0 then
			if entry.warmthValue > 0 then
				-- Warming: only warms up, caps at 24°C
				tempData.currentTemp = tempData.currentTemp + math.min(entry.warmthValue, math.max(0, 24 - tempData.currentTemp))
			else
				-- Cooling: only cools down, floors at 11°C
				tempData.currentTemp = tempData.currentTemp + math.max(entry.warmthValue, math.min(0, 11 - tempData.currentTemp))
			end
			--more human readable version:
			--if entry.warmthValue > 0 then
			--	local newTemp = tempData.currentTemp + entry.warmthValue
			--	local effectiveCap = math.max(24, tempData.currentTemp)
			--	tempData.currentTemp = math.min(newTemp, effectiveCap)
			--else
			--	local newTemp = tempData.currentTemp + entry.warmthValue
			--	local effectiveFloor = math.min(11, tempData.currentTemp)
			--	tempData.currentTemp = math.max(newTemp, effectiveFloor)
			--end
		end
		if entry and entry.warmthValue2 and entry.warmthValue2 ~= 0 then
			tempData.stewDuration = 120
			tempData.stewMagnitude = entry.warmthValue2
		end
	end
	module_temp_minute(nil,  0)
end
...
local function onLoad()
	...
	tempData.combatWarmth = tempData.combatWarmth or 0
	scanEquipment()
...
-- force buff/widget refresh; ignores call when needId is set and not "temp"
local function refreshNeeds(needId)
	if needId and needId ~= "temp" then return end
	if not NEEDS_TEMP or not tempData then return end
	module_temp_minute(nil, true)
end
...
local function settingsChanged(sectionName, setting, oldValue)
	...
	elseif setting == "TEMP_TOTSP" and TEMP_TOTSP ~= oldValue then
	...
	elseif setting == "TEMP_BAR_STYLE" then
		G_destroyTemperatureUis()
		--refreshUi()
	elseif setting == "TESTING_WIDGET2" then
		-- temperature dbg
		if TESTING_WIDGET2 ~= "off" and not tempDbg2 then
			tempDbg2 = require("scripts.SunsDusk.ui_debugWidget")("=============== Temperature ===============")
		elseif TESTING_WIDGET2 == "off" and tempDbg2 then
			tempDbg2.destroy()
			tempDbg2 = nil
		end
		-- weather dbg
		local wantWeather = TESTING_WIDGET2 == "+weather" or TESTING_WIDGET2 == "+equipment"
		if wantWeather and not weatherDbg then
			weatherDbg = require("scripts.SunsDusk.ui_debugWidget")("=============== Weather ===============")
		elseif not wantWeather and weatherDbg then
			weatherDbg.destroy()
			weatherDbg = nil
		end
		-- equipment dbg
		if TESTING_WIDGET2 == "+equipment" and not armorDbg2 then
			armorDbg2 = require("scripts.SunsDusk.ui_debugWidget")("===== Armor Equip =====")
		elseif TESTING_WIDGET2 ~= "+equipment" and armorDbg2 then
			armorDbg2.destroy()
			armorDbg2 = nil
		end
	end
