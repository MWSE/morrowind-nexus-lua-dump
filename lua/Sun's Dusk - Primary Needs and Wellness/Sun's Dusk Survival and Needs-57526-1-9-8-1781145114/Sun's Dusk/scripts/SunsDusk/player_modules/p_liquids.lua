-- Interiors (first match, checking top to bottom):
--   isSewer         = "susWater",
--   isBath          = "water",
--   isTemple        = "water",
--   isHouse         = "water",
...
--   isRedoran       = "water",
--   isMushroom      = "water",
--   isDaedric       = "susWater",  -- water if WATER_CLEAN_DUNGEONS
--   isDwemer        = "susWater",  -- water if WATER_CLEAN_DUNGEONS
--   isTomb          = "susWater",  -- water if WATER_CLEAN_DUNGEONS
--   isMine          = "susWater",  -- water if WATER_CLEAN_DUNGEONS
--   isIceCave       = "water",
--   isCave          = "susWater",  -- water if WATER_CLEAN_DUNGEONS
--   quasi-exterior  = "water",     -- quasi-exterior (interior flagged as exterior)
--   default         = "water"
--
-- Exteriors: nearest WATER_BODIES source
--   [1]             = "water"
--   [2]             = "saltWater"
--   [3]             = "susWater"
--   default         = "saltWater"
--
-- Override Settings:
--   WATER_CLEAN_ALWAYS        -> "water" everywhere
--   WATER_CLEAN_VIVEC         -> saltWater near Vivec becomes "water"
--   WATER_CLEAN_BODIES=false  -> exterior "water" becomes "susWater"
...
local function getCellWaterType(dt)
	...
	elseif not cell.isExterior and G_cellInfo.waterType then
		-- water set by dbStatics
...
local function getWellWaterType(object)
	...
	local dbEntry = refEntry and refEntry.well
	if dbEntry == nil then
		dbEntry = recordEntry and recordEntry.well
	end

	local staticLiquidType = dbEntry and dbEntry.liquidType
...
local function isWell(object, objectType)
	if objectType ~= "Static" and objectType ~= "Activator" then 
		return false
	end

	local refEntry, recordEntry = dbStatics[object.id], dbStatics[object.recordId]
	local dbEntry = refEntry and refEntry.well
	if dbEntry == nil then
		dbEntry = recordEntry and recordEntry.well
	end
	...
	for _, key in ipairs(G_wellKeywords) do
		if id:find(key, 1, true) then
			for _, blackKey in ipairs(G_wellBlacklist) do
				if id:find(blackKey, 1, true) then
					return false
				end
			end
			return true
...
G_worldInteractions.well = {
	canInteract = function(object, objectType)
		if not WATER_REFILL then return false end
		return isWell(object, objectType)
	end,
	getActions = function(object, objectType)
		local liquidType = getWellWaterType(object)
		return {{
			label = "Refill " .. (localizedLiquidNames[liquidType] or "water"),
			preferred = "Activate",
			handler = function(obj)
				log(3, "activated well ("..obj.recordId..") refilling with "..(liquidType or "water"))
				if countActivations == 0 then
					...
					core.sendGlobalEvent("SunsDusk_WaterBottles_refillSpillables", {self, liquidType})
...
-- Reset activation count when target changes
local function onTargetChanged()
	countActivations = 0
end
table.insert(G_raycastChangedJobs, onTargetChanged)

local function verifyItems(items)
	local verifiedItems = {}
	for _, recordId in pairs(items) do
...
local SALTWATER_ITEMS = verifyItems({
...
local SUSPICIOUS_WATER_ITEMS = verifyItems({
	"ingred_heather_01",
	"ingred_muck_01",
...
local function waterPurified(liquids)
	...
	if liquids["saltWater"] then
		...
		for i=1, runs do
			if math.random() > failchance then
				local randomItem = SALTWATER_ITEMS[math.random(1,#SALTWATER_ITEMS)]
	...
	if liquids["susWater"] then
		...
		for i=1, runs do
			if math.random() > failchance then
				local randomItem = SUSPICIOUS_WATER_ITEMS[math.random(1,#SUSPICIOUS_WATER_ITEMS)]
				local localizedName = types.Ingredient.records[randomItem].name
...
local function onMiscUsed(item)
	if G_isInWater > 0.1 and WATER_REFILL_SWIMMING then
		local liquidType = G_cellInfo.waterType or "saltWater"
		core.sendGlobalEvent("SunsDusk_WaterBottles_refillSingleBottle", {
			player = self, 
			item = item,
