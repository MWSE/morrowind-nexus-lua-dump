...
MODNAME = "SunsDusk"
VERSION = 6  -- API version; addons read I.SunsDusk.version (6 added: actorDied jobs, aggroActors, landedSpellHit isHostile)
...
if core.contentFiles.has("survivalmode.omwscripts") then return end
...
G_onActiveJobs                  = {} -- () fires after the player becomes active, interfaces are bound by then
G_onSaveJobs                    = {}
...
G_DialogueResponseJobs          = {} -- (data, topic, info) data is {actor, type, recordId, infoId}; topic/info resolved from core.dialogue
...
G_landedSpellHitJobs            = {} -- (target, spell, isHostile) spell is resolved activeSpell; isHostile nil/true = hostile, false = beneficial-only
G_actorDiedJobs                 = {} -- (actor) game object, fan-out of SunsDusk_actorDied from sd_a
G_fatigueSpentJobs              = {} -- (amount, dt) per-frame fatigue spent by player, excludes incoming hit damage
-- sleep
G_removeAbilitiesJobs           = {} -- () called when the rest menu opens to remove all buffs for the LevelUp screen and after chargen
...
-- dev tools
G_drinkwareTuning               = false -- true only after 'lua drinkwaretuner' this session; gates tuner hotkeys
-- player environment
...
G_currentBed                    = nil
G_combatTargetCount             = 0     -- distinct actors currently targeting the player
...
require('scripts.SunsDusk.sd_loadSkins')
...
local function actorDied(actor)
	for _, fn in pairs(G_actorDiedJobs) do fn(actor) end
end
...
local function landedSpellHit(data)
	...
	if foundSpell then
		-- any harmful effect = hostile; nil stays hostile by convention
		local isHostile
		for _, effect in pairs(foundSpell.effects) do
			local rec = effect.id and core.magic.effects.records[effect.id]
			if rec and rec.harmful then
				isHostile = true
				break
			elseif rec then
				isHostile = false
		...
		for _, func in pairs(G_landedSpellHitJobs) do
			func(target, foundSpell, isHostile)
		end
	end
end

-- if tr_spells (v2+) is present it feeds the same hits via TD_SpellAdded; defer to it to avoid
-- double-feeding. resolved in onActive, when interfaces are bound.
local trSpellFeedActive = false
table.insert(G_onActiveJobs, function()
	trSpellFeedActive = I.TRSpells ~= nil and (I.TRSpells.version or 0) >= 2
end)

local function sunsDuskLandedSpellHit(data)
	if trSpellFeedActive then return end
	landedSpellHit(data)
end

local function trSpellAdded(data)
	if data.caster and types.Player.objectIsInstance(data.caster) then
		landedSpellHit({ data.target, data.activeSpellId })
	end
end

-- combat music system reports each actor's targets, count those aiming at us
G_aggroActors = {} -- [actorId] = actor, kept while targeting the player (read by addons for hostility)
local function combatTargetsChanged(data)
	...
	if targetingPlayer then
		G_aggroActors[data.actor.id] = data.actor
	else
		G_aggroActors[data.actor.id] = nil
	end
	-- recount, dropping any that went invalid (dead, unloaded)
	local count = 0
	for actorId, actor in pairs(G_aggroActors) do
		if actor:isValid() then
			count = count + 1
		else
			G_aggroActors[actorId] = nil
		end
	end
	G_combatTargetCount = count
end

-- ╭────────────────────────────────────────────────────────────────────╮
-- │ Fatigue spent tracker                                              │
-- ╰────────────────────────────────────────────────────────────────────╯

local lastFatigue
-- circular ring; any fatigue drop within these N frames absorbs buffered hit damage
local damageBuffer = {0, 0, 0}
local bufferSize = #damageBuffer
local bufferSum = 0  -- cached total, gates the fast-path skip
local headIdx = 1    -- slot collecting current frame's hits

if I.Combat then
	I.Combat.addOnHitHandler(function(attack)
		if attack.successful and attack.damage and (attack.damage.fatigue or 0) > 0 then
			local dmg = attack.damage.fatigue
			damageBuffer[headIdx] = damageBuffer[headIdx] + dmg
			bufferSum = bufferSum + dmg
		end
	end)
end

G_onFrameJobs.fatigueSpentTracker = function(dt)
	local cur = typesPlayerStatsSelf.fatigue.current
	if lastFatigue == nil then
		lastFatigue = cur
		return
	end
	local spent = lastFatigue - cur
	lastFatigue = cur
	if spent < 0 then spent = 0 end
	-- fast path: nothing pending, no aging needed
	if bufferSum == 0 then
		if spent > 0 then
			for _, func in pairs(G_fatigueSpentJobs) do
				func(spent, dt)
			end
		end
		return
	end
	-- absorb drop against buffered damage, oldest first
	if spent > 0 then
		local idx = headIdx % bufferSize + 1
		for _ = 1, bufferSize do
			if spent == 0 then break end
			local slot = damageBuffer[idx]
			if slot > 0 then
				local take = slot < spent and slot or spent
				damageBuffer[idx] = slot - take
				bufferSum = bufferSum - take
				spent = spent - take
			end
			idx = idx % bufferSize + 1
		end
	end
	-- age: advance head, recycle the oldest slot (now next-newest)
	headIdx = headIdx % bufferSize + 1
	bufferSum = bufferSum - damageBuffer[headIdx]
	damageBuffer[headIdx] = 0
	if spent > 0 then
		for _, func in pairs(G_fatigueSpentJobs) do
			func(spent, dt)
		end
...
-- resolve the matching info once, then fan out to registered jobs
local function DialogueResponse(data)
	...
	-- resolve the matching info once, then cache
	local resolvedInfo
	local infoResolved = false
	local function resolveInfo()
		if not infoResolved then
			infoResolved = true
		...
		return resolvedInfo or {}
	end

	-- lazy proxy, walks infos only when a job reads it
	local lazyInfo = setmetatable({}, {
		__index = function(_, key)
			local resolved = resolveInfo()
			return resolved and resolved[key]
		end,
		__pairs = function()
			return pairs(resolveInfo())
		end,
	})

	for _, func in pairs(G_DialogueResponseJobs) do
		func(data, topic, lazyInfo)
	end
end
...
local function onActive()
	for _, func in pairs(G_onActiveJobs) do
		func()
	end
end
...
local function onConsoleCommand(command, str, selectedObject)
    -- lua coords - print current location info as 5 messages
	if str:match("^lua coords") then
		local pos = self.position
		local yawDeg = math.deg(camera.getYaw()) % 360
		if yawDeg < 0 then yawDeg = yawDeg + 360 end
		local pitchDeg = math.deg(camera.getPitch()) + 90
		
		ui.printToConsole(string.format("name = %q,", self.cell.name), ui.CONSOLE_COLOR.Success)
		if self.cell.isExterior then
			ui.printToConsole("cell = nil,", ui.CONSOLE_COLOR.Success)
		else
			ui.printToConsole(string.format("cell = %q,", self.cell.id), ui.CONSOLE_COLOR.Success)
		end
		ui.printToConsole(string.format("position = v3(%.1f, %.1f, %.1f),", pos.x, pos.y, pos.z), ui.CONSOLE_COLOR.Success)
		ui.printToConsole(string.format("yaw = %.1f,", yawDeg), ui.CONSOLE_COLOR.Success)
		ui.printToConsole(string.format("pitch = %.1f,", pitchDeg), ui.CONSOLE_COLOR.Success)
		return
	end
	
	-- lua tp - teleport to v3(x,y,z) with optional yaw degrees
	if str:match("^lua tp") then
		local x, y, z = str:match("v3%s*%(%s*(%-?[%d%.]+)%s*,%s*(%-?[%d%.]+)%s*,%s*(%-?[%d%.]+)%s*%)")
		if not x then
			ui.printToConsole("tp: bad v3", ui.CONSOLE_COLOR.Error)
			return
		end
		local position = util.vector3(tonumber(x), tonumber(y), tonumber(z))
		-- optional degrees after v3, comma between them is allowed
		local yawDeg = tonumber(str:match("v3%s*%([^%)]*%)%s*,?%s*(%-?[%d%.]+)%s*$"))
		local rotation = yawDeg and util.transform.rotateZ(math.rad(yawDeg)) or nil
		ui.printToConsole("tp", ui.CONSOLE_COLOR.Success)
		core.sendGlobalEvent("PMM_loadLoc", {self, {gridX = 0, gridY = 0, position = position, rotation = rotation}})
		return
	end
	...
	-- lua drinkwaretuner [min|max] - start the drinkware tuner this session
	local tunerArg = str:lower():match("^lua%s+drinkwaretuner%s*(%S*)%s*$")
	if tunerArg then
		local mode
		if tunerArg == "" or tunerArg == "max" then
			mode = 2
		elseif tunerArg == "min" then
			mode = 1
		else
			ui.printToConsole("usage: lua drinkwaretuner [min|max]", ui.CONSOLE_COLOR.Error)
			return
		end
		G_drinkwareTuning = true
		core.sendGlobalEvent("spawnDrinkwareGrid", { actor = self, mode = mode })
		ui.printToConsole("drinkware tuner started (" .. (mode == 1 and "min" or "max") .. " fill)", ui.CONSOLE_COLOR.Success)
		return
	end
...
G_eventHandlers.DialogueResponse = DialogueResponse                            --
...
G_eventHandlers.SunsDusk_landedSpellHit = sunsDuskLandedSpellHit               -- (data) {target, activeSpellId}; defers to tr_spells if present
G_eventHandlers.TD_SpellAdded = trSpellAdded                                   -- (data) tr_spells spell detection; player-cast routed to landedSpellHit
G_eventHandlers.SunsDusk_actorDied = actorDied                                 -- (actor) game object, fan-out to G_actorDiedJobs
G_eventHandlers.OMWMusicCombatTargetsChanged = combatTargetsChanged            -- (data) {actor, targets}, engine combat-music event
...
local function onKeyPress(key)
	...
	-- drinkware tuner hotkeys; live only after 'lua drinkwaretuner' this session
	if G_drinkwareTuning then
		-- 6 and 0 work without focus (6 = restore last when no focus, 0 = print)
		if key.symbol == "6" then
			local hit = G_raycastResultType and G_raycastResult.hitObject or nil
			core.sendGlobalEvent("toggleBlacklist", hit)
		elseif key.symbol == "0" then
			core.sendGlobalEvent("printAllOffsets")
		elseif G_raycastResultType then
			-- shift = 1/4 step for finer adjustments
			local payload = { obj = G_raycastResult.hitObject, mult = key.withShift and 0.25 or 1.0 }
			if key.symbol == "1" then
				core.sendGlobalEvent("scaleUp", payload)
			elseif key.symbol == "2" then
				core.sendGlobalEvent("scaleDown", payload)
			elseif key.symbol == "3" then
				core.sendGlobalEvent("moveUp", payload)
			elseif key.symbol == "4" then
				core.sendGlobalEvent("moveDown", payload)
			elseif key.code == input.KEY.LeftArrow then
				core.sendGlobalEvent("moveLeft", payload)
			elseif key.code == input.KEY.RightArrow then
				core.sendGlobalEvent("moveRight", payload)
			elseif key.code == input.KEY.UpArrow then
				core.sendGlobalEvent("moveForward", payload)
			elseif key.code == input.KEY.DownArrow then
				core.sendGlobalEvent("moveBack", payload)
			end
		end
	end
end
