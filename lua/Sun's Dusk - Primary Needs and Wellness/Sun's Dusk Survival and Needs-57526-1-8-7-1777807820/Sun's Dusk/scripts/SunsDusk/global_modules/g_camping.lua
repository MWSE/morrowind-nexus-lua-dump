	...
	if object.recordId == "sd_campingitem_tent" or object.recordId == "sd_campingitem_tent_perm" then
		table.insert(G_delayedUpdateJobs, {
			...
			function()
				...
				if isPerm then
					-- Complete tent (stage 4) with bedroll
...
local function upgradeTent(data)
	...
	-- Check and consume the required component from inventory
	...
	-- Handle multiple misc items (e.g. stacked sticks) - return extras
	...
	-- Replace the object with the upgraded version
	...
	-- Track the tent, carrying flawless state through rebuilds
	saveData.builtTents = saveData.builtTents or {}
...
local function destroyBuiltTent(data)
	...
	-- Return the removed component to the player if flawless teardown didn't happen
