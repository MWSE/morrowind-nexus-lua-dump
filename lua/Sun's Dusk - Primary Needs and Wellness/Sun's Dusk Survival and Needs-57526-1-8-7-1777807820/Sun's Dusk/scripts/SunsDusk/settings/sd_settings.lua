	...
	settings = {
		{
			...
			argument = {
				disabled = false,
				l10n = "none",
				items = { "Default", "Hard", "Hardcore", "Minimalist", "Ownly"},
			},
		},
		{
			key = "NEEDS_TIREDNESS",
...
local function difficultyPresetJob(oldValue, newValue)
	...
	G_onUpdateJobs["applyPreset"] = function()
		local oldPreset = DifficultyPresets[oldValue]
		local newPreset = DifficultyPresets[newValue]
		-- revert old preset keys to template defaults, except those the new preset will set
		if oldPreset then
			for oldSettingId, _ in pairs(oldPreset) do
				if not (newPreset and newPreset[oldSettingId] ~= nil) then
					for _, template in pairs(settingsTemplate) do
						for _, s in pairs(template.settings) do
							if s.key == oldSettingId then
								local settingsSection = storage.globalSection(template.key)
								settingsSection:set(oldSettingId, s.default)
							end
						end
					end
				end
		...
		-- apply new preset overrides
		if newPreset then
			for newSettingId, newSettingValue in pairs(newPreset) do
				for _, template in pairs(settingsTemplate) do
					for _, s in pairs(template.settings) do
...
for _, template in pairs(settingsTemplate) do
	...
	settingsSection:subscribe(async:callback(function (_,setting)
		...
		if world and setting == "DIFFICULTY_PRESET" then
