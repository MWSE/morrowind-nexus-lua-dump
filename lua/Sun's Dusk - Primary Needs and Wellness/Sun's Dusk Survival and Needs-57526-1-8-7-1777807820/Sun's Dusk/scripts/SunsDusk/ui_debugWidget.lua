...
local function makeDebugWidget(name)
	...
	-- storage for all debug widgets
	settingsSection = storage.playerSection("ownlysDebugWidgets")
	
	-- try loading position
	local function clampPosition(pos)
		local layerId = ui.layers.indexOf("HUD")
		
		-- Clamp (at least 50px visible)
		local minSize = 50
		return v2(
	...
	widget.layout.events = {
		...
		mouseMove = async:callback(function(data, elem)
			if elem.userData.isDragging then
				...
				elem.userData.lastMousePos = data.position
				local newPos = (widget.layout.props.position or v2(0, 0)) + delta
	...
	-- clear() - Resets data (does not refresh)
	local function clear()
		prints = {}
	end
	
	-- print() - Concatenates to content
	local function print(...)
		local n = select("#", ...) -- includes nil arguments
		local first = select(1, ...)
	
		-- If a single table is passed, print its contents
	...
	local function display(...)
		...
		if n == 1 and type(first) == "table" then
			...
			-- Each argument is a separate line
			for i = 1, n do
				local line = select(i, ...)
				contentFlex.content:add({
					type = ui.TYPE.Text,
