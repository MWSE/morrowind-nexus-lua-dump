...
-- ════════════════════════════════════════════════════════════════════
-- Wood Count Modifier API (for other mods)
-- ════════════════════════════════════════════════════════════════════
-- Other mods can register a function here to modify the wood drop count.
-- The function receives a context table and should return a number (the new
-- count) or nil to leave the count unchanged. Modifiers run in registration
-- order on the fractional count, before the probabilistic round-up.
--
-- Context fields:
--   woodCount         - current fractional count (modify this)
--   originalWoodCount - the unmodified base count (read-only reference)
--   item              - item id being spawned (e.g. "sd_wood_1")
--   tree              - the tree object being chopped
--   treeSize          - normalized tree size (bbox.halfSize.z / 20, typically 5-160)
--   toolValue         - effective tool quality (>= 2)
--   skill             - skill name used ("axe", "longblade", "marksman", ...)
--   difficulty        - tree difficulty value
--
-- Example:
--   table.insert(G_woodcutting_woodCountHandlers, function(ctx)
--       if ctx.skill == "axe" then return ctx.woodCount * 1.25 end
--   end)
...
completeWoodcutting = function()
	...
	if item then
		...
		-- Apply registered modifiers from other mods
		if #G_woodcutting_woodCountHandlers > 0 then
			local ctx = {
				woodCount = woodCount,
				originalWoodCount = woodCount,
				item = item,
				tree = woodcuttingState.target,
				treeSize = woodcuttingState.treeSize,
				toolValue = woodcuttingState.toolValue,
				skill = woodcuttingState.skill,
				difficulty = difficulty,
			}
			for _, fn in ipairs(G_woodcutting_woodCountHandlers) do
				local ok, result = pcall(fn, ctx)
				if ok and type(result) == "number" then
					ctx.woodCount = result
				elseif not ok then
					print("[woodcutting] modifier error:", result)
				end
			end
			woodCount = ctx.woodCount
		end
...
-- ════════════════════════════════════════════════════════════════════════════════
-- World Interaction - Add Firewood (lit)
-- ════════════════════════════════════════════════════════════════════════════════
