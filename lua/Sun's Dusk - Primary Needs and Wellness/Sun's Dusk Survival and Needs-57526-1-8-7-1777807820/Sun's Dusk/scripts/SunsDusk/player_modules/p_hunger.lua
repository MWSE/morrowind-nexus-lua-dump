...
local function updateWidget()
	...
	tooltipStr = tooltipStr..(not hungerData.currentHungerBuff and "Loading..." or tooltips[hungerData.currentHungerBuff] or "Error: "..tostring(hungerData.currentHungerBuff))
	
	if hungerData.currentFoodProfile == "fasting" then
		tooltipStr = tooltipStr.."\n\n"..(not hungerData.currentFoodProfile and "Loading..." or tooltips[hungerData.currentFoodProfile] or "Error: "..tostring(hungerData.currentFoodProfile))
	end
...
local function module_hunger_minute(clockHour, minute, minutesPassed)
	...
	if NEEDS_HUNGER_NOURISHMENT then
		...
		log(5, string.format("Meal counts - VerySmall:%d, Light:%d, Medium:%d, Filling:%d, Hearty:%d, RawMeat:%d | GreenPact:%d, Vegan:%d, Corprus:%d, BrokenPact:%d | Computed - highestCategory:%d, countCategories:%d",
	...
	if newHungerBuff ~= hungerData.currentHungerBuff then
		...
		hungerData.currentHungerBuff = nil
		if newHungerBuff and not G_preventAddingAnyBuffs then
...
local function onConsume(item)
	...
	if foodRestore > 0 then
		...
		if shouldHeal and (usefulFoodValue > 0 or overshootFoodValue > 0) and not applyDisease and not applyFoodPoisoning then
			...
			local healFromOvershoot = overshootFoodValue * baseConversion * 0.5 -- only 50% healing from wasted food
