...
local function onConsume(item)
	...
	if category == "corprus" and hungerData.currentFoodProfile ~= "corprus_eater" then
		types.Actor.stats.dynamic.health(self).current = types.Actor.stats.dynamic.health(self).current - (types.Actor.stats.dynamic.health(self).base/3 + 20)
	...
	if entry.consumeCategory and hungerData.foodProfiles[entry.consumeCategory] then
		if gotDebuff > 0 and not corprusPoisoning then
