...
local function fillCookingUi()
	...
	progressBar.content:add { 
		...
		props = {
			text = "Level "..cookingLevelModified,
			...
			if alchEffect.id and alchEffect.id ~= "" then
				...
				if gameEffect then
					...
					table.insert(effects, {
						...
						magnitude = levelMagnitudeMult,  -- contributors count, not power
						chance = chance,
...
function calculateMealStats()
	local stats = {
		...
		durationSuffix = 1,
		count = mealCount,
		foodware = "bowl",  -- "bowl", "plate", or nil (backend handles container selection)
	...
	if recipe.mods then
		log(3, "[Immersive Cooking] Special combination: " .. recipe.name)
...
function createMealPreview()
	...
	-- Container availability warnings
	local totalContainers = containerCounts.bowls + containerCounts.plates
	if totalContainers == 0 then
...
function updateCookingLevel()
	alchemyLevel = types.Player.stats.skills.alchemy(self).modified + types.Player.stats.attributes.intelligence(self).modified/10
	...
	if cookingStat then
		cookingStat.base = math.floor(cookingData.level) + cookingData.racialBonus
		cookingStat.modifier = math.floor( cookingData.alchemyBonus + cookingData.luckBonus + cookingData.shortBladeBonus)
