...
local function performBathing(usedSoap, soapItem, hasBugMusk)
	...
	-- Apply new house buff if in a qualifying location (not from wash basin)
	if not bathMenuLookingAtBasin then
...
local function isWashBasinItem(recordId)
	if not recordId then return false end
	return washBasinItems[recordId] or false
end
...
local bathMenuAnim = {}
local bathMenuLookingAtStatic = false
bathMenuLookingAtBasin = false
local bathMenuCurrentStatic = nil
local bathMenuCurrentBasin = nil
...
local function destroyBathTooltip()
	if bathMenuTooltip then
		bathMenuTooltip:destroy()
		bathMenuTooltip = nil
	end
end

local function createBathTooltip(eligible, reason, showGetWet)
	destroyBathTooltip()
	...
	local iconColor = eligible and validIconRgb or invalidIconRgb
	local textColor = eligible and WORLD_TOOLTIP_FONT_COLOR or invalidIconRgb
	
	local tooltipText = "Bathe"
	if not eligible and reason == "clothing" then
		tooltipText = "Bathe (improper clothing)"
	end
	
	local anchor = alignAnchor(v2(TOOLTIP_RELATIVE_X / 100, TOOLTIP_RELATIVE_Y / 100))
	
	bathMenuTooltip = ui.create({
		layer = 'Scene',
		name = "bathMenuTooltip",
		type = ui.TYPE.Flex,
		props = {
			relativePosition = v2(TOOLTIP_RELATIVE_X / 100, TOOLTIP_RELATIVE_Y / 100),
			anchor = anchor,
	...
	-- "Get Wet" line (R key) - only shown for wash basins
	if showGetWet then
		local getWetLine = {
			layer = 'Scene',
			name = "bathMenuTooltipGetWetLine",
			type = ui.TYPE.Flex,
			props = {
				horizontal = true,
				autoSize = true,
		...
		bathMenuTooltip.layout.content:add(getWetLine)
		
		getWetLine.content:add{
			type = ui.TYPE.Image,
			props = {
		...
		getWetLine.content:add{
			type = ui.TYPE.Text,
			props = {
				text = (WORLD_TOOLTIP_ICON_SIZE > 0 and " " or "") .. "Get Wet",
	...
	bathMenuTooltip.layout.content:add(line)
	...
	line.content:add{
		type = ui.TYPE.Text,
		props = {
			text = (WORLD_TOOLTIP_ICON_SIZE > 0 and " " or "") .. tooltipText,
			textColor = textColor,
			textShadow = true,
			textSize = math.max(1, WORLD_TOOLTIP_FONT_SIZE),
			alpha = WORLD_TOOLTIP_FONT_SIZE > 0 and 1 or 0,
	...
	return bathMenuTooltip
end

-- ═══════════════════════════════════════════════════════════════════════════════
-- BATH STATIC RAYCAST HANDLER
-- ═══════════════════════════════════════════════════════════════════════════════

local function bathStaticRaycastChanged()
	if not cleanData then return end
	if bathMenuElement then return end
	
	local hitObject = G_raycastResult and G_raycastResult.hitObject
	local hitRecordId = hitObject and hitObject.recordId
	
	-- Check for wash basin items (Miscellaneous)
	if hitRecordId and G_raycastResultType == "Miscellaneous" and isWashBasinItem(hitRecordId) then
		if not bathMenuLookingAtBasin or bathMenuCurrentBasin ~= hitObject then
			bathMenuLookingAtBasin = true
			bathMenuCurrentBasin = hitObject
			bathMenuLookingAtStatic = true
			bathMenuCurrentStatic = hitObject
			...
			createBathTooltip(eligible, reason, true)
		end
		return
	elseif bathMenuLookingAtBasin then
		bathMenuLookingAtBasin = false
		bathMenuCurrentBasin = nil
	end
	
	-- Check for bath statics
	if hitRecordId and (G_raycastResultType == "Static" or G_raycastResultType == "Activator") and isBathStatic(hitObject) then
		if not bathMenuLookingAtStatic or bathMenuCurrentStatic ~= hitObject then
			bathMenuLookingAtStatic = true
			bathMenuCurrentStatic = hitObject
			...
			createBathTooltip(eligible, reason, false)
		end
	elseif bathMenuLookingAtStatic and not bathMenuLookingAtBasin then
		bathMenuLookingAtStatic = false
		bathMenuCurrentStatic = nil
		destroyBathTooltip()
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- F KEY HANDLER FOR BATH STATICS
-- ═══════════════════════════════════════════════════════════════════════════════
...
input.registerTriggerHandler("ToggleWeapon", async:callback(function(dt, use, sneak, run)
	...
	if bathMenuTooltip and bathMenuLookingAtStatic and bathMenuCurrentStatic then
		local eligible, reason = canBathe()
		if not eligible then
			if reason == "clothing" then
				messageBox(2, "You need to remove armor to bathe (helmet and boots are fine).")
...
-- ═══════════════════════════════════════════════════════════════════════════════
-- R KEY HANDLER FOR WASH BASINS - GET WET
-- ═══════════════════════════════════════════════════════════════════════════════
...
input.registerTriggerHandler("ToggleSpell", async:callback(function(dt, use, sneak, run)
	...
	if bathMenuTooltip and bathMenuLookingAtBasin and bathMenuCurrentBasin then
		if saveData.m_temp and saveData.m_temp.water then
...
table.insert(G_raycastChangedJobs, bathStaticRaycastChanged)
