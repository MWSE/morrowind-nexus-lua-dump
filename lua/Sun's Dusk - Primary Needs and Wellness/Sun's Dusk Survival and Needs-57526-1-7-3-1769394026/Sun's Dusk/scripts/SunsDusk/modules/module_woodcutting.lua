...
-- local logItems = {} -- now in constants.lua
-- local burningLogs = {} -- now in constants.lua
...
startWoodcutting = function(object)
	...
	--woodcuttingState.treeName = object.type.record(object).name or "Tree"
...
local function raycastChanged()
	...
	if G_raycastResult.hitObject and not saveData.playerInfo.isInWerewolfForm and burningLogs[G_raycastResult.hitObject.recordId] then
		if firemakingTooltip then
			firemakingTooltip:destroy()
			firemakingTooltip = nil
		end
...
input.registerTriggerHandler("ToggleWeapon", async:callback(function(dt, use, sneak, run)
	if firemakingTooltip and G_raycastResult.hitObject and (logItems[G_raycastResult.hitObject.recordId] or burningLogs[G_raycastResult.hitObject.recordId]) then
