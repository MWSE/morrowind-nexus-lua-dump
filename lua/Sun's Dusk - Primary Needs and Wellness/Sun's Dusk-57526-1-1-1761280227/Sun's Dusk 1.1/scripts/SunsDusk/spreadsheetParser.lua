...
│ "Name the morsel. Weigh the sip. File it under fate."                │
╟──────────────────────────────────────────────────────────────────────╢
...
-- ╭──────────────────────────────────────────────────────────────────────╮
-- │ Core TSV parse                                                      │
-- │ "Tabs are tiny doors. Walk through each."                           │
-- ╰──────────────────────────────────────────────────────────────────────╯
local function parseRecipes(tsvContent)
	...
	for i = 1, #lines do
		...
		then
			...
			local isToxic    			= (fields[9] or ""):lower()=="true"
			local isGreenPact   		= (fields[10] or ""):lower()=="true"
			local isCookedMeal 			= (fields[11] or ""):lower()=="true"
			local ingredientClass		= (fields[12] or ""):lower()
			if ingredientClass == "" then
				ingredientClass = nil
			end
			local ingredientRank		= tonumber(fields[13]) or 1
			local alchEffect1			= fields[15] and fields[15] ~= "" and (spellDiffs[fields[15]] or fields[15]:lower()) or nil
			local alchEffect2           = fields[16] and fields[16] ~= "" and (spellDiffs[fields[16]] or fields[16]:lower()) or nil
			local alchEffect3           = fields[17] and fields[17] ~= "" and (spellDiffs[fields[17]] or fields[17]:lower()) or nil
...
for filename in vfs.pathsWithPrefix("SD_food_and_drinks/") do
	if filename:match("%.txt$") then
		...
		if file then
			log(3, "[SD] Loading file: " .. filename)
