...
G_onLoadJobs 				= {}
G_refreshWidgetJobs 		= {}
G_removeAbilitiesJobs 		= {}
G_onFrameJobs 				= {}
G_cellChangedJobs 			= {}
G_onConsumeJobs 			= {}
G_onMiscUsedJobs 			= {}
G_settingsChangedJobs 		= {}
G_onConsumedWaterJobs 		= {}
G_raycastChangedJobs 		= {}
G_onPlayerInfoChangedJobs 	= {}
G_onCellInfoChangedJobs 	= {}
G_equipmentChangedJobs 		= {}
G_landedHitJobs 			= {}
G_destroyHudJobs 			= {}
G_landedSpellHitJobs 		= {}
G_onFrameJobsSluggish 		= {}
G_eventHandlers  			= {}
G_postSleepJobs  			= {}
G_refreshTooltipJobs  		= {}
G_onInventoryChangedJobs	= {} -- (gained, lost, container) where gained/lost are {recordId = count}, container is from QL or nil
G_worldInteractions			= {}
G_controllerButtonPressJobs = {}
G_keyPressJobs				= {}
G_controllerButtonReleaseJobs = {}
G_keyReleaseJobs			= {}

-- world info
G_raycastResult			= nil
G_raycastResultType 	= nil
G_lastRaycastHitObject 	= nil
G_isInWater 			= 0
G_waterObject 			= nil --only checked if NEEDS_CLEAN
G_isSleeping 			= false
G_isInJail 				= nil
G_isTravelling			= false
G_isLongTravel			= nil --when >= 12 hours passed and isTravelling
G_currentBed 			= nil
G_trueExteriorTempString	= ""
G_playerCurrentTempString 	= ""
G_playerTempBuffString = ""
G_playerTargetTempString = ""
G_SW_isWet				= 0
G_liquids					= {}
G_stewNames  				= {}
G_isBathingItem = function()end
...
-- controlling vignette and heartbeat
G_heartbeatFlags = {} --[moduleName] = volume
G_vignetteFlags = {} --[moduleName] = alpha
G_vignetteColorFlags = {} --[moduleName] = colorName (hot, cold, default/nil)
G_flashVignette = 0
G_heartbeatInterval = 1.0

G_clockOffset = 0 -- for manual clockHour calculation

-- registering widgets: icons go into columns, bars into rows
G_rowWidgets   = {}
G_columnWidgets    = {}
...
function mainLogic()
	...
	-- travel ring teleport: snap time forward so mainLogic skips the elapsed hours
...
local function cellChanged(lastCell)
    ...
    if saveData.got_sd_book_1_needs and saveData.got_sd_book_2_temp and saveData.got_sd_book_3_cook_1 and saveData.got_sd_book_4_clean then
...
local function event_addConsumable(data)
	...
	local tbl = data[2]
	saveData.registeredConsumables[id] = tbl
	if tbl.timestamp and types.Potion.records[id] then
		--print(types.Potion.records[id].name, id)
		G_stewNames[types.Potion.records[id].name] = id
	end
...
local function onConsoleCommand(command, str)
    ...
    -- Handle "lua cooking" - set cooking skill level
    if str:match("^lua cooking") then
        local level = str:match("^lua cooking%s+(%d+)")
        if level then
            saveData.m_cooking.level = tonumber(level)
            ui.printToConsole("Cooking skill set to " .. level, ui.CONSOLE_COLOR.Success)
        else
            ui.printToConsole("Current cooking skill: " .. tostring(saveData.m_cooking.level or 0), ui.CONSOLE_COLOR.Info)
        end
        return
    end
end
...
G_eventHandlers.UiModeChanged = UiModeChanged
G_eventHandlers.SunsDusk_ActivatedBed = activatedBed
G_eventHandlers.NSS_showMessage = NSS_preventedSleeping
G_eventHandlers.SunsDusk_RegisterCompanion = becameCompanion
G_eventHandlers.SunsDusk_ReleaseCompanion = releasedCompanion
G_eventHandlers.SunsDusk_WaterBottles_consumedWater = onConsumedWater
G_eventHandlers.SunsDusk_messageBox = messageBox
G_eventHandlers.SunsDusk_landedHit = landedHit
G_eventHandlers.SunsDusk_landedSpellHit = landedSpellHit
G_eventHandlers.SunsDusk_checkedIfVampireWerewolf = checkedIfVampireWerewolf
G_eventHandlers.SunsDusk_addConsumable = event_addConsumable
G_eventHandlers.SunsDusk_receiveCellInfo = receiveCellInfo
G_eventHandlers.SunsDusk_playSound = playSound
G_eventHandlers.SunsDusk_equippedArmor = equippedArmor
G_eventHandlers.SunsDusk_registerFire = registeredFire
G_eventHandlers.LH_lightEquipped = lightHotkey
G_eventHandlers.SunsDusk_syncDatabases = syncDatabases
G_eventHandlers.SunsDusk_miscUsed = onMiscUsed
G_eventHandlers.SunsDusk_refreshTooltips = G_refreshTooltips
G_eventHandlers.SunsDusk_refreshInventory = refreshInventory
G_eventHandlers.SunsDusk_errorDetection = errorDetection
--G_eventHandlers.Audiobooks2_listenedOneSecond = function(data) print(data[1],data[2],data[3]) end
