			...
			if woodObject.cell then
				if woodData.phase == "falling" then
					...
					if newPos.z <= woodData.targetPos.z+1 then
						-- Snap to ground position with landing rotation (0-30Â° off)
...
G_onObjectActiveJobs.camping = function(object)
	...
	if object.recordId == "sd_campingitem_tent" or object.recordId == "sd_campingitem_tent_perm" then
		table.insert(G_delayedUpdateJobs, {
			...
			function()
				...
				if isPerm then
					-- Place as a complete built tent (stage 4) with bedroll
	...
	-- Tent Building System - Handle tent component placement
	--elseif tentRequiresStakes[object.recordId] then
	--	-- Player tried to place a tent component that requires stakes first
	--	local cell = object.cell
	--	local nearestPlayer
	--	for _, player in pairs(cell:getAll(types.Player)) do
	--		nearestPlayer = player
	...
	--		local inv = types.NPC.inventory(nearestPlayer)
	--		object:moveInto(inv)
	--		nearestPlayer:sendEvent("SunsDusk_messageBox", {2, "You must place stakes down before adding more to your tent."})
	--	end
	--
	end
end
...
local function spawnFallingWood(data)
	...
	for _, pieceData in ipairs(spawnData) do
		...
		-- Calculate landing rotation (0-30Â° off from target)
...
-- Check if an object is a built tent
