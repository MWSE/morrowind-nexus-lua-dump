...
local function getInteriorTemperature(cell)
	-- Check for explicit temperature override first
	if G_cellInfo and G_cellInfo.temperature then
		tempDbg.p(12, string.format("Interior: %s (%s) = %s", 
			cell.name or "Unknown", climateType or "???", formatTemperature(G_cellInfo.temperature)))
		local temp = G_cellInfo.temperature
