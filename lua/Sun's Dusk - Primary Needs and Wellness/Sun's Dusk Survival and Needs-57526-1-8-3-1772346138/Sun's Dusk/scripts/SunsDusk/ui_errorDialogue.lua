-- Destroy existing dialogue if it exists
...
-- Text to display (set these before requiring this file)
...
-- Color utility function for button states
local function darkenColor(color, factor)
    return util.color.rgb(color.r * factor, color.g * factor, color.b * factor)
end

-- Unique ID
local uniqueDialogueId = "errorDialogue_" .. math.random()
...
-- Create dialogue with Container
errorDialogue = ui.create({
    type = ui.TYPE.Container,
    layer = 'Modal',
    name = uniqueDialogueId,
...
-- Outer vertical flex for top/bottom padding
...
-- Horizontal flex for left/right padding
...
-- Main flex container with auto-sizing
local mainFlex = {
    name = 'mainFlex',
    type = ui.TYPE.Flex,
    props = {
...
if errorDetails and errorDetails ~= "" then
    ...
    -- Calculate TextEdit dimensions
    local errorDetailsStr = tostring(errorDetails)
    local maxWidth = G_hudLayerSize.x -- Use actual HUD width
