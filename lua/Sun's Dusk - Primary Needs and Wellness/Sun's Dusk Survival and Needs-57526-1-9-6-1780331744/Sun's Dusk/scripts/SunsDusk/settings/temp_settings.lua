...
settingsTemplate.TEMP_UI = {
	...
	order = 1,
	settings = {	
		-- built-in widgets first, every detected pack/skin appended after them
		{
			key = "TEMP_BAR_STYLE",
			name = "Temperature Bar Style",
			description = "Stylized Bar and Simple Bar are built in. Anything below is a detected texture pack or custom skin.",
			default = "Simple Bar",
			renderer = RENDERER_SELECT,
			argument = {
				disabled = false,
				l10n = "none",
				items = (function()
					local list = { "Stylized Bar", "Simple Bar" }
					for _, pack in ipairs(G_iconPacks.temp and G_iconPacks.temp.availablePacks or {}) do
						list[#list + 1] = pack
					end
					return list
				end)(),
		...
		{
			key = "TEMP_HIDE_NO_BUFF",
			name = "Hide thermometer/text when no effect is active",
			description = "Hides the thermometer icon when there is no temperature buff/debuff and no wetness debuff.\n\nThe simple/stylized temperature bar and the temperature text always stay visible.",
			renderer = "checkbox",
			default = false,
		},
		{
		...
		},
		{
			...
			argument = {
				disabled = false,
				l10n = "none",
				items = { "off", "temperature", "+weather", "+equipment" },
			},
		},
	}
}
...
for _, template in pairs(settingsTemplate) do
	...
	settingsSection:subscribe(async:callback(function (_,setting)
		...
		if G_refreshWidgetJobs and (setting == "TEMP_TEXT_COLOR" or setting == "TEMP_HIDE_NO_BUFF") then
