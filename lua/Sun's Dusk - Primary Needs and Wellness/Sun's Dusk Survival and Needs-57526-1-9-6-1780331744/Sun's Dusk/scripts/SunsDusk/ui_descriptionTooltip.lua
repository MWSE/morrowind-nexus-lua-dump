...
-- CONFIGURATION
local uiElementName = "SunsDuskTooltip"
local OPACITY = 0.8
...
function getMaxEnchantmentCharge(enchantment)
	...
	for _, effect in pairs(enchantment.effects) do
		local mfx = effect.effect
		local hasMagnitude = mfx.hasMagnitude
		local hasDuration = mfx.hasDuration
		local appliedOnce = mfx.isAppliedOnce
		...
		x = x * 0.1 * mfx.baseCost
		x = x * duration
		x = x + 0.05 * effect.area * mfx.baseCost

		if effect.range == core.magic.RANGE.Target then
			x = x * 1.5
		end

		cost = cost + math.floor(x * fEffectCostMult + 0.5)
	end

	local typeMultGMST = ({
		[core.magic.ENCHANTMENT_TYPE.CastOnce]       = "iMagicItemChargeOnce",
		[core.magic.ENCHANTMENT_TYPE.CastOnUse]      = "iMagicItemChargeUse",
		[core.magic.ENCHANTMENT_TYPE.CastOnStrike]   = "iMagicItemChargeStrike",
		[core.magic.ENCHANTMENT_TYPE.ConstantEffect] = "iMagicItemChargeConst",
	})[enchantment.type]

	if typeMultGMST then
		cost = cost * core.getGMST(typeMultGMST)
	end
