...
--if vfs.fileExists('scripts/survivalmode/global.lua') then return end
...
local aggroActors = {} -- [actorId] = actor, kept while targeting the player
local function combatTargetsChanged(data)
	...
	if targetingPlayer then
		aggroActors[data.actor.id] = data.actor
	else
		aggroActors[data.actor.id] = nil
	end
	-- recount, dropping any that went invalid (dead, unloaded)
	local count = 0
	for actorId, actor in pairs(aggroActors) do
