...
---------------------------------------------------------------------------------------------------------------------------------- DEBUG WIDGET ------------------------------------------------------------------------------------------------------------------------------------------------------
-- cumulative debug widgets: off < temperature < +weather < +equipment
if TESTING_WIDGET2 ~= "off" then
	tempDbg2 = require("scripts.SunsDusk.ui_debugWidget")("=============== Temperature ===============")
end
if TESTING_WIDGET2 == "+weather" or TESTING_WIDGET2 == "+equipment" then
	weatherDbg = require("scripts.SunsDusk.ui_debugWidget")("=============== Weather ===============")
end
if TESTING_WIDGET2 == "+equipment" then
...
function updateTemperatureWidget()
		...
		if not G_columnWidgets.m_tempText then
			G_columnWidgets.m_tempText = ui.create{
				...
				userData = {
					...
					-- always treated as fully visible; exempt from hide/invert
					effectiveAlpha = 1,
					-- temperature widgets always sit at the screen edge when visible
					stickToEdge = true,
				},
			}
			G_columnsNeedUpdate = true
	...
	-- ────────────────────────────── custom skin ──────────────────────────────
	-- custom temp skin owns its look: content(ctx) builds the tree once, optional
	-- update(ctx) mutates it per tick and reports if anything changed. engine
	-- builds the shell and feeds live state. existing styles below stay untouched.
	local selectedPack = G_iconPacks.temp[TEMP_BAR_STYLE]
	local tempSkin = selectedPack
	if tempSkin and tempSkin.content and saveData.m_temp then
		local racialThresholds = getRacialTemperatureThresholds()
		local minTemp = -11
		local maxTemp = racialThresholds.hotMin + 10
		local tempRange = (maxTemp - minTemp) / 0.95

		-- live temperature state the skin reads; generic bits filled by the renderer
		local ctx = {
			skin = tempSkin,
			currentTemp = saveData.m_temp.currentTemp,
			targetTemp = saveData.m_temp.targetTemp,
			normalizedCurrent = math.min(1, math.max(0, (saveData.m_temp.currentTemp - minTemp) / tempRange)),
			normalizedTarget = math.min(1, math.max(0, (saveData.m_temp.targetTemp - minTemp) / tempRange)),
			wetness = saveData.m_temp.water and saveData.m_temp.water.wetness or 0,
			isInRain = saveData.weatherInfo and saveData.weatherInfo.isInRain or false,
			tempBuff = saveData.m_temp.currentTempBuff and true or false,
			wetnessDebuff = saveData.m_temp.water and saveData.m_temp.water.wetnessDebuff and true or false,
			diff = (saveData.m_temp.targetTemp - saveData.m_temp.currentTemp) * G_temperatureRate,
		}

		local widget = G_renderContentSkin({ name = "m_tempThermometer", order = "5needs-temp", stickToEdge = true }, ctx)
		addTooltip(widget.layout, G_temperatureWidgetTooltip)
		return
	end

	if selectedPack and not selectedPack.content then
		--config, detected packs render as a thermometer with their own textures
		local skinData = selectedPack
		local racialThresholds = getRacialTemperatureThresholds()
		...
		-- thermometer is always opaque, only hides when no temp buff AND no wetness
		-- debuff, and only in the full buffs+debuffs mode
		local tempAlpha = 1
		if TEMP_HIDE_NO_BUFF and TEMP_BUFFS_DEBUFFS == "Buffs and debuffs"
		and saveData.m_temp.currentTempBuff == false
		and (not saveData.m_temp.water or not saveData.m_temp.water.wetnessDebuff) then
			tempAlpha = 0
		end
		--local normalizedValue = math.min(1,math.max(0,(saveData.m_temp.currentTemp + 10) / 60)) -- min -15 degrees, -15+60 = 45 degrees
		local severityScale = 1
		...
		-- visibility flips re-sort columns when sort-by-visibility is on
		local prevAlpha = G_columnWidgets.m_tempThermometer.layout.userData.effectiveAlpha or 1
		if (prevAlpha > 0) ~= (tempAlpha > 0) then
			G_columnsNeedUpdate = true
		end
		G_columnWidgets.m_tempThermometer.layout.userData.effectiveAlpha = tempAlpha
		...
		G_columnWidgets.m_tempThermometer.layout.content = ui.content {
			TEMP_BACKGROUND ~= "No Background" and { -- Damage Bar r.2.lag
				...
				props = {
					...
					alpha = TEMP_BACKGROUND == "Classic" and tempAlpha^2 or (tempAlpha > 0 and 0.5 or 0),
				}
			} or {},
		...
		if TEMP_WETNESS_BAR then
			...
			wetnessBarVertical = {
				...
				props = {
					...
					tileH = true,
					tileV = false,
					-- hides alongside the thermometer when icon is faded out
					alpha = tempAlpha > 0 and 0.8 or 0,
				}
			}
		...
		--end
		--tooltipStr = tooltipStr..(tooltips[thirstData.currentThirstBuff] or "Loading... "..tostring(thirstData.currentThirstBuff))
...
-- expose helpers for external use (e.g. StatsWindow integration)
G_makeBarSegment2 = makeBarSegment2
G_createTempMarker = createTempMarker
