...
I.InventoryExtender.registerTooltipModifier("SunsDusk_CookedFood", function(item, layout)
	...
	if data then
		...
		-- toxicity flag (only meaningful while toxicity setting is on)
...
------------------------------ CraftingFramework: armor/clothing resistance lines ------------------------------
...
if I.CraftingFramework and I.CraftingFramework.registerTooltipModifier then
	I.CraftingFramework.registerTooltipModifier({
		...
		func = function(recipe, ctx)
			...
			-- armor and clothing only, recipe preview has no item so use record fields
			...
			-- small gap, then header and resistance lines
