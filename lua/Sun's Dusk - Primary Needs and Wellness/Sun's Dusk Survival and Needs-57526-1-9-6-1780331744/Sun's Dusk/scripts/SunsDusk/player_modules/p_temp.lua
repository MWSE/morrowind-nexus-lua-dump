...
local function getMagicResistances()
	...
	for _, activeSpell in pairs(typesActorActiveSpellsSelf) do
...
local function getMagicTemperatureModifier(currentTemp, targetTemp)
	...
	if fireDamage > 0 then
		fireDamage = fireDamage / math.max(0.01, 1-typesActorActiveEffectsSelf:getEffect(core.magic.EFFECT_TYPE.ResistFire).magnitude/100)
	...
	if frostDamage > 0 then
		frostDamage = frostDamage / math.max(0.01, 1-typesActorActiveEffectsSelf:getEffect(core.magic.EFFECT_TYPE.ResistFrost).magnitude/100)
