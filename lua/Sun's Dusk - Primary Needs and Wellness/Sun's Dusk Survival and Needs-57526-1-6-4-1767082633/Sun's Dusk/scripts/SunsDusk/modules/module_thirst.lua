...
local function removeRandomConsumableBuff(consumableName)
	...
	local luck = types.Player.stats.attributes.luck(self).modified
	local removedId
	...
	if removedId then
		...
		if math.random() < 0.25 + types.Player.stats.attributes.luck(self).modified/150 then
			effects = {1}
...
local function onConsume(item)
	...
	elseif types.Potion.objectIsInstance(item) then
		...
		if item.type.record(item).name:lower():sub(-12,-1) == "l saltwater)" then
			drinkValue = -0.2
		end
		if item.type.record(item).name:lower():sub(-19,-1) == "l suspicious water)" then
			if not removeRandomConsumableBuff(item.type.record(item).name) then
				drinkValue = 0
			end
		end
		local lowerName = item.type.record(item).name:lower()
		if NEEDS_THIRST_ALCOHOL_R and lowerName:find("sujamma") then
			drinkValue = drinkValue * -0.5
		end
		if NEEDS_THIRST_ALCOHOL_R and lowerName:find("flin") then --mazte
			drinkValue = drinkValue * -0.5
		end
		if NEEDS_THIRST_ALCOHOL_R and lowerName:find("mazte") then --mazte
			drinkValue = drinkValue * -0.5
		end
		if NEEDS_THIRST_ALCOHOL_R and lowerName:find("shein") then --mazte
			drinkValue = drinkValue * -0.5
		end
		if NEEDS_THIRST_ALCOHOL_R and lowerName:find("greef") then --mazte
			drinkValue = drinkValue * -0.5
		end
		if lowerName:find("stoneflower tea") and saveData.m_sleep then --mazte
...
local function onConsumedWater(liquid, remainingWater)
	...
	if ATRONACH_WATER_MULT ~= "Full" and (ATRONACH_WATER_MULT_AFFECTS_ALL or types.Actor.activeEffects(self):getEffect(core.magic.EFFECT_TYPE.StuntedMagicka).magnitude > 0) then
