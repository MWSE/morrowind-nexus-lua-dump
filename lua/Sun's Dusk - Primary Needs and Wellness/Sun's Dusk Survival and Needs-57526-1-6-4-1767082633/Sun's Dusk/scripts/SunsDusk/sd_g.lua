...
v3 		= util.vector3
onUpdateJobs = {}
delayedUpdateJobs = {} -- {framesDelay, function} - don't use table.insert
...
local vesselLiquids = {
	[("Misc_Com_Redware_Cup"):lower()] 		= { "tea_SF", "tea_H" },
	[("Misc_DE_pot_redware_03"):lower()] 	= { "tea_SF", "tea_H" },
--	[("T_Com_CopperKettle_01"):lower()] 	= { "tea_SF", "tea_H" },
--	[("T_Com_CoppetTeapot_01"):lower()] 	= { "tea_SF", "tea_H" },
	[("AB_Misc_DeCeramicCup_01"):lower()] 	= { "tea_SF", "tea_H" },	
	[("AB_Misc_DeCeramicCup_02"):lower()] 	= { "tea_SF", "tea_H" },
	[("AB_Misc_DeCeramicFlask_01"):lower()] = { "tea_SF", "tea_H" },	
...
local stockingConfig = {
	...
	{
        itemPool = { "sd_teapot_red" },
		itemType 		= "Miscellaneous",
        serviceRequired = "Misc",
        classPatterns 	= { "trader", "merchant", "alchemist", "pawn broker" },
        startStock 		= 1,
        maxStock 		= 2,
        restockPerDay 	= 1,
    },
	-- books
	{
        itemPool = { "sd_book_3_cook_1" },
		itemType 		= "Book",
        serviceRequired = "Books",
        classPatterns 	= { "trader", "merchant", "pawn broker", "bookseller" },
        startStock 		= 0.1,
        maxStock 		= 1,
        restockPerDay 	= 0.1,
    },
...
for i = #stockingConfig, 1, -1 do
    ...
    if not typeTable then
        log(5, "[StockingConfig] Invalid itemType: " .. tostring(group.itemType) .. " - deleting group " .. i)
        ...
        for j = #group.itemPool, 1, -1 do
            ...
            if not typeTable.records[itemId] then
               log(5, "[StockingConfig] Item not found in " .. group.itemType .. ".records: " .. itemId)
                table.remove(group.itemPool, j)
            end
        end
        
        if #group.itemPool == 0 then
            log(5, "[StockingConfig] No valid items remain in group " .. i .. " - deleting group")
            table.remove(stockingConfig, i)
        end
    end
end
...
local function onUpdate(dt)
	...
	for index, t in pairs(delayedUpdateJobs) do
		t[1] = t[1] - 1
		if t[1] <= 0 then
			t[2]()
			delayedUpdateJobs[index] = nil
		end
	end
		...
		if rev then
			...
			if not isBottle then
				...
				if spillableLiquid then
					...
					local maxQ = resolveMaxQ(lc(rev.orig))
...
local function onObjectActive(object)
	if types.Actor.objectIsInstance(object) then
		delayedUpdateJobs["addScript_"..object.id] = {
			2, 
			function()
	...
	if object.recordId:sub(1,8) == "sd_wood_" or object.recordId == "sd_wood_publican" or object.recordId == "sd_wood_merchant" then
...
local function spawnBooksInChargenArea(data)
	...
	for _, bookData in pairs(books) do
		if bookCounts[bookData[1]] == 0 then
			...
			newObj:teleport(cell, bookData[2], { rotation = rotation })
		end
	end
...
return {
	...
	eventHandlers = {
		...
		SunsDusk_Tea_consumeIngredient				= teaConsumeIngredient,
		SunsDusk_Tea_refillTeacups					= teaRefillTeacups,		
		SunsDusk_Books_spawnBooksInChargenArea		= spawnBooksInChargenArea,		
	},
}