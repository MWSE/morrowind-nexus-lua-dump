...
-- ───────────────────────────────────────────────────────────── Logger gang ─────────────────────────────────────────────────────────────
DEBUG_LEVEL = 4  --  { "Silent", "Quiet", "Chatty", "Deep", "Trace" }
...
local function inWaterCheck()
	...
	if G_isInWater > 0.02 and NEEDS_CLEAN then
...
local function onFrame(dt)
	...
	if G_raycastResult.hitObject ~= G_lastRaycastHitObject then
		if G_raycastResult.hitObject then
			G_raycastResultType = tostring(G_raycastResult.hitObject.type)
		else
			G_raycastResultType = nil
		end
		...
		--print((core.getRealTime()-prevTime)*1000 .. "ms", G_raycastResult.hitObject)
...
--		messageBox(2, "Force Square "..(G_forceSquareFalloff and "ON" or "OFF"))
--    end
--end
