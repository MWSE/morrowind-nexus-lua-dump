...
local function replaceFirewood(object)
	...
	if types.Miscellaneous.objectIsInstance(object) then
		if object.count > 1 or object.recordId == "sd_wood_merchant"  then
...
local INVENTORY_SUBSTRINGS  = { 'bottle', 'canteen' }
...
local function onObjectActive(object)
	...
	if object.recordId:sub(1,8) == "sd_wood_" or object.recordId == "sd_wood_merchant" then
...
local function onLoad(data)
	...
	saveData.restockingNPCsFirewood   = saveData.restockingNPCsFirewood   or {}
...
local function removeTree(data)
	...
	-- Remove the tree using proper OpenMW API
	tree:remove()
end
...
local function upgradeFire(data)
	...
	local logInInv = types.NPC.inventory(player):find("sd_wood_1") or types.NPC.inventory(player):find("sd_wood_merchant")
...
function isHeatSource(name, mode)
    local lower = string.lower(name)
    local heatWords = { "fire", "flame", "torch", "brazier", "lava", "firepit", 
                       "incense", "burner", "tiki", "logpile", "pitfire", "forge" }
    if mode == 3 and (
		string.match(lower, "cave") 
		or string.match(lower, "_rock") 
		or string.match(lower, "boulder")
		or lower == "in_lava_blacksquare"
	) then
		return false
	end
    -- Exclude if it's off or broken
    if string.match(name, "_[Oo]ff$") or string.match(lower, "burnedout") 
       or string.match(lower, "broke") or string.match(lower, "flame light") or string.match(lower, "roht_mg_fire") then
        return false
    end
    ...
    if burningWood[name] then
		return true
	end
    return false
end

-- Cell categorization based on static objects
-- Supports both vanilla Morrowind statics and Tamriel Rebuilt (T_ prefix) statics
local function getCellInfo(player)
	...
	if (cell.isExterior or cell.isQuasiExterior) and cell.hasSky then
	...
	for _, object in pairs(cell:getAll(types.Static)) do
		...
		-- Includes Tamriel Rebuilt patterns like T_Dwe_DngRuin_, T_Dwe_DngRou_
		...
		-- Includes Tamriel Rebuilt patterns like T_Dae_DngRuin_
		...
		-- Includes Tamriel Rebuilt patterns like T_Bre_DngCrypt_
		...
		-- Includes Tamriel Rebuilt patterns like T_Com_SetMine_
		...
		-- Includes Tamriel Rebuilt patterns like T_Cnq_Cave_, T_Glb_Cave (non-ice)
		...
		-- Includes residential buildings, shacks, and interior furniture
		...
		-- Check for Castle/Fortress indicators
		-- Includes strongholds, keeps, forts, guard towers, and large defensive structures
...
--[[		if id:find("stronghold") or id:find("_keep") or id:find("fort") or
	...
	-- Prioritize more specific types over general ones
...
local function lootMisc(item, player)
...
I.Activation.addHandlerForType(types.Miscellaneous, lootMisc)
...
local function activateNPC(npc, player)
	...
	if types.Actor.isDead(npc) then
		saveData.restockingNPCsFirewood[npcId] = nil
		return
	end
	-- Check if this is a relevant merchant
	if record.servicesOffered["Misc"] and (className:find("miner") or className:find("smith") or className:find("trader") or className:find("merchant") or className:find("pawnbroker") or className:find("hunter") or className:find("farmer") or className:find("carpenter") or className:find("shipmaster") or className:find("naturalist")) 
		then
		
		-- Configuration
		local itemId = "sd_wood_merchant"
		local restockPerDay = 3
		local maxStock = 6
		local startStock = 6
		
		-- Get or initialize tracking data for this NPC
		if not saveData.restockingNPCsFirewood[npcId] then
			saveData.restockingNPCsFirewood[npcId] = {
				lastRestock = world.getGameTime() / (24*60*60),
				initialized = false
			}
		end
		
		local npcData = saveData.restockingNPCsFirewood[npcId]
		local now = world.getGameTime() / (24*60*60) -- Current time in days
		
		-- Count current stock in inventory
		...
		-- Initialize with starting stock on first encounter
		if not npcData.initialized then
			if currentStock < startStock then
				local tempItem = world.createObject(itemId, startStock - currentStock)
		...
		-- Only restock if at least ~1.2 hours have passed
		if daysSinceRestock < 0.05 then
			return
		end
		
		-- Calculate and apply restock
		if currentStock < maxStock then
			-- Can restock up to (restockPerDay * days passed), capped at 1 day
			local restockAmount = math.min(daysSinceRestock, 1.0) * restockPerDay
			local actualRestock = math.min(math.floor(restockAmount), maxStock - currentStock)
			
			if actualRestock > 0 then
				local tempItem = world.createObject(itemId, actualRestock)
				tempItem:moveInto(types.NPC.inventory(npc))
