...
local function makeDebugWidget(name)
	...
	local function clampPosition(pos)
		...
		local minSize = 50
		return v2(
			math.max(0, math.min(pos.x, hudLayerSize.x - minSize)),
			math.max(0, math.min(pos.y, hudLayerSize.y - minSize))
		)
	end
	...
	widget:update()
	if mousewheelJobs then
		table.insert(mousewheelJobs, function(dir)
