...
local function module_hunger_minute(clockHour, minute, minutesPassed)
	...
	if isInJail then
		...
		if isSleeping or isTravelling then
	...
	if DEATH_BY_STARVATION and not isTravelling and not isInJail then
		local volume = (hungerData.hunger-0.95)*20
		if volume > 0 then
			heartbeatFlags.hunger = volume
			vignetteFlags.hunger = 0.6*volume
			
			if hungerData.hunger >=0.99 then
				vignetteColorFlags.hunger = "default"
			...
			heartbeatFlags.hunger = nil
			vignetteFlags.hunger  = nil
			vignetteColorFlags.hunger = nil
		end
	else
		heartbeatFlags.hunger = nil
		vignetteFlags.hunger  = nil
...
local function onConsume(item)
	...
	-- altmer + 15%
	-- redguard + 10%
	log(3, "item ID:  "..item.recordId.."  ;  item name: "..tostring(item.type.record(item).name).."  ;  food value:  "..entry.foodValue)
