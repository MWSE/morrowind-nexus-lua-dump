...
local function module_thirst_minute(clockHour, minute, minutesPassed)
	...
	local baseStep = ((isSleeping or isTravelling) and 0.5 or 1) / HOURS_PER_THIRST_STATE / 6 / 60
	...
	if DEATH_BY_DEHYDRATION and not isTravelling and not isInJail then
		local volume = (thirstData.thirst-0.95)*20
		if volume > 0 then
			heartbeatFlags.thirst = volume
			vignetteFlags.thirst = 0.6*volume
			
			if thirstData.thirst >=0.99 then
				vignetteColorFlags.thirst = "default"
			...
			heartbeatFlags.thirst = nil
			vignetteFlags.thirst  = nil
			vignetteColorFlags.thirst = nil
		end
	else
		heartbeatFlags.thirst = nil
		vignetteFlags.thirst  = nil
...
local function onConsume(item)
	...
	if entry and entry.drinkValue and entry.drinkValue ~= 0 then
		log(3, "item ID:  "..item.recordId.."  ;  item name: "..tostring(item.type.record(item).name).."  ;  drink value:  "..entry.drinkValue)
...
table.insert(onConsumedWaterJobs, 0, onConsumedWater)
...
G_wellBlacklist = { "comkegrack", "t_de_setmh_x_well_01", "inkwell", "t_imp_furnr_tablepool_01" }

local function isWell(object)
	if raycastResultType ~= "Static" and raycastResultType ~= "Activator" then 
		return false
	end
...
input.registerTriggerHandler('Activate', async:callback(function()
	...
	if raycastResult.hitObject and isWell(raycastResult.hitObject) then 
		...
		if thirstData.thirst > 0.01 then
			ambient.playSound("Drink")
			thirstData.thirst = 0
			module_thirst_minute(nil, nil, 0)
		end
		if saveData.m_temp then
			if saveData.m_temp.currentTemp > 20 then
				...
				saveData.m_temp.currentTemp = saveData.m_temp.currentTemp - 4
			end
		end
	end
end))
...
table.insert(sluggishScheduler[5], refillSwimming)
onFrameJobsSluggish.refillSwimming=refillSwimming
