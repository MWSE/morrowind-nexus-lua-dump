...
require('scripts.SunsDusk.lib.temp_settings')
...
-- ====================================================== RACIAL TEMPERATURE SYSTEM (PHASE 5) ======================================================
...
--			ashland = 8,	-- +8°C in ashlands
		...
		twilightComfortDrift = 1.3,  -- 30% faster drift toward comfortable during twilight
...
local function getPlayerRace()
	...
	elseif saveData.playerInfo.isBeast then return "khajiit"  -- catches all cat variants
...
function getRacialTemperatureThresholds(printDebug)
	...
	if racialData.interiorThresholds then
		if not cellInfo.isExterior and racialData.interiorThresholds then
...
local function getRacialCellTypeBonus()
	...
	if not racialData or not racialData.cellTypeBonus or not cellInfo then
		return 0
	end
...
local function getClimateTempFromPosition(dt)
	...
	for _, source in ipairs(CLIMATE_SOURCES) do
		...
		if distance < source.radius then
			...
			elseif source.squareInfluence ~= false then
				influence = influence * influence
			end
			...
			table.insert(climateTempTooltip2,string.format("  %s (%s, mode=%d): dist=%.0f, influence=%.2f, intensity=%s", 
				source.name, source.climate or "none", source.blendMode or 1, distance, influence, (((localIntensity)>=0) and "+" or "")..formatTemperatureModifier(localIntensity)))
		end
	end
...
--table.insert(onFrameJobsSluggish,getClimateTempFromPosition)
table.insert(sluggishScheduler, {getClimateTempFromPosition})
onFrameJobsSluggish.getClimateTempFromPosition = getClimateTempFromPosition
...
local function getSolarHeating(cell, hour)
	...
	if timeOfDayFactor <= 0.1 then -- Night time or near night
		...
		if climateType == "ashland" or climateType == "arctic" or climateType == "cold" then
			...
			tempDbg.p(14, "  Ashland night cooling: -" .. formatTemperatureModifier(8))

		elseif climateType == "volcanic" or climateType == "tropical" then
			-- these climates stay warm from heat retention
			nightEffect = 4
			
			tempDbg.p(14, string.format("  %s night warmth: +%s", climateType, formatTemperatureModifier(3)))
...
--			nightEffect = -5
		...
		elseif climateType == "coast" or climateType == "urban" then
			...
			tempDbg.p(14, "  Coast moderate night: -" .. formatTemperatureModifier(2))
...
local function scanEquipment()
	...
	equipmentData.hasUmbrella = false
		...
		if not ignoredSlots[slot] then
			...
			if slot == types.Actor.EQUIPMENT_SLOT.CarriedLeft then
				...
				if name:find("parasol") or name:find("umbrella") then
					equipmentData.hasUmbrella = true
				else
					local recordId = record.id
...
local function getEquipmentTemperatureModifier(environmentTemp)
	...
	if environmentTemp > 30 and equipmentData.heatResistance ~= 0 then
		...
		tempDbg.p(14, string.format("[Material] Hot environment - Heat resistance: %d * 0.5 = %s cooler", 
	...
	elseif environmentTemp < 15 and equipmentData.coldResistance ~= 0 then
		...
		tempDbg.p(14, string.format("[Material] Cold environment - Cold resistance: %d * 0.5 = %s warmer", 
	...
	if environmentTemp > 30 and equipmentData.weightClassHeatMod ~= 0 then
		...
		tempDbg.p(14, string.format("[Weight] Hot environment - Weight class modifier: %s", (((equipmentData.weightClassHeatMod)>=0) and "+" or "")..formatTemperatureModifier(equipmentData.weightClassHeatMod)))
	...
	elseif environmentTemp < 15 and equipmentData.weightClassColdMod ~= 0 then
		...
		tempDbg.p(14, string.format("[Weight] Cold environment - Weight class modifier: %s", (((equipmentData.weightClassColdMod)>=0) and "+" or "")..formatTemperatureModifier(equipmentData.weightClassColdMod)))
	end
	...
	if modifier ~= 0 then
		tempDbg.p(12, string.format("Equipment mod: %s", (((modifier)>=0) and "+" or "")..formatTemperatureModifier(modifier)))
	end
...
local function onEquipmentChanged(equipped, unequipped)
...
local function getInteriorTemperature(cell)
	...
	if cellInfo then
		if cellInfo.isIceCave then
			temp = 5
			interiorType = "Ice Cave"
		
		elseif cellInfo.isDwemer then
			temp = 30
			interiorType = "Dwemer"
		
		elseif cellInfo.isDaedric then
			temp = 27
			interiorType = "Daedric"
		
		elseif cellInfo.isCave or cellInfo.isCastle or cellInfo.isTomb then
			temp = 15
			interiorType = "Castle/Fort, Tomb, or Cave/Dungeon"
		
		elseif cellInfo.isMushroom then
			temp = 25
			interiorType = "Telvanni Shroom"
			
		elseif cellInfo.isHouse or cellInfo.isMine then -- or if not in cellInfo ? 
			temp = 20
			if cellInfo.isHouse then
				interiorType = "House/Residence"
			elseif cellInfo.isMine then
				interiorType = "Mine"
			end
--[[	elseif cellInfo.isAshlander then
			temp = 22
			interiorType = "Ashlander"
]]
		-- elseif cellInfo.[whatever] then
...
-- Heat sources are pre-scanned by sd_g and stored in saveData.cellInfo.fires
local function getNearbyHeatSources()
	...
	-- Get heat sources from cellInfo (populated by sd_g.getCellInfo)
	-- Heat sources are pre-scanned when entering a cell and stored in saveData.cellInfo.fires
	if not cellInfo or not cellInfo.fires then
	...
	for _, sourceObject in ipairs(cellInfo.fires) do
...
--			if ActiveSpell.fromEquipment or hasduration....
			...
			if effect.id == "fireshield" then
				fireShield = magnitude
				
			elseif effect.id == "frostshield" then
				frostShield = magnitude
...
local function getMagicTemperatureModifier(currentTemp, targetTemp)
	...
	if racialData.resistEffectivenessMultiplier then
		local oldFireRes = fireRes
		local oldFrostRes = frostRes
		fireRes = fireRes * racialData.resistEffectivenessMultiplier
		frostRes = frostRes * racialData.resistEffectivenessMultiplier
	...
	if targetTemp > 25 then
		local netFireRes =fireRes - math.min(50, fireWeak)
	...
	if targetTemp > 25 then
		local netFireWeak = math.min(50, fireWeak) - fireRes
	...
	if targetTemp < 15 then
		local netFrostRes = frostRes - math.min(50, frostWeak)
	...
	if  targetTemp < 15 then
		local netFrostWeak = math.min(50, frostWeak) - frostRes
...
local function getWaterTemperatureModifier(currentPlayerTemp, currentEnvironmentTemp)
	local modifier = 0
	
	-- ═══════════════════════════════════════════════════════════════════
	-- NOT IN WATER - Wetness and Wind Chill Effect
	-- ═══════════════════════════════════════════════════════════════════
	...
	if isInWater <= 0 then
		...
		if cellInfo.isExterior then
			...
			if cellInfo.isDwemer then
				REGION_HUMIDITY = 0.4
			elseif cellInfo.isCave and not cellInfo.isIceCave and not cellInfo.isMine then
				REGION_HUMIDITY = 0.6
			end
		end
		...
		tempDbg.p(15, string.format("  Armor Insulation: %.2f | Water resist: %.2f | Wetness: %.2f | Humidity: %.0f%%",
			baseInsulation, waterResistance, wetness, effectiveHumidity * 100))
		
		-- ═══════════════════════════════════════════════════════════════════
		-- GETTING WET from rain
		-- ═══════════════════════════════════════════════════════════════════
		if isInRain and rainIntensity > 0 then
			...
			totalWaterProtection = math.min(0.93, totalWaterProtection)
			if equipmentData.hasUmbrella then
				totalWaterProtection = (totalWaterProtection + 2) / 3
			end
			
			local wetnessIncrease = rainIntensity/1000 * (1.0 - totalWaterProtection) * secondsPassed * 0.01
			...
			if wetnessIncrease > 0 then
				tempDbg.p(13, string.format("  Getting wet: +%.4f (protection: %.0f%%)",
					wetnessIncrease, totalWaterProtection * 100))
			end
		end
		
		-- ═══════════════════════════════════════════════════════════════════
		-- DRYING when not in rain
		-- ═══════════════════════════════════════════════════════════════════
		if not isInRain and wetness > 0 then
			local humidityPenalty = 1- (effectiveHumidity * 0.5)
			tempDbg.p(15, string.format("  %.4f base drying rate", 
						0.001 * humidityPenalty))
			local dryingRate = 0.001 * humidityPenalty
		...
		if isInRain and rainIntensity > 0 then
			...
			if equipmentData.hasUmbrella then
				headExposure = headExposure * 0.33  -- 67% protection
				bodyExposure = bodyExposure * 0.4  -- umbrella blocks 60% of body rain too
			end
		...
		-- ═══════════════════════════════════════════════════════════════════
		-- WIND CHILL EFFECT (main mechanic)
		-- ═══════════════════════════════════════════════════════════════════
		--tempDbg.p(15,tostring(saveData.weatherInfo.hasWindCover))
		--if currentEnvironmentTemp <= 10 and effectiveWindSpeed > 1.0 then
		if saveData.weatherInfo.hasWindCover ~= true then
			...
			if currentEnvironmentTemp <= 10 and effectiveWindSpeed > 1.0 then
				-- Wind chill for cold conditions (current code)
				...
				windChillEffect = windChillEffect * (1.0 - windProtection)
				
				--if windChillEffect > 0.05 then
					tempDbg.p(12, string.format("Wind chill: %s (wind: %.1f m/s, protection: %.0f%%)", 
						formatTemperatureModifier(windChillEffect), effectiveWindSpeed, windProtection * 100))
				--end
			...
			elseif currentEnvironmentTemp > 10 and currentEnvironmentTemp < 37 and effectiveWindSpeed > 1.0 then
				...
				local coolingEffect = (37 - currentEnvironmentTemp) * effectiveWindSpeed * 0.02 * (1.0 - windProtection)
				modifier = modifier + coolingEffect
				
				tempDbg.p(12, string.format("Wind cooling: %s (wind: %.1f m/s, protection: %.0f%%)", 
					formatTemperatureModifier(coolingEffect), effectiveWindSpeed, windProtection * 100))
			elseif currentEnvironmentTemp > 37 and effectiveWindSpeed > 1.0 then
				-- High temp wind effects
				local heatEffect = (currentEnvironmentTemp - 37) * effectiveWindSpeed * 0.05 * (1.0 - windProtection)
				modifier = modifier + heatEffect
				tempDbg.p(12, string.format("Wind heating: +%s (wind: %.1f m/s, protection: %.0f%%)", 
					formatTemperatureModifier(heatEffect), effectiveWindSpeed, windProtection * 100))
			end
		else
		...
		if fireDamage > 0 then
			tempDbg.p(15, string.format("  Fire Damage: +%.4f drying rate",
					fireDamage/200))
			wetness = math.max(0, wetness - fireDamage/200 * secondsPassed)
		end
		if wetness == 1 then
		...
		return modifier
	elseif isInWater < 0.9 and isInRain and rainIntensity > 0 then
		local robeProtection = equipmentData.hasRobe and 0.3 or 0
		local helmetProtection = equipmentData.hasHelmet and 0.15 or 0
		local skirtProtection = equipmentData.hasSkirt and 0.07 or 0
		local totalWaterProtection = waterResistance + robeProtection + helmetProtection + skirtProtection
	...
	local submersionLevel = math.min(1.0, isInWater)
	
	tempData.water.wetness = math.max(tempData.water.wetness, submersionLevel)
	tempData.water.lastWaterTemp = waterTemp
	tempData.water.timeInWater = tempData.water.timeInWater + secondsPassed
	
	tempDbg.p(13, string.format("  In water: %.0f%% submerged, water: %s, player: %s, air: %s", 
		submersionLevel*100, formatTemperature(waterTemp), formatTemperature(tempData.currentTemp), formatTemperature(currentEnvironmentTemp)))
	...
	local waterStrength = math.min(1.2, 0.7 + (submersionLevel * 0.5))
	-- At 0% submersion: 0.7 (shouldn't happen)
	-- At 50% submersion: 0.95
	-- At 100% submersion: 1.2 (capped)
	
	modifier = waterAirDiff * submersionLevel * waterStrength
	local waterRateMult = 1+submersionLevel*3
	...
	--if waterTemp < 10 and submersionLevel > 0.5 then
	--	-- Hypothermia effect - makes you colder than the water
	--	local hypothermiaPenalty = (10 - waterTemp) * submersionLevel * 0.8
	--	modifier = modifier - hypothermiaPenalty
	--	
	...
	--end
	--
	--if waterTemp <= 5 and submersionLevel > 0.3 then
	--	-- Arctic water - extreme additional penalty
	--	local arcticPenalty = (5 - waterTemp + 2) * submersionLevel * 1.2
	--	modifier = modifier - arcticPenalty
	--	
	--	tempDbg.p(11, string.format("  ARCTIC WATER: -%.1f°C (DEADLY!)", arcticPenalty))
	--	
	--	if submersionLevel > 0.8 then
	--		tempDbg.p(11, "  FULLY SUBMERGED - FREEZING TO DEATH!")
	--	end
	--end
	
	if waterTemp > 22 and submersionLevel > 0.5 then
		if waterTemp > 28 then
			local scaldingPenalty = (waterTemp - 28) * submersionLevel * 1.0
			modifier = modifier + scaldingPenalty
	...
	if modifier ~= 0 then
		tempDbg.p(12, string.format("Water mod: %s, Rate: x%.2f", (((modifier)>=0) and "+" or "")..formatTemperatureModifier(modifier), waterRateMult))
	end
...
local function resetWaterTimer()
	if isInWater and isInWater <= 0.1 then
		-- Just exited water
...
function calculateEnvironmentTemperature()
	...
	if cellInfo.isExterior then
		...
		if saveData.weatherInfo.sunStrength > 0 then
			if equipmentData.hasUmbrella or saveData.weatherInfo.isInShadow then
				local envMod = 1
				if climateType == "arctic"  then
					envMod = 0.5
				...
				shadowMod = saveData.weatherInfo.sunStrength * 2 * envMod
	...
	if magicMod ~= 0 or coolingMod ~= 1 or heatingMod ~= 1 then
		...
		if magicMod ~= 0 then
			debugStr = debugStr..string.format( " %s", (((magicMod)>=0) and "+" or "")..formatTemperatureModifier(magicMod))
		end
	...
	if fireShield <= 0 then
		local waterMod = getWaterTemperatureModifier(tempData.currentTemp, temperature)
		temperature = temperature + waterMod
	
		-- Reset water timer if exited water
		resetWaterTimer()
	else
		local wetnessBefore = tempData.water.wetness
		if fireDamage > 0 then
			tempDbg.p(15, string.format("  Fire Damage: +%.4f drying rate",
					fireDamage/200))
			tempData.water.wetness = math.max(0, tempData.water.wetness - fireDamage/200 * secondsPassed)
		end
		tempDbg.p(15, string.format("  Fire Shield: +%.4f drying rate",
					0.01))
		tempData.water.wetness = math.max(0, tempData.water.wetness - 0.01 * secondsPassed)
		G_wetnessChange = tempData.water.wetness - wetnessBefore / secondsPassed
	...
	if tempData.stewBonus then
		...
		if tempData.stewBonus <=0 then
			tempData.stewBonus = nil
				...
				tempDbg.p(11, string.format("Stew mod: %s", (((coldReduction)>=0) and "+" or "")..formatTemperatureModifier(coldReduction)))
		end
	end
	...
	-- ═══════════════════════════════════════════════════════════════════
	if not cellInfo.isExterior then
...
local function applyTemperatureEffects(state)
	...
	if tempData.currentTemp < CRITICAL_COLD_TEMPERATURE+10 then
		...
		if not next(vignetteColorFlags) then
			vignetteColorFlags.temp = "cold"
		end
	elseif tempData.currentTemp > CRITICAL_HOT_TEMPERATURE-10 then
		...
		if not next(vignetteColorFlags) then
			vignetteColorFlags.temp = "risingHeat"
		end
	end
	...
	-- if an item equipped is in HEAT_RESISTANT_TAGS or COLD_RESISTANT_TAGS and has resistance ~> 5 then immune to fire / frost spells
	-- if receiving warmth bonus from lava then take spell damage
		...
		if isInWater and isInWater > 0.25 and not hasColdImmunity then
		...
		if cellInfo.isExterior and TEMP_RM and isOnRedMountain > 0.26 and not hasFireImmunity then
			debuff = "sd_temp_sp_fire_slow"
			vignetteColorFlags.temp = "risingHeat"
		...
		if debuff then
			...
			local dmg = math.min(maxHealth * 0.2, (0.2+maxHealth * 0.003) * severity * secondsPassed * resistanceMult)
	...
	--print("vol",volume, vignetteColorFlags.temp, next(vignetteColorFlags), tempData.tempDamageSeverityTimer, severity)
	if TEMP_EXTREMES then
		-- determine debuff
		HEARTBEAT_INTERVAL = 1
		if tempData.currentTemp <= CRITICAL_COLD_TEMPERATURE + 0.1 then
			debuff = "sd_temp_sp_frost_slow"
			HEARTBEAT_INTERVAL = (1+(-tempData.currentTemp-CRITICAL_COLD_TEMPERATURE)/50)^0.7
	...
	if math.max(volume, volume2) >= 0.05 then
		heartbeatFlags.temp = math.min(1, math.max(volume, volume2))
		vignetteFlags.temp = math.min(1, math.max(volume, volume2))/4
	else
		heartbeatFlags.temp = nil
		vignetteFlags.temp  = nil
	end
...
local function module_temp_minute(c, refreshUi)
	...
	local heatSourcesActive = equipmentData.hasTorch or nearbyHotMod > 0 or nearbyWarmMod > 0
	local newHeatBonus = heatSourcesActive and 2 or 0
	local oldHeatBonus = tempData.heatSourceBonus or 0
	
	if newHeatBonus ~= oldHeatBonus then
		local bonusDiff = newHeatBonus - oldHeatBonus
		tempData.currentTemp = tempData.currentTemp + bonusDiff
		if bonusDiff > 0 then
			tempDbg.p(12, string.format("Heat sources active: +%s to current temp", formatTemperatureModifier(bonusDiff)))
		else
			tempDbg.p(12, string.format("Heat sources deactivated: %s to current temp", formatTemperatureModifier(bonusDiff)))
		end
	end
	tempData.heatSourceBonus = newHeatBonus
	
	--[[ leo's fire shield
	Fire Shield prevents temperature from dropping below 15°C
	if fireShield > 0 and tempData.targetTemp < 15 then
		tempData.targetTemp = math.max(tempData.currentTemp, 15)
		tempDbg.p(14, "Fire Shield: Maintaining " .. formatTemperature(15) .. " minimum")
	end
	
	-- Frost Shield prevents temperature from rising above 25°C
	if frostShield > 0 and tempData.targetTemp > 25 then
		tempData.targetTemp = math.min(tempData.currentTemp, 25)
		tempDbg.p(14, "Frost Shield: Maintaining " .. formatTemperature(25) .. " maximum")
	end
	]]
	...
	if racialData then
		...
		if racialData.exteriorGainMultiplier then
			if cellInfo.isExterior and racialData.exteriorGainMultiplier then
		...
		if racialData.twilightComfortDrift then
			...
			if isTwilight then
				-- Move toward comfortable zone faster during twilight
				local comfortableTarget = (racialData.comfortableMin + racialData.comfortableMax) / 2
				local isMovingToComfort = false
				
				-- Check if moving toward comfortable range
				if tempData.currentTemp < racialData.comfortableMin and tempDiff > 0 then
					isMovingToComfort = true
				elseif tempData.currentTemp > racialData.comfortableMax and tempDiff < 0 then
					isMovingToComfort = true
				end
				
				if isMovingToComfort then
					local oldRate = G_temperatureRate
					G_temperatureRate = G_temperatureRate * racialData.twilightComfortDrift
					tempDbg.p(13, string.format("  %s: Twilight comfort drift: %.2fx (%.3f -> %.3f)", 
						race, racialData.twilightComfortDrift, oldRate, G_temperatureRate))
				end
			end
		end
		...
		if racialData.nightCoolingBonus then
			...
			if isNight and tempDiff < 0 then  -- Cooling down at night
				...
				local inHotClimate = false
				if saveData.climateInfo and saveData.climateInfo.activeZones then
					for _, zone in ipairs(saveData.climateInfo.activeZones) do
						if zone.climate == "tropical" or zone.climate == "ashland" or 
						   zone.climate == "volcanic" or zone.climate == "grassland" then
							inHotClimate = true
							break
						end
					end
	...
	local currentlyInInterior = not cellInfo.isExterior
	...
	-- Apply Imperial fast adaptation bonus (2x decay rate)
	...
	if racialData.urbanPenalty then
		...
		if not cellInfo.isExterior then
			...
			-- Check if in urban climate zone
			if saveData.climateInfo and saveData.climateInfo.activeZones then
				for _, zone in ipairs(saveData.climateInfo.activeZones) do
					if zone.climate == "urban" then
						inUrban = true
						break
					end
				end
	...
	if race == "argonian" then
		local isWetOrSwimming = (tempData.water and tempData.water.wetness > 0.1) or (isInWater > 0.5)
		
		if isWetOrSwimming then
			-- Check if NOT in arctic or cold climate
			local inColdClimate = false
			if saveData.climateInfo and saveData.climateInfo.activeZones then
				for _, zone in ipairs(saveData.climateInfo.activeZones) do
					if zone.climate == "arctic" or zone.climate == "cold" then
						inColdClimate = true
						break
					end
				end
			...
			if not inColdClimate and waterTemp > 5 then
				local comfortTarget = 20  -- Comfortable temperature
				local driftToComfort = (comfortTarget - tempData.currentTemp) * 0.15  -- 15% drift per update
				
				-- Add the comfort drift to temperature difference
				tempDiff = tempDiff + (driftToComfort * secondsPassed * 0.01)
				
				if math.abs(driftToComfort) > 0.1 then
					local condition = (isInWater > 0.5) and "Swimming" or "Wet"
					tempDbg.p(13, string.format("  %s: %s comfort drift: %s toward %s (water: %s)", 
						race,
						condition,
						formatTemperatureModifier(driftToComfort * secondsPassed * 0.01),
						formatTemperature(comfortTarget),
						formatTemperature(waterTemp)))
				end
			end
		end
	end	
	...
	tempDbg.p(1, string.format("%s: %s (target: %s, change: %s, seconds: %.2f, rate: x%.2f)", 
		state.name, formatTemperature(tempData.currentTemp), formatTemperature(tempData.targetTemp), (((changeAmount)>=0) and "+" or "")..formatTemperatureModifier(changeAmount), secondsPassed, G_temperatureRate))
	...
	and isInWater <= 0.5
	then
		local debuffLevel = math.floor((tempData.water.wetness+0.1)*5)
...
local function onConsumedWater(liquid, remainingWater)
	if not NEEDS_TEMP then return end
	if remainingWater >=0.8 and tempData.currentTemp >= 16 then
		tempData.water.wetness = math.min(1,tempData.water.wetness + 0.2)
		tempData.currentTemp = math.max(15, tempData.currentTemp - 5)
...
table.insert(onConsumedWaterJobs, #onConsumedWaterJobs, onConsumedWater)
...
local function onConsume(item)
	...
	elseif item.type.record(item).name:lower():sub(1,4) =="stew" then
...
local function onLoad()
	...
	clockHour = math.floor((math.floor(core.getGameTime() / time.minute) / 60 + CLOCK_OFFSET))%24
	secondsPassed = 0
	module_temp_minute(nil, 0)
	updateTemperatureWidget()
end
...
table.insert(perHourJobs,module_temp_minute)

--table.insert(onFrameJobsSluggish,module_temp_minute)
table.insert(sluggishScheduler[1], module_temp_minute)
onFrameJobsSluggish.module_temp_minute = module_temp_minute

table.insert(equipmentChangedJobs, onEquipmentChanged)
--table.insert(onLoadJobs,onLoad)
onLoadJobs[9546] = onLoad
...
table.insert(G_removeBuffsJobs, removeTempBuffs)
...
--	end
--end
--table.insert(perMinuteJobs, test)