...
local woodcuttingActivators = {}
table.insert(woodcuttingActivators, 'tree')
table.insert(woodcuttingActivators, 'flora_tree')
table.insert(woodcuttingActivators, 'flora_bc_tree')
table.insert(woodcuttingActivators, 'bark')
table.insert(woodcuttingActivators, 'trunk')
table.insert(woodcuttingActivators, 'log')
table.insert(woodcuttingActivators, 'stump')

local module_cooking_activators = {}
...
woodcuttingBlacklist = {}
table.insert(woodcuttingBlacklist, 'furn_')
table.insert(woodcuttingBlacklist, 'flora_bm_shrub_02')

local function isWoodcuttingTarget(object, objectType)
	...
	if treeIgnoreList[raycastResult.hitObject.id] then
		return false
	end
	...
	for _, searchString in pairs(woodcuttingBlacklist) do
        if recordId:find(searchString) then
            return false
        end
    end
	for _, searchString in pairs(woodcuttingActivators) do
        if recordId:find(searchString) then
            return true
        end
    end
...
local progressColor = morrowindGold
...
local progressContainer = {
	...
	props = {
		size = v2(barWidth + 4, TOOLTIP_FONT_SIZE-2),
	},
	content = ui.content {}
}
...
startWoodcutting = function(target)
	...
	treeNameText.props.textColor = TOOLTIP_FONT_COLOR
...
completeWoodcutting = function()
	...
	I.SkillProgression.skillUsed(woodcuttingState.skill, {
		skillGain = 2+woodcuttingState.treeSize/3,
...
local function handleHit(groupname, key)
	...
	if isWoodcuttingTarget(raycastResult.hitObject, raycastResultType) then
		local now = core.getSimulationTime()
		local weaponSpeed = animation.getSpeed(self, groupname)
		if now < lastHit+0.25/weaponSpeed then
			return
		end
		lastHit = now
	
		local isCurrentTarget = woodcuttingState.isActive and raycastResult.hitObject == woodcuttingState.target
		
		-- Auto-start woodcutting on first hit if not active
		if not woodcuttingState.isActive or not isCurrentTarget then
			startWoodcutting(raycastResult.hitObject)
		end
		
		-- If this is our active target, add progress
		--if woodcuttingState.isActive and raycastResult.hitObject == woodcuttingState.target then
			...
			woodcuttingState.targetPos = raycastResult.hitPos
			...
			local skillValue = types.NPC.stats.skills.handtohand(self).modified*0.4
			...
			if equipped and types.Weapon.objectIsInstance(equipped) then
				...
				elseif weaponRecord.type == types.Weapon.TYPE.MarksmanBow or weaponRecord.type == types.Weapon.TYPE.MarksmanCrossbow or weaponRecord.type == types.Weapon.TYPE.MarksmanThrown then
					...
					durabilityHitMult = 0.5
					woodcuttingState.toolWarning = "Wrong tool"
					skillValue = types.NPC.stats.skills.marksman(self).modified*0.3
			...
			local progress = 1/finalDur * swingTypeMult * swingStrength
			...
			core.sendGlobalEvent("SpawnVfx", {
				...
				position = raycastResult.hitPos - v3(0, 0, 20),
				options = {scale = 0.4}
			})
...
local function raycastChanged()
	...
	if isWoodcuttingTarget(raycastResult.hitObject, raycastResultType) and I.UI.isHudVisible() then
		if woodcuttingTooltip then
			woodcuttingTooltip:destroy()
		end
		targettedTreeTime = core.getSimulationTime()
		targettingTree = true
		local treeRecordId = raycastResult.hitObject.recordId
		--local treeName = raycastResult.hitObject.type.record(raycastResult.hitObject).name or "Tree"
		local treeName = "Tree"
		...
		firstRow.content:add{
			type = ui.TYPE.Image,
			props = {
				resource = getTexture("textures/SunsDusk/Attack_symbolic.dds"),
				tileH = false,
				tileV = false,
	...
	---------------- FIREMAKING ----------------
	if raycastResult.hitObject and not saveData.playerInfo.isInWerewolfForm and logItems[raycastResult.hitObject.recordId] then
		...
		if I.UI.isHudVisible() then
			local amountOfLogs = tonumber(raycastResult.hitObject.recordId:sub(-1,-1))
			...
			local addTextColor = TOOLTIP_FONT_COLOR
			...
			local igniteTextColor = TOOLTIP_FONT_COLOR
			...
			line2.content:add{
				type = ui.TYPE.Image,
				props = {
					resource = getTexture("textures/SunsDusk/R_symbolic.dds"),
					tileH = false,
					tileV = false,
					size  = v2(TOOLTIP_FONT_SIZE,TOOLTIP_FONT_SIZE),
					alpha = 0.6,
	...
	if raycastResult.hitObject and not saveData.playerInfo.isInWerewolfForm and litLogItems[raycastResult.hitObject.recordId] then
		if firemakingTooltip then
			firemakingTooltip:destroy()
			firemakingTooltip = nil
		end
		...
		if I.UI.isHudVisible() then
			local amountOfLogs = tonumber(raycastResult.hitObject.recordId:sub(-5,-5))
			types.Player.setControlSwitch(self, types.Player.CONTROL_SWITCH.Magic, false) 
			types.Player.setControlSwitch(self, types.Player.CONTROL_SWITCH.Fighting, false)
...
input.registerTriggerHandler("ToggleSpell", async:callback(function(dt, use, sneak, run)
	if firemakingTooltip and raycastResult.hitObject and logItems[raycastResult.hitObject.recordId] then
		local amountOfLogs = tonumber(raycastResult.hitObject.recordId:sub(-1,-1))
		if amountOfLogs >= 3 then
			core.sendGlobalEvent("SunsDusk_igniteFire", {self, raycastResult.hitObject})
		end
	end
end))

input.registerTriggerHandler("ToggleWeapon", async:callback(function(dt, use, sneak, run)
	if firemakingTooltip and raycastResult.hitObject and (logItems[raycastResult.hitObject.recordId] or litLogItems[raycastResult.hitObject.recordId]) then
		local amountOfLogs = tonumber(raycastResult.hitObject.recordId:sub(-1,-1)) or tonumber(raycastResult.hitObject.recordId:sub(-5,-5))
		if amountOfLogs < 5 then
			core.sendGlobalEvent("SunsDusk_upgradeFire", {self, raycastResult.hitObject})
		end
	end
end))
