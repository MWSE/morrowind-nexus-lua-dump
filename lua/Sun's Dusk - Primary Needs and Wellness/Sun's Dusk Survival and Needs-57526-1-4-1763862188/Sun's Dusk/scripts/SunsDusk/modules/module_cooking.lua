...
local function loadUiSettings()
	...
	-- Setup storage
	totalWidth = math.max(100,math.min(hudLayerSize.x*0.9, cookingUiSettingsSection:get("WIDGET_WIDTH") or totalWidth))
	listHeight = math.max(100,math.min(math.floor(hudLayerSize.y*0.8), cookingUiSettingsSection:get("LIST_HEIGHT") or listHeight))
...
local function fillCookingUi()
	...
	progressBar.content:add { 
		...
		props = {
			text = "Level "..math.floor(cookingData.level + cookingData.racialBonus + cookingData.alchemyBonus + cookingData.shortBladeBonus + cookingData.luckBonus),
...
function makeCookingUi()
	...
	alchemySkill = types.Player.stats.skills.alchemy(self).modified + types.Player.stats.attributes.intelligence(self).modified/10
	
	cookingData.alchemyBonus = alchemySkill/10
	cookingData.shortBladeBonus = math.min(25, types.Player.stats.skills.shortblade(self).modified-5)/10
	cookingData.luckBonus = types.Player.stats.attributes.luck(self).modified/20
...
function getIngredientEffects(ingredient)
	...
	local cookingLevel = cookingData.level + cookingData.racialBonus + cookingData.alchemyBonus + cookingData.shortBladeBonus + cookingData.luckBonus
	...
	for _, idx in ipairs(sortedIndices) do
		...
		if ingredient and ingredient.data then
			...
			ingredientRow.content:add{
				...
				type = ui.TYPE.Text,
				props = {
					text = ingredient.name.." ["..(ingredient.data.ingredientRank or 1).."]",
					textColor = morrowindGold,
					textShadow = true,
					textShadowColor = util.color.rgb(0,0,0),
					textSize = textSize*0.9,
						...
						for _, effect in pairs(ingredientEffects) do
							...
							if thisEffectKey == effectKey and effect.vanillaIndex then
								if not isEffectVisible(ingredient, effect) then
									ui.showMessage("Discovered " ..effect.text.." in "..ingredient.name)
...
input.registerTriggerHandler('Activate', async:callback(function()
    if not SDCookingUI and (cookingTooltip or raycastResult and raycastResult.hitObject and raycastResult.hitObject.recordId:match( "^sd_wood_(%d+)_lit$")) then
        makeCookingUi()
...
local function raycastChanged()
	if (raycastResultType == "Static" or raycastResultType == "Light" or raycastResultType == "Activator") and not saveData.playerInfo.isInWerewolfForm then
		...
		if isCookingActivator and I.UI.isHudVisible() and not raycastResult.hitObject.recordId:find("firewat") then
...
local function onLoad(originalData)
	...
	if not cookingData.shortBladeBonus then
		cookingData.shortBladeBonus = 0
	end	
	if not cookingData.luckBonus then
		cookingData.luckBonus = 0
	end
...
local function UiModeChanged(data)
	...
	if core.magic.spells.records["startcooking_dummy"] then
		types.Actor.spells(self):remove("startcooking_dummy")
	end
end

table.insert(UiModeChangedJobs, UiModeChanged)

local function checkForStartCookingBuff()
	if isInDialogue then
		...
		if hasDummySpell then
			types.Actor.spells(self):remove("startcooking_dummy")
			cookingWithInnkeeper = true
			makeCookingUi()
		end
	end
end

--table.insert(onFrameJobsSluggish,checkForStartCookingBuff)
table.insert(sluggishScheduler[5],checkForStartCookingBuff)
onFrameJobsSluggish.checkForStartCookingBuff = checkForStartCookingBuff
