...
function updateSDVignette(dt)
	...
	if flashVignette > 0 then
		flashVignette = math.max(0, flashVignette - 0.8 * core.getRealFrameDuration())
	end
	
	-- Calculate combined alpha multiplicatively
	local remaining = (1 - flashVignette)
	
	for moduleName, alpha in pairs(vignetteFlags) do
		if alpha > 0 then
			remaining = remaining * (1 - alpha)
		end
	end
	local color = "default"
	for _, colorName in pairs(vignetteColorFlags) do
		color = colorName
	end
...
table.insert(sluggishScheduler[2], onFrame)
onFrameJobsSluggish.uiOnFrame = onFrame