local cf = mwse.loadConfig("DragonDoor!", {m = false, nost = true, nor = 300, noacs = 1800, help = true, vamp = true, nosav = true, hdist = 3000, skey = {keyCode = 56}})
local H = {}	local TH = {}		local p, mp, TIM, Dtim, AC
...
local DAR = {}	local DAT, fr
local function DoorKick(e)	if DAR[e.reference] then e.mobile.impulseVelocity = DAR[e.reference] end
	if tes3.worldController.lastFrameTime ~= fr then fr = tes3.worldController.lastFrameTime	DAT = DAT + 1	if DAT == 12 then event.unregister("calcMoveSpeed", DoorKick)	DAT = nil	DAR = {} end end
end

local list, runlist, count
...
elseif cf.nost then
	for ref in tes3.iterate(e.target.cell.actors) do if ref.mobile and not ref.mobile.isDead and e.target.position:distance(ref.position) < cf.nor then DAR[ref] = ref.sceneNode.rotation:transpose().y * -cf.noacs end end
	if table.size(DAR) > 0 and not DAT then DAT = 0		fr = tes3.worldController.lastFrameTime		event.register("calcMoveSpeed", DoorKick) end
...
local function save(e) if table.size(H) > 0 and not tes3.worldController.inputController:isKeyDown(cf.skey.keyCode) then	local ms = ""	--p.position:distance(r.position)	r.mobile.boundSize.z
...
local function loaded(e) p = tes3.player	 mp = tes3.mobilePlayer		H = {}	TH = {}		TIM = nil		Dtim = nil	end		event.register("loaded", loaded)
...
page:createSlider{label = "Backward speed for stuck NPCs", min = 500, max = 3000, step = 50, jump = 100, variable = mwse.mcm.createTableVariable{id = "noacs", table = cf}}
