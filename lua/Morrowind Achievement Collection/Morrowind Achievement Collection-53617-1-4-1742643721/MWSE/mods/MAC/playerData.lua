...
function pData.soulFilter(e)
    ...
    if (e.reference.baseObject.objectType == tes3creature and e.reference.baseObject.soul > 600) then
        ...
        event.unregister(tes3.event.filterSoulGemTarget, pData.soulFilter, {priority = -math.huge})
    end
end
function pData.stealOrdActivate(e)
    ...
    if (e.activator == tes3.player) then
        ...
        elseif (e.target.baseObject.objectType == tes3.objectType["container"] ) then
            ...
            if not event.isRegistered(tes3.event.menuExit, pData.closedCallback) then -- Thanks NullCascade!
...
function pData.initAchieveData()
    ...
    if (myData["soulachieve"] == false and not event.isRegistered(tes3.event.filterSoulGemTarget, pData.soulFilter, {priority = -math.huge})) then
        event.register(tes3.event.filterSoulGemTarget, pData.soulFilter, {priority = -math.huge})
    end
