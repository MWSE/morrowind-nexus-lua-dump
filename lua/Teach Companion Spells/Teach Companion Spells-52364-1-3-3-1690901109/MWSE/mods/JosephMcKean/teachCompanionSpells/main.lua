...
local function checkIfPotentialCompanion(ref)
	...
	if isPotentialCompanion then log:debug("%s is a potential companion", ref.id) end
...
local function checkIfCurrentCompanion(ref)
	...
	if aiPlanner then
		...
		local isFollowing = aiPackage and aiPackage.type == tes3.aiPackage.follow
		local followTarget = aiPackage and aiPackage.targetActor and aiPackage.targetActor.reference
		local isFollowingMe = isFollowing and (followTarget == tes3.player)
		isCurrentCompanion = isCurrentCompanion or isFollowingMe
		if isCurrentCompanion then log:debug("%s is currently a companion", ref.id) end
	end
...
local function buttonDisabled(playerSpells, actorSpells) return table.empty(playerSpells) and table.empty(actorSpells) end
...
local function onMenuDialogActivated(e)
	...
	if checkIfPotentialCompanion(actorRef) or checkIfCurrentCompanion(actorRef) then
		...
		if (actorRef.context and actorRef.context["companion"] and actorRef.context["companion"] ~= 1) then button.visible = false end
		...
		local function showTeachSpellsMenu()
			...
			for _, ref in ipairs({ tes3.player, actorRef }) do
				...
				if spells then for _, spell in ipairs(spells) do createSpellBlock(ref, spell) end end
			end
...
local function onInit()
	...
	if RightClickMenuExit and RightClickMenuExit.registerMenu then RightClickMenuExit.registerMenu({ menuId = "MenuTeachSpells", buttonId = "MenuTeachSpells_ok" }) end
end
...
local function registerModConfig()
	...
	template:register()
	template.onClose = function() mwse.saveConfig(configPath, config) end
	...
	preferences:createYesNoButton({
		...
		callback = function(self) log:setLogLevel(self.variable.value and "DEBUG" or "INFO") end,
	})
end
