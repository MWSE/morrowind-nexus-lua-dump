...
I.Combat.addOnHitHandler(function(attack)
	if attack.successful==true then
		if attack.weapon then
			...
			if WeaponRecord.enchant and (WeaponRecord.type==types.Weapon.TYPE.MarksmanCrossbow or WeaponRecord.type==types.Weapon.TYPE.MarksmanBow) then
...
--				print(EnchantRecord,EnchantRecord.type,core.magic.ENCHANTMENT_TYPE.CastOnUse)
				if EnchantRecord and ((onStrikeOnly==false and EnchantRecord.type==core.magic.ENCHANTMENT_TYPE.CastOnUse) or EnchantRecord.type==core.magic.ENCHANTMENT_TYPE.CastOnStrike) then
...
local function BACOSDeclareType(data)
	onStrikeOnly=data.onStrikeOnly
end


local function onActive()
	nearby.players[1]:sendEvent("BACOSDeclareTypeAsked",{Actor=self})
end
...
return {
	eventHandlers = {	
						...
						BACOSDeclareType=BACOSDeclareType,
					},
	engineHandlers = {
        onUpdate = onUpdate,
		onSave=onSave,
