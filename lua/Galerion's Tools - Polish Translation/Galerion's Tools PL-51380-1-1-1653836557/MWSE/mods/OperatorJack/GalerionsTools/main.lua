...
if (mwse.buildDate == nil) or (mwse.buildDate < 2020402) then
    local function warning()
        tes3.messageBox(
            "[Galeron's Tools ERROR] Twoje MWSE jest nieaktualne!"
            .. "Aby móc korzystać z tego moda, należy zaktualizować MWSE do nowszej wersji."
        )
    end
...
local function onInventoryItemSelected(e)
    ...
    if (e.item.enchantment == nil) then
        tes3.messageBox("Nie udało się zainicjować ekstraktora dusz. Być może twórca wie, dlaczego tak się dzieje. (Proszę zgłosić to poprzez stronę Nexus).")
        return
    end
    ...
    if (enchantment <= 15) then
        tes3.messageBox("Nie posiadasz wystarczających umiejętności w zaklinaniu, aby podjąć się tego procesu. Narzędzie reaguje na twoje próby, a ty nagle odczuwasz osłabienie.")
    ...
    elseif (enchantment > 15 and enchantment <= 30 and math.random(0, enchantment) <= 15) then
        tes3.messageBox("Próbujesz wydobyć duszę z zaklętego przedmiotu, ale tym razem nie udaje ci się zainicjować procesu. Narzędzie reaguje na twoje próby, a ty nagle odczuwasz osłabienie.")
    ...
    if (hasEmptySoulGemForEnchantmentPoints == false) then
        tes3.messageBox("Nie posiadasz wystarczająco dużego klejnotu duszy, aby pomieścić duszę tego zaklęcia.")
        return
    end
    ...
    if (success == true) then
        tes3.messageBox("Wydobywasz duszę z zaklętego przedmiotu. Przedmiot zostaje zniszczony podczas tego procesu.")
    ...
    elseif (success == false) then
        tes3.messageBox("Nie udaje ci się wydobyć duszy z zaklętego przedmiotu. Przedmiot zostaje zniszczony podczas tego procesu.")
...
local function onExtractorEquip(e)
    ...
    tes3ui.showInventorySelectMenu({
		title = "Ekstrakcja dusz",
		noResultsText = "Nie znaleziono odpowiednich przedmiotów",
		filter = filterItems,
...
local function onUiObjectTooltip(e)
    ...
    innerBlock.borderAllSides = 0
        ...
        local enchantmentPointsText = "Punkty zaklinania: " .. enchantmentPoints
    ...
    innerBlock.borderAllSides = 0 
        ...
        local extractionText = "Szansa wydobycia: " .. chance     
