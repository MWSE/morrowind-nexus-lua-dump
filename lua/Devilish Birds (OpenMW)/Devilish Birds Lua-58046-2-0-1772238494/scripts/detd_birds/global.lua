...
-- Random spawn delay between 0 and 2 minutes
...
-- Cached per-flock tuning (avoid recomputing every move tick)
local cachedSpeed = 0
...
------------------------------------------------------------
-- TUNING (CONTROL TELEPORT SPEED/DISTANCE HERE)
------------------------------------------------------------
local SPAWN_RADIUS   = 5500

-- ==========================================================
-- SCALE SETTINGS (EDIT THESE)
-- Separate controls for LEAD vs FOLLOW, and for each type.
-- ==========================================================
local LEAD_SCALE_CLIFFRACER   = 0.3
local FOLLOW_SCALE_CLIFFRACER = 1.0
...
-- ==========================================================
-- ORPHAN CLEANUP SETTINGS (PERF: only FAR sweep)
-- ==========================================================
...
-- HELPERS
------------------------------------------------------------
local function isValid(obj) return obj and obj:isValid() end

local function safeRemove(obj, label)
    if not isValid(obj) then return end
    local ok, err = pcall(function() obj:remove(1) end)
    if not ok then
...
local function safeTeleport(obj, cell, pos)
    ...
    local ok, err = pcall(function() obj:teleport(cell, pos) end)
    if not ok then
...
local function anyBirdAlive()
    for i = 1, 3 do
        if isValid(leadBirds[i]) or isValid(followBirds[i]) then return true end
    end
    return false
end
...
local function isTrackedBird(actor)
    for i = 1, 3 do
        if actor == leadBirds[i] or actor == followBirds[i] then return true end
    end
    return false
end
...
local function updateFollowGrow(dt)
    for i = 1, 3 do
        ...
        if isValid(follow) and followGrowT[i] < followGrowDur then
            ...
            pcall(function() follow:setScale(s) end)
        end
    end
end

------------------------------------------------------------
-- CLEANUP QUEUE (ORDERED)
------------------------------------------------------------
...
local function destroyAllBirds(reason)
    ...
    if reason then dprint("DestroyAllBirds:", reason) end
...
local function getLeadSpawn(player, birdType)
    ...
    local dir = (target - spawnPos):normalize()
...
local function pickSeagullVariant()
    if random() < GOLDFINCH_TEST_CHANCE then return "goldfinch" end
    return "seagull"
end
...
local function spawnPair(playerCell, pos, leadId, followId, index)
    ...
    pcall(function() lead:setScale(getLeadScaleForType(activeFlockType)) end)
    ...
    pcall(function() follow:setScale(followGrowStart) end)
...
local function spawnFlock(player, birdType)
    ...
    for i = 2, count do
        ...
        spawnPair(cell, leadPos + offset, leadIds[i], followIds[i], i)
    end
end
...
------------------------------------------------------------
-- MOVEMENT (do NOT change lead/follow mechanic)
------------------------------------------------------------
...
return {
    engineHandlers = {
        onUpdate = function(dt)
            ...
            -- Periodic FAR orphan sweep (cheap interval)
