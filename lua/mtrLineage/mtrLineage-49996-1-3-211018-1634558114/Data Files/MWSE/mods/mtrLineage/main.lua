...
local linUID = tes3ui.registerID("LineageNameUI")

local function updateLGStat()
    local menu = tes3ui.findMenu(tes3ui.registerID("MenuStat"))
    if menu then 
        local lineage = menu:findChild(linUID)
       
        if data and data.currentLineage then
            lineage.text =  lineagesList[data.currentLineage].name
        else
            lineage.text = "None"
        end
        menu:updateLayout()
...
local function createLGStat(e)
    ...
    local label = bfBlock:createLabel{ text = "Lineage"}
    label.color = tes3ui.getPalette("header_color")


    local lineageBlock = bfBlock:createBlock()
    lineageBlock.paddingLeft = 5
    lineageBlock.autoHeight = true
    lineageBlock.widthProportional = 1.0

    local lineageLabel = lineageBlock:createLabel{ id = linUID,  text = "None" }
    if data and data.currentLineage then
        lineageLabel.text = lineagesList[data.currentLineage].name
    end
    lineageLabel.wrapText = true
    lineageLabel.widthProportional = 1
    --lineage.absolutePosAlignX = 1.0
    lineageLabel.justifyText = "right"
    

    label:register("help", createLGTooltip )
    lineageBlock:register("help", createLGTooltip )
    lineageLabel:register("help", createLGTooltip )

    menu:updateLayout()
end
event.register("uiActivated", createLGStat, { filter = "MenuStat", priority = 2})
...
local function createPerkMenu()
    ...
    local noLGButton = perkListBlock:createTextSelect{ text = "-Select Lineage-" }
    do
        noLGButton.color = tes3ui.getPalette("disabled_color")
        noLGButton.widget.idle = tes3ui.getPalette("disabled_color")
        noLGButton.autoHeight = true
        noLGButton.layoutWidthFraction = 1.0
        noLGButton.paddingAllSides = 2
        noLGButton.borderAllSides = 2

        noLGButton:register("mouseClick", function()
            data.currentLineage = nil
            local header = tes3ui.findMenu(perksMenuID):findChild(descriptionHeaderID)
            header.text = "No Lineage Selected"
        
            local description = tes3ui.findMenu(perksMenuID):findChild(descriptionID)
            description.text = "Select a Lineage from the list."
            description:updateLayout()
        end)
    end
    ...
    for _, lineage in pairs(sortedList) do
        ...
        perkButton:register("mouseClick", function() clickedPerk(lineage) end )
    ...
    noLGButton:triggerEvent("mouseClick")
end
