...
local BARTER_MODE = Interfaces.UI and Interfaces.UI.MODE and Interfaces.UI.MODE.Barter or "Barter"
...
local function makeMerchantKey(recordId)
...
local function getMerchantKey(actor)
    return makeMerchantKey(actor and actor.recordId or nil)
end

local function findLegacyEntryByRecordId(recordId)
    if type(recordId) ~= "string" or recordId == "" then
        return nil, nil
    end
    ...
    for key, entry in pairs(state.merchants) do
        if type(entry) == "table" and entry.recordId == recordId then
            local entryKey = makeMerchantKey(entry.recordId)
            if entryKey == key then
                -- A canonical entry already exists; avoid migrating an arbitrary duplicate.
                return nil, nil
            end
            if matchedEntry ~= nil then
                -- Ambiguous legacy entries with same record id; don't auto-migrate.
                return nil, nil
            end
            matchedKey = key
            matchedEntry = entry
    ...
    return matchedKey, matchedEntry
end

local function getTrackedMerchantEntry(actor)
    local key = getMerchantKey(actor)
    if key and state.merchants[key] then
        return key, state.merchants[key]
    end

    local legacyKey = actor and actor.id or nil
    if legacyKey and state.merchants[legacyKey] then
        local migrated = state.merchants[legacyKey]
        if key then
            state.merchants[key] = migrated
            if legacyKey ~= key then
                state.merchants[legacyKey] = nil
            end
            return key, migrated
        end
        return legacyKey, migrated
    end

    local oldKey, oldEntry = findLegacyEntryByRecordId(actor and actor.recordId or nil)
    if oldKey and oldEntry and key then
        state.merchants[key] = oldEntry
        if oldKey ~= key then
            state.merchants[oldKey] = nil
        end
        return key, oldEntry
    end
...
local function snapshotMerchant(actor)
    ...
    local entry = {
        ...
        hadLiveRestocking = hasEntries(restockBaseline),
        lastRestockDay = gameDayNow(),
    }

    local key = getMerchantKey(actor) or actor.id
    state.merchants[key] = entry
    return entry
end

local function ensureMerchantEntry(actor)
    local _, entry = getTrackedMerchantEntry(actor)
    if entry then
        local inventories = getMerchantInventories(actor)
        local liveRestocking = convertRestockingStacksToStatic(inventories)

        entry.recordId = actor.recordId
        entry.baselineAll = entry.baselineAll or collectCounts(inventories)
        entry.restockBaseline = entry.restockBaseline or {}
        if entry.hadLiveRestocking == nil then
            entry.hadLiveRestocking = hasEntries(entry.restockBaseline)
        end
        ...
        if hasEntries(liveRestocking) then
            entry.hadLiveRestocking = true
        end
...
local function onUiModeChanged(data)
    ...
    if data.oldMode ~= BARTER_MODE and data.newMode ~= BARTER_MODE then
        return
    end
