...
local function showWealthStats(state)
    local messages = { "" }
    for _, stats in pairs(mWorld.getActiveCells()) do
        for _, npc in ipairs(stats.cell:getAll(T.NPC)) do
            local wealth = state.npcsWealth[npc.recordId]
            if wealth then
                table.insert(messages, string.format("\"%s\": total=%d, spent=%d, npcCount=%d", npc.recordId, wealth.total, wealth.spent, wealth.npcCount))
            end
        end
    end
...
local function createItem(recordId, count, modId1, lvl1, modId2, lvl2)
    local player = world.players[1]
    local item = world.createObject(recordId, 1)
    item:moveInto(player.type.inventory(player))
    core.sendGlobalEvent(mDef.events.convertTestItem, { item = item, count = count, modId1 = modId1, lvl1 = lvl1, modId2 = modId2, lvl2 = lvl2 })
end
module.createItem = createItem

local function convertTestItem(state, item, count, modId1, lvl1, modId2, lvl2)
    local lvlMods = {
    ...
    local inventoryItem = mWorld.replaceItem(world.players[1], item, record, 1, newRecord, count or 1)
    state.items[inventoryItem.item.id] = mTypes.new.itemData(inventoryItem.item, 1, record.id, false, mMod.lvlModsToLvlModIds(lvlMods))
end
