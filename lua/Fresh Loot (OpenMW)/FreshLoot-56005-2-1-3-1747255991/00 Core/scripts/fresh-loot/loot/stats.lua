...
local function addToNpcsWealth(state, recordId, wealth, isNpc)
...
local function getLootLevel(state, loot)
    ...
    -- TODO: Remove this retro-compatibility code later
    statsOverride = statsOverride and statsOverride.source or statsOverride
    ...
    if statsOverride then
        if statsOverride.ownerRecordId then
            ...
            if statsOverride.hasWares then
                local reason = string.format("merchant owner \"%s\" level %d and player level %d", statsOverride.ownerRecordId, levelOverride, state.playerLevel)
                return math.min(state.playerLevel, levelOverride), 0, { reason }
            end
