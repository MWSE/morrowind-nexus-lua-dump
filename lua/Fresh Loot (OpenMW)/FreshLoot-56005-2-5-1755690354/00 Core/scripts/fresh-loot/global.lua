...
local function onNewContainers(containers, actors, player)
    mAnalyze.analyseActors(state, actors)
    mAnalyze.analyseContainers(state, containers, actors, player)
end
...
mSTracker.trackSettings(function() return state end)
...
return {
    ...
    eventHandlers = {
        ...
        [mDef.events.onNewContainers] = function(data) onNewContainers(data.containers, data.actors, data.player) end,
