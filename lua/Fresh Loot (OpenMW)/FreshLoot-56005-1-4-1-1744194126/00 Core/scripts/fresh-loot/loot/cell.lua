...
local function analyseContainer(state, container, actors)
    ...
    if state.learned.containers.soldItems[container.id] then return end

    local levelStatsOverride
    local ownerId = container.owner.recordId

    -- Code to do only once, excluding the integration of new actors in the cell
    ...
    if owner then
        ...
        levelStatsOverride = mTypes.new.lootKeeperStats(mObj.getActorLevel(owner), levelSource, owner.recordId)
