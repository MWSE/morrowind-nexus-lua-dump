...
module.getActorScaledLevel = function(state, level, isCreature)
    if isCreature then
        level = level * math.min(1, (level / mStore.settings.endGameLootLevel.get()) ^ 0.5)
    end
    return level
end
...
local function getKeeperLevel(state, stats, levelOverride, levelOverrideRatio)
    ...
    if stats.isPassive then
        levelFactor = levelFactor * mStore.settings.passiveActorsLevelRatio.get() / 100
    end
...
module.getLootLevelStats = function(state, loot)
    ...
    local lootLevel = 0
    local chanceBonus = 0
    local endGameLevel = mStore.settings.endGameLootLevel.get()
