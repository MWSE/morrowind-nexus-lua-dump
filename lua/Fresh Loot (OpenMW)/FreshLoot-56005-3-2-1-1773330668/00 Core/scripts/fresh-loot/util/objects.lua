...
module.getRecord = function(object)
...
module.getActorLevel = function(overrides, actor)
    if overrides[actor.recordId] then
        log(string.format("Level overridden to %d for actor %s", overrides[actor.recordId], module.objectId(actor)))
        return overrides[actor.recordId]
    end
    return math.max(1, T.Actor.stats.level(actor).current)
end
...
module.itemRecordToTable = function(type, record)
    local newRecord = {}
    for _, field in ipairs(mT.itemTypes[type].recordFields) do
...
local function getArmorClass(record)
    ...
    if record.weight <= weightFactor * fLightMaxMod + epsilon then
        return mT.armorClasses.Light
    end
    if record.weight <= weightFactor * fMedMaxMod + epsilon then
        return mT.armorClasses.Medium
    end
    return mT.armorClasses.Heavy
end
...
module.getItemsValueSum = function(loot)
    ...
    for _, item in ipairs(inventory:getAll()) do
        local record = module.getRecord(item)
...
module.hasValidInventory = function(container)
    ...
    for type in pairs(mT.itemTypes) do
        for _, item in ipairs(inventory:getAll(type)) do
            if mT.itemTypes[item.type] then
                return true
            end
        end
    end
...
module.isValidContainer = function(state, record)
    return not state.cache.exceptions.excludedContainerIds[record.id]
            and (not record.mwscript or mT.validContainerScripts[record.mwscript])
            and not record.isOrganic
            and not record.isRespawning
end

module.isValidActor = function(state, record)
    return not state.cache.exceptions.excludedActorIds[record.id]
