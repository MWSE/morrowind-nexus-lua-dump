...
module.itemWindowExcludedUiModes = {
    MainMenu = true,
    Alchemy = true,
    Enchanting = true,
}
...
local function requestWindow()
    ...
    core.sendGlobalEvent(mDef.events.filterConvertedItems, mT.new.requestEvent(mDef.events.returnConvertedItems, self, items))
end
...
local function createWindow(fromRowIndex)
    ...
    if #rowHasNameColumn == 0 then
        ...
        lastShownRowIndex = math.min(fromRowIndex + mStore.settings.maxItemWindowRowsPerPage.get() - 1, #rowHasNameColumn)
...
module.showWindow = function(convertedItems)
    setItemColumns(convertedItems)
    createWindow(1)
end

module.toggleItemWindow = function()
    if itemWindow then
        itemWindow:destroy()
        if module.itemWindowExcludedUiModes[I.UI.getMode()] then
            itemWindow = nil
            return
        end
    ...
    elseif not module.itemWindowExcludedUiModes[I.UI.getMode()] then
        requestWindow()
    end
end

module.onKeyPress = function(key)
    if key.code == mStore.settings.itemsWindowKeyKeyboard.get() then
        module.toggleItemWindow()
    end
end
