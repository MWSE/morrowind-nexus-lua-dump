...
local mSettings = require("scripts.fresh-loot.settings.settings")
...
local refreshActorCallback = async:registerTimerCallback(mDef.callbacks.refreshActor, function(data)
    -- send event to get the state
    core.sendGlobalEvent(mDef.events.refreshActorCallback, data)
end)
...
local function onActorActive(actor)
    ...
    if state.actorsToRefresh[actor.id] then
        async:newSimulationTimer(
                0.5 + math.random() / 2,
                refreshActorCallback,
                { actor = actor, restoreTypes = state.actorsToRefresh[actor.id].restoreTypes })
        state.actorsToRefresh[actor.id] = nil
        return
    end
end

local function doRefreshActor(actor, restoreTypes)
    log(string.format("Actor %s is being refreshed", mObj.objectId(actor)))
...
local function onUpdate(deltaTime)
    ...
    if state.playerLevel == prevLevel
            or mStore.settings.playerLevelScaling.get() == 0
            or not mStore.settings.doEquippedItems.get() then
        return
    end
...
local function disableMod(message)
    I.Settings.updateRendererArgument(mStore.sections.global.key, mStore.settings.enabled.key, { disabled = true })
...
local function onInit()
    ...
    mSettings.setItemListsSetting(state)
    mStore.addTrackerCallback(function(key, oldValue)
        mSettings.updateSettings(state, key, oldValue)
    end)
    ...
    state.cache.exceptions = require("scripts.fresh-loot.exceptions.main")
end

local function onSave()
    return {
        state = mHelpers.addAllToMap({}, state, function(key, _) return key ~= "cache" end),
...
local function onLoad(data)
    ...
    if data then
        state = data.state
        log(string.format("Loading Fresh Loot save format v%s...", data.version))
...
local function onUnpause()
    local restoreTypes = mSettings.getSettingChanges()
...
return {
    ...
    interface = {
        ...
        revertLoot = function(loot) mConvert.restoreItems(state, mT.new.itemRestoreFilter({ [mT.itemRestoreTypes.InLoot] = true }, loot)) end,
        test = {
            pickModifiers = function(lootLevel, alpha) mTest.testPickModifiers(lootLevel, alpha) end,
    ...
    eventHandlers = {
        Unpause = onUnpause,
        [mDef.events.setSetting] = function(data) mStore.settings[data.key].set(data.value) end,
        [mDef.events.onActorActive] = onActorActive,
        [mDef.events.refreshActorCallback] = function(data) doRefreshActor(data.actor, data.restoreTypes) end,
        [mDef.events.checkEquipmentCallback] = function(actor) mAnalyze.doCheckEquipment(state, actor) end,
        [mDef.events.detachScript] = function(data) mWorld.detachScript(data.object, data.scriptPath) end,
        [mDef.events.requestActorStats] = function(data) mWorld.requestActorStats(data.actor, data.responseEvent) end,
        ...
        [mDef.events.setItemListsSetting] = function() mSettings.setItemListsSetting(state) end,
