...
module.onPBArmorEquipped = function(state, actor, items)
    log(string.format("Protected Beasts: Actor %s equipped", mObj.objectId(actor)), mT.logLevels.Debug)
    ...
    for _, item in ipairs(items) do
        ...
        if not itemData then
            log(string.format("Protected Beasts: The item %s is not an FL item", mObj.objectId(item.oldItem)))
            return
        else
            log(string.format("Protected Beasts: Replacing state item %s (PB ID \"%s\") with %s for actor %s",
                    mObj.objectId(item.oldItem), item.pbRecordId, mObj.objectId(item.newItem), mObj.objectId(actor)))
        end
