...
module.testPickModifiers = function(lootLevel, alpha)
    local prevAlpha
    if alpha then
    ...
    for _ = 1, 100000 do
        local lvl = mMod.getRandomModifierLevel(lootLevel)
...
module.testItemModifiers = function(state, itemId, lootLevel)
    local record, type
    ...
    local affixTypes = { mT.affixes.Any }
    local stats = {}
...
module.showFactionRankStats = function(state)
    local messages = { "" }
...
module.showWealthStats = function(state)
    local messages = { "" }
    for _, actor in pairs(mWorld.getActiveActors()) do
        if actor.type == T.NPC then
            local wealth = state.npcsWealth[actor.recordId]
            if wealth then
                table.insert(messages, string.format("%s: total=%d, spent=%d, npcCount=%d", mObj.objectId(actor), wealth.total, wealth.spent, wealth.npcCount))
            end
        end
    end
...
module.createItem = function(recordId, count, modId1, lvl1, modId2, lvl2, actorRecordId)
    local actor
...
module.convertTestItem = function(state, item, count, modId1, lvl1, modId2, lvl2, actor)
    local lvlMods = {
        mT.new.lvlMod(state.cache.modifiers[modId1], lvl1)
    }
    if modId2 then
        table.insert(lvlMods, mT.new.lvlMod(state.cache.modifiers[modId2], lvl2))
    end
    ...
    state.items[inventoryItem.item.id] = mT.new.itemData(inventoryItem.item, count, record.id, mMod.lvlModsToLvlModIds(lvlMods))
end
