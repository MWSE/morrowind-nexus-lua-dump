...
local itemTypeToSetting = {
    [T.Armor] = mStore.settings.doArmors,
    [T.Clothing] = mStore.settings.doClothing,
    [T.Weapon] = mStore.settings.doWeapons,
}
...
local function getItems(state, loot, ownerId)
    ...
    local function processItem(item)
        ...
        if not mT.itemTypes[item.type]
                or not itemTypeToSetting[item.type].get()
        ...
        if not get then
            if loot.type == T.Container then
                get = mStore.settings.doUnlevelledItems.get()
            else
    ...
    if addedValueSum > 0 then
        ...
        log(string.format("%s's (owner \"%s\") inventory wealth is %d", mObj.objectId(loot), ownerId, wealth))
    end
...
module.pickLevelledModifier = function(state, cacheTypes, affixTypes, itemType, ctx)
    local modLevel = mMod.getRandomModifierLevel(ctx.lootLevel)
...
local function getItemModifiers(state, item, ctx)
    ...
    if math.random() < mStore.settings.propsModifiersChance.get() / 100 then
    ...
    if math.random() > mStore.settings.secondModifierChance.get() / 100
            + ctx.chanceBoost * mStore.settings.secondModifierChanceBoostReduction.get() / 100 then
        return lvlMods
    end
...
local function convertItem(state, item, ctx)
    ...
    if item.count > 1 then
        if mT.itemTypes[item.type].convertWholeStackTypes[record.type] then
            newCount = math.ceil(item.count * mStore.settings.projectileStackReduction.get() / 100)
        else
            newCount = 1
            removeCount = 1
        end
    ...
    log(string.format("From item \"%s\", count %d, in loot %s level %.2f, generated item %s (count new %d remain %d), with key \"%s\", enchant \"%s\"",
            record.id, item.count, mObj.objectId(ctx.loot), ctx.lootLevel, newRecord.id, newCount, item.count - removeCount, key, newRecord.enchant), mT.logLevels.Debug)
...
module.restoreItems = function(state, filter, quiet)
    ...
    for actorId, itemsToEquip in pairs(equipped) do
        mWorld.equipItems(actors[actorId], itemsToEquip)
    end
...
local function setLootLevel(state, ctx)
    ...
    if ctx.loot.type == T.Container then
        ...
        ctx.lootLevel = mStats.getActorScaledLevel(state, mObj.getActorLevel(state.cache.exceptions.actorLevelOverrides, ctx.loot), ctx.loot.type == T.Creature)
        ctx.lootLevelChanceBonus = math.min(1, ctx.lootLevel / mStore.settings.endGameLootLevel.get()) ^ 2
        table.insert(reasons, string.format("actor \"%s\" (lvl %.2f)", ctx.loot.recordId, ctx.lootLevel))
    end

    if ctx.lootLevel > 0 then
        local scale = mStore.settings.playerLevelScaling.get() / 100
        if scale ~= 0 then
        ...
        if ctx.boosts.lock ~= 0 then
            ctx.lootLevel = ctx.lootLevel + ctx.boosts.lock * mStore.settings.maxLockLevelBoost.get()
            table.insert(reasons, string.format("lock boost %.2f", ctx.boosts.lock * mStore.settings.maxLockLevelBoost.get()))
        end
        if ctx.boosts.trap ~= 0 then
            ctx.lootLevel = ctx.lootLevel + ctx.boosts.trap * mStore.settings.maxTrapLevelBoost.get()
            table.insert(reasons, string.format("trap boost %.2f", ctx.boosts.trap * mStore.settings.maxTrapLevelBoost.get()))
        end
        if ctx.boosts.waterDepth ~= 0 then
            ctx.lootLevel = ctx.lootLevel + ctx.boosts.waterDepth * mStore.settings.maxWaterDepthLevelBoost.get()
            table.insert(reasons, string.format("water depth boost %.2f", ctx.boosts.waterDepth * mStore.settings.maxWaterDepthLevelBoost.get()))
        end
    end

    log(string.format("Loot %s difficulty level is %.2f because of { %s }",
            mObj.objectId(ctx.loot), ctx.lootLevel, table.concat(reasons, ", ")))
end
...
module.onContainerOpen = function(state, loot, activated)
    ...
    if not isValidLoot(state, loot, lootRecord, activated) then
        log(string.format("%s won't be processed anymore", mObj.objectId(loot)), mT.logLevels.Debug)
        if loot.type == T.Container then
    ...
    if not shouldProcessLoot(loot, activated) then
        log(string.format("%s should not be processed yet", mObj.objectId(loot)))
        return
    end
    if loot.type == T.Container then
        mT.setAccessed(state.containers, loot, mT.new.containerData)
    ...
    if ownerWealth then
        local maxWealthRatio = ownerId and mStore.settings.maxModsValueOverNpcWealth.get() or 0
        if maxWealthRatio ~= 0 then
            maxWealthValue = (ownerWealth.total / math.max(1, ownerWealth.npcCount)) * maxWealthRatio / 100
        end
        log(string.format("%s's inventory wealth is %d, max expense is %d", mObj.objectId(loot), ownerWealth.total, maxWealthValue))
    end
    ...
    local baseChance = mStore.settings.firstModifierChance.get() / 100
    local lootLevelChanceBoost = ctx.lootLevelChanceBonus * mStore.settings.maxLootLevelChanceBoost.get() / 100
    local lockChanceBoost, trapChanceBoost, waterDepthChanceBoost = 0, 0, 0, 0, 0
    if loot.type == T.Container then
        lockChanceBoost = ctx.boosts.lock * mStore.settings.maxLockChanceBoost.get() / 100
        trapChanceBoost = ctx.boosts.trap * mStore.settings.maxTrapChanceBoost.get() / 100
        waterDepthChanceBoost = ctx.boosts.waterDepth * mStore.settings.maxWaterDepthChanceBoost.get() / 100
    end
    ...
    log(string.format("First mod chance for loot %s is base %.3f + loot lvl %.3f (chance bonus %.3f) + lock %.3f + trap %.3f + water depth %.3f = %.3f",
            mObj.objectId(loot), baseChance, lootLevelChanceBoost, ctx.lootLevelChanceBonus, lockChanceBoost, trapChanceBoost, waterDepthChanceBoost, firstModChance), mT.logLevels.Debug)
    ...
    for _, itemData in ipairs(itemsData) do
        ...
        for _ = 1, (convertWholeStack and 1 or itemData.item.count) do
            ...
            if loot.type ~= T.Container and itemData.item.type == T.Weapon then
                minChance = mStore.settings.equippedWeaponMinChance.get() / 100
            end
            if math.random() < math.max(firstModChance, minChance) then
                ...
                firstModChance = firstModChance * mStore.settings.nextConversionInSameLootChance.get() / 100
                log(string.format("First mod chance for loot %s is reduced to %.2f for next items", mObj.objectId(loot), firstModChance), mT.logLevels.Debug)
            end
        end
    end
    if conversionCount > 0 then
        log(string.format("Converted %d items in loot %s", conversionCount, mObj.objectId(loot)))
        if ownerId then
            log(string.format("%s's (owner \"%s\") spent %d to convert items", mObj.objectId(loot), ownerId, state.npcsWealth[ownerId].spent))
        end
    end
    if #itemsToEquip ~= 0 then
        mWorld.equipItems(loot, itemsToEquip)
    end
