...
local function analyseNpc(state, npc)
    ...
    if not isAnalyzed then
        ...
        state.npcLevels[npc.recordId] = mObj.getActorLevel(state.cache.exceptions.actorLevelOverrides, npc)
    end
...
module.doCheckEquipment = function(state, actor)
    if mInterop.isActorEquipmentReady(actor) then
        mConvert.onContainerOpen(state, actor, false)
    end
end

local checkEquipmentCallback = async:registerTimerCallback(mDef.callbacks.checkEquipment, function(actor)
    -- send event to get the state
    core.sendGlobalEvent(mDef.events.checkEquipmentCallback, actor)
end)

local function checkEquipment(state, actor)
    if mStore.settings.doEquippedItems.get()
            and not mT.isEquipped(state.actors[actor.id]) then
        async:newSimulationTimer(0.5 + math.random() / 2, checkEquipmentCallback, actor)
    end
end
...
local function initContainer(state, container, ownerId)
    ...
    if ownerId then
        ...
        log(string.format("\"%s\" owns container %s with value %d", ownerId, mObj.objectId(container), wealth), mT.logLevels.Debug)
...
module.analyseContainers = function(state, containersData, actors, player)
    ...
    for _, containerData in ipairs(containersData) do
        ...
        if mT.isAnalyzed(state.containers[container.id])
            ...
            log(string.format("Container %s is excluded", mObj.objectId(container)), mT.logLevels.Debug)
        else
    ...
    for _, containerData in ipairs(validContainersData) do
        ...
        if containerStats
                -- Optimization: Don't analyse unprotected containers in exteriors (non-teleporting locked doors are unlikely)
                and (not player.cell.isExterior or #containerStats.actors > 0) then
