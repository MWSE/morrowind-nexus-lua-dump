...
local function assertLevels(mod, fieldName, values)
    assert(type(values) == "table", string.format("Levels field \"%s\" is not a table in mod %s", fieldName, mod.id))
    for i, level in ipairs(values) do
        assert(type(level) == "number" or level == false,
                string.format("Invalid value in levels field \"%s\" at index %d in mod %s", fieldName, i, mod.id))
    end
end
...
module.assertModFields = function(mod)
    ...
    if mod.levels then
        assertLevels(mod, "levels", mod.levels)
        hasLevels = true
    end
    if mod.effects then
        ...
        for _, effect in ipairs(mod.effects) do
            assert(effect.min and effect.max or not effect.min and not effect.max,
                    "\"min\" and \"max\" fields should be both present or both absent for mod " .. mod.id)
            for _, field in ipairs({ "min", "max", "area", "duration" }) do
                if effect[field] then
                    assertLevels(mod, field, effect[field])
                    hasLevels = true
                end
            end
            if effect.min or effect.max then
                assert(effect.min and effect.max, "Effects shall have min and max values for mod " .. mod.id)
                assert(#effect.min and #effect.max, "Min and max values shall have the same size for mod " .. mod.id)
                for i, value in ipairs(effect.min) do
                    if value == false or effect.max[i] == false then
                        assert(value == false and effect.max[i] == false, "Min and max values should be numbers or false for mod " .. mod.id)
                    else
                        assert(value <= effect.max[i], "Min value shall be lower than max value for mod " .. mod.id)
                    end
                end
            end
        end
    end
    if mod.modifiers then
        assertLevels(mod, "modifiers", mod.modifiers)
        hasLevels = true
    end
    if mod.multipliers then
        assertLevels(mod, "multipliers", mod.multipliers)
        hasLevels = true
    end
    assert(hasLevels, "No modifier level data found for mod " .. mod.id)
...
local function buildModifierMap(state, modifiers)
    ...
    if #missingLocales > 0 then
        ...
        print(string.format("Missing localizations for mods: %s", table.concat(missingLocales, ", ")))
    end
...
local function hasExcludedEffects(mod, ctx)
    ...
    if ctx.loot.type ~= T.Container and mod.castType == core.magic.ENCHANTMENT_TYPE.ConstantEffect then
        for _, effect in ipairs(mod.effects) do
    ...
    for _, effect in ipairs(mod.effects) do
        if mT.contextualModRestrictions[effect.id] and not mT.contextualModRestrictions[effect.id](mod, effect, ctx.item, ctx.loot) then
            return true
        end
    end
    return false
...
local function areModConflicting(mod1, mod2)
    if mod1.id == mod2.id or mod1.affixType == mod2.affixType then return true end
    if mod1.castType and mod2.castType and mod1.castType ~= mod2.castType then return true end
    if mod1.effects and mod2.effects then
        for _, effect1 in ipairs(mod1.effects) do
            for _, effect2 in ipairs(mod2.effects) do
...
local function filterMods(state, modLevel, ctx)
    return function(id)
        ...
        if not module.hasModifierLevel(mod, modLevel) or hasExcludedEffects(mod, ctx) then
            return false
        end
        ...
        for _, lvlMod in ipairs(ctx.item.lvlMods) do
            if areModConflicting(mod, lvlMod.mod) then
                return false
            end
        end
        return true
...
module.getRandomModifierLevel = function(lootLevel)
    local probs = {}
    local probSum = 0
    local level = math.min(lootLevel, mStore.settings.endGameLootLevel.get())
            / (mStore.settings.endGameLootLevel.get() / (mCfg.modifierLevel.maxLevel - 1)) + 1
