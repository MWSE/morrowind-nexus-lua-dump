...
local exceptions = require("scripts.fresh-loot.exceptions.main")
...
local function getLootKeeperStats(actor, container)
    ...
    return mT.new.lootKeeperStats(
            mObj.getActorLevel(exceptions.actorLevelOverrides, actor),
            actor.recordId,
...
local function addLootsLocalStats(lootsGlobalStats)
    ...
    for _, lootGlobalStats in ipairs(lootsGlobalStats) do
        ...
        local doorsBoosts
        if lootGlobalStats.toBeAnalyzed and not self.cell.isExterior then
...
local function uiModeChanged(data)
    ...
    if mUi.itemWindowExcludedUiModes[data.newMode] then
        mUi.toggleItemWindow()
    end
end

local function onInit()
    mSettings.updateAllSettingsDependencies()
    mStore.addTrackerCallback(function(key, oldValue)
        if mStore.settings[key].get() == oldValue then return end
        mSettings.updateAllSettingsDependencies()
    end)
