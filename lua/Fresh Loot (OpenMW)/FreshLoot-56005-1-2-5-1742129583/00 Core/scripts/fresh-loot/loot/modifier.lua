...
local function filterMods(state, modLevel, ctx)
    return function(id)
        ...
        if ctx.ownerId then
            ...
            if wealth.spent + getModPrice(ctx.item.record, ctx.item.type, mod, modLevel)
...
local function getRandomModifierLevel(state, lootLevel)
    local probs = {}
    local probSum = 0
    local level = lootLevel / state.settings[mStore.cfg.modifierLevelDifficultyRange.key] + 1
...
local function haveModTypeCommonSubType(type, filter1, filter2)
    ...
    elseif not filter2.types and not filter2.subTypes then
        return true
    end
    return false
end
...
local function areModsCompatible(mod1, mod2)
    ...
    for type, filter1 in pairs(mod1.itemTypes) do
        ...
        if filter2 then
            if filter1 == true and filter2 == true then
                return true
            elseif filter1 == true or filter2 == true then
                return true
