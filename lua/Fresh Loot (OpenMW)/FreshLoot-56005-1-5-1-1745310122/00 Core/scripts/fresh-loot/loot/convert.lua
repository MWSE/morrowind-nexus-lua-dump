...
local function onContainerOpen(state, loot, equipmentOnly)
    ...
    if loot.type == T.Container then
        local boosts = {}
        table.insert(boosts, state.learned.containers.boosts[loot.id])
        local accessDoors = state.learned.containers.accessDoors[loot.id]
        if accessDoors then
            for doorId in pairs(accessDoors) do
                table.insert(boosts, state.learned.doorBoosts[doorId])
            end
        end
        if #boosts > 0 then
