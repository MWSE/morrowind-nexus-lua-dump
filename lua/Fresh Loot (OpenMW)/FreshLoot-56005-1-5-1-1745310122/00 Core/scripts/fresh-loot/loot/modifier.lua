...
local function filterMods(state, modLevel, ctx)
    return function(id)
        ...
        if ctx.ownerId then
            ...
            local maxWealthRatio = state.settings[mStore.cfg.maxModsValueOverActorsWealth.key]
            if maxWealthRatio ~= 0
                    and wealth.spent + getModPrice(ctx.item.record, ctx.item.type, mod, modLevel) > wealth.total * maxWealthRatio / 100 then
...
local function getRandomModifierLevel(state, lootLevel)
    local probs = {}
    local probSum = 0
    local level = lootLevel / (state.settings[mStore.cfg.endGameLootLevel.key] / (mCfg.modifierLevel.maxLevel - 1)) + 1
