...
local function getDoorStats(actor, container, allDoorStats)
    local doorStats = {}
    -- is there a path without doors to the container?
    if getPath(actor, container, 0, nearby.NAVIGATOR_FLAGS.Walk + nearby.NAVIGATOR_FLAGS.Swim) then
        return doorStats
    end
    -- is there a path to the container, or close enough, ignoring doors
    local path = getPath(actor, container, mCfg.lootLevel.maxKeepersClosestDistance)
    if not path then
        return doorStats
    end
...
module.getDoorStats = getDoorStats
