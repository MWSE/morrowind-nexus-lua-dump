...
module.onPBArmorEquipped = function(state, actor, items)
    log(string.format("Protected Beasts: Actor \"%s\" equipped", actor.recordId), mTypes.logLevels.Debug)
    ...
    for _, item in ipairs(items) do
        ...
        state.items[item.newItem.id] = mTypes.new.itemData(item.newItem, itemData.oldCount, item.pbRecordId, itemData.lvlModIds)
