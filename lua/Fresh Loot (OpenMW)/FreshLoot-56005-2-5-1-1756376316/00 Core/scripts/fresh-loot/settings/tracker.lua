...
local function updateSettingsRequirements(state)
    local update = true
    while update do
        update = updateSettingRequirement(state)
    end
end
...
module.trackSettings = function(getState)
    for _, section in pairs(mStore.section) do
        section.get():subscribe(async:callback(function(_, key) trackSettingChanges(getState(), key) end))
    end
end
