...
local function convertItem(state, item, ctx)
    ...
    log(string.format("From item \"%s\", count %d, in loot \"%s\" level %.2f, generated item %s (count new %d remain %d), with key \"%s\", enchant \"%s\"",
            record.id, item.count, ctx.loot.recordId, ctx.lootLevel, newRecord.id, newCount, item.count - removeCount, key, newRecord.enchant), mTypes.logLevels.Debug)

    local inventoryItem = mWorld.replaceItem(ctx.loot, item, record, removeCount, newRecord, newCount)
    state.items[inventoryItem.item.id] = mTypes.new.itemData(inventoryItem.item, removeCount, record.id, mMod.lvlModsToLvlModIds(lvlMods))
...
local function onContainerOpen(state, loot, activated)
    ...
    if not isValidLoot(state, loot, lootRecord, activated) then
        log(string.format("\"%s\" won't be processed anymore", loot.recordId), mTypes.logLevels.Debug)
        if loot.type == T.Container then
    ...
    local lootLevelChanceBoost = math.min(1, (ctx.lootLevel / state.settings[mStore.cfg.endGameLootLevel.key]) ^ 2)
            * state.settings[mStore.cfg.maxLootLevelChanceBoost.key] / 100
    local crowdChanceBoost, lockChanceBoost, trapChanceBoost, waterDepthChanceBoost = 0, 0, 0, 0, 0
    if loot.type == T.Container then
    ...
    log(string.format("First mod chance for loot \"%s\" is base %.2f + crowd %.2f + loot lvl %.2f + lock %.2f + trap %.2f + water depth %.2f = %.2f",
            loot.recordId, baseChance, crowdChanceBoost, lootLevelChanceBoost, lockChanceBoost, trapChanceBoost, waterDepthChanceBoost, firstModChance), mTypes.logLevels.Debug)
    ...
    for _, itemData in ipairs(itemsData) do
        local convertWholeStack = itemData.item.count > 0 and mTypes.itemTypes[itemData.item.type].convertWholeStackTypes[itemData.record.type]
        for _ = 1, (convertWholeStack and 1 or itemData.item.count) do
            ...
            if math.random() < math.max(firstModChance, minChance) then
                ...
                log(string.format("First mod chance for loot \"%s\" is reduced to %.2f for next items", loot.recordId, firstModChance), mTypes.logLevels.Debug)
            end
        end
    end
