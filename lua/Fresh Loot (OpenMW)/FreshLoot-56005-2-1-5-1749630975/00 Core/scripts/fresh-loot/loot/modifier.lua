...
local function filterMods(state, modLevel, ctx)
    return function(id)
        ...
        if ctx.ownerWealth then
            local maxWealthRatio = state.settings[ctx.loot.type == T.NPC and mStore.cfg.maxModsValueOverNpcWealth.key or mStore.cfg.maxModsValueOverCreatureWealth.key]
            if maxWealthRatio ~= 0 then
                local maxWealthValue = (ctx.ownerWealth.total / math.max(1, ctx.ownerWealth.npcCount)) * maxWealthRatio / 100
                if ctx.ownerWealth.spent + getModPrice(ctx.item.record, ctx.item.type, mod, modLevel) > maxWealthValue then
