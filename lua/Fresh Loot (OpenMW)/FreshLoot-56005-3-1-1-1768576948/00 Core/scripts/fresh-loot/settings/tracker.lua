...
local function trackSettingChanges(state, key)
    ...
    if newValue == nil then
        local preset = mStore.presets[state.settings[mStore.cfg.chancePresets.key]][key]
        if preset ~= nil then
            core.sendGlobalEvent(mDef.events.setSetting, { key = key, value = preset })
            return
        end
    end
    if newValue == state.settings[key] then
        return
    end
...
module.loadSettings = function(state)
    ...
    if type(state.settings[mStore.cfg.itemsWindowKeyController.key]) == "number" then
