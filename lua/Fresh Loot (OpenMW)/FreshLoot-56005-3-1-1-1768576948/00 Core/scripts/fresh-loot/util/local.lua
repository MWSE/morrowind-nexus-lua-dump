...
module.getPath = function(actor, object, tolerance, includeFlags)
...
module.getDoorsBoosts = function(actor, container, allDoorStats)
    ...
    if module.getPath(actor, container, 0, nearby.NAVIGATOR_FLAGS.Walk + nearby.NAVIGATOR_FLAGS.Swim + nearby.NAVIGATOR_FLAGS.UsePathgrid) then
        return doorsBoosts
    end
    -- is there a path to the container, or close enough, ignoring doors
    local path = module.getPath(actor, container, mCfg.lootLevel.maxKeepersReachLootDistance)
    if not path then
        return doorsBoosts
    end
