...
local function getActiveCells()
    ...
    for _, player in ipairs(world.players) do
        if not cellStats[player.cell.id] then
            cellStats[player.cell.id] = mTypes.new.cellStat(player.cell, 1)
        else
            cellStats[player.cell.id].playerCount = cellStats[player.cell.id].playerCount + 1
        end
    end
    return cellStats
end
...
local function sendPlayerEvent(cell, event, data)
    local players = cell:getAll(T.Player)
    assert(#players > 0, string.format("No player found in cell \"%s\"", cell.id))
    players[1]:sendEvent(event, data or players[1])
end
module.sendPlayerEvent = sendPlayerEvent
...
local function replaceItem(container, item, record, count, newRecord, newCount)
    local slot
    if container and container.type.baseType == T.Actor then
    ...
    if condition then
        ...
        T.Item.itemData(newItem).condition = newRecord.health * conditionRatio
    end
    if container then
