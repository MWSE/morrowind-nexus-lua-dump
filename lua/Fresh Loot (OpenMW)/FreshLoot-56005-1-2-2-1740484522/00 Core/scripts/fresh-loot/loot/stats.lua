...
local function getActorFinalLevel(state, level, isCreature)
    if isCreature then
        level = level * (1 + state.settings[mStore.cfg.creatureLevelBoost.key] / 100)
    end
    return level
end
...
local function getKeeperLevel(state, stats, statsOverride)
    ...
    if statsOverride then
        mHelpers.addAllToMap(stats, statsOverride, function(_, v) return v ~= nil end)
    end
...
local function getLootLevel(state, containerStats)
    ...
    for _, levelStats in pairs(containerStats.levelStats) do
        local baseKeeperLevel, keeperLevel, reason = getKeeperLevel(state, levelStats, containerStats.levelStatsOverride)
    ...
    local maxLootLevel = 0
    local maxLootLevelIndex = 0
    local keeperRelativeLevels = {}
    for i, level in ipairs(keepersLevel) do
        local keeperRelativeLevel = (level / maxKeeperLevel) ^ 0.5
        table.insert(keeperRelativeLevels, keeperRelativeLevel)
        if keeperRelativeLevel > maxLootLevel then
            maxLootLevel = keeperRelativeLevel
            maxLootLevelIndex = i
        end
    end
    local lootLevel = maxKeeperLevel
    for i, level in ipairs(keeperRelativeLevels) do
        if i ~= maxLootLevelIndex then
            lootLevel = lootLevel + level
        end
    end
    return math.min(maxBaseKeeperLevel, math.max(lootLevel, 1)), reasons
end
