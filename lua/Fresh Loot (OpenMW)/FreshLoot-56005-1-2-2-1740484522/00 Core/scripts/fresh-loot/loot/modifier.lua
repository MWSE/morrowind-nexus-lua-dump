...
local function addModIdsFromCache(list, cache, affixType, filter)
    if affixType == mTypes.affixTypes.Any then
        mHelpers.addAllToArray(list, cache[mTypes.affixTypes.Prefix], filter)
        mHelpers.addAllToArray(list, cache[mTypes.affixTypes.Suffix], filter)
    else
        mHelpers.addAllToArray(list, cache[affixType], filter)
    end
end

local function sortLvlMods(lvlMods)
    table.sort(lvlMods, function(a, b)
        return a.mod.affixType ~= b.mod.affixType and a.mod.affixType == mTypes.affixTypes.Prefix or a.mod.id < b.mod.id
    end)
end
...
local function getModPrice(itemValue, mod, level)
    if mod.multipliers then
        return (mod.value + itemValue) * mCfg.itemConversion.propsLevelMult.priceForRelativeMod[level]
    else
        return mod.value * mCfg.itemConversion.propsLevelMult.priceForAbsoluteMod[level]
    end
end
    ...
    return function(id)
        ...
        if mod.castType then
            for _, lvlMod in ipairs(ctx.item.lvlMods) do
                if lvlMod.mod.castType and mod.castType ~= lvlMod.mod.castType then
                    return false
                end
            end
        end
        if ctx.ownerId then
            local wealth = state.learned.npcsWealth[ctx.ownerId]
            if not wealth then return false end
            if wealth.spent + getModPrice(ctx.item.record.value, mod, modLevel)
                    > wealth.total * state.settings[mStore.cfg.maxModsValueOverActorsWealth.key] / 100 then
...
local function getKey(lvlMods)
    ...
    for _, lvlMod in ipairs(lvlMods) do
        if lvlMod.mod.affixType == mTypes.affixTypes.Prefix then
            table.insert(keyElements, 1, lvlMod.lvl)
            table.insert(keyElements, 1, lvlMod.mod.id)
        else
...
local function getEnchantId(lvlMods)
    ...
    for _, lvlMod in ipairs(lvlMods) do
        if lvlMod.mod.effects then
            if lvlMod.mod.affixType == mTypes.affixTypes.Prefix then
                table.insert(enchantIdElements, 1, lvlMod.lvl)
                table.insert(enchantIdElements, 1, lvlMod.mod.id)
            else
...
local function getRandomModifierLevel(state, itemLevel)
    local probs = {}
    local probSum = 0
    local level = itemLevel / state.settings[mStore.cfg.modifierLevelDifficultyRange.key] + 1

    for modLevel = 1, mCfg.modifierLevel.maxLevel do
        local difference = math.abs(modLevel - level)
        local probability = math.exp(-mCfg.modifierLevel.pickAlpha * difference) * math.exp(-mCfg.modifierLevel.pickBeta * modLevel)
...
local function haveModTypeCommonSubType(type, props1, props2)
    if props1.types then
        if props2.types then
            for subType in pairs(props1.types) do
                if props2.types[subType] then return true end
            end
        elseif props2.notTypes then
            for subType in pairs(props1.types) do
                if not props2.notTypes[subType] then return true end
            end
        end
    elseif props1.notTypes then
        if props2.types then
            for subType in pairs(props2.types) do
                if not props1.notTypes[subType] then return true end
            end
        elseif props2.notTypes then
            for subType in pairs(type.TYPE) do
                if not props1.notTypes[subType] and not props2.notTypes[subType] then return true end
            end
        end
    elseif not props2.types and not props2.subTypes then
        return true
    end
    return false
end
...
local function areModsCompatible(mod1, mod2)
    if mod1.affixType == mod2.affixType
            or (mod1.castType and mod2.castType and mod1.castType ~= mod2.castType) then
        return false
    end
    for type, props1 in pairs(mod1.itemTypes) do
        local props2 = mod2.itemTypes[type]
        if props2 then
            if props1 == true and props2 == true then
                return true
            elseif props1 == true or props2 == true then
                return true
            elseif haveModTypeCommonSubType(type, props1, props2) then
                if props1.classes and props2.classes then
                    for class in pairs(props1.classes) do
                        if props2.classes[class] then
                            return true
                        end
                    end
                else
...
local function applyLvlModsOnItem(lvlMods, type, oldRecord, newRecord)
    ...
    for _, lvlMod in ipairs(lvlMods) do
        ...
        priceAdd = priceAdd + getModPrice(oldRecord.value, mod, level)
