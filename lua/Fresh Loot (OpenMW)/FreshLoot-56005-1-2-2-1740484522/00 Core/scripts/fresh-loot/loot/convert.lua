...
local function getItems(state, loot, equippedOnly, ownerId)
    ...
    for type in pairs(mTypes.itemTypes) do
        for _, item in ipairs(inventory:getAll(type)) do
            ...
            if mObj.areValidItemProps(item, record) then
                if not isResolved and not equippedOnly and not unlevelledItems[item.id] then
                    if not state.cache.validItemIds[record.id] then
                ...
                if state.cache.validItemIds[record.id] then
                    local get = not unlevelledItems[item.id]
                    if not get then
                        if loot.type.baseType ~= T.Actor then
                    ...
                    if get then
                        local itemData = mTypes.new.itemData(item, record, not unlevelledItems[item.id])
                        if type == T.Weapon then
                            if equippedOnly then
                                -- Prefer equipped weapon conversion over other types
                                lastWeaponIndex = lastWeaponIndex + 1
                                table.insert(itemsData, lastWeaponIndex, itemData)
                            else
                                table.insert(itemsData, math.random(lastWeaponIndex + 1, #itemsData + 1), itemData)
                            end
                        end
...
local function pickLevelledModifier(state, cacheTypes, affixTypes, itemType, ctx)
    ...
    for level = modLevel, 1, -1 do
        ...
        if #ids ~= 0 then
            local lvlMod = mTypes.new.lvlMod(state.cache.modifiers[ids[math.random(1, #ids)]], modLevel)
            if ctx.ownerId then
                state.learned.npcsWealth[ctx.ownerId].spent = state.learned.npcsWealth[ctx.ownerId].spent + mMod.getModPrice(ctx.item.record.value, lvlMod.mod, lvlMod.lvl)
            end
            return lvlMod
        end
    end
...
local function getItemModifiers(state, item, ctx)
    ...
    if lvlMod.mod.effects then
        cacheTypes = { state.cache.modifierTypes.props, state.cache.modifierTypes.effects }
    else
...
local function convertItem(state, item, ctx)
    ...
    state.processed.items[inventoryItem.item.id] = mTypes.new.convertedItem(
            inventoryItem.item, count, record.id, ctx.equipmentOnly, mMod.lvlModsToLvlModIds(lvlMods))
    return inventoryItem
end

local function shouldRestoreItem(convertedItem, filter)
    ...
    if convertedItem.equipped
            and filter.types[mTypes.itemRestoreTypes.Equipped]
            and parent
            and parent.type.baseType == T.Actor
            and not T.Actor.isDead(parent) then return true end
    return false
end
...
local function restoreItems(state, filter, quiet)
    ...
    for _, convertedItem in pairs(state.processed.items) do
        if shouldRestoreItem(convertedItem, filter) then
            local item = convertedItem.item
            local parent = item.parentContainer
            local inventoryItem = mWorld.replaceItem(
                    parent, item, mObj.getRecord(item), item.count, item.type.records[convertedItem.oldRecordId], convertedItem.oldCount)
            ...
            if convertedItem.equipped then
                state.processed.equipments[parent.id] = nil
            ...
            if parent then
                local ownerId
                if parent.type.baseType == T.Actor then
...
local function setLootLevel(state, ctx)
    ...
    if ctx.loot.type == T.Container then
        ...
        if stats then
            ...
            local levelReasons
            ctx.lootLevel, levelReasons = mStats.getLootLevel(state, stats)
    ...
    log(string.format("Loot \"%s\" difficulty level is %.2f because of%s { %s }",
            ctx.loot.recordId, ctx.lootLevel, overrideSource, table.concat(reasons, ", ")))
end
...
local function onContainerOpen(state, loot, equipmentOnly)
    ...
    local lootLevelChanceBoost = (ctx.lootLevel * state.settings[mStore.cfg.lootLevelChanceBoost.key] / 100) / 100
    local lockChanceBoost = ctx.boosts.lock * state.settings[mStore.cfg.maxLockChanceBoost.key] / 100
    local trapChanceBoost = ctx.boosts.trap * state.settings[mStore.cfg.maxTrapChanceBoost.key] / 100
    local waterDepthChanceBoost = ctx.boosts.waterDepth * state.settings[mStore.cfg.maxWaterDepthChanceBoost.key] / 100
    ...
    log(string.format("First mod chance for loot \"%s\" is base %.2f + loot lvl %.2f + lock %.2f + trap %.2f + water depth %.2f = %.2f",
            ctx.loot.recordId, baseChance, lootLevelChanceBoost, lockChanceBoost, trapChanceBoost, waterDepthChanceBoost, firstModChance))

    local itemsToEquip = {}
    for _, itemData in ipairs(itemsData) do
        if itemData.item.count > 0 and math.random() < firstModChance then
            ctx.item = mTypes.new.itemContext(itemData.record)
            local inventoryItem = convertItem(state, itemData.item, ctx)
            if inventoryItem then
                firstModChance = firstModChance * state.settings[mStore.cfg.multiConvertsChanceReduction.key] / 100
                log(string.format("First mod chance for loot \"%s\" reduced to %.2f", ctx.loot.recordId, firstModChance))
                if inventoryItem.slot then
