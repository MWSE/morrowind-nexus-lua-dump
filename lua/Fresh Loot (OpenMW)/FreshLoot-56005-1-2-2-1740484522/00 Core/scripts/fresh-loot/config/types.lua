...
module.itemRestoreTypes = { All = 0, Equipped = 1 }
...
module.new = {
    ...
    requestEvent = function(data, object, name)
        return { data = data, object = object, name = name }
    end,
    cache = function()
    ...
    containerLevelSource = function(ownerRecordId, factionId, factionRank)
    ...
    convertedItem = function(item, oldCount, oldRecordId, equipped, lvlModIds)
        return { item = item, oldCount = oldCount, oldRecordId = oldRecordId, equipped = equipped, lvlModIds = lvlModIds }
    end,
    inventoryItem = function(item, slot, valueDiff)
        return { item = item, slot = slot, valueDiff = valueDiff }
    end,
    itemRestoreFilter = function(restoreTypes, onlyCells, notCells)
        return { types = restoreTypes, onlyCells = onlyCells, notCells = notCells }
    end,
