...
if (mwse.buildDate == nil) or (mwse.buildDate < 20190501) then
    event.register("initialized", function()
        tes3.messageBox(
            "[Graphic Herbalism] Ваш MWSE устарел!"
            .. " Вам нужно будет обновиться до более поздней версии, чтобы использовать мод!"
        )
    end)
    return
end
...
mwse.log("[Graphic Herbalism] Загруженная версия 1.03")
...
if (quickloot.version == nil) or (quickloot.version < 1.3) then
    ...
    if mode == "file" then
        event.register("initialized", function()
            tes3.messageBox(
                "[Graphic Herbalism] Ваш QuickLoot устарел!"
                .. " Вам нужна версия 1.3 или выше, чтобы использовать этот мод!"
            )
        end)
    end
end
...
local function onTooltipDrawn(e)
    ...
    for ingred in getIngredients(ref.object.inventory) do
        ...
        local label = parent:createLabel{id=GUI_ID.weight, text=string.format("Вес: %.2f", ingred.weight)}
        label.wrapText = true

        local label = parent:createLabel{id=GUI_ID.value, text=string.format("Цена: %d", ingred.value)}
        label.wrapText = true
...
local function updateBlacklist()
    for obj in tes3.iterateObjects(tes3.objectType.container) do
        ...
        then
            ...
            then
                mwse.log('[Graphic Herbalism] Контейнер "%s" добавлен в черный список', id)
