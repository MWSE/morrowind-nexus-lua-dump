...
local limbs = {
    ...
    ["legs"] = {tes3.activeBodyPart.rightUpperLeg, tes3.activeBodyPart.rightKnee, tes3.activeBodyPart.rightAnkle, tes3.activeBodyPart.rightFoot,
                tes3.activeBodyPart.leftUpperLeg,  tes3.activeBodyPart.leftKnee,  tes3.activeBodyPart.leftAnkle,  tes3.activeBodyPart.leftFoot}
}
...
local severedLimbs = {
    ...
    ["legs"] = {tes3.activeBodyPart.rightKnee, tes3.activeBodyPart.rightAnkle, tes3.activeBodyPart.rightFoot,
                tes3.activeBodyPart.leftKnee,  tes3.activeBodyPart.leftAnkle,  tes3.activeBodyPart.leftFoot}
}
...
local function placeGore(closestActivePartIndex, ref)
    ...
    elseif table.find(limbs.legs, closestActivePartIndex) then
        ...
        bip01 = sceneNode:getObjectByName("Bip01 L Thigh")
        gore = mesh:getObjectByName("Tri GoreCap:LeftLeg")
        if not bip01 or not gore then return end
        bip01:attachChild(gore)
        bip01:attachChild(bloodMesh)
...
local function dismemberNPC(e)
    ...
    if e.attacker.objectType == tes3.objectType.npc or e.attacker.objectType == tes3.objectType.creature then
        local idx = table.choice({"head", "rightArm", "leaftArm", "legs"})
...
event.register("modConfigReady", function() mwse.mcm.register(require("cz.Dismember.mcm")) end)