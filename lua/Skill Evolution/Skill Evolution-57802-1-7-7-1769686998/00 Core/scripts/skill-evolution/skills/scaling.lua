...
local function useTimeConsumingSkill(skillId, hours)
    mDecay.addPassedTimePerSkill(skillId, hours)
    core.sendGlobalEvent(mDef.events.skipGameHours, { hours = hours, event = { object = self, name = mDef.events.updateStats } })
end
    ...
    if goldDiff < 0 then
        if currentUiMode == "MerchantRepair" then
            ...
            if minutes > 0 then
                ...
                core.sendGlobalEvent(mDef.events.skipGameHours, { hours = minutes / 60, event = { object = self, name = mDef.events.updateStats } })
            end
