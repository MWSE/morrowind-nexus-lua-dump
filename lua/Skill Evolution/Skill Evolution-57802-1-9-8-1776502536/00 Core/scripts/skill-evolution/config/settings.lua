...
local timeSettingsKey = "SettingsPlayerTime" .. mDef.MOD_NAME
...
local settingGroups = {
    ...
    [timeSettingsKey] = {
        ...
        settings = {
            {
                key = "minutesPerPotionCreation",
                default = { 90, 30, true },
                renderer = mDef.renderers.range,
                argument = { min = 0, max = 1000, desc = true, togglable = true },
            },
            {
                key = "potionMaxRecipeCountBasedTimeReduction",
                default = 50,
                renderer = mDef.renderers.number,
                argument = { min = 0, max = 100, integer = true, percent = true },
            },
            {
                key = "minutesPerSelfRepair",
                default = { 15, 5, true },
                renderer = mDef.renderers.range,
                argument = { min = 0, max = 1000, desc = true, togglable = true },
            },
            {
                key = "minutesPerNPCRepair",
                default = 30,
                renderer = mDef.renderers.number,
                argument = { min = 0, max = 1000, integer = true },
            },
            {
                key = "minutesPerSelfEnchanting",
                default = { 90, 30, true },
                renderer = mDef.renderers.range,
                argument = { min = 0, max = 1000, desc = true, togglable = true },
            },
            {
                key = "minutesPerNPCEnchanting",
                default = 60,
                renderer = mDef.renderers.number,
                argument = { min = 0, max = 1000, integer = true },
            },
    ...
    [skillUsesScaledSettingsKey] = {
        ...
        settings = {
            ...
            },
            {
                ...
                default = { mCfg.minScaledSkillGainPercent, 300, true },
                renderer = mDef.renderers.range,
            ...
            },
            {
                ...
                default = { mCfg.minScaledSkillGainPercent, 350, true },
                renderer = mDef.renderers.range,
...
module.init = function()
    for _, skill in ipairs(core.stats.Skill.records) do
        for useType, useTypeCfg in mH.spairs(mCfg.skillUseTypes[skill.id], function(_, a, b) return a < b end) do
            useTypeCfg.settingKey = string.format("skillUse-%s-%s", skill.id, useTypeCfg.key)
            local moddedGain = skill.skillGain[useType + 1]
            local isZero = false
            if moddedGain == 0 then
                isZero = true
            elseif mH.areFloatEqual(moddedGain, mCfg.skillZeroGainHack) then
                moddedGain = 0
            end
            table.insert(settingGroups[skillUsesGainsSettingsKey].settings, {
                key = useTypeCfg.settingKey,
                name = L("skillUses_name", { skill = skill.name, useType = module.getSkillUseTypeName(useTypeCfg.key) }),
                description = isZero
                        and L("skillUsesModdedZero_desc")
                        or (
                        mH.areFloatEqual(moddedGain, useTypeCfg.vanilla)
                                and L("skillUsesVanilla_desc", { vanilla = useTypeCfg.vanilla })
                                or L("skillUsesModded_desc", { vanilla = useTypeCfg.vanilla, modded = moddedGain })),
                default = useTypeCfg.gain,
                renderer = mDef.renderers.number,
                argument = { min = 0, max = 100, disabled = isZero },
            })
        end
    end
    ...
    module.timeStorage = getStorage(timeSettingsKey)
    ...
    I.Settings.registerGroup(settingGroups[timeSettingsKey])
