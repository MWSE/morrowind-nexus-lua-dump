...
local function refundMagicka(skillId, spell, ratio)
    local refund = mSpells.getSpellCost(spell) * ratio * (mS.magickaStorage:get("refundMult") / 5)
...
local function skillUsedHandlerUses(_, skillId, params)
    if params.useType and params.skillGain ~= 0 then
        local useTypeCfg = mCfg.skillUseTypes[skillId][params.useType]
        if useTypeCfg then
            local gainCustom = mS.skillUsesGainsStorage:get(useTypeCfg.settingKey)
            if not mH.areFloatEqual(gainCustom, useTypeCfg.vanilla) then
                log(string.format("Skill used handler: Custom base gain for skill \"%s\" is %.2f (instead of input %.2f and vanilla %.2f)",
                        skillId, gainCustom, params.skillGain, useTypeCfg.vanilla))
                params.skillGain = gainCustom
            elseif mH.areFloatEqual(params.skillGain, mCfg.skillZeroGainHack) then
                params.skillGain = 0
            end
        end
    end
...
local function skillUsedHandlerScale(_, skillId, params)
    ...
    if scale and scale <= 0 then
        print(string.format("Scale handler: Provided scale is %d for skill \"%s\", this should never happen, stopping the handler chain", scale, skillId))
        return false
    end
