...
local function getTimePassed(setting, skill)
    return setting[2] + (setting[1] - setting[2]) * (1 - math.min(mCfg.maxSkillForPassedTime, skill) / 100)
end
...
local function addPotionTime(potionData, record, recipeCount)
    ...
    local skill = T.NPC.stats.skills.alchemy(self).modified
    local timeReduction = mS.timeStorage:get("potionMaxRecipeCountBasedTimeReduction") / 100
    local minutes = 0
    ...
    log(string.format("Potion Time: Alchemy level %d, potion %s \"%s\", count %d, recipe count %d, duration %.1f minutes",
            skill, record.id, record.name, potionData.count, recipeCount, minutes))
    useTimeConsumingSkill(Skills.alchemy.id, minutes / 60)
end
...
local function addRepairTime()
    ...
    useTimeConsumingSkill(Skills.armorer.id, minutes / 60)
end
...
local function addEnchantTime()
    ...
    useTimeConsumingSkill(Skills.enchant.id, minutes / 60)
end
...
module.skipBlockScaling = function(state)
    ...
    setDelayedScaling(state, Skills.block.id, 1, state.scaled.block.handlerParams, false, message)
...
local function capSkillScale(skillId, scale)
    if mCore.magickaSkills[skillId] then
        return capSkillScaledFactor(scale, "magickaBasedSkillScaling")
    elseif mCore.weaponSkills[skillId] then
        return capSkillScaledFactor(scale, "weaponSkillScaling")
    elseif mCore.armorSkills[skillId] then
        return capSkillScaledFactor(scale, "armorSkillScaling")
    elseif skillId == Skills.block.id then
        return capSkillScaledFactor(scale, "blockSkillScaling")
    elseif skillId == Skills.security.id then
        return capSkillScaledFactor(scale, "securitySkillScaling")
    elseif skillId == Skills.acrobatics.id then
        return capSkillScaledFactor(scale, "acrobaticsSkillScaling")
    elseif skillId == Skills.athletics.id then
        return capSkillScaledFactor(scale, "athleticsSkillScaling")
    elseif skillId == Skills.alchemy.id then
        return capSkillScaledFactor(scale, "alchemySkillScaling")
    end
    return scale
end

module.skillUsedHandler = function(state, skillId, params)
    if not params.useType or not mCfg.skillUseTypes[skillId][params.useType] then return end
...
module.onActorHit = function(state, enemy)
    ...
    local factor = capSkillScaledFactor(2 * (1 - (math.max(0, chance) / 1.5) ^ 0.8), "weaponSkillScaling")
    local featProps = {
