...
local function updateSkills(params)
    ...
    for skillId, getter in pairs(T.NPC.stats.skills) do
        ...
        local actualBase = getter(self).base - (baseSkillMods[skillId] or 0)
...
local function uiModeChanged(data)
    ...
    if data.arg
        ...
        if range[1] ~= 2 or range[2] ~= 2 then
...
local function upgradeOldState(oldState)
    ...
    if oldState.savedGameVersion < 1.6 then
        local setting = mS.skillUsesScaledStorage:getCopy("magickaBasedSkillScaling")
        setting[2] = 300
        mS.skillUsesScaledStorage:set("magickaBasedSkillScaling", setting)
    end
    if oldState.savedGameVersion < 1.62 then
        for _, key in ipairs({ "weaponSkillScaling", "securitySkillScaling", "athleticsSkillScaling" }) do
            local setting = mS.skillUsesScaledStorage:getCopy(key)
            if setting[2] > mCfg.maxScaledSkillGainPercent then
                setting[2] = mCfg.maxScaledSkillGainPercent
                mS.skillUsesScaledStorage:set(key, setting)
            end
        end
        local setting = mS.skillUsesScaledStorage:getCopy("alchemySkillScaling")
        setting[2] = 350
        mS.skillUsesScaledStorage:set("alchemySkillScaling", setting)
    end
        ...
        if blockFeats then
            for _, feats in pairs(blockFeats.lists) do
                for _, feat in ipairs(feats) do
                    feat.props.hitChance = feat.props.hitChance * 100
                end
            end
        end
    end
    ...
    if oldState.savedGameVersion < 1.9 then
        local setting = mS.skillsStorage:getCopy("scaledTrainingDuration")
        if setting[1] < 2 then
            setting[1] = 2
            setting[2] = math.max(2, setting[2])
            mS.skillsStorage:set("scaledTrainingDuration", setting)
        end
    end
    return true
end
...
return {
    ...
    eventHandlers = {
        ...
        StatsWindow_Shown = function() updateStats() end,
