...
local addPotion = function(player, recordId, count)
    local newPotion = world.createObject(recordId, count)
    newPotion:moveInto(player.type.inventory(player))
end
...
local addNewPotion = function(player, basePotion, recordPatch, count)
    ...
    addPotion(player, record.id, count)
end
...
return {
    ...
    eventHandlers = {
        ...
        [mDef.events.addPotion] = function(data) addPotion(data.player, data.recordId, data.count) end,
