...
local function setPotionPrice(potionData, record, ingredientValue, positiveEffectsRatio)
    ...
    if samePotion then
        core.sendGlobalEvent(mDef.events.addPotion, {
            player = self,
...
local function addPotionTime(potionData, record)
    local minutesCfg = mS.potionsStorage:get("potionMinutesPerCreation")
    local minutes = 0
    if minutesCfg[3] then
        local skill = math.min(100, T.NPC.stats.skills.alchemy(self).modified)
        minutes = potionData.count * math.max(0, minutesCfg[2] + (minutesCfg[1] - minutesCfg[2]) * (1 - skill / 100))
        log(string.format("Potion Time: potion %s \"%s\", count %d, duration %d minutes",
                record.id, record.name, potionData.count, minutes))
...
module.skillUsedHandler = function(state, skillId, params)
    ...
    if mCore.magickaSkills[skillId] then
        ...
        factor = capSkillScaledFactor(chance < 1 and (2 - chance ^ 2) or (1 - (chance - 1) ^ 4), "magickaBasedSkillScaling")
        featProps = { spellId = spell.id, spellName = spell.name, chance = math.floor(100 * chance) }
        logMessage = string.format("Magicka skills scaling: Cast the spell \"%s\" cost %d, chance = %.2f",
                spell.name, spell.cost, chance)
    ...
    elseif skillId == Skills.alchemy.id then
        ...
        if recipeStats and ownedPotions then
            for _, potionData in pairs(addedPotions) do
                local record = potionData.object.type.record(potionData.object)
                local positiveEffectsRatio = recipeStats.positiveEffects == 0 and 0 or recipeStats.positiveEffects / recipeStats.effectCount
                setPotionPrice(potionData, record, recipeStats.value, positiveEffectsRatio)
                addPotionTime(potionData, record)
            end
            addedPotions = {}
        end
...
local function getHitDamageStats(actor, animGroup, animKey)
    ...
    if damageHealth then
        ...
        return attack.damage.health, damageHealth, damageRatio, hitChance, string.format(
                ...
                actor.recordId, damage, attack.damage.health, hitChance, damageRatio)
    else
        ...
        return damage, damageHealth, damageRatio, hitChance, string.format(
                ...
                actor.recordId, damage, hitChance, damageRatio)
    end
end
