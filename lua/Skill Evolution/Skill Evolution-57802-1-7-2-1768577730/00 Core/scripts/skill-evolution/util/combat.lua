...
module.getPotentialHitInfo = function(player, actor, animGroup, animKey, werewolfClawMult)
    ...
    elseif actor.type ~= T.Creature or record.isBiped then
        ...
        damageMax = mCore.GMSTs.fMaxHandToHandMult * handToHandSkill
    ...
    local hitChance = (0.5 + attackTerm - defenseTerm) / 100

    return math.floor(damageMax), damageHealth, math.floor(hitChance)
end
