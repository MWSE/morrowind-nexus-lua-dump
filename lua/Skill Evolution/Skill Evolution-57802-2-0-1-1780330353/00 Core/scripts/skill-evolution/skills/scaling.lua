...
local maxDelayBetweenSkillUsedAndOnHit = 0.1
...
local function capSkillScaledFactor(factor, setting)
    local range = setting.get()
    return math.min(range.to / 100, math.max(range.from / 100, factor))
end

module.formatFeatProps = function(feat)
    local props = feat.props
    if #props > 0 then
        -- Feat added by another mod
        local items = {}
        for i = 1, #props do
            local prop = props[i]
            table.insert(items, string.format("%s: %s", prop.name, prop.value))
        end
    ...
    elseif feat.skillId == "security" then
    ...
    elseif feat.skillId == "athletics" then
        return L("featRun", props)
    elseif feat.skillId == "alchemy" then
        return L("featPotion", {
            count = props.count,
    ...
    elseif feat.skillId == "block" then
    ...
    elseif feat.skillId == "acrobatics" then
    ...
    elseif feat.skillId == "mercantile" then
...
local function getFeatId(feat)
    if feat.externalId then
        return feat.externalId
    end
    local props = feat.props
    if mCore.magickaSkills[feat.skillId] then
...
local function newFeats()
    return {
        averages = {
            allTime = mHelpers.newAvg(),
            prevLevel = mHelpers.newAvg(),
            currLevel = mHelpers.newAvg(),
        },
        lists = {
            best = {},
            level = {},
...
local function newFeat(factor, skillId, useType, props, externalId)
    return {
        factor = 100 * factor,
        skillId = skillId,
        useType = useType,
        ...
        skillLvl = mCore.getSkillStat(skillId).base,
...
local function addFeat(state, skillId, feat)
    ...
    for key, avg in pairs(state.skills.feats[skillId][feat.useType].averages) do
        if key ~= "prevLevel" then
            mHelpers.addToAvg(avg, feat.factor)
        end
    end
    local lists = state.skills.feats[skillId][feat.useType].lists
    mHelpers.addToSortedTable(feat, function(f) return f.factor end, getFeatId, lists.best, mCfg.maxFeatStats)
    mHelpers.addToSortedTable(feat, function(f) return f.factor end, getFeatId, lists.level, mCfg.maxFeatStats)
...
local function setScaling(state, skillId, factor, params, featProps, message)
    ...
    if featProps then
        addFeat(state, skillId, newFeat(factor, skillId, params.useType, featProps, params.statsId))
        if mS.settings.skillScalingDebugNotifsEnabled.get() then
            local skillData = mS.settings[mSettings.getSkillUseGainsKey(skillId)].argument.config
            mNotifs.notify(string.format("Scaled %s\n%s: %.1f%%",
                    skillData.name, skillData.gains[params.useType].key, factor * 100))
        end
    end
end
...
local function getJumpMaxStats()
    ...
    local acrobatics = mCore.getSkillStat("acrobatics").modified
    local a = acrobatics
    local b = 0
    if a > 50 then
        b = a - 50
...
local function getFallLostHealth(depth, soft)
    local modified = mCore.getSkillStat("acrobatics").modified
    if soft then
    ...
    depth = math.max(0, depth - 1.5 * modified + mCore.self.activeEffects:getEffect(core.magic.EFFECT_TYPE.Jump).magnitude)
    depth = mCore.GMSTs.fFallDistanceBase + mCore.GMSTs.fFallDistanceMult * depth
    depth = depth * (mCore.GMSTs.fFallAcroBase + mCore.GMSTs.fFallAcroMult * (100 - modified))
...
local function onJumpFinished(state, stats, factor, featProps)
    setDelayedScaling(state, "acrobatics", factor, stats.handlerParams, featProps, "Acrobatics scaling: jumped")
    ...
    for slice = 1, slices do
        ...
        local function endOfSlipperySlope()
            addRiskFactor(
                    ...
                    mHelpers.angle(slipperySlope.depthInfo.depth, slipperySlope.prevDepth, slipperySlope.dist, slipperySlope.prevDist),
                    slipperySlope.step)
            slipperySlope = nil
        end
        ...
        local function handleStep()
            ...
            local slopeAngle = mHelpers.angle(depthInfo.depth, prevDepth, dist, prevDist)
...
local function onJumpLanded(state)
    ...
    local avgDepth = mHelpers.avg(stats.depth)
    ...
    local dodgeFactor = mHelpers.sum(stats.dodges)
    ...
    ), mS.settings.acrobaticsSkillScaling)
...
local function handleJumps(state, isSlowFalling, deltaTime)
    ...
    mHelpers.addToAvg(stats.depth, finalDepth)

    mHelpers.addToAvg(stats.speed, state.scaled.groundDist / deltaTime)
...
local function getSwimSpeedFactor()
    ...
    return factor * (0.01 * mCore.getSkillStat("athletics").modified * mCore.GMSTs.fSwimRunAthleticsMult + mCore.GMSTs.fSwimRunBase)
end

local function handleAthletics(state, deltaTime)
    local stats = state.scaled.athletics
    local gameTime = core.getGameTime()
    if gameTime - stats.lastGameTime > 60 then
        -- clear stats when waiting/resting/traveling...
        stats.runningDuration = 0
        stats.deltaTime = 0
        stats.deltaPos = 0
    end
    stats.lastGameTime = gameTime
    ...
    if isMoving and self.controls.run then
        ...
        if state.scaled.isOnGround or T.Actor.isSwimming(self) then
            if state.scaled.isOnGround then
                stats.deltaPos = stats.deltaPos + mHelpers.groundDist(self.position, state.lastPlayerPos)
            else
                stats.deltaPos = stats.deltaPos + (self.position - state.lastPlayerPos):length()
            end
...
local function checkIngredients()
    ...
    local removed = {}
    removedIngredients = {}
    local inventoryIngredients = mCore.self.inventory:getAll(T.Ingredient)
    for i = 1, #inventoryIngredients do
        local ingredient = inventoryIngredients[i]
...
local function checkPotions()
    local potions = {}
    addedPotions = {}
    local inventoryPotions = mCore.self.inventory:getAll(T.Potion)
    for i = 1, #inventoryPotions do
        local potion = inventoryPotions[i]
...
local function getEffectId(effect)
    return string.format("%s_%s_%s", effect.id, effect.affectedAttribute, effect.affectedSkill)
end
...
local function getLastRecipeStats()
    ...
    for id, ingredient in pairs(removedIngredients) do
        ...
        for i = 1, #record.effects do
            effects[getEffectId(record.effects[i])] = { harmful = record.effects[i].effect.harmful }
        end
...
local function setPotionPrice(potionData, record, ingredientValue, positiveEffectsRatio)
    ...
    local valueMod = mS.settings.potionValueMod.get()
    ...
    local ingredientValuePercent = mS.settings.potionMaxIngredientValuePercent.get()
    if ingredientValuePercent > 0 then
        value = math.min(value, ingredientValue * ingredientValuePercent / 100)
    end
    local positiveEffectsImpact = mS.settings.potionPositiveEffectsBasedPrice.get()
    ...
    if samePotion then
        ...
        log(string.format("No potion same as %s", mHelpers.mapToString(newRecord)))
...
local function getTimePassed(range, skill)
    return range.to + (range.from - range.to) * (1 - math.min(mCfg.maxSkillForPassedTime, skill) / 100)
end

local function useTimeConsumingSkill(skillId, hours)
    if mSettings.isDecayEnabled() then
        mDecay.passTimeForSkillUse(skillId, hours)
    else
...
local function addPotionTime(potionData, record, recipeCount)
    local minuteRange = mS.settings.minutesPerPotionCreation.get()
    if not minuteRange.enabled then return end

    local modified = mCore.getSkillStat("alchemy").modified
    local timeReduction = mS.settings.potionMaxRecipeCountBasedTimeReduction.get() / 100
    local minutes = 0
    for _ = 1, potionData.count do
        recipeCount = math.min(recipeCount + 1, mCfg.maxRecipeCount)
        minutes = minutes
                + getTimePassed(minuteRange, modified)
                * (1 - timeReduction * recipeCount / mCfg.maxRecipeCount)
    end

    log(string.format("Potion Time: alchemy level %d, potion %s \"%s\", count %d, recipe count %d, duration %.1f minutes",
            modified, record.id, record.name, potionData.count, recipeCount, minutes))
    useTimeConsumingSkill("alchemy", minutes / 60)
end

local function addRepairTime()
    local minuteRange = mS.settings.minutesPerSelfRepair.get()
    if not minuteRange.enabled then return end

    local modified = mCore.getSkillStat("armorer").modified
    local minutes = getTimePassed(minuteRange, modified)
    log(string.format("Self Repair Time: armorer level %d, duration %.1f minutes", modified, minutes))
    useTimeConsumingSkill("armorer", minutes / 60)
end

local function addEnchantTime()
    local minuteRange = mS.settings.minutesPerSelfEnchanting.get()
    if not minuteRange.enabled then return end

    local modified = mCore.getSkillStat("enchant").modified
    local minutes = getTimePassed(minuteRange, modified)
    log(string.format("Self Enchanting Time: enchant level %d, duration %.1f minutes", modified, minutes))
    useTimeConsumingSkill("enchant", minutes / 60)
end
...
local function checkGoldDiff()
    ...
    if goldDiff < 0 then
        if currentUiMode == "MerchantRepair" then
            local minutes = mS.settings.minutesPerNPCRepair.get()
            if minutes > 0 then
                log(string.format("NPC Repair Time: duration %.1f minutes", minutes))
                core.sendGlobalEvent(mDef.events.passHours, minutes / 60)
            end
        elseif currentUiMode == "Enchanting" then
            local minutes = mS.settings.minutesPerNPCEnchanting.get()
            if minutes > 0 then
...
module.skipWeaponScaling = function(state)
    if not state.scaled.weapon then return end
    local message = string.format("%s scaling: no actor hit after %.2f sec, skipping scaling",
            state.scaled.weapon.skillId, maxDelayBetweenSkillUsedAndOnHit)
    setDelayedScaling(state, state.scaled.weapon.skillId, 1, state.scaled.weapon.handlerParams, false, message)
    state.scaled.weapon = nil
end

local skipWeaponScalingCallback = async:registerTimerCallback(mDef.callbacks.skipWeaponScaling, function()
    self:sendEvent(mDef.events.skipWeaponScaling)
end)

module.skipArmorScaling = function(state)
    if not state.scaled.armor then return end
    local message = string.format("%s scaling: no incoming hit after %.2f sec, skipping scaling",
            state.scaled.armor.skillId, maxDelayBetweenSkillUsedAndOnHit)
    setDelayedScaling(state, state.scaled.armor.skillId, 1, state.scaled.armor.handlerParams, false, message)
    state.scaled.armor = nil
end

local skipArmorScalingCallback = async:registerTimerCallback(mDef.callbacks.skipArmorScaling, function()
    self:sendEvent(mDef.events.skipArmorScaling)
end)

module.skipBlockScaling = function(state)
    if not state.scaled.block then return end
    local message = string.format("Block scaling: no incoming hit after %.2f sec, skipping scaling", maxDelayBetweenSkillUsedAndOnHit)
    setDelayedScaling(state, "block", 1, state.scaled.block.handlerParams, false, message)
    state.scaled.block = nil
end

local skipBlockScalingCallback = async:registerTimerCallback(mDef.callbacks.skipBlockScaling, function()
    self:sendEvent(mDef.events.skipBlockScaling)
end)

local function capSkillScale(skillId, scale)
    if mCore.magickaSkills[skillId] then
        return capSkillScaledFactor(scale, mS.settings.magickaBasedSkillScaling)
    elseif mCore.weaponSkills[skillId] then
        return capSkillScaledFactor(scale, mS.settings.weaponSkillScaling)
    elseif mCore.armorSkills[skillId] then
        return capSkillScaledFactor(scale, mS.settings.armorSkillScaling)
    elseif skillId == "block" then
        return capSkillScaledFactor(scale, mS.settings.blockSkillScaling)
    elseif skillId == "security" then
        return capSkillScaledFactor(scale, mS.settings.securitySkillScaling)
    elseif skillId == "acrobatics" then
        return capSkillScaledFactor(scale, mS.settings.acrobaticsSkillScaling)
    elseif skillId == "athletics" then
        return capSkillScaledFactor(scale, mS.settings.athleticsSkillScaling)
    elseif skillId == "alchemy" then
        return capSkillScaledFactor(scale, mS.settings.alchemySkillScaling)
    end
    return scale
end

module.skillUsedHandler = function(state, skillId, params)
    local useTypeConfig = mS.settings[mSettings.getSkillUseGainsKey(skillId)].argument.config
    if not params.useType
            or not useTypeConfig or not useTypeConfig.gains[params.useType] then return end
    if params.stats then
        params.scale = capSkillScale(skillId, params.scale)
        setScaling(state, skillId, params.scale, params, params.stats, "External scaling")
        return
    end
    local factor = 1
    local featProps = {}
    local logMessage = ""
    if mCore.magickaSkills[skillId] then
        if not mS.settings.magickaBasedSkillScaling.get().enabled then return end
        ...
        local magickaRatio = mCore.self.magicka.base <= 0 and 0 or mSpells.getSpellCost(spell) / mCore.self.magicka.base
        local magickaRatioFactor = magickaRatio <= 0.05 and (1 - (10 * magickaRatio - 1) ^ 2) or 1
        factor = capSkillScaledFactor(magickaRatioFactor * chanceFactor, mS.settings.magickaBasedSkillScaling)
        ...
        logMessage = string.format("Magicka skills scaling: cast the spell \"%s\" cost %d, chance = %.2f, magicka ratio = %.3f, ratio factor = %.3f",
                spell.name, mSpells.getSpellCost(spell), chance, magickaRatio, magickaRatioFactor)
    elseif mCore.weaponSkills[skillId] then
        if not mS.settings.weaponSkillScaling.get().enabled then return end
        state.scaled.weapon = {
            skillId = skillId,
        ...
        params.skillGain = 0
        -- failsafe for skill used triggered by mods
        async:newSimulationTimer(maxDelayBetweenSkillUsedAndOnHit, skipWeaponScalingCallback)
        return
    elseif mCore.armorSkills[skillId] then
        if not mS.settings.armorSkillScaling.get().enabled then return end
        state.scaled.armor = {
            skillId = skillId,
            handlerParams = mCore.copyHandlerParams(params),
        }
        ...
        params.skillGain = 0
        -- failsafe for skill used triggered by mods
        async:newSimulationTimer(maxDelayBetweenSkillUsedAndOnHit, skipArmorScalingCallback)
        return
    elseif skillId == "block" then
        if not mS.settings.blockSkillScaling.get().enabled then return end
        state.scaled.block = {
            handlerParams = mCore.copyHandlerParams(params),
        ...
        params.skillGain = 0
        -- failsafe for skill used triggered by mods
        async:newSimulationTimer(maxDelayBetweenSkillUsedAndOnHit, skipBlockScalingCallback)
        return
    elseif skillId == "security" then
        if not mS.settings.securitySkillScaling.get().enabled then return end
        ...
        if lockableStats.lockLevel then
            ...
            factor = capSkillScaledFactor(2 * (math.max(0, 1 - chance / 100)), mS.settings.securitySkillScaling)
        ...
        elseif lockableStats.trapSpell then
            chance = mCore.agilityTerm(self, skillId)
                    * mCore.GMSTs.fFatigueBase
                    + mSpells.getSpellCost(lockableStats.trapSpell) * mCore.GMSTs.fTrapCostMult
            factor = capSkillScaledFactor(2 * (math.max(0, 1 - chance / 100)), mS.settings.securitySkillScaling)
            ...
            logMessage = string.format("Security scaling: disarmed a trap with spell cost %d, chance = %.2f",
                    mSpells.getSpellCost(lockableStats.trapSpell), chance)
        end
        featProps.chance = math.floor(chance)
    elseif skillId == "acrobatics" then
        if not mS.settings.acrobaticsSkillScaling.get().enabled then return end
        if params.useType == I.SkillProgression.SKILL_USE_TYPES.Acrobatics_Fall then
            ...
            local healthRatio = capFactor(damage / mCore.self.health.base)
            factor = capSkillScaledFactor(2 * healthRatio, mS.settings.acrobaticsSkillScaling)
        ...
        elseif params.useType == I.SkillProgression.SKILL_USE_TYPES.Acrobatics_Jump then
            ...
            state.scaled.acrobatics.stats = {
                ...
                depth = mHelpers.newAvg({ max = 0 }),
                speed = mHelpers.newAvg(),
    ...
    elseif skillId == "athletics" then
        if not mS.settings.athleticsSkillScaling.get().enabled then return end
        local stats = state.scaled.athletics
        ...
        factor = capSkillScaledFactor(0.5 * encumbranceRatio + 1.5 * timeRatio * velocityRatio, mS.settings.athleticsSkillScaling)
        featProps = {
    ...
    elseif skillId == "alchemy" then
        ...
        factor = capSkillScaledFactor((0.5 + recipeStats.positiveEffects / 2) * (1 - math.min(recipeCount, mCfg.maxRecipeCount) / 100) ^ 2,
                mS.settings.alchemySkillScaling)
        ...
        logMessage = string.format("Alchemy scaling: created the %s potion from a recipe (%s) with %d positive effects",
                mHelpers.ordinal(recipeCount + 1), recipeStats.key, recipeStats.positiveEffects)
        if recipeCount == mCfg.maxRecipeCount then
            self:sendEvent(mDef.events.showMessage, L("alchemyScalingRecipeLimit"))
        end
    elseif skillId == "mercantile" then
        if not merchant or #playerGoldChanges < 2 then return end
        factor = params.scale
        params.scale = 1
        local record = merchant.type.record(merchant)
        local goldDiff = playerGoldChanges[1] - playerGoldChanges[2]
        playerGoldChanges = { playerGoldChanges[1] }
    ...
    elseif skillId == "armorer" then
        addRepairTime()
        return
    elseif skillId == "enchant" then
...
module.onActorHit = function(state, enemy)
    ...
    if not stats then
        log("Weapon skills scaling: no skill use, no scaling")
        return
    end
    ...
    local factor = capSkillScaledFactor(2 * (1 - (math.max(0, chance) / 1.5) ^ 0.8), mS.settings.weaponSkillScaling)
    local featProps = {
    ...
    setDelayedScaling(state, stats.skillId, factor, stats.handlerParams, featProps, message)
    state.scaled.weapon = nil
end
...
module.onPlayerHit = function(state, attack)
    ...
    if healthDamage == 0 and fatigueDamage == 0 then
        ...
        if blockStats then
            local elapsedTime = time - blockStats.time
            if elapsedTime > maxDelayBetweenSkillUsedAndOnHit then
                log(string.format("Block scaling: too old skill use (%.2f), no scaling", elapsedTime))
                state.scaled.block = nil
            else
                log(string.format("Block scaling: incoming hit %.2f after the skill use", elapsedTime))
                blockStats.attacker = enemy
            end
        else
            log("Block scaling: no skill use, no scaling")
        end
        if armorStats then
            log("Armor skills scaling: no skill use, no scaling")
        end
        state.scaled.armor = nil
        return
    end
    ...
    if healthDamage == 0 then
        log("Armor skills scaling: no damage, no scaling")
        state.scaled.armor = nil
        return
    end
    ...
    local factor = capSkillScaledFactor(4 * damageRatio, mS.settings.armorSkillScaling)
    local featProps = {
    ...
    setDelayedScaling(state, armorStats.skillId, factor, armorStats.handlerParams, featProps, message)
    state.scaled.armor = nil
end
...
module.onActorAnimHit = function(state, enemy, animGroup, animKey)
    ...
    if hasBlocked then
        ...
        local factor = capSkillScaledFactor(blockFactor, mS.settings.blockSkillScaling)
        ...
        local featProps = {
            ...
            damageRatio = util.round(100 * damageRatio),
            hitChance = util.round(100 * hitChance),
            enemyName = enemy.type.record(enemy).name,
            enemyLevel = T.Actor.stats.level(enemy).current,
        }
        message = string.format("Block scaling: blocked %s", message)
        setDelayedScaling(state, "block", factor, params, featProps, message)
        state.scaled.block = nil
    end
...
module.uiModeChanged = function(data)
    ...
    if data.newMode == "Alchemy" then
        if mS.settings.alchemySkillScaling.get().enabled then
            ownedIngredients = {}
        end
        ownedPotions = {}
...
module.onUpdate = function(state, deltaTime)
    local acrobaticsEnabled = mS.settings.acrobaticsSkillScaling.get().enabled
    local athleticsEnabled = mS.settings.athleticsSkillScaling.get().enabled
    ...
    if acrobaticsEnabled or athleticsEnabled then
        ...
        state.scaled.groundDist = mHelpers.groundDist(self.position, state.lastPlayerPos)
    end
    ...
    if acrobaticsEnabled then
        ...
        local stats = state.scaled.acrobatics.stats
        if (state.scaled.isOnGround and (not stats or not stats.hasJumpStarted))
                or isSlowFalling
                or isLevitating
                or self.position.z > state.scaled.acrobatics.maxFallPos.z then
            state.scaled.acrobatics.maxFallPos = self.position
        end
        if stats then
            if isLevitating then
                local range = mS.settings.acrobaticsSkillScaling.get()
                log(string.format("Used levitate during a jump, scaling %.3f", range.from))
                onJumpFinished(state, stats, range.from, { hasLevitated = true })
            else
