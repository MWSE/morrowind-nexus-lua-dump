...
local mTime = require('scripts.skill-evolution.util.time')
...
local function sendWerewolfClawMult()
    for i = 1, #world.players do
        world.players[i]:sendEvent(mDef.events.setWerewolfClawMult, world.mwscript.getGlobalVariables()[mDef.mwscriptGlobalVars.werewolfClawMult])
    end
end
...
return {
    ...
    eventHandlers = {
        [mDef.events.passHours] = mTime.passHours,
