...
module.getPotentialHitInfo = function(player, actor, animGroup, animKey, werewolfClawMult)
    ...
    elseif actor.type ~= T.Creature or record.isBiped then
        local h2HSkill = mCore.getSkillStat("handtohand", actor).modified
        damageMax = mCore.GMSTs.fMaxHandToHandMult * h2HSkill
        local isWerewolf = actor.type ~= T.Creature and T.NPC.isWerewolf(actor)
        if isWerewolf then
            damageMax = damageMax * werewolfClawMult
        else
            damageHealth = false
        end
        if damageHealth then
            damageMax = damageMax * mCore.GMSTs.fHandtoHandHealthPer
        end
    end
    ...
    elseif weapon then
        skill = mCore.getSkillStat(mCore.weaponTypeToSkill[weaponRecord.type], actor).modified
    else
        skill = mCore.getSkillStat("handtohand", actor).modified
    end
    ...
    return math.floor(damageMax), damageHealth, hitChance
end
