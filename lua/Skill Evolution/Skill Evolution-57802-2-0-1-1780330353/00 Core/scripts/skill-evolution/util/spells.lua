...
module.getSpellCost = function(spell)
    ...
    for i = 1, #spell.effects do
        cost = cost + getAutoCalcEffectCost(spell.effects[i])
    end
...
local function getSpellStats(spell)
    ...
    for i = 1, #spell.effects do
        ...
        local skillTerm = 0
        local skill = mCore.getSkillStat(effect.effect.school)
        if skill then
...
module.getSchoolRatios = function(spell)
    local _, _, schoolImpacts = getSpellStats(spell)
    local sum = 0
