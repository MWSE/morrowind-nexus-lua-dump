...
local updateSkills = function()
    ...
    local skillsCappedValue = mS.skillsStorage:get("skillUncapper")
    ...
    for skillId, getter in pairs(T.NPC.stats.skills) do
        ...
        if currentBase > cappedValue then
            mCore.setSkill(skillId, cappedValue)
            currentBase = cappedValue
        end
...
local upgradeOldState = function(oldState)
...
return {
    ...
    eventHandlers = {
        ...
        [mDef.events.updateStats] = function() updateStats() end,
        ...
        [mDef.events.showModSkill] = function(data) mMsg.showModSkill(data.skillId, data.value, data.diff) end,
