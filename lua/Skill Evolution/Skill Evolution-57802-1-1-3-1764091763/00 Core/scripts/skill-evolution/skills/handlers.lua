...
local function getGainStats(skillId, skillGain)
    ...
    log(string.format("Skill \"%s\" gain %.3f (requirement %.3f), progress %.5f -> %.5f, excess %.3f",
            skillId, skillGain, skillRequirement, T.NPC.stats.skills[skillId](self).progress, progress, excess))
...
local function skillUsedHandlerFinal(state, skillId, params)
    ...
    local progress, excessGain = getGainStats(skillId, params.skillGain)
    if excessGain > 0
            and mS.skillsStorage:get("carryOverExcessSkillGain")
            and T.NPC.stats.skills[skillId](self).base < mS.getSkillMaxValue(skillId) then
        state.skills.excessGain[skillId] = excessGain
...
local function getSkillLevelUpHandler(state)
    return function(skillId, source)
        ...
        if state.skills.base[skillId] < state.skills.max[skillId] then
            ...
            self:sendEvent(mDef.events.onSkillsChanged, 1)
            return false
        end
...
module.onSkillLevelUp = function(state, skillId, skillLevel, source)
    local levelUps = T.NPC.stats.skills[skillId](self).base - skillLevel
    if levelUps <= 0 then return end
    ...
    if decayEnabled then
        mDecay.slowDownSkillDecayOnSkillLevelUp(state, skillId, levelUps)
    end

    if state.skills.excessGain[skillId] > 0 then
        local progress, excessGain = getGainStats(skillId, state.skills.excessGain[skillId])
        log(string.format("Excess skill gain after skill up %.3f, progress %.3f", excessGain, progress))
...
module.uiModeChanged = function(state, data)
    if mS.skillsStorage:get("capSkillTraining") then
        if data.newMode == "Training" then
            mTraining.capTrainedSkills(state, data.arg)
        elseif data.oldMode == "Training" then
            mTraining.uncapTrainedSkills()
        end
    end
end
