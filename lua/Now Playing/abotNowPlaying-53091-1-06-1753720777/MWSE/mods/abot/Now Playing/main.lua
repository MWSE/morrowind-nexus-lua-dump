...
--- local audioController -- NOPE gets reset on loaded 
...
local function getTrack()
	...
	local s = worldController.audioController.currentMusicFilePath
	if logLevel3 then
...
local function timerCallback()
	...
	if config.printTracks then
		---tes3.messageBox('%s:\r\n"%s"', modName, s)
		tes3.messageBox({message = string.format('%s:\r\n"%s"', modName, s), showInDialog = false})
	end
