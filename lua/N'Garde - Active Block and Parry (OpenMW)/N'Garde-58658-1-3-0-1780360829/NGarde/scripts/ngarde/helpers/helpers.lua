...
function Helpers.roundTo(number, decimalPlaces)
    ...
    local integral, fractional = modf(number)

    fractional = fractional * (10 ^ places)
    fractional = floor(fractional) / (10 ^ places)
...
function Helpers.rollNdM(self, number, dSides, difficulty)
    local successes = 0
    for i = 1, number do
        if random(1, dSides) > difficulty then
...
function Helpers.indexOfTable(table, value)
    local index = {}
...
function Helpers.indexOfArray(array, value)
    local index = {}
    for i = 1, #array do
        index[array[i]] = i
    end
