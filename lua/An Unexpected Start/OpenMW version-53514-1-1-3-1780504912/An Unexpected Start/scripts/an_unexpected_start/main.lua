...
function this.toRandomCell()
    ...
    local function getRandomCell(index)
        for i = index, cellCount do
            local cell = world.cells[i]

            if not cell or not this.isInterior(cell) then return end
    ...
    for i = 1, 2 * cellCount do
        newCell = getRandomCell(math.random(cellCount))
        if newCell then break end
    end
