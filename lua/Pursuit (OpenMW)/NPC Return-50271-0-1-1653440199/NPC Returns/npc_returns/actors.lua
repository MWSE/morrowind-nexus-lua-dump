...
local auxiliary = include("pursuit_for_omw.auxiliary") --from Pursuit mod
        ...
        if
         ...
         then
            ...
            if bestDoor and not isReturning then
                ...
                self:sendEvent("StartAIPackage", {type = "Travel", destPosition = bestDoor.position})
                isReturning = types.Door.destCell(bestDoor)
            end

            if
                bestDoor and
                    (self.position - bestDoor.position):length() - 50 <= math.abs(self.position.z - bestDoor.position.z)
             then
                ...
                isReturning = false
                self:sendEvent("StartAIPackage", {type = "Wander", distance = 2048})
            end
        end
    end
end
...
r2Timer =
    ...
    math.random() * time.second, {type = time.GameTime}
)
...
return {
    engineHandlers = {
        ...
        onInactive = function()
            ...
            if #players > 0 then
                core.sendGlobalEvent(
                    "NPC_Returns_return_eqnx",
                    {cell = oricell, position = oripos, actor = self, player = players[1]}
                )
            end
            --returnToCell()
        end,
