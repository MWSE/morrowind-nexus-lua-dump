...
local function getDescription(weaponName)
    return "Этот эффект заклинания вызывает малую Даэдру, скованную в форме магической, удивительно легкой Даэдрической удочки." ..
    " Она немедленно появляется в руке заклинателя, замещая любое оружие, которое там находилось, которое, в свою очередь, помещается в снаряжение персонажа." ..
    " Когда эффект заканчивается, призванное оружие исчезает, и оружие," .. 
    " которым пользовался заклинатель вначале, автоматически экипируется."
end
...
local function addBoundWeaponEffects()
    framework.effects.conjuration.createBasicBoundWeaponEffect({
        ...
        name = "Призвать удочку",
        description = getDescription("Призвать удочку"),
        baseCost = 1,
...
local function applyTooltips(e)
	if e.object.id:lower() == "ste_fishing_pole_deadric_01" then
		...
		local text = "Увеличить навык Рыбная ловля на 50 п : На себя"
