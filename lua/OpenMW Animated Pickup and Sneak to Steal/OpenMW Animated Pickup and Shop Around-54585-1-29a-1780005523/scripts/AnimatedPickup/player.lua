...
--	if debug and (o or target) then
--		print(o and o.recordId)
--	end
	target = o
    ...
    tooltip = ui.create { layer = 'HUD', type = ui.TYPE.Image,
            props = {
                ...
                color = icon == "steal" and uiTheme.steal or uiTheme.icon,
            },
        }
end
...
--	print(s.viewChange, s.inFirst, s.sneak)
	if not skipUpdate then			return		end
...
local function onUpdate(dt)
	if skipUpdate or dt <= 0 then		return		end
...
--	print("CALLBACK "..e)
	...
	core.sendGlobalEvent("ANP_Event", {event="uiMenu", data=e})
end
...
--	ambient.playSound("Menu Click")
	...
	I.UI.setMode(I.UI.MODE.QuickKeysMenu, {windows={}})
        core.sendGlobalEvent("ANP_Event", {event="unPause"})

	messageBox = uicode.createMenu(e)
end

local function processEvents(e)
--	ui.showMessage(e.eventName .. " " .. e.object.recordId .. (e.takeAll and " true" or "") )
	if e.eventName == "OpenPurchaseMenu" then
		uiMessageMenu(e)
		ambient.playSound("Menu Click")
	elseif e.eventName:find("^StealingBlocked") then
		local msg = "StealingBlockedItem" and "msg_usesneak" or "msg_usesneakact"
		ui.showMessage(l10n(msg))
		ambient.playSound("Menu Click")
	end
end
...
return {
	...
	eventHandlers = {
		...
		ANP_Event = processEvents,
		olhInitialized = function()
			useHelper = true
			I.luaHelper.eventRegister("playerTargetChanged", function(o)
				if useHelper then	targetChange(o)		end
			end)
		end
	},
	...
	interface = {
		version = 129,
		useSharedRayCast = function(e)
			if e == nil then
				print("Using shared raycast " .. tostring(useHelper))
			else
				useHelper = e
			end
		end,
--		debug = function(e)	debug = e		end
	}
}
