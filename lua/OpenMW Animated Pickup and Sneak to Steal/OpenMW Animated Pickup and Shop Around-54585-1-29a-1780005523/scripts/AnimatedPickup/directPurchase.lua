...
local ui = {
    ...
    menu = function(arg)	player:sendEvent("ANP_Event", arg)			end
}
...
local function getOwner(reference)
    ...
    if owner then
        ...
        if o and o:isValid() and o.count ~= 0 then
            if not o.cell:isInSameSpace(reference) then o = nil		end
            return o
        end
    end
    ...
    merchants[id] = { scanned=reference.cell, object=actor }
    return actor
end
...
local function tradesItemType(id, item)
    local services = L.npcRecords[id].servicesOffered
...
--- Barter math courtesy of ZackUtils by DaisyHasACat
local function getBarterOffer(npc, item, buying)
    ...
    if data.soul then basePrice = types.Creature.records[data.soul].soulValue end
    basePrice = basePrice * rec.value
    
    local self = player
    local playerMerc = L.mercantile(self).modified

    local playerLuck = L.luck(self).modified
    local playerPers = L.personality(self).modified
    ...
    local d = math.min(L.mercantile(npc).modified, 100)
    local e = math.min(0.1 * L.luck(npc).modified, 10)
    local f = math.min(0.2 * L.personality(npc).modified, 10)
...
function M.uiMessageMenu(e)
    ...
    if item.parentContainer or world.isWorldPaused() then
          ...
          if item.parentContainer then
              ui.menu { eventName = "PickupItemMoveInto", object = item, container = item.parentContainer }
          end
          if I.ErnBurglary then
              I.ErnBurglary.setItemsAllowed(player, true)
              async:newUnsavableSimulationTimer(0.5, function()
                  I.ErnBurglary.setItemsAllowed(player, false)
              end)
          end
    else
--        print(item, item.parentContainer)
        M.purchaseTarget = item
...
local function openPurchaseMenu(itemRef, owner, price)
    ...
    local itemName = itemRef.type.records[itemRef.recordId].name
    if itemRef.count > 1 then itemName = itemName .. " (" .. itemRef.count .. ")" end
    local ownerName = L.npcRecords[owner.recordId].name
    purchase.npc, purchase.item, purchase.price = owner, itemRef, price
    ui.menu({ message = string.format(l10n("PurchaseMessage"), ownerName),
        buttons = { string.format(l10n("PurchaseMessageYes"), itemName, price), l10n("PurchaseMessageNo") },
        object = itemRef, eventName = "OpenPurchaseMenu"
    })
--    ui.sound("Menu Click")
...
--        print("Cannot purchase", id, "- Target is gold")
    ...
    if not ownerId or not L.npcRecords[ownerId] then
...
--        print("Cannot purchase", target.recordId)
    ...
    if not owner then
        ui.message(string.format(l10n("msg_sellermissing"), L.npcRecords[target.owner.recordId].name))
        ui.sound("Menu Click")
        return false
    end
    if owner and types.Actor.isDead(owner) then
        ui.message(string.format(l10n("msg_sellerdead"), L.npcRecords[owner.recordId].name))
        ui.sound("Menu Click")
        return false
    end
    --check faction restrictions
    local factionId = types.NPC.getFactions(owner)[1]
    if factionId then
        local factionRank = types.NPC.getFactionRank(owner, factionId)
	local factionRankPlayer = types.NPC.getFactionRank(player, factionId) or 0
        if factionRank > 5 and (factionRank - factionRankPlayer) > 3 then
            ui.message(string.format(l10n("msg_factionblock"), core.factions.records[factionId].name))
            ui.sound("Menu Click")
            return false
        end
    end
