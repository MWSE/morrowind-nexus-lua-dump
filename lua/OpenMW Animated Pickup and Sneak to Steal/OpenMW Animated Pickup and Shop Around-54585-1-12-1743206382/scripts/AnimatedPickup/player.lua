...
local uiTheme = {
	...
	steal = gmstToRgb("FontColor_color_normal", {1, 0.15, 0.15}),
	baseSize = 16
	}
...
local function uiClick(_, e)
	...
	I.UI.removeMode(I.UI.MODE.QuickKeysMenu)
        core.sendGlobalEvent("anpEvent", {event="uiMenu", data=e.userData})
end
...
local function removeBox()
	messageBox:destroy()		messageBox = nil
	core.sendGlobalEvent("anpEvent", {event="uiMenu", data=0})
end

--[[
input.registerTriggerHandler("Inventory", async:callback(function(e)
	if messageBox then
		removeBox()
		if I.UI.getMode() == I.UI.MODE.QuickKeysMenu then
			self:sendEvent("anpSkipFrame", m)
		end
	end
end))
--]]
...
return {
	...
	eventHandlers = { anpUiSound = function(e) ambient.playSound(e.id, e.options)		end,
--		anpUiMessage = function(e) ui.showMessage(l10n(e))		end,
		UiModeChanged = function(e)
			if messageBox and e.newMode ~= I.UI.MODE.QuickKeysMenu then removeBox() end
		end,
		...
		anpSkipFrame = function(e)
			...
			if #I.UI.modes > 1 then I.UI.removeMode(I.UI.getMode()) else I.UI.setMode() end
		end,
