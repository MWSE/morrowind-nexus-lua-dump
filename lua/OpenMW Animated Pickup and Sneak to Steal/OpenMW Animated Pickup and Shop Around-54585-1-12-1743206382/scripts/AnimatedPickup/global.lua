...
--[[
--		if rec == "furn_imp_altar_cure_01" then		return		end

			local owner = o.owner.recordId		owner = owner and types.NPC.records[owner]
	--		if owner then print(owner.class) 				end
			if owner and owner.class == "publican" then
				print("IS PUBLICAN")
				return
			end
		end
...
local function onUpdate(dt)
	...
	for i=1, #animating do
		...
		elseif data.k < 4 and data.valid then
			...
			cleared = false
			if data.k > 3 then
				local ql = quickloot[o.id] ~= nil
				verify[#verify + 1] = { obj=o, cell=o.cell, to=o.position,
					from=data.startPos, ql=ql, c=0, actor=data.actor }
	...
	for i=1, #verify do
		...
		if o and data.c > 3 then
			if o:isValid() and o.count > 0 and o.cell == data.cell
				and (o.position - data.to):length() < 10 and not data.ql then
...
local function logQuickloot(e)
	...
	local type = types.Container.objectIsInstance(container) and types.Container or types.Actor
	if o then
...
function events.unPause()	unPause = true				end
