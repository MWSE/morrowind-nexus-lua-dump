...
local function verifyPlayer(arg)
	...
	players[arg.id] = {obj=arg, sneak=false, nosteal=false, anim=true, speed=750, speedtk=100}
end
...
--		print(faction, need, rank)
	...
	if block then actor:sendEvent("showUIMessage", "msg_usesneak") end
	return block
end
...
local function playerState(e)
	...
	if e.spdtk ~= nil then players[actor.id].speedtk = e.spdtk end
