...
function Set.union(...)
    ...
    return Set.new({vals=vals, n = n}, true)
end
...
Class.new({name="Set", 
    ...
    new_obj_func = function(p, dont_copy)
        ...
        if dont_copy == true then
            if type(p.n) == "number" then
                if p.vals and type(p.vals) == "table" then
                    return p
                else
                    local n = p.n
                    p.n = nil
                ...
                if p.vals and type(p.vals) == "table" then
                    return {vals=p.vals, n=table.size(p.vals)}
                else
                    return {vals=p, n=table.size(p.vals)}
                end
            end
        end
        ...
        -- ---------------------------------------------------------------------
        -- end of special cases, now actually make stuff
        -- ---------------------------------------------------------------------

        local vals = {}
        local n = 0

        local first_key, first_val = next(p)
        if first_key==nil then return {vals={}, n=0} end

       
        if first_val == true then -- set structure, so "keys" are values
            for v in pairs(p) do
                if not vals[v] then
                    vals[v] = true
                    n = n + 1
        ...
        else -- normal structure, so "values" are values
