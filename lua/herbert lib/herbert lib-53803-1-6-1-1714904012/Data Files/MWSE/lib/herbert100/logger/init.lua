...
Class.new({ name="Logger",
    fields={
        ...
        {"mod_dir", eq=true},
        {"module_name", eq=true},
        {"level", factory=function() return 3 end},
...
for level_str, lvl in pairs(LOG_LEVEL) do
    tbl_ext.insert(mcm_log_options, {label=level_str, value=level_str})
end

tbl_ext.sort_by_map(mcm_log_options, function(option) return LOG_LEVEL[option.value] end)
...
local old_levels = {[-1]="NONE", [0]="WARN", [1]="INFO", [2]="DEBUG", [3]="TRACE"}
