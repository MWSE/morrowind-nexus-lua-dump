...
function Class.new(class_params, base)
    ...
    if not secrets.name then
        ...
        if info then
            ...
            -- local author_prefix, s, e
            -- if info.dir_has_author_name then
            --     s, e, author_prefix = info.mod_dir:find("^%s*([a-zA-Z]+)")
            -- end
            -- if author_prefix then
            --     secrets.name = table.concat{
            --         author_prefix,
            --         ":",
            --         info.mod_name:gsub("(%w)%w*[%s_]*", string.upper), -- prefix
            --         ":",
            ...
            -- mwse.log("name parts = %s", json.encode{
            --     info.mod_name:gsub("(%w)%w*[%s_]*", string.upper), -- prefix
            --     "_",
            --     filename:sub(1,1):upper(),
            --     (filename:sub(2, -5):gsub("[_%s]+(%w)", function(w) return "_" .. w:upper() end)),
            --     -- nil
            -- })
            -- mwse.log("setting name = %q", secrets.name)
        end
    end
