...
---@field fields herbert.Field[] the fields of this class
---@field new_obj_func nil|new_obj_func_presets|fun(...):(herbert.Class|boolean|nil): the way that new objects are made
...
---@field converters table<string, fun(any): any> # OPTIONAL: a table of the form {<class_field> : fun}. 
--- when a new object is created, the appropriate functions are run on the fields of the newly created object.
--- NOTE: a converter is __ONLY__ called if the object has a value for the given field. i.e., it is only called if `rawget(obj, key) ~= nil`.
---@field factories table<string, fun(cls: herbert.Class)> # OPTIONAL: a table of the form {<class_field> : fun}. 
--- when a new object is created, the `factories` table is used to initialize default values for objects, provided those objects don't have values for the repsective fields.
...
local Class = {
    ...
    is_instance_of = function(obj, cls)
        ---@diagnostic disable-next-line: undefined-field
        if type(obj) ~= "table" or not obj.__HCI then 
            return false 
        end
        -- all the parent classes of this object
        local obj_ancestors = _CLASS_DB.all_ancestors_set[obj.__secrets.id]
    ...
    is_subclass_of = function(cls1, cls2)
        ...
        local cls1_ancestors = _CLASS_DB.all_ancestors_set[cls1.__secrets.id]
        if cls1_ancestors ~= nil then 
            return cls1_ancestors[cls2.__secrets.id] == true 
        end
        return false
    end,
...
function Class.new(class_params, base)
    ...
    do -- sanitize fields 
        ...
        for i, field in ipairs(fields) do
            ...
            if type(field.factory) == "function" then
                secrets.factories[key] = field.factory
                field.factory = nil
            end
    ...
    if not secrets.name then
        ...
        if info then
            if info.filename == "main.lua" or info.filename == "init.lua" then
                secrets.name = info.parts[#info.parts - 1]
            else
                secrets.name = info.filename:sub(1, -5)
            end
        end
    end
    ...
    do -- copy over things from ancestors 
        ...
        for _, t in ipairs({"cls_metatable", "obj_metatable", "converters", "factories"}) do
            for _, ancestor in ipairs(secrets.ancestors) do
    ...
    local c = secrets.converters
    local f = secrets.factories
    ...
    if cls.__secrets.new_obj_func == "obj_data_table" then
        ...
        function cls.new(obj_data, ...)
            ...
            if obj.__HCI and getmetatable(obj) == secrets.obj_metatable then return obj end
            ...
            for key, converter in pairs(c) do
                if rawget(obj, key) ~= nil then 
                    obj[key] = converter(obj[key])
                end
            end
            for key, factory in pairs(f) do
                if rawget(obj, key) == nil then 
                    obj[key] = factory(obj)
                end
            end
