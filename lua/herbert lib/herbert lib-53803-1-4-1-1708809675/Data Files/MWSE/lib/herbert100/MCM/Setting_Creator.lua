local Logger = require("herbert100.logger")
...
---@field convertlabel nil|fun(self:mwseMCMSetting, value):string|number
...
function SC:add_log_settings(mod_name)
    if not mod_name or type(mod_name) ~= "string" then
        mod_name = self:get_mod_name()

    end
    local logger = Herbert_Logger.get(mod_name) or Herbert_Logger.new(mod_name)
    -- if not logger then
