...
local function mod_name_factory()
    ...
    init=function (self, ...)
        ...
        if not self.mod_name then
            if info.metadata then
                self.mod_name = info.metadata.package.name
            end
            self.mod_name = self.mod_name or info.lua_parts[info.dir_has_author_name and 2 or 1]
        end 
        if not self.config then
            local cfg_path
            if info.metadata and info.metadata.tools.mwse["lua-mod"] then
                cfg_path = info.metadata.tools.mwse["lua-mod"] .. ".config"
            else
                if info.dir_has_author_name then
                    cfg_path = table.concat({info.lua_parts[1], info.lua_parts[2], "config"}, ".")
                else
                    cfg_path = table.concat({info.lua_parts[1], "config"}, ".")
                end
            end
