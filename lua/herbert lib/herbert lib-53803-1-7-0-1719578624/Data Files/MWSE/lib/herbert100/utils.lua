...
function utils.timeit(f, name)
    local log = Herbert_Logger()
    name = name or debug.getinfo(f, "n").name or "unnamed"
    return function(...)
        ...
        local diff = etime - stime
        local diff_ms = math.floor(diff * 1000)
        log("function \"%s\" took %s seconds", name, diff)
...
---@generic KEY_TYPE, VAL_TYPE, RET_TYPE, STATE
---@param iterator fun(k: KEY_TYPE): KEY_TYPE, VAL_TYPE
---@param map fun(v: VAL_TYPE): RET_TYPE
---@return fun(): KEY_TYPE, RET_TYPE
function utils.map_iterator(iterator, map)
    return coroutine.wrap(function()
        for k, v in iterator do
            coroutine.yield(k, map(v))
        end
    end)
end

---@generic KEY_TYPE, VAL_TYPE, RET_TYPE
---@param iterator fun(k: KEY_TYPE): KEY_TYPE, VAL_TYPE
---@param map fun(k: KEY_TYPE, v: VAL_TYPE): RET_TYPE
---@return fun(): KEY_TYPE, RET_TYPE
function utils.map_iterator2(iterator, map)
    return coroutine.wrap(function()
        for k, v in iterator do
            coroutine.yield(k, map(k, v))
        end
    end)
end
...
---@generic KEY_TYPE, VAL_TYPE, RET_TYPE
---@param iterator fun(k: KEY_TYPE): KEY_TYPE, VAL_TYPE
---@param predicate fun(v: VAL_TYPE): boolean
---@return fun(): KEY_TYPE, VAL_TYPE
function utils.filter_iterator(iterator, predicate)
    return coroutine.wrap(function()
        for k, v in iterator do
            if predicate(v) then
                coroutine.yield(k, v)
...
---@generic KEY_TYPE, VAL_TYPE, RET_TYPE
---@param iterator fun(k: KEY_TYPE): KEY_TYPE, VAL_TYPE
---@param predicate fun(k: KEY_TYPE, v: VAL_TYPE): boolean
---@return fun(): KEY_TYPE, VAL_TYPE
function utils.filter_iterator2(iterator, predicate)
    return coroutine.wrap(function()
        for k, v in iterator do
