...
---@class Herbert_Logger : herbert.Class
...
--- write an error message using `string.format`, if the current `log_level` permits it.
-- if the `log.level` permits writing this kind of message, then `string.format(s,..)` will be called, 
-- and the result of that will be passed to the log
---@param s string the string to print.
---@param ... any arguments to pass to `string.format(s, ...)`
function Herbert_Logger:errorf(s, ...)
    if self.level >= LOG_LEVEL.PROBLEMS then
        self:write("ERROR", string.format(s, ...)) end
end


--- write a warning message, if the current `log_level` permits 
--it. multiple arguments can be passed. each one will new line.
---@param ... any arguments to pass to `string.format(s, ...)`
function Herbert_Logger:warnf(s, ...)
    if self.level >= LOG_LEVEL.PROBLEMS then
        self:write("WARN", string.format(s, ...)) end
end

--- write an info message, if the current `log_level` permits it.
-- multiple arguments can be passed. each one will print on 
---@param ... any arguments to pass to `string.format(s, ...)`
function Herbert_Logger:infof(s, ...)
    if self.level >= LOG_LEVEL.INFO then
        self:write("INFO", string.format(s, ...)) end
end


--- write a debug message, if the current `log_level` permits it.
-- multiple arguments can be passed. each one will print on 
---@param ... any arguments to pass to `string.format(s, ...)`
function Herbert_Logger:debugf(s, ...)
    if self.level >= LOG_LEVEL.DEBUG then
        self:write("DEBUG", string.format(s, ...)) end
end

--- write a trace message, if the current `log_level` permits it.
-- multiple arguments can be passed. each one will print on 
---@param ... any arguments to pass to `string.format(s, ...)`
function Herbert_Logger:tracef(s, ...)
    if self.level >= LOG_LEVEL.TRACE then
        self:write("TRACE", string.format(s, ...)) end
end



--- add this logger to the passed MCM category/page.
---@param component mwseMCMPage|mwseMCMSideBarPage|mwseMCMCategory the component to add the setting to
