...
function Class.new(class_params, base)
    ...
    for _, k in ipairs{"name", "init", "post_init", "new_obj_func"} do
    ...
    do -- copy over things from ancestors 
        ...
        if #ancestors == 1 then
            secrets.cls_metatable.__index = ancestors[1]
        else
            secrets.cls_metatable.__index = function(self, key)
                local v
    ...
    local converters = secrets.converters
    local factories = secrets.factories
    ...
    if cls.__secrets.new_obj_func == "obj_data_table" then
        ...
        function cls.new(obj_data, ...)
            ...
            for key, converter in pairs(converters) do
