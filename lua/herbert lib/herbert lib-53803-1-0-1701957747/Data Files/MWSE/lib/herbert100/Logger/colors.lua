...
local escape_string = string.char(27) .. '[%dm'
local reset_str = escape_string:format(keys.reset)
...
local function parse_colors(str)
	local buffer = {}
	local number
	for word in str:gmatch("[a-z]+") do
		if word ~= "" then 
			...
			table.insert(buffer, escape_string:format(number))
		end
	end
...
function colors.noReset(str)
	...
	return str:gsub(pattern, parse_colors)
end
