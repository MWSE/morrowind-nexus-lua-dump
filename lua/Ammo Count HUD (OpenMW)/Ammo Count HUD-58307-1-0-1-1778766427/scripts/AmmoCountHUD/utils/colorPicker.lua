---@diagnostic disable: assign-type-mismatch, redundant-return-value, need-check-nil
...
-- =========================================================
-- =========================================================
COLOR_RENDERER_ID = "SuperColorPicker2"
...
-- =========================================================
local historySection = storage.playerSection(COLOR_RENDERER_ID .. "History")




local function loadColorHistory()
    local historyData = historySection:get("colors")
    local colors = {}
...
local function loadColorHistoryForUi(presetColors)
    ...
    --    presetLookup[color:upper()] = true
    --end

    for i, hexStr in ipairs(historyData or {}) do
        --if not presetLookup[hexStr:upper()] then
        table.insert(colors, util.color.hex(hexStr))
        --end
    end


    for i = 0, #presetColors - 1 do
        colors[MAX_HISTORY_COLORS - i] = util.color.hex(presetColors[#presetColors - i]:upper())
    end
    return colors
end
...
local function saveColorHistory(colors)
    local hexStrings = {}
    for i, col in ipairs(colors) do
        table.insert(hexStrings, col:asHex())
    end
    historySection:set("colors", hexStrings)
end

local function addColorToHistory(color)
    local history = loadColorHistory()
    local colorHex = color:asHex():upper()

    -- Check if color already exists and remove it (we'll add it to front)
...
local function makeColorPickerBox(value, set, argument)
    local presetColors    = argument and argument.presetColors or {}
    ...
    -- Store initial color for cancel/revert functionality
    ...
    -- Forward declaration for loadColorFromHistory
    ...
    -- Load a color from history into the picker
    loadColorFromHistory = function(color)
        newColor = color
        set(newColor)

        -- Update crosshair and scroll button positions
    ...
    -- COLOR HISTORY PANEL
    ----------------------------------------------------------------
    local history               = loadColorHistoryForUi(presetColors)
    local historySwatchContent  = {}
    local historySwatchContent2 = {}

    -- Create function to make a single swatch
    local function makeHistorySwatch(index, color)
        ...
        local swatchImage = {
            ...
            events = {
                mousePress = async:callback(function(coord, layout)
                    local hist = loadColorHistoryForUi(presetColors)
        ...
        -- Wrap in a box for border
        local template = I.MWUI.templates.box
        if index > MAX_HISTORY_COLORS - #presetColors then
            template = I.MWUI.templates.padding
        end
        local swatchBox = {
            template = template,
