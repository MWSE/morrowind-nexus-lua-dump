...
local function getAtronachMult()
    if types.Actor.activeEffects(self):getEffect("stuntedmagicka").magnitude == 1 then
        ...
        if I.UI.getMode() == 'Rest' then
            ...
            if timeLastFrame < timeCurrentFrame then
                ...
                if timeDifference > 1 then
                    ...
                    local willBeRestoredByPotions = magickaLastFrame + totalPotionMagicka >=
                    maxMagicka + magickaMagnitude
                    if isAtMaxMagicka or willBeRestoredByPotions then
                        restRegenReset()
                        local reason = isAtMaxMagicka and "we already are at our maximum magicka" or
                        "potions will restore our total magicka"
                    ...
                    if cultAbilityTime > 0 then
                        ...
                        regenAmountAbil, regenTimeAbil = I.IMRRegenUtil.calculateRegenAmount(maxMagicka, regenStat,
                            cultAbilityInSeconds, oneSecond)
                    ...
                    local regenAmount, regenTime = I.IMRRegenUtil.calculateRegenAmount(maxMagicka, regenStat,
                        gameToRealTime, oneSecond)
