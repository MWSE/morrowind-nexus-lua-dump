...
local function getEffectLine(effectParams, nameOnly, textColor, isConstant)
    ...
    if not nameOnly then
        if effect.hasMagnitude then
            if effectParams.magnitudeMin == effectParams.magnitudeMax then
                ...
                text = string.format("%s %s", text,
                    ZHIL10n('in_game_tooltip_effect_magnitude', { num = effectParams.magnitudeMin }))
            else
                --text = string.format("%s %d to %d pts", text, effectParams.magnitudeMin, effectParams.magnitudeMax)
                text = string.format("%s %s", text,
                    ZHIL10n('in_game_tooltip_effect_magnitude_minmax',
                        { min = effectParams.magnitudeMin, max = effectParams.magnitudeMax }))
            end
        end
        ...
        if effect.hasDuration and not isConstant then
            ...
            text = string.format('%s %s', text,
                ZHIL10n('in_game_tooltip_effect_duration', { seconds = effectParams.duration }))
        end
...
local function getSpellTooltipData(spell, showExtendedTooltips)
    ...
    addItemLine(rows,
        ZHIL10n('in_game_tooltip_spell_chance', { chance = tonumber(ZMUtility.Magic.getSpellCastChance(self, spell)) }))
...
local function getItemRecordContent(itemType, record, itemObject, showExtendedTooltips)
    ...
    elseif itemType == types.Armor then
        ...
        addItemLine(content,
            ZHIL10n('in_game_tooltip_armor_rating', { num = ZMUtility.Items.getArmorRatingForActor(self, record) }))
        if showExtendedTooltips then
            if record.baseArmor then
                ...
                addItemLine(content,
                    ZHIL10n('in_game_tooltip_base_rating_ext', { num = ZMUtility.formatNumber(record.baseArmor) }),
                    TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            end
            if record.enchantCapacity then
                ...
                addItemLine(content, ZHIL10n('in_game_tooltip_enchant_capacity_ext', { num = record.enchantCapacity }),
                    TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            end
        end
    elseif itemType == types.Book then
        ...
        if showExtendedTooltips and record.skill then
            ...
            addItemLine(content, ZHIL10n('in_game_tooltip_skill_ext', { skill = record.skill }),
                TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
        end
    ...
    elseif itemType == types.Light then
        ...
        if showExtendedTooltips then
            ...
            addItemLine(content,
                ZHIL10n('in_game_tooltip_carriable_ext', { truefalse = record.isCarriable and "true" or "false" }),
                TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            addItemLine(content, ZHIL10n('in_game_tooltip_isfire_ext', { truefalse = record.isFire and "true" or "false" }),
                TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            if record.radius then
                ...
                addItemLine(content, ZHIL10n('in_game_tooltip_radius_ext', { num = record.radius }),
                    TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            end
        end
    elseif itemType == types.Lockpick then
        ...
        if showExtendedTooltips and record.maxCondition then
            ...
            addItemLine(content, ZHIL10n('in_game_tooltip_max_uses_ext', { num = record.maxCondition }),
                TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
        end
    ...
    elseif itemType == types.Weapon then
        if record.chopMinDamage and record.chopMaxDamage then
            ...
            addItemLine(content,
                ZHIL10n('in_game_tooltip_weapon_chop', { min = record.chopMinDamage, max = record.chopMaxDamage }))
        end
        if record.slashMinDamage and record.slashMaxDamage then
            ...
            addItemLine(content,
                ZHIL10n('in_game_tooltip_weapon_slash', { min = record.slashMinDamage, max = record.slashMaxDamage }))
        end
        if record.thrustMinDamage then
            --addItemLine(content, string.format("Thrust: %d - %d", record.thrustMinDamage, record.thrustMaxDamage))
            addItemLine(content,
                ZHIL10n('in_game_tooltip_weapon_thrust', { min = record.thrustMinDamage, max = record.thrustMaxDamage }))
        end
        if showExtendedTooltips then
            if record.enchantCapacity then
                --addItemLine(content, string.format("enchant capacity: %d", record.enchantCapacity), TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            ...
            if record.speed and record.reach then
                ...
                addItemLine(content, ZHIL10n('in_game_tooltip_weapon_ext', { speed = record.speed, reach = record.reach }),
                    TT_CONSTANTS.ExtendedColor, TT_CONSTANTS.ExtendedTextSize)
            end
        end
    end
    ...
    if record.weight and record.weight > 0 then
        ...
        if itemType == types.Armor then
            weight = ZHIL10n('in_game_tooltip_weight_armor',
                { weight = ZMUtility.formatNumber(record.weight), armorclass = ZMUtility.Items.getArmorClass(record) })
        else
...
local function getEnchantmentContent(itemType, record, itemObject, showExtendedTooltips)
    ...
    if record.enchant then
        ...
        addItemLine(content, ZHIUtil.getEnchantTypeText(enchantment), TT_CONSTANTS.SubTextColor, TT_CONSTANTS
        .SubTextSize)
        -- padding
        table.insert(content, {
            type = ui.TYPE.Widget,
        ...
        for i = 1, #enchantment.effects do
            table.insert(content,
                getEffectLine(enchantment.effects[i], false, nil,
