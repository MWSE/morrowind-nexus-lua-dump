...
local function createSelectionWindow(callback)
    local header = {
        ...
        name = "header",
        props = {
            size = util.vector2(ZHIUI_MAIN_CONSTANTS.CategorySelect.Width,
                ZHIUI_MAIN_CONSTANTS.CategorySelect.HeaderHeight),
        ...
        content = ui.content({
            {
                type = ui.TYPE.Text,
                props = {
                    text = ZHIL10n('in_game_select_popup_header'),
                    textSize = 18,
    ...
    local menuWindow = ui.create({
        ...
        content = ui.content({
            {
                type = ui.TYPE.Flex,
                name = "menuWindow_flex",
                props = {
                    size = util.vector2(ZHIUI_MAIN_CONSTANTS.CategorySelect.Width,
                        ZHIUI_MAIN_CONSTANTS.CategorySelect.Height),
                    arrange = ui.ALIGNMENT.Center,
                },
                content = ui.content({
                    header,
                    TextButton.create({
                        text = ZHIL10n('in_game_select_popup_inventory'),
                        callback = ZMUtility.bindFunction(callback, 'inventory')
                    }),
                    {
                        type = ui.TYPE.Widget,
                    ...
                    TextButton.create({
                        text = ZHIL10n('in_game_select_popup_magic'),
                        callback = ZMUtility.bindFunction(callback, 'magic')
                    }),
                    {
                        type = ui.TYPE.Widget,
                        props = { size = util.vector2(1, 8) },
                    ...
                    TextButton.create({
                        text = ZHIL10n('in_game_select_popup_delete'),
                        callback = ZMUtility.bindFunction(callback, 'delete')
                    }),
                    {
                        type = ui.TYPE.Widget,
                        props = { size = util.vector2(1, 8) },
                    ...
                    TextButton.create({
                        text = ZHIL10n('in_game_select_popup_cancel'),
                        callback = ZMUtility.bindFunction(callback, 'cancel')
                    }),
                })
            },
...
local function createHotkeysWidget(hotbarNum, callbacks)
    local width = ZHIUI_MAIN_CONSTANTS.HotbarGroup.KeysPerRow * ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeySize +
    (ZHIUI_MAIN_CONSTANTS.HotbarGroup.KeysPerRow - 1) * ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeyPadding +
    constants.border * 2
    local height = ZHIUI_MAIN_CONSTANTS.HotbarGroup.NumRows * ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeySize +
    (ZHIUI_MAIN_CONSTANTS.HotbarGroup.NumRows - 1) * ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeyPadding +
    constants.border * 2
    ...
    for i = 1, 2 do
        ...
        for j = 1, keysThisRow do
            ...
            local hotkey = ZHIUI.createHotkeyAssignmentBox(tostring(keyNum % 10),
                ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeySize, ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeySize, userData,
                callbacks)
            ...
            offsetX = offsetX + ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeyPadding +
            ZHIUI_MAIN_CONSTANTS.HotbarGroup.HotkeySize
        end
        offsetX = 0
        if hotbarNum ~= 1 then
...
local function createHotbarGroup(identifier, enabled, modifier, callbacks)
    ...
    if identifier == 1 then
        mainText = ZHIL10n('in_game_hotbar_header_main')
        textSize = 24
    else
        --mainText = string.format('Hotbar ##%d - ', identifier - 1)
        local numStr = ZHIL10n('in_game_hotbar_header_num', { num = identifier - 1 })
        if enabled then
            --mainText = mainText .. 'Modifier : '
            mainText = ZHIL10n('in_game_hotbar_header_enabled_format', { hotbar_num_str = numStr })
        else
            --mainText = mainText .. 'Disabled'
            mainText = ZHIL10n('in_game_hotbar_header_disabled_format', { hotbar_num_str = numStr })
        end
    end
    ...
    local header = {
        ...
        props = {
            --autoSize = false,
            size = util.vector2(
            ZHIUI_MAIN_CONSTANTS.HotbarGroup.Width - ZHIUI_MAIN_CONSTANTS.HotbarGroup.InnerHPadding * 2,
                ZHIUI_MAIN_CONSTANTS.HotbarGroup.HeaderHeight),
            arrange = ui.ALIGNMENT.Center,
            align = ui.ALIGNMENT.Center,
            horizontal = true,
        },
    ...
    local innerPanel = {
        ...
        props = {
            ...
            size = util.vector2(
            ZHIUI_MAIN_CONSTANTS.HotbarGroup.Width - ZHIUI_MAIN_CONSTANTS.HotbarGroup.InnerHPadding * 2,
                ZHIUI_MAIN_CONSTANTS.HotbarGroup.Height),
        },
        content = ui.content({
                ...
                content = ui.content({
                    header,
                    {
                        ...
                        props = {
                            size = util.vector2(
                            ZHIUI_MAIN_CONSTANTS.HotbarGroup.Width - ZHIUI_MAIN_CONSTANTS.HotbarGroup.InnerHPadding * 2,
                                ZHIUI_MAIN_CONSTANTS.HotbarGroup.Height - ZHIUI_MAIN_CONSTANTS.HotbarGroup.HeaderHeight),
                        },
                        content = ui.content({
                            hotkeysContainer,
                        })
...
local function createMainLayout(callbacks)
    ...
    local footerButtons = {
        ...
        content = ui.content({
            ...
            TextButton.create({
                text = ZHIL10n('in_game_button_clearall'),
                callback = callbacks.onClearAllPressed
            }),
            {
                type = ui.TYPE.Widget,
                props = {
                ...
                content = ui.content({
                    {
                        ...
                        props = {
                            ...
                            text = ZHIL10n('in_game_main_footer_text'),
                            textSize = 16,
                            textColor = constants.normalColor
                        },
                    }
            ...
            },
            {
                ...
                content = ui.content({
                    TextButton.create({ text = ZHIL10n('in_game_button_ok'), callback = callbacks.onOkPressed }),
