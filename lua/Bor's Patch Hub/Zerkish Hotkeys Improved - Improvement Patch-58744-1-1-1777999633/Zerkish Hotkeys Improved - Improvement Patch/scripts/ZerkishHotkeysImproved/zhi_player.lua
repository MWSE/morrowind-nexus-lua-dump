...
local function loadDataV1(data)
    ...
    ZHISaveData.shouldSuppressStandardQuickKeys = data.shouldSuppressStandardQuickKeys ~= nil and
    data.shouldSuppressStandardQuickKeys or ZHISaveData.shouldSuppressStandardQuickKeys
    ZHISaveData.onCloseQuickKeyMenuFirstTimeFlag = data.onCloseQuickKeyMenuFirstTimeFlag ~= nil and
    data.onCloseQuickKeyMenuFirstTimeFlag or ZHISaveData.onCloseQuickKeyMenuFirstTimeFlag

    -- clean save data from items from deleted mods
    for _, hotbar in ipairs(data.hotbarData) do
        for _, slot in ipairs(hotbar) do
            for itemId, item in pairs(slot.data) do
                if not item.recordId then goto continue end

                local record = types[item.typeStr].records[item.recordId]
                if not record then
                    slot.data[itemId] = nil
                end
...
local function openQuickKeyPopup()
    ...
    zhiWindows[ZHI_WINDOWS.QuickKeyMenuPopup] = ZHIUI.createMessageBox(ZHIL10n('in_game_popup_header'), message, false,
        false)
...
local function onHotkeySelectPressed(mouseEvent, layout)
    ...
    I.ZHI.setMainWindowAlpha(BG_FADE_ALPHA)
    zhiWindows[ZHI_WINDOWS.HotkeySelect] = ZHIUIMain.createHotkeySelectionWindow(ZMUtility.bindFunction(
    onHotkeySelectMenuChoice, layout.userData.hotbar, layout.userData.hotkey))
    --I.ZHI.updateUI()
    ZHITooltip.updateTooltip(nil, nil)
end
...
local function onQuickKeyPressed(keyNum)
    ...
    elseif hotkey and hotkey.data.item then
        ...
        if itemObject and not self.type.isWerewolf(self) then
            -- Equip the item
...
return {
    ...
    engineHandlers = {
        ...
        onActive = function()
            ...
            for i = 1, 9 do
                input.registerTriggerHandler(string.format('QuickKey%d', i),
                    async:callback(ZMUtility.bindFunction(onQuickKeyPressed, i)))
            end

            input.registerTriggerHandler("ToggleHUD", async:callback(onToggleHUDHandler))

            input.registerTriggerHandler('ToggleWeapon',
                async:callback(ZMUtility.bindFunction(onToggleStance, Actor.STANCE.Weapon)))
            input.registerTriggerHandler('ToggleSpell',
                async:callback(ZMUtility.bindFunction(onToggleStance, Actor.STANCE.Spell)))
        ...
        onLoad = function(loadData)
            ...
            setOverrideDefaultQuickKeyMenu(ZHISaveData.onCloseQuickKeyMenuFirstTimeFlag and
            (not mainSettings:get('force_standard_ui')))
            loadHotbarSettings()
