...
local function DaisyUtilsTeleport(data)
...
local function DaisyUtilsTeleportToCell(data)
    ...
    for i, record in ipairs(types.NPC.records) do
        ...
        if (record.travelCount > 0) then
            ...
            if (record.class == "guild guide") then
                myRef = interfaces.DaisyUtilsG.findNPCInterior(npcId)
            else
                myRef = interfaces.DaisyUtilsG.findNPCInWorld(npcId)
            end
            --   end
            if (myRef ~= nil) then
...
local function DaisyUtilsCreate(data)
...
local function DaisyUtilsCreateInterface(itemid, cellname, position, rotation)
    if (itemid == nil) then
        interfaces.DaisyUtilsG.printToConsole(
...
local function cellCopy(sourcecellName, targetcellName, basePosition, player)
    ...
    for i, ref in ipairs(obs) do
        local newref = DaisyUtilsCreateInterface(ref.recordId, targetcellName, ref.position + basePosition, ref.rotation)
        table.insert(createdObs, newref)
    end
    player:teleport(oldCell, oldPos)
    return createdObs
end
local function DaisyUtilsDelete(data)
...
local function DaisyUtilsEmptyInto(data)
...
local function DaisyUtilsAddItem(data)
...
local function DaisyUtilsAddItems(data)
...
local function DaisyUtilsPauseWorld(doPause)
    if (doPause) then
...
local function getBarterOffer(npc, basePrice, disposition, buying)
    ...
    local disposition = types.NPC.getDisposition(npc, player) --) getDerivedDisposition(npc, false)
...
local function AdvanceTime(hours)
    ...
    --TODO: Recharge items, regenerate magic, health
...
return {
    interfaceName  = "DaisyUtils" .. interfaceModifier,
    interface      = {
        version = 1,
        ...
        DaisyUtilsCreateInterface = DaisyUtilsCreateInterface,
        cellCopy = cellCopy,
        DaisyUtilsDelete = DaisyUtilsDelete,
    ...
    eventHandlers  = {
        ...
        DaisyUtilsAddItem_AA = DaisyUtilsAddItem,
        DaisyUtilsPauseWorld_AA = DaisyUtilsPauseWorld,
        DaisyUtilsTeleport_AA = DaisyUtilsTeleport,
        teleportToExtEvent_AA = teleportToExtEvent,
        moveToActorEvent_AA = moveToActorEvent,
        DaisyUtilsDelete_AA = DaisyUtilsDelete,
        DaisyUtilsCreate_AA = DaisyUtilsCreate,
        cellScan_AA = cellScan,
        replaceOb_AA = replaceOb,
        DaisyUtilsTeleportToCell_AA = DaisyUtilsTeleportToCell,
        activateThingEvent_AA = activateThingEvent,
        removeItemCount_AA = removeItemCount,
        DaisyUtilsAddItems_AA = DaisyUtilsAddItems,
        setDisabled_AA = setDisabled,
        DaisyUtilsEmptyInto_AA = DaisyUtilsEmptyInto,
