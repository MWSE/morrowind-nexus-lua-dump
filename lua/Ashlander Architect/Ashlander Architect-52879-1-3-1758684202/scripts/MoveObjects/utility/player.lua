...
local function addItem(actor, itemId, count)
    ...
    core.sendGlobalEvent("DaisyUtilsAddItem_AA", { actor = actor, itemId = itemId, count = count })
end
...
local function teleportItem(item, position, rotation)
    core.sendGlobalEvent("DaisyUtilsTeleport_AA", { item = item, position = position, rotation = rotation })
end
...
local function createItem(itemid, cell, position, rotation)
    returnRef = nil
    core.sendGlobalEvent("DaisyUtilsCreate_AA",
...
local function deleteItem(item)
    core.sendGlobalEvent("DaisyUtilsDelete_AA", item)
end

local function removeItem(item, count, actor)
    if not actor then actor = self.object end
    core.sendGlobalEvent("removeItemCount_AA", { item = item, count = count, actor = actor })
end
local function FindGameObjectName(item)
    ...
    if item.type == types.Apparatus then
        return types.Apparatus.records[item.recordId].name
    elseif item.type == types.Armor then
        return types.Armor.records[item.recordId].name
    elseif item.type == types.Book then
        return types.Book.records[item.recordId].name
    elseif item.type == types.Clothing then
        return types.Clothing.records[item.recordId].name
    elseif item.type == types.Ingredient then
        return types.Ingredient.records[item.recordId].name
    elseif item.type == types.Light then
        return types.Light.records[item.recordId].name
    elseif item.type == types.Lockpick then
        return types.Lockpick.records[item.recordId].name
    elseif item.type == types.Miscellaneous then
        return types.Miscellaneous.records[item.recordId].name
    elseif item.type == types.Potion then
        return types.Potion.records[item.recordId].name
    elseif item.type == types.Probe then
        return types.Probe.records[item.recordId].name
    elseif item.type == types.Repair then
        return types.Repair.records[item.recordId].name
    elseif item.type == types.Weapon then
        return types.Weapon.records[item.recordId].name
    elseif item.type == types.Activator then
        return types.Activator.records[item.recordId].name
    elseif item.type == types.Container then
        return types.Container.records[item.recordId].name
    elseif item.type == types.Door then
        return types.Door.records[item.recordId].name
    elseif item.type == types.NPC then
        return types.NPC.records[item.recordId].name
    elseif item.type == types.Creature then
        return types.Creature.records[item.recordId].name
    end
    return nil
end
...
local function FindEnchant(item)
    ...
    elseif item.type == types.Clothing then
        return types.Clothing.records[item.recordId].enchant
    elseif item.type == types.Weapon then
        return types.Weapon.records[item.recordId].enchant
    end
    return false
end
local function findItemIcon(item)
    if item.type == types.Apparatus then
        return types.Apparatus.records[item.recordId].icon
    elseif item.type == types.Armor then
        return types.Armor.records[item.recordId].icon
    elseif item.type == types.Book then
        return types.Book.records[item.recordId].icon
    elseif item.type == types.Clothing then
        return types.Clothing.records[item.recordId].icon
    elseif item.type == types.Ingredient then
        return types.Ingredient.records[item.recordId].icon
    elseif item.type == types.Light then
        return types.Light.records[item.recordId].icon
    elseif item.type == types.Lockpick then
        return types.Lockpick.records[item.recordId].icon
    elseif item.type == types.Miscellaneous then
        return types.Miscellaneous.records[item.recordId].icon
    elseif item.type == types.Potion then
        return types.Potion.records[item.recordId].icon
    elseif item.type == types.Probe then
        return types.Probe.records[item.recordId].icon
    elseif item.type == types.Repair then
        return types.Repair.records[item.recordId].icon
    elseif item.type == types.Weapon then
        return types.Weapon.records[item.recordId].icon
    end
end
local function findItemWeight(item)
    if item.type == types.Apparatus then
        return types.Apparatus.records[item.recordId].weight
    elseif item.type == types.Armor then
        return types.Armor.records[item.recordId].weight
    elseif item.type == types.Book then
        return types.Book.records[item.recordId].weight
    elseif item.type == types.Clothing then
        return types.Clothing.records[item.recordId].weight
    elseif item.type == types.Ingredient then
        return types.Ingredient.records[item.recordId].weight
    elseif item.type == types.Light then
        return types.Light.records[item.recordId].weight
    elseif item.type == types.Lockpick then
        return types.Lockpick.records[item.recordId].weight
    elseif item.type == types.Miscellaneous then
        return types.Miscellaneous.records[item.recordId].weight
    elseif item.type == types.Potion then
        return types.Potion.records[item.recordId].weight
    elseif item.type == types.Probe then
        return types.Probe.records[item.recordId].weight
    elseif item.type == types.Repair then
        return types.Repair.records[item.recordId].weight
    elseif item.type == types.Weapon then
        return types.Weapon.records[item.recordId].weight
    end
end
...
local function CheckForRecord(recordId, type)
    ...
    if not type or not type.record then return false end
    return type.records[recordId] ~= nil
end
...
local function addItemEquip(actor, itemId)
    ...
    core.sendGlobalEvent("DaisyUtilsAddItem_AA", { actor = actor, itemId = itemId, count = count, equip = true })
end
...
local function PauseWorld(doPause)
    core.sendGlobalEvent("DaisyUtilsPauseWorld_AA", doPause)
end
