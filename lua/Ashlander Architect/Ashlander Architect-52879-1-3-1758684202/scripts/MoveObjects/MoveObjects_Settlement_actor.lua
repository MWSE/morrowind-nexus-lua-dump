...
local function migrateData(target)
    if (getMyShopCont() ~= nil) then
        core.sendGlobalEvent("DaisyUtilsDelete", getMyShopCont())
    end
...
local function addItemEquip(actor, itemId)
    ...
    core.sendGlobalEvent("DaisyUtilsAddItem", { actor = actor, itemId = itemId, count = count, equip = true })
end
...
local function shufflePosInCell()
    ...
    print("Shuffling")
    core.sendGlobalEvent("DaisyUtilsTeleport", { item = self, position = newPos, rotation = self.rotation })
end
    ...
    if (targetStructure ~= nil and targetDoor ~= nil) then
        if (exiting) then
            ...
            if (dist < 500) then
                ...
                core.sendGlobalEvent("DaisyUtilsTeleportToCell",
                    {
                        item = self,
