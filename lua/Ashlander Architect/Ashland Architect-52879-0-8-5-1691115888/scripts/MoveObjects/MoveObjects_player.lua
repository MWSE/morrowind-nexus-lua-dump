...
local function registerDefaultBindings()
    ...
    b.registerBinding("toggleSurfaceSnapping", "%s Surface Snapping", input.KEY.N)
...
local function updateUi()
    ...
    if (controllerMode == false) then
        ...
        if (currentCategory ~= nil and currentSubCat ~= nil) then
            table.insert(buttonTable, "Stamp Currently selected object: ".. I.AA_Bindings.getButtonLabel("orderNPC",controllerMode))
        end
        if (currentMainRef == nil) then
            local hitObject = zutils.getObjInCrosshairs(nil, 10000).hitObject
            if (hitObject) then
                table.insert(buttonTable, "Set grid reference object to targeted object: ".. I.AA_Bindings.getButtonLabel("removeGridRef",controllerMode))
            else
                table.insert(buttonTable, "Remove grid reference object: ".. I.AA_Bindings.getButtonLabel("removeGridRef",controllerMode))
            end
        end
        if (currentMainRef ~= nil and selectedItemIsReal and currentMainRef.type ~= types.NPC) then
            ...
            table.insert(buttonTable, "Drop targeted object: " .. I.AA_Bindings.getButtonLabel("grabTargetedObject",controllerMode))
        end
        if (currentMainRef == nil) then
            table.insert(buttonTable, I.AA_Bindings.getBindingDesc("grabTargetedObject", true))
        end
        if (zPosLock == 0) then
            table.insert(buttonTable, "Enable Vertical Position Lock: ".. I.AA_Bindings.getButtonLabel("toggleVerticalPositionLock",controllerMode))
        else
            table.insert(buttonTable, "Disable Vertical Position Lock: ".. I.AA_Bindings.getButtonLabel("toggleVerticalPositionLock",controllerMode))
        end
        table.insert(buttonTable,
...
local function swingAt()
    if (self.controls.use > 0 and types.Actor.stance(self) == types.Actor.STANCE.Weapon) then
        ...
        if (wasSwinging == true and not isRelease) then
            ...
            if (swingAt.hitObject) then
                ...
                elseif string.sub(swingAt.hitObject.recordId, 1, 10) == "flora_tree" or swingAt.hitObject.recordId == "zhac_aa_treedone_gl" then
                    local treeob = swingAt.hitObject
                    for index, dataOb in ipairs(treeData) do
                        if (dataOb.id == treeob.id) then
                            if (dataOb.remainingSwings > 0) then
                                table.remove(treeData, index)
                                table.insert(treeData, { id = treeob.id, remainingSwings = dataOb.remainingSwings - 1 })
                                local woodCount = math.random(2, 5)
                                ui.showMessage(string.format("You swing at the tree, cutting down %i logs from it.",
                                    woodCount))
                                core.sendGlobalEvent("ZackUtilsAddItem_AA",
                                    { actor = self, itemId = "zhac_aa_resourceitem_log", count = woodCount, equip = false })
                                zutils.addItem()
                            elseif (dataOb.remainingSwings == 0) then
                                table.remove(treeData, index)
                                local woodCount = math.random(2, 5)
                                core.sendGlobalEvent("ZackUtilsAddItem_AA",
                                    { actor = self, itemId = "zhac_aa_resourceitem_log", count = woodCount, equip = false })
                                ui.showMessage(string.format(
                                    "You swing at the tree, cutting down %i logs from it, exhausting the tree.",
                                    woodCount))
                                thingToKill = swingAt.hitObject
                            end
                            wasSwinging = false
                    ...
                    table.insert(treeData, { id = treeob.id, remainingSwings = math.random(1, 10) })
                    local woodCount = math.random(2, 5)
                    ui.showMessage(string.format("You swing at the tree, cutting down %i logs from it.", woodCount))
                    core.sendGlobalEvent("ZackUtilsAddItem_AA",
                        { actor = self, itemId = "zhac_aa_resourceitem_log", count = woodCount, equip = false })
                end
            else
    ...
    if (thingToKill) then
        zutils.teleportItem(thingToKill,
