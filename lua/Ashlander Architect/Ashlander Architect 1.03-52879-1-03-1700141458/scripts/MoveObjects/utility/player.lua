...
local function CheckForRecord(recordId, type)
    ...
    return type.record(recordId) ~= nil
end
...
local function getObjInCrosshairs(ignoreOb, mdist, force,colType)
    --ignoreOb is the object we are ignoring
    --mdist is the maximum distance we will search for
    if colType == nearby.COLLISION_TYPE.HeightMap then
      
    else
        colType = nearby.COLLISION_TYPE.AnyPhysical
    ...
    if (ignoreOb and not force) then --current functionality, have to do normal cast ray. This isn't great for placing items, since it will hit the hitbox of a bookshelf, not the shelf.
        return nearby.castRay(startPos, pos + v * dist, { ignore = ignoreOb[1],
        ...
        if (ignoreOb) then
            if useRenderingRay then
                ...
                while true do
                    local res = nearby.castRenderingRay(startPos, pos + v * dist)
               ...
               -- local ret = nearby.castRay(getPosInCrosshairs(20), pos + v * dist,{ignore = ignoreOb})
                local ret =  nearby.castRay(startPos, pos + v * dist, { ignore = ignoreOb[1],
                collisionType = colType
                

            })
            if not ret.hitPos then
return {hitPos = pos + v * dist}
            end
            return ret
            end
        end
