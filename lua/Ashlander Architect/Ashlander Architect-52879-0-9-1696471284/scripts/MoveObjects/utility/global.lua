...
local soundLines = require("scripts.MoveObjects.utility.SoundLines")
...
local function playSound(soundId)
    ...
    for index, value in ipairs(soundLines) do
        if (value:lower() == soundId:lower()) then
            print(value)
            local found = false
            local itemsinCell = world.getCellByName("_DebugModeHoldingCell"):getAll(types.Activator)
            if (itemsinCell == nil) then
                print("Item not found")
                return
            end
            for k, object in ipairs(itemsinCell) do
                if (object.recordId == "zhac_soundplayer_ob") then
                    object:teleport(player.cell, util.vector3(player.position.x, player.position.y, index))
                    found = true
                    break
                end
            end
            if (found == false) then
                local itemsinCell = world.getCellByName("ToddTest"):getAll(types.Activator)
                if (itemsinCell == nil) then
                    print("Item not found")
                    return
                end
            ...
            if (not found) then print("Sound object not found!") end
...
local function ZackUtilsAddItem(data)
    ...
    local item = world.createObject(record.id, data.count)
