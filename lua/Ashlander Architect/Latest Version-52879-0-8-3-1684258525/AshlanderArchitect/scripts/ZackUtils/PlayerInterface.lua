...
local function addItem(actor, itemId, count)
    ...
    core.sendGlobalEvent("ZackUtilsAddItem", { actor = actor, itemId = itemId, count = count })
end
...
local function teleportItem(item, position, rotation)
    core.sendGlobalEvent("ZackUtilsTeleport", { item = item, position = position, rotation = rotation })
end
...
local function deleteItem(item)
    core.sendGlobalEvent("ZackUtilsDelete", item)
end
...
local function getObjInCrosshairs(ignoreOb, mdist)
    ...
    if (ignoreOb) then
        return nearby.castRay(pos, pos + v * dist, { ignore = ignoreOb
