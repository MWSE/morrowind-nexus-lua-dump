...
---@diagnostic disable-next-line: undefined-field
    ...
    label.color = colorLabel(recastable, effect.harmful)
...
local function blocks(effect, spellId, current, max, recastable, caster, effIndex)
    ...
    icon.width = 20
    icon.height = 20
    local attrName = ((tes3.getMagicEffect(effect.id).targetsAttributes) and string.format(": %s", ((string.gsub(tes3.attributeName[attribute], "^%l", string.upper))))) or ""
    local skillName = ((tes3.getMagicEffect(effect.id).targetsSkills) and ": "..tes3.skillName[skill]) or ""
...
local function registerModConfig()
    ...
    template:createExclusionsPage{label = "Recastable Effects", leftListLabel = "Valid Effects", rightListLabel = "Unmanaged Effects", description = "Here you can configure which effects this mod will allow to recast.", variable = mwse.mcm.createTableVariable{id = "recastable", table = cf}, filters = {{label = " ", callback = getExclusionList}}}
