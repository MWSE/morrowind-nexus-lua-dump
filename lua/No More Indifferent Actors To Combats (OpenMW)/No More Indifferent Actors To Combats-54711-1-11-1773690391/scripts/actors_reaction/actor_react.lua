...
local function resetTarget()
	if goBack then -- "goBack" is a bad name: this variable means that this mod had give a travel order to this actor
					-- So if "travel order", we start a travel toward the start position (to be sure to retrieve the start position at the end of combat)
		if goingBack then -- (if actor is going back, i let him 10 more seconds...)
			...
			if activePackage and activePackage.type == "Travel" then -- if this is the expected package (Travel), we confirm the order. (If not, we don't touch...)
	...
	else -- this mod didn't give a travel order, so we just have to reset target_ variable
		target_ = nil
	end
end
        ...
        combat_detected = function(e)
			...
			if blacklistedActors[self.recordId]
			 ...
			 or (activePackage ~= nil and activePackage.type ~= "Wander"
			  ...
			  and activePackage.type ~= "Travel") -- ("or actor has active package other than "Wander", "Unknown" or "Travel" ")
			 or goingBack -- or actor is going back
			...
			if target_ and not target_:isValid() then
				resetTarget()
				return
			elseif targetDist > alertDistMax -- this targetDist variable is the distance to the nearest of the 2 opponents
			...
			if (myPosition - startPosition):length() > maxMove then -- "if actor has moved too far, then..."
				goingBack = 1 -- goingBack order + startposition travel is going to begin...
			...
			elseif targetDist > actorsDistance and baptism then -- "Since i'm close to the target/fight, but not too much, and since i had my "baptism of the fire", i stay around"...
				...
				if destPos then
					...
					if not nearby.castRay(myPositionHigh, destPosHigh, {
						...
						destPos = startPosition -- We don't know where to escape, so we go toward the start position
					end
				else
					destPos = startPosition -- We don't know where to escape, so we go toward the start position
				end
			...
			goBack = 1 -- We memorize we gave a travel order
        end
    }
}
