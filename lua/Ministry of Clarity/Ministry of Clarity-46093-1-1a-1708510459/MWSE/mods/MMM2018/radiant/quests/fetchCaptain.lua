...
local function debugMessage(string)
	if debug then
		...
		mwse.log("[fetchCaptain: DEBUG] " .. string)
	end
end
----------------------------------------------------------------------------------------------
--  fetchCaptain - Quest Logic for Mission 1: fetch ash statues for the Captain
----------------------------------------------------------------------------------------------
local this = {}
local common = require("MMM2018.radiant.common")
function this.getQuestData()
    return mwse.loadConfig("radiantQuests/fetchCaptainData")
end
...
local function checkProgress()
	...
	if status[currentStatus] == status.inProgress then
		...
		if itemCount >= requiredItemCount then
			tes3.messageBox("Your mission for Nolan Indoril is ready to hand in.")
