...
local function debugMessage(string)
	if debug then
		...
		mwse.log("[fetchElam: DEBUG] " .. string)
	end
end
----------------------------------------------------------------------------------------------
--  fetchElam - Quest Logic for Mission 1: fetch items for Elam in Ministry of Clarity
--
----------------------------------------------------------------------------------------------
local this = {}
local common = require("MMM2018.radiant.common")
function this.getQuestData()
    return mwse.loadConfig("radiantQuests/fetchElamData")
end
...
local function checkProgress()
	...
	if status[currentStatus] == status.inProgress then
		...
		if itemCount >= requiredItemCount then
			tes3.messageBox("Your mission for Elam Andas is ready to hand in.")
