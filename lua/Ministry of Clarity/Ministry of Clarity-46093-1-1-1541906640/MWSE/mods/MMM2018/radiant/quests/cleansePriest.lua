...
local function debugMessage(string)
	if debug then
		...
		mwse.log("[cleansePriest: DEBUG] " .. string)
	end
end
----------------------------------------------------------------------------------------------
--  cleansePriest - Quest Logic for Mission 1: fetch ash statues for the Captain
----------------------------------------------------------------------------------------------
local this = {}
local common = require("MMM2018.radiant.common")
function this.getQuestData()
    return mwse.loadConfig("radiantQuests/cleansePriestData")
end
...
local function checkProgress()
	...
	if status[currentStatus] == status.inProgress then
		...
		if itemCount >= requiredItemCount then
			tes3.messageBox("Your mission for Lleryn Andules is ready to hand in.")
...
function this.startTrackingQuest()
	...
	tracking = true
	debugMessage("Tracking Quest")
	local questData = this.getQuestData()
    -- Check for win conditions
	--event.register("uiActivated", checkProgress, { filter = "MenuDialog" } )
    --check for reward
	...
	--UpdateJournal
	debugMessage("registering report")
