...
local function hasLightEquipped()
    local equipped = Actor.getEquipment(self)
    if not equipped then return false end
    local leftHand = equipped[Actor.EQUIPMENT_SLOT.CarriedLeft]
    return leftHand and leftHand.type == types.Light
end
...
local cachedSettings = {
    ...
    LIGHT_EXEMPTION       = get('LIGHT_EXEMPTION'),
}
...
local function findNearestGuard()
    if cachedSettings.LIGHT_EXEMPTION and hasLightEquipped() then return nil end
