...
local function BuildRegionalOverridesTable()
    for file in lfs.dir("data files/MWSE/config/MS/") do
        ...
        if file:find("MS_r_", 1, true) and file:find("%.json$") then
			...
			MUSE.DebugLog("Found config (region override): " .. file)
			...
			local assignFolderSize = functions.checkFolder("data files/music/" .. "MS" .. "/" .. "region" .. "/" .. MSconfig.folder, ".mp3")
...
function regionMod.OnCellEnter()
	...
	MUSE.DebugLog("Region get: " .. MUSE.regionCurrent)

	local overridesFound = false
	--Assign general overrides
	for i=1, #MUSE.regionInfo do
		MUSE.DebugLog("Checking region info: " .. MUSE.regionCurrent .. "/" .. MUSE.regionInfo[i].regionName)
