...
local function registerModConfig()
    ...
    settings:createOnOffButton{
        label = string.format("Включить %s", common.modName),
        description = "Включите или выключите мод. В состоянии выкл нельзя играть на лютне. Но диалоги \"Выступить\" и \"Выучить песню\" сохраняются. Не понял как это работает. Лучше не выключать",
    ...
    local category = settings:createCategory("Выученные песни")
    local beginner = category:createCategory("Начинающий:")
    beginner:createInfo{
        text = "",
        inGameOnly = true,
        postCreate = createSongListPostCreate("beginner")
    }
    local intermediate = category:createCategory("Опытный:")
    intermediate:createInfo{
        text = "",
        inGameOnly = true,
        postCreate = createSongListPostCreate("intermediate")
    }
    local advanced = category:createCategory("Мастер:")
    advanced:createInfo{
        text = "",
        inGameOnly = true,
