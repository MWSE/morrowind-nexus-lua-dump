local mod = {
    ...
    ver = "1.2",
    cf = { magic = tes3.isLuaModActive("Elemental Effects"), fall = false, shield = true}
            }
...
if (e.reference.object.objectType == tes3.objectType.creature) and not (string.find(e.reference.id:lower(), "dagoth")) then
    local crit = e.reference.object
    if creatureDecals[crit.blood] then
        addCritDecal(crit.blood, e.reference.sceneNode)
    end

    if e.attacker and e.attacker.weaponDrawn then
        for _,layer in pairs(tes3.activeBodyPartLayer) do
