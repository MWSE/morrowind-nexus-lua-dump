...
--local New_Effects = require("Magicka of the Third Era.new_effects")
local Modifier_Logic = require("Magicka of the Third Era.modifier_logic")
...
-- Modifier logic:
-- ignore all modifiers while calculating the normal cost
-- after that, calculate modifier values, such as
-- cost
-- difficulty
-- store them and apply as modifiers in UI/cast events
--
-- HARD PART
-- learn how to treat "modifier effects" differently without breaking anything (e.g. checks if the spell is multi effect)
-- find where the effect table is stored and how we can access it (we somehow can for the synergies)
-- effect_db[id] and effect_cost -> something like this


-- TO CALCULATE 'MODDED' spell:
-- make a table of effect + weight e.g. [{"14": "0.4"}, {"15": "0.6"}]
-- cost -> cost * cost_mod
-- difficulty -> difficulty * difficulty_mod

-- (2024 re-view)
-- I've added comments with MODIFIERS: to streamline the logic a bit.
-- Here's a general gist:
-- When calculating spell (on cast or UI), skip modifier effects, treat the effect count as if modifiers are not present
-- Add a separate function to calculate impact of modifiers
  -- assesses the strength of modifiers in a spell
  -- each modifier effect needs to have some table which defines how good is it
-- This function returns cost_mod and difficulty_mod
-- Save them when saving the spell
-- On cast or UI, check for them to alter values
...
-- OR
-- 5. effects + weight of them (maybe makes 2 obsolete)
-- 6. modifier effects
    ...
    if tes3.player.data.motte_spell_storage[spell_id] then
      ...
      if config.override_costs_alwaystosucceed or not (spell.alwaysSucceeds) then
        ...
        if total_effect_cost > 0 then
          ...
          local cost_mod = 1
          local chance_mod = 1
          tes3.player.data.motte_spell_storage[spell_id] = {cost = spell_cost, skill_table = magic_skill_table2, cost_mod = cost_mod, chance_mod = chance_mod}
        end
      else
        ...
        if not(do_not_save_this) then
          ...
          tes3.player.data.motte_spell_storage[spell_id] = {cost = spell_cost, skill_table = magic_skill_table, cost_mod = cost_mod, chance_mod = chance_mod}
        end
      end
    end
...
local function spell_cost_manipulation(e)
  ...
  if tes3.player.data.motte_spell_storage[spell_id] then
    ...
    if ((not (e.spell.alwaysSucceeds) or config.override_costs_alwaystosucceed)) then
      ...
      -- count non-mod effects
      --local non_mod = count_non_modifier_effects (e.spell)
      --log:trace(string.format("Non-modifier effects: %s", non_mod))
      
      for i,effect in ipairs(e.spell.effects) do
        if effect.id ~= -1 then
          -- MODIFIERS: apply check whether it's a modifier effect, otherwise move along to the next
          effect_cost = effect_cost_advanced(effect)
      ...
      if total_effect_cost > 0 then
        ...
        -- MODIFIERS: replace this check with the one that checks for non-modifier effects only
        if #effect_db == 1 then
        ...
        -- MODIFIERS: modifier checks should go here
        ...
        -- MODIFIERS: save "cost_mod" and "chance_mod"
        local cost_mod = 1
        local chance_mod = 1
        tes3.player.data.motte_spell_storage[spell_id] = {cost = spell_cost, skill_table = magic_skill_table2, cost_mod = cost_mod, chance_mod = chance_mod}
      end
...
function count_non_modifier_effects (spell)
  local count = 0
  local mod_count = 0
  local non_mod_count = 0
  for j, effect in ipairs(spell.effects) do
    if effect.object then
      count = count + 1
  ...
  non_mod_count = count - mod_count
  return non_mod_count
end

function detect_all_modifier_effects (effect_array)
  local modifiers = {}
  local seen = {}
...
function spell_cost_advanced (effect_array, cost_array)
  ...
  print("here go non-modifier effect ids only:")
  ...
  for i=1, #non_modifier_effect_array do
    ...
    if spell_table[non_modifier_effect_array[i].id] then
      table = spell_table[non_modifier_effect_array[i].id]
      -- basic
      if table.mag_pow then
        mag_pow = table.mag_pow
      end
      ...
      if (table.range0_const_cost and non_modifier_effect_array[i].rangeType == 0) or table.const_cost or (non_modifier_effect_array[i].id == 10 and non_modifier_effect_array[i].rangeType ~= 0) then
      ...
      if table.ignore_magmin then
        ...
        effect_mag = math.max(math.max(non_modifier_effect_array[i].min, 1) + math.max(non_modifier_effect_array[i].max, 1) + 2 * mag_offset, effect_strength_min)
      end
...
function calculate_cast_chance (spell_cost, willpower, luck, magic_skill)
  ...
  -- fixing the skill values for better progression, I don't like it, but it's better for balancing early/mid game, 45 skill at lvl 1 can make mages too powerful at start
...
local function modConfigReady()
  ...
  category_main_chances:createDropdown{
      ...
      1. WIP - currently the same as 3.
      2. Flatter formula. Flatter curve. Harder at low skill, easier at high skill.
      3. Baseline formula. Recommended and default.
  ...
  category_leveling_general:createOnOffButton{
    ...
    description = [[
      Enabling this allows for skills to progress over 100.

      Default: Off
    ]],
    variable = mwse.mcm.createTableVariable{ id = "leveling_uncapped", table = config }
  }
