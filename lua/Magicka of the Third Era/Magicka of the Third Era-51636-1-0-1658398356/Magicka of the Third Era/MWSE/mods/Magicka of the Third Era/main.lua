...
-- Logging mode defines how much mwse logging will be used, set to 3 for spam mode, or to 0 for silent mode, or anything in between, 1 is fine.
...
241, 242, 243, 244, 245, 246, 247, 248, 249, 250, 251, 293, 294, 295, 296, 297, 298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310}
...
local premade_spells = {
  ...
  HealingTouch_SP_uniq = {fixed_cost = 14},
  -- mod's unique spells
  a_ve_uniqsp_01_lifetap = {fixed_cost = 0.05}, -- restore magicka for a price
  a_ve_uniqsp_02_dragonbite = {flat_mult = 0.8},
  ...
  a_ve_uniqsp_05_fireball = {flat_mult = 0.85}, -- showcase of a basic synergy that's even cheaper
  a_ve_uniqsp_06_burden = {fixed_cost = 28}, -- strongest burden spell
  ...
  a_ve_uniqsp_21_mehrunesprot = {fixed_cost = 28},
  ...
  a_ve_uniqsp_31_desperateprayer = {flat_mult = 0.7}, -- strongest instant self heal, requires Temple rank
  ...
  a_ve_uniqsp_34_hardenbone = {fixed_cost = 20, skill_table = {[0] = 0, [1] = 0.25, [2] = 0.25, [3] = 0, [4] = 0, [5] = 0.5}}, -- designed to buff undead summons, but can be used as bad damage spell, hence the split
  a_ve_uniqsp_35_livingbomb = {fixed_cost = 18, skill_table = {[0] = 0.5, [1] = 0.25, [2] = 0.25, [3] = 0, [4] = 0, [5] = 0}}, -- alteration buff / bad damage spell
...
-- Burden
spell_table[7] = {coef = 0.012, mag_pow = 0.7, dur_pow = 0.4, area_pow = 0.08, range2_coef_mod = 1.3}
-- Feather
...
-- Fire Damage --
-- Cheap, best for AOE
spell_table[14] = {coef = 0.55, mag_pow = 0.71, dur_pow = -0.21, area_pow = 0.09, range2_coef_mod = 1.4}
-- Shock Damage --
-- Expensive, ok for AOE and ranged
spell_table[15] = {coef = 0.65, mag_pow = 0.72, dur_pow = -0.14, area_pow = 0.13, range2_coef_mod = 1.4}
-- Frost Damage --
-- Cheap for touch spells, expensive for ranged
spell_table[16] = {coef = 0.53, mag_pow = 0.715, dur_pow = -0.18, area_pow = 0.17, range2_coef_mod = 1.55}
-- Poison Damage --
-- Cheap, best for DoTs
spell_table[27] = {coef = 0.57, mag_pow = 0.715, dur_pow = -0.26, area_pow = 0.13, range2_coef_mod = 1.4}
-- Damage Health --
-- Expensive, ok for DoTs and touch spells
spell_table[23] = {coef = 0.63, mag_pow = 0.725, dur_pow = -0.2, area_pow = 0.215, range2_coef_mod = 1.5}
-- Drain Attribute
...
-- 10s = 9
-- 30s = 23
spell_table[46] = {coef = 0.75, mag_pow = 0.78, area_pow = 0.08, range2_coef_mod = 1.3}
-- Blind
...
-- 20, 30s = 15
-- 44, 30s = 27
spell_table[48] = {coef = 0.4, mag_pow = 0.7, dur_pow = -0.3, range2_coef_mod = 1.3}
-- Calm Humanoid
...
spell_table[86] = {coef = 0.87, mag_pow = 0.725, dur_pow = -0.25, area_pow = 0.25, range2_coef_mod = 1.45}
-- Absorb Magicka
...
-- Restore Health
spell_table[75] = {coef = 0.5, mag_pow = 0.71, dur_pow = -0.225, area_pow = 0.07, range1_coef_mod = 0.65, range1_dur = -0.4, range2_coef_mod = 0.7, range2_dur = -0.4}
-- Restore Magicka
spell_table[76] = {const_cost = 25}
-- Restore Fatigue
spell_table[77] = {coef = 0.35, mag_pow = 0.7, dur_pow = -0.335, area_pow = 0.08, range1_coef_mod = 0.55, range1_dur = -0.45, range2_coef_mod = 0.6, range2_dur = -0.45}
-- Restore Skill
...
-- 100, 60s = 25
-- also good for buffs
spell_table[82] = {coef = 0.24, mag_pow = 0.7, dur_pow = -0.47, dur_min = 20, range1_coef_mod = 0.7, range1_dur = -0.55, range2_coef_mod = 0.7, range2_dur = -0.55, ignore_magmin = true}
-- Fortify Skill
...
local function exp_gain(e)
  ...
  if tes3.player.data.motte_spell_storage[spell_id] then
    ...
    caster:exerciseSkill(school, cost / base_const * config.leveling_rate_global / 100)
  end  
end
