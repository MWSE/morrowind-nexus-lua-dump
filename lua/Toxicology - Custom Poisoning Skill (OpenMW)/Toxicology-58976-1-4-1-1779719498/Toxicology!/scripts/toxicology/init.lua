...
local function getClassSpecializationBonus()
    ...
    if classRec and classRec.specialization == 'stealth' then
...
local function registerClassBonusModifier()
    ...
    I.SkillFramework.registerDynamicModifier(SKILL_ID, 'Toxicology_ClassSpecializationBonus', getClassSpecializationBonus)
...
local function migrateLegacyClassBonus()
    ...
    -- Older builds could permanently add the class bonus to stat.base.
    -- Remove that baked-in bonus once and let Skill Framework's dynamic
    -- modifier supply it instead, avoiding red/locked custom skills.
    if classBonusApplied and classBonusMode == 'base' then
        local stat = I.SkillFramework.getSkillStat(SKILL_ID)
        if stat then
            stat.base = math.max(0, (stat.base or 0) - config.classBonus)
        end
    end
