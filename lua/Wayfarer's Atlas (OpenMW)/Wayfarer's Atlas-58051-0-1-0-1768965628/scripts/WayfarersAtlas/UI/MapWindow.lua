...
-- Calculates the bounds the image can be moved until it hits the edge of the window content.
function MapWindow:_calcImageBounds()
	...
	return -wiggleRoom / 2, wiggleRoom / 2
end

-- Calculates the max bounds the image can comfortably be moved until it's no longer visible.
function MapWindow:_calcImageBoundsMax()
	local winSize = self._window:getProps().size
	local min, max = self:_calcImageBounds()

	return min - winSize / 2, max + winSize / 2
end
...
function MapWindow:_initWindow()
	...
	window:registerEvent("custom_resized", function()
		self._refitPending = true
		self:queueUpdate()
	end)
...
function MapWindow:_initImage()
	...
	self._notesContainer = image:addChild(UIObject.bind({
		props = { relativeSize = v2(1, 1) },
	}))

	local visibleToggle = self._windowContentRoot:addChild(UIObject.bind({
		template = I.MWUI.templates.borders,
		name = "visibleToggle",
		type = UI.TYPE.Image,
		props = {
			resource = UI.texture({ path = "icons/WayfarersAtlas/eye-square.dds" }),
			size = v2(32, 32),
			anchor = v2(1, 1),
			position = v2(-4, -4),
			relativePosition = v2(1, 1),
			color = Utils.rgb(202, 165, 96),
		},
	}))

	local function setVisible(visible)
		self._notesContainer:setVisible(visible)
		UIContext.saveData.notesVisible = self._notesContainer:isVisible()

		if self._notesContainer:isVisible() then
			visibleToggle:getProps().color = Utils.rgb(202, 165, 96)
		else
			visibleToggle:getProps().color = Utils.rgb(80, 80, 80)
		end

		self._windowContentRoot:queueUpdate()
	end

	visibleToggle:registerEvent("mousePress", function(e)
		if e.button == SharedUI.MouseButton.Left and not self._notePrompting then
			Ambient.playSound("menu click")
			setVisible(not self._notesContainer:isVisible())
		end
	end)

	setVisible(UIContext.saveData.notesVisible)
...
function MapWindow:onUpdate()
	...
	local imageAssetSize = self._map.imageSize
	local newImageSize = imageAssetSize * self._map.zoom

	self._image:getProps().size = newImageSize

	-- Force map image to be within bounds.
	if self._refitPending then
		self._refitPending = false

		self:_patchMap({
			imageOffset = Utils.v2clamp(self._map.imageOffset, self:_calcImageBoundsMax()),
		})
	end

	self._image:getProps().position = self._map.imageOffset + self._imageScrollOffset
...
function MapWindow:_updateMap()
	...
	Utils.remove(self._notesContainer:getContent(), "notesLayer")

	self._notesLayer = self._notesContainer:addChildName(
		"notesLayer",
		NotesLayer.new({
...
function MapWindow:render(props)
	...
	if not lastProps or (lastProps.windowSaveId ~= props.windowSaveId) then
		self._window:setSaveId(props.windowSaveId)
	end
