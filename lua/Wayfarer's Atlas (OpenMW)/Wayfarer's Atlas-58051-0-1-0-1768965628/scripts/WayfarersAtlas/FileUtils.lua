...
function FileUtils.withoutFileExtension(filePath)
	local fileExtension = FileUtils.fileExtension(filePath)
	if fileExtension then
		return filePath:sub(1, #filePath - (#fileExtension + 1))
	else
		return filePath
	end
end

function FileUtils.fileExtension(filePath)
	return filePath:match("%.([^%.]+)$")
end

---@param path string
function FileUtils.fileName(path)
	return FileUtils.withoutFileExtension(path):match("([^/\\]+)$")
end
