...
local function fsGetMaps()
	...
	for filePath in VFS.pathsWithPrefix(MAC_MAPS_PATH) do
		...
		local isUnderDir = pathRelMapPacksDir:find("^[^/]+/") ~= nil

		-- A map pack must be a single directory containing "maps.lua".
		if not isUnderDir or containsSubDir or not pathRelMapPacksDir:match("(maps%.lua)$") then
			goto continue
		end
		...
		local withoutExt = FileUtils.withoutFileExtension(filePath)
		...
		for key, map in pairs(externalMapPack) do
			...
			mapFileExt = FileUtils.fileExtension(map.path:lower())
