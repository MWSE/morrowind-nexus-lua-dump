...
local DEFAULT_WINDOW_SIZE = OMWUtil.vector2(700, 500)

local MapWindowUI = MapWindow.new()
local ModStorage = StorageData.new(DataVersions.multisaveData)
...
local function loadMultisaveData()
	...
	local saveData = ModStorage:getSave()
...
local function setupUIContext()
	...
	UIContext.getScreenSpace = function()
		return UI.screenSize() / ModStorage:getConfig().f_GUIScale
	end
	UIContext.saveData = Dictionary.merge(UIContext.saveData, ModStorage:getSave().UISaveData)
end
...
local function onFrame()
	...
	if updateFlag then
		...
		elseif windowMode == "large" or windowMode == "custom" then
			...
			local newWindowSize = screenSpace * 0.8

			MapWindowUI:render(Dictionary.merge(makeProps(), {
				windowOffset = (screenSpace / 2) - (newWindowSize / 2),
				windowSize = newWindowSize,
				windowSaveId = "mapWindowLarge",
			}))
