...
---@param updaters table?
function ModStorage.new(updaters)
...
function ModStorage:getSave()
	...
	data = Dictionary.merge(defaultSave, data)

	if self._updaters then
		data = self:updateVersions(self._updaters, data)
	end
