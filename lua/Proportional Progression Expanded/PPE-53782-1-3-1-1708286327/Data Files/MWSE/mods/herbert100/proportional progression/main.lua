...
local function update_variables()
	...
	local should_register = global_scale_factor ~= 1 or lvl_enabled or skill_lvl_enabled
	for k,v in pairs(config) do
		if type(v) == "number" and not keys_to_ignore[k] and v ~= 1 then
			log:trace("setting should_register = true, because %s ~= 1", k)
			should_register = true
			break
		end
	end
	if should_register then
		log("registering exerciseSkill event")
	else
		log("unregistering exerciseSkill event")
	end
