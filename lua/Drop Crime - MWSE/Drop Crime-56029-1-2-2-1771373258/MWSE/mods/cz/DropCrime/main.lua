...
local function triggerCrime(item)
    if isDwemer(item) or isEbony(item) or isGlass(item) or isContraband(item) then
        local witnesses = {}
                    ...
                    if not isCompanion(ref) then
                        if isGuard(ref) then
                            if tes3.testLineOfSight { reference1 = ref, reference2 = tes3.player } then
                                if not isKhajiit(ref.object) or not isSkooma(item) then   -- khajiit shouldn't care if you grab it
        ...
        if #witnesses > 0 then
            timer.frame.delayOneFrame(
                function()
                    tes3.triggerCrime({ type = tes3.crimeType.theft, victim = witnesses[1], value = item.value })
                end
            )
        end
    end
...
local function onItemPickup(e)
    ...
    then return
    elseif pickedItem.objectType == tes3.objectType.book        --it's not a crime to read!
    then
        currentBook = pickedItem
        return
    end

    triggerCrime(pickedItem)
end

--- @param e uiActivatedEventData
local function onBookPickup(e)
    local element = e.element
    local takebutton = element:findChild('MenuBook_button_take')
	if (takebutton ~= nil) then
		takebutton:registerBefore(
            'mouseClick',
            function()
                if currentBook ~= nil then triggerCrime(currentBook) end
            end
        )
	end
end
...
local function onInitialized()
    ...
    event.register("uiActivated", onBookPickup, { filter = "MenuBook" })
    mwse.log("[Drop Crime] initialized")
end
event.register("initialized", onInitialized)
event.register("modConfigReady", function() require("cz.DropCrime.mcm") end)