...
local function isSmuggler(actor)
    ...
    if actor.class.id == "Smuggler" or actor.class.id == "Necromancer" or (actor.faction and (actor.faction.id == "Thieves Guild" or actor.faction.id == "Camonna Tong" or actor.faction.id == "Ashlanders" or actor.faction.id == "Telvanni")) then
        return true
    end
    return false
end
    ...
    if isDwemer(droppedItem) or isEbony(droppedItem) or isGlass(droppedItem) or isContraband(droppedItem) then
        ...
        if #witnesses > 0 then
            timer.frame.delayOneFrame(
                function()
                    tes3.triggerCrime({ type = tes3.crimeType.theft, victim = witnesses[0], value = droppedItem.value })
                end
            )
        end
    end
...
local function onItemPickup(e)
    ...
    if isDwemer(pickedItem) or isEbony(pickedItem) or isGlass(pickedItem) or isContraband(pickedItem) then
        ...
        if #witnesses > 0 then
            timer.frame.delayOneFrame(
                function()
                    tes3.triggerCrime({ type = tes3.crimeType.theft, victim = witnesses[0], value = pickedItem.value })
                end
            )
        end
    end
