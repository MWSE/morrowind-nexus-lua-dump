local mod = { name = "Infectious Sixth House", ver = "1.0" }
local cf = require("AU.Infectious Sixth House.config")
...
local function getCells()
	...
	for _, cell in pairs(tes3.dataHandler.nonDynamicData.cells) do
			if 	(
					(
					string.find(cell.id:lower(), "dagoth ur") or
					string.find(cell.id:lower(), "endusal") or
					string.find(cell.id:lower(), "kogoruhn") or
					string.find(cell.id:lower(), "mamaea") or
					string.find(cell.id:lower(), "odrosal") or
					string.find(cell.id:lower(), "tureynulal") or
					string.find(cell.id:lower(), "vemynal") or

					string.find(cell.id:lower(), "abinabi") or
					string.find(cell.id:lower(), "ainab") or
					string.find(cell.id:lower(), "assemanu") or
					string.find(cell.id:lower(), "bensamsi") or
					string.find(cell.id:lower(), "falasmaryon") or
					string.find(cell.id:lower(), "hassour") or
					string.find(cell.id:lower(), "maran-adon") or
					string.find(cell.id:lower(), "missamsi") or
					string.find(cell.id:lower(), "piran") or
					string.find(cell.id:lower(), "rissun") or
					string.find(cell.id:lower(), "salmantu") or
					string.find(cell.id:lower(), "sanit") or
					string.find(cell.id:lower(), "sennananit") or
					string.find(cell.id:lower(), "sharapli") or
					string.find(cell.id:lower(), "subdun") or
					string.find(cell.id:lower(), "telasero") or 
					string.find(cell.id:lower(), "yakin")
					)
				and not
					(
					string.find(cell.id:lower(), "yurt") or
					string.find(cell.id:lower(), "piransulit") or
					string.find(cell.id:lower(), "zainab")
					)
				)
				then
...
local function diseaseMyTimer()
	...
	local resistDisease = tes3.mobilePlayer.resistBlightDisease
...
local function registerModConfig()
	...
	page.sidebar:createInfo{ text = "\nHow mod works?\n\nOnce a Sixth House Base is entered, the player's character must make a first roll, which is then compared to the value given on the \"Chance to get a disease\" slider. If the roll result is lower than the given value, a second roll is made against the character's current Blight Disease Resistance. If the roll result is higher than the resistance value, then the character becomes infected with a random disease from \"Catchable Diseases\" table. If the player's character is submerged in water, their chance of contracting a disease is higher." }
	...
	subcat:createSlider{
		...
		description = "You can choose how much time (in seconds) will elapse before the script tries once more to infect you. [Default: 60]",
		min = 0,
		max = 60,
		step = 1,
		jump = 1,
	...
	subcat:createSlider{
		label = "Chance: " .. "%s%%",
		description = "Here you can choose the odds to get a disease by wandering the Sixth House Bases. [Default:5]",
		min = 0,
		max = 100,
		step = 1,
		jump = 10,
	...
	template:createExclusionsPage{
		...
		description = "Here you can choose which disease you can contract by wandering the Sixth House Bases.",
