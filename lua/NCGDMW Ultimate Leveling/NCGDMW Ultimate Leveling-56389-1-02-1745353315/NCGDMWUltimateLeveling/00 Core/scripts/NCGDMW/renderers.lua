...
I.Settings.registerRenderer(mDef.renderers.logRange, function(value, set, argument)
    ...
    return disable(argument.disabled, {
        ...
        content = ui.content {
            {
                type = ui.TYPE.Flex,
                props = { horizontal = true, size = util.vector2(200, 0) },
                content = ui.content {
                    growingInterval,
            ...
            },
            {
                ...
                content = ui.content {
                    growingInterval,
                    textBox(L("rangeTest", {
                        factor = lastRangeTestValue == math.huge and "--" or string.format("%.1f", 100 * mDef.formulas.getLogRangeFactor(lastRangeTestValue, value[1], value[2]))
                    }), rangeLineHeight),
                    padding(5, 0),
                    paddedBox({
                        template = I.MWUI.templates.textEditLine,
...
for _, stat in pairs(core.stats.Skill.records) do
    table.insert(skillItems, { key = stat.id, name = stat.name, value = 100 })
end
for _, stat in pairs(core.stats.Attribute.records) do
    table.insert(attributeItems, { key = stat.id, name = stat.name, value = 1000 })
end

registerMultiSelectNumberRenderer(mDef.renderers.per_skill_uncapper, skillItems)
registerMultiSelectNumberRenderer(mDef.renderers.per_attribute_uncapper, attributeItems)
