...
local function getHealthFactor(state)
    ...
    for attrId, value in pairs(mCfg.healthAttributeFactors) do
        attrFactor = attrFactor + state.healthAttrs.health[attrId] * value
    end
    return attrFactor
end
...
local function getRetroactiveHealthFactor(state)
    ...
    for attrId, value in pairs(mCfg.retroactiveHealthAttributeFactors) do
        attrFactor = attrFactor + state.healthAttrs.retroactive[attrId] * value
        initialAttrFactor = initialAttrFactor + state.healthAttrs.start[attrId] * value
    end
...
local function setChargenStats(state)
    ...
    --state.chargen.favoredAttributes = playerClass.attributes
    local attributes = {}
    local raceAttributes = {}
    for attrId, value in pairs(playerRace.attributes) do
        raceAttributes[attrId] = (playerRecord.isMale and value.male or value.female)
        attributes[attrId] = raceAttributes[attrId]
    end
    state.attrs.race = raceAttributes
    state.attrs.favored = {}
    for _, attrId in ipairs(playerClass.attributes) do
        state.attrs.favored[attrId] = true
        attributes[attrId] = attributes[attrId] + 10
    end
    ...
    state.skills.room.major = roomMajor
    local roomMinor = 0
    state.skills.minor = {}
    ...
    state.skills.room.minor = roomMinor
    local roomMisc = 0
    state.skills.misc = {}
    ...
    state.skills.room.misc = roomMisc
