...
local menuWidth = mSettings.globalStorage:get("statsMenuWidth") or 1100
...
local function getMenuWidth()
    return mSettings.globalStorage:get("statsMenuWidth") or 1100
end
...
local function getStatsMenu(state)
    ...
    mHelpers.insertMultipleInArray(centerLeftHeadBlock, {
        ...
        vGap05,
        row(L("startAttributesPenalty_name"), tostring(mSettings.gameStartStorage:get("startAttributesPenalty"))),
    ...
    if mSettings.skillsStorage:get("capSkillTrainingLevel") then
        table.insert(rightCenterHeadBlock, {
            ...
            content = ui.content {
                ...
                text(tostring(state.skills.trainingSessions)),
            }
        })
    end
    ...
    for _, attrId in ipairs(state.attrs.favoredOrder) do
        table.insert(specFavBlock, text(mCommon.getStatName("attributes", attrId)))
    end
        ...
        return {
            headRow(skillTypeName,
                    L("skillsProgressHead")
                            ...
                            .. (hasSkillBonus and "-" .. L("skillsBonusHead") or "")
                            .. "-" .. L("statsStartHead")
...
local function onKeyPress(key)
    ...
    if mSettings.globalStorage:get("statsMenuToggle") == true and key.code == mSettings.globalStorage:get("statsMenuKey") and ncgdStatsMenu ~= nil then
        closeMenu()
        return
    end
...
local function onKeyRelease(key)
    if mSettings.globalStorage:get("statsMenuToggle") == false and key.code == mSettings.globalStorage:get("statsMenuKey") then
        closeMenu()
    end
end
