...
local function setSkillGrowths(state, skillId, skillValue)
    ...
    state.skills.calculate.level[skillId] = state.skills.growth[skillId]
    ...
    state.skills.calculate.attributes[skillId] = skillValue
...
local function capTrainedSkills(state, uiData)
    ...
    if uiData.newMode == "Training" then
        table.insert(messages, L("skillTrainingCap", { trainingSessions = tostring(state.skills.trainingSessions) }))
...
local function skillBooks(state, uiData)
    if uiData.newMode == "Book" then
        ...
        if bookRecord.skill then
            ...
            local maxBook = mSettings.skillsStorage:get("skillBooksMax") or 5
            local bookGain = mSettings.skillsStorage:get("skillBooksExpValue") or 4
            local skillGain = mSettings.skillsStorage:get("skillBooksSkillupValue") or 1
            if state.booksRead[bookRecord.id] then
            ...
            elseif mSettings.skillsStorage:get("skillBooksExp") then
                ...
                state.booksRead[bookRecord.id] = true
            ...
            mCommon.modStat(state, "skills", skillId, skillGain)
            ambient.playSound("skillraise")
        --[[elseif bookRecord == "bk_PathOfTheAdventurer" then
            
        elseif not state.booksRead[bookRecord.id] then
            if mSettings.globalStorage:get("trackBooksInventory") and uiData.oldMode ~= "Interface" then
                return
            end
            state.booksRead[bookRecord.id] = true--]]
        end
...
local function addSkillUsedHandlers(state)
    ...
    I.SkillProgression.addSkillLevelUpHandler(function(skillId, source)
        ...
        if source == I.SkillProgression.SKILL_INCREASE_SOURCES.Trainer then
            if mSettings.skillsStorage:get("capSkillTrainingLevel") and state.skills.trainingSessions == 0 then
                ...
                state.skills.trainingUsed = state.skills.trainingUsed + 1
            end
        end
        local skillGain = mSettings.skillsStorage:get("skillTrainingValue")
        self:sendEvent(mDef.events.updateGrowthAllAttrs)
        mCommon.modStat(state, "skills", skillId, skillGain, { details = details })
        ambient.playSound("skillraise")
        return false
    end)
end
