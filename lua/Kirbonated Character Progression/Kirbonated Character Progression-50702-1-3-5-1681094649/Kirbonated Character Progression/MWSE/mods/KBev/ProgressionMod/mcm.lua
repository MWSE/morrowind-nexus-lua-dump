...
local function registerModConfig(e)
	...
	xpFeatures:createOnOffButton{
		label = "Enable XP for Freeing Slaves?",
		description = "(Default ON) When turned on, defeating a owner of a certain legendary ring will award the player a large amount of XP.",
		variable = mwse.mcm.createTableVariable{
			id = "slaveXPEnabled",
			table = config,
	...
	xpFeatures:createOnOffButton{
		label = "Allow natural Skill Raise?",
	...
	xpRewards:createTextField{
		label = "Slave Liberation XP",
		description = "(Default 5)controls the amount of XP granted for defeating Fargoth",
		numbersOnly = true,
		variable = mwse.mcm.createTableVariable{
			id = "slaveXP",
			table = config,
