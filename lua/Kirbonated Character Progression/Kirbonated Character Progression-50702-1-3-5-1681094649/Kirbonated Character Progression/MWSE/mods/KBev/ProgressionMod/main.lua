...
local gofreeTopic = tes3.findDialogue{topic = "go free"}

--list of correct dialog reponses (by dialog ID) for succesfully freeing slaves
--number assigned to each topic directly corresponds to the "FreedSlavesCounter" variable
--already includes topics from Tamriel Rebuilt version 23.02.03
--slaves from other mods will work if they use vanilla infos, otherwise their infos need to registered
local freeResponses = {
	"2112269131757221074" = 10,
	"2615847683142510678" = 1,
	"981612079163648130" = 1,
	"57342934926223481" = 1,
	"16156231992154126317" = 1,
	"2690722498415121091" = 1,
	"301422693189712236" = 1,
	"50225287226724668" = 1,
	"835282662610315780" = 1,
	"216212375352111027" = 1,
	"324911518962131686" = 1,
...
--[[Custom Event: KCP:registerFreeResponse
registers an info response to be counted by the slave freed XP system. This shouldn't need to be done unless a mod uses a custom response for freeing a slave
]]
local function registerFreeResponse(e)
	freeResponses[e.id] = [e.slavesFreed]
end
event.register("KCP:registerFreeResponse", registerFreeResponse)
...
local function onSkillRaised(e)
	...
	if (e.source == tes3.skillRaiseSource.book) and (mcm.skillBookXPEnabled) then
		giveXP({message = "Skill Book Read", amount = mcm.bkSklXP})
	...
	if (e.source == tes3.skillRaiseSource.training) and (mcm.trainerXPEnabled) then
		giveXP({message = "Skill Trained", amount = mcm.trnSklXP})
		if (mcm.blockSkillRaise) then
			tes3.modStatistic({
				reference = tes3.player,
				skill = e.skill,
	...
	if (e.source == tes3.skillRaiseSource.progress) and (mcm.exerciseXPEnabled) then
		...
		giveXP({message = "Skill Exercised", amount = iXP})
		if (mcm.blockSkillRaise) then
			tes3.modStatistic({
				reference = tes3.player,
				skill = e.skill,
...
local function onDeath(e)
	...
	if mcm.fargothXPEnabled and e.reference.baseObject.id == "fargoth" then
		giveXP({message = "Hrisskar smiles upon you", amount = mcm.fargothXP})
	elseif mcm.bossXPEnabled and common.bossMonsters[e.reference.baseObject.id] then 
		giveXP({message = "Boss Defeated", amount = mcm.bossXP})
	end
end
...
local function onCellChanged(e)
	...
	if e.cell.id and not framework.playerData.getCellVisited(e.cell.id) then
		giveXP({message = "Location Discovered: " .. e.cell.id, amount = mcm.cellXP})
...
--slave freed detection
local function onSlaveFreed(e)
	if not freeResponses[e.info.id] then return end
	giveXP({message = "Slave Freed", amount = (mcm.slaveXP * freeResponses[e.info.id])})
end
event.register(tes3.event.infoResponse, onSlaveFreed)