...
KBProgression = include("KBev.ProgressionMod.interop")
...
local function calcArmorMasterEffects()
	...
	if tes3.mobilePlayer.readiedShield then blockMasterActive = true end
...
local function onLoaded(e)
	if not tes3.player.data.kb_basicPerks then 
		tes3.player.data.kb_basicPerks = { armorWeights = {},}
	end
...
local function onHitHandToHand(e)
	...
	if perks.handToHandMaster.activated and (e.source ~= tes3.damageSource.script) then
...
local function onPerkActivated(e)
	if (e.perk == "kb_perk_rigorousTraining") then
		KBProgression.playerData.modIncPoints({typ = "atr", mod = 3})
	end
	if (e.perk == "kb_perk_lucky") then
		tes3.modStatistic({reference = tes3.player, attribute = tes3.attribute.luck, value = 20})
	end
	if (e.perk == "kb_perk_jackOfAllTrades") then
		KBProgression.playerData.modIncPoints({typ = "msc", mod = 5})
	end
	if (e.perk == "kb_perk_expertise") then
		KBProgression.playerData.modIncPoints({typ = "mjr", mod = 5})
	end
	if (e.perk == "kb_perk_wellRounded") then
		KBProgression.playerData.modIncPoints({typ = "mnr", mod = 5})
	end
	if (e.perk == "kb_perk_lightArmorMaster") or (e.perk == "kb_perk_mediumArmorMaster") or (e.perk == "kb_perk_heavyArmorMaster") then
		updateArmorMasterEffects()
	end
end
event.register("KBProgression:perkActivated", onPerkActivated)
...
local function registerPerks()
	...
	perks.rigorousTraining = KBProgression.perk.createPerk({
	...
	perks.lucky = KBProgression.perk.createPerk({
		id = "kb_perk_lucky",
		name = "Lucky",
		description = "Your Luck increases by 20 points"
	})
	perks.magickaWell = KBProgression.perk.createPerk({
	...
	perks.magickaWell2 = KBProgression.perk.createPerk({
		id = "kb_perk_magickaWell2",
		name = "Magicka Reservoir",
		description = "Your Maximum Magicka increases by 0.5x Intelligence",
		lvlReq = 4,
		attributeReq = {intelligence = 70},
		perkReq = {"kb_perk_magickaWell"},
	...
	perks.jackOfAllTrades = KBProgression.perk.createPerk({
	...
	perks.expertise = KBProgression.perk.createPerk({
	...
	perks.wellRounded = KBProgression.perk.createPerk({
	...
	perks.favoredBeast = KBProgression.perk.createPerk({
	...
	perks.favoredDaedra = KBProgression.perk.createPerk({
	...
	perks.favoredNPC = KBProgression.perk.createPerk({
	...
	perks.favoredUndead = KBProgression.perk.createPerk({
	...
	perks.favoredConstruct = KBProgression.perk.createPerk({
	...
	perks.favoredEnemy2 = KBProgression.perk.createPerk({
	...
	perks.daeMephala = KBProgression.perk.createPerk({
	...
	--[Block perks]
	--As much as I wanted to make some cool block perks, I'm heavily limited by the fact that Block chance calculations are hidden from LUA right now.
	perks.blockMaster = KBProgression.perk.createPerk({
	...
	perks.bulwark = KBProgression.perk.createPerk({
		id = "kb_perk_bulwark",
		name = "Bulwark",
		description = "Succesfully blocking an attack grants you a temporary sanctuary effect equal to half your block skill for 5 seconds",
		lvlReq = 4,
		skillReq = {block = 30}
	})
	--[Armorer Perks]
	--[Medium Armor Perks]
	perks.mediumArmorMaster = KBProgression.perk.createPerk({
	...
	--[Heavy Armor Perks]
	perks.heavyArmorMaster = KBProgression.perk.createPerk({
	...
	--[Blunt Weapon Perks]
	perks.exhaustingBlows = KBProgression.perk.createPerk({
		id = "kb_perk_exhaustingBlows",
		name = "Exhausting Blows",
		description = "Succesful attacks with Blunt weapons damage the actors fatigue in addition to their health",
		skillReq = {bluntWeapon = 25},
		hideInMenu = true
	})
	--[Long Blade Perks]

	perks.duelist = KBProgression.perk.createPerk({
		id = "kb_perk_duelist",
		name = "Duelist",
	...
	--[Axe Perks])
	perks.treeFeller = KBProgression.perk.createPerk({
	...
	--[Spear Perks]
	perks.powerfulLunge = KBProgression.perk.createPerk({
	...
	--[Athletics Perks]
	--[Enchant Perks]
	perks.soulSiphon = KBProgression.perk.createPerk({
	...
	perks.soulSiphon2 = KBProgression.perk.createPerk({
	...
	perks.soulDistributor = KBProgression.perk.createPerk({
	...
	perks.luckyEnchant = KBProgression.perk.createPerk({
	...
	--[Alteration Perks]
	perks.trapWeaver = KBProgression.perk.createPerk({
	...
	perks.elementalWarding = KBProgression.perk.createPerk({
	...
	--[Conjuration Perks]
	--[Mysticism Perks]
	perks.bloodMage = KBProgression.perk.createPerk({
	...
	perks.hemomancer = KBProgression.perk.createPerk({
	...
	perks.mysticismMaster = KBProgression.perk.createPerk({
	...
	--[Unarmored Perks]
	perks.unarmoredMaster = KBProgression.perk.createPerk({
	...
	--[Security Perks]
	perks.reliablePicking = KBProgression.perk.createPerk({
		id = "kb_perk_reliablePicking",
		name = "Reliable Picking",
		description = "Lockpicking automatically succeeds on locks with a difficulty lower than your security skill",
		lvlReq = 12,
		skillReq = {security = 40},
	})
	perks.luckySecurity = KBProgression.perk.createPerk({
	...
	--[Acrobatics Perks]
	--[Light Armor Perks]
	perks.lightArmorMaster = KBProgression.perk.createPerk({ --is this overpowered? maybe. Getting to 100 light armor in this mod means sacrificing points from other, arguably more important skills
	...
	--[Short Blade Perks]
	perks.quickStrike = KBProgression.perk.createPerk({
	...
	--[Marksman Perks]
	perks.knifeJuggler = KBProgression.perk.createPerk({
	...
	perks.longShot = KBProgression.perk.createPerk({
		id = "kb_perk_longShot",
		name = "Long Shot",
	...
	--[Speechcraft Perks]
	--[Hand to Hand Perks]
	perks.handToHandMaster = KBProgression.perk.createPerk{
	...
	event.register("playGroup", onPlayerPlayGroup, {filter = tes3.player})
...
event.register("KBProgression:Initialized", registerPerks)