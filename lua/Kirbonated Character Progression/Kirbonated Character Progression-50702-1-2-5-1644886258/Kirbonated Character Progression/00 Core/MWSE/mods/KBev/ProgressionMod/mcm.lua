...
local function registerModConfig(e)
	...
	for file in lfs.dir("Data Files\\MWSE\\config\\KCP Presets\\") do
		common.info("Found Preset: " .. file)
		local path = "config\\KCP Presets\\" .. file
		local content = json.loadfile(path)
		if content then
			common.info("preset registered: " .. content.presetName)
	...
	local perk = menu:createSideBarPage("Perks")
	local pointAlloc = menu:createSideBarPage("Levelling")
	...
	xpFeatures:createOnOffButton{
		...
		description = "(Default ON). As the name implies, this toggles on and off the cell discovery feature of the mod. Has no effect if XP is turned off",
		variable = mwse.mcm.createTableVariable{
			id = "cellXPEnabled",
			table = config,
			defaultSetting = true
	...
	xpFeatures:createOnOffButton{
		...
		description = "(Default ON) When turned on, defeating certain extra powerful creatures, such as Ash Vampires, The Imperfect, the Udyrfrykte, Dagoth Ur, Vivec, and Almalexia, will reward a large amount of XP upon their death.",
		variable = mwse.mcm.createTableVariable{
			id = "bossXPEnabled",
			table = config,
			defaultSetting = true
	...
	perk:createOnOffButton{
		...
		callback = function() event.trigger("KBProgression:updatePerkState") end,
	}
	...
	presets:createDropdown{
		...
		variable = mwse.mcm:createVariable{
			...
			set = (function(self, newVal)
				common.info("Previous Preset: " .. config.presetName)
				updateConfig(json.loadfile(newVal))
				common.info("Loaded Preset: " .. config.presetName)
	...
	presets:createTextField {
		...
		variable = mwse.mcm:createVariable{
			...
			set = (function(self, newVal)
				...
				common.info("Saving Preset: " .. newVal)
