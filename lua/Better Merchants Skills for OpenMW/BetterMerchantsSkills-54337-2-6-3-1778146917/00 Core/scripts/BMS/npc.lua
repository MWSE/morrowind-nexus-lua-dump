...
local function npcStat(statId)
    return math.floor(state.bases.npc[statId] + mods.npc[statId].mod)
end

local function pcStat(statId)
    return math.floor(state.bases.pc[statId] + mods.pc[statId].mod)
end

local function getMinMaxPriceBarterMercantile(minPercent, maxPercent)
    local pcMerc = math.min(pcStat(Skills.mercantile.id), 100)
    local pcLuck = math.min(0.1 * pcStat(Attributes.luck.id), 10)
    local pcPers = math.min(0.2 * pcStat(Attributes.personality.id), 10)
    local npcLuck = math.min(0.1 * npcStat(Attributes.luck.id), 10)
    local npcPers = math.min(0.2 * npcStat(Attributes.personality.id), 10)

    local common = state.disposition - 50 + pcMerc + pcLuck + pcPers - npcLuck - npcPers
...
local function computeStats()
    ...
    local serviceDifficulty = mStore.settings.serviceDifficulty.get()
    local hagglingDifficulty = mStore.settings.hagglingDifficulty.get()
    local persuasionDifficulty = mStore.settings.persuasionDifficulty.get()
    ...
    local baseServiceSkills = baseSkill * serviceDifficultyPercent + mods.npc.mercantile.mod
    local baseHagglingSkills = baseSkill * hagglingDifficultyPercent + mods.npc.mercantile.mod
    local basePersuasionSkills = baseSkill * persuasionDifficultyPercent + mods.npc.speechcraft.mod
    local capMinSkills = mStore.settings.preventSkillsBelowOriginalValues.get()

    local baseGold, goldBasedMercantile, barterMercantile = 0, 0, 0
    local minMercantile, maxMercantile = 0, 0
    ...
    if hasBarter then
        ...
        minMercantile, maxMercantile = getMinMaxPriceBarterMercantile(
                mStore.settings.minItemSalePricePercent.get() / 100,
                mStore.settings.maxItemSalePricePercent.get() / 100)
        barterMercantile = (baseServiceSkills + goldBasedMercantile) / 2

        local barterDispFactor = mStore.settings.dispositionImpactOnPricesPercent.get() / 100
        barterDispDiff = (1 - barterDispFactor) * (state.disposition - 50)
        barterMercantile = barterMercantile + barterDispDiff

        if barterMercantile < minMercantile then
            state.isMaxPrice = true
            barterMercantile = minMercantile
        elseif barterMercantile > maxMercantile then
            state.isMinPrice = true
            barterMercantile = maxMercantile
        end
    ...
    state.skills.mercantile = {
        [mDef.buffType.service] = math.floor(baseServiceSkills),
        [mDef.buffType.barter] = math.floor(barterMercantile),
        [mDef.buffType.haggling] = math.floor(baseHagglingSkills),
        [mDef.buffType.persuasion] = math.floor(basePersuasionSkills),
    }
    ...
    log(string.format("Level %s, mercantile %d, speechcraft %d\n"
            ...
            baseSkill, serviceDifficultyPercent, mods.npc.mercantile.mod, baseServiceSkills,
...
local function getNewMods(kind)
    local newMods = { npc = mC.getModsCopy(mods.npc), pc = mC.getModsCopy(mods.pc) }
    if kind == mDef.buffType.persuasion then
        mC.addToMod(newMods.npc.mercantile, state.skills.mercantile[kind] - npcStat(Skills.mercantile.id))
        mC.addToMod(newMods.npc.speechcraft, state.skills.speechcraft[kind] - npcStat(Skills.speechcraft.id))
        return newMods
    end
    ...
    -- Set skill mods
    mC.addToMod(newMods.npc.mercantile, npcSkillModDiff)
    mC.addToMod(newMods.pc.mercantile, pcSkillModDiff)

    local npcHagglingModDiff, pcHagglingDispTerm = 0, 0
    if kind == mDef.buffType.barter then
        -- Mods for haggling
        local dispFactor = mStore.settings.dispositionImpactOnHagglingPercent.get() / 100
        pcHagglingDispTerm = mC.GMSTs.fDispositionMod * (state.disposition - 50) * (1 - dispFactor)
        npcHagglingModDiff = state.skills.mercantile[mDef.buffType.haggling] + pcHagglingDispTerm - state.skills.mercantile[kind]
    ...
    -- Set luck mods
    mC.addToMod(newMods.npc.luck, npcLuckModDiff)
    mC.addToMod(newMods.pc.luck, pcLuckModDiff)
...
local function modStats(op)
    log(string.format("Mod stats of type %s, restore=%s, save=%s, compute=%s", op.mod.type, op.restoreStats, op.saveStats, op.computeStats))
    ...
    if op.mod.type == mDef.operationType.buff then
    ...
    elseif op.mod.type == mDef.operationType.restore then
        if mods then
            mC.modStats(mods.npc)
            log("Restored stats")
        else
...
local function modDisp(disp, notify, progDiff)
    T.NPC.setBaseDisposition(self, player, math.floor(disp))
    ...
    newDisp = math.min(100, newDisp + disp % 1)
...
return {
    eventHandlers = {
        ...
        [mDef.events.modDisp] = function(data) modDisp(data.disp, data.notify, data.progDiff) end,
    },
}