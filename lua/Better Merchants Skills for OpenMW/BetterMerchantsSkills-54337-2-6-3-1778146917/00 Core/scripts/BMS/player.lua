...
local function checkOperation()
    ...
    log(string.format("Popped operation of type %s, %s remaining", operation.mod.type, #opQueue))
    local currNpc = npc
    local op = operation
    if operation.mod.type == mDef.operationType.restore then
        mC.modStats(stats.mods.orig)
        if operation.mod.clear then
            if state.dispProgression[npc.id] then
                -- Override the permanent disposition changes option actions
                npc:sendEvent(mDef.events.modDisp, { disp = stats.baseDisp.curr })
            end
            npc = nil
            npcBaseGold = nil
            npcSkills = nil
...
local function initStats()
    ...
    stats.mods.curr = mC.getModsCopy(stats.mods.orig)
...
local function getDispProg(dispDiff, prevProg)
    local prog = 0
    local mercDiff = math.min(100, math.max(-100, npcSkills.mercantile[mDef.buffType.barter] - stats.bases.orig.mercantile - stats.mods.orig.mercantile.mod))
    if dispDiff > 0 then
        if #pcGoldChanges > 1 then
            local goldDiff = pcGoldChanges[1] - pcGoldChanges[2]
            pcGoldChanges = { pcGoldChanges[1] }
            local gainFactor = goldDiff < 0 and mStore.settings.dispScalingMaxBuyGain.get() or mStore.settings.dispScalingMaxSellGain.get()
            local skillFactor = 1 - (mercDiff + 100) / 400
            local goldFactor = math.min(1, math.abs(goldDiff) / math.max(mStore.settings.dispScalingMinBaseGold.get(), npcBaseGold))
        ...
        local lossFactor = mStore.settings.dispScalingMaxLoss.get()
...
local function checkStatsDiff()
    ...
    local modsDiff, modsHasDiff = mC.getModsDiff(stats.mods.curr, newMods)
    local newBases = mC.getBases(self)
    local basesDiff, basesHasDiff = mC.getBasesDiff(stats.bases.curr, newBases)

    -- Don't update disposition on dialogue exit, because OpenMW tries to restore the previous value
    local checkDisp = mDef.supportedUiModes[I.UI.getMode()]
    ...
    if modsHasDiff then
        log(string.format("PC modifiers diff detected: %s (%s -> %s)",
                mC.modsToString(modsDiff), mC.modsToString(stats.mods.curr), mC.modsToString(newMods)))
        -- restore original mods altered by the dynamic change
        mC.applyModsDiff(stats.mods.orig, modsDiff)
        mC.modStats(stats.mods.orig)
        newMods = mC.getModsCopy(stats.mods.orig)
    end
    if basesHasDiff then
        log(string.format("PC base diff detected: %s (%s -> %s)",
                mC.basesToString(basesDiff), mC.basesToString(stats.bases.curr), newBases))
    end
    if baseDispDiff ~= 0 then
        ...
        if operation.mod.kind == mDef.buffType.barter
                and mStore.settings.dispScalingEnabled.get() then
            ...
            if progDiff ~= 0 then
                ...
                npc:sendEvent(mDef.events.modDisp, {
                    ...
                    notify = mStore.settings.dispScalingNotify.get(),
                    progDiff = progDiff,
                })
            end
        end
    ...
    -- Stats not already computed and saved, restore is not possible nor needed
    if #opQueue > 0 and opQueue[1].saveStats then return end
...
local function isUiModeSupported(mode)
    return mDef.supportedUiModes[mode] and mStore.settings[mode .. "UiModeEnabled"].get()
end

local function configureOp(op, newMode)
    if isUiModeSupported(newMode) then
        op.mod = {
            type = mDef.operationType.buff,
            kind = mDef.supportedUiModes[newMode],
            refreshUiMode = uiModesToRefresh[newMode] and newMode or nil,
        }
    else
        op.mod = {
            type = mDef.operationType.restore,
            clear = not newMode,
        }
    end
    if op.mod then
...
local function uiModeChanged(data)
    ...
    elseif data.arg
            ...
            and isUiModeSupported(data.newMode) then
        ...
        if hasSupportedService(npcRecord) then
            ...
            if npcRecord.servicesOffered.Training and mCompat.isQuickTrainUiEnabled() then
                notify(L("quickTrainIncompatibility"))
            end
        end
    end
