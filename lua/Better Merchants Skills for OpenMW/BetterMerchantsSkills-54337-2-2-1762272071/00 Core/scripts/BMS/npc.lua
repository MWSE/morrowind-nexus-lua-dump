...
local function computeStats()
    ...
    if hasBarter then
        ...
        local barterDispFactor = S.storage:get("dispositionImpactOnPricesPercent") / 100
...
local function getNewMods(kind)
    local newMods = { npc = C.copyMap(mods.npc), pc = C.copyMap(mods.pc) }
    if kind == C.buffType.persuasion then
        newMods.npc[Skills.mercantile.id] = C.round(state.skills.mercantile[kind] - npcStat(Skills.mercantile.id))
        newMods.npc[Skills.speechcraft.id] = C.round(state.skills.speechcraft[kind] - npcStat(Skills.speechcraft.id))
        newMods.npc[Attributes.luck.id] = C.round(mods.npc.luck)
        newMods.pc[Skills.mercantile.id] = C.round(mods.pc.mercantile)
        newMods.pc[Skills.speechcraft.id] = C.round(mods.pc.speechcraft)
        newMods.pc[Attributes.luck.id] = C.round(mods.pc.luck)
        return newMods
    end

    newMods.npc[Skills.speechcraft.id] = C.round(mods.npc.speechcraft)
    newMods.pc[Skills.speechcraft.id] = C.round(mods.pc.speechcraft)
    ...
    -- Set skill mods
    newMods.npc[Skills.mercantile.id] = C.round(mods.npc.mercantile + npcSkillModDiff)
    newMods.pc[Skills.mercantile.id] = C.round(mods.pc.mercantile + pcSkillModDiff)
    ...
    -- Set luck mods
    newMods.npc[Attributes.luck.id] = C.round(mods.npc.luck + npcLuckModDiff)
    newMods.pc[Attributes.luck.id] = C.round(mods.pc.luck + pcLuckModDiff)

    C.log(string.format("\"%s\" and player's mods for %s:\n"
...
local function modStats(op)
    ...
    if op.mod.type == C.operationType.buff then
        ...
        player:sendEvent(D.events.modPcStats, newMods.pc)
