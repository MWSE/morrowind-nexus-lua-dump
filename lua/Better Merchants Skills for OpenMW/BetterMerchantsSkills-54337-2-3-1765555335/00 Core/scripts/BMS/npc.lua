...
local function computeStats()
    ...
    if not npcLevel then
        ...
        log(string.format("\"%s\" level override to %d", self.recordId, npcLevel))
    end

    local baseSkill = D.baseSkill(npcLevel)

    local playerLevel = T.Player.stats.level(player).current
    local serviceDifficulty = S.globalStorage:get("serviceDifficulty")
    local hagglingDifficulty = S.globalStorage:get("hagglingDifficulty")
    local persuasionDifficulty = S.globalStorage:get("persuasionDifficulty")
    ...
    local capMinSkills = S.globalStorage:get("preventSkillsBelowOriginalValues")
    ...
    if hasBarter then
        ...
        minPriceMercantile, maxPriceMercantile = getMinMaxPriceBarterMercantile(
                S.globalStorage:get("minItemSalePricePercent") / 100,
                S.globalStorage:get("maxItemSalePricePercent") / 100)
        barterMercantile = (baseServiceSkills + goldBasedMercantile) / 2

        local barterDispFactor = S.globalStorage:get("dispositionImpactOnPricesPercent") / 100
    ...
    log(string.format("\"%s\": Level %s, mercantile %d, speechcraft %d\n"
...
local function getNewMods(kind)
    local newMods = { npc = H.copyMap(mods.npc), pc = H.copyMap(mods.pc) }
    ...
    if kind == C.buffType.barter then
        -- Mods for haggling
        local dispFactor = S.globalStorage:get("dispositionImpactOnHagglingPercent") / 100
        pcHagglingDispTerm = fDispositionMod * (state.disposition - 50) * (1 - dispFactor)
        npcHagglingModDiff = state.skills.mercantile[C.buffType.haggling] + pcHagglingDispTerm - state.skills.mercantile[kind]
    ...
    log(string.format("\"%s\" and player's mods for %s:\n"
...
local function modStats(op)
    ...
    if op.mod.type == C.operationType.buff then
        ...
        C.modStats(self, newMods.npc)
        player:sendEvent(D.events.modPcStats, { pcMods = newMods.pc, npcSkills = state.skills })
    elseif op.mod.type == C.operationType.restore then
        C.modStats(self, mods.npc)
        log(string.format("\"%s\": Restored stats", self.recordId))
        record = nil
        hasBarter = nil
        player = nil
        mods = nil
...
local function modDisp(disp, _player, notify, progDiff)
    ...
    player:sendEvent(D.events.notify, L(localeKey, { npc = record.name, prog = progDiff, disp = newDisp }))
end

return {
    eventHandlers = {
        [D.events.modStats] = modStats,
        [D.events.modDisp] = function(data) modDisp(data.disp, data.player, data.notify, data.progDiff) end,
    },
}