...
local globalKey = "SettingsGlobal" .. mDef.MOD_NAME
local dispScalingKey = "SettingsDispScaling" .. mDef.MOD_NAME
local compatKey = "SettingsCompat" .. mDef.MOD_NAME

local module = {
    globalKey = globalKey,
    globalStorage = storage.globalSection(globalKey),
    dispScalingStorage = storage.globalSection(dispScalingKey),
    compatStorage = storage.globalSection(compatKey),
    difficultySettings = {
...
module.initSettings = function()
    ...
    for uiMode in mH.spairs(mDef.supportedUiModes, function(_, a, b) return a < b end) do
        table.insert(settingGroups[compatKey].settings, {
            key = uiMode .. "UiModeEnabled",
            description = false,
            default = true,
            renderer = "checkbox",
        })
    ...
    I.Settings.registerGroup(settingGroups[compatKey])
end
