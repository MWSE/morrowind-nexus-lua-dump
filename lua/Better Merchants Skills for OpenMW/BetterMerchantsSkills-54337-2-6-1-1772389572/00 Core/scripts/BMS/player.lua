...
local function getDispProg(dispDiff, prevProg)
    local prog = 0
    local mercDiff = math.min(100, math.max(-100, npcSkills.mercantile[mC.buffType.barter] - stats.bases.orig.mercantile - stats.mods.orig.mercantile.mod))
    if dispDiff > 0 then
...
local function configureOp(op, newMode)
    if isUiModeSupported(newMode) then
        op.mod = {
            ...
            kind = newMode == "Barter" and mC.buffType.barter or mC.buffType.service,
            refreshUiMode = uiModesToRefresh[newMode] and newMode or nil,
        }
    elseif newMode == "Dialogue" then
        op.mod = {
            type = mC.operationType.buff,
...
local function uiModeChanged(data)
    ...
    elseif data.arg
            ...
            and (data.newMode == "Dialogue" or isUiModeSupported(data.newMode)) then
