...
--[[
l LnTDLockData = include('AdituV.DetectTrap.LockData')
...
-- Only valid for in-world objects. For inventory tiles it will always be nil
	...
	if el.text:lower():find(s:lower(), 1, true) then
		return
	end
...
local cellsBlackList = {'corprusarium','galerion','mine'}

---@param cell tes3cell
local function isCellBlacklisted(cell)
	local lcCellEditorName = cell.editorName:lower()
	for i = 1, #cellsBlackList do
		if lcCellEditorName:startswith(cellsBlackList[i]) then
			return true
		end
	end
	return false
...
local targetsBlackList = {'chargen','ladder','invhlp','entrance','practice'}
...
---["dread curse: agility"] = {minPlayerLvl = 1, minLockTier = 0}, -- nope this one is not touch, fixed 2025-07-30
...
---@param casterRef tes3reference
---@param targetRef tes3reference
---@param obj tes3physicalObject|string
---@return tes3reference|nil
...
local function createLeveledCreature(casterRef, targetRef)
	...
	for _ = 1, 10 do
		...
		if levCreaObj -- safety
		and levCreaObj.health >= 10 then
			skip = false
			break
		end
	end
...
local function spellResist(e)
	...
	local lcObjId = lastActivatedRef.object.id:lower()
	local s = 'dae'
	if lcObjId:multifind({'dwrv', 'dwem', 'dwar'}, 1, true) then
		s = 'dwe'
	elseif lcObjId:multifind({'tomb', 'urn'}, 1, true) then
		s = 'tomb'
	end
...
local function find3(s1, s2, s3, needle)
	return s1:find(needle, 1, true)
	or s2:find(needle, 1, true)
	or s3:find(needle, 1, true)
end

local function findObj(obj, needle)
	return find3(obj.id:lower(), obj.mesh:lower(), obj.name:lower(), needle)
end

local function multifind3(s1, s2, s3, pattern)
	return s1:multifind(pattern, 1, true)
	or s2:multifind(pattern, 1, true)
	or s3:multifind(pattern, 1, true)
end

local function multifindObj(obj, pattern)
	return multifind3(obj.id:lower(), obj.mesh:lower(), obj.name:lower(), pattern)
end

---@param activator tes3reference
---@param target tes3reference
local function validActivatorAndTarget(activator, target)
	local funcPrefix = string.format(
		'%s: validActivatorAndTarget("%s", "%s")', modPrefix, activator, target)
	if logLevel4 then
		mwse.log(funcPrefix)
	end
	...
	if (objType == tes3_objectType_container)
	or (objType == tes3_objectType_door) then
		return activator, target
	end
	if logLevel5 then
		local objTypeStr = mwse.longToString(objType)
		mwse.log('%s: target %s "%s" is not a door/container, skip',
			funcPrefix, objTypeStr, targetId)
	end
end

---@param activator tes3reference
---@param target tes3reference
---@param delayActivate boolean|nil
local function process(activator, target, delayActivate)
	...
	local funcPrefix = string.format('%s: process("%s", "%s", %s)',
		modPrefix, activator.id, target.id, delayActivate)
	...
	if cell.restingIsIllegal
	and config.skipRestingIsIllegal then
		if logLevel2 then
	...
	if isContainer then
		...
		if lcObjId:find('corpse', 1, true) then
			if logLevel3 then
	...
	if name then
		blackListed = multifind2(lcObjId, name:lower(), targetsBlackList)
	else
		blackListed = lcObjId:multifind(targetsBlackList)
	end
	if blackListed then
		if logLevel1 then
			mwse.log('%s: target "%s" is blacklisted, skip', funcPrefix, target.id)
		end
		return
	end
			...
			if destCell.isOrBehavesAsExterior then
				if logLevel1 then
					mwse.log('%s: skipping "%s" door loading to exterior "%s"',
						funcPrefix, targetId, destCell.editorName)
				end
				return
			end
			if destCell.id:lower():multifind({'yurt','tent'}, 1, true) then
				if logLevel1 then
					mwse.log('%s: skipping "%s" door to "%s"',
						funcPrefix, targetId, destCell.id)
				end
				return
			end
		end
	...
	local function processContainer()
		...
		elseif multifindObj(targetObj,
				{'urn', 'coffin', 'reliquary', 'sarcophagus'}) then
		...
		elseif findObj(targetObj, 'desk') then
			...
			lockTier = math.random(1, 2)
			if logLevel2 then
				mwse.log('%s: target "%s" desk', funcPrefix, targetId)
			end
		elseif multifindObj(targetObj, {'cabinet','closet','drawer'}) then
			canTrap = config.trapClosets
			canLock = config.lockClosets
			lockTier = 1
			if logLevel2 then
				mwse.log('%s: target "%s" cabinet|closet|drawer', funcPrefix, targetId)
			end
		...
		if lockTier == 0 then
			if multifindObj(targetObj,
				{'chest','barrel','keg','cabinet','coffer','supply'})
			and multifindObj(targetObj, {'dwrv','dwem','dwar','_dw','nordic','tomb','rare','gold','silver',
					'treasure','diamond','gems','smuggler','loot','ayl','daedr'}) then
				canLock = true
				canTrap = true
				lockTier = math.random(1, 3)
				if logLevel2 then
					mwse.log('%s: target "%s" chest|barrel|keg|cabinet|coffer|supply', funcPrefix, targetId)
					mwse.log('%s: target "%s" dwrv|dwem|dwar|_dw|nordic|tomb|rare|gold|silver|treasure|diamond|gems|smuggler|loot|ayl|daedr',
						funcPrefix, targetId)
				end
			end
		end
		...
		if not cell.restingIsIllegal then
			...
			or tes3.hasOwnershipAccess({target = target, reference = activator}) then -- free to use object activated
			 -- look for a free bed
				local culledCells = getCellsCulled(target, 4096)
				local ok = false
					...
					for ref in culledCell:iterateReferences(checkTypes) do
						if ref then
							...
							elseif obj.name then
								local lcName = obj.name:lower()
								if lcName:find('bed', 1, true)
								-- bedrolls are often free regardless
								and ( not lcName:find('bedroll', 1, true) )
								and tes3.hasOwnershipAccess({target = ref}) then
									if logLevel2 then
	...
	elseif lcObjId:find('door', 1, true) then
		if debugLevel >= 1 then
			canLock = true
			canTrap = true
			lockTier = math.random(0, 3)
		elseif lcObjId:multifind({'dwe','dwrv'}, 1, true) then
			canLock = true
			canTrap = true
			lockTier = math.random(1, 3)
		elseif lcObjId:multifind({'cavern_doors00','cavern_doors20'}, 1, true) then
			canLock = true
			canTrap = true
	...
	if not locked then
		if isDoor
		and target.cell.isInterior then
...
local function activate(e)
	...
	local target = e.target
	if not validActivatorAndTarget(activator, target) then
		return
	end
	e.claim = true -- need this ASAP as setting it after process() is too slow
	if logLevel5
	and target.data then
		mwse.log('%s: target.data.ab01mt = %s', funcPrefix, target.data.ab01mt)
	end
	if process(activator, target, true) then
		e.block = true
	else
		e.claim = false
	end
...
local function trapDisarm(e)
	...
	if not validActivatorAndTarget(actorRef, targetRef) then
		return
	end
	e.claim = true
...
lock/trap chances always applied for chests/barrels/rare/precious containers found in Dwemer ruins/tombs.]]
	...
	sideBarPage:createSlider({
		...
		description = 'Chance to lock a suitable door.',
		configKey = 'lockChance',
		min = 0, max = 100, step = 1, jump = 5,
	})
	sideBarPage:createSlider({
		label = 'Container Lock Chance: %s%%',
		description = 'Chance to lock a suitable container.',
		configKey = 'lockContChance',
		min = 0, max = 100, step = 1, jump = 5,
	})
	sideBarPage:createSlider({
		label = 'Door Trap Chance: %s%%',
		description = 'Chance to trap a suitable door.',
		configKey = 'trapChance',
		min = 0, max = 100, step = 1, jump = 5,
	})
	sideBarPage:createSlider({
		label = 'Container Trap Chance: %s%%',
		description = 'Chance to trap a suitable container.',
		configKey = 'trapContChance',
		min = 0, max = 100, step = 1, jump = 5,
	})
	sideBarPage:createSlider({
		label = 'Spawn Chance: %s%%',
		description = 'Chance for non-direct damage traps to spawn a leveled critter.',
		configKey = 'spawnChance',
		min = 0, max = 100, step = 1, jump = 5,
	})
...
..'it will only work after you have activated the container/door at least once.'
	...
	sideBarPage:createDropdown({
		label = 'Leveled traps:',
		options = getOptions(),
		description = 'Options are in descending trap danger/thrill level order.',
		configKey = 'leveledTraps',
	})

	sideBarPage:createSlider({
		label = 'Lock tier step',
		description = [[Lock tier step, used to calculate lock level with formula:
lockLvl = (lockTier + 1) * lockTierStep,
having:
10 <= lockTierStep <= 25,
0 <= lockTier <= 3]],
		configKey = 'lockTierStep',
		min = 10, max = 25, step = 1, jump = 5
	})

	sideBarPage:createYesNoButton({
		label = 'Skip interiors where resting is not allowed',
		description = [[Only process interior cells where you can freely rest (e.g. tombs, caves...)
Note: exterior-like cells marked as "illegal to sleep" e.g. towns are not processed by default.]],
		configKey = 'skipRestingIsIllegal'
	})

	sideBarPage:createYesNoButton({
		label = 'Skip exteriors',
		description = [[Only process interior cells.
Note: exterior-like cells marked as "illegal to sleep" e.g. towns are not processed by default.]],
		configKey = 'skipExteriors'
	})

	sideBarPage:createYesNoButton({
		label = 'Skip persistent targets',
		description = [[Skip processing persistent targets (as persistent references could be used by other scripts/quests and needing to be easily accessed from low level player).
Note that targets with identifier containing "chargen" string are always skipped.]],
		configKey = 'skipPersistent'
	})

	sideBarPage:createYesNoButton({
		label = 'Skip scripted targets',
		description = [[Skip processing scripted targets. It should be rarely needed, but option is here just in case.]],
		configKey = 'skipScripted'
	})
...
-- trappedHint 0 = Off, 1 = Skill based, 2 = Always
	...
	sideBarPage:createDropdown({
		label = 'Trapped Hint:',
		options = getOptions(),
		description = [[Display a (trapped) hint.
...
Personally I find enabling this option is against the challenge, but to each their own.]]..note1,
		configKey = 'trappedHint',
	})

	sideBarPage:createSlider({
		label = 'Trap detect Threshold',
		description = [[The skill + probe score needed to be able to detect a trap.
Only effective when "Trapped Hint" option is set to "1. Skill based".]],
		configKey = 'trapDetectThreshold',
		min = 20, max = 95, step = 1, jump = 5,
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Dwemer Barrels',
		description = 'Allow barrels trapping.',
		configKey = 'trapDwemerBarrels'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Dwemer Barrels',
		description = 'Allow barrels locking.',
		configKey = 'lockDwemerBarrels'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Barrels',
		description = 'Allow barrels trapping.',
		configKey = 'trapBarrels'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Barrels',
		description = 'Allow barrels locking.',
		configKey = 'lockBarrels'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Urns',
		description = 'Allow urns (and the like) trapping.',
		configKey = 'trapUrns'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Urns',
		description = 'Allow urns (and the like) locking.',
		configKey = 'lockUrns'
	})
	sideBarPage:createYesNoButton({
		label = 'Trap Jars',
		description = 'Allow jars trapping.',
		configKey = 'trapJars'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Jars',
		description = 'Allow jars locking.',
		configKey = 'lockJars'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Desks',
		description = 'Allow desk trapping.',
		configKey = 'trapDesks'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Desks',
		description = 'Allow desks locking.',
		configKey = 'lockDesks'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Closets',
		description = 'Allow closets (and the like) trapping.',
		configKey = 'trapClosets'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Closets',
		description = 'Allow closets locking.',
		configKey = 'lockClosets'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Tables',
		description = 'Allow tables trapping.',
		configKey = 'trapTables'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Tables',
		description = 'Allow tables locking.',
		configKey = 'lockTables'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Crates',
		description = 'Allow crates trapping.',
		configKey = 'trapCrates'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Crates',
		description = 'Allow crates locking.',
		configKey = 'lockCrates'
	})

	sideBarPage:createYesNoButton({
		label = 'Trap Baskets',
		description = 'Allow baskets trapping.',
		configKey = 'trapBaskets'
	})
	sideBarPage:createYesNoButton({
		label = 'Lock Baskets',
		description = 'Allow baskets locking.',
		configKey = 'lockBaskets'
	})
	...
	sideBarPage:createDropdown({
		label = 'Debug level:',
		options = getOptions(),
		description = [[Note: this is meant to be used mostly for testing traps,
not for normal playing!]],
		configKey = 'debugLevel',
	})
