...
local rapidcastSuppressed = false

-- effects that suppress rapidfire while selected (e.g. blink preview)
local rapidcastSuppressEffects = {
	["t_mysticism_blink"] = true,
}

local spellSuppressesRapidcastCache = {}
local function spellSuppressesRapidcast(spell)
	if not spell then return false end
	local cached = spellSuppressesRapidcastCache[spell.id]
	if cached ~= nil then return cached end
	local result = false
	for _, e in ipairs(spell.effects) do
		if e.range == core.magic.RANGE.Self and rapidcastSuppressEffects[e.effect.id] then
			result = true
			break
		end
	end
	spellSuppressesRapidcastCache[spell.id] = result
	return result
end
...
local function checkSpell(spell)
	...
	if not spellDB[spellId] then
		...
		for _, effect in ipairs(spell.effects) do
			...
			-- GameSpell method (matches engine calcSpellCost default):
			-- durationOffset = 0, minArea = 0, costMult = fEffectCostMult.
			-- GamePotion would use minArea = 1, costMult = iAlchemyMod.
			local x = 0.5 * (minMagn + maxMagn)
			x = x * (0.1 * effect.effect.baseCost)
			x = x * duration
			x = x + (0.05 * effect.area * effect.effect.baseCost)
			x = x * fEffectCostMult
...
function registerUse()
	...
	input.bindAction('Use', async:callback(function(dt, use, sneak, run)
		...
		local spell = Player.getSelectedSpell(self)
		if spell and S_rapidfire and useRapidfire and Actor.getStance(self) == Actor.STANCE.Spell and not rapidcastSuppressed and not spellSuppressesRapidcast(spell) then
			use = false
		end
...
local function suppressRapidcast(suppressed)
	if suppressed ~= nil then
		rapidcastSuppressed = suppressed
	else
		return rapidcastSuppressed
	end
end
...
return {
	engineHandlers = {},
	eventHandlers = {
		MBSP_suppressRapidcast = suppressRapidcast,
	},
	interfaceName = "MBSP",
	interface = {
		version = 4,
		...
		suppressRapidcast = suppressRapidcast,
	},
}